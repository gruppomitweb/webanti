...
var imgDir = '../' + (typeof ZtVWeb != 'undefined' && ZtVWeb.theme ? ZtVWeb.theme : window.m_cThemePath ? m_cThemePath : 'SpTheme_2') + '/formPage/';
...
var calendarFixedPos = null;

function PrepareJsCalendar(documentOpened, container) {
  if (documentOpened !== true && documentOpened !== false) {
    documentOpened = document.readyState === 'loading';
  }
  calendarFixedPos = container;
...
function BuildCalendar(documentOpened) {
  ...
  if (Empty(calendarFixedPos) && documentOpened) { // documento aperto, non si riesce ad aggiungere un elemento al body
    // setTimeout(BuildCalendar,10);
    LibJavascript.Events.addEvent(window, 'load', function() {
      BuildCalendar();
    } );
    return;
  }
  if ( !Empty(calendarFixedPos) && (document.getElementById('Calendar')
    || window.m_oFather && window.m_oFather.ShowPopUpCalendar && !window.IsInModalLayer() )) {
    return;
  } // gia presente o da caricare nel padre
  var maker = [];
  var z_index = 'zIndex' in (dragObj || {}) ? ++dragObj.zIndex : 999;
  maker.push("<div class='SPCalendar' id='Calendar' onmouseover='PrepareToDoNotHide()' onmouseout='PrepareToHide()'" + (Empty(calendarFixedPos) ? " style='position:absolute;visibility:hidden;z-index:+" + z_index + ";top:0px;left:0px;'" : '' ) + '>');
  maker.push("<table cellspacing='0' " + (Empty(calendarFixedPos) ? "width='" + (showWeekNumber == 1 ? calendarWidth : calendarWidth - 30) + "px'" : "width='100%'") + '>');
  maker.push("<tr class='SPCalendarHeader'>");
  maker.push("<td valign='middle' class='SPCalendarHeaderAll'>");
  maker.push("<table width='" + (showWeekNumber == 1 ? calendarWidth - 2 : calendarWidth - 32) + "px'>");
  maker.push('<tr>');
  maker.push("<td class='SPCalendarHeader'>");
  maker.push("<span id='calendar_caption'></span>");
  maker.push('</td>');
  if ( Empty(calendarFixedPos) ) {
    maker.push("<td class='SPCalendarDragger' onmousedown=\"dragCalendar(event,'Calendar',this)\" style='width:90px;cursor:move;'>");
    maker.push('&nbsp;');
    maker.push('</td>');
    maker.push("<td align='right'><a href='javascript:hideCalendar()' title='" + closeTheCalendar + "'>");
    maker.push("<img src='" + imgsrc[6] + "' border='0' alt='" + closeTheCalendar + "'>");
    maker.push('</a>');
    maker.push('</td>');
  }
  maker.push('</tr>');
  ...
  maker.push('</tr>');
  maker.push('<tr>');
  maker.push("<td class='SPCalendarContent'>");
  maker.push("<span id='content'></span>");
  maker.push('</td>');
  maker.push('</tr>');
  if (showToday == 1) {
    maker.push('<tr>');
    maker.push("<td class='SPCalendarFooter' style='padding:0;padding-bottom:8px;' align=center>");
    maker.push("<span id='lblToday'></span>");
    maker.push('</td>');
    maker.push('</tr>');
  }
  maker.push('</table>');
  maker.push('</div>');
  maker.push("<div id='selectMonth' class='SPCalendarMonth' style='z-index:+" + ++z_index + ";position:absolute;visibility:hidden'></div>");
  maker.push("<div id='selectYear' class='SPCalendarYear' style='z-index:+" + ++z_index + ";position:absolute;visibility:hidden'></div>");
  var d = document.createElement('div');
  d.innerHTML = maker.join('');
  while (d.childNodes.length > 0) {
    if (!Empty(calendarFixedPos)) {
      calendarFixedPos.appendChild(d.childNodes[0]);
    } else {
...
function InitCalendar() {
  ...
  sHTML1 += " onclick='decMonth()' onmouseout='clearInterval(intervalID1);clearTimeout(timeoutID1);" + (imgsrc[3] !== imgsrc[2] ? 'swapImage("changeLeft","' + imgsrc[2] + '");' : '') + "'";
  ...
  sHTML1 += " onmouseout='clearInterval(intervalID1);clearTimeout(timeoutID1);" + (imgsrc[5] !== imgsrc[4] ? 'swapImage("changeRight","' + imgsrc[4] + '");' : '') + "'";
...
function closeCalendar(notClose) {
  var localWin = ShowPopUpCalendar.localWin || window;
  if (!notClose) {
    hideCalendar();
  }
  if (typeof localWin.ZtVWeb != 'undefined' && ctlToPlaceValue instanceof localWin.ZtVWeb.StdControl)// e' un Ctrl di PS
  {
    ctlToPlaceValue.Set(new localWin.Date(yearSelected, monthSelected, dateSelected));
  } else {
	  ctlToPlaceValue.value = constructDate(dateSelected, monthSelected, yearSelected);
    try {
      if (typeof localWin[ctlToPlaceValue.id + '_Valid'] == 'function') {
        // i campi aggiunti hanno un id uguale al nome e di lunghezza variabile
        localWin[ctlToPlaceValue.id + '_Valid']();
      } else if (typeof localWin[Left(ctlToPlaceValue.id, 10) + '_Valid'] == 'function') {
        // Nello zoom, la procedura _Valid non e' presente
        localWin[Left(ctlToPlaceValue.id, 10) + '_Valid']();
      }
      SetControlFocus(ctlToPlaceValue.id);
    } catch (e) {}
  }
  ShowPopUpCalendar.localWin = null;
...
function popUpMonth() {
  ...
  month.left = (!Empty(calendarObj.left) ? parseInt(calendarObj.left) : 0 ) + 52 + 'px';
  month.top = (!Empty(calendarObj.top) ? parseInt(calendarObj.top) : 0 ) + 28 + 'px';
...
function popUpYear() {
  var leftOffset,
    year = LibJavascript.DOM.Ctrl('selectYear').style;

  constructYear();
  year.visibility = 'visible';

  leftOffset = (!Empty(calendarObj.left) ? parseInt(calendarObj.left) : 0 ) + LibJavascript.DOM.Ctrl('spanYear').offsetLeft;
  if (IsIE()) {
    leftOffset += 6;
  }
  year.zIndex = dragObj.zIndex + 1;
  year.left = leftOffset + 'px';
  year.top = (!Empty(calendarObj.top) ? parseInt(calendarObj.top) : 0 ) + 26 + 'px';
  if (window.CalculateAndResizeEntityHeight) {
    CalculateAndResizeEntityHeight();
  }
}
...
function constructCalendar() {
  ...
  for (datePointer = 1; datePointer <= numDaysInMonth; datePointer++) {
    ...
    var regexp = /\"/g;
    var href = 'javascript:dateSelected=' + datePointer + ';closeCalendar(' + (!Empty(calendarFixedPos) ? 'true' : '' ) + ');';
...
function ShowPopUpCalendar(ctrl, getPicture, sayPicture, localWin) {
  ShowPopUpCalendar.localWin = localWin = localWin ? localWin : window;
  if (window.m_oFather && window.m_oFather.ShowPopUpCalendar && !window.IsInModalLayer()) {
    window.m_oFather.ShowPopUpCalendar(ctrl, getPicture, sayPicture, localWin);
    return;
  }
  var origCtrl = ctrl;
  if (!calendarReady) {
  ...
  if (bPageLoaded) {
    if (calendarObj.visibility == 'hidden') {
      ...
      var currentDate;
      if (typeof localWin.ZtVWeb != 'undefined' && ctlToPlaceValue instanceof localWin.ZtVWeb.StdControl) {// e' un Ctrl di PS
        currentDate = localWin.ZtVWeb.applyPicture(ctrl.Value(), ctrl.type, 0, ctrl.picture);
      ...
      if (currentDate == '') {
        ...
        if (!LibJavascript.Date.CheckDate(dateSelected, monthSelected + 1, yearSelected)) { // monthSelected+1 perch� nella checkdate fa gi� -1 quindi Gennaio verrebbe -1!
          dateSelected = dateNow;
          monthSelected = monthNow;
          yearSelected = yearNow;
        }
      ...
      var a = localWin.LibJavascript.DOM.getPosFromFirstRel(ctrl, localWin.document.body);
      var w = localWin;
      while (w != window) {
        var iframe = w.frameElement;
        a.y += iframe.getBoundingClientRect().top;
        a.x += iframe.getBoundingClientRect().left;
        w = w.parent;
      }
      if (Empty(calendarFixedPos)) {
        calendarObj.left = (fixedX == -1 ? a.x + document.documentElement.scrollLeft : fixedX) + 'px';
        calendarObj.top = (fixedY == -1 ? a.y + ctrl.offsetHeight + document.documentElement.scrollTop : fixedY) + 'px';
      }
      ...
      hideCalendar();
      if (ctlNow != origCtrl) {
        ShowPopUpCalendar(origCtrl, getPicture, sayPicture, localWin);
      }
    }
    ctlNow = origCtrl;
  }
