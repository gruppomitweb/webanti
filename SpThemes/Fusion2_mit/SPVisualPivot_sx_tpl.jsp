<%/*Type:PageletEditor*/%>
<%//Def:[{"id":"pagelet","name":"pagelet","type":"pagelet","template":"7","cols_n":"20","rows_n":"3","width":"960px","height":"100%","css":"","css_Class":"","color":"","security":"none","on_security_failure":"login","roles":"","cache_time":"0","sp_theme":"true","offline":"false","shared_manifest":"false","manifest":"Custom manifest","action_code":"","description":"","steps":"","layout_step":"","is960":"false","steppable_prop":[],"css_class":"","stretch":"false","codemanifest":"","css_code":"[]"},{"id":"item_static_0","type":"static","name":"zoom_title","src":"default_pivot_title_portlet.jsp","parameters":" ","titled":"false","title":"","open":"true","in_iframe":"false","iframe_height":"","condition":"","row":"0","col":"0","height":"1","width":"20","prefx":"0","suffx":"0","inserted":"true","behavior":"show","layout_steps_values":{},"steppable_prop":["row","col","width","height","behavior","popupItems","prefx","suffx","renderWidth"],"popupItems":[],"stretch":"false","caption":"","draggable":"false","toremove":"","hidden":"false","curTab":"generic","renderWidth":"20"},{"id":"item_resources_1","type":"resource","name":"ZoomGrid","titled":"false","title":"","open":"true","in_iframe":"false","iframe_height":"","container":"","row":"1","col":"0","height":"1","width":"20","prefx":"0","suffx":"0","inserted":"true","behavior":"show","layout_steps_values":{},"steppable_prop":["row","col","width","height","behavior","popupItems","prefx","suffx","renderWidth"],"popupItems":[],"stretch":"true","draggable":"false","hidden":"false","curTab":"generic","renderWidth":"20"}]%>
<%/*Encoding:UTF-8*/%>
<%/*Description:*/%>
...
String parms_joiner;
%><!-- PageletName=SPVisualPivot_sx_tpl -->
<%
if (JSPLib.checkApplication(request, response, out, sp, 37,"SPVisualPivot_sx_tpl","UTF-8")) {return; }
 %><%response.setHeader("Cache-Control","max-age = 0");
response.setDateHeader("Expires", 0);
request.setAttribute("com.zucchetti.pagelet.template."+sp.getSkin()+".SPVisualPivot_sx_tpl","SPVisualPivot_sx_tpl");
...
request.setAttribute("com.zucchetti.pagelet.main_template","SPVisualPivot_sx_tpl");
}
...
<html>
<head>
<base href=<%=JSPLib.ToHTMLValue( JSPLib.SPWebRootURL(request)+"/jsp/SPVisualPivot_sx_tpl.jsp"+(!Library.Empty(request.getQueryString())?"?"+request.getQueryString():"") )%>/>
...
<script>
/*JS_MARKER_START*/
var m_cSPParentObjId =   ZtVWeb.POM.addObj('<%=PageletId%>','SPVisualPivot_sx_tpl','pageletObj', '<%=JSPLib.ToJSValue(SPParentObjId,false,true)%>');
/*JS_MARKER_END*/
</script>
...
String res_url;
%><%
url="default_pivot_title_portlet.jsp";
content_name= uid + ID_SEP + "default_pivot_title";
parms_joiner = "?";
res_url="default_pivot_title_portlet.jsp"+parms_joiner+"SPParentObjId="+container_id+"&PageletId="+PageletId;
%><%res_url+="&"+queryString;%><div id=<%=JSPLib.ToHTMLValue(content_id)%> style='display:block;'><script>
/*JS_MARKER_START*/
<%=PageletId%>_pagelet_structure['<%=uid%>']={'loaded': false, 'type': 'static', 'name': 'zoom_title','containerId': '<%=JSPLib.ToJSValue(container_id)%>', 'url': 'default_pivot_title_portlet.jsp', 'contentId': '<%=JSPLib.ToJSValue(content_id)%>', 'inIframe': false};
/*JS_MARKER_END*/
</script><%
out.flush();
if(!com.zucchetti.sitepainter.Library.includeResource(request,response,"/"+sp.getSkin()+"/jsp-decorators/default_pivot_title_portlet.jsp",true))
  if(!com.zucchetti.sitepainter.Library.includeResource(request,response,"/jsp-decorators/default_pivot_title_portlet.jsp",true))
    if(!com.zucchetti.sitepainter.Library.includeResource(request,response,"/"+sp.getSkin()+"/default_pivot_title_portlet.jsp",true))
      com.zucchetti.sitepainter.Library.includeResource(request,response,"/jsp/default_pivot_title_portlet.jsp",false);
%></div><script>
/*JS_MARKER_START*/
window.portlet_list.push('default_pivot_title');
/*JS_MARKER_END*/
...
<%}%>
ZtVWeb.addPagelet('<%=PageletId%>',<%=PageletId%>,'SPVisualPivot_sx_tpl','<%=JSPLib.ToJSValue(includerPageletId,false,true)%>');
