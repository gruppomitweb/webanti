...
String parms_joiner;
%><!-- PageletName=default_uic_tpl -->
<%
if (JSPLib.checkApplication(request, response, out, sp, 37,"default_uic_tpl","UTF-8")) {return; }
 %><%response.setHeader("Cache-Control","max-age = 0");
response.setDateHeader("Expires", 0);
request.setAttribute("com.zucchetti.pagelet.template."+sp.getSkin()+".default_uic_tpl","default_uic_tpl");
...
request.setAttribute("com.zucchetti.pagelet.main_template","default_uic_tpl");
}
...
<html>
<head>
<base href=<%=JSPLib.ToHTMLValue( JSPLib.SPWebRootURL(request)+"/jsp/default_uic_tpl.jsp"+(!Library.Empty(request.getQueryString())?"?"+request.getQueryString():"") )%>/>
...
<script>
/*JS_MARKER_START*/
var m_cSPParentObjId =   ZtVWeb.POM.addObj('<%=PageletId%>','default_uic_tpl','pageletObj', '<%=JSPLib.ToJSValue(SPParentObjId,false,true)%>');
/*JS_MARKER_END*/
</script>
...
title_value= "";
url= "";
%><div id=<%=JSPLib.ToHTMLValue(container_id)%> name="_area_4" class="pagelet_item item_stretch_cell flex_item   leftDark_row" ps-layout-style='row wrap'><div id=<%=JSPLib.ToHTMLValue(content_id)%> class='flexArea row wrap' style=''><script>
/*JS_MARKER_START*/
...
title_value= "";
url= "";
%><div id=<%=JSPLib.ToHTMLValue(container_id)%> name="left_side_area" class="pagelet_item item_stretch flex_item leftDark item_behavior_hide " ps-layout-style='row wrap'><div id=<%=JSPLib.ToHTMLValue(content_id)%> class='noflex' style=''><script>
/*JS_MARKER_START*/
...
title_value= "";
url= "";
%><div id=<%=JSPLib.ToHTMLValue(container_id)%> name="main_area" class="pagelet_item item_stretch_container item_stretch flex_item" ps-layout-style='row wrap'><script>
/*JS_MARKER_START*/
...
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=container_id%>','central','groupObj','<%=SPParentObjId%>');
	<%=PageletId%>_pagelet_structure['<%=uid%>']={'loaded':[], 'type': 'group', 'urls': [], 'resources': [], 'name': 'central', 'Ids': [], 'containerIds' : [], 'contentIds': [], 'inIframe': [], 'title_portlet_ids': [], 'titleIds': [], 'containerId': '<%=container_id%>', 'contentId': '<%=content_id%>'};
/*JS_MARKER_END*/
</script><div id=<%=JSPLib.ToHTMLValue(container_id)%> data-ps-name="central" class='pagelet_item item_stretch block_item '  title_as_portlet='default_title_portlet_portlet.jsp' ps-stretch-height ps-group ><div id=<%=JSPLib.ToHTMLValue(content_id)%> class='' style=''><%
{int i=0;
...
window.portlet_list.push('<%=JSPLib.ToJSValue(JSPLib.GetValidVarName(GROUP_element_resource_url))%>');
/*JS_MARKER_END*/
</script><%}}}%><div style='clear:both;'></div></div></div><%}%><!--chiude gruppo central--></div></div><%}%></div></div></div></div></div></div></div></div></div></div></div></div></div><script>
/*JS_MARKER_START*/
window.<%=PageletId%>=new function(){
  this.id='<%=PageletId%>'
  ZtVWeb.PageletCtrl(this, {id:'<%=PageletId%>', steps:"0,1024", stretch:"true", dragAndDroppable: false});
	this._area_6= new ZtVWeb.AreaCtrl( this,{"behavior":"show","height":3,"id":"_area_6","name":"_area_6","renderWidth":20,"stretchBehavior":"stretch_container","type":"area","width":20});
...
<%if(central_group_box!=null){%>
	this.central= new ZtVWeb.GroupCtrl(this, {"behavior":"show","col":6,"height":2,"id":"item_group_18","layout_steps_values":{"1024":{"behavior":"show","col":"6","height":"2","popupItems":[],"prefx":0,"renderWidth":20,"row":"1","suffx":0,"width":"14"}},"name":"central","open":true,"popupItems":[],"renderWidth":20,"row":1,"strechStructure":"{\"table\":\"_area_0\", \"row\": \"_area_2\", \"cell\": \"_area_1\"}","stretch":true,"type":"group","width":14});
<%}%>
...
<%}%>
ZtVWeb.addPagelet('<%=PageletId%>',<%=PageletId%>,'default_uic_tpl','<%=JSPLib.ToJSValue(includerPageletId,false,true)%>');
