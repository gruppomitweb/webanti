...
function Set_publickey(ctrlValue,kmode,e) {
if (Ne(w_publickey,ctrlValue)) {
var ctl = _GetCtl(e,'GOURUFIPIA');
if (_tracker.goon(ctl,ctrlValue)) {
w_publickey=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('GOURUFIPIA',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
function Set_callbackuri(ctrlValue,kmode,e) {
if (Ne(w_callbackuri,ctrlValue)) {
var ctl = _GetCtl(e,'CSWXYHZCME');
if (_tracker.goon(ctl,ctrlValue)) {
w_callbackuri=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('CSWXYHZCME',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',5096,0,'');
_ResetTracker();
return true;
}
}
...
if ( ! (m_oFather.SaveContext_spauthid_client())) {
...
parent.ZtVWeb.raiseEventToEvalParms('spauthid_client_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('spauthid_client_dataNotSent',{'action':formaction});
}
}
}
}
...
var l_bResult = true;
ResetErrorVars();
if ( ! (_ChkObl('GOURUFIPIA',1))) {
_SignErr('GOURUFIPIA');
m_cLastWorkVarErrorMsg='846681157';
l_bResult=false;
w_publickey=HtW('','M');
}
if (l_bResult) {
...
l_oWv.setValue('publickey',WtA(w_publickey,'M'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
_FillChildren.n=[];
FillWv.n=["usercode","publickey"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
if (Le(m_nCurrentRow,Rows())) {
if (( ! (_ChkObl('CSWXYHZCME',1))) && FullRow()) {
_SignErr('CSWXYHZCME');
m_cLastWorkVarErrorMsg='435231171';
l_bResult=false;
w_callbackuri=HtW('','C');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_spauthid_client();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_callbackuri));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.callbackuri={"type":"C","name":"callbackuri","title":"Callback URIs","id":"CSWXYHZCME","fixedpos":false,"len":5096,"dec":0};
...
if (enable) {
var c;
if(c=Ctrl('GOURUFIPIA')) SetDisabled(c,true);
SetQueryRow(Ctrl('GridTable'),false);
} else {
SetEditRow(Ctrl('GridTable'),false);
var c;
if(c=Ctrl('GOURUFIPIA')) SetDisabled(c,false);
