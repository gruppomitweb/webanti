...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('VAXPDZFXPZ').value,'C'),w_name)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('spauthid_scope',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'name') &&  ! (Empty(HtW(Ctrl('VAXPDZFXPZ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.name',HtW(Ctrl('VAXPDZFXPZ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_spauthid_scope_mapper(firstAlias) {
return m_cVirtName+".name = "+firstAlias+".scope_name ";
}
...
_chNotF(childAlias,'master','spauthid_scope');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('spauthid_scope',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['VAXPDZFXPZ'],i=0;i>=0;i--){
