...
// * --- Fine Area Manuale
public class spauthid_client_scope_mappingBO extends spauthid_client_scope_mappingBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        ...
        m_cSelect = m_cSelect+" ,link_2_1.description AS description201";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","scope_name","name");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","scope_name","name",true);
  ...
  // * --- Fine Area Manuale
  public spauthid_client_scope_mappingBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"spauthid_client_scope_mapping");
    }
  ...
  public static String[] m_cLoadParameterNames = {"client_code"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(double p_client_code) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_spauthid_client_scope_mapping,m_cPhName_spauthid_client_scope_mapping,m_cVirtName_spauthid_client_scope_mapping,m_cVirtName_spauthid_client_scope_mapping+".client_code="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_client_code,"N",9,0),m_cServer_spauthid_client_scope_mapping,p_client_code),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_1(m_cPhName_spauthid_scope,m_cServer_spauthid_scope);
    l_rsLoadBody = l_oSqlDetailLoader.Query();
    try {
      m_bLoaded =  ! (l_rsLoadBody.Eof());
      if (m_bLoaded) {
        m_bCalculating = true;
        w_client_code = l_rsLoadBody.GetDouble("client_code");
        // Assegna i valori degli op_
        // Assegna il contesto dei figli di testata/piedi a Empty
        SPExtenderLib.LoadExtendedFields(l_rsLoadBody,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_client_scope_mapping")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_scope_name = l_rsLoadBody.GetString("scope_name");
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            w_scope_description = l_rsLoadBody.GetString("description201");
          } else {
            Link_DSHVJUCLQJ("Load");
          }
          // Assegna il contesto dei figli del dettaglio a Empty
          work_children = CPLib.BlankChildMNTs(m_MNTs,true);
          opk_scope_name = w_scope_name;
          if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_client_scope_mapping")) {
  ...
  public boolean Delete(double p_client_code) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_spauthid_client_scope_mapping.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"client_code="+CPLib.ToSQL(w_client_code,"N",9,0)+" AND scope_name="+CPLib.ToSQL(opk_scope_name,"C",50,0);
        l_cSql = l_cSql+m_oWrInfo_spauthid_client_scope_mapping.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_client_scope_mapping")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_spauthid_client_scope_mapping,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_client_code);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"DSHVJUCLQJ")) {
      result.m_TableName = "spauthid_scope";
      result.n_KMode = 1;
      result.m_FillEmpty = false;
      result.m_LooselyLinked = false;
      result.m_dynamicprgzoom = false;
      result.m_SearchingFunction = "";
      result.m_PrgZoom = "spauthid_scope";
      result.m_rdField = new String[]{"name" , "description"};
      result.m_rdTypes = new String[]{"C" , "C"};
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("DSHVJUCLQJ",CPLib.GetLinkedFieldProps("name","scope_name","C",50,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_spauthid_client_scope_mapping);
      l_cSql = "insert into "+m_oWrInfo_spauthid_client_scope_mapping.GetTableWriteName()+" (";
      l_cSql = l_cSql+"client_code,";
      l_cSql = l_cSql+"scope_name,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_spauthid_client_scope_mapping.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_client_scope_mapping")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_client_code,"N",9,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_scope_name,"C",50,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_spauthid_client_scope_mapping.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_client_scope_mapping")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_spauthid_client_scope_mapping,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_spauthid_client_scope_mapping);
      l_cSql = "Update "+m_oWrInfo_spauthid_client_scope_mapping.GetTableWriteName()+" set";
      if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_client_scope_mapping")) {
        l_cSql = l_cSql+" CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", scope_name = "+CPLib.ToSQL(w_scope_name,"C",50,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"client_code="+CPLib.ToSQL(w_client_code,"N",9,0)+" AND scope_name="+CPLib.ToSQL(opk_scope_name,"C",50,0);
      l_cSql = l_cSql+m_oWrInfo_spauthid_client_scope_mapping.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_client_scope_mapping")) {
