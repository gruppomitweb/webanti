...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('DDYZMQMXQA').value,'C'),w_code)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('spauthid_identity_provider',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'code') &&  ! (Empty(HtW(Ctrl('DDYZMQMXQA').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.code',HtW(Ctrl('DDYZMQMXQA').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','spauthid_identity_provider');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('spauthid_identity_provider',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['DDYZMQMXQA'],i=0;i>=0;i--){
