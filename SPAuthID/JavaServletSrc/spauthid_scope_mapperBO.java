...
// * --- Fine Area Manuale
public class spauthid_scope_mapperBO extends spauthid_scope_mapperBL {
  ...
  // * --- Fine Area Manuale
  public spauthid_scope_mapperBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"spauthid_scope_mapper");
    }
  ...
  public static String[] m_cLoadParameterNames = {"scope_name"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_scope_name) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_spauthid_scope_mapper,m_cPhName_spauthid_scope_mapper,m_cVirtName_spauthid_scope_mapper,m_cVirtName_spauthid_scope_mapper+".scope_name="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_scope_name,"C",50,0),m_cServer_spauthid_scope_mapper,p_scope_name),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_scope_mapper")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_property_key = l_rsLoadBody.GetString("property_key");
          w_property_value = l_rsLoadBody.GetString("property_value");
  ...
  public boolean Delete(String p_scope_name) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_spauthid_scope_mapper.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"scope_name="+CPLib.ToSQL(w_scope_name,"C",50,0);
        l_cSql = l_cSql+m_oWrInfo_spauthid_scope_mapper.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_scope_mapper")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_spauthid_scope_mapper,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_spauthid_scope_mapper);
      l_cSql = "insert into "+m_oWrInfo_spauthid_scope_mapper.GetTableWriteName()+" (";
      l_cSql = l_cSql+"scope_name,";
      l_cSql = l_cSql+"property_key,";
      l_cSql = l_cSql+"property_value,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_spauthid_scope_mapper.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_scope_mapper")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_scope_name,"C",50,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_property_key,"C",20,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_property_value,"M",10,0,m_cServer_spauthid_scope_mapper,m_oParameters)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_spauthid_scope_mapper.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_scope_mapper")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_spauthid_scope_mapper,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_spauthid_scope_mapper);
      l_cSql = "Update "+m_oWrInfo_spauthid_scope_mapper.GetTableWriteName()+" set";
      l_cSql = l_cSql+" property_key = "+CPLib.ToSQL(w_property_key,"C",20,0);
      l_cSql = l_cSql+", property_value = "+CPLib.ToSQL(w_property_value,"M",10,0,m_cServer_spauthid_scope_mapper,m_oParameters);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"spauthid_scope_mapper")) {
