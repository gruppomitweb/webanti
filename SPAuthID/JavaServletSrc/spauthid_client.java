...
public class spauthid_client extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*fd29a56f*!";
  static public class ServletStatus extends SPMasterDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "spauthid_client_m";
    }
    public double w_usercode;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_usercode,context,"spauthid_client_m","usercode","N",9,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    spauthid_clientBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_spauthid_client_m;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "spauthid_client";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 710;
    l_eg.m_nEntityHeight = 120;
    l_eg.isPublic = false;
    ...
    l_eg.zoomableTables = new String[]{"spauthid_client_m"};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("publickey","M",10,0, "edit","Optional URI JWKS endpoint or PEM PKCS #8"),
                   new SPItem("callbackuri","C",5096,0, "edit","Callback URIs"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,spauthid_clientBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10917253735",p_Context.Translate("Client callback URIs")},
    {"1122488460",p_Context.Translate("Public key in PKCS#8 format or URL to retrieve public key in JWKS format")},
    {"11397400166",p_Context.Translate("OpenID Connect client")},
    {"1589044613",p_Context.Translate("Optional client URI JWKS endpoint or PEM PKCS #8 to encrypt response token")},
    {"435231171",p_Context.Translate("Callback URIs")},
    {"846681157",p_Context.Translate("Optional URI JWKS endpoint or PEM PKCS #8")},
  ...
  void WriteStyles(spauthid_clientBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../spauthid_client.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(spauthid_clientBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(
       "<style>" +
      "");
      status.out.println(".CSWXYHZCME {top:0px;position:absolute;left:0px;height:25px;width:480px;background-color:Transparent;z-index:1; }");
      status.out.println(
       "</style>" +
      "");
    } else {
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('spauthid_client',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"spauthid_client",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(spauthid_clientBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(spauthid_clientBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:120px;width:710px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(spauthid_clientBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_TOP_OF_PAGE_1' style='position:relative;height:72px;display:block' page='1'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      status.out.println(
       "<div name id='GridTable_Header' class='GridTable_Header' style='position:absolute;top:45px;left:208px;height:26px;width:502px;z-index:0'>" +
       "</div>" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<textarea name='publickey' id='GOURUFIPIA' class='Memo' title="+SPLib.ToHTMLValue(status.context.Translate("Optional client URI JWKS endpoint or PEM PKCS #8 to encrypt response token"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' cols='20' rows='2'>" +
       "</textarea>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='CBYVQSGURW_DIV'>" +
       "<label id='CBYVQSGURW' for='CSWXYHZCME'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Lista URI di redirect"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<span class='SPLabel' id='AHIBPPLPFU_DIV'>" +
       "<label id='AHIBPPLPFU' title="+SPLib.ToHTMLValue(status.context.Translate("Public key in PKCS#8 format or URL to retrieve public key in JWKS format"),"C",0,0)+">" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("URL chiave pubblica"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
  ...
  void WriteControls_p1_b1(spauthid_clientBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("GridTable")) {
      ...
      status.out.println(
       "<div id='GridTable_Container' style='position:relative;top:0px;left:208px;min-height:30px;max-height:96px;width:502px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:36px;width:502px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:502px;min-height:30px;max-height:96px;overflow:auto;'>" +
       "<table id='GridTable' style='position:relative;top:0px;left:0px;width:496px' cellspacing='0' cellpadding='0' border='0'>" +
       "<tr>" +
       "<td></td>" +
       "</tr>" +
       "</table>" +
      ...
      if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
        status.out.println(
         "<div id='GridTable_ButtonPanel' style='position:absolute;top:0px;left:210px;z-index:4'>" +
  ...
  void WriteControls_p1_b1_CloseBand(spauthid_clientBO BO,ServletStatus status) {
    status.out.println(
     "</div>" +
    "");
    status.out.println(
  ...
  void HTMLRowScript(spauthid_clientBO BO,ServletStatus status) {
    ...
    status.out.println("s='<div style=\"position:relative;width:480px;height:29px;\"';");
    status.out.println("s=s+'>';");
    status.out.println("var row;");
    status.out.println("var prefix;");
    status.out.println("var w_callbackuri;");
    ...
    status.out.println("}");
    status.out.println("w_callbackuri=row[prefix+'callbackuri'];");
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     "<input name='callbackuri' id='CSWXYHZCME' type='text' class='Field GridTable_EditRowItem ' value='\"+WtH(ToHTML(w_callbackuri),'C',5096,0,'')+\"' title="+SPLib.ToHTMLValue(status.context.Translate("Client callback URIs"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='5096' size='5096' fieldtype='C'>" +
    "\"");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    status.m_nTabIndex = status.m_nTabIndex-((CPLib.gt(BO.extendedFields_trs.length,0)?1:0));
    status.out.println("} else {");
    status.out.println("if ( ! (IsHiddenByStateDriver('callbackuri'))) {");
    status.out.println("s=s+\""+
     "<input type='text' class='Field Disabled GridTable_QueryRowItem CSWXYHZCME' value='\"+WtH(ToHTML(w_callbackuri),'C',5096,0,'')+\"' tabindex='-1' readonly size='5096'>" +
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:480px;height:29px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b2(spauthid_clientBO BO,ServletStatus status) {
  ...
  static void PrintState(spauthid_clientBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(spauthid_clientBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(spauthid_clientBL BO,ServletStatus status) {
    status.out.print("DeclareWVs(");
    status.out.print(SPLib.ToJSValue(BO.w_usercode,"N",9,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_publickey,"M",10,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_callbackuri,"C",5096,0));
  ...
  static void PrintStateDetailWorkVars(spauthid_clientBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(spauthid_clientBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      spauthid_clientBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (spauthid_clientBO.TrsRow) BO.m_oDeletedRows.get(k);
        l_cTrs = l_cTrs+" new TrsRow(";
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_callbackuri,"C",5096,0)+",";
  ...
  void WriteMainFormVarScript(spauthid_clientBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(120,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(710,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"spauthid_client"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("spauthid_client","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"spauthid_client",BO.GetCurrentState());
    ...
    status.out.println("var m_nMaxDetailRows = 3;");
    status.out.println("var m_bNoHorizontalLines = " + SPLib.ToJSValue(true,"L",0,0)+ ";");
    status.out.println("function TrsRow() {");
    status.out.println("if (Eq(arguments.length,1)) {");
    status.out.println("this.callbackuri=w_callbackuri;");
    ...
    status.out.println("} else {");
    status.out.println("this.callbackuri=arguments[0];");
    status.out.println("this.CPROWNUM=arguments[1];");
    ...
    status.out.println("w_callbackuri=row.callbackuri;");
    ...
    if (CPLib.ne(status.m_cClientMode,"query")) {
      ...
      status.out.println("} else {");
      status.out.println("w_callbackuri='';");
  ...
  void WriteMainFormScript(spauthid_clientBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_spauthid_client);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_spauthid_client();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('spauthid_client',Ne(m_nChildStatus,0));");
    } else {
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('GOURUFIPIA')) SetDisabled(c,true);");
      } else if (( ! (CPLib.Empty(status.m_cAltInterface))) && CPLib.eq(status.m_cClientMode,"query")) {
        status.out.println("var c;");
        status.out.println("if(c=Ctrl('CSWXYHZCME')) SetDisabled(c,true);");
      }
    ...
    if (CPLib.eq(status.m_cClientMode,"query") && CPLib.gt(status.m_nChildStatus,0)) {
      ...
      status.out.println("SetDisabled('GOURUFIPIA',m_bRowsChild);");
    }
    status.out.println("}");
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
    ...
    status.out.println("m_oFather.m_ospauthid_client=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  spauthid_clientBO getBO(ServletStatus status) {
    spauthid_clientBO BO = new spauthid_clientBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(spauthid_clientBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("spauthid_client_m",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
    t = xdc.GetXDCTable("spauthid_client",true);
    BO.extendedFields_trs = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,spauthid_clientBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,spauthid_clientBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(spauthid_clientBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_spauthid_client_m;
    String l_cPhName = BO.m_cPhName_spauthid_client_m;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = spauthid_clientBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_spauthid_client_m+".usercode",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("spauthid_client_m");
    String l_cPhName = CPSql.ManipulateTablePhName("spauthid_client_m",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+l_cPhName+".usercode",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    }
    return l_rsKeys;
  }
  void Query(ServletStatus status,spauthid_clientBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_spauthid_client_m,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_spauthid_client_m,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,spauthid_clientBO BO) {
  ...
  void Edit(ServletStatus status,spauthid_clientBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,spauthid_clientBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,spauthid_clientBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,spauthid_clientBO BO) {
  ...
  void EditLoad(ServletStatus status,spauthid_clientBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,spauthid_clientBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,spauthid_clientBO BO,SPParameterSource source) {
    BO.Load(status.w_usercode);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,spauthid_clientBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,spauthid_clientBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,spauthid_clientBO BO) {
    ...
    item.put("altInterfaceFor","spauthid_client");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"comment\":\"Optional URI JWKS endpoint or PEM PKCS #8\",\"h\":25,\"name\":\"publickey\",\"page\":1,\"scroll\":true,\"spuid\":\"GOURUFIPIA\",\"tabindex\":2,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":480,\"x\":210,\"y\":11,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"comment\":\"Public key in PKCS#8 format or URL to retrieve public key in JWKS format\",\"h\":25,\"name\":\"label_AHIBPPLPFU\",\"page\":1,\"spuid\":\"AHIBPPLPFU\",\"tabindex\":4,\"type\":\"Label\",\"value\":\"URL chiave pubblica\",\"w\":160,\"x\":40,\"y\":11,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"autoexpand\":true,\"h\":32,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"GridTable_Band\",\"type\":\"BodyDetail\",\"w\":536,\"x\":210,\"y\":72,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":25,\"name\":\"label_CBYVQSGURW\",\"page\":1,\"spuid\":\"CBYVQSGURW\",\"tabindex\":3,\"type\":\"Label\",\"value\":\"Lista URI di redirect\",\"w\":300,\"x\":210,\"y\":46,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":55,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"GridTable_Band\",\"type\":\"Box\",\"w\":480,\"x\":210,\"y\":45,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Callback URIs\",\"h\":26,\"maxlength\":5096,\"name\":\"callbackuri\",\"page\":1,\"repeated\":true,\"spuid\":\"CSWXYHZCME\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":480,\"x\":210,\"y\":74,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,spauthid_clientBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(spauthid_clientBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='spauthid_client?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/spauthid_client_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/spauthid_client_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_publickey=a[1];");
      status.out.println("w_callbackuri=a[2];");
      status.out.println("w_CPROWNUM=a[3];");
      status.out.println("m_PrimaryKeys=['CPROWNUM','usercode'];");
      status.out.println("}");
      status.out.println(
       "</script>" +
      "");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/spauthid_client_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(120,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","spauthid_client_m");
  ...
  void RaiseLoadedEntity(spauthid_clientBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(spauthid_clientBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(spauthid_clientBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(spauthid_clientBO BO,ServletStatus status) {
  ...
  void WriteExitPage(spauthid_clientBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(spauthid_clientBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(spauthid_clientBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,spauthid_clientBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,spauthid_clientBO BO) {
    boolean result = true;
