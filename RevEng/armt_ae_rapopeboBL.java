...
public class armt_ae_rapopeboBL extends armt_ae_rapopeboWV implements CPLib.QueryFilterFromApplication {
  ...
  public static final String i_StateStructure = "{'CODDIPE':CODDIPE,'RAPPORTO':RAPPORTO,'DATAOPE':DATAOPE,'TIPOOPRAP':TIPOOPRAP,'ardt_ae_intestbo':ardt_ae_intestbo,'IDBASE':IDBASE,'STATOREG':STATOREG,'ardt_ae_delegabo':ardt_ae_delegabo,'ardt_ae_soginfo':ardt_ae_soginfo"+FoundlingChildrenList("armt_ae_rapopebo",true)+"}";
  public static final String i_CompleteStateStructure = "{'CODINTER':CODINTER,'gIntemediario':gIntemediario,'CODDIPE':CODDIPE,'RAPPORTO':RAPPORTO,'DATAOPE':DATAOPE,'DATAREG':DATAREG,'TIPOOPRAP':TIPOOPRAP,'CONNESOPER':CONNESOPER,'TIPOLEG':TIPOLEG,'DATARETT':DATARETT,'ardt_ae_intestbo':ardt_ae_intestbo,'IDBASE':IDBASE,'TIPOINTER':TIPOINTER,'TIPOINF':TIPOINF,'IDEREG':IDEREG,'FLAGRAP':FLAGRAP,'OPRAP':OPRAP,'STATOREG':STATOREG,'OPERAZMOD':OPERAZMOD,'OPERATORE':OPERATORE,'AUTOM':AUTOM,'DESCCIT':DESCCIT,'PROVINCIA':PROVINCIA,'CODCAB':CODCAB,'NUMPROG':NUMPROG,'xdescrap':xdescrap,'xDescDipe':xDescDipe,'datpaseuro':datpaseuro,'contdate':contdate,'xDesReg':xDesReg,'tipreg':tipreg,'xRagSoc':xRagSoc,'sEdit':sEdit,'errmsg':errmsg,'Aggiorna':Aggiorna,'FLAGRAP2':FLAGRAP2,'lancio':lancio,'FLAGLIRE':FLAGLIRE,'VALUTA':VALUTA,'tipregn':tipregn,'PRGIMPINF':PRGIMPINF,'dataoggi':dataoggi,'CDATOPE':CDATOPE,'CITINT':CITINT,'CITDIP':CITDIP,'PROVDIP':PROVDIP,'CABDIP':CABDIP,'PROVINT':PROVINT,'CABINT':CABINT,'ardt_ae_delegabo':ardt_ae_delegabo,'ardt_ae_soginfo':ardt_ae_soginfo"+FoundlingChildrenList("armt_ae_rapopebo",true)+"}";
  public static final String i_ItemSequence = "CODDIPE,RAPPORTO,DATAOPE,TIPOOPRAP,ardt_ae_intestbo,IDBASE,STATOREG,ardt_ae_delegabo,ardt_ae_soginfo"+FoundlingChildrenList("armt_ae_rapopebo",false)+"";
  public static final com.zucchetti.sitepainter.EntityItem[] i_EditableEntityItems = com.zucchetti.sitepainter.EntityItem.buildEditableItemsForEntity(GetLinkDef(),FoundlingChildrenList("armt_ae_rapopebo"),new String[][]{new String[]{"CODDIPE","Codice Dipendenza","C","6","0","campo","nokey","false","false"},new String[]{"RAPPORTO","","C","25","0","campo","nokey","false","false"},new String[]{"DATAOPE","Data Operazione","D","8","0","campo","key2","false","true"},new String[]{"TIPOOPRAP","Tipo Informazione","C","2","0","campo","key5","false","true"},new String[]{"ardt_ae_intestbo","Soggetti conto informazioni","","","","link_PC","","false",""},new String[]{"IDBASE","idbase","C","10","0","campo","key1","false","false"},new String[]{"STATOREG","Stato Registrazione","C","1","0","campo","nokey","false","false"},new String[]{"ardt_ae_delegabo","LinkPC","","","","link_PC","","false",""},new String[]{"ardt_ae_soginfo","Soggetti Agenti per Conto Inform.","","","","link_PC","","false",""}});
  ...
  protected boolean Link_WJSVGWKSFC(String p_cType) {
    ...
    if ( ! (CPLib.Empty(w_CODINTER))) {
      ...
      if (l_rsLink!=null)
        l_rsLink.Close();
      l_rsLink = m_Sql.Query(m_cServer_intermbo,"select CODINTER,CODCAB,PROVINCIA,DESCCIT,DATANASC,SOSPMAF,TIPINTER from "+m_cPhName_intermbo+((m_cPhName_intermbo).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"CODINTER"+l_cCurrentKey+l_cWhere);
      try {
        ...
        if (l_bLoaded) {
          ...
          w_contdate = l_rsLink.GetString("SOSPMAF");
  ...
  void Link_SLTZSUMACW_blank(String p_cType) {
    w_xRagSoc = "";
  }
  ...
  protected boolean Link_TQTBMAXOZP() {
    return Link_TQTBMAXOZP("Full");
  }
  protected boolean Link_TQTBMAXOZP(String p_cType) {
    // Viene eseguita una query per chiave totale.
    boolean l_bResult;
    boolean l_bLoaded;
    String l_cWhere = "";
    ...
    if ( ! (CPLib.Empty(w_CODINTER))) {
      ...
      if (l_rsLink!=null)
        l_rsLink.Close();
      l_rsLink = m_Sql.Query(m_cServer_intermbo,"select CODINTER,CODDIPE,DATANASC,SOSPMAF,TIPINTER from "+m_cPhName_intermbo+((m_cPhName_intermbo).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"CODINTER"+l_cCurrentKey+l_cWhere);
      try {
        l_bLoaded =  ! (l_rsLink.Eof());
        if (l_bLoaded) {
          /*  E' stato trovato almeno un record: i valori del cursore vengono riportati nei
                                    ...
                                    */
          w_CODINTER = l_rsLink.GetString("CODINTER");
          w_CODDIPE = l_rsLink.GetString("CODDIPE");
          w_CODDIPE = l_rsLink.GetString("CODDIPE");
          w_datpaseuro = l_rsLink.GetDate("DATANASC");
          w_contdate = l_rsLink.GetString("SOSPMAF");
          w_TIPOINTER = l_rsLink.GetString("TIPINTER");
  ...
  public boolean Calculate() {
    ...
    if (CPLib.ne(o_CONNESOPER,w_CONNESOPER)) {
      l_bTmpRes = Link_SLTZSUMACW();
    }
    if (CPLib.ne(o_TIPOLEG,w_TIPOLEG)) {
  ...
  public void SetFromContext(String s,java.util.Properties p) {
    ...
    if ( ! (CPLib.Empty(s))) {
      ...
      w_contdate = CPLib.GetProperty(p,"contdate",w_contdate,0);
  ...
  static void GetLinkDef0(Map<String,List<Map<String,String[]>>> tables) {
    ...
    map.put("rd_kvar",new String[]{});
    map.put("rd_kfield",new String[]{});
    map.put("rd_field",new String[]{"CODINTER","CODCAB","PROVINCIA","DESCCIT","DATANASC","SOSPMAF","TIPINTER"});
    map.put("rd_var",new String[]{"w_CODINTER","w_CABINT","w_PROVINT","w_CITINT","w_datpaseuro","w_contdate","w_TIPOINTER"});
    map.put("wr_var",new String[]{});
    map.put("wr_field",new String[]{});
    map.put("wr_op",new String[]{});
    map.put("rd_var.obj_type",zeroSplit.split("C\u0000C\u0000C\u0000C\u0000D\u0000C\u0000C",-1));
    map.put("rd_kvar.obj_type",new String[]{});
    map.put("rd_var.len",zeroSplit.split("11\u00006\u00002\u000030\u00008\u00001\u00002",-1));
    map.put("rd_kvar.len",new String[]{});
    map.put("rd_var.dec",zeroSplit.split("0\u00000\u00000\u00000\u00000\u00000\u00000",-1));
    map.put("rd_kvar.dec",new String[]{});
    map.put("rd_var.comment",zeroSplit.split("Codice Intermediario\u0000CAB\u0000Provincia\u0000Città\u0000Data Passaggio EURO\u0000Passato EURO\u0000",-1));
    map.put("rd_kvar.comment",new String[]{});
    ...
    map.put("rd_kvar.obj",new String[]{});
    map.put("property.value",zeroSplit.split("personbo\u0000false\u0000true\u00002\u00000\u0000\u0000true\u0000false\u0000Persone / Società / Legali Rappresentanti\u0000\u0000\u0000false",-1));
    map.put("property.name",zeroSplit.split("arch\u0000fillemptykey\u0000looselylinked\u0000src_key\u0000hst_link_mode\u0000prg_zoom\u0000looselylinked\u0000sendkeystozoomonzoom\u0000zoom_title\u0000prg_link_search\u0000prg_link_defaults\u0000",-1));
    map = new HashMap<String,String[]>();
  ...
  public void GetContext(boolean p_bGetChildren,java.util.Properties p) {
    ...
    CPLib.PutProperty(p,"contdate",w_contdate,0);
  ...
  CallerBRImpl GetChild(String p_cName) {
    ...
    for ( i = 0; CPLib.lt(i,m_MNTs.length); i = i + (1)) {
      if (CPLib.eq(m_MNTs[i].m_cChildName,p_cName)) {
        m_MNTs[i].m_child.m_lCtxLoaded = true;
        if ( ! (m_MNTs[i].m_child.IsUpdated())) {
          if (CPLib.IsUpdated(m_MNTs[i].m_cWv) || CPLib.IsLoaded(m_MNTs[i].m_cWv)) {
            m_MNTs[i].m_child.SetFromContext(m_MNTs[i].m_cWv);
            m_MNTs[i].m_cWv = "_not_empty_";
          } else {
            if (m_bLoaded &&  ! (m_MNTs[i].m_child.IsLoaded())) {
              m_MNTs[i].m_child.Blank();
              m_MNTs[i].CopyKeys(this);
              m_MNTs[i].m_child.Load();
              if ( ! (m_MNTs[i].m_child.IsLoaded())) {
                m_MNTs[i].m_child.Initialize();
                m_MNTs[i].m_child.SetUpdated();
              }
              m_MNTs[i].m_child.FoundlingCopyWorkVar(this,false);
            }
          }
        }
        return m_MNTs[i].m_child;
      }
    }
    return null;
  }
