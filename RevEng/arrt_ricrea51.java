...
public class arrt_ricrea51 extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*WRFVIQBWIG*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_DataReg);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_ricrea51R Rcalled;
    Rcalled = new arrt_ricrea51R(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_ricrea51R(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_ricrea51R Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_ricrea51"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println(" window[f].w_DataReg.setTime(Date.parse("+SPLib.ToJSValue(status.w_DataReg,"D",0,0)+"))");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_ricrea51");
      if ( ! (CPLib.IsNull(l))) {
