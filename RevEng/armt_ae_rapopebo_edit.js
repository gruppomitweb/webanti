...
function DisplayErrorMessage_ardt_ae_intestbo() {
if ( ! (ChildToLoad(FrameRef('ardt_ae_intestbo'))) && Ne(typeof(FrameRef('ardt_ae_intestbo').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_ae_intestbo').DisplayErrorMessage();
}
}
...
function DisplayErrorMessage_ardt_ae_soginfo() {
if ( ! (ChildToLoad(FrameRef('ardt_ae_soginfo'))) && Ne(typeof(FrameRef('ardt_ae_soginfo').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_ae_soginfo').DisplayErrorMessage();
}
}
...
l_bEnabled= ! (Ne(w_TIPOOPRAP,'25') && Ne(w_TIPOOPRAP,'28')) || IsDisabledByStateDriver('Soggetti conto informazioni');
if ( ! (ChildToLoad(FrameRef('ardt_ae_intestbo'))) && Ne(typeof(FrameRef('ardt_ae_intestbo').EnableEntity),'undefined')) {
FrameRef('ardt_ae_intestbo').EnableEntity(l_bEnabled);
}
l_bEnabled= ! (Eq(w_TIPOOPRAP,'25') || Eq(w_TIPOOPRAP,'28')) || IsDisabledByStateDriver('LinkPC');
if ( ! (ChildToLoad(FrameRef('ardt_ae_delegabo'))) && Ne(typeof(FrameRef('ardt_ae_delegabo').EnableEntity),'undefined')) {
FrameRef('ardt_ae_delegabo').EnableEntity(l_bEnabled);
}
...
l_bResult=l_bResult && _ChkObl('BILHYLSMRS',0);
if (l_bResult) {
if ( ! (l_bResult)) {
w_CODDIPE=HtW('','C');
}
...
w_CODINTER=w_gIntemediario;
l_bTmpRes=Link_TQTBMAXOZP(null);
...
// * --- armt_ae_rapopebo
FrameRef('ardt_ae_intestbo').Calculate()
FrameRef('ardt_ae_intestbo').DoUpdate(true)
FrameRef('ardt_ae_delegabo').Calculate()
FrameRef('ardt_ae_delegabo').DoUpdate(true)
// * --- Fine Area Manuale
UpdateAutonumber();
...
l_Appl.SetFields('CODINTER,CODCAB,PROVINCIA,DESCCIT,DATANASC,SOSPMAF,TIPINTER');
l_Appl.SetTypes('C,C,C,C,D,C,C');
...
w_contdate=l_Appl.GetStringValue('SOSPMAF',1,0);
...
function Link_TQTBMAXOZP(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
var l_bEmp = Empty(w_CODINTER);
var l_Appl = LinkApplet();
l_Appl.SetStringKey('CODINTER',w_CODINTER,11,0);
l_Appl.SetFields('CODINTER,CODDIPE,DATANASC,SOSPMAF,TIPINTER');
l_Appl.SetTypes('C,C,D,C,C');
l_Appl.LinkTable('intermbo');
l_Appl.SetLinkzoom('armt_intermbo');
l_Appl.SetID('TQTBMAXOZP');
l_nRec=l_Appl.DoLink((Eq(kmode,null)?0:kmode));
l_bResult=Ne(l_nRec,0);
if (Eq(l_nRec,1)) {
w_CODINTER=l_Appl.GetStringValue('CODINTER',11,0);
w_CODDIPE=l_Appl.GetStringValue('CODDIPE',6,0);
w_datpaseuro=l_Appl.GetDateValue('DATANASC',8,0);
w_datpaseuro=DateFromApplet(w_datpaseuro);
w_contdate=l_Appl.GetStringValue('SOSPMAF',1,0);
w_TIPOINTER=l_Appl.GetStringValue('TIPINTER',2,0);
...
function Link_TQTBMAXOZP_Blank() {
w_CODINTER='';
w_CODDIPE='';
w_datpaseuro=NullDate();
w_contdate='';
...
l_oWv.setValue('PKFields',WtA('CODINTER','C'));
l_oWv.setValue('LinkedField',WtA('CODINTER','C'));
l_oWv.setValue('UID',WtA('TQTBMAXOZP','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['TQTBMAXOZP'][1],'C'));
return l_oWv;
}
...
parent.ZtVWeb.raiseEventToEvalParms('armt_ae_rapopebo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_ae_rapopebo_dataNotSent',{'action':formaction});
}
}
}
}
CheckChild.child=[{'prg':"ardt_ae_intestbo",'BOScId':function(){ return FrameRef('ardt_ae_intestbo')},'repeated':0},{'prg':"ardt_ae_delegabo",'BOScId':function(){ return FrameRef('ardt_ae_delegabo')},'repeated':0},{'prg':"ardt_ae_soginfo",'BOScId':function(){ return FrameRef('ardt_ae_soginfo')},'repeated':0}];
function Check() {
var l_bResult = true;
var l_bChildCheck = true;
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('AVHTGZTOSZ',1))) {
_SignErr('AVHTGZTOSZ');
...
if (Eq(Ctrl('AVHTGZTOSZ_ZOOM'),lc)) {
return 0;
}
if (Eq(Ctrl('AVHTGZTOSZ'),lc)) {
return 0;
}
if (Eq(Ctrl('UMFEJVRUFZ_ZOOM'),lc)) {
return 1;
}
if (Eq(Ctrl('UMFEJVRUFZ'),lc)) {
return 1;
}
return -1;
}
...
l_oWv.setValue('contdate',WtA(w_contdate,'C'));
...
_FillChildren.n=['ardt_ae_intestbo','ardt_ae_delegabo','ardt_ae_soginfo'];
FillWv.n=["CODINTER","CODDIPE","RAPPORTO","DATAOPE","DATAREG","TIPOOPRAP","CONNESOPER","TIPOLEG","DATARETT","IDBASE","TIPOINTER","TIPOINF","IDEREG","FLAGRAP","OPRAP","STATOREG","OPERAZMOD","OPERATORE","AUTOM","DESCCIT","PROVINCIA","CODCAB","NUMPROG","FLAGRAP2","FLAGLIRE","PRGIMPINF","CDATOPE","gIntemediario","xdescrap","xDescDipe","datpaseuro","contdate","xDesReg","tipreg","xRagSoc","sEdit","errmsg","Aggiorna","lancio","VALUTA","tipregn","dataoggi","CITINT","CITDIP","PROVDIP","CABDIP","PROVINT","CABINT"];
function SetModified() {
m_bUpdated=true;
}
function SetChildVariables_ardt_ae_intestbo() {
}
function CtxLoad_ardt_ae_intestbo() {
if (ChildToLoad(FrameRef('ardt_ae_intestbo'))) {
return [LoadContext(0,'TXFLNDGZME'),'ardt_ae_intestbo'];
}
}
...
return [LoadContext(0,'SYICQKNLUB'),'ardt_ae_delegabo'];
}
}
function SetChildVariables_ardt_ae_soginfo() {
}
function CtxLoad_ardt_ae_soginfo() {
if (ChildToLoad(FrameRef('ardt_ae_soginfo'))) {
return [LoadContext(0,'IXHEMDOSUG'),'ardt_ae_soginfo'];
}
}
