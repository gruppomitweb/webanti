...
public class arrt_imp_dati_ria extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*c70b689d*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_fscarti) ||  ! (status.m_bpassedw_creamo) ||  ! (status.m_bpassedw_imperr) ||  ! (status.m_bpassedw_chkaui) ||  ! (status.m_bpassedw_annorif) ||  ! (status.m_bpassedw_impzip) ||  ! (status.m_bpassedw_tipodest);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_imp_dati_riaR Rcalled;
    Rcalled = new arrt_imp_dati_riaR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_imp_dati_riaR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_imp_dati_riaR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_imp_dati_ria"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_imp_dati_ria");
      if ( ! (CPLib.IsNull(l))) {
