...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('PJHHWQGBUG').value,'N'),w_WCCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('wu_cases',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'WCCODICE') &&  ! (Empty(HtW(Ctrl('PJHHWQGBUG').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.WCCODICE',HtW(Ctrl('PJHHWQGBUG').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_wu_cases');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wu_cases',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['PJHHWQGBUG'],i=0;i>=0;i--){
