...
public class arrt_azzera_invii_opex extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*7641821e*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_azzera_invii_opex";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_mese) ||  ! (status.m_bpassedw_anno) ||  ! (status.m_bpassedw_dadata) ||  ! (status.m_bpassedw_a_data);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_azzera_invii_opexR Rcalled;
    Rcalled = new arrt_azzera_invii_opexR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_azzera_invii_opexR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_azzera_invii_opexR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_azzera_invii_opex"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_azzera_invii_opex");
      if ( ! (CPLib.IsNull(l))) {
