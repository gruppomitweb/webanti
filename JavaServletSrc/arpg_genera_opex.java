...
public class arpg_genera_opex extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*DPFMEXQFBT*!";
  ...
  public String getEntityTitle() {
    return "Creazione Operazioni ExtraConto da AUI/AGE";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_genera_opex";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_genera_opexBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"10709736530",p_Context.Translate("Creazione Operazioni ExtraConto da AUI/AGE")},
  ...
  void WriteStyles(arpg_genera_opexBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_genera_opex.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_genera_opexBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_genera_opexBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_genera_opexBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_genera_opexBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_genera_opexBO BO,ServletStatus status) {
  ...
  void WriteControls_1_b1_h_0(arpg_genera_opexBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_genera_opexBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_genera_opexBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_genera_opexBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_genera_opexBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_genera_opex"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_genera_opex","");
  ...
  void WriteMainFormScript(arpg_genera_opexBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_genera_opexBO getBO(ServletStatus status) {
    arpg_genera_opexBO BO = new arpg_genera_opexBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_genera_opexBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_genera_opexBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_genera_opexBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_genera_opexBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_genera_opexBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_genera_opexBO BO) {
    ...
    item.put("altInterfaceFor","arpg_genera_opex");
    items.put(item);
    String s;
  ...
  void Next(ServletStatus status,arpg_genera_opexBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_genera_opexBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_genera_opexBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_genera_opex?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_genera_opex_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_genera_opexBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_genera_opexBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_genera_opexBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_genera_opexBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_genera_opexBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_genera_opexBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_genera_opexBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_genera_opexBO BO) {
    boolean result = true;
    return result;
  }
}
