...
SetDisabled('LNSBOLUCRM',l_bEnabled);
l_bEnabled= ! (false) || IsDisabledByStateDriver('IDSUBIND');
SetDisabled('BBCHCNQIRI',l_bEnabled);
...
var c;
if(c=Ctrl('BBEEHSSDFM')) c.title=Strtran(AlertErrorMessage('1497964518',w_DESCRI),'\\n',"\n");
}
...
var ctl = _GetCtl(e,'LNSBOLUCRM');
if (_tracker.goon(ctl,ctrlValue)) {
w_IDMOD=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('LNSBOLUCRM',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
var ctl = _GetCtl(e,'ILPNZWVGRZ');
if (_tracker.goon(ctl,ctrlValue)) {
w_IDIND=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('ILPNZWVGRZ',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',10,0,'');
...
var ctl = _GetCtl(e,'BBCHCNQIRI');
if (_tracker.goon(ctl,ctrlValue)) {
w_IDSUBIND=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('BBCHCNQIRI',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',10,0,'');
...
var ctl = _GetCtl(e,'HLHLHCXGMT');
if (_tracker.goon(ctl,ctrlValue)) {
w_PROGSUBIND=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('HLHLHCXGMT',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',10,0,'');
...
var ctl = _GetCtl(e,'BBEEHSSDFM');
if (_tracker.goon(ctl,ctrlValue)) {
w_DESCRI=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('BBEEHSSDFM',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
if ( ! (m_oFather.SaveContext_aosdt_sub_ind_mit())) {
...
parent.ZtVWeb.raiseEventToEvalParms('aosdt_sub_ind_mit_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('aosdt_sub_ind_mit_dataNotSent',{'action':formaction});
}
}
}
}
...
var l_bResult = true;
ResetErrorVars();
if ((false) && ( ! (_ChkObl('LNSBOLUCRM',1)))) {
_SignErr('LNSBOLUCRM');
m_cLastWorkVarErrorMsg='10024211400';
l_bResult=false;
w_IDMOD=HtW('','C');
} else if ( ! (_ChkObl('ILPNZWVGRZ',1))) {
_SignErr('ILPNZWVGRZ');
...
_FillChildren.n=[];
FillWv.n=["IDMOD","IDIND"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
if (Le(m_nCurrentRow,Rows())) {
if (((false) && ( ! (_ChkObl('BBCHCNQIRI',1)))) && FullRow()) {
_SignErr('BBCHCNQIRI');
m_cLastWorkVarErrorMsg='10785547902';
l_bResult=false;
w_IDSUBIND=HtW('','C');
} else if (( ! (_ChkObl('HLHLHCXGMT',1))) && FullRow()) {
_SignErr('HLHLHCXGMT');
m_cLastWorkVarErrorMsg='976616780';
l_bResult=false;
w_PROGSUBIND=HtW('','C');
} else if (( ! (_ChkObl('BBEEHSSDFM',1))) && FullRow()) {
_SignErr('BBEEHSSDFM');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_aosdt_sub_ind_mit();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields={};
l_aRepeatedFields.IDSUBIND={"dec":0,"fixedpos":false,"id":"BBCHCNQIRI","len":10,"name":"IDSUBIND","title":"ID del subindicatore","type":"C"};
l_aRepeatedFields.PROGSUBIND={"dec":0,"fixedpos":false,"id":"HLHLHCXGMT","len":10,"name":"PROGSUBIND","title":"Progressivo subindicatore","type":"C"};
l_aRepeatedFields.DESCRI={"dec":0,"fixedpos":false,"id":"BBEEHSSDFM","len":10,"name":"DESCRI","title":"Descrizione del subindicatore","type":"M"};
...
if (enable) {
var c;
if(c=Ctrl('LNSBOLUCRM')) SetDisabled(c,true);
if(c=Ctrl('ILPNZWVGRZ')) SetDisabled(c,true);
if(c=Ctrl('ATCXZHNKXY')) SetDisabled(c,true);
SetQueryRow(Ctrl('GridTable'),false);
} else {
SetEditRow(Ctrl('GridTable'),false);
var c;
if(c=Ctrl('LNSBOLUCRM')) SetDisabled(c,false);
if(c=Ctrl('ILPNZWVGRZ')) SetDisabled(c,false);
