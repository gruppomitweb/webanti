...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('YNMIZJQYRY').value,'C'),w_utcodice)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('utenti',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'utcodice') &&  ! (Empty(HtW(Ctrl('YNMIZJQYRY').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.utcodice',HtW(Ctrl('YNMIZJQYRY').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_utenti');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('utenti',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['YNMIZJQYRY'],i=0;i>=0;i--){
