...
SetErrorField('YFNPTNWMJX',false);
}
...
var ctl = _GetCtl(e,'YFNPTNWMJX');
if (_tracker.goon(ctl,ctrlValue)) {
w_NOAGE=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('YFNPTNWMJX',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
w_xRagSoc=( ! (Empty(LRTrim(w_xragsocd)))?w_xragsocd:( ! (Empty(w_CODINTER))?'Soggetto non presente in anagrafica':''));
...
w_xragsocd=l_Appl.GetStringValue('RAGSOC',70,0);
...
if ( ! (m_oFather.SaveContext_ardt_pdelegabo())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_pdelegabo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_pdelegabo_dataNotSent',{'action':formaction});
}
}
}
}
...
l_bResult=false;
w_DATAFINE=HtW('','D');
} else if (( ! (_ChkObl('YFNPTNWMJX',1))) && FullRow()) {
_SignErr('YFNPTNWMJX');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_pdelegabo();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.NUMPROG1={"dec":0,"fixedpos":true,"id":"SZEBCEZTJC","len":11,"name":"NUMPROG1","title":"N\u00B0 Progressivo","type":"C"};
l_aRepeatedFields.NUMPROG2={"dec":0,"fixedpos":true,"id":"UEQWFDVELM","len":11,"name":"NUMPROG2","title":"N\u00B0 Registrazione","type":"C"};
l_aRepeatedFields.NOAGE={"dec":0,"fixedpos":true,"id":"YFNPTNWMJX","len":1,"name":"NOAGE","title":"","type":"C"};
l_aRepeatedFields.xragsocd={"dec":0,"fixedpos":true,"id":"MDOINGAXFI","len":70,"name":"xragsocd","title":"Ragione Sociale","type":"C"};
...
if (enable) {
var c;
if(c=Ctrl('YFNPTNWMJX')) SetDisabled(c,true);
SetQueryRow(Ctrl('GridTable'),false);
} else {
SetEditRow(Ctrl('GridTable'),false);
var c;
if(c=Ctrl('YFNPTNWMJX')) SetDisabled(c,false);
