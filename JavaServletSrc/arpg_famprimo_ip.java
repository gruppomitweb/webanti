...
public class arpg_famprimo_ip extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*RLCFOBIOMC*!";
  ...
  public String getEntityTitle() {
    return "Creazione File SID Primo Invio (Integrazione 2015 IP)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_famprimo_ip";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_famprimo_ipBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11706785777",p_Context.Translate("L'utilizzo di questa maschera consente date dal 2016")},
    ...
    {"71182264",p_Context.Translate("Creazione File SID Primo Invio (Integrazione 2015 IP)")},
    {"822914937",p_Context.Translate("Confermi la creazione dei file integrativi 2015 ?")}};
    return l_translations;
  }
  void WriteStyles(arpg_famprimo_ipBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_famprimo_ip.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_famprimo_ipBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_famprimo_ipBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_famprimo_ipBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_famprimo_ipBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_famprimo_ipBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(553);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_famprimo_ipBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='DMMJSVFBEV'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("CREAZIONE FILE SID PRIMO INVIO (IP)"))+"" +
     "</label>" +
     "</span>" +
    "");
    ...
    "");
    status.out.println(
     "<input name='anno' id='GTMJENMCMN' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_anno,"N",4,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='5' size='4' fieldtype='N' onkeypress=''>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
    ...
    "");
    status.out.println(
     "<div id='HXKWFJUKYF_DIV'>" +
     "<div id='HXKWFJUKYF_0_container'>" +
     "<input id='HXKWFJUKYF_0' name='HXKWFJUKYF' type='radio' value="+SPLib.ToHTMLValue("N","C",1,0)+" disabled "+(CPLib.eq("N",CPLib.Trim(BO.w_flgnuovi))?"CHECKED":"")+">" +
     "<label class='SPLabel' for='HXKWFJUKYF_0'>"+status.context.Translate("Rapporti Aperti nel 2015")+"</label>" +
     "</div>" +
     "<div id='HXKWFJUKYF_1_container'>" +
     "<input id='HXKWFJUKYF_1' name='HXKWFJUKYF' type='radio' value="+SPLib.ToHTMLValue("A","C",1,0)+" disabled "+(CPLib.eq("A",CPLib.Trim(BO.w_flgnuovi))?"CHECKED":"")+">" +
     "<label class='SPLabel' for='HXKWFJUKYF_1'>"+status.context.Translate("Rapporti con novità nel 2015")+"</label>" +
     "</div>" +
     "</div>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
  ...
  static void PrintState(arpg_famprimo_ipBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_famprimo_ipBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_famprimo_ipBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_flgnuovi,"C",1,0));
  ...
  void WriteMainFormVarScript(arpg_famprimo_ipBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_famprimo_ip"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_famprimo_ip","");
  ...
  void WriteMainFormScript(arpg_famprimo_ipBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_famprimo_ipBO getBO(ServletStatus status) {
    arpg_famprimo_ipBO BO = new arpg_famprimo_ipBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_famprimo_ipBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_famprimo_ipBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_famprimo_ipBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_famprimo_ipBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_famprimo_ipBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_famprimo_ipBO BO) {
    ...
    item.put("altInterfaceFor","arpg_famprimo_ip");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"554\",\"name\":\"Box_TTRVXROIVP\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"TTRVXROIVP\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"801\",\"x\":\"-1\",\"y\":\"-1\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\"#0000FF\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"CREAZIONE_FILE_SID_PRIMO_INVIO__IP_\",\"page\":\"1\",\"spuid\":\"DMMJSVFBEV_DIV\",\"tabindex\":\"2\",\"type\":\"Label\",\"value\":\"CREAZIONE FILE SID PRIMO INVIO (IP)\",\"w\":\"800\",\"x\":\"0\",\"y\":\"0\",\"zone\":\"TTRVXROIVP_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"784\",\"x\":\"8\",\"y\":\"294\",\"zone\":\"TTRVXROIVP_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"162\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"784\",\"x\":\"8\",\"y\":\"126\",\"zone\":\"TTRVXROIVP_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stampa contenuto dei file creati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_MPCMAMHQST()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"MPCMAMHQST\",\"src\":\"images\\/print.gif\",\"tabindex\":\"10\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"656\",\"y\":\"91\",\"zone\":\"TTRVXROIVP_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"161\",\"href\":\" \",\"name\":\"__iframe_width__750__height__400__id__elaborazione___name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"TYIWXFLLFY_DIV\",\"tabindex\":\"11\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"750"+"'"+" height="+"'"+"400"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"784\",\"x\":\"8\",\"y\":\"385\",\"zone\":\"TTRVXROIVP_11\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"anno\",\"page\":\"1\",\"spuid\":\"GTMJENMCMN\",\"type\":\"Textbox\",\"w\":\"36\",\"x\":\"48\",\"y\":\"35\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"name\":\"mese\",\"page\":\"1\",\"spuid\":\"TCZMCNHUFI\",\"tabindex\":\"13\",\"textlist\":\"Gennaio,Febbraio,Marzo,Aprile,Maggio,Giugno,Luglio,Agosto,Settembre,Ottobre,Novembre,Dicembre\",\"type\":\"Combobox\",\"valuelist\":\""+"'"+"01"+"'"+","+"'"+"02"+"'"+","+"'"+"03"+"'"+","+"'"+"04"+"'"+","+"'"+"05"+"'"+","+"'"+"06"+"'"+","+"'"+"07"+"'"+","+"'"+"08"+"'"+","+"'"+"09"+"'"+","+"'"+"10"+"'"+","+"'"+"11"+"'"+","+"'"+"12"+"'"+"\",\"w\":\"112\",\"x\":\"128\",\"y\":\"35\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Data_\",\"page\":\"1\",\"spuid\":\"NOYEUIKFXN_DIV\",\"tabindex\":\"15\",\"type\":\"Label\",\"value\":\"Da Data:\",\"w\":\"43\",\"x\":\"256\",\"y\":\"25\",\"zone\":\"TTRVXROIVP_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Data_\",\"page\":\"1\",\"spuid\":\"LFJQIXRRKM_DIV\",\"tabindex\":\"17\",\"type\":\"Label\",\"value\":\"A Data:\",\"w\":\"43\",\"x\":\"256\",\"y\":\"53\",\"zone\":\"TTRVXROIVP_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Anno_\",\"page\":\"1\",\"spuid\":\"YIXZTOKOBR_DIV\",\"tabindex\":\"18\",\"type\":\"Label\",\"value\":\"Anno:\",\"w\":\"29\",\"x\":\"15\",\"y\":\"39\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Mese_\",\"page\":\"1\",\"spuid\":\"TTKGEZTDBH_DIV\",\"tabindex\":\"19\",\"type\":\"Label\",\"value\":\"Mese:\",\"w\":\"29\",\"x\":\"95\",\"y\":\"39\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"42\",\"name\":\"flgnuovi\",\"orientation\":\"vertical\",\"page\":\"1\",\"spuid\":\"HXKWFJUKYF\",\"textlist\":\"Rapporti Aperti nel 2015,Rapporti con novit\\u00E0 nel 2015\",\"type\":\"Radio\",\"valuelist\":\""+"'"+"N"+"'"+","+"'"+"A"+"'"+"\",\"w\":\"179\",\"x\":\"8\",\"y\":\"77\",\"zone\":\"TTRVXROIVP_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"21\",\"name\":\"flgstp\",\"page\":\"1\",\"spuid\":\"AOHGKXZIJG\",\"tabindex\":\"21\",\"type\":\"Checkbox\",\"w\":\"341\",\"x\":\"232\",\"y\":\"91\",\"zone\":\"TTRVXROIVP_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_famprimo_ipBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_famprimo_ipBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_famprimo_ipBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_famprimo_ip?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_famprimo_ip_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_famprimo_ipBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_famprimo_ipBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_famprimo_ipBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_famprimo_ipBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_famprimo_ipBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_famprimo_ipBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      arrt_famprimo.GetCallableNames(callable);
      arrt_famprino_stp.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_famprimo_ipBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_famprimo_ipBO BO) {
    boolean result = true;
    return result;
  }
}
