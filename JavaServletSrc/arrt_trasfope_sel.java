...
public class arrt_trasfope_sel extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*7a87f94e*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_DaDatReg;
    public java.sql.Date w_DaDatReg;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_ADatReg;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_DaDatReg = source.Has("w_DaDatReg");
      if (m_bpassedw_DaDatReg) {
        w_DaDatReg = source.GetParameter("w_DaDatReg",source.GetParameter("DaDatReg",CPLib.NullDate()));
      } else {
        if (source.IsForwarded()) {
          w_DaDatReg = source.GetCaller().GetDate("DaDatReg","D",8,0);
          m_bpassedw_DaDatReg = true;
        } else {
          w_DaDatReg = source.GetParameter("DaDatReg",source.GetParameter("w_DaDatReg",CPLib.NullDate()));
        }
      }
      m_bpassedw_ADatReg = source.Has("w_ADatReg");
      if (m_bpassedw_ADatReg) {
        w_ADatReg = source.GetParameter("w_ADatReg",source.GetParameter("ADatReg",CPLib.NullDate()));
      } else {
        if (source.IsForwarded()) {
          w_ADatReg = source.GetCaller().GetDate("ADatReg","D",8,0);
          m_bpassedw_ADatReg = true;
        } else {
          w_ADatReg = source.GetParameter("ADatReg",source.GetParameter("w_ADatReg",CPLib.NullDate()));
        }
      }
    }
  }
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_ADatReg) ||  ! (status.m_bpassedw_DaDatReg) ||  ! (status.m_bpassedw_c_adata) ||  ! (status.m_bpassedw_cdadata) ||  ! (status.m_bpassedw_ADatOpe) ||  ! (status.m_bpassedw_DaDatOpe);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_DaDatReg)) {
      status.out.println("l_cV=WtA(FetchDate(l_oWnd('w_DaDatReg',"+SPLib.ToJSValue(status.w_DaDatReg,"D",8,0)+")),'D');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_DaDatReg,"D",8,0)+",'D');");
    }
    status.out.println("l_oWv.setValue('w_DaDatReg',l_cV);");
    if ( ! (status.m_bpassedw_ADatReg)) {
      status.out.println("l_cV=WtA(FetchDate(l_oWnd('w_ADatReg',"+SPLib.ToJSValue(status.w_ADatReg,"D",8,0)+")),'D');");
    } else {
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_trasfope_selR Rcalled;
    Rcalled = new arrt_trasfope_selR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_trasfope_selR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_trasfope_selR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_trasfope_sel"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_DaDatReg="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_DaDatReg),"?",0,0)+"");
        status.out.println("js:w_ADatReg="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_ADatReg),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_DaDatReg,null)) {");
        status.out.println(" window[f].w_DaDatReg.setTime(("+SPLib.ToJSValue(status.w_DaDatReg,"D",0,0)+").getTime())");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_ADatReg,null)) {");
        status.out.println(" window[f].w_ADatReg.setTime(("+SPLib.ToJSValue(status.w_ADatReg,"D",0,0)+").getTime())");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_trasfope_sel");
      if ( ! (CPLib.IsNull(l))) {
