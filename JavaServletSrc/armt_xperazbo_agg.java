...
public class armt_xperazbo_agg extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*1cf89624*!";
  static public class ServletStatus extends SPMasterChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "xperazbo_agg";
    }
    public String w_IDBASE;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_IDBASE,context,"xperazbo_agg","IDBASE","C",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    armt_xperazbo_aggBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_xperazbo_agg;
  ...
  public String getEntityTitle() {
    return "Dati Aggiuntivi Operazioni Prov.";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_xperazbo_agg";
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"xperazbo_agg","tbcateg01","tbcateg02","tbcateg03","tbcateg04","tbcateg05"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_xperazbo_aggBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"175600474",p_Context.Translate("Dati Aggiuntivi Operazioni Prov.")},
  ...
  void WriteStyles(armt_xperazbo_aggBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_xperazbo_agg.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_xperazbo_aggBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_xperazbo_agg',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_xperazbo_agg",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(armt_xperazbo_aggBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(armt_xperazbo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b0(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_xperazbo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(armt_xperazbo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b3_CloseBand(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(armt_xperazbo_aggBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_BEFORE_STHOKLKHLK")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_BEFORE_STHOKLKHLK' style='position:relative;height:7px;display:block' page='1'>" +
    "");
  ...
  void WriteControls_p1_b5(armt_xperazbo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b5_CloseBand(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b6(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_xperazbo_aggBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_xperazbo_aggBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(armt_xperazbo_aggBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(armt_xperazbo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_xperazbo_agg"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_xperazbo_agg","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_xperazbo_agg",BO.GetCurrentState());
  ...
  void WriteMainFormScript(armt_xperazbo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_armt_xperazbo_agg);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_armt_xperazbo_agg();");
      status.out.println("}");
    }
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_xperazbo_agg',Ne(m_nChildStatus,0));");
    } else {
    ...
    status.out.println("// * --- armt_xperazbo_agg");
    ...
    status.out.println("m_oFather.m_oarmt_xperazbo_agg=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  armt_xperazbo_aggBO getBO(ServletStatus status) {
    armt_xperazbo_aggBO BO = new armt_xperazbo_aggBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(armt_xperazbo_aggBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("xperazbo_agg",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_xperazbo_aggBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,armt_xperazbo_aggBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(armt_xperazbo_aggBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_xperazbo_agg;
    String l_cPhName = BO.m_cPhName_xperazbo_agg;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_xperazbo_aggBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_xperazbo_agg+".IDBASE",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("xperazbo_agg");
    String l_cPhName = CPSql.ManipulateTablePhName("xperazbo_agg",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,armt_xperazbo_aggBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_xperazbo_agg,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_xperazbo_agg,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,armt_xperazbo_aggBO BO) {
  ...
  void Edit(ServletStatus status,armt_xperazbo_aggBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_xperazbo_aggBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_xperazbo_aggBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_xperazbo_aggBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_xperazbo_aggBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_xperazbo_aggBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_xperazbo_aggBO BO,SPParameterSource source) {
    BO.Load(status.w_IDBASE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_xperazbo_aggBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_xperazbo_aggBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_xperazbo_aggBO BO) {
    ...
    item.put("altInterfaceFor","armt_xperazbo_agg");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"sv\",\"box_type\":\"auto_horizontal_section\",\"collapsible\":true,\"h\":90,\"name\":\"sv\",\"page\":1,\"shrinkable\":true,\"spuid\":\"JANROHXZBL\",\"stretch\":true,\"tabindex\":4,\"type\":\"Box\",\"w\":792,\"x\":0,\"y\":127,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\"1\",\"comment\":\"Operazione Fraudolenta\",\"h\":21,\"label_text\":\"Operazione Fraudolenta\",\"maxlength\":2,\"name\":\"FLAGFRAUD\",\"page\":1,\"spuid\":\"QKZHBGTGRL\",\"tabindex\":10,\"type\":\"Checkbox\",\"typevar\":\"numeric\",\"unchecked_value\":\"0\",\"w\":208,\"x\":96,\"y\":136,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"SV 58724 Modalit\\u00E0 Bonifici\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":2,\"name\":\"SV58724\",\"page\":1,\"spuid\":\"BJYOWOAAQH\",\"tabindex\":11,\"textlist\":\"On Line - E-commerce,On Line - Altro,ATM,Mobile - P2P,Mobile - C2P,Mobile - Altro\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"01"+"'"+","+"'"+"02"+"'"+","+"'"+"03"+"'"+","+"'"+"04"+"'"+","+"'"+"05"+"'"+","+"'"+"06"+"'"+"\",\"w\":272,\"x\":504,\"y\":135,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_XVUIFQMEIG\",\"page\":1,\"spuid\":\"XVUIFQMEIG\",\"tabindex\":31,\"type\":\"Label\",\"value\":\"SV58724 Modalit\\u00E0 Bonifici:\",\"w\":182,\"x\":320,\"y\":139,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Autorizzazione Forte (S\\/N)\",\"h\":21,\"label_text\":\"SV58726 Autorizzazione Forte\",\"maxlength\":1,\"name\":\"SV58726SCA\",\"page\":1,\"spuid\":\"AKMFIOWCBF\",\"tabindex\":12,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":208,\"x\":96,\"y\":162,\"zone\":\"pag1_2_2\",\"zonepath\":\"[1,2,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"SV58726  Schema di addebito\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":3,\"name\":\"SV58726SA\",\"page\":1,\"spuid\":\"WUEMBLXMFM\",\"tabindex\":13,\"textlist\":\"SEPA CT Scheme,SEPA CT Instant Scheme,ON-US (CT),Non disponibile (CT)\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"396"+"'"+","+"'"+"397"+"'"+","+"'"+"398"+"'"+","+"'"+"399"+"'"+"\",\"w\":272,\"x\":504,\"y\":161,\"zone\":\"pag1_2_2\",\"zonepath\":\"[1,2,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_WFICGFEUAH\",\"page\":1,\"spuid\":\"WFICGFEUAH\",\"tabindex\":32,\"type\":\"Label\",\"value\":\"SV58726  Schema di addebito:\",\"w\":182,\"x\":320,\"y\":165,\"zone\":\"pag1_2_2\",\"zonepath\":\"[1,2,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"SV58726 Modalit\\u00E0 PISP\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":3,\"name\":\"SV58726PISP\",\"page\":1,\"spuid\":\"NODGYYBWWX\",\"tabindex\":14,\"textlist\":\"Bonifici disposti da PISP,Bonifici non disposti da PISP\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"395"+"'"+","+"'"+"396"+"'"+"\",\"w\":272,\"x\":504,\"y\":189,\"zone\":\"pag1_2_3\",\"zonepath\":\"[1,2,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VIBYCNNDWC\",\"page\":1,\"spuid\":\"VIBYCNNDWC\",\"tabindex\":33,\"type\":\"Label\",\"value\":\"SV58726 Modalit\\u00E0 PISP:\",\"w\":182,\"x\":320,\"y\":193,\"zone\":\"pag1_2_3\",\"zonepath\":\"[1,2,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VAWAJFPLKZ\",\"page\":1,\"spuid\":\"VAWAJFPLKZ\",\"tabindex\":39,\"type\":\"Label\",\"value\":\"Mezzo di Pagamento:\",\"w\":120,\"x\":384,\"y\":270,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Mezzo di Pagamento\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"SOSPAGAM\",\"page\":1,\"spuid\":\"CIGEGYZWMZ\",\"tabindex\":40,\"textlist\":\"Contante,Bonifico,Carta di pagamento,Altro\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"S"+"'"+","+"'"+"B"+"'"+","+"'"+"C"+"'"+","+"'"+"A"+"'"+"\",\"w\":272,\"x\":504,\"y\":266,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_xperazbo_aggBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_xperazbo_agg?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_xperazbo_agg_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_xperazbo_agg_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_FLAGFRAUD=a[6];");
      status.out.println("w_SV58724=a[7];");
      status.out.println("w_SV58726SCA=a[8];");
      status.out.println("w_SV58726SA=a[9];");
      status.out.println("w_SV58726PISP=a[10];");
      status.out.println("w_CATEGORIA=a[11];");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_xperazbo_agg_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","xperazbo_agg");
  ...
  void RaiseLoadedEntity(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_xperazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_xperazbo_aggBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_xperazbo_aggBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_xperazbo_aggBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,armt_xperazbo_aggBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_xperazbo_aggBO BO) {
    boolean result = true;
