...
public class arrt_modclose2 extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*7478edbb*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_oldfile) ||  ! (status.m_bpassedw_oldrnumrig) ||  ! (status.m_bpassedw_oldrappo) ||  ! (status.m_bpassedw_oldprog) ||  ! (status.m_bpassedw_oldidbase) ||  ! (status.m_bpassedw_datarett);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_modclose2R Rcalled;
    Rcalled = new arrt_modclose2R(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_modclose2R(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_modclose2R Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_modclose2"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_modclose2");
      if ( ! (CPLib.IsNull(l))) {
