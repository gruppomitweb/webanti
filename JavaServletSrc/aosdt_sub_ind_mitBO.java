...
// * --- Fine Area Manuale
public class aosdt_sub_ind_mitBO extends aosdt_sub_ind_mitBL {
  ...
  // * --- Fine Area Manuale
  public aosdt_sub_ind_mitBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"aosdt_sub_ind_mit");
    }
  ...
  public boolean Load(String p_IDMOD,String p_IDIND) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_sub_ind_mit+".PROGSUBIND";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_sub_ind_mit,m_cPhName_sub_ind_mit,m_cVirtName_sub_ind_mit,m_cVirtName_sub_ind_mit+".IDMOD="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDMOD,"C",10,0),m_cServer_sub_ind_mit,p_IDMOD)+" AND "+m_cVirtName_sub_ind_mit+".IDIND="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDIND,"C",10,0),m_cServer_sub_ind_mit,p_IDIND),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"sub_ind_mit")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_sub_ind_mit.GetTableWriteName()+" where ";
        ...
        l_cSql = l_cSql+m_oWrInfo_sub_ind_mit.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"sub_ind_mit")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_sub_ind_mit,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_sub_ind_mit);
      l_cSql = "insert into "+m_oWrInfo_sub_ind_mit.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDMOD,";
      l_cSql = l_cSql+"IDIND,";
      ...
      l_cSql = l_cSql+m_oWrInfo_sub_ind_mit.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"sub_ind_mit")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_DESCRI,"M",10,0,m_cServer_sub_ind_mit,m_oParameters)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_sub_ind_mit.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"sub_ind_mit")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_sub_ind_mit,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_sub_ind_mit);
      l_cSql = "Update "+m_oWrInfo_sub_ind_mit.GetTableWriteName()+" set";
      l_cSql = l_cSql+" PROGSUBIND = "+CPLib.ToSQL(w_PROGSUBIND,"C",10,0);
      l_cSql = l_cSql+", DESCRI = "+CPLib.ToSQL(w_DESCRI,"M",10,0,m_cServer_sub_ind_mit,m_oParameters);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"sub_ind_mit")) {
