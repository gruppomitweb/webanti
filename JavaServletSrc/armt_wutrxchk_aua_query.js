...
WindowAlert(AlertErrorMessage('433310806'));
}
}
}
var m_bIsMq = false;
...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('LPQRPTUZNB').value,'N'),w_TRXCODPRG)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('wutrxchk',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'TRXCODPRG') &&  ! (Empty(HtW(Ctrl('LPQRPTUZNB').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.TRXCODPRG',HtW(Ctrl('LPQRPTUZNB').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_wutrxchk_aua');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wutrxchk',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['LPQRPTUZNB'],i=0;i>=0;i--){
