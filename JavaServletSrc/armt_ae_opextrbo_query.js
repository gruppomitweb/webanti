...
bResult=Empty(w_DATAANNU);
}
if (Eq(formaction,'delete')) {
bResult=Empty(w_DATAINVIO);
}
if (bResult) {
if (Eq(formaction,'delete')) {
if (m_bLoaded) {
...
WindowAlert(AlertErrorMessage('10030286045'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('11366531506'));
}
}
}
var m_bIsMq = false;
...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('RVOXKWUSWH').value,'C'),w_COLLEG)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('SZFKAURTJA').value,'C'),w_ANNO)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('opextrbo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'COLLEG') &&  ! (Empty(HtW(Ctrl('RVOXKWUSWH').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.COLLEG',HtW(Ctrl('RVOXKWUSWH').value,'C'),'C');
}
if (Ne(name,'ANNO') &&  ! (Empty(HtW(Ctrl('SZFKAURTJA').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.ANNO',HtW(Ctrl('SZFKAURTJA').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_delegaop(firstAlias) {
return m_cVirtName+".COLLEG = "+firstAlias+".COLLEG and "+m_cVirtName+".ANNO = "+firstAlias+".ANNO ";
}
...
_chNotF(childAlias,'master','armt_ae_opextrbo');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('opextrbo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['RVOXKWUSWH','SZFKAURTJA'],i=1;i>=0;i--){
