...
if(c=Ctrl('VVHHSKMSWV')) c.value = WtH(w_fileerrori,'C',50,0,'')
...
SetDisplay(Ctrl('VVHHSKMSWV'),true || IsHiddenByStateDriver('fileerrori'));
HideUI();
}
...
l_oWv.setValue('m_cParameterSequence',AsAppletValue('w_tipodest'));
SetLocationHref(null,'arrt_imp_dati_wubs_errori'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cMode=hyperlink','elaborazione')
}
...
w_Messaggi.raiseEvent('arpg_import_wuib_errori_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_import_wuib_errori_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_ListaFile),'undefined')) {
w_ListaFile.raiseEvent('arpg_import_wuib_errori_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_import_wuib_errori_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_import_wuib_errori',m_cSelectedPage);
...
l_oWv.setValue('fileerrori',WtA(w_fileerrori,'C'));
l_oWv.setValue('gBrowser',WtA(w_gBrowser,'C'));
l_oWv.setValue('gUrlApp',WtA(w_gUrlApp,'C'));
_FillChildren(l_oWv);
}
_FillChildren.n=[];
FillWv.n=["nomefile","tipodest","gAzienda","gPathApp","fileerrori","gBrowser","gUrlApp"];
