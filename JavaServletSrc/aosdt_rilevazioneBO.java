...
// * --- Fine Area Manuale
public class aosdt_rilevazioneBO extends aosdt_rilevazioneBL {
  class SqlDetailLoader {
    public boolean m_bLink_1_1 = false;
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
    String m_cMainTable;
    ...
    public void AddJoinedLink_1_1(String rightTable,String rightServer) {
      // questo link aggiunge 2 campi al cursore risultato
      String m_cLinkedFrom;
      String m_cLinkedWhere;
      String m_cJoinCond = "";
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_1_1.IDMOD AS IDMOD101";
        m_cSelect = m_cSelect+" ,link_1_1.DESCRI AS DESCRI101";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_1_1","IDMOD","IDMOD");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_1_1","IDMOD","IDMOD",true);
  ...
  // * --- Fine Area Manuale
  public aosdt_rilevazioneBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"aosdt_rilevazione");
    }
  ...
  public boolean CanSave() {
    ...
    if (l_bResult) {
      ...
      while (l_bResult && CPLib.lt(l_nRow,Rows())) {
        ...
        if (l_bResult) {
          l_bResult = CanSaveChild(aosdt_anom_ril,m_cWv_aosdt_anom_ril,"m_cWv_aosdt_anom_ril");
          if (l_bResult) {
          } else {
  ...
  public static String[] m_cLoadParameterNames = {"IDMOD"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_IDMOD) {
    CPResultSet l_rsLoadBody=null;
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_rilevazione+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_rilevazione,m_cPhName_rilevazione,m_cVirtName_rilevazione,m_cVirtName_rilevazione+".IDMOD="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDMOD,"C",10,0),m_cServer_rilevazione,p_IDMOD),l_cOrderBy);
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_1_1(m_cPhName_mod_ind_def,m_cServer_mod_ind_def);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (l_oSqlDetailLoader.m_bLink_1_1) {
          w_DESCRI = l_rsLoadBody.GetString("DESCRI101");
        } else {
          Link_SZHUMIRICR("Load");
        }
        // Assegna i valori degli op_
        // Assegna il contesto dei figli di testata/piedi a Empty
        SPExtenderLib.LoadExtendedFields(l_rsLoadBody,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"rilevazione")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          w_PROVENIENZA = l_rsLoadBody.GetString("PROVENIENZA");
  ...
  public boolean Delete(String p_IDMOD) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        // Cancella il figlio Anomalie rilevate
        aosdt_anom_ril.w_IDMOD = w_IDMOD;
        aosdt_anom_ril.w_ROWRIL = w_CPROWNUM;
        aosdt_anom_ril.Delete();
      }
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_rilevazione.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDMOD="+CPLib.ToSQL(w_IDMOD,"C",10,0)+" AND CPROWNUM="+CPLib.ToSQL(w_CPROWNUM,"N",4,0);
        l_cSql = l_cSql+m_oWrInfo_rilevazione.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"rilevazione")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_rilevazione,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_IDMOD);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"SZHUMIRICR")) {
      ...
      result.AppendFieldsHash("SZHUMIRICR",CPLib.GetLinkedFieldProps("IDMOD","IDMOD","C",10,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_rilevazione);
      l_cSql = "insert into "+m_oWrInfo_rilevazione.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDMOD,";
      l_cSql = l_cSql+"CONNES,";
      l_cSql = l_cSql+"DTRIL,";
      l_cSql = l_cSql+"PROVENIENZA,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_rilevazione.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"rilevazione")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_PROVENIENZA,"C",100,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_CPROWNUM,"N",4,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_rilevazione.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"rilevazione")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_rilevazione,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
        NotifyEvent("Insert row children start");
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          if ( ! (aosdt_anom_ril.IsUpdated()) && CPLib.IsUpdated(m_cWv_aosdt_anom_ril)) {
          ...
          if (aosdt_anom_ril.IsUpdated()) {
            aosdt_anom_ril.w_IDMOD = w_IDMOD;
            aosdt_anom_ril.w_ROWRIL = w_CPROWNUM;
            if ( ! (aosdt_anom_ril.Save(true))) {
              m_Sql.AbortTransaction();
            }
          }
        }
        NotifyEvent("Insert row children end");
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_rilevazione);
      l_cSql = "Update "+m_oWrInfo_rilevazione.GetTableWriteName()+" set";
      l_cSql = l_cSql+" CONNES = "+CPLib.ToSQL(w_CONNES,"C",16,0);
      l_cSql = l_cSql+", DTRIL = "+CPLib.ToSQL(w_DTRIL,"D",8,0);
      l_cSql = l_cSql+", PROVENIENZA = "+CPLib.ToSQL(w_PROVENIENZA,"C",100,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"rilevazione")) {
      ...
      if (CPLib.ne(l_nUpdatedRows,1)) {
        ...
        NotifyEvent("Update row children start");
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          if ( ! (aosdt_anom_ril.IsUpdated()) && CPLib.IsUpdated(m_cWv_aosdt_anom_ril)) {
            aosdt_anom_ril.SetFromContext(m_cWv_aosdt_anom_ril);
            m_cWv_aosdt_anom_ril = "_not_empty_";
        ...
        NotifyEvent("Update row children end");
        CPLib.SaveRepeatedMNTs(m_Sql,m_MNTs,this,work_children);
      }
    }
