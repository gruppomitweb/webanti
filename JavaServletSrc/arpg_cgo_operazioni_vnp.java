...
public class arpg_cgo_operazioni_vnp extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*CYTVOGCFTP*!";
  ...
  public String getEntityTitle() {
    return "Operazioni da completare (Vincite Prenotate)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_cgo_operazioni_vnp";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 829;
    ...
    l_eg.m_cQueryName = "qbe_cgo_operazioni_vnp";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_cgo_operazioni_vnpBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10454095264",p_Context.Translate("Operazioni da completare (Vincite Prenotate)")},
  ...
  void WriteStyles(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_cgo_operazioni_vnp.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(49);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_cgo_operazioni_vnpBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_cgo_operazioni_vnpBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_cgo_operazioni_vnpBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(829,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_cgo_operazioni_vnp"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_cgo_operazioni_vnp","");
  ...
  void WriteMainFormScript(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_cgo_operazioni_vnpBO getBO(ServletStatus status) {
    arpg_cgo_operazioni_vnpBO BO = new arpg_cgo_operazioni_vnpBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_cgo_operazioni_vnpBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
    ...
    item.put("altInterfaceFor","arpg_cgo_operazioni_vnp");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"49\",\"name\":\"Box_XLVFHSOSYH\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"XLVFHSOSYH\",\"tabindex\":\"11\",\"type\":\"Box\",\"w\":\"824\",\"x\":\"0\",\"y\":\"0\",\"zone\":\"pag1_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Ricerca le operazione da completare delle agenzie\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_RROILCAWGO()\",\"name\":\"images_search_gif\",\"page\":\"1\",\"spuid\":\"RROILCAWGO\",\"src\":\"images\\/search.gif\",\"tabindex\":\"12\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"725\",\"y\":\"9\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esci dalla procedura\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_IIJDTQKPBX()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"IIJDTQKPBX\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"13\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"773\",\"y\":\"9\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Estrazione Excel delle operazioni Selezionate\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_WQOTPOFGXX()\",\"name\":\"images_excel_gif\",\"page\":\"1\",\"spuid\":\"WQOTPOFGXX\",\"src\":\"images\\/excel.gif\",\"tabindex\":\"18\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"677\",\"y\":\"9\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
  ...
  void WriteBody(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_cgo_operazioni_vnpBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_cgo_operazioni_vnp?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_cgo_operazioni_vnp_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_cgo_operazioni_vnpBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_cgo_operazioni_vnpBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_cgo_operazioni_vnpBO BO) {
    boolean result = true;
    return result;
  }
}
