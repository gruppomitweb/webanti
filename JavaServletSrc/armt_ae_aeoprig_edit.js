...
l_bEnabled= ! ( ! (Empty(w_CONNESINT)) && Eq(w_ASPEDITO,1) && Eq(m_cFunction,'view'));
...
l_bEnabled= ! (Eq(m_cFunction,'view'));
SetDisabled('WEIKCFSMRK',l_bEnabled);
...
if (IsDisabledByStateDriver('box_KPRGKTCWLJ')) {
DisableInputsInContainer(Ctrl('KPRGKTCWLJ'),true);
}
if (IsDisabledByStateDriver('box_FXPPJOXLEV')) {
DisableInputsInContainer(Ctrl('FXPPJOXLEV'),true);
}
...
var ctl = _GetCtl(e,'AOOZEQQJTQ');
if (_tracker.goon(ctl,ctrlValue)) {
w_DATESITO=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('AOOZEQQJTQ',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
var ctl = _GetCtl(e,'WSCNNXSPZE');
if (_tracker.goon(ctl,ctrlValue)) {
w_RSPEDITO=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('WSCNNXSPZE',0);
DoUpdate(l_bResult);
...
var ctl = _GetCtl(e,'VXGCUGFCOM');
if (_tracker.goon(ctl,ctrlValue)) {
w_ADATARET=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('VXGCUGFCOM',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH(NullDate(),'D',8,0,TranslatePicture(GetDatePicture(false,'D'),'D'));
...
l_bResult=l_bResult && _ChkObl('IYTHKZIUFV',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',15,0,'');
...
l_bResult=l_bResult && _ChkObl('URGNQSTVHB',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',25,0,'');
...
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('aeopcanc','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
...
parent.ZtVWeb.raiseEventToEvalParms('armt_ae_aeoprig_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_ae_aeoprig_dataNotSent',{'action':formaction});
}
}
}
}
...
l_bResult=false;
w_IDESITO=HtW('','C');
} else if ( ! (_ChkObl('AOOZEQQJTQ',1))) {
_SignErr('AOOZEQQJTQ');
m_cLastWorkVarErrorMsg='10859751047';
l_bResult=false;
w_DATESITO=HtW('','D');
} else if ( ! (_ChkObl('WSCNNXSPZE',1))) {
_SignErr('WSCNNXSPZE');
m_cLastWorkVarErrorMsg='RSPEDITO';
l_bResult=false;
w_RSPEDITO=0;
} else if ( ! (_ChkObl('IYTHKZIUFV',1))) {
_SignErr('IYTHKZIUFV');
m_cLastWorkVarErrorMsg='COLDFILE';
l_bResult=false;
w_COLDFILE=HtW('','C');
} else if ( ! (_ChkObl('URGNQSTVHB',1))) {
_SignErr('URGNQSTVHB');
m_cLastWorkVarErrorMsg='COLDPROG';
l_bResult=false;
w_COLDPROG=HtW('','C');
}
if (l_bResult) {
...
l_oWv.setValue('CDATARET',WtA(w_CDATARET,'D'));
l_oWv.setValue('NOCODFISC',WtA(w_NOCODFISC,'C'));
l_oWv.setValue('IDESITO',WtA(w_IDESITO,'C'));
l_oWv.setValue('DATESITO',WtA(w_DATESITO,'D'));
l_oWv.setValue('PROGOPX',WtA(w_PROGOPX,'N'));
l_oWv.setValue('op_PROGOPX',WtA(op_PROGOPX,'N'));
...
_FillChildren.n=[];
FillWv.n=["RAPPORTO","COINT","TIPOAG","IDBASE","CONNESINT","ADATA","ADATARET","ASPEDITO","AFILE","APROG","AOLDPROG","AOLDFILE","CSPEDITO","CPROG","COLDPROG","CFILE","COLDFILE","IDRIF","ADTPRE","CDTPRE","CDATA","CDATARET","NOCODFISC","IDESITO","DATESITO","PROGOPX","RSPEDITO","xragsoc","dtcanc","spcanc","filecanc","idcanc","xcfestero","desctrap"];
