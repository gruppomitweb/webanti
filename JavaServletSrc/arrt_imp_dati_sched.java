...
public class arrt_imp_dati_sched extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*MRIIMOVEDX*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_errchk) ||  ! (status.m_bpassedw_impzip) ||  ! (status.m_bpassedgMsgProc) ||  ! (status.m_bpassedw_tipodest);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_imp_dati_schedR Rcalled;
    Rcalled = new arrt_imp_dati_schedR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_imp_dati_schedR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_imp_dati_schedR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_imp_dati_sched"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_imp_dati_sched");
      if ( ! (CPLib.IsNull(l))) {
