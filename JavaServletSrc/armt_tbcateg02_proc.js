...
var c;
if(c=Ctrl('SEJWRQHUSP')) selectCombo(c,w_C2CODTAB,'C')
SPBOUILib.SetInputValue('XREVNLNKCY',function(){return WtH(w_C2CODICE,'C',2,0,'')},w_C2CODICE);
SPBOUILib.SetInputValue('RJDJPGMTCC',function(){return WtH(w_C2DESCRI,'C',80,0,'')},w_C2DESCRI);
if (m_bExtended) {
SetExtensions();
}
...
HideUI.lblids["C2CODICE"]=['EHORPFDFFQ'];
HideUI.lblids["C2CODTAB"]=['RGYCBVWYTV'];
HideUI.lblids["C2DESCRI"]=['UKQEHGTCZD'];
function SEJWRQHUSP_Valid(e) {
if (Eq(m_cFunction,'query')) {
if (IsLastInPk('SEJWRQHUSP') ||  ! (m_bQueryOnLatestKeyOnly)) {
DoQuery('C2CODTAB','C',w_C2CODTAB,HtW(getComboValue(Ctrl('SEJWRQHUSP')),'C'),m_bQueryOnLatestKeyOnly);
}
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_C2CODTAB(HtW(getComboValue(Ctrl('SEJWRQHUSP')),'C'),null,e);
return l_bSetResult;
}
}
...
DoQuery('C2CODICE','C',w_C2CODICE,HtW(Ctrl('XREVNLNKCY').value,'C'),m_bQueryOnLatestKeyOnly);
}
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_C2CODICE(HtW(Ctrl('XREVNLNKCY').value,'C'),null,e);
return l_bSetResult;
}
}
...
if (Eq(m_cFunction,'query')) {
DoQuery('C2DESCRI','C',w_C2DESCRI,HtW(Ctrl('RJDJPGMTCC').value,'C'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_C2DESCRI(HtW(Ctrl('RJDJPGMTCC').value,'C'),null,e);
return l_bSetResult;
}
}
...
function Help() {
windowOpenForeground('../doc/armt_tbcateg02_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
m_PrimaryKeys=['C2CODICE','C2CODTAB'];
}
function i_PrimaryKey() {
return 'tbcateg02'+'\\'+CPLib.ToCPStr(w_C2CODTAB)+'\\'+CPLib.ToCPStr(w_C2CODICE);
}
...
p_d.setValue('armt_tbcateg02',m_cSelectedPage);
...
_Obli('C2CODTAB',0,'SEJWRQHUSP',false,"10376019890")
_Obli('C2CODICE',0,'XREVNLNKCY',false,"112851763")
_Obli('C2DESCRI',0,'RJDJPGMTCC',false,"285808099")
