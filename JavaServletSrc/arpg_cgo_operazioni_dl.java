...
public class arpg_cgo_operazioni_dl extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*RMOJWCBAJZ*!";
  static public class ServletStatus extends SPPageServletStatus {
    public void MergeCallerVars(SPParameterSource source) {
      source.SetParameter("datope",source.GetCaller().GetDate("datope","D",8,0));
      source.SetParameter("totope",source.GetCaller().GetNumber("totope","N",10,2));
      source.SetParameter("tipope",source.GetCaller().GetString("tipope","C",1,2));
    }
  }
  ...
  public String getEntityTitle() {
    return "Operazioni non associate (Agenzie)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_cgo_operazioni_dl";
    ...
    l_eg.items = new SPItem[]{
                   new SPItem("datope","D",8,0, "hide",""),
                   new SPItem("gCodDip","C",6,0, "hide","Dipendenza"),
                   new SPItem("totope","N",10,2, "hide",""),
                   new SPItem("tipope","C",1,2, "hide","")
                 };
    l_eg.m_lExecuteAtStart = true;
    l_eg.m_cQueryName = "qbe_cgo_operazioni_dl";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_cgo_operazioni_dlBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1948905605",p_Context.Translate("Operazioni non associate (Agenzie)")},
    {"1991862041",p_Context.Translate("Importo")}};
    return l_translations;
  }
  void WriteStyles(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_cgo_operazioni_dl.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(45);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_cgo_operazioni_dlBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_cgo_operazioni_dlBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_cgo_operazioni_dlBL BO,ServletStatus status) {
    status.out.print("DeclareWVs(");
    status.out.print(SPLib.ToJSValue(BO.w_datope,"D",8,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_gCodDip,"C",6,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_totope,"N",10,2));
    status.out.print(","+SPLib.ToJSValue(BO.w_tipope,"C",1,2));
    status.out.println(")");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      status.out.println("m_bUpdated="+SPLib.ToJSValue(BO.m_bUpdated,"L",0,0)+";");
    }
  }
  void WriteMainFormVarScript(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_cgo_operazioni_dl"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_cgo_operazioni_dl","");
  ...
  void WriteMainFormScript(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
    ...
    status.out.println("// * --- arpg_cgo_operazioni_dl");
    status.out.println("function RecuperaDL() {");
    status.out.println("  if (confirm('Confermi la selezione della registrazione ?')) {");
    status.out.println("    window.opener.Set_NUMOPEDL(c_SNAINUMOPE)");
    status.out.println("    window.opener.RecoverDL()");
  ...
  arpg_cgo_operazioni_dlBO getBO(ServletStatus status) {
    arpg_cgo_operazioni_dlBO BO = new arpg_cgo_operazioni_dlBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_cgo_operazioni_dlBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
    ...
    item.put("altInterfaceFor","arpg_cgo_operazioni_dl");
    items.put(item);
    String s;
    ...
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"45\",\"name\":\"Box_XLVFHSOSYH\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"XLVFHSOSYH\",\"tabindex\":\"10\",\"type\":\"Box\",\"w\":\"800\",\"x\":\"0\",\"y\":\"0\",\"zone\":\"pag1_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteBody(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_cgo_operazioni_dlBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_cgo_operazioni_dl?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_cgo_operazioni_dl_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_cgo_operazioni_dlBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_cgo_operazioni_dlBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_cgo_operazioni_dlBO BO) {
    boolean result = true;
    return result;
  }
}
