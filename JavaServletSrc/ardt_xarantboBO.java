...
// * --- Fine Area Manuale
public class ardt_xarantboBO extends ardt_xarantboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_xarantboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_xarantbo");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xarantbo,m_cPhName_xarantbo,m_cVirtName_xarantbo,m_cVirtName_xarantbo+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_xarantbo,p_RAPPORTO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xarantbo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_xarantbo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND CODINTER="+CPLib.ToSQL(opk_CODINTER,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_xarantbo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xarantbo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_xarantbo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xarantbo);
      l_cSql = "insert into "+m_oWrInfo_xarantbo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_xarantbo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xarantbo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_xarantbo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xarantbo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xarantbo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xarantbo);
      l_cSql = "Update "+m_oWrInfo_xarantbo.GetTableWriteName()+" set";
