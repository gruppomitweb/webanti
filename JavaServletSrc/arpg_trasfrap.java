...
public class arpg_trasfrap extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*EWNHXWJMLM*!";
  ...
  public String getEntityTitle() {
    return "Trasferimento Rapporti";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_trasfrap";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_trasfrapBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1950344097",p_Context.Translate("Trasferimento Rapporti")}};
    return l_translations;
  }
  void WriteStyles(arpg_trasfrapBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_trasfrap.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_trasfrapBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_trasfrapBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_trasfrapBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_trasfrapBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_trasfrapBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(297);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_trasfrapBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_trasfrapBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_trasfrapBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_trasfrapBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_trasfrapBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_trasfrap"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_trasfrap","");
  ...
  void WriteMainFormScript(arpg_trasfrapBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_trasfrapBO getBO(ServletStatus status) {
    arpg_trasfrapBO BO = new arpg_trasfrapBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_trasfrapBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_trasfrapBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_trasfrapBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_trasfrapBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_trasfrapBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_trasfrapBO BO) {
    ...
    item.put("altInterfaceFor","arpg_trasfrap");
    items.put(item);
    String s;
  ...
  void Next(ServletStatus status,arpg_trasfrapBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_trasfrapBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_trasfrapBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_trasfrap?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_trasfrap_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_trasfrapBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_trasfrapBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_trasfrapBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_trasfrapBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_trasfrapBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_trasfrapBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_trasfrap.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_trasfrapBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_trasfrapBO BO) {
    boolean result = true;
    return result;
  }
}
