...
// * --- Fine Area Manuale
public class arpg_pers_oper_prvBO extends arpg_pers_oper_prvBL {
  ...
  public arpg_pers_oper_prvBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_DATAREG = rs.GetDate("DATAREG");
          c_CODANA = rs.GetString("CODANA");
          c_CODVOC = rs.GetString("CODVOC");
