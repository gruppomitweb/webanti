...
// * --- arrt_cgo_operazioni_admin_multi_xls
import java.io.File; 
...
// * --- Fine Area Manuale
public class arrt_cgo_operazioni_admin_multi_xlsR implements CallerWithObjs {
  ...
  public int conta;
  // * --- Fine Area Manuale
  public arrt_cgo_operazioni_admin_multi_xlsR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_operazioni_admin_multi_xls",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_cgo_operazioni_admin_multi_xls";
  ...
  void Page_1() throws Exception {
    CPResultSet Cursor_qbe_cgo_operazioni_agenzia_multi_admin=null;
    ...
    try {
      ...
      /* nomeesito := 'multiple_dacompletare_'+DateTimeToChar(DateTime())+".xls" */
      nomeesito = "multiple_dacompletare_"+CPLib.DateTimeToChar(CPLib.DateTime())+".xls";
      ...
      // * --- Select from qbe_cgo_operazioni_agenzia_multi_admin
      ...
      _h00000026.Set("ticket",pTicket);
      _h00000026.Set("tipogio",pTipGio);
      if (Cursor_qbe_cgo_operazioni_agenzia_multi_admin!=null)
        Cursor_qbe_cgo_operazioni_agenzia_multi_admin.Close();
      Cursor_qbe_cgo_operazioni_agenzia_multi_admin = new VQRHolder("qbe_cgo_operazioni_agenzia_multi_admin",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),_h00000026,true).GetResultSet(m_Ctx);
      while ( ! (Cursor_qbe_cgo_operazioni_agenzia_multi_admin.Eof())) {
        /* codice := qbe_cgo_operazioni_agenzia_multi_admin->SNAINUMOPE */
        codice = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("SNAINUMOPE");
        /* datope := arfn_fdate(qbe_cgo_operazioni_agenzia_multi_admin->DATAOPE) */
        datope = arfn_fdateR.Make(m_Ctx,this).Run(Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetDate("DATAOPE"));
        /* coddip := qbe_cgo_operazioni_agenzia_multi_admin->CODDIPE */
        coddip = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("CODDIPE");
        /* _teldip := '' */
        _teldip = "";
        /* desdip := '' */
        desdip = "";
        ...
        m_cSql = "";
        m_cSql = m_cSql+"CODDIP="+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("CODDIPE"),"C",6,0,m_cServer),m_cServer,Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("CODDIPE"));
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on anadip into routine arrt_cgo_operazioni_admin_multi_xls returns two or more records. This item should return only a record.");
        ...
        /* codcau := qbe_cgo_operazioni_agenzia_multi_admin->CACODICE */
        codcau = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("CACODICE");
        /* descau := qbe_cgo_operazioni_agenzia_multi_admin->TRDESREG */
        descau = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("TRDESREG");
        /* totope := qbe_cgo_operazioni_agenzia_multi_admin->TOTLIRE */
        totope = CPLib.Round(Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetDouble("TOTLIRE"),2);
        /* _segno := qbe_cgo_operazioni_agenzia_multi_admin->SEGNO */
        _segno = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("SEGNO");
        /* giorni := qbe_cgo_operazioni_agenzia_multi_admin->GIORNI */
        giorni = CPLib.Round(Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetDouble("GIORNI"),0);
        /* _ticket := qbe_cgo_operazioni_agenzia_multi_admin->CAIDBIGLIETTO */
        _ticket = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("CAIDBIGLIETTO");
        /* _tipgio := qbe_cgo_operazioni_agenzia_multi_admin->TIPOGIOCO */
        _tipgio = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("TIPOGIOCO");
        /* datesi := arfn_fdate(qbe_cgo_operazioni_agenzia_multi_admin->CGODATCON) */
        datesi = arfn_fdateR.Make(m_Ctx,this).Run(Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetDate("CGODATCON"));
        /* desesi := qbe_cgo_operazioni_agenzia_multi_admin->ECDESCRI */
        desesi = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("ECDESCRI");
        /* notesi := qbe_cgo_operazioni_agenzia_multi_admin->CGONOTCON */
        notesi = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.GetString("CGONOTCON");
        ...
        foglio.addCell(stringa); 
        riga = riga +1;
        Cursor_qbe_cgo_operazioni_agenzia_multi_admin.Next();
      }
      m_cConnectivityError = Cursor_qbe_cgo_operazioni_agenzia_multi_admin.ErrorMessage();
      Cursor_qbe_cgo_operazioni_agenzia_multi_admin.Close();
      // * --- End Select
      //Aggiusta le colonne
  ...
  public static arrt_cgo_operazioni_admin_multi_xlsR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_operazioni_admin_multi_xlsR(p_Ctx, p_Caller);
  }
  public void Blank() {
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_cgo_operazioni_agenzia_multi_admin,
  public static final String m_cVQRList = ",qbe_cgo_operazioni_agenzia_multi_admin,";
