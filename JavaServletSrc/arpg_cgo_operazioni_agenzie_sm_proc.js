...
_modifyandopen('armt_snai_oper'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=view&m_cAtExit=close&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes',null)
}
_ResetTracker();
...
function Help() {
windowOpenForeground('../doc/arpg_cgo_operazioni_agenzie_sm_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_cgo_operazioni_agenzie_sm',m_cSelectedPage);
...
if (start) {
if (Empty(w_tipogioco) && Ne(opener.w_tipogioco,null)) {
TransferBetweenWindows(opener,'w_tipogioco','w_tipogioco');
}
}
}
m_bCalculating=true;
...
GroupFooter_BeginHtml(document,1,'12143326568'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'12143326568'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'11606455656'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_0B956AB0"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'11606455656'+l_nIdx+'','A')
} else {
}
Row_BeginHtml(document,l_nEvenOdd,'1077898025'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_0B947018"+l_nIdx+"' style='position:relative;height:20px'>" +
"")
document.writeln(
 "<span class=' SPLabel BZENUMNSXW_DIV'>" +
 "<label class='BZENUMNSXW'>" +
...
document.writeln(
 ...
 "<label class='VIKJFKDXAW' title="+ToHTMLValue(Translate("1179072480"))+">" +
...
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'1077898025'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
