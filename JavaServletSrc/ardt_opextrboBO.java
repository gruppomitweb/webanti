...
// * --- Fine Area Manuale
public class ardt_opextrboBO extends ardt_opextrboBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_9(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_2_9.CODVAL AS CODVAL209";
        m_cSelect = m_cSelect+" ,link_2_9.DESCRI AS DESCRI209";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_9","VALUTA","CODVAL");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_9","VALUTA","CODVAL",true);
  ...
  // * --- Fine Area Manuale
  public ardt_opextrboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_opextrbo");
    }
  ...
  public boolean Load(String p_COLLEG) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_opextrbo,m_cPhName_opextrbo,m_cVirtName_opextrbo,m_cVirtName_opextrbo+".COLLEG="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_COLLEG,"C",16,0),m_cServer_opextrbo,p_COLLEG),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_9(m_cPhName_tbvalute,m_cServer_tbvalute);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          w_DATAOPE = l_rsLoadBody.GetDate("DATAOPE");
          w_COINT = l_rsLoadBody.GetString("COINT");
          w_NOTE = l_rsLoadBody.GetString("NOTE");
          w_SALDO = l_rsLoadBody.GetDouble("SALDO");
          w_QUANTITA = l_rsLoadBody.GetDouble("QUANTITA");
          w_VALUTA = l_rsLoadBody.GetString("VALUTA");
          if (l_oSqlDetailLoader.m_bLink_2_9) {
            w_xdesval = l_rsLoadBody.GetString("DESCRI209");
          } else {
            Link_PECYSWBJIH("Load");
          }
          w_PROGOPEX = l_rsLoadBody.GetDouble("PROGOPEX");
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        ...
        l_cSql = l_cSql+"COLLEG="+CPLib.ToSQL(w_COLLEG,"C",16,0)+" AND ANNO="+CPLib.ToSQL(opk_ANNO,"C",4,0);
  ...
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"PECYSWBJIH")) {
      ...
      result.AppendFieldsHash("PECYSWBJIH",CPLib.GetLinkedFieldProps("CODVAL","VALUTA","C",3,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_NOTE,"C",50,0)+", ";
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_PROGOPEX,"N",10,0)+", ";
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      l_cSql = l_cSql+", NOTE = "+CPLib.ToSQL(w_NOTE,"C",50,0);
      ...
      l_cSql = l_cSql+", PROGOPEX = "+CPLib.ToSQL(w_PROGOPEX,"N",10,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"opextrbo")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", ANNO = "+CPLib.ToSQL(w_ANNO,"C",4,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"COLLEG="+CPLib.ToSQL(w_COLLEG,"C",16,0)+" AND ANNO="+CPLib.ToSQL(opk_ANNO,"C",4,0);
      l_cSql = l_cSql+m_oWrInfo_opextrbo.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"opextrbo")) {
