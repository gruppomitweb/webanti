...
// * --- Fine Area Manuale
public class arrt_stpoperR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_stpoperR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_stpoper",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* w_stato Char(1) // stato delle operazioni da rendere visibili sul report */
      /* w_OrderBy Char(7) */
      /* w_daCodDip Char(6) */
      /* w_aCodDip Char(6) */
      ...
      /* If w_stato<>'T' */
      if (CPLib.ne(w_stato,"T")) {
        /* statoreg := iif(w_stato='A','|0|1',iif(w_stato='M','|3','|2')) */
        statoreg = (CPLib.eqr(w_stato,"A")?"|0|1":(CPLib.eqr(w_stato,"M")?"|3":"|2"));
      } // End If
      // Begin Transaction
      ...
      try {
        /* Case w_Tipop='T' and w_stato='T' */
        if (CPLib.eqr(w_Tipop,"T") && CPLib.eqr(w_stato,"T")) {
          ...
          while ( ! (Cursor_qbe_alloperazbo.Eof())) {
            ...
            m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000028")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          // * --- End Select
          /* Case w_Tipop<>'T' and w_stato='T' */
        } else if (CPLib.ne(w_Tipop,"T") && CPLib.eqr(w_stato,"T")) {
          /* If w_Tipop='R' */
          if (CPLib.eqr(w_Tipop,"R")) {
            ...
            Cursor_qbe_alloperazbo = new VQRHolder("qbe_alloperazbo",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),this,true).GetResultSet(m_Ctx);
            while ( ! (Cursor_qbe_alloperazbo.Eof())) {
              /* If not(Empty(qbe_alloperazbo->RAPPORTO)) */
              if ( ! (CPLib.Empty(Cursor_qbe_alloperazbo.GetString("RAPPORTO")))) {
                // * --- Insert into tmp_stpoperazioni from values
                m_Sql.RequireTransaction();
                m_cServer = m_Ctx.GetServer("tmp_stpoperazioni");
                m_cPhName = m_Ctx.GetPhName("tmp_stpoperazioni");
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"0000002C")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
            ...
            Cursor_qbe_alloperazbo = new VQRHolder("qbe_alloperazbo",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),this,true).GetResultSet(m_Ctx);
            while ( ! (Cursor_qbe_alloperazbo.Eof())) {
              /* If Empty(qbe_alloperazbo->RAPPORTO) */
              if (CPLib.Empty(Cursor_qbe_alloperazbo.GetString("RAPPORTO"))) {
                // * --- Insert into tmp_stpoperazioni from values
                m_Sql.RequireTransaction();
                m_cServer = m_Ctx.GetServer("tmp_stpoperazioni");
                ...
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"0000002F")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          /* Case w_Tipop='T' and w_stato<>'T' */
        } else if (CPLib.eqr(w_Tipop,"T") && CPLib.ne(w_stato,"T")) {
          // * --- Select from qbe_alloperazbo
          if (Cursor_qbe_alloperazbo!=null)
            Cursor_qbe_alloperazbo.Close();
          Cursor_qbe_alloperazbo = new VQRHolder("qbe_alloperazbo",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),this,true).GetResultSet(m_Ctx);
          while ( ! (Cursor_qbe_alloperazbo.Eof())) {
            /* If At(LRTrim(qbe_alloperazbo->STATOREG),statoreg) > 0 */
            if (CPLib.gt(CPLib.At(CPLib.LRTrim(Cursor_qbe_alloperazbo.GetString("STATOREG")),statoreg),0)) {
              // * --- Insert into tmp_stpoperazioni from values
              m_Sql.RequireTransaction();
              m_cServer = m_Ctx.GetServer("tmp_stpoperazioni");
              m_cPhName = m_Ctx.GetPhName("tmp_stpoperazioni");
              m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000032")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          if (CPLib.eqr(w_Tipop,"R")) {
            ...
            while ( ! (Cursor_qbe_alloperazbo.Eof())) {
              /* If not(Empty(qbe_alloperazbo->RAPPORTO)) */
              if ( ! (CPLib.Empty(Cursor_qbe_alloperazbo.GetString("RAPPORTO")))) {
                /* If At(qbe_alloperazbo->STATOREG,statoreg)>0 */
                if (CPLib.gt(CPLib.At(Cursor_qbe_alloperazbo.GetString("STATOREG"),statoreg),0)) {
                  // * --- Insert into tmp_stpoperazioni from values
                  m_Sql.RequireTransaction();
                  m_cServer = m_Ctx.GetServer("tmp_stpoperazioni");
                  m_cPhName = m_Ctx.GetPhName("tmp_stpoperazioni");
                  m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                  m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                  m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000037")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
            ...
            while ( ! (Cursor_qbe_alloperazbo.Eof())) {
              ...
              if (CPLib.Empty(Cursor_qbe_alloperazbo.GetString("RAPPORTO"))) {
                ...
                if (CPLib.gt(CPLib.At(Cursor_qbe_alloperazbo.GetString("STATOREG"),statoreg),0)) {
                  ...
                  m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                  m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                  m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"0000003B")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
        ...
        if (CPLib.eqr(w_tipstp,"S")) {
          ...
          if (CPLib.eqr(_conta,0)) {
            ...
            m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
            m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
            m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000041")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
              ...
              if ( ! (CPLib.Empty(CPLib.LRTrim(Cursor_tmp_stpoperazioni.GetString("CONNESCLI"))))) {
                ...
                if (CPLib.Empty(CPLib.LRTrim(_numsto))) {
                  ...
                  while ( ! (Cursor_personbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"0000004A")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                  ...
                  while ( ! (Cursor_wersonbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"0000004C")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
              ...
              if ( ! (CPLib.Empty(CPLib.LRTrim(Cursor_tmp_stpoperazioni.GetString("CONNESOPER"))))) {
                ...
                if (CPLib.Empty(CPLib.LRTrim(_numsto))) {
                  ...
                  while ( ! (Cursor_personbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000051")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                  ...
                  while ( ! (Cursor_wersonbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000053")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
              ...
              if ( ! (CPLib.Empty(CPLib.LRTrim(Cursor_tmp_stpoperazioni.GetString("CONNESBEN"))))) {
                ...
                if (CPLib.Empty(CPLib.LRTrim(_numsto))) {
                  ...
                  while ( ! (Cursor_personbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000058")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                  ...
                  while ( ! (Cursor_wersonbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"0000005A")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
            ...
            _nomereport = "arrp_stpoper.vrp";
            /* _reportname := 'Operazioni_AUI_'+DateTimeToChar(DateTime()) // Nome del Report */
            _reportname = "Operazioni_AUI_"+CPLib.DateTimeToChar(CPLib.DateTime());
          } // End If
        } else { // Else
          /* If _conta=0 */
          if (CPLib.eqr(_conta,0)) {
            ...
            m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
            m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
            m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000060")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
              ...
              if ( ! (CPLib.Empty(CPLib.LRTrim(Cursor_tmp_stpoperazioni.GetString("CONNESCLI"))))) {
                ...
                if (CPLib.Empty(CPLib.LRTrim(_numsto))) {
                  ...
                  while ( ! (Cursor_personbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000069")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                    m_cSql = m_cSql+"RAGCLI = "+CPLib.ToSQL(Cursor_personbo.GetString("RAGSOC"),"C",70,0,m_cServer)+", ";
                    m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"tmp_stpoperazioni",false,true);
                    m_cSql = m_cSql+" where ";
                    m_cSql = m_cSql+"IDBASE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_tmp_stpoperazioni.GetString("IDBASE"),"?",0,0,m_cServer),m_cServer,Cursor_tmp_stpoperazioni.GetString("IDBASE"))+"";
                  ...
                  while ( ! (Cursor_wersonbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"0000006B")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                    m_cSql = m_cSql+"RAGCLI = "+CPLib.ToSQL(Cursor_wersonbo.GetString("RAGSOC"),"C",70,0,m_cServer)+", ";
                    m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"tmp_stpoperazioni",false,true);
                    m_cSql = m_cSql+" where ";
                    m_cSql = m_cSql+"IDBASE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_tmp_stpoperazioni.GetString("IDBASE"),"?",0,0,m_cServer),m_cServer,Cursor_tmp_stpoperazioni.GetString("IDBASE"))+"";
              ...
              if ( ! (CPLib.Empty(CPLib.LRTrim(Cursor_tmp_stpoperazioni.GetString("CONNESOPER"))))) {
                ...
                if (CPLib.Empty(CPLib.LRTrim(_numsto))) {
                  ...
                  while ( ! (Cursor_personbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000070")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                    m_cSql = m_cSql+"RAGOPE = "+CPLib.ToSQL(Cursor_personbo.GetString("RAGSOC"),"C",70,0,m_cServer)+", ";
                    m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"tmp_stpoperazioni",false,true);
                    m_cSql = m_cSql+" where ";
                    m_cSql = m_cSql+"IDBASE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_tmp_stpoperazioni.GetString("IDBASE"),"?",0,0,m_cServer),m_cServer,Cursor_tmp_stpoperazioni.GetString("IDBASE"))+"";
                  ...
                  while ( ! (Cursor_wersonbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000072")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                    m_cSql = m_cSql+"RAGOPE = "+CPLib.ToSQL(Cursor_wersonbo.GetString("RAGSOC"),"C",70,0,m_cServer)+", ";
                    m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"tmp_stpoperazioni",false,true);
                    m_cSql = m_cSql+" where ";
                    m_cSql = m_cSql+"IDBASE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_tmp_stpoperazioni.GetString("IDBASE"),"?",0,0,m_cServer),m_cServer,Cursor_tmp_stpoperazioni.GetString("IDBASE"))+"";
              ...
              if ( ! (CPLib.Empty(CPLib.LRTrim(Cursor_tmp_stpoperazioni.GetString("CONNESBEN"))))) {
                ...
                if (CPLib.Empty(CPLib.LRTrim(_numsto))) {
                  ...
                  while ( ! (Cursor_personbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000077")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                    m_cSql = m_cSql+"C_RAG = "+CPLib.ToSQL(Cursor_personbo.GetString("RAGSOC"),"C",70,0,m_cServer)+", ";
                    m_cSql = m_cSql+"C_STA = "+CPLib.ToSQL(Cursor_personbo.GetString("PAESE"),"C",3,0,m_cServer)+", ";
                    m_cSql = m_cSql+"C_CTA = "+CPLib.ToSQL(Cursor_personbo.GetString("DESCCIT"),"C",30,0,m_cServer)+", ";
                    m_cSql = m_cSql+"C_CAB = "+CPLib.ToSQL(Cursor_personbo.GetString("CODCAB"),"C",6,0,m_cServer)+", ";
                  ...
                  while ( ! (Cursor_wersonbo.Eof())) {
                    ...
                    m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
                    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                    m_cSql = CPLib.GetSQLPrefixComment("arrt_stpoper",111,"00000079")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                    m_cSql = m_cSql+"C_RAG = "+CPLib.ToSQL(Cursor_wersonbo.GetString("RAGSOC"),"C",70,0,m_cServer)+", ";
                    m_cSql = m_cSql+"C_STA = "+CPLib.ToSQL(Cursor_wersonbo.GetString("PAESE"),"C",3,0,m_cServer)+", ";
                    m_cSql = m_cSql+"C_CTA = "+CPLib.ToSQL(Cursor_wersonbo.GetString("DESCCIT"),"C",30,0,m_cServer)+", ";
                    m_cSql = m_cSql+"C_CAB = "+CPLib.ToSQL(Cursor_wersonbo.GetString("CODCAB"),"C",6,0,m_cServer)+", ";
            ...
            /* _rotate := 'PORTRAIT' */
            _rotate = "PORTRAIT";
            /* _reportname := 'Analitica_Operazioni_AUI_'+DateTimeToChar(DateTime()) // Nome del Report */
            _reportname = "Analitica_Operazioni_AUI_"+CPLib.DateTimeToChar(CPLib.DateTime());
            /* _nomereport := 'arrp_stpoperana.vrp' // stampa analitica delle operazioni */
      ...
      f.SetParameter("tipo_arch","A.U.I.");
      f.SetParameter("fonte_arch","OPERAZIONI");
      f.SetParameter("FileName",_reportname);
      f.SetParameter("ReportName",_nomereport);
      f.SetParameter("m_cParameterSequence","formatTypes"+","+"rotation"+","+"stato"+","+"c_dadata"+","+"c_adata"+","+"daCodCli"+","+"aCodCli"+","+"ONLYOPE"+","+"ONLYRAP"+","+"OrderBy"+","+"DescAzi"+","+"daCodDip"+","+"aCodDip"+","+"showParameterMask"+","+"tipo_arch"+","+"fonte_arch"+","+"FileName"+","+"ReportName");
  ...
  public Forward Run() {
    ...
    try {
      ...
      m_Caller.SetString("stato","C",1,0,w_stato);
  ...
  public static arrt_stpoperR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_stpoperR(p_Ctx, p_Caller);
  }
  public void Blank() {
    ...
    w_stato = m_Caller.GetString("stato","C",1,0);
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_alloperazbo,qbe_alloperazbo,qbe_alloperazbo,qbe_alloperazbo,qbe_alloperazbo,qbe_alloperazbo,qbe_tot_tmp_operazioni,
  public static final String m_cVQRList = ",qbe_alloperazbo,qbe_alloperazbo,qbe_alloperazbo,qbe_alloperazbo,qbe_alloperazbo,qbe_alloperazbo,qbe_tot_tmp_operazioni,";
