...
public class arrt_deletefile_saldi extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*38c93435*!";
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (m_bpassedw_nomefile) {
        ...
        if (source.IsForwarded()) {
          w_nomefile = source.GetCaller().GetString("nomefile","C",50,0);
      ...
      if (m_bpassedw_anno) {
        w_anno = source.GetParameter("w_anno",source.GetParameter("anno",""));
      } else {
        if (source.IsForwarded()) {
          w_anno = source.GetCaller().GetString("anno","C",4,0);
          m_bpassedw_anno = true;
        } else {
          w_anno = source.GetParameter("anno",source.GetParameter("w_anno",""));
        }
      }
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_deletefile_saldi";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_idbase) ||  ! (status.m_bpassedw_anno) ||  ! (status.m_bpassedw_nomefile);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_nomefile)) {
      status.out.println("l_cV=WtA(l_oWnd('w_nomefile',"+SPLib.ToJSValue(status.w_nomefile,"C",50,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_nomefile,"C",50,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_nomefile',l_cV);");
    if ( ! (status.m_bpassedw_anno)) {
      status.out.println("l_cV=WtA(l_oWnd('w_anno',"+SPLib.ToJSValue(status.w_anno,"C",4,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_anno,"C",4,0)+",'C');");
    }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_deletefile_saldiR Rcalled;
    Rcalled = new arrt_deletefile_saldiR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_deletefile_saldiR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_deletefile_saldiR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_deletefile_saldi"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("window[f].w_anno="+SPLib.ToJSValue(status.w_anno,"C",0,0)+";");
        status.out.println("}");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_deletefile_saldi");
      if ( ! (CPLib.IsNull(l))) {
