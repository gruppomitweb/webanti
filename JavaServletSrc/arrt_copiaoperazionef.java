...
public class arrt_copiaoperazionef extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*d610cba2*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_copiaoperazionef";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_STATOREG) ||  ! (status.m_bpassedw_IDBASE2) ||  ! (status.m_bpassedw_IDEREG) ||  ! (status.m_bpassedw_FLAGRAP2) ||  ! (status.m_bpassedw_NUMPROG) ||  ! (status.m_bpassedw_DATAOPE) ||  ! (status.m_bpassedw_DATARETT) ||  ! (status.m_bpassedw_OPERAZMOD) ||  ! (status.m_bpassedw_IDBASE);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_copiaoperazionefR Rcalled;
    Rcalled = new arrt_copiaoperazionefR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_copiaoperazionefR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_copiaoperazionefR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_copiaoperazionef"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_copiaoperazionef");
      if ( ! (CPLib.IsNull(l))) {
