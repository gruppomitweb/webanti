...
public class arfn_datisog_sol extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*7cf9ab88*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arfn_datisog_solR Rcalled;
    Rcalled = new arfn_datisog_solR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaaaaaa_retval = Rcalled.Run(status.pCli,status.pDatOpe,status.pRap,status.pTRap,status.pTipoR,status.pRAE,status.pSTATO,status.pDatRet);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arfn_datisog_solR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arfn_datisog_solR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arfn_datisog_sol"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
