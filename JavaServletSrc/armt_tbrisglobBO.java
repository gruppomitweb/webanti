...
// * --- Fine Area Manuale
public class armt_tbrisglobBO extends armt_tbrisglobBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbrisglobBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbrisglob");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbrisglob,m_cPhName_tbrisglob,m_cVirtName_tbrisglob,m_cVirtName_tbrisglob+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",3,0),m_cServer_tbrisglob,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        w_GIORNI = l_rsLoad.GetDouble("GIORNI");
        w_MIT = l_rsLoad.GetDouble("MIT");
        // Assegna i valori degli op_
        SPExtenderLib.LoadExtendedFields(l_rsLoad,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbrisglob")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbrisglob);
      l_cSql = "insert into "+m_oWrInfo_tbrisglob.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_tbrisglob.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbrisglob")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_CODICE,"C",3,0)+", ";
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_MIT,"N",1,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbrisglob.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbrisglob")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbrisglob,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbrisglob);
      l_cSql = "Update "+m_oWrInfo_tbrisglob.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+", MIT = "+CPLib.ToSQL(w_MIT,"N",1,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbrisglob")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",3,0),m_cServer_tbrisglob,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbrisglob.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbrisglob")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbrisglob.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",3,0),m_cServer_tbrisglob,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbrisglob.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbrisglob")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbrisglob,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
