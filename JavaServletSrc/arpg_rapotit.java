...
public class arpg_rapotit extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*VEZIPSNEXO*!";
  ...
  public String getEntityTitle() {
    return "Informazioni Titolari (A.U.I.)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_rapotit";
    ...
    l_eg.items = new SPItem[]{
                   new SPItem("CODCLI","C",16,0, "hide","")
                 };
    l_eg.m_lExecuteAtStart = true;
    l_eg.m_cQueryName = "qbe_rapotit";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_rapotitBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10203182545",p_Context.Translate("Informazioni Titolari (A.U.I.)")},
  ...
  void WriteStyles(arpg_rapotitBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_rapotit.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_rapotitBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_rapotitBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_rapotitBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_rapotitBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_rapotitBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_rapotitBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_rapotitBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_rapotit"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_rapotit","");
  ...
  void WriteMainFormScript(arpg_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_rapotitBO getBO(ServletStatus status) {
    arpg_rapotitBO BO = new arpg_rapotitBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_rapotitBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_rapotitBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_rapotitBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_rapotitBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_rapotitBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_rapotitBO BO) {
    ...
    item.put("altInterfaceFor","arpg_rapotit");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_rapotitBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_rapotitBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_rapotitBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_rapotitBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_rapotitBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_rapotitBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_rapotit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_rapotit_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_rapotitBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_rapotitBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_rapotitBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_rapotitBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_rapotitBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_rapotitBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_rapotitBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_rapotitBO BO) {
    boolean result = true;
    return result;
  }
}
