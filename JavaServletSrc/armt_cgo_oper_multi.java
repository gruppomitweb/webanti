...
public class armt_cgo_oper_multi extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*1b00fda8*!";
  ...
  public String getEntityTitle() {
    return "Nuova Multipla (CGO)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_cgo_oper_multi";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 1024;
    l_eg.m_nEntityHeight = 1036;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"cgo_operazioni_dl_multi","cgo_tbtipgio","cgo_vltplat","tbcauana","tbcitta","tbstati","cgo_tipdoc","cgo_autorita","tbtipreg","personbo","intermbo","operazbo","tbcausin","tbtipatt","anadip"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_cgo_oper_multiBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1173302013",p_Context.Translate("Nuova Multipla (CGO)")},
  ...
  void WriteStyles(armt_cgo_oper_multiBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_cgo_oper_multi.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_cgo_oper_multi',false);");
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_cgo_oper_multi",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  // GetPortlet_Documenti_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KUBMCPCHPZ_Vars(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteControls(armt_cgo_oper_multiBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    WriteControls_p2(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,3);
  }
  void WriteControls_p1(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:1036px;width:1024px;display:none'>" +
    "");
  ...
  void WriteControls_p2(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b0(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("Base")) {
      ...
      "");
      status.out.println(
       "<span class='SPLabel' id='VTHHSCUKRU_DIV'>" +
       "<label id='VTHHSCUKRU' for='BMTLGIUQTH'>" +
  ...
  void WriteControls_p1_b1_CloseBand(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("Operazione")) {
      ...
      status.out.println(
       "<script>" +
      "");
      status.out.println("ToggleCollapsibleBox.subBands['YORQOIOBQN']=['band_1_102','band_1_192','band_1_421'];");
      status.out.println(
       "</script>" +
      "");
      if (status.HasZoneAltInterface("Operazione@BEGIN") || status.HasZoneAltInterface("Operazione@END")) {
  ...
  void WriteControls_p1_b3_CloseBand(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3_b0(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_102' style='position:relative;height:90px;display:block' page='1'>" +
    "");
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<select name='CACODICE' id='WJQFIEBCQV' class='"+status.ObligatoryClass("Combobox")+"' tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"'>" +
  ...
  void WriteControls_p1_b3_b1(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_192' style='position:relative;display:block' page='1'>" +
    "");
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<iframe name='ardt_cgo_oper_multi' id='AAHJPBZVGL' frameborder='0' allowtransparency='true' toResize='no' tabindex='"+status.m_nTabIndex+"' src='javascript:[].join()'>" +
     "</iframe>" +
    "");
    status.out.println(
     "</div>" +
    ...
    status.out.println(
     "<script>" +
    "");
    status.out.println("ToggleCollapsibleBox.children['band_1_192']=['AAHJPBZVGL'];");
    status.out.println(
     "</script>" +
    "");
  }
  void WriteControls_p1_b3_b2(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_421' style='position:relative;height:124px;display:block' page='1'>" +
    "");
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    "");
    status.out.println(
     "<span class='SPLabel' id='KPHIUBZBVV_DIV'>" +
     "<label id='KPHIUBZBVV' for='TTZZVOAPLN'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("WALLET:"))+"" +
     "</label>" +
     "</span>" +
    "");
  ...
  void WriteControls_p1_b4(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b5(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<div id='AQSQVBKXAL' class='CollapsibleBody' style='position:relative;height:424px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("Soggetto")) {
      ...
      if (status.HasZoneAltInterface("Soggetto@BEGIN") || status.HasZoneAltInterface("Soggetto@END")) {
        status.out.println(
         "<div id='AQSQVBKXAL_boxContent' style='position:relative;height:424px;display:block'>" +
        "");
      }
      CPResultSet l_rsLoad=null;
      VQRHolder l_VQRHolder=null;
  ...
  void WriteControls_p1_b5_1(armt_cgo_oper_multiBO BO,ServletStatus status) {
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input name='PROFESSIONE' id='FRTNMCIRCW' type='text' class='"+status.ObligatoryClass("Field",BO.FRTNMCIRCW_isObligatory())+"' value="+SPLib.ToHTMLValue(BO.w_PROFESSIONE,"C",40,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='40' size='40' fieldtype='C' style='display:none'>" +
    "");
    status.out.println(
     "<span class='SPLabel' id='WRCLDQWIXL_DIV' style='display:none'>" +
     "<label id='WRCLDQWIXL' for='FRTNMCIRCW'>" +
  ...
  void WriteControls_p1_b5_CloseBand(armt_cgo_oper_multiBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("Soggetto")) {
      ...
      status.out.println("Ctrl('AQSQVBKXAL').setAttribute('band_height',424);");
    }
  ...
  void WriteControls_p1_b6(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteControls_p2_b0(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteControls_p2_b1(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p2_b1_CloseBand(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteControls_p2_b2(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_cgo_oper_multiBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_cgo_oper_multiBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
    if (BO.ardt_cgo_oper_multi.IsUpdated()) {
      BO.m_cWv_ardt_cgo_oper_multi = BO.ardt_cgo_oper_multi.GetContext();
    }
  }
  static void PrintStateWorkVars(armt_cgo_oper_multiBL BO,ServletStatus status) {
    ...
    if (CPLib.eq(BO.m_cWv_ardt_cgo_oper_multi,"_not_empty_") && status.m_bBrowserUpdate) {
      status.out.print(","+"m_cWv_ardt_cgo_oper_multi");
    } else if (CPLib.eq(BO.m_cWv_ardt_cgo_oper_multi,"_not_empty_")) {
      BO.m_cWv_ardt_cgo_oper_multi = BO.ardt_cgo_oper_multi.GetContext();
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_ardt_cgo_oper_multi,"C",0,0));
    } else {
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_ardt_cgo_oper_multi,"C",0,0));
    }
  ...
  void WriteMainFormVarScript(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(1036,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_cgo_oper_multi"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_cgo_oper_multi","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_cgo_oper_multi",BO.GetCurrentState());
    ...
    status.out.println("// * --- armt_cgo_oper_multi");
  ...
  void WriteMainFormScript(armt_cgo_oper_multiBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('TTZZVOAPLN')) SetDisabled(c,true);");
        ...
        status.out.println("if(c=Ctrl('FRTNMCIRCW')) SetDisabled(c,true);");
    ...
    status.out.println("if (Eq(name,'ardt_cgo_oper_multi')) {");
    status.out.println("l_cRes='"+SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_cgo_oper_multi")+"';");
    ...
    status.out.println("  arrt_cgo_read_codfisc_multi([['pObj',this],['pTipo','I']])");
    status.out.println("  Calculate()");
    status.out.println("}");
    status.out.println("");
    status.out.println("function DatiCN() {");
    status.out.println("  arrt_cgo_read_codfisc_multi([['pObj',this],['pTipo','E']])");
    status.out.println("  Calculate()");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
  ...
  armt_cgo_oper_multiBO getBO(ServletStatus status) {
    armt_cgo_oper_multiBO BO = new armt_cgo_oper_multiBO(status.context);
    // gli op_ possono essere utilizzati dalla servlet anche prima di essere inizializzati
    BO.op_SNAINUMOPE = BO.w_SNAINUMOPE;
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(armt_cgo_oper_multiBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("cgo_operazioni_dl_multi",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_cgo_oper_multiBO BO) {
  ...
  void Calculation_OATZTIMJMD(ServletStatus status,armt_cgo_oper_multiBO BO) {
  ...
  void Calculation_GCDVVJTTDD(ServletStatus status,armt_cgo_oper_multiBO BO) {
    arrt_cgo_delete_docR l_oRoutine_arrt_cgo_delete_doc;
    l_oRoutine_arrt_cgo_delete_doc = arrt_cgo_delete_docR.Make(status.context,BO);
    l_oRoutine_arrt_cgo_delete_doc.pIDDOC = BO.w_IDDOCALL;
    l_oRoutine_arrt_cgo_delete_doc.pSaved = BO.w_saveddoc;
  ...
  void Calculation_BVTIXLYUGT(ServletStatus status,armt_cgo_oper_multiBO BO) {
    BO.w_PEP = "N";
  }
  void Calculation_LAEGOZNRMQ(ServletStatus status,armt_cgo_oper_multiBO BO) {
    BO.w_IDDOCALL = (CPLib.Empty(BO.w_IDDOCALL)?LibreriaMit.nextUID():BO.w_IDDOCALL);
  }
  void WriteTableCombo(ServletStatus status,armt_cgo_oper_multiBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
    l_cTmp = "[";
    if (l_rsLoad!=null)
  ...
  CPResultSet QueryKeyCursor(armt_cgo_oper_multiBO BO,ServletStatus status) {
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_cgo_oper_multiBL.QueryFilter(status.context);
  ...
  void Query(ServletStatus status,armt_cgo_oper_multiBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
  ...
  void View(ServletStatus status,armt_cgo_oper_multiBO BO) {
  ...
  void Edit(ServletStatus status,armt_cgo_oper_multiBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_cgo_oper_multiBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_cgo_oper_multiBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_cgo_oper_multiBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_cgo_oper_multiBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_cgo_oper_multiBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_cgo_oper_multiBO BO,SPParameterSource source) {
    BO.Load(status.w_SNAINUMOPE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_cgo_oper_multiBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_cgo_oper_multiBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_cgo_oper_multiBO BO) {
    ...
    item.put("altInterfaceFor","armt_cgo_oper_multi");
    items.put(item);
    String s;
    ...
    items.put(ctrl);
    s = "{\"comment\":\"CAB Dipendenza\",\"disabled\":\"true\",\"h\":21,\"maxlength\":6,\"name\":\"CODCABDIP\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"OZEOCQRSEQ\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":56,\"x\":963,\"y\":20,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Intermediario\",\"disabled\":\"true\",\"h\":20,\"maxlength\":11,\"name\":\"gIntemediario\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"GXLWKNXGMM\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":139,\"y\":20,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_LOEAMXUYLA\",\"page\":1,\"spuid\":\"LOEAMXUYLA\",\"tabindex\":198,\"type\":\"Label\",\"value\":\"Intermediario:\",\"w\":121,\"x\":16,\"y\":24,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Codice Dipendenza\",\"h\":21,\"maxlength\":6,\"name\":\"CODDIPE\",\"page\":1,\"spuid\":\"AVHTGZTOSZ\",\"tabindex\":199,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":48,\"x\":267,\"y\":20,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Citt\\u00E0 Dipendenza\",\"disabled\":\"true\",\"h\":21,\"maxlength\":30,\"name\":\"DESCCITDIP\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"RLUMBFHPUD\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":198,\"x\":659,\"y\":20,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_GZVEXPFQCZ\",\"page\":1,\"spuid\":\"GZVEXPFQCZ\",\"tabindex\":93,\"type\":\"Label\",\"value\":\"N\\u00B0 Operazione:\",\"w\":121,\"x\":16,\"y\":52,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_CTLYJMTHDV\",\"page\":1,\"spuid\":\"CTLYJMTHDV\",\"tabindex\":202,\"type\":\"Label\",\"value\":\"Citt\\u00E0:\",\"w\":47,\"x\":613,\"y\":24,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_JWSDOSRGAG\",\"page\":1,\"spuid\":\"JWSDOSRGAG\",\"tabindex\":203,\"type\":\"Label\",\"value\":\"Pr.:\",\"w\":32,\"x\":859,\"y\":24,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_ENBRMMDVUP\",\"page\":1,\"spuid\":\"ENBRMMDVUP\",\"tabindex\":204,\"type\":\"Label\",\"value\":\"CAB:\",\"w\":42,\"x\":923,\"y\":24,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":17,\"name\":\"label_BLXOBUQVYE\",\"page\":1,\"spuid\":\"BLXOBUQVYE\",\"tabindex\":205,\"type\":\"Label\",\"w\":280,\"x\":320,\"y\":24,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Provincia Dipendenza\",\"disabled\":\"true\",\"h\":21,\"maxlength\":2,\"name\":\"PROVINCIADIP\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"OJGQABFSYN\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":24,\"x\":891,\"y\":20,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_XVRJYSKGWZ\",\"page\":1,\"spuid\":\"XVRJYSKGWZ\",\"tabindex\":201,\"type\":\"Label\",\"value\":\"Agenzia:\",\"w\":56,\"x\":211,\"y\":24,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_NRIPVBSJUQ\",\"page\":1,\"spuid\":\"NRIPVBSJUQ\",\"tabindex\":154,\"type\":\"Label\",\"value\":\"Tipo Gioco:\",\"w\":121,\"x\":16,\"y\":76,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_DJRHGZHJSR\",\"page\":1,\"spuid\":\"DJRHGZHJSR\",\"tabindex\":162,\"type\":\"Label\",\"value\":\"Tipo Piattaforma:\",\"w\":112,\"x\":392,\"y\":76,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VTHHSCUKRU\",\"page\":1,\"spuid\":\"VTHHSCUKRU\",\"tabindex\":197,\"type\":\"Label\",\"value\":\"ID BIGLIETTO:\",\"w\":136,\"x\":368,\"y\":52,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Operazione\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Dati Operazione\",\"h\":443,\"name\":\"Operazione\",\"page\":1,\"shrinkable\":true,\"spuid\":\"YORQOIOBQN\",\"stretch\":true,\"tabindex\":14,\"title_caption\":\"Dati Operazione\",\"titled\":true,\"type\":\"Box\",\"w\":1008,\"x\":8,\"y\":102,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"check\",\"comment\":\"Data Operazione\",\"h\":20,\"maxlength\":10,\"name\":\"DATAOPE\",\"page\":1,\"picker\":true,\"spuid\":\"YLIKFVPZJA\",\"tabindex\":27,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":88,\"x\":136,\"y\":109,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_HVHVAYPDJE\",\"page\":1,\"spuid\":\"HVHVAYPDJE\",\"tabindex\":108,\"type\":\"Label\",\"value\":\"Data di pagamento:\",\"w\":121,\"x\":16,\"y\":113,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_IKBMEWKQJW\",\"page\":1,\"spuid\":\"IKBMEWKQJW\",\"tabindex\":111,\"type\":\"Label\",\"value\":\"Data registrazione:\",\"w\":134,\"x\":792,\"y\":113,\"zone\":\"pag1_2_2\",\"zonepath\":\"[1,2,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Flag Wallet\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"FLAGWALLET\",\"page\":1,\"spuid\":\"DWJVPXTRND\",\"tabindex\":29,\"textlist\":\"No,Si\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"0"+"'"+","+"'"+"1"+"'"+"\",\"w\":104,\"x\":360,\"y\":137,\"zone\":\"pag1_2_3\",\"zonepath\":\"[1,2,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_GVTXMEILAZ\",\"page\":1,\"spuid\":\"GVTXMEILAZ\",\"tabindex\":110,\"type\":\"Label\",\"value\":\"Contanti:\",\"w\":121,\"x\":16,\"y\":141,\"zone\":\"pag1_2_3\",\"zonepath\":\"[1,2,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_AYZLWZINKY\",\"page\":1,\"spuid\":\"AYZLWZINKY\",\"tabindex\":194,\"type\":\"Label\",\"value\":\"Wallet:\",\"w\":80,\"x\":277,\"y\":141,\"zone\":\"pag1_2_3\",\"zonepath\":\"[1,2,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Causale\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":6,\"name\":\"CACODICE\",\"page\":1,\"spuid\":\"WJQFIEBCQV\",\"tabindex\":30,\"textlist\":\"Emissione Biglietti,Pagamento Biglietti\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,3\",\"w\":392,\"x\":136,\"y\":165,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Segno\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"SEGNO\",\"page\":1,\"spuid\":\"QNNLSEAZHF\",\"tabindex\":34,\"textlist\":\"Avere,Dare\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"A"+"'"+","+"'"+"D"+"'"+"\",\"w\":72,\"x\":704,\"y\":165,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Tipo Operazione Completa\",\"disabled\":\"true\",\"h\":20,\"maxlength\":2,\"name\":\"TIPOOPRAP\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"UJRFWINIVQ\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":24,\"x\":615,\"y\":137,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_KMCNAWQHKV\",\"page\":1,\"spuid\":\"KMCNAWQHKV\",\"tabindex\":112,\"type\":\"Label\",\"value\":\"Tipo Operazione:\",\"w\":104,\"x\":509,\"y\":141,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_THOGQCLBJA\",\"page\":1,\"spuid\":\"THOGQCLBJA\",\"tabindex\":115,\"type\":\"Label\",\"w\":361,\"x\":646,\"y\":141,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_YFGRJCQGZV\",\"page\":1,\"spuid\":\"YFGRJCQGZV\",\"tabindex\":152,\"type\":\"Label\",\"value\":\"Tipo Registrazione:\",\"w\":121,\"x\":16,\"y\":169,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_
\"zone\":\"pag1_3_14\",\"zonepath\":\"[1,3,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Numero Doc.\",\"h\":20,\"maxlength\":15,\"name\":\"NUMDOCUM\",\"page\":1,\"spuid\":\"HVEEMUJUUY\",\"tabindex\":76,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":128,\"x\":560,\"y\":836,\"zone\":\"pag1_3_14\",\"zonepath\":\"[1,3,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_YXFGMWZGNN\",\"page\":1,\"spuid\":\"YXFGMWZGNN\",\"tabindex\":135,\"type\":\"Label\",\"value\":\"Numero Doc.:\",\"w\":157,\"x\":400,\"y\":840,\"zone\":\"pag1_3_14\",\"zonepath\":\"[1,3,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_KCXIMANXKF\",\"page\":1,\"spuid\":\"KCXIMANXKF\",\"tabindex\":182,\"type\":\"Label\",\"value\":\"Tipo Documento:\",\"w\":117,\"x\":16,\"y\":840,\"zone\":\"pag1_3_14\",\"zonepath\":\"[1,3,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"check\",\"comment\":\"Rilascio Doc.\",\"h\":20,\"maxlength\":10,\"name\":\"DATARILASC\",\"page\":1,\"picker\":true,\"spuid\":\"XHWYGOPPUN\",\"tabindex\":77,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":86,\"x\":136,\"y\":861,\"zone\":\"pag1_3_15\",\"zonepath\":\"[1,3,15]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"check\",\"comment\":\"Data Fine Validit\\u00E0\",\"h\":20,\"maxlength\":10,\"name\":\"DATAVALI\",\"page\":1,\"picker\":true,\"spuid\":\"BNWXBPSIDG\",\"tabindex\":78,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":86,\"x\":344,\"y\":861,\"zone\":\"pag1_3_15\",\"zonepath\":\"[1,3,15]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_ZPQIEJPWJC\",\"page\":1,\"spuid\":\"ZPQIEJPWJC\",\"tabindex\":138,\"type\":\"Label\",\"value\":\"Autorit\\u00E0 di rilascio:\",\"w\":109,\"x\":448,\"y\":865,\"zone\":\"pag1_3_15\",\"zonepath\":\"[1,3,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_UYGHJPRVKH\",\"page\":1,\"spuid\":\"UYGHJPRVKH\",\"tabindex\":147,\"type\":\"Label\",\"value\":\"Data Fine Validit\\u00E0:\",\"w\":101,\"x\":240,\"y\":865,\"zone\":\"pag1_3_15\",\"zonepath\":\"[1,3,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Autorit\\u00E0 di rilascio\",\"disabled\":\"true\",\"h\":20,\"maxlength\":30,\"name\":\"AUTRILASC\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"TXKHUBKEOP\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":224,\"x\":560,\"y\":861,\"zone\":\"pag1_3_15\",\"zonepath\":\"[1,3,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_QUXAELRWLA\",\"page\":1,\"spuid\":\"QUXAELRWLA\",\"tabindex\":183,\"type\":\"Label\",\"value\":\"Rilascio Doc.:\",\"w\":117,\"x\":16,\"y\":865,\"zone\":\"pag1_3_15\",\"zonepath\":\"[1,3,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Autorit\\u00E0 Rilascio\",\"h\":20,\"maxlength\":2,\"name\":\"RDCODAUT\",\"page\":1,\"spuid\":\"YDUQCBNYFV\",\"tabindex\":79,\"type\":\"Combobox\",\"typevar\":\"character\",\"w\":176,\"x\":136,\"y\":886,\"zone\":\"pag1_3_16\",\"zonepath\":\"[1,3,16]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_RAVMCVENOT\",\"page\":1,\"spuid\":\"RAVMCVENOT\",\"tabindex\":184,\"type\":\"Label\",\"value\":\"Autorit\\u00E0 Rilascio:\",\"w\":117,\"x\":16,\"y\":890,\"zone\":\"pag1_3_16\",\"zonepath\":\"[1,3,16]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Luogo Autorit\\u00E0 Rilascio\",\"h\":20,\"maxlength\":20,\"name\":\"RDLOCAUT\",\"page\":1,\"spuid\":\"MPRHMTBSXX\",\"tabindex\":80,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":152,\"x\":560,\"y\":886,\"zone\":\"pag1_3_17\",\"zonepath\":\"[1,3,17]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_LSLCBQSYUK\",\"page\":1,\"spuid\":\"LSLCBQSYUK\",\"tabindex\":161,\"type\":\"Label\",\"value\":\"Luogo Autorit\\u00E0 Rilascio:\",\"w\":181,\"x\":376,\"y\":890,\"zone\":\"pag1_3_17\",\"zonepath\":\"[1,3,17]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Descrizione PEP\",\"h\":20,\"maxlength\":80,\"name\":\"PEPDESCRI\",\"page\":1,\"spuid\":\"DAEHHCRYAM\",\"tabindex\":81,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":648,\"x\":136,\"y\":911,\"zone\":\"pag1_3_18\",\"zonepath\":\"[1,3,18]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_ELBFOCJOYH\",\"page\":1,\"spuid\":\"ELBFOCJOYH\",\"tabindex\":176,\"type\":\"Label\",\"value\":\"Descrizione PEP:\",\"w\":117,\"x\":16,\"y\":915,\"zone\":\"pag1_3_18\",\"zonepath\":\"[1,3,18]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Ricerca su Soggetti Segnalati (Black List Interna)\",\"h\":24,\"keep_proportions\":\"bounded\",\"name\":\"image_BYFMKHYARA\",\"page\":1,\"spuid\":\"BYFMKHYARA\",\"src\":\"..\\/{\\\"Char\\\":\\\"60124\\\",\\\"Color\\\":\\\"#000000\\\",\\\"FontName\\\":\\\"ZucchettiDevelopement\\\",\\\"FontWeight\\\":\\\"\\\",\\\"Size\\\":\\\"25\\\"}\",\"tabindex\":189,\"type\":\"Image\",\"w\":32,\"x\":16,\"y\":555,\"zone\":\"pag1_3_19\",\"zonepath\":\"[1,3,19]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Professione\",\"h\":20,\"maxlength\":40,\"name\":\"PROFESSIONE\",\"page\":1,\"spuid\":\"FRTNMCIRCW\",\"tabindex\":191,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":136,\"y\":936,\"zone\":\"pag1_3_20\",\"zonepath\":\"[1,3,20]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_WRCLDQWIXL\",\"page\":1,\"spuid\":\"WRCLDQWIXL\",\"tabindex\":192,\"type\":\"Label\",\"value\":\"Professione:\",\"w\":117,\"x\":16,\"y\":940,\"zone\":\"pag1_3_20\",\"zonepath\":\"[1,3,20]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_cgo_oper_multiBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_cgo_oper_multi?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_cgo_oper_multi_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_cgo_oper_multi_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_FLAGFRAZ=a[75];");
      status.out.println("w_COLLEG=a[76];");
      status.out.println("w_CODCAB=a[77];");
      status.out.println("w_ATTIV=a[78];");
      status.out.println("w_AUTRILASC=a[79];");
      status.out.println("w_IDDOCALL=a[80];");
      status.out.println("w_FLGDOCALL=a[81];");
      status.out.println("w_PROFESSIONE=a[82];");
      status.out.println("w_MEZPAGAM=a[83];");
      ...
      status.out.println("w_xIMPMINTOT=a[139];");
      status.out.println("w_gAzienda=a[140];");
      status.out.println("w_xCGOFLGPROF=a[141];");
      status.out.println("w_TELDIP=a[142];");
      status.out.println("w_CODLOC=a[143];");
      status.out.println("w_ANA_FOUND=a[144];");
      status.out.println("op_codazi=a[145];");
      status.out.println("m_cWv_ardt_cgo_oper_multi=a[146];");
      status.out.println("if (Gt(a.length,147)) {");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_cgo_oper_multi_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(1036,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void RaiseLoadedEntity(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_cgo_oper_multiBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_cgo_oper_multiBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_cgo_oper_multiBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_cgo_oper_multiBO BO,ServletStatus status) throws IOException {
  ...
  void IncludeJS(ServletStatus status) {
    ...
    status.out.println(
     ...
     "</script>" +
    "");
    l_cJsUid = SPPrxycizer.proxycizedPath("arrt_cgo_read_codfisc_multi.js");
    status.out.println(
     "<script src='../"+l_cJsUid+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+arrt_cgo_setpagatore_m.m_cEntityUid+"/arrt_cgo_setpagatore_m.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      arrt_cgo_setpagatore_m.GetCallableNames(callable);
      ...
      CPLib.AddToSet(callable,"arrt_cgo_read_codfisc_multi");
      CPLib.AddToSet(callable,"ardt_cgo_oper_multi");
    } else {
  ...
  public String GetCheckVars(ServletStatus status,armt_cgo_oper_multiBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_cgo_oper_multiBO BO) {
    boolean result = true;
