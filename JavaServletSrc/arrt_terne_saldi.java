...
public class arrt_terne_saldi extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*f7ec6025*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_stpoff;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_stpoff = source.Has("w_stpoff");
      if (m_bpassedw_stpoff) {
        w_stpoff = source.GetParameter("w_stpoff",source.GetParameter("stpoff",""));
      } else {
        if (source.IsForwarded()) {
          w_stpoff = source.GetCaller().GetString("stpoff","C",1,0);
          m_bpassedw_stpoff = true;
        } else {
          w_stpoff = source.GetParameter("stpoff",source.GetParameter("w_stpoff",""));
        }
      }
    }
  }
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_stpoff) ||  ! (status.m_bpassedw_a_data) ||  ! (status.m_bpassedw_anno);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_stpoff)) {
      status.out.println("l_cV=WtA(l_oWnd('w_stpoff',"+SPLib.ToJSValue(status.w_stpoff,"C",1,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_stpoff,"C",1,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_stpoff',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_terne_saldiR Rcalled;
    Rcalled = new arrt_terne_saldiR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_terne_saldiR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_terne_saldiR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_terne_saldi"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_stpoff="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_stpoff),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_stpoff,null)) {");
        status.out.println("window[f].w_stpoff="+SPLib.ToJSValue(status.w_stpoff,"C",0,0)+";");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_terne_saldi");
      if ( ! (CPLib.IsNull(l))) {
