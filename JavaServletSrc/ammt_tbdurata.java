...
public class ammt_tbdurata extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*fb559216*!";
  static public class ServletStatus extends SPMasterServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "tbdurata";
    }
    public String w_CODICE;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_CODICE,context,"tbdurata","CODICE","C",5,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_tbdurata;
  ...
  public String getEntityTitle() {
    return "Frequenza Operazione";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "ammt_tbdurata";
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"tbdurata"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ammt_tbdurataBO BO) {
  ...
  void WriteStyles(ammt_tbdurataBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ammt_tbdurata.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ammt_tbdurataBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ammt_tbdurata',false);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ammt_tbdurata",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ammt_tbdurataBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ammt_tbdurataBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b0(ammt_tbdurataBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(ammt_tbdurataBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ammt_tbdurataBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ammt_tbdurataBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(ammt_tbdurataBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ammt_tbdurata"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ammt_tbdurata","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ammt_tbdurata",BO.GetCurrentState());
  ...
  void WriteMainFormScript(ammt_tbdurataBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  ammt_tbdurataBO getBO(ServletStatus status) {
    ammt_tbdurataBO BO = new ammt_tbdurataBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(ammt_tbdurataBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("tbdurata",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ammt_tbdurataBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ammt_tbdurataBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ammt_tbdurataBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_tbdurata;
    String l_cPhName = BO.m_cPhName_tbdurata;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ammt_tbdurataBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_tbdurata+".CODICE",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("tbdurata");
    String l_cPhName = CPSql.ManipulateTablePhName("tbdurata",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ammt_tbdurataBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_tbdurata,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_tbdurata,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,ammt_tbdurataBO BO) {
  ...
  void Edit(ServletStatus status,ammt_tbdurataBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ammt_tbdurataBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ammt_tbdurataBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ammt_tbdurataBO BO) {
  ...
  void EditLoad(ServletStatus status,ammt_tbdurataBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ammt_tbdurataBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ammt_tbdurataBO BO,SPParameterSource source) {
    BO.Load(status.w_CODICE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ammt_tbdurataBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ammt_tbdurataBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ammt_tbdurataBO BO) {
    ...
    item.put("altInterfaceFor","ammt_tbdurata");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_MZUNIMCJQC\",\"page\":1,\"spuid\":\"MZUNIMCJQC\",\"tabindex\":6,\"type\":\"Label\",\"value\":\"Indice Rischio:\",\"w\":95,\"x\":568,\"y\":18,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(ammt_tbdurataBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ammt_tbdurata?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ammt_tbdurata_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ammt_tbdurata_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ammt_tbdurata_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(ammt_tbdurataBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ammt_tbdurataBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ammt_tbdurataBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ammt_tbdurataBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ammt_tbdurataBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ammt_tbdurataBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ammt_tbdurataBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ammt_tbdurataBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ammt_tbdurataBO BO) {
    boolean result = true;
