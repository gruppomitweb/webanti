...
l_oWv.setValue('m_cParameterSequence',AsAppletValue('w_nomefile'));
SetLocationHref(null,'arrt_trasf_nascita'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cMode=hyperlink','elaborazione')
}
...
w_Messaggi.raiseEvent('arpg_trasf_nascita_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_trasf_nascita_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_trasf_nascita_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_trasf_nascita',m_cSelectedPage);
...
l_oWv.setValue('filexls',WtA(w_filexls,'C'));
l_oWv.setValue('gUrlApp',WtA(w_gUrlApp,'C'));
l_oWv.setValue('tiporep',WtA(w_tiporep,'N'));
_FillChildren(l_oWv);
}
_FillChildren.n=[];
FillWv.n=["nomefile","impzip","errchk","gAzienda","gPathApp","fscarti","gDescAzi","filexls","gUrlApp","tiporep"];
...
if (Empty(w_tiporep) && Ne(opener.w_tiporep,null)) {
TransferBetweenWindows(opener,'w_tiporep','w_tiporep');
}
}
}
m_bCalculating=true;
