...
public class ardt_xntestitBL extends ardt_xntestitWV implements CPLib.QueryFilterFromApplication {
  ...
  public static final String i_EntityName = "ardt_xntestit";
  ...
  public CPPhTableWrInfo m_oWrInfo_xntestit;
  // Variabile che contiene il nome virtuale della tabella
  public String m_cVirtName_xntestit;
  ...
  public static String[] m_EntityTables = {"xntestit"};
  // codice secondo sintassi delle business rules per produrre i dati da usare in un ideale documento che li rappresenti
  public static final String i_StateStructure = "{'CODCLI':CODCLI,'m_Detail':{'CODINTER':CODINTER,'DATAINI':DATAINI,'DATAFINE':DATAFINE,'PRGIMPTITE':PRGIMPTITE,'DESCINTER':DESCINTER,'IDBASE':IDBASE"+FoundlingChildrenList("ardt_xntestit",true,true)+"}"+FoundlingChildrenList("ardt_xntestit",true,false)+"}";
  public static final String i_CompleteStateStructure = "{'tipreg':tipreg,'CODCLI':CODCLI,'gAzienda':gAzienda,'tipooprap':tipooprap,'m_Detail':{'CODINTER':CODINTER,'DATAINI':DATAINI,'DATAFINE':DATAFINE,'TIPOPERS':TIPOPERS,'xragsoci':xragsoci,'tiporegr':tiporegr,'DOMICILIO':DOMICILIO,'DESCCIT':DESCCIT,'PROVINCIA':PROVINCIA,'CAP':CAP,'PRGIMPTITE':PRGIMPTITE,'STATOREG':STATOREG,'IDEREG':IDEREG,'TIPORAP':TIPORAP,'NUMPROG2':NUMPROG2,'NUMPROG1':NUMPROG1,'DESCINTER':DESCINTER,'IDBASE':IDBASE,'NOAGE':NOAGE"+FoundlingChildrenList("ardt_xntestit",true,true)+"}"+FoundlingChildrenList("ardt_xntestit",true,false)+"}";
  public static final String i_ItemSequence = "CODCLI,m_Detail[ CODINTER DATAINI DATAFINE PRGIMPTITE DESCINTER IDBASE"+FoundlingChildrenList("ardt_xntestit",false,true)+"]"+FoundlingChildrenList("ardt_xntestit",false,false)+"";
  public static final com.zucchetti.sitepainter.EntityItem[] i_EditableEntityItems = com.zucchetti.sitepainter.EntityItem.buildEditableItemsForEntity(GetLinkDef(),FoundlingChildrenList("ardt_xntestit"),new String[][]{new String[]{"CODCLI","","C","16","0","campo","key1","false","false"},new String[]{"CODINTER","Intestatario (Non Intermediario)","C","16","0","campo","key1","true","false"},new String[]{"DATAINI","Data Inizio","D","8","0","campo","nokey","true","false"},new String[]{"DATAFINE","Data Fine","D","8","0","campo","nokey","true","false"},new String[]{"PRGIMPTITE","","N","15","0","campo","key3","true","false"},new String[]{"DESCINTER","","C","70","0","campo","key2","true","false"},new String[]{"IDBASE","idbase","C","10","0","campo","nokey","true","false"}});
  public static final String i_TrsKeys = ",CODINTER";
  ...
  public ardt_xntestitBL (CPContext p_ContextObject) {
    ...
    m_cPhName_xntestit = p_ContextObject.GetPhName("xntestit",p_ContextObject.GetCompany(),false);
    m_cServer_xntestit = p_ContextObject.GetServer("xntestit");
    m_oWrInfo_xntestit = CPPhTableWrInfo.GetCPPhTableWrInfo(p_ContextObject,"xntestit",p_ContextObject.GetCompany());
    m_cVirtName_xntestit = CPSql.ManipulateTablePhName("xntestit",m_cServer_xntestit);
    ...
    m_MNTs = CPLib.MNTChilds("ardt_xntestit",p_ContextObject);
  }
  public void Blank() {
    ...
    w_tipooprap = "";
    w_tipooprap = Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP");
    Link_XTDHYWJPZL();
    w_tiporegr = 0;
    ...
    w_NUMPROG2 = "";
    w_NUMPROG2 = ( ! (CPLib.Empty(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"))) && (CPLib.eq(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"51") || CPLib.eq(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"53")) && CPLib.Empty(w_NUMPROG2)?Utilities.Make(m_Ctx).GetCallerString("NUMPROG"):w_NUMPROG2);
    w_NUMPROG1 = "";
    w_NUMPROG1 = ( ! (CPLib.Empty(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"))) && (CPLib.eq(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"50") || CPLib.eq(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"52") || CPLib.eq(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"58")) && CPLib.Empty(w_NUMPROG1)?Utilities.Make(m_Ctx).GetCallerString("NUMPROG"):w_NUMPROG1);
    w_NOAGE = "N";
  ...
  protected boolean Link_XTDHYWJPZL(String p_cType) {
    ...
    if ( ! (CPLib.Empty(w_CODINTER))) {
      ...
      try {
        ...
        if (CPLib.ne(p_cType,"Load")) {
          if (l_bResult) {
            l_bResult = arfn_chksoggR.Make(m_Ctx,this).Run(w_CODINTER,"6");
            if ( ! (l_bResult)) {
  ...
  public boolean Set_PRGIMPTITE(double p_workVariableValue) {
    m_nLastError = 0;
    m_cLastMsgError = "";
    boolean l_bResult;
    l_bResult = true;
    ...
    boolean l_bTmpRes;
    SetUpdatedRow();
    w_PRGIMPTITE = p_workVariableValue;
    // Esecuzione dei calcoli
    Calculate();
    SaveDependsOn();
    return l_bResult;
  ...
  public boolean getWritable_DATAINI() {
    return CPLib.ne(CPLib.At(w_tipooprap,"|50|52|58"),0);
  }
  public boolean getWritable_DATAFINE() {
    return CPLib.ne(CPLib.At(w_tipooprap,"|51|53"),0);
  }
  public boolean getVisible_CODCLI() {
    return true;
  }
  public boolean getVisible_PRGIMPTITE() {
    return true;
  }
  ...
  public boolean Calculate() {
    ...
    m_bCalculating = true;
    w_tipreg = Utilities.Make(m_Ctx).GetCallerString("tipreg");
    if (CPLib.ne(o_DATAINI,w_DATAINI) || CPLib.ne(o_CODINTER,w_CODINTER)) {
      Calculation_GVAXMWDMDH();
    }
    if (CPLib.ne(o_DATAFINE,w_DATAFINE) || CPLib.ne(o_CODINTER,w_CODINTER)) {
      Calculation_HQRGXFMPSR();
    }
    if (CPLib.ne(o_CODINTER,w_CODINTER)) {
      Calculation_TDLAKFIXPS();
    }
    w_tipooprap = Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP");
    w_tiporegr = Utilities.Make(m_Ctx).GetCallerNumber("tipregn");
    w_IDEREG = w_TIPOPERS+w_CODINTER+"0";
  ...
  void Calculation_SFWVXVZTVV() {
    w_NUMPROG1 = ( ! (CPLib.Empty(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"))) && CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|50|52|58"),0) && CPLib.Empty(w_NUMPROG1)?Utilities.Make(m_Ctx).GetCallerString("NUMPROG"):w_NUMPROG1);
    w_NUMPROG2 = ( ! (CPLib.Empty(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"))) && CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|51|53"),0) && CPLib.Empty(w_NUMPROG2)?Utilities.Make(m_Ctx).GetCallerString("NUMPROG"):w_NUMPROG2);
    w_PRGIMPTITE = Utilities.Make(m_Ctx).GetAutonumber("PRGIMPTITE\\'"+CPLib.LRTrim(w_gAzienda)+"'",0,15);
  }
  ...
  public final static String[] m_KeyColumns = {"CODCLI"};
  public static String QueryFilter(CPContext m_Ctx) {
    String l_cFilter;
    l_cFilter = CPLib.ApplicationQueryFilter("ardt_xntestit","xntestit",m_KeyColumns,m_Ctx);
    return l_cFilter;
  }
  ...
  public void SetFromContext(String s,java.util.Properties p) {
    ...
    if ( ! (CPLib.Empty(s))) {
      ...
      Properties p_PRGIMPTITE;
      p_PRGIMPTITE = CPLib.GetProperties(trs_p.getProperty("PRGIMPTITE"),new Properties());
      ...
      Properties p_DESCINTER;
      p_DESCINTER = CPLib.GetProperties(trs_p.getProperty("DESCINTER"),new Properties());
      Properties p_IDBASE;
      ...
      Properties p_opk_CODINTER;
      p_opk_CODINTER = CPLib.GetProperties(trs_p.getProperty("opk_CODINTER"),new Properties());
      ...
      while (CPLib.le(i,rows)) {
        ...
        w_PRGIMPTITE = 0;
        w_PRGIMPTITE = CPLib.GetProperty(p_PRGIMPTITE,"",w_PRGIMPTITE,i);
        w_STATOREG = "";
        ...
        w_DESCINTER = "";
        w_DESCINTER = CPLib.GetProperty(p_DESCINTER,"",w_DESCINTER,i);
        w_IDBASE = "";
        ...
        opk_CODINTER = CPLib.GetProperty(p_opk_CODINTER,"","",i);
  ...
  void SetUpdated(String p_cName) {
    ...
    if (CPLib.eq(p_cName,"PRGIMPTITE")) {
      SetUpdatedRow();
      return;
    }
    if (CPLib.eq(p_cName,"DESCINTER")) {
      SetUpdatedRow();
      return;
    }
  ...
  static void GetLinkDef0(Map<String,List<Map<String,String[]>>> tables) {
    ...
    map.put("rd_var.comment",zeroSplit.split("Intestatario (Non Intermediario)\u0000Ragione Sociale\u0000Indirizzo\u0000Città\u0000Provincia\u0000CAP",-1));
  ...
  public static Map<String,List<Map<String,String[]>>> GetLinkDef() {
    ...
    List<Map<String,String[]>> items;
    GetLinkDef0(tables);
    for(String child : CPLib.LoadableMNTs("ardt_xntestit",false)) {
      map = new HashMap<String,String[]>();
      ...
      String[][] definitions = CPMNTChild.GetM_Fkd("ardt_xntestit",child);
    ...
    for(String child : CPLib.LoadableMNTs("ardt_xntestit",true)) {
      map = new HashMap<String,String[]>();
      items = tables.get(com.zucchetti.sitepainter.Library.getEntityTables(child)[0]);
      if (CPLib.IsNull(items)) {
        items = new ArrayList<Map<String,String[]>>();
  ...
  public void Initialize() {
    ...
    m_MNTs = CPLib.MNTChilds("ardt_xntestit",m_Ctx);
  ...
  public boolean Check() {
    ...
    ResetErrorMessage();
    l_bResult = true;
    if ( ! (CPLib.Empty(w_CODINTER)) &&  ! (CPLib.Empty(w_IDBASE))) {
  ...
  public String[] GetStateSecurityKeyRoles(String p_cState) {
    ...
    if (CPLib.eq(p_cState,CPSecurity.UNKNOWN_STATE)) {
      l_Keys = new String[]{"ardt_xntestit"};
    }
    if (CPLib.eq(l_Keys.length,0)) {
      l_Keys = new String[]{"ardt_xntestit\\"+p_cState};
    }
    return l_Keys;
  }
  ...
  public void GetContext(boolean p_bGetChildren,java.util.Properties p) {
    ...
    Properties p_opk_CODINTER;
    p_opk_CODINTER = new Properties();
    Properties p_m_nRowStatus;
    p_m_nRowStatus = new Properties();
    Properties p_m_cOldRowCPCCCHK;
    p_m_cOldRowCPCCCHK = new Properties();
    CPLib.PutProperty(p,"CODCLI",w_CODCLI,0);
    ...
    while (CPLib.lt(i,Rows())) {
      ...
      CPLib.PutProperty(p_PRGIMPTITE,"",w_PRGIMPTITE,trsrow);
      ...
      CPLib.PutProperty(p_DESCINTER,"",w_DESCINTER,trsrow);
      ...
      CPLib.PutProperty(p_opk_CODINTER,"",opk_CODINTER,trsrow);
    ...
    trs_p.setProperty("PRGIMPTITE",CPLib.ToProperties(p_PRGIMPTITE));
    ...
    trs_p.setProperty("DESCINTER",CPLib.ToProperties(p_DESCINTER));
    ...
    trs_p.setProperty("opk_CODINTER",CPLib.ToProperties(p_opk_CODINTER));
  ...
  public static String PrimaryKey(Caller p_Caller) {
    return "xntestit"+"\\"+CPLib.ToCPStr(p_Caller.GetString("CODCLI","C",16,0));
  }
  public String RowPrimaryKey() {
    return "xntestit"+"\\"+CPLib.ToCPStr(w_CODINTER);
  }
  ...
  protected void TrsToWork() {
    ...
    w_PRGIMPTITE= m_oTrs.get(m_nCurrentRow).w_PRGIMPTITE;
    ...
    w_DESCINTER= m_oTrs.get(m_nCurrentRow).w_DESCINTER;
    ...
    opk_CODINTER= m_oTrs.get(m_nCurrentRow).opk_CODINTER;
  ...
  void WorkToTrs() {
    ...
    m_oTrs.get(m_nCurrentRow).w_PRGIMPTITE=w_PRGIMPTITE;
    ...
    m_oTrs.get(m_nCurrentRow).w_DESCINTER=w_DESCINTER;
    ...
    m_oTrs.get(m_nCurrentRow).opk_CODINTER=opk_CODINTER;
  ...
  public void AddTrsDeletedRow() {
    ...
    m_oDeletedRows.get(l_nRows).w_PRGIMPTITE=w_PRGIMPTITE;
    ...
    m_oDeletedRows.get(l_nRows).w_DESCINTER=w_DESCINTER;
    ...
    m_oDeletedRows.get(l_nRows).opk_CODINTER=opk_CODINTER;
  ...
  boolean FullRow() {
    return  ! (CPLib.Empty(w_CODINTER)) &&  ! (CPLib.Empty(w_IDBASE));
  }
  void SetModified() {
  ...
  void SetDeletedRow(double p_nRowNumberToSet) {
    ...
    w_PRGIMPTITE= m_oDeletedRows.get(l_RowNumberToSet).w_PRGIMPTITE;
    ...
    w_DESCINTER= m_oDeletedRows.get(l_RowNumberToSet).w_DESCINTER;
    ...
    opk_CODINTER= m_oDeletedRows.get(l_RowNumberToSet).opk_CODINTER;
