...
try {
_tracker.askwrn=true;
l_bWarn=WindowConfirm(AlertErrorMessage('11174735463'));
} finally {
_tracker.askwrn=false;
}
if ( ! (l_bWarn)) {
...
function Calculation_TRTXECEUEX() {
arrt_tit_load_aui([["pObj",this],["pOper",'L']])
}
...
if (l_bResult &&  ! (Empty(w_TIPOOPRAP) && l_bEmp)) {
l_bResult=arfn_chktiporeg(w_DATAOPE,w_TIPOOPRAP);
if ( ! (l_bResult)) {
Link_SWRTSKXMDH_Blank();
}
}
...
parent.ZtVWeb.raiseEventToEvalParms('armt_titolabo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_titolabo_dataNotSent',{'action':formaction});
}
}
}
}
...
l_bResult=false;
w_DATAREG=HtW('','D');
} else if (( ! (_ChkObl('SWRTSKXMDH',1))) || ( ! (Empty(w_TIPOOPRAP)) &&  ! (arfn_chktiporeg(w_DATAOPE,w_TIPOOPRAP)))) {
_SignErr('SWRTSKXMDH','11525797640');
l_bResult=false;
w_TIPOOPRAP=HtW('','C');
...
_FillChildren.n=[];
FillWv.n=["CODINTER","CODDIPE","IDBASE","TIPOINTER","TIPOINF","IDEREG","FLAGRAP","OPRAP","OPERATORE","AUTOM","DESCCIT","PROVINCIA","CODCAB","FLAGRAP2","FLAGLIRE","PRGIMPTIT","CONNESCLI","DATAOPE","DATAREG","CDATOPE","TIPOOPRAP","CONNESOPER","TIPOLEG","RAPPORTO","STATOREG","NUMPROG","DESCINTER","COLLEG","DATARETT","flgpf","flgpg","gIntemediario","xdescrap","xDescDipe","datpaseuro","gChkDate","xDesReg","xRagSoc","sEdit","errmsg","Aggiorna","lancio","VALUTA","gSblocco","tipreg","tipregn","dataoggi","gStatus","gDataVaria","CITINT","CITDIP","PROVDIP","CABDIP","PROVINT","CABINT","xRagSocCli","filtro","SESSO"];
