...
public class arpg_info_pre_prn_scheda_adeguata_ver_clientela extends SPDialogServlet implements SPInvokable {
  ...
  void WriteControls_1_h_0(arpg_info_pre_prn_scheda_adeguata_ver_clientelaBO BO,ServletStatus status) {
    ...
    status.out.println(
     ...
     "<label class='SPLabel' for='GMCWGKWJAP_1'>"+status.context.Translate("Cliente già acquisito")+"</label>" +
     "</div>" +
     "</div>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    status.out.println(
     ...
     "<label id='YRBRMQSERG'>" +
     ""+SPLib.ToHTMLRepresentation("Indicare il legale Rappresentante della società "+BO.w_RAGSOCM+":")+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
