...
if(c=Ctrl('HXBJUMEZIA')) c.value = WtH(w_aCodDip,'C',16,0,'')
...
if(c=Ctrl('IZTCWGWJFN')) selectRadio(c,w_Tipop,'C')
if(c=Ctrl('XEXVRCMZDB')) selectRadio(c,w_stato,'C')
...
if(c=Ctrl('JCIARNNFGA')) selectRadio(c,w_OrderBy,'C')
...
var h;
SetDisplay(Ctrl('GBFLGJEXQC'),true);
HideUI();
}
...
function IZTCWGWJFN_Valid(e) {
e=(Ne(e,null)?e:window.event);
return Set_Tipop(HtW(getRadioValue(Ctrl('IZTCWGWJFN')),'C'),null,e);
}
function XEXVRCMZDB_Valid(e) {
e=(Ne(e,null)?e:window.event);
return Set_stato(HtW(getRadioValue(Ctrl('XEXVRCMZDB')),'C'),null,e);
}
...
SetLocationHref(null,'arrt_stpfraz'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
function href_PTXYMSWBZO() {
SetLocationHref(window.location,ToResource('jsp/start_page_logo_portlet.jsp'+'')+'')
}
function JCIARNNFGA_Valid(e) {
e=(Ne(e,null)?e:window.event);
return Set_OrderBy(HtW(getRadioValue(Ctrl('JCIARNNFGA')),'C'),null,e);
}
function href_GBFLGJEXQC() {
SetLocationHref(null,'arrt_stpfraz_xls'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
_ResetTracker();
...
c=Ctrl('IZTCWGWJFN');
if ( ! (Empty(c))) {
c[0].onclick=IZTCWGWJFN_Valid;
c[1].onclick=IZTCWGWJFN_Valid;
c[2].onclick=IZTCWGWJFN_Valid;
}
c=Ctrl('XEXVRCMZDB');
if ( ! (Empty(c))) {
c[0].onclick=XEXVRCMZDB_Valid;
c[1].onclick=XEXVRCMZDB_Valid;
c[2].onclick=XEXVRCMZDB_Valid;
c[3].onclick=XEXVRCMZDB_Valid;
}
c=Ctrl('LGLKGKWNEO');
if ( ! (Empty(c))) {
...
c=Ctrl('JCIARNNFGA');
if ( ! (Empty(c))) {
c[0].onclick=JCIARNNFGA_Valid;
c[1].onclick=JCIARNNFGA_Valid;
}
}
}
...
FocusFirstComponent.otherwise={"page_1":["UQFVYWSNYY"]};
function Help() {
windowOpenForeground('../doc/arpg_stpfraz_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_stpfraz',m_cSelectedPage);
...
var ctl = _GetCtl(e,'IZTCWGWJFN');
if (_tracker.goon(ctl,ctrlValue)) {
w_Tipop=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('IZTCWGWJFN',0);
DoUpdate(l_bResult);
} else {
ctl.value=WtH('','C',1,0,'');
_ResetTracker();
...
var ctl = _GetCtl(e,'XEXVRCMZDB');
if (_tracker.goon(ctl,ctrlValue)) {
w_stato=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('XEXVRCMZDB',0);
DoUpdate(l_bResult);
} else {
ctl.value=WtH('','C',1,0,'');
_ResetTracker();
...
var ctl = _GetCtl(e,'JCIARNNFGA');
if (_tracker.goon(ctl,ctrlValue)) {
w_OrderBy=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('JCIARNNFGA',0);
DoUpdate(l_bResult);
} else {
...
var l_Appl = LinkApplet();
l_Appl.SetStringKey('CODDIP',w_aCodDip,16,0);
l_Appl.SetFields('CODDIP,DESCRIZ');
l_Appl.SetTypes('C,C');
l_Appl.LinkTable('anadip');
...
l_bResult=Ne(l_nRec,0);
if (Eq(l_nRec,1)) {
w_aCodDip=l_Appl.GetStringValue('CODDIP',16,0);
...
l_bResult=false;
w_aCodCli=HtW('','C');
} else if ( ! (_ChkObl('IZTCWGWJFN',1))) {
_SignErr('IZTCWGWJFN');
m_cLastWorkVarErrorMsg='862812006';
l_bResult=false;
w_Tipop=HtW('','C');
} else if ( ! (_ChkObl('XEXVRCMZDB',1))) {
_SignErr('XEXVRCMZDB');
m_cLastWorkVarErrorMsg='1982082983';
l_bResult=false;
w_stato=HtW('','C');
...
_FillChildren.n=[];
FillWv.n=["daCodDip","aCodDip","DaDatOpe","ADatOpe","daCodCli","aCodCli","Tipop","stato","tipstp","RAGSOC1","RAGSOC2","c_dadata","c_adata","OrderBy","DESCRIZ1","DESCRIZ2"];
...
if (Eq(w_Tipop,'T') && Ne(opener.w_Tipop,null)) {
TransferBetweenWindows(opener,'w_Tipop',null,'Set_Tipop');
}
if (Eq(w_stato,'T') && Ne(opener.w_stato,null)) {
...
if (start && Eq(At(',Tipop,',pseq),0)) {
w_Tipop='T';
}
if (start && Eq(At(',stato,',pseq),0)) {
w_stato='T';
}
...
_Obli('Tipop',0,'IZTCWGWJFN',false,"862812006")
_Obli('stato',0,'XEXVRCMZDB',false,"1982082983")
_Obli('tipstp',0,'LGLKGKWNEO',false)
_Obli('OrderBy',0,'JCIARNNFGA',false)
