...
// * --- Fine Area Manuale
public class armt_tbcondottaBO extends armt_tbcondottaBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbcondottaBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbcondotta");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbcondotta,m_cPhName_tbcondotta,m_cVirtName_tbcondotta,m_cVirtName_tbcondotta+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",5,0),m_cServer_tbcondotta,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcondotta")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbcondotta);
      l_cSql = "insert into "+m_oWrInfo_tbcondotta.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+"RISCHIO,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbcondotta.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcondotta")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_tbcondotta.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcondotta")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbcondotta,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbcondotta);
      l_cSql = "Update "+m_oWrInfo_tbcondotta.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",5,0),m_cServer_tbcondotta,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbcondotta.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcondotta")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbcondotta.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",5,0),m_cServer_tbcondotta,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbcondotta.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcondotta")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbcondotta,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
