...
l_oWv.setValue('m_cParameterSequence',AsAppletValue('w_DaDatOpe,w_ADatOpe,w_anno'));
SetLocationHref(null,'arrt_check_opex'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','elaborazione')
}
_ResetTracker();
...
w_Messaggi.raiseEvent('arpg_check_opex_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_check_opex_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_check_opex_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_check_opex',m_cSelectedPage);
