...
function DisplayErrorMessage_ardt_storinformazioni() {
if ( ! (ChildToLoad(FrameRef('ardt_storinformazioni'))) && Ne(typeof(FrameRef('ardt_storinformazioni').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_storinformazioni').DisplayErrorMessage();
}
}
function DisplayErrorMessage_ardt_storsoginfo() {
if ( ! (ChildToLoad(FrameRef('ardt_storsoginfo'))) && Ne(typeof(FrameRef('ardt_storsoginfo').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_storsoginfo').DisplayErrorMessage();
}
}
...
l_bEnabled= ! ( ! (Empty(w_DATAOPE)) && Lt(w_DATAOPE,CharToDate('01/06/2010'))) || IsDisabledByStateDriver('CONNESOPER');
SetDisabled('SLTZSUMACW',l_bEnabled);
SetDisabled(Ctrl('SLTZSUMACW_ZOOM'),l_bEnabled);
l_bEnabled= ! ( ! (Empty(w_DATAOPE)) && Lt(w_DATAOPE,CharToDate('01/06/2010'))) || IsDisabledByStateDriver('TIPOLEG');
SetDisabled('MLPGOPNAKN',l_bEnabled);
...
if (IsDisabledByStateDriver('box_HDCVKCQUDV')) {
DisableInputsInContainer(Ctrl('HDCVKCQUDV'),true);
}
if (IsDisabledByStateDriver('box_TALHUUYCXX')) {
DisableInputsInContainer(Ctrl('TALHUUYCXX'),true);
}
...
if ( ! (ChildToLoad(FrameRef('ardt_storinformazioni'))) && Ne(typeof(FrameRef('ardt_storinformazioni').DoUpdate),'undefined')) {
CopyWorkVar(FrameRef('ardt_storinformazioni'),['TIPORAP'],['FLAGRAP'],true)
}
...
if (bResult) {
bResult=arfn_chkdatreg(w_DATAOPE,w_DATAREG,w_gChkDate,w_gStatus);
if ( ! (bResult)) {
...
parent.ZtVWeb.raiseEventToEvalParms('armt_storinformazioni_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_storinformazioni_dataNotSent',{'action':formaction});
}
}
}
}
CheckChild.child=[{'prg':"ardt_storinformazioni",'BOScId':function(){ return FrameRef('ardt_storinformazioni')},'repeated':0},{'prg':"ardt_storsoginfo",'BOScId':function(){ return FrameRef('ardt_storsoginfo')},'repeated':0}];
function Check() {
var l_bResult = true;
...
l_bResult=false;
w_DATAOPE=HtW('','D');
} else if ((Ne(w_gChkDate,'U') &&  ! (Empty(w_gChkDate))) && ( ! (_ChkObl('OEIKQOZMYJ',1)))) {
_SignErr('OEIKQOZMYJ');
...
l_bResult=false;
w_TIPOOPRAP=HtW('','C');
} else if (( ! (Empty(w_DATAOPE)) && Lt(w_DATAOPE,CharToDate('01/06/2010'))) && ( ! (_ChkObl('SLTZSUMACW',1)))) {
_SignErr('SLTZSUMACW');
m_cLastWorkVarErrorMsg='11755647652';
l_bResult=false;
w_CONNESOPER=HtW('','C');
} else if (( ! (Empty(w_DATAOPE)) && Lt(w_DATAOPE,CharToDate('01/06/2010'))) && ( ! (_ChkObl('MLPGOPNAKN',1)))) {
_SignErr('MLPGOPNAKN');
...
if(w_DATAOPE>CharToDate('01/06/2010') && (!(Empty(w_CONNESOPER)) || !(Empty(w_TIPOLEG)))) {
   alert('La data operazione \xe8 maggiore del 01/06/2010 e quindi non \xe8 pi\xf9 necessario inserire la Pers. Fisica Agente x conto')
}
// * --- Fine Area Manuale
return l_bResult;
}
...
_FillChildren.n=['ardt_storinformazioni','ardt_storsoginfo'];
FillWv.n=["CODINTER","CODDIPE","RAPPORTO","DATAOPE","DATAREG","TIPOOPRAP","CONNESOPER","TIPOLEG","DATARETT","IDBASE","TIPOINTER","TIPOINF","IDEREG","FLAGRAP","OPRAP","STATOREG","OPERAZMOD","OPERATORE","AUTOM","DESCCIT","PROVINCIA","CODCAB","NUMPROG","FLAGRAP2","FLAGLIRE","PRGIMPINF","CDATOPE","gIntemediario","xdescrap","xDescDipe","datpaseuro","gChkDate","xDesReg","xRagSoc","sEdit","errmsg","Aggiorna","lancio","VALUTA","gSblocco","tipreg","tipregn","dataoggi","gStatus","gDataVaria","CITINT","CITDIP","PROVDIP","CABDIP","PROVINT","CABINT"];
function SetModified() {
m_bUpdated=true;
}
function SetChildVariables_ardt_storinformazioni() {
if ( ! (ChildToLoad(FrameRef('ardt_storinformazioni'))) && Ne(typeof(FrameRef('ardt_storinformazioni').DoUpdate),'undefined')) {
CopyWorkVar(FrameRef('ardt_storinformazioni'),['TIPORAP'],['FLAGRAP'],false)
}
}
function CtxLoad_ardt_storinformazioni() {
if (ChildToLoad(FrameRef('ardt_storinformazioni'))) {
return [LoadContext(0,'ZFLICMSKTV'),'ardt_storinformazioni'];
}
}
function SetChildVariables_ardt_storsoginfo() {
}
function CtxLoad_ardt_storsoginfo() {
if (ChildToLoad(FrameRef('ardt_storsoginfo'))) {
return [LoadContext(0,'IXHEMDOSUG'),'ardt_storsoginfo'];
}
}
