...
public class arpg_famnoinvio extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*NTEZXNEWLB*!";
  ...
  public String getEntityTitle() {
    return "Creazione File SID Famiglie mai inviate";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_famnoinvio";
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("flgexist","C",1,0, "edit","")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_famnoinvioBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"10988550450",p_Context.Translate("In essere al 01-01-2011")},
    {"11396780126",p_Context.Translate("Intermediario")},
    {"12090677005",p_Context.Translate("Visualizza file creati")},
    {"1764774550",p_Context.Translate("Creazione File SID Famiglie mai inviate")},
    {"822914937",p_Context.Translate("Confermi la creazione dei file integrativi 2015 ?")}};
    return l_translations;
  }
  void WriteStyles(arpg_famnoinvioBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_famnoinvio.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_famnoinvioBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_famnoinvioBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_famnoinvioBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_famnoinvioBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_famnoinvioBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(518);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_famnoinvioBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='DMMJSVFBEV'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("CREAZIONE FILE SID FAMIGLIE MAI INVIATE"))+"" +
     "</label>" +
     "</span>" +
    "");
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<span id='RSFTNWRBDD_DIV'>" +
     "<input name='flgexist' id='RSFTNWRBDD' type='checkbox' value="+SPLib.ToHTMLValue(BO.w_flgexist,"C",1,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq("S",CPLib.Trim(BO.w_flgexist))?"CHECKED":"")+">" +
     "<label id='RSFTNWRBDD_LBL' for='RSFTNWRBDD' class='SPLabel'>"+SPLib.ToHTMLRepresentation(status.context.Translate("In essere al 01-01-2011"))+"</label>" +
     "</span>" +
    "");
    status.themeWriter.Header_EndHtml();
  }
  static void PrintState(arpg_famnoinvioBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_famnoinvioBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_famnoinvioBL BO,ServletStatus status) {
    status.out.print("DeclareWVs(");
    status.out.print(SPLib.ToJSValue(BO.w_gIntemediario,"C",11,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_gDescAzi,"C",70,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_flgexist,"C",1,0));
    status.out.println(")");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      status.out.println("m_bUpdated="+SPLib.ToJSValue(BO.m_bUpdated,"L",0,0)+";");
    }
  }
  void WriteMainFormVarScript(arpg_famnoinvioBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_famnoinvio"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_famnoinvio","");
  ...
  void WriteMainFormScript(arpg_famnoinvioBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_famnoinvioBO getBO(ServletStatus status) {
    arpg_famnoinvioBO BO = new arpg_famnoinvioBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_famnoinvioBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_famnoinvioBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_famnoinvioBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_famnoinvioBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_famnoinvioBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_famnoinvioBO BO) {
    ...
    item.put("altInterfaceFor","arpg_famnoinvio");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"519\",\"name\":\"Box_TTRVXROIVP\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"TTRVXROIVP\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"801\",\"x\":\"-1\",\"y\":\"-1\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\"#0000FF\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"CREAZIONE_FILE_SID_FAMIGLIE_MAI_INVIATE\",\"page\":\"1\",\"spuid\":\"DMMJSVFBEV_DIV\",\"tabindex\":\"2\",\"type\":\"Label\",\"value\":\"CREAZIONE FILE SID FAMIGLIE MAI INVIATE\",\"w\":\"800\",\"x\":\"0\",\"y\":\"0\",\"zone\":\"TTRVXROIVP_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"56\",\"href\":\"javascript:ALQVOOWZEV_Click()\",\"name\":\"Crea_Comunicazione\",\"page\":\"1\",\"spuid\":\"ALQVOOWZEV\",\"tabindex\":\"3\",\"type\":\"Button\",\"value\":\"Crea Comunicazione\",\"w\":\"264\",\"x\":\"8\",\"y\":\"21\",\"zone\":\"TTRVXROIVP_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"784\",\"x\":\"8\",\"y\":\"259\",\"zone\":\"TTRVXROIVP_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"168\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"784\",\"x\":\"8\",\"y\":\"84\",\"zone\":\"TTRVXROIVP_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stampa contenuto dei file creati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_MPCMAMHQST()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"MPCMAMHQST\",\"src\":\"images\\/print.gif\",\"tabindex\":\"10\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"656\",\"y\":\"35\",\"zone\":\"TTRVXROIVP_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"161\",\"href\":\" \",\"name\":\"__iframe_width__750__height__400__id__elaborazione___name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"TYIWXFLLFY_DIV\",\"tabindex\":\"11\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"750"+"'"+" height="+"'"+"400"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"784\",\"x\":\"8\",\"y\":\"350\",\"zone\":\"TTRVXROIVP_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"21\",\"name\":\"flgexist\",\"page\":\"1\",\"spuid\":\"RSFTNWRBDD\",\"tabindex\":\"12\",\"type\":\"Checkbox\",\"w\":\"144\",\"x\":\"288\",\"y\":\"35\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_famnoinvioBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_famnoinvioBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_famnoinvioBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_famnoinvio?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_famnoinvio_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_famnoinvioBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_famnoinvioBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_famnoinvioBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_famnoinvioBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_famnoinvioBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_famnoinvioBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_famnoinvio.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_famnoinvioBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_famnoinvioBO BO) {
    boolean result = true;
    return result;
  }
}
