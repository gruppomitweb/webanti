...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('MCINQDZYTJ').value,'C'),w_CBCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_catbacheca',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CBCODICE') &&  ! (Empty(HtW(Ctrl('MCINQDZYTJ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CBCODICE',HtW(Ctrl('MCINQDZYTJ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_catbacheca');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_catbacheca',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['MCINQDZYTJ'],i=0;i>=0;i--){
