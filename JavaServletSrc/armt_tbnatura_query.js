...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('JLVPSZIHBQ').value,'N'),w_CODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbnatura',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODICE') &&  ! (Empty(HtW(Ctrl('JLVPSZIHBQ').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODICE',HtW(Ctrl('JLVPSZIHBQ').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tbnatura');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbnatura',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['JLVPSZIHBQ'],i=0;i>=0;i--){
