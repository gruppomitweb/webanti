...
// * --- Fine Area Manuale
public class arpg_cgo_operazioni_agenzie_storicoBO extends arpg_cgo_operazioni_agenzie_storicoBL {
  ...
  public arpg_cgo_operazioni_agenzie_storicoBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_CGODATDIPCOM = rs.GetDate("CGODATDIPCOM");
          AddTrsRow();
          rs.Next();
          l_nCnt = l_nCnt+1;
        }
