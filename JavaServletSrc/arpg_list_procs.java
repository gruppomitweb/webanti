...
public class arpg_list_procs extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*HOBFNASBVQ*!";
  ...
  public String getEntityTitle() {
    return "Lista Processi Eseguiti";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_list_procs";
    ...
    l_eg.m_cQueryName = "qbe_list_procs";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_list_procsBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1512313662",p_Context.Translate("Lista Processi Eseguiti")},
    {"285808099",p_Context.Translate("Descrizione")},
    {"443606",p_Context.Translate("Fine")},
    {"485092",p_Context.Translate("Tipo")}};
    return l_translations;
  }
  void WriteStyles(arpg_list_procsBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_list_procs.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_list_procsBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_list_procsBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_list_procsBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_list_procsBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_list_procsBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_list_procsBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_list_procsBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_list_procs"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_list_procs","");
  ...
  void WriteMainFormScript(arpg_list_procsBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_list_procsBO getBO(ServletStatus status) {
    arpg_list_procsBO BO = new arpg_list_procsBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_list_procsBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_list_procsBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_list_procsBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_list_procsBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_list_procsBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_list_procsBO BO) {
    ...
    item.put("altInterfaceFor","arpg_list_procs");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_list_procsBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_list_procsBO BO) {
    status.out.println("var c_PR_TIPO;");
    status.out.println("var c_PRCODICE;");
    status.out.println("var c_PRDESCRI;");
    status.out.println("var c_PRDOSTART;");
    status.out.println("var c_PRDOSTOP;");
    status.out.println("var c_PRESITO;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_list_procsBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_PR_TIPO,"C",3,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_PRCODICE,"C",10,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_PRDESCRI,"C",100,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_PRDOSTART,"T",14,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_PRDOSTOP,"T",14,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_PRESITO,"C",1,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_list_procsBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_list_procsBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_list_procsBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_list_procs?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_list_procs_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_list_procsBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_list_procsBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_list_procsBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_list_procsBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_list_procsBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_list_procsBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_list_procsBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_list_procsBO BO) {
    boolean result = true;
    return result;
  }
}
