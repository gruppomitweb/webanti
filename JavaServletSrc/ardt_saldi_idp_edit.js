...
function Set_TRIMESTRE(ctrlValue,kmode,e) {
if (Ne(w_TRIMESTRE,ctrlValue)) {
var ctl = _GetCtl(e,'GCTHIIEEAW');
if (_tracker.goon(ctl,ctrlValue)) {
w_TRIMESTRE=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('GCTHIIEEAW',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',1,0,'');
...
var ctl = _GetCtl(e,'JYXMPQWGBF');
if (_tracker.goon(ctl,ctrlValue)) {
w_ANNO=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('JYXMPQWGBF',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',4,0,'');
...
var ctl = _GetCtl(e,'NGEQBBYMQU');
if (_tracker.goon(ctl,ctrlValue)) {
w_IMPORTO=ValidNum(ctrlValue,'999999999999.99');
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('NGEQBBYMQU',0);
DoUpdate(l_bResult);
...
if ( ! (m_oFather.SaveContext_ardt_saldi_idp())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_saldi_idp_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_saldi_idp_dataNotSent',{'action':formaction});
}
}
}
}
...
if (( ! (_ChkObl('JYXMPQWGBF',1))) && FullRow()) {
_SignErr('JYXMPQWGBF');
m_cLastWorkVarErrorMsg='484641';
l_bResult=false;
w_ANNO=HtW('','C');
} else if (( ! (_ChkObl('GCTHIIEEAW',1))) && FullRow()) {
_SignErr('GCTHIIEEAW');
m_cLastWorkVarErrorMsg='11684258599';
l_bResult=false;
w_TRIMESTRE=HtW('','C');
} else if (( ! (_ChkObl('NGEQBBYMQU',1))) && FullRow()) {
_SignErr('NGEQBBYMQU');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_saldi_idp();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields={};
l_aRepeatedFields.ANNO={"dec":0,"fixedpos":false,"id":"JYXMPQWGBF","len":4,"name":"ANNO","title":"Anno","type":"C"};
l_aRepeatedFields.TRIMESTRE={"dec":0,"fixedpos":false,"id":"GCTHIIEEAW","len":1,"name":"TRIMESTRE","title":"Trimestre","type":"C"};
l_aRepeatedFields.IMPORTO={"dec":2,"fixedpos":false,"id":"NGEQBBYMQU","len":15,"name":"IMPORTO","title":"Importo","type":"N"};
l_aRepeatedFields.IDBASE={"dec":0,"fixedpos":true,"id":"NRFSGIHMEA","len":10,"name":"IDBASE","title":"IDBASE","type":"C"};
