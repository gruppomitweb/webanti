...
var ctl = _GetCtl(e,'JYSWMXVXPB');
if (_tracker.goon(ctl,ctrlValue)) {
w_CODICE=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('JYSWMXVXPB',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',3,0,'');
...
var ctl = _GetCtl(e,'LSLSVBYHTZ');
if (_tracker.goon(ctl,ctrlValue)) {
w_DESCRI=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('LSLSVBYHTZ',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
parent.ZtVWeb.raiseEventToEvalParms('armt_tbcandistr_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_tbcandistr_dataNotSent',{'action':formaction});
}
}
}
}
...
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('JYSWMXVXPB',1))) {
_SignErr('JYSWMXVXPB');
m_cLastWorkVarErrorMsg='112851763';
l_bResult=false;
w_CODICE=HtW('','C');
} else if ( ! (_ChkObl('LSLSVBYHTZ',1))) {
_SignErr('LSLSVBYHTZ');
