...
SetLocationHref(null,'arrt_chkrap_anno'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
function href_AJXISNWTIP() {
SetLocationHref(window.location,ToResource('jsp/start_page_logo_portlet.jsp'+'')+'')
}
function href_PLJISFOJIP() {
var l_oWv = InitWvApplet();
l_oWv.setValue('ReportName',AsAppletValue('arrp_chkrap_anno'));
...
l_oWv.setValue('w_DescAzi',AsAppletValue(w_DescAzi));
l_oWv.setValue('m_cParameterSequence',AsAppletValue('ReportName,rotation,outputFormat,w_anno,w_DescAzi'));
...
w_Messaggi.raiseEvent('arpg_chkrap_anno_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_chkrap_anno_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_chkrap_anno',m_cSelectedPage);
...
l_bTmpRes=Link_PAAQSVBYXQ(null);
}
...
function Link_PAAQSVBYXQ(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
...
function Link_PAAQSVBYXQ_Blank() {
w_gIntemediario='';
w_DescAzi='';
}
function LOpt_PAAQSVBYXQ() {
var l_oWv = InitWvApplet();
...
l_oWv.setValue('UID',WtA('PAAQSVBYXQ','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['PAAQSVBYXQ'][1],'C'));
return l_oWv;
}
...
l_oWv.setValue('DescAzi',WtA(w_DescAzi,'C'));
_FillChildren(l_oWv);
}
_FillChildren.n=[];
FillWv.n=["anno","dadata","a_data","gIntemediario","DescAzi"];
