...
// * --- Fine Area Manuale
public class ardt_mlientifrzBO extends ardt_mlientifrzBL {
  ...
  // * --- Fine Area Manuale
  public ardt_mlientifrzBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_mlientifrz");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_mlientifrz,m_cPhName_mlientifrz,m_cVirtName_mlientifrz,m_cVirtName_mlientifrz+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_mlientifrz,p_IDFILEBO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"mlientifrz")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_mlientifrz.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND CODSOG="+CPLib.ToSQL(opk_CODSOG,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_mlientifrz.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"mlientifrz")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_mlientifrz,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_mlientifrz);
      l_cSql = "insert into "+m_oWrInfo_mlientifrz.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDFILEBO,";
      l_cSql = l_cSql+"CODSOG,";
      l_cSql = l_cSql+"IDBASE,";
      l_cSql = l_cSql+"PROQUOTA,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_mlientifrz.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"mlientifrz")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_mlientifrz.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"mlientifrz")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_mlientifrz,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_mlientifrz);
      l_cSql = "Update "+m_oWrInfo_mlientifrz.GetTableWriteName()+" set";
