...
public class arrt_edt_personbo extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*9bdbc72f*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_TIPO) ||  ! (status.m_bpassedw_datavaria) ||  ! (status.m_bpassedw_CONNES);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_edt_personboR Rcalled;
    Rcalled = new arrt_edt_personboR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_edt_personboR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_edt_personboR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_edt_personbo"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_edt_personbo");
      if ( ! (CPLib.IsNull(l))) {
