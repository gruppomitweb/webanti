...
if(c=Ctrl('EMXLNIJICF')) ChkboxCheckUncheck(c,'S',w_flgforce)
if(c=Ctrl('EMXLNIJICF')) ChkboxValueAssign(c,'S','N','C',1,0,'')
...
SetLocationHref(null,'arrt_trasffra'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
function href_PTXYMSWBZO() {
SetLocationHref(window.location,ToResource('jsp/start_page_logo_portlet.jsp'+'')+'')
}
function EMXLNIJICF_Valid(e) {
e=(Ne(e,null)?e:window.event);
return Set_flgforce(ChkboxValueAssign(Ctrl('EMXLNIJICF'),'S','N','C',1,0,''),null,e);
}
function EMXLNIJICF_OnFocus(e) {
SetActiveField(Ctrl('EMXLNIJICF'),true);
}
function EMXLNIJICF_OnBlur(e) {
SetActiveField(Ctrl('EMXLNIJICF'),false);
}
_ResetTracker();
...
if(c=Ctrl('EMXLNIJICF')) c.onclick=EMXLNIJICF_Valid;
if(c=Ctrl('EMXLNIJICF')) c.onfocus=EMXLNIJICF_OnFocus;
if(c=Ctrl('EMXLNIJICF')) c.onblur=EMXLNIJICF_OnBlur;
}
}
function NotifyEvent(p_cEvent) {
var old_c;
if (Ne(typeof(w_Messaggi),'undefined')) {
w_Messaggi.raiseEvent('arpg_trasffra_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_trasffra_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_trasffra_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_trasffra',m_cSelectedPage);
...
function Set_flgforce(ctrlValue,kmode,e) {
if (Ne(w_flgforce,ctrlValue)) {
var ctl = _GetCtl(e,'EMXLNIJICF');
if (_tracker.goon(ctl,ctrlValue)) {
w_flgforce=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('EMXLNIJICF',0);
DoUpdate(l_bResult);
} else {
...
l_bResult=false;
w_ADatOpe=HtW('','D');
} else if ( ! (_ChkObl('EMXLNIJICF',1))) {
_SignErr('EMXLNIJICF');
m_cLastWorkVarErrorMsg='980569461';
l_bResult=false;
w_flgforce='N';
}
...
l_oWv.setValue('flgforce',WtA(w_flgforce,'C'));
_FillChildren(l_oWv);
}
_FillChildren.n=[];
FillWv.n=["DaDatOpe","ADatOpe","cdadata","c_adata","gFlgDTP","flgforce"];
...
w_cdadata=DateToChar(w_DaDatOpe);
w_c_adata=DateToChar(w_ADatOpe);
if (start && Eq(At(',flgforce,',pseq),0)) {
w_flgforce='N';
}
m_bCalculating=false;
SaveDependsOn();
...
_Obli('flgforce',0,'EMXLNIJICF',false,"980569461")
