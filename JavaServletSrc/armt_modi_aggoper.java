...
public class armt_modi_aggoper extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*GNZKOUYLZI*!";
  ...
  public String getEntityTitle() {
    return "Modifica Dati per Agenzia Entrate (Operazioni)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_modi_aggoper";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_modi_aggoperBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"12112230433",p_Context.Translate("Modifica Dati per Agenzia Entrate (Operazioni)")},
  ...
  void WriteStyles(armt_modi_aggoperBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/armt_modi_aggoper.css'>" +
      "");
    }
  ...
  void WriteMainForm(armt_modi_aggoperBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(armt_modi_aggoperBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(armt_modi_aggoperBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(421);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(armt_modi_aggoperBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(armt_modi_aggoperBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_modi_aggoperBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(armt_modi_aggoperBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(armt_modi_aggoperBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_modi_aggoper"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_modi_aggoper","");
  ...
  void WriteMainFormScript(armt_modi_aggoperBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- armt_modi_aggoper");
    ...
    status.out.println("    SendData('start',PlatformPathStart('arrt_modi_aggoper')+'',HParApplet().asString());");
  ...
  armt_modi_aggoperBO getBO(ServletStatus status) {
    armt_modi_aggoperBO BO = new armt_modi_aggoperBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(armt_modi_aggoperBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_modi_aggoperBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,armt_modi_aggoperBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,armt_modi_aggoperBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,armt_modi_aggoperBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,armt_modi_aggoperBO BO) {
    ...
    item.put("altInterfaceFor","armt_modi_aggoper");
    items.put(item);
    String s;
  ...
  void Next(ServletStatus status,armt_modi_aggoperBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,armt_modi_aggoperBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(armt_modi_aggoperBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_modi_aggoper?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/armt_modi_aggoper_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(armt_modi_aggoperBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(armt_modi_aggoperBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_modi_aggoperBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_modi_aggoperBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_modi_aggoperBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(armt_modi_aggoperBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,armt_modi_aggoperBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,armt_modi_aggoperBO BO) {
    boolean result = true;
    return result;
  }
}
