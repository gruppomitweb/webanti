...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('CDCDVNOEKS').value,'C'),w_CODVOC)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbcausin',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODVOC') &&  ! (Empty(HtW(Ctrl('CDCDVNOEKS').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODVOC',HtW(Ctrl('CDCDVNOEKS').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_causalis');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbcausin',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['CDCDVNOEKS'],i=0;i>=0;i--){
