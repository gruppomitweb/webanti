...
l_oWv.setValue('m_cParameterSequence',AsAppletValue('w_a_data,w_anno,w_CAFINVIO,w_CFINVIO,w_dadata,w_DATAINVIO,w_FILEINVIO,w_mese'));
if ( ! (Empty(w_dadata)) &&  ! (Empty(w_a_data))) {
SetLocationHref(null,'arrt_sd_aprichiudi_delope'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cMode=hyperlink','elaborazione')
}
}
function ALQVOOWZEV_Click() {
if ( ! (Empty(w_darett)) &&  ! (Empty(w_a_rett))) {
Elabora2()
}
}
...
w_Messaggi.raiseEvent('arpg_scaricodati_delope_new_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_scaricodati_delope_new_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_scaricodati_delope_new',m_cSelectedPage);
...
l_bEnabled= ! ( ! (Empty(w_darett)) &&  ! (Empty(w_a_rett)));
...
ResetErrorVars();
m_nLastError=0;
if (( ! (_ChkObl('GTMJENMCMN',1))) || ( ! (Ge(w_anno,2008)))) {
...
_FillChildren.n=[];
FillWv.n=["anno","mese","darett","a_rett","dadata","a_data","gIntemediario","CFINVIO","CAFINVIO","FILEINVIO","DATAINVIO","sogest","gDescAzi","mespre"];
