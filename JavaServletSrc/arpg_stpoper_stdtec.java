...
public class arpg_stpoper_stdtec extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*DNTFCYXZXS*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_stpoper_stdtec";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 717;
    l_eg.m_nEntityHeight = 212;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_stpoper_stdtecBO BO) {
  ...
  void WriteStyles(arpg_stpoper_stdtecBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_stpoper_stdtec.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_stpoper_stdtecBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_stpoper_stdtecBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_stpoper_stdtecBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(207);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_stpoper_stdtecBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_stpoper_stdtecBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_stpoper_stdtecBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_stpoper_stdtecBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_stpoper_stdtecBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(212,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_stpoper_stdtec"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_stpoper_stdtec","");
  ...
  void WriteMainFormScript(arpg_stpoper_stdtecBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_stpoper_stdtecBO getBO(ServletStatus status) {
    arpg_stpoper_stdtecBO BO = new arpg_stpoper_stdtecBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_stpoper_stdtecBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_stpoper_stdtecBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_stpoper_stdtecBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_stpoper_stdtecBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_stpoper_stdtecBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_stpoper_stdtecBO BO) {
    ...
    item.put("altInterfaceFor","arpg_stpoper_stdtec");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"198\",\"name\":\"Box_XLVFHSOSYH\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"XLVFHSOSYH\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"704\",\"x\":\"8\",\"y\":\"9\",\"zone\":\"pag1_1\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_XURLGQZZMU()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"XURLGQZZMU\",\"src\":\"images\\/print.gif\",\"tabindex\":\"8\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"272\",\"y\":\"171\",\"zone\":\"XLVFHSOSYH_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_PTXYMSWBZO()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"PTXYMSWBZO\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"9\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"368\",\"y\":\"171\",\"zone\":\"XLVFHSOSYH_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Ad_Agenzia_Dipendenza_\",\"page\":\"1\",\"spuid\":\"BGOVXFIBXQ_DIV\",\"tabindex\":\"20\",\"type\":\"Label\",\"value\":\"Ad Agenzia\\/Dipendenza:\",\"w\":\"157\",\"x\":\"8\",\"y\":\"50\",\"zone\":\"XLVFHSOSYH_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void Next(ServletStatus status,arpg_stpoper_stdtecBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_stpoper_stdtecBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_stpoper_stdtecBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_stpoper_stdtec?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_stpoper_stdtec_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(212,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
  ...
  void RaiseLoadedEntity(arpg_stpoper_stdtecBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_stpoper_stdtecBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_stpoper_stdtecBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_stpoper_stdtecBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_stpoper_stdtecBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_stpoper_stdtecBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_stpoper_stdtecBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_stpoper_stdtecBO BO) {
    boolean result = true;
    return result;
  }
}
