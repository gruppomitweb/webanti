...
public class arfn_datisog_sol_row extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*a2792eda*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public MemoryCursorRow_mcsoggettiall1_mcrdef aaab_retval;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arfn_datisog_sol_row";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arfn_datisog_sol_rowR Rcalled;
    Rcalled = new arfn_datisog_sol_rowR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.m_CursorResult = Rcalled.Run(status.pCli,status.pDatOpe,status.pSTATO,status.pDatRet).AsMemoryCursor();
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arfn_datisog_sol_rowR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arfn_datisog_sol_rowR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arfn_datisog_sol_row"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
