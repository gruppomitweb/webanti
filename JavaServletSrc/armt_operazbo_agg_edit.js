...
if ( ! (m_oFather.SaveContext_armt_operazbo_agg())) {
...
parent.ZtVWeb.raiseEventToEvalParms('armt_operazbo_agg_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_operazbo_agg_dataNotSent',{'action':formaction});
}
}
}
}
...
_SignErr('JQLGHMLOKO');
m_cLastWorkVarErrorMsg='157155508';
l_bResult=false;
w_CATEG04=HtW('','C');
} else if ( ! (_ChkObl('QVBZSFMTVP',1))) {
_SignErr('QVBZSFMTVP');
m_cLastWorkVarErrorMsg='157155764';
l_bResult=false;
w_CATEG05=HtW('','C');
...
m_bUpdated=true;
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_armt_operazbo_agg();
}
}
