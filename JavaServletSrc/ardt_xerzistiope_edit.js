...
if ( ! (m_oFather.SaveContext_ardt_xerzistiope())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_xerzistiope_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_xerzistiope_dataNotSent',{'action':formaction});
}
}
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_xerzistiope();
}
}
SaveContext.child=[];
function BlankRow() {
