...
public class arrt_chgoper extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*52dda975*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_chgoperR Rcalled;
    Rcalled = new arrt_chgoperR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaa_retval = Rcalled.Run(status.w_daCodCli,status.w_aCodCli,status.w_DaDatOpe,status.w_ADatOpe);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_chgoperR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_chgoperR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_chgoper"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
