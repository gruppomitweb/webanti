...
public class arpg_jperazbo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*PYUFOKKLTX*!";
  ...
  public String getEntityTitle() {
    return "Operazioni relative al rapporto (Agenzia Entrate)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_jperazbo";
    ...
    l_eg.m_cQueryName = "qbe_operazrappo_ae";
    l_eg.m_nRowsPerPage = 12;
    return l_eg;
  }
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_jperazboBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"313529672",p_Context.Translate("Operazioni relative al rapporto (Agenzia Entrate)")},
    {"7776403",p_Context.Translate("Stato")}};
    return l_translations;
  }
  void WriteStyles(arpg_jperazboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_jperazbo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_jperazboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_jperazboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_jperazboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_jperazboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_jperazboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_jperazboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_jperazboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_jperazboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_jperazbo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_jperazbo","");
    ...
    status.out.println("// * --- arpg_jperazbo");
  ...
  void WriteMainFormScript(arpg_jperazboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_jperazboBO getBO(ServletStatus status) {
    arpg_jperazboBO BO = new arpg_jperazboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_jperazboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_jperazboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_jperazboBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_jperazboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_jperazboBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_jperazboBO BO) {
    ...
    item.put("altInterfaceFor","arpg_jperazbo");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_jperazboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_jperazboBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_jperazboBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_jperazboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_jperazboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_jperazboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_jperazbo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_jperazbo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_jperazboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_jperazboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_jperazboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_jperazboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_jperazboBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_jperazboBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_jperazboBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_jperazboBO BO) {
    boolean result = true;
    return result;
  }
}
