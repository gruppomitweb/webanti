...
public class arrt_stprappo extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*13860605*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_DaSogg;
    public String w_DaSogg;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_ASogg;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_DaSogg = source.Has("w_DaSogg");
      if (m_bpassedw_DaSogg) {
        w_DaSogg = source.GetParameter("w_DaSogg",source.GetParameter("DaSogg",""));
      } else {
        if (source.IsForwarded()) {
          w_DaSogg = source.GetCaller().GetString("DaSogg","C",16,0);
          m_bpassedw_DaSogg = true;
        } else {
          w_DaSogg = source.GetParameter("DaSogg",source.GetParameter("w_DaSogg",""));
        }
      }
      m_bpassedw_ASogg = source.Has("w_ASogg");
      if (m_bpassedw_ASogg) {
        w_ASogg = source.GetParameter("w_ASogg",source.GetParameter("ASogg",""));
      } else {
        if (source.IsForwarded()) {
          w_ASogg = source.GetCaller().GetString("ASogg","C",16,0);
          m_bpassedw_ASogg = true;
        } else {
          w_ASogg = source.GetParameter("ASogg",source.GetParameter("w_ASogg",""));
        }
      }
    }
  }
  ...
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_DaSogg)) {
      status.out.println("l_cV=WtA(l_oWnd('w_DaSogg',"+SPLib.ToJSValue(status.w_DaSogg,"C",16,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_DaSogg,"C",16,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_DaSogg',l_cV);");
    if ( ! (status.m_bpassedw_ASogg)) {
      status.out.println("l_cV=WtA(l_oWnd('w_ASogg',"+SPLib.ToJSValue(status.w_ASogg,"C",16,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_ASogg,"C",16,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_ASogg',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_stprappoR Rcalled;
    Rcalled = new arrt_stprappoR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_stprappoR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_stprappoR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_stprappo"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_DaSogg="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_DaSogg),"?",0,0)+"");
        status.out.println("js:w_ASogg="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_ASogg),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_DaSogg,null)) {");
        status.out.println("window[f].w_DaSogg="+SPLib.ToJSValue(status.w_DaSogg,"C",0,0)+";");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_ASogg,null)) {");
        status.out.println("window[f].w_ASogg="+SPLib.ToJSValue(status.w_ASogg,"C",0,0)+";");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_stprappo");
      if ( ! (CPLib.IsNull(l))) {
