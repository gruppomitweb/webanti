...
WindowAlert(AlertErrorMessage('1561757871'));
}
if (Eq(formaction,'edit') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('11961152715'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('1974243272'));
}
}
}
var m_bIsMq = false;
...
m_oFather.DoQueryFromChild('wersonbo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
function DoQueryFrom_wersonbo_agg(firstAlias) {
return m_cVirtName+".CONNES = "+firstAlias+".CONNES ";
}
...
_chNotF(childAlias,'master','armt_wersonbo');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wersonbo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
