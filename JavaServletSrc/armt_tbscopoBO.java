...
// * --- Fine Area Manuale
public class armt_tbscopoBO extends armt_tbscopoBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbscopoBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbscopo");
    }
  ...
  public boolean Load(double p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbscopo,m_cPhName_tbscopo,m_cVirtName_tbscopo,m_cVirtName_tbscopo+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"N",5,0),m_cServer_tbscopo,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbscopo")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbscopo);
      l_cSql = "insert into "+m_oWrInfo_tbscopo.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbscopo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbscopo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_tbscopo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbscopo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbscopo,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbscopo);
      l_cSql = "Update "+m_oWrInfo_tbscopo.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"N",5,0),m_cServer_tbscopo,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbscopo.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbscopo")) {
  ...
  public boolean Delete(double p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbscopo.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"N",5,0),m_cServer_tbscopo,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbscopo.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbscopo")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbscopo,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
  ...
  void SaveAutonumber() {
    ...
    m_Sql.NextTableProg(this,"PRGSCOPO","w_CODICE",m_cServer_tbscopo,m_Ctx.GetCompany());
    op_CODICE = w_CODICE;
  }
