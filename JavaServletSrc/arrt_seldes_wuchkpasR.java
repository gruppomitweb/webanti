...
// * --- Fine Area Manuale
public class arrt_seldes_wuchkpasR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_seldes_wuchkpasR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_seldes_wuchkpas",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    if (CPLib.eqr(pTipo,"S")) {
      // * --- Write into tmp_impwu from values
      ...
      m_cSql = CPLib.GetSQLPrefixComment("arrt_seldes_wuchkpas",6,"00000005")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      m_cSql = m_cSql+"WUFLGSEL = "+CPLib.ToSQL("S","C",1,0,m_cServer)+", ";
      m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"tmp_impwu",false,true);
      m_cSql = m_cSql+" where ";
      ...
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_impwu",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_seldes_wuchkpas",6,"00000006")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      m_cSql = m_cSql+"WUFLGSEL = "+CPLib.ToSQL("N","C",1,0,m_cServer)+", ";
      m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"tmp_impwu",false,true);
      m_cSql = m_cSql+" where ";
      m_cSql = m_cSql+"1 = "+CPSql.SQLValueAdapter(CPLib.ToSQL(1,"?",0,0,m_cServer),m_cServer,1)+"";
      m_cSql = m_cSql+m_oWrInfo.WhereFilter();
  ...
  public static arrt_seldes_wuchkpasR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_seldes_wuchkpasR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pTipo = CPLib.Space(1);
  }
