...
public class arrt_trasf_nascita extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*df4f3672*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_trasf_nascitaR Rcalled;
    Rcalled = new arrt_trasf_nascitaR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_trasf_nascitaR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_trasf_nascitaR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_trasf_nascita"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
