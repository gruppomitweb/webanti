...
if(c=Ctrl('VFJTCKYERF')) c.value = WtH(w_STATOREG,'C',1,0,'')
...
SetDisplay(Ctrl('VFJTCKYERF'),true || IsHiddenByStateDriver('STATOREG'));
SetDisplay(Ctrl('MNNQRQRGPV'),Eq(w_STATOREG,'2') || Eq(w_STATOREG,'3') || Empty(w_datavaria));
HideUI();
}
...
e=(Ne(e,null)?e:window.event);
return Set_STATOREG(HtW(Ctrl('VFJTCKYERF').value,'C'),null,e);
}
...
if (false || WindowConfirm(Translate('11955306522'))) {
SetHypPar('m_cAction',AsAppletValue('start'));
SetHypPar('m_cMode',AsAppletValue('hyperlink'));
SetHypPar('m_cID',AsAppletValue(m_IDS['arrt_edt_rapopebo']));
SendData('start','arrt_edt_rapopebo'+'',HParApplet().asString());
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_edt_rapopebo_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_edt_rapopebo',m_cSelectedPage);
...
_SignErr('VFJTCKYERF');
m_cLastWorkVarErrorMsg='STATOREG';
l_bResult=false;
w_STATOREG=HtW('','C');
}
...
_FillChildren.n=[];
FillWv.n=["datavaria","IDBASE","STATOREG"];
...
if (Empty(w_STATOREG) && Ne(opener.w_STATOREG,null)) {
TransferBetweenWindows(opener,'w_STATOREG',null,'Set_STATOREG');
}
}
}
m_bCalculating=true;
...
_Obli('STATOREG',0,'VFJTCKYERF',false)
