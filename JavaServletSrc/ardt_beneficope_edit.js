...
if ( ! (m_oFather.SaveContext_ardt_beneficope())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_beneficope_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_beneficope_dataNotSent',{'action':formaction});
}
}
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_beneficope();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.C_RAG={"dec":0,"fixedpos":false,"id":"GAEWMPRBSY","len":70,"name":"C_RAG","title":"","type":"C"};
