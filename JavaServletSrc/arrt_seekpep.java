...
public class arrt_seekpep extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*6aeef40d*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public MemoryCursor_mcpepstructure_mcrdef aa_retval;
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_seekpepR Rcalled;
    Rcalled = new arrt_seekpepR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.m_CursorResult = Rcalled.Run(status.pRagSoc,status.pCodFisc);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_seekpepR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_seekpepR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_seekpep"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
