...
// * --- Fine Area Manuale
public class ardt_storsoginfoBO extends ardt_storsoginfoBL {
  ...
  // * --- Fine Area Manuale
  public ardt_storsoginfoBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_storsoginfo");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_woginfo+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_woginfo,m_cPhName_woginfo,m_cVirtName_woginfo,m_cVirtName_woginfo+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_woginfo,p_IDFILEBO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"woginfo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_woginfo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_woginfo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"woginfo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_woginfo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_woginfo);
      l_cSql = "insert into "+m_oWrInfo_woginfo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_woginfo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"woginfo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_woginfo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"woginfo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_woginfo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_woginfo);
      l_cSql = "Update "+m_oWrInfo_woginfo.GetTableWriteName()+" set";
      l_cSql = l_cSql+" RAPCOL = "+CPLib.ToSQL(w_RAPCOL,"C",25,0);
