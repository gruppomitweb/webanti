...
// * --- Fine Area Manuale
public class arrt_save_cgo_rapporti_errR implements CallerWithObjs {
  ...
  public String pPRGRAP;
  // * --- Area Manuale = BO - Properties
  // * --- Fine Area Manuale
  public arrt_save_cgo_rapporti_errR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_save_cgo_rapporti_err",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_save_cgo_rapporti_err";
    } else if (m_bEventRunning && CPLib.eqr("i_EntityType",p_cVarName)) {
      return "routine";
    }
    if (CPLib.eqr("pPRGRAP",p_cVarName)) {
      return pPRGRAP;
    }
    return "";
  }
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* Exec Routine arrt_writelog('VARIAZIONE','CGO_RAPPORTI_ERR',pPRGRAP) */
      arrt_writelogR.Make(m_Ctx,this).Run("VARIAZIONE","CGO_RAPPORTI_ERR",pPRGRAP);
      ...
      Cursor_cgo_rapporti_err = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"PRGRAP="+CPSql.SQLValueAdapter(CPLib.ToSQL(pPRGRAP,"?",0,0,m_cServer, m_oParameters),m_cServer,pPRGRAP)+" "+")"+(m_Ctx.IsSharedTemp("cgo_rapporti_err")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
      while ( ! (Cursor_cgo_rapporti_err.Eof())) {
        /* If cgo_rapporti_err->FLGFULL='S' */
        if (CPLib.eqr(Cursor_cgo_rapporti_err.GetString("FLGFULL"),"S")) {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arrt_save_cgo_rapporti_err",35,"00000007")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          while ( ! (Cursor_cgo_telematico_err.Eof())) {
            ...
            m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
            m_cSql = CPLib.GetSQLPrefixComment("arrt_save_cgo_rapporti_err",35,"00000009")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
            ...
            m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
            m_cSql = CPLib.GetSQLPrefixComment("arrt_save_cgo_rapporti_err",35,"0000000A")+"delete from "+m_oWrInfo.GetTableWriteName();
            m_cSql = m_cSql+" where ";
          ...
          m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
          m_cSql = CPLib.GetSQLPrefixComment("arrt_save_cgo_rapporti_err",35,"0000000B")+"delete from "+m_oWrInfo.GetTableWriteName();
          m_cSql = m_cSql+" where ";
          m_cSql = m_cSql+"PRGRAP = "+CPSql.SQLValueAdapter(CPLib.ToSQL(pPRGRAP,"?",0,0,m_cServer),m_cServer,pPRGRAP)+"";
          m_cSql = m_cSql+m_oWrInfo.WhereFilter();
          m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql,m_oParameters);
          if (CPLib.lt(m_nUpdatedRows,0)) {
            m_Sql.AbortTransaction();
      ...
      Cursor_cgo_rapporti_err.Close();
      // * --- End Select
      /* Stop arpg_cgo_rapporti_err */
      Forward f;
      f=new Forward("arpg_cgo_rapporti_err",false,this,Forward.Start,false);
  ...
  public String RunAsync(String p_pPRGRAP) {
    pPRGRAP = p_pPRGRAP;
  ...
  public Forward Run(String p_pPRGRAP) {
    pPRGRAP = p_pPRGRAP;
    return Run();
  }
  public Forward Run() {
  ...
  public static arrt_save_cgo_rapporti_errR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_save_cgo_rapporti_errR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pPRGRAP = CPLib.Space(15);
  }
  ...
  public static String[] m_cRunParameterNames={"pPRGRAP"};
