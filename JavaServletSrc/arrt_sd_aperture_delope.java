...
public class arrt_sd_aperture_delope extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*NTSWOZYKYX*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_sd_aperture_delope";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_seco) ||  ! (status.m_bpassedw_iniz) ||  ! (status.m_bpassedw_datseci) ||  ! (status.m_bpassedw_datsecf) ||  ! (status.m_bpassedw_datprf) ||  ! (status.m_bpassedw_datpri) ||  ! (status.m_bpassedw_dadata) ||  ! (status.m_bpassedw_a_data);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_sd_aperture_delopeR Rcalled;
    Rcalled = new arrt_sd_aperture_delopeR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_sd_aperture_delopeR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_sd_aperture_delopeR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_sd_aperture_delope"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println(" window[f].w_datprf.setTime(Date.parse("+SPLib.ToJSValue(status.w_datprf,"D",0,0)+"))");
        status.out.println("}");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_sd_aperture_delope");
      if ( ! (CPLib.IsNull(l))) {
