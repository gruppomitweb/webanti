...
public class arpg_intestit extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*DPIPKIIVOU*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.m_cQueryName = "qbe_intestit";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_intestitBO BO) {
  ...
  void WriteStyles(arpg_intestitBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_intestit.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_intestitBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_intestitBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_intestitBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_intestitBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_intestitBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_intestitBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_intestitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_intestit"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_intestit","");
  ...
  void WriteMainFormScript(arpg_intestitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_intestitBO getBO(ServletStatus status) {
    arpg_intestitBO BO = new arpg_intestitBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_intestitBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_intestitBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_intestitBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_intestitBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_intestitBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_intestitBO BO) {
    ...
    item.put("altInterfaceFor","arpg_intestit");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_intestitBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_intestitBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_intestitBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_intestitBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_intestitBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_intestitBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_intestit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_intestit_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_intestitBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_intestitBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_intestitBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_intestitBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_intestitBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_intestitBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_intestitBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_intestitBO BO) {
    boolean result = true;
    return result;
  }
}
