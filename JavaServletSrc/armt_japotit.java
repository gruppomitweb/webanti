...
public class armt_japotit extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*17171ed9*!";
  static public class ServletStatus extends SPMasterServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "japotit";
    }
    public String w_IDBASE;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_IDBASE,context,"japotit","IDBASE","C",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_japotit;
  ...
  public String getEntityTitle() {
    return "Rapporto Titolare Effettivo";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_japotit";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 1021;
    l_eg.m_nEntityHeight = 545;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"japotit","intermbo","anadip","personbo","tbtipreg","tbtipleg","jnarapbo"};
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","Dati","page"),new ZoneItem("DB","Dati Base","horizontal_section"),new ZoneItem("box_BOYWPGJEEZ","","auto_horizontal_section")};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_japotitBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11366670608",p_Context.Translate("Rapporto Titolare Effettivo")},
    ...
    {"1766394234",p_Context.Translate("E' indicato il sesso, quindi si tratta di persona fisica e non di Società. Non esiste Titolare effettivo di persona fisica")},
  ...
  void WriteStyles(armt_japotitBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_japotit.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"Dati")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_japotit',false);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_japotit",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(armt_japotitBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:545px;width:1021px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(armt_japotitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
     "<div name='DB' id='YORQOIOBQN_DIV' class='BoxContainer SPSection_expanded SectionContainerWithTitle'>" +
    "");
    ...
    "");
    status.includeZoneAltInterface("DB@BEGIN");
    if (status.includeZoneAltInterface("DB")) {
      WriteControls_p1_b1_CloseBand(BO,status);
    } else {
      if (status.HasZoneAltInterface("DB@BEGIN") || status.HasZoneAltInterface("DB@END")) {
        status.out.println(
      ...
      WriteControls_p1_b1_b3(BO,status);
      WriteControls_p1_b1_b4(BO,status);
      status.out.println(
       "<script>" +
      "");
      status.out.println("ToggleCollapsibleBox.subBands['YORQOIOBQN']=['band_1_7','BOYWPGJEEZ','band_1_49','band_1_216','band_1_511'];");
      status.out.println(
       "</script>" +
      "");
      if (status.HasZoneAltInterface("DB@BEGIN") || status.HasZoneAltInterface("DB@END")) {
  ...
  void WriteControls_p1_b1_CloseBand(armt_japotitBO BO,ServletStatus status) {
    status.includeZoneAltInterface("DB@END");
    status.out.println(
     "</div>" +
    "");
    status.out.println(
  ...
  void WriteControls_p1_b1_b0(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_7' style='position:relative;height:7px;display:block' page='1'>" +
    "");
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.out.println(
  ...
  void WriteControls_p1_b1_b1(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_b1_CloseBand(armt_japotitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1_b2(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_49' style='position:relative;height:167px;display:block' page='1'>" +
    "");
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    status.out.println(
     ...
     "</button>" +
     ""+EventuallyContextMenu(status, "FMTBXELZFV", "jnarapbo", "", "")+"" +
    "");
    status.out.println(
  ...
  void WriteControls_p1_b1_b3(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<iframe name='ardt_jntestit' id='QBBTFAGDFU' frameborder='0' allowtransparency='true' toResize='no' tabindex='"+status.m_nTabIndex+"' src='javascript:[].join()'>" +
     "</iframe>" +
    "");
    status.out.println(
     "</div>" +
    ...
    status.out.println(
     "<script>" +
    "");
    status.out.println("ToggleCollapsibleBox.children['band_1_216']=['QBBTFAGDFU'];");
    status.out.println(
     "</script>" +
    "");
  }
  void WriteControls_p1_b1_b4(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_511' style='position:relative;height:9px;display:block' page='1'>" +
    "");
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.out.println(
  ...
  void WriteControls_p1_b2(armt_japotitBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_japotitBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_japotitBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
    if (BO.ardt_jntestit.IsUpdated()) {
      BO.m_cWv_ardt_jntestit = BO.ardt_jntestit.GetContext();
    }
  }
  static void PrintStateWorkVars(armt_japotitBL BO,ServletStatus status) {
    ...
    if (CPLib.eq(BO.m_cWv_ardt_jntestit,"_not_empty_") && status.m_bBrowserUpdate) {
      status.out.print(","+"m_cWv_ardt_jntestit");
    } else if (CPLib.eq(BO.m_cWv_ardt_jntestit,"_not_empty_")) {
      BO.m_cWv_ardt_jntestit = BO.ardt_jntestit.GetContext();
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_ardt_jntestit,"C",0,0));
    } else {
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_ardt_jntestit,"C",0,0));
    }
  ...
  void WriteMainFormVarScript(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(545,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_japotit"),"C",0,0)+ ";");
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      status.out.print(",'FMTBXELZFV':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash(entityGlobals.name+"FMTBXELZFV",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,jnarapbo,"+"default",l_session)+"'"+"]");
      ...
      status.out.print(",'FMTBXELZFV':['','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,jnarapbo,"+"default",l_session)+"'"+"]");
      AppendCallableIDS(status,",");
    }
    status.out.println("}");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_japotit","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_japotit",BO.GetCurrentState());
    ...
    if (CPLib.eq(status.GetZoneAltInterface("DB"),"")) {
      status.quoteAndAppend("DB","hide","box");
    }
  ...
  void WriteMainFormScript(armt_japotitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      status.out.println("if (Ctrl('QBBTFAGDFU')) {");
      status.out.println("ChangeIframeSrcWithoutPushingHistory('QBBTFAGDFU',linkpc_url('QBBTFAGDFU'));");
      status.out.println("}");
    ...
    status.out.println("// * --- armt_japotit");
    status.out.println("if (m_cFunction=='new') {");
    status.out.println("  CaricaTit()");
    status.out.println("}");
    status.out.println("");
    status.out.println("// * --- Fine Area Manuale");
    ...
    status.out.println("if (Eq(name,'ardt_jntestit')) {");
    status.out.println("l_cRes='"+SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_jntestit")+"';");
    ...
    if (CPLib.ne(status.m_cClientMode,"query")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"new")) {
        ...
        status.out.println("AutonumberApplet().SetAutonumber('i_codazi,w_IDBASE','PRGJTIT1');");
        ...
        status.out.println("AutonumberApplet().SetETName('japotit');");
        status.out.println("w_IDBASE=AutonumberApplet().GetAutonumberString('IDBASE');");
        status.out.println("}");
      }
      status.out.println("if (l_b1) {");
      status.out.println("AutonumberApplet().SetAutonumber('i_codazi,w_NUMPROG','PRGJTIT2');");
      status.out.println("AutonumberApplet().SetFixedProg('i_codazi',m_cCompany,'C',10,0);");
    ...
    status.out.println("}");
    status.out.println("");
    status.out.println("function CaricaTit() {");
    status.out.println("  Ctrl('QBBTFAGDFU').src='ardt_jntestit?m_bInnerChild=true&m_cAction=editload&CODCLI='+w_CONNESCLI+'&m_bRowsChild=True'    ");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
     "</script>" +
  ...
  armt_japotitBO getBO(ServletStatus status) {
    armt_japotitBO BO = new armt_japotitBO(status.context);
  ...
  void LoadExtensions(armt_japotitBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("japotit",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_japotitBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,armt_japotitBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
    l_cTmp = "[";
    VQRHolder l_VQRHolder=null;
  ...
  CPResultSet QueryKeyCursor(armt_japotitBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_japotit;
    String l_cPhName = BO.m_cPhName_japotit;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_japotitBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_japotit+".IDBASE",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("japotit");
    String l_cPhName = CPSql.ManipulateTablePhName("japotit",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,armt_japotitBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_japotit,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_japotit,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,armt_japotitBO BO) {
  ...
  void Edit(ServletStatus status,armt_japotitBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_japotitBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_japotitBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_japotitBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_japotitBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_japotitBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_japotitBO BO,SPParameterSource source) {
    BO.Load(status.w_IDBASE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_japotitBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_japotitBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_japotitBO BO) {
    ...
    item.put("altInterfaceFor","armt_japotit");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_name\":\
\"zone\":\"pag1_1_7\",\"zonepath\":\"[1,1,7]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_AGQEPDVFBG\",\"page\":1,\"spuid\":\"AGQEPDVFBG\",\"tabindex\":74,\"type\":\"Label\",\"value\":\"Note Brevi.:\",\"w\":110,\"x\":104,\"y\":196,\"zone\":\"pag1_1_7\",\"zonepath\":\"[1,1,7]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"autoexpand\":true,\"h\":295,\"name\":\"ardt_jntestit\",\"page\":1,\"spuid\":\"QBBTFAGDFU\",\"tabindex\":73,\"type\":\"Iframe\",\"w\":765,\"x\":112,\"y\":216,\"zone\":\"pag1_1_8\",\"zonepath\":\"[1,1,8]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":20,\"name\":\"button_XPWBHUQUGP\",\"page\":1,\"picker\":true,\"spuid\":\"XPWBHUQUGP\",\"tabindex\":72,\"type\":\"Button\",\"value\":\"Collega\",\"w\":72,\"x\":805,\"y\":192,\"zone\":\"pag1_1_9\",\"zonepath\":\"[1,1,9]\",\"zoom\":\"usrz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_japotitBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_japotit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_japotit_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_japotit_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_STATOREG=a[2];");
      status.out.println("w_CODINTER=a[3];");
      status.out.println("w_CODDIPE=a[4];");
      status.out.println("w_TIPOINTER=a[5];");
      status.out.println("w_TIPOINF=a[6];");
      status.out.println("w_IDEREG=a[7];");
      status.out.println("w_FLAGRAP=a[8];");
      status.out.println("w_OPRAP=a[9];");
      status.out.println("w_OPERATORE=a[10];");
      status.out.println("w_AUTOM=a[11];");
      status.out.println("w_DESCCIT=a[12];");
      status.out.println("w_PROVINCIA=a[13];");
      status.out.println("w_CODCAB=a[14];");
      status.out.println("w_FLAGRAP2=a[15];");
      status.out.println("w_FLAGLIRE=a[16];");
      status.out.println("w_PRGIMPTIT=a[17];");
      status.out.println("op_PRGIMPTIT=a[18];");
      status.out.println("w_CONNESCLI=a[19];");
      status.out.println("w_DATAOPE=a[20];");
      status.out.println("w_DATAREG=a[21];");
      status.out.println("w_CDATOPE=a[22];");
      status.out.println("w_TIPOOPRAP=a[23];");
      status.out.println("w_CONNESOPER=a[24];");
      status.out.println("w_TIPOLEG=a[25];");
      status.out.println("w_RAPPORTO=a[26];");
      ...
      status.out.println("w_OPERAZMOD=a[31];");
      status.out.println("w_CDATREG=a[32];");
      status.out.println("w_gIntemediario=a[33];");
      status.out.println("w_gDataVaria=a[34];");
      status.out.println("w_gStatus=a[35];");
      status.out.println("w_gCodDip=a[36];");
      status.out.println("w_gSblocco=a[37];");
      status.out.println("w_gChkDate=a[38];");
      status.out.println("w_sEdit=a[39];");
      status.out.println("w_flgpf=a[40];");
      status.out.println("w_flgpg=a[41];");
      status.out.println("w_xDescDipe=a[42];");
      status.out.println("w_xdescrap=a[43];");
      status.out.println("w_datpaseuro=a[44];");
      status.out.println("w_xDesReg=a[45];");
      status.out.println("w_xRagSoc=a[46];");
      status.out.println("w_errmsg=a[47];");
      status.out.println("w_Aggiorna=a[48];");
      status.out.println("w_lancio=a[49];");
      status.out.println("w_VALUTA=a[50];");
      status.out.println("w_tipreg=a[51];");
      status.out.println("w_tipregn=a[52];");
      status.out.println("w_dataoggi=a[53];");
      ...
      status.out.println("m_cWv_ardt_jntestit=a[64];");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_japotit_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(545,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void RaiseLoadedEntity(armt_japotitBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_japotitBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_japotitBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_japotitBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_japotitBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_japotitBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_japotitBO BO,ServletStatus status) throws IOException {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      CPLib.AddToSet(callable,"ardt_jntestit");
    } else {
  ...
  public String GetCheckVars(ServletStatus status,armt_japotitBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_japotitBO BO) {
    boolean result = true;
