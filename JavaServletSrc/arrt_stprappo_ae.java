...
public class arrt_stprappo_ae extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*51f1d58*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_noaui;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_noaui = source.Has("w_noaui");
      if (m_bpassedw_noaui) {
        w_noaui = source.GetParameter("w_noaui",source.GetParameter("noaui",""));
      } else {
        if (source.IsForwarded()) {
          w_noaui = source.GetCaller().GetString("noaui","C",1,0);
          m_bpassedw_noaui = true;
        } else {
          w_noaui = source.GetParameter("noaui",source.GetParameter("w_noaui",""));
        }
      }
    }
  }
  ...
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_noaui)) {
      status.out.println("l_cV=WtA(l_oWnd('w_noaui',"+SPLib.ToJSValue(status.w_noaui,"C",1,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_noaui,"C",1,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_noaui',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_stprappo_aeR Rcalled;
    Rcalled = new arrt_stprappo_aeR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_stprappo_aeR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_stprappo_aeR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_stprappo_ae"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_noaui="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_noaui),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_noaui,null)) {");
        status.out.println("window[f].w_noaui="+SPLib.ToJSValue(status.w_noaui,"C",0,0)+";");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_stprappo_ae");
      if ( ! (CPLib.IsNull(l))) {
