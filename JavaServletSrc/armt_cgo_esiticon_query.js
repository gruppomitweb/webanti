...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('IFJYIFRJJC').value,'C'),w_ECCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_esiticon',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'ECCODICE') &&  ! (Empty(HtW(Ctrl('IFJYIFRJJC').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.ECCODICE',HtW(Ctrl('IFJYIFRJJC').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_esiticon');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_esiticon',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['IFJYIFRJJC'],i=0;i>=0;i--){
