...
public class ardt_informazioni extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*a36313f9*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "intestbo";
    }
    public String w_RAPPORTO;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_RAPPORTO,context,"intestbo","RAPPORTO","C",25,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    ardt_informazioniBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_intestbo;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "ardt_informazioni";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 774;
    l_eg.m_nEntityHeight = 344;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"intestbo","personbo"};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("NUMPROG","C",10,0, "hide",""),
                   ...
                   new SPItem("xragsoci","C",40,0, "hide","Ragione Sociale"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_informazioniBO BO) {
  ...
  void WriteStyles(ardt_informazioniBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_informazioni.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_informazioniBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(
       "<style>" +
      "");
      status.out.println(".XTDHYWJPZL {top:1px;position:absolute;left:20px;height:16px;width:108px;background-color:Transparent;z-index:1; }");
      status.out.println(".XTDHYWJPZL_ZOOM { position:absolute;top:1px;left:129px;height:17px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".HKZAUFLHMG {top:0px;position:absolute;left:580px;height:16px;width:64px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".HKZAUFLHMG_ZOOM { position:absolute;top:0px;left:645px;height:17px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".NATIUIQLJX {top:0px;position:absolute;left:668px;height:16px;width:64px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".NATIUIQLJX_ZOOM { position:absolute;top:0px;left:733px;height:17px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".VICYEWBISV {top:1px;position:absolute;left:152px;height:16px;width:423px;background-color:Transparent;z-index:1; }");
      status.out.println(".BKRVFLJLOS {top:0px;position:absolute;left:0px;border-style:none;z-index:1;clip:rect(0,16px,17px,0);}");
      status.out.println(
       "</style>" +
      "");
    } else {
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_informazioni',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_informazioni",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_informazioniBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_informazioniBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:344px;width:774px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(ardt_informazioniBO BO,ServletStatus status) {
    ...
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      status.out.println(
       "<div name id='GridTable_Header' class='GridTable_Header' style='position:absolute;top:-1px;left:2px;height:18px;width:772px;z-index:0'>" +
       "</div>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='BFMPSJTVZY_DIV'>" +
       "<label id='BFMPSJTVZY' for='XTDHYWJPZL'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Intestatario"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='XCDXQXTRHM_DIV'>" +
       "<label id='XCDXQXTRHM' for='HKZAUFLHMG'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Inizio"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='QEAAMPSBPR_DIV'>" +
       "<label id='QEAAMPSBPR' for='NATIUIQLJX'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Fine"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:absolute;top:17px;left:2px;height:186px;width:772px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:186px;width:772px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:772px;height:186px;overflow:auto;'>" +
       "<table id='GridTable' style='position:relative;top:0px;left:0px;width:766px' cellspacing='0' cellpadding='0' border='0'>" +
       "<tr>" +
       "<td></td>" +
       "</tr>" +
       "</table>" +
      ...
      if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
        status.out.println(
         "<div id='GridTable_ButtonPanel' style='position:absolute;top:0px;left:4px;z-index:4'>" +
         ""+printButtonPanelContent(status,1,"upper_right",false,getDetailMenuItems(),false,"")+"" +
         "</div>" +
        "");
      }
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:17px;left:22px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:17px;left:153px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:17px;left:581px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:17px;left:669px;height:186px;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
  ...
  void HTMLRowScript(ardt_informazioniBO BO,ServletStatus status) {
    ...
    status.out.println("s='<div style=\"position:relative;width:750px;height:17px;\"';");
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     "<input name='DATAINI' id='HKZAUFLHMG' type='text' class='"+status.ObligatoryClass("Field GridTable_EditRowItem ",BO.HKZAUFLHMG_isObligatory())+"' value='\"+WtH(w_DATAINI,'D',8,0,TranslatePicture(GetDatePicture(true,'D'),'D'))+\"' tabindex='"+(CPLib.eq(status.m_cClientMode,"query")?""+status.m_nTabIndex+"":"-1")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='10' size='8' fieldtype='D'>" +
     "<button id='HKZAUFLHMG_ZOOM' type='button' class='CalendarButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
     "</button>" +
    "\"");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     "<input name='DATAFINE' id='NATIUIQLJX' type='text' class='"+status.ObligatoryClass("Field GridTable_EditRowItem ",BO.NATIUIQLJX_isObligatory())+"' value='\"+WtH(w_DATAFINE,'D',8,0,TranslatePicture(GetDatePicture(true,'D'),'D'))+\"' tabindex='"+(CPLib.eq(status.m_cClientMode,"query")?""+status.m_nTabIndex+"":"-1")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='10' size='8' fieldtype='D'>" +
     "<button id='NATIUIQLJX_ZOOM' type='button' class='CalendarButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
     "</button>" +
    "\"");
    status.out.println("s=s+\""+
    ...
    status.out.println("s=s+\""+
     ...
     "<button type='button' class='HKZAUFLHMG_ZOOM CalendarButtonDisabled' value="+SPLib.ToHTMLValue(status.context.Translate("..."),"C",0,0)+" disabled class='HKZAUFLHMG_ZOOM CalendarButtonDisabled'>" +
     "</button>" +
    "\"");
    status.out.println("}");
    ...
    status.out.println("s=s+\""+
     ...
     "<button type='button' class='NATIUIQLJX_ZOOM CalendarButtonDisabled' value="+SPLib.ToHTMLValue(status.context.Translate("..."),"C",0,0)+" disabled class='NATIUIQLJX_ZOOM CalendarButtonDisabled'>" +
     "</button>" +
    "\"");
    status.out.println("}");
    ...
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:750px;height:17px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b1(ardt_informazioniBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<div id='VRCVINYRJO' class='SectionBody' style='position:relative;height:50px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("box_VRCVINYRJO")) {
      ...
      if (status.HasZoneAltInterface("box_VRCVINYRJO@BEGIN") || status.HasZoneAltInterface("box_VRCVINYRJO@END")) {
        status.out.println(
         "<div id='VRCVINYRJO_boxContent' style='position:relative;height:50px;display:block'>" +
        "");
      }
      status.out.println(
      ...
      "");
      status.out.println(
       "<span class='SPLabel' id='NJCDHJUQPF_DIV'>" +
       "<label id='NJCDHJUQPF' for='PCCSJPWDXE'>" +
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='PROQUOTA' id='PCCSJPWDXE' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_PROQUOTA,"N",3,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='4' size='3' fieldtype='N'>" +
      "");
      if (status.HasZoneAltInterface("box_VRCVINYRJO@BEGIN") || status.HasZoneAltInterface("box_VRCVINYRJO@END")) {
        status.out.println(
         "</div>" +
  ...
  void WriteControls_p1_b1_CloseBand(ardt_informazioniBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("box_VRCVINYRJO")) {
      ...
      status.out.println("Ctrl('VRCVINYRJO').setAttribute('band_height',50);");
    }
  ...
  void WriteControls_p1_b2(ardt_informazioniBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(ardt_informazioniBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b3_CloseBand(ardt_informazioniBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(ardt_informazioniBO BO,ServletStatus status) {
  ...
  static void PrintState(ardt_informazioniBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_informazioniBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_informazioniBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_NUMPROG,"C",10,0));
  ...
  static void PrintStateDetailWorkVars(ardt_informazioniBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_informazioniBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_informazioniBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_informazioniBO.TrsRow) BO.m_oDeletedRows.get(k);
  ...
  void WriteMainFormVarScript(ardt_informazioniBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(344,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(774,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_informazioni"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_informazioni","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_informazioni",BO.GetCurrentState());
    ...
    status.out.println("this.PRGIMPSOG=arguments[10];");
    ...
    status.out.println("this.xRagSoc=arguments[16];");
    status.out.println("this.PROQUOTA=arguments[17];");
    status.out.println("this.CPROWNUM=arguments[18];");
    status.out.println("this.m_nRowStatus=arguments[19];");
    status.out.println("this.m_cOldRowCPCCCHK=arguments[20];");
    int lastRow;
    lastRow = 21;
    for ( l = 0; CPLib.lt(l,BO.m_MNTs.length); l = l + (1)) {
      if (BO.m_MNTs[l].RepeatedChild()) {
  ...
  void WriteMainFormScript(ardt_informazioniBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_informazioni);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_informazioni();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_informazioni',Ne(m_nChildStatus,0));");
    } else {
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('PCCSJPWDXE')) SetDisabled(c,true);");
        ...
        status.out.println("if(c=Ctrl('HKZAUFLHMG_ZOOM')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('NATIUIQLJX')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('NATIUIQLJX_ZOOM')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('KQHTAXPURL')) SetDisabled(c,true);");
    ...
    status.out.println("m_oFather.m_oardt_informazioni=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_informazioniBO getBO(ServletStatus status) {
    ardt_informazioniBO BO = new ardt_informazioniBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_informazioniBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("intestbo",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_informazioniBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_informazioniBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ardt_informazioniBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_intestbo;
    String l_cPhName = BO.m_cPhName_intestbo;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_informazioniBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_intestbo+".RAPPORTO"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_intestbo+".RAPPORTO"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("intestbo");
    String l_cPhName = CPSql.ManipulateTablePhName("intestbo",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_informazioniBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_intestbo),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_intestbo,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_intestbo+".RAPPORTO) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_intestbo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_intestbo,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_intestbo+".RAPPORTO",CPLib.GetTablesWithAliases(BO.m_cPhName_intestbo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_informazioniBO BO) {
  ...
  void Edit(ServletStatus status,ardt_informazioniBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_informazioniBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_informazioniBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_informazioniBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_informazioniBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_informazioniBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_informazioniBO BO,SPParameterSource source) {
    BO.Load(status.w_RAPPORTO);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_informazioniBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_informazioniBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_informazioniBO BO) {
    ...
    item.put("altInterfaceFor","ardt_informazioni");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_XCDXQXTRHM\",\"page\":1,\"spuid\":\"XCDXQXTRHM\",\"tabindex\":4,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"Inizio\",\"w\":64,\"x\":600,\"y\":2,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_BFMPSJTVZY\",\"page\":1,\"spuid\":\"BFMPSJTVZY\",\"tabindex\":3,\"type\":\"Label\",\"typevar\":\"logic\",\"value\":\"Intestatario\",\"w\":61,\"x\":24,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_QEAAMPSBPR\",\"page\":1,\"spuid\":\"QEAAMPSBPR\",\"tabindex\":5,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"Fine\",\"w\":64,\"x\":672,\"y\":2,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":182,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"ABACSKMKPI\",\"type\":\"BodyDetail\",\"w\":788,\"x\":4,\"y\":18,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_VRCVINYRJO\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"DATI ANAGRAFICI\",\"h\":50,\"name\":\"box_VRCVINYRJO\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VRCVINYRJO\",\"stretch\":true,\"tabindex\":8,\"title_caption\":\"DATI ANAGRAFICI\",\"titled\":true,\"type\":\"Box\",\"w\":728,\"x\":16,\"y\":207,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_NJCDHJUQPF\",\"page\":1,\"spuid\":\"NJCDHJUQPF\",\"tabindex\":15,\"type\":\"Label\",\"value\":\"% Pro Quota:\",\"w\":73,\"x\":630,\"y\":236,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"% Pro Quota\",\"fixed\":true,\"h\":20,\"maxlength\":4,\"name\":\"PROQUOTA\",\"page\":1,\"repeated\":true,\"spuid\":\"PCCSJPWDXE\",\"tabindex\":18,\"type\":\"Textbox\",\"typevar\":\"numeric\",\"w\":30,\"x\":704,\"y\":232,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NAKGRMOUXS\",\"page\":1,\"spuid\":\"NAKGRMOUXS\",\"tabindex\":9,\"type\":\"Label\",\"w\":648,\"x\":24,\"y\":212,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HMPQLYVATL\",\"page\":1,\"spuid\":\"HMPQLYVATL\",\"tabindex\":10,\"type\":\"Label\",\"w\":568,\"x\":24,\"y\":233,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_MINAAOAOZC\",\"box_type\":\"auto_horizontal_section\",\"collapsible\":true,\"h\":45,\"name\":\"box_MINAAOAOZC\",\"page\":1,\"shrinkable\":true,\"spuid\":\"MINAAOAOZC\",\"stretch\":true,\"tabindex\":12,\"type\":\"Box\",\"w\":728,\"x\":16,\"y\":259,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BHLAQHLOJQ\",\"page\":1,\"spuid\":\"BHLAQHLOJQ\",\"tabindex\":13,\"type\":\"Label\",\"value\":\"Prog.Informazione Apertura:\",\"w\":171,\"x\":24,\"y\":276,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_EQUZNNIXBO\",\"page\":1,\"spuid\":\"EQUZNNIXBO\",\"tabindex\":14,\"type\":\"Label\",\"value\":\"Prog.Informazione Chiusura:\",\"w\":163,\"x\":496,\"y\":276,\"zone\":\"pag1_4_2\",\"zonepath\":\"[1,4,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Prog.Informazione Chiusura\",\"disabled\":\"true\",\"fixed\":true,\"h\":23,\"maxlength\":11,\"name\":\"NUMPROG2\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"ORIJTEDVRI\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":664,\"y\":269,\"zone\":\"pag1_4_3\",\"zonepath\":\"[1,4,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Prog.Informazione Apertura\",\"disabled\":\"true\",\"fixed\":true,\"h\":23,\"maxlength\":11,\"name\":\"NUMPROG1\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"LEZBMECOAR\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":80,\"x\":200,\"y\":269,\"zone\":\"pag1_4_4\",\"zonepath\":\"[1,4,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":39,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"FRVAQHIWAP\",\"type\":\"Box\",\"w\":732,\"x\":4,\"y\":-1,\"zone\":\"pag1_5\",\"zonepath\":\"[1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Intestatario (Non Intermediario)\",\"h\":17,\"maxlength\":16,\"name\":\"CODINTER\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"XTDHYWJPZL\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":108,\"x\":24,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Soggetto collegato all"+"'"+"informazione corrente (pallino verde)\",\"h\":17,\"keep_proportions\":\"bounded\",\"name\":\"image_BKRVFLJLOS\",\"page\":1,\"repeated\":true,\"spuid\":\"BKRVFLJLOS\",\"tabindex\":19,\"type\":\"Image\",\"w\":16,\"x\":4,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"disabled\":\"true\",\"h\":17,\"maxlength\":70,\"name\":\"xRagSoc\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"VICYEWBISV\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":423,\"x\":156,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Fine\",\"h\":17,\"maxlength\":10,\"name\":\"DATAFINE\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"NATIUIQLJX\",\"tabindex\":3,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":64,\"x\":672,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Inizio\",\"h\":17,\"maxlength\":10,\"name\":\"DATAINI\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"HKZAUFLHMG\",\"tabindex\":2,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":64,\"x\":584,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_informazioniBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_informazioniBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_informazioni?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_informazioni_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_informazioni_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_RAPPORTO=a[5];");
      status.out.println("w_PRGIMPSOG=a[6];");
      status.out.println("w_STATOREG=a[7];");
      status.out.println("w_IDEREG=a[8];");
      status.out.println("w_TIPORAP=a[9];");
      status.out.println("w_NUMPROG2=a[10];");
      status.out.println("w_NUMPROG1=a[11];");
      status.out.println("w_PROQUOTA=a[12];");
      status.out.println("w_gAzienda=a[13];");
      status.out.println("w_xragsoci=a[14];");
      status.out.println("w_tipreg=a[15];");
      status.out.println("w_appoggio=a[16];");
      status.out.println("w_tiporegr=a[17];");
      status.out.println("w_DOMICILIO=a[18];");
      status.out.println("w_DESCCIT=a[19];");
      status.out.println("w_PROVINCIA=a[20];");
      status.out.println("w_CAP=a[21];");
      status.out.println("w_xRagSoc=a[22];");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_informazioni_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(344,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void RaiseLoadedEntity(ardt_informazioniBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_informazioniBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_informazioniBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_informazioniBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_informazioniBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_informazioniBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_informazioniBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ardt_informazioniBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ardt_informazioniBO BO) {
    boolean result = true;
  ...
  static class OrderBodyByComparator implements Comparator<ardt_informazioniBL.TrsRow> {
    public int compare(ardt_informazioniBL.TrsRow o1,ardt_informazioniBL.TrsRow o2) {
