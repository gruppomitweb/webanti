...
var c;
if(c=Ctrl('SEJWRQHUSP')) selectCombo(c,w_C4CODTAB,'C')
SPBOUILib.SetInputValue('XREVNLNKCY',function(){return WtH(w_C4CODICE,'C',2,0,'')},w_C4CODICE);
SPBOUILib.SetInputValue('RJDJPGMTCC',function(){return WtH(w_C4DESCRI,'C',80,0,'')},w_C4DESCRI);
if (m_bExtended) {
SetExtensions();
}
...
HideUI.lblids["C4CODICE"]=['EHORPFDFFQ'];
HideUI.lblids["C4CODTAB"]=['RGYCBVWYTV'];
HideUI.lblids["C4DESCRI"]=['UKQEHGTCZD'];
function SEJWRQHUSP_Valid(e) {
if (Eq(m_cFunction,'query')) {
if (IsLastInPk('SEJWRQHUSP') ||  ! (m_bQueryOnLatestKeyOnly)) {
DoQuery('C4CODTAB','C',w_C4CODTAB,HtW(getComboValue(Ctrl('SEJWRQHUSP')),'C'),m_bQueryOnLatestKeyOnly);
}
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_C4CODTAB(HtW(getComboValue(Ctrl('SEJWRQHUSP')),'C'),null,e);
return l_bSetResult;
}
}
...
DoQuery('C4CODICE','C',w_C4CODICE,HtW(Ctrl('XREVNLNKCY').value,'C'),m_bQueryOnLatestKeyOnly);
}
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_C4CODICE(HtW(Ctrl('XREVNLNKCY').value,'C'),null,e);
return l_bSetResult;
}
}
...
if (Eq(m_cFunction,'query')) {
DoQuery('C4DESCRI','C',w_C4DESCRI,HtW(Ctrl('RJDJPGMTCC').value,'C'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_C4DESCRI(HtW(Ctrl('RJDJPGMTCC').value,'C'),null,e);
return l_bSetResult;
}
}
...
function Help() {
windowOpenForeground('../doc/armt_tbcateg04_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
m_PrimaryKeys=['C4CODICE','C4CODTAB'];
}
function i_PrimaryKey() {
return 'tbcateg04'+'\\'+CPLib.ToCPStr(w_C4CODTAB)+'\\'+CPLib.ToCPStr(w_C4CODICE);
}
...
p_d.setValue('armt_tbcateg04',m_cSelectedPage);
...
_Obli('C4CODTAB',0,'SEJWRQHUSP',false,"10376019890")
_Obli('C4CODICE',0,'XREVNLNKCY',false,"112851763")
_Obli('C4DESCRI',0,'RJDJPGMTCC',false,"285808099")
