...
SetLocationHref(null,'arrt_faminvio'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cMode=hyperlink','elaborazione')
}
}
...
function LEQQQPGBLM_Valid(e) {
e=(Ne(e,null)?e:window.event);
return Set_flgnew(ChkboxValueAssign(Ctrl('LEQQQPGBLM'),'S','N','C',1,0,''),null,e);
}
function LEQQQPGBLM_OnFocus(e) {
SetActiveField(Ctrl('LEQQQPGBLM'),true);
}
function LEQQQPGBLM_OnBlur(e) {
SetActiveField(Ctrl('LEQQQPGBLM'),false);
}
...
function YPNISOYHTS_Valid(e) {
e=(Ne(e,null)?e:window.event);
return Set_flgdel(ChkboxValueAssign(Ctrl('YPNISOYHTS'),'S','N','C',1,0,''),null,e);
}
function YPNISOYHTS_OnFocus(e) {
SetActiveField(Ctrl('YPNISOYHTS'),true);
}
function YPNISOYHTS_OnBlur(e) {
SetActiveField(Ctrl('YPNISOYHTS'),false);
}
...
if(c=Ctrl('LEQQQPGBLM')) c.onclick=LEQQQPGBLM_Valid;
if(c=Ctrl('LEQQQPGBLM')) c.onfocus=LEQQQPGBLM_OnFocus;
if(c=Ctrl('LEQQQPGBLM')) c.onblur=LEQQQPGBLM_OnBlur;
...
if(c=Ctrl('YPNISOYHTS')) c.onclick=YPNISOYHTS_Valid;
if(c=Ctrl('YPNISOYHTS')) c.onfocus=YPNISOYHTS_OnFocus;
if(c=Ctrl('YPNISOYHTS')) c.onblur=YPNISOYHTS_OnBlur;
...
w_Messaggi.raiseEvent('arpg_faminvio_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_faminvio_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_faminvio_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_faminvio',m_cSelectedPage);
...
function Set_flgnew(ctrlValue,kmode,e) {
if (Ne(w_flgnew,ctrlValue)) {
var ctl = _GetCtl(e,'LEQQQPGBLM');
if (_tracker.goon(ctl,ctrlValue)) {
w_flgnew=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('LEQQQPGBLM',0);
DoUpdate(l_bResult);
} else {
...
var ctl = _GetCtl(e,'YPNISOYHTS');
if (_tracker.goon(ctl,ctrlValue)) {
w_flgdel=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('YPNISOYHTS',0);
DoUpdate(l_bResult);
} else {
ctl.value=WtH('N','C',1,0,'');
_ResetTracker();
...
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('LEQQQPGBLM',1))) {
_SignErr('LEQQQPGBLM');
m_cLastWorkVarErrorMsg='flgnew';
l_bResult=false;
w_flgnew='N';
} else if ( ! (_ChkObl('IXWESXTUQB',1))) {
_SignErr('IXWESXTUQB');
m_cLastWorkVarErrorMsg='flgagg';
l_bResult=false;
w_flgagg='N';
} else if ( ! (_ChkObl('YPNISOYHTS',1))) {
_SignErr('YPNISOYHTS');
...
_FillChildren.n=[];
FillWv.n=["gIntemediario","gDescAzi","flgnew","flgagg","flgdel","anno","mese","dadata","a_data","flgstp","gFlgWU","flgdett"];
...
_Obli('flgnew',0,'LEQQQPGBLM',false)
_Obli('flgagg',0,'IXWESXTUQB',false)
_Obli('flgdel',0,'YPNISOYHTS',false)
