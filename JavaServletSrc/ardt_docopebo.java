...
public class ardt_docopebo extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*8c20e431*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "docopebo";
    }
    public String w_NUMPROG;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_NUMPROG,context,"docopebo","NUMPROG","C",11,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_docopebo;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"docopebo","personbo","tbcitta","tbstati"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_docopeboBO BO) {
  ...
  void WriteStyles(ardt_docopeboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_docopebo.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_docopeboBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_docopebo',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_docopebo",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_docopeboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_docopeboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b0(ardt_docopeboBO BO,ServletStatus status) {
  ...
  void HTMLRowScript(ardt_docopeboBO BO,ServletStatus status) {
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder = null;
    status.out.println(
     "<script>" +
  ...
  static void PrintState(ardt_docopeboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_docopeboBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_docopeboBL BO,ServletStatus status) {
  ...
  static void PrintStateDetailWorkVars(ardt_docopeboBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_docopeboBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_docopeboBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_docopeboBO.TrsRow) BO.m_oDeletedRows.get(k);
  ...
  void WriteMainFormVarScript(ardt_docopeboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_docopebo"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_docopebo","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_docopebo",BO.GetCurrentState());
  ...
  void WriteMainFormScript(ardt_docopeboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_docopebo);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_docopebo();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_docopebo',Ne(m_nChildStatus,0));");
    }
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
    ...
    status.out.println("m_oFather.m_oardt_docopebo=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_docopeboBO getBO(ServletStatus status) {
    ardt_docopeboBO BO = new ardt_docopeboBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_docopeboBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("docopebo",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_docopeboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_docopeboBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
    l_cTmp = "[";
    if (l_rsLoad!=null)
  ...
  CPResultSet QueryKeyCursor(ardt_docopeboBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_docopebo;
    String l_cPhName = BO.m_cPhName_docopebo;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_docopeboBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_docopebo+".NUMPROG"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_docopebo+".NUMPROG"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("docopebo");
    String l_cPhName = CPSql.ManipulateTablePhName("docopebo",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_docopeboBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_docopebo),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_docopebo,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_docopebo+".NUMPROG) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_docopebo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_docopebo,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_docopebo+".NUMPROG",CPLib.GetTablesWithAliases(BO.m_cPhName_docopebo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_docopeboBO BO) {
  ...
  void Edit(ServletStatus status,ardt_docopeboBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_docopeboBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_docopeboBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_docopeboBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_docopeboBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_docopeboBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_docopeboBO BO,SPParameterSource source) {
    BO.Load(status.w_NUMPROG);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_docopeboBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_docopeboBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_docopeboBO BO) {
    ...
    item.put("altInterfaceFor","ardt_docopebo");
    items.put(item);
    String s;
    ...
    items.put(ctrl);
    s = "{\"h\":218,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"QZXRURKPBB\",\"type\":\"BodyDetail\",\"w\":632,\"x\":8,\"y\":26,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":39,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"PCDGZSDANW\",\"type\":\"Box\",\"w\":576,\"x\":8,\"y\":6,\"zone\":\"pag1_6\",\"zonepath\":\"[1,6]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_docopeboBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_docopeboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_docopebo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_docopebo_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_docopebo_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_docopebo_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(ardt_docopeboBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_docopeboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_docopeboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_docopeboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_docopeboBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_docopeboBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_docopeboBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ardt_docopeboBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ardt_docopeboBO BO) {
    boolean result = true;
