...
// * --- Fine Area Manuale
public class ardt_intestitBO extends ardt_intestitBL {
  ...
  // * --- Fine Area Manuale
  public ardt_intestitBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_intestit");
    }
  ...
  public boolean Load(String p_CODCLI) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_intestit,m_cPhName_intestit,m_cVirtName_intestit,m_cVirtName_intestit+".CODCLI="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODCLI,"C",16,0),m_cServer_intestit,p_CODCLI),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"intestit")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_intestit.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"CODINTER="+CPLib.ToSQL(opk_CODINTER,"C",16,0)+" AND CODCLI="+CPLib.ToSQL(w_CODCLI,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_intestit.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"intestit")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_intestit,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_intestit);
      l_cSql = "insert into "+m_oWrInfo_intestit.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_intestit.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"intestit")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_PRGIMPTITE,"N",10,0)+", ";
      ...
      l_cSql = l_cSql+m_oWrInfo_intestit.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"intestit")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_intestit,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_intestit);
      l_cSql = "Update "+m_oWrInfo_intestit.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+", PRGIMPTITE = "+CPLib.ToSQL(w_PRGIMPTITE,"N",10,0);
