...
var ctl = _GetCtl(e,'YJLEVWVVMX');
if (_tracker.goon(ctl,ctrlValue)) {
w_IMPSARA=ValidNum(ctrlValue,'9999999999999999.99');
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('YJLEVWVVMX',0);
DoUpdate(l_bResult);
...
var ctl = _GetCtl(e,'SKLGMQFXFQ');
if (_tracker.goon(ctl,ctrlValue)) {
w_PROQUOTA=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('SKLGMQFXFQ',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','N',3,0,'');
...
if ( ! (m_oFather.SaveContext_ardt_clientiope())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_clientiope_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_clientiope_dataNotSent',{'action':formaction});
}
}
}
}
...
l_bResult=false;
w_CODSOG=HtW('','C');
} else if (( ! (_ChkObl('SKLGMQFXFQ',1))) && FullRow()) {
_SignErr('SKLGMQFXFQ');
m_cLastWorkVarErrorMsg='11230801199';
l_bResult=false;
w_PROQUOTA=HtW('','N');
} else if (( ! (_ChkObl('YJLEVWVVMX',1))) && FullRow()) {
_SignErr('YJLEVWVVMX');
m_cLastWorkVarErrorMsg='10014753831';
l_bResult=false;
w_IMPSARA=HtW('','N');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_clientiope();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.PROQUOTA={"dec":0,"fixedpos":false,"id":"SKLGMQFXFQ","len":3,"name":"PROQUOTA","title":"Pro Quota","type":"N"};
l_aRepeatedFields.IMPSARA={"dec":2,"fixedpos":false,"id":"YJLEVWVVMX","len":19,"name":"IMPSARA","title":"Importo per Sara","type":"N"};
