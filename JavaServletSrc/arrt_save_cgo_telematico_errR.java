...
// * --- Fine Area Manuale
public class arrt_save_cgo_telematico_errR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_save_cgo_telematico_errR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_save_cgo_telematico_err",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_save_cgo_telematico_err";
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* Exec Routine arrt_writelog('VARIAZIONE','CGO_TELEMATICO_ERR',pSNAINUMOPE) */
      arrt_writelogR.Make(m_Ctx,this).Run("VARIAZIONE","CGO_TELEMATICO_ERR",pSNAINUMOPE);
      ...
      Cursor_cgo_telematico_err = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"SNAINUMOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(pSNAINUMOPE,"?",0,0,m_cServer, m_oParameters),m_cServer,pSNAINUMOPE)+" "+")"+(m_Ctx.IsSharedTemp("cgo_telematico_err")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
      while ( ! (Cursor_cgo_telematico_err.Eof())) {
        /* If cgo_telematico_err->FLGFULL='S' */
        if (CPLib.eqr(Cursor_cgo_telematico_err.GetString("FLGFULL"),"S")) {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arrt_save_cgo_telematico_err",25,"00000007")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
          m_cSql = CPLib.GetSQLPrefixComment("arrt_save_cgo_telematico_err",25,"00000008")+"delete from "+m_oWrInfo.GetTableWriteName();
          m_cSql = m_cSql+" where ";
      ...
      Cursor_cgo_telematico_err.Close();
      // * --- End Select
      /* Stop arpg_cgo_telematico_err */
      Forward f;
      f=new Forward("arpg_cgo_telematico_err",false,this,Forward.Start,false);
  ...
  public Forward Run(String p_pSNAINUMOPE) {
    pSNAINUMOPE = p_pSNAINUMOPE;
    return Run();
  }
  public Forward Run() {
  ...
  public static arrt_save_cgo_telematico_errR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_save_cgo_telematico_errR(p_Ctx, p_Caller);
  }
  public void Blank() {
