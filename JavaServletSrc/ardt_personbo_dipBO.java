...
// * --- Fine Area Manuale
public class ardt_personbo_dipBO extends ardt_personbo_dipBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_2_1.CODDIP AS CODDIP201";
        m_cSelect = m_cSelect+" ,link_2_1.DESCRIZ AS DESCRIZ201";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","CODDIPE","CODDIP");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","CODDIPE","CODDIP",true);
  ...
  // * --- Fine Area Manuale
  public ardt_personbo_dipBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_personbo_dip");
    }
  ...
  public boolean Load(String p_CONNES) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_personbo_dip,m_cPhName_personbo_dip,m_cVirtName_personbo_dip,m_cVirtName_personbo_dip+".CONNES="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CONNES,"C",16,0),m_cServer_personbo_dip,p_CONNES),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_1(m_cPhName_anadip,m_cServer_anadip);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"personbo_dip")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_personbo_dip.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"CONNES="+CPLib.ToSQL(w_CONNES,"C",16,0)+" AND CODDIPE="+CPLib.ToSQL(opk_CODDIPE,"C",6,0);
        l_cSql = l_cSql+m_oWrInfo_personbo_dip.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"personbo_dip")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_personbo_dip,l_cSql);
  ...
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"JSWFSTAHBP")) {
      ...
      result.AppendFieldsHash("JSWFSTAHBP",CPLib.GetLinkedFieldProps("CODDIP","CODDIPE","C",6,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_personbo_dip);
      l_cSql = "insert into "+m_oWrInfo_personbo_dip.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CONNES,";
      l_cSql = l_cSql+"CODDIPE,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_personbo_dip.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"personbo_dip")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_personbo_dip.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"personbo_dip")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_personbo_dip,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_personbo_dip);
      l_cSql = "Update "+m_oWrInfo_personbo_dip.GetTableWriteName()+" set";
      if (CPLib.HasTableCPCCCHK(m_Ctx,"personbo_dip")) {
