...
var ctl = _GetCtl(e,'FTBJRJWXQF');
if (_tracker.goon(ctl,ctrlValue)) {
w_CODICE=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('FTBJRJWXQF',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',3,0,'');
...
var ctl = _GetCtl(e,'GIWBRWPRYB');
if (_tracker.goon(ctl,ctrlValue)) {
w_DESCRI=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('GIWBRWPRYB',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
parent.ZtVWeb.raiseEventToEvalParms('armt_tbtipope_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_tbtipope_dataNotSent',{'action':formaction});
}
}
}
}
...
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('FTBJRJWXQF',1))) {
_SignErr('FTBJRJWXQF');
m_cLastWorkVarErrorMsg='112851763';
l_bResult=false;
w_CODICE=HtW('','C');
} else if ( ! (_ChkObl('GIWBRWPRYB',1))) {
_SignErr('GIWBRWPRYB');
