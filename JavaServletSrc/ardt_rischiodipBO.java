...
// * --- Fine Area Manuale
public class ardt_rischiodipBO extends ardt_rischiodipBL {
  ...
  // * --- Fine Area Manuale
  public ardt_rischiodipBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_rischiodip");
    }
  ...
  public boolean Load(String p_CODDIP) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_rischiodip,m_cPhName_rischiodip,m_cVirtName_rischiodip,m_cVirtName_rischiodip+".CODDIP="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODDIP,"C",6,0),m_cServer_rischiodip,p_CODDIP),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_TIPOANALISI = l_rsLoadBody.GetString("TIPOANALISI");
          w_DATAINI = l_rsLoadBody.GetDate("DATAINI");
          w_DATFIN = l_rsLoadBody.GetDate("DATFIN");
          w_RISGLOB = l_rsLoadBody.GetString("RISGLOB");
          // Assegna il contesto dei figli del dettaglio a Empty
          work_children = CPLib.BlankChildMNTs(m_MNTs,true);
          opk_TIPOANALISI = w_TIPOANALISI;
          opk_DATAINI = w_DATAINI;
          if (CPLib.HasTableCPCCCHK(m_Ctx,"rischiodip")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_rischiodip.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"CODDIP="+CPLib.ToSQL(w_CODDIP,"C",6,0)+" AND TIPOANALISI="+CPLib.ToSQL(opk_TIPOANALISI,"C",1,0)+" AND DATAINI="+CPLib.ToSQL(opk_DATAINI,"D",8,0);
        l_cSql = l_cSql+m_oWrInfo_rischiodip.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"rischiodip")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_rischiodip,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_rischiodip);
      l_cSql = "insert into "+m_oWrInfo_rischiodip.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODDIP,";
      l_cSql = l_cSql+"TIPOANALISI,";
      l_cSql = l_cSql+"DATAINI,";
      l_cSql = l_cSql+"DATFIN,";
      l_cSql = l_cSql+"RISGLOB,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_rischiodip.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"rischiodip")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_TIPOANALISI,"C",1,0)+", ";
      ...
      l_cSql = l_cSql+m_oWrInfo_rischiodip.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"rischiodip")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_rischiodip,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_rischiodip);
      l_cSql = "Update "+m_oWrInfo_rischiodip.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+", TIPOANALISI = "+CPLib.ToSQL(w_TIPOANALISI,"C",1,0);
