...
public class arpg_stp_stdtec_tit extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*CPFDMBIRJO*!";
  ...
  public String getEntityTitle() {
    return "Stampa Verifica Legami tra soggetti (Standard Tecnici fino al 31/12/2013)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_stp_stdtec_tit";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_stp_stdtec_titBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10454176095",p_Context.Translate("Stampa Verifica Legami tra soggetti (Standard Tecnici fino al 31/12/2013)")},
  ...
  void WriteStyles(arpg_stp_stdtec_titBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_stp_stdtec_tit.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_stp_stdtec_titBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_stp_stdtec_titBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_stp_stdtec_titBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(216);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_stp_stdtec_titBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_stp_stdtec_titBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_stp_stdtec_titBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_stp_stdtec_titBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_stp_stdtec_titBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_stp_stdtec_tit"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_stp_stdtec_tit","");
  ...
  void WriteMainFormScript(arpg_stp_stdtec_titBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_stp_stdtec_titBO getBO(ServletStatus status) {
    arpg_stp_stdtec_titBO BO = new arpg_stp_stdtec_titBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_stp_stdtec_titBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_stp_stdtec_titBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_stp_stdtec_titBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_stp_stdtec_titBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_stp_stdtec_titBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_stp_stdtec_titBO BO) {
    ...
    item.put("altInterfaceFor","arpg_stp_stdtec_tit");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_XURLGQZZMU()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"XURLGQZZMU\",\"src\":\"images\\/print.gif\",\"tabindex\":\"8\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"288\",\"y\":\"171\",\"zone\":\"XLVFHSOSYH_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_PTXYMSWBZO()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"PTXYMSWBZO\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"9\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"384\",\"y\":\"171\",\"zone\":\"XLVFHSOSYH_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void Next(ServletStatus status,arpg_stp_stdtec_titBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_stp_stdtec_titBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_stp_stdtec_titBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_stp_stdtec_tit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_stp_stdtec_tit_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_stp_stdtec_titBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_stp_stdtec_titBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_stp_stdtec_titBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_stp_stdtec_titBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_stp_stdtec_titBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_stp_stdtec_titBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_stp_stdtec_tit.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_stp_stdtec_titBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_stp_stdtec_titBO BO) {
    boolean result = true;
    return result;
  }
}
