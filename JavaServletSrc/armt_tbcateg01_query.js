...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SEJWRQHUSP').value,'C'),w_C1CODTAB)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('XREVNLNKCY').value,'C'),w_C1CODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbcateg01',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'C1CODTAB') &&  ! (Empty(HtW(Ctrl('SEJWRQHUSP').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.C1CODTAB',HtW(Ctrl('SEJWRQHUSP').value,'C'),'C');
}
if (Ne(name,'C1CODICE') &&  ! (Empty(HtW(Ctrl('XREVNLNKCY').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.C1CODICE',HtW(Ctrl('XREVNLNKCY').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tbcateg01');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbcateg01',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
