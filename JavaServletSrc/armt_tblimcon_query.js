...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('WSIVJMMEOM').value,'C'),w_LCCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tblimcon',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'LCCODICE') &&  ! (Empty(HtW(Ctrl('WSIVJMMEOM').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.LCCODICE',HtW(Ctrl('WSIVJMMEOM').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tblimcon');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tblimcon',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['WSIVJMMEOM'],i=0;i>=0;i--){
