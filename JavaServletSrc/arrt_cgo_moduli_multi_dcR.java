...
// * --- Fine Area Manuale
public class arrt_cgo_moduli_multi_dcR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_moduli_multi_dcR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_moduli_multi_dc",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_cgo_moduli_multi_dc";
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* _coddip := '' */
      _coddip = "";
      // * --- Read from cgo_operazioni_dl_multi_dc
      ...
      if (m_Ctx.IsSharedTemp("cgo_operazioni_dl_multi_dc")) {
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on cgo_operazioni_dl_multi_dc into routine arrt_cgo_moduli_multi_dc returns two or more records. This item should return only a record.");
      ...
      if (CPLib.eqr(_coddip,gCodDip)) {
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on anadip into routine arrt_cgo_moduli_multi_dc returns two or more records. This item should return only a record.");
            CPStdCounter.Error(l_oErrorFault);
          }
          Read_Cursor.Close();
        } else {
        ...
        while ( ! (Cursor_cgo_operazioni_dl_multi_dc.Eof())) {
          /* w_CODFISC := cgo_operazioni_dl_multi_dc->CODFISC // Codice Fiscale */
          w_CODFISC = Cursor_cgo_operazioni_dl_multi_dc.GetString("CODFISC");
          /* w_CODDIPE := cgo_operazioni_dl_multi_dc->CODDIPE // Codice Dipendenza */
          w_CODDIPE = Cursor_cgo_operazioni_dl_multi_dc.GetString("CODDIPE");
          /* w_CAIDBIGLIETTO := cgo_operazioni_dl_multi_dc->CAIDBIGLIETTO // ID BIGLIETTO */
          w_CAIDBIGLIETTO = Cursor_cgo_operazioni_dl_multi_dc.GetString("CAIDBIGLIETTO");
          ...
          Cursor_cgo_operazioni_dl_multi_dt_dc = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"SNAINUMOPE="+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("SNAINUMOPE"),"?",0,0)+" "+")"+(m_Ctx.IsSharedTemp("cgo_operazioni_dl_multi_dt_dc")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
          while ( ! (Cursor_cgo_operazioni_dl_multi_dt_dc.Eof())) {
            /* w_MIDBIGLIETTO := w_MIDBIGLIETTO+cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO+"-" // ID BIGLIETTO */
            w_MIDBIGLIETTO = w_MIDBIGLIETTO+Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO")+"-";
            /* Case conta2 = 1 */
            if (CPLib.eqr(conta2,1)) {
              /* w_MIDBIGLIETTO1 := w_MIDBIGLIETTO1 + iif(not(Empty(LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO))),LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO)+" - ",'') // ID BIGLIETTO */
              w_MIDBIGLIETTO1 = w_MIDBIGLIETTO1+( ! (CPLib.Empty(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))))?CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))+" - ":"");
              /* Case conta2 = 2 */
            } else if (CPLib.eqr(conta2,2)) {
              /* w_MIDBIGLIETTO2 := w_MIDBIGLIETTO2 + iif(not(Empty(LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO))),LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO)+" - ",'') // ID BIGLIETTO */
              w_MIDBIGLIETTO2 = w_MIDBIGLIETTO2+( ! (CPLib.Empty(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))))?CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))+" - ":"");
              /* Case conta2 = 3 */
            } else if (CPLib.eqr(conta2,3)) {
              /* w_MIDBIGLIETTO3 := w_MIDBIGLIETTO3 + iif(not(Empty(LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO))),LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO)+" - ",'') // ID BIGLIETTO */
              w_MIDBIGLIETTO3 = w_MIDBIGLIETTO3+( ! (CPLib.Empty(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))))?CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))+" - ":"");
              /* Case conta2 = 4 */
            } else if (CPLib.eqr(conta2,4)) {
              /* w_MIDBIGLIETTO4 := w_MIDBIGLIETTO4 + iif(not(Empty(LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO))),LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO)+" - ",'') // ID BIGLIETTO */
              w_MIDBIGLIETTO4 = w_MIDBIGLIETTO4+( ! (CPLib.Empty(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))))?CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))+" - ":"");
            } // End Case
            /* w_MIDBIGLIETTO := w_MIDBIGLIETTO + iif(not(Empty(LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO))),LRTrim(cgo_operazioni_dl_multi_dt_dc->CAIDBIGLIETTO)+" - ",'') // ID BIGLIETTO */
            w_MIDBIGLIETTO = w_MIDBIGLIETTO+( ! (CPLib.Empty(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))))?CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt_dc.GetString("CAIDBIGLIETTO"))+" - ":"");
            /* conta := conta + 1 */
          ...
          /* Case cgo_operazioni_dl_multi_dc->MEZPAGAM='A' */
          if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"A")) {
            /* _mpagam := 'Altro Mezzo di Pagamento' */
            _mpagam = "Altro Mezzo di Pagamento";
            /* Case cgo_operazioni_dl_multi_dc->MEZPAGAM='I' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"I")) {
            /* _mpagam := 'Bonifico Italia' */
            _mpagam = "Bonifico Italia";
            /* Case cgo_operazioni_dl_multi_dc->MEZPAGAM='E' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"E")) {
            /* _mpagam := 'Bonifico Estero' */
            _mpagam = "Bonifico Estero";
            /* Case cgo_operazioni_dl_multi_dc->MEZPAGAM='P' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"P")) {
            /* _mpagam := 'Bonifico Postale' */
            _mpagam = "Bonifico Postale";
            /* Case cgo_operazioni_dl_multi_dc->MEZPAGAM='C' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"C")) {
            /* _mpagam := 'Assegno Circolare' */
            _mpagam = "Assegno Circolare";
            /* Case cgo_operazioni_dl_multi_dc->MEZPAGAM='B' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"B")) {
            /* _mpagam := 'Assegno Bancario' */
            _mpagam = "Assegno Bancario";
            /* Case cgo_operazioni_dl_multi_dc->MEZPAGAM='V' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"V")) {
            /* _mpagam := 'Carta di Credito' */
            _mpagam = "Carta di Credito";
            /* Case cgo_operazioni_dl_multi_dc->MEZPAGAM='S' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"S")) {
            /* _mpagam := 'Contante' */
            _mpagam = "Contante";
            /* Case cgo_operazioni_dl_multi_dc->MEZPAGAM='W' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"W")) {
            /* _mpagam := 'SnayPay Wallet' */
            _mpagam = "SnayPay Wallet";
          } // End Case
          /* _tipgio := arfn_cgo_tbtipgio(cgo_operazioni_dl_multi_dc->TIPOGIOCO,cgo_operazioni_dl_multi_dc->VLTPLAT) */
          _tipgio = arfn_cgo_tbtipgioR.Make(m_Ctx,this).Run(Cursor_cgo_operazioni_dl_multi_dc.GetString("TIPOGIOCO"),Cursor_cgo_operazioni_dl_multi_dc.GetString("VLTPLAT"));
          /* If cgo_operazioni_dl_multi_dc->MEZPAGAM='S' or cgo_operazioni_dl_multi_dc->MEZPAGAM='W' */
          if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"S") || CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"W")) {
            /* _pagam := LRTrim(_mpagam) + ' € ' +LRTrim(Str(cgo_operazioni_dl_multi_dc->TOTLIRE,15,2)) */
            _pagam = CPLib.LRTrim(_mpagam)+" \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("TOTLIRE"),15,2));
          } else { // Else
            /* If cgo_operazioni_dl_multi_dc->TOTLIRE - (cgo_operazioni_dl_multi_dc->TOTCONT + cgo_operazioni_dl_multi_dc->IMPWALLET)=0 */
            if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("TOTLIRE")-(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("TOTCONT")+Cursor_cgo_operazioni_dl_multi_dc.GetDouble("IMPWALLET")),0)) {
              /* _pagam := iif(cgo_operazioni_dl_multi_dc->FLAGCONT='1','Contanti € '+LRTrim(Str(cgo_operazioni_dl_multi_dc->TOTCONT,15,2)),'') */
              _pagam = (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("FLAGCONT"),"1")?"Contanti \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("TOTCONT"),15,2)):"");
              /* Case cgo_operazioni_dl_multi_dc->FLAGWALLET='1' and cgo_operazioni_dl_multi_dc->FLAGCONT='1' */
              if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("FLAGWALLET"),"1") && CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("FLAGCONT"),"1")) {
                /* _pagam := _pagam + ' - SnaiPay Wallet € '+LRTrim(Str(cgo_operazioni_dl_multi_dc->IMPWALLET,15,2)) */
                _pagam = _pagam+" - SnaiPay Wallet \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("IMPWALLET"),15,2));
                /* Case cgo_operazioni_dl_multi_dc->FLAGWALLET='1' and cgo_operazioni_dl_multi_dc->FLAGCONT='0' */
              } else if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("FLAGWALLET"),"1") && CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("FLAGCONT"),"0")) {
                /* _pagam := 'SnaiPay Wallet € '+LRTrim(Str(cgo_operazioni_dl_multi_dc->IMPWALLET,15,2)) */
                _pagam = "SnaiPay Wallet \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("IMPWALLET"),15,2));
              } // End Case
            } else { // Else
              /* _pagam := LRTrim(_mpagam) + ' € ' +LRTrim(Str((cgo_operazioni_dl_multi_dc->TOTLIRE - cgo_operazioni_dl_multi_dc->TOTCONT  - cgo_operazioni_dl_multi_dc->IMPWALLET),15,2)) */
              _pagam = CPLib.LRTrim(_mpagam)+" \u20ac "+CPLib.LRTrim(CPLib.Str((Cursor_cgo_operazioni_dl_multi_dc.GetDouble("TOTLIRE")-Cursor_cgo_operazioni_dl_multi_dc.GetDouble("TOTCONT")-Cursor_cgo_operazioni_dl_multi_dc.GetDouble("IMPWALLET")),15,2));
              /* _pagam := _pagam + iif(cgo_operazioni_dl_multi_dc->FLAGCONT='1',' - Contanti € '+LRTrim(Str(cgo_operazioni_dl_multi_dc->TOTCONT,15,2)),'') */
              _pagam = _pagam+(CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("FLAGCONT"),"1")?" - Contanti \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("TOTCONT"),15,2)):"");
              /* _pagam := _pagam + iif(cgo_operazioni_dl_multi_dc->FLAGWALLET='1',' - SnaiPay Wallet € '+LRTrim(Str(cgo_operazioni_dl_multi_dc->IMPWALLET,15,2)),'') */
              _pagam = _pagam+(CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("FLAGWALLET"),"1")?" - SnaiPay Wallet \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("IMPWALLET"),15,2)):"");
            } // End If
          } // End If
          /* _desaut := '' */
          _desaut = "";
          ...
          m_cSql = "";
          m_cSql = m_cSql+"ARCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("RDCODAUT"),"C",2,0,m_cServer),m_cServer,Cursor_cgo_operazioni_dl_multi_dc.GetString("RDCODAUT"));
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on cgo_autorita into routine arrt_cgo_moduli_multi_dc returns two or more records. This item should return only a record.");
              CPStdCounter.Error(l_oErrorFault);
            }
            Read_Cursor.Close();
          } else {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_moduli_multi_dc",99,"0000006C")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          m_cSql = m_cSql+GetFieldsName_0000006C(m_Ctx,m_oWrInfo);
          m_cSql = m_cSql+") values (";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("ATTIV"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("AUTRILASC"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("C_CAB"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("C_CAP"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("C_CTA"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("C_IND"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("C_PRV"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("C_RAG"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("C_STA"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("CAMBIO"),"?",0,0)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("CODANA"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("CODCAB"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("CODCABDIP"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("CODDIPE"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("CODFISC"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("CODINTER"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("CODVOC"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("COGNOME"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("COLLEG"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("CONNESBEN"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("CONNESCLI"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetDate("DATAREG"),"?",0,0)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("DESCCITDIP"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("FLAGFRAZ"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("NUMPROG"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("PROVINCIADIP"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("SEGNO"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("SNAINUMOPE"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("SOTGRUP"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("STATOREG"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("TIPOOPRAP"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dc.GetString("VALUTA"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("CAFLGOPE"),"V")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("CAFLGOPE"),"G")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("CAFLGOPE"),"P")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("CAFLGOPE"),"R")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"I") || CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"E") || CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"P")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"B") || CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"C")?"X":""),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"V")?"X":""),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("FLAGCONT"),"1") || CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dc.GetString("MEZPAGAM"),"S")?"X":""),"?",0,0,m_cServer)+", ";
        ...
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_moduli_multi_dc",99,"00000070")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
  ...
  public static arrt_cgo_moduli_multi_dcR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_moduli_multi_dcR(p_Ctx, p_Caller);
  }
  public static arrt_cgo_moduli_multi_dcR Make(CPContext p_Ctx) {
    return new arrt_cgo_moduli_multi_dcR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_cgo_tbtipgio,arrt_cgo_moduli_multi_dc,
  public static final String i_InvokedRoutines = ",arfn_cgo_tbtipgio,arrt_cgo_moduli_multi_dc,";
