...
// * --- Fine Area Manuale
public class arrt_cgo_moduli_newR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_moduli_newR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_moduli_new",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on cgo_operazioni_dl into routine arrt_cgo_moduli_new returns two or more records. This item should return only a record.");
      ...
      if (CPLib.eqr(_coddip,gCodDip)) {
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on anadip into routine arrt_cgo_moduli_new returns two or more records. This item should return only a record.");
            CPStdCounter.Error(l_oErrorFault);
          }
          Read_Cursor.Close();
        } else {
        ...
        while ( ! (Cursor_cgo_operazioni_dl.Eof())) {
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on cgo_autorita into routine arrt_cgo_moduli_new returns two or more records. This item should return only a record.");
              CPStdCounter.Error(l_oErrorFault);
            }
            Read_Cursor.Close();
          } else {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_moduli_new",89,"0000004A")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(w_CAIDBIGLIETTO,"?",0,0,m_cServer, m_oParameters)+", ";
        ...
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_moduli_new",89,"0000004E")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
  ...
  public static arrt_cgo_moduli_newR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_moduli_newR(p_Ctx, p_Caller);
  }
  public static arrt_cgo_moduli_newR Make(CPContext p_Ctx) {
    return new arrt_cgo_moduli_newR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_cgo_tbtipgio,arrt_cgo_moduli_new,
  public static final String i_InvokedRoutines = ",arfn_cgo_tbtipgio,arrt_cgo_moduli_new,";
