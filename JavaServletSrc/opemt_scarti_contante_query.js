...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('VEWEVUUXXI').value,'C'),w_NUMPROG)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('scarto_contante',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'NUMPROG') &&  ! (Empty(HtW(Ctrl('VEWEVUUXXI').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.NUMPROG',HtW(Ctrl('VEWEVUUXXI').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','opemt_scarti_contante');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('scarto_contante',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['VEWEVUUXXI'],i=0;i>=0;i--){
