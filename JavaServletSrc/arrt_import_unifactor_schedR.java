...
// * --- Fine Area Manuale
public class arrt_import_unifactor_schedR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_import_unifactor_schedR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_import_unifactor_sched",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_import_unifactor_sched";
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      while ( ! (Cursor_intermbo.Eof())) {
        ...
        /* _portmail := Val(LRTrim(intermbo->ALTPRTMAIL)) */
        _portmail = CPLib.Round(CPLib.Val(CPLib.LRTrim(Cursor_intermbo.GetString("ALTPRTMAIL"))),0);
      ...
      /* _nomefile := LRTrim(f[_i].getName().toString()) */
      _nomefile = CPLib.LRTrim(f[_i].getName().toString());
      /* mcListaFile.AppendBlank() */
      mcListaFile.AppendBlank();
      /* mcListaFile.nomefile := _nomefile */
      mcListaFile.row.nomefile = _nomefile;
      /* mcListaFile.SaveRow() */
      mcListaFile.SaveRow();
      /* FileLibMit.MoveFile(LRTrim(_pathimport)+"//"+LRTrim(_nomefile),LRTrim(gPathDoc)+"/appo/"+LRTrim(gAzienda)+"/"+LRTrim(_nomefile)) */
      FileLibMit.MoveFile(CPLib.LRTrim(_pathimport)+"//"+CPLib.LRTrim(_nomefile),CPLib.LRTrim(gPathDoc)+"/appo/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(_nomefile));
      // Chiude il ciclo
      }
      /* If mcListaFile.RecCount() > 0 */
      if (CPLib.gt(mcListaFile.RecCount(),0)) {
        /* _logmsg := arrt_import_unifactor(mcListaFile) */
        _logmsg = arrt_import_unifactorR.Make(m_Ctx,this).Run(mcListaFile);
        /* If not(Empty(_servermail)) */
        if ( ! (CPLib.Empty(_servermail))) {
          try {
  ...
  public static arrt_import_unifactor_schedR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_import_unifactor_schedR(p_Ctx, p_Caller);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arrt_import_unifactor,
  public static final String i_InvokedRoutines = ",arrt_import_unifactor,";
