...
// * --- Fine Area Manuale
public class arrt_trasffra_age_selR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_trasffra_age_selR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_trasffra_age_sel",m_Caller);
  ...
  void Page_1() throws Exception {
    CPResultSet Cursor_qbe_trasffra_age=null;
    ...
    try {
      ...
      // * --- Select from qbe_trasffra_age
      if (Cursor_qbe_trasffra_age!=null)
        Cursor_qbe_trasffra_age.Close();
      Cursor_qbe_trasffra_age = new VQRHolder("qbe_trasffra_age",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),this,true).GetResultSet(m_Ctx);
      while ( ! (Cursor_qbe_trasffra_age.Eof())) {
        /* _agente := '' */
        _agente = "";
        // * --- Read from anadip
        ...
        m_cSql = "";
        m_cSql = m_cSql+"CODDIP="+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_qbe_trasffra_age.GetString("CODDIPE"),"C",6,0,m_cServer),m_cServer,Cursor_qbe_trasffra_age.GetString("CODDIPE"));
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on anadip into routine arrt_trasffra_age_sel returns two or more records. This item should return only a record.");
        ...
        if (CPLib.eqr(_agente,"S")) {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arrt_trasffra_age_sel",17,"00000015")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          m_cSql = m_cSql+GetFieldsName_00000015(m_Ctx,m_oWrInfo);
          m_cSql = m_cSql+") values (";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetString("NUMPROG"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetDate("DATAOPE"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetString("RAPPORTO"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetString("VALUTA"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetDouble("TOTLIRE")/100,"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetDouble("TOTCONT")/100,"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetString("CONNESCLI"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL("S","?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetString("IDBASE"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetDate("DATAREG"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasffra_age.GetString("CODDIPE"),"?",0,0,m_cServer)+", ";
        ...
        Cursor_qbe_trasffra_age.Next();
      }
      m_cConnectivityError = Cursor_qbe_trasffra_age.ErrorMessage();
      Cursor_qbe_trasffra_age.Close();
      // * --- End Select
      /* Return  */
      Forward f;
      f=new Forward("arpg_trasffra_age",false,this,Forward.Start,false);
  ...
  public static arrt_trasffra_age_selR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_trasffra_age_selR(p_Ctx, p_Caller);
  }
  public void Blank() {
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_trasffra_age,
  public static final String m_cVQRList = ",qbe_trasffra_age,";
  // ENTITY_BATCHES: ,
