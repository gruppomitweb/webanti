...
public class arpg_viewinfoprv extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*NOEWPBRGVQ*!";
  ...
  public String getEntityTitle() {
    return "Informazioni (Provvisorie)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_viewinfoprv";
    ...
    l_eg.m_cQueryName = "qbe_viewinfoprv";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_viewinfoprvBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"12074447034",p_Context.Translate("Informazioni (Provvisorie)")},
    {"1723736132",p_Context.Translate("Tipo Rapporto")},
    {"36228043",p_Context.Translate("N° Informazione")}};
    return l_translations;
  }
  void WriteStyles(arpg_viewinfoprvBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_viewinfoprv.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_viewinfoprvBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_viewinfoprvBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_viewinfoprvBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_viewinfoprvBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_viewinfoprvBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_viewinfoprvBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_viewinfoprvBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_viewinfoprvBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_viewinfoprv"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_viewinfoprv","");
  ...
  void WriteMainFormScript(arpg_viewinfoprvBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_viewinfoprvBO getBO(ServletStatus status) {
    arpg_viewinfoprvBO BO = new arpg_viewinfoprvBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_viewinfoprvBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_viewinfoprvBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_viewinfoprvBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_viewinfoprvBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_viewinfoprvBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_viewinfoprvBO BO) {
    ...
    item.put("altInterfaceFor","arpg_viewinfoprv");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_viewinfoprvBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_viewinfoprvBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_viewinfoprvBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_viewinfoprvBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_viewinfoprvBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_viewinfoprvBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_viewinfoprv?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_viewinfoprv_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_viewinfoprvBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_viewinfoprvBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_viewinfoprvBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_viewinfoprvBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_viewinfoprvBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_viewinfoprvBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_viewinfoprvBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_viewinfoprvBO BO) {
    boolean result = true;
    return result;
  }
}
