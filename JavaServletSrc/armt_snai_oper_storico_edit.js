...
function DisplayErrorMessage_armt_cgo_qadv_operazioni_storico() {
if ( ! (ChildToLoad(FrameRef('armt_cgo_qadv_operazioni_storico'))) && Ne(typeof(FrameRef('armt_cgo_qadv_operazioni_storico').DisplayErrorMessage),'undefined')) {
FrameRef('armt_cgo_qadv_operazioni_storico').DisplayErrorMessage();
}
}
...
if ( ! (ChildToLoad(FrameRef('armt_cgo_qadv_operazioni_storico'))) && Ne(typeof(FrameRef('armt_cgo_qadv_operazioni_storico').EnableEntity),'undefined')) {
FrameRef('armt_cgo_qadv_operazioni_storico').EnableEntity(l_bEnabled);
}
...
DisableInputsInContainer(Ctrl('RWCKHDGZVX'),true);
}
...
w_FILTROMP=(Ne(w_TOTCONT,w_TOTLIRE)?'\'I\',\'E\',\'P\',\'B\',\'C\',\'V\'':'\'I\',\'E\',\'P\',\'B\',\'C\',\'V\',\'S\'');
}
if ((Ne(o_TOTLIRE,w_TOTLIRE)) || (Ne(o_TOTCONT,w_TOTCONT)) || (Ne(o_FILTROMP,w_FILTROMP)) || (Ne(o_FILTROMP,w_FILTROMP)) || (Ne(o_FLGPAGAM,w_FLGPAGAM))) {
...
w_edtdoc=Empty(w_IDDOCALL) || Empty(w_TIPOGIOCO) || Le(w_TOTLIRE,0) || (Eq(w_FLAGCONT,'1') && Le(w_TOTCONT,0));
w_AUTRILASC=Upper(LRTrim(w_xARDESCRI)+' '+LRTrim(w_RDLOCAUT));
if ((Ne(o_idcitta2,w_idcitta2))) {
l_bTmpRes=Link_PSLKLXOQMT(null);
}
...
w_MEZPAGAM=(Eq(w_TOTLIRE,w_TOTCONT)?'S':'');
}
function Calculation_DIXETYMKJA() {
arrt_cgo_setpagatore_s([["pObj",this]])
}
...
function Link_PSLKLXOQMT(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
...
function Link_PSLKLXOQMT_Blank() {
w_idcitta2='';
w_PROVINCIA2='';
w_CAP2='';
}
function LOpt_PSLKLXOQMT() {
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('tbcitta','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
l_oWv.setValue('PKFields',WtA('IDBASE','C'));
l_oWv.setValue('LinkedField',WtA('IDBASE','C'));
l_oWv.setValue('UID',WtA('PSLKLXOQMT','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['PSLKLXOQMT'][1],'C'));
return l_oWv;
}
...
parent.ZtVWeb.raiseEventToEvalParms('armt_snai_oper_storico_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_snai_oper_storico_dataNotSent',{'action':formaction});
}
}
}
}
CheckChild.child=[{'prg':"ardt_cgo_oper_storico_multi",'BOScId':function(){ return FrameRef('ardt_cgo_oper_storico_multi')},'repeated':0},{'prg':"armt_cgo_qadv_operazioni_storico",'BOScId':function(){ return FrameRef('armt_cgo_qadv_operazioni_storico')},'repeated':0}];
function Check() {
var l_bResult = true;
var l_bChildCheck = true;
ResetErrorVars();
m_nLastError=0;
if ((false) && (( ! (_ChkObl('HUAGBHPUNH',1))) || ( ! (Gt(w_TOTLIRE,0) && Ge(w_TOTLIRE,(w_TOTCONT+w_IMPWALLET)))))) {
_SignErr('HUAGBHPUNH');
m_cLastWorkVarErrorMsg='1825772649';
l_bResult=false;
w_TOTLIRE=HtW('','N');
} else if ((Eq(w_FLAGCONT,'1')) && (( ! (_ChkObl('HUDOCMLKKT',1))) || ( ! (arfn_chktotalelire_cont(w_DATAOPE,w_datpaseuro,w_TOTCONT,w_TOTLIRE))))) {
...
l_oWv.setValue('FLGSBLOCCO',WtA(w_FLGSBLOCCO,'C'));
...
_FillChildren.n=['ardt_cgo_oper_storico_multi','armt_cgo_qadv_operazioni_storico'];
FillWv.n=["TIPOGIOCO","TIPOPERS","CGOFLGVNP","TOTLIRE","TOTCONT","IMPWALLET","CODWALLET","CODTRANSZ","CGOFLGVLT","MEZPAGAM","CC4CIFRE","CCMESSCA","CCANNSCA","IBAN","ASSEGNO","CFESTERO","PEP","CODFISC","COGNOME","NOME","NASCOMUN","TIPINTER","SESSO","NASSTATO","DATANASC","PAESE","RAGSOC","DESCCIT","PROVINCIA","CAP","CODCAB","DOMICILIO","NUMTEL","ATTIV","SOTGRUP","RAMOGRUP","SETTSINT","SNAINUMOPE","DATAOPE","DATAREG","FLAGCONT","FLAGWALLET","TIPOOPRAP","CODANA","CODVOC","VALUTA","CAMBIO","CONNESCLI","CONNESBEN","C_RAG","C_CTA","C_PRV","C_CAP","C_CAB","C_IND","C_STA","CODINTER","CODDIPE","DESCCITDIP","FLAGFRAZ","COLLEG","PROVINCIADIP","CODCABDIP","CACODICE","CAIDBIGLIETTO","CGOTIPOPE","NUMOPEDL","CAFLGOPE","IDDOCALL","FLGDOCALL","VLTPLAT","CCDATSCA","TIPODOC","NUMDOCUM","DATARILASC","DATAVALI","RDCODAUT","RDLOCAUT","AUTRILASC","PEPDESCRI","PAESE2","DESCCIT2","PROVINCIA2","CAP2","DOMICILIO2","FLGDOM","FLGSBLOCCO","SEGNO","PROFESSIONE","gIntemediario","xSOGTPGIO","xDEFTPGIO","xEDTTPGIO","xEDTSPGIO","xDEFSPGIO","FLGPAGAM","FILTROMP","idcitta","xARDESCRI","dessta","tipsot","STATOREG","IDBASE","NUMPROG","DESCRI","gSeekAos","gDataVaria","gFlgDoc","xDescDipe","datpaseuro","xdes1","desccauana","CITINT","CITDIP","PROVDIP","CABDIP","PROVINT","CABINT","xdesccausin","gChkDate","xdesctipatt","gVLT","gVNP","gCodDip","verifica","gStpDwn","MIDBIGLIETTO","MIDBIGLIETTO1","MIDBIGLIETTO2","MIDBIGLIETTO3","MIDBIGLIETTO4","TELDIP","CODLOC","xFLGPLGIO","xVPFLGATT","DOC03","DOC04","tipop","gGesDoc","edtdoc","saveddoc","idcitta2","dessta2","xCGOFLGPROF"];
...
return [LoadContext(0,'CJPXRAUTXZ'),'ardt_cgo_oper_storico_multi'];
}
}
function SetChildVariables_armt_cgo_qadv_operazioni_storico() {
}
function CtxLoad_armt_cgo_qadv_operazioni_storico() {
if (ChildToLoad(FrameRef('armt_cgo_qadv_operazioni_storico'))) {
return [LoadContext(0,'FGDNXNYKKB'),'armt_cgo_qadv_operazioni_storico'];
}
}
