...
if(c=Ctrl('OUJUYXLCZS')) selectCombo(c,w_scelta,'N')
...
return Set_scelta(HtW(getComboValue(Ctrl('OUJUYXLCZS')),'N'),null,e);
}
function OUJUYXLCZS_OnFocus(e) {
SetActiveField(Ctrl('OUJUYXLCZS'),true);
}
function OUJUYXLCZS_OnBlur(e) {
SetActiveField(Ctrl('OUJUYXLCZS'),false);
}
...
if(c=Ctrl('OUJUYXLCZS')) c.onchange=OUJUYXLCZS_Valid;
if(c=Ctrl('OUJUYXLCZS')) c.onfocus=OUJUYXLCZS_OnFocus;
if(c=Ctrl('OUJUYXLCZS')) c.onblur=OUJUYXLCZS_OnBlur;
}
}
function NotifyEvent(p_cEvent) {
var old_c;
if (Ne(typeof(w_Messaggi),'undefined')) {
w_Messaggi.raiseEvent('arpg_esiti_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_esiti_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_ListaFile),'undefined')) {
w_ListaFile.raiseEvent('arpg_esiti_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_esiti_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_esiti',m_cSelectedPage);
...
SetDisabled(Ctrl('OUJUYXLCZS'),l_bEnabled);
l_bEnabled= ! (Eq(w_okstampe,1));
