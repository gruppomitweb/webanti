...
// * --- Fine Area Manuale
public class arrt_calccodfisR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_calccodfisR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_calccodfis",m_Caller);
      ...
      if (CPLib.eqr(nocf,0)) {
        ...
        if ( ! (CPLib.Empty(CPLib.LRTrim(w_NASSTATO))) && (CPLib.ne(CPLib.LRTrim(w_NASSTATO),"ITALIA") && CPLib.ne(CPLib.LRTrim(w_NASSTATO),"ITALY"))) {
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on tbstatna into routine arrt_calccodfis returns two or more records. This item should return only a record.");
          ...
          if (CPLib.Empty(cSigla)) {
            ...
            m_cSql = "";
            m_cSql = m_cSql+"DESCRI="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.Upper(CPLib.LRTrim(w_NASSTATO)),"C",40,0,m_cServer),m_cServer,CPLib.Upper(CPLib.LRTrim(w_NASSTATO)));
            ...
            if ( ! (Read_Cursor.Eof())) {
              ...
              Read_Cursor.Next();
              if ( ! (Read_Cursor.Eof())) {
                Error l_oErrorFault = new Error("Read on tbstati into routine arrt_calccodfis returns two or more records. This item should return only a record.");
                CPStdCounter.Error(l_oErrorFault);
              }
              Read_Cursor.Close();
            } else {
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on tbcittna into routine arrt_calccodfis returns two or more records. This item should return only a record.");
              CPStdCounter.Error(l_oErrorFault);
            }
            Read_Cursor.Close();
          } else {
          ...
          if (CPLib.Empty(cSigla)) {
            ...
            if ( ! (Read_Cursor.Eof())) {
              ...
              Read_Cursor.Next();
              if ( ! (Read_Cursor.Eof())) {
                Error l_oErrorFault = new Error("Read on tbcitta into routine arrt_calccodfis returns two or more records. This item should return only a record.");
                CPStdCounter.Error(l_oErrorFault);
              }
              Read_Cursor.Close();
            } else {
        ...
        Page_5();
        /* w_CODFISC := cCog + cNom + Anno + Mese + Giorno + iif(Empty(cSiglaN),cSigla,cSiglaN) + cCheck // Codice Fiscale */
        w_CODFISC = cCog+cNom+Anno+Mese+Giorno+(CPLib.Empty(cSiglaN)?cSigla:cSiglaN)+cCheck;
      } // End If
    } finally {
      try {
        if (Cursor_intermbo!=null)
  ...
  public static arrt_calccodfisR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_calccodfisR(p_Ctx, p_Caller);
  }
  public void Blank() {
