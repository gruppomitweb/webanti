...
public class arrt_inviomese extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*d67d6232*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_datprf;
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw__errore;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_datprf = source.Has("w_datprf");
      if (m_bpassedw_datprf) {
        w_datprf = source.GetParameter("w_datprf",source.GetParameter("datprf",CPLib.NullDate()));
      } else {
        if (source.IsForwarded()) {
          w_datprf = source.GetCaller().GetDate("datprf","D",8,0);
          m_bpassedw_datprf = true;
        } else {
          w_datprf = source.GetParameter("datprf",source.GetParameter("w_datprf",CPLib.NullDate()));
        }
      }
      ...
      m_bpassedw__errore = source.Has("w__errore");
      if (m_bpassedw__errore) {
        w__errore = source.GetParameter("w__errore",source.GetParameter("_errore",""));
      } else {
        if (source.IsForwarded()) {
          w__errore = source.GetCaller().GetString("_errore","C",120,0);
          m_bpassedw__errore = true;
        } else {
          w__errore = source.GetParameter("_errore",source.GetParameter("w__errore",""));
        }
      }
    }
  }
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw__errore) ||  ! (status.m_bpassedw_bloccaedit) ||  ! (status.m_bpassedw_seco) ||  ! (status.m_bpassedw_iniz) ||  ! (status.m_bpassedw_datseci) ||  ! (status.m_bpassedw_datsecf) ||  ! (status.m_bpassedw_datpri) ||  ! (status.m_bpassedw_datprf) ||  ! (status.m_bpassedw_dadata) ||  ! (status.m_bpassedw_a_data);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_datprf)) {
      status.out.println("l_cV=WtA(FetchDate(l_oWnd('w_datprf',"+SPLib.ToJSValue(status.w_datprf,"D",8,0)+")),'D');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_datprf,"D",8,0)+",'D');");
    }
    status.out.println("l_oWv.setValue('w_datprf',l_cV);");
    ...
    if ( ! (status.m_bpassedw__errore)) {
      status.out.println("l_cV=WtA(l_oWnd('w__errore',"+SPLib.ToJSValue(status.w__errore,"C",120,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w__errore,"C",120,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w__errore',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_inviomeseR Rcalled;
    Rcalled = new arrt_inviomeseR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_inviomeseR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_inviomeseR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_inviomese"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_datprf="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_datprf),"?",0,0)+"");
        ...
        status.out.println("js:w__errore="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w__errore),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_datprf,null)) {");
        status.out.println(" window[f].w_datprf.setTime(("+SPLib.ToJSValue(status.w_datprf,"D",0,0)+").getTime())");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_datpri,null)) {");
        status.out.println(" window[f].w_datpri.setTime(("+SPLib.ToJSValue(status.w_datpri,"D",0,0)+").getTime())");
        status.out.println("}");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w__errore,null)) {");
        status.out.println("window[f].w__errore="+SPLib.ToJSValue(status.w__errore,"C",0,0)+";");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_inviomese");
      if ( ! (CPLib.IsNull(l))) {
