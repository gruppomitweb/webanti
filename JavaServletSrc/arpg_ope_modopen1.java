...
public class arpg_ope_modopen1 extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*RLQFPMVCVT*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("olddatac","D",8,0, "hide",""),
                   new SPItem("newdatec","D",8,0, "hide",""),
                   new SPItem("oldappo","C",16,0, "hide",""),
                   new SPItem("idesito","C",8,0, "edit","ID Esito AGE")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_ope_modopen1BO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1795952569",p_Context.Translate("ID Esito AGE")},
  ...
  void WriteStyles(arpg_ope_modopen1BO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_ope_modopen1.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_ope_modopen1BO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_ope_modopen1BO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_ope_modopen1BO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(504);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_ope_modopen1BO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<option value="+SPLib.ToHTMLValue("","C",0,0)+">" +
     "</option>" +
     "<option value="+SPLib.ToHTMLValue("1","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("1",CPLib.Trim(BO.w_xsesso))?"SELECTED":"")+">" +
     ""+status.context.Translate("Maschile")+"" +
     "</option>" +
     "<option value="+SPLib.ToHTMLValue("2","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("2",CPLib.Trim(BO.w_xsesso))?"SELECTED":"")+">" +
    ...
    status.out.println(
     ...
     "<label id='AWATMAIZMU'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("MODIFICA DATI PER SCARTO OPERAZIONE DA ENTRATEL ?"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input name='idesito' id='PSISLKDZBG' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_idesito,"C",8,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='8' size='8' fieldtype='C'>" +
    "");
    status.out.println(
     "<span class='SPLabel' id='NLAWUZNXPY_DIV'>" +
     "<label id='NLAWUZNXPY' for='PSISLKDZBG'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("ID Esito AGE:"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.themeWriter.Header_EndHtml();
  }
  static void PrintState(arpg_ope_modopen1BL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_ope_modopen1BL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_ope_modopen1BL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_ope_modopen1BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_ope_modopen1"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_ope_modopen1","");
  ...
  void WriteMainFormScript(arpg_ope_modopen1BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_ope_modopen1");
    ...
    status.out.println("	  alert('Obbligatorio specificare Data Prima Operazione ExtraConto')");
    status.out.println("	  return");
    status.out.println("  }");
    ...
    status.out.println("    SendData('start',PlatformPathStart('arrt_ope_modopen1')+'',HParApplet().asString());");
    status.out.println("  }  ");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
  ...
  arpg_ope_modopen1BO getBO(ServletStatus status) {
    arpg_ope_modopen1BO BO = new arpg_ope_modopen1BO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_ope_modopen1BO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_ope_modopen1BO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_ope_modopen1BO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_ope_modopen1BO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_ope_modopen1BO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_ope_modopen1BO BO) {
    ...
    item.put("altInterfaceFor","arpg_ope_modopen1");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Sesso\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"name\":\"xsesso\",\"page\":\"1\",\"spuid\":\"VHDBEIXHTA\",\"tabindex\":\"11\",\"textlist\":\"Maschile,Femminile\",\"type\":\"Combobox\",\"valuelist\":\""+"'"+"1"+"'"+","+"'"+"2"+"'"+"\",\"w\":\"96\",\"x\":\"560\",\"y\":\"185\",\"zone\":\"ODCPZZXLHY_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Nuova_Apertura_\",\"page\":\"1\",\"spuid\":\"OLVLBDQTYW_DIV\",\"tabindex\":\"12\",\"type\":\"Label\",\"value\":\"Nuova Apertura:\",\"w\":\"116\",\"x\":\"448\",\"y\":\"39\",\"zone\":\"ODCPZZXLHY_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"newdate\",\"page\":\"1\",\"spuid\":\"WYJHXYCTSP\",\"tabindex\":\"13\",\"type\":\"Textbox\",\"w\":\"78\",\"x\":\"568\",\"y\":\"35\",\"zone\":\"ODCPZZXLHY_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Apertura_\",\"page\":\"1\",\"spuid\":\"NRLLUEOMLT_DIV\",\"tabindex\":\"14\",\"type\":\"Label\",\"value\":\"Data Apertura:\",\"w\":\"108\",\"x\":\"240\",\"y\":\"39\",\"zone\":\"ODCPZZXLHY_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Nuovo_Intestatario_\",\"page\":\"1\",\"spuid\":\"ISRGWEYPBE_DIV\",\"tabindex\":\"15\",\"type\":\"Label\",\"value\":\"Nuovo Intestatario:\",\"w\":\"110\",\"x\":\"22\",\"y\":\"441\",\"zone\":\"QEMAGEFKQK_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"newintest\",\"page\":\"1\",\"spuid\":\"NOGBUOIVWB\",\"tabindex\":\"16\",\"type\":\"Textbox\",\"w\":\"118\",\"x\":\"136\",\"y\":\"437\",\"zone\":\"QEMAGEFKQK_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_xragnew\",\"page\":\"1\",\"spuid\":\"EHGZXQREHH_DIV\",\"tabindex\":\"18\",\"type\":\"Label\",\"value\":\"w_xragnew\",\"w\":\"376\",\"x\":\"280\",\"y\":\"441\",\"zone\":\"QEMAGEFKQK_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Vecchio_Intestatario_\",\"page\":\"1\",\"spuid\":\"YMQGLGBXMV_DIV\",\"tabindex\":\"19\",\"type\":\"Label\",\"value\":\"Vecchio Intestatario:\",\"w\":\"118\",\"x\":\"14\",\"y\":\"116\",\"zone\":\"ODCPZZXLHY_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_xragold\",\"page\":\"1\",\"spuid\":\"PUHCDZJWVF_DIV\",\"tabindex\":\"20\",\"type\":\"Label\",\"value\":\"w_xragold\",\"w\":\"384\",\"x\":\"280\",\"y\":\"116\",\"zone\":\"ODCPZZXLHY_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"19\",\"name\":\"xtipopers\",\"orientation\":\"horizontal\",\"page\":\"1\",\"spuid\":\"FRGFQEIHFB\",\"tabindex\":\"21\",\"textlist\":\"Persona Fisica,Persona Giuridica\",\"type\":\"Radio\",\"valuelist\":\""+"'"+"P"+"'"+","+"'"+"D"+"'"+"\",\"w\":\"214\",\"x\":\"136\",\"y\":\"140\",\"zone\":\"ODCPZZXLHY_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_YBBLCLWLHJ()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"YBBLCLWLHJ\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"22\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"344\",\"y\":\"469\",\"zone\":\"ODCPZZXLHY_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui aggiornamento dati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_MDWFLPZWFN()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"MDWFLPZWFN\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"23\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"296\",\"y\":\"469\",\"zone\":\"ODCPZZXLHY_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"19\",\"name\":\"tipochg\",\"orientation\":\"horizontal\",\"page\":\"1\",\"spuid\":\"ZSRVHHSRRD\",\"tabindex\":\"28\",\"textlist\":\"Modifica dati Intestatario,Cambio Intestatario\",\"type\":\"Radio\",\"valuelist\":\""+"'"+"M"+"'"+","+"'"+"C"+"'"+"\",\"w\":\"279\",\"x\":\"136\",\"y\":\"91\",\"zone\":\"ODCPZZXLHY_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"88\",\"name\":\"Box_VYHZGJKBSG\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"VYHZGJKBSG\",\"tabindex\":\"29\",\"type\":\"Box\",\"w\":\"648\",\"x\":\"16\",\"y\":\"206\",\"zone\":\"ODCPZZXLHY_11\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Modifica_Dati_Intestatario__Persona_Fisica_\",\"page\":\"1\",\"spuid\":\"HMTQCDJJDS_DIV\",\"tabindex\":\"30\",\"type\":\"Label\",\"value\":\"Modifica Dati Intestatario (Persona Fisica)\",\"w\":\"238\",\"x\":\"16\",\"y\":\"189\",\"zone\":\"ODCPZZXLHY_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Codice_Fiscale_\",\"page\":\"1\",\"spuid\":\"AJUVDYOZZL_DIV\",\"tabindex\":\"31\",\"type\":\"Label\",\"value\":\"Codice Fiscale:\",\"w\":\"72\",\"x\":\"60\",\"y\":\"165\",\"zone\":\"ODCPZZXLHY_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Cognome_\",\"page\":\"1\",\"spuid\":\"SSUWRDQRDI_DIV\",\"tabindex\":\"32\",\"type\":\"Label\",\"value\":\"Cognome:\",\"w\":\"49\",\"x\":\"83\",\"y\":\"217\",\"zone\":\"VYHZGJKBSG_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Nome_\",\"page\":\"1\",\"spuid\":\"HHYAFDYFPB_DIV\",\"tabindex\":\"33\",\"type\":\"Label\",\"value\":\"Nome:\",\"w\":\"31\",\"x\":\"317\",\"y\":\"217\",\"zone\":\"VYHZGJKBSG_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Sesso_\",\"page\":\"1\",\"spuid\":\"HAZSNHXYTH_DIV\",\"tabindex\":\"34\",\"type\":\"Label\",\"value\":\"Sesso:\",\"w\":\"32\",\"x\":\"524\",\"y\":\"217\",\"zone\":\"VYHZGJKBSG_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Comune_di_Nascita_\",\"page\":\"1\",\"spuid\":\"DFMOVYUECJ_DIV\",\"tabindex\":\"35\",\"type\":\"Label\",\"value\":\"Comune di Nascita:\",\"w\":\"92\",\"x\":\"40\",\"y\":\"242\",\"zone\":\"VYHZGJKBSG_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Luogo di Nascita\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"40\",\"name\":\"xnascomun\",\"page\":\"1\",\"spuid\":\"HBSWFKBFZJ\",\"tabindex\":\"36\",\"type\":\"Textbox\",\"w\":\"262\",\"x\":\"136\",\"y\":\"238\",\"zone\":\"VYHZGJKBSG_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Provincia__EE_se_estero_\",\"page\":\"1\",\"spuid\":\"WHBMDRJRIX_DIV\",\"tabindex\":\"37\",\"type\":\"Label\",\"value\":\"Provincia (EE se estero)\",\"w\":\"114\",\"x\":\"425\",\"y\":\"241\",\"zone\":\"VYHZGJKBSG_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Provincia\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"2\",\"name\":\"xtipinter\",\"page\":\"1\",\"spuid\":\"MPXBUDNRPY\",\"tabindex\":\"38\",\"type\":\"Textbox\",\"w\":\"32\",\"x\":\"544\",\"y\":\"238\",\"zone\":\"VYHZGJKBSG_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Stato_di_Nascita_\",\"page\":\"1\",\"spuid\":\"RNCFPZEWNP_DIV\",\"tabindex\":\"39\",\"type\":\"Label\",\"value\":\"Stato di Nascita:\",\"w\":\"92\",\"x\":\"40\",\"y\":\"266\",\"zone\":\"VYHZGJKBSG_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stato di nascita\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"40\",\"name\":\"xnasstato\",\"page\":\"1\",\"spuid\":\"YDHXQFESNA\",\"tabindex\":\"40\",\"type\":\"Textbox\",\"w\":\"262\",\"x\":\"136\",\"y\":\"262\",\"zone\":\"VYHZGJKBSG_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_di_Nascita_\",\"page\":\"1\",\"spuid\":\"OSVHVNFSTF_DIV\",\"tabindex\":\"41\",\"type\":\"Label\",\"value\":\"Data di Nascita:\",\"w\":\"76\",\"x\":\"463\",\"y\":\"266\",\"zone\":\"VYHZGJKBSG_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Data Nascita\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"xdatnasc\",\"page\":\"1\",\"spuid\":\"SBDBIBIUBZ\",\"tabindex\":\"42\",\"type\":\"Textbox\",\"w\":\"72\",\"x\":\"544\",\"y\":\"262\",\"zone\":\"VYHZGJKBSG_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"93\",\"name\":\"Box_OQKDUTOEEM\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"OQKDUTOEEM\",\"tabindex\":\"43\",\"type\":\"Box\",\"w\":\"648\",\"x\":\"16\",\"y\":\"318\",\"zone\":\"ODCPZZXLHY_13\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Ragione Sociale\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"70\",\"name\":\"xragold\",\"page\":\"1\",\"spuid\":\"ZAOZVSXTQF\",\"tabindex\":\"44\",\"type\":\"Textbox\",\"w\":\"438\",\"x\":\"136\",\"y\":\"325\",\"zone\":\"OQKDUTOEEM_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Modifica_Dati_Intestatario__Persona_Giuridica_\",\"page\":\"1\",\"spuid\":\"TMGNSIMJWZ_DIV\",\"tabindex\":\"45\",\"type\":\"Label\",\"value\":\"Modifica Dati Intestatario (Persona Giuridica)\",\"w\":\"256\",\"x\":\"16\",\"y\":\"301\",\"zone\":\"ODCPZZXLHY_12\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Citt\\u00E0\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"40\",\"name\":\"xdesccit\",\"page\":\"1\",\"spuid\":\"WFUJRKYFYT\",\"tabindex\":\"46\",\"type\":\"Textbox\",\"w\":\"262\",\"x\":\"136\",\"y\":\"353\",\"zone\":\"OQKDUTOEEM_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Ragione_Sociale_\",\"page\":\"1\",\"spuid\":\"DECZZZAVTS_DIV\",\"tabindex\":\"47\",\"type\":\"Label\",\"value\":\"Ragione Sociale:\",\"w\":\"79\",\"x\":\"53\",\"y\":\"329\",\"zone\":\"OQKDUTOEEM_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Citt\\u00E0_Sede_Legale_\",\"page\":\"1\",\"spuid\":\"FPPPNGTASY_DIV\",\"tabindex\":\"48\",\"type\":\"Label\",\"value\":\"Citt\\u00E0 Sede Legale:\",\"w\":\"88\",\"x\":\"44\",\"y\":\"357\",\"zone\":\"OQKDUTOEEM_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Provincia\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"2\",\"name\":\"xprovincia\",\"page\":\"1\",\"spuid\":\"QAZVAJDCHQ\",\"tabindex\":\"49\",\"type\":\"Textbox\",\"w\":\"32\",\"x\":\"480\",\"y\":\"353\",\"zone\":\"OQKDUTOEEM_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"CAP\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"9\",\"name\":\"xcap\",\"page\":\"1\",\"spuid\":\"ZRHYPFBGWI\",\"tabindex\":\"50\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"568\",\"y\":\"325\",\"zone\":\"OQKDUTOEEM_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Provincia_\",\"page\":\"1\",\"spuid\":\"SXBLVGTGMP_DIV\",\"tabindex\":\"51\",\"type\":\"Label\",\"value\":\"Provincia:\",\"w\":\"47\",\"x\":\"428\",\"y\":\"357\",\"zone\":\"OQKDUTOEEM_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Nazione__UIC__\",\"page\":\"1\",\"spuid\":\"HWPUVGUGYG_DIV\",\"tabindex\":\"52\",\"type\":\"Label\",\"value\":\"Nazione (UIC):\",\"w\":\"71\",\"x\":\"61\",\"y\":\"385\",\"zone\":\"OQKDUTOEEM_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stato\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"3\",\"name\":\"xpaese\",\"page\":\"1\",\"spuid\":\"CIECFYJCOG\",\"tabindex\":\"53\",\"type\":\"Textbox\",\"w\":\"30\",\"x\":\"136\",\"y\":\"381\",\"zone\":\"OQKDUTOEEM_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"name\":\"Box_QEMAGEFKQK\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"QEMAGEFKQK\",\"tabindex\":\"54\",\"type\":\"Box\",\"w\":\"648\",\"x\":\"16\",\"y\":\"433\",\"zone\":\"ODCPZZXLHY_15\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Cambio_Intestatario\",\"page\":\"1\",\"spuid\":\"AFTPFXBXCR_DIV\",\"tabindex\":\"55\",\"type\":\"Label\",\"value\":\"Cambio Intestatario\",\"w\":\"256\",\"x\":\"16\",\"y\":\"416\",\"zone\":\"ODCPZZXLHY_14\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_xdessta\",\"page\":\"1\",\"spuid\":\"WSJMXQEYEG_DIV\",\"tabindex\":\"57\",\"type\":\"Label\",\"value\":\"w_xdessta\",\"w\":\"384\",\"x\":\"192\",\"y\":\"385\",\"zone\":\"OQKDUTOEEM_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"MODIFICA_DATI_PER_SCARTO_OPERAZIONE_DA_ENTRATEL__\",\"page\":\"1\",\"spuid\":\"AWATMAIZMU_DIV\",\"tabindex\":\"58\",\"type\":\"Label\",\"value\":\"MODIFICA DATI PER SCARTO OPERAZIONE DA ENTRATEL ?\",\"w\":\"322\",\"x\":\"11\",\"y\":\"15\",\"zone\":\"ODCPZZXLHY_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"C_A_P__\",\"page\":\"1\",\"spuid\":\"XZISIOGTIO_DIV\",\"tabindex\":\"59\",\"type\":\"Label\",\"value\":\"C.A.P.:\",\"w\":\"47\",\"x\":\"520\",\"y\":\"357\",\"zone\":\"OQKDUTOEEM_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"ID Esito AGE\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"8\",\"name\":\"idesito\",\"page\":\"1\",\"spuid\":\"PSISLKDZBG\",\"tabindex\":\"64\",\"type\":\"Textbox\",\"w\":\"80\",\"x\":\"136\",\"y\":\"63\",\"zone\":\"ODCPZZXLHY_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"ID_Esito_AGE_\",\"page\":\"1\",\"spuid\":\"NLAWUZNXPY_DIV\",\"tabindex\":\"65\",\"type\":\"Label\",\"value\":\"ID Esito AGE:\",\"w\":\"70\",\"x\":\"66\",\"y\":\"67\",\"zone\":\"ODCPZZXLHY_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_ope_modopen1BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_ope_modopen1BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_ope_modopen1BO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_ope_modopen1?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_ope_modopen1_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    l_cJsUid = SPPrxycizer.proxycizedPath("arrt_ope_modopen1.js");
    status.out.println(
     "<script src='../"+l_cJsUid+"' type='text/javascript'>" +
     "</script>" +
    "");
  ...
  void RaiseLoadedEntity(arpg_ope_modopen1BO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_ope_modopen1BO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_ope_modopen1BO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_ope_modopen1BO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_ope_modopen1BO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_ope_modopen1BO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      CPLib.AddToSet(callable,"arrt_ope_modopen1");
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_ope_modopen1BO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_ope_modopen1BO BO) {
    boolean result = true;
    return result;
  }
}
