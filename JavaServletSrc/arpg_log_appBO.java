...
// * --- Fine Area Manuale
public class arpg_log_appBO extends arpg_log_appBL {
  ...
  public arpg_log_appBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_LOGDATORA = rs.GetDateTime("LOGDATORA");
          c_LOGUTENTE = rs.GetString("LOGUTENTE");
          c_LOGTIPOPE = rs.GetString("LOGTIPOPE");
          c_LOGTABELLA = rs.GetString("LOGTABELLA");
          c_LOGCHIAVE = rs.GetString("LOGCHIAVE");
          c_LOGTESTO = rs.GetString("LOGTESTO");
          c_code = rs.GetDouble("code");
