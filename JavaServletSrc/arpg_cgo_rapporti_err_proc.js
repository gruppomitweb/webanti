...
SetHypPar('pPRGRAP',AsAppletValue(c_PRGRAP));
SetHypPar('m_cParameterSequence',AsAppletValue('pPRGRAP'));
SetHypPar('m_cID',AsAppletValue(m_IDS['arrt_cgo_rapporti_err_load']));
SendData('start','arrt_cgo_rapporti_err_load'+'',HParApplet().asString());
}
_ResetTracker();
...
function Help() {
windowOpenForeground('../doc/arpg_cgo_rapporti_err_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_cgo_rapporti_err',m_cSelectedPage);
...
function TrsQueryRow() {
  this.c_PRGRAP = arguments[0]
  this.c_RAPPORTO = arguments[1]
  this.c_DESCRAP = arguments[2]
  this.c_DATAOPE = arguments[3]
  this.c_CODINTEST = arguments[4]
  this.c_TIPOMOV = arguments[5]
}
function TrsToWork() {
var row;
if (Lt(m_nCurrentRow,m_oTrsQuery.length)) {
row=m_oTrsQuery[m_nCurrentRow];
c_PRGRAP=row.c_PRGRAP;
c_RAPPORTO=row.c_RAPPORTO;
c_DESCRAP=row.c_DESCRAP;
c_DATAOPE=row.c_DATAOPE;
c_CODINTEST=row.c_CODINTEST;
c_TIPOMOV=row.c_TIPOMOV;
}
}
function WorkToTrs() {
var row;
if (Lt(m_nCurrentRow,m_oTrsQuery.length)) {
row=m_oTrsQuery[m_nCurrentRow];
row.c_PRGRAP=c_PRGRAP;
row.c_RAPPORTO=c_RAPPORTO;
row.c_DESCRAP=c_DESCRAP;
row.c_DATAOPE=c_DATAOPE;
row.c_CODINTEST=c_CODINTEST;
row.c_TIPOMOV=c_TIPOMOV;
}
}
...
GroupFooter_BeginHtml(document,1,'1399249802'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'1399249802'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'1432804234'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_0B9A23B8"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
 "<span class=' SPLabel KRLVDZKIGO_DIV'>" +
 "<label class='KRLVDZKIGO'>" +
 ""+ToHTag(Translate("1017725727"))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'1432804234'+l_nIdx+'','A')
} else {
}
Row_BeginHtml(document,l_nEvenOdd,'1600576345'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_0B94C040"+l_nIdx+"' style='position:relative;height:18px'>" +
"")
document.writeln(
...
"")
document.writeln(
 "<span class=' SPLabel SZDHRCOJST_DIV'>" +
 "<label class='SZDHRCOJST'>" +
 ""+ToHTag(WtH((Eq(c_TIPOMOV,'A')?'APERTURA':(Eq(c_TIPOMOV,'C')?'CHIUSURA':'NON SPECIFICATO')),'C',15,0,''))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'1600576345'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
