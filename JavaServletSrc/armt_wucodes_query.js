...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('WXGOCNXMMY').value,'C'),w_WUCODSTA)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('wucodes',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'WUCODSTA') &&  ! (Empty(HtW(Ctrl('WXGOCNXMMY').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.WUCODSTA',HtW(Ctrl('WXGOCNXMMY').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_wucodes');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wucodes',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['WXGOCNXMMY'],i=0;i>=0;i--){
