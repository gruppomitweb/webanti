...
// * --- Fine Area Manuale
public class armt_tbspecie_pBO extends armt_tbspecie_pBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbspecie_pBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbspecie_p");
    }
  ...
  public static String[] m_cLoadParameterNames = {"CODSPE"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_CODSPE) {
    CPResultSet l_rsLoad=null;
    SqlLoader l_oSqlLoader;
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbspecie_p,m_cPhName_tbspecie_p,m_cVirtName_tbspecie_p,m_cVirtName_tbspecie_p+".CODSPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODSPE,"C",4,0),m_cServer_tbspecie_p,p_CODSPE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        /*  Assegna i valori letti dal record appena caricato nelle variabili di work.
                                    ...
                                    degli item.
                                  */
        w_CODSPE = l_rsLoad.GetString("CODSPE");
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbspecie_p")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbspecie_p);
      l_cSql = "insert into "+m_oWrInfo_tbspecie_p.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODSPE,";
      l_cSql = l_cSql+"RISCHIO,";
      l_cSql = l_cSql+"FLGPERS,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbspecie_p.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbspecie_p")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbspecie_p,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbspecie_p);
      l_cSql = "Update "+m_oWrInfo_tbspecie_p.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODSPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODSPE,"C",4,0),m_cServer_tbspecie_p,w_CODSPE);
      l_cSql = l_cSql+m_oWrInfo_tbspecie_p.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbspecie_p")) {
  ...
  public boolean Delete(String p_CODSPE) {
    String l_cSql;
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbspecie_p.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODSPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODSPE,"C",4,0),m_cServer_tbspecie_p,w_CODSPE);
          l_cSql = l_cSql+m_oWrInfo_tbspecie_p.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbspecie_p")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbspecie_p,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
