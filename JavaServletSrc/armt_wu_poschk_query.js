...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('BUQHTQZEWF').value,'C'),w_CPCODMAG)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('PUGRYTGEHQ').value,'C'),w_CPCODAJW)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('wuposchk',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CPCODMAG') &&  ! (Empty(HtW(Ctrl('BUQHTQZEWF').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CPCODMAG',HtW(Ctrl('BUQHTQZEWF').value,'C'),'C');
}
if (Ne(name,'CPCODAJW') &&  ! (Empty(HtW(Ctrl('PUGRYTGEHQ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CPCODAJW',HtW(Ctrl('PUGRYTGEHQ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_wu_poschk');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wuposchk',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['BUQHTQZEWF','PUGRYTGEHQ'],i=1;i>=0;i--){
