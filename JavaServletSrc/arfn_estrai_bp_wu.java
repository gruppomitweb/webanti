...
public class arfn_estrai_bp_wu extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*4e0dce73*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arfn_estrai_bp_wuR Rcalled;
    Rcalled = new arfn_estrai_bp_wuR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aa_retval = Rcalled.Run(status.pAnno,status.pMese);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arfn_estrai_bp_wuR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arfn_estrai_bp_wuR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arfn_estrai_bp_wu"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
