...
// * --- Fine Area Manuale
public class arrt_cgo_moduliR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_moduliR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_moduli",m_Caller);
  ...
  public double GetNumber(String p_cVarName,String p_cType,int len,int dec) {
    ...
    if (CPLib.eqr("IMPWALLET",p_cVarName)) {
      return w_IMPWALLET;
    }
    return 0;
  }
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    ...
    if (CPLib.eqr("ASSEGNO",p_cVarName)) {
      return w_ASSEGNO;
    }
    if (CPLib.eqr("VLTPLAT",p_cVarName)) {
      return w_VLTPLAT;
    }
    if (CPLib.eqr("CC4CIFRE",p_cVarName)) {
      return w_CC4CIFRE;
    }
    if (CPLib.eqr("CCDATSCA",p_cVarName)) {
      return w_CCDATSCA;
    }
    if (CPLib.eqr("NUMTEL",p_cVarName)) {
      return w_NUMTEL;
    }
    if (CPLib.eqr("TIPOGIOCO",p_cVarName)) {
      return w_TIPOGIOCO;
    }
    if (CPLib.eqr("RDCODAUT",p_cVarName)) {
      return w_RDCODAUT;
    }
    if (CPLib.eqr("RDLOCAUT",p_cVarName)) {
      return w_RDLOCAUT;
    }
    if (CPLib.eqr("CGOFLGVLT",p_cVarName)) {
      return w_CGOFLGVLT;
    }
    if (CPLib.eqr("PEPDESCRI",p_cVarName)) {
      return w_PEPDESCRI;
    }
    if (CPLib.eqr("FLGDOM",p_cVarName)) {
      return w_FLGDOM;
    }
    if (CPLib.eqr("PAESE2",p_cVarName)) {
      return w_PAESE2;
    }
    if (CPLib.eqr("DOMICILIO2",p_cVarName)) {
      return w_DOMICILIO2;
    }
    if (CPLib.eqr("PROVINCIA2",p_cVarName)) {
      return w_PROVINCIA2;
    }
    if (CPLib.eqr("DESCCIT2",p_cVarName)) {
      return w_DESCCIT2;
    }
    if (CPLib.eqr("CAP2",p_cVarName)) {
      return w_CAP2;
    }
    if (CPLib.eqr("PROFESSIONE",p_cVarName)) {
      return w_PROFESSIONE;
    }
    if (CPLib.eqr("CODWALLET",p_cVarName)) {
      return w_CODWALLET;
    }
    if (CPLib.eqr("FLAGWALLET",p_cVarName)) {
      return w_FLAGWALLET;
    }
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* w_ATTIV Char(10) */
      /* w_MIDBIGLIETTO Memo */
      /* w_MIDBIGLIETTO1 Memo */
      /* w_MIDBIGLIETTO2 Memo */
      /* w_MIDBIGLIETTO3 Memo */
      /* w_MIDBIGLIETTO4 Memo */
      /* w_ASSEGNO Char(25) // Assegno */
      /* w_VLTPLAT Char(2) // Piattaforma VLT */
      /* w_CC4CIFRE Char(4) // Ultime Cifre CC */
      /* w_CCDATSCA Char(7) // Scadenza */
      /* w_NUMTEL Char(20) // Numero Telefono */
      /* w_TIPOGIOCO Char(3) // Tipo GIoco */
      /* w_RDCODAUT Char(2) // Autorità Rilascio */
      /* w_RDLOCAUT Char(20) // Luogo Rilascio */
      /* w_CGOFLGVLT Char(1) */
      /* w_PEPDESCRI Char(80) */
      /* w_FLGDOM Char(1) */
      /* w_PAESE2 Char(3) */
      /* w_DOMICILIO2 Char(35) // Indirizzo */
      /* w_PROVINCIA2 Char(2) // Provincia */
      /* w_DESCCIT2 Char(40) // Città */
      /* w_CAP2 Char(9) // CAP */
      /* w_PROFESSIONE Char(10) // CAP */
      /* w_IMPWALLET Numeric(12, 2) */
      /* w_CODWALLET Char(14) */
      /* w_FLAGWALLET Char(1) */
      /* w_CODTRANSZ Char(1) */
      /* cProg Char(10) */
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on anadip into routine arrt_cgo_moduli returns two or more records. This item should return only a record.");
      ...
      if (CPLib.eqr(w_MEZPAGAM,"A")) {
        ...
        _mpagam = "Assegno Bancario";
        /* Case w_MEZPAGAM='V' */
      } else if (CPLib.eqr(w_MEZPAGAM,"V")) {
        /* _mpagam := 'Carta di Credito' */
        _mpagam = "Carta di Credito";
        /* Case w_MEZPAGAM='S' */
      } else if (CPLib.eqr(w_MEZPAGAM,"S")) {
        /* _mpagam := 'Contante' */
        _mpagam = "Contante";
        /* Case w_MEZPAGAM='W' */
      } else if (CPLib.eqr(w_MEZPAGAM,"W")) {
        /* _mpagam := 'SnayPay Wallet' */
        _mpagam = "SnayPay Wallet";
      } // End Case
      /* _tipgio := arfn_cgo_tbtipgio(w_TIPOGIOCO,w_VLTPLAT) */
      _tipgio = arfn_cgo_tbtipgioR.Make(m_Ctx,this).Run(w_TIPOGIOCO,w_VLTPLAT);
      /* If w_MEZPAGAM='S' or w_MEZPAGAM='W' */
      if (CPLib.eqr(w_MEZPAGAM,"S") || CPLib.eqr(w_MEZPAGAM,"W")) {
        /* _pagam := LRTrim(_mpagam) + ' € ' +LRTrim(Str(w_TOTLIRE,15,2)) */
        _pagam = CPLib.LRTrim(_mpagam)+" \u20ac "+CPLib.LRTrim(CPLib.Str(w_TOTLIRE,15,2));
      } else { // Else
        /* If w_TOTLIRE - (w_TOTCONT + w_IMPWALLET)=0 */
        if (CPLib.eqr(w_TOTLIRE-(w_TOTCONT+w_IMPWALLET),0)) {
          /* _pagam := iif(w_FLAGCONT='1',' Contanti € '+LRTrim(Str(w_TOTCONT,15,2)),'')  + iif(w_FLAGWALLET='1' and w_FLAGCONT='1',' - SnaiPay Wallet € '+LRTrim(Str(w_IMPWALLET,15,2)),'') + iif(w_FLAGWALLET='1' and w_FLAGCONT='0',' SnaiPay Wallet € '+LRTrim(Str(w_IMPWALLET,15,2)),'') */
          _pagam = (CPLib.eqr(w_FLAGCONT,"1")?" Contanti \u20ac "+CPLib.LRTrim(CPLib.Str(w_TOTCONT,15,2)):"")+(CPLib.eqr(w_FLAGWALLET,"1") && CPLib.eqr(w_FLAGCONT,"1")?" - SnaiPay Wallet \u20ac "+CPLib.LRTrim(CPLib.Str(w_IMPWALLET,15,2)):"")+(CPLib.eqr(w_FLAGWALLET,"1") && CPLib.eqr(w_FLAGCONT,"0")?" SnaiPay Wallet \u20ac "+CPLib.LRTrim(CPLib.Str(w_IMPWALLET,15,2)):"");
        } else { // Else
          /* _pagam := LRTrim(_mpagam) + ' € ' +LRTrim(Str((w_TOTLIRE - w_TOTCONT - w_IMPWALLET),15,2)) + iif(w_FLAGCONT='1',' - Contanti € '+LRTrim(Str(w_TOTCONT,15,2)),'')  + iif(w_FLAGWALLET='1',' - SnaiPay Wallet € '+LRTrim(Str(w_IMPWALLET,15,2)),'') */
          _pagam = CPLib.LRTrim(_mpagam)+" \u20ac "+CPLib.LRTrim(CPLib.Str((w_TOTLIRE-w_TOTCONT-w_IMPWALLET),15,2))+(CPLib.eqr(w_FLAGCONT,"1")?" - Contanti \u20ac "+CPLib.LRTrim(CPLib.Str(w_TOTCONT,15,2)):"")+(CPLib.eqr(w_FLAGWALLET,"1")?" - SnaiPay Wallet \u20ac "+CPLib.LRTrim(CPLib.Str(w_IMPWALLET,15,2)):"");
        } // End If
      } // End If
      /* _desaut := '' */
      _desaut = "";
      ...
      m_cSql = "";
      m_cSql = m_cSql+"ARCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_RDCODAUT,"C",2,0,m_cServer),m_cServer,w_RDCODAUT);
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on cgo_autorita into routine arrt_cgo_moduli returns two or more records. This item should return only a record.");
          CPStdCounter.Error(l_oErrorFault);
        }
        Read_Cursor.Close();
      } else {
      ...
      m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_moduli",83,"00000096")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
      ...
      m_cSql = m_cSql+""+CPLib.ToSQL(w_ASSEGNO,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_VLTPLAT,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_CC4CIFRE,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_CCDATSCA,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_NUMTEL,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(_tipgio,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(w_MEZPAGAM,"V")?"X":""),"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(_desaut,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_RDLOCAUT,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(w_FLAGCONT,"1") || CPLib.eqr(w_MEZPAGAM,"S")?"X":""),"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_CGOFLGVLT,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_PEPDESCRI,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_DOMICILIO2,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_DESCCIT2,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_PROVINCIA2,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_PAESE2,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_CAP2,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_FLGDOM,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_PROFESSIONE,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_IMPWALLET,"?",0,0)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_CODWALLET,"?",0,0,m_cServer)+", ";
      m_cSql = m_cSql+""+CPLib.ToSQL(w_FLAGWALLET,"?",0,0,m_cServer)+", ";
      ...
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_moduli",83,"00000099")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
      ...
      bReport = _report.makeXmlStrReport();
      /* Stop "DownloadFileServlet?id="+cProg */
      Forward f;
      f=new Forward("DownloadFileServlet?id="+cProg,false,this,Forward.NoStatus,true);
  ...
  public Forward Run() {
    ...
    try {
      ...
      m_Caller.SetString("CAIDBIGLIETTO","C",30,0,w_CAIDBIGLIETTO);
      ...
      m_Caller.SetString("MIDBIGLIETTO1","M",10,0,w_MIDBIGLIETTO1);
      m_Caller.SetString("MIDBIGLIETTO2","M",10,0,w_MIDBIGLIETTO2);
      m_Caller.SetString("MIDBIGLIETTO3","M",10,0,w_MIDBIGLIETTO3);
      m_Caller.SetString("MIDBIGLIETTO4","M",10,0,w_MIDBIGLIETTO4);
      m_Caller.SetString("ASSEGNO","C",25,0,w_ASSEGNO);
      m_Caller.SetString("VLTPLAT","C",2,0,w_VLTPLAT);
      m_Caller.SetString("CC4CIFRE","C",4,0,w_CC4CIFRE);
      m_Caller.SetString("CCDATSCA","C",7,0,w_CCDATSCA);
      m_Caller.SetString("NUMTEL","C",20,0,w_NUMTEL);
      m_Caller.SetString("TIPOGIOCO","C",3,0,w_TIPOGIOCO);
      m_Caller.SetString("RDCODAUT","C",2,0,w_RDCODAUT);
      m_Caller.SetString("RDLOCAUT","C",20,0,w_RDLOCAUT);
      m_Caller.SetString("CGOFLGVLT","C",1,0,w_CGOFLGVLT);
      m_Caller.SetString("PEPDESCRI","C",80,0,w_PEPDESCRI);
      m_Caller.SetString("FLGDOM","C",1,0,w_FLGDOM);
      m_Caller.SetString("PAESE2","C",3,0,w_PAESE2);
      m_Caller.SetString("DOMICILIO2","C",35,0,w_DOMICILIO2);
      m_Caller.SetString("PROVINCIA2","C",2,0,w_PROVINCIA2);
      m_Caller.SetString("DESCCIT2","C",40,0,w_DESCCIT2);
      m_Caller.SetString("CAP2","C",9,0,w_CAP2);
      m_Caller.SetString("PROFESSIONE","C",10,0,w_PROFESSIONE);
      m_Caller.SetNumber("IMPWALLET","N",12,2,w_IMPWALLET);
      m_Caller.SetString("CODWALLET","C",14,0,w_CODWALLET);
      m_Caller.SetString("FLAGWALLET","C",1,0,w_FLAGWALLET);
      m_Caller.SetString("CODTRANSZ","C",1,0,w_CODTRANSZ);
  ...
  public static arrt_cgo_moduliR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_moduliR(p_Ctx, p_Caller);
  }
  public void Blank() {
    ...
    w_CAIDBIGLIETTO = m_Caller.GetString("CAIDBIGLIETTO","C",30,0);
    ...
    w_MIDBIGLIETTO1 = m_Caller.GetString("MIDBIGLIETTO1","M",10,0);
    w_MIDBIGLIETTO2 = m_Caller.GetString("MIDBIGLIETTO2","M",10,0);
    w_MIDBIGLIETTO3 = m_Caller.GetString("MIDBIGLIETTO3","M",10,0);
    w_MIDBIGLIETTO4 = m_Caller.GetString("MIDBIGLIETTO4","M",10,0);
    w_ASSEGNO = m_Caller.GetString("ASSEGNO","C",25,0);
    w_VLTPLAT = m_Caller.GetString("VLTPLAT","C",2,0);
    w_CC4CIFRE = m_Caller.GetString("CC4CIFRE","C",4,0);
    w_CCDATSCA = m_Caller.GetString("CCDATSCA","C",7,0);
    w_NUMTEL = m_Caller.GetString("NUMTEL","C",20,0);
    w_TIPOGIOCO = m_Caller.GetString("TIPOGIOCO","C",3,0);
    w_RDCODAUT = m_Caller.GetString("RDCODAUT","C",2,0);
    w_RDLOCAUT = m_Caller.GetString("RDLOCAUT","C",20,0);
    w_CGOFLGVLT = m_Caller.GetString("CGOFLGVLT","C",1,0);
    w_PEPDESCRI = m_Caller.GetString("PEPDESCRI","C",80,0);
    w_FLGDOM = m_Caller.GetString("FLGDOM","C",1,0);
    w_PAESE2 = m_Caller.GetString("PAESE2","C",3,0);
    w_DOMICILIO2 = m_Caller.GetString("DOMICILIO2","C",35,0);
    w_PROVINCIA2 = m_Caller.GetString("PROVINCIA2","C",2,0);
    w_DESCCIT2 = m_Caller.GetString("DESCCIT2","C",40,0);
    w_CAP2 = m_Caller.GetString("CAP2","C",9,0);
    w_PROFESSIONE = m_Caller.GetString("PROFESSIONE","C",10,0);
    w_IMPWALLET = m_Caller.GetNumber("IMPWALLET","N",12,2);
    w_CODWALLET = m_Caller.GetString("CODWALLET","C",14,0);
    w_FLAGWALLET = m_Caller.GetString("FLAGWALLET","C",1,0);
    w_CODTRANSZ = m_Caller.GetString("CODTRANSZ","C",1,0);
  ...
  // ENTITY_BATCHES: ,arfn_cgo_tbtipgio,
  public static final String i_InvokedRoutines = ",arfn_cgo_tbtipgio,";
