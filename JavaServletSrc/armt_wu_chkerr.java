...
public class armt_wu_chkerr extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*77ff2cd3*!";
  static public class ServletStatus extends SPMasterServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "wuchkimp";
    }
    public double w_WUCODICE;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_WUCODICE,context,"wuchkimp","WUCODICE","N",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_wuchkimp;
  ...
  public String getEntityTitle() {
    return "WU Import Analisys";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_wu_chkerr";
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"wuchkimp","anadip","tbcitta","tbstati","tbmacage"};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("WUDATOPE","C",10,0, "show","Data Operazione"),
                   ...
                   new SPItem("WUNTDOCCLI","C",1,0, "edit","Tipo Documento"),
                   ...
                   new SPItem("WUTDOCCLI","C",1,0, "show","Tipo Documento"),
                   ...
                   new SPItem("WUNSTACLI","C",30,0, "edit","Nuovo Stato Residenza Cliente"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_wu_chkerrBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"916054212",p_Context.Translate("WU Import Analisys")},
    {"98786908",p_Context.Translate("Sesso Cliente")}};
    return l_translations;
  }
  void WriteStyles(armt_wu_chkerrBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_wu_chkerr.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_wu_chkerrBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_wu_chkerr',false);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_wu_chkerr",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(armt_wu_chkerrBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(armt_wu_chkerrBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b0(armt_wu_chkerrBO BO,ServletStatus status) {
    ...
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      status.out.println(
       ...
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("S","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("S",CPLib.Trim(BO.w_WUTIPOPE))?"SELECTED":"")+">" +
       ""+status.context.Translate("Send")+"" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("P","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("P",CPLib.Trim(BO.w_WUTIPOPE))?"SELECTED":"")+">" +
  ...
  void WriteControls_p1_b1(armt_wu_chkerrBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("OldOnes")) {
      ...
      "");
      status.out.println(
       "<input name='WUDATOPE' id='ZWACREYOWM' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_WUDATOPE,"C",10,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='10' size='10' fieldtype='C'>" +
      "");
      status.out.println(
      ...
      status.out.println(
       ...
       "<option value="+SPLib.ToHTMLValue("","C",0,0)+">" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("A","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("A",CPLib.Trim(BO.w_WUTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Passaporto")+"" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("B","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("B",CPLib.Trim(BO.w_WUTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Carta Identità")+"" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("C","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("C",CPLib.Trim(BO.w_WUTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Patente Guida")+"" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("D","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("D",CPLib.Trim(BO.w_WUTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Altro")+"" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("E","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("E",CPLib.Trim(BO.w_WUTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Altro")+"" +
       "</option>" +
       "</select>" +
       "<script type='text/javascript'>" +
  ...
  void WriteControls_p1_b1_CloseBand(armt_wu_chkerrBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_wu_chkerrBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(armt_wu_chkerrBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("NewOnes")) {
      ...
      status.out.println(
       ...
       "<option value="+SPLib.ToHTMLValue("","C",0,0)+">" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("A","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("A",CPLib.Trim(BO.w_WUNTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Passaporto")+"" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("B","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("B",CPLib.Trim(BO.w_WUNTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Carta Identità")+"" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("C","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("C",CPLib.Trim(BO.w_WUNTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Patente Guida")+"" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("D","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("D",CPLib.Trim(BO.w_WUNTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Altro")+"" +
       "</option>" +
       "<option value="+SPLib.ToHTMLValue("E","C",1,0,"", status.m_oRegSettings)+" "+(CPLib.eq("E",CPLib.Trim(BO.w_WUNTDOCCLI))?"SELECTED":"")+">" +
       ""+status.context.Translate("Altro")+"" +
       "</option>" +
       "</select>" +
       "<script type='text/javascript'>" +
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='WUNSTACLI' id='WVLDFVKALY' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_WUNSTACLI,"C",30,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='30' size='30' fieldtype='C'>" +
  ...
  void WriteControls_p1_b3_CloseBand(armt_wu_chkerrBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(armt_wu_chkerrBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_wu_chkerrBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_wu_chkerrBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(armt_wu_chkerrBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_WUDATOPE,"C",10,0));
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_WUNTDOCCLI,"C",1,0));
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_WUTDOCCLI,"C",1,0));
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_WUNSTACLI,"C",30,0));
  ...
  void WriteMainFormVarScript(armt_wu_chkerrBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_wu_chkerr"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_wu_chkerr","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_wu_chkerr",BO.GetCurrentState());
  ...
  void WriteMainFormScript(armt_wu_chkerrBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  armt_wu_chkerrBO getBO(ServletStatus status) {
    armt_wu_chkerrBO BO = new armt_wu_chkerrBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(armt_wu_chkerrBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("wuchkimp",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_wu_chkerrBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,armt_wu_chkerrBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
    l_cTmp = "[";
    if (l_rsLoad!=null)
      l_rsLoad.Close();
    l_rsLoad = BO.m_Sql.Query(BO.m_cServer_wucodes,CPSql.BuildSQLPhrase("WUCODSTA,WUDESCRI",BO.m_cPhName_wucodes,"",""));
    try {
      l_cTmp = l_cTmp+"["+SPLib.ToJSValue("","C",3,0)+","+SPLib.ToJSValue("","C",0,0)+"]";
      while ( ! (l_rsLoad.Eof())) {
        l_cTmp = l_cTmp+",["+SPLib.ToJSValue(l_rsLoad.GetString("WUCODSTA"),"C",3,0)+","+SPLib.ToJSValue(l_rsLoad.GetString("WUDESCRI"),"C",0,0)+"]";
        l_rsLoad.Next();
      }
    } finally {
      l_rsLoad.Close();
  ...
  CPResultSet QueryKeyCursor(armt_wu_chkerrBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_wuchkimp;
    String l_cPhName = BO.m_cPhName_wuchkimp;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_wu_chkerrBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_wuchkimp+".WUCODICE",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("wuchkimp");
    String l_cPhName = CPSql.ManipulateTablePhName("wuchkimp",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,armt_wu_chkerrBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_wuchkimp,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_wuchkimp,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,armt_wu_chkerrBO BO) {
  ...
  void Edit(ServletStatus status,armt_wu_chkerrBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_wu_chkerrBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_wu_chkerrBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_wu_chkerrBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_wu_chkerrBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_wu_chkerrBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_wu_chkerrBO BO,SPParameterSource source) {
    BO.Load(status.w_WUCODICE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_wu_chkerrBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_wu_chkerrBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_wu_chkerrBO BO) {
    ...
    item.put("altInterfaceFor","armt_wu_chkerr");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"MTCN\",\"disabled\":\"true\",\"h\":20,\"maxlength\":10,\"name\":\"WURIFMTCN\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"KRPCHHEAQX\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":408,\"y\":14,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_NMZCDYSEPV\",\"page\":1,\"spuid\":\"NMZCDYSEPV\",\"tabindex\":6,\"type\":\"Label\",\"value\":\"MTCN:\",\"w\":32,\"x\":375,\"y\":18,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Send o Payment\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"WUTIPOPE\",\"page\":1,\"spuid\":\"LDWZRELAZY\",\"tabindex\":7,\"textlist\":\"Send,Payment\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"S"+"'"+","+"'"+"P"+"'"+"\",\"w\":128,\"x\":584,\"y\":14,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Importo Operazione\",\"disabled\":\"true\",\"h\":20,\"maxlength\":15,\"name\":\"WUIMPORTO\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"EJLVDSGOYF\",\"type\":\"Textbox\",\"typevar\":\"numeric\",\"w\":72,\"x\":888,\"y\":14,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_JQDVXZROHF\",\"page\":1,\"spuid\":\"JQDVXZROHF\",\"tabindex\":13,\"type\":\"Label\",\"value\":\"Importo Operazione:\",\"w\":100,\"x\":787,\"y\":18,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"MacroAgente (Network)\",\"disabled\":\"true\",\"h\":20,\"maxlength\":5,\"name\":\"WUCODMAGE\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"FHWIGGJJLC\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":48,\"x\":152,\"y\":42,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_MLTVMLJQKT\",\"page\":1,\"spuid\":\"MLTVMLJQKT\",\"tabindex\":79,\"type\":\"Label\",\"value\":\"MacroAgente (Network):\",\"w\":119,\"x\":33,\"y\":46,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_PGAQTUPTSI\",\"page\":1,\"spuid\":\"PGAQTUPTSI\",\"tabindex\":81,\"type\":\"Label\",\"w\":328,\"x\":208,\"y\":46,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Agenzia\",\"disabled\":\"true\",\"h\":20,\"maxlength\":6,\"name\":\"WUCODDIP\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"HHRNUENTOX\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":48,\"x\":584,\"y\":42,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_FVRMRWOVXX\",\"page\":1,\"spuid\":\"FVRMRWOVXX\",\"tabindex\":83,\"type\":\"Label\",\"value\":\"Agenzia:\",\"w\":42,\"x\":542,\"y\":46,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NIVGQXYPQK\",\"page\":1,\"spuid\":\"NIVGQXYPQK\",\"tabindex\":85,\"type\":\"Label\",\"w\":320,\"x\":640,\"y\":46,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Elenco Errori\",\"h\":119,\"name\":\"WUNOTE\",\"page\":1,\"scroll\":true,\"spuid\":\"AUBWLVTBES\",\"tabindex\":86,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":520,\"x\":232,\"y\":77,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"OldOnes\",\"box_type\":\"collapsible\",\"collapsible\":true,\"comment\":\"Dati Da Import\",\"h\":259,\"name\":\"OldOnes\",\"page\":1,\"shrinkable\":true,\"spuid\":\"RSDQEMGAOL\",\"tabindex\":2,\"title_caption\":\"Dati Da Import\",\"titled\":true,\"type\":\"Box\",\"w\":960,\"x\":8,\"y\":205,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Codice Dipendenza\",\"disabled\":\"true\",\"h\":20,\"maxlength\":6,\"name\":\"WUCODDIP\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"SAINYUGXKT\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":56,\"x\":152,\"y\":212,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VFPEGKTENT\",\"page\":1,\"spuid\":\"VFPEGKTENT\",\"tabindex\":9,\"type\":\"Label\",\"value\":\"Codice Dipendenza:\",\"w\":95,\"x\":56,\"y\":216,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Operazione\",\"disabled\":\"true\",\"h\":20,\"maxlength\":10,\"name\":\"WUDATOPE\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"ZWACREYOWM\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":400,\"y\":212,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_FULJTPTEYV\",\"page\":1,\"spuid\":\"FULJTPTEYV\",\"tabindex\":11,\"type\":\"Label\",\"value\":\"Data Operazione:\",\"w\":85,\"x\":314,\"y\":216,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Stato Destinazione WU\",\"disabled\":\"true\",\"h\":20,\"maxlength\":3,\"name\":\"WUSTADES\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"KDZAQHEPAZ\",\"type\":\"Combobox\",\"typevar\":\"character\",\"w\":256,\"x\":704,\"y\":212,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_MYHPJZTTBD\",\"page\":1,\"spuid\":\"MYHPJZTTBD\",\"tabindex\":15,\"type\":\"Label\",\"value\":\"Stato Destinazione:\",\"w\":114,\"x\":589,\"y\":216,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nome Cliente\",\"disabled\":\"true\",\"h\":20,\"maxlength\":40,\"name\":\"WUNOMCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"LGVFOMYNPG\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":152,\"y\":240,\"zone\":\"pag1_2_2\",\"zonepath\":\"[1,2,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_IYEFJVHBUP\",\"page\":1,\"spuid\":\"IYEFJVHBUP\",\"tabindex\":42,\"type\":\"Label\",\"value\":\"Nome Cliente:\",\"w\":67,\"x\":84,\"y\":244,\"zone\":\"pag1_2_2\",\"zonepath\":\"[1,2,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Cognome Cliente\",\"disabled\":\"true\",\"h\":20,\"maxlength\":40,\"name\":\"WUCOGCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"KXQAAJEHME\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":704,\"y\":240,\"zone\":\"pag1_2_3\",\"zonepath\":\"[1,2,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VXLPQPNMRE\",\"page\":1,\"spuid\":\"VXLPQPNMRE\",\"tabindex\":44,\"type\":\"Label\",\"value\":\"Cognome Cliente:\",\"w\":85,\"x\":618,\"y\":244,\"zone\":\"pag1_2_3\",\"zonepath\":\"[1,2,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Citt\\u00E0 Cliente\",\"disabled\":\"true\",\"h\":20,\"maxlength\":40,\"name\":\"WUCITCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"DSDXXCNFTB\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":152,\"y\":268,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_KHKCQNKAVF\",\"page\":1,\"spuid\":\"KHKCQNKAVF\",\"tabindex\":46,\"type\":\"Label\",\"value\":\"Citt\\u00E0 Cliente:\",\"w\":63,\"x\":88,\"y\":272,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"CAP Cliente\",\"disabled\":\"true\",\"h\":20,\"maxlength\":5,\"name\":\"WUCAPCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"VGRIHHYXXD\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":42,\"x\":704,\"y\":268,\"zone\":\"pag1_2_5\",\"zonepath\":\"[1,2,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_ANNDLELTUI\",\"page\":1,\"spuid\":\"ANNDLELTUI\",\"tabindex\":48,\"type\":\"Label\",\"value\":\"CAP Cliente:\",\"w\":60,\"x\":643,\"y\":272,\"zone\":\"pag1_2_5\",\"zonepath\":\"[1,2,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Indirizzo Cliente\",\"disabled\":\"true\",\"h\":20,\"maxlength\":60,\"name\":\"WUINDCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"CGAYBPFHFD\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":372,\"x\":152,\"y\":296,\"zone\":\"pag1_2_6\",\"zonepath\":\"[1,2,6]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_CXQFMZWQBL\",\"page\":1,\"spuid\":\"CXQFMZWQBL\",\"tabindex\":50,\"type\":\"Label\",\"value\":\"Indirizzo Cliente:\",\"w\":80,\"x\":71,\"y\":300,\"zone\":\"pag1_2_6\",\"zonepath\":\"[1,2,6]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nome Controparte\",\"disabled\":\"true\",\"h\":20,\"maxlength\":40,\"name\":\"WUNOMCON\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"LVLAUGOCRH\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":152,\"y\":324,\"zone\":\"pag1_2_7\",\"zonepath\":\"[1,2,7]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_MZJRCEQHDY\",\"page\":1,\"spuid\":\"MZJRCEQHDY\",\"tabindex\":52,\"type\":\"Label\",\"value\":\"Nome Controparte:\",\"w\":93,\"x\":58,\"y\":328,\"zone\":\"pag1_2_7\",\"zonepath\":\"[1,2,7]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Cognome Controparte\",\"disabled\":\"true\",\"h\":20,\"maxlength\":40,\"name\":\"WUCOGCON\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"UAFIMUEGRJ\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":704,\"y\":324,\"zone\":\"pag1_2_8\",\"zonepath\":\"[1,2,8]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_TNNHERZGME\",\"page\":1,\"spuid\":\"TNNHERZGME\",\"tabindex\":54,\"type\":\"Label\",\"value\":\"Cognome Controparte:\",\"w\":111,\"x\":592,\"y\":328,\"zone\":\"pag1_2_8\",\"zonepath\":\"[1,2,8]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Citt\\u00E0 Nascita Cliente\",\"disabled\":\"true\",\"h\":20,\"maxlength\":40,\"name\":\"WUBCITCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"XYZGDIEBQV\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":152,\"y\":352,\"zone\":\"pag1_2_9\",\"zonepath\":\"[1,2,9]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_TABSWVHCZN\",\"page\":1,\"spuid\":\"TABSWVHCZN\",\"tabindex\":56,\"type\":\"Label\",\"value\":\"Citt\\u00E0 Nascita Cliente:\",\"w\":101,\"x\":50,\"y\":356,\"zone\":\"pag1_2_9\",\"zonepath\":\"[1,2,9]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Stato Nascita Cliente\",\"disabled\":\"true\",\"h\":20,\"maxlength\":40,\"name\":\"WUBSTACLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"JDYYUWYTGY\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":704,\"y\":352,\"zone\":\"pag1_2_10\",\"zonepath\":\"[1,2,10]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_MZKPVDVQSA\",\"page\":1,\"spuid\":\"MZKPVDVQSA\",\"tabindex\":58,\"type\":\"Label\",\"value\":\"Stato Nascita Cliente:\",\"w\":104,\"x\":599,\"y\":356,\"zone\":\"pag1_2_10\",\"zonepath\":\"[1,2,10]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Nascita Cliente\",\"disabled\":\"true\",\"h\":20,\"maxlength\":10,\"name\":\"WUDATNAS\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"KTVPFMPGUX\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":152,\"y\":380,\"zone\":\"pag1_2_11\",\"zonepath\":\"[1,2,11]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_CSGIQWNGXO\",\"page\":1,\"spuid\":\"CSGIQWNGXO\",\"tabindex\":60,\"type\":\"Label\",\"value\":\"Data Nascita Cliente:\",\"w\":101,\"x\":50,\"y\":384,\"zone\":\"pag1_2_11\",\"zonepath\":\"[1,2,11]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_XBYNQJGSTD\",\"page\":1,\"spuid\":\"XBYNQJGSTD\",\"tabindex\":61,\"type\":\"Label\",\"value\":\"Sesso Cliente:\",\"w\":68,\"x\":331,\"y\":384,\"zone\":\"pag1_2_11\",\"zonepath\":\"[1,2,11]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Sesso Cliente\",\"disabled\":\"true\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"WUSEXCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"RMCFXDIVEO\",\"textlist\":\"Maschio,Femmina\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"M"+"'"+","+"'"+"F"+"'"+"\",\"w\":112,\"x\":400,\"y\":380,\"zone\":\"pag1_2_11\",\"zonepath\":\"[1,2,11]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Codice Fiscale\",\"disabled\":\"true\",\"h\":20,\"maxlength\":16,\"name\":\"WUCODFIS\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"JMOAJRUZVG\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":136,\"x\":704,\"y\":380,\"zone\":\"pag1_2_12\",\"zonepath\":\"[1,2,12]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_JEPAFHMKJO\",\"page\":1,\"spuid\":\"JEPAFHMKJO\",\"tabindex\":63,\"type\":\"Label\",\"value\":\"Codice Fiscale:\",\"w\":71,\"x\":632,\"y\":384,\"zone\":\"pag1_2_12\",\"zonepath\":\"[1,2,12]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_ADLMRZLWWG\",\"page\":1,\"spuid\":\"ADLMRZLWWG\",\"tabindex\":64,\"type\":\"Label\",\"value\":\"Tipo Documento:\",\"w\":81,\"x\":70,\"y\":412,\"zone\":\"pag1_2_13\",\"zonepath\":\"[1,2,13]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"N\\u00B0 Documento\",\"disabled\":\"true\",\"h\":20,\"maxlength\":25,\"name\":\"WUNDOCCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"JNCAEVBDYL\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":162,\"x\":400,\"y\":408,\"zone\":\"pag1_2_13\",\"zonepath\":\"[1,2,13]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_CSXRNANFRD\",\"page\":1,\"spuid\":\"CSXRNANFRD\",\"tabindex\":66,\"type\":\"Label\",\"value\":\"N\\u00B0 Documento:\",\"w\":73,\"x\":326,\"y\":412,\"zone\":\"pag1_2_13\",\"zonepath\":\"[1,2,13]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Tipo Documento\",\"disabled\":\"true\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"WUTDOCCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"JOQUHREGBT\",\"textlist\":\"Passaporto,Carta Identit\\u00E0,Patente Guida,Altro,Altro\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"A"+"'"+","+"'"+"B"+"'"+","+"'"+"C"+"'"+","+"'"+"D"+"'"+","+"'"+"E"+"'"+"\",\"w\":152,\"x\":152,\"y\":408,\"zone\":\"pag1_2_13\",\"zonepath\":\"[1,2,13]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Autorit\\u00E0 Rilascio\",\"disabled\":\"true\",\"h\":20,\"maxlength\":30,\"name\":\"WUADOCRIL\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"DGJAFVSPZS\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":192,\"x\":704,\"y\":436,\"zone\":\"pag1_2_14\",\"zonepath\":\"[1,2,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VPSNKTDSLR\",\"page\":1,\"spuid\":\"VPSNKTDSLR\",\"tabindex\":68,\"type\":\"Label\",\"value\":\"Autorit\\u00E0 Rilascio:\",\"w\":81,\"x\":622,\"y\":440,\"zone\":\"pag1_2_14\",\"zonepath\":\"[1,2,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Codice Fiscale da ACF\",\"disabled\":\"true\",\"h\":20,\"maxlength\":16,\"name\":\"WUORIGCF\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"KKARTLOBRM\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":136,\"x\":704,\"y\":408,\"zone\":\"pag1_2_14\",\"zonepath\":\"[1,2,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_QZLLMZQCPB\",\"page\":1,\"spuid\":\"QZLLMZQCPB\",\"tabindex\":89,\"type\":\"Label\",\"value\":\"Codice Fiscale da ACF:\",\"w\":109,\"x\":595,\"y\":412,\"zone\":\"pag1_2_14\",\"zonepath\":\"[1,2,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Rilascio Documento\",\"disabled\":\"true\",\"h\":20,\"maxlength\":10,\"name\":\"WURDOCCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"DRAAKQTXFV\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":152,\"y\":436,\"zone\":\"pag1_2_15\",\"zonepath\":\"[1,2,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_CLEHMQOZVF\",\"page\":1,\"spuid\":\"CLEHMQOZVF\",\"tabindex\":70,\"type\":\"Label\",\"value\":\"Data Rilascio Documento:\",\"w\":122,\"x\":29,\"y\":440,\"zone\":\"pag1_2_15\",\"zonepath\":\"[1,2,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Scadenza Documento\",\"disabled\":\"true\",\"h\":20,\"maxlength\":10,\"name\":\"WUSDOCCLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"MGEWPRXRDF\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":400,\"y\":436,\"zone\":\"pag1_2_15\",\"zonepath\":\"[1,2,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_LQIMPKLXCK\",\"page\":1,\"spuid\":\"LQIMPKLXCK\",\"tabindex\":72,\"type\":\"Label\",\"value\":\"Data Scadenza Documento:\",\"w\":133,\"x\":266,\"y\":440,\"zone\":\"pag1_2_15\",\"zonepath\":\"[1,2,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Stato Nascita Cliente\",\"disabled\":\"true\",\"h\":20,\"maxlength\":40,\"name\":\"WUSTACLI\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"NIKRQOIEUN\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":704,\"y\":296,\"zone\":\"pag1_2_16\",\"zonepath\":\"[1,2,16]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_CIPIVMPJDI\",\"page\":1,\"spuid\":\"CIPIVMPJDI\",\"tabindex\":91,\"type\":\"Label\",\"value\":\"Stato Residenza Cliente:\",\"w\":129,\"x\":574,\"y\":300,\"zone\":\"pag1_2_16\",\"zonepath\":\"[1,2,16]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"NewOnes\",\"box_type\":\"collapsible\",\"collapsible\":true,\"comment\":\"Dati Modificati\",\"h\":178,\"name\":\"NewOnes\",\"page\":1,\"shrinkable\":true,\"spuid\":\"JKOQQOCGZL\",\"tabindex\":18,\"title_caption\":\"Dati Modificati\",\"titled\":true,\"type\":\"Box\",\"w\":960,\"x\":8,\"y\":466,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nuova Data Operazione\",\"h\":20,\"maxlength\":10,\"name\":\"WUNDATOPE\",\"page\":1,\"spuid\":\"BQFSVTKPAE\",\"tabindex\":19,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":72,\"x\":400,\"y\":473,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_RPXOHMZYSG\",\"page\":1,\"spuid\":\"RPXOHMZYSG\",\"tabindex\":25,\"type\":\"Label\",\"value\":\"Data Operazione:\",\"w\":85,\"x\":314,\"y\":477,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nuovo CAP Cliente\",\"h\":20,\"maxlength\":5,\"name\":\"WUNCAPCLI\",\"page\":1,\"spuid\":\"DNFJVJRGTH\",\"tabindex\":20,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":42,\"x\":704,\"y\":501,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BDSYRBXZOM\",\"page\":1,\"spuid\":\"BDSYRBXZOM\",\"tabindex\":28,\"type\":\"Label\",\"value\":\"CAP Cliente:\",\"w\":60,\"x\":643,\"y\":505,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nuova Data Nascita Cliente\",\"h\":20,\"maxlength\":10,\"name\":\"WUNDATNAS\",\"page\":1,\"spuid\":\"DJOIOBUZTW\",\"tabindex\":21,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":72,\"x\":152,\"y\":586,\"zone\":\"pag1_3_3\",\"zonepath\":\"[1,3,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_JLXIRLXRUF\",\"page\":1,\"spuid\":\"JLXIRLXRUF\",\"tabindex\":31,\"type\":\"Label\",\"value\":\"Data Nascita Cliente:\",\"w\":101,\"x\":50,\"y\":590,\"zone\":\"pag1_3_3\",\"zonepath\":\"[1,3,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nuova Data Rilascio Documento\",\"h\":20,\"maxlength\":10,\"name\":\"WUNRDOCCLI\",\"page\":1,\"spuid\":\"PKOBXVTTQB\",\"tabindex\":22,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":72,\"x\":400,\"y\":614,\"zone\":\"pag1_3_4\",\"zonepath\":\"[1,3,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_LXZNAJSOTJ\",\"page\":1,\"spuid\":\"LXZNAJSOTJ\",\"tabindex\":32,\"type\":\"Label\",\"value\":\"Tipo Documento:\",\"w\":81,\"x\":70,\"y\":618,\"zone\":\"pag1_3_4\",\"zonepath\":\"[1,3,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_POXLHOKSCG\",\"page\":1,\"spuid\":\"POXLHOKSCG\",\"tabindex\":33,\"type\":\"Label\",\"value\":\"Data Rilascio Doc.:\",\"w\":122,\"x\":277,\"y\":618,\"zone\":\"pag1_3_4\",\"zonepath\":\"[1,3,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Tipo Documento\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"WUNTDOCCLI\",\"page\":1,\"spuid\":\"MCJVDAFNOQ\",\"tabindex\":35,\"textlist\":\"Passaporto,Carta Identit\\u00E0,Patente Guida,Altro,Altro\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"A"+"'"+","+"'"+"B"+"'"+","+"'"+"C"+"'"+","+"'"+"D"+"'"+","+"'"+"E"+"'"+"\",\"w\":152,\"x\":152,\"y\":614,\"zone\":\"pag1_3_4\",\"zonepath\":\"[1,3,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nuova Data Scadenza Documento\",\"h\":20,\"maxlength\":10,\"name\":\"WUNSDOCCLI\",\"page\":1,\"spuid\":\"YCVYLQBAVQ\",\"tabindex\":23,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":72,\"x\":704,\"y\":614,\"zone\":\"pag1_3_5\",\"zonepath\":\"[1,3,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BOJZSPVVUA\",\"page\":1,\"spuid\":\"BOJZSPVVUA\",\"tabindex\":34,\"type\":\"Label\",\"value\":\"Data Scadenza Documento:\",\"w\":133,\"x\":570,\"y\":618,\"zone\":\"pag1_3_5\",\"zonepath\":\"[1,3,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_PNNFJQAVIA\",\"page\":1,\"spuid\":\"PNNFJQAVIA\",\"tabindex\":26,\"type\":\"Label\",\"value\":\"Stato Destinazione:\",\"w\":114,\"x\":589,\"y\":477,\"zone\":\"pag1_3_6\",\"zonepath\":\"[1,3,6]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nuovo Stato Destinazione WU\",\"h\":20,\"maxlength\":3,\"name\":\"WUNSTADES\",\"page\":1,\"spuid\":\"DQRQHDIWWC\",\"tabindex\":36,\"type\":\"Combobox\",\"typevar\":\"character\",\"w\":256,\"x\":704,\"y\":473,\"zone\":\"pag1_3_6\",\"zonepath\":\"[1,3,6]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_ATMIRGZJIT\",\"page\":1,\"spuid\":\"ATMIRGZJIT\",\"tabindex\":24,\"type\":\"Label\",\"value\":\"Codice Dipendenza:\",\"w\":95,\"x\":56,\"y\":477,\"zone\":\"pag1_3_7\",\"zonepath\":\"[1,3,7]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Nuovo Codice Dipendenza\",\"h\":20,\"maxlength\":6,\"name\":\"WUNCODDIP\",\"page\":1,\"picker\":true,\"spuid\":\"RPXEDSQTUE\",\"tabindex\":37,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":70,\"x\":152,\"y\":473,\"zone\":\"pag1_3_7\",\"zonepath\":\"[1,3,7]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_OZMVUXIVZE\",\"page\":1,\"spuid\":\"OZMVUXIVZE\",\"tabindex\":27,\"type\":\"Label\",\"value\":\"Citt\\u00E0 Cliente:\",\"w\":63,\"x\":88,\"y\":505,\"zone\":\"pag1_3_8\",\"zonepath\":\"[1,3,8]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Citt\\u00E0 Cliente\",\"h\":20,\"maxlength\":40,\"name\":\"WUNCITCLI\",\"page\":1,\"picker\":true,\"spuid\":\"AZSYSFMRUL\",\"tabindex\":38,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":152,\"y\":501,\"zone\":\"pag1_3_8\",\"zonepath\":\"[1,3,8]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_EYEPLDXQKM\",\"page\":1,\"spuid\":\"EYEPLDXQKM\",\"tabindex\":30,\"type\":\"Label\",\"value\":\"Stato Nascita Cliente:\",\"w\":104,\"x\":599,\"y\":562,\"zone\":\"pag1_3_9\",\"zonepath\":\"[1,3,9]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Nuovo Stato Nascita Cliente\",\"h\":20,\"maxlength\":40,\"name\":\"WUNBSTACLI\",\"page\":1,\"picker\":true,\"spuid\":\"GPWUEBYSIT\",\"tabindex\":39,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":238,\"x\":704,\"y\":558,\"zone\":\"pag1_3_9\",\"zonepath\":\"[1,3,9]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VDBPAWYBBX\",\"page\":1,\"spuid\":\"VDBPAWYBBX\",\"tabindex\":29,\"type\":\"Label\",\"value\":\"Citt\\u00E0 Nascita Cliente:\",\"w\":101,\"x\":50,\"y\":562,\"zone\":\"pag1_3_10\",\"zonepath\":\"[1,3,10]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Nuova Citt\\u00E0 Nascita Cliente\",\"h\":20,\"maxlength\":40,\"name\":\"WUNBCITCLI\",\"page\":1,\"picker\":true,\"spuid\":\"SQSTTNXNEF\",\"tabindex\":40,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":152,\"y\":558,\"zone\":\"pag1_3_10\",\"zonepath\":\"[1,3,10]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Completata\",\"h\":21,\"label_text\":\"Correzione Completa\",\"maxlength\":1,\"name\":\"WUFLGALL\",\"page\":1,\"spuid\":\"ZZOJSEEQWY\",\"tabindex\":75,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":128,\"x\":832,\"y\":614,\"zone\":\"pag1_3_11\",\"zonepath\":\"[1,3,11]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Codice Fiscale\",\"h\":20,\"maxlength\":16,\"name\":\"WUNCODFIS\",\"page\":1,\"spuid\":\"UBNCXWNHSA\",\"tabindex\":76,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":120,\"x\":704,\"y\":586,\"zone\":\"pag1_3_12\",\"zonepath\":\"[1,3,12]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_GHZQTXZJUL\",\"page\":1,\"spuid\":\"GHZQTXZJUL\",\"tabindex\":77,\"type\":\"Label\",\"value\":\"Codice Fiscale:\",\"w\":71,\"x\":632,\"y\":590,\"zone\":\"pag1_3_12\",\"zonepath\":\"[1,3,12]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Forza caricamento movimento\",\"h\":21,\"label_text\":\"Forza Scrittura\",\"maxlength\":1,\"name\":\"WUFLGFRC\",\"page\":1,\"spuid\":\"YYWDAHCCHA\",\"tabindex\":87,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":128,\"x\":832,\"y\":586,\"zone\":\"pag1_3_12\",\"zonepath\":\"[1,3,12]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_HKSYSSTTTW\",\"page\":1,\"spuid\":\"HKSYSSTTTW\",\"tabindex\":92,\"type\":\"Label\",\"value\":\"Stato Residenza Cliente:\",\"w\":129,\"x\":22,\"y\":533,\"zone\":\"pag1_3_13\",\"zonepath\":\"[1,3,13]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Nuovo Stato Residenza Cliente\",\"h\":20,\"maxlength\":30,\"name\":\"WUNSTACLI\",\"page\":1,\"picker\":true,\"spuid\":\"WVLDFVKALY\",\"tabindex\":93,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":252,\"x\":152,\"y\":529,\"zone\":\"pag1_3_13\",\"zonepath\":\"[1,3,13]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_MVOVQAXRBO\",\"page\":1,\"spuid\":\"MVOVQAXRBO\",\"tabindex\":94,\"type\":\"Label\",\"value\":\"Sesso Cliente:\",\"w\":68,\"x\":331,\"y\":592,\"zone\":\"pag1_3_14\",\"zonepath\":\"[1,3,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nuovo Sesso Cliente\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"WUNSEXCLI\",\"page\":1,\"spuid\":\"TADITPUPOP\",\"tabindex\":95,\"textlist\":\"Maschio,Femmina\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"M"+"'"+","+"'"+"F"+"'"+"\",\"w\":112,\"x\":400,\"y\":588,\"zone\":\"pag1_3_14\",\"zonepath\":\"[1,3,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_wu_chkerrBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_wu_chkerr?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_wu_chkerr_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_wu_chkerr_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_WUBSTACLI=a[28];");
      status.out.println("w_WUDATNAS=a[29];");
      status.out.println("w_WUCODFIS=a[30];");
      status.out.println("w_WUNDOCCLI=a[31];");
      status.out.println("w_WUADOCRIL=a[32];");
      status.out.println("w_WURDOCCLI=a[33];");
      status.out.println("w_WUSDOCCLI=a[34];");
      status.out.println("w_WUSEXCLI=a[35];");
      status.out.println("w_WUTDOCCLI=a[36];");
      status.out.println("w_WUFLGALL=a[37];");
      status.out.println("w_WUNCODFIS=a[38];");
      status.out.println("w_WUCODMAGE=a[39];");
      status.out.println("w_WUNOTE=a[40];");
      status.out.println("w_WUFLGFRC=a[41];");
      status.out.println("w_WUORIGCF=a[42];");
      status.out.println("w_WUSTACLI=a[43];");
      status.out.println("w_WUNSTACLI=a[44];");
      status.out.println("w_WUNSEXCLI=a[45];");
      status.out.println("w_xMADESCRI=a[46];");
      status.out.println("w_xDESDIP=a[47];");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_wu_chkerr_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(armt_wu_chkerrBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_wu_chkerrBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_wu_chkerrBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_wu_chkerrBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_wu_chkerrBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_wu_chkerrBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_wu_chkerrBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,armt_wu_chkerrBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_wu_chkerrBO BO) {
    boolean result = true;
