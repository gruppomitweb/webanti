...
public class arpg_readerr_sid extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*AZCTDPCPLT*!";
  ...
  public String getEntityTitle() {
    return "Lettura File Errori SID";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_readerr_sid";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_readerr_sidBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1207496193",p_Context.Translate("Lettura File Errori SID")},
  ...
  void WriteStyles(arpg_readerr_sidBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_readerr_sid.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_readerr_sidBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_readerr_sidBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_readerr_sidBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_readerr_sidBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_readerr_sidBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(350);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_readerr_sidBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_readerr_sidBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_readerr_sidBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_readerr_sidBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_readerr_sidBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_readerr_sid"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_readerr_sid","");
  ...
  void WriteMainFormScript(arpg_readerr_sidBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_readerr_sid");
  ...
  arpg_readerr_sidBO getBO(ServletStatus status) {
    arpg_readerr_sidBO BO = new arpg_readerr_sidBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_readerr_sidBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_readerr_sidBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_readerr_sidBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_readerr_sidBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_readerr_sidBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_readerr_sidBO BO) {
    ...
    item.put("altInterfaceFor","arpg_readerr_sid");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stampa il risultato dell"+"'"+"elaborazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_ZBTTWAQZVY()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"ZBTTWAQZVY\",\"src\":\"images\\/print.gif\",\"tabindex\":\"11\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"688\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui elaborazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_XWPYHZIJDE()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"XWPYHZIJDE\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"13\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_readerr_sidBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_readerr_sidBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_readerr_sidBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_readerr_sid?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_readerr_sid_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_readerr_sidBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_readerr_sidBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_readerr_sidBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_readerr_sidBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_readerr_sidBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_readerr_sidBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_readerr_sid.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_readerr_sidBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_readerr_sidBO BO) {
    boolean result = true;
    return result;
  }
}
