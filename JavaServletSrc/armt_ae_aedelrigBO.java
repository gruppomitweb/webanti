...
// * --- Fine Area Manuale
public class armt_ae_aedelrigBO extends armt_ae_aedelrigBL {
  class SqlLoader {
  ...
  // * --- Fine Area Manuale
  public armt_ae_aedelrigBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_ae_aedelrig");
    }
  ...
  public boolean Load(String p_IDBASE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_aederig,m_cPhName_aederig,m_cVirtName_aederig,m_cVirtName_aederig+".IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDBASE,"C",10,0),m_cServer_aederig,p_IDBASE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        w_PROGDLG = l_rsLoad.GetDouble("PROGDLG");
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"aederig")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_aederig);
      l_cSql = "insert into "+m_oWrInfo_aederig.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_PROGDLG,"N",15,0)+", ";
      ...
      l_cSql = l_cSql+m_oWrInfo_aederig.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"aederig")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_aederig,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_aederig);
      l_cSql = "Update "+m_oWrInfo_aederig.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+", PROGDLG = "+CPLib.ToSQL(w_PROGDLG,"N",15,0);
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_IDBASE,"C",10,0),m_cServer_aederig,w_IDBASE);
      l_cSql = l_cSql+m_oWrInfo_aederig.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"aederig")) {
  ...
  public boolean Delete(String p_IDBASE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_aederig.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_IDBASE,"C",10,0),m_cServer_aederig,w_IDBASE);
          l_cSql = l_cSql+m_oWrInfo_aederig.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"aederig")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_aederig,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
  ...
  void SaveAutonumber() {
    ...
    m_Sql.NextTableProg(this,"PRGAEDELEG","i_codazi,w_IDBASE",m_cServer_aederig,m_Ctx.GetCompany());
    ...
    op_IDBASE = w_IDBASE;
    // Segna le variabili coinvolte in un progressivo: se al salvataggio hanno lo stesso valore vuol dire che l' utente accetta il progressivo proposto.
    m_Sql.NextTableProg(this,"PROGAED","i_codazi,w_PROGDLG",m_cServer_aederig,m_Ctx.GetCompany());
    op_PROGDLG = w_PROGDLG;
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"DZWSXMBIHR")) {
      ...
      result.AppendFilter("TIPOPERS","C","=",false);
    } else if (CPLib.eq(p_cID,"VQOCGQDTYC")) {
      result.m_TableName = "aedecanc";
      result.n_KMode = 1;
      result.m_FillEmpty = false;
