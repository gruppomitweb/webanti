...
// * --- Fine Area Manuale
public class ardt_informazioniBO extends ardt_informazioniBL {
  ...
  // * --- Fine Area Manuale
  public ardt_informazioniBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_informazioni");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_intestbo+".CPROWNUM";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_intestbo,m_cPhName_intestbo,m_cVirtName_intestbo,m_cVirtName_intestbo+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_intestbo,p_RAPPORTO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"intestbo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_intestbo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_intestbo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"intestbo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_intestbo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_intestbo);
      l_cSql = "insert into "+m_oWrInfo_intestbo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_intestbo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"intestbo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_intestbo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"intestbo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_intestbo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_intestbo);
      l_cSql = "Update "+m_oWrInfo_intestbo.GetTableWriteName()+" set";
