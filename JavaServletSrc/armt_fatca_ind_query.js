...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('NSYKFFGVCJ').value,'C'),w_CONNES)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('fatca_ind',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CONNES') &&  ! (Empty(HtW(Ctrl('NSYKFFGVCJ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CONNES',HtW(Ctrl('NSYKFFGVCJ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_fatca_ind');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('fatca_ind',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['NSYKFFGVCJ'],i=0;i>=0;i--){
