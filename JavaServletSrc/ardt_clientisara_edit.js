...
var ctl = _GetCtl(e,'SXMWFOXFHW');
if (_tracker.goon(ctl,ctrlValue)) {
w_PROQUOTA=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('SXMWFOXFHW',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','N',3,0,'');
...
if ( ! (m_oFather.SaveContext_ardt_clientisara())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_clientisara_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_clientisara_dataNotSent',{'action':formaction});
}
}
}
}
...
l_bResult=false;
w_CODSOG=HtW('','C');
} else if (( ! (_ChkObl('SXMWFOXFHW',1))) && FullRow()) {
_SignErr('SXMWFOXFHW');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_clientisara();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.PROQUOTA={"dec":0,"fixedpos":false,"id":"SXMWFOXFHW","len":3,"name":"PROQUOTA","title":"% Pro Quota","type":"N"};
