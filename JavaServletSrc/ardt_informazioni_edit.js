...
SetDisabled(Ctrl('HKZAUFLHMG_ZOOM'),l_bEnabled);
...
SetDisabled(Ctrl('NATIUIQLJX_ZOOM'),l_bEnabled);
...
SetErrorField('KQHTAXPURL',false);
SetErrorField('PCCSJPWDXE',false);
}
function Set_TIPORAP(ctrlValue,kmode,e) {
if (Ne(w_TIPORAP,ctrlValue)) {
var ctl = _GetCtl(e,'KQHTAXPURL');
if (_tracker.goon(ctl,ctrlValue)) {
w_TIPORAP=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('KQHTAXPURL',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',1,0,'');
...
var ctl = _GetCtl(e,'PCCSJPWDXE');
if (_tracker.goon(ctl,ctrlValue)) {
w_PROQUOTA=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('PCCSJPWDXE',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','N',3,0,'');
...
w_NUMPROG2=( ! (Empty(Utilities.Make(window).GetCallerString('TIPOOPRAP'))) && Ne(At(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'|26|36|38|46'),0) && Empty(w_NUMPROG2) &&  ! (Empty(w_DATAFINE))?Utilities.Make(window).GetCallerString('NUMPROG'):w_NUMPROG2);
w_NUMPROG1=( ! (Empty(Utilities.Make(window).GetCallerString('TIPOOPRAP'))) && Ne(At(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'|21|22|23|24|27|33|34|35|43|44|45'),0) && Empty(w_NUMPROG1) &&  ! (Empty(w_DATAINI))?Utilities.Make(window).GetCallerString('NUMPROG'):w_NUMPROG1);
...
if ( ! (m_oFather.SaveContext_ardt_informazioni())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_informazioni_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_informazioni_dataNotSent',{'action':formaction});
}
}
}
}
...
// * --- Fine Area Manuale
if (l_bResult &&  ! (Le(1,Rows()))) {
l_bResult=false;
m_nLastError=2;
m_cLastMsgError=Translate('MSG_BE_ONE_ROW');
}
return l_bResult;
}
...
if (Eq('HKZAUFLHMG_ZOOM',lc)) {
return 1;
}
if (Eq('NATIUIQLJX_ZOOM',lc)) {
return 1;
}
return -1;
}
...
_FillChildren.n=[];
FillWv.n=["IDBASE","RAPPORTO","gAzienda","tipreg","appoggio","NUMPROG"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
...
l_bResult=false;
w_DATAFINE=HtW('','D');
} else if (( ! (_ChkObl('KQHTAXPURL',1))) && FullRow()) {
_SignErr('KQHTAXPURL');
m_cLastWorkVarErrorMsg='1723736132';
l_bResult=false;
w_TIPORAP=HtW('','C');
} else if (( ! (_ChkObl('PCCSJPWDXE',1))) && FullRow()) {
_SignErr('PCCSJPWDXE');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_informazioni();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.TIPOPERS={"dec":0,"fixedpos":false,"id":"ZDLIYOESSB","len":1,"name":"TIPOPERS","title":"Tipo Soggetto","type":"C"};
l_aRepeatedFields.PRGIMPSOG={"dec":0,"fixedpos":false,"id":"CHJPRCYMKT","len":15,"name":"PRGIMPSOG","title":"Progressivo Import","type":"N"};
l_aRepeatedFields.STATOREG={"dec":0,"fixedpos":false,"id":"EFONTWNVOG","len":1,"name":"STATOREG","title":"Stato Registrazione","type":"C"};
l_aRepeatedFields.IDEREG={"dec":0,"fixedpos":false,"id":"MOQGCUSNQF","len":20,"name":"IDEREG","title":"IDEREG","type":"C"};
...
l_aRepeatedFields.PROQUOTA={"dec":0,"fixedpos":true,"id":"PCCSJPWDXE","len":3,"name":"PROQUOTA","title":"% Pro Quota","type":"N"};
...
if (enable) {
var c;
if(c=Ctrl('KQHTAXPURL')) SetDisabled(c,true);
if(c=Ctrl('PCCSJPWDXE')) SetDisabled(c,true);
SetQueryRow(Ctrl('GridTable'),false);
} else {
SetEditRow(Ctrl('GridTable'),false);
var c;
if(c=Ctrl('KQHTAXPURL')) SetDisabled(c,false);
if(c=Ctrl('PCCSJPWDXE')) SetDisabled(c,false);
