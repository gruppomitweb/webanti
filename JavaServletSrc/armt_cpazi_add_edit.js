...
function Set_flgotp(ctrlValue,kmode,e) {
if (Ne(w_flgotp,ctrlValue)) {
var ctl = _GetCtl(e,'JSKCTWPJGM');
if (_tracker.goon(ctl,ctrlValue)) {
w_flgotp=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('JSKCTWPJGM',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
if ( ! (m_oFather.SaveContext_armt_cpazi_add())) {
...
parent.ZtVWeb.raiseEventToEvalParms('armt_cpazi_add_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_cpazi_add_dataNotSent',{'action':formaction});
}
}
}
}
...
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('JSKCTWPJGM',1))) {
_SignErr('JSKCTWPJGM');
m_cLastWorkVarErrorMsg='10983166297';
l_bResult=false;
w_flgotp='N';
}
if (l_bResult) {
...
l_oWv.setValue('flgotp',WtA(w_flgotp,'C'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
_FillChildren.n=[];
FillWv.n=["codazi","flgotp"];
function SetModified() {
m_bUpdated=true;
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_armt_cpazi_add();
}
}
function EnableEntityWhenReady(enable) {
if (enable) {
var c;
if(c=Ctrl('JSKCTWPJGM')) SetDisabled(c,true);
} else {
var c;
if(c=Ctrl('JSKCTWPJGM')) SetDisabled(c,false);
