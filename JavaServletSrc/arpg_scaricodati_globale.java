...
public class arpg_scaricodati_globale extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*BQVIQTOOZO*!";
  ...
  public String getEntityTitle() {
    return "Creazione File Reimpianto Anag. Rapporti Agenzia Entrate";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_scaricodati_globale";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_scaricodati_globaleBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"10580857810",p_Context.Translate("Creazione File Reimpianto Anag. Rapporti Agenzia Entrate")},
  ...
  void WriteStyles(arpg_scaricodati_globaleBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_scaricodati_globale.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_scaricodati_globaleBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_scaricodati_globaleBO BO,ServletStatus status) {
  ...
  void WriteControls(arpg_scaricodati_globaleBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_scaricodati_globaleBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(329);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_scaricodati_globaleBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='DMMJSVFBEV'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Creazione File Reimpianto Rapporti per Agenzia delle Entrate"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input id='NMNMXABGKO' type='button' class='SPButton' value="+SPLib.ToHTMLValue(status.context.Translate("Crea File Reimpianto"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' onkeydown='ManageKeys(event,false,false,true)' onclick='NMNMXABGKO_Click()'>" +
    "");
    status.out.println(
  ...
  static void PrintState(arpg_scaricodati_globaleBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_scaricodati_globaleBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_scaricodati_globaleBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_scaricodati_globaleBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_scaricodati_globale"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_scaricodati_globale","");
  ...
  void WriteMainFormScript(arpg_scaricodati_globaleBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_scaricodati_globale");
    status.out.println("function Elabora() {");
    status.out.println("  w_bloccaedit = 'S';");
    status.out.println("  EnableControlsUnderCondition();");
    status.out.println("  SetLocationHref(null,'arrt_sd_scaricodati_globale'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')   ");
    status.out.println("}");
    status.out.println("");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
  ...
  arpg_scaricodati_globaleBO getBO(ServletStatus status) {
    arpg_scaricodati_globaleBO BO = new arpg_scaricodati_globaleBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_scaricodati_globaleBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_scaricodati_globaleBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_scaricodati_globaleBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_scaricodati_globaleBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_scaricodati_globaleBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_scaricodati_globaleBO BO) {
    ...
    item.put("altInterfaceFor","arpg_scaricodati_globale");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"322\",\"name\":\"Box_TTRVXROIVP\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"TTRVXROIVP\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"14\",\"href\":\" \",\"name\":\"__iframe_width__655__height__170__id__elaborazione__name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"FGMOCXNBFY_DIV\",\"tabindex\":\"2\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"655"+"'"+" height="+"'"+"170"+"'"+" id="+"'"+"elaborazione"+"'"+" name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"80\",\"x\":\"16\",\"y\":\"113\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Creazione_File_Reimpianto_Rapporti_per_Agenzia_delle_Entrate\",\"page\":\"1\",\"spuid\":\"DMMJSVFBEV_DIV\",\"tabindex\":\"3\",\"type\":\"Label\",\"value\":\"Creazione File Reimpianto Rapporti per Agenzia delle Entrate\",\"w\":\"776\",\"x\":\"8\",\"y\":\"14\",\"zone\":\"TTRVXROIVP_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:NMNMXABGKO_Click()\",\"name\":\"Crea_File_Reimpianto\",\"page\":\"1\",\"spuid\":\"NMNMXABGKO\",\"tabindex\":\"4\",\"type\":\"Button\",\"value\":\"Crea File Reimpianto\",\"w\":\"264\",\"x\":\"512\",\"y\":\"70\",\"zone\":\"TTRVXROIVP_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"dadata\",\"page\":\"1\",\"spuid\":\"FSXHJAAZGG\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"72\",\"y\":\"74\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Data_\",\"page\":\"1\",\"spuid\":\"NOYEUIKFXN_DIV\",\"tabindex\":\"6\",\"type\":\"Label\",\"value\":\"Da Data:\",\"w\":\"43\",\"x\":\"24\",\"y\":\"76\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"a_data\",\"page\":\"1\",\"spuid\":\"CLVVMIYSQG\",\"tabindex\":\"7\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"216\",\"y\":\"74\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Data_\",\"page\":\"1\",\"spuid\":\"LFJQIXRRKM_DIV\",\"tabindex\":\"8\",\"type\":\"Label\",\"value\":\"A Data:\",\"w\":\"43\",\"x\":\"168\",\"y\":\"76\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_AJXISNWTIP()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"AJXISNWTIP\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"9\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"112\",\"zone\":\"TTRVXROIVP_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"__Comunicazione_rapporti_mensili__\",\"page\":\"1\",\"spuid\":\"LJMNABGSBL_DIV\",\"tabindex\":\"10\",\"type\":\"Label\",\"value\":\"- Comunicazione rapporti mensili -\",\"w\":\"776\",\"x\":\"8\",\"y\":\"35\",\"zone\":\"TTRVXROIVP_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"1\",\"name\":\"Box_GSJBAHVGWP\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"GSJBAHVGWP\",\"tabindex\":\"11\",\"type\":\"Box\",\"w\":\"778\",\"x\":\"7\",\"y\":\"147\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Visualizza file creati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_JBNBDKBIAJ()\",\"name\":\"images_search_gif\",\"page\":\"1\",\"spuid\":\"JBNBDKBIAJ\",\"src\":\"images\\/search.gif\",\"tabindex\":\"14\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"688\",\"y\":\"112\",\"zone\":\"TTRVXROIVP_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Dati_Impegno_Alla_Presentazione_Telematica\",\"page\":\"1\",\"spuid\":\"UFZQBBFAGD_DIV\",\"tabindex\":\"15\",\"type\":\"Label\",\"value\":\"Dati Impegno Alla Presentazione Telematica\",\"w\":\"768\",\"x\":\"8\",\"y\":\"154\",\"zone\":\"TTRVXROIVP_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Codice Fiscale Organo Invio File\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"CFINVIO\",\"page\":\"1\",\"spuid\":\"MPIOZQPPPI\",\"tabindex\":\"17\",\"type\":\"Textbox\",\"w\":\"128\",\"x\":\"176\",\"y\":\"175\",\"zone\":\"TTRVXROIVP_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Codice_Fiscale_Organo_Invio_File_\",\"page\":\"1\",\"spuid\":\"CTFPYTZIIH_DIV\",\"tabindex\":\"18\",\"type\":\"Label\",\"value\":\"Codice Fiscale Organo Invio File:\",\"w\":\"156\",\"x\":\"20\",\"y\":\"179\",\"zone\":\"TTRVXROIVP_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Codice CAF Organo Invio\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"CAFINVIO\",\"page\":\"1\",\"spuid\":\"YFPMFPWESC\",\"tabindex\":\"19\",\"type\":\"Textbox\",\"w\":\"48\",\"x\":\"616\",\"y\":\"175\",\"zone\":\"TTRVXROIVP_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Codice_CAF_Organo_Invio_\",\"page\":\"1\",\"spuid\":\"LASRRWOCQH_DIV\",\"tabindex\":\"20\",\"type\":\"Label\",\"value\":\"Codice CAF Organo Invio:\",\"w\":\"125\",\"x\":\"491\",\"y\":\"179\",\"zone\":\"TTRVXROIVP_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Tipo Preparazione FIle\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"name\":\"FILEINVIO\",\"page\":\"1\",\"spuid\":\"HMYRBXBREF\",\"tabindex\":\"21\",\"textlist\":\"Comunicazione predisposta da contribuente,Comunicazione predisposta da chi effettua la trasmissione\",\"type\":\"Combobox\",\"valuelist\":\""+"'"+"1"+"'"+","+"'"+"2"+"'"+"\",\"w\":\"264\",\"x\":\"176\",\"y\":\"203\",\"zone\":\"TTRVXROIVP_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Tipo_Preparazione_FIle_\",\"page\":\"1\",\"spuid\":\"HAINTPIJUS_DIV\",\"tabindex\":\"22\",\"type\":\"Label\",\"value\":\"Tipo Preparazione FIle:\",\"w\":\"111\",\"x\":\"65\",\"y\":\"207\",\"zone\":\"TTRVXROIVP_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Data Impegno Organo Invio\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"DATAINVIO\",\"page\":\"1\",\"spuid\":\"CQATNPZMEC\",\"tabindex\":\"23\",\"type\":\"Textbox\",\"w\":\"80\",\"x\":\"616\",\"y\":\"203\",\"zone\":\"TTRVXROIVP_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Impegno_Organo_Invio_\",\"page\":\"1\",\"spuid\":\"FHSMWCMRII_DIV\",\"tabindex\":\"24\",\"type\":\"Label\",\"value\":\"Data Impegno Organo Invio:\",\"w\":\"138\",\"x\":\"478\",\"y\":\"207\",\"zone\":\"TTRVXROIVP_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stampa contenuto dei file creati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_MPCMAMHQST()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"MPCMAMHQST\",\"src\":\"images\\/print.gif\",\"tabindex\":\"25\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"640\",\"y\":\"112\",\"zone\":\"TTRVXROIVP_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"238\",\"zone\":\"TTRVXROIVP_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_scaricodati_globaleBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_scaricodati_globaleBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_scaricodati_globaleBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_scaricodati_globale?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_scaricodati_globale_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    l_cJsUid = SPPrxycizer.proxycizedPath("arrt_sd_scaricodati_globale.js");
    status.out.println(
     "<script src='../"+l_cJsUid+"' type='text/javascript'>" +
     "</script>" +
    "");
  ...
  void RaiseLoadedEntity(arpg_scaricodati_globaleBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_scaricodati_globaleBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_scaricodati_globaleBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_scaricodati_globaleBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_scaricodati_globaleBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_scaricodati_globaleBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      CPLib.AddToSet(callable,"arrt_sd_scaricodati_globale");
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_scaricodati_globaleBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_scaricodati_globaleBO BO) {
    boolean result = true;
    return result;
  }
}
