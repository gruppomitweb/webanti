...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('RCAIOHOYCN').value,'C'),w_CODTPGIO)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_tbtipgio',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODTPGIO') &&  ! (Empty(HtW(Ctrl('RCAIOHOYCN').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODTPGIO',HtW(Ctrl('RCAIOHOYCN').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_cgo_tgtiporeg(firstAlias) {
return m_cVirtName+".CODTPGIO = "+firstAlias+".TRCODGIO ";
}
...
_chNotF(childAlias,'master','armt_cgo_tbtipgio');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_tbtipgio',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['RCAIOHOYCN'],i=0;i>=0;i--){
