...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('BGJKUPJQFN').value,'C'),w_DACODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_dest_alert',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'DACODICE') &&  ! (Empty(HtW(Ctrl('BGJKUPJQFN').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.DACODICE',HtW(Ctrl('BGJKUPJQFN').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_dest_alert');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_dest_alert',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['BGJKUPJQFN'],i=0;i>=0;i--){
