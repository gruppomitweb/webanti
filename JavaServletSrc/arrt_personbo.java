...
public class arrt_personbo extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*18d52b4c*!";
  public class ServletStatus extends SPRoutineServletStatus {
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_IDBASE;
    public String w_IDBASE;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_NEWID;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_NEWID = source.Has("w_NEWID");
      if (m_bpassedw_NEWID) {
        w_NEWID = source.GetParameter("w_NEWID",source.GetParameter("NEWID",""));
      } else {
        if (source.IsForwarded()) {
          w_NEWID = source.GetCaller().GetString("NEWID","C",10,0);
          m_bpassedw_NEWID = true;
        } else {
          w_NEWID = source.GetParameter("NEWID",source.GetParameter("w_NEWID",""));
        }
      }
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_datavaria) ||  ! (status.m_bpassedw_tipooper) ||  ! (status.m_bpassedw_DATARETT) ||  ! (status.m_bpassedw_CONNES) ||  ! (status.m_bpassedw_NEWID) ||  ! (status.m_bpassedw_IDBASE);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_NEWID)) {
      status.out.println("l_cV=WtA(l_oWnd('w_NEWID',"+SPLib.ToJSValue(status.w_NEWID,"C",10,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_NEWID,"C",10,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_NEWID',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_personboR Rcalled;
    Rcalled = new arrt_personboR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_personboR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_personboR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_personbo"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
          if (CPLib.Empty(Rcalled.m_cLastMsgError) &&  ! (status.m_bApplet)) {
            status.out.println("Error message:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(status.m_cFaultTrace)+"");
          }
        }
        status.out.println("js:w_IDBASE="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_IDBASE),"?",0,0)+"");
        status.out.println("js:w_NEWID="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_NEWID),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_NEWID,null)) {");
        status.out.println("window[f].w_NEWID="+SPLib.ToJSValue(status.w_NEWID,"C",0,0)+";");
        status.out.println("}");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_personbo");
      if ( ! (CPLib.IsNull(l))) {
