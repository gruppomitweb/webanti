...
SPBOUILib.SetInputValue('KWLXEUCJLS',function(){return WtH(w_PROQUOTA,'N',3,0,'')},w_PROQUOTA);
}
if (m_bExtended) {
SetExtensions();
}
...
function KWLXEUCJLS_Valid(e) {
SetActiveField(Ctrl('KWLXEUCJLS'),false);
if (Eq(m_cFunction,'query')) {
DoQuery('PROQUOTA','N',w_PROQUOTA,HtW(Ctrl('KWLXEUCJLS').value,'N'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_PROQUOTA(HtW(Ctrl('KWLXEUCJLS').value,'N'),null,e);
return l_bSetResult;
}
}
function KWLXEUCJLS_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('KWLXEUCJLS'))) {
DisplayErrorMessage();
}
SetActiveField(Ctrl('KWLXEUCJLS'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('KWLXEUCJLS'),e);
}
_ResetTracker();
...
function KWLXEUCJLS_wrap_Valid(event) {
return KWLXEUCJLS_Valid(event);
}
SPBOUILib.SetInputExit('KWLXEUCJLS',KWLXEUCJLS_wrap_Valid,SPBOUILib.Row.IN_GRID);
function KWLXEUCJLS_wrap_OnFocus(event) {
return KWLXEUCJLS_OnFocus(event);
}
SPBOUILib.SetInputEnter('KWLXEUCJLS',KWLXEUCJLS_wrap_OnFocus,SPBOUILib.Row.IN_GRID);
function KWLXEUCJLS_wrap_OnKeyPress(event) {
return CheckNum(event);
}
SPBOUILib.SetInputKeyPress('KWLXEUCJLS',KWLXEUCJLS_wrap_OnKeyPress,SPBOUILib.Row.IN_GRID);
function KWLXEUCJLS_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('KWLXEUCJLS',KWLXEUCJLS_wrap_OnKeyDown,SPBOUILib.Row.IN_GRID);
if (m_bExtended) {
...
m_oFather.i_last_focused_item='child_ardt_clientifrz';
...
function Help() {
windowOpenForeground('../doc/ardt_clientifrz_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
return 'clientifrz'+'\\'+CPLib.ToCPStr(w_IDFILEBO);
}
...
p_d.setValue('ardt_clientifrz',m_cSelectedPage);
...
_Obli('PROQUOTA',1,'KWLXEUCJLS',false,"11230801199")
