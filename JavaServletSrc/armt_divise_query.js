...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('TFYVTTXWGC').value,'C'),w_CODVAL)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbvalute',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODVAL') &&  ! (Empty(HtW(Ctrl('TFYVTTXWGC').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODVAL',HtW(Ctrl('TFYVTTXWGC').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_divise');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbvalute',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['TFYVTTXWGC'],i=0;i>=0;i--){
