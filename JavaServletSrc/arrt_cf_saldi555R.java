...
// * --- Fine Area Manuale
public class arrt_cf_saldi555R implements CallerWithObjs {
  ...
  public String m_cPhName_tmp_agebase;
  public String m_cServer_tmp_agebase;
  ...
  public int _r;
  // * --- Fine Area Manuale
  public arrt_cf_saldi555R (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cf_saldi555",m_Caller);
    ...
    if (m_Ctx.IsTmpAlreadyCreated("tmp_agebase")) {
      m_cPhName_tmp_agebase = p_ContextObject.GetPhName("tmp_agebase");
      if (m_cPhName_tmp_agebase.endsWith(CPSecurity.FILTER_QUERY_POSTFIX)) {
        m_cPhName_tmp_agebase = m_cPhName_tmp_agebase+" "+m_Ctx.GetWritePhName("tmp_agebase");
      }
      m_cServer_tmp_agebase = p_ContextObject.GetServer("tmp_agebase");
    }
  ...
  public double GetNumber(String p_cVarName,String p_cType,int len,int dec) {
    ...
    if (CPLib.eqr("_annsel",p_cVarName)) {
      return _annsel;
    }
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"0000003A")+"delete from "+m_oWrInfo.GetTableWriteName();
      m_cSql = m_cSql+" where ";
      ...
      // * --- Drop temporary table tmp_agebase
      if (m_Ctx.IsSharedTemp("tmp_agebase")) {
        if (m_Ctx.IsTmpAlreadyCreated("tmp_agebase")) {
          m_cServer = m_Ctx.GetServer("tmp_agebase");
          m_cPhName = m_Ctx.GetPhName("tmp_agebase");
          m_cSql = "delete from "+m_cPhName+" where context_id='"+m_Ctx.GetTempID()+"'";
          m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql);
        }
      } else {
        m_Ctx.DropTmp("tmp_agebase");
      }
      // * --- Create temporary table tmp_agebase
      if (m_Ctx.IsSharedTemp("tmp_agebase")) {
        if (m_Ctx.IsTmpAlreadyCreated("tmp_agebase")) {
          m_cServer = m_Ctx.GetServer("tmp_agebase");
          m_cPhName = m_Ctx.GetPhName("tmp_agebase");
      ...
      m_cPhName = m_Ctx.CreateTmpPhName("tmp_agebase");
      if ( ! (m_Ctx.IsSharedTemp("tmp_agebase"))) {
        m_cServer = m_Ctx.GetServer("tmp_agebase");
        m_Sql.SrvQuery(m_cServer,"* ","from "+m_Ctx.PhNameAdapter(m_Sql.ManipulateTablePhName("tmp_agebase",m_cServer,"proto")),m_cPhName,"tmp_agebase",m_Ctx);
      }
      m_cPhName_tmp_agebase = m_cPhName;
      /* _contaglob := 0 */
      _contaglob = CPLib.Round(0,0);
      /* _annsel := w_anno */
      _annsel = CPLib.Round(w_anno,0);
      // Begin Transaction
      m_Sql.RequireTransaction();
      ...
      try {
        ...
        Cursor_aesaldi = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"ANNO="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.Str(w_anno,4,0),"?",0,0,m_cServer, m_oParameters),m_cServer,CPLib.Str(w_anno,4,0))+"  and  ANNULLATO <> 1  and  TIPO='2' "+")"+(m_Ctx.IsSharedTemp("aesaldi")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
        while ( ! (Cursor_aesaldi.Eof())) {
          ...
          w_fileprodotto = _nomefile;
          /* Exec "Crea il file di annullamento" Page 2:Page_2 */
          Page_2();
          ...
          m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
          m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"00000048")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
          ...
          m_cSql = m_cSql+" and TIPO = "+CPSql.SQLValueAdapter(CPLib.ToSQL("4","?",0,0,m_cServer),m_cServer,"4")+"";
        ...
        // * --- End Select
        /* w_numcicli := _conta */
        w_numcicli = CPLib.Round(_conta,0);
        /* _nomefile := LRTrim(gIntemediario)+"_"+Str(w_anno,4,0)+DateTimeToChar(DateTime()) */
        _nomefile = CPLib.LRTrim(gIntemediario)+"_"+CPLib.Str(w_anno,4,0)+CPLib.DateTimeToChar(CPLib.DateTime());
        /* w_fileprodotto := _nomefile */
        w_fileprodotto = _nomefile;
        ...
        /* Exec "Crea il file di testo" Page 5:Page_5 */
        Page_5();
        if ( ! (CPLib.Empty(m_Sql.TransactionErrorMessage()))) {
          throw new RoutineException();
        }
  ...
  void Page_2() throws Exception {
    ...
    try {
      ...
      /* _file := '' */
      _file = "";
      /* cNomeFile := LRTrim(gPathApp)+'/SID/INVII/555/'+LRTrim(gAzienda)+"/"+LRTrim(Str(_conta,2,0))+"/"+LRTrim(_nomefile) */
      cNomeFile = CPLib.LRTrim(gPathApp)+"/SID/INVII/555/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(CPLib.Str(_conta,2,0))+"/"+CPLib.LRTrim(_nomefile);
      /* FileLibMit.CreateDir('SID/INVII/555/'+LRTrim(gAzienda)+"/"+LRTrim(Str(_conta,2,0))) */
      FileLibMit.CreateDir("SID/INVII/555/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(CPLib.Str(_conta,2,0)));
      /* fhand := FileLibMit.OpenWrite('/SID/INVII/555/'+LRTrim(gAzienda)+"/"+LRTrim(Str(_conta,2,0))+"/"+LRTrim(_nomefile)) */
      fhand = FileLibMit.OpenWrite("/SID/INVII/555/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(CPLib.Str(_conta,2,0))+"/"+CPLib.LRTrim(_nomefile));
      ...
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"aesaldi",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"0000008B")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
    ...
    try {
      ...
      while ( ! (Cursor_qbe_inviosaldi_rapd.Eof())) {
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on aecanc into routine arrt_cf_saldi555 returns two or more records. This item should return only a record.");
          ...
          while ( ! (Cursor_aetesta.Eof())) {
            ...
            if (CPLib.ne(CPLib.At(Cursor_aetesta.GetString("TIPOAG"),"|01|02|03|04|05|06|07|09|10|14|15|23|24"),0)) {
              ...
              if ( ! (Read_Cursor.Eof())) {
                ...
                Read_Cursor.Next();
                if ( ! (Read_Cursor.Eof())) {
                  Error l_oErrorFault = new Error("Read on tbvalute into routine arrt_cf_saldi555 returns two or more records. This item should return only a record.");
                  CPStdCounter.Error(l_oErrorFault);
                }
                Read_Cursor.Close();
              } else {
            ...
            while ( ! (Cursor_aeannora.Eof())) {
              ...
              m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_saldi",m_Ctx.GetCompany());
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"000000A8")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
              ...
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"000000A9")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
          ...
          while ( ! (Cursor_aerighe.Eof())) {
            ...
            if (CPLib.le(Cursor_aerighe.GetDate("ADATA"),w_a_data) && (CPLib.Empty(Cursor_aerighe.GetDate("CDATA")) || CPLib.ge(Cursor_aerighe.GetDate("CDATA"),w_dadata))) {
              ...
              if (CPLib.Empty(_prgdel)) {
                ...
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_saldi",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"000000B4")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          while ( ! (Cursor_aederig.Eof())) {
            ...
            if (CPLib.le(Cursor_aederig.GetDate("ADATA"),w_a_data) && (CPLib.Empty(Cursor_aederig.GetDate("CDATA")) || CPLib.ge(Cursor_aederig.GetDate("CDATA"),w_dadata))) {
              ...
              if ( ! (Read_Cursor.Eof())) {
                ...
                Read_Cursor.Next();
                if ( ! (Read_Cursor.Eof())) {
                  Error l_oErrorFault = new Error("Read on aedecanc into routine arrt_cf_saldi555 returns two or more records. This item should return only a record.");
                  CPStdCounter.Error(l_oErrorFault);
                }
                Read_Cursor.Close();
              } else {
              ...
              if (CPLib.Empty(_prgdel)) {
                ...
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_saldi",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"000000BF")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
      ...
      if (CPLib.eqr(w_flgope,"N")) {
            ...
            public void run() {
              if (CPLib.gt(m_TheCursor.CurrentRow()-1,1000*1.1)) {
                ...
                if ( ! (CPLib.IsNull(info))) {
                  info.log("Routine arrt_cf_saldi555: query on tmp_saldi returns "+(m_TheCursor.CurrentRow()-1)+" rows larger then the expected 1000");
                }
              }
            }
            });
        ...
        m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_saldi",m_Ctx.GetCompany());
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"000000C4")+"delete from "+m_oWrInfo.GetTableWriteName();
        m_cSql = m_cSql+" where ";
        m_cSql = m_cSql+m_oWrInfo.WhereFilter("1=1");
        m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql,m_oParameters);
        if (CPLib.lt(m_nUpdatedRows,0)) {
          ...
          while ( ! (Cursor_qbe_inviosaldi_ope.Eof())) {
            ...
            if (CPLib.Empty(CPLib.LRTrim(Cursor_qbe_inviosaldi_ope.GetString("PROGRES")))) {
              ...
              if (CPLib.eqr(w_flgope,"S")) {
                ...
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_saldi",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"000000F0")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
                ...
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"000000F1")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
              ...
              if (CPLib.eqr(w_flgope,"S")) {
                ...
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_saldi",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"00000108")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
                ...
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"aeinvio",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"00000109")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
              ...
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"00000119")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
              m_cSql = m_cSql+"FILEANNO = "+CPLib.ToSQL(_nomefile,"C",50,0,m_cServer)+", ";
  ...
  void Page_5() throws Exception {
    CPResultSet Cursor_intermbo=null;
    CPResultSet Cursor_tmp_agebase=null;
    try {
      ...
      /* _entra := 'N' */
      _entra = "N";
      // * --- Select from tmp_agebase
      m_cServer = m_Ctx.GetServer("tmp_agebase");
      m_cPhName = m_Ctx.GetPhName("tmp_agebase");
      if (Cursor_tmp_agebase!=null)
        Cursor_tmp_agebase.Close();
      Cursor_tmp_agebase = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+(m_Ctx.IsSharedTemp("tmp_agebase")?"context_id='"+m_Ctx.GetTempID()+"'":"1=1")+" order by "+"NUMPROG,PROGR,TIPODAT,PROGR2 ",m_cServer,m_Ctx,true));
      while ( ! (Cursor_tmp_agebase.Eof())) {
        /* _entra := 'S' */
        _entra = "S";
        /* If _i=1 */
        if (CPLib.eqr(_i,1)) {
          ...
          /* _file := 'C' */
          _file = "C";
          /* If Val(tmp_agebase->NUMPROG) <> _ContaC */
          if (CPLib.ne(CPLib.Val(Cursor_tmp_agebase.GetString("NUMPROG")),_ContaC)) {
            // Exit Loop
            if (true) {
              break;
            }
          } else { // Else
            /* _file := _file + tmp_agebase->NUMPROG */
            _file = _file+Cursor_tmp_agebase.GetString("NUMPROG");
          } // End If
        } // End If
        /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->TIPODAT,1) */
        _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("TIPODAT"),1);
        /* If tmp_agebase->TIPODAT='1' */
        if (CPLib.eqr(Cursor_tmp_agebase.GetString("TIPODAT"),"1")) {
          /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->PROGR,10) */
          _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("PROGR"),10);
          /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->TIPORAP,2) */
          _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("TIPORAP"),2);
          /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->DESCRAP,24) */
          _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("DESCRAP"),24);
          /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->CODRAP,50) */
          _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("CODRAP"),50);
          /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->CAB,5) */
          _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("CAB"),5);
          /* _file := _file + tmp_agebase->IMP1 */
          _file = _file+Cursor_tmp_agebase.GetString("IMP1");
          /* _file := _file + tmp_agebase->IMP2 */
          _file = _file+Cursor_tmp_agebase.GetString("IMP2");
          /* _file := _file + tmp_agebase->IMP3 */
          _file = _file+Cursor_tmp_agebase.GetString("IMP3");
          /* _file := _file + tmp_agebase->IMP4 */
          _file = _file+Cursor_tmp_agebase.GetString("IMP4");
          /* _file := _file + tmp_agebase->NUM */
          _file = _file+Cursor_tmp_agebase.GetString("NUM");
          /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->VALUTA,3) */
          _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("VALUTA"),3);
          ...
          /* ElseIf tmp_agebase->TIPODAT='2' */
        } else if (CPLib.eqr(Cursor_tmp_agebase.GetString("TIPODAT"),"2")) {
          /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->PROGR2,5) */
          _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("PROGR2"),5);
          /* _file := _file + LibreriaMit.SpacePad(iif(Empty(tmp_agebase->OLDINTER),_cfinter,tmp_agebase->OLDINTER),16) */
          _file = _file+LibreriaMit.SpacePad((CPLib.Empty(Cursor_tmp_agebase.GetString("OLDINTER"))?_cfinter:Cursor_tmp_agebase.GetString("OLDINTER")),16);
          /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->IDFILE,15) */
          _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("IDFILE"),15);
          /* _file := _file + LibreriaMit.SpacePad(tmp_agebase->IDPROG,25) */
          _file = _file+LibreriaMit.SpacePad(Cursor_tmp_agebase.GetString("IDPROG"),25);
          /* _file := _file + Space(10) */
          _file = _file+CPLib.Space(10);
        ...
        Cursor_tmp_agebase.Next();
      }
      m_cConnectivityError = Cursor_tmp_agebase.ErrorMessage();
      Cursor_tmp_agebase.Close();
      // * --- End Select
      /* If _i > 1 and _i < 11 */
      ...
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"aesaldi",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_cf_saldi555",252,"00000189")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
  ...
  public Forward Run() {
    ...
    try {
      ...
      m_Caller.SetNumber("numcicli","N",2,0,w_numcicli);
      m_Caller.SetString("flgope","C",0,0,w_flgope);
  ...
  public static arrt_cf_saldi555R Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cf_saldi555R(p_Ctx, p_Caller);
  }
  public void Blank() {
    ...
    w_numcicli = m_Caller.GetNumber("numcicli","N",2,0);
    w_flgope = m_Caller.GetString("flgope","C",0,0);
    _min = 0;
    _max = 0;
    _tot = 0;
    _mono = 0;
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_inviosaldi_rapd,qbe_inviosaldi_minmax_ope,qbe_inviosaldi_ope,
  public static final String m_cVQRList = ",qbe_inviosaldi_rapd,qbe_inviosaldi_minmax_ope,qbe_inviosaldi_ope,";
