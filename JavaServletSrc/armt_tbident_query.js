...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('JNWKSQPZVP').value,'C'),w_CODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbident',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODICE') &&  ! (Empty(HtW(Ctrl('JNWKSQPZVP').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODICE',HtW(Ctrl('JNWKSQPZVP').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tbident');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbident',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['JNWKSQPZVP'],i=0;i>=0;i--){
