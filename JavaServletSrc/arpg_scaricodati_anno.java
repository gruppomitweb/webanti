...
public class arpg_scaricodati_anno extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*UREQGSFWBE*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_scaricodati_anno";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_scaricodati_annoBO BO) {
  ...
  void WriteStyles(arpg_scaricodati_annoBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_scaricodati_anno.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_scaricodati_annoBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_scaricodati_annoBO BO,ServletStatus status) {
  ...
  void WriteControls(arpg_scaricodati_annoBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_scaricodati_annoBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(364);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_scaricodati_annoBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<input name='daanno' id='GTMJENMCMN' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_daanno,"N",4,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='5' size='4' fieldtype='N' onkeypress=''>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='YIXZTOKOBR'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Da Anno:"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input name='a_anno' id='YNNFPJLGCM' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_a_anno,"N",4,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='5' size='4' fieldtype='N' onkeypress=''>" +
    "");
    status.out.println(
     "<span class='SPLabel' id='XJFGVEGRXL_DIV'>" +
     "<label id='XJFGVEGRXL'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("A Anno:"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.themeWriter.Header_EndHtml();
  }
  static void PrintState(arpg_scaricodati_annoBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_scaricodati_annoBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_scaricodati_annoBL BO,ServletStatus status) {
    status.out.print("DeclareWVs(");
    status.out.print(SPLib.ToJSValue(BO.w_daanno,"N",4,0));
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_a_anno,"N",4,0));
    status.out.println(")");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      status.out.println("m_bUpdated="+SPLib.ToJSValue(BO.m_bUpdated,"L",0,0)+";");
    }
  }
  void WriteMainFormVarScript(arpg_scaricodati_annoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_scaricodati_anno"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_scaricodati_anno","");
  ...
  void WriteMainFormScript(arpg_scaricodati_annoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_scaricodati_anno");
  ...
  arpg_scaricodati_annoBO getBO(ServletStatus status) {
    arpg_scaricodati_annoBO BO = new arpg_scaricodati_annoBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_scaricodati_annoBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_scaricodati_annoBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_scaricodati_annoBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_scaricodati_annoBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_scaricodati_annoBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_scaricodati_annoBO BO) {
    ...
    item.put("altInterfaceFor","arpg_scaricodati_anno");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"357\",\"name\":\"Box_UZOTDDZZSQ\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"UZOTDDZZSQ\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"daanno\",\"page\":\"1\",\"spuid\":\"GTMJENMCMN\",\"tabindex\":\"2\",\"type\":\"Textbox\",\"w\":\"36\",\"x\":\"104\",\"y\":\"77\",\"zone\":\"UZOTDDZZSQ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:ZUXUDAETFH_Click()\",\"name\":\"Crea_Comunicazione_Deleghe___Operazioni_Extraconto\",\"page\":\"1\",\"spuid\":\"ZUXUDAETFH\",\"tabindex\":\"3\",\"type\":\"Button\",\"value\":\"Crea Comunicazione Deleghe \\/ Operazioni Extraconto\",\"w\":\"302\",\"x\":\"472\",\"y\":\"70\",\"zone\":\"UZOTDDZZSQ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Estrazione_mensile_Deleghe_Operazioni_Extraconto_per_Agenzia_delle_Entrate\",\"page\":\"1\",\"spuid\":\"DMMJSVFBEV_DIV\",\"tabindex\":\"4\",\"type\":\"Label\",\"value\":\"Estrazione mensile Deleghe\\/Operazioni Extraconto per Agenzia delle Entrate\",\"w\":\"760\",\"x\":\"16\",\"y\":\"14\",\"zone\":\"UZOTDDZZSQ_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"__Comunicazione_deleghe_e_o_operazioni_extraconto_mensili__\",\"page\":\"1\",\"spuid\":\"LJMNABGSBL_DIV\",\"tabindex\":\"5\",\"type\":\"Label\",\"value\":\"- Comunicazione deleghe e\\/o operazioni extraconto mensili -\",\"w\":\"760\",\"x\":\"16\",\"y\":\"35\",\"zone\":\"UZOTDDZZSQ_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"name\":\"Box_ZCSEKJZUCO\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"ZCSEKJZUCO\",\"tabindex\":\"6\",\"type\":\"Box\",\"w\":\"760\",\"x\":\"16\",\"y\":\"240\",\"zone\":\"UZOTDDZZSQ_12\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"N_B___I_NOMI_DEI_FILE_CREATI_NON_DEVONO_ESSERE_MODIFICATI_PER_NESSUN_MOTIVO____\",\"page\":\"1\",\"spuid\":\"VKDDBMFRDF_DIV\",\"tabindex\":\"7\",\"type\":\"Label\",\"value\":\"N.B.: I NOMI DEI FILE CREATI NON DEVONO ESSERE MODIFICATI PER NESSUN MOTIVO !!!\",\"w\":\"744\",\"x\":\"24\",\"y\":\"247\",\"zone\":\"ZCSEKJZUCO_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Anno_\",\"page\":\"1\",\"spuid\":\"YIXZTOKOBR_DIV\",\"tabindex\":\"8\",\"type\":\"Label\",\"value\":\"Da Anno:\",\"w\":\"49\",\"x\":\"51\",\"y\":\"81\",\"zone\":\"UZOTDDZZSQ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"14\",\"href\":\" \",\"name\":\"__iframe_width__655__height__170__id__elaborazione__name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"UDOTRXKADP_DIV\",\"tabindex\":\"9\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"655"+"'"+" height="+"'"+"170"+"'"+" id="+"'"+"elaborazione"+"'"+" name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"16\",\"x\":\"20\",\"y\":\"14\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Dati_Impegno_Alla_Presentazione_Telematica\",\"page\":\"1\",\"spuid\":\"UFZQBBFAGD_DIV\",\"tabindex\":\"11\",\"type\":\"Label\",\"value\":\"Dati Impegno Alla Presentazione Telematica\",\"w\":\"776\",\"x\":\"8\",\"y\":\"122\",\"zone\":\"UZOTDDZZSQ_11\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Codice Fiscale Organo Invio File\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"CFINVIO\",\"page\":\"1\",\"spuid\":\"MPIOZQPPPI\",\"tabindex\":\"12\",\"type\":\"Textbox\",\"w\":\"128\",\"x\":\"176\",\"y\":\"161\",\"zone\":\"UZOTDDZZSQ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Codice_Fiscale_Organo_Invio_File_\",\"page\":\"1\",\"spuid\":\"CTFPYTZIIH_DIV\",\"tabindex\":\"13\",\"type\":\"Label\",\"value\":\"Codice Fiscale Organo Invio File:\",\"w\":\"156\",\"x\":\"20\",\"y\":\"165\",\"zone\":\"UZOTDDZZSQ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Codice CAF Organo Invio\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"CAFINVIO\",\"page\":\"1\",\"spuid\":\"YFPMFPWESC\",\"tabindex\":\"14\",\"type\":\"Textbox\",\"w\":\"48\",\"x\":\"616\",\"y\":\"161\",\"zone\":\"UZOTDDZZSQ_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Codice_CAF_Organo_Invio_\",\"page\":\"1\",\"spuid\":\"LASRRWOCQH_DIV\",\"tabindex\":\"15\",\"type\":\"Label\",\"value\":\"Codice CAF Organo Invio:\",\"w\":\"125\",\"x\":\"491\",\"y\":\"165\",\"zone\":\"UZOTDDZZSQ_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Tipo Preparazione FIle\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"name\":\"FILEINVIO\",\"page\":\"1\",\"spuid\":\"HMYRBXBREF\",\"tabindex\":\"16\",\"textlist\":\"Comunicazione predisposta da contribuente,Comunicazione predisposta da chi effettua la trasmissione\",\"type\":\"Combobox\",\"valuelist\":\""+"'"+"1"+"'"+","+"'"+"2"+"'"+"\",\"w\":\"264\",\"x\":\"176\",\"y\":\"189\",\"zone\":\"UZOTDDZZSQ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Tipo_Preparazione_FIle_\",\"page\":\"1\",\"spuid\":\"HAINTPIJUS_DIV\",\"tabindex\":\"17\",\"type\":\"Label\",\"value\":\"Tipo Preparazione FIle:\",\"w\":\"111\",\"x\":\"65\",\"y\":\"193\",\"zone\":\"UZOTDDZZSQ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Data Impegno Organo Invio\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"DATAINVIO\",\"page\":\"1\",\"spuid\":\"CQATNPZMEC\",\"tabindex\":\"18\",\"type\":\"Textbox\",\"w\":\"72\",\"x\":\"616\",\"y\":\"189\",\"zone\":\"UZOTDDZZSQ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Impegno_Organo_Invio_\",\"page\":\"1\",\"spuid\":\"FHSMWCMRII_DIV\",\"tabindex\":\"19\",\"type\":\"Label\",\"value\":\"Data Impegno Organo Invio:\",\"w\":\"138\",\"x\":\"478\",\"y\":\"193\",\"zone\":\"UZOTDDZZSQ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"275\",\"zone\":\"UZOTDDZZSQ_13\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"1\",\"name\":\"Box_UABNRUNBPR\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"UABNRUNBPR\",\"tabindex\":\"22\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"119\",\"zone\":\"UZOTDDZZSQ_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_CUBHMYHWXO()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"CUBHMYHWXO\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"23\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"210\",\"zone\":\"UZOTDDZZSQ_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Visualizza file creati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_JBNBDKBIAJ()\",\"name\":\"images_search_gif\",\"page\":\"1\",\"spuid\":\"JBNBDKBIAJ\",\"src\":\"images\\/search.gif\",\"tabindex\":\"24\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"140\",\"zone\":\"UZOTDDZZSQ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stampa contenuto dei file creati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_MPCMAMHQST()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"MPCMAMHQST\",\"src\":\"images\\/print.gif\",\"tabindex\":\"25\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"175\",\"zone\":\"UZOTDDZZSQ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"a_anno\",\"page\":\"1\",\"spuid\":\"YNNFPJLGCM\",\"tabindex\":\"26\",\"type\":\"Textbox\",\"w\":\"36\",\"x\":\"224\",\"y\":\"77\",\"zone\":\"UZOTDDZZSQ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Anno_\",\"page\":\"1\",\"spuid\":\"XJFGVEGRXL_DIV\",\"tabindex\":\"27\",\"type\":\"Label\",\"value\":\"A Anno:\",\"w\":\"42\",\"x\":\"178\",\"y\":\"81\",\"zone\":\"UZOTDDZZSQ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_scaricodati_annoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_scaricodati_annoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_scaricodati_annoBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_scaricodati_anno?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_scaricodati_anno_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_scaricodati_annoBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_scaricodati_annoBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_scaricodati_annoBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_scaricodati_annoBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_scaricodati_annoBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_scaricodati_annoBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_multi_sd_aprichiudi_delope.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_scaricodati_annoBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_scaricodati_annoBO BO) {
    boolean result = true;
    return result;
  }
}
