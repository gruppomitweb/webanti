...
public class arrt_import_fileae_xls extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*QWSKILJWAX*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_import_fileae_xls";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_f_ricaui) ||  ! (status.m_bpassedw_codval) ||  ! (status.m_bpassedw_prefrap) ||  ! (status.m_bpassedw_msgproc);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_import_fileae_xlsR Rcalled;
    Rcalled = new arrt_import_fileae_xlsR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_import_fileae_xlsR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_import_fileae_xlsR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_import_fileae_xls"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_import_fileae_xls");
      if ( ! (CPLib.IsNull(l))) {
