...
SetHypPar('m_cID',AsAppletValue(m_IDS['arrt_snai_oper_load_dl_multi']));
SendData('start','arrt_snai_oper_load_dl_multi'+'',HParApplet().asString());
}
...
function href_ISPPWIOQCR(p_nRow) {
ChangeRow(p_nRow);
var l_oWv = InitWvApplet();
l_oWv.setValue('w_SNAINUMOPE',AsAppletValue(c_SNAINUMOPE));
l_oWv.setValue('m_cParameterSequence',AsAppletValue('w_SNAINUMOPE'));
_modifyandopen('arrt_cgo_moduli_multi_new'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes',null)
}
...
var action = (Eq(Left('arpg_cgo_operazioni_agenzie_dl'+''+'?',40),'arpg_cgo_operazioni_agenzie_dl_multipli?')?'query':'start');
SetHypPar('m_cAction',AsAppletValue(action));
SetHypPar('m_cMode',AsAppletValue('hyperlink'));
SetHypPar('m_cAtExit',AsAppletValue(close));
SendData(action,'arpg_cgo_operazioni_agenzie_dl'+'',HParApplet().asString());
}
function href_NALIHRVZOR(p_nRow) {
ChangeRow(p_nRow);
var l_oWv = InitWvApplet();
l_oWv.setValue('w_SNAINUMOPE',AsAppletValue(c_SNAINUMOPE));
l_oWv.setValue('m_cParameterSequence',AsAppletValue('w_SNAINUMOPE'));
SetLocationHref(null,'arrt_cgo_moduli_multi_new'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','elaborazione')
}
_ResetTracker();
...
function Help() {
windowOpenForeground('../doc/arpg_cgo_operazioni_agenzie_dl_multipli_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_cgo_operazioni_agenzie_dl_multipli',m_cSelectedPage);
...
GroupFooter_BeginHtml(document,1,'1347923348'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'1347923348'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'1347923350'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_0B97EEE0"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'1347923350'+l_nIdx+'','A')
} else {
}
Row_BeginHtml(document,l_nEvenOdd,'1347909280'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_0BB5B4D8"+l_nIdx+"' style='position:relative;height:19px'>" +
"")
document.writeln(
...
"")
document.writeln(
 "<a href='javascript:href_ISPPWIOQCR("+l_nIdx+")' id='ISPPWIOQCR_"+l_nIdx+"_HREF'>" +
 "<img id='ISPPWIOQCR_"+l_nIdx+"' class='ISPPWIOQCR' style='"+( ! (Eq(w_gStpDwn,'S'))?'':'display:none;visibility:hidden;')+"' src="+"'"+ToResource('images/printer_ico.gif')+"'"+" title="+ToHTMLValue(Translate("10734924728"))+" alt="+ToHTMLValue(Translate("10734924728"),"C",0,0)+" onload=SetSectionsHeight('DIV_0BB5B4D8',"+l_nIdx+")>" +
 "</a>" +
"")
document.writeln(
 "<a href='javascript:href_NALIHRVZOR("+l_nIdx+")' id='NALIHRVZOR_"+l_nIdx+"_HREF'>" +
 "<img id='NALIHRVZOR_"+l_nIdx+"' class='NALIHRVZOR' style='"+( ! (Eq(w_gStpDwn,'N'))?'':'display:none;visibility:hidden;')+"' src="+"'"+ToResource('images/printer_ico.gif')+"'"+" title="+ToHTMLValue(Translate("10734924728"))+" alt="+ToHTMLValue(Translate("10734924728"),"C",0,0)+" onload=SetSectionsHeight('DIV_0BB5B4D8',"+l_nIdx+")>" +
 "</a>" +
"")
document.writeln(
 "<a href='javascript:href_LUNECZXPTF("+l_nIdx+")' id='LUNECZXPTF_"+l_nIdx+"_HREF'>" +
 "<img id='LUNECZXPTF_"+l_nIdx+"' class='LUNECZXPTF' src="+"'"+ToResource('images/grid_delete.gif')+"'"+" title="+ToHTMLValue(Translate("10687827381"))+" alt="+ToHTMLValue(Translate("10687827381"),"C",0,0)+" onload=SetSectionsHeight('DIV_0BB5B4D8',"+l_nIdx+")>" +
 "</a>" +
"")
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'1347909280'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
