...
public class arfn_anomes_rapp extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*4ee62ce3*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arfn_anomes_rappR Rcalled;
    Rcalled = new arfn_anomes_rappR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arfn_anomes_rappR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arfn_anomes_rappR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arfn_anomes_rapp"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
