...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('IHXPLCYXNH').value,'C'),w_RAPCOL)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('xoginfo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'RAPCOL') &&  ! (Empty(HtW(Ctrl('IHXPLCYXNH').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.RAPCOL',HtW(Ctrl('IHXPLCYXNH').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'detail','ardt_provsoginfo');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('xoginfo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['IHXPLCYXNH'],i=0;i>=0;i--){
