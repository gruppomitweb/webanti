...
public class arrt_aelistanomi extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*f94be182*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_datsecf) ||  ! (status.m_bpassedw_datseci) ||  ! (status.m_bpassedw_datprif) ||  ! (status.m_bpassedw_datpris) ||  ! (status.m_bpassedw_a_data) ||  ! (status.m_bpassedw_dadata) ||  ! (status.m_bpassedw_iniz);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_aelistanomiR Rcalled;
    Rcalled = new arrt_aelistanomiR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_aelistanomiR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_aelistanomiR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_aelistanomi"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_aelistanomi");
      if ( ! (CPLib.IsNull(l))) {
