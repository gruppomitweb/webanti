...
// * --- Fine Area Manuale
public class ardt_sogopefrazprovBO extends ardt_sogopefrazprovBL {
  ...
  // * --- Fine Area Manuale
  public ardt_sogopefrazprovBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_sogopefrazprov");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_xogopefbo+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xogopefbo,m_cPhName_xogopefbo,m_cVirtName_xogopefbo,m_cVirtName_xogopefbo+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_xogopefbo,p_IDFILEBO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xogopefbo")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          if (l_oSqlDetailLoader.m_bLink_2_2) {
            w_xdescriz = l_rsLoadBody.GetString("DESCRIZ202");
          } else {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_xogopefbo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND CPROWNUM="+CPLib.ToSQL(w_CPROWNUM,"N",4,0);
        l_cSql = l_cSql+m_oWrInfo_xogopefbo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xogopefbo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_xogopefbo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xogopefbo);
      l_cSql = "insert into "+m_oWrInfo_xogopefbo.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDFILEBO,";
      l_cSql = l_cSql+"CODSOG,";
      l_cSql = l_cSql+"LEGAME,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_xogopefbo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xogopefbo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_xogopefbo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xogopefbo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xogopefbo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xogopefbo);
      l_cSql = "Update "+m_oWrInfo_xogopefbo.GetTableWriteName()+" set";
