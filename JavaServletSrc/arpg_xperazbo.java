...
public class arpg_xperazbo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*IWBEFHYXWH*!";
  ...
  public String getEntityTitle() {
    return "Operazioni relative al rapporto (Provvisorie)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_xperazbo";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_xperazboBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1746849582",p_Context.Translate("Operazioni relative al rapporto (Provvisorie)")},
    {"1991862041",p_Context.Translate("Importo")}};
    return l_translations;
  }
  void WriteStyles(arpg_xperazboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_xperazbo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_xperazboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_xperazboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_xperazboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_xperazboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_xperazboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_xperazboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_xperazboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_xperazboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_xperazbo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_xperazbo","");
    ...
    status.out.println("// * --- arpg_xperazbo");
  ...
  void WriteMainFormScript(arpg_xperazboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_xperazboBO getBO(ServletStatus status) {
    arpg_xperazboBO BO = new arpg_xperazboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_xperazboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_xperazboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_xperazboBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_xperazboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_xperazboBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_xperazboBO BO) {
    ...
    item.put("altInterfaceFor","arpg_xperazbo");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_xperazboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_xperazboBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_xperazboBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_xperazboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_xperazboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_xperazboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_xperazbo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_xperazbo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_xperazboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_xperazboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_xperazboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_xperazboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_xperazboBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_xperazboBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_xperazboBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_xperazboBO BO) {
    boolean result = true;
    return result;
  }
}
