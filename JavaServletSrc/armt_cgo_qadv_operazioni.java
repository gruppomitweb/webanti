...
public class armt_cgo_qadv_operazioni extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*4b6faa64*!";
  static public class ServletStatus extends SPMasterChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "cgo_qadv_operazioni";
    }
    public String w_SNAINUMOPE;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_SNAINUMOPE,context,"cgo_qadv_operazioni","SNAINUMOPE","C",15,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    armt_cgo_qadv_operazioniBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_cgo_qadv_operazioni;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "armt_cgo_qadv_operazioni";
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"cgo_qadv_operazioni"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_cgo_qadv_operazioniBO BO) {
  ...
  void WriteStyles(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_cgo_qadv_operazioni.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_cgo_qadv_operazioni',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_cgo_qadv_operazioni",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b0(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_cgo_qadv_operazioniBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_cgo_qadv_operazioniBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(armt_cgo_qadv_operazioniBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_cgo_qadv_operazioni"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_cgo_qadv_operazioni","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_cgo_qadv_operazioni",BO.GetCurrentState());
  ...
  void WriteMainFormScript(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_armt_cgo_qadv_operazioni);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_armt_cgo_qadv_operazioni();");
      status.out.println("}");
    }
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_cgo_qadv_operazioni',Ne(m_nChildStatus,0));");
    } else {
    ...
    status.out.println("// * --- armt_cgo_qadv_operazioni");
    ...
    status.out.println("m_oFather.m_oarmt_cgo_qadv_operazioni=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  armt_cgo_qadv_operazioniBO getBO(ServletStatus status) {
    armt_cgo_qadv_operazioniBO BO = new armt_cgo_qadv_operazioniBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(armt_cgo_qadv_operazioniBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("cgo_qadv_operazioni",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_cgo_qadv_operazioniBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,armt_cgo_qadv_operazioniBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_cgo_qadv_operazioni;
    String l_cPhName = BO.m_cPhName_cgo_qadv_operazioni;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_cgo_qadv_operazioniBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_cgo_qadv_operazioni+".SNAINUMOPE",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("cgo_qadv_operazioni");
    String l_cPhName = CPSql.ManipulateTablePhName("cgo_qadv_operazioni",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,armt_cgo_qadv_operazioniBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_cgo_qadv_operazioni,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_cgo_qadv_operazioni,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,armt_cgo_qadv_operazioniBO BO) {
  ...
  void Edit(ServletStatus status,armt_cgo_qadv_operazioniBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_cgo_qadv_operazioniBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_cgo_qadv_operazioniBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_cgo_qadv_operazioniBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_cgo_qadv_operazioniBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_cgo_qadv_operazioniBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_cgo_qadv_operazioniBO BO,SPParameterSource source) {
    BO.Load(status.w_SNAINUMOPE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_cgo_qadv_operazioniBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_cgo_qadv_operazioniBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_cgo_qadv_operazioniBO BO) {
    ...
    item.put("altInterfaceFor","armt_cgo_qadv_operazioni");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Motivo No2\",\"h\":21,\"label_text\":\"Tempo di gioco molto breve (meno di 10 minuti)\",\"maxlength\":1,\"name\":\"QADV_DOMANDA04_2\",\"page\":1,\"spuid\":\"TJQKPGPGYX\",\"tabindex\":10,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":264,\"x\":104,\"y\":287,\"zone\":\"pag1_1_9\",\"zonepath\":\"[1,1,9]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Motivo No 4\",\"h\":21,\"label_text\":\"Altro\",\"maxlength\":1,\"name\":\"QADV_DOMANDA04_4\",\"page\":1,\"spuid\":\"AJOULWEBQJ\",\"tabindex\":12,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":56,\"x\":392,\"y\":287,\"zone\":\"pag1_1_9\",\"zonepath\":\"[1,1,9]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_cgo_qadv_operazioni?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_cgo_qadv_operazioni_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_cgo_qadv_operazioni_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_cgo_qadv_operazioni_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","cgo_qadv_operazioni");
  ...
  void RaiseLoadedEntity(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_cgo_qadv_operazioniBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_cgo_qadv_operazioniBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_cgo_qadv_operazioniBO BO,ServletStatus status) throws IOException {
  ...
  void IncludeJS(ServletStatus status) {
    String l_cJsUid;
    status.out.println(
     "<script src='SPPrxy/"+arrt_cgo_punteggio_adv.m_cEntityUid+"/arrt_cgo_punteggio_adv.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_cgo_punteggio_adv.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,armt_cgo_qadv_operazioniBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_cgo_qadv_operazioniBO BO) {
    boolean result = true;
