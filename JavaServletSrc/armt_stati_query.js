...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('LEUKOZNIUN').value,'C'),w_CODSTA)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbstati',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODSTA') &&  ! (Empty(HtW(Ctrl('LEUKOZNIUN').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODSTA',HtW(Ctrl('LEUKOZNIUN').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_stati');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbstati',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['LEUKOZNIUN'],i=0;i>=0;i--){
