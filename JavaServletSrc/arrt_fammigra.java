...
public class arrt_fammigra extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*fefe4c9c*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_fammigraR Rcalled;
    Rcalled = new arrt_fammigraR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_fammigraR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_fammigraR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_fammigra"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
