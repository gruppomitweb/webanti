...
m_oFather.DoQueryFromChild('cgo_operazioni_dl_multi_dt',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
_chNotF(childAlias,'detail','ardt_cgo_oper_multi');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_operazioni_dl_multi_dt',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
