...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SEJWRQHUSP').value,'C'),w_C5CODTAB)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('XREVNLNKCY').value,'C'),w_C5CODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbcateg05',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'C5CODTAB') &&  ! (Empty(HtW(Ctrl('SEJWRQHUSP').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.C5CODTAB',HtW(Ctrl('SEJWRQHUSP').value,'C'),'C');
}
if (Ne(name,'C5CODICE') &&  ! (Empty(HtW(Ctrl('XREVNLNKCY').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.C5CODICE',HtW(Ctrl('XREVNLNKCY').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tbcateg05');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbcateg05',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
