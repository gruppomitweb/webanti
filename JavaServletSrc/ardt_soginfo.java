...
public class ardt_soginfo extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*c37d171b*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "soginfo";
    }
    public String w_RAPCOL;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_RAPCOL,context,"soginfo","RAPCOL","C",25,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_soginfo;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "ardt_soginfo";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 588;
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"soginfo","personbo","tbtipleg"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_soginfoBO BO) {
  ...
  void WriteStyles(ardt_soginfoBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_soginfo.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_soginfoBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(".KQROMFNGNZ {top:0px;position:absolute;left:486px;height:16px;width:56px;background-color:Transparent;z-index:1; }");
      status.out.println(".KQROMFNGNZ_ZOOM { position:absolute;top:0px;left:543px;height:17px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".LRZLVXCRIO {top:0px;position:absolute;left:134px;height:16px;width:344px;background-color:Transparent;z-index:1; }");
      status.out.println(
       "</style>" +
      "");
    } else {
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_soginfo',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_soginfo",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_soginfoBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_soginfoBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:304px;width:588px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(ardt_soginfoBO BO,ServletStatus status) {
    ...
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      status.out.println(
       "<div name id='GridTable_Header' class='GridTable_Header' style='position:absolute;top:2px;left:6px;height:19px;width:582px;z-index:0'>" +
       "</div>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='JWSZDAXYGH_DIV'>" +
       "<label id='JWSZDAXYGH' for='FVKNSPNHTX'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Codice Soggetto"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='PQGSBATIPW_DIV'>" +
       "<label id='PQGSBATIPW' for='KQROMFNGNZ'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Legame"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:absolute;top:21px;left:6px;height:134px;width:582px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:134px;width:582px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:582px;height:134px;overflow:auto;'>" +
       "<table id='GridTable' style='position:relative;top:0px;left:0px;width:576px' cellspacing='0' cellpadding='0' border='0'>" +
       "<tr>" +
       "<td></td>" +
       "</tr>" +
       "</table>" +
      ...
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:21px;left:139px;height:134px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:21px;left:490px;height:134px;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
      status.out.println(
  ...
  void HTMLRowScript(ardt_soginfoBO BO,ServletStatus status) {
    ...
    status.out.println("s='<div style=\"position:relative;width:560px;height:15px;\"';");
    ...
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:560px;height:15px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b1(ardt_soginfoBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(ardt_soginfoBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(ardt_soginfoBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(ardt_soginfoBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b3_CloseBand(ardt_soginfoBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(ardt_soginfoBO BO,ServletStatus status) {
  ...
  static void PrintState(ardt_soginfoBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_soginfoBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_soginfoBL BO,ServletStatus status) {
  ...
  static void PrintStateDetailWorkVars(ardt_soginfoBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_soginfoBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_soginfoBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_soginfoBO.TrsRow) BO.m_oDeletedRows.get(k);
  ...
  void WriteMainFormVarScript(ardt_soginfoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(588,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_soginfo"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_soginfo","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_soginfo",BO.GetCurrentState());
  ...
  void WriteMainFormScript(ardt_soginfoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_soginfo);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_soginfo();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_soginfo',Ne(m_nChildStatus,0));");
    } else {
    ...
    status.out.println("m_oFather.m_oardt_soginfo=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_soginfoBO getBO(ServletStatus status) {
    ardt_soginfoBO BO = new ardt_soginfoBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_soginfoBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("soginfo",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_soginfoBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_soginfoBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ardt_soginfoBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_soginfo;
    String l_cPhName = BO.m_cPhName_soginfo;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_soginfoBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_soginfo+".RAPCOL"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_soginfo+".RAPCOL"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("soginfo");
    String l_cPhName = CPSql.ManipulateTablePhName("soginfo",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_soginfoBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_soginfo),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_soginfo,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_soginfo+".RAPCOL) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_soginfo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_soginfo,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_soginfo+".RAPCOL",CPLib.GetTablesWithAliases(BO.m_cPhName_soginfo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_soginfoBO BO) {
  ...
  void Edit(ServletStatus status,ardt_soginfoBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_soginfoBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_soginfoBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_soginfoBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_soginfoBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_soginfoBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_soginfoBO BO,SPParameterSource source) {
    BO.Load(status.w_RAPCOL);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_soginfoBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_soginfoBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_soginfoBO BO) {
    ...
    item.put("altInterfaceFor","ardt_soginfo");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"h\":16,\"name\":\"label_JWSZDAXYGH\",\"page\":1,\"spuid\":\"JWSZDAXYGH\",\"tabindex\":2,\"type\":\"Label\",\"typevar\":\"logic\",\"value\":\"Codice Soggetto\",\"w\":128,\"x\":8,\"y\":3,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_PQGSBATIPW\",\"page\":1,\"spuid\":\"PQGSBATIPW\",\"tabindex\":3,\"type\":\"Label\",\"value\":\"Legame\",\"w\":56,\"x\":494,\"y\":3,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":130,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"XGAFVNMHHB\",\"type\":\"BodyDetail\",\"w\":598,\"x\":8,\"y\":22,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":39,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"PMXISLDUXZ\",\"type\":\"Box\",\"w\":542,\"x\":8,\"y\":2,\"zone\":\"pag1_5\",\"zonepath\":\"[1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Ragione Sociale\",\"disabled\":\"true\",\"h\":17,\"maxlength\":70,\"name\":\"xragsoc\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"LRZLVXCRIO\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":344,\"x\":142,\"y\":24,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Tipo Legame\",\"h\":17,\"maxlength\":1,\"name\":\"LEGAME\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"KQROMFNGNZ\",\"tabindex\":2,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":56,\"x\":494,\"y\":24,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_soginfoBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_soginfoBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_soginfo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_soginfo_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_soginfo_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_soginfo_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(ardt_soginfoBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_soginfoBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_soginfoBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_soginfoBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_soginfoBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_soginfoBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_soginfoBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ardt_soginfoBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ardt_soginfoBO BO) {
    boolean result = true;
