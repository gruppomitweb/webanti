...
function DisplayErrorMessage_armt_wersonbo_agg() {
if ( ! (ChildToLoad(FrameRef('armt_wersonbo_agg'))) && Ne(typeof(FrameRef('armt_wersonbo_agg').DisplayErrorMessage),'undefined')) {
FrameRef('armt_wersonbo_agg').DisplayErrorMessage();
}
}
...
Z.Tabs.Get("tabs_2_Altri").SetDisable('AYKUXGKLPO',true);
}
if (IsDisabledByStateDriver('Dipendenza')) {
Z.Tabs.Get("tabs_2_Altri").SetDisable('ZETZWJBOFM',true);
}
if (IsDisabledByStateDriver('SogCollegati')) {
Z.Tabs.Get("tabs_2_Altri").SetDisable('XDDGTPJDHN',true);
}
...
Z.Tabs.Get("tabs_4_Rischio").SetDisable('WJVOGIFRCK',true);
}
if (IsDisabledByStateDriver('MitRisk')) {
Z.Tabs.Get("tabs_4_Rischio").SetDisable('XTDFJTMZAQ',true);
}
if (IsDisabledByStateDriver('Patrimonio')) {
Z.Tabs.Get("tabs_4_Rischio").SetDisable('TKKVDEVHBF',true);
}
if (IsDisabledByStateDriver('Reddito')) {
Z.Tabs.Get("tabs_4_Rischio").SetDisable('STJESIOURE',true);
}
...
l_Appl.SetFields('CODINTER,ITFLGPEP');
l_Appl.SetTypes('C,C');
...
parent.ZtVWeb.raiseEventToEvalParms('armt_wersonbo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_wersonbo_dataNotSent',{'action':formaction});
}
}
}
}
CheckChild.child=[{'prg':"ardt_personbo_dip",'BOScId':function(){ return FrameRef('ardt_personbo_dip')},'repeated':0},{'prg':"armt_wersonbo_agg",'BOScId':function(){ return FrameRef('armt_wersonbo_agg')},'repeated':0},{'prg':"ardt_soggope",'BOScId':function(){ return FrameRef('ardt_soggope')},'repeated':0},{'prg':"ardt_fatca_ind",'BOScId':function(){ return FrameRef('ardt_fatca_ind')},'repeated':0},{'prg':"ardt_patrimonio",'BOScId':function(){ return FrameRef('ardt_patrimonio')},'repeated':0},{'prg':"ardt_reddito",'BOScId':function(){ return FrameRef('ardt_reddito')},'repeated':0}];
function Check() {
var l_bResult = true;
...
_FillChildren.n=['ardt_personbo_dip','armt_wersonbo_agg','ardt_soggope','ardt_fatca_ind','ardt_patrimonio','ardt_reddito'];
FillWv.n=["TIPOPERS","COGNOME","NOME","CFESTERO","RAGSOC","PAESE","DESCCIT","idcitta","PROVINCIA","CAP","CODCAB","DOMICILIO","IDENT","IDNASCOMUN","NASCOMUN","TIPINTER","IDNASSTATO","NASSTATO","SESSO","DATANASC","TIPODOC","NUMDOCUM","DATARILASC","DATAVALI","AUTRILASC","CODFISC","PARTIVA","CONTO","SOTGRUP","RAMOGRUP","ATTIV","SETTSINT","CONNES","NOSARA","NOTIT","PROGIMPORT","OLDSETSIN","CRIMEDATE","PEPDATE","DATAFINE","DATAINI","DISATTIVO","TIPOSOGGETTO","RAPSEGNA","SOGAFFIDATARIO","CAUSALE","FATTURATO","FLGNOTRIM","DATAIDENT","CODINTER","NUMIMP","CODDIPE","SOSPMAF","OLDCONNES","DATEXTRA","ALLINEATO","ALLINEATONOMECNOME","RAGSOCOLD","PROGIMP_C","IDFILE","OLDCODFISC","MACROAGENTE","SPOTID","DATAVARAGE","SETTSINT2","TELEF","TIPOORO","CODORO","FLGVALIDO","DAC6","ALTRODOM","ALTROCAP","IMPORTOMAX","FREQUENZA","NUMOPERAZ","DATASIGN","IDBASE","FLAGDAC6","BANCABEN","DATASEGN","DATARAUT","CODLEGRAP","FATCA","TIN","ODB","CITTADIN1","CITTADIN2","RESFISC","AREAGEO","RAREA","PKTBSPECIE","RNATGIU","RATTIV","COMPORT","RCOMP","DOCFILE","RISGLOB","DATAPROF","DATAREVPROF","PEP","CRIME","MITIG","MITIGDOC","VERIFICA","sanumpro","datamodi","NOTERIS","gIntemediario","gFlgWU","gSeekAos","gDataVaria","gFlgDoc","gTipInter","gVerSim","gCodDip","dataoggi","nascomunid","nasstatoid","DATARETT","STATOREG","NUMPROG","tipsot","DESCSGRU","DESCRAMGRU","DESCUIC","dessta","DESCRI","xstatonas","finesae","finesint","xFLGVALIDO","xFLGANAVAL","xDSETSIN","xdesctipatt","xdescareag","xdescnatgiu","xdesccomp","xITFLGPEP","COMPLETA","ragbanben","xDESDIPE","xRAGLEGRAP","xIDLEGRAP","xdesfatca","xDesCit1","xDesCit2","xResFis","ATTIVR","FLGALTO"];
...
function SetChildVariables_armt_wersonbo_agg() {
}
function CtxLoad_armt_wersonbo_agg() {
if (ChildToLoad(FrameRef('armt_wersonbo_agg'))) {
return [LoadContext(0,'OIREAEXNKZ'),'armt_wersonbo_agg'];
}
}
