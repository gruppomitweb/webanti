...
function Help() {
windowOpenForeground('../doc/arpg_kersonbo_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_kersonbo',m_cSelectedPage);
...
function TrsQueryRow() {
  this.c_datamodi = arguments[0]
  this.c_OLDCONNES = arguments[1]
...
GroupFooter_BeginHtml(document,1,'10931431147'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'10931431147'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'10897876715'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_074A6498"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
 "<span class=' SPLabel RGZCLGJAYL_DIV'>" +
 "<label class='RGZCLGJAYL'>" +
 ""+ToHTag(Translate("12038003070"))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'10897876715'+l_nIdx+'','A')
} else {
}
Row_BeginHtml(document,l_nEvenOdd,'10730104604'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_071AD3D0"+l_nIdx+"' style='position:relative;height:18px'>" +
"")
document.writeln(
 "<a href='javascript:href_XZMBQDKCOI("+l_nIdx+")' id='XZMBQDKCOI_"+l_nIdx+"_HREF'>" +
 "<img id='XZMBQDKCOI_"+l_nIdx+"' class='XZMBQDKCOI' src="+"'"+ToResource('images/view.png')+"'"+" alt='' onload=SetSectionsHeight('DIV_071AD3D0',"+l_nIdx+")>" +
 "</a>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'10730104604'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
