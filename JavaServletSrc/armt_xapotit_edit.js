...
function DisplayErrorMessage_ardt_xntestit() {
if ( ! (ChildToLoad(FrameRef('ardt_xntestit'))) && Ne(typeof(FrameRef('ardt_xntestit').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_xntestit').DisplayErrorMessage();
}
}
...
l_bWarn=WindowConfirm(AlertErrorMessage('1016682754'));
} finally {
...
if ( ! (ChildToLoad(FrameRef('ardt_xntestit'))) && Ne(typeof(FrameRef('ardt_xntestit').DoUpdate),'undefined')) {
CopyWorkVar(FrameRef('ardt_xntestit'),['w_tipooprap'],['TIPOOPRAP'],true)
}
...
parent.ZtVWeb.raiseEventToEvalParms('armt_xapotit_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_xapotit_dataNotSent',{'action':formaction});
}
}
}
}
CheckChild.child=[{'prg':"ardt_xntestit",'BOScId':function(){ return FrameRef('ardt_xntestit')},'repeated':0}];
function Check() {
var l_bResult = true;
...
_FillChildren.n=['ardt_xntestit'];
FillWv.n=["CODINTER","CODDIPE","CONNESCLI","DATAOPE","CDATOPE","DATAREG","TIPOOPRAP","CONNESOPER","TIPOLEG","RAPPORTO","DATARETT","IDBASE","TIPOINTER","TIPOINF","IDEREG","FLAGRAP","OPRAP","STATOREG","OPERAZMOD","OPERATORE","AUTOM","DESCCIT","PROVINCIA","CODCAB","NUMPROG","FLAGRAP2","FLAGLIRE","PRGIMPTIT","DESCINTER","CDATREG","flgpg","flgpf","gCodDip","gIntemediario","xdescrap","xDescDipe","datpaseuro","gChkDate","xDesReg","xRagSoc","sEdit","errmsg","Aggiorna","lancio","VALUTA","gSblocco","tipreg","tipregn","dataoggi","gStatus","gDataVaria","CITINT","CITDIP","PROVDIP","CABDIP","PROVINT","CABINT","xRagSocCli","filtro","SESSO"];
function SetModified() {
m_bUpdated=true;
}
function SetChildVariables_ardt_xntestit() {
if ( ! (ChildToLoad(FrameRef('ardt_xntestit'))) && Ne(typeof(FrameRef('ardt_xntestit').DoUpdate),'undefined')) {
CopyWorkVar(FrameRef('ardt_xntestit'),['w_tipooprap'],['TIPOOPRAP'],false)
}
}
function CtxLoad_ardt_xntestit() {
if (ChildToLoad(FrameRef('ardt_xntestit'))) {
return [LoadContext(0,'XSGICXLWWX'),'ardt_xntestit'];
}
}
