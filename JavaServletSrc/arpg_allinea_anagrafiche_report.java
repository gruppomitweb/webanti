...
public class arpg_allinea_anagrafiche_report extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*SQYWZCZAJT*!";
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    arpg_allinea_anagrafiche_reportBO BO;
    BO = getBO(status);
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_allinea_anagrafiche_report";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 285;
    ...
    l_eg.m_cQueryName = "qbe_aa_files";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_allinea_anagrafiche_reportBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1531009962",p_Context.Translate("Elenco stampe allineamento anagrafiche")},
    {"1783054285",p_Context.Translate("Nome Report")},
    {"345316",p_Context.Translate("TIPO")}};
    return l_translations;
  }
  void WriteStyles(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_allinea_anagrafiche_report.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_allinea_anagrafiche_reportBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_allinea_anagrafiche_reportBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_allinea_anagrafiche_reportBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(285,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_allinea_anagrafiche_report"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_allinea_anagrafiche_report","");
  ...
  void WriteMainFormScript(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_allinea_anagrafiche_reportBO getBO(ServletStatus status) {
    arpg_allinea_anagrafiche_reportBO BO = new arpg_allinea_anagrafiche_reportBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_allinea_anagrafiche_reportBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    ...
    item.put("altInterfaceFor","arpg_allinea_anagrafiche_report");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    status.out.println("var c_AATIPERR;");
    status.out.println("var c_AANOMREP;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_allinea_anagrafiche_reportBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_AATIPERR,"C",3,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_AANOMREP,"C",30,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_allinea_anagrafiche_report?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_allinea_anagrafiche_report_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_allinea_anagrafiche_reportBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_allinea_anagrafiche_reportBO BO) {
    boolean result = true;
    return result;
  }
}
