...
// * --- Fine Area Manuale
public class ardt_delegaopBO extends ardt_delegaopBL {
  ...
  // * --- Fine Area Manuale
  public ardt_delegaopBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_delegaop");
    }
  ...
  public boolean Load(String p_COLLEG,String p_ANNO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_delegaop,m_cPhName_delegaop,m_cVirtName_delegaop,m_cVirtName_delegaop+".COLLEG="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_COLLEG,"C",16,0),m_cServer_delegaop,p_COLLEG)+" AND "+m_cVirtName_delegaop+".ANNO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_ANNO,"C",4,0),m_cServer_delegaop,p_ANNO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"delegaop")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_delegaop.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"COLLEG="+CPLib.ToSQL(w_COLLEG,"C",16,0)+" AND ANNO="+CPLib.ToSQL(w_ANNO,"C",4,0)+" AND CODINTER="+CPLib.ToSQL(opk_CODINTER,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_delegaop.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"delegaop")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_delegaop,l_cSql);
  ...
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"QVNGTSMQDW")) {
      ...
      result.AppendFieldsHash("QVNGTSMQDW",CPLib.GetLinkedFieldProps("CONNES","CODINTER","C",16,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_delegaop);
      l_cSql = "insert into "+m_oWrInfo_delegaop.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_delegaop.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"delegaop")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_delegaop.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"delegaop")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_delegaop,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_delegaop);
      l_cSql = "Update "+m_oWrInfo_delegaop.GetTableWriteName()+" set";
