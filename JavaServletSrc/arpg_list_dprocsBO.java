...
// * --- Fine Area Manuale
public class arpg_list_dprocsBO extends arpg_list_dprocsBL {
  ...
  public arpg_list_dprocsBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_DPRPROGRE = rs.GetDouble("DPRPROGRE");
          c_DPRTXTMSG = rs.GetString("DPRTXTMSG");
          c_DPRNUMRIG = rs.GetDouble("DPRNUMRIG");
          c_DPRESITO = rs.GetString("DPRESITO");
          AddTrsRow();
          rs.Next();
          l_nCnt = l_nCnt+1;
        }
