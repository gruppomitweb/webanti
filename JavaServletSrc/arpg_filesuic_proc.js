...
function Help() {
windowOpenForeground('../doc/arpg_filesuic_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_filesuic',m_cSelectedPage);
...
function TrsFWRow() {
  ...
  this.m_nRowStatus = arguments[1]
}
function TrsQueryRow() {
  this.c_idfile = arguments[0]
  this.c_nomefile = arguments[1]
  this.c_datafile = arguments[2]
  this.c_descfile = arguments[3]
}
function TrsToWork() {
var row;
...
GroupFooter_BeginHtml(document,1,'1814033479'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'1814033479'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'1847587911'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_040FC890"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
 "<span class=' SPLabel MUEMGMMASE_DIV'>" +
 "<label class='MUEMGMMASE'>" +
 ""+ToHTag(Translate("894014601"))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'1847587911'+l_nIdx+'','A')
} else {
document.writeln(
 "<input name='gUrlApp_"+l_nIdx+"' id='LNTUFAZHYS_"+l_nIdx+"' type='hidden' value="+ToHTMLValue(WtH(w_gUrlApp,"C",80,0,''))+" size='80'>" +
"")
}
Row_BeginHtml(document,l_nEvenOdd,'2015360022'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_039A9128"+l_nIdx+"' style='position:relative;height:18px'>" +
"")
document.writeln(
 "<span class=' SPLabel LUPXATMLBQ_DIV'>" +
 "<label class='LUPXATMLBQ'>" +
 ""+ToHTag(WtH(c_idfile,'C',10,0,''))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'2015360022'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
