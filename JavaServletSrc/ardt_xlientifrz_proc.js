...
SPBOUILib.SetInputValue('MHVIWLRXHO',function(){return WtH(w_PROQUOTA,'N',3,0,'')},w_PROQUOTA);
}
if (m_bExtended) {
SetExtensions();
}
...
function MHVIWLRXHO_Valid(e) {
SetActiveField(Ctrl('MHVIWLRXHO'),false);
if (Eq(m_cFunction,'query')) {
DoQuery('PROQUOTA','N',w_PROQUOTA,HtW(Ctrl('MHVIWLRXHO').value,'N'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_PROQUOTA(HtW(Ctrl('MHVIWLRXHO').value,'N'),null,e);
return l_bSetResult;
}
}
function MHVIWLRXHO_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('MHVIWLRXHO'))) {
DisplayErrorMessage();
}
SetActiveField(Ctrl('MHVIWLRXHO'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('MHVIWLRXHO'),e);
}
_ResetTracker();
...
function MHVIWLRXHO_wrap_Valid(event) {
return MHVIWLRXHO_Valid(event);
}
SPBOUILib.SetInputExit('MHVIWLRXHO',MHVIWLRXHO_wrap_Valid,SPBOUILib.Row.IN_GRID);
function MHVIWLRXHO_wrap_OnFocus(event) {
return MHVIWLRXHO_OnFocus(event);
}
SPBOUILib.SetInputEnter('MHVIWLRXHO',MHVIWLRXHO_wrap_OnFocus,SPBOUILib.Row.IN_GRID);
function MHVIWLRXHO_wrap_OnKeyPress(event) {
return CheckNum(event);
}
SPBOUILib.SetInputKeyPress('MHVIWLRXHO',MHVIWLRXHO_wrap_OnKeyPress,SPBOUILib.Row.IN_GRID);
function MHVIWLRXHO_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('MHVIWLRXHO',MHVIWLRXHO_wrap_OnKeyDown,SPBOUILib.Row.IN_GRID);
if (m_bExtended) {
...
m_oFather.i_last_focused_item='child_ardt_xlientifrz';
...
function Help() {
windowOpenForeground('../doc/ardt_xlientifrz_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
return 'xlientifrz'+'\\'+CPLib.ToCPStr(w_IDFILEBO);
}
...
p_d.setValue('ardt_xlientifrz',m_cSelectedPage);
...
_Obli('PROQUOTA',1,'MHVIWLRXHO',false,"11230801199")
