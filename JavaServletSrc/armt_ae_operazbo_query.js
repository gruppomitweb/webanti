...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('YPYBCLFHOV').value,'C'),w_IDBASE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('jperazbo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDBASE') &&  ! (Empty(HtW(Ctrl('YPYBCLFHOV').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDBASE',HtW(Ctrl('YPYBCLFHOV').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_ae_operazbo');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('jperazbo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['YPYBCLFHOV'],i=0;i>=0;i--){
