...
// * --- Fine Area Manuale
public class arrt_cgo_alert_operazioniR implements CallerWithObjs {
  ...
  public DateTime dt;
  // * --- Fine Area Manuale
  public arrt_cgo_alert_operazioniR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_alert_operazioni",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_cgo_alert_operazioni";
  ...
  void Page_1() throws Exception {
    ...
    CPResultSet Cursor_cgo_dest_alert=null;
    try {
      /* _oldazi Char(10) */
      ...
      while ( ! (Cursor_cpazi.Eof())) {
        ...
        try {
          ...
          Cursor_intermbo = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select ALTSRVMAIL,ALTPRTMAIL,ALTEMLMAIL,ALTUSRMAIL,ALTPWDMAIL  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+(m_Ctx.IsSharedTemp("intermbo")?"context_id='"+m_Ctx.GetTempID()+"'":"1=1"),m_cServer,m_Ctx,true));
          ...
          // * --- End Select
          /* _10start := 0 */
          _10start = CPLib.Round(0,0);
          /* _20start := 0 */
          _20start = CPLib.Round(0,0);
          /* _testa := "Operazioni_10_giorni_"+DateToChar(Date())+".xls" */
          _testa = "Operazioni_10_giorni_"+CPLib.DateToChar(CPLib.Date())+".xls";
          ...
          Cursor_cgo_operazioni = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"CGOFLGVLT <> 'S' "+")"+(m_Ctx.IsSharedTemp("cgo_operazioni")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
          while ( ! (Cursor_cgo_operazioni.Eof())) {
            /* _col1 := arfn_fdate(cgo_operazioni->DATAOPE) */
            _col1 = arfn_fdateR.Make(m_Ctx,this).Run(Cursor_cgo_operazioni.GetDate("DATAOPE"));
            /* _col2 := LRTrim(cgo_operazioni->CAIDBIGLIETTO) */
            _col2 = CPLib.LRTrim(Cursor_cgo_operazioni.GetString("CAIDBIGLIETTO"));
            /* _col3 := cgo_operazioni->TOTLIRE */
            _col3 = CPLib.Round(Cursor_cgo_operazioni.GetDouble("TOTLIRE"),2);
            /* _codloc := '' */
            _codloc = "";
            /* _nomloc := '' */
            _nomloc = "";
            ...
            m_cSql = "";
            m_cSql = m_cSql+"CODDIP="+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_cgo_operazioni.GetString("CODDIPE"),"C",6,0,m_cServer),m_cServer,Cursor_cgo_operazioni.GetString("CODDIPE"));
            ...
            if ( ! (Read_Cursor.Eof())) {
              ...
              Read_Cursor.Next();
              if ( ! (Read_Cursor.Eof())) {
                Error l_oErrorFault = new Error("Read on anadip into routine arrt_cgo_alert_operazioni returns two or more records. This item should return only a record.");
            ...
            m_cSql = "";
            m_cSql = m_cSql+"CODTPGIO="+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_cgo_operazioni.GetString("TIPOGIOCO"),"C",3,0,m_cServer),m_cServer,Cursor_cgo_operazioni.GetString("TIPOGIOCO"));
            ...
            if ( ! (Read_Cursor.Eof())) {
              ...
              Read_Cursor.Next();
              if ( ! (Read_Cursor.Eof())) {
                Error l_oErrorFault = new Error("Read on cgo_tbtipgio into routine arrt_cgo_alert_operazioni returns two or more records. This item should return only a record.");
                CPStdCounter.Error(l_oErrorFault);
              }
              Read_Cursor.Close();
            } else {
            ...
            /* _col9 :=  LRTrim(arfn_dectabs('TPO', LRTrim(cgo_operazioni->CAFLGOPE))) */
            _col9 = CPLib.LRTrim(arfn_dectabsR.Make(m_Ctx,this).Run("TPO",CPLib.LRTrim(Cursor_cgo_operazioni.GetString("CAFLGOPE"))));
            /* If DateDiff(cgo_operazioni->DATAOPE,Date()) >= 10 */
            if (CPLib.ge(CPLib.DateDiff(Cursor_cgo_operazioni.GetDate("DATAOPE"),CPLib.Date()),10)) {
              /* If _10start=0 */
              if (CPLib.eqr(_10start,0)) {
                /* _mail10 := 'Segnalazione operazioni non chiuse registrate da almeno 10 giorni' */
                _mail10 = "Segnalazione operazioni non chiuse registrate da almeno 10 giorni";
                /* _10start := 1 */
                _10start = CPLib.Round(1,0);
              } // End If
              // Scrive i dati 
          ...
          prova.close();
          /* If _10start=1 */
          if (CPLib.eqr(_10start,1)) {
            // * --- Select from cgo_dest_alert
            m_cServer = m_Ctx.GetServer("cgo_dest_alert");
            m_cPhName = m_Ctx.GetPhName("cgo_dest_alert");
            if (Cursor_cgo_dest_alert!=null)
              Cursor_cgo_dest_alert.Close();
            Cursor_cgo_dest_alert = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"DATIPALT='1' "+")"+(m_Ctx.IsSharedTemp("cgo_dest_alert")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
            while ( ! (Cursor_cgo_dest_alert.Eof())) {
              /* Sp_Lib_SendMail.sendMailPers(_mail10,'TXT','Segnalazione Operazioni non chiuse',LRTrim(_srvmail),LRTrim(cgo_dest_alert->DAEMAIL),'','',LRTrim(_emlmail),LRTrim(nomefile),LRTrim(_usrmail),LRTrim(_pwdmail),'N',LRTrim(_prtmail),'','WEBANTI') */
              Sp_Lib_SendMail.sendMailPers(_mail10,"TXT","Segnalazione Operazioni non chiuse",CPLib.LRTrim(_srvmail),CPLib.LRTrim(Cursor_cgo_dest_alert.GetString("DAEMAIL")),"","",CPLib.LRTrim(_emlmail),CPLib.LRTrim(nomefile),CPLib.LRTrim(_usrmail),CPLib.LRTrim(_pwdmail),"N",CPLib.LRTrim(_prtmail),"","WEBANTI");
              Cursor_cgo_dest_alert.Next();
            }
            m_cConnectivityError = Cursor_cgo_dest_alert.ErrorMessage();
            Cursor_cgo_dest_alert.Close();
            // * --- End Select
          } // End If
          /* _testa := "Operazioni_20_giorni_"+DateToChar(Date())+".xls" */
          _testa = "Operazioni_20_giorni_"+CPLib.DateToChar(CPLib.Date())+".xls";
          /* nomefile := LRTrim(_gPathApp)+"/export/"+LRTrim(_gAzienda)+"/"+LRTrim(_testa) */
          nomefile = CPLib.LRTrim(_gPathApp)+"/export/"+CPLib.LRTrim(_gAzienda)+"/"+CPLib.LRTrim(_testa);
          // Crea il foglio e scrive la testata
          ws = new WorkbookSettings();
          prova = Workbook.createWorkbook(new File(nomefile), ws);
          foglio = prova.createSheet("Operazioni", 0);
          stringa = new Label(0,0,"Data Operazione",arial11format);
          foglio.addCell(stringa); 
          stringa = new Label(1,0,"ID Ticket",arial11format);
          foglio.addCell(stringa);
          stringa = new Label(2,0,"Importo",arial11format);
          ...
          while ( ! (Cursor_cgo_operazioni.Eof())) {
            ...
            /* If DateDiff(cgo_operazioni->DATAOPE,Date()) >= 20 */
            if (CPLib.ge(CPLib.DateDiff(Cursor_cgo_operazioni.GetDate("DATAOPE"),CPLib.Date()),20)) {
              /* If _20start=0 */
              if (CPLib.eqr(_20start,0)) {
                /* _mail20 := 'Segnalazione operazioni non chiuse registrate da almeno 20 giorni' */
                _mail20 = "Segnalazione operazioni non chiuse registrate da almeno 20 giorni";
                /* _20start := 1 */
                _20start = CPLib.Round(1,0);
              } // End If
              // Scrive i dati 
              stringa = new Label(0,riga,_col1);
              foglio.addCell(stringa); 
          ...
          prova.close();
          /* If _20start=1 */
          if (CPLib.eqr(_20start,1)) {
            // * --- Select from cgo_dest_alert
            m_cServer = m_Ctx.GetServer("cgo_dest_alert");
            m_cPhName = m_Ctx.GetPhName("cgo_dest_alert");
            if (Cursor_cgo_dest_alert!=null)
              Cursor_cgo_dest_alert.Close();
            Cursor_cgo_dest_alert = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"DATIPALT='2' "+")"+(m_Ctx.IsSharedTemp("cgo_dest_alert")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
            while ( ! (Cursor_cgo_dest_alert.Eof())) {
              /* Sp_Lib_SendMail.sendMailPers(_mail20,'TXT','Segnalazione Operazioni non chiuse',LRTrim(_srvmail),LRTrim(cgo_dest_alert->DAEMAIL),'','',LRTrim(_emlmail),LRTrim(nomefile),LRTrim(_usrmail),LRTrim(_pwdmail),'N',LRTrim(_prtmail),'','WEBANTI') */
              Sp_Lib_SendMail.sendMailPers(_mail20,"TXT","Segnalazione Operazioni non chiuse",CPLib.LRTrim(_srvmail),CPLib.LRTrim(Cursor_cgo_dest_alert.GetString("DAEMAIL")),"","",CPLib.LRTrim(_emlmail),CPLib.LRTrim(nomefile),CPLib.LRTrim(_usrmail),CPLib.LRTrim(_pwdmail),"N",CPLib.LRTrim(_prtmail),"","WEBANTI");
              Cursor_cgo_dest_alert.Next();
            }
            m_cConnectivityError = Cursor_cgo_dest_alert.ErrorMessage();
            Cursor_cgo_dest_alert.Close();
  ...
  public static arrt_cgo_alert_operazioniR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_alert_operazioniR(p_Ctx, p_Caller);
  }
  public void Blank() {
