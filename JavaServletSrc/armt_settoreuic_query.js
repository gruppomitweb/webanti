...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('VPSAQWGZDD').value,'C'),w_SETSINT)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('LFVVYXTUJL').value,'N'),w_IDSETSIN)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbsetsin',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'SETSINT') &&  ! (Empty(HtW(Ctrl('VPSAQWGZDD').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.SETSINT',HtW(Ctrl('VPSAQWGZDD').value,'C'),'C');
}
if (Ne(name,'IDSETSIN') &&  ! (Empty(HtW(Ctrl('LFVVYXTUJL').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDSETSIN',HtW(Ctrl('LFVVYXTUJL').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_settoreuic');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbsetsin',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['VPSAQWGZDD','LFVVYXTUJL'],i=1;i>=0;i--){
