...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('NSUYYWAOJV').value,'C'),w_IDAMLANNO)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('amldati',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDAMLANNO') &&  ! (Empty(HtW(Ctrl('NSUYYWAOJV').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDAMLANNO',HtW(Ctrl('NSUYYWAOJV').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'detail','ardt_amldati');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('amldati',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['NSUYYWAOJV'],i=0;i>=0;i--){
