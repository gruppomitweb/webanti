...
var ctl = _GetCtl(e,'ZXGAEXNNNE');
if (_tracker.goon(ctl,ctrlValue)) {
w_RAPCOL=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('ZXGAEXNNNE',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
var ctl = _GetCtl(e,'MPDPVTSMAO');
if (_tracker.goon(ctl,ctrlValue)) {
w_CODSOG=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('MPDPVTSMAO',0);
if (l_bResult) {
l_bResult=Link_MPDPVTSMAO(kmode);
if ( ! (l_bResult)) {
w_CODSOG=HtW('','C');
}
}
...
function Link_MPDPVTSMAO(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
...
if (Eq(l_nRec,2)) {
var openername = 'linkview_MPDPVTSMAO';
...
function Link_MPDPVTSMAO_Blank() {
w_CODSOG='';
w_xragsoc='';
w_DOMICILIO='';
w_DESCCIT='';
...
function LOpt_MPDPVTSMAO() {
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('personbo','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
l_oWv.setValue('PKFields',WtA('CONNES','C'));
l_oWv.setValue('LinkedField',WtA('CONNES','C'));
l_oWv.setValue('UID',WtA('MPDPVTSMAO','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['MPDPVTSMAO'][1],'C'));
l_oWv.setValue('Caption',WtA(Translate('195812931'),'C'));
return l_oWv;
}
function ReportLinkValue(varName,value) {
var bResult = false;
if (Eq(varName,'MPDPVTSMAO')) {
last_focused_comp=GetLastFocusedElementOfLink('MPDPVTSMAO');
function SetMethod_MPDPVTSMAO() {
Set_CODSOG(HtW(value,'C'),0);
}
if (IsDeviceMobile()) {
SetMethod_MPDPVTSMAO();
} else {
setTimeout(SetMethod_MPDPVTSMAO,1);
}
bResult=true;
}
return bResult;
...
if ( ! (m_oFather.SaveContext_ardt_fiduciabo())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_fiduciabo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_fiduciabo_dataNotSent',{'action':formaction});
}
}
}
}
...
var l_bResult = true;
ResetErrorVars();
if ( ! (_ChkObl('ZXGAEXNNNE',1))) {
_SignErr('ZXGAEXNNNE');
m_cLastWorkVarErrorMsg='1699714614';
l_bResult=false;
w_RAPCOL=HtW('','C');
}
if (l_bResult) {
...
l_oWv.setValue('NUMCOL',WtA(w_NUMCOL,'C'));
...
_FillChildren.n=[];
FillWv.n=["IDFILEBO","RAPCOL","NUMCOL","NUMIMP"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
if (Le(m_nCurrentRow,Rows())) {
if (( ! (_ChkObl('MPDPVTSMAO',1))) && FullRow()) {
_SignErr('MPDPVTSMAO');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_fiduciabo();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields={};
l_aRepeatedFields.CODSOG={"dec":0,"fixedpos":false,"id":"MPDPVTSMAO","len":16,"name":"CODSOG","title":"Codice Soggetto","type":"C"};
l_aRepeatedFields.TIPOFILE={"dec":0,"fixedpos":true,"id":"GEWYTHKXUG","len":1,"name":"TIPOFILE","title":"Tipo FIle","type":"C"};
l_aRepeatedFields.LEGAME={"dec":0,"fixedpos":true,"id":"SPUZYHEUJG","len":1,"name":"LEGAME","title":"Tipo Legame","type":"C"};
l_aRepeatedFields.UCODE={"dec":0,"fixedpos":true,"id":"WNIJZZPPAJ","len":7,"name":"UCODE","title":"Codice Univoco","type":"C"};
l_aRepeatedFields.IDBASE={"dec":0,"fixedpos":true,"id":"TPSVGPZIIN","len":10,"name":"IDBASE","title":"Identificativo riga","type":"C"};
l_aRepeatedFields.xragsoc={"dec":0,"fixedpos":false,"id":"KBRBGGSVYB","len":70,"name":"xragsoc","title":"Ragione Sociale","type":"C"};
...
if (enable) {
var c;
if(c=Ctrl('ZXGAEXNNNE')) SetDisabled(c,true);
SetQueryRow(Ctrl('GridTable'),false);
} else {
SetEditRow(Ctrl('GridTable'),false);
var c;
if(c=Ctrl('ZXGAEXNNNE')) SetDisabled(c,false);
