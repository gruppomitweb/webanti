...
public class arpg_cgo_anadip_err extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*IZKWZFGJDF*!";
  ...
  public String getEntityTitle() {
    return "Correzione Agenzie Scartate (CGO)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_cgo_anadip_err";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 743;
    ...
    l_eg.m_cQueryName = "qbe_cgo_anadip_err";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_cgo_anadip_errBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1455048849",p_Context.Translate("Codice SNAI")},
    {"15186643",p_Context.Translate("Città")},
    {"1745963953",p_Context.Translate("Agenzia")},
    {"1771737972",p_Context.Translate("Correzione Agenzie Scartate (CGO)")}};
    return l_translations;
  }
  void WriteStyles(arpg_cgo_anadip_errBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_cgo_anadip_err.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_cgo_anadip_errBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_cgo_anadip_errBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_cgo_anadip_errBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_cgo_anadip_errBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_cgo_anadip_errBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_cgo_anadip_errBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_cgo_anadip_errBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(743,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_cgo_anadip_err"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_cgo_anadip_err","");
  ...
  void WriteMainFormScript(arpg_cgo_anadip_errBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_cgo_anadip_errBO getBO(ServletStatus status) {
    arpg_cgo_anadip_errBO BO = new arpg_cgo_anadip_errBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_cgo_anadip_errBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_cgo_anadip_errBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_cgo_anadip_errBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_cgo_anadip_errBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_cgo_anadip_errBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_cgo_anadip_errBO BO) {
    ...
    item.put("altInterfaceFor","arpg_cgo_anadip_err");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_cgo_anadip_errBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_cgo_anadip_errBO BO) {
    status.out.println("var c_PRGDIP;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_cgo_anadip_errBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_PRGDIP,"C",15,0);
  ...
  void Next(ServletStatus status,arpg_cgo_anadip_errBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_cgo_anadip_errBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_cgo_anadip_errBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_cgo_anadip_err?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_cgo_anadip_err_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_cgo_anadip_errBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_cgo_anadip_errBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_cgo_anadip_errBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_cgo_anadip_errBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_cgo_anadip_errBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_cgo_anadip_errBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_cgo_anadip_err_load.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_cgo_anadip_errBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_cgo_anadip_errBO BO) {
    boolean result = true;
    return result;
  }
}
