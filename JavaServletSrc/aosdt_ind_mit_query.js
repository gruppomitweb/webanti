...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('ACPSBOFDKQ').value,'C'),w_IDMOD)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('ind_mit',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDMOD') &&  ! (Empty(HtW(Ctrl('ACPSBOFDKQ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDMOD',HtW(Ctrl('ACPSBOFDKQ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_sub_ind_mit(firstAlias) {
...
_chNotF(childAlias,'detail','aosdt_ind_mit');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('ind_mit',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['ACPSBOFDKQ'],i=0;i>=0;i--){
