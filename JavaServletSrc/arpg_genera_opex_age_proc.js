...
w_Messaggi.raiseEvent('arpg_genera_opex_age_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_genera_opex_age_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_genera_opex_age_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_genera_opex_age',m_cSelectedPage);
...
m_bCalculating=true;
if ((Ne(o_mese,w_mese))) {
w_DaDatOpe=( ! (Empty(w_anno)) &&  ! (Empty(w_mese))?CharToDate(Str(w_anno,4,0)+w_mese+'01'):( ! (Empty(w_anno)) && Empty(w_mese)?CharToDate(Str(w_anno,4,0)+'0101'):NullDate()));
}
if ((Ne(o_mese,w_mese))) {
w_ADatOpe=( ! (Empty(w_anno)) &&  ! (Empty(w_mese))?arfn_finemese(w_DaDatOpe):( ! (Empty(w_anno)) && Empty(w_mese)?CharToDate(Str(w_anno,4,0)+'1231'):NullDate()));
}
