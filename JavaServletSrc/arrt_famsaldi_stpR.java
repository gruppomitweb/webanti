...
// * --- Fine Area Manuale
public class arrt_famsaldi_stpR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_famsaldi_stpR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_famsaldi_stp",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    CPResultSet Cursor_qbe_fam_totsaldi=null;
    ...
    try {
      ...
      try {
        ...
        while ( ! (Cursor_tmp_list_imprich.Eof())) {
          ...
          m_nUpdatedRows=m_Sql.InsertIntoSQL(CPLib.GetSQLPrefixComment("arrt_famsaldi_stp",75,"0000000F"),m_cServer,m_oWrInfo,"* ","from "+m_cPhName_sid_invio1+" where " + QueryLoader.SetSqlFunctions("CODINVIO1="+CPLib.ToSQL(CPLib.LRTrim(Cursor_tmp_list_imprich.GetString("nomefile")),"?",0,0)+" ",m_cServer,m_Ctx)+"","tmp_sid_invio1",m_Ctx,CPLib.HasTableCPCCCHK(m_Ctx,"tmp_sid_invio1"));
          {
            if (CPLib.gt(m_nUpdatedRows,10000000*1.1)) {
              com.zucchetti.sitepainter.Logger info = com.zucchetti.sitepainter.Logger.getWarningLogger();
              if ( ! (CPLib.IsNull(info))) {
                info.log("Routine arrt_famsaldi_stp: Insert on tmp_sid_invio1 inserts "+(m_nUpdatedRows)+" rows larger then the expected 10000000");
              }
            }
          }
          ...
          m_nUpdatedRows=m_Sql.InsertIntoSQL(CPLib.GetSQLPrefixComment("arrt_famsaldi_stp",75,"00000010"),m_cServer,m_oWrInfo,"* ","from "+m_cPhName_sid_invio3+" where " + QueryLoader.SetSqlFunctions("CODINVIO3="+CPLib.ToSQL(CPLib.LRTrim(Cursor_tmp_list_imprich.GetString("nomefile")),"?",0,0)+" ",m_cServer,m_Ctx)+"","tmp_sid_invio3",m_Ctx,CPLib.HasTableCPCCCHK(m_Ctx,"tmp_sid_invio3"));
          {
            if (CPLib.gt(m_nUpdatedRows,10000000*1.1)) {
              com.zucchetti.sitepainter.Logger info = com.zucchetti.sitepainter.Logger.getWarningLogger();
              if ( ! (CPLib.IsNull(info))) {
                info.log("Routine arrt_famsaldi_stp: Insert on tmp_sid_invio3 inserts "+(m_nUpdatedRows)+" rows larger then the expected 10000000");
              }
            }
          }
          if (CPLib.lt(m_nUpdatedRows,0)) {
        ...
        Cursor_tmp_list_imprich.Close();
        // * --- End Select
        // * --- Select from qbe_fam_totsaldi
        if (Cursor_qbe_fam_totsaldi!=null)
          Cursor_qbe_fam_totsaldi.Close();
        Cursor_qbe_fam_totsaldi = new VQRHolder("qbe_fam_totsaldi",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),this,true).GetResultSet(m_Ctx);
        while ( ! (Cursor_qbe_fam_totsaldi.Eof())) {
          /* _record := qbe_fam_totsaldi->TOTALE */
          _record = CPLib.Round(Cursor_qbe_fam_totsaldi.GetDouble("TOTALE"),0);
          Cursor_qbe_fam_totsaldi.Next();
        }
        m_cConnectivityError = Cursor_qbe_fam_totsaldi.ErrorMessage();
        Cursor_qbe_fam_totsaldi.Close();
        // * --- End Select
        // Commit
        m_cLastMsgError = m_Sql.TransactionErrorMessage();
      ...
      f.SetParameter("w_mese","00");
      f.SetParameter("w_rapporti",_record);
      f.SetParameter("ReportName","arrp_fam_stpsaldi.vrp");
      f.SetParameter("m_cParameterSequence","hideCover"+","+"hideLanguage"+","+"hideSettings"+","+"DescAzi"+","+"w_anno"+","+"w_mese"+","+"w_rapporti"+","+"ReportName");
  ...
  public static arrt_famsaldi_stpR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_famsaldi_stpR(p_Ctx, p_Caller);
  }
  public void Blank() {
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_fam_totsaldi,
  public static final String m_cVQRList = ",qbe_fam_totsaldi,";
  // ENTITY_BATCHES: ,
