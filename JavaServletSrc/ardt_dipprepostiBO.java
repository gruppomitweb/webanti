...
// * --- Fine Area Manuale
public class ardt_dipprepostiBO extends ardt_dipprepostiBL {
  ...
  // * --- Fine Area Manuale
  public ardt_dipprepostiBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_dippreposti");
    }
  ...
  public boolean Load(String p_CODDIP) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_dippreposti+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_dippreposti,m_cPhName_dippreposti,m_cVirtName_dippreposti,m_cVirtName_dippreposti+".CODDIP="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODDIP,"C",6,0),m_cServer_dippreposti,p_CODDIP),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"dippreposti")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_dippreposti.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"CODDIP="+CPLib.ToSQL(w_CODDIP,"C",6,0)+" AND CPROWNUM="+CPLib.ToSQL(w_CPROWNUM,"N",3,0);
        l_cSql = l_cSql+m_oWrInfo_dippreposti.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"dippreposti")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_dippreposti,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_dippreposti);
      l_cSql = "insert into "+m_oWrInfo_dippreposti.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_dippreposti.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"dippreposti")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_dippreposti.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"dippreposti")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_dippreposti,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_dippreposti);
      l_cSql = "Update "+m_oWrInfo_dippreposti.GetTableWriteName()+" set";
