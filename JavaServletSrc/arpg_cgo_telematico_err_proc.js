...
SetHypPar('m_cParameterSequence',AsAppletValue('pNumOpe'));
SetHypPar('m_cID',AsAppletValue(m_IDS['arrt_cgo_telematico_err_load']));
SendData('start','arrt_cgo_telematico_err_load'+'',HParApplet().asString());
}
_ResetTracker();
...
function Help() {
windowOpenForeground('../doc/arpg_cgo_telematico_err_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_cgo_telematico_err',m_cSelectedPage);
...
function TrsQueryRow() {
  this.c_SNAINUMOPE = arguments[0]
  this.c_CAIDBIGLIETTO = arguments[1]
  this.c_RAPPORTO = arguments[2]
  this.c_DESCRAP = arguments[3]
  this.c_TOTLIRE = arguments[4]
  this.c_CADATAORA = arguments[5]
}
function TrsToWork() {
var row;
...
GroupFooter_BeginHtml(document,1,'1868411119'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'1868411119'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'1868411121'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_0A0B1D58"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
 "<span class=' SPLabel KRLVDZKIGO_DIV'>" +
 "<label class='KRLVDZKIGO'>" +
 ""+ToHTag(Translate("48543475"))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
 ...
 ""+ToHTag(Translate("10036629821"))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
 "<span class=' SPLabel IMOMNXOPKV_DIV'>" +
 "<label class='IMOMNXOPKV'>" +
 ""+ToHTag(Translate("48541742"))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'1868411121'+l_nIdx+'','A')
} else {
}
Row_BeginHtml(document,l_nEvenOdd,'1868397051'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_0BACD3C8"+l_nIdx+"' style='position:relative;height:18px'>" +
"")
document.writeln(
 "<span class=' SPLabel CGJORROSRQ_DIV'>" +
 "<label class='CGJORROSRQ'>" +
 "<a href='javascript:href_CGJORROSRQ("+l_nIdx+")'>" +
 ""+ToHTag(WtH(c_SNAINUMOPE,'C',15,0,''))+"" +
 "</a>" +
 "</label>" +
 "</span>" +
"")
...
document.writeln(
 ...
 ""+ToHTag(WtH(c_CAIDBIGLIETTO,'C',20,0,''))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'1868397051'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
