...
function DisplayErrorMessage_armt_cgo_qadv_operazioni_dl() {
if ( ! (ChildToLoad(FrameRef('armt_cgo_qadv_operazioni_dl'))) && Ne(typeof(FrameRef('armt_cgo_qadv_operazioni_dl').DisplayErrorMessage),'undefined')) {
FrameRef('armt_cgo_qadv_operazioni_dl').DisplayErrorMessage();
}
}
...
SetDisabled('TRGEEVWGRS',l_bEnabled);
...
SetObligatory(Ctrl('TRGEEVWGRS'),Eq(w_FLAGWALLET,'1') || IsObligatoryByStateDriver('CODWALLET'));
...
SetObligatory(Ctrl('NKJDWBKDWM'),Eq(w_FLGDOM,'S') || IsObligatoryByStateDriver('PROVINCIA2'));
...
SetObligatory(Ctrl('PAQVIKBAGR'),Eq(w_xCGOFLGPROF,'S') || IsObligatoryByStateDriver('PROFESSIONE'));
}
...
var ctl = _GetCtl(e,'QZTTRBWAQL');
if (_tracker.goon(ctl,ctrlValue)) {
w_TIPOGIOCO=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('QZTTRBWAQL',0);
if (l_bResult) {
l_bResult=Link_QZTTRBWAQL(kmode);
if ( ! (l_bResult)) {
w_TIPOGIOCO=HtW('','C');
}
}
...
if (l_bResult) {
l_bResult=(Eq(w_TIPOGIOCO,'V')?true:arfn_cgo_chkidb(w_CAIDBIGLIETTO));
if ( ! (l_bResult)) {
m_nLastError=2;
...
var ctl = _GetCtl(e,'TRGEEVWGRS');
if (_tracker.goon(ctl,ctrlValue)) {
w_CODWALLET=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('TRGEEVWGRS',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',20,0,'');
...
var ctl = _GetCtl(e,'GRTAUFQUJZ');
if (_tracker.goon(ctl,ctrlValue)) {
w_NUMTEL=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('GRTAUFQUJZ',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',20,0,'');
...
var ctl = _GetCtl(e,'PAQVIKBAGR');
if (_tracker.goon(ctl,ctrlValue)) {
w_PROFESSIONE=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('PAQVIKBAGR',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',40,0,'');
...
if (l_bResult &&  ! (Check_GAUOPXUMYI(false)) && Ne(m_nLastError,-3)) {
...
if (false) {
arrt_cgo_setpagatore([["pObj",this]])
}
}
...
function Check_GAUOPXUMYI(p_bForce) {
var l_bResult = true;
if (p_bForce || Ne(o_CAIDBIGLIETTO,w_CAIDBIGLIETTO) || Ne(o_TIPOGIOCO,w_TIPOGIOCO)) {
l_bResult=(Eq(w_TIPOGIOCO,'V')?true:arfn_cgo_chkidb(w_CAIDBIGLIETTO));
if ( ! (l_bResult)) {
m_cLastMsgError=AlertErrorMessage('11525797640');
}
...
l_Appl.SetFields('CODTPGIO,SOGTPGIO,DEFTPGIO,EDTTPGIO,EDTSPGIO,DEFSPGIO,FLGPLGIO,FLTIPGIO,IMPMINGIO');
...
w_xIMPMINGIO=l_Appl.GetDoubleValue('IMPMINGIO',10,2);
l_Appl.GetLinkHash(m_CheckVars);
} else {
Link_QZTTRBWAQL_Blank();
}
if (Eq(l_nRec,2)) {
var openername = 'linkview_QZTTRBWAQL';
...
if (l_bResult &&  ! (Empty(w_TIPOGIOCO) && l_bEmp)) {
l_bResult=Ne(w_TIPOGIOCO,w_gTipMul);
if ( ! (l_bResult)) {
Link_QZTTRBWAQL_Blank();
}
}
...
m_cLastMsgError=AlertErrorMessage('1686225832');
}
}
}
return l_bResult;
...
var bResult = false;
if (Eq(varName,'QZTTRBWAQL')) {
last_focused_comp=GetLastFocusedElementOfLink('QZTTRBWAQL');
function SetMethod_QZTTRBWAQL() {
Set_TIPOGIOCO(HtW(value,'C'),0);
}
if (IsDeviceMobile()) {
SetMethod_QZTTRBWAQL();
} else {
setTimeout(SetMethod_QZTTRBWAQL,1);
}
bResult=true;
}
...
parent.ZtVWeb.raiseEventToEvalParms('armt_cgo_oper_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_cgo_oper_dataNotSent',{'action':formaction});
}
}
}
}
CheckChild.child=[{'prg':"armt_cgo_qadv_operazioni_dl",'BOScId':function(){ return FrameRef('armt_cgo_qadv_operazioni_dl')},'repeated':0}];
function Check() {
var l_bResult = true;
var l_bChildCheck = true;
ResetErrorVars();
m_nLastError=0;
if (( ! (_ChkObl('QZTTRBWAQL',1))) || ( ! (Empty(w_TIPOGIOCO)) &&  ! (Ne(w_TIPOGIOCO,w_gTipMul)))) {
_SignErr('QZTTRBWAQL','1686225832');
l_bResult=false;
w_TIPOGIOCO=HtW('','C');
...
l_bResult=false;
w_VLTPLAT=HtW('','C');
} else if (( ! (_ChkObl('BMTLGIUQTH',1))) || ( ! ((Eq(w_TIPOGIOCO,'V')?true:arfn_cgo_chkidb(w_CAIDBIGLIETTO))))) {
_SignErr('BMTLGIUQTH','11525797640');
l_bResult=false;
w_CAIDBIGLIETTO=HtW('','C');
...
l_bResult=false;
w_FLAGWALLET=HtW('','C');
} else if ( ! (_ChkObl('YJDISDDJUY',1))) {
_SignErr('YJDISDDJUY');
m_cLastWorkVarErrorMsg='995100987';
l_bResult=false;
w_CAFLGOPE=HtW('','C');
} else if (( ! (_ChkObl('HUAGBHPUNH',1))) || ( ! (Gt(w_TOTLIRE,0) && Ge(w_TOTLIRE,(w_TOTCONT+w_IMPWALLET))))) {
...
l_bResult=false;
w_IMPWALLET=HtW('','N');
} else if ((Eq(w_FLAGWALLET,'1') &&  ! (Empty(w_DATAOPE))) && ( ! (_ChkObl('TRGEEVWGRS',1)))) {
_SignErr('TRGEEVWGRS');
...
_SignErr('YVIKEGPYPM');
m_cLastWorkVarErrorMsg='227238806';
l_bResult=false;
w_PAESE=HtW('','C');
...
if (l_bResult) {
  if (w_gQADV=='S') {
    if (w_TIPOGIOCO=='V' && w_FLGQADVFULL=='N') {
      m_cLastMsgError='Obbligatorio completare il questionario ADV prima di procedere'
      l_bResult=false
    }
  }
}
...
l_oWv.setValue('xIMPMINGIO',WtA(w_xIMPMINGIO,'N'));
...
l_oWv.setValue('dirupl',WtA(w_dirupl,'C'));
...
_FillChildren.n=['armt_cgo_qadv_operazioni_dl'];
FillWv.n=["TIPOGIOCO","CGOTIPOPE","VLTPLAT","CAIDBIGLIETTO","DATAOPE","FLAGCONT","FLAGWALLET","CAFLGOPE","TOTLIRE","TOTCONT","IMPWALLET","CODWALLET","CODTRANSZ","CGOFLGVLT","CGOFLGVNP","MEZPAGAM","CC4CIFRE","CCMESSCA","CCANNSCA","ASSEGNO","IBAN","CCDATSCA","CFESTERO","PEP","CODFISC","COGNOME","NOME","NASCOMUN","TIPINTER","SESSO","NASSTATO","DATANASC","PAESE","DESCCIT","PROVINCIA","CAP","DOMICILIO","NUMTEL","ATTIV","TIPOPERS","RAGSOC","SOTGRUP","RAMOGRUP","SETTSINT","SNAINUMOPE","DATAREG","TIPOOPRAP","CODANA","CODVOC","VALUTA","CAMBIO","CONNESCLI","CONNESBEN","C_RAG","C_CTA","C_PRV","C_CAP","C_CAB","C_IND","C_STA","CODINTER","SEGNO","FLAGFRAZ","COLLEG","CACODICE","CODCAB","IDDOCALL","FLGDOCALL","TIPODOC","NUMDOCUM","DATARILASC","DATAVALI","RDCODAUT","RDLOCAUT","AUTRILASC","PEPDESCRI","PAESE2","DESCCIT2","PROVINCIA2","CAP2","DOMICILIO2","FLGDOM","PROFESSIONE","CODDIPE","DESCCITDIP","PROVINCIADIP","CODCABDIP","gIntemediario","xSOGTPGIO","xDEFTPGIO","xEDTTPGIO","xEDTSPGIO","xDEFSPGIO","xFLGPLGIO","FILTROMP","FLGPAGAM","idcitta","xARDESCRI","dessta","tipsot","STATOREG","IDBASE","NUMPROG","DESCRI","gSeekAos","gDataVaria","gFlgDoc","gCodDip","xDescDipe","datpaseuro","xdes1","desccauana","CITINT","CITDIP","PROVDIP","CABDIP","PROVINT","CABINT","xdesccausin","gChkDate","xdesctipatt","gVLT","gVNP","gStpDwn","MIDBIGLIETTO","MIDBIGLIETTO1","MIDBIGLIETTO4","MIDBIGLIETTO2","MIDBIGLIETTO3","gPathApp","xVPFLGATT","DOC03","DOC04","gGesDoc","edtdoc","saveddoc","xIMPMINGIO","idcitta2","dessta2","gTipMul","FLGQADVFULL","gQADV","xCGOFLGPROF","TELDIP","CODLOC","dirupl","ANA_FOUND"];
function SetModified() {
m_bUpdated=true;
}
function SetChildVariables_armt_cgo_qadv_operazioni_dl() {
}
function CtxLoad_armt_cgo_qadv_operazioni_dl() {
if (ChildToLoad(FrameRef('armt_cgo_qadv_operazioni_dl'))) {
return [LoadContext(0,'BZRUFRMVDO'),'armt_cgo_qadv_operazioni_dl'];
}
}
