...
public class arrt_agg_famiglie extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*7e8217a7*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_flgaggfam) ||  ! (status.m_bpassedw_CFESTERO) ||  ! (status.m_bpassedw_CONNES);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_agg_famiglieR Rcalled;
    Rcalled = new arrt_agg_famiglieR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_agg_famiglieR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_agg_famiglieR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_agg_famiglie"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_agg_famiglie");
      if ( ! (CPLib.IsNull(l))) {
