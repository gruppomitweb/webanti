...
// * --- Fine Area Manuale
public class arrt_fam_chksaldiR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_fam_chksaldiR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_fam_chksaldi",m_Caller);
  ...
  void Page_1() throws Exception {
    CPResultSet Cursor_qbe_fam_chkperiodo_saldi_01_minmax=null;
    CPResultSet Cursor_qbe_fam_chkperiodo_saldi_01=null;
    ...
    try {
      ...
      /* FileName Char(0) := 'Controllo_saldi_anno_'+ Str(w_anno,4,0)+'_il_'+DateToChar(Date()) // nome file del report */
      FileName = "Controllo_saldi_anno_"+CPLib.Str(w_anno,4,0)+"_il_"+CPLib.DateToChar(CPLib.Date());
      ...
      // * --- Select from qbe_fam_chkperiodo_saldi_01_minmax
      ...
      if (Cursor_qbe_fam_chkperiodo_saldi_01_minmax!=null)
        Cursor_qbe_fam_chkperiodo_saldi_01_minmax.Close();
      Cursor_qbe_fam_chkperiodo_saldi_01_minmax = new VQRHolder("qbe_fam_chkperiodo_saldi_01_minmax",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),_h00000022,true).GetResultSet(m_Ctx);
      while ( ! (Cursor_qbe_fam_chkperiodo_saldi_01_minmax.Eof())) {
        /* _totrecord := _totrecord + qbe_fam_chkperiodo_saldi_01_minmax->TOTPRG */
        _totrecord = CPLib.Round(_totrecord+Cursor_qbe_fam_chkperiodo_saldi_01_minmax.GetDouble("TOTPRG"),0);
        Cursor_qbe_fam_chkperiodo_saldi_01_minmax.Next();
      }
      m_cConnectivityError = Cursor_qbe_fam_chkperiodo_saldi_01_minmax.ErrorMessage();
      Cursor_qbe_fam_chkperiodo_saldi_01_minmax.Close();
      // * --- End Select
      /* _errori := 0 */
      ...
      if (CPLib.gt(_totrecord,0)) {
        ...
        try {
          // * --- Fill memory cursor mcTemp on qbe_fam_chkperiodo_saldi_01
          mcTemp.Zap();
          ...
          if (Cursor_qbe_fam_chkperiodo_saldi_01!=null)
            Cursor_qbe_fam_chkperiodo_saldi_01.Close();
          Cursor_qbe_fam_chkperiodo_saldi_01 = new VQRHolder("qbe_fam_chkperiodo_saldi_01",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),_h00000028,true).GetResultSet(m_Ctx);
          {
            final CPResultSet m_TheCursor = Cursor_qbe_fam_chkperiodo_saldi_01;
            Cursor_qbe_fam_chkperiodo_saldi_01.ActionOnClose(new Runnable() {
              public void run() {
                if (CPLib.gt(m_TheCursor.CurrentRow()-1,1000*1.1)) {
                  com.zucchetti.sitepainter.Logger info = com.zucchetti.sitepainter.Logger.getWarningLogger();
                  if ( ! (CPLib.IsNull(info))) {
                    info.log("Routine arrt_fam_chksaldi: query on qbe_fam_chkperiodo_saldi_01 returns "+(m_TheCursor.CurrentRow()-1)+" rows larger then the expected 1000");
                  }
                }
              }
              });
          }
          while ( ! (Cursor_qbe_fam_chkperiodo_saldi_01.Eof())) {
            mcTemp.AppendBlank();
            mcTemp.row.IDBASE = Cursor_qbe_fam_chkperiodo_saldi_01.GetString("IDBASE");
            mcTemp.row.RAPPORTO = Cursor_qbe_fam_chkperiodo_saldi_01.GetString("RAPPORTO");
            mcTemp.row.CODINTER = Cursor_qbe_fam_chkperiodo_saldi_01.GetString("CODINTER");
            mcTemp.row.DATAINI = Cursor_qbe_fam_chkperiodo_saldi_01.GetDate("DATAINI");
            mcTemp.row.DATAFINE = Cursor_qbe_fam_chkperiodo_saldi_01.GetDate("DATAFINE");
            mcTemp.row.CODFISC = "";
            mcTemp.row.TIPO = Cursor_qbe_fam_chkperiodo_saldi_01.GetString("TIPO");
            mcTemp.row.RIFIMP = "";
            mcTemp.row.INFORM = "";
            Cursor_qbe_fam_chkperiodo_saldi_01.Next();
          }
          m_cConnectivityError = Cursor_qbe_fam_chkperiodo_saldi_01.ErrorMessage();
          Cursor_qbe_fam_chkperiodo_saldi_01.Close();
          mcTemp.GoTop();
          ...
          for (MemoryCursorRow_mcchkperiodo_mcrdef rowTemp : mcTemp._iterable_()) {
            ...
            m_Ctx.SetGlobalString("gMsgImp",gMsgImp);
            /* _errore := arfn_anomsaldi_rapp(rowTemp.RAPPORTO,rowTemp.TIPO,w_anno) */
            _errore = arfn_anomsaldi_rappR.Make(m_Ctx,this).Run(rowTemp.RAPPORTO,rowTemp.TIPO,w_anno);
            ...
            if ( ! (CPLib.Empty(CPLib.LRTrim(_errore)))) {
              ...
              m_cSql = CPLib.GetSQLPrefixComment("arrt_fam_chksaldi",41,"00000030")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
              ...
              m_cSql = m_cSql+""+CPLib.ToSQL(rowTemp.CODINTER,"?",0,0,m_cServer)+", ";
          ...
          /* gMsgProc := gMsgProc + NL + 'Sono stati effettuati '+LRTrim(Str(_conta,10,0))+" controlli" +NL // Messaggio */
          gMsgProc = gMsgProc+"\n"+"Sono stati effettuati "+CPLib.LRTrim(CPLib.Str(_conta,10,0))+" controlli"+"\n";
          m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
        ...
        /* gMsgProc := gMsgProc + "Non esistono rapporti di cui inviare i saldi per il periodo considerato!" + NL // Messaggio */
        gMsgProc = gMsgProc+"Non esistono rapporti di cui inviare i saldi per il periodo considerato!"+"\n";
        m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
        /* gMsgImp := "Elaborazione Terminata! Non ci sono elementi da esaminare" // Messaggio Import */
        gMsgImp = "Elaborazione Terminata! Non ci sono elementi da esaminare";
        m_Ctx.SetGlobalString("gMsgImp",gMsgImp);
      } // End If
      /* If _errori > 0 */
      if (CPLib.gt(_errori,0)) {
        ...
        f.SetParameter("FileName",FileName);
        f.SetParameter("ReportName","arrp_fam_chkperiodo.vrp");
        f.SetParameter("m_cParameterSequence","_dadata"+","+"_adata"+","+"hideCover"+","+"hideLanguage"+","+"hideSettings"+","+"hideParameterMask"+","+"DescAzi"+","+"FileName"+","+"ReportName");
        ...
        m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_chkperiodo",m_Ctx.GetCompany());
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_fam_chksaldi",41,"0000003D")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
  ...
  void Page_2() throws Exception {
    ...
    try {
      ...
      _h00000044.Set("_dadata",w_DaDatOpe);
      _h00000044.Set("_adata",w_ADatOpe);
      _h00000044.Set("_inviato",1);
      if (Cursor_qbe_famperiodo_opx_min_max!=null)
        Cursor_qbe_famperiodo_opx_min_max.Close();
      Cursor_qbe_famperiodo_opx_min_max = new VQRHolder("qbe_famperiodo_opx_min_max",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),_h00000044,true).GetResultSet(m_Ctx);
      ...
      while (CPLib.le(_contacicli,_cicli)) {
        ...
        if (CPLib.ge(_top,_min)) {
          ...
          _h00000064.Set("dadata",w_DaDatOpe);
          _h00000064.Set("a_data",w_ADatOpe);
          _h00000064.Set("_bottom",_bottom);
          _h00000064.Set("_top",_top);
          if (Cursor_qbe_fam_chkperiodo_opx_2015!=null)
            Cursor_qbe_fam_chkperiodo_opx_2015.Close();
          Cursor_qbe_fam_chkperiodo_opx_2015 = new VQRHolder("qbe_fam_chkperiodo_opx_2015",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),_h00000064,true).GetResultSet(m_Ctx);
          {
            ...
            Cursor_qbe_fam_chkperiodo_opx_2015.ActionOnClose(new Runnable() {
              public void run() {
                if (CPLib.gt(m_TheCursor.CurrentRow()-1,5000*1.1)) {
                  com.zucchetti.sitepainter.Logger info = com.zucchetti.sitepainter.Logger.getWarningLogger();
                  if ( ! (CPLib.IsNull(info))) {
                    info.log("Routine arrt_fam_chksaldi: query on qbe_fam_chkperiodo_opx_2015 returns "+(m_TheCursor.CurrentRow()-1)+" rows larger then the expected 5000");
                  }
                }
              }
              });
          ...
          for (MemoryCursorRow_mcchkperiodo_mcrdef rowTemp : mcTemp._iterable_()) {
            ...
            /* _conta := _conta + 1 */
            _conta = CPLib.Round(_conta+1,0);
            /* _errore := arfn_anomsaldi_opex(rowTemp.RAPPORTO,rowTemp.TIPO,Str(w_anno,4,0)) */
            _errore = arfn_anomsaldi_opexR.Make(m_Ctx,this).Run(rowTemp.RAPPORTO,rowTemp.TIPO,CPLib.Str(w_anno,4,0));
            /* If not(Empty(LRTrim(_errore))) */
            if ( ! (CPLib.Empty(CPLib.LRTrim(_errore)))) {
              /* _errori := _errori + 1 */
              _errori = CPLib.Round(_errori+1,0);
              ...
              m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_chkperiodo",m_Ctx.GetCompany());
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("arrt_fam_chksaldi",41,"0000006B")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
  ...
  public String RunAsync(double p_w_anno,java.sql.Date p_w_DaDatOpe,java.sql.Date p_w_ADatOpe) {
    w_anno = p_w_anno;
  ...
  public Forward Run(double p_w_anno,java.sql.Date p_w_DaDatOpe,java.sql.Date p_w_ADatOpe) {
    w_anno = p_w_anno;
    w_DaDatOpe = p_w_DaDatOpe;
    w_ADatOpe = p_w_ADatOpe;
    return Run();
  ...
  public static arrt_fam_chksaldiR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_fam_chksaldiR(p_Ctx, p_Caller);
  }
  public void Blank() {
    w_anno = 0;
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_fam_chkperiodo_saldi_01_minmax,qbe_fam_chkperiodo_saldi_01,qbe_famperiodo_opx_min_max,qbe_fam_chkperiodo_opx_2015,
  public static final String m_cVQRList = ",qbe_fam_chkperiodo_saldi_01_minmax,qbe_fam_chkperiodo_saldi_01,qbe_famperiodo_opx_min_max,qbe_fam_chkperiodo_opx_2015,";
  // ENTITY_BATCHES: ,arfn_anomsaldi_opex,arfn_anomsaldi_rapp,arfn_fdatetime,
  public static final String i_InvokedRoutines = ",arfn_anomsaldi_opex,arfn_anomsaldi_rapp,arfn_fdatetime,";
  public static String[] m_cRunParameterNames={"w_anno","w_DaDatOpe","w_ADatOpe"};
