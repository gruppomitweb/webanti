...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('QKQADIZQMK').value,'N'),w_PBCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbprcbck',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'PBCODICE') &&  ! (Empty(HtW(Ctrl('QKQADIZQMK').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.PBCODICE',HtW(Ctrl('QKQADIZQMK').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_tbprcbck_param(firstAlias) {
return m_cVirtName+".PBCODICE = "+firstAlias+".PPCODICE ";
}
...
_chNotF(childAlias,'master','armt_tbprcbck');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbprcbck',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['QKQADIZQMK'],i=0;i>=0;i--){
