...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('LNSBOLUCRM').value,'C'),w_IDMOD)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('ILPNZWVGRZ').value,'C'),w_IDIND)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('sub_ind_mit',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDMOD') &&  ! (Empty(HtW(Ctrl('LNSBOLUCRM').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDMOD',HtW(Ctrl('LNSBOLUCRM').value,'C'),'C');
}
if (Ne(name,'IDIND') &&  ! (Empty(HtW(Ctrl('ILPNZWVGRZ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDIND',HtW(Ctrl('ILPNZWVGRZ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'detail','aosdt_sub_ind_mit');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('sub_ind_mit',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['LNSBOLUCRM','ILPNZWVGRZ'],i=1;i>=0;i--){
