...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SZHUMIRICR').value,'C'),w_IDMOD)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('rilevazione',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDMOD') &&  ! (Empty(HtW(Ctrl('SZHUMIRICR').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDMOD',HtW(Ctrl('SZHUMIRICR').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_ano_ril(firstAlias) {
return m_cVirtName+".IDMOD = "+firstAlias+".IDMOD and "+m_cVirtName+".CPROWNUM = "+firstAlias+".ROWRIL ";
}
...
_chNotF(childAlias,'detail','aosdt_rilevazione');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('rilevazione',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['SZHUMIRICR'],i=0;i>=0;i--){
