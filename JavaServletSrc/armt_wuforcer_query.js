...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('FURCXEJBAK').value,'C'),w_FRCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('wuforcer',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'FRCODICE') &&  ! (Empty(HtW(Ctrl('FURCXEJBAK').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.FRCODICE',HtW(Ctrl('FURCXEJBAK').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_wuforcer');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wuforcer',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['FURCXEJBAK'],i=0;i>=0;i--){
