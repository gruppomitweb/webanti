...
public class arrt_stpverage_do extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*851e17c7*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_noninvio) ||  ! (status.m_bpassedw_chkcfis) ||  ! (status.m_bpassedw_cdadata) ||  ! (status.m_bpassedw_c_adata) ||  ! (status.m_bpassedw_ADatOpe) ||  ! (status.m_bpassedw_DaDatOpe);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_stpverage_doR Rcalled;
    Rcalled = new arrt_stpverage_doR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_stpverage_doR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_stpverage_doR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_stpverage_do"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_stpverage_do");
      if ( ! (CPLib.IsNull(l))) {
