...
w_Messaggi.raiseEvent('arpg_chkdiana_new_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_chkdiana_new_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_chkdiana_new_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_chkdiana_new',m_cSelectedPage);
...
if (l_bResult) {
l_bResult=Ge(w_DaData,CharToDate('20140101'));
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='DaData';
m_cLastMsgError=AlertErrorMessage('10435768771');
}
if ( ! (l_bResult)) {
w_DaData=HtW('','D');
}
...
if (l_bResult) {
l_bResult=Ge(w_AData,w_DaData) && Ge(w_AData,CharToDate('20140101'));
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='AData';
m_cLastMsgError=AlertErrorMessage('11089820069');
}
if ( ! (l_bResult)) {
w_AData=HtW('','D');
}
...
l_bResult=false;
w_ragben=HtW('','C');
} else if (( ! (_ChkObl('HJLJFAMGNR',1))) || ( ! (Ge(w_DaData,CharToDate('20140101'))))) {
_SignErr('HJLJFAMGNR','10435768771');
l_bResult=false;
w_DaData=HtW('','D');
} else if (( ! (_ChkObl('XWZHWTENET',1))) || ( ! (Ge(w_AData,w_DaData) && Ge(w_AData,CharToDate('20140101'))))) {
_SignErr('XWZHWTENET','11089820069');
l_bResult=false;
w_AData=HtW('','D');
