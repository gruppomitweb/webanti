...
w_NUMPROG1=( ! (Empty(Utilities.Make(window).GetCallerString('TIPOOPRAP'))) && Ne(At(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'|21|22|23|24|27|33|34|35|43|44|45'),0) && Empty(w_NUMPROG1)?Utilities.Make(window).GetCallerString('NUMPROG'):w_NUMPROG1);
if ((Ne(o_CODINTER,w_CODINTER))) {
Calculation_XSORDCKKLR();
}
if ((Ne(o_DATAFINE,w_DATAFINE))) {
Calculation_SWVKAWGUOH();
}
if ((Ne(o_DATAINI,w_DATAINI))) {
...
if ( ! (m_oFather.SaveContext_ardt_ae_intestbo())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_ae_intestbo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_ae_intestbo_dataNotSent',{'action':formaction});
}
}
}
}
...
_FillChildren.n=[];
FillWv.n=["IDBASE","RAPPORTO","tipreg","appoggio","gAzienda"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_ae_intestbo();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.PRGIMPSOG={"dec":0,"fixedpos":true,"id":"CHJPRCYMKT","len":15,"name":"PRGIMPSOG","title":"Progressivo Import","type":"N"};
