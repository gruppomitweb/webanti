...
public class arpg_filesuic extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*FAGQDNJRVM*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_filesuic";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 870;
    ...
    l_eg.m_cQueryName = "qbe_file_discouic";
    return l_eg;
  }
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_filesuicBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"894014601",p_Context.Translate("ID Gen.")}};
    return l_translations;
  }
  void WriteStyles(arpg_filesuicBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_filesuic.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_filesuicBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_filesuicBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_filesuicBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_filesuicBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_filesuicBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_filesuicBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_filesuicBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(870,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_filesuic"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_filesuic","");
  ...
  void WriteMainFormScript(arpg_filesuicBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_filesuicBO getBO(ServletStatus status) {
    arpg_filesuicBO BO = new arpg_filesuicBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_filesuicBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_filesuicBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_filesuicBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_filesuicBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_filesuicBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_filesuicBO BO) {
    ...
    item.put("altInterfaceFor","arpg_filesuic");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_filesuicBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_filesuicBO BO) {
    status.out.println("var c_idfile;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_filesuicBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_idfile,"C",10,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_nomefile,"C",30,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_datafile,"D",8,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_descfile,"C",80,0);
    l_cTrsQ = l_cTrsQ+")";
  ...
  void Next(ServletStatus status,arpg_filesuicBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_filesuicBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_filesuicBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_filesuic?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_filesuic_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_filesuicBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_filesuicBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_filesuicBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_filesuicBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_filesuicBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_filesuicBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_filesuicBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_filesuicBO BO) {
    boolean result = true;
    return result;
  }
}
