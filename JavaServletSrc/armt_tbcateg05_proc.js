...
var c;
if(c=Ctrl('SEJWRQHUSP')) selectCombo(c,w_C5CODTAB,'C')
SPBOUILib.SetInputValue('XREVNLNKCY',function(){return WtH(w_C5CODICE,'C',2,0,'')},w_C5CODICE);
SPBOUILib.SetInputValue('RJDJPGMTCC',function(){return WtH(w_C5DESCRI,'C',80,0,'')},w_C5DESCRI);
if (m_bExtended) {
SetExtensions();
}
...
HideUI.lblids["C5CODICE"]=['EHORPFDFFQ'];
HideUI.lblids["C5CODTAB"]=['RGYCBVWYTV'];
HideUI.lblids["C5DESCRI"]=['UKQEHGTCZD'];
function SEJWRQHUSP_Valid(e) {
if (Eq(m_cFunction,'query')) {
if (IsLastInPk('SEJWRQHUSP') ||  ! (m_bQueryOnLatestKeyOnly)) {
DoQuery('C5CODTAB','C',w_C5CODTAB,HtW(getComboValue(Ctrl('SEJWRQHUSP')),'C'),m_bQueryOnLatestKeyOnly);
}
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_C5CODTAB(HtW(getComboValue(Ctrl('SEJWRQHUSP')),'C'),null,e);
return l_bSetResult;
}
}
...
DoQuery('C5CODICE','C',w_C5CODICE,HtW(Ctrl('XREVNLNKCY').value,'C'),m_bQueryOnLatestKeyOnly);
}
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_C5CODICE(HtW(Ctrl('XREVNLNKCY').value,'C'),null,e);
return l_bSetResult;
}
}
...
if (Eq(m_cFunction,'query')) {
DoQuery('C5DESCRI','C',w_C5DESCRI,HtW(Ctrl('RJDJPGMTCC').value,'C'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_C5DESCRI(HtW(Ctrl('RJDJPGMTCC').value,'C'),null,e);
return l_bSetResult;
}
}
...
function Help() {
windowOpenForeground('../doc/armt_tbcateg05_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
m_PrimaryKeys=['C5CODICE','C5CODTAB'];
}
function i_PrimaryKey() {
return 'tbcateg05'+'\\'+CPLib.ToCPStr(w_C5CODTAB)+'\\'+CPLib.ToCPStr(w_C5CODICE);
}
...
p_d.setValue('armt_tbcateg05',m_cSelectedPage);
...
_Obli('C5CODTAB',0,'SEJWRQHUSP',false,"10376019890")
_Obli('C5CODICE',0,'XREVNLNKCY',false,"112851763")
_Obli('C5DESCRI',0,'RJDJPGMTCC',false,"285808099")
