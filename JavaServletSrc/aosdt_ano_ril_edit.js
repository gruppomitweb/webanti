...
SetDisabled(Ctrl('NBMHJRJGQX'),l_bEnabled);
l_bEnabled= ! (false) || Eq(m_cFunction,'edit') || IsDisabledByStateDriver('ROWRIL');
SetDisabled(Ctrl('KHBDKLNADV'),l_bEnabled);
...
var c;
if(c=Ctrl('AOKURBYNFX')) c.title=Strtran(AlertErrorMessage('462843542',w_DESCRI_IND),'\\n',"\n");
if(c=Ctrl('PSHCVHVAPS')) c.title=Strtran(AlertErrorMessage('584841097',w_DESCRI_SUBIND),'\\n',"\n");
}
...
if (res) {
render_suggest(res,'PWBPXBGJRH','PWBPXBGJRH',true,'GridTable_Band_DIV','GridTable_Band_HEADER',0);
} else {
render_suggest('{Data:[]}','PWBPXBGJRH','PWBPXBGJRH',true,'GridTable_Band_DIV','GridTable_Band_HEADER');
}
return l_bResult;
}
select_suggest.PWBPXBGJRH=["IDIND",[null,null,null],function(newval) { return HtW(newval,'C')}];
...
var res = l_Appl.GetJSONData();
if (res) {
render_suggest(res,'OSQTGJONIV','OSQTGJONIV',true,'GridTable_Band_DIV','GridTable_Band_HEADER',0);
} else {
render_suggest('{Data:[]}','OSQTGJONIV','OSQTGJONIV',true,'GridTable_Band_DIV','GridTable_Band_HEADER');
}
return l_bResult;
}
select_suggest.OSQTGJONIV=["IDSUBIND",[null,null,null],function(newval) { return HtW(newval,'C')}];
...
if ( ! (m_oFather.SaveContext_aosdt_ano_ril())) {
...
parent.ZtVWeb.raiseEventToEvalParms('aosdt_ano_ril_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('aosdt_ano_ril_dataNotSent',{'action':formaction});
}
}
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_aosdt_ano_ril();
}
}
SaveContext.child=[];
function BlankRow() {
