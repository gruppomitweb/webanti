...
public class arrt_sd_scaricodati_globale extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*TPAYWBARVJ*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_sd_scaricodati_globale";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_CAFINVIO) ||  ! (status.m_bpassedw_CFINVIO) ||  ! (status.m_bpassedw_DATAINVIO) ||  ! (status.m_bpassedw_FILEINVIO) ||  ! (status.m_bpassedw_mese) ||  ! (status.m_bpassedw_anno) ||  ! (status.m_bpassedw_files) ||  ! (status.m_bpassedw_seco) ||  ! (status.m_bpassedw_iniz) ||  ! (status.m_bpassedw_datseci) ||  ! (status.m_bpassedw_datsecf) ||  ! (status.m_bpassedw_datpri) ||  ! (status.m_bpassedw_dadata) ||  ! (status.m_bpassedw_a_data);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_sd_scaricodati_globaleR Rcalled;
    Rcalled = new arrt_sd_scaricodati_globaleR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_sd_scaricodati_globaleR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_sd_scaricodati_globaleR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_sd_scaricodati_globale"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_sd_scaricodati_globale");
      if ( ! (CPLib.IsNull(l))) {
