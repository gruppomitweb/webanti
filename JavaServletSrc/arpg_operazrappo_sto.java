...
public class arpg_operazrappo_sto extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*KIILBTUWHA*!";
  ...
  public String getEntityTitle() {
    return "Operazioni/Frazionarie relative al rapporto (Storico)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_operazrappo_sto";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 689;
    l_eg.m_nEntityHeight = 69;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    ...
    l_eg.m_cQueryName = "qbe_operazrappo_sto";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_operazrappo_stoBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10166499690",p_Context.Translate("Operazioni/Frazionarie relative al rapporto (Storico)")},
  ...
  void WriteStyles(arpg_operazrappo_stoBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_operazrappo_sto.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_operazrappo_stoBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_operazrappo_stoBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_operazrappo_stoBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_operazrappo_stoBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_operazrappo_stoBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_operazrappo_stoBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_operazrappo_stoBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_operazrappo_stoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(69,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_operazrappo_sto"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_operazrappo_sto","");
  ...
  void WriteMainFormScript(arpg_operazrappo_stoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_operazrappo_stoBO getBO(ServletStatus status) {
    arpg_operazrappo_stoBO BO = new arpg_operazrappo_stoBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_operazrappo_stoBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_operazrappo_stoBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_operazrappo_stoBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_operazrappo_stoBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_operazrappo_stoBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_operazrappo_stoBO BO) {
    ...
    item.put("altInterfaceFor","arpg_operazrappo_sto");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_operazrappo_stoBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_operazrappo_stoBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_operazrappo_stoBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_operazrappo_stoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_operazrappo_stoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_operazrappo_stoBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_operazrappo_sto?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_operazrappo_sto_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(69,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void RaiseLoadedEntity(arpg_operazrappo_stoBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_operazrappo_stoBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_operazrappo_stoBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_operazrappo_stoBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_operazrappo_stoBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_operazrappo_stoBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_operazrappo_stoBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_operazrappo_stoBO BO) {
    boolean result = true;
    return result;
  }
}
