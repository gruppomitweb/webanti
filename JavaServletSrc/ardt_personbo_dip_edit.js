...
var ctl = _GetCtl(e,'JSWFSTAHBP');
if (_tracker.goon(ctl,ctrlValue)) {
w_CODDIPE=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('JSWFSTAHBP',0);
if (l_bResult) {
l_bResult=Link_JSWFSTAHBP(kmode);
if ( ! (l_bResult)) {
w_CODDIPE=HtW('','C');
}
}
...
function Link_JSWFSTAHBP(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
...
if (Eq(l_nRec,2)) {
var openername = 'linkview_JSWFSTAHBP';
...
function Link_JSWFSTAHBP_Blank() {
w_CODDIPE='';
w_xDESCRIZ='';
}
function LOpt_JSWFSTAHBP() {
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('anadip','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
l_oWv.setValue('PKFields',WtA('CODDIP','C'));
l_oWv.setValue('LinkedField',WtA('CODDIP','C'));
l_oWv.setValue('UID',WtA('JSWFSTAHBP','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['JSWFSTAHBP'][1],'C'));
l_oWv.setValue('Caption',WtA(Translate('551482149'),'C'));
return l_oWv;
}
function ReportLinkValue(varName,value) {
var bResult = false;
if (Eq(varName,'JSWFSTAHBP')) {
last_focused_comp=GetLastFocusedElementOfLink('JSWFSTAHBP');
function SetMethod_JSWFSTAHBP() {
Set_CODDIPE(HtW(value,'C'),0);
}
if (IsDeviceMobile()) {
SetMethod_JSWFSTAHBP();
} else {
setTimeout(SetMethod_JSWFSTAHBP,1);
}
bResult=true;
}
return bResult;
...
if ( ! (m_oFather.SaveContext_ardt_personbo_dip())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_personbo_dip_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_personbo_dip_dataNotSent',{'action':formaction});
}
}
}
}
...
if (( ! (_ChkObl('JSWFSTAHBP',1))) && FullRow()) {
_SignErr('JSWFSTAHBP');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_personbo_dip();
}
}
SaveContext.child=[];
function BlankRow() {
w_CODDIPE='';
w_xDESCRIZ='';
if ( ! (EmptyString(w_CODDIPE))) {
Link_JSWFSTAHBP(null);
}
BlankRowChild();
if (m_bExtended) {
...
function FullRow() {
return  ! (Empty(w_CODDIPE));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.CODDIPE={"dec":0,"fixedpos":false,"id":"JSWFSTAHBP","len":6,"name":"CODDIPE","title":"Codice Dipendenza","type":"C"};
l_aRepeatedFields.xDESCRIZ={"dec":0,"fixedpos":false,"id":"KUPRLIFZIR","len":30,"name":"xDESCRIZ","title":"Descrizione","type":"C"};
