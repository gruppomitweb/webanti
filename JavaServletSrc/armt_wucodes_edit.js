...
function Set_WUCODSTA(ctrlValue,kmode,e) {
if (Ne(w_WUCODSTA,ctrlValue)) {
var ctl = _GetCtl(e,'WXGOCNXMMY');
if (_tracker.goon(ctl,ctrlValue)) {
w_WUCODSTA=ctrlValue;
SetModified();
var l_bResult = true;
...
function Set_WUDESCRI(ctrlValue,kmode,e) {
if (Ne(w_WUDESCRI,ctrlValue)) {
var ctl = _GetCtl(e,'QNVZYIVBZB');
if (_tracker.goon(ctl,ctrlValue)) {
w_WUDESCRI=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
...
function Set_WUCODUIF(ctrlValue,kmode,e) {
if (Ne(w_WUCODUIF,ctrlValue)) {
var ctl = _GetCtl(e,'JRJGWESTVE');
if (_tracker.goon(ctl,ctrlValue)) {
w_WUCODUIF=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
...
function Set_WUCODISO(ctrlValue,kmode,e) {
if (Ne(w_WUCODISO,ctrlValue)) {
var ctl = _GetCtl(e,'RYRRYSUJBK');
if (_tracker.goon(ctl,ctrlValue)) {
w_WUCODISO=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('RYRRYSUJBK',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',5,0,'');
...
var l_nRec = 0;
var l_cLinkUrl;
var l_bEmp = Empty(w_WUCODUIF);
var l_Appl = LinkApplet();
l_Appl.SetStringKey('CODSTA',w_WUCODUIF,3,0);
...
if (Eq(l_nRec,1)) {
w_WUCODUIF=l_Appl.GetStringValue('CODSTA',3,0);
...
parent.ZtVWeb.raiseEventToEvalParms('armt_wucodes_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_wucodes_dataNotSent',{'action':formaction});
}
}
}
}
...
_SignErr('JRJGWESTVE');
m_cLastWorkVarErrorMsg='2046929315';
l_bResult=false;
w_WUCODUIF=HtW('','C');
} else if ( ! (_ChkObl('RYRRYSUJBK',1))) {
_SignErr('RYRRYSUJBK');
m_cLastWorkVarErrorMsg='2046968547';
l_bResult=false;
w_WUCODISO=HtW('','C');
}
if (l_bResult) {
...
l_oWv.setValue('WUCODSTA',WtA(w_WUCODSTA,'C'));
l_oWv.setValue('WUDESCRI',WtA(w_WUDESCRI,'C'));
l_oWv.setValue('WUCODUIF',WtA(w_WUCODUIF,'C'));
l_oWv.setValue('WUCODISO',WtA(w_WUCODISO,'C'));
...
_FillChildren.n=[];
FillWv.n=["WUCODSTA","WUDESCRI","WUCODUIF","WUCODISO","xDESCRI"];
