...
public class arpg_xraziobo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*OIFHMCFPAN*!";
  ...
  public String getEntityTitle() {
    return "Frazionarie relative al rapporto (Provvisorie)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_xraziobo";
    ...
    l_eg.m_cQueryName = "qbe_operazrappoprv2";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_xrazioboBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10154848271",p_Context.Translate("Frazionarie relative al rapporto (Provvisorie)")},
  ...
  void WriteStyles(arpg_xrazioboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_xraziobo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_xrazioboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_xrazioboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_xrazioboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_xrazioboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_xrazioboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_xrazioboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_xrazioboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_xrazioboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_xraziobo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_xraziobo","");
    ...
    status.out.println("// * --- arpg_xraziobo");
  ...
  void WriteMainFormScript(arpg_xrazioboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_xrazioboBO getBO(ServletStatus status) {
    arpg_xrazioboBO BO = new arpg_xrazioboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_xrazioboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_xrazioboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_xrazioboBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_xrazioboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_xrazioboBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_xrazioboBO BO) {
    ...
    item.put("altInterfaceFor","arpg_xraziobo");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_xrazioboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_xrazioboBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_xrazioboBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_xrazioboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_xrazioboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_xrazioboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_xraziobo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_xraziobo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_xrazioboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_xrazioboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_xrazioboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_xrazioboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_xrazioboBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_xrazioboBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_xrazioboBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_xrazioboBO BO) {
    boolean result = true;
    return result;
  }
}
