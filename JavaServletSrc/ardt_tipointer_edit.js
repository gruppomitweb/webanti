...
var ctl = _GetCtl(e,'GCFQVRXGQC');
if (_tracker.goon(ctl,ctrlValue)) {
w_tipinter=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('GCFQVRXGQC',0);
if (l_bResult) {
l_bResult=Link_GCFQVRXGQC(kmode);
if ( ! (l_bResult)) {
w_tipinter=HtW('','C');
}
}
...
function Link_GCFQVRXGQC(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
...
function Link_GCFQVRXGQC_Blank() {
w_tipinter='';
w_xdescinter='';
}
function LOpt_GCFQVRXGQC() {
...
l_oWv.setValue('UID',WtA('GCFQVRXGQC','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['GCFQVRXGQC'][1],'C'));
l_oWv.setValue('Caption',WtA(Translate('11120464402'),'C'));
return l_oWv;
}
function ReportLinkValue(varName,value) {
var bResult = false;
if (Eq(varName,'GCFQVRXGQC')) {
last_focused_comp=GetLastFocusedElementOfLink('GCFQVRXGQC');
function SetMethod_GCFQVRXGQC() {
Set_tipinter(HtW(value,'C'),0);
}
if (IsDeviceMobile()) {
SetMethod_GCFQVRXGQC();
} else {
setTimeout(SetMethod_GCFQVRXGQC,1);
}
bResult=true;
}
return bResult;
...
if ( ! (m_oFather.SaveContext_ardt_tipointer())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_tipointer_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_tipointer_dataNotSent',{'action':formaction});
}
}
}
}
...
if (( ! (_ChkObl('GCFQVRXGQC',1))) && FullRow()) {
_SignErr('GCFQVRXGQC');
m_cLastWorkVarErrorMsg='1250573741';
l_bResult=false;
w_tipinter=HtW('','C');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_tipointer();
}
}
SaveContext.child=[];
function BlankRow() {
w_tipinter='';
w_xdescinter='';
if ( ! (EmptyString(w_tipinter))) {
Link_GCFQVRXGQC(null);
}
BlankRowChild();
if (m_bExtended) {
...
function FullRow() {
return  ! (Empty(w_tipinter));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.tipinter={"dec":0,"fixedpos":false,"id":"GCFQVRXGQC","len":2,"name":"tipinter","title":"Valido per tipo intermediario","type":"C"};
l_aRepeatedFields.xdescinter={"dec":0,"fixedpos":false,"id":"IZLSVDYFRS","len":80,"name":"xdescinter","title":"Descrizione","type":"C"};
