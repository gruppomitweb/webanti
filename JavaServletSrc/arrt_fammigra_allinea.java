...
public class arrt_fammigra_allinea extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*4bb085d0*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_fammigra_allinea";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_fammigra_allineaR Rcalled;
    Rcalled = new arrt_fammigra_allineaR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aa_retval = Rcalled.Run(status.w_dadata,status.w_a_data);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_fammigra_allineaR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_fammigra_allineaR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_fammigra_allinea"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
