...
// * --- Fine Area Manuale
public class arrt_ae_delinfo_agg_aggR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_ae_delinfo_agg_aggR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_ae_delinfo_agg_agg",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_ae_delinfo_agg_agg";
  ...
  void Page_1() throws Exception {
    ...
    if (CPLib.eqr(w_TIPOOPRAP,"23")) {
      ...
      m_cSql = CPLib.GetSQLPrefixComment("arrt_ae_delinfo_agg_agg",7,"00000007")+"delete from "+m_oWrInfo.GetTableWriteName();
      m_cSql = m_cSql+" where ";
      m_cSql = m_cSql+"RAPPORTO = "+CPSql.SQLValueAdapter(CPLib.ToSQL(w_RAPPORTO,"?",0,0,m_cServer),m_cServer,w_RAPPORTO)+"";
      m_cSql = m_cSql+" and NUMPROG1 = "+CPSql.SQLValueAdapter(CPLib.ToSQL(w_NUMPROG,"?",0,0,m_cServer),m_cServer,w_NUMPROG)+"";
      ...
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"jntestbo",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_ae_delinfo_agg_agg",7,"00000008")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      ...
      /* ElseIf w_TIPOOPRAP='25' */
    } else if (CPLib.eqr(w_TIPOOPRAP,"25")) {
      ...
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_ae_delinfo_agg_agg",7,"00000009")+"delete from "+m_oWrInfo.GetTableWriteName();
      m_cSql = m_cSql+" where ";
      m_cSql = m_cSql+"RAPPORTO = "+CPSql.SQLValueAdapter(CPLib.ToSQL(w_RAPPORTO,"?",0,0,m_cServer),m_cServer,w_RAPPORTO)+"";
      m_cSql = m_cSql+" and NUMPROG1 = "+CPSql.SQLValueAdapter(CPLib.ToSQL(w_NUMPROG,"?",0,0,m_cServer),m_cServer,w_NUMPROG)+"";
      m_cSql = m_cSql+m_oWrInfo.WhereFilter();
      ...
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"jelegabo",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_ae_delinfo_agg_agg",7,"0000000A")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      m_cSql = m_cSql+"NUMPROG2 = "+CPLib.ToSQL("","C",11,0,m_cServer)+", ";
      m_cSql = m_cSql+"DATAFINE = "+CPLib.ToSQL(CPLib.NullDate(),"D",8,0)+", ";
      m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"jelegabo",false,true);
      m_cSql = m_cSql+" where ";
      m_cSql = m_cSql+"RAPPORTO = "+CPSql.SQLValueAdapter(CPLib.ToSQL(w_RAPPORTO,"?",0,0,m_cServer),m_cServer,w_RAPPORTO)+"";
      m_cSql = m_cSql+" and NUMPROG2 = "+CPSql.SQLValueAdapter(CPLib.ToSQL(w_NUMPROG,"?",0,0,m_cServer),m_cServer,w_NUMPROG)+"";
      m_cSql = m_cSql+m_oWrInfo.WhereFilter();
      m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql,m_oParameters);
      if (CPLib.lt(m_nUpdatedRows,0)) {
        m_Sql.AbortTransaction();
  ...
  public static arrt_ae_delinfo_agg_aggR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_ae_delinfo_agg_aggR(p_Ctx, p_Caller);
  }
  public void Blank() {
