...
public class arrt_sd_aprichiudi_delope extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*BYEDTTPMII*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_sd_aprichiudi_delope";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_sd_aprichiudi_delopeR Rcalled;
    Rcalled = new arrt_sd_aprichiudi_delopeR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.forwardedto = Rcalled.Run(status.w_a_data,status.w_dadata,status.w_FILEINVIO,status.w_DATAINVIO,status.w_CFINVIO,status.w_CAFINVIO,status.w_anno,status.w_mese);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_sd_aprichiudi_delopeR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_sd_aprichiudi_delopeR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      status.out.println(
       "<!--" +
      "");
      status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(Rcalled.RunAsync(status.w_a_data,status.w_dadata,status.w_FILEINVIO,status.w_DATAINVIO,status.w_CFINVIO,status.w_CAFINVIO,status.w_anno,status.w_mese)))+"");
      status.out.println(
       " -->" +
      "");
    } else {
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_sd_aprichiudi_delope"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_sd_aprichiudi_delope");
      if ( ! (CPLib.IsNull(l))) {
