...
// * --- Fine Area Manuale
public class ardt_provsoginfoBO extends ardt_provsoginfoBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_2(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_2_2.TIPLEG AS TIPLEG202";
        m_cSelect = m_cSelect+" ,link_2_2.DESCRIZ AS DESCRIZ202";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_2","LEGAME","TIPLEG");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_2","LEGAME","TIPLEG",true);
          if (CPLib.eq(m_cLinkedWhere,"")) {
            m_cFrom = m_cFrom+m_cJoinCond;
          } else {
            m_cWhere = m_cWhere+m_cLinkedWhere+m_cJoinCond;
  ...
  // * --- Fine Area Manuale
  public ardt_provsoginfoBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_provsoginfo");
    }
  ...
  public static String[] m_cLoadParameterNames = {"RAPCOL"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_RAPCOL) {
    CPResultSet l_rsLoadBody=null;
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_xoginfo+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xoginfo,m_cPhName_xoginfo,m_cVirtName_xoginfo,m_cVirtName_xoginfo+".RAPCOL="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPCOL,"C",25,0),m_cServer_xoginfo,p_RAPCOL),l_cOrderBy);
    ...
    l_oSqlDetailLoader.AddJoinedLink_2_2(m_cPhName_tbtipleg,m_cServer_tbtipleg);
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            ...
            Link_FVKNSPNHTX("Load");
          }
          w_LEGAME = l_rsLoadBody.GetString("LEGAME");
          if (l_oSqlDetailLoader.m_bLink_2_2) {
            w_xdescleg = l_rsLoadBody.GetString("DESCRIZ202");
          } else {
            Link_KQROMFNGNZ("Load");
          }
  ...
  public boolean Delete(String p_RAPCOL) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_xoginfo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPCOL="+CPLib.ToSQL(w_RAPCOL,"C",25,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_xoginfo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xoginfo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_xoginfo,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_RAPCOL);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"FVKNSPNHTX")) {
      ...
      result.AppendFieldsHash("FVKNSPNHTX",CPLib.GetLinkedFieldProps("CONNES","CODSOG","C",16,0));
    } else if (CPLib.eq(p_cID,"KQROMFNGNZ")) {
      ...
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("KQROMFNGNZ",CPLib.GetLinkedFieldProps("TIPLEG","LEGAME","C",1,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xoginfo);
      l_cSql = "insert into "+m_oWrInfo_xoginfo.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDFILEBO,";
      l_cSql = l_cSql+"RAPCOL,";
      l_cSql = l_cSql+"CODSOG,";
      l_cSql = l_cSql+"LEGAME,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_xoginfo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xoginfo")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_RAPCOL,"C",25,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_CODSOG,"C",16,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_LEGAME,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(m_nCPRowNum,"N",10,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_xoginfo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xoginfo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xoginfo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xoginfo);
      l_cSql = "Update "+m_oWrInfo_xoginfo.GetTableWriteName()+" set";
      l_cSql = l_cSql+" IDFILEBO = "+CPLib.ToSQL(w_IDFILEBO,"C",10,0);
      l_cSql = l_cSql+", CODSOG = "+CPLib.ToSQLNull(w_CODSOG,"C",16,0);
      l_cSql = l_cSql+", LEGAME = "+CPLib.ToSQLNull(w_LEGAME,"C",1,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xoginfo")) {
