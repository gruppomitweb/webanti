...
public class arrt_edtrapotit extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ec39736c*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (m_bpassedw_STATOREG) {
        ...
        if (source.IsForwarded()) {
          w_STATOREG = source.GetCaller().GetString("STATOREG","C",10,0);
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_STATOREG) ||  ! (status.m_bpassedw_IDBASE2) ||  ! (status.m_bpassedw_IDEREG) ||  ! (status.m_bpassedw_NUMPROG) ||  ! (status.m_bpassedw_DATAOPE) ||  ! (status.m_bpassedw_DATARETT) ||  ! (status.m_bpassedw_OPERAZMOD) ||  ! (status.m_bpassedw_IDBASE);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_STATOREG)) {
      status.out.println("l_cV=WtA(l_oWnd('w_STATOREG',"+SPLib.ToJSValue(status.w_STATOREG,"C",10,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_STATOREG,"C",10,0)+",'C');");
    }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_edtrapotitR Rcalled;
    Rcalled = new arrt_edtrapotitR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_edtrapotitR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_edtrapotitR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_edtrapotit"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_edtrapotit");
      if ( ! (CPLib.IsNull(l))) {
