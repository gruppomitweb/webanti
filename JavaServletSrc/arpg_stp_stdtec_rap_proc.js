...
HideUI.lblids["aRapporto"]=['JGQCNSZOCQ'];
HideUI.lblids["daCodDip"]=['ZWZOQIGZCX'];
HideUI.lblids["daRapporto"]=['PCILMATBXL'];
...
l_oWv.setValue('m_cParameterSequence',AsAppletValue('pAUI'));
_modifyandopen('arrt_stp_stdtec_rap'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes,height=500,width=700',null)
}
...
function Help() {
windowOpenForeground('../doc/arpg_stp_stdtec_rap_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_stp_stdtec_rap',m_cSelectedPage);
...
if (l_bResult) {
l_bResult=Le(w_DaDatOpe,CharToDate('20131231'));
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='DaDatOpe';
m_cLastMsgError=AlertErrorMessage('1839236176');
}
if ( ! (l_bResult)) {
w_DaDatOpe=HtW('','D');
}
...
if (l_bResult) {
l_bResult=Ge(w_ADatOpe,w_DaDatOpe) && Le(w_ADatOpe,CharToDate('20131231'));
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='ADatOpe';
m_cLastMsgError=AlertErrorMessage('623827580');
}
if ( ! (l_bResult)) {
w_ADatOpe=HtW('','D');
}
...
l_bResult=false;
w_aCodDip=HtW('','C');
} else if (( ! (_ChkObl('DPKVLWHCIG',1))) || ( ! (Le(w_DaDatOpe,CharToDate('20131231'))))) {
_SignErr('DPKVLWHCIG','1839236176');
l_bResult=false;
w_DaDatOpe=HtW('','D');
} else if (( ! (_ChkObl('MTJDZQTEIR',1))) || ( ! (Ge(w_ADatOpe,w_DaDatOpe) && Le(w_ADatOpe,CharToDate('20131231'))))) {
_SignErr('MTJDZQTEIR','623827580');
l_bResult=false;
w_ADatOpe=HtW('','D');
...
_FillChildren.n=[];
FillWv.n=["daCodDip","aCodDip","DaDatOpe","ADatOpe","daRapporto","aRapporto","RAGSOC1","RAGSOC2","DESCRIZ1","DESCRIZ2"];
