...
// * --- Fine Area Manuale
public class arpg_cgo_rapporti_errBO extends arpg_cgo_rapporti_errBL {
  ...
  public arpg_cgo_rapporti_errBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_PRGRAP = rs.GetString("PRGRAP");
          c_RAPPORTO = rs.GetString("RAPPORTO");
          c_DESCRAP = rs.GetString("DESCRAP");
          c_DATAOPE = rs.GetDate("DATAOPE");
          c_CODINTEST = rs.GetString("CODINTEST");
          c_TIPOMOV = rs.GetString("TIPOMOV");
          AddTrsRow();
          rs.Next();
          l_nCnt = l_nCnt+1;
        }
