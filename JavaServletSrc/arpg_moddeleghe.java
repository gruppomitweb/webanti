...
public class arpg_moddeleghe extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*JOOGZDQCKD*!";
  ...
  public String getEntityTitle() {
    return "Aggiorna i dati di apertura o delegato";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_moddeleghe";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 677;
    l_eg.m_nEntityHeight = 376;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("xtipopers","C",1,0, "hide",""),
                   ...
                   new SPItem("idesito","C",8,0, "show","Esito Agenzia Entrate"),
                   new SPItem("newdatec","D",8,0, "edit",""),
                   new SPItem("newdate","D",8,0, "edit",""),
                   new SPItem("xdesccit","C",30,0, "hide","Città"),
                   new SPItem("xpaese","C",3,0, "hide","Stato"),
                   new SPItem("xprovincia","C",2,0, "hide","Provincia"),
                   new SPItem("xcap","C",9,0, "hide","CAP"),
                   new SPItem("xragold","C",70,0, "hide","Ragione Sociale"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_moddelegheBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1826587046",p_Context.Translate("Aggiorna i dati di apertura o delegato")},
  ...
  void WriteStyles(arpg_moddelegheBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_moddeleghe.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_moddelegheBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_moddelegheBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_moddelegheBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(371);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_moddelegheBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='ISRGWEYPBE'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Nuovo Delegato:"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    status.out.println(
     ...
     "<label id='YMQGLGBXMV'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Vecchio Delegato:"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
    ...
    status.out.println(
     ...
     "<label class='SPLabel' for='ZSRVHHSRRD_0'>"+status.context.Translate("Modifica dati Delegato")+"</label>" +
     "</span>" +
     ...
     "<label class='SPLabel' for='ZSRVHHSRRD_1'>"+status.context.Translate("Cambio Delegato")+"</label>" +
     "</span>" +
     "</div>" +
    "");
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='HMTQCDJJDS'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Modifica Dati Delegato"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='AFTPFXBXCR'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Cambio Delegato"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
     ...
     "<label id='AWATMAIZMU'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("MODIFICA DATI PER SCARTO DELEGA DA ENTRATEL ?"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    "");
    status.out.println(
     "<input name='idesito' id='RROSTBCQKE' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_idesito,"C",8,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='8' size='8' fieldtype='C'>" +
    "");
    status.out.println(
  ...
  static void PrintState(arpg_moddelegheBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_moddelegheBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_moddelegheBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_xdesccit,"C",30,0));
  ...
  void WriteMainFormVarScript(arpg_moddelegheBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(376,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_moddeleghe"),"C",0,0)+ ";");
    ...
    status.out.print(",'EEESEXUZFK':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash("CONNESCONNES,RAGSOC,CODFISC,NASCOMUN,DATANASC,SESSO,NASSTATO,COGNOME,NOME,CFESTERO,DESCCIT,PAESE,PROVINCIA,CAP,TIPINTERC,C,C,C,D,C,C,C,C,N,C,C,C,C,Cpersonbo",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,personbo,"+"default",l_session)+"'"+"]");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_moddeleghe","");
  ...
  void WriteMainFormScript(arpg_moddelegheBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_moddeleghe");
    ...
    status.out.println("	  alert('Obbligatorio specificare data Inizio Delega')");
    status.out.println("	  return");
    status.out.println("  }");
    status.out.println("  if (w_tipochg=='M') {");
    status.out.println("	  if (Empty(w_xcodfisc) && w_xcfestero==0){");
    status.out.println("		  alert('Obbligatorio specificare Codice Fiscale Delegato')");
    status.out.println("		  return");
    status.out.println("    }  ");
    status.out.println("    if (w_xtipopers=='P') {");
    status.out.println("		  if (Empty(w_xcognome)) {");
    status.out.println("			  alert('Obbligatorio specificare Cognome Delegato')");
    status.out.println("			  return");
    status.out.println("		  }");
    ...
    status.out.println("			  return");
    status.out.println("		  }");
    status.out.println("		  if (Empty(w_xnascomun)) {");
    status.out.println("			  alert('Obbligatorio specificare Comune di nascita Intestatario')");
    status.out.println("			  return");
    status.out.println("		  }");
    status.out.println("		  if (Empty(w_xnasstato)) {");
    status.out.println("			  alert('Obbligatorio specificare Stato di nascita Intestatario')");
    status.out.println("			  return");
    status.out.println("		  }      ");
    ...
    status.out.println("	  }");
    status.out.println("  }      ");
    status.out.println("  if (confirm('La modifica comporta La modifica comporta obbligo di ricomunicare i dati!! \\n Si conferma la modifica?')) {");
    status.out.println("    SetHypPar('m_cAction',AsAppletValue('start'));");
    status.out.println("    SetHypPar('m_cMode',AsAppletValue('hyperlink'));");
    status.out.println("    SendData('start',PlatformPathStart('arrt_delopen1')+'',HParApplet().asString());");
    status.out.println("  }  ");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
  ...
  arpg_moddelegheBO getBO(ServletStatus status) {
    arpg_moddelegheBO BO = new arpg_moddelegheBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_moddelegheBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_moddelegheBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_moddelegheBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_moddelegheBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_moddelegheBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_moddelegheBO BO) {
    ...
    item.put("altInterfaceFor","arpg_moddeleghe");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"364\",\"name\":\"Box_ODCPZZXLHY\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"ODCPZZXLHY\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"664\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Codice Fiscale\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"xcodfisc\",\"page\":\"1\",\"spuid\":\"KXGHBTYQDK\",\"tabindex\":\"8\",\"type\":\"Textbox\",\"w\":\"136\",\"x\":\"136\",\"y\":\"140\",\"zone\":\"ODCPZZXLHY_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Cognome\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"26\",\"name\":\"xcognome\",\"page\":\"1\",\"spuid\":\"ZLEFBXLUTT\",\"tabindex\":\"9\",\"type\":\"Textbox\",\"w\":\"168\",\"x\":\"136\",\"y\":\"192\",\"zone\":\"VYHZGJKBSG_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Nome\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"25\",\"name\":\"xnome\",\"page\":\"1\",\"spuid\":\"TRZRNHUQSZ\",\"tabindex\":\"10\",\"type\":\"Textbox\",\"w\":\"162\",\"x\":\"352\",\"y\":\"192\",\"zone\":\"VYHZGJKBSG_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Sesso\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"name\":\"xsesso\",\"page\":\"1\",\"spuid\":\"VHDBEIXHTA\",\"tabindex\":\"11\",\"textlist\":\"Maschile,Femminile\",\"type\":\"Combobox\",\"valuelist\":\""+"'"+"M"+"'"+","+"'"+"F"+"'"+"\",\"w\":\"96\",\"x\":\"560\",\"y\":\"192\",\"zone\":\"VYHZGJKBSG_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Nuovo_Delegato_\",\"page\":\"1\",\"spuid\":\"ISRGWEYPBE_DIV\",\"tabindex\":\"13\",\"type\":\"Label\",\"value\":\"Nuovo Delegato:\",\"w\":\"110\",\"x\":\"22\",\"y\":\"305\",\"zone\":\"QEMAGEFKQK_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"newintest\",\"page\":\"1\",\"spuid\":\"NOGBUOIVWB\",\"tabindex\":\"14\",\"type\":\"Textbox\",\"w\":\"118\",\"x\":\"136\",\"y\":\"301\",\"zone\":\"QEMAGEFKQK_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_xragnew\",\"page\":\"1\",\"spuid\":\"EHGZXQREHH_DIV\",\"tabindex\":\"16\",\"type\":\"Label\",\"value\":\"w_xragnew\",\"w\":\"376\",\"x\":\"280\",\"y\":\"305\",\"zone\":\"QEMAGEFKQK_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Vecchio_Delegato_\",\"page\":\"1\",\"spuid\":\"YMQGLGBXMV_DIV\",\"tabindex\":\"17\",\"type\":\"Label\",\"value\":\"Vecchio Delegato:\",\"w\":\"118\",\"x\":\"14\",\"y\":\"116\",\"zone\":\"ODCPZZXLHY_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_YBBLCLWLHJ()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"YBBLCLWLHJ\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"20\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"336\",\"y\":\"333\",\"zone\":\"ODCPZZXLHY_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui aggiornamento dati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_MDWFLPZWFN()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"MDWFLPZWFN\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"21\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"288\",\"y\":\"333\",\"zone\":\"ODCPZZXLHY_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"19\",\"name\":\"tipochg\",\"orientation\":\"horizontal\",\"page\":\"1\",\"spuid\":\"ZSRVHHSRRD\",\"tabindex\":\"27\",\"textlist\":\"Modifica dati Delegato,Cambio Delegato\",\"type\":\"Radio\",\"valuelist\":\""+"'"+"M"+"'"+","+"'"+"C"+"'"+"\",\"w\":\"279\",\"x\":\"136\",\"y\":\"91\",\"zone\":\"ODCPZZXLHY_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"88\",\"name\":\"Box_VYHZGJKBSG\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"VYHZGJKBSG\",\"tabindex\":\"28\",\"type\":\"Box\",\"w\":\"648\",\"x\":\"16\",\"y\":\"185\",\"zone\":\"ODCPZZXLHY_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Modifica_Dati_Delegato\",\"page\":\"1\",\"spuid\":\"HMTQCDJJDS_DIV\",\"tabindex\":\"29\",\"type\":\"Label\",\"value\":\"Modifica Dati Delegato\",\"w\":\"238\",\"x\":\"16\",\"y\":\"168\",\"zone\":\"ODCPZZXLHY_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Codice_Fiscale_\",\"page\":\"1\",\"spuid\":\"AJUVDYOZZL_DIV\",\"tabindex\":\"30\",\"type\":\"Label\",\"value\":\"Codice Fiscale:\",\"w\":\"72\",\"x\":\"60\",\"y\":\"144\",\"zone\":\"ODCPZZXLHY_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Cognome_\",\"page\":\"1\",\"spuid\":\"SSUWRDQRDI_DIV\",\"tabindex\":\"31\",\"type\":\"Label\",\"value\":\"Cognome:\",\"w\":\"49\",\"x\":\"83\",\"y\":\"196\",\"zone\":\"VYHZGJKBSG_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Nome_\",\"page\":\"1\",\"spuid\":\"HHYAFDYFPB_DIV\",\"tabindex\":\"32\",\"type\":\"Label\",\"value\":\"Nome:\",\"w\":\"31\",\"x\":\"317\",\"y\":\"196\",\"zone\":\"VYHZGJKBSG_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Sesso_\",\"page\":\"1\",\"spuid\":\"HAZSNHXYTH_DIV\",\"tabindex\":\"33\",\"type\":\"Label\",\"value\":\"Sesso:\",\"w\":\"32\",\"x\":\"524\",\"y\":\"196\",\"zone\":\"VYHZGJKBSG_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Comune_di_Nascita_\",\"page\":\"1\",\"spuid\":\"DFMOVYUECJ_DIV\",\"tabindex\":\"34\",\"type\":\"Label\",\"value\":\"Comune di Nascita:\",\"w\":\"92\",\"x\":\"40\",\"y\":\"221\",\"zone\":\"VYHZGJKBSG_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Luogo di Nascita\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"40\",\"name\":\"xnascomun\",\"page\":\"1\",\"spuid\":\"HBSWFKBFZJ\",\"tabindex\":\"35\",\"type\":\"Textbox\",\"w\":\"262\",\"x\":\"136\",\"y\":\"217\",\"zone\":\"VYHZGJKBSG_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Provincia__EE_se_estero_\",\"page\":\"1\",\"spuid\":\"WHBMDRJRIX_DIV\",\"tabindex\":\"36\",\"type\":\"Label\",\"value\":\"Provincia (EE se estero)\",\"w\":\"114\",\"x\":\"425\",\"y\":\"220\",\"zone\":\"VYHZGJKBSG_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Provincia\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"2\",\"name\":\"xtipinter\",\"page\":\"1\",\"spuid\":\"MPXBUDNRPY\",\"tabindex\":\"37\",\"type\":\"Textbox\",\"w\":\"32\",\"x\":\"544\",\"y\":\"217\",\"zone\":\"VYHZGJKBSG_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Stato_di_Nascita_\",\"page\":\"1\",\"spuid\":\"RNCFPZEWNP_DIV\",\"tabindex\":\"38\",\"type\":\"Label\",\"value\":\"Stato di Nascita:\",\"w\":\"92\",\"x\":\"40\",\"y\":\"245\",\"zone\":\"VYHZGJKBSG_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stato di nascita\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"40\",\"name\":\"xnasstato\",\"page\":\"1\",\"spuid\":\"YDHXQFESNA\",\"tabindex\":\"39\",\"type\":\"Textbox\",\"w\":\"262\",\"x\":\"136\",\"y\":\"241\",\"zone\":\"VYHZGJKBSG_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_di_Nascita_\",\"page\":\"1\",\"spuid\":\"OSVHVNFSTF_DIV\",\"tabindex\":\"40\",\"type\":\"Label\",\"value\":\"Data di Nascita:\",\"w\":\"76\",\"x\":\"463\",\"y\":\"245\",\"zone\":\"VYHZGJKBSG_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Data Nascita\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"xdatnasc\",\"page\":\"1\",\"spuid\":\"SBDBIBIUBZ\",\"tabindex\":\"41\",\"type\":\"Textbox\",\"w\":\"72\",\"x\":\"544\",\"y\":\"241\",\"zone\":\"VYHZGJKBSG_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"name\":\"Box_QEMAGEFKQK\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"QEMAGEFKQK\",\"tabindex\":\"42\",\"type\":\"Box\",\"w\":\"648\",\"x\":\"16\",\"y\":\"297\",\"zone\":\"ODCPZZXLHY_11\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Cambio_Delegato\",\"page\":\"1\",\"spuid\":\"AFTPFXBXCR_DIV\",\"tabindex\":\"43\",\"type\":\"Label\",\"value\":\"Cambio Delegato\",\"w\":\"256\",\"x\":\"16\",\"y\":\"280\",\"zone\":\"ODCPZZXLHY_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"MODIFICA_DATI_PER_SCARTO_DELEGA_DA_ENTRATEL__\",\"page\":\"1\",\"spuid\":\"AWATMAIZMU_DIV\",\"tabindex\":\"45\",\"type\":\"Label\",\"value\":\"MODIFICA DATI PER SCARTO DELEGA DA ENTRATEL ?\",\"w\":\"320\",\"x\":\"16\",\"y\":\"15\",\"zone\":\"ODCPZZXLHY_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"olddatac\",\"page\":\"1\",\"spuid\":\"NCHAEUMUPJ\",\"tabindex\":\"46\",\"type\":\"Textbox\",\"w\":\"80\",\"x\":\"568\",\"y\":\"35\",\"zone\":\"ODCPZZXLHY_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Chiusura_\",\"page\":\"1\",\"spuid\":\"EVXMXJTANP_DIV\",\"tabindex\":\"47\",\"type\":\"Label\",\"value\":\"Data Chiusura:\",\"w\":\"108\",\"x\":\"456\",\"y\":\"39\",\"zone\":\"ODCPZZXLHY_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esito Agenzia Entrate\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"8\",\"name\":\"idesito\",\"page\":\"1\",\"spuid\":\"RROSTBCQKE\",\"type\":\"Textbox\",\"w\":\"80\",\"x\":\"136\",\"y\":\"63\",\"zone\":\"ODCPZZXLHY_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Esito_Agenzia_Entrate_\",\"page\":\"1\",\"spuid\":\"EFJEEIKTHM_DIV\",\"tabindex\":\"49\",\"type\":\"Label\",\"value\":\"Esito Agenzia Entrate:\",\"w\":\"128\",\"x\":\"8\",\"y\":\"67\",\"zone\":\"ODCPZZXLHY_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Nuova_Apertura_\",\"page\":\"1\",\"spuid\":\"OLVLBDQTYW_DIV\",\"tabindex\":\"51\",\"type\":\"Label\",\"value\":\"Nuova Apertura:\",\"w\":\"116\",\"x\":\"232\",\"y\":\"63\",\"zone\":\"ODCPZZXLHY_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"newdatec\",\"page\":\"1\",\"spuid\":\"NICAEATMFV\",\"tabindex\":\"52\",\"type\":\"Textbox\",\"w\":\"78\",\"x\":\"568\",\"y\":\"59\",\"zone\":\"ODCPZZXLHY_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Nuova_Chiusura_\",\"page\":\"1\",\"spuid\":\"VYPUVNDTCE_DIV\",\"tabindex\":\"53\",\"type\":\"Label\",\"value\":\"Nuova Chiusura:\",\"w\":\"108\",\"x\":\"456\",\"y\":\"63\",\"zone\":\"ODCPZZXLHY_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"newdate\",\"page\":\"1\",\"spuid\":\"WYJHXYCTSP\",\"tabindex\":\"54\",\"type\":\"Textbox\",\"w\":\"78\",\"x\":\"352\",\"y\":\"59\",\"zone\":\"ODCPZZXLHY_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
  ...
  void Next(ServletStatus status,arpg_moddelegheBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_moddelegheBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_moddelegheBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_moddeleghe?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_moddeleghe_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(376,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
  ...
  void RaiseLoadedEntity(arpg_moddelegheBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_moddelegheBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_moddelegheBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_moddelegheBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_moddelegheBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_moddelegheBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_moddelegheBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_moddelegheBO BO) {
    boolean result = true;
    return result;
  }
}
