...
public class arpg_import_rtra extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*KHKKJBONRP*!";
  ...
  public String getEntityTitle() {
    return "Import dati da procedure esterne (RTRA)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_import_rtra";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 799;
    l_eg.m_nEntityHeight = 586;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("idmage","C",5,0, "edit","MacroAgente"),
                   ...
                   new SPItem("xMANAIDWU","C",5,0, "hide","NAID"),
                   new SPItem("xMAFLGCHKDAT","C",1,0, "hide","Dati certi per Soggetti"),
                   new SPItem("delprec","C",10,0, "edit","")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_import_rtraBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10901286719",p_Context.Translate("Path Applicazione")},
    ...
    {"11957619892",p_Context.Translate("Cancella dati esistenti")},
    ...
    {"1256120364",p_Context.Translate("Import dati da procedure esterne (RTRA)")},
    ...
    {"292774027",p_Context.Translate("Confermi l'import dei dati ?")},
  ...
  void WriteStyles(arpg_import_rtraBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_import_rtra.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_import_rtraBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_import_rtraBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_import_rtraBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  // GetPortlet_ListaFile_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_MVXXCZOQGP_Vars(arpg_import_rtraBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_import_rtraBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_import_rtraBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(581);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_import_rtraBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input name='idmage' id='XFVQTXPEXL' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_idmage,"C",5,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='5' size='5' fieldtype='C'>" +
    ...
    "");
    status.out.println(
     "<span class='SPLabel' id='YKEVRMKLNK_DIV'>" +
     "<label id='YKEVRMKLNK'>" +
    ...
    "");
    status.out.println(
     "<span class='SPLabel' id='KRFHEOVRDJ_DIV'>" +
     "<label id='KRFHEOVRDJ'>" +
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<span id='GSBFOKJJZN_DIV'>" +
     "<input name='delprec' id='GSBFOKJJZN' type='checkbox' value="+SPLib.ToHTMLValue(BO.w_delprec,"C",10,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq("S",CPLib.Trim(BO.w_delprec))?"CHECKED":"")+">" +
     "<label id='GSBFOKJJZN_LBL' for='GSBFOKJJZN' class='SPLabel'>"+SPLib.ToHTMLRepresentation(status.context.Translate("Cancella dati esistenti"))+"</label>" +
     "</span>" +
    "");
    status.themeWriter.Header_EndHtml();
  }
  static void PrintState(arpg_import_rtraBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_import_rtraBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_import_rtraBL BO,ServletStatus status) {
    status.out.print("DeclareWVs(");
    status.out.print(SPLib.ToJSValue(BO.w_tipodest,"C",1,0));
    status.out.print(","+"["+SPLib.ToJSValue(BO.w_idmage,"C",5,0)+","+SPLib.ToJSValue(BO.o_idmage,"C",5,0)+"]");
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_xMANAIDWU,"C",5,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_xMAFLGCHKDAT,"C",1,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_delprec,"C",10,0));
    status.out.println(")");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      status.out.println("m_bUpdated="+SPLib.ToJSValue(BO.m_bUpdated,"L",0,0)+";");
    }
  }
  void WriteMainFormVarScript(arpg_import_rtraBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    PrintState(BO,status);
    status.out.println("var e_FGMOCXNBFY = " + SPLib.ToJSValue(m_cEntityUid,"C",0,0)+ ";");
    status.out.println("var e_KRFHEOVRDJ = " + SPLib.ToJSValue(m_cEntityUid,"C",0,0)+ ";");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(586,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_import_rtra"),"C",0,0)+ ";");
    status.out.println("var m_bShowSuggest = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    status.out.println("var m_bShowNotifyBtn = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    status.out.println("var m_bShowExtendReduceBtn = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    HttpSession l_session = ((SPContext)status.context).session;
    status.out.print("var m_IDS={");
    status.out.print("'XFVQTXPEXL':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash("MACODICEMACODICE,MADESCRI,MAFLGRAP,MAFLGOPE,MAFLGBON,MAFLGCKR,MAFLGASR,MADIPSTD,MANAIDWU,MAFLGCHKDATC,C,C,C,C,C,C,C,C,Ctbmacage",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,tbmacage,"+"default",l_session)+"'"+"]");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_import_rtra","");
  ...
  void WriteMainFormScript(arpg_import_rtraBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_import_rtra");
    status.out.println("function UploadDoc() {");
    status.out.println("  m_bUpdated=true");
    status.out.println("  windowOpenForeground(PlatformPathStart('vRepuploadWU')+'?m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','','toolbar=0,menubar=0,directories=0,width=500,height=400,resizable=1,scrollbars=1')  ");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
     "</script>" +
  ...
  arpg_import_rtraBO getBO(ServletStatus status) {
    arpg_import_rtraBO BO = new arpg_import_rtraBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_import_rtraBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_import_rtraBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_import_rtraBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_import_rtraBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_import_rtraBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_import_rtraBO BO) {
    ...
    item.put("altInterfaceFor","arpg_import_rtra");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"574\",\"name\":\"Box_YWGFSYCMOZ\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"YWGFSYCMOZ\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"25\",\"href\":\"javascript:DZUBEERBGN_Click()\",\"name\":\"Seleziona\",\"page\":\"1\",\"spuid\":\"DZUBEERBGN\",\"tabindex\":\"3\",\"type\":\"Button\",\"value\":\"Seleziona\",\"w\":\"88\",\"x\":\"688\",\"y\":\"35\",\"zone\":\"YWGFSYCMOZ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"19\",\"name\":\"tipodest\",\"orientation\":\"horizontal\",\"page\":\"1\",\"spuid\":\"WIRRRKLEWO\",\"tabindex\":\"4\",\"textlist\":\"Definitivo,Provvisorio\",\"type\":\"Radio\",\"valuelist\":\""+"'"+"D"+"'"+","+"'"+"P"+"'"+"\",\"w\":\"161\",\"x\":\"88\",\"y\":\"63\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"MacroAgente\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"idmage\",\"page\":\"1\",\"spuid\":\"XFVQTXPEXL\",\"tabindex\":\"5\",\"type\":\"Textbox\",\"w\":\"54\",\"x\":\"88\",\"y\":\"112\",\"zone\":\"YWGFSYCMOZ_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui elaborazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_XWPYHZIJDE()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"XWPYHZIJDE\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"12\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"688\",\"y\":\"105\",\"zone\":\"YWGFSYCMOZ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"19\",\"name\":\"destfin\",\"orientation\":\"horizontal\",\"page\":\"1\",\"spuid\":\"RQAECCWOBF\",\"tabindex\":\"14\",\"textlist\":\"Operazioni,Frazionate\",\"type\":\"Radio\",\"valuelist\":\""+"'"+"O"+"'"+","+"'"+"F"+"'"+"\",\"w\":\"161\",\"x\":\"88\",\"y\":\"82\",\"zone\":\"YWGFSYCMOZ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"483\",\"zone\":\"YWGFSYCMOZ_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"147\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"140\",\"zone\":\"YWGFSYCMOZ_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"198\",\"name\":\"Portlet_MVXXCZOQGP\",\"page\":\"1\",\"src\":\"\\/jsp\\/list_imprich_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____MVXXCZOQGP\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"287\",\"zone\":\"YWGFSYCMOZ_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Chiudi la maschera\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_JSKZJOHJOM()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"JSKZJOHJOM\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"22\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"105\",\"zone\":\"YWGFSYCMOZ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"MacroAgente_\",\"page\":\"1\",\"spuid\":\"YKEVRMKLNK_DIV\",\"tabindex\":\"28\",\"type\":\"Label\",\"value\":\"MacroAgente:\",\"w\":\"68\",\"x\":\"20\",\"y\":\"116\",\"zone\":\"YWGFSYCMOZ_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_xMADESCRI\",\"page\":\"1\",\"spuid\":\"KRFHEOVRDJ_DIV\",\"tabindex\":\"29\",\"type\":\"Label\",\"value\":\"w_xMADESCRI\",\"w\":\"408\",\"x\":\"168\",\"y\":\"116\",\"zone\":\"YWGFSYCMOZ_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Visualizza file creati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_JBNBDKBIAJ()\",\"name\":\"images_search_gif\",\"page\":\"1\",\"spuid\":\"JBNBDKBIAJ\",\"src\":\"images\\/search.gif\",\"tabindex\":\"31\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"640\",\"y\":\"105\",\"zone\":\"YWGFSYCMOZ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"name\":\"delprec\",\"page\":\"1\",\"spuid\":\"GSBFOKJJZN\",\"tabindex\":\"34\",\"type\":\"Checkbox\",\"w\":\"124\",\"x\":\"648\",\"y\":\"63\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_import_rtraBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_import_rtraBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_import_rtraBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_import_rtra?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_import_rtra_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(586,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
  ...
  void RaiseLoadedEntity(arpg_import_rtraBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_import_rtraBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_import_rtraBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_import_rtraBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_import_rtraBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_import_rtraBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_imp_dati_rtra.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_import_rtraBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_import_rtraBO BO) {
    boolean result = true;
    return result;
  }
}
