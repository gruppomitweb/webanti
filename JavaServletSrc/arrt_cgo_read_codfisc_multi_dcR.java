...
// * --- Fine Area Manuale
public class arrt_cgo_read_codfisc_multi_dcR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_read_codfisc_multi_dcR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_read_codfisc_multi_dc",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_cgo_read_codfisc_multi_dc";
    ...
    try {
      ...
      if (CPLib.eqr(pTipo,"I")) {
        ...
        while ( ! (Cursor_personbo.Eof())) {
          ...
          /* pObj.w_CONNESCLI := personbo->CONNES */
          pObj.w_CONNESCLI = Cursor_personbo.GetString("CONNES");
      ...
      /* If not(Empty(_lastope)) */
      if ( ! (CPLib.Empty(_lastope))) {
        ...
        Cursor_cgo_operazioni_storico = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"SNAINUMOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(_lastope,"?",0,0,m_cServer, m_oParameters),m_cServer,_lastope)+" "+")"+(m_Ctx.IsSharedTemp("cgo_operazioni_storico")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
        while ( ! (Cursor_cgo_operazioni_storico.Eof())) {
          /* If not(Empty(cgo_operazioni_storico->IDDOCALL)) */
          if ( ! (CPLib.Empty(Cursor_cgo_operazioni_storico.GetString("IDDOCALL")))) {
            ...
            while ( ! (Cursor_cgo_docope.Eof())) {
              /* If Empty(cgo_docope->DCDOCUNICO) */
              if (CPLib.Empty(Cursor_cgo_docope.GetString("DCDOCUNICO"))) {
                /* If cgo_operazioni_storico->CODFISC=pObj.w_CODFISC */
                if (CPLib.eqr(Cursor_cgo_operazioni_storico.GetString("CODFISC"),pObj.w_CODFISC)) {
                  /* cDoc := arfn_catchfile(cgo_operazioni_storico->IDDOCALL,cgo_docope->DCDOCNOM3,'X',pObj.w_CODDIPE) */
                  cDoc = arfn_catchfileR.Make(m_Ctx,this).Run(Cursor_cgo_operazioni_storico.GetString("IDDOCALL"),Cursor_cgo_docope.GetString("DCDOCNOM3"),"X",pObj.w_CODDIPE);
                  /* cDoc := cgo_docope->DCDOCNOM3 */
                  cDoc = Cursor_cgo_docope.GetString("DCDOCNOM3");
                  /* pObj.Set_DOC03(cDoc) */
                  pObj.Set_DOC03(cDoc);
                } // End If
                /* If cgo_operazioni_storico->TIPODOC=pObj.w_TIPODOC and cgo_operazioni_storico->NUMDOCUM=pObj.w_NUMDOCUM and pObj.w_DATAVALI >= pObj.w_DATAOPE */
                if (CPLib.eqr(Cursor_cgo_operazioni_storico.GetString("TIPODOC"),pObj.w_TIPODOC) && CPLib.eqr(Cursor_cgo_operazioni_storico.GetString("NUMDOCUM"),pObj.w_NUMDOCUM) && CPLib.ge(pObj.w_DATAVALI,pObj.w_DATAOPE)) {
                  /* cDoc := arfn_catchfile(cgo_operazioni_storico->IDDOCALL,cgo_docope->DCDOCNOM4,'X',pObj.w_CODDIPE) */
                  cDoc = arfn_catchfileR.Make(m_Ctx,this).Run(Cursor_cgo_operazioni_storico.GetString("IDDOCALL"),Cursor_cgo_docope.GetString("DCDOCNOM4"),"X",pObj.w_CODDIPE);
                  /* cDoc := cgo_docope->DCDOCNOM4 */
                  cDoc = Cursor_cgo_docope.GetString("DCDOCNOM4");
                  /* pObj.Set_DOC04(cDoc) */
                  pObj.Set_DOC04(cDoc);
                } // End If
              } // End If
  ...
  public String RunAsync(armt_cgo_oper_multi_dcBO p_pObj,String p_pTipo) {
    pObj = p_pObj;
    pTipo = p_pTipo;
  ...
  public Forward Run(armt_cgo_oper_multi_dcBO p_pObj,String p_pTipo) {
    pObj = p_pObj;
    pTipo = p_pTipo;
    return Run();
  }
  ...
  public static arrt_cgo_read_codfisc_multi_dcR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_read_codfisc_multi_dcR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pObj = null;
    pTipo = CPLib.Space(1);
