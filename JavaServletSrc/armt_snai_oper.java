...
public class armt_snai_oper extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*90afc6f5*!";
  ...
  public String getEntityTitle() {
    return "Operazioni da completare (CGO)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_snai_oper";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 1021;
    l_eg.m_nEntityHeight = 886;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"cgo_operazioni","cgo_tbtipgio","tbcitta","tbstati","tbtipatt","tbtipreg","tbcauana","personbo","intermbo","operazbo","tbcausin","cgo_causali","cgo_tipdoc","cgo_autorita","anadip"};
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","Dati Base","page"),new ZoneItem("page_2","Documenti","page"),new ZoneItem("page_3","Contatto","page"),new ZoneItem("page_4","Questionario ADV","page"),new ZoneItem("Base","Dati Base","horizontal_section"),new ZoneItem("Operazione","Dati Operazione","horizontal_section"),new ZoneItem("Soggetto","Dati Soggetto","horizontal_section"),new ZoneItem("Documenti","Documenti","horizontal_section"),new ZoneItem("Contatto","Contatto","horizontal_section")};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("CGOFLGVLT","C",1,0, "edit","Pagamento"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_snai_operBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11262256689",p_Context.Translate("Pagamento")},
    ...
    {"12116129702",p_Context.Translate("Operazioni da completare (CGO)")},
  ...
  void WriteStyles(armt_snai_operBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_snai_oper.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println("var tabs = new Z.Tabs({container:'tabsContainer',useseparator:GetStyleVariable('formTabUseSeparator', false, ''),expandcollapse:(typeof(sv_TabstripCanCollapse)!='undefined'?sv_TabstripCanCollapse:GetStyleVariable('formTabCanCollapse', false, '')),contents:[{name:\"page_1\", element:\"page_1\", caption:"+SPLib.ToJSValue(status.context.Translate("Dati Base"),"C",0,0)+"},{name:\"page_2\", element:\"page_2\", caption:"+SPLib.ToJSValue(status.context.Translate("Documenti"),"C",0,0)+"},{name:\"page_3\", element:\"page_3\", caption:"+SPLib.ToJSValue(status.context.Translate("Contatto"),"C",0,0)+"},{name:\"page_4\", element:\"page_4\", caption:"+SPLib.ToJSValue(status.context.Translate("Questionario ADV"),"C",0,0)+"}],tabstrip_class: ''});Z.Tabs.UseDeprecation();");
    status.out.println("Z.Tabs.Set('tabs',tabs);");
    status.out.println("} else {");
    status.out.println("var tabs = Z.Tabs.Get('tabs')");
    status.out.println("}");
    AddExtendedTabs(status);
    status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_snai_oper',false);");
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_snai_oper",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  // GetPortlet_Documenti_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KUBMCPCHPZ_Vars(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p1(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:886px;width:1021px;display:none'>" +
    "");
  ...
  void WriteControls_p2(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p3(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_3' class='PageContainer' style='position:relative;height:313px;width:1021px;display:none'>" +
    "");
  ...
  void WriteControls_p4(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_4' class='PageContainer' style='position:relative;height:812px;width:1021px;display:none'>" +
    "");
    WriteControls_p4_b0(BO,status);
    WriteExtendedCBoxChildren(BO.m_MNTs,status,4,"Questionario ADV");
  ...
  void WriteControls_p1_b0(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("Operazione")) {
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input id='OJFOJNAVBL' type='button' class='SPButton' value="+SPLib.ToHTMLValue(status.context.Translate("Verificare operazioni da associare"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"view") || CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" style='display:none'>" +
      "");
      status.out.println(
      ...
      "");
      status.out.println(
       "<span class='SPLabel' id='FSGHTGLEKR_DIV'>" +
       "<label id='FSGHTGLEKR' for='QNNLSEAZHF'>" +
      ...
      "");
      status.out.println(
       "<span class='SPLabel' id='DHXSDCQBBL_DIV'>" +
       "<label id='DHXSDCQBBL' for='LQVQUGYYJF'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("WALLET:"))+"" +
       "</label>" +
       "</span>" +
      "");
  ...
  void WriteControls_p1_b3_CloseBand(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b5(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<div id='AQSQVBKXAL' class='SectionBody' style='position:relative;height:507px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("Soggetto")) {
      ...
      if (status.HasZoneAltInterface("Soggetto@BEGIN") || status.HasZoneAltInterface("Soggetto@END")) {
        status.out.println(
         "<div id='AQSQVBKXAL_boxContent' style='position:relative;height:507px;display:block'>" +
        "");
      }
      CPResultSet l_rsLoad=null;
      VQRHolder l_VQRHolder=null;
  ...
  void WriteControls_p1_b5_1(armt_snai_operBO BO,ServletStatus status) {
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input name='PROFESSIONE' id='XFPPPEOSWP' type='text' class='"+status.ObligatoryClass("Field",BO.XFPPPEOSWP_isObligatory())+"' value="+SPLib.ToHTMLValue(BO.w_PROFESSIONE,"C",40,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='40' size='40' fieldtype='C' style='display:none'>" +
    "");
    status.out.println(
     "<span class='SPLabel' id='LWNOIDKLVA_DIV' style='display:none'>" +
     "<label id='LWNOIDKLVA' for='XFPPPEOSWP'>" +
  ...
  void WriteControls_p1_b5_CloseBand(armt_snai_operBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("Soggetto")) {
      ...
      status.out.println("Ctrl('AQSQVBKXAL').setAttribute('band_height',507);");
    }
  ...
  void WriteControls_p1_b6(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p2_b0(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p2_b1(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p2_b1_CloseBand(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p2_b2(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p3_b0(armt_snai_operBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_TOP_OF_PAGE_3")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_TOP_OF_PAGE_3' style='position:relative;height:8px;display:block' page='3'>" +
    "");
  ...
  void WriteControls_p3_b1(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
     "<div name='Contatto' id='NYSCBMMEEX_DIV' class='BoxContainer SPSection_expanded SectionContainerWithTitle'>" +
    "");
    if (CPLib.eq(status.GetBoxHeaderInclude(),"")) {
      status.out.println(
       "<div id='NYSCBMMEEX_HEADER' class='BoxHeader'>" +
      "");
      status.out.println(
       "<span id='NYSCBMMEEX_HEADER_CAPTION' class='BoxHeaderCaption'>" +
       ""+status.context.Translate("Contatto")+"" +
       "</span>" +
      "");
      status.out.println(
       "<img id='NYSCBMMEEX_HEADER_ICON' class='BoxHeaderIcon' src='"+com.zucchetti.sitepainter.Library.GetStyleVariable(status.m_cTheme, "boxHeaderCollapseImage", "../"+status.m_cTheme+"/formPage/box_collapse.gif")+"' onclick='ToggleCollapsibleBox(\"NYSCBMMEEX\")' alt='"+status.context.Translate("Expand/Collapse")+"' title='"+status.context.Translate("Expand/Collapse")+"' tabindex='"+status.m_nTabIndex+"'>" +
      "");
      status.out.println(
       "</div>" +
      "");
    } else {
      SPLib.Include(servletContext,status,status.GetBoxHeaderInclude()+"?ForcedPortletUID=NYSCBMMEEX_HEADER&m_cBoxId=NYSCBMMEEX&m_cInitialState=open&m_cTheme="+status.m_cTheme+"&m_nTabIndex="+status.m_nTabIndex+"&m_cCaption="+com.zucchetti.sitepainter.Library.URLencode(status.context.Translate("Contatto"))+"");
    }
    status.out.println(
     "<div id='NYSCBMMEEX' class='SectionBody' style='position:relative;height:280px;display:block'>" +
    "");
    status.includeZoneAltInterface("Contatto@BEGIN");
    if (status.includeZoneAltInterface("Contatto")) {
      WriteControls_p3_b1_CloseBand(BO,status);
    } else {
      if (status.HasZoneAltInterface("Contatto@BEGIN") || status.HasZoneAltInterface("Contatto@END")) {
        status.out.println(
         "<div id='NYSCBMMEEX_boxContent' style='position:relative;height:280px;display:block'>" +
        "");
      }
      CPResultSet l_rsLoad=null;
      VQRHolder l_VQRHolder=null;
  ...
  void WriteControls_p3_b1_CloseBand(armt_snai_operBO BO,ServletStatus status) {
    status.includeZoneAltInterface("Contatto@END");
    status.out.println(
     "</div>" +
    "");
    status.out.println(
    ...
    status.out.println(
     "<script>" +
    "");
    if (status.HasZoneAnyAltInterface("Contatto")) {
      status.out.println("Ctrl('NYSCBMMEEX').style.height='auto';");
    } else {
      status.out.println("Ctrl('NYSCBMMEEX').setAttribute('band_height',280);");
    }
    status.out.println("Ctrl('NYSCBMMEEX').setAttribute('status','open');");
    status.out.println(
     "</script>" +
    "");
  }
  void WriteControls_p3_b2(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteControls_p4_b0(armt_snai_operBO BO,ServletStatus status) {
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<iframe name='armt_cgo_qadv_operazioni' id='BZRUFRMVDO' frameborder='0' allowtransparency='true' toResize='no' tabindex='"+status.m_nTabIndex+"' src='javascript:[].join()'>" +
     "</iframe>" +
    "");
    status.out.println(
     "</div>" +
  ...
  static void PrintState(armt_snai_operBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_snai_operBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
    if (BO.armt_cgo_qadv_operazioni.IsUpdated()) {
      BO.m_cWv_armt_cgo_qadv_operazioni = BO.armt_cgo_qadv_operazioni.GetContext();
    }
  }
  static void PrintStateWorkVars(armt_snai_operBL BO,ServletStatus status) {
    ...
    if (CPLib.eq(BO.m_cWv_armt_cgo_qadv_operazioni,"_not_empty_") && status.m_bBrowserUpdate) {
      status.out.print(","+"m_cWv_armt_cgo_qadv_operazioni");
    } else if (CPLib.eq(BO.m_cWv_armt_cgo_qadv_operazioni,"_not_empty_")) {
      BO.m_cWv_armt_cgo_qadv_operazioni = BO.armt_cgo_qadv_operazioni.GetContext();
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_armt_cgo_qadv_operazioni,"C",0,0));
    } else {
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_armt_cgo_qadv_operazioni,"C",0,0));
    }
  ...
  void WriteMainFormVarScript(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(886,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_snai_oper"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_snai_oper","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_snai_oper",BO.GetCurrentState());
    ...
    if (CPLib.eq(status.GetZoneAltInterface("Contatto"),"")) {
      status.quoteAndAppend("Contatto","hide","box");
    }
    ...
    status.out.println("// * --- armt_snai_oper");
  ...
  void WriteMainFormScript(armt_snai_operBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('LQVQUGYYJF')) SetDisabled(c,true);");
        ...
        status.out.println("if(c=Ctrl('XFPPPEOSWP')) SetDisabled(c,true);");
    ...
    status.out.println("if (Eq(name,'armt_cgo_qadv_operazioni')) {");
    status.out.println("l_cRes='"+SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_cgo_qadv_operazioni")+"';");
    ...
    status.out.println("// * --- armt_snai_oper");
    status.out.println("function RecoverDL() {");
    status.out.println("  arrt_cgo_read_dl([['pObj',this]])");
    status.out.println("}");
    status.out.println("");
    ...
    status.out.println("  arrt_cgo_read_codfisc([['pObj',this],['pTipo','I']])");
    status.out.println("  Calculate()");
    status.out.println("}");
    status.out.println("");
    status.out.println("function DatiCN() {");
    status.out.println("  arrt_cgo_read_codfisc([['pObj',this],['pTipo','E']])");
    status.out.println("  Calculate()");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
  ...
  armt_snai_operBO getBO(ServletStatus status) {
    armt_snai_operBO BO = new armt_snai_operBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(armt_snai_operBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("cgo_operazioni",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_snai_operBO BO) {
  ...
  void Calculation_LAEGOZNRMQ(ServletStatus status,armt_snai_operBO BO) {
    BO.w_IDDOCALL = (CPLib.Empty(BO.w_IDDOCALL)?LibreriaMit.nextUID():BO.w_IDDOCALL);
  }
  void Calculation_OATZTIMJMD(ServletStatus status,armt_snai_operBO BO) {
  ...
  void Calculation_GCDVVJTTDD(ServletStatus status,armt_snai_operBO BO) {
    arrt_cgo_delete_docR l_oRoutine_arrt_cgo_delete_doc;
    l_oRoutine_arrt_cgo_delete_doc = arrt_cgo_delete_docR.Make(status.context,BO);
    l_oRoutine_arrt_cgo_delete_doc.pIDDOC = BO.w_IDDOCALL;
    l_oRoutine_arrt_cgo_delete_doc.pSaved = BO.w_saveddoc;
  ...
  void Calculation_BVTIXLYUGT(ServletStatus status,armt_snai_operBO BO) {
    BO.w_PEP = "N";
  }
  void WriteTableCombo(ServletStatus status,armt_snai_operBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
    l_cTmp = "[";
    if (l_rsLoad!=null)
  ...
  CPResultSet QueryKeyCursor(armt_snai_operBO BO,ServletStatus status) {
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_snai_operBL.QueryFilter(status.context);
  ...
  void Query(ServletStatus status,armt_snai_operBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
  ...
  void View(ServletStatus status,armt_snai_operBO BO) {
  ...
  void Edit(ServletStatus status,armt_snai_operBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_snai_operBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_snai_operBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_snai_operBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_snai_operBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_snai_operBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_snai_operBO BO,SPParameterSource source) {
    BO.Load(status.w_SNAINUMOPE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_snai_operBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_snai_operBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_snai_operBO BO) {
    ...
    item.put("pages_names","Dati Base,Documenti,Contatto,Questionario ADV");
    item.put("altInterfaceFor","armt_snai_oper");
    items.put(item);
    String s;
    ...
    items.put(ctrl);
    s = "{\"comment\":\"Data registrazione\",\"disabled\":\"true\",\"h\":20,\"maxlength\":10,\"name\":\"DATAREG\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"OEIKQOZMYJ\",\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":80,\"x\":920,\"y\":104,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_IKBMEWKQJW\",\"page\":1,\"spuid\":\"IKBMEWKQJW\",\"tabindex\":100,\"type\":\"Label\",\"value\":\"Data registrazione:\",\"w\":104,\"x\":816,\"y\":108,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_HVHVAYPDJE\",\"page\":1,\"spuid\":\"HVHVAYPDJE\",\"tabindex\":97,\"type\":\"Label\",\"value\":\"Data di pagamento:\",\"w\":117,\"x\":16,\"y\":112,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Totale Operazione\",\"disabled\":\"true\",\"h\":20,\"maxlength\":21,\"name\":\"TOTLIRE\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"HUAGBHPUNH\",\"type\":\"Textbox\",\"typevar\":\"numeric\",\"w\":120,\"x\":136,\"y\":195,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"check\",\"comment\":\"IMporto\",\"h\":20,\"maxlength\":21,\"name\":\"IMPWALLET\",\"page\":1,\"spuid\":\"BELMGUMGMW\",\"tabindex\":15,\"type\":\"Textbox\",\"typevar\":\"numeric\",\"w\":84,\"x\":584,\"y\":193,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Contanti (S\\/N)\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"FLAGCONT\",\"page\":1,\"spuid\":\"PURPSXEVAI\",\"tabindex\":79,\"textlist\":\"No,Si\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"0"+"'"+","+"'"+"1"+"'"+"\",\"w\":104,\"x\":136,\"y\":136,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Flag Wallet\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"FLAGWALLET\",\"page\":1,\"spuid\":\"DWJVPXTRND\",\"tabindex\":80,\"textlist\":\"No,Si\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"0"+"'"+","+"'"+"1"+"'"+"\",\"w\":104,\"x\":360,\"y\":136,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Tipo Operazione Completa\",\"disabled\":\"true\",\"h\":20,\"maxlength\":2,\"name\":\"TIPOOPRAP\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"UJRFWINIVQ\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":24,\"x\":608,\"y\":136,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_GVTXMEILAZ\",\"page\":1,\"spuid\":\"GVTXMEILAZ\",\"tabindex\":99,\"type\":\"Label\",\"value\":\"Contanti:\",\"w\":117,\"x\":16,\"y\":140,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_KMCNAWQHKV\",\"page\":1,\"spuid\":\"KMCNAWQHKV\",\"tabindex\":101,\"type\":\"Label\",\"value\":\"Tipo Operazione:\",\"w\":136,\"x\":472,\"y\":140,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Segno\",\"disabled\":\"true\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"SEGNO\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"QNNLSEAZHF\",\"textlist\":\"Avere,Dare\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"A"+"'"+","+"'"+"D"+"'"+"\",\"w\":88,\"x\":680,\"y\":164,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_THOGQCLBJA\",\"page\":1,\"spuid\":\"THOGQCLBJA\",\"tabindex\":105,\"type\":\"Label\",\"w\":361,\"x\":639,\"y\":140,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":25,\"name\":\"button_OJFOJNAVBL\",\"page\":1,\"picker\":true,\"spuid\":\"OJFOJNAVBL\",\"tabindex\":133,\"type\":\"Button\",\"value\":\"Verificare operazioni da associare\",\"w\":209,\"x\":784,\"y\":159,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\",\"zoom\":\"usrz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Tipo Registrazione\",\"disabled\":\"true\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"CAFLGOPE\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"YJDISDDJUY\",\"textlist\":\"Giocata,Versamento,Vincita,Prelievo\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"G"+"'"+","+"'"+"R"+"'"+","+"'"+"V"+"'"+","+"'"+"P"+"'"+"\",\"w\":128,\"x\":480,\"y\":164,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_FSGHTGLEKR\",\"page\":1,\"spuid\":\"FSGHTGLEKR\",\"tabindex\":193,\"type\":\"Label\",\"value\":\"Segno:\",\"w\":72,\"x\":608,\"y\":168,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_DWKJKOCJCQ\",\"page\":1,\"spuid\":\"DWKJKOCJCQ\",\"tabindex\":200,\"type\":\"Label\",\"value\":\"di cui Wallet:\",\"w\":94,\"x\":488,\"y\":197,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_AYZLWZINKY\",\"page\":1,\"spuid\":\"AYZLWZINKY\",\"tabindex\":201,\"type\":\"Label\",\"value\":\"Wallet:\",\"w\":80,\"x\":277,\"y\":140,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Causale\",\"disabled\":\"true\",\"h\":20,\"maxlength\":6,\"name\":\"CACODICE\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"WJQFIEBCQV\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"w\":312,\"x\":136,\"y\":164,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_YFGRJCQGZV\",\"page\":1,\"spuid\":\"YFGRJCQGZV\",\"tabindex\":122,\"type\":\"Label\",\"value\":\"Tipo Registrazione:\",\"w\":117,\"x\":16,\"y\":168,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"WALLET\",\"h\":20,\"maxlength\":20,\"name\":\"CODWALLET\",\"page\":1,\"spuid\":\"LQVQUGYYJF\",\"tabindex\":16,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":144,\"x\":136,\"y\":224,\"zone\":\"pag1_1_5\",\"zonepath\":\"[1,1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"
\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Citt\\u00E0 Dipendenza\",\"disabled\":\"true\",\"h\":21,\"maxlength\":30,\"name\":\"DESCCITDIP\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"RLUMBFHPUD\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":198,\"x\":656,\"y\":21,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_CTLYJMTHDV\",\"page\":1,\"spuid\":\"CTLYJMTHDV\",\"tabindex\":208,\"type\":\"Label\",\"value\":\"Citt\\u00E0:\",\"w\":47,\"x\":610,\"y\":25,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":17,\"name\":\"label_BLXOBUQVYE\",\"page\":1,\"spuid\":\"BLXOBUQVYE\",\"tabindex\":209,\"type\":\"Label\",\"w\":280,\"x\":317,\"y\":25,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Codice Dipendenza\",\"h\":21,\"maxlength\":6,\"name\":\"CODDIPE\",\"page\":1,\"spuid\":\"AVHTGZTOSZ\",\"tabindex\":210,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":48,\"x\":264,\"y\":21,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_XVRJYSKGWZ\",\"page\":1,\"spuid\":\"XVRJYSKGWZ\",\"tabindex\":211,\"type\":\"Label\",\"value\":\"Agenzia:\",\"w\":56,\"x\":208,\"y\":25,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_LOEAMXUYLA\",\"page\":1,\"spuid\":\"LOEAMXUYLA\",\"tabindex\":212,\"type\":\"Label\",\"value\":\"Intermediario:\",\"w\":110,\"x\":24,\"y\":25,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Tipo gioco\",\"disabled\":\"true\",\"h\":20,\"maxlength\":3,\"name\":\"TIPOGIOCO\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"QZTTRBWAQL\",\"type\":\"Combobox\",\"typevar\":\"character\",\"w\":208,\"x\":136,\"y\":72,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_JNQVUOSVPV\",\"page\":1,\"spuid\":\"JNQVUOSVPV\",\"tabindex\":130,\"type\":\"Label\",\"value\":\"Tipo Gioco:\",\"w\":120,\"x\":16,\"y\":76,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_DJRHGZHJSR\",\"page\":1,\"spuid\":\"DJRHGZHJSR\",\"tabindex\":145,\"type\":\"Label\",\"value\":\"Tipo Piattaforma:\",\"w\":112,\"x\":392,\"y\":76,\"zone\":\"pag1_3_3\",\"zonepath\":\"[1,3,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Piattaforma VLT\",\"disabled\":\"true\",\"h\":20,\"maxlength\":2,\"name\":\"VLTPLAT\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"LVYFMQXNZH\",\"type\":\"Combobox\",\"typevar\":\"character\",\"w\":280,\"x\":504,\"y\":72,\"zone\":\"pag1_3_3\",\"zonepath\":\"[1,3,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Contatto\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Contatto\",\"h\":280,\"name\":\"Contatto\",\"page\":3,\"shrinkable\":true,\"spuid\":\"NYSCBMMEEX\",\"stretch\":true,\"tabindex\":1,\"title_caption\":\"Contatto\",\"titled\":true,\"type\":\"Box\",\"w\":784,\"x\":8,\"y\":8,\"zone\":\"pag3_1\",\"zonepath\":\"[3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":799,\"name\":\"armt_cgo_qadv_operazioni\",\"page\":4,\"spuid\":\"BZRUFRMVDO\",\"tabindex\":1,\"type\":\"Iframe\",\"w\":1005,\"x\":8,\"y\":8,\"zone\":\"pag4_1\",\"zonepath\":\"[4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_snai_operBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_snai_oper?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_snai_oper_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_snai_oper_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_FLAGFRAZ=a[58];");
      status.out.println("w_COLLEG=a[59];");
      status.out.println("w_CACODICE=a[60];");
      status.out.println("w_CAIDBIGLIETTO=a[61];");
      status.out.println("w_CGOTIPOPE=a[62];");
      status.out.println("w_NUMOPEDL=a[63];");
      status.out.println("w_CAFLGOPE=a[64];");
      status.out.println("w_CGODATCON=a[65];");
      status.out.println("w_CGOESICON=a[66];");
      status.out.println("w_CGONOTCON=a[67];");
      ...
      status.out.println("w_PROVINCIADIP=a[88];");
      status.out.println("w_DESCCITDIP=a[89];");
      status.out.println("w_CODDIPE=a[90];");
      status.out.println("w_gIntemediario=a[91];");
      status.out.println("w_xSOGTPGIO=a[92];");
      status.out.println("w_xDEFTPGIO=a[93];");
      status.out.println("w_xEDTTPGIO=a[94];");
      status.out.println("w_xEDTSPGIO=a[95];");
      status.out.println("w_xDEFSPGIO=a[96];");
      status.out.println("w_FLGPAGAM=a[97];");
      status.out.println("w_FILTROMP=a[98];");
      status.out.println("w_idcitta=a[99];");
      status.out.println("w_xARDESCRI=a[100];");
      status.out.println("w_dessta=a[101];");
      status.out.println("w_tipsot=a[102];");
      status.out.println("w_STATOREG=a[103];");
      status.out.println("w_IDBASE=a[104];");
      status.out.println("w_NUMPROG=a[105];");
      status.out.println("w_DESCRI=a[106];");
      status.out.println("w_gSeekAos=a[107];");
      status.out.println("w_gDataVaria=a[108];");
      status.out.println("w_gFlgDoc=a[109];");
      status.out.println("w_xDescDipe=a[110];");
      status.out.println("w_datpaseuro=a[111];");
      status.out.println("w_xdes1=a[112];");
      status.out.println("w_desccauana=a[113];");
      status.out.println("w_CITINT=a[114];");
      status.out.println("w_CITDIP=a[115];");
      status.out.println("w_PROVDIP=a[116];");
      status.out.println("w_CABDIP=a[117];");
      status.out.println("w_PROVINT=a[118];");
      status.out.println("w_CABINT=a[119];");
      status.out.println("w_xdesccausin=a[120];");
      status.out.println("w_gChkDate=a[121];");
      status.out.println("w_xdesctipatt=a[122];");
      status.out.println("w_gVLT=a[123];");
      status.out.println("w_gVNP=a[124];");
      status.out.println("w_gCodDip=a[125];");
      status.out.println("w_verifica=a[126];");
      status.out.println("w_gStpDwn=a[127];");
      ...
      status.out.println("w_TELDIP=a[133];");
      status.out.println("w_CODLOC=a[134];");
      status.out.println("w_xFLGPLGIO=a[135];");
      status.out.println("w_xVPFLGATT=a[136];");
      status.out.println("w_DOC03=a[137];");
      status.out.println("w_DOC04=a[138];");
      status.out.println("w_tipop=a[139];");
      status.out.println("w_gGesDoc=a[140];");
      status.out.println("w_edtdoc=a[141];");
      status.out.println("w_saveddoc=a[142];");
      status.out.println("w_idcitta2=a[143];");
      status.out.println("w_dessta2=a[144];");
      status.out.println("w_FLGQADVFULL=a[145];");
      status.out.println("w_gQADV=a[146];");
      status.out.println("w_xCGOFLGPROF=a[147];");
      status.out.println("w_ANA_FOUND=a[148];");
      status.out.println("m_cWv_armt_cgo_qadv_operazioni=a[149];;");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_snai_oper_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(886,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void RaiseLoadedEntity(armt_snai_operBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_snai_operBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_snai_operBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_snai_operBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_snai_operBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_snai_operBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_snai_operBO BO,ServletStatus status) throws IOException {
  ...
  void IncludeJS(ServletStatus status) {
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+arrt_cgo_setpagatore_c.m_cEntityUid+"/arrt_cgo_setpagatore_c.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      arrt_cgo_setpagatore_c.GetCallableNames(callable);
      ...
      CPLib.AddToSet(callable,"armt_cgo_qadv_operazioni");
    } else {
  ...
  public String GetCheckVars(ServletStatus status,armt_snai_operBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_snai_operBO BO) {
    boolean result = true;
