...
public class arrt_storoper extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*28eea70c*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_tipoope;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_tipoope = source.Has("w_tipoope");
      if (m_bpassedw_tipoope) {
        w_tipoope = source.GetParameter("w_tipoope",source.GetParameter("tipoope",""));
      } else {
        if (source.IsForwarded()) {
          w_tipoope = source.GetCaller().GetString("tipoope","C",1,0);
          m_bpassedw_tipoope = true;
        } else {
          w_tipoope = source.GetParameter("tipoope",source.GetParameter("w_tipoope",""));
        }
      }
    }
  }
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_tipoope) ||  ! (status.m_bpassedw_ADatOpe) ||  ! (status.m_bpassedw_DaDatOpe);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_tipoope)) {
      status.out.println("l_cV=WtA(l_oWnd('w_tipoope',"+SPLib.ToJSValue(status.w_tipoope,"C",1,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_tipoope,"C",1,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_tipoope',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_storoperR Rcalled;
    Rcalled = new arrt_storoperR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_storoperR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_storoperR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_storoper"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_tipoope="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_tipoope),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_tipoope,null)) {");
        status.out.println("window[f].w_tipoope="+SPLib.ToJSValue(status.w_tipoope,"C",0,0)+";");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_storoper");
      if ( ! (CPLib.IsNull(l))) {
