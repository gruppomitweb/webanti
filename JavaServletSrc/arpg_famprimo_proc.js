...
var c;
var c;
if(c=Ctrl('HXKWFJUKYF')) selectRadio(c,w_flgnuovi,'C')
...
SetLocationHref(null,'arrt_famprimo'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
}
...
return Set_flgnuovi(HtW(getRadioValue(Ctrl('HXKWFJUKYF')),'C'),null,e);
}
...
function FQLSATGBOG_Click() {
if (false || WindowConfirm(Translate('822914937'))) {
SetLocationHref(null,'arrt_casprimo'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
}
_ResetTracker();
...
c=Ctrl('HXKWFJUKYF');
if ( ! (Empty(c))) {
c[0].onclick=HXKWFJUKYF_Valid;
c[1].onclick=HXKWFJUKYF_Valid;
}
...
w_Messaggi.raiseEvent('arpg_famprimo_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_famprimo_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_famprimo_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_famprimo',m_cSelectedPage);
...
function Set_flgnuovi(ctrlValue,kmode,e) {
if (Ne(w_flgnuovi,ctrlValue)) {
var ctl = _GetCtl(e,'HXKWFJUKYF');
if (_tracker.goon(ctl,ctrlValue)) {
w_flgnuovi=ctrlValue;
SetModified(false);
var l_bResult = true;
...
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('HXKWFJUKYF',1))) {
_SignErr('HXKWFJUKYF');
m_cLastWorkVarErrorMsg='flgnuovi';
l_bResult=false;
w_flgnuovi=HtW('','C');
...
l_oWv.setValue('flgnuovi',WtA(w_flgnuovi,'C'));
...
_FillChildren.n=[];
FillWv.n=["gIntemediario","gDescAzi","flgnuovi","anno","mese","flgstp"];
...
m_bCalculating=true;
if (start && Eq(At(',flgnuovi,',pseq),0)) {
w_flgnuovi='N';
}
w_anno=2015;
...
_Obli('flgnuovi',0,'HXKWFJUKYF',false)
