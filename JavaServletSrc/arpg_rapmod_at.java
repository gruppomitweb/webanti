...
public class arpg_rapmod_at extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*RHYWCTEYZO*!";
  ...
  public String getEntityTitle() {
    return "Crea Modifiche Per Anagrafe Tributaria";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_rapmod_at";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 805;
    l_eg.m_nEntityHeight = 115;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    ...
    l_eg.m_cQueryName = "qbe_rapp_mod_at";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_rapmod_atBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10013744089",p_Context.Translate("Crea Modifiche Per Anagrafe Tributaria")},
    {"10198810383",p_Context.Translate("Esecuzione trasferimento dati da provvisorie a definitive")},
    {"10345646977",p_Context.Translate("Nuova o modificata")},
    {"1125427419",p_Context.Translate("Deseleziona tutti i rapporti")},
    {"113933141",p_Context.Translate("Update")},
    {"1311240643",p_Context.Translate("Visualizza rapporti trovati")},
    {"1564677233",p_Context.Translate("Seleziona tutti i rapporti")},
    {"1805022818",p_Context.Translate("Rapporto")},
    {"340732011",p_Context.Translate("Crea la modifica per l'anagrafe tributaria")},
    {"727419286",p_Context.Translate("Conferma i record selezionati")}};
    return l_translations;
  }
  void WriteStyles(arpg_rapmod_atBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_rapmod_at.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_rapmod_atBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_rapmod_atBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_rapmod_atBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(45);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_rapmod_atBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<img id='VRHQBWKFPP' src="+SPLib.ToHTMLValue(SPLib.ToResource("images/search.gif"),"C",0,0)+" title="+SPLib.ToHTMLValue(status.context.Translate("Visualizza rapporti trovati"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' alt="+SPLib.ToHTMLValue(status.context.Translate("Visualizza rapporti trovati"),"C",0,0)+">" +
     "</a>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
  ...
  static void PrintState(arpg_rapmod_atBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_rapmod_atBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_rapmod_atBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_rapmod_atBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(115,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_rapmod_at"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_rapmod_at","");
  ...
  void WriteMainFormScript(arpg_rapmod_atBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_rapmod_atBO getBO(ServletStatus status) {
    arpg_rapmod_atBO BO = new arpg_rapmod_atBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_rapmod_atBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_rapmod_atBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_rapmod_atBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_rapmod_atBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
  ...
  void Discard(ServletStatus status,arpg_rapmod_atBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_rapmod_atBO BO) {
    ...
    item.put("altInterfaceFor","arpg_rapmod_at");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"45\",\"name\":\"Box_XLVFHSOSYH\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"XLVFHSOSYH\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"800\",\"x\":\"0\",\"y\":\"0\",\"zone\":\"pag1_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esecuzione trasferimento dati da provvisorie a definitive\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_FLFHSXPWZI()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"FLFHSXPWZI\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"4\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"704\",\"y\":\"9\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Visualizza rapporti trovati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_VRHQBWKFPP()\",\"name\":\"images_search_gif\",\"page\":\"1\",\"spuid\":\"VRHQBWKFPP\",\"src\":\"images\\/search.gif\",\"tabindex\":\"5\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"656\",\"y\":\"9\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Conferma i record selezionati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_HDSXZTLKVK()\",\"name\":\"images_save_gif\",\"page\":\"1\",\"spuid\":\"HDSXZTLKVK\",\"src\":\"images\\/save.gif\",\"tabindex\":\"6\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"608\",\"y\":\"9\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Deseleziona tutti i rapporti\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_ZWTNEMXOFQ()\",\"name\":\"images_dese_all_gif\",\"page\":\"1\",\"spuid\":\"ZWTNEMXOFQ\",\"src\":\"images\\/dese_all.gif\",\"tabindex\":\"7\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"560\",\"y\":\"9\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Seleziona tutti i rapporti\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_FCATLNLMSB()\",\"name\":\"images_sel_all_gif\",\"page\":\"1\",\"spuid\":\"FCATLNLMSB\",\"src\":\"images\\/sel_all.gif\",\"tabindex\":\"8\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"512\",\"y\":\"9\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_PTXYMSWBZO()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"PTXYMSWBZO\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"10\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"752\",\"y\":\"9\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void WriteBody(ServletStatus status,arpg_rapmod_atBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_rapmod_atBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_rapmod_atBO BO) {
    String l_cTrsQ;
    ...
    l_cTrsF = l_cTrsF+SPLib.ToJSValue(BO.w_TIPO,"C",1,0);
    l_cTrsF = l_cTrsF+","+SPLib.ToJSValue(BO.w_TIPO,"C",1,0);
    l_cTrsF = l_cTrsF+","+SPLib.ToJSValue(BO.c_RAPPORTO,"C",25,0);
    l_cTrsF = l_cTrsF+",0)";
    status.out.println("m_oTrsFW["+p_nCnt+"] = "+l_cTrsF+"");
  }
  void Next(ServletStatus status,arpg_rapmod_atBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_rapmod_atBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_rapmod_atBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_rapmod_at?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_rapmod_at_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(115,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void RaiseLoadedEntity(arpg_rapmod_atBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_rapmod_atBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_rapmod_atBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_rapmod_atBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_rapmod_atBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_rapmod_atBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  void IncludeJS(ServletStatus status) {
    String l_cJsUid;
    status.out.println(
     "<script src='SPPrxy/"+arrt_rapmod_at.m_cEntityUid+"/arrt_rapmod_at.js'>" +
     "</script>" +
    "");
    status.out.println(
     "<script src='SPPrxy/"+arrt_seldes_rapmod_at.m_cEntityUid+"/arrt_seldes_rapmod_at.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_rapmod_at.GetCallableNames(callable);
      arrt_seldes_rapmod_at.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_rapmod_atBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_rapmod_atBO BO) {
    boolean result = true;
    return result;
  }
}
