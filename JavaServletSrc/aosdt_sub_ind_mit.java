...
public class aosdt_sub_ind_mit extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*c5d2e23f*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "sub_ind_mit";
    }
    public String w_IDMOD;
    public String w_IDIND;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_IDMOD,context,"sub_ind_mit","IDMOD","C",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_IDIND,context,"sub_ind_mit","IDIND","C",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    aosdt_sub_ind_mitBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_sub_ind_mit;
  ...
  public String getEntityTitle() {
    return "Sub Indicatori anomalia mit";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "aosdt_sub_ind_mit";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 1256;
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"sub_ind_mit"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,aosdt_sub_ind_mitBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11707617423",p_Context.Translate("Sub Indicatori anomalia mit")},
    {"1305674908",p_Context.Translate("ID del indicatore")},
    {"1497964518",p_Context.Translate("Descrizione del subindicatore %1")},
  ...
  void WriteStyles(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../aosdt_sub_ind_mit.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(
       "<style>" +
      "");
      status.out.println(".BBCHCNQIRI {top:0px;position:absolute;left:0px;height:18px;width:138px;background-color:Transparent;z-index:1; }");
      status.out.println(".HLHLHCXGMT {top:0px;position:absolute;left:141px;height:18px;width:202px;background-color:Transparent;z-index:1; }");
      status.out.println(".KWRCVYJSPV {top:0px;position:absolute;left:346px;color:#000000;font-family:icons8_win10;font-size:14px;cursor:pointer;border-style:none;z-index:3;}");
      status.out.println(".BBEEHSSDFM {top:0px;position:absolute;left:367px;height:18px;width:868px;overflow:auto;background-color:Transparent;z-index:1; }");
      status.out.println(
       "</style>" +
      "");
    } else {
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('aosdt_sub_ind_mit',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"aosdt_sub_ind_mit",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:62px;width:1256px;display:none'>" +
    "");
  ...
  void WriteControls_p1_l0(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b0(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    ...
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      status.out.println(
       "<div name id='GridTable_Header' class='GridTable_Header' style='position:absolute;top:0px;left:-1px;height:20px;width:1257px;z-index:0'>" +
       "</div>" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='IDMOD' id='LNSBOLUCRM' type='text' class='Field PrimaryKeyField "+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+"' value="+SPLib.ToHTMLValue(BO.w_IDMOD,"C",10,0,"", status.m_oRegSettings)+" title="+SPLib.ToHTMLValue(status.context.Translate("ID del modello"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query")?""+status.m_nTabIndex+"":"-1")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"readonly":"")+" maxlength='10' size='10' fieldtype='C' style='display:none'>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle PrimaryKeyHeaderLabel' id='STIWBITFVZ_DIV'>" +
       "<label id='STIWBITFVZ' for='BBCHCNQIRI'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("ID del subindicatore"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='SCYMKSACPR_DIV'>" +
       "<label id='SCYMKSACPR' for='HLHLHCXGMT'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Progressivo subindicatore"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='DGTNNOYUNI_DIV'>" +
       "<label id='DGTNNOYUNI' for='BBEEHSSDFM'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Descrizione del subindicatore (Cliccare sulla matita per aprire il dettaglio)"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
  ...
  void WriteControls_p1_b1(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("GridTable")) {
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='IDIND' id='ILPNZWVGRZ' type='text' class='Field PrimaryKeyField "+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+"' value="+SPLib.ToHTMLValue(BO.w_IDIND,"C",10,0,"", status.m_oRegSettings)+" title="+SPLib.ToHTMLValue(status.context.Translate("ID del indicatore"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"readonly":"")+" maxlength='10' size='10' fieldtype='C' style='display:none'>" +
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:relative;top:0px;left:-1px;min-height:75px;max-height:156px;width:1257px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:81px;width:1257px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:1257px;min-height:75px;max-height:156px;overflow:auto;'>" +
       "<table id='GridTable' style='position:relative;top:0px;left:0px;width:1251px' cellspacing='0' cellpadding='0' border='0'>" +
       "<tr>" +
       "<td></td>" +
       "</tr>" +
       "</table>" +
      ...
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:140px;height:100%;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:345px;height:100%;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:367px;height:100%;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
      if (status.HasZoneAltInterface("GridTable@BEGIN") || status.HasZoneAltInterface("GridTable@END")) {
  ...
  void WriteControls_p1_b1_CloseBand(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "</div>" +
    "");
    status.out.println(
  ...
  void HTMLRowScript(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    ...
    status.out.println("s='<div style=\"position:relative;width:1235px;height:14px;\"';");
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     "<textarea name='DESCRI' id='BBEEHSSDFM' class='Memo GridTable_EditRowItem ' title='\"+Strtran(AlertErrorMessage('1497964518',w_DESCRI),'\\\\n','&#10;')+\"' tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" cols='20' rows='2'>" +
     "</textarea>" +
    "\"");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    status.out.println("s=s+\""+
     "<input type='text' class='Field PrimaryKeyField "+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+" Disabled GridTable_QueryRowItem BBCHCNQIRI' value='\"+WtH(ToHTML(w_IDSUBIND),'C',10,0,'')+\"' tabindex='-1' readonly size='10'>" +
    "\"");
    status.out.println("}");
    status.out.println("if ( ! (IsHiddenByStateDriver('PROGSUBIND'))) {");
    status.out.println("s=s+\""+
     "<input type='text' class='Field Disabled GridTable_QueryRowItem HLHLHCXGMT' value='\"+WtH(ToHTML(w_PROGSUBIND),'C',10,0,'')+\"' tabindex='-1' readonly size='10'>" +
    "\"");
    status.out.println("}");
    status.out.println("s=s+\""+
    ...
    status.out.println("s=s+\""+
     "<textarea class='GridTable_QueryRowItem BBEEHSSDFM' disabled cols='20' rows='2'>" +
     "\"+WtH(w_DESCRI,'M',10,0,'"+""+"')+\"" +
     "</textarea>" +
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:1235px;height:14px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b2(aosdt_sub_ind_mitBO BO,ServletStatus status) {
  ...
  static void PrintState(aosdt_sub_ind_mitBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(aosdt_sub_ind_mitBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(aosdt_sub_ind_mitBL BO,ServletStatus status) {
  ...
  static void PrintStateDetailWorkVars(aosdt_sub_ind_mitBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(aosdt_sub_ind_mitBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      aosdt_sub_ind_mitBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (aosdt_sub_ind_mitBO.TrsRow) BO.m_oDeletedRows.get(k);
  ...
  void WriteMainFormVarScript(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(1256,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"aosdt_sub_ind_mit"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("aosdt_sub_ind_mit","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"aosdt_sub_ind_mit",BO.GetCurrentState());
    ...
    status.out.println("this.opk_IDSUBIND=arguments[4];");
  ...
  void WriteMainFormScript(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_aosdt_sub_ind_mit);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_aosdt_sub_ind_mit();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('aosdt_sub_ind_mit',Ne(m_nChildStatus,0));");
    } else {
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('LNSBOLUCRM')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('ILPNZWVGRZ')) SetDisabled(c,true);");
        ...
        status.out.println("var c;");
        status.out.println("if(c=Ctrl('BBCHCNQIRI')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('HLHLHCXGMT')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('BBEEHSSDFM')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('ATCXZHNKXY')) SetDisabled(c,true);");
      }
    ...
    if (CPLib.eq(status.m_cClientMode,"query") && CPLib.gt(status.m_nChildStatus,0)) {
      ...
      status.out.println("SetDisabled('LNSBOLUCRM',m_bRowsChild);");
      status.out.println("SetDisabled('ILPNZWVGRZ',m_bRowsChild);");
    }
    status.out.println("}");
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
    ...
    status.out.println("m_oFather.m_oaosdt_sub_ind_mit=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  aosdt_sub_ind_mitBO getBO(ServletStatus status) {
    aosdt_sub_ind_mitBO BO = new aosdt_sub_ind_mitBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(aosdt_sub_ind_mitBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("sub_ind_mit",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,aosdt_sub_ind_mitBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,aosdt_sub_ind_mitBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_sub_ind_mit;
    String l_cPhName = BO.m_cPhName_sub_ind_mit;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = aosdt_sub_ind_mitBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_sub_ind_mit+".IDMOD, "+BO.m_cVirtName_sub_ind_mit+".IDIND"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_sub_ind_mit+".IDMOD, "+BO.m_cVirtName_sub_ind_mit+".IDIND"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("sub_ind_mit");
    String l_cPhName = CPSql.ManipulateTablePhName("sub_ind_mit",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,aosdt_sub_ind_mitBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_sub_ind_mit),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_sub_ind_mit,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_sub_ind_mit+".IDMOD, "+BO.m_cVirtName_sub_ind_mit+".IDIND) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_sub_ind_mit,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_sub_ind_mit,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_sub_ind_mit+".IDMOD, "+BO.m_cVirtName_sub_ind_mit+".IDIND",CPLib.GetTablesWithAliases(BO.m_cPhName_sub_ind_mit,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,aosdt_sub_ind_mitBO BO) {
  ...
  void Edit(ServletStatus status,aosdt_sub_ind_mitBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,aosdt_sub_ind_mitBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,aosdt_sub_ind_mitBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,aosdt_sub_ind_mitBO BO) {
  ...
  void EditLoad(ServletStatus status,aosdt_sub_ind_mitBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,aosdt_sub_ind_mitBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,aosdt_sub_ind_mitBO BO,SPParameterSource source) {
    BO.Load(status.w_IDMOD,status.w_IDIND);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,aosdt_sub_ind_mitBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,aosdt_sub_ind_mitBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,aosdt_sub_ind_mitBO BO) {
    ...
    item.put("altInterfaceFor","aosdt_sub_ind_mit");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"h\":18,\"name\":\"label_SCYMKSACPR\",\"page\":1,\"spuid\":\"SCYMKSACPR\",\"tabindex\":4,\"type\":\"Label\",\"value\":\"Progressivo subindicatore\",\"w\":197,\"x\":142,\"y\":1,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":18,\"name\":\"label_STIWBITFVZ\",\"page\":1,\"spuid\":\"STIWBITFVZ\",\"tabindex\":3,\"type\":\"Label\",\"value\":\"ID del subindicatore\",\"w\":138,\"x\":1,\"y\":1,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":18,\"name\":\"label_DGTNNOYUNI\",\"page\":1,\"spuid\":\"DGTNNOYUNI\",\"tabindex\":5,\"type\":\"Label\",\"value\":\"Descrizione del subindicatore (Cliccare sulla matita per aprire il dettaglio)\",\"w\":889,\"x\":347,\"y\":1,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"autoexpand\":true,\"h\":77,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"GridTable_Band\",\"type\":\"BodyDetail\",\"w\":1291,\"x\":1,\"y\":21,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Descrizione\",\"box_type\":\"layer\",\"comment\":\"Descrizione del subindicatore\",\"fixed\":true,\"h\":77,\"name\":\"Descrizione\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VGASZRXNOJ\",\"tabindex\":5,\"title_caption\":\"Descrizione del subindicatore\",\"type\":\"Box\",\"w\":1236,\"x\":0,\"y\":101,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Descrizione\",\"container\":\"VGASZRXNOJ\",\"fixed\":true,\"h\":71,\"name\":\"DESCRI_layer\",\"page\":1,\"repeated\":true,\"scroll\":true,\"spuid\":\"ATCXZHNKXY\",\"tabindex\":6,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":1232,\"x\":1,\"y\":104,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":42,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"GridTable_Band\",\"type\":\"Box\",\"w\":1235,\"x\":1,\"y\":0,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Descrizione del subindicatore\",\"h\":19,\"name\":\"DESCRI\",\"page\":1,\"repeated\":true,\"scroll\":true,\"spuid\":\"BBEEHSSDFM\",\"tabindex\":4,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":868,\"x\":368,\"y\":23,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":19,\"name\":\"button_KWRCVYJSPV\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"KWRCVYJSPV\",\"src\":\"..\\/{\\\"Char\\\":\\\"59899\\\",\\\"Color\\\":\\\"#000000\\\",\\\"FontName\\\":\\\"icons8_win10\\\",\\\"FontWeight\\\":\\\"\\\",\\\"Size\\\":\\\"14\\\"}\",\"tabindex\":3,\"type\":\"Image\",\"value\":\"Apri Descrizione\",\"w\":19,\"x\":347,\"y\":23,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\",\"zoom\":\"usrz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"ID del subindicatore\",\"h\":19,\"maxlength\":10,\"name\":\"IDSUBIND\",\"page\":1,\"repeated\":true,\"spuid\":\"BBCHCNQIRI\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":138,\"x\":1,\"y\":23,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Progressivo subindicatore\",\"h\":19,\"maxlength\":10,\"name\":\"PROGSUBIND\",\"page\":1,\"repeated\":true,\"spuid\":\"HLHLHCXGMT\",\"tabindex\":2,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":202,\"x\":142,\"y\":23,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,aosdt_sub_ind_mitBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='aosdt_sub_ind_mit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/aosdt_sub_ind_mit_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/aosdt_sub_ind_mit_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_DESCRI_layer=a[5];");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/aosdt_sub_ind_mit_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","sub_ind_mit");
  ...
  void RaiseLoadedEntity(aosdt_sub_ind_mitBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(aosdt_sub_ind_mitBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(aosdt_sub_ind_mitBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(aosdt_sub_ind_mitBO BO,ServletStatus status) {
  ...
  void WriteExitPage(aosdt_sub_ind_mitBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(aosdt_sub_ind_mitBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(aosdt_sub_ind_mitBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,aosdt_sub_ind_mitBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,aosdt_sub_ind_mitBO BO) {
    boolean result = true;
  ...
  static class OrderBodyByComparator implements Comparator<aosdt_sub_ind_mitBL.TrsRow> {
    public int compare(aosdt_sub_ind_mitBL.TrsRow o1,aosdt_sub_ind_mitBL.TrsRow o2) {
