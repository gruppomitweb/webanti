...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('CSRZTJZJBA').value,'C'),w_WUTDCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('wutipdoc',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'WUTDCODICE') &&  ! (Empty(HtW(Ctrl('CSRZTJZJBA').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.WUTDCODICE',HtW(Ctrl('CSRZTJZJBA').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_wutipdoc');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wutipdoc',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['CSRZTJZJBA'],i=0;i>=0;i--){
