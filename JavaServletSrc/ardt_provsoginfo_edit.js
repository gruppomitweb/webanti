...
if (IsDisabledByStateDriver('box_JUPHOBFSPW')) {
DisableInputsInContainer(Ctrl('JUPHOBFSPW'),true);
}
...
function Set_RAPCOL(ctrlValue,kmode,e) {
if (Ne(w_RAPCOL,ctrlValue)) {
var ctl = _GetCtl(e,'IHXPLCYXNH');
if (_tracker.goon(ctl,ctrlValue)) {
w_RAPCOL=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('IHXPLCYXNH',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
var ctl = _GetCtl(e,'FVKNSPNHTX');
if (_tracker.goon(ctl,ctrlValue)) {
w_CODSOG=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('FVKNSPNHTX',0);
if (l_bResult) {
l_bResult=Link_FVKNSPNHTX(kmode);
if ( ! (l_bResult)) {
w_CODSOG=HtW('','C');
}
}
...
var ctl = _GetCtl(e,'KQROMFNGNZ');
if (_tracker.goon(ctl,ctrlValue)) {
w_LEGAME=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('KQROMFNGNZ',0);
if (l_bResult) {
l_bResult=Link_KQROMFNGNZ(kmode);
if ( ! (l_bResult)) {
w_LEGAME=HtW('','C');
}
}
...
function Link_FVKNSPNHTX(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
...
if (Eq(l_nRec,2)) {
var openername = 'linkview_FVKNSPNHTX';
...
function Link_FVKNSPNHTX_Blank() {
w_CODSOG='';
w_xragsoc='';
w_DOMICILIO='';
w_DESCCIT='';
...
function LOpt_FVKNSPNHTX() {
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('personbo','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
l_oWv.setValue('PKFields',WtA('CONNES','C'));
l_oWv.setValue('LinkedField',WtA('CONNES','C'));
l_oWv.setValue('UID',WtA('FVKNSPNHTX','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['FVKNSPNHTX'][1],'C'));
l_oWv.setValue('Caption',WtA(Translate('1729299794'),'C'));
return l_oWv;
}
function Link_KQROMFNGNZ(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
...
l_Appl.SetID('KQROMFNGNZ');
l_nRec=l_Appl.DoLink((Eq(kmode,null)?2:kmode));
l_bResult=Ne(l_nRec,0);
if (Eq(l_nRec,1)) {
w_LEGAME=l_Appl.GetStringValue('TIPLEG',1,0);
w_xdescleg=l_Appl.GetStringValue('DESCRIZ',50,0);
l_Appl.GetLinkHash(GetRepeatedCheckVars());
} else {
Link_KQROMFNGNZ_Blank();
}
if (Eq(l_nRec,2)) {
var openername = 'linkview_KQROMFNGNZ';
var l_cFieldFilter = l_Appl.GetWhereFieldExprUnencoded();
var l_cFixedFilter = l_Appl.GetWhereFixedExprUnencoded();
var l_cAndOpForField = '';
if ( ! (Empty(l_cFieldFilter))) {
...
function Link_KQROMFNGNZ_Blank() {
w_LEGAME='';
w_xdescleg='';
}
function LOpt_KQROMFNGNZ() {
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('tbtipleg','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
l_oWv.setValue('PKFields',WtA('TIPLEG','C'));
l_oWv.setValue('LinkedField',WtA('TIPLEG','C'));
l_oWv.setValue('UID',WtA('KQROMFNGNZ','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['KQROMFNGNZ'][1],'C'));
l_oWv.setValue('Caption',WtA(Translate('910117298'),'C'));
return l_oWv;
}
function ReportLinkValue(varName,value) {
var bResult = false;
if (Eq(varName,'FVKNSPNHTX')) {
last_focused_comp=GetLastFocusedElementOfLink('FVKNSPNHTX');
function SetMethod_FVKNSPNHTX() {
Set_CODSOG(HtW(value,'C'),0);
}
if (IsDeviceMobile()) {
SetMethod_FVKNSPNHTX();
} else {
setTimeout(SetMethod_FVKNSPNHTX,1);
}
bResult=true;
}
if (Eq(varName,'KQROMFNGNZ')) {
last_focused_comp=GetLastFocusedElementOfLink('KQROMFNGNZ');
function SetMethod_KQROMFNGNZ() {
Set_LEGAME(HtW(value,'C'),0);
}
if (IsDeviceMobile()) {
SetMethod_KQROMFNGNZ();
} else {
setTimeout(SetMethod_KQROMFNGNZ,1);
}
bResult=true;
}
return bResult;
...
if (Eq(varName,'FVKNSPNHTX')) {
return [Ctrl('FVKNSPNHTX')];
}
if (Eq(varName,'KQROMFNGNZ')) {
return [Ctrl('KQROMFNGNZ')];
}
}
...
if ( ! (m_oFather.SaveContext_ardt_provsoginfo())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_provsoginfo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_provsoginfo_dataNotSent',{'action':formaction});
}
}
}
}
...
var l_bResult = true;
ResetErrorVars();
if ( ! (_ChkObl('IHXPLCYXNH',1))) {
_SignErr('IHXPLCYXNH');
m_cLastWorkVarErrorMsg='RAPCOL';
l_bResult=false;
w_RAPCOL=HtW('','C');
}
if (l_bResult) {
...
if (Eq('FVKNSPNHTX_ZOOM',lc)) {
return 0;
}
if (Eq('FVKNSPNHTX',lc)) {
return 0;
}
if (Eq('KQROMFNGNZ_ZOOM',lc)) {
return 1;
}
if (Eq('KQROMFNGNZ',lc)) {
return 1;
...
l_oWv.setValue('RAPCOL',WtA(w_RAPCOL,'C'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
_FillChildren.n=[];
FillWv.n=["IDFILEBO","RAPCOL"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
if (Le(m_nCurrentRow,Rows())) {
if (( ! (_ChkObl('FVKNSPNHTX',1))) && FullRow()) {
_SignErr('FVKNSPNHTX');
m_cLastWorkVarErrorMsg='10847988462';
l_bResult=false;
w_CODSOG=HtW('','C');
} else if (( ! (_ChkObl('KQROMFNGNZ',1))) && FullRow()) {
_SignErr('KQROMFNGNZ');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_provsoginfo();
}
}
SaveContext.child=[];
function BlankRow() {
...
Link_FVKNSPNHTX(null);
}
if ( ! (EmptyString(w_LEGAME))) {
Link_KQROMFNGNZ(null);
}
BlankRowChild();
if (m_bExtended) {
...
l_aRepeatedFields={};
l_aRepeatedFields.CODSOG={"dec":0,"fixedpos":false,"id":"FVKNSPNHTX","len":16,"name":"CODSOG","title":"Codice Soggetto","type":"C"};
l_aRepeatedFields.LEGAME={"dec":0,"fixedpos":false,"id":"KQROMFNGNZ","len":1,"name":"LEGAME","title":"Tipo Legame","type":"C"};
l_aRepeatedFields.xragsoc={"dec":0,"fixedpos":false,"id":"LRZLVXCRIO","len":70,"name":"xragsoc","title":"Ragione Sociale","type":"C"};
l_aRepeatedFields.xdescleg={"dec":0,"fixedpos":true,"id":"LPKMVNCKBI","len":50,"name":"xdescleg","title":"Descrizione","type":"C"};
...
if (enable) {
var c;
if(c=Ctrl('IHXPLCYXNH')) SetDisabled(c,true);
SetQueryRow(Ctrl('GridTable'),false);
} else {
SetEditRow(Ctrl('GridTable'),false);
var c;
if(c=Ctrl('IHXPLCYXNH')) SetDisabled(c,false);
