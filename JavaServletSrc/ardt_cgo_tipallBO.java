...
// * --- Fine Area Manuale
public class ardt_cgo_tipallBO extends ardt_cgo_tipallBL {
  ...
  // * --- Fine Area Manuale
  public ardt_cgo_tipallBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_cgo_tipall");
    }
  ...
  public static String[] m_cLoadParameterNames = {"TACODINT"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_TACODINT) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cgo_tipall,m_cPhName_cgo_tipall,m_cVirtName_cgo_tipall,m_cVirtName_cgo_tipall+".TACODINT="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_TACODINT,"C",11,0),m_cServer_cgo_tipall,p_TACODINT),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_TATIPALL = l_rsLoadBody.GetString("TATIPALL");
          w_TAPESUNI = l_rsLoadBody.GetDouble("TAPESUNI");
          w_TAPESSIN = l_rsLoadBody.GetDouble("TAPESSIN");
          // Assegna il contesto dei figli del dettaglio a Empty
          work_children = CPLib.BlankChildMNTs(m_MNTs,true);
          opk_TATIPALL = w_TATIPALL;
          if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tipall")) {
  ...
  public boolean Delete(String p_TACODINT) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cgo_tipall.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"TACODINT="+CPLib.ToSQL(w_TACODINT,"C",11,0)+" AND TATIPALL="+CPLib.ToSQL(opk_TATIPALL,"C",4,0);
        l_cSql = l_cSql+m_oWrInfo_cgo_tipall.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tipall")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_tipall,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_tipall);
      l_cSql = "insert into "+m_oWrInfo_cgo_tipall.GetTableWriteName()+" (";
      l_cSql = l_cSql+"TACODINT,";
      l_cSql = l_cSql+"TATIPALL,";
      l_cSql = l_cSql+"TAPESUNI,";
      l_cSql = l_cSql+"TAPESSIN,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_tipall.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tipall")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_TACODINT,"C",11,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_TATIPALL,"C",4,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_TAPESUNI,"N",5,2)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_TAPESSIN,"N",5,2)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_tipall.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tipall")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_tipall,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_tipall);
      l_cSql = "Update "+m_oWrInfo_cgo_tipall.GetTableWriteName()+" set";
      l_cSql = l_cSql+" TAPESUNI = "+CPLib.ToSQL(w_TAPESUNI,"N",5,2);
      l_cSql = l_cSql+", TAPESSIN = "+CPLib.ToSQL(w_TAPESSIN,"N",5,2);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tipall")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", TATIPALL = "+CPLib.ToSQL(w_TATIPALL,"C",4,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"TACODINT="+CPLib.ToSQL(w_TACODINT,"C",11,0)+" AND TATIPALL="+CPLib.ToSQL(opk_TATIPALL,"C",4,0);
      l_cSql = l_cSql+m_oWrInfo_cgo_tipall.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tipall")) {
