...
function Help() {
windowOpenForeground('../doc/arpg_filesage_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_filesage',m_cSelectedPage);
...
function TrsQueryRow() {
  ...
  this.c_mese = arguments[3]
  this.c_anno = arguments[4]
}
function TrsToWork() {
var row;
...
GroupFooter_BeginHtml(document,1,'1814151239'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'1814151239'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'1847705671'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_0399AC80"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
...
"")
document.writeln(
 "<span class=' SPLabel DPGOVUFTDG_DIV'>" +
 "<label class='DPGOVUFTDG'>" +
 ""+ToHTag(Translate("1974864032"))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'1847705671'+l_nIdx+'','A')
} else {
document.writeln(
 "<input name='gAzienda_"+l_nIdx+"' id='VGYOFPCRVR_"+l_nIdx+"' type='hidden' value="+ToHTMLValue(WtH(w_gAzienda,"C",10,0,''))+" size='10'>" +
"")
document.writeln(
 "<input name='gUrlApp_"+l_nIdx+"' id='LNTUFAZHYS_"+l_nIdx+"' type='hidden' value="+ToHTMLValue(WtH(w_gUrlApp,"C",80,0,''))+" size='80'>" +
"")
}
Row_BeginHtml(document,l_nEvenOdd,'2015477782'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_039E9640"+l_nIdx+"' style='position:relative;height:18px'>" +
"")
document.writeln(
...
"")
document.writeln(
 "<span class=' SPLabel SXLDVMKGOE_DIV'>" +
 "<label class='SXLDVMKGOE'>" +
 ""+ToHTag(WtH(c_mese+'-'+Str(c_anno,4,0),'C',7,0,''))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'2015477782'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
