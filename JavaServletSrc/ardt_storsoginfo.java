...
public class ardt_storsoginfo extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*6e7a574*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "woginfo";
    }
    public String w_IDFILEBO;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_IDFILEBO,context,"woginfo","IDFILEBO","C",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    ardt_storsoginfoBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_woginfo;
  ...
  public String getEntityTitle() {
    return "Storico Soggetti Agenti per Conto Inform.";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "ardt_storsoginfo";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 580;
    l_eg.m_nEntityHeight = 254;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"woginfo","personbo","tbtipleg"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_storsoginfoBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"10258747293",p_Context.Translate("Storico Soggetti Agenti per Conto Inform.")},
  ...
  void WriteStyles(ardt_storsoginfoBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_storsoginfo.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_storsoginfoBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_storsoginfo',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_storsoginfo",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_storsoginfoBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_storsoginfoBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:254px;width:580px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(ardt_storsoginfoBO BO,ServletStatus status) {
    status.out.println(
    ...
    "");
    status.out.println(
     "<span class='SPLabel' id='PQGSBATIPW_DIV'>" +
     "<label id='PQGSBATIPW'>" +
    ...
    status.out.println(
     "<input name='RAPCOL' id='ABZSLYXSCO' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_RAPCOL,"C",25,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='25' size='25' fieldtype='C' style='display:none'>" +
    "");
    status.out.println(
    ...
    HTMLRowScript(BO,status);
    status.out.println(
     "<input name='xdescleg' id='LPKMVNCKBI' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_xdescleg,"C",50,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='50' size='50' fieldtype='C'>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+1;
    status.out.println(
     "<span class='SPLabel' id='SRGXXZYNNI_DIV'>" +
     "<label id='SRGXXZYNNI'>" +
  ...
  void HTMLRowScript(ardt_storsoginfoBO BO,ServletStatus status) {
    ...
    status.out.println("var w_RAPCOL;");
    ...
    status.out.println("w_RAPCOL=row[prefix+'RAPCOL'];");
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     "<input name='LEGAME' id='KQROMFNGNZ' type='text' class='Field GridTable_EditRowItem ' value='\"+WtH(ToHTML(w_LEGAME),'C',1,0,'')+\"' tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='1' size='1' fieldtype='C'>" +
     "<button id='KQROMFNGNZ_ZOOM' type='button' class='GridZoomButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
     "</button>" +
    "\"");
    status.out.println("s=s+\""+
  ...
  static void PrintState(ardt_storsoginfoBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_storsoginfoBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_storsoginfoBL BO,ServletStatus status) {
  ...
  static void PrintStateDetailWorkVars(ardt_storsoginfoBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_storsoginfoBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_storsoginfoBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_storsoginfoBO.TrsRow) BO.m_oDeletedRows.get(k);
  ...
  void WriteMainFormVarScript(ardt_storsoginfoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(254,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(580,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_storsoginfo"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_storsoginfo","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_storsoginfo",BO.GetCurrentState());
    ...
    status.out.println("w_RAPCOL=row.RAPCOL;");
  ...
  void WriteMainFormScript(ardt_storsoginfoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_storsoginfo);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_storsoginfo();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_storsoginfo',Ne(m_nChildStatus,0));");
    }
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("var c;");
        status.out.println("if(c=Ctrl('ABZSLYXSCO')) SetDisabled(c,true);");
    ...
    status.out.println("m_oFather.m_oardt_storsoginfo=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_storsoginfoBO getBO(ServletStatus status) {
    ardt_storsoginfoBO BO = new ardt_storsoginfoBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_storsoginfoBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("woginfo",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_storsoginfoBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_storsoginfoBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ardt_storsoginfoBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_woginfo;
    String l_cPhName = BO.m_cPhName_woginfo;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_storsoginfoBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_woginfo+".IDFILEBO"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_woginfo+".IDFILEBO"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("woginfo");
    String l_cPhName = CPSql.ManipulateTablePhName("woginfo",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_storsoginfoBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_woginfo),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_woginfo,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_woginfo+".IDFILEBO) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_woginfo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_woginfo,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_woginfo+".IDFILEBO",CPLib.GetTablesWithAliases(BO.m_cPhName_woginfo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_storsoginfoBO BO) {
  ...
  void Edit(ServletStatus status,ardt_storsoginfoBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_storsoginfoBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_storsoginfoBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_storsoginfoBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_storsoginfoBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_storsoginfoBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_storsoginfoBO BO,SPParameterSource source) {
    BO.Load(status.w_IDFILEBO);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_storsoginfoBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_storsoginfoBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_storsoginfoBO BO) {
    ...
    item.put("altInterfaceFor","ardt_storsoginfo");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"h\":16,\"name\":\"label_JWSZDAXYGH\",\"page\":1,\"spuid\":\"JWSZDAXYGH\",\"tabindex\":2,\"type\":\"Label\",\"typevar\":\"logic\",\"value\":\"Codice Soggetto\",\"w\":91,\"x\":8,\"y\":8,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":130,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"PUDSNEKGLR\",\"type\":\"BodyDetail\",\"w\":608,\"x\":8,\"y\":22,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_VRCVINYRJO\",\"h\":70,\"name\":\"box_VRCVINYRJO\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VRCVINYRJO\",\"tabindex\":4,\"type\":\"Box\",\"w\":552,\"x\":8,\"y\":184,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Descrizione\",\"disabled\":\"true\",\"fixed\":true,\"h\":20,\"maxlength\":50,\"name\":\"xdescleg\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"LPKMVNCKBI\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":464,\"x\":96,\"y\":160,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_AVTVZXZUWT\",\"page\":1,\"spuid\":\"AVTVZXZUWT\",\"tabindex\":5,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"DATI ANAGRAFICI\",\"w\":536,\"x\":16,\"y\":191,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NAKGRMOUXS\",\"page\":1,\"spuid\":\"NAKGRMOUXS\",\"tabindex\":6,\"type\":\"Label\",\"w\":360,\"x\":16,\"y\":212,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HMPQLYVATL\",\"page\":1,\"spuid\":\"HMPQLYVATL\",\"tabindex\":7,\"type\":\"Label\",\"w\":416,\"x\":16,\"y\":233,\"zone\":\"pag1_3_3\",\"zonepath\":\"[1,3,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":34,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"CQBJQPMCVX\",\"type\":\"Box\",\"w\":552,\"x\":8,\"y\":7,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Codice Soggetto\",\"h\":17,\"maxlength\":16,\"name\":\"CODSOG\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"FVKNSPNHTX\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":110,\"x\":8,\"y\":24,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Tipo Legame\",\"h\":17,\"maxlength\":1,\"name\":\"LEGAME\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"KQROMFNGNZ\",\"tabindex\":2,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":30,\"x\":512,\"y\":24,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Ragione Sociale\",\"disabled\":\"true\",\"h\":17,\"maxlength\":70,\"name\":\"xragsoc\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"LRZLVXCRIO\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":344,\"x\":152,\"y\":24,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_storsoginfoBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_storsoginfoBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_storsoginfo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_storsoginfo_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_storsoginfo_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_storsoginfo_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(254,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void RaiseLoadedEntity(ardt_storsoginfoBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_storsoginfoBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_storsoginfoBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_storsoginfoBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_storsoginfoBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_storsoginfoBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_storsoginfoBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ardt_storsoginfoBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ardt_storsoginfoBO BO) {
    boolean result = true;
