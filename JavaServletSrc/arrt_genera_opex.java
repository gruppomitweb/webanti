...
public class arrt_genera_opex extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*98287eef*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_genera_opexR Rcalled;
    Rcalled = new arrt_genera_opexR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaa_retval = Rcalled.Run(status.w_DaDatOpe,status.w_ADatOpe,status.w_anno);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_genera_opexR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_genera_opexR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_genera_opex"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
