...
public class arrt_seekpep_rs_rp extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*8f53b970*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_seekpep_rs_rpR Rcalled;
    Rcalled = new arrt_seekpep_rs_rpR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aa_retval = Rcalled.Run(status.pTicket,status.pCodFis);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_seekpep_rs_rpR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_seekpep_rs_rpR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_seekpep_rs_rp"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
