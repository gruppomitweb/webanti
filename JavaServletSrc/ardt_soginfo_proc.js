...
SPBOUILib.SetInputValue('UJDYOPDWZF',function(){return WtH(w_RAPCOL,'C',25,0,'')},w_RAPCOL);
...
SetDisplay(Ctrl('UJDYOPDWZF'),Eq(1,1) || IsHiddenByStateDriver('RAPCOL'));
...
HideUI.lblids["CODSOG"]=['JWSZDAXYGH'];
HideUI.lblids["LEGAME"]=['PQGSBATIPW'];
function ExpandCollapseBoxesUnderCondition() {
var cond;
}
function UJDYOPDWZF_Valid(e) {
SetActiveField(Ctrl('UJDYOPDWZF'),false);
if (Eq(m_cFunction,'query')) {
if (IsLastInPk('UJDYOPDWZF') ||  ! (m_bQueryOnLatestKeyOnly)) {
DoQuery('RAPCOL','C',w_RAPCOL,HtW(Ctrl('UJDYOPDWZF').value,'C'),m_bQueryOnLatestKeyOnly);
}
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_RAPCOL(HtW(Ctrl('UJDYOPDWZF').value,'C'),null,e);
return l_bSetResult;
}
}
function UJDYOPDWZF_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('UJDYOPDWZF'))) {
DisplayErrorMessage();
}
SetActiveField(Ctrl('UJDYOPDWZF'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('UJDYOPDWZF'),e);
}
...
function UJDYOPDWZF_wrap_Valid(event) {
return UJDYOPDWZF_Valid(event);
}
SPBOUILib.SetInputExit('UJDYOPDWZF',UJDYOPDWZF_wrap_Valid);
function UJDYOPDWZF_wrap_OnFocus(event) {
return UJDYOPDWZF_OnFocus(event);
}
SPBOUILib.SetInputEnter('UJDYOPDWZF',UJDYOPDWZF_wrap_OnFocus);
function UJDYOPDWZF_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('UJDYOPDWZF',UJDYOPDWZF_wrap_OnKeyDown);
...
m_oFather.i_last_focused_item='child_ardt_soginfo';
...
function Help() {
windowOpenForeground('../doc/ardt_soginfo_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
return 'soginfo'+'\\'+CPLib.ToCPStr(w_RAPCOL);
}
...
p_d.setValue('ardt_soginfo',m_cSelectedPage);
...
_Obli('RAPCOL',0,'UJDYOPDWZF',false)
