...
public class arrt_imp_dati2 extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*4286483a*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public String aaaaaabaa_retval;
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_impzip","w_impzip");
      }        
      w_impzip = source.GetParameter("w_impzip",0);
      if (source instanceof SPRequestSource) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_fscarti","w_fscarti");
      }        
      w_fscarti = source.GetParameter("w_fscarti",0);
      if (source instanceof SPRequestSource) {
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_imp_dati2R Rcalled;
    Rcalled = new arrt_imp_dati2R(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaaaabaa_retval = Rcalled.Run(status.w_tipodest,status.w_impzip,status.w_annorif,status.w_chkaui,status.w_imperr,status.w_creamo,status.w_fscarti,status.w_flgaggfam,status.w_flgimposv);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_imp_dati2R(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_imp_dati2R Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_imp_dati2"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
      if (CPLib.Empty(Rcalled.m_cLastMsgError) &&  ! (status.m_bApplet)) {
        status.out.println("Error message:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(status.m_cFaultTrace)+"");
      }
    }
    status.PrintMutableParameters();
    status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(status.aaaaaabaa_retval))+"");
    status.out.println(
     " -->" +
    "");
    status.out.println(
