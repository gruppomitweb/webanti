...
public class arpg_calcsaldi_opex_periodo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*DJOATSSTRQ*!";
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    arpg_calcsaldi_opex_periodoBO BO;
    BO = getBO(status);
  ...
  public String getEntityTitle() {
    return "Calcolo Saldi Oper. Extraconto MoneyTransfer/IDP (Periodale)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_calcsaldi_opex_periodo";
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("A_Data","D",8,0, "edit","A Data Operazione")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_calcsaldi_opex_periodoBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11574566225",p_Context.Translate("Calcolo Saldi Oper. Extraconto MoneyTransfer/IDP (Periodale)")},
  ...
  void WriteStyles(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_calcsaldi_opex_periodo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
  ...
  void WriteControls(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(511);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<a href='javascript:href_YAOZTJHJZC()' tabindex='-1' style='display:none' id='YAOZTJHJZC_HREF'>" +
     "<img id='YAOZTJHJZC' src="+SPLib.ToHTMLValue(SPLib.ToResource("images/elaborazione.gif"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' alt=''>" +
     "</a>" +
    "");
    status.themeWriter.Header_EndHtml();
  }
  static void PrintState(arpg_calcsaldi_opex_periodoBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_calcsaldi_opex_periodoBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_calcsaldi_opex_periodoBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_calcsaldi_opex_periodo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_calcsaldi_opex_periodo","");
  ...
  void WriteMainFormScript(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_calcsaldi_opex_periodo");
    status.out.println("function Lancio() {");
    status.out.println("  w_pMsg.Timer0.Start()");
    status.out.println("  href_YAOZTJHJZC()");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
     "</script>" +
  ...
  arpg_calcsaldi_opex_periodoBO getBO(ServletStatus status) {
    arpg_calcsaldi_opex_periodoBO BO = new arpg_calcsaldi_opex_periodoBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_calcsaldi_opex_periodoBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_calcsaldi_opex_periodoBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_calcsaldi_opex_periodoBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_calcsaldi_opex_periodoBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_calcsaldi_opex_periodoBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_calcsaldi_opex_periodoBO BO) {
    ...
    item.put("altInterfaceFor","arpg_calcsaldi_opex_periodo");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"504\",\"name\":\"Box_VFFMUWZZJS\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"VFFMUWZZJS\",\"tabindex\":\"3\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"href\":\"javascript:href_MAYNQUHJSS()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"MAYNQUHJSS\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"4\",\"type\":\"Image\",\"w\":\"42\",\"x\":\"352\",\"y\":\"70\",\"zone\":\"VFFMUWZZJS_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Chiudi la procedura senza elaborare i dati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"href\":\"javascript:href_NACPPABTRW()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"NACPPABTRW\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"5\",\"type\":\"Image\",\"w\":\"42\",\"x\":\"400\",\"y\":\"70\",\"zone\":\"VFFMUWZZJS_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_xdesval\",\"page\":\"1\",\"spuid\":\"UAIBAUYUDZ_DIV\",\"tabindex\":\"6\",\"type\":\"Label\",\"value\":\"w_xdesval\",\"w\":\"352\",\"x\":\"416\",\"y\":\"18\",\"zone\":\"VFFMUWZZJS_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"91\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"768\",\"x\":\"12\",\"y\":\"105\",\"zone\":\"VFFMUWZZJS_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Anno di riferimento\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"annorif\",\"page\":\"1\",\"spuid\":\"QJYFRLEIBB\",\"tabindex\":\"11\",\"type\":\"Textbox\",\"w\":\"48\",\"x\":\"160\",\"y\":\"14\",\"zone\":\"VFFMUWZZJS_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Anno_di_riferimento_\",\"page\":\"1\",\"spuid\":\"QJBHNDQNQE_DIV\",\"tabindex\":\"12\",\"type\":\"Label\",\"value\":\"Anno di riferimento:\",\"w\":\"95\",\"x\":\"65\",\"y\":\"18\",\"zone\":\"VFFMUWZZJS_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Valuta Prevalente\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"3\",\"name\":\"valuta\",\"page\":\"1\",\"spuid\":\"SZZYVNKOQS\",\"tabindex\":\"13\",\"type\":\"Textbox\",\"w\":\"30\",\"x\":\"360\",\"y\":\"14\",\"zone\":\"VFFMUWZZJS_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Valuta_Prevalente_\",\"page\":\"1\",\"spuid\":\"QVLNYNNMIY_DIV\",\"tabindex\":\"14\",\"type\":\"Label\",\"value\":\"Valuta Prevalente:\",\"w\":\"89\",\"x\":\"271\",\"y\":\"18\",\"zone\":\"VFFMUWZZJS_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"301\",\"href\":\" \",\"name\":\"__iframe_width__750__height__400__id__elaborazione___name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"TYIWXFLLFY_DIV\",\"tabindex\":\"16\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"750"+"'"+" height="+"'"+"400"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"768\",\"x\":\"12\",\"y\":\"203\",\"zone\":\"VFFMUWZZJS_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Da Data Operazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"DaData\",\"page\":\"1\",\"spuid\":\"AUEHRDRUOQ\",\"tabindex\":\"19\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"160\",\"y\":\"42\",\"zone\":\"VFFMUWZZJS_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Data_Operazione_\",\"page\":\"1\",\"spuid\":\"IOUFTUSNKM_DIV\",\"tabindex\":\"20\",\"type\":\"Label\",\"value\":\"Da Data Operazione:\",\"w\":\"111\",\"x\":\"49\",\"y\":\"46\",\"zone\":\"VFFMUWZZJS_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"A Data Operazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"A_Data\",\"page\":\"1\",\"spuid\":\"BXZPTJLRQT\",\"tabindex\":\"21\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"360\",\"y\":\"42\",\"zone\":\"VFFMUWZZJS_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Data_Operazione_\",\"page\":\"1\",\"spuid\":\"GDRAKDZNJZ_DIV\",\"tabindex\":\"22\",\"type\":\"Label\",\"value\":\"A Data Operazione:\",\"w\":\"111\",\"x\":\"249\",\"y\":\"46\",\"zone\":\"VFFMUWZZJS_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"href\":\"javascript:href_YAOZTJHJZC()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"YAOZTJHJZC\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"23\",\"type\":\"Image\",\"w\":\"42\",\"x\":\"736\",\"y\":\"70\",\"zone\":\"VFFMUWZZJS_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_calcsaldi_opex_periodoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_calcsaldi_opex_periodoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_calcsaldi_opex_periodo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_calcsaldi_opex_periodo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_calcsaldi_opex_periodoBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_calcsaldi_opex_periodo.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_calcsaldi_opex_periodoBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_calcsaldi_opex_periodoBO BO) {
    boolean result = true;
    return result;
  }
}
