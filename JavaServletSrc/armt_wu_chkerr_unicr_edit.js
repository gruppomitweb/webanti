...
function Set_WUNDATFIN(ctrlValue,kmode,e) {
if (Ne(w_WUNDATFIN,ctrlValue)) {
var ctl = _GetCtl(e,'GEZZWCCIJK');
if (_tracker.goon(ctl,ctrlValue)) {
w_WUNDATFIN=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('GEZZWCCIJK',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
function Set_WUNDATINI(ctrlValue,kmode,e) {
if (Ne(w_WUNDATINI,ctrlValue)) {
var ctl = _GetCtl(e,'WQBGAJEIXP');
if (_tracker.goon(ctl,ctrlValue)) {
w_WUNDATINI=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('WQBGAJEIXP',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH(NullDate(),'D',8,0,TranslatePicture(GetDatePicture(false,'D'),'D'));
...
parent.ZtVWeb.raiseEventToEvalParms('armt_wu_chkerr_unicr_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_wu_chkerr_unicr_dataNotSent',{'action':formaction});
}
}
}
}
...
l_bResult=false;
w_WUFLGFRC='N';
} else if ( ! (_ChkObl('WQBGAJEIXP',1))) {
_SignErr('WQBGAJEIXP');
m_cLastWorkVarErrorMsg='2032298989';
l_bResult=false;
w_WUNDATINI=HtW('','D');
} else if ( ! (_ChkObl('GEZZWCCIJK',1))) {
_SignErr('GEZZWCCIJK');
m_cLastWorkVarErrorMsg='11006832099';
l_bResult=false;
w_WUNDATFIN=HtW('','D');
...
l_oWv.setValue('WUDATINI',WtA(w_WUDATINI,'D'));
...
l_oWv.setValue('WUDATFIN',WtA(w_WUDATFIN,'D'));
l_oWv.setValue('WUCODRAP',WtA(w_WUCODRAP,'C'));
l_oWv.setValue('WUNDATINI',WtA(w_WUNDATINI,'D'));
l_oWv.setValue('WUNDATFIN',WtA(w_WUNDATFIN,'D'));
...
_FillChildren.n=[];
FillWv.n=["WUNOMEFILE","WUTIPOPE","WUCODDIP","WUDATINI","WUTIPERR","WUCODICE","WUNCAPCLI","WUNDATNAS","WUNRDOCCLI","WUNSDOCCLI","WUNTDOCCLI","WUNCODDIP","WUNCITCLI","WUNBSTACLI","WUNBCITCLI","WUNOMCLI","WUCOGCLI","WUCITCLI","WUCAPCLI","WUINDCLI","WUBCITCLI","WUBSTACLI","WUDATNAS","WUCODFIS","WUNDOCCLI","WUADOCRIL","WURDOCCLI","WUSDOCCLI","WUSEXCLI","WUTDOCCLI","WUFLGALL","WUNCODFIS","WUCODMAGE","WUNOTE","WUFLGFRC","WUDATFIN","WUCODRAP","WUNDATINI","WUNDATFIN","WUSTACLI","WUNSEXCLI","xMADESCRI","xDESDIP"];
