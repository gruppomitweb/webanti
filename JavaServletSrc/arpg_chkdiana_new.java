...
public class arpg_chkdiana_new extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*QUPXZAGFWJ*!";
  ...
  public String getEntityTitle() {
    return "Preparazione file per Check Scarico OffLine (Dal 01/01/2014)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_chkdiana_new";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_chkdiana_newBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"10435768771",p_Context.Translate("La data di inizio periodo non deve essere inferiore al 01/01/2014")},
    ...
    {"11089820069",p_Context.Translate("La data di fine periodo deve essere maggiore di quella di inizio e non inferiore al 01/01/2014")},
    ...
    {"695153413",p_Context.Translate("Preparazione file per Check Scarico OffLine (Dal 01/01/2014)")},
  ...
  void WriteStyles(arpg_chkdiana_newBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_chkdiana_new.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_chkdiana_newBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_chkdiana_newBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_chkdiana_newBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_chkdiana_newBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_chkdiana_newBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(658);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_chkdiana_newBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_chkdiana_newBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_chkdiana_newBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_chkdiana_newBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_chkdiana_newBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_chkdiana_new"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_chkdiana_new","");
  ...
  void WriteMainFormScript(arpg_chkdiana_newBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_chkdiana_newBO getBO(ServletStatus status) {
    arpg_chkdiana_newBO BO = new arpg_chkdiana_newBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_chkdiana_newBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_chkdiana_newBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_chkdiana_newBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_chkdiana_newBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_chkdiana_newBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_chkdiana_newBO BO) {
    ...
    item.put("altInterfaceFor","arpg_chkdiana_new");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"21\",\"name\":\"flgchk\",\"page\":\"1\",\"spuid\":\"LJVZZPXJUZ\",\"tabindex\":\"23\",\"type\":\"Checkbox\",\"w\":\"184\",\"x\":\"128\",\"y\":\"342\",\"zone\":\"VFFMUWZZJS_12\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Controparte_\",\"page\":\"1\",\"spuid\":\"PNUSYHLNFS_DIV\",\"tabindex\":\"42\",\"type\":\"Label\",\"value\":\"Controparte:\",\"w\":\"69\",\"x\":\"59\",\"y\":\"165\",\"zone\":\"VFFMUWZZJS_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void Next(ServletStatus status,arpg_chkdiana_newBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_chkdiana_newBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_chkdiana_newBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_chkdiana_new?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_chkdiana_new_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_chkdiana_newBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_chkdiana_newBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_chkdiana_newBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_chkdiana_newBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_chkdiana_newBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_chkdiana_newBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_chkdiana_newBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_chkdiana_newBO BO) {
    boolean result = true;
    return result;
  }
}
