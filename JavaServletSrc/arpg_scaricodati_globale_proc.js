...
if ( ! (Empty(w_a_data)) && Eq(w_bloccaedit,'N')) {
Elabora()
}
}
...
l_oWv.setValue('m_cParameterSequence',AsAppletValue('ReportName,outputFormat,rotation,DescAzi'));
...
w_Messaggi.raiseEvent('arpg_scaricodati_globale_'+Strtran(p_cEvent,' ','_'));
}
}
...
FocusFirstComponent.otherwise={"page_1":["NMNMXABGKO","CLVVMIYSQG"]};
function Help() {
windowOpenForeground('../doc/arpg_scaricodati_globale_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_scaricodati_globale',m_cSelectedPage);
...
l_bEnabled= ! ( ! (Empty(w_a_data)) && Eq(w_bloccaedit,'N'));
...
ResetErrorVars();
m_nLastError=0;
if (( ! (_ChkObl('CLVVMIYSQG',1))) || ( ! (Ge(w_a_data,w_dadata)))) {
...
_FillChildren.n=[];
FillWv.n=["dadata","a_data","bloccaedit","files","gIntemediario","CFINVIO","CAFINVIO","FILEINVIO","DATAINVIO","gDescAzi"];
