...
public class arpg_allinea_age extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*WSDIQDBFAT*!";
  ...
  public String getEntityTitle() {
    return "Allineamento Registrazione AGE";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_allinea_age";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_allinea_ageBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"2008039581",p_Context.Translate("Allineamento Registrazione AGE")},
    {"285808099",p_Context.Translate("Descrizione")},
    {"863017169",p_Context.Translate("Anno di riferimento")},
    {"984756930",p_Context.Translate("Tipo Intermediario")}};
    return l_translations;
  }
  void WriteStyles(arpg_allinea_ageBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_allinea_age.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_allinea_ageBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_allinea_ageBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_allinea_ageBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_allinea_ageBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_allinea_ageBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(294);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_allinea_ageBO BO,ServletStatus status) {
    status.out.println(
    ...
    "");
    status.out.println(
     "<span class='SPLabel' id='HUEJGVANPJ_DIV' style='display:none'>" +
     "<label id='HUEJGVANPJ'>" +
     ""+SPLib.ToHTMLRepresentation("<iframe width='880' height='500' id='elaborazione' name='elaborazione' frameborder='1'  border='1' align='TOP' marginwidth='1' marginheight='1'></iframe>")+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input name='anno' id='KICJFCXJWH' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_anno,"C",4,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='4' size='4' fieldtype='C'>" +
    "");
    status.out.println(
     "<span class='SPLabel' id='NBRHLGOACP_DIV'>" +
     "<label id='NBRHLGOACP' for='KICJFCXJWH'>" +
  ...
  static void PrintState(arpg_allinea_ageBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_allinea_ageBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_allinea_ageBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_allinea_ageBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    PrintState(BO,status);
    status.out.println("var e_HUEJGVANPJ = " + SPLib.ToJSValue(m_cEntityUid,"C",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_allinea_age"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_allinea_age","");
  ...
  void WriteMainFormScript(arpg_allinea_ageBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_allinea_ageBO getBO(ServletStatus status) {
    arpg_allinea_ageBO BO = new arpg_allinea_ageBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_allinea_ageBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_allinea_ageBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_allinea_ageBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_allinea_ageBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_allinea_ageBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_allinea_ageBO BO) {
    ...
    item.put("altInterfaceFor","arpg_allinea_age");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"287\",\"name\":\"Box_VFFMUWZZJS\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"VFFMUWZZJS\",\"tabindex\":\"3\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"href\":\"javascript:href_MAYNQUHJSS()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"MAYNQUHJSS\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"4\",\"type\":\"Image\",\"w\":\"42\",\"x\":\"688\",\"y\":\"14\",\"zone\":\"VFFMUWZZJS_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Chiudi la procedura senza elaborare i dati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"href\":\"javascript:href_NACPPABTRW()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"NACPPABTRW\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"5\",\"type\":\"Image\",\"w\":\"42\",\"x\":\"736\",\"y\":\"14\",\"zone\":\"VFFMUWZZJS_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"91\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"196\",\"zone\":\"VFFMUWZZJS_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"21\",\"href\":\" \",\"name\":\"__iframe_width__880__height__500__id__elaborazione__name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"HUEJGVANPJ_DIV\",\"tabindex\":\"11\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"880"+"'"+" height="+"'"+"500"+"'"+" id="+"'"+"elaborazione"+"'"+" name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"48\",\"x\":\"632\",\"y\":\"21\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"147\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"49\",\"zone\":\"VFFMUWZZJS_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Anno di riferimento\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"4\",\"name\":\"anno\",\"page\":\"1\",\"spuid\":\"KICJFCXJWH\",\"tabindex\":\"13\",\"type\":\"Textbox\",\"w\":\"48\",\"x\":\"120\",\"y\":\"14\",\"zone\":\"VFFMUWZZJS_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Anno_di_riferimento_\",\"page\":\"1\",\"spuid\":\"NBRHLGOACP_DIV\",\"tabindex\":\"14\",\"type\":\"Label\",\"value\":\"Anno di riferimento:\",\"w\":\"104\",\"x\":\"16\",\"y\":\"18\",\"zone\":\"VFFMUWZZJS_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_allinea_ageBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_allinea_ageBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_allinea_ageBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_allinea_age?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_allinea_age_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_allinea_ageBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_allinea_ageBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_allinea_ageBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_allinea_ageBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_allinea_ageBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_allinea_ageBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_allinea_age.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_allinea_ageBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_allinea_ageBO BO) {
    boolean result = true;
    return result;
  }
}
