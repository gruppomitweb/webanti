...
public class ardt_provinformazioni extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ce66d1df*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "xntestbo";
    }
    public String w_RAPPORTO;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_RAPPORTO,context,"xntestbo","RAPPORTO","C",25,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    ardt_provinformazioniBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_xntestbo;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "ardt_provinformazioni";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 764;
    l_eg.m_nEntityHeight = 320;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"xntestbo","personbo"};
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","","page"),new ZoneItem("box_MINAAOAOZC","",""),new ZoneItem("box_VRCVINYRJO","DATI ANAGRAFICI","horizontal_section")};
    l_eg.securedBoxes = new String[][] {};
    l_eg.securedTabs = new String[][] {};
    l_eg.keyColumns = new String[]{"RAPPORTO"};
    l_eg.items = new SPItem[]{
                   new SPItem("IDBASE","C",10,0, "hide","idbase"),
                   new SPItem("tipreg","N",1,0, "hide",""),
                   ...
                   new SPItem("tiporegr","C",1,0, "hide",""),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_provinformazioniBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"338683184",p_Context.Translate("Inserire P per Intestatario o D per Delegato")},
  ...
  void WriteStyles(ardt_provinformazioniBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_provinformazioni.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_provinformazioniBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(
       "<style>" +
      "");
      status.out.println(".XTDHYWJPZL {top:2px;position:absolute;left:24px;height:16px;width:108px;background-color:Transparent;z-index:1; }");
      status.out.println(".XTDHYWJPZL_ZOOM { position:absolute;top:2px;left:133px;height:17px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".HKZAUFLHMG {top:2px;position:absolute;left:600px;height:16px;width:64px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".NATIUIQLJX {top:2px;position:absolute;left:672px;height:16px;width:64px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".VICYEWBISV {top:2px;position:absolute;left:165px;height:16px;width:423px;background-color:Transparent;z-index:1; }");
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_provinformazioni',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_provinformazioni",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_provinformazioniBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_provinformazioniBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:320px;width:764px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(ardt_provinformazioniBO BO,ServletStatus status) {
    ...
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:absolute;top:15px;left:6px;height:186px;width:758px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
      ...
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:15px;left:28px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:15px;left:165px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:15px;left:602px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:15px;left:676px;height:186px;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
  ...
  void HTMLRowScript(ardt_provinformazioniBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(ardt_provinformazioniBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<div id='VRCVINYRJO' class='SectionBody' style='position:relative;height:45px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("box_VRCVINYRJO")) {
      ...
      if (status.HasZoneAltInterface("box_VRCVINYRJO@BEGIN") || status.HasZoneAltInterface("box_VRCVINYRJO@END")) {
        status.out.println(
         "<div id='VRCVINYRJO_boxContent' style='position:relative;height:45px;display:block'>" +
        "");
      }
      status.out.println(
      ...
      "");
      status.out.println(
       "<span class='SPLabel' id='SEPJTDHOKN_DIV'>" +
       "<label id='SEPJTDHOKN' for='VYVWCXNUMC'>" +
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='PROQUOTA' id='VYVWCXNUMC' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_PROQUOTA,"N",3,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='4' size='3' fieldtype='N'>" +
      "");
      if (status.HasZoneAltInterface("box_VRCVINYRJO@BEGIN") || status.HasZoneAltInterface("box_VRCVINYRJO@END")) {
        status.out.println(
         "</div>" +
  ...
  void WriteControls_p1_b1_CloseBand(ardt_provinformazioniBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("box_VRCVINYRJO")) {
      ...
      status.out.println("Ctrl('VRCVINYRJO').setAttribute('band_height',45);");
    }
  ...
  void WriteControls_p1_b2(ardt_provinformazioniBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_END_OF_PAGE_1")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_END_OF_PAGE_1' style='position:relative;height:48px;display:block' page='1'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_END_OF_PAGE_1")) {
      ...
      status.out.println(
       "<div id='MINAAOAOZC_DIV' class='UntitledBox'>" +
       "<div id='MINAAOAOZC' class='BoxBody'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
      ...
      "");
      status.out.println(
       "<input name='NUMPROG1' id='IOJVSZRBNX' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_NUMPROG1,"C",11,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='11' size='11' fieldtype='C'>" +
      "");
      status.out.println(
       "<input name='NUMPROG2' id='VNGVCVDHTS' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_NUMPROG2,"C",11,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='11' size='11' fieldtype='C'>" +
      "");
      status.out.println(
       "</div>" +
      "");
  ...
  static void PrintState(ardt_provinformazioniBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_provinformazioniBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_provinformazioniBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_tiporegr,"C",1,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_xragsoci,"C",40,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_tipreg,"N",1,0));
  ...
  static void PrintStateDetailWorkVars(ardt_provinformazioniBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_provinformazioniBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_provinformazioniBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_provinformazioniBO.TrsRow) BO.m_oDeletedRows.get(k);
        l_cTrs = l_cTrs+" new TrsRow(";
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_CODINTER,"C",16,0)+",";
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_tiporegr,"C",1,0)+",";
  ...
  void WriteMainFormVarScript(ardt_provinformazioniBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(320,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_provinformazioni"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_provinformazioni","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_provinformazioni",BO.GetCurrentState());
    ...
    status.out.println("this.tiporegr=arguments[1];");
    status.out.println("this.DATAINI=arguments[2];");
    status.out.println("this.DATAFINE=arguments[3];");
    status.out.println("this.TIPOPERS=arguments[4];");
    status.out.println("this.NUMPROG1=arguments[5];");
    status.out.println("this.NUMPROG2=arguments[6];");
    status.out.println("this.xragsoci=arguments[7];");
    ...
    status.out.println("this.xRagSoc=arguments[13];");
    status.out.println("this.PROQUOTA=arguments[14];");
    status.out.println("this.CPROWNUM=arguments[15];");
    status.out.println("this.m_nRowStatus=arguments[16];");
    status.out.println("this.m_cOldRowCPCCCHK=arguments[17];");
    int lastRow;
    lastRow = 18;
    for ( l = 0; CPLib.lt(l,BO.m_MNTs.length); l = l + (1)) {
      if (BO.m_MNTs[l].RepeatedChild()) {
  ...
  void WriteMainFormScript(ardt_provinformazioniBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_provinformazioni);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_provinformazioni();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_provinformazioni',Ne(m_nChildStatus,0));");
    } else {
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('VYVWCXNUMC')) SetDisabled(c,true);");
        ...
        status.out.println("if(c=Ctrl('IOJVSZRBNX')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('VNGVCVDHTS')) SetDisabled(c,true);");
    ...
    status.out.println("m_oFather.m_oardt_provinformazioni=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_provinformazioniBO getBO(ServletStatus status) {
    ardt_provinformazioniBO BO = new ardt_provinformazioniBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_provinformazioniBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("xntestbo",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_provinformazioniBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_provinformazioniBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ardt_provinformazioniBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_xntestbo;
    String l_cPhName = BO.m_cPhName_xntestbo;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_provinformazioniBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_xntestbo+".RAPPORTO"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_xntestbo+".RAPPORTO"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("xntestbo");
    String l_cPhName = CPSql.ManipulateTablePhName("xntestbo",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_provinformazioniBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_xntestbo),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_xntestbo,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_xntestbo+".RAPPORTO) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_xntestbo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_xntestbo,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_xntestbo+".RAPPORTO",CPLib.GetTablesWithAliases(BO.m_cPhName_xntestbo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_provinformazioniBO BO) {
  ...
  void Edit(ServletStatus status,ardt_provinformazioniBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_provinformazioniBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_provinformazioniBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_provinformazioniBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_provinformazioniBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_provinformazioniBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_provinformazioniBO BO,SPParameterSource source) {
    BO.Load(status.w_RAPPORTO);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_provinformazioniBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_provinformazioniBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_provinformazioniBO BO) {
    ...
    item.put("altInterfaceFor","ardt_provinformazioni");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"h\":16,\"name\":\"label_BFMPSJTVZY\",\"page\":1,\"spuid\":\"BFMPSJTVZY\",\"tabindex\":4,\"type\":\"Label\",\"typevar\":\"logic\",\"value\":\"Intestatario\",\"w\":61,\"x\":32,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_XCDXQXTRHM\",\"page\":1,\"spuid\":\"XCDXQXTRHM\",\"tabindex\":3,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"Inizio\",\"w\":64,\"x\":608,\"y\":2,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_QEAAMPSBPR\",\"page\":1,\"spuid\":\"QEAAMPSBPR\",\"tabindex\":5,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"Fine\",\"w\":64,\"x\":680,\"y\":2,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":182,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"FXIHFOINFE\",\"type\":\"BodyDetail\",\"w\":792,\"x\":8,\"y\":16,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_VRCVINYRJO\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"DATI ANAGRAFICI\",\"h\":45,\"name\":\"box_VRCVINYRJO\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VRCVINYRJO\",\"stretch\":true,\"tabindex\":8,\"title_caption\":\"DATI ANAGRAFICI\",\"titled\":true,\"type\":\"Box\",\"w\":728,\"x\":16,\"y\":207,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_SEPJTDHOKN\",\"page\":1,\"spuid\":\"SEPJTDHOKN\",\"tabindex\":14,\"type\":\"Label\",\"value\":\"% Pro Quota:\",\"w\":73,\"x\":632,\"y\":233,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"% Pro Quota\",\"fixed\":true,\"h\":20,\"maxlength\":4,\"name\":\"PROQUOTA\",\"page\":1,\"repeated\":true,\"spuid\":\"VYVWCXNUMC\",\"tabindex\":15,\"type\":\"Textbox\",\"typevar\":\"numeric\",\"w\":30,\"x\":705,\"y\":229,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NAKGRMOUXS\",\"page\":1,\"spuid\":\"NAKGRMOUXS\",\"tabindex\":9,\"type\":\"Label\",\"w\":648,\"x\":24,\"y\":214,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HMPQLYVATL\",\"page\":1,\"spuid\":\"HMPQLYVATL\",\"tabindex\":10,\"type\":\"Label\",\"w\":608,\"x\":24,\"y\":235,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_MINAAOAOZC\",\"h\":45,\"name\":\"box_MINAAOAOZC\",\"page\":1,\"shrinkable\":true,\"spuid\":\"MINAAOAOZC\",\"tabindex\":2,\"type\":\"Box\",\"w\":728,\"x\":16,\"y\":255,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BHLAQHLOJQ\",\"page\":1,\"spuid\":\"BHLAQHLOJQ\",\"tabindex\":12,\"type\":\"Label\",\"value\":\"Prog.Informazione Apertura:\",\"w\":147,\"x\":48,\"y\":273,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_EQUZNNIXBO\",\"page\":1,\"spuid\":\"EQUZNNIXBO\",\"tabindex\":13,\"type\":\"Label\",\"value\":\"Prog.Informazione Chiusura:\",\"w\":163,\"x\":496,\"y\":273,\"zone\":\"pag1_4_2\",\"zonepath\":\"[1,4,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Prog.Informazione Apertura\",\"disabled\":\"true\",\"fixed\":true,\"h\":23,\"maxlength\":11,\"name\":\"NUMPROG1\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"IOJVSZRBNX\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":80,\"x\":200,\"y\":266,\"zone\":\"pag1_4_3\",\"zonepath\":\"[1,4,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Prog.Informazione Chiusura\",\"disabled\":\"true\",\"fixed\":true,\"h\":23,\"maxlength\":11,\"name\":\"NUMPROG2\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"VNGVCVDHTS\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":664,\"y\":266,\"zone\":\"pag1_4_4\",\"zonepath\":\"[1,4,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":38,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"HNOHMAMZUQ\",\"type\":\"Box\",\"w\":736,\"x\":8,\"y\":-1,\"zone\":\"pag1_5\",\"zonepath\":\"[1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Intestatario (Non Intermediario)\",\"h\":17,\"maxlength\":16,\"name\":\"CODINTER\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"XTDHYWJPZL\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":108,\"x\":32,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Soggetto collegato all"+"'"+"informazione corrente (pallino verde)\",\"h\":17,\"keep_proportions\":\"bounded\",\"name\":\"image_BKRVFLJLOS\",\"page\":1,\"repeated\":true,\"spuid\":\"BKRVFLJLOS\",\"tabindex\":16,\"type\":\"Image\",\"w\":16,\"x\":8,\"y\":18,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"disabled\":\"true\",\"h\":17,\"maxlength\":70,\"name\":\"xRagSoc\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"VICYEWBISV\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":423,\"x\":173,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Fine\",\"h\":17,\"maxlength\":10,\"name\":\"DATAFINE\",\"page\":1,\"repeated\":true,\"spuid\":\"NATIUIQLJX\",\"tabindex\":4,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":64,\"x\":680,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Inizio\",\"h\":17,\"maxlength\":10,\"name\":\"DATAINI\",\"page\":1,\"repeated\":true,\"spuid\":\"HKZAUFLHMG\",\"tabindex\":3,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":64,\"x\":608,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_provinformazioniBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_provinformazioniBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_provinformazioni?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_provinformazioni_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_provinformazioni_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_PROQUOTA=a[9];");
      status.out.println("w_tiporegr=a[10];");
      status.out.println("w_xragsoci=a[11];");
      status.out.println("w_tipreg=a[12];");
      status.out.println("w_appoggio=a[13];");
      status.out.println("w_DOMICILIO=a[14];");
      status.out.println("w_DESCCIT=a[15];");
      status.out.println("w_PROVINCIA=a[16];");
      status.out.println("w_CAP=a[17];");
      status.out.println("w_xRagSoc=a[18];");
      status.out.println("if (Gt(a.length,19)) {");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_provinformazioni_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(320,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void RaiseLoadedEntity(ardt_provinformazioniBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_provinformazioniBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_provinformazioniBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_provinformazioniBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_provinformazioniBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_provinformazioniBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_provinformazioniBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ardt_provinformazioniBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ardt_provinformazioniBO BO) {
    boolean result = true;
