...
// * --- Fine Area Manuale
public class arrt_cgo_save_esiticon_multiR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_save_esiticon_multiR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_save_esiticon_multi",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_cgo_save_esiticon_multi";
    ...
    try {
      ...
      if (CPLib.eqr(pAll,"S")) {
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on cgo_operazioni_dl_multi into routine arrt_cgo_save_esiticon_multi returns two or more records. This item should return only a record.");
        ...
        if ( ! (CPLib.Empty(_coddip))) {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_save_esiticon_multi",16,"0000000D")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
        ...
        m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"cgo_operazioni_dl_multi",m_Ctx.GetCompany());
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_save_esiticon_multi",16,"0000000E")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
        m_cSql = m_cSql+"CGODATCON = "+CPLib.ToSQL(pDatCon,"D",8,0)+", ";
        m_cSql = m_cSql+"CGOESICON = "+CPLib.ToSQL(pEsiCon,"C",2,0,m_cServer)+", ";
        m_cSql = m_cSql+"CGONOTCON = "+CPLib.ToSQL(pNotCon,"M",10,0,m_cServer,m_oParameters)+", ";
        m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"cgo_operazioni_dl_multi",false,true);
  ...
  public static arrt_cgo_save_esiticon_multiR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_save_esiticon_multiR(p_Ctx, p_Caller);
  }
  public static arrt_cgo_save_esiticon_multiR Make(CPContext p_Ctx) {
    return new arrt_cgo_save_esiticon_multiR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arrt_cgo_save_esiticon_multi,
  public static final String i_InvokedRoutines = ",arrt_cgo_save_esiticon_multi,";
