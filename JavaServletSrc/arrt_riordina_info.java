...
public class arrt_riordina_info extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ae064c7e*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_rapuif) ||  ! (status.m_bpassedw_rapage) ||  ! (status.m_bpassedw_opeuif) ||  ! (status.m_bpassedw_opeage) ||  ! (status.m_bpassedw_frzuif);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_riordina_infoR Rcalled;
    Rcalled = new arrt_riordina_infoR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_riordina_infoR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_riordina_infoR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_riordina_info"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_riordina_info");
      if ( ! (CPLib.IsNull(l))) {
