...
w_DATAINI=(Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'50')?Utilities.Make(window).GetCallerDate('DATAOPE'):w_DATAINI);
w_DATAFINE=(Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'51')?Utilities.Make(window).GetCallerDate('DATAOPE'):w_DATAFINE);
}
...
if ( ! (m_oFather.SaveContext_ardt_intestit())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_intestit_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_intestit_dataNotSent',{'action':formaction});
}
}
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_intestit();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_IDBASE)) &&  ! (Empty(w_CODINTER));
}
...
l_aRepeatedFields.PRGIMPTITE={"dec":0,"fixedpos":false,"id":"CHJPRCYMKT","len":10,"name":"PRGIMPTITE","title":"","type":"N"};
...
l_aRepeatedFields.IDBASE={"dec":0,"fixedpos":false,"id":"QDIDYGGMQV","len":10,"name":"IDBASE","title":"idbase","type":"C"};
