...
// * --- Fine Area Manuale
public class ardt_cgo_tgtiporegBO extends ardt_cgo_tgtiporegBL {
  ...
  // * --- Fine Area Manuale
  public ardt_cgo_tgtiporegBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_cgo_tgtiporeg");
    }
  ...
  public static String[] m_cLoadParameterNames = {"TRCODGIO"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_TRCODGIO) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cgo_tgtiporeg,m_cPhName_cgo_tgtiporeg,m_cVirtName_cgo_tgtiporeg,m_cVirtName_cgo_tgtiporeg+".TRCODGIO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_TRCODGIO,"C",3,0),m_cServer_cgo_tgtiporeg,p_TRCODGIO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_TRTIPREG = l_rsLoadBody.GetString("TRTIPREG");
          w_TRDESREG = l_rsLoadBody.GetString("TRDESREG");
          // Assegna il contesto dei figli del dettaglio a Empty
          work_children = CPLib.BlankChildMNTs(m_MNTs,true);
          opk_TRTIPREG = w_TRTIPREG;
          if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tgtiporeg")) {
  ...
  public boolean Delete(String p_TRCODGIO) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cgo_tgtiporeg.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"TRCODGIO="+CPLib.ToSQL(w_TRCODGIO,"C",3,0)+" AND TRTIPREG="+CPLib.ToSQL(opk_TRTIPREG,"C",1,0);
        l_cSql = l_cSql+m_oWrInfo_cgo_tgtiporeg.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tgtiporeg")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_tgtiporeg,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_tgtiporeg);
      l_cSql = "insert into "+m_oWrInfo_cgo_tgtiporeg.GetTableWriteName()+" (";
      l_cSql = l_cSql+"TRCODGIO,";
      l_cSql = l_cSql+"TRTIPREG,";
      l_cSql = l_cSql+"TRDESREG,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_tgtiporeg.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tgtiporeg")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_TRCODGIO,"C",3,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_TRTIPREG,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_TRDESREG,"C",30,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_tgtiporeg.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tgtiporeg")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_tgtiporeg,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_tgtiporeg);
      l_cSql = "Update "+m_oWrInfo_cgo_tgtiporeg.GetTableWriteName()+" set";
      l_cSql = l_cSql+" TRDESREG = "+CPLib.ToSQL(w_TRDESREG,"C",30,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tgtiporeg")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", TRTIPREG = "+CPLib.ToSQL(w_TRTIPREG,"C",1,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"TRCODGIO="+CPLib.ToSQL(w_TRCODGIO,"C",3,0)+" AND TRTIPREG="+CPLib.ToSQL(opk_TRTIPREG,"C",1,0);
      l_cSql = l_cSql+m_oWrInfo_cgo_tgtiporeg.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_tgtiporeg")) {
