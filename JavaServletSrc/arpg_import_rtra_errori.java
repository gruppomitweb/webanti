...
public class arpg_import_rtra_errori extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*AAUVSJBOVG*!";
  ...
  public String getEntityTitle() {
    return "Import Correzioni (RTRA)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_import_rtra_errori";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_import_rtra_erroriBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"12075952367",p_Context.Translate("Import Correzioni (RTRA)")},
  ...
  void WriteStyles(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_import_rtra_errori.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_import_rtra_erroriBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  // GetPortlet_ListaFile_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_MVXXCZOQGP_Vars(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(539);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_import_rtra_erroriBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_import_rtra_erroriBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_import_rtra_erroriBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_import_rtra_errori"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_import_rtra_errori","");
  ...
  void WriteMainFormScript(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_import_rtra_errori");
  ...
  arpg_import_rtra_erroriBO getBO(ServletStatus status) {
    arpg_import_rtra_erroriBO BO = new arpg_import_rtra_erroriBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_import_rtra_erroriBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_import_rtra_erroriBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_import_rtra_erroriBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_import_rtra_erroriBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_import_rtra_erroriBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_import_rtra_erroriBO BO) {
    ...
    item.put("altInterfaceFor","arpg_import_rtra_errori");
    items.put(item);
    String s;
  ...
  void Next(ServletStatus status,arpg_import_rtra_erroriBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_import_rtra_erroriBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_import_rtra_errori?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_import_rtra_errori_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_import_rtra_erroriBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_import_rtra_erroriBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_import_rtra_erroriBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_import_rtra_erroriBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_import_rtra_erroriBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_import_rtra_erroriBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_import_rtra_erroriBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_import_rtra_erroriBO BO) {
    boolean result = true;
    return result;
  }
}
