...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SGWBXGCVUI').value,'N'),w_CICODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbcauint',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CICODICE') &&  ! (Empty(HtW(Ctrl('SGWBXGCVUI').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CICODICE',HtW(Ctrl('SGWBXGCVUI').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','opemt_tbcauint');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbcauint',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['SGWBXGCVUI'],i=0;i>=0;i--){
