...
public class arpg_ope_modopen2 extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*KESZUGPIWL*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_ope_modopen2";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 677;
    l_eg.m_nEntityHeight = 47;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_ope_modopen2BO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"12095201017",p_Context.Translate("Aggiorna i dati di apertura o intestatario")},
    {"382511098",p_Context.Translate("Esegui aggiornamento dati")}};
    return l_translations;
  }
  void WriteStyles(arpg_ope_modopen2BO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_ope_modopen2.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_ope_modopen2BO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_ope_modopen2BO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_ope_modopen2BO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(42);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_ope_modopen2BO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_ope_modopen2BL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_ope_modopen2BL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_ope_modopen2BL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_ope_modopen2BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(47,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_ope_modopen2"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_ope_modopen2","");
    ...
    status.out.println("// * --- arpg_ope_modopen2");
  ...
  void WriteMainFormScript(arpg_ope_modopen2BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_ope_modopen2BO getBO(ServletStatus status) {
    arpg_ope_modopen2BO BO = new arpg_ope_modopen2BO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_ope_modopen2BO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_ope_modopen2BO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_ope_modopen2BO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_ope_modopen2BO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_ope_modopen2BO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_ope_modopen2BO BO) {
    ...
    item.put("altInterfaceFor","arpg_ope_modopen2");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"35\",\"name\":\"Box_ODCPZZXLHY\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"ODCPZZXLHY\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"664\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Cancellazione_\",\"page\":\"1\",\"spuid\":\"XQSJGZVMDV_DIV\",\"tabindex\":\"2\",\"type\":\"Label\",\"value\":\"Data Cancellazione:\",\"w\":\"110\",\"x\":\"22\",\"y\":\"18\",\"zone\":\"ODCPZZXLHY_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_YBBLCLWLHJ()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"YBBLCLWLHJ\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"4\",\"type\":\"Image\",\"w\":\"42\",\"x\":\"626\",\"y\":\"9\",\"zone\":\"ODCPZZXLHY_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui aggiornamento dati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_MDWFLPZWFN()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"MDWFLPZWFN\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"5\",\"type\":\"Image\",\"w\":\"42\",\"x\":\"578\",\"y\":\"9\",\"zone\":\"ODCPZZXLHY_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
  ...
  void Next(ServletStatus status,arpg_ope_modopen2BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_ope_modopen2BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_ope_modopen2BO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_ope_modopen2?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_ope_modopen2_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(47,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void RaiseLoadedEntity(arpg_ope_modopen2BO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_ope_modopen2BO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_ope_modopen2BO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_ope_modopen2BO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_ope_modopen2BO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_ope_modopen2BO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  void IncludeJS(ServletStatus status) {
    String l_cJsUid;
    status.out.println(
     "<script src='SPPrxy/"+arrt_ope_modopen2.m_cEntityUid+"/arrt_ope_modopen2.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_ope_modopen2.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_ope_modopen2BO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_ope_modopen2BO BO) {
    boolean result = true;
    return result;
  }
}
