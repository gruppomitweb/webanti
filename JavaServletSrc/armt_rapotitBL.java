...
public class armt_rapotitBL extends armt_rapotitWV implements CPLib.QueryFilterFromApplication {
  ...
  public static final String i_EntityName = "armt_rapotit";
  ...
  public static final String i_StateStructure = "{'CODDIPE':CODDIPE,'IDBASE':IDBASE,'CONNESCLI':CONNESCLI,'DATAOPE':DATAOPE,'DATAREG':DATAREG,'TIPOOPRAP':TIPOOPRAP,'TIPOLEG':TIPOLEG,'RAPPORTO':RAPPORTO,'STATOREG':STATOREG,'DESCINTER':DESCINTER,'ardt_intestit':ardt_intestit,'DATARETT':DATARETT"+FoundlingChildrenList("armt_rapotit",true)+"}";
  public static final String i_CompleteStateStructure = "{'flgpf':flgpf,'flgpg':flgpg,'CODINTER':CODINTER,'gCodDip':gCodDip,'gIntemediario':gIntemediario,'CODDIPE':CODDIPE,'IDBASE':IDBASE,'TIPOINTER':TIPOINTER,'TIPOINF':TIPOINF,'IDEREG':IDEREG,'FLAGRAP':FLAGRAP,'OPRAP':OPRAP,'OPERATORE':OPERATORE,'AUTOM':AUTOM,'DESCCIT':DESCCIT,'PROVINCIA':PROVINCIA,'CODCAB':CODCAB,'xdescrap':xdescrap,'xDescDipe':xDescDipe,'datpaseuro':datpaseuro,'gChkDate':gChkDate,'xDesReg':xDesReg,'xRagSoc':xRagSoc,'sEdit':sEdit,'errmsg':errmsg,'Aggiorna':Aggiorna,'FLAGRAP2':FLAGRAP2,'lancio':lancio,'FLAGLIRE':FLAGLIRE,'VALUTA':VALUTA,'gSblocco':gSblocco,'tipreg':tipreg,'tipregn':tipregn,'PRGIMPTIT':PRGIMPTIT,'dataoggi':dataoggi,'gStatus':gStatus,'gDataVaria':gDataVaria,'CITINT':CITINT,'CITDIP':CITDIP,'PROVDIP':PROVDIP,'CABDIP':CABDIP,'PROVINT':PROVINT,'CABINT':CABINT,'xRagSocCli':xRagSocCli,'filtro':filtro,'CONNESCLI':CONNESCLI,'DATAOPE':DATAOPE,'DATAREG':DATAREG,'CDATOPE':CDATOPE,'TIPOOPRAP':TIPOOPRAP,'CONNESOPER':CONNESOPER,'TIPOLEG':TIPOLEG,'RAPPORTO':RAPPORTO,'STATOREG':STATOREG,'NUMPROG':NUMPROG,'DESCINTER':DESCINTER,'ardt_intestit':ardt_intestit,'COLLEG':COLLEG,'SESSO':SESSO,'OPERAZMOD':OPERAZMOD,'CDATREG':CDATREG,'DATARETT':DATARETT"+FoundlingChildrenList("armt_rapotit",true)+"}";
  public static final String i_ItemSequence = "CODDIPE,IDBASE,CONNESCLI,DATAOPE,DATAREG,TIPOOPRAP,TIPOLEG,RAPPORTO,STATOREG,DESCINTER,ardt_intestit,DATARETT"+FoundlingChildrenList("armt_rapotit",false)+"";
  public static final com.zucchetti.sitepainter.EntityItem[] i_EditableEntityItems = com.zucchetti.sitepainter.EntityItem.buildEditableItemsForEntity(GetLinkDef(),FoundlingChildrenList("armt_rapotit"),new String[][]{new String[]{"CODDIPE","Codice Dipendenza","C","6","0","campo","nokey","false","false"},new String[]{"IDBASE","idbase","C","10","0","campo","key1","false","false"},new String[]{"CONNESCLI","","C","16","0","campo","key4","false","false"},new String[]{"DATAOPE","Data Operazione","D","8","0","campo","key2","false","true"},new String[]{"DATAREG","Data registrazione","D","8","0","campo","nokey","false","true"},new String[]{"TIPOOPRAP","Tipo Informazione","C","2","0","campo","key5","false","true"},new String[]{"TIPOLEG","Tipo Legame Sogg.","C","1","0","campo","nokey","false","false"},new String[]{"RAPPORTO","","C","25","0","campo","nokey","false","true"},new String[]{"STATOREG","Stato Registrazione","C","1","0","campo","nokey","false","false"},new String[]{"DESCINTER","","C","50","0","campo","nokey","false","false"},new String[]{"ardt_intestit","LinkPC","","","","link_PC","","false",""},new String[]{"DATARETT","Data Rettifica","D","8","0","campo","nokey","false","false"}});
  ...
  public armt_rapotitBL (CPContext p_ContextObject) {
    ...
    ardt_intestit = new ardt_intestitBO(p_ContextObject);
    m_MNTs = CPLib.MNTChilds("armt_rapotit",p_ContextObject);
  }
  public void Blank() {
  ...
  public boolean IsUpdated() {
    ...
    l_bResult = l_bResult || ardt_intestit.IsUpdated() || CPLib.IsUpdated(m_cWv_ardt_intestit);
    return l_bResult;
  }
  ...
  public boolean Calculate() {
    ...
    UpdateAutonumber();
    CopyWorkVar(this,ardt_intestit,new String[]{"tipoprap"},new String[]{"TIPOOPRAP"},true);
  ...
  void Calculation_KGBNFVNJHO() {
    ...
    l_oRoutine_arrt_writelog.pChiave = w_IDBASE;
    l_oRoutine_arrt_writelog.Run();
    arrt_ae_agg_aetesta_titR l_oRoutine_arrt_ae_agg_aetesta_tit;
    l_oRoutine_arrt_ae_agg_aetesta_tit = arrt_ae_agg_aetesta_titR.Make(m_Ctx,this);
    l_oRoutine_arrt_ae_agg_aetesta_tit.pSoggetto = w_CONNESCLI;
    l_oRoutine_arrt_ae_agg_aetesta_tit.Run();
  }
  ...
  public static String QueryFilter(CPContext m_Ctx) {
    String l_cFilter;
    l_cFilter = CPLib.ApplicationQueryFilter("armt_rapotit","rapotit",m_KeyColumns,m_Ctx);
    return l_cFilter;
  }
  ...
  public void SetFromContext(String s,java.util.Properties p) {
    ...
    if ( ! (CPLib.Empty(s))) {
      ...
      m_cWv_ardt_intestit = CPLib.GetProperty(p,"m_cWv_ardt_intestit",m_cWv_ardt_intestit,0);
  ...
  static void GetLinkDef0(Map<String,List<Map<String,String[]>>> tables) {
    ...
    items = tables.get("intestit");
    if (CPLib.IsNull(items)) {
      items = new ArrayList<Map<String,String[]>>();
      tables.put("intestit",items);
    }
    items.add(map);
    ...
    map.put("property.value",zeroSplit.split("intestit\u0000false\u0000false\u00001\u00000\u0000ardt_intestit\u0000false\u0000false\u0000\u0000\u0000\u0000false",-1));
  ...
  public static Map<String,List<Map<String,String[]>>> GetLinkDef() {
    ...
    List<Map<String,String[]>> items;
    GetLinkDef0(tables);
    for(String child : CPLib.LoadableMNTs("armt_rapotit",false)) {
      map = new HashMap<String,String[]>();
      ...
      String[][] definitions = CPMNTChild.GetM_Fkd("armt_rapotit",child);
  ...
  public void Initialize() {
    ...
    InitAutonumber();
    op_codazi = m_Ctx.GetCompany();
    ardt_intestit = new ardt_intestitBO(m_Ctx);
    m_MNTs = CPLib.MNTChilds("armt_rapotit",m_Ctx);
  ...
  public boolean Check() {
    ...
    if (l_bResult && (( ! (ardt_intestit.m_bLoaded) && CPLib.IsLoaded(m_cWv_ardt_intestit)) || ( ! (ardt_intestit.m_bUpdated) && CPLib.IsUpdated(m_cWv_ardt_intestit)))) {
      ardt_intestit.SetFromContext(m_cWv_ardt_intestit);
      m_cWv_ardt_intestit = "_not_empty_";
    }
    if (l_bResult && (ardt_intestit.m_bLoaded || ardt_intestit.m_bUpdated)) {
      l_bResult = l_bResult && ardt_intestit.Check();
  ...
  public String[] GetStateSecurityKeyRoles(String p_cState) {
    ...
    if (CPLib.eq(p_cState,CPSecurity.UNKNOWN_STATE)) {
      l_Keys = new String[]{"armt_rapotit"};
    }
    if (CPLib.eq(l_Keys.length,0)) {
      l_Keys = new String[]{"armt_rapotit\\"+p_cState};
    }
    return l_Keys;
  }
  ...
  public void GetContext(boolean p_bGetChildren,java.util.Properties p) {
    ...
    if (p_bGetChildren) {
      if (ardt_intestit.IsUpdated()) {
        m_cWv_ardt_intestit = ardt_intestit.GetContext();
      }
      CPLib.PutProperty(p,"m_cWv_ardt_intestit",m_cWv_ardt_intestit,0);
    }
    if (p_bGetChildren) {
  ...
  public boolean ChildExists_ardt_intestit() {
    if (ardt_intestit.IsUpdated() || (CPLib.IsUpdated(m_cWv_ardt_intestit) || CPLib.IsLoaded(m_cWv_ardt_intestit))) {
      return true;
    } else if (m_bLoaded &&  ! (ardt_intestit.m_bLoaded)) {
      ardt_intestit.Blank();
      ardt_intestit.w_CODCLI = w_CONNESCLI;
      ardt_intestit.Load();
    }
    return ardt_intestit.m_bLoaded;
  }
  public void CtxLoad_ardt_intestit() {
    if ( ! (ardt_intestit.m_lCtxLoaded)) {
      ardt_intestit.Blank();
    }
    ardt_intestit.m_lCtxLoaded = true;
    if ( ! (ardt_intestit.IsUpdated())) {
      if (CPLib.IsUpdated(m_cWv_ardt_intestit) || CPLib.IsLoaded(m_cWv_ardt_intestit)) {
        ardt_intestit.SetFromContext(m_cWv_ardt_intestit);
        m_cWv_ardt_intestit = "_not_empty_";
      } else {
        if (m_bLoaded &&  ! (ardt_intestit.m_bLoaded)) {
          ardt_intestit.Blank();
          ardt_intestit.w_CODCLI = w_CONNESCLI;
          ardt_intestit.Load();
          if ( ! (ardt_intestit.m_bLoaded)) {
            ardt_intestit.Initialize();
            ardt_intestit.SetUpdated();
          }
          CopyWorkVar(this,ardt_intestit,new String[]{"tipoprap"},new String[]{"TIPOOPRAP"},false);
        } else if ( ! (m_bLoaded)) {
          CopyWorkVar(this,ardt_intestit,new String[]{"tipoprap"},new String[]{"TIPOOPRAP"},true);
        }
      }
  ...
  CallerBRImpl GetChild(String p_cName) {
    if (CPLib.eq(p_cName,"ardt_intestit")) {
      CtxLoad_ardt_intestit();
      return ardt_intestit;
    }
