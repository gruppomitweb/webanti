...
// * --- Fine Area Manuale
public class arfn_chkspersR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_chkspersR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_chkspers",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      while ( ! (Cursor_wersonbo.Eof())) {
        /* If wersonbo->datamodi > pOpe */
        if (CPLib.gt(Cursor_wersonbo.GetDate("datamodi"),pOpe)) {
          /* cProg := wersonbo->sanumpro */
          cProg = Cursor_wersonbo.GetString("sanumpro");
        } else { // Else
          // Exit Loop
          if (true) {
            break;
  ...
  public String Run(String p_pCli,java.sql.Date p_pOpe) {
    pCli = p_pCli;
    pOpe = p_pOpe;
    return Run();
  }
  ...
  public static arfn_chkspersR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_chkspersR(p_Ctx, p_Caller);
  }
  public static arfn_chkspersR Make(CPContext p_Ctx) {
    return new arfn_chkspersR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
    pCli = CPLib.Space(16);
    pOpe = CPLib.NullDate();
