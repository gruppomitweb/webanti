...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('TGRCYWSFQA').value,'C'),w_ANNO)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('BTKJJRZUIH').value,'C'),w_SEMESTRE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('basea7',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'ANNO') &&  ! (Empty(HtW(Ctrl('TGRCYWSFQA').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.ANNO',HtW(Ctrl('TGRCYWSFQA').value,'C'),'C');
}
if (Ne(name,'SEMESTRE') &&  ! (Empty(HtW(Ctrl('BTKJJRZUIH').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.SEMESTRE',HtW(Ctrl('BTKJJRZUIH').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_detta7(firstAlias) {
return m_cVirtName+".ANNO = "+firstAlias+".ANNO and "+m_cVirtName+".SEMESTRE = "+firstAlias+".SEMESTRE ";
}
...
_chNotF(childAlias,'master','armt_basea7');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('basea7',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['TGRCYWSFQA','BTKJJRZUIH'],i=1;i>=0;i--){
