...
SetLocationHref(null,'arrt_chkdiana_old'+'?m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','elaborazione')
}
...
w_Messaggi.raiseEvent('arpg_chkdiana_old_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_chkdiana_old_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_chkdiana_old_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_chkdiana_old',m_cSelectedPage);
...
if (l_bResult) {
l_bResult=Le(w_DaData,CharToDate('20100531'));
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='DaData';
m_cLastMsgError=AlertErrorMessage('1387360972');
}
if ( ! (l_bResult)) {
w_DaData=HtW('','D');
}
...
if (l_bResult) {
l_bResult=Ge(w_AData,w_DaData) && Le(w_AData,CharToDate('20100531'));
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='AData';
m_cLastMsgError=AlertErrorMessage('1003562505');
}
if ( ! (l_bResult)) {
w_AData=HtW('','D');
}
...
l_bResult=false;
w_ragben=HtW('','C');
} else if (( ! (_ChkObl('HJLJFAMGNR',1))) || ( ! (Le(w_DaData,CharToDate('20100531'))))) {
_SignErr('HJLJFAMGNR','1387360972');
l_bResult=false;
w_DaData=HtW('','D');
} else if (( ! (_ChkObl('XWZHWTENET',1))) || ( ! (Ge(w_AData,w_DaData) && Le(w_AData,CharToDate('20100531'))))) {
_SignErr('XWZHWTENET','1003562505');
l_bResult=false;
w_AData=HtW('','D');
