...
var ctl = _GetCtl(e,'XMMPEEVCWS');
if (_tracker.goon(ctl,ctrlValue)) {
w_CODDIP=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('XMMPEEVCWS',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',6,0,'');
...
function Set_TIPOANALISI(ctrlValue,kmode,e) {
if (Ne(w_TIPOANALISI,ctrlValue)) {
var ctl = _GetCtl(e,'QWGGDAWZKO');
if (_tracker.goon(ctl,ctrlValue)) {
w_TIPOANALISI=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('QWGGDAWZKO',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',1,0,'');
...
var ctl = _GetCtl(e,'JUUWNCIHQE');
if (_tracker.goon(ctl,ctrlValue)) {
w_DATAINI=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('JUUWNCIHQE',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
function Set_DATFIN(ctrlValue,kmode,e) {
if (Ne(w_DATFIN,ctrlValue)) {
var ctl = _GetCtl(e,'BLDMLQOFYI');
if (_tracker.goon(ctl,ctrlValue)) {
w_DATFIN=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('BLDMLQOFYI',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH(NullDate(),'D',8,0,TranslatePicture(GetDatePicture(false,'D'),'D'));
...
var ctl = _GetCtl(e,'RNIHJWFMTF');
if (_tracker.goon(ctl,ctrlValue)) {
w_RISGLOB=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('RNIHJWFMTF',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',1,0,'');
...
if ( ! (m_oFather.SaveContext_ardt_rischiodip())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_rischiodip_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_rischiodip_dataNotSent',{'action':formaction});
}
}
}
}
...
if (Eq('JUUWNCIHQE_ZOOM',lc)) {
return 0;
}
if (Eq('BLDMLQOFYI_ZOOM',lc)) {
return 0;
}
return -1;
}
...
if (( ! (_ChkObl('QWGGDAWZKO',1))) && FullRow()) {
_SignErr('QWGGDAWZKO');
m_cLastWorkVarErrorMsg='1631071168';
l_bResult=false;
w_TIPOANALISI=HtW('','C');
} else if (( ! (_ChkObl('JUUWNCIHQE',1))) && FullRow()) {
_SignErr('JUUWNCIHQE');
m_cLastWorkVarErrorMsg='718958839';
l_bResult=false;
w_DATAINI=HtW('','D');
} else if (( ! (_ChkObl('BLDMLQOFYI',1))) && FullRow()) {
_SignErr('BLDMLQOFYI');
m_cLastWorkVarErrorMsg='10307319262';
l_bResult=false;
w_DATFIN=HtW('','D');
} else if (( ! (_ChkObl('RNIHJWFMTF',1))) && FullRow()) {
_SignErr('RNIHJWFMTF');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_rischiodip();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_TIPOANALISI));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.TIPOANALISI={"dec":0,"fixedpos":false,"id":"QWGGDAWZKO","len":1,"name":"TIPOANALISI","title":"Annuale, Semestrale, ecc.","type":"C"};
l_aRepeatedFields.DATAINI={"dec":0,"fixedpos":false,"id":"JUUWNCIHQE","len":8,"name":"DATAINI","title":"Data Inizio Analisi","type":"D"};
l_aRepeatedFields.DATFIN={"dec":0,"fixedpos":false,"id":"BLDMLQOFYI","len":8,"name":"DATFIN","title":"Data Fine Analisi","type":"D"};
l_aRepeatedFields.RISGLOB={"dec":0,"fixedpos":false,"id":"RNIHJWFMTF","len":1,"name":"RISGLOB","title":"Rischio","type":"C"};
...
if (enable) {
var c;
if(c=Ctrl('XMMPEEVCWS')) SetDisabled(c,true);
SetQueryRow(Ctrl('GridTable'),false);
} else {
SetEditRow(Ctrl('GridTable'),false);
var c;
if(c=Ctrl('XMMPEEVCWS')) SetDisabled(c,false);
