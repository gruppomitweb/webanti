...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('ATUKAEMOXS').value,'C'),w_TIPOINT)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbtipint',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'TIPOINT') &&  ! (Empty(HtW(Ctrl('ATUKAEMOXS').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.TIPOINT',HtW(Ctrl('ATUKAEMOXS').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tipinte');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbtipint',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['ATUKAEMOXS'],i=0;i>=0;i--){
