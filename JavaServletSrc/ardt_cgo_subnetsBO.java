...
// * --- Fine Area Manuale
public class ardt_cgo_subnetsBO extends ardt_cgo_subnetsBL {
  ...
  // * --- Fine Area Manuale
  public ardt_cgo_subnetsBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_cgo_subnets");
    }
  ...
  public static String[] m_cLoadParameterNames = {"SNCODINT"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_SNCODINT) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cgo_subnet,m_cPhName_cgo_subnet,m_cVirtName_cgo_subnet,m_cVirtName_cgo_subnet+".SNCODINT="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_SNCODINT,"C",11,0),m_cServer_cgo_subnet,p_SNCODINT),"");
  ...
  public boolean Delete(String p_SNCODINT) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cgo_subnet.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"SNCODINT="+CPLib.ToSQL(w_SNCODINT,"C",11,0)+" AND SNSUBNET="+CPLib.ToSQL(opk_SNSUBNET,"C",15,0);
        l_cSql = l_cSql+m_oWrInfo_cgo_subnet.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_subnet")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_subnet,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_subnet);
      l_cSql = "insert into "+m_oWrInfo_cgo_subnet.GetTableWriteName()+" (";
      l_cSql = l_cSql+"SNCODINT,";
      l_cSql = l_cSql+"SNSUBNET,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_subnet.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_subnet")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_SNCODINT,"C",11,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_SNSUBNET,"C",15,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_subnet.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_subnet")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_subnet,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_subnet);
      l_cSql = "Update "+m_oWrInfo_cgo_subnet.GetTableWriteName()+" set";
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_subnet")) {
        l_cSql = l_cSql+" CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", SNSUBNET = "+CPLib.ToSQL(w_SNSUBNET,"C",15,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"SNCODINT="+CPLib.ToSQL(w_SNCODINT,"C",11,0)+" AND SNSUBNET="+CPLib.ToSQL(opk_SNSUBNET,"C",15,0);
      l_cSql = l_cSql+m_oWrInfo_cgo_subnet.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_subnet")) {
