...
// * --- Fine Area Manuale
public class ardt_xocfraboBO extends ardt_xocfraboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_xocfraboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_xocfrabo");
    }
  ...
  public boolean Load(String p_NUMPROG) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xocfrabo,m_cPhName_xocfrabo,m_cVirtName_xocfrabo,m_cVirtName_xocfrabo+".NUMPROG="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_NUMPROG,"C",11,0),m_cServer_xocfrabo,p_NUMPROG),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xocfrabo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_xocfrabo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"NUMPROG="+CPLib.ToSQL(w_NUMPROG,"C",11,0)+" AND CONNESCLI="+CPLib.ToSQL(opk_CONNESCLI,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_xocfrabo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xocfrabo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_xocfrabo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xocfrabo);
      l_cSql = "insert into "+m_oWrInfo_xocfrabo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_xocfrabo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xocfrabo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_xocfrabo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xocfrabo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xocfrabo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xocfrabo);
      l_cSql = "Update "+m_oWrInfo_xocfrabo.GetTableWriteName()+" set";
