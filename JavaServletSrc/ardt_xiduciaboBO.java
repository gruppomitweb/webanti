...
// * --- Fine Area Manuale
public class ardt_xiduciaboBO extends ardt_xiduciaboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_xiduciaboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_xiduciabo");
    }
  ...
  public boolean Load(String p_RAPCOL) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xiduciabo,m_cPhName_xiduciabo,m_cVirtName_xiduciabo,m_cVirtName_xiduciabo+".RAPCOL="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPCOL,"C",25,0),m_cServer_xiduciabo,p_RAPCOL),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xiduciabo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_xiduciabo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPCOL="+CPLib.ToSQL(w_RAPCOL,"C",25,0)+" AND CODSOG="+CPLib.ToSQL(opk_CODSOG,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_xiduciabo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xiduciabo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_xiduciabo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xiduciabo);
      l_cSql = "insert into "+m_oWrInfo_xiduciabo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_xiduciabo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xiduciabo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_xiduciabo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xiduciabo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xiduciabo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xiduciabo);
      l_cSql = "Update "+m_oWrInfo_xiduciabo.GetTableWriteName()+" set";
