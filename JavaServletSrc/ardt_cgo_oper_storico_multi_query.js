...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('RWWIDDSYYG').value,'N'),w_CPROWNUM)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_operazioni_storico_multi',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
_chNotF(childAlias,'detail','ardt_cgo_oper_storico_multi');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_operazioni_storico_multi',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
