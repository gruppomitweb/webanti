...
public class ardt_fiduciabo extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*93f2ff22*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "fiduciabo";
    }
    public String w_RAPCOL;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_RAPCOL,context,"fiduciabo","RAPCOL","C",25,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_fiduciabo;
  ...
  public String getEntityTitle() {
    return "Fiducianti Rapporto (A.U.I.)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "ardt_fiduciabo";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 608;
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"fiduciabo","personbo"};
    ...
    l_eg.items = new SPItem[]{
                   new SPItem("IDFILEBO","C",10,0, "hide","IDBase Informazione"),
                   new SPItem("RAPCOL","C",25,0, "edit","Codice rapporto Collegato"),
                   new SPItem("NUMCOL","C",11,0, "hide","Codice Operazione"),
                   new SPItem("CODSOG","C",16,0, "edit","Codice Soggetto"),
                   new SPItem("TIPOFILE","C",1,0, "hide","Tipo FIle"),
                   new SPItem("LEGAME","C",1,0, "hide","Tipo Legame"),
                   new SPItem("UCODE","C",7,0, "hide","Codice Univoco"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_fiduciaboBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"10951166810",p_Context.Translate("IDBase Informazione")},
    {"1148763605",p_Context.Translate("Identificativo riga")},
    {"15186643",p_Context.Translate("Città")},
    {"1699714614",p_Context.Translate("Codice rapporto Collegato")},
    ...
    {"226977609",p_Context.Translate("Tipo FIle")},
    {"554236824",p_Context.Translate("Indirizzo")},
    {"832453450",p_Context.Translate("Fiducianti Rapporto (A.U.I.)")},
  ...
  void WriteStyles(ardt_fiduciaboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_fiduciabo.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_fiduciaboBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(
       "<style>" +
      "");
      status.out.println(".MPDPVTSMAO {top:1px;position:absolute;left:0px;height:16px;width:108px;background-color:Transparent;z-index:1; }");
      status.out.println(".MPDPVTSMAO_ZOOM { position:absolute;top:1px;left:109px;height:17px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".KBRBGGSVYB {top:0px;position:absolute;left:144px;height:16px;width:432px;background-color:Transparent;z-index:1; }");
      status.out.println(
       "</style>" +
      "");
    } else {
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_fiduciabo',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_fiduciabo",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_fiduciaboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_fiduciaboBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:274px;width:608px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(ardt_fiduciaboBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_TOP_OF_PAGE_1' style='position:relative;height:154px;display:block' page='2'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      status.out.println(
       "<div name id='GridTable_Header' class='GridTable_Header' style='position:absolute;top:6px;left:6px;height:19px;width:598px;z-index:0'>" +
       "</div>" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='RAPCOL' id='ZXGAEXNNNE' type='text' class='Field PrimaryKeyField "+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+"' value="+SPLib.ToHTMLValue(BO.w_RAPCOL,"C",25,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"readonly":"")+" maxlength='25' size='25' fieldtype='C' style='display:none'>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle PrimaryKeyHeaderLabel' id='RNFAUKYRCS_DIV'>" +
       "<label id='RNFAUKYRCS' for='MPDPVTSMAO'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("C51. Fiduciante"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:absolute;top:25px;left:6px;height:126px;width:598px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:126px;width:598px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:598px;height:126px;overflow:auto;'>" +
      ...
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:25px;left:143px;height:126px;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
      status.m_nTabIndex = status.m_nTabIndex+1;
  ...
  void HTMLRowScript(ardt_fiduciaboBO BO,ServletStatus status) {
    ...
    status.out.println("w_TIPOFILE=row[prefix+'TIPOFILE'];");
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     "<input name='CODSOG' id='MPDPVTSMAO' type='text' class='Field PrimaryKeyField "+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+" GridTable_EditRowItem ' value='\"+WtH(ToHTML(w_CODSOG),'C',16,0,'')+\"' tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='16' size='16' fieldtype='C'>" +
     "<button id='MPDPVTSMAO_ZOOM' type='button' class='GridZoomButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
     "</button>" +
    "\"");
    status.out.println("s=s+\""+
     "<input name='xragsoc' id='KBRBGGSVYB' type='text' class='Field Disabled GridTable_EditRowItem ' value='\"+WtH(ToHTML(w_xragsoc),'C',70,0,'')+\"' tabindex='-1' readonly maxlength='70' size='70' fieldtype='C'>" +
    "\"");
    ...
    status.out.println("s=s+\""+
     "<input type='text' class='Field PrimaryKeyField "+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+" Disabled GridTable_QueryRowItem MPDPVTSMAO' value='\"+WtH(ToHTML(w_CODSOG),'C',16,0,'')+\"' tabindex='-1' readonly size='16'>" +
     "<button type='button' class='MPDPVTSMAO_ZOOM GridZoomButtonEnabled' value="+SPLib.ToHTMLValue(status.context.Translate("..."),"C",0,0)+" disabled class='MPDPVTSMAO_ZOOM GridZoomButtonEnabled'>" +
     "</button>" +
    "\"");
    status.out.println("}");
    status.out.println("if ( ! (IsHiddenByStateDriver('xragsoc'))) {");
    status.out.println("s=s+\""+
     "<input type='text' class='Field Disabled Disabled GridTable_QueryRowItem KBRBGGSVYB' value='\"+WtH(ToHTML(w_xragsoc),'C',70,0,'')+\"' tabindex='-1' readonly size='70'>" +
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:592px;height:19px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b1(ardt_fiduciaboBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<div id='VRCVINYRJO' class='SectionBody' style='position:relative;height:53px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("box_VRCVINYRJO")) {
      ...
      if (status.HasZoneAltInterface("box_VRCVINYRJO@BEGIN") || status.HasZoneAltInterface("box_VRCVINYRJO@END")) {
        status.out.println(
         "<div id='VRCVINYRJO_boxContent' style='position:relative;height:53px;display:block'>" +
        "");
      }
      status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(ardt_fiduciaboBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("box_VRCVINYRJO")) {
      ...
      status.out.println("Ctrl('VRCVINYRJO').setAttribute('band_height',53);");
    }
  ...
  void WriteControls_p1_b2(ardt_fiduciaboBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_BEFORE_JUPHOBFSPW")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_BEFORE_JUPHOBFSPW' style='position:relative;height:3px;display:block' page='2'>" +
    "");
  ...
  void WriteControls_p1_b3(ardt_fiduciaboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b3_CloseBand(ardt_fiduciaboBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(ardt_fiduciaboBO BO,ServletStatus status) {
  ...
  static void PrintState(ardt_fiduciaboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_fiduciaboBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_fiduciaboBL BO,ServletStatus status) {
    status.out.print("DeclareWVs(");
    status.out.print(SPLib.ToJSValue(BO.w_IDFILEBO,"C",10,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_RAPCOL,"C",25,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_NUMCOL,"C",11,0));
  ...
  static void PrintStateDetailWorkVars(ardt_fiduciaboBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_fiduciaboBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_fiduciaboBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_fiduciaboBO.TrsRow) BO.m_oDeletedRows.get(k);
        l_cTrs = l_cTrs+" new TrsRow(";
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_CODSOG,"C",16,0)+",";
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_TIPOFILE,"C",1,0)+",";
  ...
  void WriteMainFormVarScript(ardt_fiduciaboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(608,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_fiduciabo"),"C",0,0)+ ";");
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      status.out.print("'MPDPVTSMAO':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash(entityGlobals.name+"MPDPVTSMAO",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,personbo,"+"default",l_session)+"'"+"]");
      AppendCallableIDS(status,",");
    } else {
      status.out.print("'MPDPVTSMAO':['','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,personbo,"+"default",l_session)+"'"+"]");
      AppendCallableIDS(status,",");
    }
    status.out.println("}");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_fiduciabo","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_fiduciabo",BO.GetCurrentState());
    ...
    status.out.println("this.TIPOFILE=w_TIPOFILE;");
    ...
    status.out.println("this.TIPOFILE=arguments[1];");
    status.out.println("this.LEGAME=arguments[2];");
    status.out.println("this.UCODE=arguments[3];");
    status.out.println("this.IDBASE=arguments[4];");
    status.out.println("this.xragsoc=arguments[5];");
    ...
    status.out.println("this.DOMICILIO=arguments[9];");
    status.out.println("this.opk_CODSOG=arguments[10];");
    ...
    status.out.println("w_TIPOFILE=row.TIPOFILE;");
  ...
  void WriteMainFormScript(ardt_fiduciaboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_fiduciabo);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_fiduciabo();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_fiduciabo',Ne(m_nChildStatus,0));");
    } else {
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('ZXGAEXNNNE')) SetDisabled(c,true);");
      } else if (( ! (CPLib.Empty(status.m_cAltInterface))) && CPLib.eq(status.m_cClientMode,"query")) {
        status.out.println("var c;");
        status.out.println("if(c=Ctrl('MPDPVTSMAO')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('MPDPVTSMAO_ZOOM')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('KBRBGGSVYB')) SetDisabled(c,true);");
      }
    ...
    if (CPLib.eq(status.m_cClientMode,"query") && CPLib.gt(status.m_nChildStatus,0)) {
      ...
      status.out.println("SetDisabled('ZXGAEXNNNE',m_bRowsChild);");
    }
    status.out.println("}");
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
    ...
    status.out.println("m_oFather.m_oardt_fiduciabo=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_fiduciaboBO getBO(ServletStatus status) {
    ardt_fiduciaboBO BO = new ardt_fiduciaboBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_fiduciaboBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("fiduciabo",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_fiduciaboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_fiduciaboBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ardt_fiduciaboBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_fiduciabo;
    String l_cPhName = BO.m_cPhName_fiduciabo;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_fiduciaboBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_fiduciabo+".RAPCOL"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_fiduciabo+".RAPCOL"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("fiduciabo");
    String l_cPhName = CPSql.ManipulateTablePhName("fiduciabo",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_fiduciaboBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_fiduciabo),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_fiduciabo,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_fiduciabo+".RAPCOL) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_fiduciabo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_fiduciabo,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_fiduciabo+".RAPCOL",CPLib.GetTablesWithAliases(BO.m_cPhName_fiduciabo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_fiduciaboBO BO) {
  ...
  void Edit(ServletStatus status,ardt_fiduciaboBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_fiduciaboBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_fiduciaboBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_fiduciaboBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_fiduciaboBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_fiduciaboBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_fiduciaboBO BO,SPParameterSource source) {
    BO.Load(status.w_RAPCOL);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_fiduciaboBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_fiduciaboBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_fiduciaboBO BO) {
    ...
    item.put("altInterfaceFor","ardt_fiduciabo");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"h\":16,\"name\":\"label_RNFAUKYRCS\",\"page\":1,\"spuid\":\"RNFAUKYRCS\",\"tabindex\":4,\"type\":\"Label\",\"value\":\"C51. Fiduciante\",\"w\":108,\"x\":8,\"y\":7,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":122,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"HNGEKHPDLP\",\"type\":\"BodyDetail\",\"w\":632,\"x\":8,\"y\":26,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_VRCVINYRJO\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"DATI ANAGRAFICI\",\"h\":53,\"name\":\"box_VRCVINYRJO\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VRCVINYRJO\",\"stretch\":true,\"tabindex\":11,\"title_caption\":\"DATI ANAGRAFICI\",\"titled\":true,\"type\":\"Box\",\"w\":600,\"x\":8,\"y\":154,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"fixed\":true,\"h\":15,\"name\":\"label_NAKGRMOUXS\",\"page\":1,\"spuid\":\"NAKGRMOUXS\",\"tabindex\":12,\"type\":\"Label\",\"w\":584,\"x\":16,\"y\":160,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"fixed\":true,\"h\":15,\"name\":\"label_HMPQLYVATL\",\"page\":1,\"spuid\":\"HMPQLYVATL\",\"tabindex\":13,\"type\":\"Label\",\"w\":584,\"x\":16,\"y\":181,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_JUPHOBFSPW\",\"box_type\":\"auto_horizontal_section\",\"collapsible\":true,\"h\":24,\"name\":\"box_JUPHOBFSPW\",\"page\":1,\"shrinkable\":true,\"spuid\":\"JUPHOBFSPW\",\"stretch\":true,\"tabindex\":14,\"type\":\"Box\",\"w\":600,\"x\":8,\"y\":210,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":40,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"GQQOINZPWR\",\"type\":\"Box\",\"w\":576,\"x\":8,\"y\":6,\"zone\":\"pag1_5\",\"zonepath\":\"[1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Ragione Sociale\",\"disabled\":\"true\",\"h\":17,\"maxlength\":70,\"name\":\"xragsoc\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"KBRBGGSVYB\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":432,\"x\":152,\"y\":28,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Codice Soggetto\",\"h\":17,\"maxlength\":16,\"name\":\"CODSOG\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"MPDPVTSMAO\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":108,\"x\":8,\"y\":29,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_fiduciaboBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_fiduciaboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_fiduciabo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_fiduciabo_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_fiduciabo_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_RAPCOL=a[1];");
      status.out.println("w_NUMCOL=a[2];");
      status.out.println("w_CODSOG=a[3];");
      status.out.println("w_TIPOFILE=a[4];");
      status.out.println("w_LEGAME=a[5];");
      status.out.println("w_UCODE=a[6];");
      status.out.println("w_IDBASE=a[7];");
      status.out.println("w_NUMIMP=a[8];");
      status.out.println("w_xragsoc=a[9];");
      status.out.println("w_PROVINCIA=a[10];");
      status.out.println("w_CAP=a[11];");
      status.out.println("w_DESCCIT=a[12];");
      status.out.println("w_DOMICILIO=a[13];");
      status.out.println("m_PrimaryKeys=['CODSOG','RAPCOL'];");
      status.out.println("}");
      status.out.println(
       "</script>" +
      "");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_fiduciabo_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","fiduciabo");
  ...
  void RaiseLoadedEntity(ardt_fiduciaboBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_fiduciaboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_fiduciaboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_fiduciaboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_fiduciaboBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_fiduciaboBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_fiduciaboBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ardt_fiduciaboBO BO,String p_cUID) {
    ...
    } else if (CPLib.eq(p_cUID,"MPDPVTSMAO")) {
      SPLib.FillLinkHash(hCreator,BO.GetLinkProperties(p_cUID),BO);
    }
    return GetEntityHash(p_cUID,status,hCreator);
  }
  public boolean CheckVars(ServletStatus status,ardt_fiduciaboBO BO) {
    boolean result = true;
      ...
      while (CPLib.lt(l_nRow,BO.Rows())) {
        ...
        if (BO.IsRowUpdated()) {
          ...
          if (result &&  ! (CPLib.Empty(BO.w_CODSOG))) {
            a = GetCheckVars(status,BO,"MPDPVTSMAO");
            b = _row.optString("MPDPVTSMAO");
            result = CPLib.eq(a,b);
          }
        }
        l_nRow = l_nRow+1;
