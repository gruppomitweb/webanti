...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('WXGOCNXMMY').value,'C'),w_CODSTA)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('mtstati',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODSTA') &&  ! (Empty(HtW(Ctrl('WXGOCNXMMY').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODSTA',HtW(Ctrl('WXGOCNXMMY').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_mtstati');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('mtstati',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
