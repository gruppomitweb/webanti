...
public class armt_operazbo_agg extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*3e3e03f6*!";
  static public class ServletStatus extends SPMasterChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "operazbo_agg";
    }
    public String w_IDBASE;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_IDBASE,context,"operazbo_agg","IDBASE","C",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    armt_operazbo_aggBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_operazbo_agg;
  ...
  public String getEntityTitle() {
    return "Dati Aggiuntivi Operazioni";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_operazbo_agg";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 792;
    l_eg.m_nEntityHeight = 361;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"operazbo_agg","tbcateg01","tbcateg02","tbcateg03","tbcateg04","tbcateg05"};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("SEGNO","C",1,0, "hide","segno dell'operazione"),
                   ...
                   new SPItem("CATEG04","C",2,0, "edit","Categoria 4"),
                   new SPItem("CATEG05","C",2,0, "edit","Categoria 5"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_operazbo_aggBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1068089849",p_Context.Translate("segno dell'operazione")},
    {"1118764577",p_Context.Translate("Mezzo di Pagamento")},
    {"11263356912",p_Context.Translate("SV58726 Modalità PISP")},
    {"11391439014",p_Context.Translate("Dati Aggiuntivi Operazioni")},
  ...
  void WriteStyles(armt_operazbo_aggBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_operazbo_agg.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_operazbo_aggBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_operazbo_agg',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_operazbo_agg",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(armt_operazbo_aggBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(armt_operazbo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:361px;width:792px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(armt_operazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_operazbo_aggBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("Categorie")) {
      ...
      status.out.println(
       "<input name='CATEG01' id='CANUTVNGFD' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_CATEG01,"C",2,0,"", status.m_oRegSettings)+" title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 1"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='2' size='2' fieldtype='C'>" +
       "<button id='CANUTVNGFD_ZOOM' type='button' class='ZoomButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 1"),"C",0,0)+" tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"view") || CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
       "</button>" +
       ""+EventuallyContextMenu(status, "CANUTVNGFD", "tbcateg01", "opemt_tbcateg01", "")+"" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='CATEG02' id='IXGRJLAKIQ' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_CATEG02,"C",2,0,"", status.m_oRegSettings)+" title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 2"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='2' size='2' fieldtype='C'>" +
       "<button id='IXGRJLAKIQ_ZOOM' type='button' class='ZoomButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 2"),"C",0,0)+" tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"view") || CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
       "</button>" +
       ""+EventuallyContextMenu(status, "IXGRJLAKIQ", "tbcateg02", "opemt_tbcateg02", "")+"" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='CATEG03' id='NKIXCWZJHO' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_CATEG03,"C",2,0,"", status.m_oRegSettings)+" title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 3"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='2' size='2' fieldtype='C'>" +
       "<button id='NKIXCWZJHO_ZOOM' type='button' class='ZoomButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 3"),"C",0,0)+" tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"view") || CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
       "</button>" +
       ""+EventuallyContextMenu(status, "NKIXCWZJHO", "tbcateg03", "opemt_tbcateg03", "")+"" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='CATEG04' id='JQLGHMLOKO' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_CATEG04,"C",2,0,"", status.m_oRegSettings)+" title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 4"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='2' size='2' fieldtype='C'>" +
       "<button id='JQLGHMLOKO_ZOOM' type='button' class='ZoomButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 4"),"C",0,0)+" tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"view") || CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
       "</button>" +
       ""+EventuallyContextMenu(status, "JQLGHMLOKO", "tbcateg04", "opemt_tbcateg04", "")+"" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='CATEG05' id='QVBZSFMTVP' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_CATEG05,"C",2,0,"", status.m_oRegSettings)+" title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 5"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='2' size='2' fieldtype='C'>" +
       "<button id='QVBZSFMTVP_ZOOM' type='button' class='ZoomButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' title="+SPLib.ToHTMLValue(status.context.Translate("Categoria 5"),"C",0,0)+" tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"view") || CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
       "</button>" +
  ...
  void WriteControls_p1_b1_CloseBand(armt_operazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_operazbo_aggBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_BEFORE_JANROHXZBL")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_BEFORE_JANROHXZBL' style='position:relative;height:2px;display:block' page='1'>" +
    "");
  ...
  void WriteControls_p1_b3(armt_operazbo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<div name='sv' id='JANROHXZBL_DIV' class='BoxContainer SPSection_expanded SectionContainerWithoutTitle' style='display:none'>" +
    "");
    status.out.println(
     "<div id='JANROHXZBL' class='SectionBody' style='position:relative;height:90px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("sv")) {
      ...
      if (status.HasZoneAltInterface("sv@BEGIN") || status.HasZoneAltInterface("sv@END")) {
        status.out.println(
         "<div id='JANROHXZBL_boxContent' style='position:relative;height:90px;display:block'>" +
        "");
      }
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<span title="+SPLib.ToHTMLValue(status.context.Translate("Operazione Fraudolenta"),"C",0,0)+" id='QKZHBGTGRL_DIV'>" +
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<select name='SV58724' id='BJYOWOAAQH' class='Combobox' title="+SPLib.ToHTMLValue(status.context.Translate("SV 58724 Modalità Bonifici"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"'>" +
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<span title="+SPLib.ToHTMLValue(status.context.Translate("Autorizzazione Forte (S/N)"),"C",0,0)+" id='AKMFIOWCBF_DIV'>" +
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<select name='SV58726SA' id='WUEMBLXMFM' class='Combobox' title="+SPLib.ToHTMLValue(status.context.Translate("SV58726  Schema di addebito"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"'>" +
       "<option value="+SPLib.ToHTMLValue("","C",0,0)+">" +
       "</option>" +
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<select name='SV58726PISP' id='NODGYYBWWX' class='Combobox' title="+SPLib.ToHTMLValue(status.context.Translate("SV58726 Modalità PISP"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"'>" +
       "<option value="+SPLib.ToHTMLValue("","C",0,0)+">" +
       "</option>" +
      ...
      "");
      status.out.println(
       "<span class='SPLabel' id='XVUIFQMEIG_DIV'>" +
      ...
      "");
      status.out.println(
       "<span class='SPLabel' id='WFICGFEUAH_DIV'>" +
      ...
      "");
      status.out.println(
       "<span class='SPLabel' id='VIBYCNNDWC_DIV'>" +
  ...
  void WriteControls_p1_b3_CloseBand(armt_operazbo_aggBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("sv")) {
      ...
      status.out.println("Ctrl('JANROHXZBL').setAttribute('band_height',90);");
    }
  ...
  void WriteControls_p1_b4(armt_operazbo_aggBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_BEFORE_STHOKLKHLK")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_BEFORE_STHOKLKHLK' style='position:relative;height:6px;display:block' page='1'>" +
    "");
  ...
  void WriteControls_p1_b5(armt_operazbo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<div name='SOS' id='STHOKLKHLK_DIV' class='BoxContainer SPSection_expanded SectionContainerWithTitle' style='display:none'>" +
    "");
  ...
  void WriteControls_p1_b5_CloseBand(armt_operazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b6(armt_operazbo_aggBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_operazbo_aggBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_operazbo_aggBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(armt_operazbo_aggBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(armt_operazbo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(361,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_operazbo_agg"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_operazbo_agg","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_operazbo_agg",BO.GetCurrentState());
  ...
  void WriteMainFormScript(armt_operazbo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_armt_operazbo_agg);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_armt_operazbo_agg();");
      status.out.println("}");
    }
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_operazbo_agg',Ne(m_nChildStatus,0));");
    } else {
    ...
    status.out.println("// * --- armt_operazbo_agg");
    ...
    status.out.println("m_oFather.m_oarmt_operazbo_agg=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  armt_operazbo_aggBO getBO(ServletStatus status) {
    armt_operazbo_aggBO BO = new armt_operazbo_aggBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(armt_operazbo_aggBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("operazbo_agg",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_operazbo_aggBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,armt_operazbo_aggBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(armt_operazbo_aggBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_operazbo_agg;
    String l_cPhName = BO.m_cPhName_operazbo_agg;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_operazbo_aggBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_operazbo_agg+".IDBASE",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("operazbo_agg");
    String l_cPhName = CPSql.ManipulateTablePhName("operazbo_agg",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,armt_operazbo_aggBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_operazbo_agg,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_operazbo_agg,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,armt_operazbo_aggBO BO) {
  ...
  void Edit(ServletStatus status,armt_operazbo_aggBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_operazbo_aggBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_operazbo_aggBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_operazbo_aggBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_operazbo_aggBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_operazbo_aggBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_operazbo_aggBO BO,SPParameterSource source) {
    BO.Load(status.w_IDBASE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_operazbo_aggBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_operazbo_aggBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_operazbo_aggBO BO) {
    ...
    item.put("altInterfaceFor","armt_operazbo_agg");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Categorie\",\"box_type\":\"auto_horizontal_section\",\"collapsible\":true,\"comment\":\"Categorie\",\"h\":126,\"name\":\"Categorie\",\"page\":1,\"shrinkable\":true,\"spuid\":\"RFAIYHSGII\",\"stretch\":true,\"tabindex\":3,\"title_caption\":\"Categorie\",\"type\":\"Box\",\"w\":792,\"x\":0,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NRWNAJJVRA\",\"page\":1,\"spuid\":\"NRWNAJJVRA\",\"tabindex\":21,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":11,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_WWRYOVTMFY\",\"page\":1,\"spuid\":\"WWRYOVTMFY\",\"tabindex\":22,\"type\":\"Label\",\"value\":\"Categoria 1:\",\"w\":84,\"x\":8,\"y\":11,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_LNOFOPAOYD\",\"page\":1,\"spuid\":\"LNOFOPAOYD\",\"tabindex\":23,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":35,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_HNQZARYXGS\",\"page\":1,\"spuid\":\"HNQZARYXGS\",\"tabindex\":24,\"type\":\"Label\",\"value\":\"Categoria 2:\",\"w\":84,\"x\":8,\"y\":35,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_WBTKNDJDFF\",\"page\":1,\"spuid\":\"WBTKNDJDFF\",\"tabindex\":25,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":59,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BGEVFUGGOS\",\"page\":1,\"spuid\":\"BGEVFUGGOS\",\"tabindex\":26,\"type\":\"Label\",\"value\":\"Categoria 3:\",\"w\":84,\"x\":8,\"y\":59,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Categoria 4\",\"h\":20,\"maxlength\":2,\"name\":\"CATEG04\",\"page\":1,\"picker\":true,\"spuid\":\"JQLGHMLOKO\",\"tabindex\":8,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":32,\"x\":96,\"y\":79,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_MWCFKQVNHF\",\"page\":1,\"spuid\":\"MWCFKQVNHF\",\"tabindex\":27,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":83,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_RHXCIUHRTU\",\"page\":1,\"spuid\":\"RHXCIUHRTU\",\"tabindex\":28,\"type\":\"Label\",\"value\":\"Categoria 4:\",\"w\":84,\"x\":8,\"y\":83,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Categoria 5\",\"h\":20,\"maxlength\":2,\"name\":\"CATEG05\",\"page\":1,\"picker\":true,\"spuid\":\"QVBZSFMTVP\",\"tabindex\":9,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":32,\"x\":96,\"y\":103,\"zone\":\"pag1_1_5\",\"zonepath\":\"[1,1,5]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HADVHTQAPE\",\"page\":1,\"spuid\":\"HADVHTQAPE\",\"tabindex\":29,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":107,\"zone\":\"pag1_1_5\",\"zonepath\":\"[1,1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_GRCYFEKJWU\",\"page\":1,\"spuid\":\"GRCYFEKJWU\",\"tabindex\":30,\"type\":\"Label\",\"value\":\"Categoria 5:\",\"w\":84,\"x\":8,\"y\":107,\"zone\":\"pag1_1_5\",\"zonepath\":\"[1,1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"sv\",\"box_type\":\"auto_horizontal_section\",\"collapsible\":true,\"h\":90,\"name\":\"sv\",\"page\":1,\"shrinkable\":true,\"spuid\":\"JANROHXZBL\",\"stretch\":true,\"tabindex\":4,\"type\":\"Box\",\"w\":792,\"x\":0,\"y\":128,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\"1\",\"comment\":\"Operazione Fraudolenta\",\"h\":21,\"label_text\":\"Operazione Fraudolenta\",\"maxlength\":2,\"name\":\"FLAGFRAUD\",\"page\":1,\"spuid\":\"QKZHBGTGRL\",\"tabindex\":16,\"type\":\"Checkbox\",\"typevar\":\"numeric\",\"unchecked_value\":\"0\",\"w\":208,\"x\":96,\"y\":136,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"SV 58724 Modalit\\u00E0 Bonifici\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":2,\"name\":\"SV58724\",\"page\":1,\"spuid\":\"BJYOWOAAQH\",\"tabindex\":17,\"textlist\":\"On Line - E-commerce,On Line - Altro,ATM,Mobile - P2P,Mobile - C2P,Mobile - Altro\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"01"+"'"+","+"'"+"02"+"'"+","+"'"+"03"+"'"+","+"'"+"04"+"'"+","+"'"+"05"+"'"+","+"'"+"06"+"'"+"\",\"w\":272,\"x\":504,\"y\":135,\"zone\":\"pag1_2_2\",\"zonepath\":\"[1,2,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_XVUIFQMEIG\",\"page\":1,\"spuid\":\"XVUIFQMEIG\",\"tabindex\":31,\"type\":\"Label\",\"value\":\"SV58724 Modalit\\u00E0 Bonifici:\",\"w\":166,\"x\":336,\"y\":139,\"zone\":\"pag1_2_2\",\"zonepath\":\"[1,2,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Autorizzazione Forte (S\\/N)\",\"h\":21,\"label_text\":\"SV58726 Autorizzazione Forte\",\"maxlength\":1,\"name\":\"SV58726SCA\",\"page\":1,\"spuid\":\"AKMFIOWCBF\",\"tabindex\":18,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":208,\"x\":96,\"y\":162,\"zone\":\"pag1_2_3\",\"zonepath\":\"[1,2,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"SV58726  Schema di addebito\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":3,\"name\":\"SV58726SA\",\"page\":1,\"spuid\":\"WUEMBLXMFM\",\"tabindex\":19,\"textlist\":\"SEPA CT Scheme,SEPA CT Instant Scheme,ON-US (CT),Non disponibile (CT)\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"396"+"'"+","+"'"+"397"+"'"+","+"'"+"398"+"'"+","+"'"+"399"+"'"+"\",\"w\":272,\"x\":504,\"y\":161,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_WFICGFEUAH\",\"page\":1,\"spuid\":\"WFICGFEUAH\",\"tabindex\":32,\"type\":\"Label\",\"value\":\"SV58726  Schema di addebito:\",\"w\":166,\"x\":336,\"y\":165,\"zone\":\"pag1_2_4\",\"zonepath\":\"[1,2,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"SV58726 Modalit\\u00E0 PISP\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":3,\"name\":\"SV58726PISP\",\"page\":1,\"spuid\":\"NODGYYBWWX\",\"tabindex\":20,\"textlist\":\"Bonifici disposti da PISP,Bonifici non disposti da PISP\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"395"+"'"+","+"'"+"396"+"'"+"\",\"w\":272,\"x\":504,\"y\":189,\"zone\":\"pag1_2_5\",\"zonepath\":\"[1,2,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VIBYCNNDWC\",\"page\":1,\"spuid\":\"VIBYCNNDWC\",\"tabindex\":33,\"type\":\"Label\",\"value\":\"SV58726 Modalit\\u00E0 PISP:\",\"w\":166,\"x\":336,\"y\":193,\"zone\":\"pag1_2_5\",\"zonepath\":\"[1,2,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"SOS\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"SOS\",\"h\":77,\"name\":\"SOS\",\"page\":1,\"shrinkable\":true,\"spuid\":\"STHOKLKHLK\",\"stretch\":true,\"tabindex\":34,\"title_caption\":\"SOS\",\"titled\":true,\"type\":\"Box\",\"w\":792,\"x\":0,\"y\":224,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Cognome Beneficiario per SOS\",\"h\":20,\"maxlength\":50,\"name\":\"C_COGNOME\",\"page\":1,\"spuid\":\"LUQZFCFSYI\",\"tabindex\":35,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":312,\"x\":192,\"y\":238,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_FVAZAUXFYY\",\"page\":1,\"spuid\":\"FVAZAUXFYY\",\"tabindex\":36,\"type\":\"Label\",\"value\":\"Cognome Beneficiario per SOS:\",\"w\":184,\"x\":8,\"y\":242,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nome Beneficiario per SOS\",\"h\":20,\"maxlength\":30,\"name\":\"C_NOME\",\"page\":1,\"spuid\":\"DXUKTRNNCS\",\"tabindex\":37,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":192,\"x\":192,\"y\":266,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_EYRDOOPCVW\",\"page\":1,\"spuid\":\"EYRDOOPCVW\",\"tabindex\":38,\"type\":\"Label\",\"value\":\"Nome Beneficiario per SOS:\",\"w\":184,\"x\":8,\"y\":270,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Mezzo di Pagamento\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"SOSPAGAM\",\"page\":1,\"spuid\":\"CIGEGYZWMZ\",\"tabindex\":39,\"textlist\":\"Contante,Bonifico,Carta di pagamento,Altro\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"S"+"'"+","+"'"+"B"+"'"+","+"'"+"C"+"'"+","+"'"+"A"+"'"+"\",\"w\":272,\"x\":504,\"y\":266,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VAWAJFPLKZ\",\"page\":1,\"spuid\":\"VAWAJFPLKZ\",\"tabindex\":40,\"type\":\"Label\",\"value\":\"Mezzo di Pagamento:\",\"w\":120,\"x\":384,\"y\":270,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_operazbo_aggBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_operazbo_agg?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_operazbo_agg_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_operazbo_agg_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_operazbo_agg_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(361,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","operazbo_agg");
  ...
  void RaiseLoadedEntity(armt_operazbo_aggBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_operazbo_aggBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_operazbo_aggBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_operazbo_aggBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_operazbo_aggBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_operazbo_aggBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_operazbo_aggBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,armt_operazbo_aggBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_operazbo_aggBO BO) {
    boolean result = true;
