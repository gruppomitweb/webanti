...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('EHBBFZYKCC').value,'C'),w_TCCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbtipconcessioni',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'TCCODICE') &&  ! (Empty(HtW(Ctrl('EHBBFZYKCC').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.TCCODICE',HtW(Ctrl('EHBBFZYKCC').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tbtipconcessioni');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbtipconcessioni',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['EHBBFZYKCC'],i=0;i>=0;i--){
