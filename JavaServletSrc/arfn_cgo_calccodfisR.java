...
// * --- Fine Area Manuale
public class arfn_cgo_calccodfisR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_cgo_calccodfisR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_cgo_calccodfis",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    ...
    if (CPLib.eqr("IDCITTA",p_cVarName)) {
      return w_IDCITTA;
    }
  ...
  void Page_1() throws Exception {
    ...
    CPResultSet Cursor_tbcittna=null;
    try {
      ...
      /* w_NASSTATO Char(30) // Luogo di Nascita */
      /* w_SESSO Char(1) // Sesso */
      /* w_TIPINTER Char(2) // Provincia di nascita */
      /* w_CF Char(16) // Codice Fiscale Presentato */
      /* w_IDCITTA Char(40) // ID Città Nascita */
      /* Mese Char(1) */
      /* Giorno Char(2) */
      /* Anno Char(2) */
      ...
      Page_4();
      /* If NumberUtils.isDigits(Substr(w_CF,13,3)) */
      if (NumberUtils.isDigits(CPLib.Substr(w_CF,13,3))) {
        /* If Upper(LRTrim(w_TIPINTER))='EE' */
        if (CPLib.eqr(CPLib.Upper(CPLib.LRTrim(w_TIPINTER)),"EE")) {
          /* cSigla := "" */
          cSigla = "";
          ...
          Cursor_tbstatna = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select SIGLA  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"DESCRI="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.LRTrim(w_NASSTATO),"?",0,0,m_cServer, m_oParameters),m_cServer,CPLib.LRTrim(w_NASSTATO))+" "+")"+(m_Ctx.IsSharedTemp("tbstatna")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
          while ( ! (Cursor_tbstatna.Eof())) {
            /* bReturn := Left(w_CF,3)=cCog and Substr(w_CF,4,3)=cNom and Substr(w_CF,7,2)=Anno and Substr(w_CF,9,1)=Mese and Substr(w_CF,10,2)=Giorno and Substr(w_CF,12,4)=tbstatna->SIGLA */
            bReturn = CPLib.eqr(CPLib.Left(w_CF,3),cCog) && CPLib.eqr(CPLib.Substr(w_CF,4,3),cNom) && CPLib.eqr(CPLib.Substr(w_CF,7,2),Anno) && CPLib.eqr(CPLib.Substr(w_CF,9,1),Mese) && CPLib.eqr(CPLib.Substr(w_CF,10,2),Giorno) && CPLib.eqr(CPLib.Substr(w_CF,12,4),Cursor_tbstatna.GetString("SIGLA"));
            /* If bReturn */
            if (bReturn) {
              // Exit Loop
              if (true) {
          ...
          /* If not(Empty(w_IDCITTA)) */
          if ( ! (CPLib.Empty(w_IDCITTA))) {
            // * --- Read from tbcittna
            m_cServer = m_Ctx.GetServer("tbcittna");
            m_cPhName = m_Ctx.GetPhName("tbcittna");
            m_cSql = "";
            m_cSql = m_cSql+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_IDCITTA,"C",40,0,m_cServer),m_cServer,w_IDCITTA);
            ...
            if ( ! (Read_Cursor.Eof())) {
              ...
              Read_Cursor.Next();
              if ( ! (Read_Cursor.Eof())) {
                Error l_oErrorFault = new Error("Read on tbcittna into routine arfn_cgo_calccodfis returns two or more records. This item should return only a record.");
            ...
            /* bReturn := Left(w_CF,3)=cCog and Substr(w_CF,4,3)=cNom and Substr(w_CF,7,2)=Anno and Substr(w_CF,9,1)=Mese and Substr(w_CF,10,2)=Giorno and Substr(w_CF,12,4)=cSigla */
            bReturn = CPLib.eqr(CPLib.Left(w_CF,3),cCog) && CPLib.eqr(CPLib.Substr(w_CF,4,3),cNom) && CPLib.eqr(CPLib.Substr(w_CF,7,2),Anno) && CPLib.eqr(CPLib.Substr(w_CF,9,1),Mese) && CPLib.eqr(CPLib.Substr(w_CF,10,2),Giorno) && CPLib.eqr(CPLib.Substr(w_CF,12,4),cSigla);
          } else { // Else
            // * --- Select from tbcittna
            m_cServer = m_Ctx.GetServer("tbcittna");
            m_cPhName = m_Ctx.GetPhName("tbcittna");
            if (Cursor_tbcittna!=null)
              Cursor_tbcittna.Close();
            Cursor_tbcittna = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select CITTA,PROV,SIGLA  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+(m_Ctx.IsSharedTemp("tbcittna")?"context_id='"+m_Ctx.GetTempID()+"'":"1=1"),m_cServer,m_Ctx,true));
            while ( ! (Cursor_tbcittna.Eof())) {
              /* If LRTrim(Left(tbcittna->CITTA,30))=Upper(LRTrim(w_NASCOMUN)) and tbcittna->PROV=w_TIPINTER */
              if (CPLib.eqr(CPLib.LRTrim(CPLib.Left(Cursor_tbcittna.GetString("CITTA"),30)),CPLib.Upper(CPLib.LRTrim(w_NASCOMUN))) && CPLib.eqr(Cursor_tbcittna.GetString("PROV"),w_TIPINTER)) {
                /* bReturn := Left(w_CF,3)=cCog and Substr(w_CF,4,3)=cNom and Substr(w_CF,7,2)=Anno and Substr(w_CF,9,1)=Mese and Substr(w_CF,10,2)=Giorno and Substr(w_CF,12,4)=tbcittna->SIGLA */
                bReturn = CPLib.eqr(CPLib.Left(w_CF,3),cCog) && CPLib.eqr(CPLib.Substr(w_CF,4,3),cNom) && CPLib.eqr(CPLib.Substr(w_CF,7,2),Anno) && CPLib.eqr(CPLib.Substr(w_CF,9,1),Mese) && CPLib.eqr(CPLib.Substr(w_CF,10,2),Giorno) && CPLib.eqr(CPLib.Substr(w_CF,12,4),Cursor_tbcittna.GetString("SIGLA"));
                /* If bReturn */
                if (bReturn) {
                  // Exit Loop
                  if (true) {
              ...
              Cursor_tbcittna.Next();
            }
            m_cConnectivityError = Cursor_tbcittna.ErrorMessage();
            Cursor_tbcittna.Close();
            // * --- End Select
          } // End If
        } // End If
      } else { // Else
        /* bReturn := Left(w_CF,3)=cCog and Substr(w_CF,4,3)=cNom and Substr(w_CF,7,2)=Anno and Substr(w_CF,9,1)=Mese and Substr(w_CF,10,2)=Giorno */
        bReturn = CPLib.eqr(CPLib.Left(w_CF,3),cCog) && CPLib.eqr(CPLib.Substr(w_CF,4,3),cNom) && CPLib.eqr(CPLib.Substr(w_CF,7,2),Anno) && CPLib.eqr(CPLib.Substr(w_CF,9,1),Mese) && CPLib.eqr(CPLib.Substr(w_CF,10,2),Giorno);
      } // End If
      /* Return bReturn */
  ...
  void Page_2() throws Exception {
    ...
    /* Giorno := iif(w_SESSO='M',Right(cData,2),Str(Day(w_DATANASC) + 40,2)) */
    Giorno = (CPLib.eqr(w_SESSO,"M")?CPLib.Right(cData,2):CPLib.Str(CPLib.Day(w_DATANASC)+40,2));
    /* Variabile anno */
  ...
  void Page_3() throws Exception {
    ...
    while (CPLib.le(c1,CPLib.Len(CPLib.LRTrim(cognome)))) {
      /* If Substr(cognome,c1,1) <> 'A' and Substr(cognome,c1,1) <> 'E' and Substr(cognome,c1,1) <> 'I' and Substr(cognome,c1,1) <> 'O' and Substr(cognome,c1,1) <> 'U' and Substr(cognome,c1,1) <> ' ' and Substr(cognome,c1,1) <> "'" */
      if (CPLib.ne(CPLib.Substr(cognome,c1,1),"A") && CPLib.ne(CPLib.Substr(cognome,c1,1),"E") && CPLib.ne(CPLib.Substr(cognome,c1,1),"I") && CPLib.ne(CPLib.Substr(cognome,c1,1),"O") && CPLib.ne(CPLib.Substr(cognome,c1,1),"U") && CPLib.ne(CPLib.Substr(cognome,c1,1)," ") && CPLib.ne(CPLib.Substr(cognome,c1,1),"'")) {
        /* If c2 < 3 */
        if (CPLib.lt(c2,3)) {
    ...
    if (CPLib.lt(c2,3)) {
      ...
      /* While c1 <= Len(LRTrim(cognome)) */
      while (CPLib.le(c1,CPLib.Len(CPLib.LRTrim(cognome)))) {
        /* If (Substr(cognome,c1,1) = 'A' or Substr(cognome,c1,1) = 'E' or Substr(cognome,c1,1) = 'I' or Substr(cognome,c1,1) = 'O' or Substr(cognome,c1,1) = 'U') and Substr(cognome,c1,1) <> ' ' and Substr(cognome,c1,1) <> "'" */
        if ((CPLib.eqr(CPLib.Substr(cognome,c1,1),"A") || CPLib.eqr(CPLib.Substr(cognome,c1,1),"E") || CPLib.eqr(CPLib.Substr(cognome,c1,1),"I") || CPLib.eqr(CPLib.Substr(cognome,c1,1),"O") || CPLib.eqr(CPLib.Substr(cognome,c1,1),"U")) && CPLib.ne(CPLib.Substr(cognome,c1,1)," ") && CPLib.ne(CPLib.Substr(cognome,c1,1),"'")) {
          /* If c2 < 3 */
          if (CPLib.lt(c2,3)) {
            /* cCog := cCog + Substr(cognome,c1,1) // parte del Cognome per CF */
            cCog = cCog+CPLib.Substr(cognome,c1,1);
  ...
  void Page_4() throws Exception {
    ...
    while (CPLib.le(c1,CPLib.Len(CPLib.LRTrim(nome)))) {
      /* If Substr(nome,c1,1) <> 'A' and Substr(nome,c1,1) <> 'E' and Substr(nome,c1,1) <> 'I' and Substr(nome,c1,1) <> 'O' and Substr(nome,c1,1) <> 'U' and Substr(nome,c1,1) <> ' ' and Substr(nome,c1,1) <> "'" */
      if (CPLib.ne(CPLib.Substr(nome,c1,1),"A") && CPLib.ne(CPLib.Substr(nome,c1,1),"E") && CPLib.ne(CPLib.Substr(nome,c1,1),"I") && CPLib.ne(CPLib.Substr(nome,c1,1),"O") && CPLib.ne(CPLib.Substr(nome,c1,1),"U") && CPLib.ne(CPLib.Substr(nome,c1,1)," ") && CPLib.ne(CPLib.Substr(nome,c1,1),"'")) {
        /* If c2 < 4 */
        if (CPLib.lt(c2,4)) {
    ...
    if (CPLib.lt(c2,3)) {
      ...
      /* While c1 <= Len(LRTrim(nome)) */
      while (CPLib.le(c1,CPLib.Len(CPLib.LRTrim(nome)))) {
        /* If (Substr(nome,c1,1) = 'A' or Substr(nome,c1,1) = 'E' or Substr(nome,c1,1) = 'I' or Substr(nome,c1,1) = 'O' or Substr(nome,c1,1) = 'U') and Substr(nome,c1,1) <> ' ' and Substr(nome,c1,1) <> "'" */
        if ((CPLib.eqr(CPLib.Substr(nome,c1,1),"A") || CPLib.eqr(CPLib.Substr(nome,c1,1),"E") || CPLib.eqr(CPLib.Substr(nome,c1,1),"I") || CPLib.eqr(CPLib.Substr(nome,c1,1),"O") || CPLib.eqr(CPLib.Substr(nome,c1,1),"U")) && CPLib.ne(CPLib.Substr(nome,c1,1)," ") && CPLib.ne(CPLib.Substr(nome,c1,1),"'")) {
          /* If c2 < 3 */
          if (CPLib.lt(c2,3)) {
            /* cNom := cNom + Substr(nome,c1,1) // Parte del nome per CF */
            cNom = cNom+CPLib.Substr(nome,c1,1);
  ...
  public String RunAsync(String p_w_COGNOME,String p_w_NOME,java.sql.Date p_w_DATANASC,String p_w_NASCOMUN,String p_w_NASSTATO,String p_w_SESSO,String p_w_TIPINTER,String p_w_CF,String p_w_IDCITTA) {
  ...
  public boolean Run(String p_w_COGNOME,String p_w_NOME,java.sql.Date p_w_DATANASC,String p_w_NASCOMUN,String p_w_NASSTATO,String p_w_SESSO,String p_w_TIPINTER,String p_w_CF,String p_w_IDCITTA) {
    w_COGNOME = p_w_COGNOME;
    w_NOME = p_w_NOME;
    w_DATANASC = p_w_DATANASC;
    w_NASCOMUN = p_w_NASCOMUN;
  ...
  public static arfn_cgo_calccodfisR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_cgo_calccodfisR(p_Ctx, p_Caller);
  }
  public static arfn_cgo_calccodfisR Make(CPContext p_Ctx) {
    return new arfn_cgo_calccodfisR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_cgo_calccodfis,
  public static final String i_InvokedRoutines = ",arfn_cgo_calccodfis,";
  public static String[] m_cRunParameterNames={"w_COGNOME","w_NOME","w_DATANASC","w_NASCOMUN","w_NASSTATO","w_SESSO","w_TIPINTER","w_CF","w_IDCITTA"};
}
