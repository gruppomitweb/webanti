...
public class arrt_imp_dati_wubs extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*7ec7a104*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_imp_dati_wubsR Rcalled;
    Rcalled = new arrt_imp_dati_wubsR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_imp_dati_wubsR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_imp_dati_wubsR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_imp_dati_wubs"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
