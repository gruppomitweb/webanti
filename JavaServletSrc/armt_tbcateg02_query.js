...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SEJWRQHUSP').value,'C'),w_C2CODTAB)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('XREVNLNKCY').value,'C'),w_C2CODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbcateg02',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'C2CODTAB') &&  ! (Empty(HtW(Ctrl('SEJWRQHUSP').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.C2CODTAB',HtW(Ctrl('SEJWRQHUSP').value,'C'),'C');
}
if (Ne(name,'C2CODICE') &&  ! (Empty(HtW(Ctrl('XREVNLNKCY').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.C2CODICE',HtW(Ctrl('XREVNLNKCY').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tbcateg02');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbcateg02',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
