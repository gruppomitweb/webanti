...
if(c=Ctrl('QKKSDYKFHN')) selectCombo(c,w_mese,'N')
...
SetDisplay(Ctrl('QKKSDYKFHN'),Ne(w_flgmod,'C') || IsHiddenByStateDriver('mese'));
SetDisplay(Ctrl('ALJCTCJMTJ'),Ne(w_flgmod,'C'));
SetDisplay(Ctrl('VVOFXMCQWC'),true);
HideUI();
}
HideUI.lblids["annorif"]=['QJBHNDQNQE','TVAOJABIZC'];
HideUI.lblids["mese"]=['ALJCTCJMTJ'];
...
function QKKSDYKFHN_Valid(e) {
e=(Ne(e,null)?e:window.event);
return Set_mese(HtW(getComboValue(Ctrl('QKKSDYKFHN')),'N'),null,e);
}
function QKKSDYKFHN_OnFocus(e) {
SetActiveField(Ctrl('QKKSDYKFHN'),true);
}
function QKKSDYKFHN_OnBlur(e) {
SetActiveField(Ctrl('QKKSDYKFHN'),false);
}
function href_VVOFXMCQWC() {
SetLocationHref(null,'arrt_calcsaldi_opex_wu'+'?m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','elaborazione')
}
_ResetTracker();
...
if(c=Ctrl('QKKSDYKFHN')) c.onchange=QKKSDYKFHN_Valid;
if(c=Ctrl('QKKSDYKFHN')) c.onfocus=QKKSDYKFHN_OnFocus;
if(c=Ctrl('QKKSDYKFHN')) c.onblur=QKKSDYKFHN_OnBlur;
}
}
function NotifyEvent(p_cEvent) {
var old_c;
if (Ne(typeof(w_pMsg),'undefined')) {
w_pMsg.raiseEvent('arpg_calcsaldi_opex_wu_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_calcsaldi_opex_wu_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_calcsaldi_opex_wu',m_cSelectedPage);
...
var ctl = _GetCtl(e,'QKKSDYKFHN');
if (_tracker.goon(ctl,ctrlValue)) {
w_mese=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('QKKSDYKFHN',0);
DoUpdate(l_bResult);
} else {
ctl.value=WtH('','N',2,0,'');
_ResetTracker();
...
l_bResult=false;
w_flgmopx='N';
} else if ( ! (_ChkObl('QKKSDYKFHN',1))) {
_SignErr('QKKSDYKFHN');
m_cLastWorkVarErrorMsg='10181710152';
l_bResult=false;
w_mese=HtW('','N');
}
...
l_oWv.setValue('mese',WtA(w_mese,'N'));
_FillChildren(l_oWv);
}
_FillChildren.n=[];
FillWv.n=["gIntemediario","gTipInter","gUrlApp","gAzienda","gFlgWU","annorif","valuta","xdesval","flgmod","flgazz","ndgcli","xragsoc","flgmopx","mese"];
...
_Obli('mese',1,'QKKSDYKFHN',false,"10181710152")
