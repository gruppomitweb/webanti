...
public class arpg_xntestit extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*IBHBAHEKSZ*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.m_cQueryName = "qbe_xntestit";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_xntestitBO BO) {
  ...
  void WriteStyles(arpg_xntestitBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_xntestit.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_xntestitBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_xntestitBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_xntestitBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_xntestitBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_xntestitBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_xntestitBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_xntestitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_xntestit"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_xntestit","");
  ...
  void WriteMainFormScript(arpg_xntestitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_xntestitBO getBO(ServletStatus status) {
    arpg_xntestitBO BO = new arpg_xntestitBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_xntestitBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_xntestitBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_xntestitBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_xntestitBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_xntestitBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_xntestitBO BO) {
    ...
    item.put("altInterfaceFor","arpg_xntestit");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_xntestitBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_xntestitBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_xntestitBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_xntestitBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_xntestitBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_xntestitBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_xntestit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_xntestit_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_xntestitBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_xntestitBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_xntestitBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_xntestitBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_xntestitBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_xntestitBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_xntestitBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_xntestitBO BO) {
    boolean result = true;
    return result;
  }
}
