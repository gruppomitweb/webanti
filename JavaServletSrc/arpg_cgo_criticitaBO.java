...
// * --- Fine Area Manuale
public class arpg_cgo_criticitaBO extends arpg_cgo_criticitaBL {
  ...
  public arpg_cgo_criticitaBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_DESCRIZ = rs.GetString("DESCRIZ");
          c_DESCCIT = rs.GetString("DESCCIT");
          c_IDBASE = rs.GetString("IDBASE");
          c_DATAOPE = rs.GetDate("DATAOPE");
          c_FLGCRTC = rs.GetString("FLGCRTC");
          c_NUMDAYS = rs.GetDouble("NUMDAYS");
          AddTrsRow();
          rs.Next();
          l_nCnt = l_nCnt+1;
        }
