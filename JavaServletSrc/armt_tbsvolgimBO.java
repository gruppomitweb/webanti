...
// * --- Fine Area Manuale
public class armt_tbsvolgimBO extends armt_tbsvolgimBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbsvolgimBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbsvolgim");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbsvolgim,m_cPhName_tbsvolgim,m_cVirtName_tbsvolgim,m_cVirtName_tbsvolgim+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",5,0),m_cServer_tbsvolgim,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsvolgim")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbsvolgim);
      l_cSql = "insert into "+m_oWrInfo_tbsvolgim.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+"RISCHIO,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbsvolgim.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsvolgim")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_tbsvolgim.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsvolgim")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbsvolgim,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbsvolgim);
      l_cSql = "Update "+m_oWrInfo_tbsvolgim.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",5,0),m_cServer_tbsvolgim,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbsvolgim.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsvolgim")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbsvolgim.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",5,0),m_cServer_tbsvolgim,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbsvolgim.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsvolgim")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbsvolgim,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
