...
// * --- Fine Area Manuale
public class ardt_jntestitBO extends ardt_jntestitBL {
  ...
  // * --- Fine Area Manuale
  public ardt_jntestitBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_jntestit");
    }
  ...
  public boolean Load(String p_CODCLI) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_jntestit,m_cPhName_jntestit,m_cVirtName_jntestit,m_cVirtName_jntestit+".CODCLI="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODCLI,"C",16,0),m_cServer_jntestit,p_CODCLI),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"jntestit")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_jntestit.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"CODINTER="+CPLib.ToSQL(opk_CODINTER,"C",16,0)+" AND CODCLI="+CPLib.ToSQL(w_CODCLI,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_jntestit.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"jntestit")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_jntestit,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_jntestit);
      l_cSql = "insert into "+m_oWrInfo_jntestit.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_jntestit.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"jntestit")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_jntestit.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"jntestit")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_jntestit,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_jntestit);
      l_cSql = "Update "+m_oWrInfo_jntestit.GetTableWriteName()+" set";
