...
// * --- Fine Area Manuale
public class arrt_li_frazioboR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_li_frazioboR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_li_fraziobo",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* pObj Object(armt_ofrazionateBL) */
      /* Variabili locali */
      /* conta Numeric(2, 0) */
      /* _tiporap Char(1) */
      /* Carica il contesto del figlio */
      /* pObj.CtxLoad_ardt_clientifrz() */
      pObj.CtxLoad_ardt_clientifrz();
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on knarapbo into routine arrt_li_fraziobo returns two or more records. This item should return only a record.");
      ...
      if (CPLib.eqr(_tiporap,"D")) {
        ...
        while ( ! (Cursor_intestbo.Eof())) {
          ...
          if (CPLib.eqr(conta,0)) {
            ...
            /* pObj.ardt_clientifrz.AddRow() */
            pObj.ardt_clientifrz.AddRow();
            /* pObj.ardt_clientifrz.Set_CODSOG(intestbo->CODINTER) */
            pObj.ardt_clientifrz.Set_CODSOG(Cursor_intestbo.GetString("CODINTER"));
            /* pObj.ardt_clientifrz.SaveRow() */
            pObj.ardt_clientifrz.SaveRow();
            /* pObj.ardt_clientifrz.SetUpdated() */
            pObj.ardt_clientifrz.SetUpdated();
          } // End If
        ...
        while ( ! (Cursor_xntestbo.Eof())) {
          ...
          if (CPLib.eqr(conta,0)) {
            ...
            /* pObj.ardt_clientifrz.AddRow() */
            pObj.ardt_clientifrz.AddRow();
            /* pObj.ardt_clientifrz.Set_CODSOG(xntestbo->CODINTER) */
            pObj.ardt_clientifrz.Set_CODSOG(Cursor_xntestbo.GetString("CODINTER"));
            /* pObj.ardt_clientifrz.SaveRow() */
            pObj.ardt_clientifrz.SaveRow();
            /* pObj.ardt_clientifrz.SetUpdated() */
            pObj.ardt_clientifrz.SetUpdated();
  ...
  public String RunAsync(armt_ofrazionateBL p_pObj) {
    pObj = p_pObj;
  ...
  public Forward Run(armt_ofrazionateBL p_pObj) {
    pObj = p_pObj;
    return Run();
  }
  public Forward Run() {
  ...
  public static arrt_li_frazioboR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_li_frazioboR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pObj = null;
    conta = 0;
