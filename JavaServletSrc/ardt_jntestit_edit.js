...
l_bEnabled= ! (Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'50') || Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'52') || Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'58')) || IsDisabledByStateDriver('DATAINI');
SetDisabled('HKZAUFLHMG',l_bEnabled);
l_bEnabled= ! (Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'51') || Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'53')) || IsDisabledByStateDriver('DATAFINE');
SetDisabled('NATIUIQLJX',l_bEnabled);
if (IsDisabledByStateDriver('DatiAN')) {
DisableInputsInContainer(Ctrl('VRCVINYRJO'),true);
}
if (IsDisabledByStateDriver('Prog')) {
...
if ( ! (m_oFather.SaveContext_ardt_jntestit())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_jntestit_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_jntestit_dataNotSent',{'action':formaction});
}
}
}
}
...
_FillChildren.n=[];
FillWv.n=["CODCLI","tipreg","gAzienda"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
if (Le(m_nCurrentRow,Rows())) {
if (( ! (_ChkObl('XTDHYWJPZL',1))) && FullRow()) {
_SignErr('XTDHYWJPZL');
m_cLastWorkVarErrorMsg='942986941';
l_bResult=false;
w_CODINTER=HtW('','C');
} else if (((Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'50') || Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'52') || Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'58')) && ( ! (_ChkObl('HKZAUFLHMG',1)))) && FullRow()) {
_SignErr('HKZAUFLHMG');
m_cLastWorkVarErrorMsg='45547614';
l_bResult=false;
w_DATAINI=HtW('','D');
} else if (((Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'51') || Eq(Utilities.Make(window).GetCallerString('TIPOOPRAP'),'53')) && ( ! (_ChkObl('NATIUIQLJX',1)))) && FullRow()) {
_SignErr('NATIUIQLJX');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_jntestit();
}
}
SaveContext.child=[];
function BlankRow() {
