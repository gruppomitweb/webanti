...
public class arpg_trasffra_age extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*XSQHXAXNYB*!";
  ...
  public String getEntityTitle() {
    return "Trasferimento frazionate a operazioni (Agenti)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_trasffra_age";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 789;
    l_eg.m_nEntityHeight = 329;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_trasffra_ageBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"522312053",p_Context.Translate("Confermi il trasferimento delle operazioni?")},
    {"569495331",p_Context.Translate("Trasferimento frazionate a operazioni (Agenti)")}};
    return l_translations;
  }
  void WriteStyles(arpg_trasffra_ageBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_trasffra_age.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_trasffra_ageBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_trasffra_ageBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_trasffra_ageBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_trasffra_ageBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_trasffra_ageBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(324);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_trasffra_ageBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<img id='ROZQLWLMOH' src="+SPLib.ToHTMLValue(SPLib.ToResource("images/elaborazione.gif"),"C",0,0)+" title="+SPLib.ToHTMLValue(status.context.Translate("Esecuzione trasferimento dati da frazionate a operazioni"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' alt="+SPLib.ToHTMLValue(status.context.Translate("Esecuzione trasferimento dati da frazionate a operazioni"),"C",0,0)+">" +
     "</a>" +
    "");
    status.out.println(
  ...
  static void PrintState(arpg_trasffra_ageBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_trasffra_ageBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_trasffra_ageBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_trasffra_ageBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(329,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_trasffra_age"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_trasffra_age","");
  ...
  void WriteMainFormScript(arpg_trasffra_ageBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_trasffra_age");
  ...
  arpg_trasffra_ageBO getBO(ServletStatus status) {
    arpg_trasffra_ageBO BO = new arpg_trasffra_ageBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_trasffra_ageBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_trasffra_ageBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_trasffra_ageBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_trasffra_ageBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_trasffra_ageBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_trasffra_ageBO BO) {
    ...
    item.put("altInterfaceFor","arpg_trasffra_age");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"315\",\"name\":\"Box_IDIZVJSJFR\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"IDIZVJSJFR\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"9\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esecuzione trasferimento dati da frazionate a operazioni\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"href\":\"javascript:href_ROZQLWLMOH()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"ROZQLWLMOH\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"2\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"352\",\"y\":\"288\",\"zone\":\"IDIZVJSJFR_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"25\",\"href\":\" \",\"name\":\"__iframe_width__460__height__80__id___elaborazione__name__elaborazione__frameborder__0___border__0__align__TOP__marginwidth__0__marginheight__0____iframe__\",\"page\":\"1\",\"spuid\":\"FGMOCXNBFY_DIV\",\"tabindex\":\"3\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"460"+"'"+" height="+"'"+"80"+"'"+" id ="+"'"+"elaborazione"+"'"+" name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"0"+"'"+"  border="+"'"+"0"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"0"+"'"+" marginheight="+"'"+"0"+"'"+"><\\/iframe>\\\"\",\"w\":\"112\",\"x\":\"16\",\"y\":\"18\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Data_Frazionata_\",\"page\":\"1\",\"spuid\":\"HPGJXQIVYG_DIV\",\"tabindex\":\"4\",\"type\":\"Label\",\"value\":\"Da Data Frazionata:\",\"w\":\"132\",\"x\":\"168\",\"y\":\"22\",\"zone\":\"IDIZVJSJFR_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"DaDatOpe\",\"page\":\"1\",\"spuid\":\"DPKVLWHCIG\",\"tabindex\":\"5\",\"type\":\"Textbox\",\"w\":\"72\",\"x\":\"304\",\"y\":\"18\",\"zone\":\"IDIZVJSJFR_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Data_Frazionata_\",\"page\":\"1\",\"spuid\":\"BGHAITOPMU_DIV\",\"tabindex\":\"6\",\"type\":\"Label\",\"value\":\"A Data Frazionata:\",\"w\":\"132\",\"x\":\"376\",\"y\":\"22\",\"zone\":\"IDIZVJSJFR_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"ADatOpe\",\"page\":\"1\",\"spuid\":\"MTJDZQTEIR\",\"tabindex\":\"7\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"512\",\"y\":\"18\",\"zone\":\"IDIZVJSJFR_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"30\",\"href\":\"javascript:href_PTXYMSWBZO()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"PTXYMSWBZO\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"8\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"400\",\"y\":\"288\",\"zone\":\"IDIZVJSJFR_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"196\",\"zone\":\"IDIZVJSJFR_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"147\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"46\",\"zone\":\"IDIZVJSJFR_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_trasffra_ageBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_trasffra_ageBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_trasffra_ageBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_trasffra_age?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_trasffra_age_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(329,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void RaiseLoadedEntity(arpg_trasffra_ageBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_trasffra_ageBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_trasffra_ageBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_trasffra_ageBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_trasffra_ageBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_trasffra_ageBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_trasffra_ageBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_trasffra_ageBO BO) {
    boolean result = true;
    return result;
  }
}
