...
public class armt_wersonbo_agg extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*6b74bcf2*!";
  static public class ServletStatus extends SPMasterChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "wersonbo_agg";
    }
    public String w_sanumpro;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_sanumpro,context,"wersonbo_agg","sanumpro","C",15,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    armt_wersonbo_aggBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_wersonbo_agg;
  ...
  public String getEntityTitle() {
    return "Storico Dati Aggiuntivi Persone/Clienti";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_wersonbo_agg";
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"wersonbo_agg","tbstati","tbcateg01","tbcateg02","tbcateg03","tbcateg04","tbcateg05"};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("sanumpro","C",15,0, "hide","Progressivo Variazione")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_wersonbo_aggBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11191788998",p_Context.Translate("Progressivo Variazione")},
    ...
    {"11900822717",p_Context.Translate("Storico Dati Aggiuntivi Persone/Clienti")},
  ...
  void WriteStyles(armt_wersonbo_aggBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_wersonbo_agg.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_wersonbo_aggBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_wersonbo_agg',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_wersonbo_agg",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(armt_wersonbo_aggBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(armt_wersonbo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b0(armt_wersonbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_wersonbo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(armt_wersonbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_wersonbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(armt_wersonbo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b3_CloseBand(armt_wersonbo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(armt_wersonbo_aggBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_wersonbo_aggBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_wersonbo_aggBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(armt_wersonbo_aggBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(armt_wersonbo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_wersonbo_agg"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_wersonbo_agg","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_wersonbo_agg",BO.GetCurrentState());
  ...
  void WriteMainFormScript(armt_wersonbo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_armt_wersonbo_agg);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_armt_wersonbo_agg();");
      status.out.println("}");
    }
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_wersonbo_agg',Ne(m_nChildStatus,0));");
    } else {
    ...
    status.out.println("m_oFather.m_oarmt_wersonbo_agg=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  armt_wersonbo_aggBO getBO(ServletStatus status) {
    armt_wersonbo_aggBO BO = new armt_wersonbo_aggBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(armt_wersonbo_aggBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("wersonbo_agg",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_wersonbo_aggBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,armt_wersonbo_aggBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(armt_wersonbo_aggBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_wersonbo_agg;
    String l_cPhName = BO.m_cPhName_wersonbo_agg;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_wersonbo_aggBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_wersonbo_agg+".sanumpro",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("wersonbo_agg");
    String l_cPhName = CPSql.ManipulateTablePhName("wersonbo_agg",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,armt_wersonbo_aggBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_wersonbo_agg,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_wersonbo_agg,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,armt_wersonbo_aggBO BO) {
  ...
  void Edit(ServletStatus status,armt_wersonbo_aggBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_wersonbo_aggBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_wersonbo_aggBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_wersonbo_aggBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_wersonbo_aggBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_wersonbo_aggBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_wersonbo_aggBO BO,SPParameterSource source) {
    BO.Load(status.w_sanumpro);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_wersonbo_aggBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_wersonbo_aggBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_wersonbo_aggBO BO) {
    ...
    item.put("altInterfaceFor","armt_wersonbo_agg");
    items.put(item);
    String s;
  ...
  void WriteFormPage(armt_wersonbo_aggBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_wersonbo_agg?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_wersonbo_agg_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_wersonbo_agg_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_sanumpro=a[28];");
      status.out.println("w_gTipInter=a[29];");
      status.out.println("w_xDesNaz=a[30];");
      status.out.println("w_xC1DESCRI=a[31];");
      status.out.println("w_xC2DESCRI=a[32];");
      status.out.println("w_xC3DESCRI=a[33];");
      status.out.println("w_xC4DESCRI=a[34];");
      status.out.println("w_xC5DESCRI=a[35];");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_wersonbo_agg_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","wersonbo_agg");
  ...
  void RaiseLoadedEntity(armt_wersonbo_aggBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_wersonbo_aggBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_wersonbo_aggBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_wersonbo_aggBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_wersonbo_aggBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_wersonbo_aggBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_wersonbo_aggBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,armt_wersonbo_aggBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_wersonbo_aggBO BO) {
    boolean result = true;
