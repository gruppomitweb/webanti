...
// * --- Fine Area Manuale
public class ardt_categrisBO extends ardt_categrisBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+3,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_2_1.CODICE AS CODICE201";
        m_cSelect = m_cSelect+" ,link_2_1.DESCRI AS DESCRI201";
        m_cSelect = m_cSelect+" ,link_2_1.GIORNI AS GIORNI201";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","CODICE","CODICE");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","CODICE","CODICE",true);
  ...
  // * --- Fine Area Manuale
  public ardt_categrisBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_categris");
    }
  ...
  public static String[] m_cLoadParameterNames = {"IDCATEG"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_IDCATEG) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_categris,m_cPhName_categris,m_cVirtName_categris,m_cVirtName_categris+".IDCATEG="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDCATEG,"C",10,0),m_cServer_categris,p_IDCATEG),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_1(m_cPhName_tbrisglob,m_cServer_tbrisglob);
    l_rsLoadBody = l_oSqlDetailLoader.Query();
    try {
      m_bLoaded =  ! (l_rsLoadBody.Eof());
      if (m_bLoaded) {
        m_bCalculating = true;
        w_IDCATEG = l_rsLoadBody.GetString("IDCATEG");
        // Assegna i valori degli op_
        // Assegna il contesto dei figli di testata/piedi a Empty
        SPExtenderLib.LoadExtendedFields(l_rsLoadBody,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"categris")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_CODICE = l_rsLoadBody.GetString("CODICE");
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            w_xDESCRI = l_rsLoadBody.GetString("DESCRI201");
            w_GIORNI = l_rsLoadBody.GetDouble("GIORNI201");
          } else {
            Link_EQQSGWXFEN("Load");
          }
          w_GIORNI = l_rsLoadBody.GetDouble("GIORNI");
  ...
  public boolean Delete(String p_IDCATEG) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_categris.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDCATEG="+CPLib.ToSQL(w_IDCATEG,"C",10,0)+" AND CODICE="+CPLib.ToSQL(opk_CODICE,"C",3,0);
        l_cSql = l_cSql+m_oWrInfo_categris.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"categris")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_categris,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_IDCATEG);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"EQQSGWXFEN")) {
      ...
      result.m_PrgZoom = "";
      result.m_rdField = new String[]{"CODICE" , "DESCRI" , "GIORNI"};
      result.m_rdTypes = new String[]{"C" , "C" , "N"};
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("EQQSGWXFEN",CPLib.GetLinkedFieldProps("CODICE","CODICE","C",3,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_categris);
      l_cSql = "insert into "+m_oWrInfo_categris.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDCATEG,";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"GIORNI,";
      l_cSql = l_cSql+"IDBASE,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_categris.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"categris")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_IDCATEG,"C",10,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_CODICE,"C",3,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_GIORNI,"N",5,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IDBASE,"C",10,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_categris.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"categris")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_categris,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_categris);
      l_cSql = "Update "+m_oWrInfo_categris.GetTableWriteName()+" set";
      l_cSql = l_cSql+" GIORNI = "+CPLib.ToSQL(w_GIORNI,"N",5,0);
      l_cSql = l_cSql+", IDBASE = "+CPLib.ToSQL(w_IDBASE,"C",10,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"categris")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", CODICE = "+CPLib.ToSQL(w_CODICE,"C",3,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"IDCATEG="+CPLib.ToSQL(w_IDCATEG,"C",10,0)+" AND CODICE="+CPLib.ToSQL(opk_CODICE,"C",3,0);
      l_cSql = l_cSql+m_oWrInfo_categris.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"categris")) {
