...
public class arpg_fam_rstp_invio_extra extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*XPYEMUSRLY*!";
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    arpg_fam_rstp_invio_extraBO BO;
    BO = getBO(status);
  ...
  public String getEntityTitle() {
    return "Stampa Invii Speciali";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_fam_rstp_invio_extra";
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("xfileinvio","C",80,0, "hide","Nome File Creato"),
                   new SPItem("xanno","C",4,0, "hide","Anno di Riferimento"),
                   new SPItem("xmese","C",2,0, "hide","Mese di Riferimento")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_fam_rstp_invio_extraBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11744623197",p_Context.Translate("Nome File Creato")},
    {"2085511508",p_Context.Translate("Stampa Invii Speciali")},
    {"862977325",p_Context.Translate("Mese di Riferimento")},
    {"863017137",p_Context.Translate("Anno di Riferimento")}};
    return l_translations;
  }
  void WriteStyles(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_fam_rstp_invio_extra.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(477);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_fam_rstp_invio_extraBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_fam_rstp_invio_extraBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_fam_rstp_invio_extraBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_xfileinvio,"C",80,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_xanno,"C",4,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_xmese,"C",2,0));
    status.out.println(")");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      status.out.println("m_bUpdated="+SPLib.ToJSValue(BO.m_bUpdated,"L",0,0)+";");
    }
  }
  void WriteMainFormVarScript(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_fam_rstp_invio_extra"),"C",0,0)+ ";");
    status.out.println("var m_bShowSuggest = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    status.out.println("var m_bShowNotifyBtn = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    status.out.println("var m_bShowExtendReduceBtn = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    HttpSession l_session = ((SPContext)status.context).session;
    status.out.print("var m_IDS={");
    status.out.print("'SCGTJGESFA':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash("CODINVIOCODINVIO,TIPINVIO,TPCINVIO,DATINVIO,FLEINVIO,ANNINVIO,MESINVIOC,C,C,D,C,C,Csid_invio",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,sid_invio,"+"default",l_session)+"'"+"]");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_fam_rstp_invio_extra","");
  ...
  void WriteMainFormScript(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_fam_rstp_invio_extraBO getBO(ServletStatus status) {
    arpg_fam_rstp_invio_extraBO BO = new arpg_fam_rstp_invio_extraBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_fam_rstp_invio_extraBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_fam_rstp_invio_extraBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_fam_rstp_invio_extraBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_fam_rstp_invio_extraBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_fam_rstp_invio_extraBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_fam_rstp_invio_extraBO BO) {
    ...
    item.put("altInterfaceFor","arpg_fam_rstp_invio_extra");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"768\",\"x\":\"16\",\"y\":\"225\",\"zone\":\"XLVFHSOSYH_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"153\",\"href\":\" \",\"name\":\"__iframe_width__750__height__400__id__elaborazione___name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"TYIWXFLLFY_DIV\",\"tabindex\":\"5\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"750"+"'"+" height="+"'"+"400"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"768\",\"x\":\"16\",\"y\":\"315\",\"zone\":\"XLVFHSOSYH_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"147\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"768\",\"x\":\"16\",\"y\":\"72\",\"zone\":\"XLVFHSOSYH_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"stpinvio\",\"page\":\"1\",\"spuid\":\"SCGTJGESFA\",\"tabindex\":\"9\",\"type\":\"Textbox\",\"w\":\"72\",\"x\":\"144\",\"y\":\"18\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Creazione_\",\"page\":\"1\",\"spuid\":\"JVVRQWIIHF_DIV\",\"tabindex\":\"12\",\"type\":\"Label\",\"value\":\"Data Creazione:\",\"w\":\"85\",\"x\":\"275\",\"y\":\"22\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Tipo_Invio_\",\"page\":\"1\",\"spuid\":\"WPSKYXARLO_DIV\",\"tabindex\":\"13\",\"type\":\"Label\",\"value\":\"Tipo Invio:\",\"w\":\"56\",\"x\":\"88\",\"y\":\"49\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Tipo_Comunicazione_\",\"page\":\"1\",\"spuid\":\"WZBNOLRUGW_DIV\",\"tabindex\":\"15\",\"type\":\"Label\",\"value\":\"Tipo Comunicazione:\",\"w\":\"107\",\"x\":\"253\",\"y\":\"49\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
  ...
  void Next(ServletStatus status,arpg_fam_rstp_invio_extraBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_fam_rstp_invio_extraBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_fam_rstp_invio_extra?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_fam_rstp_invio_extra_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_fam_rstp_invio_extraBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_fam_rstp_invio_extraBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_famextra_rstp.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_fam_rstp_invio_extraBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_fam_rstp_invio_extraBO BO) {
    boolean result = true;
    return result;
  }
}
