...
public class arrt_imp_dati_storico_sched_tabs extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ABZFYRGSEM*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_imp_dati_storico_sched_tabs";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_imp_dati_storico_sched_tabsR Rcalled;
    Rcalled = new arrt_imp_dati_storico_sched_tabsR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_imp_dati_storico_sched_tabsR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_imp_dati_storico_sched_tabsR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_imp_dati_storico_sched_tabs"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_imp_dati_storico_sched_tabs");
      if ( ! (CPLib.IsNull(l))) {
