...
public class arrt_calccodfis_mt extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*4a31ebc6*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_SESSO) ||  ! (status.m_bpassedw_NOME) ||  ! (status.m_bpassedw_RAGSOC) ||  ! (status.m_bpassedw_COGNOME) ||  ! (status.m_bpassedw_NASSTATO) ||  ! (status.m_bpassedw_NASCOMUN) ||  ! (status.m_bpassedw_DATANASC) ||  ! (status.m_bpassedw_CODFISC);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_calccodfis_mtR Rcalled;
    Rcalled = new arrt_calccodfis_mtR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_calccodfis_mtR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_calccodfis_mtR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_calccodfis_mt"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_calccodfis_mt");
      if ( ! (CPLib.IsNull(l))) {
