...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('ANDQNEUQUI').value,'C'),w_CODINTER)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('inter2bo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODINTER') &&  ! (Empty(HtW(Ctrl('ANDQNEUQUI').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODINTER',HtW(Ctrl('ANDQNEUQUI').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_intercon');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('inter2bo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['ANDQNEUQUI'],i=0;i>=0;i--){
