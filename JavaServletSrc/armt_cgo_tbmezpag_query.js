...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('FJTSUZOYFI').value,'C'),w_MPCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_tbmezpag',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'MPCODICE') &&  ! (Empty(HtW(Ctrl('FJTSUZOYFI').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.MPCODICE',HtW(Ctrl('FJTSUZOYFI').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_tbmezpag');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_tbmezpag',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['FJTSUZOYFI'],i=0;i>=0;i--){
