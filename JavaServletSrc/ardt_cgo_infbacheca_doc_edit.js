...
SetErrorField('ODWFKNKZRQ',false);
}
function Set_IDBNOMFILE(ctrlValue,kmode,e) {
if (Ne(w_IDBNOMFILE,ctrlValue)) {
var ctl = _GetCtl(e,'ODWFKNKZRQ');
if (_tracker.goon(ctl,ctrlValue)) {
w_IDBNOMFILE=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('ODWFKNKZRQ',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
function Set_IBDDESFILE(ctrlValue,kmode,e) {
if (Ne(w_IBDDESFILE,ctrlValue)) {
var ctl = _GetCtl(e,'LYWFRZTWYR');
if (_tracker.goon(ctl,ctrlValue)) {
w_IBDDESFILE=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('LYWFRZTWYR',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
if ( ! (m_oFather.SaveContext_ardt_cgo_infbacheca_doc())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_infbacheca_doc_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_infbacheca_doc_dataNotSent',{'action':formaction});
}
}
}
}
...
l_oWv.setValue('IBDCODICE',WtA(w_IBDCODICE,'N'));
...
_FillChildren.n=[];
FillWv.n=["IBDCODICE","gAzienda"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
if (Le(m_nCurrentRow,Rows())) {
if (( ! (_ChkObl('ODWFKNKZRQ',1))) && FullRow()) {
_SignErr('ODWFKNKZRQ');
m_cLastWorkVarErrorMsg='763806883';
l_bResult=false;
w_IDBNOMFILE=HtW('','C');
} else if (( ! (_ChkObl('LYWFRZTWYR',1))) && FullRow()) {
_SignErr('LYWFRZTWYR');
m_cLastWorkVarErrorMsg='10740223517';
l_bResult=false;
w_IBDDESFILE=HtW('','C');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_cgo_infbacheca_doc();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_IDBNOMFILE)) &&  ! (Empty(w_IBDDESFILE));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.IDBNOMFILE={"dec":0,"fixedpos":true,"id":"ODWFKNKZRQ","len":80,"name":"IDBNOMFILE","title":"Nome File","type":"C"};
l_aRepeatedFields.IBDDESFILE={"dec":0,"fixedpos":false,"id":"LYWFRZTWYR","len":70,"name":"IBDDESFILE","title":"Descrizione File","type":"C"};
...
if (enable) {
var c;
if(c=Ctrl('ODWFKNKZRQ')) SetDisabled(c,true);
SetQueryRow(Ctrl('GridTable'),false);
} else {
SetEditRow(Ctrl('GridTable'),false);
var c;
if(c=Ctrl('ODWFKNKZRQ')) SetDisabled(c,false);
