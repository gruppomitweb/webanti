...
public class arrt_fam_chkperiodo_2015 extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*a8c2bc15*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_fam_chkperiodo_2015";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_ADatOpe) ||  ! (status.m_bpassedw_DaDatOpe);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_fam_chkperiodo_2015R Rcalled;
    Rcalled = new arrt_fam_chkperiodo_2015R(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_fam_chkperiodo_2015R(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_fam_chkperiodo_2015R Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_fam_chkperiodo_2015"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_fam_chkperiodo_2015");
      if ( ! (CPLib.IsNull(l))) {
