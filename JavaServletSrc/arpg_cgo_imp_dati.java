...
public class arpg_cgo_imp_dati extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*FBGTEUBBFU*!";
  ...
  public String getEntityTitle() {
    return "Import operazioni non elaborate";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_cgo_imp_dati";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_cgo_imp_datiBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"31974851",p_Context.Translate("Import operazioni non elaborate")}};
    return l_translations;
  }
  void WriteStyles(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_cgo_imp_dati.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_cgo_imp_datiBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(343);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_cgo_imp_datiBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_cgo_imp_datiBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_cgo_imp_datiBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_cgo_imp_dati"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_cgo_imp_dati","");
  ...
  void WriteMainFormScript(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_cgo_imp_datiBO getBO(ServletStatus status) {
    arpg_cgo_imp_datiBO BO = new arpg_cgo_imp_datiBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_cgo_imp_datiBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_cgo_imp_datiBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_cgo_imp_datiBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_cgo_imp_datiBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_cgo_imp_datiBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_cgo_imp_datiBO BO) {
    ...
    item.put("altInterfaceFor","arpg_cgo_imp_dati");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"336\",\"name\":\"Box_YWGFSYCMOZ\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"YWGFSYCMOZ\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"254\",\"zone\":\"YWGFSYCMOZ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Chiudi la maschera\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_JSKZJOHJOM()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"JSKZJOHJOM\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"10\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_cgo_imp_datiBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_cgo_imp_datiBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_cgo_imp_dati?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_cgo_imp_dati_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_cgo_imp_datiBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_cgo_imp_datiBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_cgo_imp_datiBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_cgo_imp_datiBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_cgo_imp_datiBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_cgo_imp_datiBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_cgo_imp_dati_ext.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_cgo_imp_datiBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_cgo_imp_datiBO BO) {
    boolean result = true;
    return result;
  }
}
