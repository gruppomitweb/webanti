...
public class arfn_imp_newtabs extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*31985ab2*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public String aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa_retval;
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_tbnaturaage","w_tbnaturaage");
      }        
      w_tbnaturaage = source.GetParameter("w_tbnaturaage","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_tblimcon","w_tblimcon");
      }        
      w_tblimcon = source.GetParameter("w_tblimcon","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_tbaml","w_tbaml");
      }        
      w_tbaml = source.GetParameter("w_tbaml","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_tabellea7","w_tabellea7");
      }        
      w_tabellea7 = source.GetParameter("w_tabellea7","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_tbvasp","w_tbvasp");
      }        
      w_tbvasp = source.GetParameter("w_tbvasp","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_tbstatisv","w_tbstatisv");
      }        
      w_tbstatisv = source.GetParameter("w_tbstatisv","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_tbnindris","w_tbnindris");
      }        
      w_tbnindris = source.GetParameter("w_tbnindris","");
    }
  }
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arfn_imp_newtabsR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arfn_imp_newtabsR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arfn_imp_newtabs"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
      if (CPLib.Empty(Rcalled.m_cLastMsgError) &&  ! (status.m_bApplet)) {
        status.out.println("Error message:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(status.m_cFaultTrace)+"");
      }
    }
    status.PrintMutableParameters();
    status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(status.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa_retval))+"");
    status.out.println(
     " -->" +
    "");
    status.out.println(
