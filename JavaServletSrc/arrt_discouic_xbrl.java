...
public class arrt_discouic_xbrl extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*7f4fd1d8*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_datinv","w_datinv");
      }        
      w_datinv = source.GetParameter("w_datinv",CPLib.NullDate());
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_numinv","w_numinv");
      }        
      w_numinv = source.GetParameter("w_numinv",0);
      if (source instanceof SPRequestSource) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_storico","w_storico");
      }        
      w_storico = source.GetParameter("w_storico","");
    }
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_discouic_xbrlR Rcalled;
    Rcalled = new arrt_discouic_xbrlR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaabaaa_retval = Rcalled.Run(status.w_DaDatOpe,status.w_ADatOpe,status.w_tipocr,status.w_codcr,status.w_datinv,status.w_numinv,status.w_flgfrazionate,status.w_storico);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_discouic_xbrlR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_discouic_xbrlR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_discouic_xbrl"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
      if (CPLib.Empty(Rcalled.m_cLastMsgError) &&  ! (status.m_bApplet)) {
        status.out.println("Error message:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(status.m_cFaultTrace)+"");
      }
    }
    status.PrintMutableParameters();
    status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(status.aaaabaaa_retval))+"");
    status.out.println(
     " -->" +
    "");
    status.out.println(
