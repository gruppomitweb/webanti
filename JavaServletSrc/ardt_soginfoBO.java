...
// * --- Fine Area Manuale
public class ardt_soginfoBO extends ardt_soginfoBL {
  ...
  // * --- Fine Area Manuale
  public ardt_soginfoBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_soginfo");
    }
  ...
  public boolean Load(String p_RAPCOL) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_soginfo+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_soginfo,m_cPhName_soginfo,m_cVirtName_soginfo,m_cVirtName_soginfo+".RAPCOL="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPCOL,"C",25,0),m_cServer_soginfo,p_RAPCOL),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"soginfo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_soginfo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPCOL="+CPLib.ToSQL(w_RAPCOL,"C",25,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_soginfo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"soginfo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_soginfo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_soginfo);
      l_cSql = "insert into "+m_oWrInfo_soginfo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_soginfo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"soginfo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_soginfo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"soginfo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_soginfo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_soginfo);
      l_cSql = "Update "+m_oWrInfo_soginfo.GetTableWriteName()+" set";
