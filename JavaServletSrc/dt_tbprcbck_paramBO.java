...
// * --- Fine Area Manuale
public class dt_tbprcbck_paramBO extends dt_tbprcbck_paramBL {
  ...
  // * --- Fine Area Manuale
  public dt_tbprcbck_paramBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"dt_tbprcbck_param");
    }
  ...
  public static String[] m_cLoadParameterNames = {"PPCODICE"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(double p_PPCODICE) {
    CPResultSet l_rsLoadBody=null;
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_tbprcbck_param+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_tbprcbck_param,m_cPhName_tbprcbck_param,m_cVirtName_tbprcbck_param,m_cVirtName_tbprcbck_param+".PPCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_PPCODICE,"N",10,0),m_cServer_tbprcbck_param,p_PPCODICE),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_PPDESCRI = l_rsLoadBody.GetString("PPDESCRI");
          w_PPTIPPAR = l_rsLoadBody.GetString("PPTIPPAR");
          w_PPVALPAR = l_rsLoadBody.GetString("PPVALPAR");
  ...
  public boolean Delete(double p_PPCODICE) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_tbprcbck_param.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"PPCODICE="+CPLib.ToSQL(w_PPCODICE,"N",10,0)+" AND CPROWNUM="+CPLib.ToSQL(w_CPROWNUM,"N",3,0);
        l_cSql = l_cSql+m_oWrInfo_tbprcbck_param.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbprcbck_param")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_tbprcbck_param,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbprcbck_param);
      l_cSql = "insert into "+m_oWrInfo_tbprcbck_param.GetTableWriteName()+" (";
      l_cSql = l_cSql+"PPCODICE,";
      l_cSql = l_cSql+"PPDESCRI,";
      l_cSql = l_cSql+"PPTIPPAR,";
      l_cSql = l_cSql+"PPVALPAR,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbprcbck_param.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbprcbck_param")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_PPCODICE,"N",10,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_PPDESCRI,"C",50,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_PPTIPPAR,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_PPVALPAR,"C",20,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_CPROWNUM,"N",3,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbprcbck_param.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbprcbck_param")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbprcbck_param,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbprcbck_param);
      l_cSql = "Update "+m_oWrInfo_tbprcbck_param.GetTableWriteName()+" set";
      l_cSql = l_cSql+" PPDESCRI = "+CPLib.ToSQL(w_PPDESCRI,"C",50,0);
      l_cSql = l_cSql+", PPTIPPAR = "+CPLib.ToSQL(w_PPTIPPAR,"C",1,0);
      l_cSql = l_cSql+", PPVALPAR = "+CPLib.ToSQL(w_PPVALPAR,"C",20,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbprcbck_param")) {
