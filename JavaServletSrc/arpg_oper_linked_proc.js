...
if(c=Ctrl('TRYUFLJTGP')) c.value = WtH(w_DaDatReg,'D',8,0,TranslatePicture(GetDatePicture(true,'D'),'D'))
if(c=Ctrl('MZRYSMFIDB')) c.value = WtH(w_ADatReg,'D',8,0,TranslatePicture(GetDatePicture(true,'D'),'D'))
...
HideUI.lblids["ADatReg"]=['LXZLJIKQAP'];
HideUI.lblids["DaDatOpe"]=['HPGJXQIVYG'];
HideUI.lblids["DaDatReg"]=['GPUCWFFSHU'];
...
l_oWv.setValue('w_aCodCli',AsAppletValue(w_aCodCli));
l_oWv.setValue('w_ADatOpe',AsAppletValue(w_ADatOpe));
l_oWv.setValue('w_ADatReg',AsAppletValue(w_ADatReg));
l_oWv.setValue('w_daCodCli',AsAppletValue(w_daCodCli));
l_oWv.setValue('w_DaDatOpe',AsAppletValue(w_DaDatOpe));
l_oWv.setValue('w_DaDatReg',AsAppletValue(w_DaDatReg));
l_oWv.setValue('m_cParameterSequence',AsAppletValue('w_aCodCli,w_ADatOpe,w_ADatReg,w_daCodCli,w_DaDatOpe,w_DaDatReg'));
_modifyandopen('arrt_oper_linked'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes,height=500,width=700',null)
}
...
function TRYUFLJTGP_Valid(e) {
var dateisok = false;
if (Eq(m_oEnteredInDate,Ctrl('TRYUFLJTGP'))) {
if ( ! (IsMobile() && LibJavascript.HTML5Tests && LibJavascript.HTML5Tests.InputTypes && LibJavascript.HTML5Tests.InputTypes.date)) {
Ctrl('TRYUFLJTGP').value=ApplyPictureToDate(Ctrl('TRYUFLJTGP').value,TranslatePicture(datePattern),'TRYUFLJTGP');
} else if (IsMobile() && LibJavascript.HTML5Tests && LibJavascript.HTML5Tests.InputTypes && LibJavascript.HTML5Tests.InputTypes.date) {
CheckDateValidity(Ctrl('TRYUFLJTGP'));
}
m_oEnteredInDate=null;
dateisok=lastDateIsOk;
if ( ! (dateisok)) {
m_nLastError=-2;
if (IsMobile()) {
m_oEnteredInDate=Ctrl('TRYUFLJTGP');
}
}
}
SetActiveField(Ctrl('TRYUFLJTGP'),false);
e=(Ne(e,null)?e:window.event);
return Set_DaDatReg(HtW(Ctrl('TRYUFLJTGP').value,'D'),null,e) && dateisok;
}
function TRYUFLJTGP_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('TRYUFLJTGP'))) {
DisplayErrorMessage();
}
if ( ! (IsMobile() && LibJavascript.HTML5Tests && LibJavascript.HTML5Tests.InputTypes && LibJavascript.HTML5Tests.InputTypes.date)) {
Ctrl('TRYUFLJTGP').value=WtH(w_DaDatReg,'D',8,0,TranslatePicture(GetDatePicture(false,'D'),'D'));
}
m_oEnteredInDate=Ctrl('TRYUFLJTGP');
SetActiveField(Ctrl('TRYUFLJTGP'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('TRYUFLJTGP'),e);
}
function TRYUFLJTGP_ZOOM_Click() {
LaunchCalendar(Ctrl('TRYUFLJTGP'));
}
function MZRYSMFIDB_Valid(e) {
var dateisok = false;
if (Eq(m_oEnteredInDate,Ctrl('MZRYSMFIDB'))) {
if ( ! (IsMobile() && LibJavascript.HTML5Tests && LibJavascript.HTML5Tests.InputTypes && LibJavascript.HTML5Tests.InputTypes.date)) {
Ctrl('MZRYSMFIDB').value=ApplyPictureToDate(Ctrl('MZRYSMFIDB').value,TranslatePicture(datePattern),'MZRYSMFIDB');
} else if (IsMobile() && LibJavascript.HTML5Tests && LibJavascript.HTML5Tests.InputTypes && LibJavascript.HTML5Tests.InputTypes.date) {
CheckDateValidity(Ctrl('MZRYSMFIDB'));
}
m_oEnteredInDate=null;
dateisok=lastDateIsOk;
if ( ! (dateisok)) {
m_nLastError=-2;
if (IsMobile()) {
m_oEnteredInDate=Ctrl('MZRYSMFIDB');
}
}
}
SetActiveField(Ctrl('MZRYSMFIDB'),false);
e=(Ne(e,null)?e:window.event);
return Set_ADatReg(HtW(Ctrl('MZRYSMFIDB').value,'D'),null,e) && dateisok;
}
function MZRYSMFIDB_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('MZRYSMFIDB'))) {
DisplayErrorMessage();
}
if (Ne(_tracker.ctrl,Ctrl('MZRYSMFIDB'))) {
if ( ! (IsMobile() && LibJavascript.HTML5Tests && LibJavascript.HTML5Tests.InputTypes && LibJavascript.HTML5Tests.InputTypes.date)) {
Ctrl('MZRYSMFIDB').value=WtH(w_ADatReg,'D',8,0,TranslatePicture(GetDatePicture(false,'D'),'D'));
}
}
m_oEnteredInDate=Ctrl('MZRYSMFIDB');
SetActiveField(Ctrl('MZRYSMFIDB'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('MZRYSMFIDB'),e);
}
function MZRYSMFIDB_ZOOM_Click() {
LaunchCalendar(Ctrl('MZRYSMFIDB'));
}
_ResetTracker();
...
if(c=Ctrl('TRYUFLJTGP')) c.onblur=TRYUFLJTGP_Valid;
if(c=Ctrl('TRYUFLJTGP')) c.onfocus=TRYUFLJTGP_OnFocus;
if ( ! (IsMobile() && LibJavascript.HTML5Tests && LibJavascript.HTML5Tests.InputTypes && LibJavascript.HTML5Tests.InputTypes.date)) {
if(c=Ctrl('TRYUFLJTGP')) c.onkeypress=CheckDateChar;
}
if(c=Ctrl('MZRYSMFIDB')) c.onblur=MZRYSMFIDB_Valid;
if(c=Ctrl('MZRYSMFIDB')) c.onfocus=MZRYSMFIDB_OnFocus;
if ( ! (IsMobile() && LibJavascript.HTML5Tests && LibJavascript.HTML5Tests.InputTypes && LibJavascript.HTML5Tests.InputTypes.date)) {
if(c=Ctrl('MZRYSMFIDB')) c.onkeypress=CheckDateChar;
}
}
}
...
w_Messaggi.raiseEvent('arpg_oper_linked_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_oper_linked_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_oper_linked',m_cSelectedPage);
...
var ctl = _GetCtl(e,'MZRYSMFIDB');
if (_tracker.goon(ctl,ctrlValue)) {
w_ADatReg=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('MZRYSMFIDB',0);
if (l_bResult) {
l_bResult=Ge(w_ADatReg,w_DaDatReg);
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='ADatReg';
m_cLastMsgError=AlertErrorMessage('800457462');
}
if ( ! (l_bResult)) {
w_ADatReg=HtW('','D');
}
...
var ctl = _GetCtl(e,'TRYUFLJTGP');
if (_tracker.goon(ctl,ctrlValue)) {
w_DaDatReg=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('TRYUFLJTGP',0);
DoUpdate(l_bResult);
} else {
ctl.value=WtH(NullDate(),'D',8,0,TranslatePicture(GetDatePicture(false,'D'),'D'));
_ResetTracker();
...
l_bResult=false;
w_aCodCli=HtW('','C');
} else if ( ! (_ChkObl('TRYUFLJTGP',1))) {
_SignErr('TRYUFLJTGP');
m_cLastWorkVarErrorMsg='DaDatReg';
l_bResult=false;
w_DaDatReg=HtW('','D');
} else if (( ! (_ChkObl('MZRYSMFIDB',1))) || ( ! (Ge(w_ADatReg,w_DaDatReg)))) {
_SignErr('MZRYSMFIDB','800457462');
l_bResult=false;
w_ADatReg=HtW('','D');
}
...
if (Eq('TRYUFLJTGP_ZOOM',lc)) {
return 2;
}
if (Eq('MZRYSMFIDB_ZOOM',lc)) {
return 2;
}
return -1;
}
...
_FillChildren.n=[];
FillWv.n=["DaDatOpe","ADatOpe","RAGSOC1","RAGSOC2","c_dadata","c_adata","daCodCli","aCodCli","DaDatReg","ADatReg"];
...
_Obli('ADatReg',2,'MZRYSMFIDB',false)
_Obli('DaDatReg',2,'TRYUFLJTGP',false)
