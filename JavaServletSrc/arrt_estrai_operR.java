...
// * --- Fine Area Manuale
public class arrt_estrai_operR implements CallerWithObjs {
  ...
  public DateTime dt;
  // * --- Fine Area Manuale
  public arrt_estrai_operR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_estrai_oper",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* _testa := "operazioni_"+dataoggi+".xls" */
      _testa = "operazioni_"+dataoggi+".xls";
      ...
      Cursor_qbe_estrai_ope = new VQRHolder("qbe_estrai_ope",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),this,true).GetResultSet(m_Ctx);
      while ( ! (Cursor_qbe_estrai_ope.Eof())) {
        /* _sqr := iif(qbe_estrai_ope->SEGNO='A','S','P') */
        _sqr = (CPLib.eqr(Cursor_qbe_estrai_ope.GetString("SEGNO"),"A")?"S":"P");
        ...
        Cursor_personbo = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"CONNES="+CPLib.ToSQL(Cursor_qbe_estrai_ope.GetString("CONNESCLI"),"?",0,0)+" "+")"+(m_Ctx.IsSharedTemp("personbo")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
        ...
        Cursor_kersonbo = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"CONNES="+CPLib.ToSQL(Cursor_qbe_estrai_ope.GetString("CONNESDOC"),"?",0,0)+" "+")"+(m_Ctx.IsSharedTemp("kersonbo")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
        while ( ! (Cursor_kersonbo.Eof())) {
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on tbtipdoc into routine arrt_estrai_oper returns two or more records. This item should return only a record.");
  ...
  public static arrt_estrai_operR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_estrai_operR(p_Ctx, p_Caller);
  }
  public void Blank() {
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_estrai_ope,
  public static final String m_cVQRList = ",qbe_estrai_ope,";
