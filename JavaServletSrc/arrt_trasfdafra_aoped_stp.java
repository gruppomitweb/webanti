...
public class arrt_trasfdafra_aoped_stp extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*501918c4*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_trasfdafra_aoped_stp";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_DaDatOpe) ||  ! (status.m_bpassedw_ADatOpe);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_trasfdafra_aoped_stpR Rcalled;
    Rcalled = new arrt_trasfdafra_aoped_stpR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_trasfdafra_aoped_stpR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_trasfdafra_aoped_stpR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_trasfdafra_aoped_stp"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_trasfdafra_aoped_stp");
      if ( ! (CPLib.IsNull(l))) {
