...
public class arpg_operazrappoprv extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*CGXXGKYBOL*!";
  ...
  public String getEntityTitle() {
    return "Operazioni/Frazionarie relative al rapporto (Provvisorie)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_operazrappoprv";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 689;
    l_eg.m_nEntityHeight = 64;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    ...
    l_eg.items = new SPItem[]{
                   new SPItem("RAPPORTO","C",25,0, "hide","")
                 };
    l_eg.m_lExecuteAtStart = true;
    l_eg.m_cQueryName = "qbe_operazrappoprv";
    l_eg.m_nRowsPerPage = 10;
    return l_eg;
  }
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_operazrappoprvBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10943726013",p_Context.Translate("di cui Contanti")},
    {"109822566",p_Context.Translate("Valuta")},
    {"10988377690",p_Context.Translate("Data Oper.")},
    {"11270238829",p_Context.Translate("Operazioni/Frazionarie relative al rapporto (Provvisorie)")},
  ...
  void WriteStyles(arpg_operazrappoprvBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_operazrappoprv.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_operazrappoprvBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_operazrappoprvBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_operazrappoprvBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_operazrappoprvBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_operazrappoprvBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_operazrappoprvBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_operazrappoprvBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_operazrappoprvBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    PrintState(BO,status);
    status.out.println("var e_ADICXGPZZE = " + SPLib.ToJSValue(m_cEntityUid,"C",0,0)+ ";");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(64,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(689,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_operazrappoprv"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_operazrappoprv","");
    ...
    status.out.println("// * --- arpg_operazrappoprv");
  ...
  void WriteMainFormScript(arpg_operazrappoprvBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_operazrappoprvBO getBO(ServletStatus status) {
    arpg_operazrappoprvBO BO = new arpg_operazrappoprvBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_operazrappoprvBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_operazrappoprvBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_operazrappoprvBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_operazrappoprvBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_operazrappoprvBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_operazrappoprvBO BO) {
    ...
    item.put("altInterfaceFor","arpg_operazrappoprv");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_operazrappoprvBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_operazrappoprvBO BO) {
    ...
    status.out.println("var c_STATOREG;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_operazrappoprvBO BO) {
    String l_cTrsQ;
    ...
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_STATOREG,"C",1,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_VALUTA,"C",3,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_TIPO,"C",11,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_operazrappoprvBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_operazrappoprvBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_operazrappoprvBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_operazrappoprv?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_operazrappoprv_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(64,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void RaiseLoadedEntity(arpg_operazrappoprvBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_operazrappoprvBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_operazrappoprvBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_operazrappoprvBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_operazrappoprvBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_operazrappoprvBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_operazrappoprvBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_operazrappoprvBO BO) {
    boolean result = true;
    return result;
  }
}
