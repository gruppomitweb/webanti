...
function Set_PPTIPPAR(ctrlValue,kmode,e) {
if (Ne(w_PPTIPPAR,ctrlValue)) {
var ctl = _GetCtl(e,'CBRNDMRBGB');
if (_tracker.goon(ctl,ctrlValue)) {
w_PPTIPPAR=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('CBRNDMRBGB',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',1,0,'');
...
function Set_PPDESCRI(ctrlValue,kmode,e) {
if (Ne(w_PPDESCRI,ctrlValue)) {
var ctl = _GetCtl(e,'BIBFBTHHAY');
if (_tracker.goon(ctl,ctrlValue)) {
w_PPDESCRI=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('BIBFBTHHAY',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
function Set_PPVALPAR(ctrlValue,kmode,e) {
if (Ne(w_PPVALPAR,ctrlValue)) {
var ctl = _GetCtl(e,'JGOKTHOLYL');
if (_tracker.goon(ctl,ctrlValue)) {
w_PPVALPAR=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('JGOKTHOLYL',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
if ( ! (m_oFather.SaveContext_dt_tbprcbck_param())) {
...
parent.ZtVWeb.raiseEventToEvalParms('dt_tbprcbck_param_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('dt_tbprcbck_param_dataNotSent',{'action':formaction});
}
}
}
}
...
l_oWv.setValue('PPCODICE',WtA(w_PPCODICE,'N'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
if (( ! (_ChkObl('BIBFBTHHAY',1))) && FullRow()) {
_SignErr('BIBFBTHHAY');
m_cLastWorkVarErrorMsg='285808099';
l_bResult=false;
w_PPDESCRI=HtW('','C');
} else if (( ! (_ChkObl('CBRNDMRBGB',1))) && FullRow()) {
_SignErr('CBRNDMRBGB');
m_cLastWorkVarErrorMsg='2140278436';
l_bResult=false;
w_PPTIPPAR=HtW('','C');
} else if (( ! (_ChkObl('JGOKTHOLYL',1))) && FullRow()) {
_SignErr('JGOKTHOLYL');
m_cLastWorkVarErrorMsg='113861222';
l_bResult=false;
w_PPVALPAR=HtW('','C');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_dt_tbprcbck_param();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_PPDESCRI));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.PPDESCRI={"dec":0,"fixedpos":false,"id":"BIBFBTHHAY","len":50,"name":"PPDESCRI","title":"Descrizione","type":"C"};
l_aRepeatedFields.PPTIPPAR={"dec":0,"fixedpos":false,"id":"CBRNDMRBGB","len":1,"name":"PPTIPPAR","title":"Tipo (Stringa,Double,Int)","type":"C"};
l_aRepeatedFields.PPVALPAR={"dec":0,"fixedpos":false,"id":"JGOKTHOLYL","len":20,"name":"PPVALPAR","title":"Valore","type":"C"};
