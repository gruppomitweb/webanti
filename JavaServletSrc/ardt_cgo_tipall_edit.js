...
function Set_TATIPALL(ctrlValue,kmode,e) {
if (Ne(w_TATIPALL,ctrlValue)) {
var ctl = _GetCtl(e,'ODNAMZWXHO');
if (_tracker.goon(ctl,ctrlValue)) {
w_TATIPALL=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('ODNAMZWXHO',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',4,0,'');
...
function Set_TAPESUNI(ctrlValue,kmode,e) {
if (Ne(w_TAPESUNI,ctrlValue)) {
var ctl = _GetCtl(e,'XMUSIQKUML');
if (_tracker.goon(ctl,ctrlValue)) {
w_TAPESUNI=ValidNum(ctrlValue,'99.99');
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('XMUSIQKUML',0);
DoUpdate(l_bResult);
SPBOUILib.SetInputValue('XMUSIQKUML',function(){return WtH(w_TAPESUNI,'N',5,2,'99.99')},w_TAPESUNI);
return l_bResult;
} else {
ctl.value=WtH('','N',5,2,'99.99');
_ResetTracker();
return true;
}
}
...
function Set_TAPESSIN(ctrlValue,kmode,e) {
if (Ne(w_TAPESSIN,ctrlValue)) {
var ctl = _GetCtl(e,'JECRGTDJFG');
if (_tracker.goon(ctl,ctrlValue)) {
w_TAPESSIN=ValidNum(ctrlValue,'99.99');
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('JECRGTDJFG',0);
DoUpdate(l_bResult);
...
if ( ! (m_oFather.SaveContext_ardt_cgo_tipall())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_tipall_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_tipall_dataNotSent',{'action':formaction});
}
}
}
}
...
l_oWv.setValue('TACODINT',WtA(w_TACODINT,'C'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
if (( ! (_ChkObl('ODNAMZWXHO',1))) && FullRow()) {
_SignErr('ODNAMZWXHO');
m_cLastWorkVarErrorMsg='816713401';
l_bResult=false;
w_TATIPALL=HtW('','C');
} else if (( ! (_ChkObl('XMUSIQKUML',1))) && FullRow()) {
_SignErr('XMUSIQKUML');
m_cLastWorkVarErrorMsg='450498531';
l_bResult=false;
w_TAPESUNI=HtW('','N');
} else if (( ! (_ChkObl('JECRGTDJFG',1))) && FullRow()) {
_SignErr('JECRGTDJFG');
m_cLastWorkVarErrorMsg='37098542';
l_bResult=false;
w_TAPESSIN=HtW('','N');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_cgo_tipall();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_TATIPALL));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.TATIPALL={"dec":0,"fixedpos":false,"id":"ODNAMZWXHO","len":4,"name":"TATIPALL","title":"Tipo Allegato","type":"C"};
l_aRepeatedFields.TAPESUNI={"dec":2,"fixedpos":false,"id":"XMUSIQKUML","len":5,"name":"TAPESUNI","title":"Peso Documento Unico","type":"N"};
l_aRepeatedFields.TAPESSIN={"dec":2,"fixedpos":false,"id":"JECRGTDJFG","len":5,"name":"TAPESSIN","title":"Peso Documento Singolo","type":"N"};
