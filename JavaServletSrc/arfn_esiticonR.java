...
// * --- Fine Area Manuale
public class arfn_esiticonR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_esiticonR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_esiticon",m_Caller);
    ...
    if (m_cPhName_cgo_esiticon.endsWith(CPSecurity.FILTER_QUERY_POSTFIX)) {
      m_cPhName_cgo_esiticon = m_cPhName_cgo_esiticon+" "+m_Ctx.GetWritePhName("cgo_esiticon");
    }
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* cRitorno := '' */
      cRitorno = "";
      // * --- Read from cgo_esiticon
      m_cServer = m_Ctx.GetServer("cgo_esiticon");
      m_cPhName = m_Ctx.GetPhName("cgo_esiticon");
      m_cSql = "";
      m_cSql = m_cSql+"ECCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(pCode,"C",2,0,m_cServer),m_cServer,pCode);
      if (m_Ctx.IsSharedTemp("cgo_esiticon")) {
        m_cSql = m_cSql+" and context_id='"+m_Ctx.GetTempID()+"'";
      }
      if (Read_Cursor!=null)
        Read_Cursor.Close();
      Read_Cursor = m_Sql.Query(m_cServer,"select "+CPLib.GetMultilanguageField("ECDESCRI",m_Ctx,m_cServer)+" from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+m_cSql);
      if ( ! (Read_Cursor.Eof())) {
        cRitorno = Read_Cursor.GetString("ECDESCRI");
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on cgo_esiticon into routine arfn_esiticon returns two or more records. This item should return only a record.");
  ...
  public static arfn_esiticonR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_esiticonR(p_Ctx, p_Caller);
  }
  public static arfn_esiticonR Make(CPContext p_Ctx) {
    return new arfn_esiticonR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
    pCode = CPLib.Space(2);
