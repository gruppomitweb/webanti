...
// * --- Fine Area Manuale
public class ardt_cgo_infbacheca_usrBO extends ardt_cgo_infbacheca_usrBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        ...
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","IBUCODUSR","code");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","IBUCODUSR","code",true);
  ...
  // * --- Fine Area Manuale
  public ardt_cgo_infbacheca_usrBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_cgo_infbacheca_usr");
    }
  ...
  public static String[] m_cLoadParameterNames = {"IBUCODICE"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(double p_IBUCODICE) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cgo_infbacheca_usr,m_cPhName_cgo_infbacheca_usr,m_cVirtName_cgo_infbacheca_usr,m_cVirtName_cgo_infbacheca_usr+".IBUCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IBUCODICE,"N",10,0),m_cServer_cgo_infbacheca_usr,p_IBUCODICE),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_1(m_cPhName_cpusers,m_cServer_cpusers);
    l_rsLoadBody = l_oSqlDetailLoader.Query();
    try {
      m_bLoaded =  ! (l_rsLoadBody.Eof());
      if (m_bLoaded) {
        m_bCalculating = true;
        w_IBUCODICE = l_rsLoadBody.GetDouble("IBUCODICE");
        // Assegna i valori degli op_
        // Assegna il contesto dei figli di testata/piedi a Empty
        SPExtenderLib.LoadExtendedFields(l_rsLoadBody,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_usr")) {
  ...
  public boolean Delete(double p_IBUCODICE) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cgo_infbacheca_usr.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IBUCODICE="+CPLib.ToSQL(w_IBUCODICE,"N",10,0)+" AND IBUCODUSR="+CPLib.ToSQL(opk_IBUCODUSR,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_cgo_infbacheca_usr.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_usr")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_infbacheca_usr,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_IBUCODICE);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"BRCAXXLEPY")) {
      ...
      result.AppendFieldsHash("BRCAXXLEPY",CPLib.GetLinkedFieldProps("code","IBUCODUSR","N",10,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_infbacheca_usr);
      l_cSql = "insert into "+m_oWrInfo_cgo_infbacheca_usr.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IBUCODICE,";
      l_cSql = l_cSql+"IBUCODUSR,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_infbacheca_usr.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_usr")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_IBUCODICE,"N",10,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_IBUCODUSR,"N",10,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_infbacheca_usr.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_usr")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_infbacheca_usr,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_infbacheca_usr);
      l_cSql = "Update "+m_oWrInfo_cgo_infbacheca_usr.GetTableWriteName()+" set";
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_usr")) {
        l_cSql = l_cSql+" CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", IBUCODUSR = "+CPLib.ToSQL(w_IBUCODUSR,"N",10,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"IBUCODICE="+CPLib.ToSQL(w_IBUCODICE,"N",10,0)+" AND IBUCODUSR="+CPLib.ToSQL(opk_IBUCODUSR,"N",10,0);
      l_cSql = l_cSql+m_oWrInfo_cgo_infbacheca_usr.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_usr")) {
