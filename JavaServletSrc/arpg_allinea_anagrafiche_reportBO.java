...
// * --- Fine Area Manuale
public class arpg_allinea_anagrafiche_reportBO extends arpg_allinea_anagrafiche_reportBL {
  ...
  public arpg_allinea_anagrafiche_reportBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_AATIPERR = rs.GetString("AATIPERR");
          c_AANOMREP = rs.GetString("AANOMREP");
          AddTrsRow();
          rs.Next();
          l_nCnt = l_nCnt+1;
        }
