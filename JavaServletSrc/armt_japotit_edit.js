...
function DisplayErrorMessage_ardt_jntestit() {
if ( ! (ChildToLoad(FrameRef('ardt_jntestit'))) && Ne(typeof(FrameRef('ardt_jntestit').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_jntestit').DisplayErrorMessage();
}
}
...
if ( ! (ChildToLoad(FrameRef('ardt_jntestit'))) && Ne(typeof(FrameRef('ardt_jntestit').DoUpdate),'undefined')) {
CopyWorkVar(FrameRef('ardt_jntestit'),['w_tipoprap'],['TIPOOPRAP'],true)
}
...
w_TIPOLEG=(Eq(w_TIPOOPRAP,'50') || Eq(w_TIPOOPRAP,'51')?'5':(Eq(w_TIPOOPRAP,'52') || Eq(w_TIPOOPRAP,'53')?'6':''));
}
...
parent.ZtVWeb.raiseEventToEvalParms('armt_japotit_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_japotit_dataNotSent',{'action':formaction});
}
}
}
}
CheckChild.child=[{'prg':"ardt_jntestit",'BOScId':function(){ return FrameRef('ardt_jntestit')},'repeated':0}];
function Check() {
var l_bResult = true;
var l_bChildCheck = true;
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('FTYILIQWQK',1))) {
_SignErr('FTYILIQWQK');
m_cLastWorkVarErrorMsg='11258238247';
l_bResult=false;
w_STATOREG=HtW('','C');
} else if (( ! (Utilities.Make(window).IsInGroup(3))) && ( ! (_ChkObl('AVHTGZTOSZ',1)))) {
_SignErr('AVHTGZTOSZ');
...
_FillChildren.n=['ardt_jntestit'];
FillWv.n=["IDBASE","STATOREG","CODINTER","CODDIPE","TIPOINTER","TIPOINF","IDEREG","FLAGRAP","OPRAP","OPERATORE","AUTOM","DESCCIT","PROVINCIA","CODCAB","FLAGRAP2","FLAGLIRE","PRGIMPTIT","CONNESCLI","DATAOPE","DATAREG","CDATOPE","TIPOOPRAP","CONNESOPER","TIPOLEG","RAPPORTO","NUMPROG","DESCINTER","COLLEG","OPERAZMOD","CDATREG","gIntemediario","gDataVaria","gStatus","gCodDip","gSblocco","gChkDate","sEdit","flgpf","flgpg","xDescDipe","xdescrap","datpaseuro","xDesReg","xRagSoc","errmsg","Aggiorna","lancio","VALUTA","tipreg","tipregn","dataoggi","CITINT","CITDIP","PROVDIP","CABDIP","PROVINT","CABINT","xRagSocCli","filtro","SESSO"];
function SetModified() {
m_bUpdated=true;
}
function SetChildVariables_ardt_jntestit() {
if ( ! (ChildToLoad(FrameRef('ardt_jntestit'))) && Ne(typeof(FrameRef('ardt_jntestit').DoUpdate),'undefined')) {
CopyWorkVar(FrameRef('ardt_jntestit'),['w_tipoprap'],['TIPOOPRAP'],false)
}
}
function CtxLoad_ardt_jntestit() {
if (ChildToLoad(FrameRef('ardt_jntestit'))) {
return [LoadContext(0,'QBBTFAGDFU'),'ardt_jntestit'];
}
}
