...
public class arfn_calccodfis extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*b7c7b355*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arfn_calccodfisR Rcalled;
    Rcalled = new arfn_calccodfisR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaaaaa_retval = Rcalled.Run(status.w_COGNOME,status.w_NOME,status.w_DATANASC,status.w_NASCOMUN,status.w_NASSTATO,status.w_SESSO,status.w_TIPINTER);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arfn_calccodfisR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arfn_calccodfisR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arfn_calccodfis"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
