...
var ctl = _GetCtl(e,'WWCVGFCWRT');
if (_tracker.goon(ctl,ctrlValue)) {
w_IMPSARA=ValidNum(ctrlValue,'9999999999999999.99');
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('WWCVGFCWRT',0);
DoUpdate(l_bResult);
...
var ctl = _GetCtl(e,'FRMNDCFDLT');
if (_tracker.goon(ctl,ctrlValue)) {
w_PROQUOTA=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('FRMNDCFDLT',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','N',3,0,'');
...
if ( ! (m_oFather.SaveContext_ardt_mlientiope())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_mlientiope_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_mlientiope_dataNotSent',{'action':formaction});
}
}
}
}
...
l_bResult=false;
w_CODSOG=HtW('','C');
} else if (( ! (_ChkObl('FRMNDCFDLT',1))) && FullRow()) {
_SignErr('FRMNDCFDLT');
m_cLastWorkVarErrorMsg='11230801199';
l_bResult=false;
w_PROQUOTA=HtW('','N');
} else if (( ! (_ChkObl('WWCVGFCWRT',1))) && FullRow()) {
_SignErr('WWCVGFCWRT');
m_cLastWorkVarErrorMsg='1992290172';
l_bResult=false;
w_IMPSARA=HtW('','N');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_mlientiope();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.PROQUOTA={"dec":0,"fixedpos":false,"id":"FRMNDCFDLT","len":3,"name":"PROQUOTA","title":"Pro Quota","type":"N"};
l_aRepeatedFields.IMPSARA={"dec":2,"fixedpos":false,"id":"WWCVGFCWRT","len":19,"name":"IMPSARA","title":"Importo Sara","type":"N"};
