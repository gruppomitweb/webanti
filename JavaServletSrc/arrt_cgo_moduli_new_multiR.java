...
// * --- Fine Area Manuale
public class arrt_cgo_moduli_new_multiR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_moduli_new_multiR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_moduli_new_multi",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_cgo_moduli_new_multi";
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      if (m_Ctx.IsSharedTemp("cgo_operazioni_dl")) {
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on cgo_operazioni_dl into routine arrt_cgo_moduli_new_multi returns two or more records. This item should return only a record.");
      ...
      if (CPLib.eqr(_coddip,gCodDip)) {
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on anadip into routine arrt_cgo_moduli_new_multi returns two or more records. This item should return only a record.");
            CPStdCounter.Error(l_oErrorFault);
          }
          Read_Cursor.Close();
        } else {
        ...
        Cursor_cgo_operazioni_dl = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"SNAINUMOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_SNAINUMOPE,"?",0,0,m_cServer, m_oParameters),m_cServer,w_SNAINUMOPE)+" "+")"+(m_Ctx.IsSharedTemp("cgo_operazioni_dl")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
        while ( ! (Cursor_cgo_operazioni_dl.Eof())) {
          /* w_CODFISC := cgo_operazioni_dl->CODFISC // Codice Fiscale */
          w_CODFISC = Cursor_cgo_operazioni_dl.GetString("CODFISC");
          /* w_CODDIPE := cgo_operazioni_dl->CODDIPE // Codice Dipendenza */
          w_CODDIPE = Cursor_cgo_operazioni_dl.GetString("CODDIPE");
          /* w_CAIDBIGLIETTO := cgo_operazioni_dl->CAIDBIGLIETTO // ID BIGLIETTO */
          w_CAIDBIGLIETTO = Cursor_cgo_operazioni_dl.GetString("CAIDBIGLIETTO");
          /* w_MIDBIGLIETTO := cgo_operazioni_dl->CAIDBIGLIETTO // ID BIGLIETTO */
          w_MIDBIGLIETTO = Cursor_cgo_operazioni_dl.GetString("CAIDBIGLIETTO");
          /* Case cgo_operazioni_dl->MEZPAGAM='A' */
          if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"A")) {
            /* _mpagam := 'Altro Mezzo di Pagamento' */
            _mpagam = "Altro Mezzo di Pagamento";
            /* Case cgo_operazioni_dl->MEZPAGAM='I' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"I")) {
            /* _mpagam := 'Bonifico Italia' */
            _mpagam = "Bonifico Italia";
            /* Case cgo_operazioni_dl->MEZPAGAM='E' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"E")) {
            /* _mpagam := 'Bonifico Estero' */
            _mpagam = "Bonifico Estero";
            /* Case cgo_operazioni_dl->MEZPAGAM='P' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"P")) {
            /* _mpagam := 'Bonifico Postale' */
            _mpagam = "Bonifico Postale";
            /* Case cgo_operazioni_dl->MEZPAGAM='C' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"C")) {
            /* _mpagam := 'Assegno Circolare' */
            _mpagam = "Assegno Circolare";
            /* Case cgo_operazioni_dl->MEZPAGAM='B' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"B")) {
            /* _mpagam := 'Assegno Bancario' */
            _mpagam = "Assegno Bancario";
            /* Case cgo_operazioni_dl->MEZPAGAM='V' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"V")) {
            /* _mpagam := 'Carta di Credito' */
            _mpagam = "Carta di Credito";
            /* Case cgo_operazioni_dl->MEZPAGAM='S' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"S")) {
            /* _mpagam := 'Contante' */
            _mpagam = "Contante";
            /* Case cgo_operazioni_dl->MEZPAGAM='W' */
          } else if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"W")) {
            /* _mpagam := 'SnayPay Wallet' */
            _mpagam = "SnayPay Wallet";
          } // End Case
          /* _tipgio := arfn_cgo_tbtipgio(cgo_operazioni_dl->TIPOGIOCO,cgo_operazioni_dl->VLTPLAT) */
          _tipgio = arfn_cgo_tbtipgioR.Make(m_Ctx,this).Run(Cursor_cgo_operazioni_dl.GetString("TIPOGIOCO"),Cursor_cgo_operazioni_dl.GetString("VLTPLAT"));
          /* If cgo_operazioni_dl->MEZPAGAM='S' or cgo_operazioni_dl->MEZPAGAM='W' */
          if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"S") || CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"W")) {
            /* _pagam := LRTrim(_mpagam) + ' € ' +LRTrim(Str(cgo_operazioni_dl->TOTLIRE,15,2)) */
            _pagam = CPLib.LRTrim(_mpagam)+" \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl.GetDouble("TOTLIRE"),15,2));
          } else { // Else
            /* If cgo_operazioni_dl->TOTLIRE - (cgo_operazioni_dl->TOTCONT + cgo_operazioni_dl->IMPWALLET)=0 */
            if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetDouble("TOTLIRE")-(Cursor_cgo_operazioni_dl.GetDouble("TOTCONT")+Cursor_cgo_operazioni_dl.GetDouble("IMPWALLET")),0)) {
              /* _pagam := iif(cgo_operazioni_dl->FLAGCONT='1','Contanti € '+LRTrim(Str(cgo_operazioni_dl->TOTCONT,15,2)),'') */
              _pagam = (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("FLAGCONT"),"1")?"Contanti \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl.GetDouble("TOTCONT"),15,2)):"");
              /* Case cgo_operazioni_dl->FLAGWALLET='1' and cgo_operazioni_dl->FLAGCONT='1' */
              if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("FLAGWALLET"),"1") && CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("FLAGCONT"),"1")) {
                /* _pagam := _pagam + '- SnayPay Wallet € '+LRTrim(Str(cgo_operazioni_dl->IMPWALLET,15,2)) */
                _pagam = _pagam+"- SnayPay Wallet \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl.GetDouble("IMPWALLET"),15,2));
                /* Case cgo_operazioni_dl->FLAGWALLET='1' and cgo_operazioni_dl->FLAGCONT='0' */
              } else if (CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("FLAGWALLET"),"1") && CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("FLAGCONT"),"0")) {
                /* _pagam := 'SnayPay Wallet € '+LRTrim(Str(cgo_operazioni_dl->IMPWALLET,15,2)) */
                _pagam = "SnayPay Wallet \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl.GetDouble("IMPWALLET"),15,2));
              } // End Case
            } else { // Else
              /* _pagam := LRTrim(_mpagam) + ' € ' +LRTrim(Str((cgo_operazioni_dl->TOTLIRE - cgo_operazioni_dl->TOTCONT - cgo_operazioni_dl->IMPWALLET),15,2)) */
              _pagam = CPLib.LRTrim(_mpagam)+" \u20ac "+CPLib.LRTrim(CPLib.Str((Cursor_cgo_operazioni_dl.GetDouble("TOTLIRE")-Cursor_cgo_operazioni_dl.GetDouble("TOTCONT")-Cursor_cgo_operazioni_dl.GetDouble("IMPWALLET")),15,2));
              /* _pagam := _pagam + iif(cgo_operazioni_dl->FLAGCONT='1',' - Contanti € '+LRTrim(Str(cgo_operazioni_dl->TOTCONT,15,2)),'') */
              _pagam = _pagam+(CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("FLAGCONT"),"1")?" - Contanti \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl.GetDouble("TOTCONT"),15,2)):"");
              /* _pagam := _pagam + iif(cgo_operazioni_dl->FLAGWALLET='1',' - SnayPay Wallet € '+LRTrim(Str(cgo_operazioni_dl->IMPWALLET,15,2)),'') */
              _pagam = _pagam+(CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("FLAGWALLET"),"1")?" - SnayPay Wallet \u20ac "+CPLib.LRTrim(CPLib.Str(Cursor_cgo_operazioni_dl.GetDouble("IMPWALLET"),15,2)):"");
            } // End If
          } // End If
          /* _desaut := '' */
          _desaut = "";
          ...
          m_cSql = "";
          m_cSql = m_cSql+"ARCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("RDCODAUT"),"C",2,0,m_cServer),m_cServer,Cursor_cgo_operazioni_dl.GetString("RDCODAUT"));
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on cgo_autorita into routine arrt_cgo_moduli_new_multi returns two or more records. This item should return only a record.");
              CPStdCounter.Error(l_oErrorFault);
            }
            Read_Cursor.Close();
          } else {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_moduli_new_multi",84,"0000004A")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("C_CAB"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("C_CAP"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("C_CTA"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("C_IND"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("C_PRV"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("C_RAG"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("C_STA"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetDouble("CACODICE"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("CAIDBIGLIETTO"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetDouble("CAMBIO"),"?",0,0)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("CODANA"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("CODCAB"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("CODCABDIP"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("CODVOC"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("COGNOME"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("COLLEG"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("CONNESBEN"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("CONNESCLI"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetDate("DATANASC"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetDate("DATAOPE"),"?",0,0)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("DESCCITDIP"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("FLAGFRAZ"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("NUMPROG"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("PROVINCIADIP"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("SEGNO"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("SNAINUMOPE"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("SOTGRUP"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("STATOREG"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("TIPOOPRAP"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetDouble("TOTLIRE"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("VALUTA"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("CAFLGOPE"),"V")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("CAFLGOPE"),"G")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("CAFLGOPE"),"P")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("CAFLGOPE"),"R")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"I") || CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"E") || CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"P")?"X":""),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"B") || CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"C")?"X":""),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("VLTPLAT"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("CC4CIFRE"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("CCDATSCA"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl.GetString("NUMTEL"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(_tipgio,"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"V")?"X":""),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL((CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("FLAGCONT"),"1") || CPLib.eqr(Cursor_cgo_operazioni_dl.GetString("MEZPAGAM"),"S")?"X":""),"?",0,0,m_cServer)+", ";
        ...
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_moduli_new_multi",84,"0000004E")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
  ...
  public static arrt_cgo_moduli_new_multiR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_moduli_new_multiR(p_Ctx, p_Caller);
  }
  public static arrt_cgo_moduli_new_multiR Make(CPContext p_Ctx) {
    return new arrt_cgo_moduli_new_multiR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_cgo_tbtipgio,arrt_cgo_moduli_new_multi,
  public static final String i_InvokedRoutines = ",arfn_cgo_tbtipgio,arrt_cgo_moduli_new_multi,";
