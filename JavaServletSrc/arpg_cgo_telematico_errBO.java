...
// * --- Fine Area Manuale
public class arpg_cgo_telematico_errBO extends arpg_cgo_telematico_errBL {
  ...
  public arpg_cgo_telematico_errBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_CADATAORA = rs.GetDateTime("CADATAORA");
          AddTrsRow();
          rs.Next();
          l_nCnt = l_nCnt+1;
        }
