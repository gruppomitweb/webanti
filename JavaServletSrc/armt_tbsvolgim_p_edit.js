...
var ctl = _GetCtl(e,'RSWDMOYYIT');
if (_tracker.goon(ctl,ctrlValue)) {
w_RISCHIO=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('RSWDMOYYIT',0);
DoUpdate(l_bResult);
...
function Set_oldris(ctrlValue,kmode,e) {
if (Ne(w_oldris,ctrlValue)) {
var ctl = _GetCtl(e,'UIYMSMXTZJ');
if (_tracker.goon(ctl,ctrlValue)) {
w_oldris=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('UIYMSMXTZJ',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','N',5,0,'');
...
var l_bResult = true;
m_bCalculating=true;
w_FLGPERS=(Ne(w_RISCHIO,w_oldris)?'S':'N');
...
if ( ! (m_oFather.SaveContext_armt_tbsvolgim_p())) {
...
parent.ZtVWeb.raiseEventToEvalParms('armt_tbsvolgim_p_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_tbsvolgim_p_dataNotSent',{'action':formaction});
}
}
}
}
...
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('RSWDMOYYIT',1))) {
_SignErr('RSWDMOYYIT');
m_cLastWorkVarErrorMsg='1701052417';
l_bResult=false;
w_RISCHIO=HtW('','N');
} else if ( ! (_ChkObl('UIYMSMXTZJ',1))) {
_SignErr('UIYMSMXTZJ');
m_cLastWorkVarErrorMsg='oldris';
l_bResult=false;
w_oldris=HtW('','N');
}
if (l_bResult) {
...
l_oWv.setValue('FLGPERS',WtA(w_FLGPERS,'C'));
l_oWv.setValue('oldris',WtA(w_oldris,'N'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
_FillChildren.n=[];
FillWv.n=["CODICE","RISCHIO","FLGPERS","oldris"];
function SetModified() {
m_bUpdated=true;
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_armt_tbsvolgim_p();
}
}
function EnableEntityWhenReady(enable) {
if (enable) {
var c;
if(c=Ctrl('RSWDMOYYIT')) SetDisabled(c,true);
if(c=Ctrl('UIYMSMXTZJ')) SetDisabled(c,true);
} else {
var c;
if(c=Ctrl('RSWDMOYYIT')) SetDisabled(c,false);
if(c=Ctrl('UIYMSMXTZJ')) SetDisabled(c,false);
