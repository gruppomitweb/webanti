...
// * --- Fine Area Manuale
public class ardt_docautBO extends ardt_docautBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_2_1.TIPDOC AS TIPDOC201";
        m_cSelect = m_cSelect+" ,link_2_1.DESCRI AS DESCRI201";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","DATIPDOC","TIPDOC");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","DATIPDOC","TIPDOC",true);
  ...
  // * --- Fine Area Manuale
  public ardt_docautBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_docaut");
    }
  ...
  public static String[] m_cLoadParameterNames = {"DACODICE"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_DACODICE) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cgo_docaut,m_cPhName_cgo_docaut,m_cVirtName_cgo_docaut,m_cVirtName_cgo_docaut+".DACODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_DACODICE,"C",2,0),m_cServer_cgo_docaut,p_DACODICE),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_1(m_cPhName_cgo_tipdoc,m_cServer_cgo_tipdoc);
    l_rsLoadBody = l_oSqlDetailLoader.Query();
    try {
      m_bLoaded =  ! (l_rsLoadBody.Eof());
      if (m_bLoaded) {
        m_bCalculating = true;
        w_DACODICE = l_rsLoadBody.GetString("DACODICE");
        // Assegna i valori degli op_
        // Assegna il contesto dei figli di testata/piedi a Empty
        SPExtenderLib.LoadExtendedFields(l_rsLoadBody,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_docaut")) {
  ...
  public boolean Delete(String p_DACODICE) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cgo_docaut.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"DACODICE="+CPLib.ToSQL(w_DACODICE,"C",2,0)+" AND DATIPDOC="+CPLib.ToSQL(opk_DATIPDOC,"C",2,0);
        l_cSql = l_cSql+m_oWrInfo_cgo_docaut.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_docaut")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_docaut,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_DACODICE);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"SSYFQFAQAO")) {
      ...
      result.m_PrgZoom = "armt_cgo_tipdoc";
      result.m_rdField = new String[]{"TIPDOC" , "DESCRI"};
      result.m_rdTypes = new String[]{"C" , "C"};
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("SSYFQFAQAO",CPLib.GetLinkedFieldProps("TIPDOC","DATIPDOC","C",2,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_docaut);
      l_cSql = "insert into "+m_oWrInfo_cgo_docaut.GetTableWriteName()+" (";
      l_cSql = l_cSql+"DACODICE,";
      l_cSql = l_cSql+"DATIPDOC,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_docaut.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_docaut")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_DACODICE,"C",2,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_DATIPDOC,"C",2,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_docaut.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_docaut")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_docaut,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_docaut);
      l_cSql = "Update "+m_oWrInfo_cgo_docaut.GetTableWriteName()+" set";
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_docaut")) {
        l_cSql = l_cSql+" CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", DATIPDOC = "+CPLib.ToSQL(w_DATIPDOC,"C",2,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"DACODICE="+CPLib.ToSQL(w_DACODICE,"C",2,0)+" AND DATIPDOC="+CPLib.ToSQL(opk_DATIPDOC,"C",2,0);
      l_cSql = l_cSql+m_oWrInfo_cgo_docaut.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_docaut")) {
