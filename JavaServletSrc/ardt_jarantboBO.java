...
// * --- Fine Area Manuale
public class ardt_jarantboBO extends ardt_jarantboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_jarantboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_jarantbo");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_jarantbo,m_cPhName_jarantbo,m_cVirtName_jarantbo,m_cVirtName_jarantbo+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_jarantbo,p_RAPPORTO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"jarantbo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_jarantbo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND CODINTER="+CPLib.ToSQL(opk_CODINTER,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_jarantbo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"jarantbo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_jarantbo,l_cSql);
  ...
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"FPBSSVXDQL")) {
      ...
      result.m_PrgZoom = "armt_personbo_age";
      result.m_rdField = new String[]{"CONNES" , "RAGSOC"};
      result.m_rdTypes = new String[]{"C" , "C"};
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("FPBSSVXDQL",CPLib.GetLinkedFieldProps("CONNES","CODINTER","C",16,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_jarantbo);
      l_cSql = "insert into "+m_oWrInfo_jarantbo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_jarantbo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"jarantbo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_jarantbo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"jarantbo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_jarantbo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_jarantbo);
      l_cSql = "Update "+m_oWrInfo_jarantbo.GetTableWriteName()+" set";
