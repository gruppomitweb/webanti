...
public class arpg_esitiage extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*VPBXBSXDOL*!";
  ...
  public String getEntityTitle() {
    return "Dati da modificare";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_esitiage";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 939;
    ...
    l_eg.m_cQueryName = "qbe_esitiage";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_esitiageBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10513536909",p_Context.Translate("Dati da modificare")},
    {"113933141",p_Context.Translate("Update")},
    {"11983148991",p_Context.Translate("Apertura")},
    {"11983201341",p_Context.Translate("Chiusura")},
    {"1805022818",p_Context.Translate("Rapporto")},
    {"1806425667",p_Context.Translate("Soggetto")}};
    return l_translations;
  }
  void WriteStyles(arpg_esitiageBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_esitiage.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_esitiageBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_esitiageBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_esitiageBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_esitiageBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_esitiageBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_esitiageBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_esitiageBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(939,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_esitiage"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_esitiage","");
  ...
  void WriteMainFormScript(arpg_esitiageBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_esitiageBO getBO(ServletStatus status) {
    arpg_esitiageBO BO = new arpg_esitiageBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_esitiageBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_esitiageBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_esitiageBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_esitiageBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_esitiageBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_esitiageBO BO) {
    ...
    item.put("altInterfaceFor","arpg_esitiage");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_esitiageBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_esitiageBO BO) {
    status.out.println("var c_trueident;");
    status.out.println("var c_rapporto;");
    status.out.println("var c_connes;");
    status.out.println("var c_ragsoc;");
    status.out.println("var c_aprog;");
    status.out.println("var c_afile;");
    status.out.println("var c_cprog;");
    status.out.println("var c_cfile;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_esitiageBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_trueident,"C",15,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_rapporto,"C",25,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_connes,"C",16,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_ragsoc,"C",30,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_aprog,"C",25,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_afile,"C",15,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_cprog,"C",25,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_cfile,"C",15,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_esitiageBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_esitiageBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_esitiageBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_esitiage?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_esitiage_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_esitiageBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_esitiageBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_esitiageBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_esitiageBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_esitiageBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_esitiageBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_esitiageBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_esitiageBO BO) {
    boolean result = true;
    return result;
  }
}
