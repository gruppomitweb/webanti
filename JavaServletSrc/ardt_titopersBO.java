...
// * --- Fine Area Manuale
public class ardt_titopersBO extends ardt_titopersBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        ...
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","CONPERS","CONNES");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","CONPERS","CONNES",true);
  ...
  // * --- Fine Area Manuale
  public ardt_titopersBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_titopers");
    }
  ...
  public static String[] m_cLoadParameterNames = {"CONTITO"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_CONTITO) {
    CPResultSet l_rsLoadBody=null;
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_titopers+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_titopers,m_cPhName_titopers,m_cVirtName_titopers,m_cVirtName_titopers+".CONTITO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CONTITO,"C",16,0),m_cServer_titopers,p_CONTITO),l_cOrderBy);
  ...
  public boolean Delete(String p_CONTITO) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_titopers.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"CONTITO="+CPLib.ToSQL(w_CONTITO,"C",16,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_titopers.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"titopers")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_titopers,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_CONTITO);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"AGYYKHSHUA")) {
      ...
      result.AppendFieldsHash("AGYYKHSHUA",CPLib.GetLinkedFieldProps("CONNES","CONPERS","C",16,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_titopers);
      l_cSql = "insert into "+m_oWrInfo_titopers.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CONTITO,";
      l_cSql = l_cSql+"CONPERS,";
      l_cSql = l_cSql+"DATAINI,";
      l_cSql = l_cSql+"DATAFINE,";
      l_cSql = l_cSql+"NOTEBREV,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_titopers.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"titopers")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_CONTITO,"C",16,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_CONPERS,"C",16,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DATAINI,"D",8,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DATAFINE,"D",8,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_NOTEBREV,"M",10,0,m_cServer_titopers,m_oParameters)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(m_nCPRowNum,"N",10,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_titopers.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"titopers")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_titopers,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_titopers);
      l_cSql = "Update "+m_oWrInfo_titopers.GetTableWriteName()+" set";
      l_cSql = l_cSql+" CONPERS = "+CPLib.ToSQLNull(w_CONPERS,"C",16,0);
      l_cSql = l_cSql+", DATAINI = "+CPLib.ToSQL(w_DATAINI,"D",8,0);
      l_cSql = l_cSql+", DATAFINE = "+CPLib.ToSQL(w_DATAFINE,"D",8,0);
      l_cSql = l_cSql+", NOTEBREV = "+CPLib.ToSQL(w_NOTEBREV,"M",10,0,m_cServer_titopers,m_oParameters);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"titopers")) {
