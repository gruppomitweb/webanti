...
public class arrt_moverap extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*4d6b75af*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_msgproc) ||  ! (status.m_bpassedw_datreg) ||  ! (status.m_bpassedw_datchi);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_moverapR Rcalled;
    Rcalled = new arrt_moverapR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_moverapR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_moverapR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_moverap"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_moverap");
      if ( ! (CPLib.IsNull(l))) {
