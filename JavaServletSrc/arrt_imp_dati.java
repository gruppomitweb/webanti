...
public class arrt_imp_dati extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*7eca37df*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public String _tmp;
    public boolean _tmp_logic;
    public java.sql.Timestamp _numimp;
    public String _tabelle;
    public java.sql.Date _date_oggi;
    public java.sql.Date _date_oggi_venti_anni_fa;
    public double _flgfraud;
    public String _rapcol;
    public String fileName;
    public double righeTot;
    public String _charset;
    public double _novar;
    public String _codintope;
    public java.sql.Date _dataeuro;
    ...
    public String w_opeunivoco;
    public double aaaaaaaaa_retval;
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_opeunivoco","w_opeunivoco");
      }        
      w_opeunivoco = source.GetParameter("w_opeunivoco","");
    }
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_imp_datiR Rcalled;
    Rcalled = new arrt_imp_datiR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaaaaaaa_retval = Rcalled.Run(status.w_tipodest,status.w_chkaui,status.w_imperr,status.w_flgaggfam,status.w_flgimposv,status.w_creamo,status.w_norifimp,status.w_flgforzaagg,status.w_opeunivoco);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_imp_datiR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_imp_datiR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_imp_dati"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
