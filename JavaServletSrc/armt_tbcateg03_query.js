...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SEJWRQHUSP').value,'C'),w_C3CODTAB)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('XREVNLNKCY').value,'C'),w_C3CODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbcateg03',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'C3CODTAB') &&  ! (Empty(HtW(Ctrl('SEJWRQHUSP').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.C3CODTAB',HtW(Ctrl('SEJWRQHUSP').value,'C'),'C');
}
if (Ne(name,'C3CODICE') &&  ! (Empty(HtW(Ctrl('XREVNLNKCY').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.C3CODICE',HtW(Ctrl('XREVNLNKCY').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_categris(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDCATEG ";
}
...
_chNotF(childAlias,'master','armt_tbcateg03');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbcateg03',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['SEJWRQHUSP','XREVNLNKCY'],i=1;i>=0;i--){
