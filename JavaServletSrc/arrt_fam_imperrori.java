...
public class arrt_fam_imperrori extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*62f10daa*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_fam_imperroriR Rcalled;
    Rcalled = new arrt_fam_imperroriR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_fam_imperroriR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_fam_imperroriR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_fam_imperrori"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
