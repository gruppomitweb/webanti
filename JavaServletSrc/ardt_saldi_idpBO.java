...
// * --- Fine Area Manuale
public class ardt_saldi_idpBO extends ardt_saldi_idpBL {
  ...
  // * --- Fine Area Manuale
  public ardt_saldi_idpBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_saldi_idp");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_saldi_idp,m_cPhName_saldi_idp,m_cVirtName_saldi_idp,m_cVirtName_saldi_idp+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_saldi_idp,p_RAPPORTO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"saldi_idp")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          w_TRIMESTRE = l_rsLoadBody.GetString("TRIMESTRE");
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_saldi_idp.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND ANNO="+CPLib.ToSQL(opk_ANNO,"C",4,0)+" AND TRIMESTRE="+CPLib.ToSQL(opk_TRIMESTRE,"C",1,0);
        l_cSql = l_cSql+m_oWrInfo_saldi_idp.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"saldi_idp")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_saldi_idp,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_saldi_idp);
      l_cSql = "insert into "+m_oWrInfo_saldi_idp.GetTableWriteName()+" (";
      l_cSql = l_cSql+"RAPPORTO,";
      l_cSql = l_cSql+"ANNO,";
      l_cSql = l_cSql+"TRIMESTRE,";
      l_cSql = l_cSql+"IMPORTO,";
      l_cSql = l_cSql+"IDBASE,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_saldi_idp.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"saldi_idp")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_saldi_idp,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_saldi_idp);
      l_cSql = "Update "+m_oWrInfo_saldi_idp.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+", TRIMESTRE = "+CPLib.ToSQL(w_TRIMESTRE,"C",1,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND ANNO="+CPLib.ToSQL(opk_ANNO,"C",4,0)+" AND TRIMESTRE="+CPLib.ToSQL(opk_TRIMESTRE,"C",1,0);
      l_cSql = l_cSql+m_oWrInfo_saldi_idp.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"saldi_idp")) {
