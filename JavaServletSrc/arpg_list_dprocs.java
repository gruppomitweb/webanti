...
public class arpg_list_dprocs extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ZTXSGNKJFC*!";
  static public class ServletStatus extends SPPageServletStatus {
    public void MergeCallerVars(SPParameterSource source) {
      ...
      source.SetParameter("codice",source.GetCaller().GetString("codice","C",10,0));
    }
  }
  ...
  public String getEntityTitle() {
    return "Dettaglio Processo";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_list_dprocs";
    ...
    l_eg.items = new SPItem[]{
                   new SPItem("tipo","C",3,0, "hide","Tipo Processo"),
                   new SPItem("codice","C",10,0, "hide","Codice Processo")
                 };
    l_eg.m_lExecuteAtStart = true;
    l_eg.m_cQueryName = "qbe_list_dprocs";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_list_dprocsBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"11092727986",p_Context.Translate("N° Righe")},
    {"112851763",p_Context.Translate("Codice")},
    {"113933141",p_Context.Translate("Update")},
    {"11635830682",p_Context.Translate("Dettaglio Processo")},
    {"11654409835",p_Context.Translate("Codice Processo")},
    {"1738412183",p_Context.Translate("Tipo Processo")},
    {"285808099",p_Context.Translate("Descrizione")}};
    return l_translations;
  }
  void WriteStyles(arpg_list_dprocsBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_list_dprocs.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_list_dprocsBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_list_dprocsBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_list_dprocsBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_list_dprocsBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_list_dprocsBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_list_dprocsBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_list_dprocsBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_list_dprocsBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    PrintState(BO,status);
    status.out.println("var e_SDOMWPEROY = " + SPLib.ToJSValue(m_cEntityUid,"C",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_list_dprocs"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_list_dprocs","");
  ...
  void WriteMainFormScript(arpg_list_dprocsBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_list_dprocsBO getBO(ServletStatus status) {
    arpg_list_dprocsBO BO = new arpg_list_dprocsBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_list_dprocsBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_list_dprocsBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_list_dprocsBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_list_dprocsBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_list_dprocsBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_list_dprocsBO BO) {
    ...
    item.put("altInterfaceFor","arpg_list_dprocs");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_list_dprocsBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_list_dprocsBO BO) {
    status.out.println("var c_DPRPROGRE;");
    status.out.println("var c_DPRTXTMSG;");
    status.out.println("var c_DPRNUMRIG;");
    status.out.println("var c_DPRESITO;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_list_dprocsBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_DPRPROGRE,"N",4,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_DPRTXTMSG,"M",10,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_DPRNUMRIG,"N",10,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_DPRESITO,"C",1,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_list_dprocsBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_list_dprocsBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_list_dprocsBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_list_dprocs?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_list_dprocs_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_list_dprocsBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_list_dprocsBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_list_dprocsBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_list_dprocsBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_list_dprocsBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_list_dprocsBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_list_dprocsBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_list_dprocsBO BO) {
    boolean result = true;
    return result;
  }
}
