...
WindowAlert(AlertErrorMessage('11157094597'));
}
if (Eq(formaction,'edit') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('11638313586'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('1198480061'));
}
}
}
var m_bIsMq = false;
...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('UOVJKJLZBV').value,'C'),w_RAPPORTO)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('aerighe',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'RAPPORTO') &&  ! (Empty(HtW(Ctrl('UOVJKJLZBV').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.RAPPORTO',HtW(Ctrl('UOVJKJLZBV').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_aeannora(firstAlias) {
...
_chNotF(childAlias,'master/detail','armd_anagrappo');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('aerighe',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['UOVJKJLZBV'],i=0;i>=0;i--){
