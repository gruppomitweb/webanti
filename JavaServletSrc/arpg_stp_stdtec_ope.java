...
public class arpg_stp_stdtec_ope extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*HNAELHILUQ*!";
  ...
  public String getEntityTitle() {
    return "Stampa Verifica Operazioni (Standard Tecnici fino al 31/12/2013)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_stp_stdtec_ope";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_stp_stdtec_opeBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1817011351",p_Context.Translate("Stampa Verifica Operazioni (Standard Tecnici fino al 31/12/2013)")},
  ...
  void WriteStyles(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_stp_stdtec_ope.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(225);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_stp_stdtec_opeBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_stp_stdtec_opeBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_stp_stdtec_opeBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_stp_stdtec_ope"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_stp_stdtec_ope","");
  ...
  void WriteMainFormScript(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_stp_stdtec_opeBO getBO(ServletStatus status) {
    arpg_stp_stdtec_opeBO BO = new arpg_stp_stdtec_opeBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_stp_stdtec_opeBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_stp_stdtec_opeBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_stp_stdtec_opeBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_stp_stdtec_opeBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_stp_stdtec_opeBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_stp_stdtec_opeBO BO) {
    ...
    item.put("altInterfaceFor","arpg_stp_stdtec_ope");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_XURLGQZZMU()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"XURLGQZZMU\",\"src\":\"images\\/print.gif\",\"tabindex\":\"8\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"304\",\"y\":\"189\",\"zone\":\"XLVFHSOSYH_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_PTXYMSWBZO()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"PTXYMSWBZO\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"9\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"360\",\"y\":\"189\",\"zone\":\"XLVFHSOSYH_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Rae\\/ateco\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"name\":\"flgrae\",\"page\":\"1\",\"spuid\":\"QNMKJLGOKW\",\"tabindex\":\"24\",\"textlist\":\"Utilizza codifica RAE,Utilizza codifica ATECO\",\"type\":\"Combobox\",\"valuelist\":\""+"'"+"R"+"'"+","+"'"+"A"+"'"+"\",\"w\":\"424\",\"x\":\"168\",\"y\":\"153\",\"zone\":\"XLVFHSOSYH_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_stp_stdtec_opeBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_stp_stdtec_opeBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_stp_stdtec_ope?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_stp_stdtec_ope_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_stp_stdtec_opeBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_stp_stdtec_opeBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_stp_stdtec_opeBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_stp_stdtec_opeBO BO) {
    boolean result = true;
    return result;
  }
}
