...
public class arpg_viewinfo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*UCUUCUAOMP*!";
  ...
  public String getEntityTitle() {
    return "Informazioni (A.U.I.)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_viewinfo";
    ...
    l_eg.m_cQueryName = "qbe_viewinfo";
    l_eg.m_nRowsPerPage = 3;
    return l_eg;
  }
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_viewinfoBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10632911230",p_Context.Translate("Data Reg.")},
    ...
    {"1809989379",p_Context.Translate("Stato Rapporto")},
    {"36228043",p_Context.Translate("N° Informazione")},
    {"740069047",p_Context.Translate("Informazioni (A.U.I.)")}};
    return l_translations;
  }
  void WriteStyles(arpg_viewinfoBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_viewinfo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_viewinfoBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_viewinfoBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_viewinfoBO BO,ServletStatus status) {
    ...
    status.out.println(
     ...
     "</script>" +
    "");
    status.themeWriter.Footer_BeginHtml(24);
    WriteControls_1_f_0(BO,status);
  }
  void WriteControls_1_f_0(arpg_viewinfoBO BO,ServletStatus status) {
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input name='rapporto' id='LTKZIXLTWO' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_rapporto,"C",25,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='25' size='25' fieldtype='C' style='display:none'>" +
    "");
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_viewinfoBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_viewinfoBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_viewinfoBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_viewinfoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_viewinfo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_viewinfo","");
  ...
  void WriteMainFormScript(arpg_viewinfoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_viewinfoBO getBO(ServletStatus status) {
    arpg_viewinfoBO BO = new arpg_viewinfoBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_viewinfoBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_viewinfoBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_viewinfoBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_viewinfoBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_viewinfoBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_viewinfoBO BO) {
    ...
    item.put("altInterfaceFor","arpg_viewinfo");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"25\",\"name\":\"rapporto\",\"page\":\"1\",\"spuid\":\"LTKZIXLTWO\",\"tabindex\":\"11\",\"type\":\"Textbox\",\"w\":\"162\",\"x\":\"8\",\"y\":\"4\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void WriteBody(ServletStatus status,arpg_viewinfoBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_viewinfoBO BO) {
    status.out.println("var c_NUMPROG;");
    status.out.println("var c_DATAOPE;");
    status.out.println("var c_DATAREG;");
    status.out.println("var c_TIPOOPRAP;");
    status.out.println("var c_DES1;");
    status.out.println("var c_IDBASE;");
    status.out.println("var c_RAPPORTO;");
    status.out.println("var c_STATOREG;");
    status.out.println("var c_STATO;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_viewinfoBO BO) {
    String l_cTrsQ;
    ...
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_DATAREG,"D",8,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_TIPOOPRAP,"C",2,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_DES1,"C",90,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_IDBASE,"C",10,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_RAPPORTO,"C",25,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_STATOREG,"C",20,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_STATO,"C",1,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_viewinfoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_viewinfoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_viewinfoBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_viewinfo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_viewinfo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_viewinfoBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_viewinfoBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_viewinfoBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_viewinfoBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_viewinfoBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_viewinfoBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_viewinfoBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_viewinfoBO BO) {
    boolean result = true;
    return result;
  }
}
