...
public class arrt_esito_rap_report extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*366b3da*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.pOut","pOut");
      }        
      pOut = source.GetParameter("pOut","");
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_esito_rap_report";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_tipoelab);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.pOut,"C",1,0)+",'C');");
    status.out.println("l_oWv.setValue('pOut',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_esito_rap_reportR Rcalled;
    Rcalled = new arrt_esito_rap_reportR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.forwardedto = Rcalled.Run(status.pOut);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_esito_rap_reportR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_esito_rap_reportR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      status.out.println(
       "<!--" +
      "");
      status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(Rcalled.RunAsync(status.pOut)))+"");
      status.out.println(
       " -->" +
      "");
    } else {
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_esito_rap_report"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_esito_rap_report");
      if ( ! (CPLib.IsNull(l))) {
