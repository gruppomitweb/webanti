...
// * --- Fine Area Manuale
public class arpg_closerapBO extends arpg_closerapBL {
  ...
  public arpg_closerapBO (CPContext p_ContextObject) {
  ...
  public boolean SaveDetailRow() {
    ...
    if (CPLib.eq(m_nRowStatus,1)) {
      if (CPLib.ne(w_FLGSEL,t_FLGSEL)) {
        ...
        l_cSql = "Update "+m_cPhName_tmp_rapportipd+" set FLGSEL="+CPLib.ToSQL(w_FLGSEL,"C",1,0);
        l_cSql = l_cSql+", CPCCCHK="+CPLib.ToSQL(CPLib.NewCPCCCHK(),"C",10,0);
        l_cSql = l_cSql+" where RAPPORTO="+CPLib.ToSQL(k_FLGSEL_RAPPORTO,"C",25,0);
        l_nUpdatedRows = m_Sql.Update(m_cServer_tmp_rapportipd,l_cSql);
        if (CPLib.eq(l_nUpdatedRows,0)) {
          // Aggiornamento fallito: si tenta un inserimento
          l_cSql = "Insert into "+m_cPhName_tmp_rapportipd+" (FLGSEL,RAPPORTO,CPCCCHK) values ("+CPLib.ToSQL(w_FLGSEL,"C",1,0)+", "+CPLib.ToSQL(k_FLGSEL_RAPPORTO,"C",25,0)+", "+CPLib.ToSQL(CPLib.NewCPCCCHK(),"C",10,0)+")";
          l_nUpdatedRows = m_Sql.Update(m_cServer_tmp_rapportipd,l_cSql);
        }
