...
public class arpg_stp_stdtec_xapBL extends arpg_stp_stdtec_xapWV implements CPLib.QueryFilterFromApplication {
  ...
  public static final String i_EntityName = "arpg_stp_stdtec_xap";
  ...
  public static final String i_StateStructure = "{'daCodDip':daCodDip,'aCodDip':aCodDip,'DaDatOpe':DaDatOpe,'ADatOpe':ADatOpe,'daRapporto':daRapporto,'aRapporto':aRapporto}";
  public static final String i_CompleteStateStructure = "{'daCodDip':daCodDip,'aCodDip':aCodDip,'DaDatOpe':DaDatOpe,'ADatOpe':ADatOpe,'daRapporto':daRapporto,'aRapporto':aRapporto,'RAGSOC1':RAGSOC1,'RAGSOC2':RAGSOC2,'DESCRIZ1':DESCRIZ1,'DESCRIZ2':DESCRIZ2,'ADatReg':ADatReg,'DaDatReg':DaDatReg}";
  public static final String i_ItemSequence = "daCodDip,aCodDip,DaDatOpe,ADatOpe,daRapporto,aRapporto";
  public static final com.zucchetti.sitepainter.EntityItem[] i_EditableEntityItems = com.zucchetti.sitepainter.EntityItem.buildEditableItemsForEntity(GetLinkDef(),"",new String[][]{new String[]{"daCodDip","","C","6","0","variabile","nokey","false","false"},new String[]{"aCodDip","","C","6","0","variabile","nokey","false","false"},new String[]{"DaDatOpe","","D","8","0","variabile","nokey","false","false"},new String[]{"ADatOpe","","D","8","0","variabile","nokey","false","false"},new String[]{"daRapporto","","C","25","0","variabile","nokey","false","false"},new String[]{"aRapporto","","C","25","0","variabile","nokey","false","false"}});
  ...
  public arpg_stp_stdtec_xapBL (CPContext p_ContextObject) {
  ...
  protected boolean Link_UYVYTEMEXX(String p_cType) {
    ...
    if ( ! (CPLib.Empty(w_daRapporto))) {
      ...
      // Costruzione della where per la chiave corrente e per le chiavi fisse
      l_cCurrentKey = "="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_daRapporto,"C",25,0),m_cServer_xnarapbo,w_daRapporto);
      if (m_Ctx.IsSharedTemp("xnarapbo")) {
        l_cWhere = l_cWhere+" and context_id='"+m_Ctx.GetTempID()+"'";
      }
      if (l_rsLink!=null)
        l_rsLink.Close();
      l_rsLink = m_Sql.Query(m_cServer_xnarapbo,"select RAPPORTO,DESCRAP from "+m_cPhName_xnarapbo+((m_cPhName_xnarapbo).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"RAPPORTO"+l_cCurrentKey+l_cWhere);
      try {
        l_bLoaded =  ! (l_rsLink.Eof());
        if (CPLib.ne(p_cType,"Load")) {
          if ( ! (l_bLoaded)) {
            l_cCurrentKey = "="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_daRapporto,"C",70,0),m_cServer_xnarapbo,w_daRapporto);
            if (l_rsLink!=null)
              l_rsLink.Close();
            l_rsLink = m_Sql.Query(m_cServer_xnarapbo,"select RAPPORTO,DESCRAP from "+m_cPhName_xnarapbo+((m_cPhName_xnarapbo).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"DESCRAP"+l_cCurrentKey+l_cWhere);
            l_bLoaded =  ! (l_rsLink.Eof());
          }
        }
        if (l_bLoaded) {
  ...
  protected boolean Link_CJWUHWULAR(String p_cType) {
    ...
    if ( ! (CPLib.Empty(w_aRapporto))) {
      ...
      // Costruzione della where per la chiave corrente e per le chiavi fisse
      l_cCurrentKey = "="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_aRapporto,"C",25,0),m_cServer_xnarapbo,w_aRapporto);
      if (m_Ctx.IsSharedTemp("xnarapbo")) {
        l_cWhere = l_cWhere+" and context_id='"+m_Ctx.GetTempID()+"'";
      }
      if (l_rsLink!=null)
      ...
      try {
        ...
        if (CPLib.ne(p_cType,"Load")) {
          if ( ! (l_bLoaded)) {
            l_cCurrentKey = "="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_aRapporto,"C",70,0),m_cServer_xnarapbo,w_aRapporto);
            if (l_rsLink!=null)
              l_rsLink.Close();
            l_rsLink = m_Sql.Query(m_cServer_xnarapbo,"select RAPPORTO,DESCRAP from "+m_cPhName_xnarapbo+((m_cPhName_xnarapbo).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"DESCRAP"+l_cCurrentKey+l_cWhere);
            l_bLoaded =  ! (l_rsLink.Eof());
  ...
  public static String QueryFilter(CPContext m_Ctx) {
    String l_cFilter;
    l_cFilter = CPLib.ApplicationQueryFilter("arpg_stp_stdtec_xap","",m_KeyColumns,m_Ctx);
    return l_cFilter;
  }
  ...
  static void GetLinkDef0(Map<String,List<Map<String,String[]>>> tables) {
    ...
    map.put("property.name",zeroSplit.split("arch\u0000fillemptykey\u0000looselylinked\u0000src_key\u0000hst_link_mode\u0000prg_zoom\u0000looselylinked\u0000sendkeystozoomonzoom\u0000zoom_title\u0000prg_link_search\u0000prg_link_defaults\u0000",-1));
    map = new HashMap<String,String[]>();
    items = tables.get("xnarapbo");
    if (CPLib.IsNull(items)) {
      items = new ArrayList<Map<String,String[]>>();
      tables.put("xnarapbo",items);
    }
    items.add(map);
    map.put("rd_kvar",new String[]{});
    map.put("rd_kfield",new String[]{});
    ...
    map.put("rd_var.obj",zeroSplit.split("variabile\u0000variabile",-1));
    map.put("rd_kvar.obj",new String[]{});
    map.put("property.value",zeroSplit.split("xnarapbo\u0000false\u0000false\u00002\u00000\u0000\u0000false\u0000false\u0000Rapporti Continuativi\u0000\u0000\u0000false",-1));
    map.put("property.name",zeroSplit.split("arch\u0000fillemptykey\u0000looselylinked\u0000src_key\u0000hst_link_mode\u0000prg_zoom\u0000looselylinked\u0000sendkeystozoomonzoom\u0000zoom_title\u0000prg_link_search\u0000prg_link_defaults\u0000",-1));
    map = new HashMap<String,String[]>();
    items = tables.get("xnarapbo");
    if (CPLib.IsNull(items)) {
