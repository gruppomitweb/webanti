...
WindowAlert(AlertErrorMessage('453648321'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('11004525328'));
}
}
}
var m_bIsMq = false;
...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('NJDOYSOENB').value,'C'),w_IDBASE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('fraziobo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDBASE') &&  ! (Empty(HtW(Ctrl('NJDOYSOENB').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDBASE',HtW(Ctrl('NJDOYSOENB').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_clientifrz(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_mlientifrz(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_sogopefbo(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_mogopefbo(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_meneficfrz(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_beneficfrz(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_terzistifrz(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_merzistifrz(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_docfrabo(firstAlias) {
return m_cVirtName+".NUMPROG = "+firstAlias+".NUMPROG ";
}
function DoQueryFrom_fraziobo_agg(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDBASE ";
}
function DoQueryFrom_frz_contante(firstAlias) {
...
_chNotF(childAlias,'master','armt_ofrazionate');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('fraziobo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['NJDOYSOENB'],i=0;i>=0;i--){
