...
public class arrt_stpfraz_prv extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*5611c7ce*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_aCodDip) ||  ! (status.m_bpassedw_daCodDip) ||  ! (status.m_bpassedw_OrderBy) ||  ! (status.m_bpassedw_Tipop) ||  ! (status.m_bpassedw_tipstp) ||  ! (status.m_bpassedw_c_dadata) ||  ! (status.m_bpassedw_c_adata) ||  ! (status.m_bpassedw_aCodCli) ||  ! (status.m_bpassedw_daCodCli) ||  ! (status.m_bpassedw_ADatOpe) ||  ! (status.m_bpassedw_DaDatOpe);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_stpfraz_prvR Rcalled;
    Rcalled = new arrt_stpfraz_prvR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_stpfraz_prvR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_stpfraz_prvR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_stpfraz_prv"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_stpfraz_prv");
      if ( ! (CPLib.IsNull(l))) {
