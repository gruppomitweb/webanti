...
public class arpg_kersonbo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*GZQTUYIMWE*!";
  ...
  public String getEntityTitle() {
    return "Variazioni Documenti";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_kersonbo";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 779;
    ...
    l_eg.m_cQueryName = "qbe_kersonbo";
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11397589335",p_Context.Translate("Variazioni Documenti")},
    {"11921223220",p_Context.Translate("Codice Fiscale")},
    {"12038003070",p_Context.Translate("Data Var.")}};
    return l_translations;
  }
  void WriteStyles(arpg_kersonboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_kersonbo.css'>" +
      "");
    }
  ...
  void WriteSenderForm(ServletStatus status) {
    ...
    "");
    status.out.println(
     "<input name='m_cPrevRowsNum' type='hidden' value="+SPLib.ToHTMLValue(status.m_cPrevRowsNum,"C",0,0,"", status.m_oRegSettings)+">" +
    "");
    status.out.println(
  ...
  void WriteMainForm(arpg_kersonboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_kersonboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_kersonboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_kersonboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_kersonboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_kersonboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_kersonboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_kersonboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(779,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_kersonbo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_kersonbo","");
  ...
  void WriteMainFormScript(arpg_kersonboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_kersonboBO getBO(ServletStatus status) {
    arpg_kersonboBO BO = new arpg_kersonboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_kersonboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_kersonboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_kersonboBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_kersonboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_kersonboBO BO) {
    status.m_bExit = true;
  }
  void WriteBody(ServletStatus status,arpg_kersonboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_kersonboBO BO) {
    status.out.println("var c_datamodi;");
    status.out.println("var c_OLDCONNES;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_kersonboBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_datamodi,"D",8,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_OLDCONNES,"C",16,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_CONNES,"C",40,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_RAGSOC,"C",70,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_kersonboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_kersonboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_kersonboBO BO,ServletStatus status) {
    ...
    String l_cJsUid;
    status.out.println(
     "<script src='arpg_kersonbo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_kersonbo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_kersonboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_kersonboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_kersonboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_kersonboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_kersonboBO BO,ServletStatus status) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0));
  }
