...
if(c=Ctrl('VEABFZIUKQ')) ChkboxCheckUncheck(c,'S',w_NOAGE)
if(c=Ctrl('VEABFZIUKQ')) ChkboxValueAssign(c,'S','N','C',1,0,'')
} else {
var tmp_NAKGRMOUXS = ToHTag(AsControlValue(w_DOMICILIO));
if (Ne(e_NAKGRMOUXS,tmp_NAKGRMOUXS)) {
SPBOUILib.SetLabelValue('NAKGRMOUXS','innerHTML',tmp_NAKGRMOUXS);
...
SetDisplay(Ctrl('VORELZCTGG'),Ne(m_cFunction,'view'));
...
HideUI.lblids["CODINTER"]=['RQPAWNXLEF'];
HideUI.lblids["DATAFINE"]=['DGFLOKNCET'];
HideUI.lblids["DATAINI"]=['MAUGJNUVWH'];
...
function BJKHVZBXDR_ZOOM_Click() {
LaunchCalendar(Ctrl('BJKHVZBXDR'));
}
...
function JCFUNSQXBI_ZOOM_Click() {
LaunchCalendar(Ctrl('JCFUNSQXBI'));
}
function VEABFZIUKQ_Valid(e) {
if (Eq(m_cFunction,'query')) {
DoQuery('NOAGE','C',w_NOAGE,ChkboxValueAssign(Ctrl('VEABFZIUKQ'),'S','N','C',1,0,''));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_NOAGE(ChkboxValueAssign(Ctrl('VEABFZIUKQ'),'S','N','C',1,0,''),null,e);
return l_bSetResult;
}
}
function VEABFZIUKQ_OnFocus(e) {
SetActiveField(Ctrl('VEABFZIUKQ'),true);
}
function VEABFZIUKQ_OnBlur(e) {
SetActiveField(Ctrl('VEABFZIUKQ'),false);
}
function VORELZCTGG_Click() {
if (false || WindowConfirm(Translate('11929722326'))) {
arrt_delega_noage([["pRapporto",w_RAPPORTO],["pDelega",w_CODINTER],["pAGE",w_NOAGE]])
}
}
_ResetTracker();
function setEventHandlers() {
var c;
function VORELZCTGG_wrap_OnClick(event) {
return VORELZCTGG_Click();
}
SPBOUILib.SetButtonClick('VORELZCTGG',VORELZCTGG_wrap_OnClick);
...
function BJKHVZBXDR_ZOOM_setHandlers() {
function BJKHVZBXDR_ZOOM_wrap_OnClick(event) {
return (Ne(m_cFunction,'view') && Ne(m_cFunction,'query')?BJKHVZBXDR_ZOOM_Click():void(0));
}
SPBOUILib.SetButtonClick('BJKHVZBXDR_ZOOM',BJKHVZBXDR_ZOOM_wrap_OnClick,SPBOUILib.Row.IN_GRID);
}
SPBOUILib.SetPicker('BJKHVZBXDR',BJKHVZBXDR_ZOOM_setHandlers);
...
function JCFUNSQXBI_ZOOM_setHandlers() {
function JCFUNSQXBI_ZOOM_wrap_OnClick(event) {
return (Ne(m_cFunction,'view') && Ne(m_cFunction,'query')?JCFUNSQXBI_ZOOM_Click():void(0));
}
SPBOUILib.SetButtonClick('JCFUNSQXBI_ZOOM',JCFUNSQXBI_ZOOM_wrap_OnClick,SPBOUILib.Row.IN_GRID);
}
SPBOUILib.SetPicker('JCFUNSQXBI',JCFUNSQXBI_ZOOM_setHandlers);
function VEABFZIUKQ_wrap_Valid(event) {
return VEABFZIUKQ_Valid(event);
}
SPBOUILib.SetCheckboxClick('VEABFZIUKQ',VEABFZIUKQ_wrap_Valid);
function VEABFZIUKQ_wrap_OnFocus(event) {
return VEABFZIUKQ_OnFocus(event);
}
SPBOUILib.SetInputEnter('VEABFZIUKQ',VEABFZIUKQ_wrap_OnFocus);
function VEABFZIUKQ_wrap_Blur(event) {
return VEABFZIUKQ_OnBlur(event);
}
SPBOUILib.SetCheckboxExit('VEABFZIUKQ',VEABFZIUKQ_wrap_Blur);
if (m_bExtended) {
...
m_oFather.i_last_focused_item='child_ardt_delegabo';
...
function Help() {
windowOpenForeground('../doc/ardt_delegabo_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
return 'delegabo'+'\\'+CPLib.ToCPStr(w_RAPPORTO);
}
...
p_d.setValue('ardt_delegabo',m_cSelectedPage);
...
_Obli('NOAGE',0,'VEABFZIUKQ',false)
