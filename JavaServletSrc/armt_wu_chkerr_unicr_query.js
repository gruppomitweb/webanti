...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('TYZCFXYXSP').value,'N'),w_WUCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('wuchkunicr',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'WUCODICE') &&  ! (Empty(HtW(Ctrl('TYZCFXYXSP').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.WUCODICE',HtW(Ctrl('TYZCFXYXSP').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_wu_chkerr_unicr');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wuchkunicr',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['TYZCFXYXSP'],i=0;i>=0;i--){
