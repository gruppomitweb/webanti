...
public class aosdt_ind_mit extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ba7d6961*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "ind_mit";
    }
    public String w_IDMOD;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_IDMOD,context,"ind_mit","IDMOD","C",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_ind_mit;
  ...
  public String getEntityTitle() {
    return "Indicatori anomalia mit";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "aosdt_ind_mit";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 1290;
    l_eg.m_nEntityHeight = 355;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"ind_mit"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,aosdt_ind_mitBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11999868303",p_Context.Translate("Descrizione del indicatore %1")},
    {"1305674908",p_Context.Translate("ID del indicatore")},
    {"1590172100",p_Context.Translate("Apri Descrizione")},
    {"1776130599",p_Context.Translate("Indicatori anomalia mit")},
  ...
  void WriteStyles(aosdt_ind_mitBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../aosdt_ind_mit.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(
       "<style>" +
      "");
      status.out.println(".UZHWXXEKCO {top:0px;position:absolute;left:0px;height:18px;width:116px;background-color:Transparent;z-index:1; }");
      status.out.println(".EALKDPXAET {top:0px;position:absolute;left:119px;height:18px;width:167px;background-color:Transparent;z-index:1; }");
      status.out.println(".KWRCVYJSPV {top:0px;position:absolute;left:290px;color:#000000;font-family:icons8_win10;font-size:14px;cursor:pointer;border-style:none;z-index:3;}");
      status.out.println(".YMZFJACJWK {top:0px;position:absolute;left:313px;height:18px;width:932px;overflow:auto;background-color:Transparent;z-index:1; }");
      status.out.println(
       "</style>" +
      "");
    } else {
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('aosdt_ind_mit',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"aosdt_ind_mit",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(aosdt_ind_mitBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:355px;width:1290px;display:none'>" +
    "");
  ...
  void WriteControls_p1_l0(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface(status.context,"Descrizione")) {
      ...
      status.out.println(
       "<textarea name='DESCRI_layer' id='ATCXZHNKXY' class='Memo "+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+"' title="+SPLib.ToHTMLValue(status.context.Translate("Descrizione"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" cols='20' rows='2'>" +
       "</textarea>" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
  ...
  void WriteControls_p1_b0(aosdt_ind_mitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("Indicatore")) {
      ...
      status.out.println(
       "<script>" +
      "");
      status.out.println("ToggleCollapsibleBox.subBands['ROSLSYYTGJ']=['band_1_0','GridTable_Band','band_2_98'];");
      status.out.println(
       "</script>" +
      "");
      if (status.HasZoneAltInterface("Indicatore@BEGIN") || status.HasZoneAltInterface("Indicatore@END")) {
  ...
  void WriteControls_p1_b1_CloseBand(aosdt_ind_mitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1_b0(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_0' style='position:relative;height:21px;display:block' page='1'>" +
    "");
    status.out.println(
     "<div name id='GridTable_Header' class='GridTable_Header' style='position:absolute;top:0px;left:-1px;height:20px;width:1267px;z-index:0'>" +
     "</div>" +
    "");
    status.out.println(
     "<span class='SPLabel GridTable_HeaderTitle PrimaryKeyHeaderLabel' id='BJEPNVQTTI_DIV'>" +
     "<label id='BJEPNVQTTI' for='UZHWXXEKCO'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("ID del indicatore"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
     "<span class='SPLabel GridTable_HeaderTitle' id='EHNCCRAZSJ_DIV'>" +
     "<label id='EHNCCRAZSJ' for='EALKDPXAET'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Progressivo indicatore"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
     "<span class='SPLabel GridTable_HeaderTitle' id='TMVRTXYLAV_DIV'>" +
     "<label id='TMVRTXYLAV' for='YMZFJACJWK'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Descrizione del indicatore (Cliccare sulla matita per aprire il dettaglio)"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
  ...
  void WriteControls_p1_b1_b1(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("GridTable")) {
      ...
      status.out.println(
       "<div id='GridTable_Container' style='position:relative;top:0px;left:-1px;min-height:75px;max-height:156px;width:1267px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:81px;width:1267px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:1267px;min-height:75px;max-height:156px;overflow:auto;'>" +
       "<table id='GridTable' style='position:relative;top:0px;left:0px;width:1261px' cellspacing='0' cellpadding='0' border='0'>" +
       "<tr>" +
       "<td></td>" +
       "</tr>" +
       "</table>" +
      ...
      if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
        status.out.println(
         "<div id='GridTable_ButtonPanel' style='position:absolute;top:0px;left:1px;z-index:4'>" +
         ""+printButtonPanelContent(status,1,"upper_right",true,getDetailMenuItems(),false,"")+"" +
         "</div>" +
        "");
      }
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:118px;height:100%;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:289px;height:100%;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:312px;height:100%;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
      if (status.HasZoneAltInterface("GridTable@BEGIN") || status.HasZoneAltInterface("GridTable@END")) {
  ...
  void WriteControls_p1_b1_b1_CloseBand(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "</div>" +
    "");
    status.out.println(
  ...
  void HTMLRowScript(aosdt_ind_mitBO BO,ServletStatus status) {
    ...
    status.out.println("s='<div style=\"position:relative;width:1245px;height:14px;\"';");
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     "<input name='IDIND' id='UZHWXXEKCO' type='text' class='Field PrimaryKeyField "+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+" GridTable_EditRowItem ' value='\"+WtH(ToHTML(w_IDIND),'C',10,0,'')+\"' title="+SPLib.ToHTMLValue(status.context.Translate("ID del indicatore"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query")?""+status.m_nTabIndex+"":"-1")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='10' size='10' fieldtype='C'>" +
    "\"");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     "<input name='PROGIND' id='EALKDPXAET' type='text' class='Field GridTable_EditRowItem ' value='\"+WtH(ToHTML(w_PROGIND),'C',10,0,'')+\"' title="+SPLib.ToHTMLValue(status.context.Translate("Progressivo indicatore"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='10' size='10' fieldtype='C'>" +
    "\"");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     "<textarea name='DESCRI' id='YMZFJACJWK' class='Memo GridTable_EditRowItem ' title='\"+Strtran(AlertErrorMessage('11999868303',w_DESCRI),'\\\\n','&#10;')+\"' tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" cols='20' rows='2'>" +
     "</textarea>" +
    "\"");
    ...
    status.out.println("s=s+\""+
     "<input type='text' class='Field PrimaryKeyField "+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+" Disabled GridTable_QueryRowItem UZHWXXEKCO' value='\"+WtH(ToHTML(w_IDIND),'C',10,0,'')+\"' tabindex='-1' readonly size='10'>" +
    "\"");
    status.out.println("}");
    status.out.println("if ( ! (IsHiddenByStateDriver('PROGIND'))) {");
    status.out.println("s=s+\""+
     "<input type='text' class='Field Disabled GridTable_QueryRowItem EALKDPXAET' value='\"+WtH(ToHTML(w_PROGIND),'C',10,0,'')+\"' tabindex='-1' readonly size='10'>" +
    "\"");
    status.out.println("}");
    status.out.println("s=s+\""+
    ...
    status.out.println("s=s+\""+
     "<textarea class='GridTable_QueryRowItem YMZFJACJWK' disabled cols='20' rows='2'>" +
     "\"+WtH(w_DESCRI,'M',10,0,'"+""+"')+\"" +
     "</textarea>" +
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:1245px;height:14px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b1_b2(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_2_98' style='position:relative;height:8px;display:block' page='2'>" +
    "");
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls_p1_b2(aosdt_ind_mitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("Subindicatori")) {
      ...
      status.out.println(
       "<script>" +
      "");
      status.out.println("ToggleCollapsibleBox.subBands['ZURPIOBYMT']=['band_2_107','band_2_111','band_2_311'];");
      status.out.println(
       "</script>" +
      "");
      if (status.HasZoneAltInterface("Subindicatori@BEGIN") || status.HasZoneAltInterface("Subindicatori@END")) {
  ...
  void WriteControls_p1_b3_CloseBand(aosdt_ind_mitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3_b0(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_2_107' style='position:relative;height:4px;display:block' page='2'>" +
    "");
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls_p1_b3_b1(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_2_111' style='position:relative;display:block' page='2'>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<iframe name='aosdt_sub_ind_mit' id='ABXVVUNOEJ' frameborder='0' allowtransparency='true' toResize='no' tabindex='"+status.m_nTabIndex+"' src='javascript:[].join()'>" +
     "</iframe>" +
    "");
    status.out.println(
     "</div>" +
    ...
    status.out.println(
     "<script>" +
    "");
    status.out.println("ToggleCollapsibleBox.children['band_2_111']=['ABXVVUNOEJ'];");
    status.out.println(
     "</script>" +
    "");
  }
  void WriteControls_p1_b3_b2(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_2_311' style='position:relative;height:4px;display:block' page='2'>" +
    "");
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls_p1_b4(aosdt_ind_mitBO BO,ServletStatus status) {
  ...
  static void PrintState(aosdt_ind_mitBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(aosdt_ind_mitBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
    double i;
    i = 0;
    while (CPLib.lt(i,BO.Rows())) {
      BO.SetRow(i);
      if (BO.aosdt_sub_ind_mit.IsUpdated()) {
        BO.m_cWv_aosdt_sub_ind_mit = BO.aosdt_sub_ind_mit.GetContext();
      }
      BO.WorkToTrs();
      i = i+1;
    }
  }
  static void PrintStateWorkVars(aosdt_ind_mitBL BO,ServletStatus status) {
    ...
    if (CPLib.eq(BO.m_cWv_aosdt_sub_ind_mit,"_not_empty_") && status.m_bBrowserUpdate) {
      status.out.print(","+"m_cWv_aosdt_sub_ind_mit");
    } else if (CPLib.eq(BO.m_cWv_aosdt_sub_ind_mit,"_not_empty_")) {
      BO.m_cWv_aosdt_sub_ind_mit = BO.aosdt_sub_ind_mit.GetContext();
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_aosdt_sub_ind_mit,"C",0,0));
    } else {
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_aosdt_sub_ind_mit,"C",0,0));
    }
  ...
  static void PrintStateDetailWorkVars(aosdt_ind_mitBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(aosdt_ind_mitBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      aosdt_ind_mitBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (aosdt_ind_mitBO.TrsRow) BO.m_oDeletedRows.get(k);
        ...
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.m_cWv_aosdt_sub_ind_mit,"C",0,0)+",";
  ...
  void WriteMainFormVarScript(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(355,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(1290,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"aosdt_ind_mit"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("aosdt_ind_mit","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"aosdt_ind_mit",BO.GetCurrentState());
    ...
    status.out.println("var m_nLayerInBodyRowHeight = 77;");
    ...
    status.out.println("this.m_cWv_aosdt_sub_ind_mit=m_cWv_aosdt_sub_ind_mit;");
    ...
    status.out.println("this.DESCRI_layer=arguments[3];");
    status.out.println("this.opk_IDIND=arguments[4];");
    status.out.println("this.m_cWv_aosdt_sub_ind_mit=arguments[5];");
    ...
    if (CPLib.ne(status.m_cClientMode,"query")) {
      ...
      status.out.println("m_cWv_aosdt_sub_ind_mit=row.m_cWv_aosdt_sub_ind_mit;");
      ...
      status.out.println("m_cWv_aosdt_sub_ind_mit='';");
  ...
  void WriteMainFormScript(aosdt_ind_mitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_aosdt_ind_mit);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_aosdt_ind_mit();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('aosdt_ind_mit',Ne(m_nChildStatus,0));");
    } else {
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('ACPSBOFDKQ')) SetDisabled(c,true);");
        ...
        status.out.println("var c;");
        status.out.println("if(c=Ctrl('UZHWXXEKCO')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('EALKDPXAET')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('YMZFJACJWK')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('ATCXZHNKXY')) SetDisabled(c,true);");
      }
    ...
    if (CPLib.eq(status.m_cClientMode,"query") && CPLib.gt(status.m_nChildStatus,0)) {
      ...
      status.out.println("SetDisabled('ACPSBOFDKQ',m_bRowsChild);");
    }
    status.out.println("}");
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      status.out.println("if (Ctrl('ABXVVUNOEJ')) {");
      status.out.println("ChangeIframeSrcWithoutPushingHistory('ABXVVUNOEJ',linkpc_url('ABXVVUNOEJ'));");
      status.out.println("}");
    ...
    status.out.println("m_oFather.m_oaosdt_ind_mit=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
    status.out.println("function ChildSelectedPage(name) {");
    status.out.println("var l_cRes = '';");
    status.out.println("if (Eq(name,'aosdt_sub_ind_mit')) {");
    status.out.println("l_cRes='"+SPLib.GetSelectedPage(status.m_cSelectedPage,"aosdt_sub_ind_mit")+"';");
  ...
  aosdt_ind_mitBO getBO(ServletStatus status) {
    aosdt_ind_mitBO BO = new aosdt_ind_mitBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(aosdt_ind_mitBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("ind_mit",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,aosdt_ind_mitBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,aosdt_ind_mitBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(aosdt_ind_mitBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_ind_mit;
    String l_cPhName = BO.m_cPhName_ind_mit;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = aosdt_ind_mitBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_ind_mit+".IDMOD"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_ind_mit+".IDMOD"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("ind_mit");
    String l_cPhName = CPSql.ManipulateTablePhName("ind_mit",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,aosdt_ind_mitBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_ind_mit),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_ind_mit,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_ind_mit+".IDMOD) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_ind_mit,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_ind_mit,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_ind_mit+".IDMOD",CPLib.GetTablesWithAliases(BO.m_cPhName_ind_mit,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,aosdt_ind_mitBO BO) {
  ...
  void Edit(ServletStatus status,aosdt_ind_mitBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,aosdt_ind_mitBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,aosdt_ind_mitBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,aosdt_ind_mitBO BO) {
  ...
  void EditLoad(ServletStatus status,aosdt_ind_mitBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,aosdt_ind_mitBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,aosdt_ind_mitBO BO,SPParameterSource source) {
    BO.Load(status.w_IDMOD);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,aosdt_ind_mitBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,aosdt_ind_mitBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,aosdt_ind_mitBO BO) {
    ...
    item.put("altInterfaceFor","aosdt_ind_mit");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Indicatore\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Indicatori\",\"groupName\":\"Indicatori\",\"h\":106,\"name\":\"Indicatore\",\"page\":1,\"shrinkable\":true,\"spuid\":\"ROSLSYYTGJ\",\"stretch\":true,\"tabindex\":1,\"title_caption\":\"Indicatori\",\"titled\":true,\"type\":\"Box\",\"w\":1286,\"x\":0,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"autoexpand\":true,\"h\":77,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"GridTable_Band\",\"type\":\"BodyDetail\",\"w\":1301,\"x\":1,\"y\":21,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":18,\"name\":\"label_BJEPNVQTTI\",\"page\":1,\"spuid\":\"BJEPNVQTTI\",\"tabindex\":2,\"type\":\"Label\",\"value\":\"ID del indicatore\",\"w\":116,\"x\":1,\"y\":1,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":18,\"name\":\"label_EHNCCRAZSJ\",\"page\":1,\"spuid\":\"EHNCCRAZSJ\",\"tabindex\":3,\"type\":\"Label\",\"value\":\"Progressivo indicatore\",\"w\":167,\"x\":120,\"y\":1,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":18,\"name\":\"label_TMVRTXYLAV\",\"page\":1,\"spuid\":\"TMVRTXYLAV\",\"tabindex\":4,\"type\":\"Label\",\"value\":\"Descrizione del indicatore (Cliccare sulla matita per aprire il dettaglio)\",\"w\":944,\"x\":291,\"y\":1,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Subindicatori\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Subindicatori\",\"groupName\":\"Indicatori\",\"h\":208,\"name\":\"Subindicatori\",\"page\":1,\"shrinkable\":true,\"spuid\":\"ZURPIOBYMT\",\"stretch\":true,\"tabindex\":2,\"title_caption\":\"Subindicatori\",\"titled\":true,\"type\":\"Box\",\"w\":1290,\"x\":0,\"y\":107,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"autoexpand\":true,\"fixed\":true,\"h\":200,\"name\":\"aosdt_sub_ind_mit\",\"page\":1,\"repeated\":true,\"spuid\":\"ABXVVUNOEJ\",\"tabindex\":7,\"type\":\"Iframe\",\"w\":1285,\"x\":1,\"y\":111,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Descrizione\",\"box_type\":\"layer\",\"comment\":\"Descrizione dell"+"'"+"indicatore\",\"fixed\":true,\"h\":77,\"name\":\"Descrizione\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VGASZRXNOJ\",\"tabindex\":8,\"title_caption\":\"Descrizione dell"+"'"+"indicatore\",\"type\":\"Box\",\"w\":1245,\"x\":1,\"y\":318,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Descrizione\",\"container\":\"VGASZRXNOJ\",\"fixed\":true,\"h\":72,\"name\":\"DESCRI_layer\",\"page\":1,\"repeated\":true,\"scroll\":true,\"spuid\":\"ATCXZHNKXY\",\"tabindex\":9,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":1240,\"x\":3,\"y\":320,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":42,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"GridTable_Band\",\"type\":\"Box\",\"w\":1245,\"x\":1,\"y\":0,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":19,\"name\":\"button_KWRCVYJSPV\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"KWRCVYJSPV\",\"src\":\"..\\/{\\\"Char\\\":\\\"59899\\\",\\\"Color\\\":\\\"#000000\\\",\\\"FontName\\\":\\\"icons8_win10\\\",\\\"FontWeight\\\":\\\"\\\",\\\"Size\\\":\\\"14\\\"}\",\"tabindex\":5,\"type\":\"Image\",\"value\":\"Apri Descrizione\",\"w\":20,\"x\":291,\"y\":23,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\",\"zoom\":\"usrz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"ID del indicatore\",\"h\":19,\"maxlength\":10,\"name\":\"IDIND\",\"page\":1,\"repeated\":true,\"spuid\":\"UZHWXXEKCO\",\"tabindex\":3,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":116,\"x\":1,\"y\":23,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Descrizione del indicatore\",\"h\":19,\"name\":\"DESCRI\",\"page\":1,\"repeated\":true,\"scroll\":true,\"spuid\":\"YMZFJACJWK\",\"tabindex\":6,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":932,\"x\":314,\"y\":23,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Progressivo indicatore\",\"h\":19,\"maxlength\":10,\"name\":\"PROGIND\",\"page\":1,\"repeated\":true,\"spuid\":\"EALKDPXAET\",\"tabindex\":4,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":167,\"x\":120,\"y\":23,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,aosdt_ind_mitBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(aosdt_ind_mitBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='aosdt_ind_mit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/aosdt_ind_mit_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/aosdt_ind_mit_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_DESCRI_layer=a[4];");
      status.out.println("m_cWv_aosdt_sub_ind_mit=a[5];");
      status.out.println("if (Gt(a.length,6)) {");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/aosdt_ind_mit_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(355,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void RaiseLoadedEntity(aosdt_ind_mitBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(aosdt_ind_mitBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(aosdt_ind_mitBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(aosdt_ind_mitBO BO,ServletStatus status) {
  ...
  void WriteExitPage(aosdt_ind_mitBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(aosdt_ind_mitBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(aosdt_ind_mitBO BO,ServletStatus status) throws IOException {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      CPLib.AddToSet(callable,"aosdt_sub_ind_mit");
    } else {
  ...
  public String GetCheckVars(ServletStatus status,aosdt_ind_mitBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,aosdt_ind_mitBO BO) {
    boolean result = true;
  ...
  static class OrderBodyByComparator implements Comparator<aosdt_ind_mitBL.TrsRow> {
    public int compare(aosdt_ind_mitBL.TrsRow o1,aosdt_ind_mitBL.TrsRow o2) {
