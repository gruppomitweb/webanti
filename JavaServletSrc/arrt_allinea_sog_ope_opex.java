...
public class arrt_allinea_sog_ope_opex extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*e497f284*!";
  public class ServletStatus extends SPRoutineServletStatus {
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_nomefile;
    public String w_nomefile;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_tipoall;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_tipoall = source.Has("w_tipoall");
      if (m_bpassedw_tipoall) {
        w_tipoall = source.GetParameter("w_tipoall",source.GetParameter("tipoall",""));
      } else {
        if (source.IsForwarded()) {
          w_tipoall = source.GetCaller().GetString("tipoall","C",1,0);
          m_bpassedw_tipoall = true;
        } else {
          w_tipoall = source.GetParameter("tipoall",source.GetParameter("w_tipoall",""));
        }
      }
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_allinea_sog_ope_opex";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_anno) ||  ! (status.m_bpassedw_tipoall) ||  ! (status.m_bpassedw_nomefile);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_tipoall)) {
      status.out.println("l_cV=WtA(l_oWnd('w_tipoall',"+SPLib.ToJSValue(status.w_tipoall,"C",1,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_tipoall,"C",1,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_tipoall',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_allinea_sog_ope_opexR Rcalled;
    Rcalled = new arrt_allinea_sog_ope_opexR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_allinea_sog_ope_opexR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_allinea_sog_ope_opexR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_allinea_sog_ope_opex"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
          if (CPLib.Empty(Rcalled.m_cLastMsgError) &&  ! (status.m_bApplet)) {
            status.out.println("Error message:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(status.m_cFaultTrace)+"");
          }
        }
        status.out.println("js:w_nomefile="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_nomefile),"?",0,0)+"");
        status.out.println("js:w_tipoall="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_tipoall),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_tipoall,null)) {");
        status.out.println("window[f].w_tipoall="+SPLib.ToJSValue(status.w_tipoall,"C",0,0)+";");
        status.out.println("}");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_allinea_sog_ope_opex");
      if ( ! (CPLib.IsNull(l))) {
