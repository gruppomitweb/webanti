...
// * --- Fine Area Manuale
public class armt_tbspecieBO extends armt_tbspecieBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbspecieBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbspecie");
    }
  ...
  public boolean Load(String p_CODSPE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbspecie,m_cPhName_tbspecie,m_cVirtName_tbspecie,m_cVirtName_tbspecie+".CODSPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODSPE,"C",4,0),m_cServer_tbspecie,p_CODSPE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbspecie")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbspecie);
      l_cSql = "insert into "+m_oWrInfo_tbspecie.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODSPE,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+"RISCHIO,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbspecie.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbspecie")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_tbspecie.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbspecie")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbspecie,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbspecie);
      l_cSql = "Update "+m_oWrInfo_tbspecie.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODSPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODSPE,"C",4,0),m_cServer_tbspecie,w_CODSPE);
      l_cSql = l_cSql+m_oWrInfo_tbspecie.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbspecie")) {
  ...
  public boolean Delete(String p_CODSPE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbspecie.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODSPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODSPE,"C",4,0),m_cServer_tbspecie,w_CODSPE);
          l_cSql = l_cSql+m_oWrInfo_tbspecie.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbspecie")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbspecie,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
