...
public class armt_snai_oper_storico extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*bda95530*!";
  static public class ServletStatus extends SPMasterServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "cgo_operazioni_storico";
    }
    public String w_SNAINUMOPE;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_SNAINUMOPE,context,"cgo_operazioni_storico","SNAINUMOPE","C",15,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    armt_snai_oper_storicoBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_cgo_operazioni_storico;
  ...
  public String getEntityTitle() {
    return "CGO Operazioni Agenzie (Storico)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_snai_oper_storico";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 1023;
    l_eg.m_nEntityHeight = 981;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"cgo_operazioni_storico","cgo_tbtipgio","tbcitta","tbstati","tbtipatt","tbtipreg","tbcauana","personbo","intermbo","anadip","operazbo","tbcausin","cgo_causali","cgo_tipdoc","cgo_autorita"};
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","Dati Base","page"),new ZoneItem("page_2","Documenti","page"),new ZoneItem("page_3","Questionario ADV","page"),new ZoneItem("Base","Dati Base","horizontal_section"),new ZoneItem("Operazione","Dati Operazione","horizontal_section"),new ZoneItem("Soggetto","Dati Soggetto","horizontal_section"),new ZoneItem("Multiple","Dati Operazioni Multiple","horizontal_section"),new ZoneItem("Documenti","Documenti","horizontal_section")};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("CCDATSCA","C",7,0, "hide","Carta Di Credito Scadenza"),
                   ...
                   new SPItem("FLGSBLOCCO","C",1,0, "hide","Sblocco Operazione"),
                   new SPItem("SEGNO","C",1,0, "show","Segno"),
                   new SPItem("xCGOFLGPROF","C",1,0, "hide","Flag Richiesta Professione"),
                   new SPItem("PROFESSIONE","C",40,0, "show","Professione")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_snai_oper_storicoBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"10693963392",p_Context.Translate("Sblocco Operazione")},
    {"10787648910",p_Context.Translate("Stampa Download")},
    {"10848850431",p_Context.Translate("CAB Dipendenza")},
    {"10924022783",p_Context.Translate("Impossibile cancellare un'operazione storicizzata")},
    ...
    {"1846422530",p_Context.Translate("Impossibile modificare un'operazione bloccata")},
    {"1874354174",p_Context.Translate("CGO Operazioni Agenzie (Storico)")},
  ...
  void WriteStyles(armt_snai_oper_storicoBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_snai_oper_storico.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println("var tabs = new Z.Tabs({container:'tabsContainer',useseparator:GetStyleVariable('formTabUseSeparator', false, ''),expandcollapse:(typeof(sv_TabstripCanCollapse)!='undefined'?sv_TabstripCanCollapse:GetStyleVariable('formTabCanCollapse', false, '')),contents:[{name:\"page_1\", element:\"page_1\", caption:"+SPLib.ToJSValue(status.context.Translate("Dati Base"),"C",0,0)+"},{name:\"page_2\", element:\"page_2\", caption:"+SPLib.ToJSValue(status.context.Translate("Documenti"),"C",0,0)+"},{name:\"page_3\", element:\"page_3\", caption:"+SPLib.ToJSValue(status.context.Translate("Questionario ADV"),"C",0,0)+"}],tabstrip_class: ''});Z.Tabs.UseDeprecation();");
    status.out.println("Z.Tabs.Set('tabs',tabs);");
    status.out.println("} else {");
    status.out.println("var tabs = Z.Tabs.Get('tabs')");
    status.out.println("}");
    AddExtendedTabs(status);
    status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_snai_oper_storico',false);");
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_snai_oper_storico",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  // GetPortlet_Documenti_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KUBMCPCHPZ_Vars(armt_snai_oper_storicoBO BO,ServletStatus status) {
    ...
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.addAttribute(inclusionVars,"aOrigine","S");
  ...
  void WriteControls(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void WriteControls_p1(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:981px;width:1023px;display:none'>" +
    "");
  ...
  void WriteControls_p2(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_2' class='PageContainer' style='position:relative;height:497px;width:1023px;display:none'>" +
    "");
  ...
  void WriteControls_p3(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_3' class='PageContainer' style='position:relative;height:812px;width:1023px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<div id='YORQOIOBQN' class='SectionBody' style='position:relative;height:211px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("Operazione")) {
      ...
      if (status.HasZoneAltInterface("Operazione@BEGIN") || status.HasZoneAltInterface("Operazione@END")) {
        status.out.println(
         "<div id='YORQOIOBQN_boxContent' style='position:relative;height:211px;display:block'>" +
        "");
      }
      CPResultSet l_rsLoad=null;
      VQRHolder l_VQRHolder=null;
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='TOTLIRE' id='HUAGBHPUNH' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_TOTLIRE,"N",15,2,"999,999,999,999.99", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query")?""+status.m_nTabIndex+"":"-1")+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='21' size='15' fieldtype='N'>" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
      ...
      "");
      status.out.println(
       "<input name='DATAOPE' id='YLIKFVPZJA' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_DATAOPE,"D",8,0,SPLib.GetUserDatePicture(status.context.GetLanguage(),"DD-MM-YYYY"), status.m_oRegSettings)+" tabindex='-1' readonly maxlength='10' size='8' fieldtype='D'>" +
      "");
      status.out.println(
      ...
      status.out.println(
       ...
       "<label id='RRAEQSPNFR' for='UIMDWYEZSX'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Segno:"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<select name='SEGNO' id='UIMDWYEZSX' class='Combobox Disabled' disabled>" +
       "<option value="+SPLib.ToHTMLValue("","C",0,0)+">" +
       "</option>" +
       ...
       "</select>" +
       "<script type='text/javascript'>" +
       "selectCombo(Ctrl('UIMDWYEZSX'),w_SEGNO,'C');" +
       "</script>" +
      "");
      status.out.println(
  ...
  void WriteControls_p1_b3_CloseBand(armt_snai_oper_storicoBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("Operazione")) {
      ...
      status.out.println("Ctrl('YORQOIOBQN').setAttribute('band_height',211);");
    }
  ...
  void WriteControls_p1_b4(armt_snai_oper_storicoBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_BEFORE_RWCKHDGZVX")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_BEFORE_RWCKHDGZVX' style='position:relative;height:2px;display:block' page='1'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_BEFORE_RWCKHDGZVX")) {
      status.out.println(
       "</div>" +
      "");
      status.out.println(
       "<script>" +
      "");
      status.out.println("Ctrl('shadowBand_BEFORE_RWCKHDGZVX').style.height='auto';");
      status.out.println(
       "</script>" +
      "");
    } else {
  ...
  void WriteControls_p1_b5(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
     "<div name='Multiple' id='RWCKHDGZVX_DIV' class='BoxContainer SPSection_collapsed SectionContainerWithTitle'>" +
    "");
    if (CPLib.eq(status.GetBoxHeaderInclude(),"")) {
      status.out.println(
       "<div id='RWCKHDGZVX_HEADER' class='BoxHeader'>" +
      "");
      status.out.println(
       "<span id='RWCKHDGZVX_HEADER_CAPTION' class='BoxHeaderCaption'>" +
       ""+status.context.Translate("Dati Operazioni Multiple")+"" +
       "</span>" +
      "");
      status.out.println(
       "<img id='RWCKHDGZVX_HEADER_ICON' class='BoxHeaderIcon' src='"+com.zucchetti.sitepainter.Library.GetStyleVariable(status.m_cTheme, "boxHeaderExpandImage", "../"+status.m_cTheme+"/formPage/box_expand.gif")+"' onclick='ToggleCollapsibleBox(\"RWCKHDGZVX\")' alt='"+status.context.Translate("Expand/Collapse")+"' title='"+status.context.Translate("Expand/Collapse")+"' tabindex='"+status.m_nTabIndex+"'>" +
      "");
      status.out.println(
       "</div>" +
      "");
    } else {
      SPLib.Include(servletContext,status,status.GetBoxHeaderInclude()+"?ForcedPortletUID=RWCKHDGZVX_HEADER&m_cBoxId=RWCKHDGZVX&m_cInitialState=close&m_cTheme="+status.m_cTheme+"&m_nTabIndex="+status.m_nTabIndex+"&m_cCaption="+com.zucchetti.sitepainter.Library.URLencode(status.context.Translate("Dati Operazioni Multiple"))+"");
    }
    status.out.println(
     "<div id='RWCKHDGZVX' class='SectionBody' style='position:relative;height:138px;display:none'>" +
    "");
    ...
    if (status.includeZoneAltInterface("Multiple")) {
      ...
      if (status.HasZoneAltInterface("Multiple@BEGIN") || status.HasZoneAltInterface("Multiple@END")) {
        status.out.println(
         "<div id='RWCKHDGZVX_boxContent' style='position:relative;height:138px;display:block'>" +
        "");
      }
      CPResultSet l_rsLoad=null;
      VQRHolder l_VQRHolder=null;
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<iframe name='ardt_cgo_oper_storico_multi' id='CJPXRAUTXZ' frameborder='0' allowtransparency='true' toResize='no' tabindex='"+status.m_nTabIndex+"' src='javascript:[].join()'>" +
       "</iframe>" +
      "");
      if (status.HasZoneAltInterface("Multiple@BEGIN") || status.HasZoneAltInterface("Multiple@END")) {
        status.out.println(
  ...
  void WriteControls_p1_b5_CloseBand(armt_snai_oper_storicoBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("Multiple")) {
      status.out.println("Ctrl('RWCKHDGZVX').style.height='auto';");
    } else {
      status.out.println("Ctrl('RWCKHDGZVX').setAttribute('band_height',138);");
    }
    status.out.println("Ctrl('RWCKHDGZVX').setAttribute('status','close');");
    status.out.println(
     "</script>" +
    "");
    status.out.println(
     "<script>" +
    "");
    status.out.println("ToggleCollapsibleBox.children['RWCKHDGZVX']=['CJPXRAUTXZ'];");
    status.out.println(
     "</script>" +
    "");
  }
  void WriteControls_p1_b6(armt_snai_oper_storicoBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_BEFORE_AQSQVBKXAL")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_BEFORE_AQSQVBKXAL' style='position:relative;height:1px;display:block' page='1'>" +
    "");
  ...
  void WriteControls_p1_b7(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<div id='AQSQVBKXAL' class='SectionBody' style='position:relative;height:503px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("Soggetto")) {
      ...
      if (status.HasZoneAltInterface("Soggetto@BEGIN") || status.HasZoneAltInterface("Soggetto@END")) {
        status.out.println(
         "<div id='AQSQVBKXAL_boxContent' style='position:relative;height:503px;display:block'>" +
        "");
      }
      CPResultSet l_rsLoad=null;
      VQRHolder l_VQRHolder=null;
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='NUMTEL' id='VFKGHJLVGB' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_NUMTEL,"C",20,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='20' size='20' fieldtype='C'>" +
      "");
      status.out.println(
      ...
      "");
      WriteControls_p1_b7_1(BO,status);
    }
  }
  void WriteControls_p1_b7_1(armt_snai_oper_storicoBO BO,ServletStatus status) {
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
    ...
    "");
    status.out.println(
     "<span class='SPLabel' id='KUMAOEWIHF_DIV'>" +
     "<label id='KUMAOEWIHF' for='KPSCOABZLB'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Prov.:"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
     "<span class='SPLabel' id='GELECSVPUB_DIV'>" +
     "<label id='GELECSVPUB' for='RDAVOEETJG'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("CAP:"))+"" +
     "</label>" +
     "</span>" +
    "");
  ...
  void WriteControls_p1_b7_CloseBand(armt_snai_oper_storicoBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("Soggetto")) {
      ...
      status.out.println("Ctrl('AQSQVBKXAL').setAttribute('band_height',503);");
    }
  ...
  void WriteControls_p1_b8(armt_snai_oper_storicoBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_END_OF_PAGE_1")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_END_OF_PAGE_1' style='position:relative;height:83px;display:block' page='1'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_END_OF_PAGE_1")) {
      ...
      VQRHolder l_VQRHolder=null;
      status.out.println(
       "<input name='PROFESSIONE' id='EWZFJXYEWM' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_PROFESSIONE,"C",40,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='40' size='40' fieldtype='C' style='display:none'>" +
      "");
      status.out.println(
       "<span class='SPLabel' id='EXZMNOSILZ_DIV' style='display:none'>" +
       "<label id='EXZMNOSILZ' for='EWZFJXYEWM'>" +
  ...
  void WriteControls_p2_b0(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void WriteControls_p2_b1(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p2_b1_CloseBand(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void WriteControls_p2_b2(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void WriteControls_p3_b0(armt_snai_oper_storicoBO BO,ServletStatus status) {
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.out.println(
     "<div style='position:relative;height:812px'>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<iframe name='armt_cgo_qadv_operazioni_storico' id='FGDNXNYKKB' frameborder='0' allowtransparency='true' toResize='no' tabindex='"+status.m_nTabIndex+"' src='javascript:[].join()'>" +
     "</iframe>" +
    "");
    status.out.println(
     "</div>" +
  ...
  static void PrintState(armt_snai_oper_storicoBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_snai_oper_storicoBL BO,ServletStatus status) {
    ...
    if (BO.armt_cgo_qadv_operazioni_storico.IsUpdated()) {
      BO.m_cWv_armt_cgo_qadv_operazioni_storico = BO.armt_cgo_qadv_operazioni_storico.GetContext();
    }
  }
  static void PrintStateWorkVars(armt_snai_oper_storicoBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_FLGSBLOCCO,"C",1,0));
    ...
    if (CPLib.eq(BO.m_cWv_armt_cgo_qadv_operazioni_storico,"_not_empty_") && status.m_bBrowserUpdate) {
      status.out.print(","+"m_cWv_armt_cgo_qadv_operazioni_storico");
    } else if (CPLib.eq(BO.m_cWv_armt_cgo_qadv_operazioni_storico,"_not_empty_")) {
      BO.m_cWv_armt_cgo_qadv_operazioni_storico = BO.armt_cgo_qadv_operazioni_storico.GetContext();
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_armt_cgo_qadv_operazioni_storico,"C",0,0));
    } else {
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_armt_cgo_qadv_operazioni_storico,"C",0,0));
    }
  ...
  void WriteMainFormVarScript(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(981,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(1023,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_snai_oper_storico"),"C",0,0)+ ";");
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      status.out.print(",'PSLKLXOQMT':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash(entityGlobals.name+"PSLKLXOQMT",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,tbcitta,"+"default",l_session)+"'"+"]");
      ...
      status.out.print(",'PSLKLXOQMT':['','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,tbcitta,"+"default",l_session)+"'"+"]");
      AppendCallableIDS(status,",");
    }
    status.out.println("}");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_snai_oper_storico","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_snai_oper_storico",BO.GetCurrentState());
    ...
    status.out.println("// * --- armt_snai_oper_storico");
  ...
  void WriteMainFormScript(armt_snai_oper_storicoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('VFKGHJLVGB')) SetDisabled(c,true);");
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      status.out.println("if (Ctrl('CJPXRAUTXZ')) {");
      status.out.println("ChangeIframeSrcWithoutPushingHistory('CJPXRAUTXZ',linkpc_url('CJPXRAUTXZ'));");
      status.out.println("}");
      status.out.println("if (Ctrl('FGDNXNYKKB')) {");
      status.out.println("ChangeIframeSrcWithoutPushingHistory('FGDNXNYKKB',linkpc_url('FGDNXNYKKB'));");
      status.out.println("}");
    ...
    status.out.println("}");
    status.out.println("if (Eq(name,'armt_cgo_qadv_operazioni_storico')) {");
    status.out.println("l_cRes='"+SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_cgo_qadv_operazioni_storico")+"';");
    status.out.println("return l_cRes;");
    status.out.println("}");
    ...
    status.out.println("  arrt_cgo_read_codfisc_s([['pObj',this],['pTipo','I']])");
    status.out.println("  Calculate()");
    status.out.println("}");
    status.out.println("");
    status.out.println("function DatiCN() {");
    status.out.println("  arrt_cgo_read_codfisc_s([['pObj',this],['pTipo','E']])");
    status.out.println("  Calculate()");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
  ...
  armt_snai_oper_storicoBO getBO(ServletStatus status) {
    armt_snai_oper_storicoBO BO = new armt_snai_oper_storicoBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(armt_snai_oper_storicoBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("cgo_operazioni_storico",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_snai_oper_storicoBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    if (CPLib.eq(p_cEvent,"New record")) {
      Calculation_LAEGOZNRMQ(status,BO);
      Calculation_BVTIXLYUGT(status,BO);
    }
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Calculation_LAEGOZNRMQ(ServletStatus status,armt_snai_oper_storicoBO BO) {
    BO.w_IDDOCALL = LibreriaMit.nextUID();
  }
  void Calculation_BVTIXLYUGT(ServletStatus status,armt_snai_oper_storicoBO BO) {
    BO.w_PEP = "N";
  }
  void WriteTableCombo(ServletStatus status,armt_snai_oper_storicoBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
    l_cTmp = "[";
    if (l_rsLoad!=null)
  ...
  CPResultSet QueryKeyCursor(armt_snai_oper_storicoBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_cgo_operazioni_storico;
    String l_cPhName = BO.m_cPhName_cgo_operazioni_storico;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_snai_oper_storicoBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_cgo_operazioni_storico+".SNAINUMOPE",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("cgo_operazioni_storico");
    String l_cPhName = CPSql.ManipulateTablePhName("cgo_operazioni_storico",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,armt_snai_oper_storicoBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_cgo_operazioni_storico,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_cgo_operazioni_storico,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,armt_snai_oper_storicoBO BO) {
  ...
  void Edit(ServletStatus status,armt_snai_oper_storicoBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_snai_oper_storicoBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_snai_oper_storicoBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_snai_oper_storicoBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_snai_oper_storicoBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_snai_oper_storicoBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_snai_oper_storicoBO BO,SPParameterSource source) {
    BO.Load(status.w_SNAINUMOPE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_snai_oper_storicoBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_snai_oper_storicoBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_snai_oper_storicoBO BO) {
    ...
    item.put("pages_names","Dati Base,Documenti,Questionario ADV");
    item.put("altInterfaceFor","armt_snai_oper_storico");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Operazione\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Dati Operazione\",\"h\":211,\"name\":\"Operazione\",\"page\":1,\"shrinkable\":true,\"spuid\":\"YORQOIOBQN\",\"stretch\":true,\"tabindex\":3,\"title_caption\":\"Dati Operazione\",\"titled\":true,\"type\":\"Box\",\"w\":1008,\"x\":8,\"y\":101,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data registrazione\",\"disabled\":\"true\",\"h\":20,\"maxlength\":10,\"name\":\"DATAREG\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"OEIKQOZMYJ\",\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":80,\"x\":927,\"y\":107,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_IKBMEWKQJW\",\"page\":1,\"spuid\":\"IKBMEWKQJW\",\"tabindex\":101,\"type\":\"Label\",\"value\":\"Data registrazione:\",\"w\":151,\"x\":776,\"y\":111,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Operazione\",\"disabled\":\"true\",\"h\":20,\"maxlength\":10,\"name\":\"DATAOPE\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"YLIKFVPZJA\",\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":88,\"x\":136,\"y\":108,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_HVHVAYPDJE\",\"page\":1,\"spuid\":\"HVHVAYPDJE\",\"tabindex\":95,\"type\":\"Label\",\"value\":\"Data di pagamento:\",\"w\":117,\"x\":16,\"y\":112,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"check\",\"comment\":\"
",\"page\":1,\"readonly\":\"true\",\"spuid\":\"RLUMBFHPUD\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":198,\"x\":658,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Tipo gioco\",\"disabled\":\"true\",\"h\":20,\"maxlength\":3,\"name\":\"TIPOGIOCO\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"QZTTRBWAQL\",\"type\":\"Combobox\",\"typevar\":\"character\",\"w\":208,\"x\":136,\"y\":72,\"zone\":\"pag1_5_2\",\"zonepath\":\"[1,5,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_JNQVUOSVPV\",\"page\":1,\"spuid\":\"JNQVUOSVPV\",\"tabindex\":138,\"type\":\"Label\",\"value\":\"Tipo Gioco:\",\"w\":120,\"x\":16,\"y\":76,\"zone\":\"pag1_5_2\",\"zonepath\":\"[1,5,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Piattaforma VLT\",\"disabled\":\"true\",\"h\":20,\"maxlength\":2,\"name\":\"VLTPLAT\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"LVYFMQXNZH\",\"type\":\"Combobox\",\"typevar\":\"character\",\"w\":280,\"x\":504,\"y\":72,\"zone\":\"pag1_5_3\",\"zonepath\":\"[1,5,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_DJRHGZHJSR\",\"page\":1,\"spuid\":\"DJRHGZHJSR\",\"tabindex\":152,\"type\":\"Label\",\"value\":\"Tipo Piattaforma:\",\"w\":112,\"x\":392,\"y\":76,\"zone\":\"pag1_5_3\",\"zonepath\":\"[1,5,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_EZWFUWOKXC\",\"page\":1,\"spuid\":\"EZWFUWOKXC\",\"tabindex\":132,\"type\":\"Label\",\"value\":\"ID BIGLIETTO:\",\"w\":96,\"x\":408,\"y\":52,\"zone\":\"pag1_5_4\",\"zonepath\":\"[1,5,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"ID BIGLIETTO\",\"h\":20,\"maxlength\":30,\"name\":\"CAIDBIGLIETTO\",\"page\":1,\"spuid\":\"BMTLGIUQTH\",\"tabindex\":131,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":280,\"x\":504,\"y\":48,\"zone\":\"pag1_5_4\",\"zonepath\":\"[1,5,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Documenti\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Documenti\",\"h\":464,\"name\":\"Documenti\",\"page\":2,\"shrinkable\":true,\"spuid\":\"XWJLXLYKJC\",\"stretch\":true,\"tabindex\":1,\"title_caption\":\"Documenti\",\"titled\":true,\"type\":\"Box\",\"w\":816,\"x\":8,\"y\":8,\"zone\":\"pag2_1\",\"zonepath\":\"[2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":448,\"name\":\"Documenti\",\"page\":2,\"spuid\":\"KUBMCPCHPZ\",\"src\":\"\\/jsp\\/pg_allegati_portlet.jsp?ForcedPortletUID=KUBMCPCHPZ\",\"tabindex\":2,\"type\":\"Portlet\",\"w\":800,\"x\":16,\"y\":16,\"zone\":\"pag2_1_1\",\"zonepath\":\"[2,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":799,\"name\":\"armt_cgo_qadv_operazioni_storico\",\"page\":3,\"spuid\":\"FGDNXNYKKB\",\"tabindex\":1,\"type\":\"Iframe\",\"w\":1005,\"x\":8,\"y\":8,\"zone\":\"pag3_1\",\"zonepath\":\"[3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_snai_oper_storicoBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_snai_oper_storico?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_snai_oper_storico_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_snai_oper_storico_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_TOTCONT=a[4];");
      status.out.println("w_IMPWALLET=a[5];");
      status.out.println("w_CODWALLET=a[6];");
      status.out.println("w_CODTRANSZ=a[7];");
      status.out.println("w_CGOFLGVLT=a[8];");
      status.out.println("w_MEZPAGAM=a[9];");
      status.out.println("w_CC4CIFRE=a[10];");
      status.out.println("w_CCMESSCA=a[11];");
      status.out.println("w_CCANNSCA=a[12];");
      status.out.println("w_IBAN=a[13];");
      status.out.println("w_ASSEGNO=a[14];");
      status.out.println("w_CFESTERO=a[15];");
      status.out.println("w_PEP=a[16];");
      status.out.println("w_CODFISC=a[17];");
      status.out.println("w_COGNOME=a[18];");
      status.out.println("w_NOME=a[19];");
      status.out.println("w_NASCOMUN=a[20];");
      status.out.println("w_TIPINTER=a[21];");
      status.out.println("w_SESSO=a[22];");
      status.out.println("w_NASSTATO=a[23];");
      status.out.println("w_DATANASC=a[24];");
      status.out.println("w_PAESE=a[25];");
      status.out.println("w_RAGSOC=a[26];");
      status.out.println("w_DESCCIT=a[27];");
      status.out.println("w_PROVINCIA=a[28];");
      status.out.println("w_CAP=a[29];");
      status.out.println("w_CODCAB=a[30];");
      status.out.println("w_DOMICILIO=a[31];");
      status.out.println("w_NUMTEL=a[32];");
      status.out.println("w_ATTIV=a[33];");
      status.out.println("w_SOTGRUP=a[34];");
      status.out.println("w_RAMOGRUP=a[35];");
      status.out.println("w_SETTSINT=a[36];");
      status.out.println("w_SNAINUMOPE=a[37];");
      status.out.println("w_DATAOPE=a[38];");
      status.out.println("w_DATAREG=a[39];");
      status.out.println("w_FLAGCONT=a[40];");
      status.out.println("w_FLAGWALLET=a[41];");
      status.out.println("w_TIPOOPRAP=a[42];");
      status.out.println("w_CODANA=a[43];");
      status.out.println("w_CODVOC=a[44];");
      status.out.println("w_VALUTA=a[45];");
      status.out.println("w_CAMBIO=a[46];");
      status.out.println("w_CONNESCLI=a[47];");
      status.out.println("w_CONNESBEN=a[48];");
      status.out.println("w_C_RAG=a[49];");
      status.out.println("w_C_CTA=a[50];");
      status.out.println("w_C_PRV=a[51];");
      status.out.println("w_C_CAP=a[52];");
      status.out.println("w_C_CAB=a[53];");
      status.out.println("w_C_IND=a[54];");
      status.out.println("w_C_STA=a[55];");
      status.out.println("w_CODINTER=a[56];");
      status.out.println("w_CODDIPE=a[57];");
      status.out.println("w_DESCCITDIP=a[58];");
      status.out.println("w_FLAGFRAZ=a[59];");
      status.out.println("w_COLLEG=a[60];");
      status.out.println("w_PROVINCIADIP=a[61];");
      status.out.println("w_CODCABDIP=a[62];");
      status.out.println("w_CACODICE=a[63];");
      status.out.println("w_CAIDBIGLIETTO=a[64];");
      status.out.println("w_CGOTIPOPE=a[65];");
      status.out.println("w_NUMOPEDL=a[66];");
      status.out.println("w_CAFLGOPE=a[67];");
      status.out.println("w_IDDOCALL=a[68];");
      status.out.println("w_FLGDOCALL=a[69];");
      status.out.println("w_VLTPLAT=a[70];");
      status.out.println("w_CCDATSCA=a[71];");
      status.out.println("w_TIPODOC=a[72];");
      status.out.println("w_NUMDOCUM=a[73];");
      status.out.println("w_DATARILASC=a[74];");
      status.out.println("w_DATAVALI=a[75];");
      status.out.println("w_RDCODAUT=a[76];");
      status.out.println("w_RDLOCAUT=a[77];");
      status.out.println("w_AUTRILASC=a[78];");
      status.out.println("w_PEPDESCRI=a[79];");
      status.out.println("w_PAESE2=a[80];");
      status.out.println("w_DESCCIT2=a[81];");
      status.out.println("w_PROVINCIA2=a[82];");
      status.out.println("w_CAP2=a[83];");
      status.out.println("w_DOMICILIO2=a[84];");
      status.out.println("w_FLGDOM=a[85];");
      status.out.println("w_FLGSBLOCCO=a[86];");
      status.out.println("w_SEGNO=a[87];");
      status.out.println("w_PROFESSIONE=a[88];");
      status.out.println("w_gIntemediario=a[89];");
      status.out.println("w_xSOGTPGIO=a[90];");
      status.out.println("w_xDEFTPGIO=a[91];");
      status.out.println("w_xEDTTPGIO=a[92];");
      status.out.println("w_xEDTSPGIO=a[93];");
      status.out.println("w_xDEFSPGIO=a[94];");
      status.out.println("w_FLGPAGAM=a[95];");
      status.out.println("w_FILTROMP=a[96];");
      ...
      status.out.println("w_xDescDipe=a[108];");
      status.out.println("w_datpaseuro=a[109];");
      status.out.println("w_xdes1=a[110];");
      status.out.println("w_desccauana=a[111];");
      status.out.println("w_CITINT=a[112];");
      status.out.println("w_CITDIP=a[113];");
      status.out.println("w_PROVDIP=a[114];");
      status.out.println("w_CABDIP=a[115];");
      status.out.println("w_PROVINT=a[116];");
      status.out.println("w_CABINT=a[117];");
      status.out.println("w_xdesccausin=a[118];");
      status.out.println("w_gChkDate=a[119];");
      status.out.println("w_xdesctipatt=a[120];");
      status.out.println("w_gVLT=a[121];");
      status.out.println("w_gVNP=a[122];");
      status.out.println("w_gCodDip=a[123];");
      status.out.println("w_verifica=a[124];");
      status.out.println("w_gStpDwn=a[125];");
      status.out.println("w_MIDBIGLIETTO=a[126];");
      status.out.println("w_MIDBIGLIETTO1=a[127];");
      status.out.println("w_MIDBIGLIETTO2=a[128];");
      status.out.println("w_MIDBIGLIETTO3=a[129];");
      status.out.println("w_MIDBIGLIETTO4=a[130];");
      status.out.println("w_TELDIP=a[131];");
      status.out.println("w_CODLOC=a[132];");
      status.out.println("w_xFLGPLGIO=a[133];");
      status.out.println("w_xVPFLGATT=a[134];");
      status.out.println("w_DOC03=a[135];");
      status.out.println("w_DOC04=a[136];");
      status.out.println("w_tipop=a[137];");
      status.out.println("w_gGesDoc=a[138];");
      status.out.println("w_edtdoc=a[139];");
      status.out.println("w_saveddoc=a[140];");
      status.out.println("w_idcitta2=a[141];");
      status.out.println("w_dessta2=a[142];");
      status.out.println("w_xCGOFLGPROF=a[143];");
      status.out.println("o_RWCKHDGZVX_expand=null;");
      status.out.println("o_RWCKHDGZVX_collapse=null;");
      status.out.println("m_cWv_ardt_cgo_oper_storico_multi=a[144];");
      status.out.println("m_cWv_armt_cgo_qadv_operazioni_storico=a[145];");
      ...
      status.out.println("o_CODFISC=w_CODFISC;");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_snai_oper_storico_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(981,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","cgo_operazioni_storico");
  ...
  void RaiseLoadedEntity(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_snai_oper_storicoBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_snai_oper_storicoBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_snai_oper_storicoBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_snai_oper_storicoBO BO,ServletStatus status) throws IOException {
  ...
  void IncludeJS(ServletStatus status) {
    ...
    status.out.println(
     ...
     "</script>" +
    "");
    l_cJsUid = SPPrxycizer.proxycizedPath("arrt_cgo_read_codfisc_s.js");
    status.out.println(
     "<script src='../"+l_cJsUid+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+arfn_cgo_contarm.m_cEntityUid+"/arfn_cgo_contarm.js'>" +
     "</script>" +
    "");
    status.out.println(
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+arrt_cgo_setpagatore_s.m_cEntityUid+"/arrt_cgo_setpagatore_s.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      arfn_cgo_contarm.GetCallableNames(callable);
      ...
      arrt_cgo_setpagatore_s.GetCallableNames(callable);
      CPLib.AddToSet(callable,"Utilities");
      CPLib.AddToSet(callable,"arrt_cgo_moduli");
      CPLib.AddToSet(callable,"arrt_cgo_read_codfisc_s");
      CPLib.AddToSet(callable,"ardt_cgo_oper_storico_multi");
      CPLib.AddToSet(callable,"armt_cgo_qadv_operazioni_storico");
    } else {
  ...
  public String GetCheckVars(ServletStatus status,armt_snai_oper_storicoBO BO,String p_cUID) {
    ...
    if (CPLib.eq(p_cUID,m_cEntityUid)) {
      ...
      SPLib.FillLinkHash(hCreator,BO.GetLinkProperties(p_cUID),BO);
    } else if (CPLib.eq(p_cUID,"PSLKLXOQMT")) {
      SPLib.FillLinkHash(hCreator,BO.GetLinkProperties(p_cUID),BO);
    }
    return GetEntityHash(p_cUID,status,hCreator);
  }
  public boolean CheckVars(ServletStatus status,armt_snai_oper_storicoBO BO) {
    boolean result = true;
    if (SPServlet.m_nEnableCheckHash) {
      ...
      if (result &&  ! (CPLib.Empty(BO.w_idcitta2))) {
        a = GetCheckVars(status,BO,"PSLKLXOQMT");
        b = status.m_CheckVars.optString("PSLKLXOQMT");
        result = CPLib.eq(a,b);
      }
    }
    return result;
