...
public class arrt_discouic_ver extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*b57bbed1*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_discouic_verR Rcalled;
    Rcalled = new arrt_discouic_verR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaaaaa_retval = Rcalled.Run(status.pTipo,status.w_DaDatOpe,status.w_ADatOpe,status.w_tipocr,status.w_codcr,status.w_flgfrazionate,status.w_storico);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_discouic_verR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_discouic_verR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_discouic_ver"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
