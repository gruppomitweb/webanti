...
SPBOUILib.SetInputValue('IOJVSZRBNX',function(){return WtH(w_NUMPROG1,'C',11,0,'')},w_NUMPROG1);
SPBOUILib.SetInputValue('VNGVCVDHTS',function(){return WtH(w_NUMPROG2,'C',11,0,'')},w_NUMPROG2);
SPBOUILib.SetInputValue('VYVWCXNUMC',function(){return WtH(w_PROQUOTA,'N',3,0,'')},w_PROQUOTA);
} else {
var tmp_NAKGRMOUXS = ToHTag(AsControlValue(w_DOMICILIO));
if (Ne(e_NAKGRMOUXS,tmp_NAKGRMOUXS)) {
SPBOUILib.SetLabelValue('NAKGRMOUXS','innerHTML',tmp_NAKGRMOUXS);
...
HideUI.lblids["PROQUOTA"]=['SEPJTDHOKN'];
...
function VYVWCXNUMC_Valid(e) {
SetActiveField(Ctrl('VYVWCXNUMC'),false);
if (Eq(m_cFunction,'query')) {
DoQuery('PROQUOTA','N',w_PROQUOTA,HtW(Ctrl('VYVWCXNUMC').value,'N'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_PROQUOTA(HtW(Ctrl('VYVWCXNUMC').value,'N'),null,e);
return l_bSetResult;
}
}
function VYVWCXNUMC_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('VYVWCXNUMC'))) {
DisplayErrorMessage();
}
SetActiveField(Ctrl('VYVWCXNUMC'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('VYVWCXNUMC'),e);
}
_ResetTracker();
...
function VYVWCXNUMC_wrap_Valid(event) {
return VYVWCXNUMC_Valid(event);
}
SPBOUILib.SetInputExit('VYVWCXNUMC',VYVWCXNUMC_wrap_Valid);
function VYVWCXNUMC_wrap_OnFocus(event) {
return VYVWCXNUMC_OnFocus(event);
}
SPBOUILib.SetInputEnter('VYVWCXNUMC',VYVWCXNUMC_wrap_OnFocus);
function VYVWCXNUMC_wrap_OnKeyPress(event) {
return CheckNum(event);
}
SPBOUILib.SetInputKeyPress('VYVWCXNUMC',VYVWCXNUMC_wrap_OnKeyPress);
function VYVWCXNUMC_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('VYVWCXNUMC',VYVWCXNUMC_wrap_OnKeyDown);
if (m_bExtended) {
...
m_oFather.i_last_focused_item='child_ardt_provinformazioni';
...
function Help() {
windowOpenForeground('../doc/ardt_provinformazioni_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
return 'xntestbo'+'\\'+CPLib.ToCPStr(w_RAPPORTO);
}
...
p_d.setValue('ardt_provinformazioni',m_cSelectedPage);
...
_Obli('PROQUOTA',1,'VYVWCXNUMC',false,"11552491820")
