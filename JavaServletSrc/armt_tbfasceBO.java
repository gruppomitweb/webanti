...
// * --- Fine Area Manuale
public class armt_tbfasceBO extends armt_tbfasceBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbfasceBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbfasce");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbfascepr,m_cPhName_tbfascepr,m_cVirtName_tbfascepr,m_cVirtName_tbfascepr+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",10,0),m_cServer_tbfascepr,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        w_TIPO = l_rsLoad.GetDouble("TIPO");
        w_DESCRI = l_rsLoad.GetString("DESCRI");
        w_DAIMP = l_rsLoad.GetDouble("DAIMP");
        w_AIMP = l_rsLoad.GetDouble("AIMP");
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbfascepr")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbfascepr);
      l_cSql = "insert into "+m_oWrInfo_tbfascepr.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_tbfascepr.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbfascepr")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_CODICE,"C",10,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_TIPO,"N",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DESCRI,"C",150,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DAIMP,"N",18,2)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_AIMP,"N",18,2)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_RISCHIO,"N",3,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IDBASE,"C",10,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbfascepr.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbfascepr")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbfascepr,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbfascepr);
      l_cSql = "Update "+m_oWrInfo_tbfascepr.GetTableWriteName()+" set";
      l_cSql = l_cSql+" TIPO = "+CPLib.ToSQL(w_TIPO,"N",1,0);
      l_cSql = l_cSql+", DESCRI = "+CPLib.ToSQL(w_DESCRI,"C",150,0);
      l_cSql = l_cSql+", DAIMP = "+CPLib.ToSQL(w_DAIMP,"N",18,2);
      l_cSql = l_cSql+", AIMP = "+CPLib.ToSQL(w_AIMP,"N",18,2);
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",10,0),m_cServer_tbfascepr,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbfascepr.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbfascepr")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbfascepr.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",10,0),m_cServer_tbfascepr,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbfascepr.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbfascepr")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbfascepr,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
