...
// * --- Fine Area Manuale
public class arpg_cgo_operazioni_areafin_storicoBO extends arpg_cgo_operazioni_areafin_storicoBL {
  ...
  public arpg_cgo_operazioni_areafin_storicoBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_CGODATFINCOM = rs.GetDate("CGODATFINCOM");
          AddTrsRow();
          rs.Next();
          l_nCnt = l_nCnt+1;
        }
