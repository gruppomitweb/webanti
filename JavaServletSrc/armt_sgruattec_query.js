...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('CZPCNBGKRB').value,'C'),w_SOTGRU)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbstgru',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'SOTGRU') &&  ! (Empty(HtW(Ctrl('CZPCNBGKRB').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.SOTGRU',HtW(Ctrl('CZPCNBGKRB').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_sgruattec');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbstgru',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['CZPCNBGKRB'],i=0;i>=0;i--){
