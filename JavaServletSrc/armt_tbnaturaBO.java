...
// * --- Fine Area Manuale
public class armt_tbnaturaBO extends armt_tbnaturaBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbnaturaBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbnatura");
    }
  ...
  public boolean Load(double p_CODICE) {
    CPResultSet l_rsLoad=null;
    SqlLoader l_oSqlLoader;
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbnatura,m_cPhName_tbnatura,m_cVirtName_tbnatura,m_cVirtName_tbnatura+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"N",5,0),m_cServer_tbnatura,p_CODICE),"");
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbnatura);
      l_cSql = "insert into "+m_oWrInfo_tbnatura.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbnatura.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnatura")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_CODICE,"N",5,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DESCRI,"C",100,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbnatura.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnatura")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbnatura,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbnatura);
      l_cSql = "Update "+m_oWrInfo_tbnatura.GetTableWriteName()+" set";
      l_cSql = l_cSql+" DESCRI = "+CPLib.ToSQL(w_DESCRI,"C",100,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnatura")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"N",5,0),m_cServer_tbnatura,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbnatura.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnatura")) {
  ...
  public boolean Delete(double p_CODICE) {
    String l_cSql;
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbnatura.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"N",5,0),m_cServer_tbnatura,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbnatura.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnatura")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbnatura,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
  ...
  void SaveAutonumber() {
    ...
    m_Sql.NextTableProg(this,"PRGNATURA","w_CODICE",m_cServer_tbnatura,m_Ctx.GetCompany());
    op_CODICE = w_CODICE;
  }
