...
function Set_DATIPDOC(ctrlValue,kmode,e) {
if (Ne(w_DATIPDOC,ctrlValue)) {
var ctl = _GetCtl(e,'SSYFQFAQAO');
if (_tracker.goon(ctl,ctrlValue)) {
w_DATIPDOC=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('SSYFQFAQAO',0);
if (l_bResult) {
l_bResult=Link_SSYFQFAQAO(kmode);
if ( ! (l_bResult)) {
w_DATIPDOC=HtW('','C');
}
}
...
function Link_SSYFQFAQAO(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
var l_bEmp = Empty(w_DATIPDOC);
var l_Appl = LinkApplet();
l_Appl.SetStringKey('TIPDOC',w_DATIPDOC,2,0);
l_Appl.SetFields('TIPDOC,DESCRI');
l_Appl.SetTypes('C,C');
l_Appl.LinkTable('cgo_tipdoc');
l_Appl.SetLinkzoom('armt_cgo_tipdoc');
l_Appl.SetID('SSYFQFAQAO');
l_nRec=l_Appl.DoLink((Eq(kmode,null)?1:kmode));
l_bResult=Ne(l_nRec,0);
if (Eq(l_nRec,1)) {
w_DATIPDOC=l_Appl.GetStringValue('TIPDOC',2,0);
w_xDESCRI=l_Appl.GetStringValue('DESCRI',30,0);
l_Appl.GetLinkHash(GetRepeatedCheckVars());
} else {
Link_SSYFQFAQAO_Blank();
}
if (Eq(l_nRec,2)) {
var openername = 'linkview_SSYFQFAQAO';
...
function Link_SSYFQFAQAO_Blank() {
w_DATIPDOC='';
w_xDESCRI='';
}
function LOpt_SSYFQFAQAO() {
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('cgo_tipdoc','C'));
l_oWv.setValue('Linkzoomprg',WtA('armt_cgo_tipdoc','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
l_oWv.setValue('PKFields',WtA('TIPDOC','C'));
l_oWv.setValue('LinkedField',WtA('TIPDOC','C'));
l_oWv.setValue('UID',WtA('SSYFQFAQAO','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['SSYFQFAQAO'][1],'C'));
l_oWv.setValue('Caption',WtA(Translate('11429032151'),'C'));
return l_oWv;
}
function ReportLinkValue(varName,value) {
var bResult = false;
if (Eq(varName,'SSYFQFAQAO')) {
last_focused_comp=GetLastFocusedElementOfLink('SSYFQFAQAO');
function SetMethod_SSYFQFAQAO() {
Set_DATIPDOC(HtW(value,'C'),0);
}
if (IsDeviceMobile()) {
SetMethod_SSYFQFAQAO();
} else {
setTimeout(SetMethod_SSYFQFAQAO,1);
}
bResult=true;
}
return bResult;
...
if ( ! (m_oFather.SaveContext_ardt_docaut())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_docaut_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_docaut_dataNotSent',{'action':formaction});
}
}
}
}
...
if (( ! (_ChkObl('SSYFQFAQAO',1))) && FullRow()) {
_SignErr('SSYFQFAQAO');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_docaut();
}
}
SaveContext.child=[];
function BlankRow() {
w_DATIPDOC='';
w_xDESCRI='';
if ( ! (EmptyString(w_DATIPDOC))) {
Link_SSYFQFAQAO(null);
}
BlankRowChild();
if (m_bExtended) {
...
function FullRow() {
return  ! (Empty(w_DATIPDOC));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.DATIPDOC={"dec":0,"fixedpos":false,"id":"SSYFQFAQAO","len":2,"name":"DATIPDOC","title":"Tipo Documento","type":"C"};
l_aRepeatedFields.xDESCRI={"dec":0,"fixedpos":false,"id":"PPFKYXMARY","len":30,"name":"xDESCRI","title":"Descrizione","type":"C"};
