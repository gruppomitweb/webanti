...
public class arpg_cgo_operazioni_agenzie extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*VJYLVSOILZ*!";
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    arpg_cgo_operazioni_agenzieBO BO;
    BO = getBO(status);
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_cgo_operazioni_agenzie";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 838;
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_cgo_operazioni_agenzieBO BO) {
  ...
  void WriteStyles(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_cgo_operazioni_agenzie.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_cgo_operazioni_agenzieBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_cgo_operazioni_agenzieBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_cgo_operazioni_agenzieBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(838,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_cgo_operazioni_agenzie"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_cgo_operazioni_agenzie","");
  ...
  void WriteMainFormScript(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_cgo_operazioni_agenzieBO getBO(ServletStatus status) {
    arpg_cgo_operazioni_agenzieBO BO = new arpg_cgo_operazioni_agenzieBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_cgo_operazioni_agenzieBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
    ...
    item.put("altInterfaceFor","arpg_cgo_operazioni_agenzie");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_cgo_operazioni_agenzieBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_cgo_operazioni_agenzie?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_cgo_operazioni_agenzie_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_cgo_operazioni_agenzieBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_cgo_operazioni_agenzieBO BO) {
    boolean result = true;
    return result;
  }
}
