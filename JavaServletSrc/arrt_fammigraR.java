...
import org.json.*;
// * --- Fine Area Manuale
public class arrt_fammigraR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_fammigraR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_fammigra",m_Caller);
  ...
  public double GetNumber(String p_cVarName,String p_cType,int len,int dec) {
    ...
    if (CPLib.eqr("_delfile",p_cVarName)) {
      return _delfile;
    }
    return 0;
  }
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      if (CPLib.eqr(pAzzera,"S")) {
        ...
        m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"00000047")+"delete from "+m_oWrInfo.GetTableWriteName();
        m_cSql = m_cSql+" where ";
        ...
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"00000048")+"delete from "+m_oWrInfo.GetTableWriteName();
        m_cSql = m_cSql+" where ";
        m_cSql = m_cSql+m_oWrInfo.WhereFilter("1=1");
        m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql,m_oParameters);
        if (CPLib.lt(m_nUpdatedRows,0)) {
        ...
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"00000049")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
        m_cSql = m_cSql+"FLGNOE = "+CPLib.ToSQL("N","C",1,0,m_cServer)+", ";
        ...
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"0000004A")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
        m_cSql = m_cSql+"FLGNOE = "+CPLib.ToSQL("N","C",1,0,m_cServer)+", ";
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on cpwarn into routine arrt_fammigra returns two or more records. This item should return only a record.");
      ...
      if (CPLib.Empty(CPLib.LRTrim(_semaforo))) {
        ...
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"0000004F")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
        ...
        // Legge i file degli esiti
        di = new File(CPLib.LRTrim(gPathDoc)+"/SID/MIGRA/ELB/"+CPLib.LRTrim(gAzienda)+"/");
        File fl3[] = di.listFiles();
        count=0;
        for (_i=0; _i < fl3.length; _i++)  {
        /* _delfile := 0 */
        _delfile = CPLib.Round(0,0);
        ...
        Cursor_tmp_list_imprich = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select nomefile  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"nomefile="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.LRTrim(fl3[_i].getName()),"?",0,0,m_cServer, m_oParameters),m_cServer,CPLib.LRTrim(fl3[_i].getName()))+" "+")"+(m_Ctx.IsSharedTemp("tmp_list_imprich")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
        while ( ! (Cursor_tmp_list_imprich.Eof())) {
          /* _delfile := 1 */
          _delfile = CPLib.Round(1,0);
          Cursor_tmp_list_imprich.Next();
        }
        m_cConnectivityError = Cursor_tmp_list_imprich.ErrorMessage();
        Cursor_tmp_list_imprich.Close();
        // * --- End Select
        //Cancella file presenti se non nella lista
          if (_delfile==0) {
            fl3[_i].delete();
          }
        }
        ...
        while ( ! (Cursor_tmp_list_imprich.Eof())) {
          ...
          if (CPLib.ne(CPLib.At(".zip",CPLib.Lower(CPLib.LRTrim(Cursor_tmp_list_imprich.GetString("nomefile")))),0)) {
            ...
            /* Exec "Scompatta ZIP" Page 4:Page_4 */
            Page_4();
          } // End If
          Cursor_tmp_list_imprich.Next();
        }
        ...
        if (CPLib.eqr(_tipo,"W")) {
          /* _dir1 := Strtran(LRTrim(gPathDoc),'/','\')+"\SID\MIGRA\ELB\"+LRTrim(gAzienda)+"\" */
          _dir1 = CPLib.Strtran(CPLib.LRTrim(gPathDoc),"/","\\")+"\\SID\\MIGRA\\ELB\\"+CPLib.LRTrim(gAzienda)+"\\";
          /* _dir2 := Strtran(LRTrim(gPathDoc),'/','\')+"\SID\MIGRA\ESI\"+LRTrim(gAzienda)+"\" */
          _dir2 = CPLib.Strtran(CPLib.LRTrim(gPathDoc),"/","\\")+"\\SID\\MIGRA\\ESI\\"+CPLib.LRTrim(gAzienda)+"\\";
          /* _dir3 := Strtran(LRTrim(gPathDoc),'/','\')+"\SID\MIGRA\RIC\"+LRTrim(gAzienda)+"\" */
          _dir3 = CPLib.Strtran(CPLib.LRTrim(gPathDoc),"/","\\")+"\\SID\\MIGRA\\RIC\\"+CPLib.LRTrim(gAzienda)+"\\";
        ...
        // Begin Transaction
        m_Sql.RequireTransaction();
        double nTry00000080status;
        nTry00000080status = m_Sql.GetTransactionStatus();
        String cTry00000080msg;
        cTry00000080msg = m_Sql.TransactionErrorMessage();
        try {
          // * --- Select from tmp_list_imprich
          m_cServer = m_Ctx.GetServer("tmp_list_imprich");
          m_cPhName = m_Ctx.GetPhName("tmp_list_imprich");
          ...
          while ( ! (Cursor_tmp_list_imprich.Eof())) {
            ...
            while ( ! (FileLibMit.Eof(fhand))) {
              ...
              if (CPLib.eqr(CPLib.Left(_riga,1),"1")) {
                ...
                /* _anno := iif(Substr(_riga,78,10)='0001-01-01',NullString(),Left(Substr(_riga,78,10),4)) */
                _anno = (CPLib.eqr(CPLib.Substr(_riga,78,10),"0001-01-01")?CPLib.NullString():CPLib.Left(CPLib.Substr(_riga,78,10),4));
                ...
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"famesiti",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"0000008E")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
                m_cSql = m_cSql+GetFieldsName_0000008E(m_Ctx,m_oWrInfo);
                m_cSql = m_cSql+") values (";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,2,50)),"?",0,0,m_cServer)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,52,2)),"?",0,0,m_cServer)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,54,24)),"?",0,0,m_cServer)+", ";
                ...
                /* If LRTrim(Substr(_riga,52,2))='98' */
                if (CPLib.eqr(CPLib.LRTrim(CPLib.Substr(_riga,52,2)),"98")) {
                  ...
                  m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"opextrbo",m_Ctx.GetCompany());
                  m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                  m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"00000090")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                  m_cSql = m_cSql+"FLGNOE = "+CPLib.ToSQL("S","C",1,0,m_cServer)+", ";
                  m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"opextrbo",false,true);
                  m_cSql = m_cSql+" where ";
                  m_cSql = m_cSql+"UNIQUER = "+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,2,50)),"?",0,0,m_cServer),m_cServer,CPLib.LRTrim(CPLib.Substr(_riga,2,50)))+"";
                  ...
                  m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"aetesta",m_Ctx.GetCompany());
                  m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                  m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"00000091")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
                  m_cSql = m_cSql+"FLGNOE = "+CPLib.ToSQL("S","C",1,0,m_cServer)+", ";
                  m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"aetesta",false,true);
                  m_cSql = m_cSql+" where ";
                  m_cSql = m_cSql+"UNIQUECODE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,2,50)),"?",0,0,m_cServer),m_cServer,CPLib.LRTrim(CPLib.Substr(_riga,2,50)))+"";
                  m_cSql = m_cSql+m_oWrInfo.WhereFilter();
                  m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql,m_oParameters);
                  if (CPLib.lt(m_nUpdatedRows,0)) {
                    m_Sql.AbortTransaction();
                ...
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"famesitid",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"00000095")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
                m_cSql = m_cSql+GetFieldsName_00000095(m_Ctx,m_oWrInfo);
                m_cSql = m_cSql+") values (";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,2,50)),"?",0,0,m_cServer)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.Val(CPLib.Substr(_riga,52,9)),"?",0,0)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,61,16)),"?",0,0,m_cServer)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.Val(CPLib.Substr(_riga,77,2)),"?",0,0)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(_data1,"?",0,0)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(_data2,"?",0,0)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,99,60)),"?",0,0,m_cServer)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,159,1)),"?",0,0,m_cServer)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(_data3,"?",0,0)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,170,40)),"?",0,0,m_cServer)+", ";
                m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(CPLib.Substr(_riga,210,2)),"?",0,0,m_cServer)+", ";
                m_cSql = m_cSql+m_oWrInfo.InsertValues();
                ...
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"00000097")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
          ...
          if (CPLib.eqr(m_Sql.GetTransactionStatus(),0) || CPLib.ne(nTry00000080status,0)) {
            m_Sql.SetTransactionStatus(nTry00000080status,cTry00000080msg);
          }
        }
        ...
        m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"cpwarn",m_Ctx.GetCompany());
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"000000A0")+"delete from "+m_oWrInfo.GetTableWriteName();
        m_cSql = m_cSql+" where ";
  ...
  void Page_2() throws Exception {
    ...
    try
    {
      ...
      String[] cmd = {CPLib.LRTrim(_dirsid)+"/prog/runRicevuteMIT.bat",CPLib.LRTrim(_dir1),"FCO05",CPLib.Upper(_pwdsog)};
  ...
  void Page_3() throws Exception {
    ...
    {
      File _mydir = new File(CPLib.LRTrim(_dirsid)+"/prog");
      String[] cmd = {CPLib.LRTrim(_dirsid)+"/prog/runRicevuteMIT.sh",CPLib.LRTrim(_dirjava)+"/bin/java","FCO05",CPLib.Upper(_pwdsog)};
      Runtime rt = Runtime.getRuntime();
      Process proc = rt.exec(cmd,null,_mydir);
      // any error message?
      StreamGobbler errorGobbler = new StreamGobbler(proc.getErrorStream(), "ERRORE", m_Ctx);            
  ...
  void Page_4() throws Exception {
    ...
    m_Ctx.SetGlobalString("gMsgImp",gMsgImp);
    /* cpercorsozip Char(200) := LRTrim(gPathDoc)+"/SID/MIGRA/ELB/"+LRTrim(gAzienda)+"/"+czipfile */
    String cpercorsozip;
    cpercorsozip = CPLib.LRTrim(gPathDoc)+"/SID/MIGRA/ELB/"+CPLib.LRTrim(gAzienda)+"/"+czipfile;
    /* cdirectoryfile Char(200) := LRTrim(gPathDoc)+"/SID/MIGRA/ELB/"+LRTrim(gAzienda)+"/" */
    String cdirectoryfile;
    cdirectoryfile = CPLib.LRTrim(gPathDoc)+"/SID/MIGRA/ELB/"+CPLib.LRTrim(gAzienda)+"/";
    ...
    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
    m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"000000B1")+"delete from "+m_oWrInfo.GetTableWriteName();
    m_cSql = m_cSql+" where ";
    ...
    while (CPLib.lt(contanome,conteggio)) {
      ...
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_list_imprich",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_fammigra",372,"000000B5")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
    ...
    /* FileLibMit.DeleteFilePath(LRTrim(gPathDoc)+"/SID/MIGRA/ELB/"+LRTrim(gAzienda)+"/"+czipfile) */
    FileLibMit.DeleteFilePath(CPLib.LRTrim(gPathDoc)+"/SID/MIGRA/ELB/"+CPLib.LRTrim(gAzienda)+"/"+czipfile);
  ...
  public static arrt_fammigraR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_fammigraR(p_Ctx, p_Caller);
  }
  public static arrt_fammigraR Make(CPContext p_Ctx) {
    return new arrt_fammigraR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_fdatetime,arrt_fammigra,
  public static final String i_InvokedRoutines = ",arfn_fdatetime,arrt_fammigra,";
