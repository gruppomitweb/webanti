...
public class arrt_aelistanomi_do extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*cd9c7999*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_datpris;
    public java.sql.Date w_datpris;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_datprif;
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_tipostp;
    public String w_tipostp;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_datpris = source.Has("w_datpris");
      if (m_bpassedw_datpris) {
        w_datpris = source.GetParameter("w_datpris",source.GetParameter("datpris",CPLib.NullDate()));
      } else {
        if (source.IsForwarded()) {
          w_datpris = source.GetCaller().GetDate("datpris","D",8,0);
          m_bpassedw_datpris = true;
        } else {
          w_datpris = source.GetParameter("datpris",source.GetParameter("w_datpris",CPLib.NullDate()));
        }
      }
      m_bpassedw_datprif = source.Has("w_datprif");
      if (m_bpassedw_datprif) {
        w_datprif = source.GetParameter("w_datprif",source.GetParameter("datprif",CPLib.NullDate()));
      } else {
        if (source.IsForwarded()) {
          w_datprif = source.GetCaller().GetDate("datprif","D",8,0);
          m_bpassedw_datprif = true;
        } else {
          w_datprif = source.GetParameter("datprif",source.GetParameter("w_datprif",CPLib.NullDate()));
        }
      }
      ...
      m_bpassedw_tipostp = source.Has("w_tipostp");
      if (m_bpassedw_tipostp) {
        w_tipostp = source.GetParameter("w_tipostp",source.GetParameter("tipostp",""));
      } else {
        if (source.IsForwarded()) {
          w_tipostp = source.GetCaller().GetString("tipostp","C",1,0);
          m_bpassedw_tipostp = true;
        } else {
          w_tipostp = source.GetParameter("tipostp",source.GetParameter("w_tipostp",""));
        }
      }
    }
  }
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_tipostp) ||  ! (status.m_bpassedw_datsecf) ||  ! (status.m_bpassedw_datseci) ||  ! (status.m_bpassedw_datprif) ||  ! (status.m_bpassedw_datpris) ||  ! (status.m_bpassedw_a_data) ||  ! (status.m_bpassedw_dadata) ||  ! (status.m_bpassedw_iniz);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_datpris)) {
      status.out.println("l_cV=WtA(FetchDate(l_oWnd('w_datpris',"+SPLib.ToJSValue(status.w_datpris,"D",8,0)+")),'D');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_datpris,"D",8,0)+",'D');");
    }
    status.out.println("l_oWv.setValue('w_datpris',l_cV);");
    if ( ! (status.m_bpassedw_datprif)) {
      status.out.println("l_cV=WtA(FetchDate(l_oWnd('w_datprif',"+SPLib.ToJSValue(status.w_datprif,"D",8,0)+")),'D');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_datprif,"D",8,0)+",'D');");
    }
    status.out.println("l_oWv.setValue('w_datprif',l_cV);");
    ...
    if ( ! (status.m_bpassedw_tipostp)) {
      status.out.println("l_cV=WtA(l_oWnd('w_tipostp',"+SPLib.ToJSValue(status.w_tipostp,"C",1,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_tipostp,"C",1,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_tipostp',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_aelistanomi_doR Rcalled;
    Rcalled = new arrt_aelistanomi_doR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_aelistanomi_doR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_aelistanomi_doR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_aelistanomi_do"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_datpris="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_datpris),"?",0,0)+"");
        status.out.println("js:w_datprif="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_datprif),"?",0,0)+"");
        ...
        status.out.println("js:w_tipostp="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_tipostp),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_datpris,null)) {");
        status.out.println(" window[f].w_datpris.setTime(("+SPLib.ToJSValue(status.w_datpris,"D",0,0)+").getTime())");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_datprif,null)) {");
        status.out.println(" window[f].w_datprif.setTime(("+SPLib.ToJSValue(status.w_datprif,"D",0,0)+").getTime())");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_datseci,null)) {");
        status.out.println(" window[f].w_datseci.setTime(("+SPLib.ToJSValue(status.w_datseci,"D",0,0)+").getTime())");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_datsecf,null)) {");
        status.out.println(" window[f].w_datsecf.setTime(("+SPLib.ToJSValue(status.w_datsecf,"D",0,0)+").getTime())");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_tipostp,null)) {");
        status.out.println("window[f].w_tipostp="+SPLib.ToJSValue(status.w_tipostp,"C",0,0)+";");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_aelistanomi_do");
      if ( ! (CPLib.IsNull(l))) {
