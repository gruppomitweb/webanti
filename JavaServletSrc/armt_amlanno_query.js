...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('VBSBVXAQLN').value,'C'),w_IDBASE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('amlanno',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDBASE') &&  ! (Empty(HtW(Ctrl('VBSBVXAQLN').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDBASE',HtW(Ctrl('VBSBVXAQLN').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_amlanno');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('amlanno',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['VBSBVXAQLN'],i=0;i>=0;i--){
