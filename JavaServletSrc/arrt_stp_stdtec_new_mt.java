...
public class arrt_stp_stdtec_new_mt extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*40e3ab36*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.pPDF","pPDF");
      }        
      pPDF = source.GetParameter("pPDF","");
      if (source instanceof SPRequestSource) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_daCodDip","w_daCodDip");
      }        
      w_daCodDip = source.GetParameter("w_daCodDip","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_aCodDip","w_aCodDip");
      }        
      w_aCodDip = source.GetParameter("w_aCodDip","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_flgrae","w_flgrae");
      }        
      w_flgrae = source.GetParameter("w_flgrae","A");
    }
  }
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_stp_stdtec_new_mt";
  ...
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.pPDF,"C",6,0)+",'C');");
    status.out.println("l_oWv.setValue('pPDF',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_stp_stdtec_new_mtR Rcalled;
    Rcalled = new arrt_stp_stdtec_new_mtR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.forwardedto = Rcalled.Run(status.pTipo,status.pAUI,status.pPDF,status.w_DaDatOpe,status.w_ADatOpe,status.w_DaDatReg,status.w_ADatReg,status.w_daCodCli,status.w_aCodCli,status.w_daCodDip,status.w_aCodDip,status.w_flgrae);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_stp_stdtec_new_mtR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_stp_stdtec_new_mtR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      status.out.println(
       "<!--" +
      "");
      status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(Rcalled.RunAsync(status.pTipo,status.pAUI,status.pPDF,status.w_DaDatOpe,status.w_ADatOpe,status.w_DaDatReg,status.w_ADatReg,status.w_daCodCli,status.w_aCodCli,status.w_daCodDip,status.w_aCodDip,status.w_flgrae)))+"");
      status.out.println(
       " -->" +
      "");
    } else {
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_stp_stdtec_new_mt"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_stp_stdtec_new_mt");
      if ( ! (CPLib.IsNull(l))) {
