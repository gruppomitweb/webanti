...
public class arpg_viewinfoae extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*SXYBPMXMQC*!";
  ...
  public String getEntityTitle() {
    return "Informazioni (AgE)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_viewinfoae";
    ...
    l_eg.m_cQueryName = "qbe_viewinfo_ae";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_viewinfoaeBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11141873321",p_Context.Translate("Informazioni (AgE)")},
  ...
  void WriteStyles(arpg_viewinfoaeBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_viewinfoae.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_viewinfoaeBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_viewinfoaeBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_viewinfoaeBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_viewinfoaeBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_viewinfoaeBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_viewinfoaeBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_viewinfoaeBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_viewinfoaeBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_viewinfoae"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_viewinfoae","");
  ...
  void WriteMainFormScript(arpg_viewinfoaeBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_viewinfoaeBO getBO(ServletStatus status) {
    arpg_viewinfoaeBO BO = new arpg_viewinfoaeBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_viewinfoaeBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_viewinfoaeBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_viewinfoaeBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_viewinfoaeBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_viewinfoaeBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_viewinfoaeBO BO) {
    ...
    item.put("altInterfaceFor","arpg_viewinfoae");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_viewinfoaeBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_viewinfoaeBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_viewinfoaeBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_viewinfoaeBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_viewinfoaeBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_viewinfoaeBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_viewinfoae?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_viewinfoae_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_viewinfoaeBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_viewinfoaeBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_viewinfoaeBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_viewinfoaeBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_viewinfoaeBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_viewinfoaeBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_viewinfoaeBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_viewinfoaeBO BO) {
    boolean result = true;
    return result;
  }
}
