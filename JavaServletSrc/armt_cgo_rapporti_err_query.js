...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('IKBJDXURXZ').value,'C'),w_PRGRAP)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_rapporti_err',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'PRGRAP') &&  ! (Empty(HtW(Ctrl('IKBJDXURXZ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.PRGRAP',HtW(Ctrl('IKBJDXURXZ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_rapporti_err');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_rapporti_err',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['IKBJDXURXZ'],i=0;i>=0;i--){
