...
public class arrt_import extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*5db4ef7e*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_importR Rcalled;
    Rcalled = new arrt_importR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_importR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_importR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_import"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_import");
      if ( ! (CPLib.IsNull(l))) {
