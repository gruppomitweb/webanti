...
public class arpg_filesage extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*VPOMVMZUHB*!";
  ...
  public String getEntityTitle() {
    return "Files Generati per Agenzia Entrate";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_filesage";
    ...
    l_eg.m_cQueryName = "qbe_filesage";
    return l_eg;
  }
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_filesageBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1592918102",p_Context.Translate("Files Generati per Agenzia Entrate")},
    {"1739899105",p_Context.Translate("Azienda")},
    {"1974864032",p_Context.Translate("Periodo")},
    {"251106009",p_Context.Translate("Data Generazione")},
    {"285808099",p_Context.Translate("Descrizione")}};
    return l_translations;
  }
  void WriteStyles(arpg_filesageBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_filesage.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_filesageBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_filesageBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_filesageBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_filesageBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_filesageBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_filesageBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_filesageBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_filesage"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_filesage","");
  ...
  void WriteMainFormScript(arpg_filesageBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_filesageBO getBO(ServletStatus status) {
    arpg_filesageBO BO = new arpg_filesageBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_filesageBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_filesageBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_filesageBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_filesageBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_filesageBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_filesageBO BO) {
    ...
    item.put("altInterfaceFor","arpg_filesage");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_filesageBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_filesageBO BO) {
    status.out.println("var c_datafile;");
    status.out.println("var c_nomefile;");
    status.out.println("var c_descfile;");
    status.out.println("var c_mese;");
    status.out.println("var c_anno;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_filesageBO BO) {
    String l_cTrsQ;
    ...
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_mese,"C",2,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_anno,"N",4,0);
    l_cTrsQ = l_cTrsQ+")";
  ...
  void Next(ServletStatus status,arpg_filesageBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_filesageBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_filesageBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_filesage?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_filesage_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_filesageBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_filesageBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_filesageBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_filesageBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_filesageBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_filesageBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_filesageBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_filesageBO BO) {
    boolean result = true;
    return result;
  }
}
