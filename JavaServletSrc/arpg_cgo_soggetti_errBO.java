...
// * --- Fine Area Manuale
public class arpg_cgo_soggetti_errBO extends arpg_cgo_soggetti_errBL {
  ...
  public arpg_cgo_soggetti_errBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_PRGSOG = rs.GetString("PRGSOG");
          c_CONNES = rs.GetString("CONNES");
          c_COGNOME = rs.GetString("COGNOME");
          c_NOME = rs.GetString("NOME");
          c_DATANASC = rs.GetDate("DATANASC");
          c_NASCOMUN = rs.GetString("NASCOMUN");
          c_TXTERR = rs.GetString("TXTERR");
          AddTrsRow();
          rs.Next();
          l_nCnt = l_nCnt+1;
        }
