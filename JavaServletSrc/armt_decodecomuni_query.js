...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('ZGQEJMVUST').value,'N'),w_ID)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('DecodeCitta',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'ID') &&  ! (Empty(HtW(Ctrl('ZGQEJMVUST').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.ID',HtW(Ctrl('ZGQEJMVUST').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_decodecomuni');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('DecodeCitta',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['ZGQEJMVUST'],i=0;i>=0;i--){
