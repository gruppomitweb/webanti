...
l_Appl.SetLinkzoom('opemt_tbtipatt');
...
l_oWv.setValue('Linkzoomprg',WtA('opemt_tbtipatt','C'));
...
if ( ! (m_oFather.SaveContext_ardt_ope_contante())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_ope_contante_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_ope_contante_dataNotSent',{'action':formaction});
}
}
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_ope_contante();
}
}
SaveContext.child=[];
function BlankRow() {
