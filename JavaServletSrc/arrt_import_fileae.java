...
public class arrt_import_fileae extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*GFSAEDZPQQ*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_faggp;
    public String w_faggp;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_faggr;
    public String w_faggr;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_oldinter;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_faggp = source.Has("w_faggp");
      if (m_bpassedw_faggp) {
        w_faggp = source.GetParameter("w_faggp",source.GetParameter("faggp",""));
      } else {
        if (source.IsForwarded()) {
          w_faggp = source.GetCaller().GetString("faggp","C",1,0);
          m_bpassedw_faggp = true;
        } else {
          w_faggp = source.GetParameter("faggp",source.GetParameter("w_faggp",""));
        }
      }
      m_bpassedw_faggr = source.Has("w_faggr");
      if (m_bpassedw_faggr) {
        w_faggr = source.GetParameter("w_faggr",source.GetParameter("faggr",""));
      } else {
        if (source.IsForwarded()) {
          w_faggr = source.GetCaller().GetString("faggr","C",1,0);
          m_bpassedw_faggr = true;
        } else {
          w_faggr = source.GetParameter("faggr",source.GetParameter("w_faggr",""));
        }
      }
      m_bpassedw_oldinter = source.Has("w_oldinter");
      if (m_bpassedw_oldinter) {
        w_oldinter = source.GetParameter("w_oldinter",source.GetParameter("oldinter",""));
      } else {
        if (source.IsForwarded()) {
          w_oldinter = source.GetCaller().GetString("oldinter","C",16,0);
          m_bpassedw_oldinter = true;
        } else {
          w_oldinter = source.GetParameter("oldinter",source.GetParameter("w_oldinter",""));
        }
      }
    }
  }
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_oldinter) ||  ! (status.m_bpassedw_faggr) ||  ! (status.m_bpassedw_faggp) ||  ! (status.m_bpassedw_tipofile) ||  ! (status.m_bpassedw_impzip) ||  ! (status.m_bpassedw_codval) ||  ! (status.m_bpassedw_f_ricaui) ||  ! (status.m_bpassedw_prefrap) ||  ! (status.m_bpassedw_f_denom) ||  ! (status.m_bpassedw_offset);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_faggp)) {
      status.out.println("l_cV=WtA(l_oWnd('w_faggp',"+SPLib.ToJSValue(status.w_faggp,"C",1,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_faggp,"C",1,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_faggp',l_cV);");
    if ( ! (status.m_bpassedw_faggr)) {
      status.out.println("l_cV=WtA(l_oWnd('w_faggr',"+SPLib.ToJSValue(status.w_faggr,"C",1,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_faggr,"C",1,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_faggr',l_cV);");
    if ( ! (status.m_bpassedw_oldinter)) {
      status.out.println("l_cV=WtA(l_oWnd('w_oldinter',"+SPLib.ToJSValue(status.w_oldinter,"C",16,0)+"),'C');");
    } else {
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_import_fileaeR Rcalled;
    Rcalled = new arrt_import_fileaeR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_import_fileaeR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_import_fileaeR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_import_fileae"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_faggp="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_faggp),"?",0,0)+"");
        status.out.println("js:w_faggr="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_faggr),"?",0,0)+"");
        status.out.println("js:w_oldinter="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_oldinter),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_faggp,null)) {");
        status.out.println("window[f].w_faggp="+SPLib.ToJSValue(status.w_faggp,"C",0,0)+";");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_faggr,null)) {");
        status.out.println("window[f].w_faggr="+SPLib.ToJSValue(status.w_faggr,"C",0,0)+";");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_oldinter,null)) {");
        status.out.println("window[f].w_oldinter="+SPLib.ToJSValue(status.w_oldinter,"C",0,0)+";");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_import_fileae");
      if ( ! (CPLib.IsNull(l))) {
