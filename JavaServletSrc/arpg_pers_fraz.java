...
public class arpg_pers_fraz extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*CMXVRRNYQI*!";
  ...
  public String getEntityTitle() {
    return "Frazionate relative alla Persone / Società / Cliente rapporto (Effettive)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_pers_fraz";
    ...
    l_eg.m_cQueryName = "qbe_pers_fraz";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_pers_frazBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10438573013",p_Context.Translate("Frazionate relative alla Persone / Società / Cliente rapporto (Effettive)")},
  ...
  void WriteStyles(arpg_pers_frazBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_pers_fraz.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_pers_frazBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_pers_frazBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_pers_frazBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_pers_frazBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_pers_frazBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_pers_frazBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_pers_frazBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_pers_frazBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_pers_fraz"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_pers_fraz","");
  ...
  void WriteMainFormScript(arpg_pers_frazBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_pers_frazBO getBO(ServletStatus status) {
    arpg_pers_frazBO BO = new arpg_pers_frazBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_pers_frazBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_pers_frazBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_pers_frazBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_pers_frazBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_pers_frazBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_pers_frazBO BO) {
    ...
    item.put("altInterfaceFor","arpg_pers_fraz");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_pers_frazBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_pers_frazBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_pers_frazBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_pers_frazBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_pers_frazBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_pers_frazBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_pers_fraz?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_pers_fraz_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_pers_frazBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_pers_frazBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_pers_frazBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_pers_frazBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_pers_frazBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_pers_frazBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_pers_frazBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_pers_frazBO BO) {
    boolean result = true;
    return result;
  }
}
