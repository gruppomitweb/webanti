...
bResult=Eq(w_gCodDip,w_SOSCODDIP) || Utilities.Make(window).IsInGroup(1) || Utilities.Make(window).IsInGroup(9) || Utilities.Make(window).IsInGroup(11) || Utilities.Make(window).IsInGroup(12);
}
...
WindowAlert(AlertErrorMessage('11753347534'));
}
if (Eq(formaction,'edit') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('10201018318'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('1464959469'));
}
}
}
var m_bIsMq = false;
...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('IFUSPOLLJT').value,'N'),w_SOSCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_sos',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'SOSCODICE') &&  ! (Empty(HtW(Ctrl('IFUSPOLLJT').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.SOSCODICE',HtW(Ctrl('IFUSPOLLJT').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_sos');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_sos',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['IFUSPOLLJT'],i=0;i>=0;i--){
