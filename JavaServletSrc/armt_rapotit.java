...
public class armt_rapotit extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ad883699*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","Dati","page"),new ZoneItem("box_YORQOIOBQN","","auto_horizontal_section"),new ZoneItem("box_BOYWPGJEEZ","","auto_horizontal_section"),new ZoneItem("box_KIVHKOCKYW","Dati Rettifiche","horizontal_section")};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_rapotitBO BO) {
  ...
  void WriteStyles(armt_rapotitBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_rapotit.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"Dati")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_rapotit',false);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_rapotit",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(armt_rapotitBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:710px;width:807px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(armt_rapotitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(armt_rapotitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<div name='box_YORQOIOBQN' id='YORQOIOBQN_DIV' class='BoxContainer SPSection_expanded SectionContainerWithoutTitle'>" +
    "");
    status.out.println(
    ...
    if (status.includeZoneAltInterface("box_YORQOIOBQN")) {
      ...
      CPResultSet l_rsLoad=null;
      VQRHolder l_VQRHolder=null;
      WriteControls_p1_b2_b0(BO,status);
      WriteControls_p1_b2_b1(BO,status);
      WriteControls_p1_b2_b2(BO,status);
      status.out.println(
       "<script>" +
      "");
      status.out.println("ToggleCollapsibleBox.subBands['YORQOIOBQN']=['band_1_48','band_1_248','band_1_596'];");
      status.out.println(
       "</script>" +
      "");
      if (status.HasZoneAltInterface("box_YORQOIOBQN@BEGIN") || status.HasZoneAltInterface("box_YORQOIOBQN@END")) {
  ...
  void WriteControls_p1_b2_CloseBand(armt_rapotitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2_b0(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_48' style='position:relative;height:200px;display:block' page='1'>" +
    "");
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input id='QIETKIRGON' type='button' class='SPButton' value="+SPLib.ToHTMLValue(status.context.Translate("Crea Informazioni 54 e 55"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' style='display:none'>" +
    "");
    status.out.println(
  ...
  void WriteControls_p1_b2_b1(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_248' style='position:relative;display:block' page='1'>" +
    "");
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<iframe name='ardt_intestit' id='QBBTFAGDFU' frameborder='0' allowtransparency='true' toResize='no' tabindex='"+status.m_nTabIndex+"' src='javascript:[].join()'>" +
     "</iframe>" +
    "");
    status.out.println(
     "</div>" +
    ...
    status.out.println(
     "<script>" +
    "");
    status.out.println("ToggleCollapsibleBox.children['band_1_248']=['QBBTFAGDFU'];");
    status.out.println(
     "</script>" +
    "");
  }
  void WriteControls_p1_b2_b2(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='band_1_596' style='position:relative;height:14px;display:block' page='1'>" +
    "");
    CPResultSet l_rsLoad=null;
    VQRHolder l_VQRHolder=null;
    status.out.println(
  ...
  void WriteControls_p1_b3(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<div name='box_KIVHKOCKYW' id='KIVHKOCKYW_DIV' class='BoxContainer SPSection_expanded SectionContainerWithTitle'>" +
    "");
    ...
    status.out.println(
     "<div id='KIVHKOCKYW' class='SectionBody' style='position:relative;height:36px;display:block'>" +
    "");
  ...
  void WriteControls_p1_b3_CloseBand(armt_rapotitBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(armt_rapotitBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_rapotitBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_rapotitBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
    if (BO.ardt_intestit.IsUpdated()) {
      BO.m_cWv_ardt_intestit = BO.ardt_intestit.GetContext();
    }
  }
  static void PrintStateWorkVars(armt_rapotitBL BO,ServletStatus status) {
    ...
    if (CPLib.eq(BO.m_cWv_ardt_intestit,"_not_empty_") && status.m_bBrowserUpdate) {
      status.out.print(","+"m_cWv_ardt_intestit");
    } else if (CPLib.eq(BO.m_cWv_ardt_intestit,"_not_empty_")) {
      BO.m_cWv_ardt_intestit = BO.ardt_intestit.GetContext();
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_ardt_intestit,"C",0,0));
    } else {
      status.out.print(","+SPLib.ToJSValue(BO.m_cWv_ardt_intestit,"C",0,0));
    }
  ...
  void WriteMainFormVarScript(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_rapotit"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_rapotit","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_rapotit",BO.GetCurrentState());
  ...
  void WriteMainFormScript(armt_rapotitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- armt_rapotit");
    ...
    status.out.println("if (Eq(name,'ardt_intestit')) {");
    status.out.println("l_cRes='"+SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_intestit")+"';");
    ...
    status.out.println("  Ctrl('QBBTFAGDFU').src='ardt_intestit?m_bInnerChild=true&m_cAction=editload&CODCLI='+w_CONNESCLI+'&m_bRowsChild=True'    ");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
     "</script>" +
  ...
  armt_rapotitBO getBO(ServletStatus status) {
    armt_rapotitBO BO = new armt_rapotitBO(status.context);
  ...
  void LoadExtensions(armt_rapotitBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("rapotit",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_rapotitBO BO) {
  ...
  void Calculation_XMORYLJOLP(ServletStatus status,armt_rapotitBO BO) {
  ...
  void WriteTableCombo(ServletStatus status,armt_rapotitBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
    l_cTmp = "[";
    VQRHolder l_VQRHolder=null;
  ...
  CPResultSet QueryKeyCursor(armt_rapotitBO BO,ServletStatus status) {
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_rapotitBL.QueryFilter(status.context);
  ...
  void Query(ServletStatus status,armt_rapotitBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
  ...
  void View(ServletStatus status,armt_rapotitBO BO) {
  ...
  void Edit(ServletStatus status,armt_rapotitBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_rapotitBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_rapotitBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_rapotitBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_rapotitBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_rapotitBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_rapotitBO BO,SPParameterSource source) {
    BO.Load(status.w_IDBASE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_rapotitBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_rapotitBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_rapotitBO BO) {
    ...
    item.put("altInterfaceFor","armt_rapotit");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_BOYWPGJEEZ\",\"box_type\":\"auto_horizontal_section\",\"collapsible\":true,\"h\":35,\"name\":\"box_BOYWPGJEEZ\",\"page\":1,\"shrinkable\":true,\"spuid\":\"BOYWPGJEEZ\",\"stretch\":true,\"tabindex\":4,\"type\":\"Box\",\"w\":769,\"x\":16,\"y\":14,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Codice Dipendenza\",\"h\":21,\"maxlength\":6,\"name\":\"CODDIPE\",\"page\":1,\"picker\":true,\"spuid\":\"AVHTGZTOSZ\",\"tabindex\":8,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":48,\"x\":232,\"y\":21,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_LOEAMXUYLA\",\"page\":1,\"spuid\":\"LOEAMXUYLA\",\"tabindex\":17,\"type\":\"Label\",\"value\":\"Intermediario:\",\"w\":78,\"x\":24,\"y\":25,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_XVRJYSKGWZ\",\"page\":1,\"spuid\":\"XVRJYSKGWZ\",\"tabindex\":21,\"type\":\"Label\",\"value\":\"Dipen.:\",\"w\":40,\"x\":192,\"y\":25,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_CTLYJMTHDV\",\"page\":1,\"spuid\":\"CTLYJMTHDV\",\"tabindex\":22,\"type\":\"Label\",\"value\":\"Citt\\u00E0:\",\"w\":28,\"x\":461,\"y\":25,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_JWSDOSRGAG\",\"page\":1,\"spuid\":\"JWSDOSRGAG\",\"tabindex\":23,\"type\":\"Label\",\"value\":\"Pr.:\",\"w\":24,\"x\":646,\"y\":25,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":17,\"name\":\"label_ENBRMMDVUP\",\"page\":1,\"spuid\":\"ENBRMMDVUP\",\"tabindex\":24,\"type\":\"Label\",\"value\":\"CAB:\",\"w\":27,\"x\":703,\"y\":25,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":17,\"name\":\"label_BLXOBUQVYE\",\"page\":1,\"spuid\":\"BLXOBUQVYE\",\"tabindex\":25,\"type\":\"Label\",\"w\":152,\"x\":304,\"y\":25,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_YORQOIOBQN\",\"box_type\":\"auto_horizontal_section\",\"collapsible\":true,\"h\
",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Dati Rettifiche\",\"h\":36,\"name\":\"box_KIVHKOCKYW\",\"page\":1,\"shrinkable\":true,\"spuid\":\"KIVHKOCKYW\",\"stretch\":true,\"tabindex\":83,\"title_caption\":\"Dati Rettifiche\",\"titled\":true,\"type\":\"Box\",\"w\":784,\"x\":8,\"y\":609,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Rettifica\",\"h\":20,\"maxlength\":10,\"name\":\"DATARETT\",\"page\":1,\"spuid\":\"VYBZEWHKFF\",\"tabindex\":89,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":80,\"x\":529,\"y\":617,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_PXIMPOLVHG\",\"page\":1,\"spuid\":\"PXIMPOLVHG\",\"tabindex\":90,\"type\":\"Label\",\"value\":\"A54B:\",\"w\":32,\"x\":497,\"y\":621,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Stato\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":1,\"name\":\"STATOREG\",\"page\":1,\"spuid\":\"OJXGBVSJHY\",\"tabindex\":91,\"textlist\":\"0 - Registrazione in essere mai modificata,1 - Registrazione in essere sostitutiva di precedente registrazione,2 - Registrazione annullata (cancellata per rettifica),3 - Registrazione annullata (sostituita per rettifica)\",\"type\":\"Combobox\",\"typevar\":\"character\",\"valuelist\":\""+"'"+"0"+"'"+","+"'"+"1"+"'"+","+"'"+"2"+"'"+","+"'"+"3"+"'"+"\",\"w\":336,\"x\":153,\"y\":617,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_MBISTIEAGL\",\"page\":1,\"spuid\":\"MBISTIEAGL\",\"tabindex\":92,\"type\":\"Label\",\"value\":\"A54A. Stato Registrazione:\",\"w\":144,\"x\":9,\"y\":621,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Operazione Modificata\",\"disabled\":\"true\",\"h\":20,\"maxlength\":20,\"name\":\"OPERAZMOD\",\"page\":1,\"readonly\":\"true\",\"spuid\":\"NSRIXGWOLV\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":132,\"x\":649,\"y\":617,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_AGZEQLWBPS\",\"page\":1,\"spuid\":\"AGZEQLWBPS\",\"tabindex\":94,\"type\":\"Label\",\"value\":\"A54C:\",\"w\":40,\"x\":609,\"y\":621,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_rapotitBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_rapotit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_rapotit_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_rapotit_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_DATARETT=a[33];");
      status.out.println("w_flgpf=a[34];");
      status.out.println("w_flgpg=a[35];");
      status.out.println("w_gCodDip=a[36];");
      status.out.println("w_gIntemediario=a[37];");
      status.out.println("w_xdescrap=a[38];");
      status.out.println("w_xDescDipe=a[39];");
      status.out.println("w_datpaseuro=a[40];");
      status.out.println("w_gChkDate=a[41];");
      status.out.println("w_xDesReg=a[42];");
      status.out.println("w_xRagSoc=a[43];");
      ...
      status.out.println("w_gSblocco=a[49];");
      status.out.println("w_tipreg=a[50];");
      status.out.println("w_tipregn=a[51];");
      status.out.println("w_dataoggi=a[52];");
      status.out.println("w_gStatus=a[53];");
      status.out.println("w_gDataVaria=a[54];");
      status.out.println("w_CITINT=a[55];");
      status.out.println("w_CITDIP=a[56];");
      status.out.println("w_PROVDIP=a[57];");
      status.out.println("w_CABDIP=a[58];");
      status.out.println("w_PROVINT=a[59];");
      status.out.println("w_CABINT=a[60];");
      status.out.println("w_xRagSocCli=a[61];");
      status.out.println("w_filtro=a[62];");
      status.out.println("w_SESSO=a[63];");
      status.out.println("op_codazi=a[64];");
      status.out.println("m_cWv_ardt_intestit=a[65];");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_rapotit_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(armt_rapotitBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_rapotitBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_rapotitBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_rapotitBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_rapotitBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_rapotitBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_rapotitBO BO,ServletStatus status) throws IOException {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      CPLib.AddToSet(callable,"ardt_intestit");
    } else {
  ...
  public String GetCheckVars(ServletStatus status,armt_rapotitBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_rapotitBO BO) {
    boolean result = true;
