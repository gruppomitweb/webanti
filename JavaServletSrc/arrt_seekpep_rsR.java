...
import java.util.Base64;
// * --- Fine Area Manuale
public class arrt_seekpep_rsR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_seekpep_rsR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_seekpep_rs",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      Page_2();
      /* Exec "Legge il ticket e attende risposta" Page 3:Page_3 */
      Page_3();
      /* Return _ticket */
      throw new Stop(_ticket);
    } finally {
      try {
        if (Cursor_intermbo!=null)
          Cursor_intermbo.Close();
  ...
  void Page_2() throws Exception {
    ...
    JSONObject jsonObj;
    jsonObj = null;
    double nTry0000003Cstatus;
    nTry0000003Cstatus = m_Sql.GetTransactionStatus();
    String cTry0000003Cmsg;
    cTry0000003Cmsg = m_Sql.TransactionErrorMessage();
    try {
      /* Colloquoi XML */
      ...
      // Settaggio parametri
      NameValuePair nv1 = new BasicNameValuePair("tipo","REPORTPEXMLTIPOO");
      nvPairList.add(nv1);
      NameValuePair nv2 = new BasicNameValuePair("parametri.CFISCP",CPLib.LRTrim(pCodFisc));
      nvPairList.add(nv2);
      NameValuePair nv3 = new BasicNameValuePair("parametri.IDTIPOREPORT","102");
      nvPairList.add(nv3);
      NameValuePair nv4 = new BasicNameValuePair("parametri.TIPOPERSONA",CPLib.LRTrim(pTipo));
      nvPairList.add(nv4);
      
      /* If pTipo='F' */
      if (CPLib.eqr(pTipo,"F")) {
        // Parametri per Persona Fisica
        NameValuePair nv5 = new BasicNameValuePair("parametri.COGNOMEDENOM",CPLib.LRTrim(pCognome));
        nvPairList.add(nv5); 
        NameValuePair nv6 = new BasicNameValuePair("parametri.NOME",CPLib.LRTrim(pNome));
        nvPairList.add(nv6);
      } else { // Else
        // Parametri per persona giuridica
        NameValuePair nv5 = new BasicNameValuePair("parametri.COGNOMEDENOM",CPLib.LRTrim(pRagSoc));
        nvPairList.add(nv5);
      } // End If
      ...
      if (CPLib.eqr(m_Sql.GetTransactionStatus(),0) || CPLib.ne(nTry0000003Cstatus,0)) {
        m_Sql.SetTransactionStatus(nTry0000003Cstatus,cTry0000003Cmsg);
      }
    }
  }
  void Page_3() throws Exception {
    ...
    if (jsonObject.has("RECORD")) {
      ...
      /* If jsonRecord.has("RICORRENZA") */
      if (jsonRecord.has("RICORRENZA")) {
        // Crea oggetto per lettura
        JSONObject jsonRicorrenza = jsonRecord.getJSONObject("RICORRENZA");
        /* If jsonRicorrenza.has("TICKET") */
        if (jsonRicorrenza.has("TICKET")) {
          /* _ticket := jsonRicorrenza.get("TICKET").toString() */
          _ticket = jsonRicorrenza.get("TICKET").toString();
        } // End If
        /* ElseIf jsonRecord.has("TICKET") */
      } else if (jsonRecord.has("TICKET")) {
        /* _ticket := jsonRecord.get("TICKET").toString() */
        _ticket = jsonRecord.get("TICKET").toString();
      } // End If
    } // End If
  }
  ...
  public String RunAsync(String p_pRagSoc,String p_pCognome,String p_pNome,String p_pCodFisc,String p_pTipo) {
    pRagSoc = p_pRagSoc;
    pCognome = p_pCognome;
    pNome = p_pNome;
    pCodFisc = p_pCodFisc;
  ...
  public String Run(String p_pRagSoc,String p_pCognome,String p_pNome,String p_pCodFisc,String p_pTipo) {
    pRagSoc = p_pRagSoc;
    pCognome = p_pCognome;
    pNome = p_pNome;
    pCodFisc = p_pCodFisc;
  ...
  public static arrt_seekpep_rsR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_seekpep_rsR(p_Ctx, p_Caller);
  }
  public static arrt_seekpep_rsR Make(CPContext p_Ctx) {
    return new arrt_seekpep_rsR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arrt_seekpep_rs,
  public static final String i_InvokedRoutines = ",arrt_seekpep_rs,";
  public static String[] m_cRunParameterNames={"pRagSoc","pCognome","pNome","pCodFisc","pTipo"};
}
