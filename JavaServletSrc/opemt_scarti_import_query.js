...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('QXNYFHJEGV').value,'N'),w_IMPPROG)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('scarti_import',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IMPPROG') &&  ! (Empty(HtW(Ctrl('QXNYFHJEGV').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IMPPROG',HtW(Ctrl('QXNYFHJEGV').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','opemt_scarti_import');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('scarti_import',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['QXNYFHJEGV'],i=0;i>=0;i--){
