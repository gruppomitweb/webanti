...
public class arfn_import_correzioni_sv_all extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ed373c15*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arfn_import_correzioni_sv_all";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arfn_import_correzioni_sv_allR Rcalled;
    Rcalled = new arfn_import_correzioni_sv_allR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.a_retval = Rcalled.Run(status.nomefile);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arfn_import_correzioni_sv_allR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arfn_import_correzioni_sv_allR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arfn_import_correzioni_sv_all"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
