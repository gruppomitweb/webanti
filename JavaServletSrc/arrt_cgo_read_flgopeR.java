...
// * --- Fine Area Manuale
public class arrt_cgo_read_flgopeR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_read_flgopeR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_read_flgope",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      /* w_CACODICE Numeric(5, 0) */
      /* w_CAFLGOPE Char(1) */
      // * --- Select from cgo_causali
      ...
      Cursor_cgo_causali = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select CAFLGOPE  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"CACODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CACODICE,"?",0,0,m_cServer, m_oParameters),m_cServer,w_CACODICE)+" "+")"+(m_Ctx.IsSharedTemp("cgo_causali")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
      while ( ! (Cursor_cgo_causali.Eof())) {
        /* w_CAFLGOPE := cgo_causali->CAFLGOPE */
        w_CAFLGOPE = Cursor_cgo_causali.GetString("CAFLGOPE");
  ...
  public static arrt_cgo_read_flgopeR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_read_flgopeR(p_Ctx, p_Caller);
  }
  public void Blank() {
