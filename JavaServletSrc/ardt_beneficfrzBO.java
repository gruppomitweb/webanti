...
// * --- Fine Area Manuale
public class ardt_beneficfrzBO extends ardt_beneficfrzBL {
  ...
  // * --- Fine Area Manuale
  public ardt_beneficfrzBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_beneficfrz");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_beneficfrz+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_beneficfrz,m_cPhName_beneficfrz,m_cVirtName_beneficfrz,m_cVirtName_beneficfrz+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_beneficfrz,p_IDFILEBO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"beneficfrz")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_beneficfrz.GetTableWriteName()+" where ";
        ...
        l_cSql = l_cSql+m_oWrInfo_beneficfrz.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"beneficfrz")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_beneficfrz,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_beneficfrz);
      l_cSql = "insert into "+m_oWrInfo_beneficfrz.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_beneficfrz.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"beneficfrz")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_beneficfrz.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"beneficfrz")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_beneficfrz,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_beneficfrz);
      l_cSql = "Update "+m_oWrInfo_beneficfrz.GetTableWriteName()+" set";
