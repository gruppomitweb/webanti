...
public class arpg_revimp extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*MDEKKXGIFB*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_revimp";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 789;
    l_eg.m_nEntityHeight = 306;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_revimpBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1470614271",p_Context.Translate("Confermi la creazione dei file di import ?")},
    {"1952592827",p_Context.Translate("Reverse Import")}};
    return l_translations;
  }
  void WriteStyles(arpg_revimpBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_revimp.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_revimpBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_revimpBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_revimpBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_revimpBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_revimpBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(301);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_revimpBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_revimpBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_revimpBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_revimpBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_revimpBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(306,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_revimp"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_revimp","");
  ...
  void WriteMainFormScript(arpg_revimpBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_revimp");
  ...
  arpg_revimpBO getBO(ServletStatus status) {
    arpg_revimpBO BO = new arpg_revimpBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_revimpBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_revimpBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_revimpBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_revimpBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_revimpBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_revimpBO BO) {
    ...
    item.put("altInterfaceFor","arpg_revimp");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"294\",\"name\":\"Box_YWGFSYCMOZ\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"YWGFSYCMOZ\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui elaborazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_XWPYHZIJDE()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"XWPYHZIJDE\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"3\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"680\",\"y\":\"14\",\"zone\":\"YWGFSYCMOZ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"210\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"147\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"56\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Chiudi la maschera\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_JSKZJOHJOM()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"JSKZJOHJOM\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"6\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"728\",\"y\":\"14\",\"zone\":\"YWGFSYCMOZ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_revimpBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_revimpBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_revimpBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_revimp?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_revimp_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(306,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void RaiseLoadedEntity(arpg_revimpBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_revimpBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_revimpBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_revimpBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_revimpBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_revimpBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_revimp.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_revimpBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_revimpBO BO) {
    boolean result = true;
    return result;
  }
}
