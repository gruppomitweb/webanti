...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('HGAXRLVKSP').value,'C'),w_RAMGRU)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbramgru',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'RAMGRU') &&  ! (Empty(HtW(Ctrl('HGAXRLVKSP').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.RAMGRU',HtW(Ctrl('HGAXRLVKSP').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_ramogruppo');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbramgru',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['HGAXRLVKSP'],i=0;i>=0;i--){
