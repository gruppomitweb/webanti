...
public class arrt_fammigra_old extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*e6101ac3*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.pAzzera","pAzzera");
      }        
      pAzzera = source.GetParameter("pAzzera","");
    }
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_fammigra_oldR Rcalled;
    Rcalled = new arrt_fammigra_oldR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.a_retval = Rcalled.Run(status.pAzzera);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_fammigra_oldR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_fammigra_oldR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_fammigra_old"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
