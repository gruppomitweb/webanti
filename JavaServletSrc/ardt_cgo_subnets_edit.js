...
function Set_SNSUBNET(ctrlValue,kmode,e) {
if (Ne(w_SNSUBNET,ctrlValue)) {
var ctl = _GetCtl(e,'LXOGHRVGTK');
if (_tracker.goon(ctl,ctrlValue)) {
w_SNSUBNET=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('LXOGHRVGTK',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
if ( ! (m_oFather.SaveContext_ardt_cgo_subnets())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_subnets_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_subnets_dataNotSent',{'action':formaction});
}
}
}
}
...
l_oWv.setValue('SNCODINT',WtA(w_SNCODINT,'C'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
if (( ! (_ChkObl('LXOGHRVGTK',1))) && FullRow()) {
_SignErr('LXOGHRVGTK');
m_cLastWorkVarErrorMsg='SNSUBNET';
l_bResult=false;
w_SNSUBNET=HtW('','C');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_cgo_subnets();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_SNSUBNET));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.SNSUBNET={"dec":0,"fixedpos":false,"id":"LXOGHRVGTK","len":15,"name":"SNSUBNET","title":"","type":"C"};
