...
public class arpg_operazbo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*YYLEKYPMKD*!";
  ...
  public String getEntityTitle() {
    return "Operazioni relative al rapporto (A.U.I.)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_operazbo";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_operazboBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11982518865",p_Context.Translate("Operazioni relative al rapporto (A.U.I.)")},
    {"1991862041",p_Context.Translate("Importo")},
    {"7776403",p_Context.Translate("Stato")}};
    return l_translations;
  }
  void WriteStyles(arpg_operazboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_operazbo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_operazboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_operazboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_operazboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_operazboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_operazboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_operazboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_operazboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_operazboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_operazbo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_operazbo","");
    ...
    status.out.println("// * --- arpg_operazbo");
  ...
  void WriteMainFormScript(arpg_operazboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_operazboBO getBO(ServletStatus status) {
    arpg_operazboBO BO = new arpg_operazboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_operazboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_operazboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_operazboBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_operazboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_operazboBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_operazboBO BO) {
    ...
    item.put("altInterfaceFor","arpg_operazbo");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_operazboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_operazboBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_operazboBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_operazboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_operazboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_operazboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_operazbo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_operazbo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_operazboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_operazboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_operazboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_operazboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_operazboBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_operazboBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_operazboBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_operazboBO BO) {
    boolean result = true;
    return result;
  }
}
