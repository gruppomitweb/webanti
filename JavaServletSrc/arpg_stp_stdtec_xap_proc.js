...
SetDisplay(Ctrl('BQKVLSRQVU'),true);
HideUI();
}
...
function href_BQKVLSRQVU() {
var l_oWv = InitWvApplet();
l_oWv.setValue('pAUI',AsAppletValue('P'));
l_oWv.setValue('pPDF',AsAppletValue('PDF'));
...
w_Messaggi.raiseEvent('arpg_stp_stdtec_xap_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_stp_stdtec_xap_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_stp_stdtec_xap_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_stp_stdtec_xap',m_cSelectedPage);
...
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('xnarapbo','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
...
_FillChildren.n=[];
FillWv.n=["daCodDip","aCodDip","DaDatOpe","ADatOpe","daRapporto","aRapporto","RAGSOC1","RAGSOC2","DESCRIZ1","DESCRIZ2","ADatReg","DaDatReg"];
...
TransferBetweenWindows(opener,'w_ADatReg','w_ADatReg');
}
if (Empty(w_DaDatReg) && Ne(opener.w_DaDatReg,null)) {
TransferBetweenWindows(opener,'w_DaDatReg','w_DaDatReg');
}
}
}
m_bCalculating=true;
