...
// * --- Fine Area Manuale
public class arrt_li_kperazboR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_li_kperazboR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_li_kperazbo",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* pObj Object(armt_kperazboBL) */
      /* Variabili locali */
      /* conta Numeric(2, 0) */
      /* _tiporap Char(1) */
      /* Carica il contesto del figlio */
      /* pObj.CtxLoad_ardt_clientisara() */
      pObj.CtxLoad_ardt_clientisara();
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on knarapbo into routine arrt_li_kperazbo returns two or more records. This item should return only a record.");
      ...
      if (CPLib.eqr(_tiporap,"D")) {
        ...
        while ( ! (Cursor_intestbo.Eof())) {
          ...
          if (CPLib.eqr(conta,0)) {
            ...
            /* pObj.ardt_clientisara.AddRow() */
            pObj.ardt_clientisara.AddRow();
            /* pObj.ardt_clientisara.Set_CODSOG(intestbo->CODINTER) */
            pObj.ardt_clientisara.Set_CODSOG(Cursor_intestbo.GetString("CODINTER"));
            /* pObj.ardt_clientisara.SaveRow() */
            pObj.ardt_clientisara.SaveRow();
            /* pObj.ardt_clientisara.SetUpdated() */
            pObj.ardt_clientisara.SetUpdated();
          } // End If
        ...
        while ( ! (Cursor_xntestbo.Eof())) {
          ...
          if (CPLib.eqr(conta,0)) {
            ...
            /* pObj.ardt_clientisara.AddRow() */
            pObj.ardt_clientisara.AddRow();
            /* pObj.ardt_clientisara.Set_CODSOG(xntestbo->CODINTER) */
            pObj.ardt_clientisara.Set_CODSOG(Cursor_xntestbo.GetString("CODINTER"));
            /* pObj.ardt_clientisara.SaveRow() */
            pObj.ardt_clientisara.SaveRow();
            /* pObj.ardt_clientisara.SetUpdated() */
            pObj.ardt_clientisara.SetUpdated();
  ...
  public String RunAsync(armt_kperazboBL p_pObj) {
    pObj = p_pObj;
  ...
  public Forward Run(armt_kperazboBL p_pObj) {
    pObj = p_pObj;
    return Run();
  }
  public Forward Run() {
  ...
  public static arrt_li_kperazboR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_li_kperazboR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pObj = null;
    conta = 0;
