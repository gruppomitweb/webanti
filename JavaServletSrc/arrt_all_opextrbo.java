...
public class arrt_all_opextrbo extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*75c678fd*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_c_adata) ||  ! (status.m_bpassedw_c_dadata) ||  ! (status.m_bpassedw_ADatOpe) ||  ! (status.m_bpassedw_DaDatOpe);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_all_opextrboR Rcalled;
    Rcalled = new arrt_all_opextrboR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_all_opextrboR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_all_opextrboR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_all_opextrbo"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_all_opextrbo");
      if ( ! (CPLib.IsNull(l))) {
