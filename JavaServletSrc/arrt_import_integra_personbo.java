...
public class arrt_import_integra_personbo extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*78546a4a*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_import_integra_personbo";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_impzip);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_import_integra_personboR Rcalled;
    Rcalled = new arrt_import_integra_personboR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_import_integra_personboR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_import_integra_personboR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_import_integra_personbo"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_import_integra_personbo");
      if ( ! (CPLib.IsNull(l))) {
