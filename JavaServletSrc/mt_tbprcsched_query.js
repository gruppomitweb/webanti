...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SQTPJSDVUJ').value,'C'),w_PSPROCESSO)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbprcsched',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'PSPROCESSO') &&  ! (Empty(HtW(Ctrl('SQTPJSDVUJ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.PSPROCESSO',HtW(Ctrl('SQTPJSDVUJ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','mt_tbprcsched');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbprcsched',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['SQTPJSDVUJ'],i=0;i>=0;i--){
