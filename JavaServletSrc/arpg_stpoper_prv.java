...
public class arpg_stpoper_prv extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*MESBYSNOHN*!";
  ...
  public String getEntityTitle() {
    return "Stampa Operazioni (Provvisorie)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_stpoper_prv";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_stpoper_prvBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"2042593877",p_Context.Translate("Stampa Operazioni (Provvisorie)")},
    {"285808099",p_Context.Translate("Descrizione")},
    {"862812006",p_Context.Translate("Tipo operazione")}};
    return l_translations;
  }
  void WriteStyles(arpg_stpoper_prvBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_stpoper_prv.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_stpoper_prvBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_stpoper_prvBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_stpoper_prvBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(657);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_stpoper_prvBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_stpoper_prvBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_stpoper_prvBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_stpoper_prvBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_stpoper_prvBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_stpoper_prv"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_stpoper_prv","");
  ...
  void WriteMainFormScript(arpg_stpoper_prvBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_stpoper_prvBO getBO(ServletStatus status) {
    arpg_stpoper_prvBO BO = new arpg_stpoper_prvBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_stpoper_prvBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_stpoper_prvBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_stpoper_prvBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_stpoper_prvBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_stpoper_prvBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_stpoper_prvBO BO) {
    ...
    item.put("altInterfaceFor","arpg_stpoper_prv");
    items.put(item);
    String s;
  ...
  void Next(ServletStatus status,arpg_stpoper_prvBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_stpoper_prvBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_stpoper_prvBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_stpoper_prv?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_stpoper_prv_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_stpoper_prvBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_stpoper_prvBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_stpoper_prvBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_stpoper_prvBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_stpoper_prvBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_stpoper_prvBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_stpoper_prv.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_stpoper_prvBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_stpoper_prvBO BO) {
    boolean result = true;
    return result;
  }
}
