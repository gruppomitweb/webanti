...
// * --- Fine Area Manuale
public class arrt_start_wuimportjobR implements CallerWithObjs {
  ...
  public String _lancio;
  public String _tipimp;
  // * --- Area Manuale = BO - Properties
  // * --- Fine Area Manuale
  public arrt_start_wuimportjobR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_start_wuimportjob",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_start_wuimportjob";
    } else if (m_bEventRunning && CPLib.eqr("i_EntityType",p_cVarName)) {
      return "routine";
    }
    if (CPLib.eqr("_lancio",p_cVarName)) {
      return _lancio;
    }
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* _lancio Char(1) */
      /* _tipimp Char(1) */
      /* Info Msg 'Import File RTRA' */
      if ( ! (CPLib.IsNull(m_Info))) {
        m_Info.log(CPLib.FormatMsg(m_Ctx,"Import File RTRA"));
      }
      ...
      Cursor_intermbo = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select FLGTIPIMP  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+(m_Ctx.IsSharedTemp("intermbo")?"context_id='"+m_Ctx.GetTempID()+"'":"1=1"),m_cServer,m_Ctx,true));
      ...
      Cursor_intermbo.Close();
      // * --- End Select
      /* _lancio := arrt_imp_dati_rtra(_tipimp,'S','S','B','I','N','N','N') */
      _lancio = arrt_imp_dati_rtraR.Make(m_Ctx,this).Run(_tipimp,"S","S","B","I","N","N","N");
    } finally {
      try {
        if (Cursor_intermbo!=null)
          Cursor_intermbo.Close();
  ...
  public static arrt_start_wuimportjobR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_start_wuimportjobR(p_Ctx, p_Caller);
  }
  public void Blank() {
    _lancio = CPLib.Space(1);
