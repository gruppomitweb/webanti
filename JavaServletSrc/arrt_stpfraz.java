...
public class arrt_stpfraz extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*e359f969*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_tipstp;
    public String w_tipstp;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_Tipop;
    public String w_Tipop;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_stato;
    public String w_stato;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_OrderBy;
    public String w_OrderBy;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_daCodDip;
    public String w_daCodDip;
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_aCodDip;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_tipstp = source.Has("w_tipstp");
      if (m_bpassedw_tipstp) {
        w_tipstp = source.GetParameter("w_tipstp",source.GetParameter("tipstp",""));
      } else {
        if (source.IsForwarded()) {
          w_tipstp = source.GetCaller().GetString("tipstp","C",1,0);
          m_bpassedw_tipstp = true;
        } else {
          w_tipstp = source.GetParameter("tipstp",source.GetParameter("w_tipstp",""));
        }
      }
      m_bpassedw_Tipop = source.Has("w_Tipop");
      if (m_bpassedw_Tipop) {
        w_Tipop = source.GetParameter("w_Tipop",source.GetParameter("Tipop",""));
      } else {
        if (source.IsForwarded()) {
          w_Tipop = source.GetCaller().GetString("Tipop","C",1,0);
          m_bpassedw_Tipop = true;
        } else {
          w_Tipop = source.GetParameter("Tipop",source.GetParameter("w_Tipop",""));
        }
      }
      m_bpassedw_stato = source.Has("w_stato");
      if (m_bpassedw_stato) {
        w_stato = source.GetParameter("w_stato",source.GetParameter("stato",""));
      } else {
        if (source.IsForwarded()) {
          w_stato = source.GetCaller().GetString("stato","C",1,0);
          m_bpassedw_stato = true;
        } else {
          w_stato = source.GetParameter("stato",source.GetParameter("w_stato",""));
        }
      }
      ...
      if (m_bpassedw_OrderBy) {
        ...
        if (source.IsForwarded()) {
          w_OrderBy = source.GetCaller().GetString("OrderBy","C",7,0);
          m_bpassedw_OrderBy = true;
        } else {
          w_OrderBy = source.GetParameter("OrderBy",source.GetParameter("w_OrderBy",""));
        }
      }
      m_bpassedw_daCodDip = source.Has("w_daCodDip");
      if (m_bpassedw_daCodDip) {
        w_daCodDip = source.GetParameter("w_daCodDip",source.GetParameter("daCodDip",""));
      } else {
        if (source.IsForwarded()) {
          w_daCodDip = source.GetCaller().GetString("daCodDip","C",6,0);
          m_bpassedw_daCodDip = true;
        } else {
          w_daCodDip = source.GetParameter("daCodDip",source.GetParameter("w_daCodDip",""));
        }
      }
      m_bpassedw_aCodDip = source.Has("w_aCodDip");
      if (m_bpassedw_aCodDip) {
        w_aCodDip = source.GetParameter("w_aCodDip",source.GetParameter("aCodDip",""));
      } else {
        if (source.IsForwarded()) {
          w_aCodDip = source.GetCaller().GetString("aCodDip","C",6,0);
          m_bpassedw_aCodDip = true;
        } else {
          w_aCodDip = source.GetParameter("aCodDip",source.GetParameter("w_aCodDip",""));
        }
      }
    }
  }
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_aCodDip) ||  ! (status.m_bpassedw_daCodDip) ||  ! (status.m_bpassedw_OrderBy) ||  ! (status.m_bpassedw_stato) ||  ! (status.m_bpassedw_Tipop) ||  ! (status.m_bpassedw_tipstp) ||  ! (status.m_bpassedw_c_dadata) ||  ! (status.m_bpassedw_c_adata) ||  ! (status.m_bpassedw_aCodCli) ||  ! (status.m_bpassedw_daCodCli) ||  ! (status.m_bpassedw_ADatOpe) ||  ! (status.m_bpassedw_DaDatOpe);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_tipstp)) {
      status.out.println("l_cV=WtA(l_oWnd('w_tipstp',"+SPLib.ToJSValue(status.w_tipstp,"C",1,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_tipstp,"C",1,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_tipstp',l_cV);");
    if ( ! (status.m_bpassedw_Tipop)) {
      status.out.println("l_cV=WtA(l_oWnd('w_Tipop',"+SPLib.ToJSValue(status.w_Tipop,"C",1,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_Tipop,"C",1,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_Tipop',l_cV);");
    if ( ! (status.m_bpassedw_stato)) {
      status.out.println("l_cV=WtA(l_oWnd('w_stato',"+SPLib.ToJSValue(status.w_stato,"C",1,0)+"),'C');");
    } else {
    ...
    if ( ! (status.m_bpassedw_OrderBy)) {
      status.out.println("l_cV=WtA(l_oWnd('w_OrderBy',"+SPLib.ToJSValue(status.w_OrderBy,"C",7,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_OrderBy,"C",7,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_OrderBy',l_cV);");
    if ( ! (status.m_bpassedw_daCodDip)) {
      status.out.println("l_cV=WtA(l_oWnd('w_daCodDip',"+SPLib.ToJSValue(status.w_daCodDip,"C",6,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_daCodDip,"C",6,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_daCodDip',l_cV);");
    if ( ! (status.m_bpassedw_aCodDip)) {
      status.out.println("l_cV=WtA(l_oWnd('w_aCodDip',"+SPLib.ToJSValue(status.w_aCodDip,"C",6,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_aCodDip,"C",6,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_aCodDip',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_stpfrazR Rcalled;
    Rcalled = new arrt_stpfrazR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_stpfrazR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_stpfrazR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_stpfraz"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_tipstp="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_tipstp),"?",0,0)+"");
        status.out.println("js:w_Tipop="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_Tipop),"?",0,0)+"");
        status.out.println("js:w_stato="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_stato),"?",0,0)+"");
        status.out.println("js:w_OrderBy="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_OrderBy),"?",0,0)+"");
        status.out.println("js:w_daCodDip="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_daCodDip),"?",0,0)+"");
        status.out.println("js:w_aCodDip="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_aCodDip),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_tipstp,null)) {");
        status.out.println("window[f].w_tipstp="+SPLib.ToJSValue(status.w_tipstp,"C",0,0)+";");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_Tipop,null)) {");
        status.out.println("window[f].w_Tipop="+SPLib.ToJSValue(status.w_Tipop,"C",0,0)+";");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_stato,null)) {");
        status.out.println("window[f].w_stato="+SPLib.ToJSValue(status.w_stato,"C",0,0)+";");
        status.out.println("}");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_daCodDip,null)) {");
        status.out.println("window[f].w_daCodDip="+SPLib.ToJSValue(status.w_daCodDip,"C",0,0)+";");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_aCodDip,null)) {");
        status.out.println("window[f].w_aCodDip="+SPLib.ToJSValue(status.w_aCodDip,"C",0,0)+";");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_stpfraz");
      if ( ! (CPLib.IsNull(l))) {
