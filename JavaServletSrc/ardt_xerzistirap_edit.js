...
if ( ! (m_oFather.SaveContext_ardt_xerzistirap())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_xerzistirap_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_xerzistirap_dataNotSent',{'action':formaction});
}
}
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_xerzistirap();
}
}
SaveContext.child=[];
function BlankRow() {
