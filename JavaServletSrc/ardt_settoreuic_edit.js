...
function Set_CODATECO(ctrlValue,kmode,e) {
if (Ne(w_CODATECO,ctrlValue)) {
var ctl = _GetCtl(e,'XSIJFYBHVF');
if (_tracker.goon(ctl,ctrlValue)) {
w_CODATECO=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('XSIJFYBHVF',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',3,0,'');
...
if ( ! (m_oFather.SaveContext_ardt_settoreuic())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_settoreuic_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_settoreuic_dataNotSent',{'action':formaction});
}
}
}
}
...
_FillChildren.n=[];
FillWv.n=["SETSINT","IDSETSIN"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
if (Le(m_nCurrentRow,Rows())) {
if (( ! (_ChkObl('XSIJFYBHVF',1))) && FullRow()) {
_SignErr('XSIJFYBHVF');
m_cLastWorkVarErrorMsg='11512725744';
l_bResult=false;
w_CODATECO=HtW('','C');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_settoreuic();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_CODATECO));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.CODATECO={"dec":0,"fixedpos":false,"id":"XSIJFYBHVF","len":3,"name":"CODATECO","title":"Codice ATECO (Primi Tre)","type":"C"};
