...
public class arrt_exec_sid extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*d23d1f11*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (m_bpassedw_numcicli) {
        ...
        if (source.IsForwarded()) {
          w_numcicli = source.GetCaller().GetNumber("numcicli","N",10,0);
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_numcicli) ||  ! (status.m_bpassedw_flg555) ||  ! (status.m_bpassedw_anno) ||  ! (status.m_bpassedw_fileprodotto);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_numcicli)) {
      status.out.println("l_cV=WtA(l_oWnd('w_numcicli',"+SPLib.ToJSValue(status.w_numcicli,"N",10,0)+"),'N');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_numcicli,"N",10,0)+",'N');");
    }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_exec_sidR Rcalled;
    Rcalled = new arrt_exec_sidR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_exec_sidR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_exec_sidR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_exec_sid"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_exec_sid");
      if ( ! (CPLib.IsNull(l))) {
