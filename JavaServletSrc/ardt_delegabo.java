...
public class ardt_delegabo extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*d44fda88*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "delegabo";
    }
    public String w_RAPPORTO;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_RAPPORTO,context,"delegabo","RAPPORTO","C",25,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_delegabo;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "ardt_delegabo";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 858;
    l_eg.m_nEntityHeight = 337;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"delegabo","personbo"};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("xragsoci","C",70,0, "hide","Ragione Sociale"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_delegaboBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11929722326",p_Context.Translate("Vuoi modificare lo stato dell'invio all'Agenzia delle Entrate ?")},
    ...
    {"1690021978",p_Context.Translate("Premere il bottone per modificare il flag senza modificare i dati AUI")},
  ...
  void WriteStyles(ardt_delegaboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_delegabo.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_delegaboBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(
       "<style>" +
      "");
      status.out.println(".CROCGCIGKF {top:0px;position:absolute;left:24px;height:16px;width:110px;background-color:Transparent;z-index:1; }");
      status.out.println(".CROCGCIGKF_ZOOM { position:absolute;top:0px;left:135px;height:17px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".BJKHVZBXDR {top:0px;position:absolute;left:656px;height:16px;width:72px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".BJKHVZBXDR_ZOOM { position:absolute;top:0px;left:729px;height:17px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".JCFUNSQXBI {top:0px;position:absolute;left:752px;height:16px;width:60px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".JCFUNSQXBI_ZOOM { position:absolute;top:0px;left:813px;height:17px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".VICYEWBISV {top:0px;position:absolute;left:158px;height:16px;width:490px;background-color:Transparent;z-index:1; }");
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_delegabo',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_delegabo",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_delegaboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_delegaboBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:337px;width:858px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(ardt_delegaboBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_TOP_OF_PAGE_1' style='position:relative;height:206px;display:block' page='1'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      status.out.println(
       "<div name id='GridTable_Header' class='GridTable_Header' style='position:absolute;top:-1px;left:6px;height:19px;width:852px;z-index:0'>" +
       "</div>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='RQPAWNXLEF_DIV'>" +
       "<label id='RQPAWNXLEF' for='CROCGCIGKF'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Delegato"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='MAUGJNUVWH_DIV'>" +
       "<label id='MAUGJNUVWH' for='BJKHVZBXDR'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Inizio"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle' id='DGFLOKNCET_DIV'>" +
       "<label id='DGFLOKNCET' for='JCFUNSQXBI'>" +
       ""+SPLib.ToHTMLRepresentation(status.context.Translate("Fine"))+"" +
       "</label>" +
       "</span>" +
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:absolute;top:18px;left:6px;height:186px;width:852px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:186px;width:852px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:852px;height:186px;overflow:auto;'>" +
       "<table id='GridTable' style='position:relative;top:0px;left:0px;width:846px' cellspacing='0' cellpadding='0' border='0'>" +
       "<tr>" +
       "<td></td>" +
       "</tr>" +
       "</table>" +
      ...
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:18px;left:28px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:18px;left:163px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:18px;left:660px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:18px;left:757px;height:186px;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
  ...
  void HTMLRowScript(ardt_delegaboBO BO,ServletStatus status) {
    ...
    status.out.println("s='<div style=\"position:relative;width:830px;height:17px;\"';");
    ...
    status.out.println("s=s+\""+
     ...
     "<button id='BJKHVZBXDR_ZOOM' type='button' class='CalendarButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
     "</button>" +
    "\"");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     ...
     "<button id='JCFUNSQXBI_ZOOM' type='button' class='CalendarButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
     "</button>" +
    "\"");
    status.out.println("s=s+\""+
    ...
    status.out.println("s=s+\""+
     ...
     "<button type='button' class='BJKHVZBXDR_ZOOM CalendarButtonDisabled' value="+SPLib.ToHTMLValue(status.context.Translate("..."),"C",0,0)+" disabled class='BJKHVZBXDR_ZOOM CalendarButtonDisabled'>" +
     "</button>" +
    "\"");
    status.out.println("}");
    ...
    status.out.println("s=s+\""+
     ...
     "<button type='button' class='JCFUNSQXBI_ZOOM CalendarButtonDisabled' value="+SPLib.ToHTMLValue(status.context.Translate("..."),"C",0,0)+" disabled class='JCFUNSQXBI_ZOOM CalendarButtonDisabled'>" +
     "</button>" +
    "\"");
    status.out.println("}");
    ...
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:830px;height:17px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b1(ardt_delegaboBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<div id='VRCVINYRJO' class='SectionBody' style='position:relative;height:46px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("box_VRCVINYRJO")) {
      ...
      if (status.HasZoneAltInterface("box_VRCVINYRJO@BEGIN") || status.HasZoneAltInterface("box_VRCVINYRJO@END")) {
        status.out.println(
         "<div id='VRCVINYRJO_boxContent' style='position:relative;height:46px;display:block'>" +
        "");
      }
      status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(ardt_delegaboBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("box_VRCVINYRJO")) {
      ...
      status.out.println("Ctrl('VRCVINYRJO').setAttribute('band_height',46);");
    }
  ...
  void WriteControls_p1_b2(ardt_delegaboBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_BEFORE_MINAAOAOZC")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_BEFORE_MINAAOAOZC' style='position:relative;height:3px;display:block' page='1'>" +
    "");
  ...
  void WriteControls_p1_b3(ardt_delegaboBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("box_MINAAOAOZC")) {
      ...
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<span id='VEABFZIUKQ_DIV'>" +
       "<input name='NOAGE' id='VEABFZIUKQ' type='checkbox' value="+SPLib.ToHTMLValue(BO.w_NOAGE,"C",1,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" "+(CPLib.eq("S",CPLib.Trim(BO.w_NOAGE))?"CHECKED":"")+">" +
       "<label id='VEABFZIUKQ_LBL' for='VEABFZIUKQ' class='SPLabel'>"+SPLib.ToHTMLRepresentation(status.context.Translate("Non inviare all'Agenzia delle Entrate"))+"</label>" +
       "</span>" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input id='VORELZCTGG' type='button' class='SPButton' value="+SPLib.ToHTMLValue(status.context.Translate("!"),"C",0,0)+" title="+SPLib.ToHTMLValue(status.context.Translate("Premere il bottone per modificare il flag senza modificare i dati AUI"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' style='display:none'>" +
      "");
      if (status.HasZoneAltInterface("box_MINAAOAOZC@BEGIN") || status.HasZoneAltInterface("box_MINAAOAOZC@END")) {
        status.out.println(
         "</div>" +
  ...
  void WriteControls_p1_b3_CloseBand(ardt_delegaboBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(ardt_delegaboBO BO,ServletStatus status) {
  ...
  static void PrintState(ardt_delegaboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_delegaboBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_delegaboBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_xragsoci,"C",70,0));
  ...
  static void PrintStateDetailWorkVars(ardt_delegaboBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_delegaboBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_delegaboBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_delegaboBO.TrsRow) BO.m_oDeletedRows.get(k);
        l_cTrs = l_cTrs+" new TrsRow(";
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_CODINTER,"C",16,0)+",";
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_xragsoci,"C",70,0)+",";
  ...
  void WriteMainFormVarScript(ardt_delegaboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(337,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(858,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_delegabo"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_delegabo","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_delegabo",BO.GetCurrentState());
    ...
    status.out.println("this.xragsoci=arguments[1];");
    status.out.println("this.DESCINTER=arguments[2];");
  ...
  void WriteMainFormScript(ardt_delegaboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_delegabo);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_delegabo();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_delegabo',Ne(m_nChildStatus,0));");
    } else {
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('VEABFZIUKQ')) SetDisabled(c,true);");
        ...
        status.out.println("if(c=Ctrl('BJKHVZBXDR_ZOOM')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('JCFUNSQXBI')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('JCFUNSQXBI_ZOOM')) SetDisabled(c,true);");
    ...
    status.out.println("m_oFather.m_oardt_delegabo=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_delegaboBO getBO(ServletStatus status) {
    ardt_delegaboBO BO = new ardt_delegaboBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_delegaboBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("delegabo",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_delegaboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_delegaboBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ardt_delegaboBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_delegabo;
    String l_cPhName = BO.m_cPhName_delegabo;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_delegaboBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_delegabo+".RAPPORTO"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_delegabo+".RAPPORTO"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("delegabo");
    String l_cPhName = CPSql.ManipulateTablePhName("delegabo",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_delegaboBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_delegabo),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_delegabo,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_delegabo+".RAPPORTO) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_delegabo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_delegabo,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_delegabo+".RAPPORTO",CPLib.GetTablesWithAliases(BO.m_cPhName_delegabo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_delegaboBO BO) {
  ...
  void Edit(ServletStatus status,ardt_delegaboBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_delegaboBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_delegaboBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_delegaboBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_delegaboBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_delegaboBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_delegaboBO BO,SPParameterSource source) {
    BO.Load(status.w_RAPPORTO);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_delegaboBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_delegaboBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_delegaboBO BO) {
    ...
    item.put("altInterfaceFor","ardt_delegabo");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"h\":16,\"name\":\"label_MAUGJNUVWH\",\"page\":1,\"spuid\":\"MAUGJNUVWH\",\"tabindex\":2,\"type\":\"Label\",\"value\":\"Inizio\",\"w\":72,\"x\":664,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_RQPAWNXLEF\",\"page\":1,\"spuid\":\"RQPAWNXLEF\",\"tabindex\":1,\"type\":\"Label\",\"value\":\"Delegato\",\"w\":110,\"x\":32,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_DGFLOKNCET\",\"page\":1,\"spuid\":\"DGFLOKNCET\",\"tabindex\":3,\"type\":\"Label\",\"value\":\"Fine\",\"w\":60,\"x\":760,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":182,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"KTZHPWSTJY\",\"type\":\"BodyDetail\",\"w\":868,\"x\":8,\"y\":19,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_VRCVINYRJO\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"DATI ANAGRAFICI\",\"h\":46,\"name\":\"box_VRCVINYRJO\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VRCVINYRJO\",\"stretch\":true,\"tabindex\":4,\"title_caption\":\"DATI ANAGRAFICI\",\"titled\":true,\"type\":\"Box\",\"w\":824,\"x\":16,\"y\":203,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NAKGRMOUXS\",\"page\":1,\"spuid\":\"NAKGRMOUXS\",\"tabindex\":5,\"type\":\"Label\",\"w\":648,\"x\":24,\"y\":207,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HMPQLYVATL\",\"page\":1,\"spuid\":\"HMPQLYVATL\",\"tabindex\":6,\"type\":\"Label\",\"w\":648,\"x\":24,\"y\":228,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_MINAAOAOZC\",\"box_type\":\"auto_horizontal_section\",\"collapsible\":true,\"h\":45,\"name\":\"box_MINAAOAOZC\",\"page\":1,\"shrinkable\":true,\"spuid\":\"MINAAOAOZC\",\"stretch\":true,\"tabindex\":8,\"type\":\"Box\",\"w\":824,\"x\":16,\"y\":252,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_EQUZNNIXBO\",\"page\":1,\"spuid\":\"EQUZNNIXBO\",\"tabindex\":9,\"type\":\"Label\",\"value\":\"Prog.Informazione Chiusura:\",\"w\":163,\"x\":281,\"y\":266,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BHLAQHLOJQ\",\"page\":1,\"spuid\":\"BHLAQHLOJQ\",\"tabindex\":10,\"type\":\"Label\",\"value\":\"Prog.Informazione Apertura:\",\"w\":163,\"x\":24,\"y\":266,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"fixed\":true,\"h\":18,\"name\":\"button_VORELZCTGG\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"VORELZCTGG\",\"tabindex\":24,\"type\":\"Button\",\"value\":\"!\",\"w\":16,\"x\":816,\"y\":262,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\",\"zoom\":\"usrz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"fixed\":true,\"h\":21,\"label_text\":\"Non inviare all"+"'"+"Agenzia delle Entrate\",\"maxlength\":1,\"name\":\"NOAGE\",\"page\":1,\"repeated\":true,\"spuid\":\"VEABFZIUKQ\",\"tabindex\":23,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":232,\"x\":576,\"y\":262,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"N\\u00B0 Registrazione\",\"disabled\":\"true\",\"fixed\":true,\"h\":20,\"maxlength\":11,\"name\":\"NUMPROG2\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"QCGWEEVSON\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":80,\"x\":449,\"y\":262,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"N\\u00B0 Progressivo\",\"disabled\":\"true\",\"fixed\":true,\"h\":20,\"maxlength\":11,\"name\":\"NUMPROG1\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"XXEVJVFWWX\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":80,\"x\":193,\"y\":262,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":39,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"ZZSBIWDICX\",\"type\":\"Box\",\"w\":812,\"x\":8,\"y\":-1,\"zone\":\"pag1_5\",\"zonepath\":\"[1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Soggetto collegato all"+"'"+"informazione corrente (pallino verde)\",\"h\":17,\"keep_proportions\":\"bounded\",\"name\":\"image_BKRVFLJLOS\",\"page\":1,\"repeated\":true,\"spuid\":\"BKRVFLJLOS\",\"tabindex\":25,\"type\":\"Image\",\"w\":16,\"x\":8,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Inizio\",\"h\":17,\"maxlength\":10,\"name\":\"DATAINI\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"BJKHVZBXDR\",\"tabindex\":4,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":72,\"x\":664,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Fine\",\"h\":17,\"maxlength\":10,\"name\":\"DATAFINE\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"JCFUNSQXBI\",\"tabindex\":12,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":60,\"x\":760,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"disabled\":\"true\",\"h\":17,\"maxlength\":70,\"name\":\"xRagSoc\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"VICYEWBISV\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":490,\"x\":166,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"h\":17,\"maxlength\":16,\"name\":\"CODINTER\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"CROCGCIGKF\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":110,\"x\":32,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_delegaboBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_delegaboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_delegabo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_delegabo_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_delegabo_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_xragsoci=a[17];");
      ...
      status.out.println("w_NUMPROG=a[24];");
      status.out.println("if (Gt(a.length,25)) {");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_delegabo_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(337,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void RaiseLoadedEntity(ardt_delegaboBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_delegaboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_delegaboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_delegaboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_delegaboBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_delegaboBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_delegaboBO BO,ServletStatus status) throws IOException {
  ...
  void IncludeJS(ServletStatus status) {
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+arrt_delega_noage.m_cEntityUid+"/arrt_delega_noage.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      ...
      arrt_delega_noage.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,ardt_delegaboBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ardt_delegaboBO BO) {
    boolean result = true;
