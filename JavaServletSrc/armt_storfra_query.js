...
if (Eq(formaction,'new') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('382921594'));
}
if (Eq(formaction,'edit') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('619750558'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('10935874817'));
}
}
}
var m_bIsMq = false;
...
m_oFather.DoQueryFromChild('wraziobo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
function DoQueryFrom_wlientifrz(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_wogopefbo(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_weneficfrz(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_werzistifrz(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
...
_chNotF(childAlias,'master','armt_storfra');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wraziobo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
