...
var ctl = _GetCtl(e,'CDCDVNOEKS');
if (_tracker.goon(ctl,ctrlValue)) {
w_CODVOC=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('CDCDVNOEKS',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',2,0,'');
...
var ctl = _GetCtl(e,'SBAKLFGRFP');
if (_tracker.goon(ctl,ctrlValue)) {
w_DESCRIZ=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('SBAKLFGRFP',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
parent.ZtVWeb.raiseEventToEvalParms('armt_causalis_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_causalis_dataNotSent',{'action':formaction});
}
}
}
}
...
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('CDCDVNOEKS',1))) {
_SignErr('CDCDVNOEKS');
m_cLastWorkVarErrorMsg='112851763';
l_bResult=false;
w_CODVOC=HtW('','C');
} else if ( ! (_ChkObl('SBAKLFGRFP',1))) {
_SignErr('SBAKLFGRFP');
