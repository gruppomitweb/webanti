...
// * --- Fine Area Manuale
public class ardt_werzistifrzBO extends ardt_werzistifrzBL {
  ...
  // * --- Fine Area Manuale
  public ardt_werzistifrzBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_werzistifrz");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_werzistifrz,m_cPhName_werzistifrz,m_cVirtName_werzistifrz,m_cVirtName_werzistifrz+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_werzistifrz,p_IDFILEBO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"werzistifrz")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_werzistifrz.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND CODSOG="+CPLib.ToSQL(opk_CODSOG,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_werzistifrz.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"werzistifrz")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_werzistifrz,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_werzistifrz);
      l_cSql = "insert into "+m_oWrInfo_werzistifrz.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDFILEBO,";
      l_cSql = l_cSql+"CODSOG,";
      l_cSql = l_cSql+"IDBASE,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_werzistifrz.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"werzistifrz")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_werzistifrz.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"werzistifrz")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_werzistifrz,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_werzistifrz);
      l_cSql = "Update "+m_oWrInfo_werzistifrz.GetTableWriteName()+" set";
