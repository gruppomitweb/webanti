...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('QAIJWTCCHF').value,'C'),w_PRGDIP)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_anadip_err',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'PRGDIP') &&  ! (Empty(HtW(Ctrl('QAIJWTCCHF').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.PRGDIP',HtW(Ctrl('QAIJWTCCHF').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_anadip_err');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_anadip_err',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['QAIJWTCCHF'],i=0;i>=0;i--){
