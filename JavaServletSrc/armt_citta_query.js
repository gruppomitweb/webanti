...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('FFTGLGUVGJ').value,'C'),w_IDBASE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbcitta',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDBASE') &&  ! (Empty(HtW(Ctrl('FFTGLGUVGJ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDBASE',HtW(Ctrl('FFTGLGUVGJ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_citta');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbcitta',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['FFTGLGUVGJ'],i=0;i>=0;i--){
