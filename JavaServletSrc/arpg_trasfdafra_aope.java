...
public class arpg_trasfdafra_aope extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*BLIJLWYBLK*!";
  ...
  public String getEntityTitle() {
    return "Consolidamento frazionate (Periodo Singolo)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_trasfdafra_aope";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 789;
    l_eg.m_nEntityHeight = 581;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("DaDatOpe","D",8,0, "show",""),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_trasfdafra_aopeBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11630489605",p_Context.Translate("Consolidamento frazionate (Periodo Singolo)")},
    {"125394623",p_Context.Translate("La data di fine deve essere superiore a quella di inizio e la differenza deve essere di almeno 7 giorni")},
  ...
  void WriteStyles(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_trasfdafra_aope.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(576);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='GOHLMIEYRI'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Il controllo va effettuato nei sette giorni precedenti."))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
    ...
    "");
    status.out.println(
     "<input name='DaDatOpe' id='DDNRZXUWRD' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_DaDatOpe,"D",8,0,SPLib.GetUserDatePicture(status.context.GetLanguage(),"DD-MM-YYYY"), status.m_oRegSettings)+" tabindex='-1' readonly maxlength='10' size='8' fieldtype='D' onkeydown='ManageKeys(event,false,false,true,true)' onkeypress=''>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<a href='javascript:href_BRZJBIMUYF()' tabindex='-1' id='BRZJBIMUYF_HREF'>" +
     "<img id='BRZJBIMUYF' src="+SPLib.ToHTMLValue(SPLib.ToResource("images/elaborazione.gif"),"C",0,0)+" title="+SPLib.ToHTMLValue(status.context.Translate("Trasferimento dati da Frazionate a Operazioni"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' alt="+SPLib.ToHTMLValue(status.context.Translate("Trasferimento dati da Frazionate a Operazioni"),"C",0,0)+">" +
     "</a>" +
    "");
    status.out.println(
     "<div id='KQMKKBMXYE'>" +
  ...
  static void PrintState(arpg_trasfdafra_aopeBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_trasfdafra_aopeBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_trasfdafra_aopeBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(581,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_trasfdafra_aope"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_trasfdafra_aope","");
  ...
  void WriteMainFormScript(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_trasfdafra_aopeBO getBO(ServletStatus status) {
    arpg_trasfdafra_aopeBO BO = new arpg_trasfdafra_aopeBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_trasfdafra_aopeBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_trasfdafra_aopeBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_trasfdafra_aopeBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_trasfdafra_aopeBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_trasfdafra_aopeBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_trasfdafra_aopeBO BO) {
    ...
    item.put("altInterfaceFor","arpg_trasfdafra_aope");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"576\",\"name\":\"Box_TFDCIULGCB\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"TFDCIULGCB\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"784\",\"x\":\"0\",\"y\":\"0\",\"zone\":\"pag1_1\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"ADatOpe\",\"page\":\"1\",\"spuid\":\"DPKVLWHCIG\",\"tabindex\":\"3\",\"type\":\"Textbox\",\"w\":\"80\",\"x\":\"328\",\"y\":\"36\",\"zone\":\"TFDCIULGCB_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"27\",\"name\":\"Box_ASMWKJOELK\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"ASMWKJOELK\",\"tabindex\":\"4\",\"type\":\"Box\",\"w\":\"760\",\"x\":\"8\",\"y\":\"66\",\"zone\":\"TFDCIULGCB_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stampa i record selezionati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_TLMOPHOHJY()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"TLMOPHOHJY\",\"src\":\"images\\/print.gif\",\"tabindex\":\"7\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"726\",\"y\":\"5\",\"zone\":\"TFDCIULGCB_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Numero_di_Giorni_di_Controllo_Raggruppamento_Operazioni_\",\"page\":\"1\",\"spuid\":\"SEBDBFIOVZ_DIV\",\"tabindex\":\"8\",\"type\":\"Label\",\"value\":\"Numero di Giorni di Controllo Raggruppamento Operazioni:\",\"w\":\"374\",\"x\":\"8\",\"y\":\"13\",\"zone\":\"TFDCIULGCB_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"N_B___Il_controllo_viene_effettuato_sulle_operazioni_avvenute_precedentemente_alla_data_inserita_e_aggregato_in_base_al_numero_giorni_di_controllo_scelti\",\"page\":\"1\",\"spuid\":\"UTYOLNFMZP_DIV\",\"tabindex\":\"9\",\"type\":\"Label\",\"value\":\"N.B.: Il controllo viene effettuato sulle operazioni avvenute precedentemente alla data inserita e aggregato in base al numero giorni di controllo scelti\",\"w\":\"744\",\"x\":\"16\",\"y\":\"72\",\"zone\":\"ASMWKJOELK_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_di_fine_periodo_\",\"page\":\"1\",\"spuid\":\"LEOXCMDAMB_DIV\",\"tabindex\":\"10\",\"type\":\"Label\",\"value\":\"Data di fine periodo:\",\"w\":\"98\",\"x\":\"224\",\"y\":\"40\",\"zone\":\"TFDCIULGCB_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"name\":\"Box_RVMEWEGDFJ\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"RVMEWEGDFJ\",\"tabindex\":\"11\",\"type\":\"Box\",\"w\":\"760\",\"x\":\"8\",\"y\":\"98\",\"zone\":\"TFDCIULGCB_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Il_controllo_va_effettuato_nei_sette_giorni_precedenti_\",\"page\":\"1\",\"spuid\":\"GOHLMIEYRI_DIV\",\"tabindex\":\"12\",\"type\":\"Label\",\"value\":\"Il controllo va effettuato nei sette giorni precedenti.\",\"w\":\"744\",\"x\":\"16\",\"y\":\"105\",\"zone\":\"RVMEWEGDFJ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"41\",\"name\":\"Box_ANTONSWNXP\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"ANTONSWNXP\",\"tabindex\":\"13\",\"type\":\"Box\",\"w\":\"760\",\"x\":\"8\",\"y\":\"130\",\"zone\":\"TFDCIULGCB_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"29\",\"href\":\" \",\"name\":\"Le_frazionate_che_non_sono_hanno_i_requisiti_di_consolidamento__che_non_vengono_trasferite___ad_eccezione_per_quelle_con_parte_di_importo_in_contante___vengono_eliminate_\",\"page\":\"1\",\"spuid\":\"EGLUZXVOLX_DIV\",\"tabindex\":\"14\",\"type\":\"Label\",\"value\":\"Le frazionate che non sono hanno i requisiti di consolidamento (che non vengono trasferite), ad eccezione per quelle con parte di importo in contante, \\nvengono eliminate.\",\"w\":\"744\",\"x\":\"16\",\"y\":\"135\",\"zone\":\"ANTONSWNXP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_di_inizio_periodo_\",\"page\":\"1\",\"spuid\":\"FAFOJNJJXI_DIV\",\"tabindex\":\"15\",\"type\":\"Label\",\"value\":\"Data di inizio periodo:\",\"w\":\"103\",\"x\":\"8\",\"y\":\"40\",\"zone\":\"TFDCIULGCB_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"21\",\"name\":\"flgdel\",\"page\":\"1\",\"spuid\":\"FIAJYROYOB\",\"tabindex\":\"16\",\"type\":\"Checkbox\",\"w\":\"236\",\"x\":\"528\",\"y\":\"36\",\"zone\":\"TFDCIULGCB_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"8\",\"y\":\"326\",\"zone\":\"TFDCIULGCB_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"DaDatOpe\",\"page\":\"1\",\"spuid\":\"DDNRZXUWRD\",\"type\":\"Textbox\",\"w\":\"80\",\"x\":\"112\",\"y\":\"36\",\"zone\":\"TFDCIULGCB_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Trasferimento dati da Frazionate a Operazioni\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_BRZJBIMUYF()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"BRZJBIMUYF\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"19\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"679\",\"y\":\"5\",\"zone\":\"TFDCIULGCB_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"147\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"8\",\"y\":\"173\",\"zone\":\"TFDCIULGCB_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"153\",\"href\":\" \",\"name\":\"__iframe_width__750__height__400__id__elaborazione___name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"TYIWXFLLFY_DIV\",\"tabindex\":\"22\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"750"+"'"+" height="+"'"+"400"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"768\",\"x\":\"8\",\"y\":\"416\",\"zone\":\"TFDCIULGCB_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
  ...
  void Next(ServletStatus status,arpg_trasfdafra_aopeBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_trasfdafra_aopeBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_trasfdafra_aope?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_trasfdafra_aope_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    l_cJsUid = SPPrxycizer.proxycizedPath("arpg_msg_frazoper.js");
    status.out.println(
     "<script src='../"+l_cJsUid+"' type='text/javascript'>" +
     "</script>" +
    "");
    ...
    status.themeWriter.WriteBody_Begin(581,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
  ...
  void RaiseLoadedEntity(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_trasfdafra_aopeBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_trasfdafra_aopeBO BO,ServletStatus status) {
  ...
  void IncludeJS(ServletStatus status) {
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+arfn_dateminus.m_cEntityUid+"/arfn_dateminus.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arfn_dateminus.GetCallableNames(callable);
      arrt_trasfdafra_aoped_stp.GetCallableNames(callable);
      arrt_trasfdafra_aoped.GetCallableNames(callable);
      CPLib.AddToSet(callable,"arpg_msg_frazoper");
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_trasfdafra_aopeBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_trasfdafra_aopeBO BO) {
    boolean result = true;
    return result;
  }
}
