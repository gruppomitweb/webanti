...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('JYSWMXVXPB').value,'C'),w_CODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbcandistr',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODICE') &&  ! (Empty(HtW(Ctrl('JYSWMXVXPB').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODICE',HtW(Ctrl('JYSWMXVXPB').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tbcandistr');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbcandistr',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['JYSWMXVXPB'],i=0;i>=0;i--){
