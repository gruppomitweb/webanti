...
public class arrt_ae_delinfo_agg_agg extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*18e19bc0*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_ae_delinfo_agg_agg";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_RAPPORTO) ||  ! (status.m_bpassedw_TIPOOPRAP) ||  ! (status.m_bpassedw_NUMPROG);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_ae_delinfo_agg_aggR Rcalled;
    Rcalled = new arrt_ae_delinfo_agg_aggR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_ae_delinfo_agg_aggR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_ae_delinfo_agg_aggR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_ae_delinfo_agg_agg"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_ae_delinfo_agg_agg");
      if ( ! (CPLib.IsNull(l))) {
