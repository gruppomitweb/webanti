...
public class arfn_catchfile extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*af9e5e94*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arfn_catchfileR Rcalled;
    Rcalled = new arfn_catchfileR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaa_retval = Rcalled.Run(status.pID,status.pFile,status.pTipo,status.pCodDip);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arfn_catchfileR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arfn_catchfileR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arfn_catchfile"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
