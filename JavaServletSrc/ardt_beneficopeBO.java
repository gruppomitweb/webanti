...
// * --- Fine Area Manuale
public class ardt_beneficopeBO extends ardt_beneficopeBL {
  ...
  // * --- Fine Area Manuale
  public ardt_beneficopeBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_beneficope");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_beneficope+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_beneficope,m_cPhName_beneficope,m_cVirtName_beneficope,m_cVirtName_beneficope+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_beneficope,p_IDFILEBO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"beneficope")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_beneficope.GetTableWriteName()+" where ";
        ...
        l_cSql = l_cSql+m_oWrInfo_beneficope.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"beneficope")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_beneficope,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_beneficope);
      l_cSql = "insert into "+m_oWrInfo_beneficope.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_beneficope.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"beneficope")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_beneficope.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"beneficope")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_beneficope,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_beneficope);
      l_cSql = "Update "+m_oWrInfo_beneficope.GetTableWriteName()+" set";
