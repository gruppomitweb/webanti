...
public class arpg_jntestbo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*SBZNJRJFNN*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.m_cQueryName = "qbe_statorap_ae";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_jntestboBO BO) {
  ...
  void WriteStyles(arpg_jntestboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_jntestbo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_jntestboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_jntestboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_jntestboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_jntestboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_jntestboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_jntestboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_jntestboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_jntestboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_jntestbo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_jntestbo","");
  ...
  void WriteMainFormScript(arpg_jntestboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_jntestboBO getBO(ServletStatus status) {
    arpg_jntestboBO BO = new arpg_jntestboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_jntestboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_jntestboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_jntestboBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_jntestboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_jntestboBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_jntestboBO BO) {
    ...
    item.put("altInterfaceFor","arpg_jntestbo");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_jntestboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_jntestboBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_jntestboBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_jntestboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_jntestboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_jntestboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_jntestbo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_jntestbo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_jntestboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_jntestboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_jntestboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_jntestboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_jntestboBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_jntestboBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_jntestboBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_jntestboBO BO) {
    boolean result = true;
    return result;
  }
}
