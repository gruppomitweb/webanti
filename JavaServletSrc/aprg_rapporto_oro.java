...
@com.zucchetti.sitepainter.EntityRoles({"agenzia"})
public class aprg_rapporto_oro extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*EEYUDAPMBJ*!";
  static public class ServletStatus extends SPPageServletStatus {
    public void MergeCallerVars(SPParameterSource source) {
      ...
      source.SetParameter("risultato",source.GetCaller().GetString("risultato","C",40,0));
    }
  }
  ...
  public String getEntityTitle() {
    return "Apertura/Chiusura Rapporto Oro";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "aprg_rapporto_oro";
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("risultato","C",40,0, "hide","")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,aprg_rapporto_oroBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"11001061275",p_Context.Translate("Apertura/Chiusura Rapporto Oro")},
    {"11271491468",p_Context.Translate("Confermi l'uscita ?")},
    {"11512327745",p_Context.Translate("Salva i dati appena immessi")},
    {"11978929877",p_Context.Translate("Inserire Chiusura al 31/12")},
    {"1799462310",p_Context.Translate("Chiudi la finestra senza salvare i dati")},
    {"829201110",p_Context.Translate("Data Operazione")},
    {"99923805",p_Context.Translate("Confermi la creazione dei dati del rapporto?")}};
    return l_translations;
  }
  void WriteStyles(aprg_rapporto_oroBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/aprg_rapporto_oro.css'>" +
      "");
    }
  ...
  void WriteMainForm(aprg_rapporto_oroBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(aprg_rapporto_oroBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(aprg_rapporto_oroBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(171);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(aprg_rapporto_oroBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<span id='IHCYXKNNTS_DIV'>" +
     "<input name='FLGCLOSE' id='IHCYXKNNTS' type='checkbox' value="+SPLib.ToHTMLValue(BO.w_FLGCLOSE,"C",1,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq("S",CPLib.Trim(BO.w_FLGCLOSE))?"CHECKED":"")+">" +
     "<label id='IHCYXKNNTS_LBL' for='IHCYXKNNTS' class='SPLabel'>"+SPLib.ToHTMLRepresentation(status.context.Translate("Inserire Chiusura al 31/12"))+"</label>" +
     "</span>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
  ...
  static void PrintState(aprg_rapporto_oroBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(aprg_rapporto_oroBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(aprg_rapporto_oroBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_risultato,"C",40,0));
    status.out.println(")");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      status.out.println("m_bUpdated="+SPLib.ToJSValue(BO.m_bUpdated,"L",0,0)+";");
    }
  }
  void WriteMainFormVarScript(aprg_rapporto_oroBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"aprg_rapporto_oro"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"aprg_rapporto_oro","");
  ...
  void WriteMainFormScript(aprg_rapporto_oroBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- aprg_rapporto_oro");
  ...
  aprg_rapporto_oroBO getBO(ServletStatus status) {
    aprg_rapporto_oroBO BO = new aprg_rapporto_oroBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(aprg_rapporto_oroBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,aprg_rapporto_oroBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,aprg_rapporto_oroBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,aprg_rapporto_oroBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,aprg_rapporto_oroBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,aprg_rapporto_oroBO BO) {
    ...
    item.put("altInterfaceFor","aprg_rapporto_oro");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"162\",\"name\":\"Box_IDIZVJSJFR\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"IDIZVJSJFR\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"280\",\"x\":\"8\",\"y\":\"9\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Chiudi la finestra senza salvare i dati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"29\",\"href\":\"javascript:href_ZQEKBHAOQV()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"ZQEKBHAOQV\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"2\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"240\",\"y\":\"135\",\"zone\":\"IDIZVJSJFR_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\"#0000FF\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"APERTURA_CHIUSURA_RAPPORTO\",\"page\":\"1\",\"spuid\":\"HWULSRGDNM_DIV\",\"tabindex\":\"3\",\"type\":\"Label\",\"value\":\"APERTURA\\/CHIUSURA RAPPORTO\",\"w\":\"280\",\"x\":\"8\",\"y\":\"9\",\"zone\":\"IDIZVJSJFR_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Data Operazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"dataope\",\"page\":\"1\",\"spuid\":\"PYGHSMNTBK\",\"tabindex\":\"4\",\"type\":\"Textbox\",\"w\":\"80\",\"x\":\"128\",\"y\":\"36\",\"zone\":\"IDIZVJSJFR_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Apertura_\",\"page\":\"1\",\"spuid\":\"BAATYILHGP_DIV\",\"tabindex\":\"5\",\"type\":\"Label\",\"value\":\"Data Apertura:\",\"w\":\"93\",\"x\":\"35\",\"y\":\"40\",\"zone\":\"IDIZVJSJFR_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Salva i dati appena immessi\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"29\",\"href\":\"javascript:href_GJGIOZNGOM()\",\"name\":\"images_save_gif\",\"page\":\"1\",\"spuid\":\"GJGIOZNGOM\",\"src\":\"images\\/save.gif\",\"tabindex\":\"6\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"192\",\"y\":\"135\",\"zone\":\"IDIZVJSJFR_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"14\",\"href\":\" \",\"name\":\"__iframe_width__655__height__170__id__elaborazione___name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"FGMOCXNBFY_DIV\",\"tabindex\":\"7\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"655"+"'"+" height="+"'"+"170"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"88\",\"x\":\"24\",\"y\":\"144\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"21\",\"name\":\"FLGCLOSE\",\"page\":\"1\",\"spuid\":\"IHCYXKNNTS\",\"tabindex\":\"8\",\"type\":\"Checkbox\",\"w\":\"184\",\"x\":\"88\",\"y\":\"72\",\"zone\":\"IDIZVJSJFR_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"CONNES\",\"page\":\"1\",\"spuid\":\"TJVNAGXDHC\",\"tabindex\":\"9\",\"type\":\"Textbox\",\"w\":\"56\",\"x\":\"16\",\"y\":\"72\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_risultato\",\"page\":\"1\",\"spuid\":\"ECTSKPAITW_DIV\",\"tabindex\":\"11\",\"type\":\"Label\",\"value\":\"w_risultato\",\"w\":\"264\",\"x\":\"16\",\"y\":\"108\",\"zone\":\"IDIZVJSJFR_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,aprg_rapporto_oroBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,aprg_rapporto_oroBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(aprg_rapporto_oroBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='aprg_rapporto_oro?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/aprg_rapporto_oro_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(aprg_rapporto_oroBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(aprg_rapporto_oroBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(aprg_rapporto_oroBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(aprg_rapporto_oroBO BO,ServletStatus status) {
  ...
  void WriteExitPage(aprg_rapporto_oroBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(aprg_rapporto_oroBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,aprg_rapporto_oroBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,aprg_rapporto_oroBO BO) {
    boolean result = true;
    return result;
  }
}
