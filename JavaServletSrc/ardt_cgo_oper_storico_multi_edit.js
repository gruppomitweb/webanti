...
w_tipogio=Utilities.Make(window).GetCallerString('TIPOGIOCO');
l_bTmpRes=Link_FHBFFVNZIS(null);
...
function Link_FHBFFVNZIS(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
var l_bEmp = Empty(w_tipogio);
var l_Appl = LinkApplet();
l_Appl.SetStringKey('CODTPGIO',w_tipogio,2,0);
l_Appl.SetFields('CODTPGIO,IMPMINGIO');
l_Appl.SetTypes('C,N');
l_Appl.LinkTable('cgo_tbtipgio');
l_Appl.SetID('FHBFFVNZIS');
l_nRec=l_Appl.DoLink((Eq(kmode,null)?0:kmode));
l_bResult=Ne(l_nRec,0);
if (Eq(l_nRec,1)) {
w_tipogio=l_Appl.GetStringValue('CODTPGIO',2,0);
...
function Link_FHBFFVNZIS_Blank() {
w_tipogio='';
w_impmin=0;
}
function LOpt_FHBFFVNZIS() {
...
l_oWv.setValue('UID',WtA('FHBFFVNZIS','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['FHBFFVNZIS'][1],'C'));
return l_oWv;
}
...
if ( ! (m_oFather.SaveContext_ardt_cgo_oper_storico_multi())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_oper_storico_multi_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_oper_storico_multi_dataNotSent',{'action':formaction});
}
}
}
}
...
l_oWv.setValue('impmin',WtA(w_impmin,'N'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
_FillChildren.n=[];
FillWv.n=["SNAINUMOPE","TOTALE","CGOTIPOPE","tipogio","impmin"];
function CheckRow(w) {
var l_bResult = true;
ResetErrorVars();
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_cgo_oper_storico_multi();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.TOTLIRE={"dec":2,"fixedpos":false,"id":"OGHOMBFTXJ","len":12,"name":"TOTLIRE","title":"Importo","type":"N"};
