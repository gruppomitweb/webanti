...
// * --- Fine Area Manuale
public class arrt_calccodfis_wuR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_calccodfis_wuR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_calccodfis_wu",m_Caller);
    ...
    try {
      ...
      if (CPLib.eqr(nocf,0)) {
        ...
        if ( ! (CPLib.Empty(CPLib.LRTrim(w_NASSTATO))) && (CPLib.ne(CPLib.LRTrim(w_NASSTATO),"ITALIA") && CPLib.ne(CPLib.LRTrim(w_NASSTATO),"ITALY"))) {
          ...
          m_cSql = "";
          m_cSql = m_cSql+"DESCRI="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.Upper(CPLib.LRTrim(w_NASSTATO)),"C",60,0,m_cServer),m_cServer,CPLib.Upper(CPLib.LRTrim(w_NASSTATO)));
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on tbluonas into routine arrt_calccodfis_wu returns two or more records. This item should return only a record.");
          ...
          m_cPhName = m_Ctx.GetPhName("tbluonas");
          m_cSql = "";
          m_cSql = m_cSql+"DESCRI="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.Upper(CPLib.LRTrim(w_NASCOMUN)),"C",60,0,m_cServer),m_cServer,CPLib.Upper(CPLib.LRTrim(w_NASCOMUN)));
          if (m_Ctx.IsSharedTemp("tbluonas")) {
            m_cSql = m_cSql+" and context_id='"+m_Ctx.GetTempID()+"'";
          }
          if (Read_Cursor!=null)
  ...
  void Page_3() throws Exception {
    /* Calcolo cognome */
    /* cognome := arfn_agechar(w_COGNOME,'').replaceAll("['`]","") */
    cognome = arfn_agecharR.Make(m_Ctx,this).Run(w_COGNOME,"").replaceAll("['`]","");
  ...
  void Page_4() throws Exception {
    /* Calcolo cognome */
    /* nome := arfn_agechar(w_NOME,'').replaceAll("['`]","") */
    nome = arfn_agecharR.Make(m_Ctx,this).Run(w_NOME,"").replaceAll("['`]","");
    /* Consonanti */
    /* c1 := 1 // contatore 1 */
    c1 = CPLib.Round(1,0);
    /* c2 := 0 // contatore 1 */
  ...
  public static arrt_calccodfis_wuR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_calccodfis_wuR(p_Ctx, p_Caller);
  }
  public void Blank() {
