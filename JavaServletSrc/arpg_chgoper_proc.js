...
l_oWv.setValue('m_cParameterSequence',AsAppletValue('w_aCodCli,w_ADatOpe,w_daCodCli,w_DaDatOpe'));
if (false || WindowConfirm(Translate('1905722088'))) {
SetLocationHref(null,'arrt_chgoper'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cMode=hyperlink','elaborazione')
}
}
...
w_Messaggi.raiseEvent('arpg_chgoper_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_chgoper_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_chgoper_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_chgoper',m_cSelectedPage);
...
_FillChildren.n=[];
FillWv.n=["DaDatOpe","ADatOpe","daCodCli","aCodCli","RAGSOC1","RAGSOC2"];
