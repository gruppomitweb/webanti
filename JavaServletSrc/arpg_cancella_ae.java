...
public class arpg_cancella_ae extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*OEDTFVPPVC*!";
  ...
  public String getEntityTitle() {
    return "Cancellazione Massiva Rapporti AGE";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_cancella_ae";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_cancella_aeBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10545141903",p_Context.Translate("Confermi la cancellazione delle informazioni dell'Anagrafe dei Rapporti ?")},
    {"2114741767",p_Context.Translate("Cancellazione Massiva Rapporti AGE")}};
    return l_translations;
  }
  void WriteStyles(arpg_cancella_aeBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_cancella_ae.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_cancella_aeBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_cancella_aeBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_cancella_aeBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_cancella_aeBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_cancella_aeBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(322);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_cancella_aeBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='QDZKSWCHXH'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Data di cancellazione:"))+"" +
     "</label>" +
     "</span>" +
    "");
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input id='XIFZDMHWTB' type='button' class='SPButton' value="+SPLib.ToHTMLValue(status.context.Translate("Cancellazione"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' onkeydown='ManageKeys(event,false,false,true)' onclick='XIFZDMHWTB_Click()'>" +
    "");
    status.out.println(
     "<div id='DYMVKIQWCA'>" +
    "");
  ...
  static void PrintState(arpg_cancella_aeBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_cancella_aeBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_cancella_aeBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_cancella_aeBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_cancella_ae"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_cancella_ae","");
  ...
  void WriteMainFormScript(arpg_cancella_aeBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_cancella_ae");
  ...
  arpg_cancella_aeBO getBO(ServletStatus status) {
    arpg_cancella_aeBO BO = new arpg_cancella_aeBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_cancella_aeBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_cancella_aeBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_cancella_aeBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_cancella_aeBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_cancella_aeBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_cancella_aeBO BO) {
    ...
    item.put("altInterfaceFor","arpg_cancella_ae");
    items.put(item);
    String s;
    ...
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"14\",\"href\":\" \",\"name\":\"__iframe_width__655__height__170__id__elaborazione___name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"FGMOCXNBFY_DIV\",\"tabindex\":\"2\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"655"+"'"+" height="+"'"+"170"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"88\",\"x\":\"16\",\"y\":\"35\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_di_cancellazione_\",\"page\":\"1\",\"spuid\":\"QDZKSWCHXH_DIV\",\"tabindex\":\"3\",\"type\":\"Label\",\"value\":\"Data di cancellazione:\",\"w\":\"114\",\"x\":\"18\",\"y\":\"18\",\"zone\":\"YWGFSYCMOZ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"datchi\",\"page\":\"1\",\"spuid\":\"NPEVPSEORB\",\"tabindex\":\"4\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"136\",\"y\":\"14\",\"zone\":\"YWGFSYCMOZ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"25\",\"href\":\"javascript:XIFZDMHWTB_Click()\",\"name\":\"Cancellazione\",\"page\":\"1\",\"spuid\":\"XIFZDMHWTB\",\"tabindex\":\"5\",\"type\":\"Button\",\"value\":\"Cancellazione\",\"w\":\"128\",\"x\":\"648\",\"y\":\"14\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void Next(ServletStatus status,arpg_cancella_aeBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_cancella_aeBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_cancella_aeBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_cancella_ae?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_cancella_ae_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_cancella_aeBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_cancella_aeBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_cancella_aeBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_cancella_aeBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_cancella_aeBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_cancella_aeBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_cancella_ae.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_cancella_aeBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_cancella_aeBO BO) {
    boolean result = true;
    return result;
  }
}
