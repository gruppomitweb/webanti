...
var c;
var tmp_subtitle = ToHTag(AsControlValue('Numero Operazione: '+w_SNAINUMOPE));
...
SPBOUILib.SetInputValue('RUNOCZKNEF',function(){return WtH(w_CODWALLET,'C',20,0,'')},w_CODWALLET);
...
SPBOUILib.SetInputValue('NTWMYVQDWZ',function(){return WtH(w_PROFESSIONE,'C',40,0,'')},w_PROFESSIONE);
...
SetDisplay(Ctrl('NTWMYVQDWZ'),Eq(w_xCGOFLGPROF,'N') || IsHiddenByStateDriver('PROFESSIONE'));
SetDisplay(Ctrl('FLTWHVLGEG'),Eq(w_xCGOFLGPROF,'N'));
...
HideUI.lblids["CAIDBIGLIETTO"]=['RKDCRUGGJP'];
...
HideUI.lblids["CODWALLET"]=['HMWSYZMWAI'];
...
HideUI.lblids["PROFESSIONE"]=['FLTWHVLGEG'];
...
function RUNOCZKNEF_Valid(e) {
SetActiveField(Ctrl('RUNOCZKNEF'),false);
if (Eq(m_cFunction,'query')) {
DoQuery('CODWALLET','C',w_CODWALLET,HtW(Ctrl('RUNOCZKNEF').value,'C'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_CODWALLET(HtW(Ctrl('RUNOCZKNEF').value,'C'),null,e);
return l_bSetResult;
}
}
function RUNOCZKNEF_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('RUNOCZKNEF'))) {
DisplayErrorMessage();
}
SetActiveField(Ctrl('RUNOCZKNEF'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('RUNOCZKNEF'),e);
}
...
function NTWMYVQDWZ_Valid(e) {
SetActiveField(Ctrl('NTWMYVQDWZ'),false);
if (Eq(m_cFunction,'query')) {
DoQuery('PROFESSIONE','C',w_PROFESSIONE,HtW(Ctrl('NTWMYVQDWZ').value,'C'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_PROFESSIONE(HtW(Ctrl('NTWMYVQDWZ').value,'C'),null,e);
return l_bSetResult;
}
}
function NTWMYVQDWZ_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('NTWMYVQDWZ'))) {
DisplayErrorMessage();
}
SetActiveField(Ctrl('NTWMYVQDWZ'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('NTWMYVQDWZ'),e);
}
...
linkpc_url.u('AAHJPBZVGL',function(){return 'ardt_cgo_oper_multi_dc?m_bInnerChild=true'+GetLinkPCKeys('ardt_cgo_oper_multi_dc')+'&m_bRowsChild='+EnableChild('ardt_cgo_oper_multi_dc')})
_ResetTracker();
...
function RUNOCZKNEF_wrap_Valid(event) {
return RUNOCZKNEF_Valid(event);
}
SPBOUILib.SetInputExit('RUNOCZKNEF',RUNOCZKNEF_wrap_Valid);
function RUNOCZKNEF_wrap_OnFocus(event) {
return RUNOCZKNEF_OnFocus(event);
}
SPBOUILib.SetInputEnter('RUNOCZKNEF',RUNOCZKNEF_wrap_OnFocus);
function RUNOCZKNEF_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('RUNOCZKNEF',RUNOCZKNEF_wrap_OnKeyDown);
...
function NTWMYVQDWZ_wrap_Valid(event) {
return NTWMYVQDWZ_Valid(event);
}
SPBOUILib.SetInputExit('NTWMYVQDWZ',NTWMYVQDWZ_wrap_Valid);
function NTWMYVQDWZ_wrap_OnFocus(event) {
return NTWMYVQDWZ_OnFocus(event);
}
SPBOUILib.SetInputEnter('NTWMYVQDWZ',NTWMYVQDWZ_wrap_OnFocus);
function NTWMYVQDWZ_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('NTWMYVQDWZ',NTWMYVQDWZ_wrap_OnKeyDown);
...
w_PrtDocAll.raiseEvent('armt_cgo_oper_multi_dc_'+Strtran(p_cEvent,' ','_'));
}
}
...
FocusFirstComponent.edit={"page_1":["LVYFMQXNZH","PURPSXEVAI"]};
FocusFirstComponent.query={"page_1":["XFUOZLSACP","LVYFMQXNZH","BMTLGIUQTH","PURPSXEVAI"]};
FocusFirstComponent.otherwise={"page_1":["LVYFMQXNZH","PURPSXEVAI"]};
function Help() {
windowOpenForeground('../doc/armt_cgo_oper_multi_dc_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
function SetModified_ardt_cgo_oper_multi_dc() {
SetModified();
}
...
w_ANA_FOUND=a[143];
op_codazi=a[144];
m_cWv_ardt_cgo_oper_multi_dc=a[145];
if (Gt(a.length,146)) {
m_cErrorInChild='';
}
if (Gt(a.length,146)) {
...
return 'cgo_operazioni_dl_multi_dc'+'\\'+CPLib.ToCPStr(w_SNAINUMOPE);
}
GetLinkPCKeys.set_o('ardt_cgo_oper_multi_dc',false);
GetLinkPCKeys.o['ardt_cgo_oper_multi_dc']['SNAINUMOPE']=function(t){
if (Ne(m_cFunction,'new')) {
return t(w_SNAINUMOPE,'C');
}
}
resizeChildIframe.children['ardt_cgo_oper_multi_dc']=true;
...
LoadContext.f('AAHJPBZVGL',function(){return FrameRef('ardt_cgo_oper_multi_dc')},1,'ardt_cgo_oper_multi_dc',1,1)
GetSelectedChildPage.child={'ardt_cgo_oper_multi_dc': function(){return FrameRef('ardt_cgo_oper_multi_dc')}}
function GetSelectedPage(p_d) {
p_d.setValue('armt_cgo_oper_multi_dc',m_cSelectedPage);
...
_Obli('CODWALLET',0,'RUNOCZKNEF',true,"92934247",null,null,function() { return Eq(w_FLAGWALLET,'1') })
...
_Obli('PROFESSIONE',0,'NTWMYVQDWZ',true,"178012863",null,null,function() { return Eq(w_xCGOFLGPROF,'S') })
