...
function Set_TRTIPREG(ctrlValue,kmode,e) {
if (Ne(w_TRTIPREG,ctrlValue)) {
var ctl = _GetCtl(e,'KUVQXAZDHJ');
if (_tracker.goon(ctl,ctrlValue)) {
w_TRTIPREG=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('KUVQXAZDHJ',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',1,0,'');
...
function Set_TRDESREG(ctrlValue,kmode,e) {
if (Ne(w_TRDESREG,ctrlValue)) {
var ctl = _GetCtl(e,'JGDIWWMKFV');
if (_tracker.goon(ctl,ctrlValue)) {
w_TRDESREG=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('JGDIWWMKFV',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
if ( ! (m_oFather.SaveContext_ardt_cgo_tgtiporeg())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_tgtiporeg_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_tgtiporeg_dataNotSent',{'action':formaction});
}
}
}
}
...
l_oWv.setValue('TRCODGIO',WtA(w_TRCODGIO,'C'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
if (( ! (_ChkObl('KUVQXAZDHJ',1))) && FullRow()) {
_SignErr('KUVQXAZDHJ');
m_cLastWorkVarErrorMsg='995100987';
l_bResult=false;
w_TRTIPREG=HtW('','C');
} else if (( ! (_ChkObl('JGDIWWMKFV',1))) && FullRow()) {
_SignErr('JGDIWWMKFV');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_cgo_tgtiporeg();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_TRTIPREG));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.TRTIPREG={"dec":0,"fixedpos":false,"id":"KUVQXAZDHJ","len":1,"name":"TRTIPREG","title":"Tipo Registrazione","type":"C"};
l_aRepeatedFields.TRDESREG={"dec":0,"fixedpos":false,"id":"JGDIWWMKFV","len":30,"name":"TRDESREG","title":"Descrizione","type":"C"};
