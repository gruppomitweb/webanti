...
import org.json.*;
// * --- Fine Area Manuale
public class arfn_exec_sid_smR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_exec_sid_smR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_exec_sid_sm",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on cpwarn into routine arfn_exec_sid_sm returns two or more records. This item should return only a record.");
      ...
      if (CPLib.Empty(CPLib.LRTrim(_semaforo))) {
        ...
        m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid_sm",343,"0000002E")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
        ...
        File di = new File(CPLib.LRTrim(gPathDoc)+"/SID/MASSCF/ESI/"+CPLib.LRTrim(gAzienda)+"/");
        ...
        di = new File(CPLib.LRTrim(gPathDoc)+"/SID/MASSCF/PRE/"+CPLib.LRTrim(gAzienda)+"/");
        ...
        if (CPLib.eqr(_tipo,"W")) {
          /* _dir1 := Strtran(LRTrim(gPathDoc),'/','\')+"\SID\MASSCF\DAP\"+LRTrim(gAzienda) */
          _dir1 = CPLib.Strtran(CPLib.LRTrim(gPathDoc),"/","\\")+"\\SID\\MASSCF\\DAP\\"+CPLib.LRTrim(gAzienda);
          /* _dir2 := Strtran(LRTrim(gPathDoc),'/','\')+"\SID\MASSCF\ESI\"+LRTrim(gAzienda) */
          _dir2 = CPLib.Strtran(CPLib.LRTrim(gPathDoc),"/","\\")+"\\SID\\MASSCF\\ESI\\"+CPLib.LRTrim(gAzienda);
          /* _dir3 := Strtran(LRTrim(gPathDoc),'/','\')+"\SID\MASSCF\PRE\"+LRTrim(gAzienda) */
          _dir3 = CPLib.Strtran(CPLib.LRTrim(gPathDoc),"/","\\")+"\\SID\\MASSCF\\PRE\\"+CPLib.LRTrim(gAzienda);
        ...
        di = new File(CPLib.LRTrim(gPathDoc)+"/SID/MASSCF/ESI/"+CPLib.LRTrim(gAzienda)+"/");
        ...
        /* fhand := FileLibMit.OpenReadPath(LRTrim(gPathDoc)+"/SID/MASSCF/ESI/"+LRTrim(gAzienda)+"/"+LRTrim(_fileesi)) // Handle File */
        fhand = FileLibMit.OpenReadPath(CPLib.LRTrim(gPathDoc)+"/SID/MASSCF/ESI/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(_fileesi));
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on tberrorisid into routine arfn_exec_sid_sm returns two or more records. This item should return only a record.");
            CPStdCounter.Error(l_oErrorFault);
          }
          Read_Cursor.Close();
        } else {
        ...
        if (CPLib.eqr(_risultato,"000")) {
          ...
          m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
          m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid_sm",343,"0000006B")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          /* cNomeFileZip := gPathDoc+"/flussi/"+LRTrim(gAzienda)+"/"+LRTrim(pFile)+".zip" */
          cNomeFileZip = gPathDoc+"/flussi/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(pFile)+".zip";
          ...
          mcRitorno.AppendBlank();
          /* mcRitorno.testata := 'File: '+ LRTrim(pFile)+" elaborato correttamente" */
          mcRitorno.row.testata = "File: "+CPLib.LRTrim(pFile)+" elaborato correttamente";
          /* mcRitorno.SaveRow() */
          mcRitorno.SaveRow();
          ...
          /* mcRitorno.AppendBlank() */
          mcRitorno.AppendBlank();
          /* mcRitorno.testata := 'ESITO:'+_risultato + " - "+LRTrim(pFile) */
          mcRitorno.row.testata = "ESITO:"+_risultato+" - "+CPLib.LRTrim(pFile);
          /* mcRitorno.SaveRow() */
          mcRitorno.SaveRow();
          /* fhand := FileLibMit.OpenReadPath(LRTrim(gPathDoc)+"/SID/MASSCF/ESI/"+LRTrim(gAzienda)+"/"+LRTrim(_filedgn)) // Handle File */
          fhand = FileLibMit.OpenReadPath(CPLib.LRTrim(gPathDoc)+"/SID/MASSCF/ESI/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(_filedgn));
          /* While not(FileLibMit.Eof(fhand)) */
          while ( ! (FileLibMit.Eof(fhand))) {
            /* _riga := FileLibMit.ReadLine(fhand) */
            _riga = FileLibMit.ReadLine(fhand);
            ...
            if (CPLib.gt(CPLib.Len(CPLib.LRTrim(_riga)),0) && CPLib.eqr(CPLib.Left(_riga,1),"B")) {
              ...
              /* mcRitorno.AppendBlank() */
              mcRitorno.AppendBlank();
              /* mcRitorno.testata := 'Tipo Riga: '+Substr(_riga,5,1)+' - N. Record: '+Substr(_riga,6,9) +' - Campo:'+Substr(_riga,30,2) + ' - Errore: '+_risultato */
              mcRitorno.row.testata = "Tipo Riga: "+CPLib.Substr(_riga,5,1)+" - N. Record: "+CPLib.Substr(_riga,6,9)+" - Campo:"+CPLib.Substr(_riga,30,2)+" - Errore: "+_risultato;
              /* mcRitorno.SaveRow() */
              mcRitorno.SaveRow();
            } // End If
          } // End While
        ...
        /* CriptoLibrary.AEScryptFile(LRTrim(gPathDoc)+'/SID/MASSCF/DAP/'+LRTrim(gAzienda)+"/"+LRTrim(pFile),LRTrim(gPathDoc)+'/SID/MASSCF/STO/'+LRTrim(gAzienda)+"/"+LRTrim(pFile)+".crypted",_pwdsog) */
        CriptoLibrary.AEScryptFile(CPLib.LRTrim(gPathDoc)+"/SID/MASSCF/DAP/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(pFile),CPLib.LRTrim(gPathDoc)+"/SID/MASSCF/STO/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(pFile)+".crypted",_pwdsog);
        /* FileLibMit.DeleteFilePath(LRTrim(gPathDoc)+'/SID/MASSCF/DAP/'+LRTrim(gAzienda)+"/"+LRTrim(pFile)) */
        FileLibMit.DeleteFilePath(CPLib.LRTrim(gPathDoc)+"/SID/MASSCF/DAP/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(pFile));
        // * --- Delete from cpwarn
        m_Sql.RequireTransaction();
        m_cServer = m_Ctx.GetServer("cpwarn");
        m_cPhName = m_Ctx.GetPhName("cpwarn");
        m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"cpwarn",m_Ctx.GetCompany());
        m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
        m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid_sm",343,"00000082")+"delete from "+m_oWrInfo.GetTableWriteName();
        m_cSql = m_cSql+" where ";
  ...
  void Page_2() throws Exception {
    ...
    try
    {
      File _mydir = new File(CPLib.LRTrim(_dirsid)+"/prog");
        String[] cmd = {CPLib.LRTrim(_dirsid)+"/prog/runPredisposizioneMIT.bat",CPLib.LRTrim(_dir1),"CF100",CPLib.Upper(_pwdsog)};
  ...
  void Page_3() throws Exception {
    ...
    {
      File _mydir = new File(CPLib.LRTrim(_dirsid)+"/prog");
      String[] cmd = {CPLib.LRTrim(_dirsid)+"/prog/runPredisposizioneMIT.sh",CPLib.LRTrim(_dirjava)+"/bin/java","CF100",CPLib.LRTrim(CPLib.Upper(_pwdsog))};
      Runtime rt = Runtime.getRuntime();
      Process proc = rt.exec(cmd,null,_mydir);
      String line="";
      java.io.BufferedReader is = new BufferedReader(new InputStreamReader(proc.getInputStream( )));
      while ((line = is.readLine(  )) != null)
    /* gMsgProc := gMsgProc + line + NL // Messaggio */
    gMsgProc = gMsgProc+line+"\n";
    m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
    } catch (IOException ex) { 
      ex.printStackTrace();   
    }
  ...
  public MemoryCursor_mcmessaggi_mcrdef Run(String p_pFile) {
    pFile = p_pFile;
    return Run();
  }
  ...
  public static arfn_exec_sid_smR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_exec_sid_smR(p_Ctx, p_Caller);
  }
  public static arfn_exec_sid_smR Make(CPContext p_Ctx) {
    return new arfn_exec_sid_smR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_exec_sid_sm,
  public static final String i_InvokedRoutines = ",arfn_exec_sid_sm,";
