...
// * --- Fine Area Manuale
public class arrt_lancia_stpesito_sempliceR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_lancia_stpesito_sempliceR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_lancia_stpesito_semplice",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_lancia_stpesito_semplice";
  ...
  void Page_1() throws Exception {
    /* _nrep Numeric(3, 0) // Numero di report */
    ...
    /* _nomerep := 'arrp_stpesito_'+Left(DateTimeToChar(DateTime()),12) // Nome Stampa */
    _nomerep = "arrp_stpesito_"+CPLib.Left(CPLib.DateTimeToChar(CPLib.DateTime()),12);
    /* If _report.makeReport('arrp_stpesito_semplice',LRTrim(gPathApp)+"/stampe_off/"+LRTrim(_nomerep),'A4','PORTRAIT','PDF') */
    if (_report.makeReport("arrp_stpesito_semplice",CPLib.LRTrim(gPathApp)+"/stampe_off/"+CPLib.LRTrim(_nomerep),"A4","PORTRAIT","PDF")) {
      ...
      m_cSql = CPLib.GetSQLPrefixComment("arrt_lancia_stpesito_semplice",8,"0000000D")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
      m_cSql = m_cSql+GetFieldsName_0000000D(m_Ctx,m_oWrInfo);
      m_cSql = m_cSql+") values (";
      m_cSql = m_cSql+""+CPLib.ToSQL("Stampa semplice","?",0,0,m_cServer)+", ";
  ...
  public static arrt_lancia_stpesito_sempliceR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_lancia_stpesito_sempliceR(p_Ctx, p_Caller);
  }
  public void Blank() {
    _nrep = 0;
