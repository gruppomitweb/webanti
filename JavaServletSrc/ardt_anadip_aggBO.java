...
// * --- Fine Area Manuale
public class ardt_anadip_aggBO extends ardt_anadip_aggBL {
  ...
  // * --- Fine Area Manuale
  public ardt_anadip_aggBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_anadip_agg");
    }
  ...
  public static String[] m_cLoadParameterNames = {"CODDIPE"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_CODDIPE) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_anadip_agg,m_cPhName_anadip_agg,m_cVirtName_anadip_agg,m_cVirtName_anadip_agg+".CODDIPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODDIPE,"C",6,0),m_cServer_anadip_agg,p_CODDIPE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_ANNORIF = l_rsLoadBody.GetDouble("ANNORIF");
          w_MESERIF = l_rsLoadBody.GetDouble("MESERIF");
          w_DATAINI = l_rsLoadBody.GetDate("DATAINI");
          w_DATAFINE = l_rsLoadBody.GetDate("DATAFINE");
          w_ANOMAGE = l_rsLoadBody.GetDouble("ANOMAGE");
          w_SOSCLIENTI = l_rsLoadBody.GetDouble("SOSCLIENTI");
          w_SOSAGENZIA = l_rsLoadBody.GetDouble("SOSAGENZIA");
          // Assegna il contesto dei figli del dettaglio a Empty
          work_children = CPLib.BlankChildMNTs(m_MNTs,true);
          opk_ANNORIF = w_ANNORIF;
          opk_MESERIF = w_MESERIF;
          if (CPLib.HasTableCPCCCHK(m_Ctx,"anadip_agg")) {
  ...
  public boolean Delete(String p_CODDIPE) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_anadip_agg.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"CODDIPE="+CPLib.ToSQL(w_CODDIPE,"C",6,0)+" AND ANNORIF="+CPLib.ToSQL(opk_ANNORIF,"N",4,0)+" AND MESERIF="+CPLib.ToSQL(opk_MESERIF,"N",2,0);
        l_cSql = l_cSql+m_oWrInfo_anadip_agg.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"anadip_agg")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_anadip_agg,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_anadip_agg);
      l_cSql = "insert into "+m_oWrInfo_anadip_agg.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODDIPE,";
      l_cSql = l_cSql+"ANNORIF,";
      l_cSql = l_cSql+"MESERIF,";
      l_cSql = l_cSql+"DATAINI,";
      l_cSql = l_cSql+"DATAFINE,";
      l_cSql = l_cSql+"ANOMAGE,";
      l_cSql = l_cSql+"SOSCLIENTI,";
      l_cSql = l_cSql+"SOSAGENZIA,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_anadip_agg.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"anadip_agg")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_ANNORIF,"N",4,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_MESERIF,"N",2,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DATAINI,"D",8,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DATAFINE,"D",8,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_ANOMAGE,"N",3,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_SOSCLIENTI,"N",3,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_SOSAGENZIA,"N",3,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_anadip_agg.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"anadip_agg")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_anadip_agg,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_anadip_agg);
      l_cSql = "Update "+m_oWrInfo_anadip_agg.GetTableWriteName()+" set";
      l_cSql = l_cSql+" DATAINI = "+CPLib.ToSQL(w_DATAINI,"D",8,0);
      l_cSql = l_cSql+", DATAFINE = "+CPLib.ToSQL(w_DATAFINE,"D",8,0);
      l_cSql = l_cSql+", ANOMAGE = "+CPLib.ToSQL(w_ANOMAGE,"N",3,0);
      l_cSql = l_cSql+", SOSCLIENTI = "+CPLib.ToSQL(w_SOSCLIENTI,"N",3,0);
      l_cSql = l_cSql+", SOSAGENZIA = "+CPLib.ToSQL(w_SOSAGENZIA,"N",3,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"anadip_agg")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", ANNORIF = "+CPLib.ToSQL(w_ANNORIF,"N",4,0);
      l_cSql = l_cSql+", MESERIF = "+CPLib.ToSQL(w_MESERIF,"N",2,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODDIPE="+CPLib.ToSQL(w_CODDIPE,"C",6,0)+" AND ANNORIF="+CPLib.ToSQL(opk_ANNORIF,"N",4,0)+" AND MESERIF="+CPLib.ToSQL(opk_MESERIF,"N",2,0);
      l_cSql = l_cSql+m_oWrInfo_anadip_agg.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"anadip_agg")) {
