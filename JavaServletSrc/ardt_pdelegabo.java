...
public class ardt_pdelegabo extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*62c86cba*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "xelegabo";
    }
    public String w_RAPPORTO;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_RAPPORTO,context,"xelegabo","RAPPORTO","C",25,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_xelegabo;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "ardt_pdelegabo";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 768;
    l_eg.m_nEntityHeight = 324;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"xelegabo","personbo"};
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","","page"),new ZoneItem("box_VRCVINYRJO","DATI ANAGRAFICI","horizontal_section"),new ZoneItem("box_MINAAOAOZC","","")};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("xragsocd","C",70,0, "hide","Ragione Sociale"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_pdelegaboBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"10200886678",p_Context.Translate("Non Inviare Agenzia Entrate")},
  ...
  void WriteStyles(ardt_pdelegaboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_pdelegabo.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_pdelegaboBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(".BJKHVZBXDR {top:0px;position:absolute;left:600px;height:16px;width:72px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".JCFUNSQXBI {top:0px;position:absolute;left:680px;height:16px;width:60px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".VICYEWBISV {top:0px;position:absolute;left:165px;height:16px;width:423px;background-color:Transparent;z-index:1; }");
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_pdelegabo',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_pdelegabo",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_pdelegaboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_pdelegaboBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:324px;width:768px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(ardt_pdelegaboBO BO,ServletStatus status) {
    ...
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:absolute;top:18px;left:6px;height:186px;width:762px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:186px;width:762px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:762px;height:186px;overflow:auto;'>" +
       "<table id='GridTable' style='position:relative;top:0px;left:0px;width:756px' cellspacing='0' cellpadding='0' border='0'>" +
       "<tr>" +
       "<td></td>" +
       "</tr>" +
       "</table>" +
      ...
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:18px;left:166px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:18px;left:602px;height:186px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:18px;left:684px;height:186px;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
  ...
  void HTMLRowScript(ardt_pdelegaboBO BO,ServletStatus status) {
    ...
    status.out.println("s='<div style=\"position:relative;width:740px;height:17px;\"';");
    ...
    status.out.println("var w_xragsocd;");
    ...
    status.out.println("w_xragsocd=row[prefix+'xragsocd'];");
    ...
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:740px;height:17px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b1(ardt_pdelegaboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(ardt_pdelegaboBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(ardt_pdelegaboBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_END_OF_PAGE_1")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_END_OF_PAGE_1' style='position:relative;height:52px;display:block' page='1'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_END_OF_PAGE_1")) {
      ...
      "");
      status.out.println(
       "<input name='NUMPROG1' id='SZEBCEZTJC' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_NUMPROG1,"C",11,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='11' size='11' fieldtype='C'>" +
      "");
      status.out.println(
       "<input name='NUMPROG2' id='UEQWFDVELM' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_NUMPROG2,"C",11,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='11' size='11' fieldtype='C'>" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<span id='YFNPTNWMJX_DIV'>" +
       "<input name='NOAGE' id='YFNPTNWMJX' type='checkbox' value="+SPLib.ToHTMLValue(BO.w_NOAGE,"C",1,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" "+(CPLib.eq("S",CPLib.Trim(BO.w_NOAGE))?"CHECKED":"")+">" +
       "<label id='YFNPTNWMJX_LBL' for='YFNPTNWMJX' class='SPLabel'>"+SPLib.ToHTMLRepresentation(status.context.Translate("Non Inviare Agenzia Entrate"))+"</label>" +
       "</span>" +
      "");
      status.out.println(
       "</div>" +
  ...
  static void PrintState(ardt_pdelegaboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_pdelegaboBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_pdelegaboBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_xragsocd,"C",70,0));
  ...
  static void PrintStateDetailWorkVars(ardt_pdelegaboBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_pdelegaboBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_pdelegaboBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_pdelegaboBO.TrsRow) BO.m_oDeletedRows.get(k);
        l_cTrs = l_cTrs+" new TrsRow(";
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_CODINTER,"C",16,0)+",";
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_xragsocd,"C",70,0)+",";
  ...
  void WriteMainFormVarScript(ardt_pdelegaboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(324,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(768,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_pdelegabo"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_pdelegabo","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_pdelegabo",BO.GetCurrentState());
    ...
    status.out.println("this.xragsocd=w_xragsocd;");
    ...
    status.out.println("this.xragsocd=arguments[1];");
    status.out.println("this.DATAINI=arguments[2];");
    status.out.println("this.TIPORAP=arguments[3];");
    status.out.println("this.TIPOPERS=arguments[4];");
    status.out.println("this.STATOREG=arguments[5];");
    status.out.println("this.OPERAZMOD=arguments[6];");
    status.out.println("this.DATARETT=arguments[7];");
    status.out.println("this.IDEREG=arguments[8];");
    status.out.println("this.COLLEGAMEN=arguments[9];");
    status.out.println("this.DATAFINE=arguments[10];");
    status.out.println("this.IDBASE=arguments[11];");
    status.out.println("this.UCODE=arguments[12];");
    status.out.println("this.tiporegr=arguments[13];");
    status.out.println("this.DOMICILIO=arguments[14];");
    status.out.println("this.DESCCIT=arguments[15];");
    status.out.println("this.PROVINCIA=arguments[16];");
    status.out.println("this.CAP=arguments[17];");
    status.out.println("this.NUMPROG1=arguments[18];");
    status.out.println("this.NUMPROG2=arguments[19];");
    status.out.println("this.xRagSoc=arguments[20];");
    status.out.println("this.NOAGE=arguments[21];");
    status.out.println("this.CPROWNUM=arguments[22];");
    status.out.println("this.m_nRowStatus=arguments[23];");
    status.out.println("this.m_cOldRowCPCCCHK=arguments[24];");
    int lastRow;
    lastRow = 25;
    for ( l = 0; CPLib.lt(l,BO.m_MNTs.length); l = l + (1)) {
      if (BO.m_MNTs[l].RepeatedChild()) {
    ...
    status.out.println("w_xragsocd=row.xragsocd;");
  ...
  void WriteMainFormScript(ardt_pdelegaboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_pdelegabo);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_pdelegabo();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_pdelegabo',Ne(m_nChildStatus,0));");
    } else {
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ...
      if (CPLib.eq(status.m_cClientMode,"view")) {
        ...
        status.out.println("if(c=Ctrl('YFNPTNWMJX')) SetDisabled(c,true);");
        ...
        status.out.println("if(c=Ctrl('SZEBCEZTJC')) SetDisabled(c,true);");
        status.out.println("if(c=Ctrl('UEQWFDVELM')) SetDisabled(c,true);");
    ...
    status.out.println("m_oFather.m_oardt_pdelegabo=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_pdelegaboBO getBO(ServletStatus status) {
    ardt_pdelegaboBO BO = new ardt_pdelegaboBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_pdelegaboBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("xelegabo",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_pdelegaboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_pdelegaboBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ardt_pdelegaboBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_xelegabo;
    String l_cPhName = BO.m_cPhName_xelegabo;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_pdelegaboBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_xelegabo+".RAPPORTO"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_xelegabo+".RAPPORTO"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("xelegabo");
    String l_cPhName = CPSql.ManipulateTablePhName("xelegabo",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_pdelegaboBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_xelegabo),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_xelegabo,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_xelegabo+".RAPPORTO) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_xelegabo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_xelegabo,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_xelegabo+".RAPPORTO",CPLib.GetTablesWithAliases(BO.m_cPhName_xelegabo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_pdelegaboBO BO) {
  ...
  void Edit(ServletStatus status,ardt_pdelegaboBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_pdelegaboBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_pdelegaboBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_pdelegaboBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_pdelegaboBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_pdelegaboBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_pdelegaboBO BO,SPParameterSource source) {
    BO.Load(status.w_RAPPORTO);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_pdelegaboBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_pdelegaboBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_pdelegaboBO BO) {
    ...
    item.put("altInterfaceFor","ardt_pdelegabo");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"h\":16,\"name\":\"label_MAUGJNUVWH\",\"page\":1,\"spuid\":\"MAUGJNUVWH\",\"tabindex\":2,\"type\":\"Label\",\"value\":\"Inizio\",\"w\":43,\"x\":616,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_RQPAWNXLEF\",\"page\":1,\"spuid\":\"RQPAWNXLEF\",\"tabindex\":1,\"type\":\"Label\",\"value\":\"Delegato\",\"w\":43,\"x\":32,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_DGFLOKNCET\",\"page\":1,\"spuid\":\"DGFLOKNCET\",\"tabindex\":3,\"type\":\"Label\",\"value\":\"Fine\",\"w\":43,\"x\":704,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":182,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"XSTHJDEUAR\",\"type\":\"BodyDetail\",\"w\":796,\"x\":8,\"y\":19,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_VRCVINYRJO\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"DATI ANAGRAFICI\",\"h\":49,\"name\":\"box_VRCVINYRJO\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VRCVINYRJO\",\"stretch\":true,\"tabindex\":4,\"title_caption\":\"DATI ANAGRAFICI\",\"titled\":true,\"type\":\"Box\",\"w\":736,\"x\":16,\"y\":203,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NAKGRMOUXS\",\"page\":1,\"spuid\":\"NAKGRMOUXS\",\"tabindex\":5,\"type\":\"Label\",\"w\":648,\"x\":24,\"y\":210,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HMPQLYVATL\",\"page\":1,\"spuid\":\"HMPQLYVATL\",\"tabindex\":6,\"type\":\"Label\",\"w\":648,\"x\":24,\"y\":231,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"box_MINAAOAOZC\",\"h\":45,\"name\":\"box_MINAAOAOZC\",\"page\":1,\"shrinkable\":true,\"spuid\":\"MINAAOAOZC\",\"tabindex\":8,\"type\":\"Box\",\"w\":736,\"x\":16,\"y\":259,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_EQUZNNIXBO\",\"page\":1,\"spuid\":\"EQUZNNIXBO\",\"tabindex\":9,\"type\":\"Label\",\"value\":\"Prog.Informazione Chiusura:\",\"w\":163,\"x\":288,\"y\":277,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"fixed\":true,\"h\":21,\"label_text\":\"Non Inviare Agenzia Entrate\",\"maxlength\":1,\"name\":\"NOAGE\",\"page\":1,\"repeated\":true,\"spuid\":\"YFNPTNWMJX\",\"tabindex\":22,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":168,\"x\":576,\"y\":273,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"N\\u00B0 Registrazione\",\"disabled\":\"true\",\"fixed\":true,\"h\":20,\"maxlength\":11,\"name\":\"NUMPROG2\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"UEQWFDVELM\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":80,\"x\":456,\"y\":273,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"N\\u00B0 Progressivo\",\"disabled\":\"true\",\"fixed\":true,\"h\":20,\"maxlength\":11,\"name\":\"NUMPROG1\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"SZEBCEZTJC\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":80,\"x\":200,\"y\":273,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BHLAQHLOJQ\",\"page\":1,\"spuid\":\"BHLAQHLOJQ\",\"tabindex\":10,\"type\":\"Label\",\"value\":\"Prog.Informazione Apertura:\",\"w\":147,\"x\":48,\"y\":277,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":39,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"YLLGELXQWG\",\"type\":\"Box\",\"w\":740,\"x\":8,\"y\":-1,\"zone\":\"pag1_5\",\"zonepath\":\"[1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"disabled\":\"true\",\"h\":17,\"maxlength\":70,\"name\":\"xRagSoc\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"VICYEWBISV\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":423,\"x\":173,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Inizio\",\"h\":17,\"maxlength\":10,\"name\":\"DATAINI\",\"page\":1,\"repeated\":true,\"spuid\":\"BJKHVZBXDR\",\"tabindex\":3,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":72,\"x\":608,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Fine\",\"h\":17,\"maxlength\":10,\"name\":\"DATAFINE\",\"page\":1,\"repeated\":true,\"spuid\":\"JCFUNSQXBI\",\"tabindex\":11,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":60,\"x\":688,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Soggetto collegato all"+"'"+"informazione corrente (pallino verde)\",\"h\":17,\"keep_proportions\":\"bounded\",\"name\":\"image_BKRVFLJLOS\",\"page\":1,\"repeated\":true,\"spuid\":\"BKRVFLJLOS\",\"tabindex\":24,\"type\":\"Image\",\"w\":16,\"x\":8,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_pdelegaboBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_pdelegaboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_pdelegabo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_pdelegabo_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_pdelegabo_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_STATOREG=a[4];");
      status.out.println("w_OPERAZMOD=a[5];");
      status.out.println("w_DATARETT=a[6];");
      status.out.println("w_IDEREG=a[7];");
      status.out.println("w_COLLEGAMEN=a[8];");
      status.out.println("w_DATAFINE=a[9];");
      status.out.println("w_IDBASE=a[10];");
      status.out.println("w_UCODE=a[11];");
      status.out.println("w_RAPPORTO=a[12];");
      status.out.println("w_NUMPROG1=a[13];");
      status.out.println("w_NUMPROG2=a[14];");
      status.out.println("w_NOAGE=a[15];");
      status.out.println("w_DESCINTER=a[16];");
      status.out.println("w_xragsocd=a[17];");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_pdelegabo_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(324,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void RaiseLoadedEntity(ardt_pdelegaboBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_pdelegaboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_pdelegaboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_pdelegaboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_pdelegaboBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_pdelegaboBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_pdelegaboBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ardt_pdelegaboBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ardt_pdelegaboBO BO) {
    boolean result = true;
