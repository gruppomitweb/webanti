...
public class arrt_trasfdafra_aoped_multiplo extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*b1fcaf6f*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_numgio;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_numgio = source.Has("w_numgio");
      if (m_bpassedw_numgio) {
        w_numgio = source.GetParameter("w_numgio",source.GetParameter("numgio",0));
      } else {
        if (source.IsForwarded()) {
          w_numgio = source.GetCaller().GetNumber("numgio","N",2,0);
          m_bpassedw_numgio = true;
        } else {
          w_numgio = source.GetParameter("numgio",source.GetParameter("w_numgio",0));
        }
      }
    }
  }
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_trasfdafra_aoped_multiplo";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_numgio) ||  ! (status.m_bpassedw_flgdel) ||  ! (status.m_bpassedw_DaDatOpe) ||  ! (status.m_bpassedw_ADatOpe);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_numgio)) {
      status.out.println("l_cV=WtA(l_oWnd('w_numgio',"+SPLib.ToJSValue(status.w_numgio,"N",2,0)+"),'N');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_numgio,"N",2,0)+",'N');");
    }
    status.out.println("l_oWv.setValue('w_numgio',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_trasfdafra_aoped_multiploR Rcalled;
    Rcalled = new arrt_trasfdafra_aoped_multiploR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_trasfdafra_aoped_multiploR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_trasfdafra_aoped_multiploR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_trasfdafra_aoped_multiplo"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_numgio="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_numgio),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_numgio,null)) {");
        status.out.println("window[f].w_numgio="+SPLib.ToJSValue(status.w_numgio,"N",0,0)+";");
        status.out.println("}");
        if (status.m_bError) {
          l_cErrMsg = status.context.Translate(com.zucchetti.sitepainter.Library.explicativeErrorMessage(status.m_cErrMsg));
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_trasfdafra_aoped_multiplo");
      if ( ! (CPLib.IsNull(l))) {
