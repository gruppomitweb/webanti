...
// * --- Fine Area Manuale
public class ardt_cgo_infbacheca_docBO extends ardt_cgo_infbacheca_docBL {
  ...
  // * --- Fine Area Manuale
  public ardt_cgo_infbacheca_docBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_cgo_infbacheca_doc");
    }
  ...
  public static String[] m_cLoadParameterNames = {"IBDCODICE"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(double p_IBDCODICE) {
    CPResultSet l_rsLoadBody=null;
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_cgo_infbacheca_doc+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cgo_infbacheca_doc,m_cPhName_cgo_infbacheca_doc,m_cVirtName_cgo_infbacheca_doc,m_cVirtName_cgo_infbacheca_doc+".IBDCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IBDCODICE,"N",10,0),m_cServer_cgo_infbacheca_doc,p_IBDCODICE),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_IDBNOMFILE = l_rsLoadBody.GetString("IDBNOMFILE");
          w_IBDDESFILE = l_rsLoadBody.GetString("IBDDESFILE");
  ...
  public boolean Delete(double p_IBDCODICE) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cgo_infbacheca_doc.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IBDCODICE="+CPLib.ToSQL(w_IBDCODICE,"N",10,0)+" AND CPROWNUM="+CPLib.ToSQL(w_CPROWNUM,"N",4,0);
        l_cSql = l_cSql+m_oWrInfo_cgo_infbacheca_doc.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_doc")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_infbacheca_doc,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_infbacheca_doc);
      l_cSql = "insert into "+m_oWrInfo_cgo_infbacheca_doc.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IBDCODICE,";
      l_cSql = l_cSql+"IDBNOMFILE,";
      l_cSql = l_cSql+"IBDDESFILE,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_infbacheca_doc.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_doc")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_IBDCODICE,"N",10,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IDBNOMFILE,"C",80,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IBDDESFILE,"C",70,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_CPROWNUM,"N",4,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cgo_infbacheca_doc.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_doc")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_infbacheca_doc,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_infbacheca_doc);
      l_cSql = "Update "+m_oWrInfo_cgo_infbacheca_doc.GetTableWriteName()+" set";
      l_cSql = l_cSql+" IDBNOMFILE = "+CPLib.ToSQL(w_IDBNOMFILE,"C",80,0);
      l_cSql = l_cSql+", IBDDESFILE = "+CPLib.ToSQL(w_IBDDESFILE,"C",70,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_infbacheca_doc")) {
