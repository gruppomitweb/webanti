...
// * --- Fine Area Manuale
public class armt_cittaBO extends armt_cittaBL {
  class SqlLoader {
    ...
    public void AddJoinedLink_1_11(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+3,m_cServer,rightServer)) {
        ...
        m_cSelect = m_cSelect+" ,link_1_11.SIGLA AS SIGLA111";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_1_11","PKTBSTATI","CODSTA");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_1_11","PKTBSTATI","CODSTA",true);
  ...
  // * --- Fine Area Manuale
  public armt_cittaBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_citta");
    }
  ...
  public boolean Load(String p_IDBASE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbcitta,m_cPhName_tbcitta,m_cVirtName_tbcitta,m_cVirtName_tbcitta+".IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDBASE,"C",10,0),m_cServer_tbcitta,p_IDBASE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (l_oSqlLoader.m_bLink_1_11) {
          w_dessta = l_rsLoad.GetString("DESCRI111");
          w_SIGLA = l_rsLoad.GetString("SIGLA111");
        } else {
          Link_ILAKLAWXVC("Load");
        }
        w_SIGLA = l_rsLoad.GetString("SIGLA");
        w_PREF = l_rsLoad.GetString("PREF");
        w_IDISTAT = l_rsLoad.GetString("IDISTAT");
        w_IDREGIO = l_rsLoad.GetString("IDREGIO");
        // Assegna i valori degli op_
        SPExtenderLib.LoadExtendedFields(l_rsLoad,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcitta")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbcitta);
      l_cSql = "insert into "+m_oWrInfo_tbcitta.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+"SIGLA,";
      l_cSql = l_cSql+"PREF,";
      l_cSql = l_cSql+"IDISTAT,";
      l_cSql = l_cSql+"IDREGIO,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbcitta.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcitta")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_CITTA,"C",60,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_CAB,"C",6,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_PROV,"C",2,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_CAP,"C",9,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_PKTBSTATI,"C",3,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_SIGLA,"C",4,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_PREF,"C",4,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IDISTAT,"C",6,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IDREGIO,"C",2,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbcitta.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcitta")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbcitta,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbcitta);
      l_cSql = "Update "+m_oWrInfo_tbcitta.GetTableWriteName()+" set";
      l_cSql = l_cSql+" CITTA = "+CPLib.ToSQL(w_CITTA,"C",60,0);
      ...
      l_cSql = l_cSql+", PKTBSTATI = "+CPLib.ToSQLNull(w_PKTBSTATI,"C",3,0);
      l_cSql = l_cSql+", SIGLA = "+CPLib.ToSQL(w_SIGLA,"C",4,0);
      l_cSql = l_cSql+", PREF = "+CPLib.ToSQL(w_PREF,"C",4,0);
      l_cSql = l_cSql+", IDISTAT = "+CPLib.ToSQL(w_IDISTAT,"C",6,0);
      l_cSql = l_cSql+", IDREGIO = "+CPLib.ToSQL(w_IDREGIO,"C",2,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcitta")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_IDBASE,"C",10,0),m_cServer_tbcitta,w_IDBASE);
      l_cSql = l_cSql+m_oWrInfo_tbcitta.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcitta")) {
  ...
  public boolean Delete(String p_IDBASE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbcitta.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_IDBASE,"C",10,0),m_cServer_tbcitta,w_IDBASE);
          l_cSql = l_cSql+m_oWrInfo_tbcitta.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcitta")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbcitta,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
  ...
  void SaveAutonumber() {
    ...
    m_Sql.NextTableProg(this,"PRCITTA","w_IDBASE",m_cServer_tbcitta,m_Ctx.GetCompany());
  ...
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"ILAKLAWXVC")) {
      ...
      result.m_rdField = new String[]{"CODSTA" , "DESCRI" , "SIGLA"};
      result.m_rdTypes = new String[]{"C" , "C" , "C"};
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("ILAKLAWXVC",CPLib.GetLinkedFieldProps("CODSTA","PKTBSTATI","C",3,0));
    }
    return result;
  }
