...
// * --- Fine Area Manuale
public class arrt_creaxls_decodecomuniR implements CallerWithObjs {
  ...
  public int conta;
  // * --- Fine Area Manuale
  public arrt_creaxls_decodecomuniR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_creaxls_decodecomuni",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_creaxls_decodecomuni";
  ...
  void Page_1() throws Exception {
    CPResultSet Cursor_qbe_decodecomuni_empty=null;
    ...
    try {
      ...
      /* nomeesito := 'DecodeComuni_'+DateTimeToChar(DateTime())+".xls" */
      nomeesito = "DecodeComuni_"+CPLib.DateTimeToChar(CPLib.DateTime())+".xls";
      ...
      Label stringa = new Label(0,0,"ID COMUNE",arial11format);
      foglio.addCell(stringa); 
      stringa = new Label(1,0,"LOCALITA'",arial11format);
      foglio.addCell(stringa);
      stringa = new Label(2,0,"Comune UIF",arial11format);
      foglio.addCell(stringa);
      stringa = new Label(3,0,"Provincia UIF",arial11format);
      foglio.addCell(stringa);
      stringa = new Label(4,0,"Terminale",arial11format);
      foglio.addCell(stringa);
      /* leggo la tabella ordinata x esito */
      // * --- Select from qbe_decodecomuni_empty
      if (Cursor_qbe_decodecomuni_empty!=null)
        Cursor_qbe_decodecomuni_empty.Close();
      Cursor_qbe_decodecomuni_empty = new VQRHolder("qbe_decodecomuni_empty",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),this,true).GetResultSet(m_Ctx);
      while ( ! (Cursor_qbe_decodecomuni_empty.Eof())) {
        /* _id := LRTrim(Str(qbe_decodecomuni_empty->ID,10,0)) */
        _id = CPLib.LRTrim(CPLib.Str(Cursor_qbe_decodecomuni_empty.GetDouble("ID"),10,0));
        /* _comune := LRTrim(qbe_decodecomuni_empty->CITTA) */
        _comune = CPLib.LRTrim(Cursor_qbe_decodecomuni_empty.GetString("CITTA"));
        // Scrive le celle excel
        stringa = new Label(0,riga,_id);
        foglio.addCell(stringa); 
        stringa = new Label(1,riga,_comune);
        foglio.addCell(stringa); 
        stringa = new Label(2,riga,CPLib.Space(40));
        foglio.addCell(stringa);
        stringa = new Label(3,riga,CPLib.Space(2));
        foglio.addCell(stringa);
        stringa = new Label(4,riga,"XXX");
        foglio.addCell(stringa); 
        riga = riga +1;
        Cursor_qbe_decodecomuni_empty.Next();
      }
      m_cConnectivityError = Cursor_qbe_decodecomuni_empty.ErrorMessage();
      Cursor_qbe_decodecomuni_empty.Close();
      // * --- End Select
      //Aggiusta le colonne
  ...
  public static arrt_creaxls_decodecomuniR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_creaxls_decodecomuniR(p_Ctx, p_Caller);
  }
  public void Blank() {
    _conta = 0;
    _id = CPLib.Space(10);
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_decodecomuni_empty,
  public static final String m_cVQRList = ",qbe_decodecomuni_empty,";
  // ENTITY_BATCHES: ,
