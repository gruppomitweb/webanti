...
// * --- Fine Area Manuale
public class ardt_storinformazioniBO extends ardt_storinformazioniBL {
  ...
  // * --- Fine Area Manuale
  public ardt_storinformazioniBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_storinformazioni");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_wntestbo+".CPROWNUM";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_wntestbo,m_cPhName_wntestbo,m_cVirtName_wntestbo,m_cVirtName_wntestbo+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_wntestbo,p_RAPPORTO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"wntestbo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_wntestbo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_wntestbo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"wntestbo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_wntestbo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_wntestbo);
      l_cSql = "insert into "+m_oWrInfo_wntestbo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_wntestbo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wntestbo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_wntestbo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wntestbo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_wntestbo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_wntestbo);
      l_cSql = "Update "+m_oWrInfo_wntestbo.GetTableWriteName()+" set";
