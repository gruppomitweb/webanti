...
public class opert_imp_dati extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*28d76da*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_tipoform","w_tipoform");
      }        
      w_tipoform = source.GetParameter("w_tipoform","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_PAGMEZ","w_PAGMEZ");
      }        
      w_PAGMEZ = source.GetParameter("w_PAGMEZ","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.pScheduler","pScheduler");
      }        
      pScheduler = source.GetParameter("pScheduler","");
    }
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    opert_imp_datiR Rcalled;
    Rcalled = new opert_imp_datiR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaaaa_retval = Rcalled.Run(status.w_nomefile,status.w_tipodest,status.w_tipofile,status.w_tipoform,status.w_PAGMEZ,status.pScheduler);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new opert_imp_datiR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,opert_imp_datiR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","opert_imp_dati"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
