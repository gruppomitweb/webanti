...
// * --- Fine Area Manuale
public class ardt_xrz_contanteBO extends ardt_xrz_contanteBL {
  ...
  // * --- Fine Area Manuale
  public ardt_xrz_contanteBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_xrz_contante");
    }
  ...
  public boolean Load(String p_IDOPE) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xrz_contante,m_cPhName_xrz_contante,m_cVirtName_xrz_contante,m_cVirtName_xrz_contante+".IDOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDOPE,"C",10,0),m_cServer_xrz_contante,p_IDOPE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xrz_contante")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_xrz_contante.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDOPE="+CPLib.ToSQL(w_IDOPE,"C",10,0)+" AND TAGLIO="+CPLib.ToSQL(opk_TAGLIO,"N",7,2);
        l_cSql = l_cSql+m_oWrInfo_xrz_contante.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xrz_contante")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_xrz_contante,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xrz_contante);
      l_cSql = "insert into "+m_oWrInfo_xrz_contante.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDOPE,";
      ...
      l_cSql = l_cSql+m_oWrInfo_xrz_contante.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xrz_contante")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_xrz_contante.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xrz_contante")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xrz_contante,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xrz_contante);
      l_cSql = "Update "+m_oWrInfo_xrz_contante.GetTableWriteName()+" set";
