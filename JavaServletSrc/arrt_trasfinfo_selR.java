...
// * --- Fine Area Manuale
public class arrt_trasfinfo_selR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_trasfinfo_selR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_trasfinfo_sel",m_Caller);
  ...
  void Page_1() throws Exception {
    CPResultSet Cursor_qbe_trasfinfo=null;
    ...
    try {
      ...
      // * --- Select from qbe_trasfinfo
      if (Cursor_qbe_trasfinfo!=null)
        Cursor_qbe_trasfinfo.Close();
      Cursor_qbe_trasfinfo = new VQRHolder("qbe_trasfinfo",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),this,true).GetResultSet(m_Ctx);
      while ( ! (Cursor_qbe_trasfinfo.Eof())) {
        ...
        m_cSql = CPLib.GetSQLPrefixComment("arrt_trasfinfo_sel",14,"00000010")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
        m_cSql = m_cSql+GetFieldsName_00000010(m_Ctx,m_oWrInfo);
        m_cSql = m_cSql+") values (";
        m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasfinfo.GetString("NUMPROG"),"?",0,0,m_cServer)+", ";
        m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasfinfo.GetDate("DATAOPE"),"?",0,0)+", ";
        m_cSql = m_cSql+""+CPLib.ToSQL("S","?",0,0,m_cServer)+", ";
        m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasfinfo.GetString("IDBASE"),"?",0,0,m_cServer)+", ";
        m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasfinfo.GetDate("DATAREG"),"?",0,0)+", ";
        m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasfinfo.GetString("IDBASE"),"?",0,0,m_cServer)+", ";
        m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_trasfinfo.GetString("DESCRAP"),"?",0,0,m_cServer)+", ";
        ...
        Cursor_qbe_trasfinfo.Next();
      }
      m_cConnectivityError = Cursor_qbe_trasfinfo.ErrorMessage();
      Cursor_qbe_trasfinfo.Close();
      // * --- End Select
      /* Stop arpg_trasfinfo */
      Forward f;
      f=new Forward("arpg_trasfinfo",false,this,Forward.Start,false);
  ...
  public static arrt_trasfinfo_selR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_trasfinfo_selR(p_Ctx, p_Caller);
  }
  public void Blank() {
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_trasfinfo,
  public static final String m_cVQRList = ",qbe_trasfinfo,";
  // ENTITY_BATCHES: ,
