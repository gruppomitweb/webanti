...
SetHypPar('m_cID',AsAppletValue(m_IDS['arrt_trasffra_sel']));
SendData('start','arrt_trasffra_sel'+'',HParApplet().asString());
}
...
function Help() {
windowOpenForeground('../doc/arpg_trasffra_wu_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_trasffra_wu',m_cSelectedPage);
...
_FillChildren.n=[];
FillWv.n=["FLGSEL","DaDatOpe","ADatOpe","cdadata","c_adata"];
...
GroupFooter_BeginHtml(document,1,'1459742739'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'1459742739'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'1459742771'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_0A148BA0"+l_nIdx+"' style='position:relative;height:16px'>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'1459742771'+l_nIdx+'','A')
} else {
}
Row_BeginHtml(document,l_nEvenOdd,'1459517651'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_0B9978E8"+l_nIdx+"' style='position:relative;height:22px'>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'1459517651'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
