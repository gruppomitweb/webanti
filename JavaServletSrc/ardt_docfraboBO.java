...
// * --- Fine Area Manuale
public class ardt_docfraboBO extends ardt_docfraboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_docfraboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_docfrabo");
    }
  ...
  public boolean Load(String p_NUMPROG) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_docfrabo,m_cPhName_docfrabo,m_cVirtName_docfrabo,m_cVirtName_docfrabo+".NUMPROG="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_NUMPROG,"C",11,0),m_cServer_docfrabo,p_NUMPROG),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"docfrabo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_docfrabo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"NUMPROG="+CPLib.ToSQL(w_NUMPROG,"C",11,0)+" AND CONNESCLI="+CPLib.ToSQL(opk_CONNESCLI,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_docfrabo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"docfrabo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_docfrabo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_docfrabo);
      l_cSql = "insert into "+m_oWrInfo_docfrabo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_docfrabo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"docfrabo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_docfrabo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"docfrabo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_docfrabo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_docfrabo);
      l_cSql = "Update "+m_oWrInfo_docfrabo.GetTableWriteName()+" set";
