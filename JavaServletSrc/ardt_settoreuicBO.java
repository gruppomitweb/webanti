...
// * --- Fine Area Manuale
public class ardt_settoreuicBO extends ardt_settoreuicBL {
  ...
  // * --- Fine Area Manuale
  public ardt_settoreuicBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_settoreuic");
    }
  ...
  public boolean Load(String p_SETSINT,double p_IDSETSIN) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_tbsetsina,m_cPhName_tbsetsina,m_cVirtName_tbsetsina,m_cVirtName_tbsetsina+".SETSINT="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_SETSINT,"C",3,0),m_cServer_tbsetsina,p_SETSINT)+" AND "+m_cVirtName_tbsetsina+".IDSETSIN="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDSETSIN,"N",5,0),m_cServer_tbsetsina,p_IDSETSIN),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        m_bCalculating = true;
        w_SETSINT = l_rsLoadBody.GetString("SETSINT");
        w_IDSETSIN = l_rsLoadBody.GetDouble("IDSETSIN");
        // Assegna i valori degli op_
        // Assegna il contesto dei figli di testata/piedi a Empty
        SPExtenderLib.LoadExtendedFields(l_rsLoadBody,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsetsina")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_tbsetsina.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"SETSINT="+CPLib.ToSQL(w_SETSINT,"C",3,0)+" AND CODATECO="+CPLib.ToSQL(opk_CODATECO,"C",3,0)+" AND IDSETSIN="+CPLib.ToSQL(w_IDSETSIN,"N",5,0);
        l_cSql = l_cSql+m_oWrInfo_tbsetsina.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsetsina")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_tbsetsina,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbsetsina);
      l_cSql = "insert into "+m_oWrInfo_tbsetsina.GetTableWriteName()+" (";
      l_cSql = l_cSql+"SETSINT,";
      l_cSql = l_cSql+"IDSETSIN,";
      l_cSql = l_cSql+"CODATECO,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbsetsina.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsetsina")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_CODATECO,"C",3,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbsetsina.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsetsina")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbsetsina,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbsetsina);
      l_cSql = "Update "+m_oWrInfo_tbsetsina.GetTableWriteName()+" set";
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsetsina")) {
        l_cSql = l_cSql+" CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", CODATECO = "+CPLib.ToSQL(w_CODATECO,"C",3,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"SETSINT="+CPLib.ToSQL(w_SETSINT,"C",3,0)+" AND CODATECO="+CPLib.ToSQL(opk_CODATECO,"C",3,0)+" AND IDSETSIN="+CPLib.ToSQL(w_IDSETSIN,"N",5,0);
      l_cSql = l_cSql+m_oWrInfo_tbsetsina.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbsetsina")) {
