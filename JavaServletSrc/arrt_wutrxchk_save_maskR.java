...
import org.apache.poi.xssf.streaming.SXSSFWorkbook;
// * --- Fine Area Manuale
public class arrt_wutrxchk_save_maskR implements CallerWithObjs {
  ...
  public Font f;
  // * --- Fine Area Manuale
  public arrt_wutrxchk_save_maskR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_wutrxchk_save_mask",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_wutrxchk_save_mask";
  ...
  void Page_Main() throws Exception {
    ...
    try {
      ...
      /* gMsgProc := 'Ora Inizio Elaborazione Correzioni AUAi: '+ arfn_fdatetime(DateTime()) + NL // Messaggio */
      gMsgProc = "Ora Inizio Elaborazione Correzioni AUAi: "+arfn_fdatetimeR.Make(m_Ctx,this).Run(CPLib.DateTime())+"\n";
      m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
      /* gMsgImp := 'Elaborazione Correzioni AUA' */
      gMsgImp = "Elaborazione Correzioni AUA";
      ...
      Cursor_intermbo.Close();
      // * --- End Select
      /* gAzienda := 'AUIWUPSIL' // Azienda */
      gAzienda = "AUIWUPSIL";
      /* gPathApp := Strtran(FileLibMit.GetLocation(),'\','/') // Path Applicazione */
      gPathApp = CPLib.Strtran(FileLibMit.GetLocation(),"\\","/");
      /* gPathApp := Strtran(gPathApp,'/servlet/','') // Path Applicazione */
      gPathApp = CPLib.Strtran(gPathApp,"/servlet/","");
          ...
          public void run() {
            if (CPLib.gt(m_TheCursor.CurrentRow()-1,1000*1.1)) {
              ...
              if ( ! (CPLib.IsNull(info))) {
                info.log("Routine arrt_wutrxchk_save_mask: query on wutrxchk_checked returns "+(m_TheCursor.CurrentRow()-1)+" rows larger then the expected 1000");
              }
            }
          }
          });
      ...
      Cursor_wutrxchk_checked.Close();
      mcTransazioni.GoTop();
      /* If mcTransazioni.RecCount() > 0 */
      if (CPLib.gt(mcTransazioni.RecCount(),0)) {
        for (MemoryCursorRow_wutrxchk_checked rowTransazioni : mcTransazioni._iterable_()) {
          /* gMsgImp := 'Lettura Operazione con codice: '+rowTransazioni.TRXCODPRG */
          gMsgImp = "Lettura Operazione con codice: "+rowTransazioni.TRXCODPRG;
          m_Ctx.SetGlobalString("gMsgImp",gMsgImp);
        ...
        /* gMsgProc := gMsgProc + 'Ora Inizio Creazione file excel per Elaborazione Correzioni AUA: '+ arfn_fdatetime(DateTime()) + NL // Messaggio */
        gMsgProc = gMsgProc+"Ora Inizio Creazione file excel per Elaborazione Correzioni AUA: "+arfn_fdatetimeR.Make(m_Ctx,this).Run(CPLib.DateTime())+"\n";
        m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
        /* Exec "Scrittura file excel" Page 2:Page_XLS */
        Page_XLS();
        /* gMsgProc := gMsgProc + 'Ora Fine Creazione file excel per Elaborazione Correzioni AUA: '+ arfn_fdatetime(DateTime()) + NL // Messaggio */
        gMsgProc = gMsgProc+"Ora Fine Creazione file excel per Elaborazione Correzioni AUA: "+arfn_fdatetimeR.Make(m_Ctx,this).Run(CPLib.DateTime())+"\n";
        m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
        ...
        m_cSql = CPLib.GetSQLPrefixComment("arrt_wutrxchk_save_mask",57,"00000070")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
        ...
        /* Exec Routine arrt_imp_dati_rtra(_tipimp,'N','N','Y','I','N','N','N') */
        arrt_imp_dati_rtraR.Make(m_Ctx,this).Run(_tipimp,"N","N","Y","I","N","N","N");
      } else { // Else
        /* gMsgProc := gMsgProc + 'Non ci sono transazioni da elaborare.' + NL // Messaggio */
        gMsgProc = gMsgProc+"Non ci sono transazioni da elaborare."+"\n";
        m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
      } // End If
      /* gMsgProc := gMsgProc + 'Ora Fine Elaborazione Correzioni AUA: '+ arfn_fdatetime(DateTime()) + NL // Messaggio */
      gMsgProc = gMsgProc+"Ora Fine Elaborazione Correzioni AUA: "+arfn_fdatetimeR.Make(m_Ctx,this).Run(CPLib.DateTime())+"\n";
      m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
    } finally {
      try {
        if (Cursor_intermbo!=null)
  ...
  void Page_XLS() throws Exception {
    /* Formato Excel */
    /* _nfile := LRTrim(gPathDoc)+"/appo/"+LRTrim(gAzienda)+"/"+LRTrim(_filen) */
    _nfile = CPLib.LRTrim(gPathDoc)+"/appo/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(_filen);
    /* Seleziona i record */
  ...
  public static arrt_wutrxchk_save_maskR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_wutrxchk_save_maskR(p_Ctx, p_Caller);
  }
  public static arrt_wutrxchk_save_maskR Make(CPContext p_Ctx) {
    return new arrt_wutrxchk_save_maskR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_fdatetime,arrt_imp_dati_rtra,arrt_wutrxchk_save_mask,
  public static final String i_InvokedRoutines = ",arfn_fdatetime,arrt_imp_dati_rtra,arrt_wutrxchk_save_mask,";
