...
public class arpg_xntestbo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*OITGPCORGS*!";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_xntestboBO BO) {
  ...
  void WriteStyles(arpg_xntestboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_xntestbo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_xntestboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_xntestboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_xntestboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_xntestboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_xntestboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_xntestboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_xntestboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_xntestboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_xntestbo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_xntestbo","");
  ...
  void WriteMainFormScript(arpg_xntestboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_xntestboBO getBO(ServletStatus status) {
    arpg_xntestboBO BO = new arpg_xntestboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_xntestboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_xntestboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_xntestboBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_xntestboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_xntestboBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_xntestboBO BO) {
    ...
    item.put("altInterfaceFor","arpg_xntestbo");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_xntestboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_xntestboBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_xntestboBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_xntestboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_xntestboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_xntestboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_xntestbo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_xntestbo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_xntestboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_xntestboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_xntestboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_xntestboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_xntestboBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_xntestboBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_xntestboBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_xntestboBO BO) {
    boolean result = true;
    return result;
  }
}
