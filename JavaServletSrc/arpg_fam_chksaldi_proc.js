...
var h;
SetDisplay('NRFKISPWKP_DIV',IsHiddenByStateDriver('Parametri di Base'));
...
SetLocationHref(null,'arrt_fam_chksaldi'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
_ResetTracker();
...
w_Messaggi.raiseEvent('arpg_fam_chksaldi_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_fam_chksaldi_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_fam_chksaldi_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
GetLayerID.Layers.push({"id":"NRFKISPWKP","name":"Parametri di Base"});
function DeclareWVs() {
var a = arguments;
w_anno=a[0];
w_DaDatOpe=a[1];
...
p_d.setValue('arpg_fam_chksaldi',m_cSelectedPage);
...
if (IsDisabledByStateDriver('Parametri di Base')) {
DisableInputsInContainer(Ctrl('NRFKISPWKP'),true);
}
DisableUI();
...
var l_bResult = true;
m_bCalculating=true;
w_DaDatOpe=(Eq(w_anno,0)?NullDate():CharToDate(Str(w_anno,4,0)+'0101'));
w_ADatOpe=(Eq(w_anno,0)?NullDate():CharToDate(Str(w_anno,4,0)+'1231'));
...
_FillChildren.n=[];
FillWv.n=["anno","DaDatOpe","ADatOpe"];
