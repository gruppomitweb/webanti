...
// * --- Fine Area Manuale
public class arrt_mod_opextrboR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_mod_opextrboR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_mod_opextrbo",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    m_cSql = CPLib.GetSQLPrefixComment("arrt_mod_opextrbo",2,"0000000A")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
    m_cSql = m_cSql+"SALDO = "+CPLib.ToSQL(w_impsaldo,"N",17,0)+", ";
    m_cSql = m_cSql+"QUANTITA = "+CPLib.ToSQL(w_nummov,"N",6,0)+", ";
    m_cSql = m_cSql+"VALUTA = "+CPLib.ToSQL(w_codval,"C",3,0,m_cServer)+", ";
    m_cSql = m_cSql+"NOTE = "+CPLib.ToSQL(w_txtnote,"C",50,0,m_cServer)+", ";
    ...
    m_cSql = m_cSql+" and ANNO = "+CPSql.SQLValueAdapter(CPLib.ToSQL(w_annorif,"?",0,0,m_cServer),m_cServer,w_annorif)+"";
  ...
  public static arrt_mod_opextrboR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_mod_opextrboR(p_Ctx, p_Caller);
  }
  public void Blank() {
