...
// * --- Fine Area Manuale
public class ardt_clientiopeBO extends ardt_clientiopeBL {
  ...
  // * --- Fine Area Manuale
  public ardt_clientiopeBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_clientiope");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_clientiope,m_cPhName_clientiope,m_cVirtName_clientiope,m_cVirtName_clientiope+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_clientiope,p_IDFILEBO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"clientiope")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_clientiope.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND CODSOG="+CPLib.ToSQL(opk_CODSOG,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_clientiope.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"clientiope")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_clientiope,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_clientiope);
      l_cSql = "insert into "+m_oWrInfo_clientiope.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_clientiope.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"clientiope")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_clientiope.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"clientiope")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_clientiope,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_clientiope);
      l_cSql = "Update "+m_oWrInfo_clientiope.GetTableWriteName()+" set";
