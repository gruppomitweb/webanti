...
// * --- Fine Area Manuale
public class ardt_cgo_oper_multi_dcBO extends ardt_cgo_oper_multi_dcBL {
  ...
  // * --- Fine Area Manuale
  public ardt_cgo_oper_multi_dcBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_cgo_oper_multi_dc");
    }
  ...
  public boolean Load(String p_SNAINUMOPE) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_cgo_operazioni_dl_multi_dt_dc+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cgo_operazioni_dl_multi_dt_dc,m_cPhName_cgo_operazioni_dl_multi_dt_dc,m_cVirtName_cgo_operazioni_dl_multi_dt_dc,m_cVirtName_cgo_operazioni_dl_multi_dt_dc+".SNAINUMOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_SNAINUMOPE,"C",15,0),m_cServer_cgo_operazioni_dl_multi_dt_dc,p_SNAINUMOPE),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_operazioni_dl_multi_dt_dc")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          w_CGOTIPOPE = l_rsLoadBody.GetString("CGOTIPOPE");
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cgo_operazioni_dl_multi_dt_dc.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"SNAINUMOPE="+CPLib.ToSQL(w_SNAINUMOPE,"C",15,0)+" AND CPROWNUM="+CPLib.ToSQL(w_CPROWNUM,"N",4,0);
        l_cSql = l_cSql+m_oWrInfo_cgo_operazioni_dl_multi_dt_dc.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_operazioni_dl_multi_dt_dc")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_operazioni_dl_multi_dt_dc,l_cSql);
  ...
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"BENOJFALBB")) {
      ...
      result.AppendFieldsHash("BENOJFALBB",CPLib.GetLinkedFieldProps("CODTPGIO","TIPOGIOCO","C",3,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_operazioni_dl_multi_dt_dc);
      l_cSql = "insert into "+m_oWrInfo_cgo_operazioni_dl_multi_dt_dc.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_cgo_operazioni_dl_multi_dt_dc.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_operazioni_dl_multi_dt_dc")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_cgo_operazioni_dl_multi_dt_dc.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cgo_operazioni_dl_multi_dt_dc")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cgo_operazioni_dl_multi_dt_dc,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cgo_operazioni_dl_multi_dt_dc);
      l_cSql = "Update "+m_oWrInfo_cgo_operazioni_dl_multi_dt_dc.GetTableWriteName()+" set";
  ...
  // * --- Fine Area Manuale
  // ENTITY_BATCHES: ,arfn_cgo_chkidb,arrt_aggiornamento_multiple_dc,
  public static final String i_InvokedRoutines = ",arfn_cgo_chkidb,arrt_aggiornamento_multiple_dc,";
}
