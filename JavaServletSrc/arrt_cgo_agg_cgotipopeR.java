...
// * --- Fine Area Manuale
public class arrt_cgo_agg_cgotipopeR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_agg_cgotipopeR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_agg_cgotipope",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_cgo_agg_cgotipope";
    } else if (m_bEventRunning && CPLib.eqr("i_EntityType",p_cVarName)) {
      return "routine";
    }
    if (CPLib.eqr("pCodGio",p_cVarName)) {
      return pCodGio;
    }
  ...
  void Page_1() throws Exception {
    /* pCodGio Char(3) // Codice Gioco */
    /* pTipGio Char(1) // Tipo Gioco */
    /* If not(Empty(pTipGio)) */
    if ( ! (CPLib.Empty(pTipGio))) {
      ...
      m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_agg_cgotipope",2,"00000006")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      m_cSql = m_cSql+"CGOTIPOPE = "+CPLib.ToSQL(pTipGio,"C",1,0,m_cServer)+", ";
      m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"cgo_operazioni_dl",false,true);
      m_cSql = m_cSql+" where ";
      m_cSql = m_cSql+"TIPOGIOCO = "+CPSql.SQLValueAdapter(CPLib.ToSQL(pCodGio,"?",0,0,m_cServer),m_cServer,pCodGio)+"";
      ...
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_agg_cgotipope",2,"00000007")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      m_cSql = m_cSql+"CGOTIPOPE = "+CPLib.ToSQL(pTipGio,"C",1,0,m_cServer)+", ";
  ...
  public String RunAsync(String p_pCodGio,String p_pTipGio) {
    pCodGio = p_pCodGio;
    pTipGio = p_pTipGio;
  ...
  public Forward Run(String p_pCodGio,String p_pTipGio) {
    pCodGio = p_pCodGio;
    pTipGio = p_pTipGio;
    return Run();
  }
  ...
  public static arrt_cgo_agg_cgotipopeR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_agg_cgotipopeR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pCodGio = CPLib.Space(3);
    pTipGio = CPLib.Space(1);
  }
  ...
  public static String[] m_cRunParameterNames={"pCodGio","pTipGio"};
}
