...
public class arpg_fraziobo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*VCXUVZEZYV*!";
  ...
  public String getEntityTitle() {
    return "Frazionarie relative al rapporto (A.U.I.)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_fraziobo";
    ...
    l_eg.items = new SPItem[]{
                   new SPItem("rapporto","C",25,0, "hide","Rapporto Collegato")
                 };
    l_eg.m_lExecuteAtStart = true;
    l_eg.m_cQueryName = "qbe_operazrappo2";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_frazioboBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"304798647",p_Context.Translate("Frazionarie relative al rapporto (A.U.I.)")},
    {"7776403",p_Context.Translate("Stato")}};
    return l_translations;
  }
  void WriteStyles(arpg_frazioboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_fraziobo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_frazioboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_frazioboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_frazioboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_frazioboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_frazioboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_frazioboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_frazioboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_frazioboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_fraziobo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_fraziobo","");
    ...
    status.out.println("// * --- arpg_fraziobo");
  ...
  void WriteMainFormScript(arpg_frazioboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_frazioboBO getBO(ServletStatus status) {
    arpg_frazioboBO BO = new arpg_frazioboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_frazioboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_frazioboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_frazioboBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_frazioboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_frazioboBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_frazioboBO BO) {
    ...
    item.put("altInterfaceFor","arpg_fraziobo");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_frazioboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_frazioboBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_frazioboBO BO) {
    String l_cTrsQ;
    ...
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_STATO,"C",20,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_frazioboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_frazioboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_frazioboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_fraziobo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_fraziobo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_frazioboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_frazioboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_frazioboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_frazioboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_frazioboBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_frazioboBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_frazioboBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_frazioboBO BO) {
    boolean result = true;
    return result;
  }
}
