    ...
    try {
      ...
      while ( ! (FileLibrary.Eof(fhand))) {
        ...
        if (CPLib.gt(CPLib.Len(CPLib.LRTrim(riga)),0)) {
          ...
          /* Verifico se il soggetto in esame non è già presente l'inserisco */
            ...
            if (CPLib.eqr(CPLib.Len(CPLib.LRTrim(CPLib.Substr(riga,112,16))),16)) {
              ...
              if (CPLib.eqr(_allineatonomecnome,0) && CPLib.eqr(_allineamento,0)) {
                ...
                if ( ! (CPLib.eqr(_ragsocold,CPLib.Substr(riga,7,70)))) {
                  ...
                  if (CPLib.lt(m_nUpdatedRows,1)) {
                    // No rows updated
                    /* gMsgProc := gMsgProc +"Rilevati errori nel file "+nomefileimport+" per il soggetto: "+Substr(riga,7,70)+" con codice fiscale: "+Substr(riga,112,16)+" Ricreare il file e rieseguire l'import dei dati. L'attuale procedura di importazione verrà annulata automaticamente."+ NL */
                    gMsgProc = gMsgProc+"Rilevati errori nel file "+nomefileimport+" per il soggetto: "+CPLib.Substr(riga,7,70)+" con codice fiscale: "+CPLib.Substr(riga,112,16)+" Ricreare il file e rieseguire l'import dei dati. L'attuale procedura di importazione verrà annulata automaticamente."+"\n";
                    m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
                    /* _esito := 1 */
                    _esito = CPLib.Round(1,0);
                    /* m_cLastMsgError = "" */
