...
WindowAlert(AlertErrorMessage('1064994114'));
}
if (Eq(formaction,'edit') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('930815549'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('11255051226'));
}
}
}
var m_bIsMq = false;
...
m_oFather.DoQueryFromChild('wperazbo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
function DoQueryFrom_wogopebo(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_wlientiope(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_weneficope(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
function DoQueryFrom_werzistiope(firstAlias) {
return m_cVirtName+".IDBASE = "+firstAlias+".IDFILEBO ";
}
...
_chNotF(childAlias,'master','armt_storope');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wperazbo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
