...
public class arrt_import_fileae_esiti extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*CXNAELKRWQ*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    // Segnala se e' stata passata con la start della servlet
    public boolean m_bpassedw_fileape;
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      m_bpassedw_fileape = source.Has("w_fileape");
      if (m_bpassedw_fileape) {
        w_fileape = source.GetParameter("w_fileape",source.GetParameter("fileape",""));
      } else {
        if (source.IsForwarded()) {
          w_fileape = source.GetCaller().GetString("fileape","C",80,0);
          m_bpassedw_fileape = true;
        } else {
          w_fileape = source.GetParameter("fileape",source.GetParameter("w_fileape",""));
        }
      }
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_import_fileae_esiti";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_impzip) ||  ! (status.m_bpassedw_fileape) ||  ! (status.m_bpassedw_codval) ||  ! (status.m_bpassedw_f_ricaui) ||  ! (status.m_bpassedw_prefrap) ||  ! (status.m_bpassedw_f_denom) ||  ! (status.m_bpassedw_offset);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_fileape)) {
      status.out.println("l_cV=WtA(l_oWnd('w_fileape',"+SPLib.ToJSValue(status.w_fileape,"C",80,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_fileape,"C",80,0)+",'C');");
    }
    status.out.println("l_oWv.setValue('w_fileape',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_import_fileae_esitiR Rcalled;
    Rcalled = new arrt_import_fileae_esitiR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_import_fileae_esitiR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_import_fileae_esitiR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_import_fileae_esiti"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println("js:w_fileape="+SPLib.ToJSValue(CPLib.ToProperties(Rcalled.w_fileape),"?",0,0)+"");
        ...
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_fileape,null)) {");
        status.out.println("window[f].w_fileape="+SPLib.ToJSValue(status.w_fileape,"C",0,0)+";");
        status.out.println("}");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_import_fileae_esiti");
      if ( ! (CPLib.IsNull(l))) {
