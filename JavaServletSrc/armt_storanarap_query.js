...
WindowAlert(AlertErrorMessage('MSG_CANNOT_ADD_DATA'));
}
if (Eq(formaction,'edit') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('MSG_CANNOT_EDIT_DATA'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('MSG_CANNOT_DEL_DATA'));
}
}
}
var m_bIsMq = false;
...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('LXORWVRYJJ').value,'C'),w_RAPPORTO)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('wnarapbo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'RAPPORTO') &&  ! (Empty(HtW(Ctrl('LXORWVRYJJ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.RAPPORTO',HtW(Ctrl('LXORWVRYJJ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_delegabo(firstAlias) {
return m_cVirtName+".RAPPORTO = "+firstAlias+".RAPPORTO ";
}
...
_chNotF(childAlias,'master','armt_storanarap');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wnarapbo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['LXORWVRYJJ'],i=0;i>=0;i--){
