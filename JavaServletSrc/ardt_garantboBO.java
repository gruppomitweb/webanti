...
// * --- Fine Area Manuale
public class ardt_garantboBO extends ardt_garantboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_garantboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_garantbo");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_garantbo,m_cPhName_garantbo,m_cVirtName_garantbo,m_cVirtName_garantbo+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_garantbo,p_RAPPORTO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"garantbo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_garantbo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND CODINTER="+CPLib.ToSQL(opk_CODINTER,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_garantbo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"garantbo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_garantbo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_garantbo);
      l_cSql = "insert into "+m_oWrInfo_garantbo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_garantbo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"garantbo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_garantbo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"garantbo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_garantbo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_garantbo);
      l_cSql = "Update "+m_oWrInfo_garantbo.GetTableWriteName()+" set";
