...
_modifyandopen('arpg_edt_fraziobo'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes',null)
}
...
l_oWv.setValue('m_cParameterSequence',AsAppletValue('IDBASE,STATOREG'));
_modifyandopen('arpg_del_fraziobo'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes',null)
}
...
_modifyandopen('armt_ofrazionate'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=view&m_cAtExit=close&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes',null)
}
_ResetTracker();
...
function Help() {
windowOpenForeground('../doc/arpg_pers_fraz_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_pers_fraz',m_cSelectedPage);
...
function TrsQueryRow() {
  ...
  this.c_CODANA = arguments[3]
  this.c_CODVOC = arguments[4]
  this.c_VALUTA = arguments[5]
  this.c_TOTLIRE = arguments[6]
  this.c_TOTCONT = arguments[7]
  this.c_STATOREG = arguments[8]
...
c_CODANA=row.c_CODANA;
c_CODVOC=row.c_CODVOC;
...
row.c_CODANA=c_CODANA;
row.c_CODVOC=c_CODVOC;
...
GroupFooter_BeginHtml(document,1,'1399748435'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'1399748435'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'1936619347'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_09782000"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
...
"")
document.writeln(
 "<span class=' SPLabel EPXKLDOUQE_DIV'>" +
 "<label class='EPXKLDOUQE'>" +
...
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'1936619347'+l_nIdx+'','A')
} else {
}
Row_BeginHtml(document,l_nEvenOdd,'326005732'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_09822CF8"+l_nIdx+"' style='position:relative;height:18px'>" +
"")
document.writeln(
 "<a href='javascript:href_XZUXXWWVEZ("+l_nIdx+")' id='XZUXXWWVEZ_"+l_nIdx+"_HREF'>" +
 "<img id='XZUXXWWVEZ_"+l_nIdx+"' class='XZUXXWWVEZ' style='"+( ! (Ne(c_STATOREG,'0') && Ne(c_STATOREG,'1'))?'':'display:none;visibility:hidden;')+"' src="+"'"+ToResource('images/edit.gif')+"'"+" alt='' onload=SetSectionsHeight('DIV_09822CF8',"+l_nIdx+")>" +
 "</a>" +
"")
document.writeln(
 "<a href='javascript:href_XZMBQDKCOI("+l_nIdx+")' id='XZMBQDKCOI_"+l_nIdx+"_HREF'>" +
 "<img id='XZMBQDKCOI_"+l_nIdx+"' class='XZMBQDKCOI' style='"+( ! (Ne(c_STATOREG,'0') && Ne(c_STATOREG,'1'))?'':'display:none;visibility:hidden;')+"' src="+"'"+ToResource('images/view.png')+"'"+" alt='' onload=SetSectionsHeight('DIV_09822CF8',"+l_nIdx+")>" +
 "</a>" +
"")
document.writeln(
 "<a href='javascript:href_AWKKPZGIBE("+l_nIdx+")' id='AWKKPZGIBE_"+l_nIdx+"_HREF'>" +
 "<img id='AWKKPZGIBE_"+l_nIdx+"' class='AWKKPZGIBE' style='"+( ! (Ne(c_STATOREG,'0') && Ne(c_STATOREG,'1'))?'':'display:none;visibility:hidden;')+"' src="+"'"+ToResource('images/remove.gif')+"'"+" alt='' onload=SetSectionsHeight('DIV_09822CF8',"+l_nIdx+")>" +
 "</a>" +
"")
document.writeln(
...
document.writeln(
 ...
 ""+ToHTag(WtH(c_DATAREG,'D',10,0,'DD-MM-YYYY'))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
...
"")
document.writeln(
 "<span class=' SPLabel PNJJQRMNRI_DIV'>" +
 "<label class='PNJJQRMNRI'>" +
 ""+ToHTag(WtH(LRTrim(c_CODANA)+' '+LRTrim(c_CODVOC),'C',7,0,''))+"" +
 "</label>" +
 "</span>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'326005732'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
