...
public class arrt_seekpep_rs_mc extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*bd52e4f2*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public MemoryCursor_mcpep2structure_mcrdef aa_retval;
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_seekpep_rs_mcR Rcalled;
    Rcalled = new arrt_seekpep_rs_mcR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.m_CursorResult = Rcalled.Run(status.pTicket,status.pCodFisc);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_seekpep_rs_mcR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_seekpep_rs_mcR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_seekpep_rs_mc"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
