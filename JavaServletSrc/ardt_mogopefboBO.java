...
// * --- Fine Area Manuale
public class ardt_mogopefboBO extends ardt_mogopefboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_mogopefboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_mogopefbo");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_mogopefbo+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_mogopefbo,m_cPhName_mogopefbo,m_cVirtName_mogopefbo,m_cVirtName_mogopefbo+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_mogopefbo,p_IDFILEBO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"mogopefbo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_mogopefbo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_mogopefbo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"mogopefbo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_mogopefbo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_mogopefbo);
      l_cSql = "insert into "+m_oWrInfo_mogopefbo.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDFILEBO,";
      l_cSql = l_cSql+"CODSOG,";
      l_cSql = l_cSql+"LEGAME,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_mogopefbo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"mogopefbo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_mogopefbo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"mogopefbo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_mogopefbo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_mogopefbo);
      l_cSql = "Update "+m_oWrInfo_mogopefbo.GetTableWriteName()+" set";
