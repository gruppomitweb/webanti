...
function Set_WUCODCLC(ctrlValue,kmode,e) {
if (Ne(w_WUCODCLC,ctrlValue)) {
var ctl = _GetCtl(e,'ZRZGZRKXLU');
if (_tracker.goon(ctl,ctrlValue)) {
w_WUCODCLC=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('ZRZGZRKXLU',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
function Set_WUOPETYPE(ctrlValue,kmode,e) {
if (Ne(w_WUOPETYPE,ctrlValue)) {
var ctl = _GetCtl(e,'GCKPQHWXSD');
if (_tracker.goon(ctl,ctrlValue)) {
w_WUOPETYPE=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('GCKPQHWXSD',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',1,0,'');
...
var l_Appl = LinkApplet();
l_Appl.SetStringKey('DESCRI',w_WUNSTACLI,50,0);
l_Appl.SetFields('DESCRI');
l_Appl.SetTypes('C');
l_Appl.LinkTable('tbstati');
...
l_bResult=Ne(l_nRec,0);
if (Eq(l_nRec,1)) {
w_WUNSTACLI=l_Appl.GetStringValue('DESCRI',50,0);
l_Appl.GetLinkHash(m_CheckVars);
} else {
...
parent.ZtVWeb.raiseEventToEvalParms('armt_wu_chkerr_sf_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_wu_chkerr_sf_dataNotSent',{'action':formaction});
}
}
}
}
...
l_bResult=false;
w_WUNSTACLI=HtW('','C');
} else if ( ! (_ChkObl('GCKPQHWXSD',1))) {
_SignErr('GCKPQHWXSD');
m_cLastWorkVarErrorMsg='1662988105';
l_bResult=false;
w_WUOPETYPE=HtW('','C');
} else if ( ! (_ChkObl('ZRZGZRKXLU',1))) {
_SignErr('ZRZGZRKXLU');
m_cLastWorkVarErrorMsg='1356244169';
l_bResult=false;
w_WUCODCLC=0;
...
if (Eq('BQFSVTKPAE_ZOOM',lc)) {
return 0;
}
if (Eq('PKOBXVTTQB_ZOOM',lc)) {
return 0;
}
if (Eq('YCVYLQBAVQ_ZOOM',lc)) {
return 0;
}
...
l_oWv.setValue('WUDATOPE',WtA(w_WUDATOPE,'D'));
...
l_oWv.setValue('WURDOCCLI',WtA(w_WURDOCCLI,'D'));
l_oWv.setValue('WUSDOCCLI',WtA(w_WUSDOCCLI,'D'));
...
l_oWv.setValue('WUCONTANTI',WtA(w_WUCONTANTI,'N'));
l_oWv.setValue('WUOPETYPE',WtA(w_WUOPETYPE,'C'));
l_oWv.setValue('WUCODCLC',WtA(w_WUCODCLC,'N'));
...
_FillChildren.n=[];
FillWv.n=["WUNOMEFILE","WURIFMTCN","WUTIPOPE","WUCODDIP","WUDATOPE","WUIMPORTO","WUSTADES","WUTIPERR","WUCODICE","WUNDATOPE","WUNCAPCLI","WUNDATNAS","WUNRDOCCLI","WUNSDOCCLI","WUNTDOCCLI","WUNSTADES","WUNCODDIP","WUNCITCLI","WUNBSTACLI","WUNBCITCLI","WUNOMCLI","WUCOGCLI","WUCITCLI","WUCAPCLI","WUINDCLI","WUNOMCON","WUCOGCON","WUBCITCLI","WUDATNAS","WUCODFIS","WUNDOCCLI","WUADOCRIL","WURDOCCLI","WUSDOCCLI","WUSEXCLI","WUTDOCCLI","WUFLGALL","WUNCODFIS","WUCODMAGE","WUNOTE","WUFLGFRC","WUORIGCF","WUSTACLI","WUNSTACLI","WUCONTANTI","WUOPETYPE","WUCODCLC","WUNSEXCLI","xMADESCRI","xDESDIP"];
