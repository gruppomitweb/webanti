...
// * --- Fine Area Manuale
public class armt_tbdurata_pBO extends armt_tbdurata_pBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbdurata_pBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbdurata_p");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbdurata_p,m_cPhName_tbdurata_p,m_cVirtName_tbdurata_p,m_cVirtName_tbdurata_p+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",5,0),m_cServer_tbdurata_p,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbdurata_p")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbdurata_p);
      l_cSql = "insert into "+m_oWrInfo_tbdurata_p.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"RISCHIO,";
      l_cSql = l_cSql+"FLGPERS,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbdurata_p.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbdurata_p")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_tbdurata_p.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbdurata_p")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbdurata_p,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbdurata_p);
      l_cSql = "Update "+m_oWrInfo_tbdurata_p.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",5,0),m_cServer_tbdurata_p,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbdurata_p.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbdurata_p")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbdurata_p.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",5,0),m_cServer_tbdurata_p,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbdurata_p.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbdurata_p")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbdurata_p,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
