...
public class arrt_cgo_moduli_storico extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*445403a5*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_cgo_moduli_storico";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_cgo_moduli_storicoR Rcalled;
    Rcalled = new arrt_cgo_moduli_storicoR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_cgo_moduli_storicoR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_cgo_moduli_storicoR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_cgo_moduli_storico"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
