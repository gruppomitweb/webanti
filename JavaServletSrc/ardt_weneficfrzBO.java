...
// * --- Fine Area Manuale
public class ardt_weneficfrzBO extends ardt_weneficfrzBL {
  ...
  // * --- Fine Area Manuale
  public ardt_weneficfrzBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_weneficfrz");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_weneficfrz+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_weneficfrz,m_cPhName_weneficfrz,m_cVirtName_weneficfrz,m_cVirtName_weneficfrz+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_weneficfrz,p_IDFILEBO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"weneficfrz")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_weneficfrz.GetTableWriteName()+" where ";
        ...
        l_cSql = l_cSql+m_oWrInfo_weneficfrz.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"weneficfrz")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_weneficfrz,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_weneficfrz);
      l_cSql = "insert into "+m_oWrInfo_weneficfrz.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_weneficfrz.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"weneficfrz")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_weneficfrz.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"weneficfrz")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_weneficfrz,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_weneficfrz);
      l_cSql = "Update "+m_oWrInfo_weneficfrz.GetTableWriteName()+" set";
