...
var bResult = true;
if (Eq(formaction,'query')) {
bResult=Eq(w_gCodDip,w_CODDIPE) || Utilities.Make(window).IsInGroup(1) || Utilities.Make(window).IsInGroup(9) || Utilities.Make(window).IsInGroup(11) || Utilities.Make(window).IsInGroup(12);
}
if (Eq(formaction,'edit')) {
bResult=Eq(w_FLGSBLOCCO,'S') && (Eq(w_gCodDip,w_CODDIPE) || Utilities.Make(window).IsInGroup(1) || Utilities.Make(window).IsInGroup(9) || Utilities.Make(window).IsInGroup(11) || Utilities.Make(window).IsInGroup(12));
}
...
if (Eq(formaction,'query') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('10966082329'));
}
if (Eq(formaction,'edit') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('1846422530'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('10924022783'));
}
}
}
var m_bIsMq = false;
...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('XFUOZLSACP').value,'C'),w_SNAINUMOPE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_operazioni_storico',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'SNAINUMOPE') &&  ! (Empty(HtW(Ctrl('XFUOZLSACP').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.SNAINUMOPE',HtW(Ctrl('XFUOZLSACP').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_cgo_operazioni_storico_multi(firstAlias) {
return m_cVirtName+".SNAINUMOPE = "+firstAlias+".SNAINUMOPE ";
}
function DoQueryFrom_cgo_qadv_operazioni_storico(firstAlias) {
return m_cVirtName+".SNAINUMOPE = "+firstAlias+".SNAINUMOPE ";
}
...
_chNotF(childAlias,'master','armt_snai_oper_storico');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_operazioni_storico',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['XFUOZLSACP'],i=0;i>=0;i--){
