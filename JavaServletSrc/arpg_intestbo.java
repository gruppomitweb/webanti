...
public class arpg_intestbo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*AFAVVEXCVH*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_intestbo";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 714;
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_intestboBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11463735382",p_Context.Translate("Intestatario")},
    {"11464190038",p_Context.Translate("Intestatari")},
    {"123801641",p_Context.Translate("Inizio")},
    {"443606",p_Context.Translate("Fine")}};
    return l_translations;
  }
  void WriteStyles(arpg_intestboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_intestbo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_intestboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_intestboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_intestboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_intestboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_intestboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_intestboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_intestboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_intestboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(714,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_intestbo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_intestbo","");
  ...
  void WriteMainFormScript(arpg_intestboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_intestboBO getBO(ServletStatus status) {
    arpg_intestboBO BO = new arpg_intestboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_intestboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_intestboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_intestboBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_intestboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_intestboBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_intestboBO BO) {
    ...
    item.put("altInterfaceFor","arpg_intestbo");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_intestboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_intestboBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_intestboBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_intestboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_intestboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_intestboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_intestbo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_intestbo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_intestboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_intestboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_intestboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_intestboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_intestboBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_intestboBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_intestboBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_intestboBO BO) {
    boolean result = true;
    return result;
  }
}
