...
SetHypPar('pFlgT',AsAppletValue('V'));
...
var action = (Eq(Left('arpg_cgo_operazioni_vlt'+''+'?',24),'arpg_cgo_operazioni_vlt?')?'query':'start');
SetHypPar('m_cAction',AsAppletValue(action));
SetHypPar('m_cMode',AsAppletValue('hyperlink'));
SendData(action,'arpg_cgo_operazioni_vlt'+'',HParApplet().asString());
}
...
l_oWv.setValue('pTipo',AsAppletValue('V'));
...
function Help() {
windowOpenForeground('../doc/arpg_cgo_operazioni_vlt_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_cgo_operazioni_vlt',m_cSelectedPage);
...
GroupFooter_BeginHtml(document,1,'10648329643'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'10648329643'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'10648329641'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_0BAAF3E0"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'10648329641'+l_nIdx+'','A')
} else {
}
Row_BeginHtml(document,l_nEvenOdd,'10648343711'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_0BF99090"+l_nIdx+"' style='position:relative;height:19px'>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'10648343711'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
