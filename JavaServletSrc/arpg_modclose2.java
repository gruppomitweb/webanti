...
public class arpg_modclose2 extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*HTYINCFCTI*!";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_modclose2BO BO) {
  ...
  void WriteStyles(arpg_modclose2BO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_modclose2.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_modclose2BO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_modclose2BO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_modclose2BO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(42);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_modclose2BO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_modclose2BL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_modclose2BL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_modclose2BL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_modclose2BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_modclose2"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_modclose2","");
    ...
    status.out.println("// * --- arpg_modclose2");
  ...
  void WriteMainFormScript(arpg_modclose2BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_modclose2BO getBO(ServletStatus status) {
    arpg_modclose2BO BO = new arpg_modclose2BO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_modclose2BO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_modclose2BO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_modclose2BO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_modclose2BO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_modclose2BO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_modclose2BO BO) {
    ...
    item.put("altInterfaceFor","arpg_modclose2");
    items.put(item);
    String s;
    ...
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Cancellazione_\",\"page\":\"1\",\"spuid\":\"XQSJGZVMDV_DIV\",\"tabindex\":\"2\",\"type\":\"Label\",\"value\":\"Data Cancellazione:\",\"w\":\"121\",\"x\":\"11\",\"y\":\"18\",\"zone\":\"ODCPZZXLHY_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_YBBLCLWLHJ()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"YBBLCLWLHJ\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"9\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"626\",\"y\":\"9\",\"zone\":\"ODCPZZXLHY_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui aggiornamento dati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_MDWFLPZWFN()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"MDWFLPZWFN\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"10\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"578\",\"y\":\"9\",\"zone\":\"ODCPZZXLHY_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_modclose2BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_modclose2BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_modclose2BO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_modclose2?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_modclose2_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_modclose2BO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_modclose2BO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_modclose2BO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_modclose2BO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_modclose2BO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_modclose2BO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  void IncludeJS(ServletStatus status) {
    String l_cJsUid;
    status.out.println(
     "<script src='SPPrxy/"+arrt_modclose2.m_cEntityUid+"/arrt_modclose2.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_modclose2.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_modclose2BO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_modclose2BO BO) {
    boolean result = true;
    return result;
  }
}
