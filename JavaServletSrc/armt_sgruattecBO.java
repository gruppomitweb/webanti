...
// * --- Fine Area Manuale
public class armt_sgruattecBO extends armt_sgruattecBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_sgruattecBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_sgruattec");
    }
  ...
  public static String[] m_cLoadParameterNames = {"SOTGRU"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_SOTGRU) {
    CPResultSet l_rsLoad=null;
    SqlLoader l_oSqlLoader;
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbstgru,m_cPhName_tbstgru,m_cVirtName_tbstgru,m_cVirtName_tbstgru+".SOTGRU="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_SOTGRU,"C",3,0),m_cServer_tbstgru,p_SOTGRU),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        /*  Assegna i valori letti dal record appena caricato nelle variabili di work.
                                    ...
                                    degli item.
                                  */
        w_SOTGRU = l_rsLoad.GetString("SOTGRU");
        w_DESCRI = l_rsLoad.GetString("DESCRI");
        w_TIPOSOT = l_rsLoad.GetString("TIPOSOT");
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbstgru")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbstgru);
      l_cSql = "insert into "+m_oWrInfo_tbstgru.GetTableWriteName()+" (";
      l_cSql = l_cSql+"SOTGRU,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+"TIPOSOT,";
      l_cSql = l_cSql+"DATAINI,";
      l_cSql = l_cSql+"DATAFINE,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbstgru.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbstgru")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbstgru,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbstgru);
      l_cSql = "Update "+m_oWrInfo_tbstgru.GetTableWriteName()+" set";
      l_cSql = l_cSql+" DESCRI = "+CPLib.ToSQL(w_DESCRI,"C",200,0);
      l_cSql = l_cSql+", TIPOSOT = "+CPLib.ToSQL(w_TIPOSOT,"C",1,0);
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"SOTGRU="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_SOTGRU,"C",3,0),m_cServer_tbstgru,w_SOTGRU);
      l_cSql = l_cSql+m_oWrInfo_tbstgru.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbstgru")) {
  ...
  public boolean Delete(String p_SOTGRU) {
    String l_cSql;
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbstgru.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"SOTGRU="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_SOTGRU,"C",3,0),m_cServer_tbstgru,w_SOTGRU);
          l_cSql = l_cSql+m_oWrInfo_tbstgru.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbstgru")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbstgru,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
