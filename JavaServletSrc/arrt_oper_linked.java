...
public class arrt_oper_linked extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*7a663832*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_oper_linkedR Rcalled;
    Rcalled = new arrt_oper_linkedR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.forwardedto = Rcalled.Run(status.w_DaDatOpe,status.w_ADatOpe,status.w_DaDatReg,status.w_ADatReg,status.w_daCodCli,status.w_aCodCli);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_oper_linkedR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_oper_linkedR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      status.out.println(
       "<!--" +
      "");
      status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(Rcalled.RunAsync(status.w_DaDatOpe,status.w_ADatOpe,status.w_DaDatReg,status.w_ADatReg,status.w_daCodCli,status.w_aCodCli)))+"");
      status.out.println(
       " -->" +
      "");
    } else {
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_oper_linked"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_oper_linked");
      if ( ! (CPLib.IsNull(l))) {
