...
public class ardt_ae_intestbo extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*236c1add*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "jntestbo";
    }
    public String w_RAPPORTO;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_RAPPORTO,context,"jntestbo","RAPPORTO","C",25,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    ardt_ae_intestboBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_jntestbo;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"jntestbo","personbo"};
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","","page"),new ZoneItem("DatiAna","Dati Anagrafici","horizontal_section"),new ZoneItem("Prog","Progressivi Informazione","horizontal_section")};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_ae_intestboBO BO) {
  ...
  void WriteStyles(ardt_ae_intestboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_ae_intestbo.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_ae_intestboBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_ae_intestbo',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_ae_intestbo",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_ae_intestboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_ae_intestboBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:274px;width:740px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(ardt_ae_intestboBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_TOP_OF_PAGE_1' style='position:relative;height:135px;display:block' page='1'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:absolute;top:24px;left:6px;height:106px;width:734px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
      ...
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:24px;left:139px;height:106px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:24px;left:580px;height:106px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:24px;left:652px;height:106px;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
  ...
  void HTMLRowScript(ardt_ae_intestboBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(ardt_ae_intestboBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<div id='VRCVINYRJO' class='SectionBody' style='position:relative;height:54px;display:block'>" +
    "");
    ...
    if (status.includeZoneAltInterface("DatiAna")) {
      ...
      if (status.HasZoneAltInterface("DatiAna@BEGIN") || status.HasZoneAltInterface("DatiAna@END")) {
        status.out.println(
         "<div id='VRCVINYRJO_boxContent' style='position:relative;height:54px;display:block'>" +
        "");
      }
      status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(ardt_ae_intestboBO BO,ServletStatus status) {
    ...
    if (status.HasZoneAnyAltInterface("DatiAna")) {
      ...
      status.out.println("Ctrl('VRCVINYRJO').setAttribute('band_height',54);");
    }
  ...
  void WriteControls_p1_b2(ardt_ae_intestboBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.eq(status.GetBoxHeaderInclude(),"")) {
      ...
      status.out.println(
       ...
       ""+status.context.Translate("Progressivi Informazione")+"" +
       "</span>" +
      "");
      status.out.println(
      ...
      SPLib.Include(servletContext,status,status.GetBoxHeaderInclude()+"?ForcedPortletUID=MINAAOAOZC_HEADER&m_cBoxId=MINAAOAOZC&m_cInitialState=open&m_cTheme="+status.m_cTheme+"&m_nTabIndex="+status.m_nTabIndex+"&m_cCaption="+com.zucchetti.sitepainter.Library.URLencode(status.context.Translate("Progressivi Informazione"))+"");
    }
    status.out.println(
  ...
  void WriteControls_p1_b2_CloseBand(ardt_ae_intestboBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(ardt_ae_intestboBO BO,ServletStatus status) {
  ...
  static void PrintState(ardt_ae_intestboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_ae_intestboBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_ae_intestboBL BO,ServletStatus status) {
  ...
  static void PrintStateDetailWorkVars(ardt_ae_intestboBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_ae_intestboBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_ae_intestboBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_ae_intestboBO.TrsRow) BO.m_oDeletedRows.get(k);
  ...
  void WriteMainFormVarScript(ardt_ae_intestboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_ae_intestbo"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_ae_intestbo","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_ae_intestbo",BO.GetCurrentState());
  ...
  void WriteMainFormScript(ardt_ae_intestboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_ae_intestbo);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_ae_intestbo();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_ae_intestbo',Ne(m_nChildStatus,0));");
    } else {
    ...
    status.out.println("m_oFather.m_oardt_ae_intestbo=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_ae_intestboBO getBO(ServletStatus status) {
    ardt_ae_intestboBO BO = new ardt_ae_intestboBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_ae_intestboBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("jntestbo",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_ae_intestboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_ae_intestboBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ardt_ae_intestboBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_jntestbo;
    String l_cPhName = BO.m_cPhName_jntestbo;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_ae_intestboBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_jntestbo+".RAPPORTO"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_jntestbo+".RAPPORTO"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("jntestbo");
    String l_cPhName = CPSql.ManipulateTablePhName("jntestbo",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_ae_intestboBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_jntestbo),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_jntestbo,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_jntestbo+".RAPPORTO) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_jntestbo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_jntestbo,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_jntestbo+".RAPPORTO",CPLib.GetTablesWithAliases(BO.m_cPhName_jntestbo,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_ae_intestboBO BO) {
  ...
  void Edit(ServletStatus status,ardt_ae_intestboBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_ae_intestboBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_ae_intestboBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_ae_intestboBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_ae_intestboBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_ae_intestboBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_ae_intestboBO BO,SPParameterSource source) {
    BO.Load(status.w_RAPPORTO);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_ae_intestboBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_ae_intestboBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_ae_intestboBO BO) {
    ...
    item.put("altInterfaceFor","ardt_ae_intestbo");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"h\":16,\"name\":\"label_BFMPSJTVZY\",\"page\":1,\"spuid\":\"BFMPSJTVZY\",\"tabindex\":3,\"type\":\"Label\",\"typevar\":\"logic\",\"value\":\"Intestatario\",\"w\":128,\"x\":8,\"y\":7,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_XCDXQXTRHM\",\"page\":1,\"spuid\":\"XCDXQXTRHM\",\"tabindex\":2,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"Inizio\",\"w\":64,\"x\":584,\"y\":9,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_QEAAMPSBPR\",\"page\":1,\"spuid\":\"QEAAMPSBPR\",\"tabindex\":4,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"Fine\",\"w\":64,\"x\":656,\"y\":9,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":102,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"NNOTBPRYAG\",\"type\":\"BodyDetail\",\"w\":768,\"x\":8,\"y\":25,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"DatiAna\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Dati Anagrafici\",\"h\":54,\"name\":\"DatiAna\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VRCVINYRJO\",\"stretch\":true,\"tabindex\":7,\"title_caption\":\"Dati Anagrafici\",\"titled\":true,\"type\":\"Box\",\"w\":728,\"x\":8,\"y\":135,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NAKGRMOUXS\",\"page\":1,\"spuid\":\"NAKGRMOUXS\",\"tabindex\":8,\"type\":\"Label\",\"w\":648,\"x\":16,\"y\":144,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HMPQLYVATL\",\"page\":1,\"spuid\":\"HMPQLYVATL\",\"tabindex\":9,\"type\":\"Label\",\"w\":648,\"x\":16,\"y\":165,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Prog\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Progressivi Informazione\",\"h\":45,\"name\":\"Prog\",\"page\":1,\"shrinkable\":true,\"spuid\":\"MINAAOAOZC\",\"stretch\":true,\"tabindex\":12,\"title_caption\":\"Progressivi Informazione\",\"titled\":true,\"type\":\"Box\",\"w\":728,\"x\":8,\"y\":189,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BHLAQHLOJQ\",\"page\":1,\"spuid\":\"BHLAQHLOJQ\",\"tabindex\":13,\"type\":\"Label\",\"value\":\"Prog.Informazione Apertura:\",\"w\":147,\"x\":40,\"y\":207,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_EQUZNNIXBO\",\"page\":1,\"spuid\":\"EQUZNNIXBO\",\"tabindex\":14,\"type\":\"Label\",\"value\":\"Prog.Informazione Chiusura:\",\"w\":163,\"x\":488,\"y\":207,\"zone\":\"pag1_4_2\",\"zonepath\":\"[1,4,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Prog.Informazione Chiusura\",\"disabled\":\"true\",\"fixed\":true,\"h\":23,\"maxlength\":11,\"name\":\"NUMPROG2\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"ORIJTEDVRI\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":656,\"y\":200,\"zone\":\"pag1_4_3\",\"zonepath\":\"[1,4,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Prog.Informazione Apertura\",\"disabled\":\"true\",\"fixed\":true,\"h\":23,\"maxlength\":11,\"name\":\"NUMPROG1\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"LEZBMECOAR\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":80,\"x\":192,\"y\":200,\"zone\":\"pag1_4_4\",\"zonepath\":\"[1,4,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":39,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"YFCAXGURUJ\",\"type\":\"Box\",\"w\":712,\"x\":8,\"y\":6,\"zone\":\"pag1_5\",\"zonepath\":\"[1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Ragione Sociale\",\"disabled\":\"true\",\"h\":17,\"maxlength\":40,\"name\":\"xragsoci\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"ZUSOGKEVZC\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":432,\"x\":144,\"y\":28,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Inizio\",\"h\":17,\"maxlength\":10,\"name\":\"DATAINI\",\"page\":1,\"repeated\":true,\"spuid\":\"HKZAUFLHMG\",\"tabindex\":2,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":64,\"x\":584,\"y\":27,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Fine\",\"h\":17,\"maxlength\":10,\"name\":\"DATAFINE\",\"page\":1,\"repeated\":true,\"spuid\":\"NATIUIQLJX\",\"tabindex\":3,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":64,\"x\":656,\"y\":27,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Intestatario (Non Intermediario)\",\"h\":17,\"maxlength\":16,\"name\":\"CODINTER\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"XTDHYWJPZL\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":108,\"x\":8,\"y\":28,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_ae_intestboBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_ae_intestboBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_ae_intestbo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_ae_intestbo_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_ae_intestbo_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_ae_intestbo_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(ardt_ae_intestboBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_ae_intestboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_ae_intestboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_ae_intestboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_ae_intestboBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_ae_intestboBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_ae_intestboBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ardt_ae_intestboBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ardt_ae_intestboBO BO) {
    boolean result = true;
  ...
  static class OrderBodyByComparator implements Comparator<ardt_ae_intestboBL.TrsRow> {
    public int compare(ardt_ae_intestboBL.TrsRow o1,ardt_ae_intestboBL.TrsRow o2) {
