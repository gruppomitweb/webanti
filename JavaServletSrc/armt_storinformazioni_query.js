...
WindowAlert(AlertErrorMessage('10431160596'));
}
}
}
var m_bIsMq = false;
...
m_oFather.DoQueryFromChild('wapopebo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
function DoQueryFrom_wntestbo(firstAlias) {
return m_cVirtName+".RAPPORTO = "+firstAlias+".RAPPORTO ";
}
function DoQueryFrom_woginfo(firstAlias) {
...
_chNotF(childAlias,'master','armt_storinformazioni');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wapopebo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
