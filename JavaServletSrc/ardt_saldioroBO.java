...
// * --- Fine Area Manuale
public class ardt_saldioroBO extends ardt_saldioroBL {
  ...
  // * --- Fine Area Manuale
  public ardt_saldioroBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_saldioro");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_saldioro,m_cPhName_saldioro,m_cVirtName_saldioro,m_cVirtName_saldioro+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_saldioro,p_RAPPORTO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"saldioro")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_saldioro.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND ANNO="+CPLib.ToSQL(opk_ANNO,"C",4,0);
        l_cSql = l_cSql+m_oWrInfo_saldioro.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"saldioro")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_saldioro,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_saldioro);
      l_cSql = "insert into "+m_oWrInfo_saldioro.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_saldioro.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"saldioro")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_saldioro.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"saldioro")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_saldioro,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_saldioro);
      l_cSql = "Update "+m_oWrInfo_saldioro.GetTableWriteName()+" set";
