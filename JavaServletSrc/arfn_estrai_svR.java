...
// * --- Fine Area Manuale
public class arfn_estrai_svR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_estrai_svR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_estrai_sv",m_Caller);
    Blank();
  }
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      try {
        // * --- Select from qbe_estrai_sv
        SPBridge.HMCaller _h00000027;
        _h00000027 = new SPBridge.HMCaller();
        _h00000027.Set("m_cVQRList",m_cVQRList);
        _h00000027.Set("pDADATA",_datini);
        _h00000027.Set("pA_DATA",_datfin);
        if (Cursor_qbe_estrai_sv!=null)
          Cursor_qbe_estrai_sv.Close();
        Cursor_qbe_estrai_sv = new VQRHolder("qbe_estrai_sv",com.zucchetti.sitepainter.Library.getVQRReaderFactory(m_Ctx.GetInstance()),_h00000027,true).GetResultSet(m_Ctx);
        while ( ! (Cursor_qbe_estrai_sv.Eof())) {
          /* conta := conta + 1 */
          ...
          if (CPLib.eqr(pSegno,"1")) {
            /* riga := riga + iif(qbe_estrai_sv->SEGNO='A','22','14') */
            riga = riga+(CPLib.eqr(Cursor_qbe_estrai_sv.GetString("SEGNO"),"A")?"22":"14");
            /* ElseIf pSegno='2' */
          } else if (CPLib.eqr(pSegno,"2")) {
            /* riga := riga + iif(qbe_estrai_sv->SEGNO='D','22','14') */
            riga = riga+(CPLib.eqr(Cursor_qbe_estrai_sv.GetString("SEGNO"),"D")?"22":"14");
          } // End If
          /* _importo := Round(qbe_estrai_sv->IMPORTO/100,0) */
          _importo = CPLib.Round(CPLib.Round(Cursor_qbe_estrai_sv.GetDouble("IMPORTO")/100,0),2);
          /* riga := riga + Str(_importo*100,18,0) */
          riga = riga+CPLib.Str(_importo*100,18,0);
          /* riga := riga + Str(qbe_estrai_sv->NUMOPE,10,0) */
          riga = riga+CPLib.Str(Cursor_qbe_estrai_sv.GetDouble("NUMOPE"),10,0);
          /* riga := riga + LibreriaMit.SpacePad(qbe_estrai_sv->SOTGRUP,4) */
          riga = riga+LibreriaMit.SpacePad(Cursor_qbe_estrai_sv.GetString("SOTGRUP"),4);
          /* riga := riga + LibreriaMit.SpacePad(qbe_estrai_sv->PROVINCIA1,5) */
          riga = riga+LibreriaMit.SpacePad(Cursor_qbe_estrai_sv.GetString("PROVINCIA1"),5);
          /* riga := riga + LibreriaMit.SpacePad(qbe_estrai_sv->PAESE,3) */
          riga = riga+LibreriaMit.SpacePad(Cursor_qbe_estrai_sv.GetString("PAESE"),3);
          /* riga := riga + LibreriaMit.SpacePad(qbe_estrai_sv->PROVINCIA,5) */
          riga = riga+LibreriaMit.SpacePad(Cursor_qbe_estrai_sv.GetString("PROVINCIA"),5);
          /* riga := riga + LibreriaMit.SpacePad(qbe_estrai_sv->TIPOSV,3) */
          riga = riga+LibreriaMit.SpacePad(Cursor_qbe_estrai_sv.GetString("TIPOSV"),3);
          /* riga := riga + LibreriaMit.SpacePad(qbe_estrai_sv->TIPOCONT,3) */
          riga = riga+LibreriaMit.SpacePad(Cursor_qbe_estrai_sv.GetString("TIPOCONT"),3);
          /* riga := riga + LibreriaMit.SpacePad(qbe_estrai_sv->VALUTA,1) */
          riga = riga+LibreriaMit.SpacePad(Cursor_qbe_estrai_sv.GetString("VALUTA"),1);
          /* FileLibMit.WriteLine(fhand,riga) */
          FileLibMit.WriteLine(fhand,riga);
          Cursor_qbe_estrai_sv.Next();
        }
        m_cConnectivityError = Cursor_qbe_estrai_sv.ErrorMessage();
        Cursor_qbe_estrai_sv.Close();
        // * --- End Select
  ...
  public String RunAsync(double p_pAnno,double p_pTri,String p_pSegno) {
    pAnno = p_pAnno;
    pTri = p_pTri;
    pSegno = p_pSegno;
  ...
  public String Run(double p_pAnno,double p_pTri,String p_pSegno) {
    pAnno = p_pAnno;
    pTri = p_pTri;
    pSegno = p_pSegno;
    return Run();
  ...
  public static arfn_estrai_svR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_estrai_svR(p_Ctx, p_Caller);
  }
  public static arfn_estrai_svR Make(CPContext p_Ctx) {
    return new arfn_estrai_svR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
    pAnno = 0;
    pTri = 0;
  ...
  // * --- Fine Area Manuale
  // ENTITY_VQR: ,qbe_estrai_sv,
  public static final String m_cVQRList = ",qbe_estrai_sv,";
  // ENTITY_BATCHES: ,arfn_estrai_sv,arfn_fdatetime,
  public static final String i_InvokedRoutines = ",arfn_estrai_sv,arfn_fdatetime,";
  public static String[] m_cRunParameterNames={"pAnno","pTri","pSegno"};
}
