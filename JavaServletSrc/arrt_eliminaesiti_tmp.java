...
public class arrt_eliminaesiti_tmp extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*8c58cbbe*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_eliminaesiti_tmp";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedgMsgProc) ||  ! (status.m_bpassedw_msgproc) ||  ! (status.m_bpassedw_scelta);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_eliminaesiti_tmpR Rcalled;
    Rcalled = new arrt_eliminaesiti_tmpR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_eliminaesiti_tmpR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_eliminaesiti_tmpR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_eliminaesiti_tmp"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_eliminaesiti_tmp");
      if ( ! (CPLib.IsNull(l))) {
