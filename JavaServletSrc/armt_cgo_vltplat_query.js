...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('ZGGIFYYPOE').value,'C'),w_VPCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_vltplat',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'VPCODICE') &&  ! (Empty(HtW(Ctrl('ZGGIFYYPOE').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.VPCODICE',HtW(Ctrl('ZGGIFYYPOE').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_vltplat');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_vltplat',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['ZGGIFYYPOE'],i=0;i>=0;i--){
