...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('UZYCBNGKAF').value,'C'),w_TIPDOC)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_tipdoc',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'TIPDOC') &&  ! (Empty(HtW(Ctrl('UZYCBNGKAF').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.TIPDOC',HtW(Ctrl('UZYCBNGKAF').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_tipdoc');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_tipdoc',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['UZYCBNGKAF'],i=0;i>=0;i--){
