...
public class arpg_stpoper_sa extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*GEJXYHKPUD*!";
  ...
  public String getEntityTitle() {
    return "Stampa Operazioni con Dettaglio Anagrafico (Pagina)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_stpoper_sa";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 797;
    l_eg.m_nEntityHeight = 590;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("OrderBy","C",22,0, "edit","")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_stpoper_saBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"1929092706",p_Context.Translate("Stampa Operazioni con Dettaglio Anagrafico (Pagina)")}};
    return l_translations;
  }
  void WriteStyles(arpg_stpoper_saBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_stpoper_sa.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_stpoper_saBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_stpoper_saBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_stpoper_saBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(585);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_stpoper_saBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<input id='JCIARNNFGA_0' name='JCIARNNFGA' type='radio' value="+SPLib.ToHTMLValue("DATAOPE,NUMPROG,IDBASE","C",22,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq("DATAOPE,NUMPROG,IDBASE",CPLib.Trim(BO.w_OrderBy))?"CHECKED":"")+">" +
     "<label class='SPLabel' for='JCIARNNFGA_0'>"+status.context.Translate("Data Operazione")+"</label>" +
     "</div>" +
     "<div id='JCIARNNFGA_1_container'>" +
     "<input id='JCIARNNFGA_1' name='JCIARNNFGA' type='radio' value="+SPLib.ToHTMLValue("DATAREG,NUMPROG,IDBASE","C",22,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq("DATAREG,NUMPROG,IDBASE",CPLib.Trim(BO.w_OrderBy))?"CHECKED":"")+">" +
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<a href='javascript:href_WBCVTRNSQS()' tabindex='"+status.m_nTabIndex+"' id='WBCVTRNSQS_HREF' curTabIndex='"+status.m_nTabIndex+"'>" +
     "<span id='WBCVTRNSQS'>" +
  ...
  static void PrintState(arpg_stpoper_saBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_stpoper_saBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_stpoper_saBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_OrderBy,"C",22,0));
    status.out.println(")");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      status.out.println("m_bUpdated="+SPLib.ToJSValue(BO.m_bUpdated,"L",0,0)+";");
    }
  }
  void WriteMainFormVarScript(arpg_stpoper_saBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(590,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_stpoper_sa"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_stpoper_sa","");
  ...
  void WriteMainFormScript(arpg_stpoper_saBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_stpoper_saBO getBO(ServletStatus status) {
    arpg_stpoper_saBO BO = new arpg_stpoper_saBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_stpoper_saBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_stpoper_saBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_stpoper_saBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_stpoper_saBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_stpoper_saBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_stpoper_saBO BO) {
    ...
    item.put("altInterfaceFor","arpg_stpoper_sa");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"576\",\"name\":\"Box_XLVFHSOSYH\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"XLVFHSOSYH\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"784\",\"x\":\"8\",\"y\":\"9\",\"zone\":\"pag1_1\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"ADatOpe\",\"page\":\"1\",\"spuid\":\"MTJDZQTEIR\",\"tabindex\":\"3\",\"type\":\"Textbox\",\"w\":\"72\",\"x\":\"144\",\"y\":\"45\",\"zone\":\"XLVFHSOSYH_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Data_Operazione_\",\"page\":\"1\",\"spuid\":\"HPGJXQIVYG_DIV\",\"tabindex\":\"4\",\"type\":\"Label\",\"value\":\"Da Data Operazione:\",\"w\":\"124\",\"x\":\"16\",\"y\":\"22\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Data_Operazione_\",\"page\":\"1\",\"spuid\":\"BGHAITOPMU_DIV\",\"tabindex\":\"5\",\"type\":\"Label\",\"value\":\"A Data Operazione:\",\"w\":\"124\",\"x\":\"16\",\"y\":\"49\",\"zone\":\"XLVFHSOSYH_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"19\",\"href\":\" \",\"name\":\"OPZIONI_DI_STAMPA\",\"page\":\"1\",\"spuid\":\"JIVTSBWFAR_DIV\",\"tabindex\":\"10\",\"type\":\"Label\",\"value\":\"OPZIONI DI STAMPA\",\"w\":\"784\",\"x\":\"8\",\"y\":\"135\",\"zone\":\"XLVFHSOSYH_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"daCodCli\",\"page\":\"1\",\"spuid\":\"UYVYTEMEXX\",\"tabindex\":\"11\",\"type\":\"Textbox\",\"w\":\"112\",\"x\":\"144\",\"y\":\"72\",\"zone\":\"XLVFHSOSYH_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"aCodCli\",\"page\":\"1\",\"spuid\":\"CJWUHWULAR\",\"tabindex\":\"12\",\"type\":\"Textbox\",\"w\":\"112\",\"x\":\"144\",\"y\":\"100\",\"zone\":\"XLVFHSOSYH_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"53\",\"name\":\"OrderBy\",\"orientation\":\"vertical\",\"page\":\"1\",\"spuid\":\"JCIARNNFGA\",\"tabindex\":\"13\",\"textlist\":\"Data Operazione,Data Registrazione\",\"type\":\"Radio\",\"valuelist\":\""+"'"+"DATAOPE,NUMPROG,IDBASE"+"'"+","+"'"+"DATAREG,NUMPROG,IDBASE"+"'"+"\",\"w\":\"129\",\"x\":\"16\",\"y\":\"189\",\"zone\":\"XLVFHSOSYH_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"19\",\"href\":\" \",\"name\":\"ORDINAMENTO\",\"page\":\"1\",\"spuid\":\"LKERTFTGKI_DIV\",\"tabindex\":\"14\",\"type\":\"Label\",\"value\":\"ORDINAMENTO\",\"w\":\"136\",\"x\":\"16\",\"y\":\"162\",\"zone\":\"XLVFHSOSYH_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Cliente_\",\"page\":\"1\",\"spuid\":\"PCILMATBXL_DIV\",\"tabindex\":\"15\",\"type\":\"Label\",\"value\":\"Da Cliente:\",\"w\":\"61\",\"x\":\"80\",\"y\":\"76\",\"zone\":\"XLVFHSOSYH_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_RAGSOC1\",\"page\":\"1\",\"spuid\":\"VXJAZUGUIO_DIV\",\"tabindex\":\"16\",\"type\":\"Label\",\"value\":\"w_RAGSOC1\",\"w\":\"392\",\"x\":\"280\",\"y\":\"76\",\"zone\":\"XLVFHSOSYH_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Cliente_\",\"page\":\"1\",\"spuid\":\"JGQCNSZOCQ_DIV\",\"tabindex\":\"17\",\"type\":\"Label\",\"value\":\"A Cliente:\",\"w\":\"61\",\"x\":\"80\",\"y\":\"104\",\"zone\":\"XLVFHSOSYH_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_RAGSOC2\",\"page\":\"1\",\"spuid\":\"XCBBNKIAUV_DIV\",\"tabindex\":\"18\",\"type\":\"Label\",\"value\":\"w_RAGSOC2\",\"w\":\"392\",\"x\":\"280\",\"y\":\"104\",\"zone\":\"XLVFHSOSYH_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_XURLGQZZMU()\",\"name\":\"_60165\",\"page\":\"1\",\"spuid\":\"XURLGQZZMU\",\"tabindex\":\"19\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"288\",\"y\":\"228\",\"zone\":\"XLVFHSOSYH_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_WBCVTRNSQS()\",\"name\":\"_59890\",\"page\":\"1\",\"spuid\":\"WBCVTRNSQS\",\"tabindex\":\"20\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"392\",\"y\":\"228\",\"zone\":\"XLVFHSOSYH_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"279\",\"href\":\" \",\"name\":\"__iframe_width__775__height__290__id__elaborazione___name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"FGMOCXNBFY_DIV\",\"tabindex\":\"21\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"775"+"'"+" height="+"'"+"290"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"768\",\"x\":\"16\",\"y\":\"297\",\"zone\":\"XLVFHSOSYH_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_stpoper_saBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_stpoper_saBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_stpoper_saBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_stpoper_sa?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_stpoper_sa_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(590,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
  ...
  void RaiseLoadedEntity(arpg_stpoper_saBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_stpoper_saBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_stpoper_saBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_stpoper_saBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_stpoper_saBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_stpoper_saBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_stpoper_sa.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_stpoper_saBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_stpoper_saBO BO) {
    boolean result = true;
    return result;
  }
}
