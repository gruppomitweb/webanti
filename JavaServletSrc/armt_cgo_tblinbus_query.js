...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('FTQDGTJKVC').value,'C'),w_CODLINBUS)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_tblinbus',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODLINBUS') &&  ! (Empty(HtW(Ctrl('FTQDGTJKVC').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODLINBUS',HtW(Ctrl('FTQDGTJKVC').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_tblinbus');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_tblinbus',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['FTQDGTJKVC'],i=0;i>=0;i--){
