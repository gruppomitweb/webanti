...
public class arpg_cgo_rapporti_err extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*NDGQYDPTWP*!";
  ...
  public String getEntityTitle() {
    return "Correzione Rapporti Scartati (CGO)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_cgo_rapporti_err";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 682;
    ...
    l_eg.m_cQueryName = "qbe_cgo_rapporti_err";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_cgo_rapporti_errBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"1017725727",p_Context.Translate("Rapporto (Conto di Gioco)")},
    {"10988377690",p_Context.Translate("Data Oper.")},
    {"113933141",p_Context.Translate("Update")},
    {"94330374",p_Context.Translate("Correzione Rapporti Scartati (CGO)")}};
    return l_translations;
  }
  void WriteStyles(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_cgo_rapporti_err.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_cgo_rapporti_errBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_cgo_rapporti_errBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_cgo_rapporti_errBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(682,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_cgo_rapporti_err"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_cgo_rapporti_err","");
  ...
  void WriteMainFormScript(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_cgo_rapporti_errBO getBO(ServletStatus status) {
    arpg_cgo_rapporti_errBO BO = new arpg_cgo_rapporti_errBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_cgo_rapporti_errBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    ...
    item.put("altInterfaceFor","arpg_cgo_rapporti_err");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    status.out.println("var c_PRGRAP;");
    status.out.println("var c_RAPPORTO;");
    status.out.println("var c_DESCRAP;");
    status.out.println("var c_DATAOPE;");
    status.out.println("var c_CODINTEST;");
    status.out.println("var c_TIPOMOV;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_cgo_rapporti_errBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_PRGRAP,"C",15,0);
    ...
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_CODINTEST,"C",16,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_TIPOMOV,"C",1,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_cgo_rapporti_err?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_cgo_rapporti_err_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_cgo_rapporti_errBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_cgo_rapporti_errBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_cgo_rapporti_err_load.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_cgo_rapporti_errBO BO) {
    boolean result = true;
    return result;
  }
}
