...
public class arrt_sistemat extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*494e73f9*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_tipoarch) ||  ! (status.m_bpassedw_msgproc) ||  ! (status.m_bpassedw_datreg) ||  ! (status.m_bpassedw_datchi);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_sistematR Rcalled;
    Rcalled = new arrt_sistematR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_sistematR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_sistematR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_sistemat"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_sistemat");
      if ( ! (CPLib.IsNull(l))) {
