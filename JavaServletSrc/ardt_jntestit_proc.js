...
SPBOUILib.SetInputValue('CHJPRCYMKT',function(){return WtH(w_PRGIMPTITE,'N',15,0,'')},w_PRGIMPTITE);
...
var h;
SetDisplay('VRCVINYRJO_DIV',IsHiddenByStateDriver('DatiAN'));
SetDisplay(Ctrl('QPLXZEEVEI'),true || IsHiddenByStateDriver('CODCLI'));
SetDisplay('MINAAOAOZC_DIV',IsHiddenByStateDriver('Prog'));
...
HideUI.lblids["DESCINTER"]=['RUCUNEMGZA'];
HideUI.lblids["NUMPROG1"]=['BHLAQHLOJQ'];
HideUI.lblids["NUMPROG2"]=['EQUZNNIXBO'];
...
SPBOUILib.SetInputExit('CHJPRCYMKT',CHJPRCYMKT_wrap_Valid);
function CHJPRCYMKT_wrap_OnFocus(event) {
return CHJPRCYMKT_OnFocus(event);
}
SPBOUILib.SetInputEnter('CHJPRCYMKT',CHJPRCYMKT_wrap_OnFocus);
function CHJPRCYMKT_wrap_OnKeyPress(event) {
return CheckNum(event);
}
SPBOUILib.SetInputKeyPress('CHJPRCYMKT',CHJPRCYMKT_wrap_OnKeyPress);
function CHJPRCYMKT_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('CHJPRCYMKT',CHJPRCYMKT_wrap_OnKeyDown);
function QDIDYGGMQV_wrap_Valid(event) {
return QDIDYGGMQV_Valid(event);
}
SPBOUILib.SetInputExit('QDIDYGGMQV',QDIDYGGMQV_wrap_Valid);
function QDIDYGGMQV_wrap_OnFocus(event) {
return QDIDYGGMQV_OnFocus(event);
}
SPBOUILib.SetInputEnter('QDIDYGGMQV',QDIDYGGMQV_wrap_OnFocus);
function QDIDYGGMQV_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('QDIDYGGMQV',QDIDYGGMQV_wrap_OnKeyDown);
if (m_bExtended) {
...
m_oFather.i_last_focused_item='child_ardt_jntestit';
...
function Help() {
windowOpenForeground('../doc/ardt_jntestit_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
GetLayerID.Layers.push({"id":"VRCVINYRJO","name":"DatiAN"});
GetLayerID.Layers.push({"id":"MINAAOAOZC","name":"Prog"});
...
return 'jntestit'+'\\'+CPLib.ToCPStr(w_CODCLI);
}
...
p_d.setValue('ardt_jntestit',m_cSelectedPage);
