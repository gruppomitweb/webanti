...
// * --- Fine Area Manuale
public class armt_tbtipattBO extends armt_tbtipattBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbtipattBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbtipatt");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbtipatt,m_cPhName_tbtipatt,m_cVirtName_tbtipatt,m_cVirtName_tbtipatt+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",10,0),m_cServer_tbtipatt,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        w_FLGALTO = l_rsLoad.GetDouble("FLGALTO");
        // Assegna i valori degli op_
        SPExtenderLib.LoadExtendedFields(l_rsLoad,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipatt")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbtipatt);
      l_cSql = "insert into "+m_oWrInfo_tbtipatt.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+"RISCHIO,";
      l_cSql = l_cSql+"FLGALTO,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbtipatt.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipatt")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_DESCRI,"C",250,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_RISCHIO,"N",3,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_FLGALTO,"N",1,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbtipatt.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipatt")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbtipatt,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbtipatt);
      l_cSql = "Update "+m_oWrInfo_tbtipatt.GetTableWriteName()+" set";
      l_cSql = l_cSql+" DESCRI = "+CPLib.ToSQL(w_DESCRI,"C",250,0);
      l_cSql = l_cSql+", RISCHIO = "+CPLib.ToSQL(w_RISCHIO,"N",3,0);
      l_cSql = l_cSql+", FLGALTO = "+CPLib.ToSQL(w_FLGALTO,"N",1,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipatt")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",10,0),m_cServer_tbtipatt,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbtipatt.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipatt")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbtipatt.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",10,0),m_cServer_tbtipatt,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbtipatt.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipatt")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbtipatt,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
