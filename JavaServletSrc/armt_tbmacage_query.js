...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('LVIYDAHUQL').value,'C'),w_MACODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbmacage',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'MACODICE') &&  ! (Empty(HtW(Ctrl('LVIYDAHUQL').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.MACODICE',HtW(Ctrl('LVIYDAHUQL').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_ageresleg(firstAlias) {
return m_cVirtName+".MACODICE = "+firstAlias+".CODMAGE ";
}
...
_chNotF(childAlias,'master','armt_tbmacage');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbmacage',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['LVIYDAHUQL'],i=0;i>=0;i--){
