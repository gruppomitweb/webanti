...
public class arpg_imp_ariu_m extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*IAKZUWQZUX*!";
  ...
  public String getEntityTitle() {
    return "Import da ARIU_M";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_imp_ariu_m";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_imp_ariu_mBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11671968134",p_Context.Translate("Import da ARIU_M")}};
    return l_translations;
  }
  void WriteStyles(arpg_imp_ariu_mBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_imp_ariu_m.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_imp_ariu_mBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_imp_ariu_mBO BO,ServletStatus status) {
  ...
  // GetPortlet_Object_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_imp_ariu_mBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_imp_ariu_mBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_imp_ariu_mBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(322);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_imp_ariu_mBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_imp_ariu_mBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_imp_ariu_mBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_imp_ariu_mBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_imp_ariu_mBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_imp_ariu_m"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_imp_ariu_m","");
  ...
  void WriteMainFormScript(arpg_imp_ariu_mBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_imp_ariu_m");
  ...
  arpg_imp_ariu_mBO getBO(ServletStatus status) {
    arpg_imp_ariu_mBO BO = new arpg_imp_ariu_mBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_imp_ariu_mBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_imp_ariu_mBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_imp_ariu_mBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_imp_ariu_mBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_imp_ariu_mBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_imp_ariu_mBO BO) {
    ...
    item.put("altInterfaceFor","arpg_imp_ariu_m");
    items.put(item);
    String s;
    ...
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"25\",\"href\":\"javascript:PIRPTHHLHK_Click()\",\"name\":\"Esegui\",\"page\":\"1\",\"spuid\":\"PIRPTHHLHK\",\"tabindex\":\"2\",\"type\":\"Button\",\"value\":\"Esegui\",\"w\":\"88\",\"x\":\"688\",\"y\":\"17\",\"zone\":\"YWGFSYCMOZ_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"169\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"56\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_imp_ariu_mBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_imp_ariu_mBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_imp_ariu_mBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_imp_ariu_m?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_imp_ariu_m_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_imp_ariu_mBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_imp_ariu_mBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_imp_ariu_mBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_imp_ariu_mBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_imp_ariu_mBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_imp_ariu_mBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_imp_ariu_m.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_imp_ariu_mBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_imp_ariu_mBO BO) {
    boolean result = true;
    return result;
  }
}
