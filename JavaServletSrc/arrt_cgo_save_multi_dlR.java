...
// * --- Fine Area Manuale
public class arrt_cgo_save_multi_dlR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_save_multi_dlR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_save_multi_dl",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      Cursor_cgo_operazioni_dl_multi_dt = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"SNAINUMOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_SNAINUMOPE,"?",0,0,m_cServer, m_oParameters),m_cServer,w_SNAINUMOPE)+" "+")"+(m_Ctx.IsSharedTemp("cgo_operazioni_dl_multi_dt")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
      while ( ! (Cursor_cgo_operazioni_dl_multi_dt.Eof())) {
        /* If cgo_operazioni_dl_multi_dt->FLGFULL='N' */
        if (CPLib.eqr(Cursor_cgo_operazioni_dl_multi_dt.GetString("FLGFULL"),"N")) {
          /* _omulti := 'N' */
          _omulti = "N";
        } // End If
      ...
      if (CPLib.eqr(_omulti,"S")) {
        ...
        while ( ! (Cursor_cgo_operazioni_dl_multi.Eof())) {
          ...
          /* _tiporap := '' */
          _tiporap = "";
          /* If not(Empty(LRTrim(cgo_operazioni_dl_multi->CODFISC))) */
          if ( ! (CPLib.Empty(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi.GetString("CODFISC"))))) {
            // * --- Select from intestbo
            m_cServer = m_Ctx.GetServer("intestbo");
            m_cPhName = m_Ctx.GetPhName("intestbo");
            if (Cursor_intestbo!=null)
              Cursor_intestbo.Close();
            Cursor_intestbo = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"CODINTER="+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CODFISC"),"?",0,0)+" "+")"+(m_Ctx.IsSharedTemp("intestbo")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
            while ( ! (Cursor_intestbo.Eof())) {
              ...
              if ( ! (Read_Cursor.Eof())) {
                ...
                Read_Cursor.Next();
                if ( ! (Read_Cursor.Eof())) {
                  Error l_oErrorFault = new Error("Read on anarapbo into routine arrt_cgo_save_multi_dl returns two or more records. This item should return only a record.");
            ...
            if (CPLib.Empty(CPLib.LRTrim(_rapporto))) {
              ...
              Cursor_xntestbo = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"CODINTER="+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CODFISC"),"?",0,0)+" "+")"+(m_Ctx.IsSharedTemp("xntestbo")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
              while ( ! (Cursor_xntestbo.Eof())) {
                ...
                if ( ! (Read_Cursor.Eof())) {
                  ...
                  Read_Cursor.Next();
                  if ( ! (Read_Cursor.Eof())) {
                    Error l_oErrorFault = new Error("Read on xnarapbo into routine arrt_cgo_save_multi_dl returns two or more records. This item should return only a record.");
                    CPStdCounter.Error(l_oErrorFault);
                  }
                  Read_Cursor.Close();
                } else {
          ...
          /* _tipope := '' */
          _tipope = "";
          /* _mezpagam := iif(LRTrim(cgo_operazioni_dl_multi->MEZPAGAM) <> 'I' and LRTrim(cgo_operazioni_dl_multi->MEZPAGAM) <> 'E' and LRTrim(cgo_operazioni_dl_multi->MEZPAGAM) <> 'P','A',LRTrim(cgo_operazioni_dl_multi->MEZPAGAM)) */
          _mezpagam = (CPLib.ne(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi.GetString("MEZPAGAM")),"I") && CPLib.ne(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi.GetString("MEZPAGAM")),"E") && CPLib.ne(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi.GetString("MEZPAGAM")),"P")?"A":CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi.GetString("MEZPAGAM")));
          // * --- Read from cgo_causali
          m_cServer = m_Ctx.GetServer("cgo_causali");
          m_cPhName = m_Ctx.GetPhName("cgo_causali");
          m_cSql = "";
          m_cSql = m_cSql+"CACODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetDouble("CACODICE"),"N",5,0,m_cServer),m_cServer,Cursor_cgo_operazioni_dl_multi.GetDouble("CACODICE"));
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on cgo_causali into routine arrt_cgo_save_multi_dl returns two or more records. This item should return only a record.");
              CPStdCounter.Error(l_oErrorFault);
            }
            Read_Cursor.Close();
          } else {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_save_multi_dl",38,"00000044")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetDateTime("CADATAORA"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CAFLGOPE"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetDouble("CAPV_COD"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetDouble("CATIPCON"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetDouble("CATIPGIO"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CC4CIFRE"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetDouble("CCANNSCA"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CCDATSCA"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetDouble("CCMESSCA"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.DateToChar(Cursor_cgo_operazioni_dl_multi.GetDate("DATAOPE")),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetDate("CGODATCON"),"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL("S","?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CGOESICON"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CGOFLGVLT"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CGOFLGVNP"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CGONOTCON"),"?",0,0,m_cServer, m_oParameters)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CGOTIPOPE"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("CODLOB"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("FLGDOCALL"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("IBAN"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("IDDOCALL"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("NOTE"),"?",0,0,m_cServer, m_oParameters)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("RDCODAUT"),"?",0,0,m_cServer)+", ";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi.GetString("TIPOGIOCO"),"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL("1","?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(_tipope,"?",0,0,m_cServer)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL("M","?",0,0,m_cServer)+", ";
          ...
          Cursor_cgo_operazioni_dl_multi_dt = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"SNAINUMOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_SNAINUMOPE,"?",0,0,m_cServer, m_oParameters),m_cServer,w_SNAINUMOPE)+" "+")"+(m_Ctx.IsSharedTemp("cgo_operazioni_dl_multi_dt")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
          while ( ! (Cursor_cgo_operazioni_dl_multi_dt.Eof())) {
            /* If not(Empty(LRTrim(cgo_operazioni_dl_multi_dt->NUMOPEDL))) */
            if ( ! (CPLib.Empty(CPLib.LRTrim(Cursor_cgo_operazioni_dl_multi_dt.GetString("NUMOPEDL"))))) {
              /* xSNAINUMOPE := '' */
              xSNAINUMOPE = "";
              ...
              m_cPhName = m_Ctx.GetPhName("cgo_operazioni");
              m_cSql = "";
              m_cSql = m_cSql+"SNAINUMOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dt.GetString("NUMOPEDL"),"C",15,0,m_cServer),m_cServer,Cursor_cgo_operazioni_dl_multi_dt.GetString("NUMOPEDL"));
              ...
              if ( ! (Read_Cursor.Eof())) {
                ...
                Read_Cursor.Next();
                if ( ! (Read_Cursor.Eof())) {
                  Error l_oErrorFault = new Error("Read on cgo_operazioni into routine arrt_cgo_save_multi_dl returns two or more records. This item should return only a record.");
                  CPStdCounter.Error(l_oErrorFault);
                }
                Read_Cursor.Close();
              } else {
              ...
              if ( ! (CPLib.Empty(xSNAINUMOPE))) {
                ...
                m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"cgo_operazioni",m_Ctx.GetCompany());
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_save_multi_dl",38,"0000004A")+"delete from "+m_oWrInfo.GetTableWriteName();
                m_cSql = m_cSql+" where ";
                m_cSql = m_cSql+"SNAINUMOPE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dt.GetString("NUMOPEDL"),"?",0,0,m_cServer),m_cServer,Cursor_cgo_operazioni_dl_multi_dt.GetString("NUMOPEDL"))+"";
                ...
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_save_multi_dl",38,"0000004B")+"delete from "+m_oWrInfo.GetTableWriteName();
                m_cSql = m_cSql+" where ";
                m_cSql = m_cSql+"SNAINUMOPE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dt.GetString("NUMOPEDL"),"?",0,0,m_cServer),m_cServer,Cursor_cgo_operazioni_dl_multi_dt.GetString("NUMOPEDL"))+"";
                m_cSql = m_cSql+m_oWrInfo.WhereFilter();
                m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql,m_oParameters);
                ...
                m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
                m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_save_multi_dl",38,"0000004C")+"delete from "+m_oWrInfo.GetTableWriteName();
                m_cSql = m_cSql+" where ";
                m_cSql = m_cSql+"SNAINUMOPE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(Cursor_cgo_operazioni_dl_multi_dt.GetString("NUMOPEDL"),"?",0,0,m_cServer),m_cServer,Cursor_cgo_operazioni_dl_multi_dt.GetString("NUMOPEDL"))+"";
                m_cSql = m_cSql+m_oWrInfo.WhereFilter();
                m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql,m_oParameters);
  ...
  public static arrt_cgo_save_multi_dlR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_save_multi_dlR(p_Ctx, p_Caller);
  }
  public void Blank() {
