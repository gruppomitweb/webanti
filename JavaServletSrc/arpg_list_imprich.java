...
public class arpg_list_imprich extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*EIGRWZOPCL*!";
  ...
  public String getEntityTitle() {
    return "Elenco file da importare";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_list_imprich";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 613;
    l_eg.m_nEntityHeight = 49;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    ...
    l_eg.m_cQueryName = "qbe_list_imprich";
    l_eg.m_nRowsPerPage = 6;
    return l_eg;
  }
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_list_imprichBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"106712731",p_Context.Translate("Elenco file da importare")},
    {"113933141",p_Context.Translate("Update")}};
    return l_translations;
  }
  void WriteStyles(arpg_list_imprichBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_list_imprich.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_list_imprichBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_list_imprichBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_list_imprichBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_list_imprichBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_list_imprichBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_list_imprichBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_list_imprichBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(49,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(613,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_list_imprich"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_list_imprich","");
  ...
  void WriteMainFormScript(arpg_list_imprichBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_list_imprichBO getBO(ServletStatus status) {
    arpg_list_imprichBO BO = new arpg_list_imprichBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_list_imprichBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_list_imprichBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_list_imprichBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_list_imprichBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_list_imprichBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_list_imprichBO BO) {
    ...
    item.put("altInterfaceFor","arpg_list_imprich");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_list_imprichBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_list_imprichBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_list_imprichBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_nomefile,"C",100,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_list_imprichBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_list_imprichBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_list_imprichBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_list_imprich?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_list_imprich_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(49,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void RaiseLoadedEntity(arpg_list_imprichBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_list_imprichBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_list_imprichBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_list_imprichBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_list_imprichBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_list_imprichBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_list_imprichBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_list_imprichBO BO) {
    boolean result = true;
    return result;
  }
}
