...
public class arrt_inviomese_delope_globale extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*fa8d74b5*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_inviomese_delope_globale";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_dadatag) ||  ! (status.m_bpassedw_a_datag);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_inviomese_delope_globaleR Rcalled;
    Rcalled = new arrt_inviomese_delope_globaleR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_inviomese_delope_globaleR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_inviomese_delope_globaleR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_inviomese_delope_globale"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_inviomese_delope_globale");
      if ( ! (CPLib.IsNull(l))) {
