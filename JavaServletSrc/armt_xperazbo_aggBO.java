...
// * --- Fine Area Manuale
public class armt_xperazbo_aggBO extends armt_xperazbo_aggBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_xperazbo_aggBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_xperazbo_agg");
    }
  ...
  public boolean Load(String p_IDBASE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_xperazbo_agg,m_cPhName_xperazbo_agg,m_cVirtName_xperazbo_agg,m_cVirtName_xperazbo_agg+".IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDBASE,"C",10,0),m_cServer_xperazbo_agg,p_IDBASE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xperazbo_agg")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xperazbo_agg);
      l_cSql = "insert into "+m_oWrInfo_xperazbo_agg.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_xperazbo_agg.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xperazbo_agg")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_xperazbo_agg.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xperazbo_agg")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xperazbo_agg,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xperazbo_agg);
      l_cSql = "Update "+m_oWrInfo_xperazbo_agg.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_IDBASE,"C",10,0),m_cServer_xperazbo_agg,w_IDBASE);
      l_cSql = l_cSql+m_oWrInfo_xperazbo_agg.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xperazbo_agg")) {
  ...
  public boolean Delete(String p_IDBASE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_xperazbo_agg.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_IDBASE,"C",10,0),m_cServer_xperazbo_agg,w_IDBASE);
          l_cSql = l_cSql+m_oWrInfo_xperazbo_agg.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"xperazbo_agg")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_xperazbo_agg,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
