...
// * --- Fine Area Manuale
public class arrt_cgo_delreg_multiR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_cgo_delreg_multiR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_cgo_delreg_multi",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_delreg_multi",37,"00000003")+"delete from "+m_oWrInfo.GetTableWriteName();
    m_cSql = m_cSql+" where ";
    ...
    m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
    m_cSql = CPLib.GetSQLPrefixComment("arrt_cgo_delreg_multi",37,"00000004")+"delete from "+m_oWrInfo.GetTableWriteName();
    m_cSql = m_cSql+" where ";
    m_cSql = m_cSql+"SNAINUMOPE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(pSNAINUMOPE,"?",0,0,m_cServer),m_cServer,pSNAINUMOPE)+"";
    m_cSql = m_cSql+m_oWrInfo.WhereFilter();
    m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql,m_oParameters);
    ...
    /* Stop arpg_cgo_operazioni_agenzie_dl_multipli */
    Forward f;
    f=new Forward("arpg_cgo_operazioni_agenzie_dl_multipli",false,this,Forward.Start,false);
  ...
  public static arrt_cgo_delreg_multiR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_cgo_delreg_multiR(p_Ctx, p_Caller);
  }
  public void Blank() {
