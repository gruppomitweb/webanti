...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('KYSFYTLMNO').value,'C'),w_TIPLEG)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbtipleg',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'TIPLEG') &&  ! (Empty(HtW(Ctrl('KYSFYTLMNO').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.TIPLEG',HtW(Ctrl('KYSFYTLMNO').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tipileg');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbtipleg',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['KYSFYTLMNO'],i=0;i>=0;i--){
