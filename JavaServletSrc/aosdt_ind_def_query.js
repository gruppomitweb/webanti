...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('UUNKNVCACF').value,'C'),w_IDMOD)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('ATQBDCKYMD').value,'C'),w_IDIND)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('ind_def',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDMOD') &&  ! (Empty(HtW(Ctrl('UUNKNVCACF').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDMOD',HtW(Ctrl('UUNKNVCACF').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_sub_ind_def(firstAlias) {
return m_cVirtName+".IDMOD = "+firstAlias+".IDMOD and "+m_cVirtName+".IDIND = "+firstAlias+".IDIND ";
}
...
_chNotF(childAlias,'detail','aosdt_ind_def');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('ind_def',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['UUNKNVCACF'],i=0;i>=0;i--){
