...
public class arrt_copiarapotit extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "ADEEYLLMDY";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_NUMPROG) ||  ! (status.m_bpassedw_DATAOPE) ||  ! (status.m_bpassedw_DATARETT) ||  ! (status.m_bpassedw_OPERAZMOD) ||  ! (status.m_bpassedw_IDBASE);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_copiarapotitR Rcalled;
    Rcalled = new arrt_copiarapotitR(status.context,status);
  ...
  Caller RoutineInstance(SPServletStatus status,Caller dataFromClient) {
    return new arrt_copiarapotitR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_copiarapotitR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (status.m_bError) {
      ...
      status.out.println(
       "<body onkeydown='ManageKeys(event,false,false)'>" +
      "");
    } else {
      status.out.println(
       "<body>" +
    ...
    if (status.m_bError) {
      status.out.println(
       "<script type='text/javascript'>" +
      "");
      l_cErrMsg = status.context.Translate(status.m_cErrMsg);
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      status.out.println(
       "<!--" +
      "");
      status.out.println("Function return value:"+CPLib.ToProperties(Rcalled.RunAsync())+"");
      status.out.println(
       " -->" +
      "");
    } else {
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_copiarapotit"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
        ...
        status.out.println(" window[f].w_DATARETT.setTime(Date.parse("+SPLib.ToJSValue(status.w_DATARETT,"D",0,0)+"))");
        status.out.println("}");
        status.out.println("if (Ne(window[f].w_DATAOPE,null)) {");
        status.out.println(" window[f].w_DATAOPE.setTime(Date.parse("+SPLib.ToJSValue(status.w_DATAOPE,"D",0,0)+"))");
        status.out.println("}");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_copiarapotit");
      if ( ! (CPLib.IsNull(l))) {
