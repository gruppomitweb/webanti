...
public class arrt_estrai_bp_check_wu extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*f46a4cf3*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_estrai_bp_check_wu";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_estrai_bp_check_wuR Rcalled;
    Rcalled = new arrt_estrai_bp_check_wuR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaa_retval = Rcalled.Run(status.pAnno,status.pMese,status.pTipo);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_estrai_bp_check_wuR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_estrai_bp_check_wuR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_estrai_bp_check_wu"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
