...
public class arpg_xntestit_sel extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*CYSYDJSZGH*!";
  ...
  public String getEntityTitle() {
    return "Titolari Effettivi (Prov.)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_xntestit_sel";
    ...
    l_eg.m_cQueryName = "qbe_xntestit_sel";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_xntestit_selBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10358490344",p_Context.Translate("Titolari Effettivi (Prov.)")},
    {"10930911516",p_Context.Translate("Titolare")},
  ...
  void WriteStyles(arpg_xntestit_selBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_xntestit_sel.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_xntestit_selBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_xntestit_selBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_xntestit_selBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_xntestit_selBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_xntestit_selBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_xntestit_selBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_xntestit_selBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_xntestit_sel"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_xntestit_sel","");
  ...
  void WriteMainFormScript(arpg_xntestit_selBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_xntestit_selBO getBO(ServletStatus status) {
    arpg_xntestit_selBO BO = new arpg_xntestit_selBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_xntestit_selBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_xntestit_selBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_xntestit_selBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_xntestit_selBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_xntestit_selBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_xntestit_selBO BO) {
    ...
    item.put("altInterfaceFor","arpg_xntestit_sel");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_xntestit_selBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_xntestit_selBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_xntestit_selBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_xntestit_selBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_xntestit_selBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_xntestit_selBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_xntestit_sel?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_xntestit_sel_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_xntestit_selBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_xntestit_selBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_xntestit_selBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_xntestit_selBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_xntestit_selBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_xntestit_selBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_xntestit_selBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_xntestit_selBO BO) {
    boolean result = true;
    return result;
  }
}
