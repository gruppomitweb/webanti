...
// * --- Fine Area Manuale
public class ardt_xlientiopeBO extends ardt_xlientiopeBL {
  ...
  // * --- Fine Area Manuale
  public ardt_xlientiopeBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_xlientiope");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xlientiope,m_cPhName_xlientiope,m_cVirtName_xlientiope,m_cVirtName_xlientiope+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_xlientiope,p_IDFILEBO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xlientiope")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          w_IMPSARA = l_rsLoadBody.GetDouble("IMPSARA");
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_xlientiope.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND CODSOG="+CPLib.ToSQL(opk_CODSOG,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_xlientiope.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xlientiope")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_xlientiope,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xlientiope);
      l_cSql = "insert into "+m_oWrInfo_xlientiope.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_xlientiope.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xlientiope")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_xlientiope.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xlientiope")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xlientiope,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xlientiope);
      l_cSql = "Update "+m_oWrInfo_xlientiope.GetTableWriteName()+" set";
