...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('UJDYOPDWZF').value,'C'),w_RAPCOL)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('joginfo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'RAPCOL') &&  ! (Empty(HtW(Ctrl('UJDYOPDWZF').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.RAPCOL',HtW(Ctrl('UJDYOPDWZF').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'detail','ardt_ae_soginfo');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('joginfo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['UJDYOPDWZF'],i=0;i>=0;i--){
