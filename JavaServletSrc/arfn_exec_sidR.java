...
import org.json.*;
// * --- Fine Area Manuale
public class arfn_exec_sidR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_exec_sidR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_exec_sid",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on cpwarn into routine arfn_exec_sid returns two or more records. This item should return only a record.");
      ...
      if (CPLib.Empty(CPLib.LRTrim(_semaforo))) {
        ...
        // Cancella vecchi file generati
        di = new File(CPLib.LRTrim(gPathDoc)+"/SID/INVII/DAP/"+CPLib.LRTrim(gAzienda)+"/");
        File fl_dap[] = di.listFiles();
        count = fl_dap.length;
        /* If count=1 */
        if (CPLib.eqr(count,1)) {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid",335,"00000037")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          if (CPLib.eqr(_tipo,"W")) {
            /* _dir1 := Strtran(LRTrim(gPathDoc),'/','\')+"\SID\INVII\DAP\"+LRTrim(gAzienda)+"\" */
            _dir1 = CPLib.Strtran(CPLib.LRTrim(gPathDoc),"/","\\")+"\\SID\\INVII\\DAP\\"+CPLib.LRTrim(gAzienda)+"\\";
            /* _dir2 := Strtran(LRTrim(gPathDoc),'/','\')+"\SID\INVII\ESI\"+LRTrim(gAzienda)+"\" */
            _dir2 = CPLib.Strtran(CPLib.LRTrim(gPathDoc),"/","\\")+"\\SID\\INVII\\ESI\\"+CPLib.LRTrim(gAzienda)+"\\";
            /* _dir3 := Strtran(LRTrim(gPathDoc),'/','\')+"\SID\INVII\PRE\"+LRTrim(gAzienda)+"\" */
            _dir3 = CPLib.Strtran(CPLib.LRTrim(gPathDoc),"/","\\")+"\\SID\\INVII\\PRE\\"+CPLib.LRTrim(gAzienda)+"\\";
          ...
          jsonUtente.put("ambienteSicurezzaPath",LibreriaMit.UTFtoISO(_dir4));
          /* jsonUtente.put("file_da_predisporre",LibreriaMit.UTFtoISO(_dir1)) */
          jsonUtente.put("file_da_predisporre",LibreriaMit.UTFtoISO(_dir1));
          /* jsonUtente.put("file_da_inviare",LibreriaMit.UTFtoISO(_dir3)) */
          jsonUtente.put("file_da_inviare",LibreriaMit.UTFtoISO(_dir3));
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on tberrorisid into routine arfn_exec_sid returns two or more records. This item should return only a record.");
              CPStdCounter.Error(l_oErrorFault);
            }
            Read_Cursor.Close();
          } else {
          ...
          if (CPLib.eqr(_risultato,"000")) {
            ...
            // These are the files to include in the ZIP file
            filenames.add(CPLib.LRTrim(fl4[_i].getAbsolutePath()));
            fileonames.add(CPLib.LRTrim(fl4[_i].getName()));
            /* _nomefile := LRTrim(fl4[_i].getName()) */
            _nomefile = CPLib.LRTrim(fl4[_i].getName());
            /* If At('PECATZA.',LRTrim(_nomefile)) <> 0 */
            if (CPLib.ne(CPLib.At("PECATZA.",CPLib.LRTrim(_nomefile)),0)) {
              // * --- Write into sid_invio from values
              m_Sql.RequireTransaction();
              ...
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid",335,"0000006E")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
              m_cSql = m_cSql+"FL1INVIO = "+CPLib.ToSQL(_nomefile,"C",80,0,m_cServer)+", ";
              m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"sid_invio",false,true);
              m_cSql = m_cSql+" where ";
              m_cSql = m_cSql+"FLEINVIO = "+CPSql.SQLValueAdapter(CPLib.ToSQL(pFile,"?",0,0,m_cServer),m_cServer,pFile)+"";
              ...
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid",335,"0000006F")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
              m_cSql = m_cSql+"FILEINV1 = "+CPLib.ToSQL(CPLib.Left(CPLib.LRTrim(_nomefile),50),"C",50,0,m_cServer)+", ";
              m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"aesaldi",false,true);
              m_cSql = m_cSql+" where ";
              m_cSql = m_cSql+"NOMEFILE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.Left(pFile,50),"?",0,0,m_cServer),m_cServer,CPLib.Left(pFile,50))+"";
              m_cSql = m_cSql+" and ANNO = "+CPSql.SQLValueAdapter(CPLib.ToSQL(pAnno,"?",0,0,m_cServer),m_cServer,pAnno)+"";
              m_cSql = m_cSql+m_oWrInfo.WhereFilter();
              m_nUpdatedRows = m_Sql.Update(m_cServer,m_cSql,m_oParameters);
              if (CPLib.lt(m_nUpdatedRows,0)) {
                m_Sql.AbortTransaction();
              ...
              m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"sid_invio",m_Ctx.GetCompany());
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid",335,"00000070")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
              m_cSql = m_cSql+"FL2INVIO = "+CPLib.ToSQL(_nomefile,"C",80,0,m_cServer)+", ";
              m_cSql = m_cSql+"FLGINVIO = "+CPLib.ToSQL("S","C",1,0,m_cServer)+", ";
              m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"sid_invio",false,true);
              m_cSql = m_cSql+" where ";
              m_cSql = m_cSql+"FLEINVIO = "+CPSql.SQLValueAdapter(CPLib.ToSQL(pFile,"?",0,0,m_cServer),m_cServer,pFile)+"";
              m_cSql = m_cSql+m_oWrInfo.WhereFilter();
              ...
              m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"aesaldi",m_Ctx.GetCompany());
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid",335,"00000071")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
              m_cSql = m_cSql+"FILEINV2 = "+CPLib.ToSQL(CPLib.Left(CPLib.LRTrim(_nomefile),50),"C",50,0,m_cServer)+", ";
              m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"aesaldi",false,true);
              m_cSql = m_cSql+" where ";
              m_cSql = m_cSql+"NOMEFILE = "+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.Left(pFile,50),"?",0,0,m_cServer),m_cServer,CPLib.Left(pFile,50))+"";
              m_cSql = m_cSql+" and ANNO = "+CPSql.SQLValueAdapter(CPLib.ToSQL(pAnno,"?",0,0,m_cServer),m_cServer,pAnno)+"";
            ...
            m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
            m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid",335,"00000073")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
            m_cSql = m_cSql+GetFieldsName_00000073(m_Ctx,m_oWrInfo);
            m_cSql = m_cSql+") values (";
            m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(pFile)+".zip","?",0,0,m_cServer)+", ";
            m_cSql = m_cSql+""+CPLib.ToSQL(1,"?",0,0)+", ";
            m_cSql = m_cSql+""+CPLib.ToSQL(CPLib.LRTrim(gUrlApp)+"flussi/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(pFile)+".zip","?",0,0,m_cServer)+", ";
            m_cSql = m_cSql+m_oWrInfo.InsertValues();
            ...
            /* cNomeFileZip := LRTrim(gPathDoc)+"/flussi/"+LRTrim(gAzienda)+"/"+LRTrim(pFile)+".zip" */
            cNomeFileZip = CPLib.LRTrim(gPathDoc)+"/flussi/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(pFile)+".zip";
            /* Exec "Creazione ZIP con i file" Page 4:Page_4 */
            Page_4();
            /* gMsgProc := gMsgProc + 'Terminata Elaborazione File!'+NL // Log Messaggi */
            gMsgProc = gMsgProc+"Terminata Elaborazione File!"+"\n";
            m_Ctx.SetGlobalString("gMsgProc",gMsgProc);
            /* _uscita := '0' */
            _uscita = "0";
            /* ElseIf _risultato <> '000'  */
          } else if (CPLib.ne(_risultato,"000")) {
            /* mcRitorno.AppendBlank() */
            mcRitorno.AppendBlank();
            /* mcRitorno.testata := 'ESITO:'+_risultato + " - "+_categsid + " - " +_descrsid */
            mcRitorno.row.testata = "ESITO:"+_risultato+" - "+_categsid+" - "+_descrsid;
            /* mcRitorno.SaveRow() */
            mcRitorno.SaveRow();
            /* fhand := FileLibMit.OpenReadPath(LRTrim(gPathDoc)+"/SID/INVII/ESI/"+LRTrim(gAzienda)+"/"+LRTrim(_filedgn)) // Handle File */
            fhand = FileLibMit.OpenReadPath(CPLib.LRTrim(gPathDoc)+"/SID/INVII/ESI/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(_filedgn));
            /* While not(FileLibMit.Eof(fhand)) */
            while ( ! (FileLibMit.Eof(fhand))) {
              /* _riga := FileLibMit.ReadLine(fhand) */
              _riga = FileLibMit.ReadLine(fhand);
              /* If Len(LRTrim(_riga)) > 0 and Left(_riga,1)='B' */
              if (CPLib.gt(CPLib.Len(CPLib.LRTrim(_riga)),0) && CPLib.eqr(CPLib.Left(_riga,1),"B")) {
                ...
                m_cSql = m_cSql+" and DGNCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.Substr(_riga,15,3),"C",3,0,m_cServer),m_cServer,CPLib.Substr(_riga,15,3));
                ...
                if ( ! (Read_Cursor.Eof())) {
                  ...
                  Read_Cursor.Next();
                  if ( ! (Read_Cursor.Eof())) {
                    Error l_oErrorFault = new Error("Read on tberrsiddgn into routine arfn_exec_sid returns two or more records. This item should return only a record.");
                    CPStdCounter.Error(l_oErrorFault);
                  }
                  Read_Cursor.Close();
                } else {
                ...
                /* mcRitorno.AppendBlank() */
                mcRitorno.AppendBlank();
                /* mcRitorno.testata := iif(Left(_riga,1)='A','ESITO:','ERRORE: ')+_risultato + " - "+_categsid + " - " +_descrsid */
                mcRitorno.row.testata = (CPLib.eqr(CPLib.Left(_riga,1),"A")?"ESITO:":"ERRORE: ")+_risultato+" - "+_categsid+" - "+_descrsid;
                /* mcRitorno.SaveRow() */
                mcRitorno.SaveRow();
                /* mcRitorno.AppendBlank() */
                mcRitorno.AppendBlank();
                /* mcRitorno.testata := 'Tipo Riga: '+Substr(_riga,11,1)+' - N. Record: '+Substr(_riga,2,9) +' - Posizione:'+Substr(_riga,12,3) */
                mcRitorno.row.testata = "Tipo Riga: "+CPLib.Substr(_riga,11,1)+" - N. Record: "+CPLib.Substr(_riga,2,9)+" - Posizione:"+CPLib.Substr(_riga,12,3);
                /* mcRitorno.SaveRow() */
                mcRitorno.SaveRow();
              } // End If
            } // End While
          ...
          /* FileLibMit.DeleteFilePath(LRTrim(gPathDoc)+'/SID/INVII/DAP/'+LRTrim(gAzienda)+"/"+LRTrim(pFile)) */
          FileLibMit.DeleteFilePath(CPLib.LRTrim(gPathDoc)+"/SID/INVII/DAP/"+CPLib.LRTrim(gAzienda)+"/"+CPLib.LRTrim(pFile));
          // * --- Delete from cpwarn
          m_Sql.RequireTransaction();
          m_cServer = m_Ctx.GetServer("cpwarn");
          m_cPhName = m_Ctx.GetPhName("cpwarn");
          m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"cpwarn",m_Ctx.GetCompany());
          m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
          m_cSql = CPLib.GetSQLPrefixComment("arfn_exec_sid",335,"0000008D")+"delete from "+m_oWrInfo.GetTableWriteName();
          m_cSql = m_cSql+" where ";
          ...
          /* mcRitorno.AppendBlank() */
          mcRitorno.AppendBlank();
          /* mcRitorno.testata := "Numero Eccessivo di file da elaborare - Rilevati "+LRTrim(Str(count,2,0))+" file." */
          mcRitorno.row.testata = "Numero Eccessivo di file da elaborare - Rilevati "+CPLib.LRTrim(CPLib.Str(count,2,0))+" file.";
          /* mcRitorno.SaveRow() */
          mcRitorno.SaveRow();
        } // End If
      } else { // Else
  ...
  void Page_3() throws Exception {
    ...
    {
      File _mydir = new File(CPLib.LRTrim(_dirsid)+"/prog");
      //String[] cmd = {"sh", "-c",CPLib.LRTrim(_dirsid)+"/prog/runPredisposizioneMIT.sh",CPLib.LRTrim(_dirjava)+"/bin/java",CPLib.LRTrim(CPLib.Upper(_pwdsog))};
  ...
  public static arfn_exec_sidR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_exec_sidR(p_Ctx, p_Caller);
  }
  public static arfn_exec_sidR Make(CPContext p_Ctx) {
    return new arfn_exec_sidR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
