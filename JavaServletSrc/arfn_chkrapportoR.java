...
// * --- Fine Area Manuale
public class arfn_chkrapportoR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_chkrapportoR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_chkrapporto",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    ...
    if (CPLib.eqr("xRappo",p_cVarName)) {
      return w_xRappo;
    }
    return "";
  }
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* pRapporto Char(25) */
      /* Variabili locali */
      /* w_xRappo Char(25) // Codice rapporto */
      /* bRitorno Bool */
      ...
      if ( ! (Read_Cursor.Eof())) {
        w_xRappo = Read_Cursor.GetString("RAPPORTO");
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on anarapbo into routine arfn_chkrapporto returns two or more records. This item should return only a record.");
      ...
      /* If not(Empty(w_xRappo)) */
      if ( ! (CPLib.Empty(w_xRappo))) {
        /* bRitorno := False */
        bRitorno = false;
      } else { // Else
        // * --- Read from xnarapbo
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on xnarapbo into routine arfn_chkrapporto returns two or more records. This item should return only a record.");
            CPStdCounter.Error(l_oErrorFault);
          }
          Read_Cursor.Close();
        } else {
      ...
      if ( ! (bRitorno)) {
        // Transaction Error
        m_Sql.AbortTransactionMarkExplicit(CPLib.FormatMsg(m_Ctx,"Esiste già un rapporto con lo stesso codice"));
      } // End If
    } finally {
      try {
        if (Read_Cursor!=null)
  ...
  public Forward Run(String p_pRapporto) {
    pRapporto = p_pRapporto;
    return Run();
  }
  public Forward Run() {
  ...
  public static arfn_chkrapportoR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_chkrapportoR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pRapporto = CPLib.Space(25);
    w_xRappo = CPLib.Space(25);
    bRitorno = false;
  }
