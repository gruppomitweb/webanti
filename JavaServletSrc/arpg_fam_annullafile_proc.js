...
if (false || WindowConfirm(Translate('10542056431'))) {
SetLocationHref(null,'arrt_fam_annullafile'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
}
...
w_Messaggi.raiseEvent('arpg_fam_annullafile_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_fam_annullafile_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_fam_annullafile',m_cSelectedPage);
...
l_Appl.SetFields('NOMEFILE,ANNO,IDBASE,MESE,TIPOINVIO,TIPOCOMU,CODINVIO');
...
w_anno=l_Appl.GetStringValue('ANNO',4,0);
w_idbase=l_Appl.GetStringValue('IDBASE',10,0);
w_mese=l_Appl.GetStringValue('MESE',2,0);
w_TIPOINVIO=l_Appl.GetStringValue('TIPOINVIO',1,0);
w_TIPOCOMU=l_Appl.GetStringValue('TIPOCOMU',1,0);
w_CODINVIO=l_Appl.GetStringValue('CODINVIO',12,0);
} else {
...
l_cFieldFilter=l_cFieldFilter+l_cAndOpForField+_flt_('ANNULLATO',0,null,true,'=');
if ( ! (Empty(l_cFieldFilter))) {
l_cAndOpForField=' and ';
}
...
l_oWv.setValue('CODINVIO',WtA(w_CODINVIO,'C'));
l_oWv.setValue('mese',WtA(w_mese,'C'));
l_oWv.setValue('TIPOINVIO',WtA(w_TIPOINVIO,'C'));
l_oWv.setValue('TIPOCOMU',WtA(w_TIPOCOMU,'C'));
_FillChildren(l_oWv);
}
_FillChildren.n=[];
FillWv.n=["nomefile","anno","idbase","CODINVIO","mese","TIPOINVIO","TIPOCOMU"];
