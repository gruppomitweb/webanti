...
// * --- Fine Area Manuale
public class ardt_ae_intestboBO extends ardt_ae_intestboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_ae_intestboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_ae_intestbo");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_jntestbo+".CPROWNUM";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_jntestbo,m_cPhName_jntestbo,m_cVirtName_jntestbo,m_cVirtName_jntestbo+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_jntestbo,p_RAPPORTO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"jntestbo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_jntestbo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_jntestbo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"jntestbo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_jntestbo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_jntestbo);
      l_cSql = "insert into "+m_oWrInfo_jntestbo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_jntestbo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"jntestbo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_jntestbo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"jntestbo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_jntestbo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_jntestbo);
      l_cSql = "Update "+m_oWrInfo_jntestbo.GetTableWriteName()+" set";
