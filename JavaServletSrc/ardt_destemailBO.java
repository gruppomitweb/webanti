...
// * --- Fine Area Manuale
public class ardt_destemailBO extends ardt_destemailBL {
  ...
  // * --- Fine Area Manuale
  public ardt_destemailBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_destemail");
    }
  ...
  public static String[] m_cLoadParameterNames = {"DWCODINT"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_DWCODINT) {
    CPResultSet l_rsLoadBody=null;
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_destwarn+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_destwarn,m_cPhName_destwarn,m_cVirtName_destwarn,m_cVirtName_destwarn+".DWCODINT="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_DWCODINT,"C",11,0),m_cServer_destwarn,p_DWCODINT),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_DWNOMINATIVO = l_rsLoadBody.GetString("DWNOMINATIVO");
          w_DWEMAIL = l_rsLoadBody.GetString("DWEMAIL");
          w_CPROWNUM = l_rsLoadBody.GetDouble("CPROWNUM");
          w_DWFLG380 = l_rsLoadBody.GetString("DWFLG380");
          w_DWFLGVLT = l_rsLoadBody.GetString("DWFLGVLT");
          w_DWFLGQADV = l_rsLoadBody.GetString("DWFLGQADV");
  ...
  public boolean Delete(String p_DWCODINT) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_destwarn.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"DWCODINT="+CPLib.ToSQL(w_DWCODINT,"C",11,0)+" AND CPROWNUM="+CPLib.ToSQL(w_CPROWNUM,"N",3,0);
        l_cSql = l_cSql+m_oWrInfo_destwarn.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"destwarn")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_destwarn,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_destwarn);
      l_cSql = "insert into "+m_oWrInfo_destwarn.GetTableWriteName()+" (";
      l_cSql = l_cSql+"DWCODINT,";
      l_cSql = l_cSql+"DWNOMINATIVO,";
      l_cSql = l_cSql+"DWEMAIL,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+"DWFLG380,";
      l_cSql = l_cSql+"DWFLGVLT,";
      l_cSql = l_cSql+"DWFLGQADV,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_destwarn.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"destwarn")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_DWCODINT,"C",11,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DWNOMINATIVO,"C",50,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DWEMAIL,"C",50,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_CPROWNUM,"N",3,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DWFLG380,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DWFLGVLT,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DWFLGQADV,"C",1,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_destwarn.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"destwarn")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_destwarn,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_destwarn);
      l_cSql = "Update "+m_oWrInfo_destwarn.GetTableWriteName()+" set";
      l_cSql = l_cSql+" DWNOMINATIVO = "+CPLib.ToSQL(w_DWNOMINATIVO,"C",50,0);
      l_cSql = l_cSql+", DWEMAIL = "+CPLib.ToSQL(w_DWEMAIL,"C",50,0);
      l_cSql = l_cSql+", DWFLG380 = "+CPLib.ToSQL(w_DWFLG380,"C",1,0);
      l_cSql = l_cSql+", DWFLGVLT = "+CPLib.ToSQL(w_DWFLGVLT,"C",1,0);
      l_cSql = l_cSql+", DWFLGQADV = "+CPLib.ToSQL(w_DWFLGQADV,"C",1,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"destwarn")) {
