...
_modifyandopen('arrt_discouic'+'?m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes,height=500,width=600',null)
}
}
...
_modifyandopen('arrt_discouic_ver'+'?m_cAction=start&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes',null)
}
_ResetTracker();
...
function Help() {
windowOpenForeground('../doc/arpg_discouic_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_discouic',m_cSelectedPage);
...
SetDisabled(Ctrl('RROILCAWGO'),l_bEnabled);
DisableUI();
...
if (l_bResult) {
l_bResult=Le(w_ADatOpe,CharToDate('20111231'));
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='ADatOpe';
m_cLastMsgError=AlertErrorMessage('11474139734');
}
if ( ! (l_bResult)) {
w_ADatOpe=HtW('','D');
}
...
ResetErrorVars();
m_nLastError=0;
if (( ! (_ChkObl('DPKVLWHCIG',1))) || ( ! (Empty(w_ADatOpe) || Le(w_DaDatOpe,w_ADatOpe)))) {
_SignErr('DPKVLWHCIG','11533347096');
l_bResult=false;
w_DaDatOpe=HtW('','D');
} else if (( ! (_ChkObl('MTJDZQTEIR',1))) || ( ! (Le(w_ADatOpe,CharToDate('20111231'))))) {
_SignErr('MTJDZQTEIR','11474139734');
l_bResult=false;
w_ADatOpe=HtW('','D');
...
_FillChildren.n=[];
FillWv.n=["gIntemediario","gTipInter","DaDatOpe","ADatOpe","tipocr","codcr","datinv","numinv"];
...
_Obli('datinv',2,'FAWXPLZBPU',true,null,null,"1581346268")
