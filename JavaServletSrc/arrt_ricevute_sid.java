...
public class arrt_ricevute_sid extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*a19c626f*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_anno) ||  ! (status.m_bpassedw_nomefile);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_ricevute_sidR Rcalled;
    Rcalled = new arrt_ricevute_sidR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_ricevute_sidR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_ricevute_sidR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_ricevute_sid"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_ricevute_sid");
      if ( ! (CPLib.IsNull(l))) {
