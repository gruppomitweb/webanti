...
public class arpg_statoidrapprv extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*IWCLQXSHGU*!";
  ...
  public String getEntityTitle() {
    return "Stato Intermediari / Delegati";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_statoidrapprv";
    ...
    l_eg.m_cQueryName = "qbe_statorapprv";
    return l_eg;
  }
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_statoidrapprvBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10722030546",p_Context.Translate("Stato Intermediari / Delegati")},
    {"113933141",p_Context.Translate("Update")},
    {"12105997361",p_Context.Translate("Intestatario / Delegato")},
  ...
  void WriteStyles(arpg_statoidrapprvBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_statoidrapprv.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_statoidrapprvBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_statoidrapprvBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_statoidrapprvBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_statoidrapprvBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_statoidrapprvBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_statoidrapprvBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_statoidrapprvBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_statoidrapprvBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_statoidrapprv"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_statoidrapprv","");
    ...
    status.out.println("// * --- arpg_statoidrapprv");
  ...
  void WriteMainFormScript(arpg_statoidrapprvBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_statoidrapprvBO getBO(ServletStatus status) {
    arpg_statoidrapprvBO BO = new arpg_statoidrapprvBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_statoidrapprvBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_statoidrapprvBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_statoidrapprvBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_statoidrapprvBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_statoidrapprvBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_statoidrapprvBO BO) {
    ...
    item.put("altInterfaceFor","arpg_statoidrapprv");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_statoidrapprvBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_statoidrapprvBO BO) {
    status.out.println("var c_CODINTER;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_statoidrapprvBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_CODINTER,"C",16,0);
    ...
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_STATORAP,"C",6,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_TIPOPERS,"C",1,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_statoidrapprvBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_statoidrapprvBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_statoidrapprvBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_statoidrapprv?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_statoidrapprv_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_statoidrapprvBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_statoidrapprvBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_statoidrapprvBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_statoidrapprvBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_statoidrapprvBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_statoidrapprvBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_statoidrapprvBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_statoidrapprvBO BO) {
    boolean result = true;
    return result;
  }
}
