...
public class ardt_informazioniBL extends ardt_informazioniWV implements CPLib.QueryFilterFromApplication {
  ...
  public static final String i_EntityName = "ardt_informazioni";
  ...
  public CPPhTableWrInfo m_oWrInfo_intestbo;
  // Variabile che contiene il nome virtuale della tabella
  public String m_cVirtName_intestbo;
  ...
  public static String[] m_EntityTables = {"intestbo"};
  // codice secondo sintassi delle business rules per produrre i dati da usare in un ideale documento che li rappresenti
  public static final String i_StateStructure = "{'RAPPORTO':RAPPORTO,'m_Detail':{'CODINTER':CODINTER,'DATAINI':DATAINI,'DATAFINE':DATAFINE,'TIPORAP':TIPORAP,'PROQUOTA':PROQUOTA"+FoundlingChildrenList("ardt_informazioni",true,true)+"}"+FoundlingChildrenList("ardt_informazioni",true,false)+"}";
  public static final String i_CompleteStateStructure = "{'gAzienda':gAzienda,'IDBASE':IDBASE,'tipreg':tipreg,'appoggio':appoggio,'RAPPORTO':RAPPORTO,'NUMPROG':NUMPROG,'m_Detail':{'CODINTER':CODINTER,'DATAINI':DATAINI,'DATAFINE':DATAFINE,'TIPOPERS':TIPOPERS,'xragsoci':xragsoci,'tiporegr':tiporegr,'DOMICILIO':DOMICILIO,'DESCCIT':DESCCIT,'PROVINCIA':PROVINCIA,'CAP':CAP,'PRGIMPSOG':PRGIMPSOG,'STATOREG':STATOREG,'IDEREG':IDEREG,'TIPORAP':TIPORAP,'NUMPROG2':NUMPROG2,'NUMPROG1':NUMPROG1,'xRagSoc':xRagSoc,'PROQUOTA':PROQUOTA"+FoundlingChildrenList("ardt_informazioni",true,true)+"}"+FoundlingChildrenList("ardt_informazioni",true,false)+"}";
  public static final String i_ItemSequence = "RAPPORTO,m_Detail[ CODINTER DATAINI DATAFINE TIPORAP PROQUOTA"+FoundlingChildrenList("ardt_informazioni",false,true)+"]"+FoundlingChildrenList("ardt_informazioni",false,false)+"";
  public static final com.zucchetti.sitepainter.EntityItem[] i_EditableEntityItems = com.zucchetti.sitepainter.EntityItem.buildEditableItemsForEntity(GetLinkDef(),FoundlingChildrenList("ardt_informazioni"),new String[][]{new String[]{"RAPPORTO","Rapporto (Copia della testata)","C","25","0","campo","key1","false","false"},new String[]{"CODINTER","Intestatario (Non Intermediario)","C","16","0","campo","key2","true","false"},new String[]{"DATAINI","Data Inizio","D","8","0","campo","key7","true","true"},new String[]{"DATAFINE","Data Fine","D","8","0","campo","key8","true","true"},new String[]{"TIPORAP","Tipo Rapporto","C","1","0","campo","nokey","true","false"},new String[]{"PROQUOTA","% Pro Quota","N","3","0","campo","nokey","true","false"}});
  ...
  public int m_nCPRowNum;
  public ardt_informazioniBL (CPContext p_ContextObject) {
    ...
    m_cPhName_intestbo = p_ContextObject.GetPhName("intestbo",p_ContextObject.GetCompany(),false);
    m_cServer_intestbo = p_ContextObject.GetServer("intestbo");
    m_oWrInfo_intestbo = CPPhTableWrInfo.GetCPPhTableWrInfo(p_ContextObject,"intestbo",p_ContextObject.GetCompany());
    m_cVirtName_intestbo = CPSql.ManipulateTablePhName("intestbo",m_cServer_intestbo);
    ...
    m_MNTs = CPLib.MNTChilds("ardt_informazioni",p_ContextObject);
  }
  public void Blank() {
    ...
    w_NUMPROG = "";
    w_NUMPROG = Utilities.Make(m_Ctx).GetCallerString("NUMPROG");
    Link_XTDHYWJPZL();
    w_tiporegr = 0;
    ...
    w_NUMPROG2 = "";
    w_NUMPROG2 = ( ! (CPLib.Empty(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"))) && CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|26|36|38|46"),0) && CPLib.Empty(w_NUMPROG2) &&  ! (CPLib.Empty(w_DATAFINE))?Utilities.Make(m_Ctx).GetCallerString("NUMPROG"):w_NUMPROG2);
    w_NUMPROG1 = "";
    w_NUMPROG1 = ( ! (CPLib.Empty(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"))) && CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|21|22|23|24|27|33|34|35|43|44|45"),0) && CPLib.Empty(w_NUMPROG1) &&  ! (CPLib.Empty(w_DATAINI))?Utilities.Make(m_Ctx).GetCallerString("NUMPROG"):w_NUMPROG1);
    w_xRagSoc = "";
    w_xRagSoc = ( ! (CPLib.Empty(CPLib.LRTrim(w_xragsoci)))?w_xragsoci:( ! (CPLib.Empty(w_CODINTER))?"Soggetto non presente in anagrafica":""));
  ...
  protected boolean Link_XTDHYWJPZL(String p_cType) {
    ...
    if ( ! (CPLib.Empty(w_CODINTER))) {
      ...
      try {
        ...
        if (CPLib.ne(p_cType,"Load")) {
          if (l_bResult) {
            l_bResult = arfn_chksoggR.Make(m_Ctx,this).Run(w_CODINTER,"7");
            if ( ! (l_bResult)) {
  ...
  public boolean HKZAUFLHMG_isObligatory() {
    // Condizione di obbligatorietà
    return CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|21|22|23|24|27|33|34|35|43|44|45"),0);
  }
  ...
  public boolean NATIUIQLJX_isObligatory() {
    // Condizione di obbligatorietà
    return CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|26|36|38|46"),0);
  }
  ...
  void Calculation_XSORDCKKLR() {
    w_DATAINI = (CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|21|22|23|24|27|33|34|35|43|44|45"),0)?Utilities.Make(m_Ctx).GetCallerDate("DATAOPE"):w_DATAINI);
    w_DATAFINE = (CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|26|36|38|46"),0)?Utilities.Make(m_Ctx).GetCallerDate("DATAOPE"):w_DATAFINE);
  }
  ...
  void Calculation_DKXYLUMNWM() {
    w_NUMPROG1 = ( ! (CPLib.Empty(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"))) && CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|21|22|23|24|27|33|34|35|43|44|45"),0) && CPLib.Empty(w_NUMPROG1)?Utilities.Make(m_Ctx).GetCallerString("NUMPROG"):(CPLib.Empty(w_DATAINI)?"":w_NUMPROG1));
  }
  void Calculation_SWVKAWGUOH() {
    w_NUMPROG2 = ( ! (CPLib.Empty(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"))) && CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|26|36|38|46"),0) && CPLib.Empty(w_NUMPROG2)?Utilities.Make(m_Ctx).GetCallerString("NUMPROG"):(CPLib.Empty(w_DATAFINE)?"":w_NUMPROG2));
  }
  ...
  public static String QueryFilter(CPContext m_Ctx) {
    String l_cFilter;
    l_cFilter = CPLib.ApplicationQueryFilter("ardt_informazioni","intestbo",m_KeyColumns,m_Ctx);
    return l_cFilter;
  }
  ...
  public void SetFromContext(String s,java.util.Properties p) {
    ...
    if ( ! (CPLib.Empty(s))) {
      ...
      Properties p_PROQUOTA;
      p_PROQUOTA = CPLib.GetProperties(trs_p.getProperty("PROQUOTA"),new Properties());
      ...
      while (CPLib.le(i,rows)) {
        ...
        w_PROQUOTA = 0;
        w_PROQUOTA = CPLib.GetProperty(p_PROQUOTA,"",w_PROQUOTA,i);
        w_xragsoci = "";
  ...
  public static Map<String,List<Map<String,String[]>>> GetLinkDef() {
    ...
    List<Map<String,String[]>> items;
    GetLinkDef0(tables);
    for(String child : CPLib.LoadableMNTs("ardt_informazioni",false)) {
      map = new HashMap<String,String[]>();
      ...
      String[][] definitions = CPMNTChild.GetM_Fkd("ardt_informazioni",child);
    ...
    for(String child : CPLib.LoadableMNTs("ardt_informazioni",true)) {
      map = new HashMap<String,String[]>();
      items = tables.get(com.zucchetti.sitepainter.Library.getEntityTables(child)[0]);
      if (CPLib.IsNull(items)) {
        items = new ArrayList<Map<String,String[]>>();
  ...
  public void Initialize() {
    ...
    m_MNTs = CPLib.MNTChilds("ardt_informazioni",m_Ctx);
  ...
  public boolean Check() {
    ...
    l_nRows = Rows();
    if ( ! (CPLib.le(1,l_nRows))) {
      m_nLastError = 2;
      m_cLastMsgError = "MSG_BE_ONE_ROW";
    }
    return l_bResult;
  }
  ...
  public String[] GetStateSecurityKeyRoles(String p_cState) {
    ...
    if (CPLib.eq(p_cState,CPSecurity.UNKNOWN_STATE)) {
      l_Keys = new String[]{"ardt_informazioni"};
    }
    if (CPLib.eq(l_Keys.length,0)) {
      l_Keys = new String[]{"ardt_informazioni\\"+p_cState};
    }
    return l_Keys;
  }
  ...
  public void GetContext(boolean p_bGetChildren,java.util.Properties p) {
    ...
    while (CPLib.lt(i,Rows())) {
      ...
      CPLib.PutProperty(p_PROQUOTA,"",w_PROQUOTA,trsrow);
    ...
    trs_p.setProperty("PROQUOTA",CPLib.ToProperties(p_PROQUOTA));
  ...
  public static String PrimaryKey(Caller p_Caller) {
    return "intestbo"+"\\"+CPLib.ToCPStr(p_Caller.GetString("RAPPORTO","C",25,0));
  }
  public String RowPrimaryKey() {
    return "intestbo"+"\\"+CPLib.ToCPStr(m_nCPRowNum);
  }
  ...
  protected void TrsToWork() {
    ...
    w_PROQUOTA= m_oTrs.get(m_nCurrentRow).w_PROQUOTA;
  ...
  void WorkToTrs() {
    ...
    m_oTrs.get(m_nCurrentRow).w_PROQUOTA=w_PROQUOTA;
  ...
  public void AddTrsDeletedRow() {
    ...
    m_oDeletedRows.get(l_nRows).w_PROQUOTA=w_PROQUOTA;
  ...
  boolean CanSaveConditionallyRow(boolean p_bSaveWork) {
    ...
    if (CPLib.ge(m_nCurrentRow,Rows())) {
      ...
      if ((CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|21|22|23|24|27|33|34|35|43|44|45"),0)) && (CPLib.Empty(w_DATAINI) && HKZAUFLHMG_isObligatory())) {
      ...
      } else if ((CPLib.ne(CPLib.At(Utilities.Make(m_Ctx).GetCallerString("TIPOOPRAP"),"|26|36|38|46"),0)) && (CPLib.Empty(w_DATAFINE) && NATIUIQLJX_isObligatory())) {
  ...
  void SetDeletedRow(double p_nRowNumberToSet) {
    ...
    w_PROQUOTA= m_oDeletedRows.get(l_RowNumberToSet).w_PROQUOTA;
