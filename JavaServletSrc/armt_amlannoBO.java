...
// * --- Fine Area Manuale
public class armt_amlannoBO extends armt_amlannoBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_amlannoBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_amlanno");
    }
  ...
  public boolean Load(String p_IDBASE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_amlanno,m_cPhName_amlanno,m_cVirtName_amlanno,m_cVirtName_amlanno+".IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDBASE,"C",10,0),m_cServer_amlanno,p_IDBASE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"amlanno")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_amlanno);
      l_cSql = "insert into "+m_oWrInfo_amlanno.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDBASE,";
      l_cSql = l_cSql+"ANNO,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_amlanno.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"amlanno")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_amlanno.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"amlanno")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_amlanno,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_amlanno);
      l_cSql = "Update "+m_oWrInfo_amlanno.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_IDBASE,"C",10,0),m_cServer_amlanno,w_IDBASE);
      l_cSql = l_cSql+m_oWrInfo_amlanno.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"amlanno")) {
  ...
  public boolean Delete(String p_IDBASE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_amlanno.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_IDBASE,"C",10,0),m_cServer_amlanno,w_IDBASE);
          l_cSql = l_cSql+m_oWrInfo_amlanno.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"amlanno")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_amlanno,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
