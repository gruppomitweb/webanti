...
var h;
SetDisplay(Ctrl('XURLGQZZMU'),Empty(w_DaDatOpe) || Empty(w_ADatOpe));
HideUI();
}
...
SetLocationHref(null,'arrt_fam_chkperiodo_2015'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
...
w_Messaggi.raiseEvent('arpg_fam_chkperiodo_2015_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_fam_chkperiodo_2015_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_fam_chkperiodo_2015_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_fam_chkperiodo_2015',m_cSelectedPage);
...
if (l_bResult) {
l_bResult=Ge(w_DaDatOpe,CharToDate('20150101')) && Le(w_DaDatOpe,CharToDate('20151231'));
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='DaDatOpe';
m_cLastMsgError=AlertErrorMessage('10144017222');
}
if ( ! (l_bResult)) {
w_DaDatOpe=HtW('','D');
}
...
if (l_bResult) {
l_bResult=Ge(w_ADatOpe,w_DaDatOpe) && Ge(w_ADatOpe,CharToDate('20150101')) && Le(w_ADatOpe,CharToDate('20151231'));
if ( ! (l_bResult)) {
m_nLastError=2;
m_cLastWorkVarErrorMsg='ADatOpe';
m_cLastMsgError=AlertErrorMessage('116209495');
}
if ( ! (l_bResult)) {
w_ADatOpe=HtW('','D');
}
...
ResetErrorVars();
m_nLastError=0;
if (( ! (_ChkObl('DPKVLWHCIG',1))) || ( ! (Ge(w_DaDatOpe,CharToDate('20150101')) && Le(w_DaDatOpe,CharToDate('20151231'))))) {
_SignErr('DPKVLWHCIG','10144017222');
l_bResult=false;
w_DaDatOpe=HtW('','D');
} else if (( ! (_ChkObl('MTJDZQTEIR',1))) || ( ! (Ge(w_ADatOpe,w_DaDatOpe) && Ge(w_ADatOpe,CharToDate('20150101')) && Le(w_ADatOpe,CharToDate('20151231'))))) {
_SignErr('MTJDZQTEIR','116209495');
l_bResult=false;
w_ADatOpe=HtW('','D');
}
...
_FillChildren.n=[];
FillWv.n=["DaDatOpe","ADatOpe","cdadata","c_adata"];
