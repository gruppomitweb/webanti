...
if (false || WindowConfirm(Translate('10740633720'))) {
SetLocationHref(null,'arrt_chk_codfis'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
}
...
w_Messaggi.raiseEvent('arpg_chk_codfis_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_chk_codfis_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_chk_codfis_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_chk_codfis',m_cSelectedPage);
