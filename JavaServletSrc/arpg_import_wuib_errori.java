...
public class arpg_import_wuib_errori extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*MNZBHLQPYT*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_import_wuib_errori";
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("fileerrori","C",50,0, "show",""),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_import_wuib_erroriBO BO) {
  ...
  void WriteStyles(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_import_wuib_errori.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_import_wuib_erroriBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  // GetPortlet_ListaFile_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_MVXXCZOQGP_Vars(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(546);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<a href='javascript:href_DMWAJLXSEC()' tabindex='-1' id='DMWAJLXSEC_HREF'>" +
    ...
    "");
    status.out.println(
     "<input name='fileerrori' id='VVHHSKMSWV' type='text' class='Field Disabled' value="+SPLib.ToHTMLValue(BO.w_fileerrori,"C",50,0,"", status.m_oRegSettings)+" tabindex='-1' readonly maxlength='50' size='50' fieldtype='C' style='display:none'>" +
    "");
    status.themeWriter.Header_EndHtml();
  }
  static void PrintState(arpg_import_wuib_erroriBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_import_wuib_erroriBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_import_wuib_erroriBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_fileerrori,"C",50,0));
  ...
  void WriteMainFormVarScript(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_import_wuib_errori"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_import_wuib_errori","");
  ...
  void WriteMainFormScript(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_import_wuib_errori");
    ...
    status.out.println("}");
    status.out.println("");
    status.out.println("function ScaricaErrori() {");
    status.out.println("  if(Empty(w_fileerrori)) {");
    status.out.println("    alert('Non ci sono file di errori!')");
    status.out.println("  } else {");
    status.out.println("    var filepath=LRTrim(w_gPathApp)+'/output/'+LRTrim(w_gAzienda)+'/'+LRTrim(w_fileerrori)");
    status.out.println("    if (w_gBrowser=='I') {");
    status.out.println("      var stile = 'top=10, left=10, width=640, height=480, status=no, menubar=no, toolbar=no scrollbars=no resizable=yes';  ");
    status.out.println("    } else {");
    status.out.println("      var stile = 'top=10, left=10, width=640, height=480, status=no, menubar=no, toolbar=no scrollbars=no';  ");
    status.out.println("    }   ");
    status.out.println("    window.open(w_gUrlApp+'jsp/downloadXLS.jsp?file='+LRTrim(filepath)+'&fname='+LRTrim(w_fileerrori),'',stile);   ");
    status.out.println("  }  ");
    status.out.println("}");
    status.out.println("// * --- Fine Area Manuale");
    status.out.println(
  ...
  arpg_import_wuib_erroriBO getBO(ServletStatus status) {
    arpg_import_wuib_erroriBO BO = new arpg_import_wuib_erroriBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_import_wuib_erroriBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_import_wuib_erroriBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_import_wuib_erroriBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_import_wuib_erroriBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_import_wuib_erroriBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_import_wuib_erroriBO BO) {
    ...
    item.put("altInterfaceFor","arpg_import_wuib_errori");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"19\",\"name\":\"tipodest\",\"orientation\":\"horizontal\",\"page\":\"1\",\"spuid\":\"WIRRRKLEWO\",\"tabindex\":\"5\",\"textlist\":\"Definitivo,Provvisorio\",\"type\":\"Radio\",\"valuelist\":\""+"'"+"D"+"'"+","+"'"+"P"+"'"+"\",\"w\":\"161\",\"x\":\"88\",\"y\":\"63\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Destinazione_\",\"page\":\"1\",\"spuid\":\"NJQSNRJAXM_DIV\",\"tabindex\":\"6\",\"type\":\"Label\",\"value\":\"Destinazione:\",\"w\":\"65\",\"x\":\"16\",\"y\":\"66\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui elaborazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_XWPYHZIJDE()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"XWPYHZIJDE\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"8\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"688\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Chiudi la maschera\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_JSKZJOHJOM()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"JSKZJOHJOM\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"12\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Preleva i file degli scarti\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_DMWAJLXSEC()\",\"name\":\"images_upload_gif\",\"page\":\"1\",\"spuid\":\"DMWAJLXSEC\",\"src\":\"images\\/upload.gif\",\"tabindex\":\"13\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"640\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"50\",\"name\":\"fileerrori\",\"page\":\"1\",\"spuid\":\"VVHHSKMSWV\",\"type\":\"Textbox\",\"w\":\"200\",\"x\":\"440\",\"y\":\"14\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
  ...
  void Next(ServletStatus status,arpg_import_wuib_erroriBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_import_wuib_erroriBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_import_wuib_errori?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_import_wuib_errori_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_import_wuib_erroriBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_import_wuib_erroriBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_import_wuib_erroriBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_import_wuib_erroriBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_import_wuib_erroriBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_import_wuib_erroriBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_imp_dati_wubs_errori.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_import_wuib_erroriBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_import_wuib_erroriBO BO) {
    boolean result = true;
    return result;
  }
}
