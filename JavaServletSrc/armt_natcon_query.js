...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('EPZTAICZCA').value,'C'),w_TIPONAT)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbtipnat',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'TIPONAT') &&  ! (Empty(HtW(Ctrl('EPZTAICZCA').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.TIPONAT',HtW(Ctrl('EPZTAICZCA').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_natcon');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbtipnat',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['EPZTAICZCA'],i=0;i>=0;i--){
