...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SWIXXXOOHL').value,'N'),w_IBCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_infbacheca',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IBCODICE') &&  ! (Empty(HtW(Ctrl('SWIXXXOOHL').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IBCODICE',HtW(Ctrl('SWIXXXOOHL').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_cgo_infbacheca_doc(firstAlias) {
return m_cVirtName+".IBCODICE = "+firstAlias+".IBDCODICE ";
}
...
_chNotF(childAlias,'master','armt_cgo_infbacheca');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_infbacheca',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['SWIXXXOOHL'],i=0;i>=0;i--){
