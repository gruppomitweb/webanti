...
// * --- Fine Area Manuale
public class arfn_snai_oper_load_multiR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_snai_oper_load_multiR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_snai_oper_load_multi",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arfn_snai_oper_load_multi";
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      Cursor_cgo_operazioni_dl_multi_dc = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"SNAINUMOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(pNumOpe,"?",0,0,m_cServer, m_oParameters),m_cServer,pNumOpe)+" "+")"+(m_Ctx.IsSharedTemp("cgo_operazioni_dl_multi_dc")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
      while ( ! (Cursor_cgo_operazioni_dl_multi_dc.Eof())) {
        /* _importo := cgo_operazioni_dl_multi_dc->TOTLIRE */
        _importo = CPLib.Round(Cursor_cgo_operazioni_dl_multi_dc.GetDouble("TOTLIRE"),2);
        /* _dataope := cgo_operazioni_dl_multi_dc->DATAOPE */
        _dataope = Cursor_cgo_operazioni_dl_multi_dc.GetDate("DATAOPE");
        /* _tipope := cgo_operazioni_dl_multi_dc->CGOTIPOPE */
        _tipope = Cursor_cgo_operazioni_dl_multi_dc.GetString("CGOTIPOPE");
      ...
      Cursor_cgo_operazioni_dl_multi = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"CODDIPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(gCodDip,"?",0,0,m_cServer, m_oParameters),m_cServer,gCodDip)+"  and  DATAOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(_dataope,"?",0,0,m_cServer, m_oParameters),m_cServer,_dataope)+"  and  TOTLIRE="+CPSql.SQLValueAdapter(CPLib.ToSQL(_importo,"?",0,0,m_cServer, m_oParameters),m_cServer,_importo)+"  and  CGOTIPOPE="+CPSql.SQLValueAdapter(CPLib.ToSQL(_tipope,"?",0,0,m_cServer, m_oParameters),m_cServer,_tipope)+" "+")"+(m_Ctx.IsSharedTemp("cgo_operazioni_dl_multi")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
  ...
  public static arfn_snai_oper_load_multiR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_snai_oper_load_multiR(p_Ctx, p_Caller);
  }
  public static arfn_snai_oper_load_multiR Make(CPContext p_Ctx) {
    return new arfn_snai_oper_load_multiR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_snai_oper_load_multi,
  public static final String i_InvokedRoutines = ",arfn_snai_oper_load_multi,";
