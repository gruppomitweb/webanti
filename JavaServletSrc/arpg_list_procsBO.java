...
// * --- Fine Area Manuale
public class arpg_list_procsBO extends arpg_list_procsBL {
  ...
  public arpg_list_procsBO (CPContext p_ContextObject) {
    ...
    if (m_bQuery) {
      while ( ! (l_bEof) &&  ! (m_bExitRow)) {
        ...
        if ( ! (l_bEof) &&  ! (m_bExitRow)) {
          ...
          c_PR_TIPO = rs.GetString("PR_TIPO");
          c_PRCODICE = rs.GetString("PRCODICE");
          c_PRDESCRI = rs.GetString("PRDESCRI");
          c_PRDOSTART = rs.GetDateTime("PRDOSTART");
          c_PRDOSTOP = rs.GetDateTime("PRDOSTOP");
          c_PRESITO = rs.GetString("PRESITO");
          AddTrsRow();
          rs.Next();
          l_nCnt = l_nCnt+1;
        }
