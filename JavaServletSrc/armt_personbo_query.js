...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('YTPWDRMFUS').value,'C'),w_CONNES)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('personbo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CONNES') &&  ! (Empty(HtW(Ctrl('YTPWDRMFUS').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CONNES',HtW(Ctrl('YTPWDRMFUS').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_personbo_dip(firstAlias) {
return m_cVirtName+".CONNES = "+firstAlias+".CONNES ";
}
function DoQueryFrom_personbo_agg(firstAlias) {
return m_cVirtName+".CONNES = "+firstAlias+".CONNES ";
}
function DoQueryFrom_soggope(firstAlias) {
return m_cVirtName+".CONNES = "+firstAlias+".COLLEG ";
}
function DoQueryFrom_fatca_ind(firstAlias) {
return m_cVirtName+".CONNES = "+firstAlias+".CONNES ";
}
function DoQueryFrom_patrimonio(firstAlias) {
return m_cVirtName+".CONNES = "+firstAlias+".CONNES ";
}
function DoQueryFrom_reddito(firstAlias) {
return m_cVirtName+".CONNES = "+firstAlias+".CONNES ";
}
...
_chNotF(childAlias,'master','armt_personbo');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('personbo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['YTPWDRMFUS'],i=0;i>=0;i--){
