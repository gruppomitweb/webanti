...
public class arrt_chg_connes_xls extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*44ba173b*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_chg_connes_xlsR Rcalled;
    Rcalled = new arrt_chg_connes_xlsR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_chg_connes_xlsR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_chg_connes_xlsR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_chg_connes_xls"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
