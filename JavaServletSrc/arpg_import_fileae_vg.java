...
public class arpg_import_fileae_vg extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ZXQLXOODGY*!";
  static public class ServletStatus extends SPPageServletStatus {
    public void MergeCallerVars(SPParameterSource source) {
      ...
      source.SetParameter("msgproc",source.GetCaller().GetString("msgproc","M",10,0));
    }
  }
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_import_fileae_vg";
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("msgproc","M",10,0, "show",""),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_import_fileae_vgBO BO) {
  ...
  void WriteStyles(arpg_import_fileae_vgBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_import_fileae_vg.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_import_fileae_vgBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_import_fileae_vgBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_import_fileae_vgBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(567);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_import_fileae_vgBO BO,ServletStatus status) {
    status.out.println(
    ...
    "");
    status.out.println(
     "<span class='SPLabel' id='KXNOVGROJL_DIV'>" +
     "<label id='KXNOVGROJL'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("MESSAGGI DELLA PROCEDURA"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
     "<span class='SPLabel' id='GORGDGDNGD_DIV'>" +
     "<label id='GORGDGDNGD'>" +
     ""+SPLib.ToHTMLRepresentation("<iframe width='776' height='300' id ='informazioni' name='informazioni' src='../servlet/arpg_list_imprich?nomefile="+BO.w_nomefile+"'"+" frameborder='1'  border='1' align='TOP' marginwidth='1' marginheight='1'></iframe>")+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='SSRCOAMDBL'>" +
     ""+SPLib.ToHTMLRepresentation("<iframe width='776' height='155' id ='messaggi' src='arpg_msg_import' frameborder='1'  border='1' align='TOP' marginwidth='1' marginheight='1'></iframe>")+"" +
     "</label>" +
     "</span>" +
    "");
    status.out.println(
     "<textarea name='msgproc' id='BESVSXEZBJ' class='Memo Disabled' tabindex='-1' readonly cols='20' rows='2'>" +
     "</textarea>" +
    "");
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input id='NCXAJLIDKA' type='button' class='SPButton' value="+SPLib.ToHTMLValue(status.context.Translate("Excel"),"C",0,0)+" title="+SPLib.ToHTMLValue(status.context.Translate("Import dei dati"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' onkeydown='ManageKeys(event,false,false,true)' onclick='NCXAJLIDKA_Click()'>" +
    "");
    status.themeWriter.Header_EndHtml();
  }
  static void PrintState(arpg_import_fileae_vgBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_import_fileae_vgBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_import_fileae_vgBL BO,ServletStatus status) {
    status.out.print("DeclareWVs(");
    status.out.print(SPLib.ToJSValue(BO.w_nomefile,"C",100,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_msgproc,"M",10,0));
  ...
  void WriteMainFormVarScript(arpg_import_fileae_vgBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    PrintState(BO,status);
    status.out.println("var e_GORGDGDNGD = " + SPLib.ToJSValue(m_cEntityUid,"C",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_import_fileae_vg"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_import_fileae_vg","");
  ...
  void WriteMainFormScript(arpg_import_fileae_vgBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_import_fileae_vg");
  ...
  arpg_import_fileae_vgBO getBO(ServletStatus status) {
    arpg_import_fileae_vgBO BO = new arpg_import_fileae_vgBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_import_fileae_vgBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_import_fileae_vgBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_import_fileae_vgBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_import_fileae_vgBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_import_fileae_vgBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_import_fileae_vgBO BO) {
    ...
    item.put("altInterfaceFor","arpg_import_fileae_vg");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"560\",\"name\":\"Box_YWGFSYCMOZ\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"YWGFSYCMOZ\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"776\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"25\",\"href\":\"javascript:PIRPTHHLHK_Click()\",\"name\":\"Esegui\",\"page\":\"1\",\"spuid\":\"PIRPTHHLHK\",\"tabindex\":\"5\",\"type\":\"Button\",\"value\":\"Esegui\",\"w\":\"88\",\"x\":\"688\",\"y\":\"91\",\"zone\":\"YWGFSYCMOZ_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"MESSAGGI_DELLA_PROCEDURA\",\"page\":\"1\",\"spuid\":\"KXNOVGROJL_DIV\",\"tabindex\":\"6\",\"type\":\"Label\",\"value\":\"MESSAGGI DELLA PROCEDURA\",\"w\":\"760\",\"x\":\"16\",\"y\":\"175\",\"zone\":\"YWGFSYCMOZ_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"175\",\"href\":\" \",\"name\":\"__iframe_width__776__height__300__id___informazioni__name__informazioni__src_____servlet_arpg_list_imprich_nomefile___w_nomefile__________frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"GORGDGDNGD_DIV\",\"tabindex\":\"7\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"776"+"'"+" height="+"'"+"300"+"'"+" id ="+"'"+"informazioni"+"'"+" name="+"'"+"informazioni"+"'"+" src="+"'"+"..\\/servlet\\/arpg_list_imprich?nomefile=\\\"+w_nomefile+ \\\""+"'"+"\\\" + \\\" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"760\",\"x\":\"16\",\"y\":\"287\",\"zone\":\"YWGFSYCMOZ_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"14\",\"href\":\" \",\"name\":\"__iframe_width__655__height__170__id__elaborazione___name__elaborazione___frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"FGMOCXNBFY_DIV\",\"tabindex\":\"8\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"655"+"'"+" height="+"'"+"170"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+"  frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"16\",\"x\":\"760\",\"y\":\"14\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"91\",\"href\":\" \",\"name\":\"__iframe_width__776__height__155__id___messaggi__src__arpg_msg_import__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"SSRCOAMDBL_DIV\",\"tabindex\":\"9\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"776"+"'"+" height="+"'"+"155"+"'"+" id ="+"'"+"messaggi"+"'"+" src="+"'"+"arpg_msg_import"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"760\",\"x\":\"16\",\"y\":\"469\",\"zone\":\"YWGFSYCMOZ_11\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"86\",\"name\":\"msgproc\",\"page\":\"1\",\"scroll\":\"true\",\"spuid\":\"BESVSXEZBJ\",\"type\":\"Textbox\",\"w\":\"760\",\"x\":\"16\",\"y\":\"196\",\"zone\":\"YWGFSYCMOZ_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Impostazione_della_lunghezza_del_record__nel_tracciato_ufficiale_\\u00E8_1800_\",\"page\":\"1\",\"spuid\":\"VBEYRFASEL_DIV\",\"tabindex\":\"12\",\"type\":\"Label\",\"value\":\"Impostazione della lunghezza del record (nel tracciato ufficiale \\u00E8 1800)\",\"w\":\"337\",\"x\":\"19\",\"y\":\"67\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"6\",\"name\":\"offset\",\"page\":\"1\",\"spuid\":\"VNFRMCXRZW\",\"tabindex\":\"13\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"360\",\"y\":\"63\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"21\",\"name\":\"f_denom\",\"page\":\"1\",\"spuid\":\"BLSWRPJBJG\",\"tabindex\":\"14\",\"type\":\"Checkbox\",\"w\":\"377\",\"x\":\"16\",\"y\":\"91\",\"zone\":\"YWGFSYCMOZ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"name\":\"f_ricaui\",\"page\":\"1\",\"spuid\":\"EQEVYTAUZT\",\"tabindex\":\"15\",\"textlist\":\"Ricostruzione Archivio Unico Informatico (Menu 2),Ricostruzione Archivio Agenzia Entrate (Menu 5)\",\"type\":\"Combobox\",\"valuelist\":\""+"'"+"A"+"'"+","+"'"+"G"+"'"+"\",\"w\":\"408\",\"x\":\"16\",\"y\":\"120\",\"zone\":\"YWGFSYCMOZ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Prefisso_Nome_Rapporti_\",\"page\":\"1\",\"spuid\":\"CDTQPZIPTH_DIV\",\"tabindex\":\"16\",\"type\":\"Label\",\"value\":\"Prefisso Nome Rapporti:\",\"w\":\"116\",\"x\":\"440\",\"y\":\"123\",\"zone\":\"YWGFSYCMOZ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"prefrap\",\"page\":\"1\",\"spuid\":\"VQLMVVYLUU\",\"tabindex\":\"17\",\"type\":\"Textbox\",\"w\":\"58\",\"x\":\"560\",\"y\":\"119\",\"zone\":\"YWGFSYCMOZ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"3\",\"name\":\"codval\",\"page\":\"1\",\"spuid\":\"HKGEZJGNAE\",\"tabindex\":\"18\",\"type\":\"Textbox\",\"w\":\"40\",\"x\":\"128\",\"y\":\"147\",\"zone\":\"YWGFSYCMOZ_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Codice_Valuta_\",\"page\":\"1\",\"spuid\":\"SSTUTBOCAN_DIV\",\"tabindex\":\"19\",\"type\":\"Label\",\"value\":\"Codice Valuta:\",\"w\":\"108\",\"x\":\"16\",\"y\":\"151\",\"zone\":\"YWGFSYCMOZ_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_xdesval\",\"page\":\"1\",\"spuid\":\"RMHDQIMWME_DIV\",\"tabindex\":\"21\",\"type\":\"Label\",\"value\":\"w_xdesval\",\"w\":\"352\",\"x\":\"194\",\"y\":\"151\",\"zone\":\"YWGFSYCMOZ_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"25\",\"href\":\"javascript:NCXAJLIDKA_Click()\",\"name\":\"Excel\",\"page\":\"1\",\"spuid\":\"NCXAJLIDKA\",\"tabindex\":\"23\",\"type\":\"Button\",\"value\":\"Excel\",\"w\":\"88\",\"x\":\"688\",\"y\":\"63\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_import_fileae_vgBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_import_fileae_vgBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_import_fileae_vgBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_import_fileae_vg?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_import_fileae_vg_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_import_fileae_vgBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_import_fileae_vgBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_import_fileae_vgBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_import_fileae_vgBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_import_fileae_vgBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_import_fileae_vgBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  void IncludeJS(ServletStatus status) {
    String l_cJsUid;
    status.out.println(
     "<script src='SPPrxy/"+arfn_readgmsgproc.m_cEntityUid+"/arfn_readgmsgproc.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arfn_readgmsgproc.GetCallableNames(callable);
      arrt_import_fileae_vg.GetCallableNames(callable);
      arrt_import_fileae_xls.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_import_fileae_vgBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_import_fileae_vgBO BO) {
    boolean result = true;
    return result;
  }
}
