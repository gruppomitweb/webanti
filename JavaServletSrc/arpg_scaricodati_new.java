...
public class arpg_scaricodati_new extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*RKNBVXVWFR*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_scaricodati_new";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 808;
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_scaricodati_newBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11240429377",p_Context.Translate("Mese non valido per l'anno impostato")},
    ...
    {"1966663129",p_Context.Translate("L'utilizzo di questa maschera consente date dal 2007 (Mese di maggio) in avanti")},
    ...
    {"799342670",p_Context.Translate("La data fine periodo deve essere superiore a quella di inizio periodo")}};
    return l_translations;
  }
  void WriteStyles(arpg_scaricodati_newBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_scaricodati_new.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_scaricodati_newBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_scaricodati_newBO BO,ServletStatus status) {
  ...
  void WriteControls(arpg_scaricodati_newBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_scaricodati_newBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(455);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_scaricodati_newBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     "<input name='anno' id='GTMJENMCMN' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_anno,"N",4,0,"", status.m_oRegSettings)+" tabindex='-1' curTabIndex='"+status.m_nTabIndex+"' maxlength='5' size='4' fieldtype='N' onkeypress=''>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<select name='mese' id='TCZMCNHUFI' class='"+status.ObligatoryClass("Combobox")+"' tabindex='-1' curTabIndex='"+status.m_nTabIndex+"'>" +
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input id='NMNMXABGKO' type='button' class='SPButton' value="+SPLib.ToHTMLValue(status.context.Translate("Crea Comunicazione Aperture / Chiusure Rapporti"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' onkeydown='ManageKeys(event,false,false,true)' onclick='NMNMXABGKO_Click()'>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input name='a_rett' id='DIUBYKKRGY' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_a_rett,"D",8,0,SPLib.GetUserDatePicture(status.context.GetLanguage(),"DD-MM-YYYY"), status.m_oRegSettings)+" tabindex='-1' curTabIndex='"+status.m_nTabIndex+"' maxlength='10' size='8' fieldtype='D' onkeydown='ManageKeys(event,false,false,true,true)' onkeypress=''>" +
    ...
    "");
    status.out.println(
     "<div id='NJQZGCHZPP_DIV' class='UntitledBox'>" +
     "<div id='NJQZGCHZPP' class='BoxBody'>" +
     "</div>" +
     "</div>" +
    "");
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<span id='XCZYKCMMXR_DIV'>" +
     "<input name='sogest' id='XCZYKCMMXR' type='checkbox' value="+SPLib.ToHTMLValue(BO.w_sogest,"C",1,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq("S",CPLib.Trim(BO.w_sogest))?"CHECKED":"")+">" +
     "<label id='XCZYKCMMXR_LBL' for='XCZYKCMMXR' class='SPLabel'>"+SPLib.ToHTMLRepresentation(status.context.Translate("Rettifica di codice fiscale per soggetti esteri"))+"</label>" +
     "</span>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
  ...
  static void PrintState(arpg_scaricodati_newBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_scaricodati_newBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_scaricodati_newBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_scaricodati_newBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(808,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_scaricodati_new"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_scaricodati_new","");
  ...
  void WriteMainFormScript(arpg_scaricodati_newBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_scaricodati_new");
    status.out.println("function Elabora() {");
    status.out.println("  w_bloccaedit = 'S';");
    status.out.println("  EnableControlsUnderCondition();");
    status.out.println("  SetLocationHref(null,'arrt_sd_aprichiudi'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')   ");
    status.out.println("}");
  ...
  arpg_scaricodati_newBO getBO(ServletStatus status) {
    arpg_scaricodati_newBO BO = new arpg_scaricodati_newBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_scaricodati_newBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_scaricodati_newBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_scaricodati_newBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_scaricodati_newBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_scaricodati_newBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_scaricodati_newBO BO) {
    ...
    item.put("altInterfaceFor","arpg_scaricodati_new");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"448\",\"name\":\"Box_TTRVXROIVP\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"TTRVXROIVP\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"784\",\"x\":\"8\",\"y\":\"7\",\"zone\":\"pag1_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"14\",\"href\":\" \",\"name\":\"__iframe_width__655__height__170__id__elaborazione__name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"FGMOCXNBFY_DIV\",\"tabindex\":\"2\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"655"+"'"+" height="+"'"+"170"+"'"+" id="+"'"+"elaborazione"+"'"+" name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"256\",\"x\":\"400\",\"y\":\"189\",\"zone\":\"\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Estrazione_mensile_Rapporti_per_Agenzia_delle_Entrate\",\"page\":\"1\",\"spuid\":\"DMMJSVFBEV_DIV\",\"tabindex\":\"3\",\"type\":\"Label\",\"value\":\"Estrazione mensile Rapporti per Agenzia delle Entrate\",\"w\":\"784\",\"x\":\"8\",\"y\":\"14\",\"zone\":\"TTRVXROIVP_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"anno\",\"page\":\"1\",\"spuid\":\"GTMJENMCMN\",\"tabindex\":\"4\",\"type\":\"Textbox\",\"w\":\"36\",\"x\":\"56\",\"y\":\"77\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"name\":\"mese\",\"page\":\"1\",\"spuid\":\"TCZMCNHUFI\",\"tabindex\":\"5\",\"textlist\":\"Gennaio,Febbraio,Marzo,Aprile,Maggio,Giugno,Luglio,Agosto,Settembre,Ottobre,Novembre,Dicembre\",\"type\":\"Combobox\",\"valuelist\":\""+"'"+"01"+"'"+","+"'"+"02"+"'"+","+"'"+"03"+"'"+","+"'"+"04"+"'"+","+"'"+"05"+"'"+","+"'"+"06"+"'"+","+"'"+"07"+"'"+","+"'"+"08"+"'"+","+"'"+"09"+"'"+","+"'"+"10"+"'"+","+"'"+"11"+"'"+","+"'"+"12"+"'"+"\",\"w\":\"112\",\"x\":\"136\",\"y\":\"77\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:NMNMXABGKO_Click()\",\"name\":\"Crea_Comunicazione_Aperture___Chiusure_Rapporti\",\"page\":\"1\",\"spuid\":\"NMNMXABGKO\",\"tabindex\":\"6\",\"type\":\"Button\",\"value\":\"Crea Comunicazione Aperture \\/ Chiusure Rapporti\",\"w\":\"264\",\"x\":\"521\",\"y\":\"70\",\"zone\":\"TTRVXROIVP_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"darett\",\"page\":\"1\",\"spuid\":\"CFLYSPTTDU\",\"tabindex\":\"7\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"72\",\"y\":\"154\",\"zone\":\"TTRVXROIVP_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"a_rett\",\"page\":\"1\",\"spuid\":\"DIUBYKKRGY\",\"tabindex\":\"8\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"248\",\"y\":\"154\",\"zone\":\"TTRVXROIVP_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:ALQVOOWZEV_Click()\",\"name\":\"Crea_Modifiche___Annullamento_Rapporti\",\"page\":\"1\",\"spuid\":\"ALQVOOWZEV\",\"tabindex\":\"9\",\"type\":\"Button\",\"value\":\"Crea Modifiche \\/ Annullamento Rapporti\",\"w\":\"264\",\"x\":\"521\",\"y\":\"152\",\"zone\":\"TTRVXROIVP_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"dadata\",\"page\":\"1\",\"spuid\":\"FSXHJAAZGG\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"312\",\"y\":\"63\",\"zone\":\"TTRVXROIVP_12\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Data_\",\"page\":\"1\",\"spuid\":\"NOYEUIKFXN_DIV\",\"tabindex\":\"11\",\"type\":\"Label\",\"value\":\"Da Data:\",\"w\":\"43\",\"x\":\"264\",\"y\":\"67\",\"zone\":\"TTRVXROIVP_12\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"a_data\",\"page\":\"1\",\"spuid\":\"CLVVMIYSQG\",\"type\":\"Textbox\",\"w\":\"66\",\"x\":\"312\",\"y\":\"91\",\"zone\":\"TTRVXROIVP_13\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Data_\",\"page\":\"1\",\"spuid\":\"LFJQIXRRKM_DIV\",\"tabindex\":\"13\",\"type\":\"Label\",\"value\":\"A Data:\",\"w\":\"43\",\"x\":\"264\",\"y\":\"95\",\"zone\":\"TTRVXROIVP_13\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_AJXISNWTIP()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"AJXISNWTIP\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"14\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"729\",\"y\":\"203\",\"zone\":\"TTRVXROIVP_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"__Comunicazione_rapporti_mensili__\",\"page\":\"1\",\"spuid\":\"LJMNABGSBL_DIV\",\"tabindex\":\"15\",\"type\":\"Label\",\"value\":\"- Comunicazione rapporti mensili -\",\"w\":\"784\",\"x\":\"8\",\"y\":\"35\",\"zone\":\"TTRVXROIVP_11\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"name\":\"Box_ZCSEKJZUCO\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"ZCSEKJZUCO\",\"tabindex\":\"16\",\"type\":\"Box\",\"w\":\"768\",\"x\":\"16\",\"y\":\"329\",\"zone\":\"TTRVXROIVP_19\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"N_B___I_NOMI_DEI_FILE_CREATI_NON_DEVONO_ESSERE_MODIFICATI_PER_NESSUN_MOTIVO____\",\"page\":\"1\",\"spuid\":\"VKDDBMFRDF_DIV\",\"tabindex\":\"17\",\"type\":\"Label\",\"value\":\"N.B.: I NOMI DEI FILE CREATI NON DEVONO ESSERE MODIFICATI PER NESSUN MOTIVO !!!\",\"w\":\"752\",\"x\":\"24\",\"y\":\"335\",\"zone\":\"ZCSEKJZUCO_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Selezione_Rettifiche___Cancellazioni\",\"page\":\"1\",\"spuid\":\"LOFHMITIDO_DIV\",\"tabindex\":\"18\",\"type\":\"Label\",\"value\":\"Selezione Rettifiche \\/ Cancellazioni\",\"w\":\"784\",\"x\":\"8\",\"y\":\"126\",\"zone\":\"TTRVXROIVP_15\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Data_\",\"page\":\"1\",\"spuid\":\"WUVFEJTHRU_DIV\",\"tabindex\":\"19\",\"type\":\"Label\",\"value\":\"Da Data:\",\"w\":\"43\",\"x\":\"24\",\"y\":\"158\",\"zone\":\"TTRVXROIVP_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Data_\",\"page\":\"1\",\"spuid\":\"PKXDCHNEXM_DIV\",\"tabindex\":\"20\",\"type\":\"Label\",\"value\":\"A Data:\",\"w\":\"43\",\"x\":\"200\",\"y\":\"158\",\"zone\":\"TTRVXROIVP_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Anno_\",\"page\":\"1\",\"spuid\":\"YIXZTOKOBR_DIV\",\"tabindex\":\"21\",\"type\":\"Label\",\"value\":\"Anno:\",\"w\":\"29\",\"x\":\"23\",\"y\":\"81\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Mese_\",\"page\":\"1\",\"spuid\":\"TTKGEZTDBH_DIV\",\"tabindex\":\"22\",\"type\":\"Label\",\"value\":\"Mese:\",\"w\":\"29\",\"x\":\"103\",\"y\":\"81\",\"zone\":\"TTRVXROIVP_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"1\",\"name\":\"Box_GSJBAHVGWP\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"GSJBAHVGWP\",\"tabindex\":\"23\",\"type\":\"Box\",\"w\":\"784\",\"x\":\"8\",\"y\":\"238\",\"zone\":\"TTRVXROIVP_16\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"1\",\"name\":\"Box_YANYATMIPH\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"YANYATMIPH\",\"tabindex\":\"24\",\"type\":\"Box\",\"w\":\"784\",\"x\":\"8\",\"y\":\"119\",\"zone\":\"TTRVXROIVP_14\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Visualizza file creati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_JBNBDKBIAJ()\",\"name\":\"images_search_gif\",\"page\":\"1\",\"spuid\":\"JBNBDKBIAJ\",\"src\":\"images\\/search.gif\",\"tabindex\":\"27\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"681\",\"y\":\"203\",\"zone\":\"TTRVXROIVP_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"1\",\"name\":\"Box_NJQZGCHZPP\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"NJQZGCHZPP\",\"tabindex\":\"28\",\"type\":\"Box\",\"w\":\"784\",\"x\":\"8\",\"y\":\"322\",\"zone\":\"TTRVXROIVP_18\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Dati_Impegno_Alla_Presentazione_Telematica\",\"page\":\"1\",\"spuid\":\"UFZQBBFAGD_DIV\",\"tabindex\":\"29\",\"type\":\"Label\",\"value\":\"Dati Impegno Alla Presentazione Telematica\",\"w\":\"784\",\"x\":\"8\",\"y\":\"245\",\"zone\":\"TTRVXROIVP_17\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Codice Fiscale Organo Invio File\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"CFINVIO\",\"page\":\"1\",\"spuid\":\"MPIOZQPPPI\",\"tabindex\":\"31\",\"type\":\"Textbox\",\"w\":\"128\",\"x\":\"176\",\"y\":\"266\",\"zone\":\"TTRVXROIVP_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Codice_Fiscale_Organo_Invio_File_\",\"page\":\"1\",\"spuid\":\"CTFPYTZIIH_DIV\",\"tabindex\":\"32\",\"type\":\"Label\",\"value\":\"Codice Fiscale Organo Invio File:\",\"w\":\"156\",\"x\":\"20\",\"y\":\"270\",\"zone\":\"TTRVXROIVP_7\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Codice CAF Organo Invio\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"CAFINVIO\",\"page\":\"1\",\"spuid\":\"YFPMFPWESC\",\"tabindex\":\"33\",\"type\":\"Textbox\",\"w\":\"48\",\"x\":\"616\",\"y\":\"266\",\"zone\":\"TTRVXROIVP_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Codice_CAF_Organo_Invio_\",\"page\":\"1\",\"spuid\":\"LASRRWOCQH_DIV\",\"tabindex\":\"34\",\"type\":\"Label\",\"value\":\"Codice CAF Organo Invio:\",\"w\":\"125\",\"x\":\"491\",\"y\":\"270\",\"zone\":\"TTRVXROIVP_8\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Tipo Preparazione FIle\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"name\":\"FILEINVIO\",\"page\":\"1\",\"spuid\":\"HMYRBXBREF\",\"tabindex\":\"35\",\"textlist\":\"Comunicazione predisposta da contribuente,Comunicazione predisposta da chi effettua la trasmissione\",\"type\":\"Combobox\",\"valuelist\":\""+"'"+"1"+"'"+","+"'"+"2"+"'"+"\",\"w\":\"264\",\"x\":\"176\",\"y\":\"294\",\"zone\":\"TTRVXROIVP_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Tipo_Preparazione_FIle_\",\"page\":\"1\",\"spuid\":\"HAINTPIJUS_DIV\",\"tabindex\":\"36\",\"type\":\"Label\",\"value\":\"Tipo Preparazione FIle:\",\"w\":\"111\",\"x\":\"65\",\"y\":\"298\",\"zone\":\"TTRVXROIVP_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Data Impegno Organo Invio\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"DATAINVIO\",\"page\":\"1\",\"spuid\":\"CQATNPZMEC\",\"tabindex\":\"37\",\"type\":\"Textbox\",\"w\":\"80\",\"x\":\"616\",\"y\":\"294\",\"zone\":\"TTRVXROIVP_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Impegno_Organo_Invio_\",\"page\":\"1\",\"spuid\":\"FHSMWCMRII_DIV\",\"tabindex\":\"38\",\"type\":\"Label\",\"value\":\"Data Impegno Organo Invio:\",\"w\":\"138\",\"x\":\"478\",\"y\":\"298\",\"zone\":\"TTRVXROIVP_9\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stampa contenuto dei file creati\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"28\",\"href\":\"javascript:href_MPCMAMHQST()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"MPCMAMHQST\",\"src\":\"images\\/print.gif\",\"tabindex\":\"39\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"633\",\"y\":\"203\",\"zone\":\"TTRVXROIVP_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"768\",\"x\":\"16\",\"y\":\"364\",\"zone\":\"TTRVXROIVP_20\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"21\",\"name\":\"sogest\",\"page\":\"1\",\"spuid\":\"XCZYKCMMXR\",\"tabindex\":\"42\",\"type\":\"Checkbox\",\"w\":\"228\",\"x\":\"24\",\"y\":\"203\",\"zone\":\"TTRVXROIVP_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"21\",\"name\":\"mespre\",\"page\":\"1\",\"spuid\":\"MOHZUWAWZD\",\"tabindex\":\"43\",\"type\":\"Checkbox\",\"w\":\"328\",\"x\":\"272\",\"y\":\"203\",\"zone\":\"TTRVXROIVP_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_scaricodati_newBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_scaricodati_newBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_scaricodati_newBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_scaricodati_new?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_scaricodati_new_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_scaricodati_newBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_scaricodati_newBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_scaricodati_newBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_scaricodati_newBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_scaricodati_newBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_scaricodati_newBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_scaricodati_newBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_scaricodati_newBO BO) {
    boolean result = true;
    return result;
  }
}
