...
// * --- Fine Area Manuale
public class arfn_cgo_delregR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_cgo_delregR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_cgo_delreg",m_Caller);
    ...
    try {
      ...
      try {
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on cgo_operazioni_dl into routine arfn_cgo_delreg returns two or more records. This item should return only a record.");
        ...
        if (CPLib.eqr(_iddoc_ope,1) || CPLib.eqr(_iddoc_ope,0)) {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arfn_cgo_delreg",40,"00000012")+"delete from "+m_oWrInfo.GetTableWriteName();
          m_cSql = m_cSql+" where ";
          ...
          m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
          m_cSql = CPLib.GetSQLPrefixComment("arfn_cgo_delreg",40,"00000013")+"delete from "+m_oWrInfo.GetTableWriteName();
          m_cSql = m_cSql+" where ";
  ...
  public static arfn_cgo_delregR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_cgo_delregR(p_Ctx, p_Caller);
  }
  public static arfn_cgo_delregR Make(CPContext p_Ctx) {
    return new arfn_cgo_delregR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_cgo_delreg,
  public static final String i_InvokedRoutines = ",arfn_cgo_delreg,";
