...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('NBMHJRJGQX').value,'C'),w_IDMOD)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('KHBDKLNADV').value,'N'),w_ROWRIL)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('anom_ril',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDMOD') &&  ! (Empty(HtW(Ctrl('NBMHJRJGQX').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDMOD',HtW(Ctrl('NBMHJRJGQX').value,'C'),'C');
}
if (Ne(name,'ROWRIL') &&  ! (Empty(HtW(Ctrl('KHBDKLNADV').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.ROWRIL',HtW(Ctrl('KHBDKLNADV').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'detail','aosdt_anom_ril');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('anom_ril',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['NBMHJRJGQX','KHBDKLNADV'],i=1;i>=0;i--){
