...
SetLocationHref(null,'arrt_deletefile_do'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
}
...
w_Messaggi.raiseEvent('arpg_deletefile_do_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_deletefile_do_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_deletefile_do_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_deletefile_do',m_cSelectedPage);
...
l_Appl.LinkTable('aefile_do_glob');
...
l_oWv.setValue('Table',WtA('aefile_do_glob','C'));
