...
// * --- Fine Area Manuale
public class ardt_rapcollBO extends ardt_rapcollBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_2_1.RAPPORTO AS RAPPORTO201";
        m_cSelect = m_cSelect+" ,link_2_1.DESCRAP AS DESCRAP201";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","RAPPORTO","RAPPORTO");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","RAPPORTO","RAPPORTO",true);
  ...
  // * --- Fine Area Manuale
  public ardt_rapcollBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_rapcoll");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_rapcoll,m_cPhName_rapcoll,m_cVirtName_rapcoll,m_cVirtName_rapcoll+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_rapcoll,p_IDFILEBO),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_1(m_cPhName_anarapbo,m_cServer_anarapbo);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"rapcoll")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            w_DESCRAP = l_rsLoadBody.GetString("DESCRAP201");
          } else {
            Link_ADJOKPLCJT("Load");
          }
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_rapcoll.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND RAPPORTO="+CPLib.ToSQL(opk_RAPPORTO,"C",25,0);
        l_cSql = l_cSql+m_oWrInfo_rapcoll.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"rapcoll")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_rapcoll,l_cSql);
  ...
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"ADJOKPLCJT")) {
      ...
      result.m_PrgZoom = "armt_anarap";
      result.m_rdField = new String[]{"RAPPORTO" , "DESCRAP"};
      result.m_rdTypes = new String[]{"C" , "C"};
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("ADJOKPLCJT",CPLib.GetLinkedFieldProps("RAPPORTO","RAPPORTO","C",25,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_rapcoll);
      l_cSql = "insert into "+m_oWrInfo_rapcoll.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDFILEBO,";
      l_cSql = l_cSql+"RAPPORTO,";
      l_cSql = l_cSql+"IDBASE,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_rapcoll.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"rapcoll")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_rapcoll.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"rapcoll")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_rapcoll,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_rapcoll);
      l_cSql = "Update "+m_oWrInfo_rapcoll.GetTableWriteName()+" set";
