...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('PBPAZFLKHV').value,'C'),w_CODDIP)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('anadip',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODDIP') &&  ! (Empty(HtW(Ctrl('PBPAZFLKHV').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODDIP',HtW(Ctrl('PBPAZFLKHV').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_dippreposti(firstAlias) {
return m_cVirtName+".CODDIP = "+firstAlias+".CODDIP ";
}
...
_chNotF(childAlias,'master','armt_anadip');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('anadip',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['PBPAZFLKHV'],i=0;i>=0;i--){
