...
var ctl = _GetCtl(e,'ABZSLYXSCO');
if (_tracker.goon(ctl,ctrlValue)) {
w_RAPCOL=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('ABZSLYXSCO',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
if ( ! (m_oFather.SaveContext_ardt_storsoginfo())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_storsoginfo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_storsoginfo_dataNotSent',{'action':formaction});
}
}
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_storsoginfo();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.RAPCOL={"dec":0,"fixedpos":false,"id":"ABZSLYXSCO","len":25,"name":"RAPCOL","title":"","type":"C"};
