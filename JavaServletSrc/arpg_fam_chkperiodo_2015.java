...
public class arpg_fam_chkperiodo_2015 extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*HNUFIIARLX*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arpg_fam_chkperiodo_2015";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_fam_chkperiodo_2015BO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10144017222",p_Context.Translate("La data deve essere compresa tra il 01-01-2015 e il 31-12-2015")},
    {"10378353731",p_Context.Translate("Controlli Periodali")},
    {"116209495",p_Context.Translate("La data di fine periodo non deve essere minore di quella di inizio periodo e deve essere compresa tra il 01-01-2015 e il 31-12-2015")}};
    return l_translations;
  }
  void WriteStyles(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_fam_chkperiodo_2015.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(459);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_fam_chkperiodo_2015BL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_fam_chkperiodo_2015BL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_fam_chkperiodo_2015BL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_fam_chkperiodo_2015"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_fam_chkperiodo_2015","");
  ...
  void WriteMainFormScript(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_fam_chkperiodo_2015BO getBO(ServletStatus status) {
    arpg_fam_chkperiodo_2015BO BO = new arpg_fam_chkperiodo_2015BO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_fam_chkperiodo_2015BO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_fam_chkperiodo_2015BO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_fam_chkperiodo_2015BO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_fam_chkperiodo_2015BO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_fam_chkperiodo_2015BO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_fam_chkperiodo_2015BO BO) {
    ...
    item.put("altInterfaceFor","arpg_fam_chkperiodo_2015");
    items.put(item);
    String s;
  ...
  void Next(ServletStatus status,arpg_fam_chkperiodo_2015BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_fam_chkperiodo_2015BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_fam_chkperiodo_2015?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_fam_chkperiodo_2015_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_fam_chkperiodo_2015BO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_fam_chkperiodo_2015BO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_fam_chkperiodo_2015.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_fam_chkperiodo_2015BO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_fam_chkperiodo_2015BO BO) {
    boolean result = true;
    return result;
  }
}
