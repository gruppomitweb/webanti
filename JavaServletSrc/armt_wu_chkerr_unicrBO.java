...
// * --- Fine Area Manuale
public class armt_wu_chkerr_unicrBO extends armt_wu_chkerr_unicrBL {
  class SqlLoader {
    ...
    public void AddJoinedLink_1_64(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_1_64.MACODICE AS MACODICE164";
        m_cSelect = m_cSelect+" ,link_1_64.MADESCRI AS MADESCRI164";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_1_64","WUCODMAGE","MACODICE");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_1_64","WUCODMAGE","MACODICE",true);
  ...
  // * --- Fine Area Manuale
  public armt_wu_chkerr_unicrBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_wu_chkerr_unicr");
    }
  ...
  public boolean Load(double p_WUCODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_wuchkunicr,m_cPhName_wuchkunicr,m_cVirtName_wuchkunicr,m_cVirtName_wuchkunicr+".WUCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_WUCODICE,"N",10,0),m_cServer_wuchkunicr,p_WUCODICE),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    l_oSqlLoader.AddJoinedLink_1_64(m_cPhName_tbmacage,m_cServer_tbmacage);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        w_WUDATINI = l_rsLoad.GetDate("WUDATINI");
        ...
        if (l_oSqlLoader.m_bLink_1_64) {
          w_xMADESCRI = l_rsLoad.GetString("MADESCRI164");
        } else {
          Link_FHWIGGJJLC("Load");
        }
        w_WUNOTE = l_rsLoad.GetString("WUNOTE");
        w_WUFLGFRC = l_rsLoad.GetString("WUFLGFRC");
        w_WUDATFIN = l_rsLoad.GetDate("WUDATFIN");
        w_WUCODRAP = l_rsLoad.GetString("WUCODRAP");
        w_WUNDATINI = l_rsLoad.GetDate("WUNDATINI");
        w_WUNDATFIN = l_rsLoad.GetDate("WUNDATFIN");
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"wuchkunicr")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_wuchkunicr);
      l_cSql = "insert into "+m_oWrInfo_wuchkunicr.GetTableWriteName()+" (";
      l_cSql = l_cSql+"WUNOMEFILE,";
      l_cSql = l_cSql+"WUTIPOPE,";
      l_cSql = l_cSql+"WUCODDIP,";
      l_cSql = l_cSql+"WUDATINI,";
      ...
      l_cSql = l_cSql+"WUDATFIN,";
      l_cSql = l_cSql+"WUCODRAP,";
      l_cSql = l_cSql+"WUNDATINI,";
      l_cSql = l_cSql+"WUNDATFIN,";
      l_cSql = l_cSql+"WUSTACLI,";
      l_cSql = l_cSql+"WUNSEXCLI,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_wuchkunicr.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wuchkunicr")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_WUNOMEFILE,"C",50,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUTIPOPE,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUCODDIP,"C",6,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUDATINI,"D",8,0)+", ";
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_WUNOTE,"M",10,0,m_cServer_wuchkunicr,m_oParameters)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUFLGFRC,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUDATFIN,"D",8,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUCODRAP,"C",16,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUNDATINI,"D",8,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUNDATFIN,"D",8,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUSTACLI,"C",40,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_WUNSEXCLI,"C",1,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_wuchkunicr.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wuchkunicr")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_wuchkunicr,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_wuchkunicr);
      l_cSql = "Update "+m_oWrInfo_wuchkunicr.GetTableWriteName()+" set";
      l_cSql = l_cSql+" WUNOMEFILE = "+CPLib.ToSQL(w_WUNOMEFILE,"C",50,0);
      l_cSql = l_cSql+", WUTIPOPE = "+CPLib.ToSQL(w_WUTIPOPE,"C",1,0);
      l_cSql = l_cSql+", WUCODDIP = "+CPLib.ToSQL(w_WUCODDIP,"C",6,0);
      l_cSql = l_cSql+", WUDATINI = "+CPLib.ToSQL(w_WUDATINI,"D",8,0);
      ...
      l_cSql = l_cSql+", WUNOTE = "+CPLib.ToSQL(w_WUNOTE,"M",10,0,m_cServer_wuchkunicr,m_oParameters);
      l_cSql = l_cSql+", WUFLGFRC = "+CPLib.ToSQL(w_WUFLGFRC,"C",1,0);
      l_cSql = l_cSql+", WUDATFIN = "+CPLib.ToSQL(w_WUDATFIN,"D",8,0);
      l_cSql = l_cSql+", WUCODRAP = "+CPLib.ToSQL(w_WUCODRAP,"C",16,0);
      l_cSql = l_cSql+", WUNDATINI = "+CPLib.ToSQL(w_WUNDATINI,"D",8,0);
      l_cSql = l_cSql+", WUNDATFIN = "+CPLib.ToSQL(w_WUNDATFIN,"D",8,0);
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"WUCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_WUCODICE,"N",10,0),m_cServer_wuchkunicr,w_WUCODICE);
      l_cSql = l_cSql+m_oWrInfo_wuchkunicr.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wuchkunicr")) {
  ...
  public boolean Delete(double p_WUCODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_wuchkunicr.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"WUCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_WUCODICE,"N",10,0),m_cServer_wuchkunicr,w_WUCODICE);
          l_cSql = l_cSql+m_oWrInfo_wuchkunicr.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"wuchkunicr")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_wuchkunicr,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
  ...
  // * --- Fine Area Manuale
  // ENTITY_BATCHES: ,arrt_wu_chkerr_unicr_save,
  public static final String i_InvokedRoutines = ",arrt_wu_chkerr_unicr_save,";
}
