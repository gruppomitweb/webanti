...
if (IsDisabledByStateDriver('DatiAna')) {
...
if ( ! (m_oFather.SaveContext_ardt_ae_soginfo())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_ae_soginfo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_ae_soginfo_dataNotSent',{'action':formaction});
}
}
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_ae_soginfo();
}
}
SaveContext.child=[];
function BlankRow() {
