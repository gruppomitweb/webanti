...
public class arpg_modopen3 extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*RSFSZWIPWF*!";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_modopen3BO BO) {
  ...
  void WriteStyles(arpg_modopen3BO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_modopen3.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_modopen3BO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_modopen3BO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_modopen3BO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(42);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_modopen3BO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_modopen3BL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_modopen3BL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_modopen3BL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_modopen3BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_modopen3"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_modopen3","");
    ...
    status.out.println("// * --- arpg_modopen3");
  ...
  void WriteMainFormScript(arpg_modopen3BO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_modopen3BO getBO(ServletStatus status) {
    arpg_modopen3BO BO = new arpg_modopen3BO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_modopen3BO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_modopen3BO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_modopen3BO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_modopen3BO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_modopen3BO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_modopen3BO BO) {
    ...
    item.put("altInterfaceFor","arpg_modopen3");
    items.put(item);
    String s;
  ...
  void Next(ServletStatus status,arpg_modopen3BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_modopen3BO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_modopen3BO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_modopen3?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_modopen3_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_modopen3BO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_modopen3BO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_modopen3BO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_modopen3BO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_modopen3BO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_modopen3BO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  void IncludeJS(ServletStatus status) {
    String l_cJsUid;
    status.out.println(
     "<script src='SPPrxy/"+arrt_modopen3.m_cEntityUid+"/arrt_modopen3.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_modopen3.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_modopen3BO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_modopen3BO BO) {
    boolean result = true;
    return result;
  }
}
