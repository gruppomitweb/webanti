...
public class arrt_stplegami_elenco extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*692cd8d3*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_stplegami_elenco";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_ADatFin) ||  ! (status.m_bpassedw_DaDatFin) ||  ! (status.m_bpassedw_ADatIni) ||  ! (status.m_bpassedw_DaDatIni) ||  ! (status.m_bpassedw_aRappo) ||  ! (status.m_bpassedw_daRappo);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_stplegami_elencoR Rcalled;
    Rcalled = new arrt_stplegami_elencoR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_stplegami_elencoR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_stplegami_elencoR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_stplegami_elenco"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_stplegami_elenco");
      if ( ! (CPLib.IsNull(l))) {
