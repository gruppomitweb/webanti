...
public class arrt_sd_ricrea extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*IJMXGSLTBS*!";
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (m_bpassedw_nomefile) {
        ...
        if (source.IsForwarded()) {
          w_nomefile = source.GetCaller().GetString("nomefile","C",15,0);
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_files) ||  ! (status.m_bpassedw_tipofile) ||  ! (status.m_bpassedw_nomefile);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_nomefile)) {
      status.out.println("l_cV=WtA(l_oWnd('w_nomefile',"+SPLib.ToJSValue(status.w_nomefile,"C",15,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_nomefile,"C",15,0)+",'C');");
    }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_sd_ricreaR Rcalled;
    Rcalled = new arrt_sd_ricreaR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_sd_ricreaR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_sd_ricreaR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_sd_ricrea"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_sd_ricrea");
      if ( ! (CPLib.IsNull(l))) {
