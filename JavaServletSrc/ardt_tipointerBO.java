...
// * --- Fine Area Manuale
public class ardt_tipointerBO extends ardt_tipointerBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_2_1.TIPOINT AS TIPOINT201";
        m_cSelect = m_cSelect+" ,link_2_1.DESCRI AS DESCRI201";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","tipinter","TIPOINT");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","tipinter","TIPOINT",true);
  ...
  // * --- Fine Area Manuale
  public ardt_tipointerBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_tipointer");
    }
  ...
  public static String[] m_cLoadParameterNames = {"codcau"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_codcau) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cauanainter,m_cPhName_cauanainter,m_cVirtName_cauanainter,m_cVirtName_cauanainter+".codcau="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_codcau,"C",4,0),m_cServer_cauanainter,p_codcau),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_1(m_cPhName_tbtipint,m_cServer_tbtipint);
    l_rsLoadBody = l_oSqlDetailLoader.Query();
    try {
      m_bLoaded =  ! (l_rsLoadBody.Eof());
      if (m_bLoaded) {
        m_bCalculating = true;
        w_codcau = l_rsLoadBody.GetString("codcau");
        // Assegna i valori degli op_
        // Assegna il contesto dei figli di testata/piedi a Empty
        SPExtenderLib.LoadExtendedFields(l_rsLoadBody,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cauanainter")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_tipinter = l_rsLoadBody.GetString("tipinter");
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            w_xdescinter = l_rsLoadBody.GetString("DESCRI201");
          } else {
            Link_GCFQVRXGQC("Load");
          }
          // Assegna il contesto dei figli del dettaglio a Empty
          work_children = CPLib.BlankChildMNTs(m_MNTs,true);
          opk_tipinter = w_tipinter;
          if (CPLib.HasTableCPCCCHK(m_Ctx,"cauanainter")) {
  ...
  public boolean Delete(String p_codcau) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cauanainter.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"codcau="+CPLib.ToSQL(w_codcau,"C",4,0)+" AND tipinter="+CPLib.ToSQL(opk_tipinter,"C",2,0);
        l_cSql = l_cSql+m_oWrInfo_cauanainter.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cauanainter")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cauanainter,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_codcau);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"GCFQVRXGQC")) {
      ...
      result.AppendFieldsHash("GCFQVRXGQC",CPLib.GetLinkedFieldProps("TIPOINT","tipinter","C",2,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cauanainter);
      l_cSql = "insert into "+m_oWrInfo_cauanainter.GetTableWriteName()+" (";
      l_cSql = l_cSql+"codcau,";
      l_cSql = l_cSql+"tipinter,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cauanainter.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cauanainter")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_cauanainter.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cauanainter")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cauanainter,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cauanainter);
      l_cSql = "Update "+m_oWrInfo_cauanainter.GetTableWriteName()+" set";
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cauanainter")) {
