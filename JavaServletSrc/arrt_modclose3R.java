...
// * --- Fine Area Manuale
public class arrt_modclose3R implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_modclose3R (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_modclose3",m_Caller);
    ...
    try {
      ...
      if ( ! (CPLib.Empty(w_datarett))) {
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on aetesta into routine arrt_modclose3 returns two or more records. This item should return only a record.");
        ...
        if ( ! (CPLib.Empty(w_oldrnumrig))) {
          ...
          while ( ! (Cursor_aerighe.Eof())) {
            ...
            m_cSql = CPLib.GetSQLPrefixComment("arrt_modclose3",36,"00000018")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
            ...
            m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
            m_cSql = CPLib.GetSQLPrefixComment("arrt_modclose3",36,"0000001A")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
  ...
  public static arrt_modclose3R Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_modclose3R(p_Ctx, p_Caller);
  }
  public void Blank() {
