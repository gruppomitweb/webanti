...
// * --- Fine Area Manuale
public class arfn_datisog_sol_mcR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_datisog_sol_mcR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_datisog_sol_mc",m_Caller);
  ...
  void Page_2() throws Exception {
    /* Record Tipo 2 */
    /* _numsto := chkspers(iif(pSTATO='1',pDatRet,pDatOpe)) */
    _numsto = chkspers((CPLib.eqr(pSTATO,"1")?pDatRet:pDatOpe));
    ...
    if (CPLib.Empty(CPLib.LRTrim(_numsto))) {
      for (MemoryCursorRow_personbo rwPersonbo : mcPersonbo._iterable_()) {
        /* _ramate := iif(pRAE='R',LRTrim(rwPersonbo.RAMOGRUP),iif(Left(rwPersonbo.ATTIV,1)='P' or Left(rwPersonbo.ATTIV,3)='999',Space(3),Left(LRTrim(rwPersonbo.ATTIV),3))) */
        _ramate = (CPLib.eqr(pRAE,"R")?CPLib.LRTrim(rwPersonbo.RAMOGRUP):(CPLib.eqr(CPLib.Left(rwPersonbo.ATTIV,1),"P") || CPLib.eqr(CPLib.Left(rwPersonbo.ATTIV,3),"999")?CPLib.Space(3):CPLib.Left(CPLib.LRTrim(rwPersonbo.ATTIV),3)));
        /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwPersonbo.RAGSOC,' '))+Space(70),70) + Left(LRTrim(rwPersonbo.PAESE)+Space(3),3) */
        cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwPersonbo.RAGSOC," "))+CPLib.Space(70),70)+CPLib.Left(CPLib.LRTrim(rwPersonbo.PAESE)+CPLib.Space(3),3);
        /* cRitorno := cRitorno + Left(LRTrim(rwPersonbo.CODCAB)+Space(6),6) + Left(LRTrim(arfn_agechar(rwPersonbo.DESCCIT,' '))+Space(30),30) + Left(LRTrim(rwPersonbo.PROVINCIA)+Space(2),2) */
        cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(rwPersonbo.CODCAB)+CPLib.Space(6),6)+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwPersonbo.DESCCIT," "))+CPLib.Space(30),30)+CPLib.Left(CPLib.LRTrim(rwPersonbo.PROVINCIA)+CPLib.Space(2),2);
        /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwPersonbo.DOMICILIO,' '))+Space(35),35) + Left(LRTrim(rwPersonbo.CAP)+ Space(5),5) */
        cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwPersonbo.DOMICILIO," "))+CPLib.Space(35),35)+CPLib.Left(CPLib.LRTrim(rwPersonbo.CAP)+CPLib.Space(5),5);
        /* cRitorno := cRitorno + iif(rwPersonbo.CFESTERO=1,Space(16),Left(LRTrim(rwPersonbo.CODFISC)+Space(16),16)) */
        cRitorno = cRitorno+(CPLib.eqr(rwPersonbo.CFESTERO,1)?CPLib.Space(16):CPLib.Left(CPLib.LRTrim(rwPersonbo.CODFISC)+CPLib.Space(16),16));
        /* If Empty(rwPersonbo.DATANASC) or DateToChar(rwPersonbo.DATANASC)='01000101' */
        if (CPLib.Empty(rwPersonbo.DATANASC) || CPLib.eqr(CPLib.DateToChar(rwPersonbo.DATANASC),"01000101")) {
          /* cRitorno := cRitorno + Space(8) */
          cRitorno = cRitorno+CPLib.Space(8);
        } else { // Else
          /* cRitorno := cRitorno + Left(LRTrim(DateToChar(rwPersonbo.DATANASC))+Space(8),8) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(CPLib.DateToChar(rwPersonbo.DATANASC))+CPLib.Space(8),8);
        } // End If
        /* If not(Empty(rwPersonbo.NASSTATO)) */
        if ( ! (CPLib.Empty(rwPersonbo.NASSTATO))) {
          /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwPersonbo.NASSTATO,' '))+Space(30),30) + Left(LRTrim(rwPersonbo.SOTGRUP)+Space(3),3) + Left(LRTrim(_ramate)+Space(3),3) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwPersonbo.NASSTATO," "))+CPLib.Space(30),30)+CPLib.Left(CPLib.LRTrim(rwPersonbo.SOTGRUP)+CPLib.Space(3),3)+CPLib.Left(CPLib.LRTrim(_ramate)+CPLib.Space(3),3);
        } else { // Else
          /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwPersonbo.NASCOMUN,' '))+Space(30),30) + Left(LRTrim(rwPersonbo.SOTGRUP)+Space(3),3) + Left(LRTrim(_ramate)+Space(3),3) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwPersonbo.NASCOMUN," "))+CPLib.Space(30),30)+CPLib.Left(CPLib.LRTrim(rwPersonbo.SOTGRUP)+CPLib.Space(3),3)+CPLib.Left(CPLib.LRTrim(_ramate)+CPLib.Space(3),3);
        } // End If
        /* cRitorno := cRitorno + Left(LRTrim(rwPersonbo.SETTSINT)+Space(3),3) + Left(LRTrim(arfn_agechar(pRap,' '))+Space(25),25) */
        cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(rwPersonbo.SETTSINT)+CPLib.Space(3),3)+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(pRap," "))+CPLib.Space(25),25);
        /* cRitorno := cRitorno + Left(LRTrim(pTRap)+" ",1) + Left(LRTrim(rwPersonbo.TIPODOC)+Space(2),2) + Left(LRTrim(rwPersonbo.NUMDOCUM)+Space(15),15) */
        cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(pTRap)+" ",1)+CPLib.Left(CPLib.LRTrim(rwPersonbo.TIPODOC)+CPLib.Space(2),2)+CPLib.Left(CPLib.LRTrim(rwPersonbo.NUMDOCUM)+CPLib.Space(15),15);
        /* If Empty(rwPersonbo.DATARILASC) or DateToChar(rwPersonbo.DATARILASC)='01000101' */
        if (CPLib.Empty(rwPersonbo.DATARILASC) || CPLib.eqr(CPLib.DateToChar(rwPersonbo.DATARILASC),"01000101")) {
          /* cRitorno := cRitorno + Space(8) */
          cRitorno = cRitorno+CPLib.Space(8);
        } else { // Else
          /* cRitorno := cRitorno + Left(LRTrim(DateToChar(rwPersonbo.DATARILASC))+Space(8),8)  */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(CPLib.DateToChar(rwPersonbo.DATARILASC))+CPLib.Space(8),8);
        } // End If
        /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwPersonbo.AUTRILASC,' '))+Space(30),30) */
        cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwPersonbo.AUTRILASC," "))+CPLib.Space(30),30);
        /* cRitorno := cRitorno + Left(LRTrim(rwPersonbo.SESSO)+" ",1) + Space(22) */
        cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(rwPersonbo.SESSO)+" ",1)+CPLib.Space(22);
        /* cRitorno := cRitorno + iif(not(Empty(rwPersonbo.DATAVALI)),DateToChar(rwPersonbo.DATAVALI),Space(8)) */
        cRitorno = cRitorno+( ! (CPLib.Empty(rwPersonbo.DATAVALI))?CPLib.DateToChar(rwPersonbo.DATAVALI):CPLib.Space(8));
        /* cRitorno := cRitorno + iif(rwPersonbo.CFESTERO=1,'NOCF',Space(4)) */
        cRitorno = cRitorno+(CPLib.eqr(rwPersonbo.CFESTERO,1)?"NOCF":CPLib.Space(4));
      }
    } else { // Else
      for (MemoryCursorRow_wersonbo rwWersonbo : mcWersonbo._iterable_()) {
        /* If rwWersonbo.sanumpro = _numsto */
        if (CPLib.eqr(rwWersonbo.sanumpro,_numsto)) {
          /* _ramate := iif(pRAE='R',LRTrim(rwWersonbo.RAMOGRUP),iif(Left(rwWersonbo.ATTIV,1)='P' or Left(rwWersonbo.ATTIV,3)='999',Space(3),Left(LRTrim(rwWersonbo.ATTIV),3))) */
          _ramate = (CPLib.eqr(pRAE,"R")?CPLib.LRTrim(rwWersonbo.RAMOGRUP):(CPLib.eqr(CPLib.Left(rwWersonbo.ATTIV,1),"P") || CPLib.eqr(CPLib.Left(rwWersonbo.ATTIV,3),"999")?CPLib.Space(3):CPLib.Left(CPLib.LRTrim(rwWersonbo.ATTIV),3)));
          /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwWersonbo.RAGSOC,' '))+Space(70),70) + Left(LRTrim(rwWersonbo.PAESE)+Space(3),3) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwWersonbo.RAGSOC," "))+CPLib.Space(70),70)+CPLib.Left(CPLib.LRTrim(rwWersonbo.PAESE)+CPLib.Space(3),3);
          /* cRitorno := cRitorno + Left(LRTrim(rwWersonbo.CODCAB)+Space(6),6) + Left(LRTrim(arfn_agechar(rwWersonbo.DESCCIT,' '))+Space(30),30) + Left(LRTrim(rwWersonbo.PROVINCIA)+Space(2),2) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(rwWersonbo.CODCAB)+CPLib.Space(6),6)+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwWersonbo.DESCCIT," "))+CPLib.Space(30),30)+CPLib.Left(CPLib.LRTrim(rwWersonbo.PROVINCIA)+CPLib.Space(2),2);
          /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwWersonbo.DOMICILIO,' '))+Space(35),35) + Left(LRTrim(rwWersonbo.CAP)+ Space(5),5) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwWersonbo.DOMICILIO," "))+CPLib.Space(35),35)+CPLib.Left(CPLib.LRTrim(rwWersonbo.CAP)+CPLib.Space(5),5);
          /* cRitorno := cRitorno + iif(rwWersonbo.CFESTERO=1,Space(16),Left(LRTrim(rwWersonbo.CODFISC)+Space(16),16)) */
          cRitorno = cRitorno+(CPLib.eqr(rwWersonbo.CFESTERO,1)?CPLib.Space(16):CPLib.Left(CPLib.LRTrim(rwWersonbo.CODFISC)+CPLib.Space(16),16));
          /* If Empty(rwWersonbo.DATANASC) or DateToChar(rwWersonbo.DATANASC)='01000101' */
          if (CPLib.Empty(rwWersonbo.DATANASC) || CPLib.eqr(CPLib.DateToChar(rwWersonbo.DATANASC),"01000101")) {
            /* cRitorno := cRitorno + Space(8) */
            cRitorno = cRitorno+CPLib.Space(8);
          } else { // Else
            /* cRitorno := cRitorno + Left(LRTrim(DateToChar(rwWersonbo.DATANASC))+Space(8),8) */
            cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(CPLib.DateToChar(rwWersonbo.DATANASC))+CPLib.Space(8),8);
          } // End If
          /* If not(Empty(rwWersonbo.NASSTATO)) */
          if ( ! (CPLib.Empty(rwWersonbo.NASSTATO))) {
            /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwWersonbo.NASSTATO,' '))+Space(30),30) + Left(LRTrim(rwWersonbo.SOTGRUP)+Space(3),3) + Left(LRTrim(_ramate)+Space(3),3) */
            cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwWersonbo.NASSTATO," "))+CPLib.Space(30),30)+CPLib.Left(CPLib.LRTrim(rwWersonbo.SOTGRUP)+CPLib.Space(3),3)+CPLib.Left(CPLib.LRTrim(_ramate)+CPLib.Space(3),3);
          } else { // Else
            /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwWersonbo.NASCOMUN,' '))+Space(30),30) + Left(LRTrim(rwWersonbo.SOTGRUP)+Space(3),3) + Left(LRTrim(_ramate)+Space(3),3) */
            cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwWersonbo.NASCOMUN," "))+CPLib.Space(30),30)+CPLib.Left(CPLib.LRTrim(rwWersonbo.SOTGRUP)+CPLib.Space(3),3)+CPLib.Left(CPLib.LRTrim(_ramate)+CPLib.Space(3),3);
          } // End If
          /* cRitorno := cRitorno + Left(LRTrim(rwWersonbo.SETTSINT)+Space(3),3) + Left(LRTrim(arfn_agechar(pRap,' '))+Space(25),25) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(rwWersonbo.SETTSINT)+CPLib.Space(3),3)+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(pRap," "))+CPLib.Space(25),25);
          /* cRitorno := cRitorno + Left(LRTrim(pTRap)+" ",1) + Left(LRTrim(rwWersonbo.TIPODOC)+Space(2),2) + Left(LRTrim(rwWersonbo.NUMDOCUM)+Space(15),15) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(pTRap)+" ",1)+CPLib.Left(CPLib.LRTrim(rwWersonbo.TIPODOC)+CPLib.Space(2),2)+CPLib.Left(CPLib.LRTrim(rwWersonbo.NUMDOCUM)+CPLib.Space(15),15);
          /* If Empty(rwWersonbo.DATARILASC) or DateToChar(rwWersonbo.DATARILASC)='01000101' */
          if (CPLib.Empty(rwWersonbo.DATARILASC) || CPLib.eqr(CPLib.DateToChar(rwWersonbo.DATARILASC),"01000101")) {
            /* cRitorno := cRitorno + Space(8) */
            cRitorno = cRitorno+CPLib.Space(8);
          } else { // Else
            /* cRitorno := cRitorno + Left(LRTrim(DateToChar(rwWersonbo.DATARILASC))+Space(8),8)  */
            cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(CPLib.DateToChar(rwWersonbo.DATARILASC))+CPLib.Space(8),8);
          } // End If
          /* cRitorno := cRitorno + Left(LRTrim(arfn_agechar(rwWersonbo.AUTRILASC,' '))+Space(30),30) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(arfn_agecharR.Make(m_Ctx,this).Run(rwWersonbo.AUTRILASC," "))+CPLib.Space(30),30);
          /* cRitorno := cRitorno + Left(LRTrim(rwWersonbo.SESSO)+" ",1) + Space(22) */
          cRitorno = cRitorno+CPLib.Left(CPLib.LRTrim(rwWersonbo.SESSO)+" ",1)+CPLib.Space(22);
          /* cRitorno := cRitorno + iif(not(Empty(rwWersonbo.DATAVALI)),DateToChar(rwWersonbo.DATAVALI),Space(8)) */
          cRitorno = cRitorno+( ! (CPLib.Empty(rwWersonbo.DATAVALI))?CPLib.DateToChar(rwWersonbo.DATAVALI):CPLib.Space(8));
          /* cRitorno := cRitorno + iif(rwWersonbo.CFESTERO=1,'NOCF',Space(4)) */
          cRitorno = cRitorno+(CPLib.eqr(rwWersonbo.CFESTERO,1)?"NOCF":CPLib.Space(4));
        } // End If
      }
    } // End If
  }
      ...
      if (CPLib.Empty(CPLib.LRTrim(_numsto))) {
        ...
        while ( ! (Cursor_personbo.Eof())) {
          ...
          if ( ! (Read_Cursor.Eof())) {
            ...
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on tbstgru into routine arfn_datisog_sol_mc returns two or more records. This item should return only a record.");
  ...
  String chkspers(java.sql.Date pData) throws Exception {
    /* pData Date */
    /* Variabili Locali */
    /* cProg Char(15) */
    String cProg;
    cProg = CPLib.Space(15);
    /* dData Date */
    java.sql.Date dData;
    dData = CPLib.NullDate();
    /* Verifica presenza storico anagrafico */
    /* cProg := '' */
    cProg = "";
    /* dData := NullDate() */
    dData = CPLib.NullDate();
    for (MemoryCursorRow_wersonbo rwWersonbo : mcWersonbo._iterable_()) {
      /* If rwWersonbo.datamodi > pData */
      if (CPLib.gt(rwWersonbo.datamodi,pData)) {
        /* cProg := rwWersonbo.sanumpro */
        cProg = rwWersonbo.sanumpro;
      } else { // Else
        // Exit Loop
        if (true) {
  ...
  public String RunAsync(String p_pCli,java.sql.Date p_pDatOpe,String p_pRap,String p_pTRap,String p_pTipoR,String p_pRAE,String p_pSTATO,java.sql.Date p_pDatRet,MemoryCursor_personbo p_mcPersonbo,MemoryCursor_wersonbo p_mcWersonbo) {
    pCli = p_pCli;
    pDatOpe = p_pDatOpe;
    pRap = p_pRap;
    pTRap = p_pTRap;
    ...
    pSTATO = p_pSTATO;
    pDatRet = p_pDatRet;
    mcPersonbo = p_mcPersonbo;
    mcWersonbo = p_mcWersonbo;
  ...
  public String Run(String p_pCli,java.sql.Date p_pDatOpe,String p_pRap,String p_pTRap,String p_pTipoR,String p_pRAE,String p_pSTATO,java.sql.Date p_pDatRet,MemoryCursor_personbo p_mcPersonbo,MemoryCursor_wersonbo p_mcWersonbo) {
    pCli = p_pCli;
    pDatOpe = p_pDatOpe;
    pRap = p_pRap;
    pTRap = p_pTRap;
  ...
  public static arfn_datisog_sol_mcR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_datisog_sol_mcR(p_Ctx, p_Caller);
  }
  public static arfn_datisog_sol_mcR Make(CPContext p_Ctx) {
    return new arfn_datisog_sol_mcR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
    pCli = CPLib.Space(16);
  ...
  // ENTITY_BATCHES: ,arfn_agechar,arfn_chkspers,arfn_datisog_sol_mc,
  public static final String i_InvokedRoutines = ",arfn_agechar,arfn_chkspers,arfn_datisog_sol_mc,";
  public static String[] m_cRunParameterNames={"pCli","pDatOpe","pRap","pTRap","pTipoR","pRAE","pSTATO","pDatRet","mcPersonbo","mcWersonbo"};
}
