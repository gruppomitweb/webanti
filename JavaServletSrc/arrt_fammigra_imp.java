...
public class arrt_fammigra_imp extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*f45b70c*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_fammigra_impR Rcalled;
    Rcalled = new arrt_fammigra_impR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_fammigra_impR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_fammigra_impR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_fammigra_imp"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
