...
l_oWv.setValue('w_flgnuovi',AsAppletValue(w_flgnuovi));
...
l_oWv.setValue('m_cParameterSequence',AsAppletValue('w_flgnuovi,w_anno,w_mese,w_flgstp'));
if (false || WindowConfirm(Translate('822914937'))) {
SetLocationHref(null,'arrt_famprimo'+'?m_cWv='+URLenc(l_oWv.asString())+'&m_cAction=start&m_cMode=hyperlink','elaborazione')
}
}
...
w_Messaggi.raiseEvent('arpg_famprimo_ip_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_famprimo_ip_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_famprimo_ip_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_famprimo_ip',m_cSelectedPage);
...
_FillChildren.n=[];
FillWv.n=["gIntemediario","gDescAzi","anno","mese","dadata","a_data","flgnuovi","flgstp"];
