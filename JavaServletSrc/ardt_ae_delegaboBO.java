...
// * --- Fine Area Manuale
public class ardt_ae_delegaboBO extends ardt_ae_delegaboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_ae_delegaboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_ae_delegabo");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_jelegabo+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_jelegabo,m_cPhName_jelegabo,m_cVirtName_jelegabo,m_cVirtName_jelegabo+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_jelegabo,p_RAPPORTO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"jelegabo")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            w_descdele = l_rsLoadBody.GetString("RAGSOC201");
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_jelegabo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_jelegabo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"jelegabo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_jelegabo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_jelegabo);
      l_cSql = "insert into "+m_oWrInfo_jelegabo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_jelegabo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"jelegabo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_jelegabo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"jelegabo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_jelegabo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_jelegabo);
      l_cSql = "Update "+m_oWrInfo_jelegabo.GetTableWriteName()+" set";
