...
public class arrt_imp_dati_multipli_ac extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*GVWNQFSNYA*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_imp_dati_multipli_ac";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_xMADIPSTD) ||  ! (status.m_bpassedw_xMAFLGASR) ||  ! (status.m_bpassedw_xMAFLGCKR) ||  ! (status.m_bpassedw_xMAFLGBON) ||  ! (status.m_bpassedw_xMAFLGRAP) ||  ! (status.m_bpassedw_destfin) ||  ! (status.m_bpassedw_codmage) ||  ! (status.m_bpassedw_opextc) ||  ! (status.m_bpassedw_impzip) ||  ! (status.m_bpassedw_tipodest);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_imp_dati_multipli_acR Rcalled;
    Rcalled = new arrt_imp_dati_multipli_acR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_imp_dati_multipli_acR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_imp_dati_multipli_acR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_imp_dati_multipli_ac"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_imp_dati_multipli_ac");
      if ( ! (CPLib.IsNull(l))) {
