...
public class arpg_stpfraz extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*JKPKKDPLKR*!";
  ...
  public String getEntityTitle() {
    return "Stampa Frazionate";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_stpfraz";
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("aCodDip","C",16,0, "edit",""),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_stpfrazBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"786254202",p_Context.Translate("Stampa Frazionate")},
    {"862812006",p_Context.Translate("Tipo operazione")}};
    return l_translations;
  }
  void WriteStyles(arpg_stpfrazBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_stpfraz.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_stpfrazBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_stpfrazBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_stpfrazBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(648);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_stpfrazBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<input name='aCodDip' id='HXBJUMEZIA' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_aCodDip,"C",16,0,"", status.m_oRegSettings)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"-1":""+status.m_nTabIndex+"")+"' curTabIndex='"+status.m_nTabIndex+"' maxlength='16' size='16' fieldtype='C'>" +
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<a href='javascript:href_GBFLGJEXQC()' tabindex='"+status.m_nTabIndex+"' id='GBFLGJEXQC_HREF' curTabIndex='"+status.m_nTabIndex+"' style='display:none'>" +
     "<span id='GBFLGJEXQC'>" +
  ...
  static void PrintState(arpg_stpfrazBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_stpfrazBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_stpfrazBL BO,ServletStatus status) {
    status.out.print("DeclareWVs(");
    status.out.print(SPLib.ToJSValue(BO.w_daCodDip,"C",6,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_aCodDip,"C",16,0));
  ...
  void WriteMainFormVarScript(arpg_stpfrazBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_stpfraz"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_stpfraz","");
  ...
  void WriteMainFormScript(arpg_stpfrazBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_stpfrazBO getBO(ServletStatus status) {
    arpg_stpfrazBO BO = new arpg_stpfrazBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_stpfrazBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_stpfrazBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_stpfrazBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_stpfrazBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_stpfrazBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_stpfrazBO BO) {
    ...
    item.put("altInterfaceFor","arpg_stpfraz");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_type\":\"\",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"639\",\"name\":\"Box_XLVFHSOSYH\",\"page\":\"1\",\"shrinkable\":\"true\",\"spuid\":\"XLVFHSOSYH\",\"tabindex\":\"1\",\"type\":\"Box\",\"w\":\"784\",\"x\":\"8\",\"y\":\"9\",\"zone\":\"pag1_1\"}";
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"aCodDip\",\"page\":\"1\",\"spuid\":\"HXBJUMEZIA\",\"tabindex\":\"3\",\"type\":\"Textbox\",\"w\":\"70\",\"x\":\"144\",\"y\":\"46\",\"zone\":\"XLVFHSOSYH_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"daCodCli\",\"page\":\"1\",\"spuid\":\"UYVYTEMEXX\",\"tabindex\":\"6\",\"type\":\"Textbox\",\"w\":\"120\",\"x\":\"144\",\"y\":\"99\",\"zone\":\"XLVFHSOSYH_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"16\",\"name\":\"aCodCli\",\"page\":\"1\",\"spuid\":\"CJWUHWULAR\",\"tabindex\":\"7\",\"type\":\"Textbox\",\"w\":\"120\",\"x\":\"144\",\"y\":\"127\",\"zone\":\"XLVFHSOSYH_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_XURLGQZZMU()\",\"name\":\"_60165\",\"page\":\"1\",\"spuid\":\"XURLGQZZMU\",\"tabindex\":\"11\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"328\",\"y\":\"324\",\"zone\":\"XLVFHSOSYH_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_PTXYMSWBZO()\",\"name\":\"_59890\",\"page\":\"1\",\"spuid\":\"PTXYMSWBZO\",\"tabindex\":\"12\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"424\",\"y\":\"324\",\"zone\":\"XLVFHSOSYH_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_RAGSOC1\",\"page\":\"1\",\"spuid\":\"VXJAZUGUIO_DIV\",\"tabindex\":\"17\",\"type\":\"Label\",\"value\":\"w_RAGSOC1\",\"w\":\"384\",\"x\":\"288\",\"y\":\"103\",\"zone\":\"XLVFHSOSYH_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_RAGSOC2\",\"page\":\"1\",\"spuid\":\"XCBBNKIAUV_DIV\",\"tabindex\":\"19\",\"type\":\"Label\",\"value\":\"w_RAGSOC2\",\"w\":\"384\",\"x\":\"288\",\"y\":\"131\",\"zone\":\"XLVFHSOSYH_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"33\",\"href\":\"javascript:href_GBFLGJEXQC()\",\"name\":\"_60044\",\"page\":\"1\",\"spuid\":\"GBFLGJEXQC\",\"tabindex\":\"27\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"376\",\"y\":\"324\",\"zone\":\"XLVFHSOSYH_10\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Agenzia_\",\"page\":\"1\",\"spuid\":\"ZWZOQIGZCX_DIV\",\"tabindex\":\"28\",\"type\":\"Label\",\"value\":\"Da Agenzia:\",\"w\":\"66\",\"x\":\"75\",\"y\":\"22\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_DESCRIZ1\",\"page\":\"1\",\"spuid\":\"RXKVDVJPTZ_DIV\",\"tabindex\":\"29\",\"type\":\"Label\",\"value\":\"w_DESCRIZ1\",\"w\":\"432\",\"x\":\"240\",\"y\":\"22\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Ad_Agenzia_\",\"page\":\"1\",\"spuid\":\"BGOVXFIBXQ_DIV\",\"tabindex\":\"30\",\"type\":\"Label\",\"value\":\"Ad Agenzia:\",\"w\":\"66\",\"x\":\"75\",\"y\":\"50\",\"zone\":\"XLVFHSOSYH_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_DESCRIZ2\",\"page\":\"1\",\"spuid\":\"LNFAXDLCMF_DIV\",\"tabindex\":\"31\",\"type\":\"Label\",\"value\":\"w_DESCRIZ2\",\"w\":\"432\",\"x\":\"240\",\"y\":\"50\",\"zone\":\"XLVFHSOSYH_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"279\",\"href\":\" \",\"name\":\"__iframe_width__775__height__290__id__elaborazione___name__elaborazione__frameborder__1___border__1__align__TOP__marginwidth__1__marginheight__1____iframe__\",\"page\":\"1\",\"spuid\":\"FGMOCXNBFY_DIV\",\"tabindex\":\"34\",\"type\":\"Label\",\"value\":\"\\\"<iframe width="+"'"+"775"+"'"+" height="+"'"+"290"+"'"+" id="+"'"+"elaborazione"+"'"+"  name="+"'"+"elaborazione"+"'"+" frameborder="+"'"+"1"+"'"+"  border="+"'"+"1"+"'"+" align="+"'"+"TOP"+"'"+" marginwidth="+"'"+"1"+"'"+" marginheight="+"'"+"1"+"'"+"><\\/iframe>\\\"\",\"w\":\"768\",\"x\":\"16\",\"y\":\"360\",\"zone\":\"XLVFHSOSYH_12\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_stpfrazBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_stpfrazBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_stpfrazBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_stpfraz?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_stpfraz_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_stpfrazBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_stpfrazBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_stpfrazBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_stpfrazBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_stpfrazBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_stpfrazBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_stpfraz.GetCallableNames(callable);
      arrt_stpfraz_xls.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_stpfrazBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_stpfrazBO BO) {
    boolean result = true;
    return result;
  }
}
