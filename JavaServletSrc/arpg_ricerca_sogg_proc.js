...
if(c=Ctrl('HOBEZFAZAD')) c.value = WtH(w_codfisc,'C',16,0,'@!')
...
HideUI.lblids["codfisc"]=['XQCCNEFYPD'];
...
function HOBEZFAZAD_Valid(e) {
SetActiveField(Ctrl('HOBEZFAZAD'),false);
e=(Ne(e,null)?e:window.event);
return Set_codfisc(HtW(Ctrl('HOBEZFAZAD').value,'C'),null,e);
}
function HOBEZFAZAD_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('HOBEZFAZAD'))) {
DisplayErrorMessage();
}
Ctrl('HOBEZFAZAD').value=WtH(w_codfisc,'C',16,0,'@!');
SetActiveField(Ctrl('HOBEZFAZAD'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('HOBEZFAZAD'),e);
}
function HOBEZFAZAD_CheckNumWithPict(e) {
return CheckNumWithPict(e,GetModDecPict("@!"));
}
...
function href_KFBPLHZCBX() {
_modifyandopen('arrt_ricerca_sogg'+'?m_cAction=start&m_cAtExit=close&m_cMode=hyperlink','toolbar=0,menubar=0,directories=0,resizable=1,scrollbars=yes,height=600,width=800',null)
}
_ResetTracker();
...
if(c=Ctrl('HOBEZFAZAD')) c.onblur=HOBEZFAZAD_Valid;
if(c=Ctrl('HOBEZFAZAD')) c.onfocus=HOBEZFAZAD_OnFocus;
if(c=Ctrl('HOBEZFAZAD')) c.onkeypress=CheckUpper;
...
function Help() {
windowOpenForeground('../doc/arpg_ricerca_sogg_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_ricerca_sogg',m_cSelectedPage);
...
l_bEnabled= ! (Eq(w_criterio,'C')) || IsDisabledByStateDriver('codfisc');
SetDisabled(Ctrl('HOBEZFAZAD'),l_bEnabled);
...
var ctl = _GetCtl(e,'HOBEZFAZAD');
if (_tracker.goon(ctl,ctrlValue)) {
w_codfisc=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('HOBEZFAZAD',0);
DoUpdate(l_bResult);
if(c=Ctrl('HOBEZFAZAD')) c.value = WtH(w_codfisc,'C',16,0,'@!')
} else {
...
l_bResult=false;
w_denomin=HtW('','C');
} else if ((Eq(w_criterio,'C')) && ( ! (_ChkObl('HOBEZFAZAD',1)))) {
_SignErr('HOBEZFAZAD');
m_cLastWorkVarErrorMsg='codfisc';
l_bResult=false;
w_codfisc=HtW('','C');
...
_FillChildren.n=[];
FillWv.n=["criterio","denomin","codfisc","simili","flgnote","flgnega"];
...
TransferBetweenWindows(opener,'w_codfisc',null,'Set_codfisc');
}
...
_Obli('codfisc',0,'HOBEZFAZAD',false)
