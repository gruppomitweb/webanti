...
// * --- Fine Area Manuale
public class arrt_writelog_impR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_writelog_impR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_writelog_imp",m_Caller);
    ...
    try {
      ...
      if (CPLib.eqr(cActLog,"S")) {
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on cpusers into routine arrt_writelog_imp returns two or more records. This item should return only a record.");
        ...
        try {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arrt_writelog_imp",35,"00000027")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
  ...
  public static arrt_writelog_impR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_writelog_impR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pOperazione = CPLib.Space(15);
    pTabella = CPLib.Space(20);
    pChiave = CPLib.Space(0);
    pTesto = "";
  ...
  public static String[] m_cRunParameterNames={"pOperazione","pTabella","pChiave","pTesto"};
