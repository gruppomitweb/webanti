...
// * --- Fine Area Manuale
public class ardt_provinformazioniBO extends ardt_provinformazioniBL {
  ...
  // * --- Fine Area Manuale
  public ardt_provinformazioniBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_provinformazioni");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_xntestbo+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xntestbo,m_cPhName_xntestbo,m_cVirtName_xntestbo,m_cVirtName_xntestbo+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_xntestbo,p_RAPPORTO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xntestbo")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_xntestbo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_xntestbo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xntestbo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_xntestbo,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xntestbo);
      l_cSql = "insert into "+m_oWrInfo_xntestbo.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+"PRGIMPSOG,";
      l_cSql = l_cSql+"PROQUOTA,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_xntestbo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xntestbo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xntestbo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xntestbo);
      l_cSql = "Update "+m_oWrInfo_xntestbo.GetTableWriteName()+" set";
      l_cSql = l_cSql+" IDBASE = "+CPLib.ToSQL(w_IDBASE,"C",10,0);
      l_cSql = l_cSql+", CODINTER = "+CPLib.ToSQLNull(w_CODINTER,"C",16,0);
      ...
      l_cSql = l_cSql+", PRGIMPSOG = "+CPLib.ToSQL(w_PRGIMPSOG,"N",15,0);
