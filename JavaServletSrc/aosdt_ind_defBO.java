...
// * --- Fine Area Manuale
public class aosdt_ind_defBO extends aosdt_ind_defBL {
  ...
  // * --- Fine Area Manuale
  public aosdt_ind_defBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"aosdt_ind_def");
    }
  ...
  public boolean CanSave() {
    ...
    if (l_bResult) {
      ...
      while (l_bResult && CPLib.lt(l_nRow,Rows())) {
        ...
        if (l_bResult) {
          l_bResult = CanSaveChild(aosdt_sub_ind_def,m_cWv_aosdt_sub_ind_def,"m_cWv_aosdt_sub_ind_def");
          if (l_bResult) {
          } else {
  ...
  public boolean Load(String p_IDMOD) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_ind_def+".PROGIND";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_ind_def,m_cPhName_ind_def,m_cVirtName_ind_def,m_cVirtName_ind_def+".IDMOD="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDMOD,"C",10,0),m_cServer_ind_def,p_IDMOD),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"ind_def")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          w_PROGIND = l_rsLoadBody.GetString("PROGIND");
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        // Cancella il figlio Sub Indicatori anomalia
        aosdt_sub_ind_def.w_IDMOD = w_IDMOD;
        aosdt_sub_ind_def.w_IDIND = w_IDIND;
        aosdt_sub_ind_def.Delete();
      }
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_ind_def.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDMOD="+CPLib.ToSQL(w_IDMOD,"C",10,0)+" AND IDIND="+CPLib.ToSQL(opk_IDIND,"C",10,0);
        l_cSql = l_cSql+m_oWrInfo_ind_def.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"ind_def")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_ind_def,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_ind_def);
      l_cSql = "insert into "+m_oWrInfo_ind_def.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDMOD,";
      l_cSql = l_cSql+"IDIND,";
      l_cSql = l_cSql+"PROGIND,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+"LOCKMIT,";
      l_cSql = l_cSql+"NOAPPLIC,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_ind_def.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"ind_def")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_PROGIND,"C",10,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DESCRI,"M",10,0,m_cServer_ind_def,m_oParameters)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_LOCKMIT,"L",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_NOAPPLIC,"L",1,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_ind_def.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"ind_def")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_ind_def,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
        NotifyEvent("Insert row children start");
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          if ( ! (aosdt_sub_ind_def.IsUpdated()) && CPLib.IsUpdated(m_cWv_aosdt_sub_ind_def)) {
          ...
          if (aosdt_sub_ind_def.IsUpdated()) {
            aosdt_sub_ind_def.w_IDMOD = w_IDMOD;
            aosdt_sub_ind_def.w_IDIND = w_IDIND;
            if ( ! (aosdt_sub_ind_def.Save(true))) {
              m_Sql.AbortTransaction();
            }
          }
        }
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_ind_def);
      l_cSql = "Update "+m_oWrInfo_ind_def.GetTableWriteName()+" set";
      l_cSql = l_cSql+" PROGIND = "+CPLib.ToSQL(w_PROGIND,"C",10,0);
      l_cSql = l_cSql+", DESCRI = "+CPLib.ToSQL(w_DESCRI,"M",10,0,m_cServer_ind_def,m_oParameters);
