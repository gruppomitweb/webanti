...
// * --- Fine Area Manuale
public class armt_tbcandistrBO extends armt_tbcandistrBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbcandistrBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbcandistr");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbcandistr,m_cPhName_tbcandistr,m_cVirtName_tbcandistr,m_cVirtName_tbcandistr+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",3,0),m_cServer_tbcandistr,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcandistr")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbcandistr);
      l_cSql = "insert into "+m_oWrInfo_tbcandistr.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbcandistr.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcandistr")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_DESCRI,"C",50,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbcandistr.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcandistr")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbcandistr,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbcandistr);
      l_cSql = "Update "+m_oWrInfo_tbcandistr.GetTableWriteName()+" set";
      l_cSql = l_cSql+" DESCRI = "+CPLib.ToSQL(w_DESCRI,"C",50,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcandistr")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",3,0),m_cServer_tbcandistr,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbcandistr.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcandistr")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbcandistr.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",3,0),m_cServer_tbcandistr,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbcandistr.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbcandistr")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbcandistr,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
