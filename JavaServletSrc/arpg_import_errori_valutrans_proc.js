...
HideUI.lblids["tipodest"]=['FKEESJLEWC'];
...
SetLocationHref(null,'arrt_import_rtra_errori'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')
}
...
w_Messaggi.raiseEvent('arpg_import_errori_valutrans_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_Log),'undefined')) {
w_Log.raiseEvent('arpg_import_errori_valutrans_'+Strtran(p_cEvent,' ','_'));
}
if (Ne(typeof(w_ListaFile),'undefined')) {
w_ListaFile.raiseEvent('arpg_import_errori_valutrans_'+Strtran(p_cEvent,' ','_'));
}
}
...
function Help() {
windowOpenForeground('../doc/arpg_import_errori_valutrans_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_import_errori_valutrans',m_cSelectedPage);
...
_SignErr('WIRRRKLEWO');
m_cLastWorkVarErrorMsg='139951853';
l_bResult=false;
w_tipodest=HtW('','C');
}
...
_FillChildren.n=[];
FillWv.n=["nomefile","tipodest","impzip","gAzienda","gPathApp"];
...
_Obli('tipodest',0,'WIRRRKLEWO',false,"139951853")
