...
// * --- Fine Area Manuale
public class arrt_seldes_rapmod_atR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_seldes_rapmod_atR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_seldes_rapmod_at",m_Caller);
  ...
  void Page_1() throws Exception {
    /* Parametro */
    /* pTipo Char(1) */
    /* Verifica il parametro e aggiorna */
    /* If pTipo='S' */
    if (CPLib.eqr(pTipo,"S")) {
      // * --- Write into tmp_newrapp from values
      m_Sql.RequireTransaction();
      m_cServer = m_Ctx.GetServer("tmp_newrapp");
      m_cPhName = m_Ctx.GetPhName("tmp_newrapp");
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_newrapp",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_seldes_rapmod_at",6,"00000005")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      m_cSql = m_cSql+"TIPO = "+CPLib.ToSQL("S","C",1,0,m_cServer)+", ";
      m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"tmp_newrapp",false,true);
      m_cSql = m_cSql+" where ";
      ...
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_newrapp",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_seldes_rapmod_at",6,"00000006")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      m_cSql = m_cSql+"TIPO = "+CPLib.ToSQL("N","C",1,0,m_cServer)+", ";
      m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"tmp_newrapp",false,true);
      m_cSql = m_cSql+" where ";
      m_cSql = m_cSql+"1 = "+CPSql.SQLValueAdapter(CPLib.ToSQL(1,"?",0,0,m_cServer),m_cServer,1)+"";
      m_cSql = m_cSql+m_oWrInfo.WhereFilter();
    ...
    /* Return  */
    Forward f;
    f=new Forward("arpg_rapmod_at",false,this,Forward.Start,false);
  ...
  public static arrt_seldes_rapmod_atR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_seldes_rapmod_atR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pTipo = CPLib.Space(1);
  }
