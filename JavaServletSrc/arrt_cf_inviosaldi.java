...
public class arrt_cf_inviosaldi extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*b4f87614*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_nomefile) ||  ! (status.m_bpassedw_flgope) ||  ! (status.m_bpassedw_flgsos) ||  ! (status.m_bpassedw_flgann) ||  ! (status.m_bpassedw_fileprodotto) ||  ! (status.m_bpassedw_dadata) ||  ! (status.m_bpassedw_a_data) ||  ! (status.m_bpassedw_anno);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_cf_inviosaldiR Rcalled;
    Rcalled = new arrt_cf_inviosaldiR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_cf_inviosaldiR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_cf_inviosaldiR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_cf_inviosaldi"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_cf_inviosaldi");
      if ( ! (CPLib.IsNull(l))) {
