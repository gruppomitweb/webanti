...
public class arfn_anomsaldi_opex extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*adb751c8*!";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arfn_anomsaldi_opexR Rcalled;
    Rcalled = new arfn_anomsaldi_opexR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arfn_anomsaldi_opexR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arfn_anomsaldi_opexR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arfn_anomsaldi_opex"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
