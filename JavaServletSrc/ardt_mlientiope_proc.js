...
SPBOUILib.SetInputValue('FRMNDCFDLT',function(){return WtH(w_PROQUOTA,'N',3,0,'')},w_PROQUOTA);
SPBOUILib.SetInputValue('WWCVGFCWRT',function(){return WtH(w_IMPSARA,'N',19,2,'99,999,999,999,999.99')},w_IMPSARA);
}
if (m_bExtended) {
SetExtensions();
}
...
HideUI.lblids["IMPSARA"]=['MXACIGAZQB'];
HideUI.lblids["PROQUOTA"]=['ZTLCEVOAHM'];
...
function FRMNDCFDLT_Valid(e) {
SetActiveField(Ctrl('FRMNDCFDLT'),false);
if (Eq(m_cFunction,'query')) {
DoQuery('PROQUOTA','N',w_PROQUOTA,HtW(Ctrl('FRMNDCFDLT').value,'N'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_PROQUOTA(HtW(Ctrl('FRMNDCFDLT').value,'N'),null,e);
return l_bSetResult;
}
}
function FRMNDCFDLT_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('FRMNDCFDLT'))) {
DisplayErrorMessage();
}
SetActiveField(Ctrl('FRMNDCFDLT'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('FRMNDCFDLT'),e);
}
function WWCVGFCWRT_Valid(e) {
SetActiveField(Ctrl('WWCVGFCWRT'),false);
if (Eq(m_cFunction,'query')) {
DoQuery('IMPSARA','N',w_IMPSARA,HtW(Ctrl('WWCVGFCWRT').value,'N'));
} else {
e=(Ne(e,null)?e:window.event);
var l_bSetResult;
l_bSetResult=Set_IMPSARA(HtW(Ctrl('WWCVGFCWRT').value,'N'),null,e);
return l_bSetResult;
}
}
function WWCVGFCWRT_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('WWCVGFCWRT'))) {
DisplayErrorMessage();
}
SPBOUILib.WtH('WWCVGFCWRT',w_IMPSARA,'N',19,2,'9999999999999999.99');
SetActiveField(Ctrl('WWCVGFCWRT'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('WWCVGFCWRT'),e);
}
function WWCVGFCWRT_CheckNumWithPict(e) {
...
function FRMNDCFDLT_wrap_Valid(event) {
return FRMNDCFDLT_Valid(event);
}
SPBOUILib.SetInputExit('FRMNDCFDLT',FRMNDCFDLT_wrap_Valid,SPBOUILib.Row.IN_GRID);
function FRMNDCFDLT_wrap_OnFocus(event) {
return FRMNDCFDLT_OnFocus(event);
}
SPBOUILib.SetInputEnter('FRMNDCFDLT',FRMNDCFDLT_wrap_OnFocus,SPBOUILib.Row.IN_GRID);
function FRMNDCFDLT_wrap_OnKeyPress(event) {
return CheckNum(event);
}
SPBOUILib.SetInputKeyPress('FRMNDCFDLT',FRMNDCFDLT_wrap_OnKeyPress,SPBOUILib.Row.IN_GRID);
function FRMNDCFDLT_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('FRMNDCFDLT',FRMNDCFDLT_wrap_OnKeyDown,SPBOUILib.Row.IN_GRID);
function WWCVGFCWRT_wrap_Valid(event) {
return WWCVGFCWRT_Valid(event);
}
SPBOUILib.SetInputExit('WWCVGFCWRT',WWCVGFCWRT_wrap_Valid,SPBOUILib.Row.IN_GRID);
function WWCVGFCWRT_wrap_OnFocus(event) {
return WWCVGFCWRT_OnFocus(event);
}
SPBOUILib.SetInputEnter('WWCVGFCWRT',WWCVGFCWRT_wrap_OnFocus,SPBOUILib.Row.IN_GRID);
function WWCVGFCWRT_wrap_OnKeyPress(event) {
return WWCVGFCWRT_CheckNumWithPict(event);
}
SPBOUILib.SetInputKeyPress('WWCVGFCWRT',WWCVGFCWRT_wrap_OnKeyPress,SPBOUILib.Row.IN_GRID);
function WWCVGFCWRT_wrap_OnKeyDown(event) {
return ManageKeys(event,Eq(window.m_cFunction,'query'),false,false,false);
}
SPBOUILib.SetInputKeyDown('WWCVGFCWRT',WWCVGFCWRT_wrap_OnKeyDown,SPBOUILib.Row.IN_GRID);
if (m_bExtended) {
...
m_oFather.i_last_focused_item='child_ardt_mlientiope';
...
function Help() {
windowOpenForeground('../doc/ardt_mlientiope_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
return 'mlientiope'+'\\'+CPLib.ToCPStr(w_IDFILEBO);
}
...
p_d.setValue('ardt_mlientiope',m_cSelectedPage);
...
_Obli('IMPSARA',1,'WWCVGFCWRT',false,"1992290172")
_Obli('CODSOG',0,'TMTNRFTARW',false,"10847988462")
_Obli('PROQUOTA',1,'FRMNDCFDLT',false,"11230801199")
