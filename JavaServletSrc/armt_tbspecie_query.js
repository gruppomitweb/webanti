...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('KQFYMJDVWR').value,'C'),w_CODSPE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbspecie',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODSPE') &&  ! (Empty(HtW(Ctrl('KQFYMJDVWR').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODSPE',HtW(Ctrl('KQFYMJDVWR').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_tbspecie');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbspecie',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['KQFYMJDVWR'],i=0;i>=0;i--){
