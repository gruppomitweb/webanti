...
// * --- Fine Area Manuale
public class armt_tipregBO extends armt_tipregBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tipregBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tipreg");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbtipreg,m_cPhName_tbtipreg,m_cVirtName_tbtipreg,m_cVirtName_tbtipreg+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",2,0),m_cServer_tbtipreg,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        w_DES1 = l_rsLoad.GetString("DES1");
        w_DES2 = l_rsLoad.GetString("DES2");
        w_TIPREG = l_rsLoad.GetString("TIPREG");
        w_OPERAZIO = l_rsLoad.GetString("OPERAZIO");
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipreg")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbtipreg);
      l_cSql = "insert into "+m_oWrInfo_tbtipreg.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"DES1,";
      l_cSql = l_cSql+"DES2,";
      l_cSql = l_cSql+"TIPREG,";
      l_cSql = l_cSql+"OPERAZIO,";
      l_cSql = l_cSql+"DATAINI,";
      l_cSql = l_cSql+"DATAFINE,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbtipreg.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipreg")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_DES1,"C",90,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DES2,"C",60,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_TIPREG,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_OPERAZIO,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DATAINI,"D",8,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DATAFINE,"D",8,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbtipreg.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipreg")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbtipreg,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbtipreg);
      l_cSql = "Update "+m_oWrInfo_tbtipreg.GetTableWriteName()+" set";
      l_cSql = l_cSql+" DES1 = "+CPLib.ToSQL(w_DES1,"C",90,0);
      l_cSql = l_cSql+", DES2 = "+CPLib.ToSQL(w_DES2,"C",60,0);
      l_cSql = l_cSql+", TIPREG = "+CPLib.ToSQL(w_TIPREG,"C",1,0);
      l_cSql = l_cSql+", OPERAZIO = "+CPLib.ToSQL(w_OPERAZIO,"C",1,0);
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",2,0),m_cServer_tbtipreg,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbtipreg.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipreg")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbtipreg.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",2,0),m_cServer_tbtipreg,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbtipreg.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipreg")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbtipreg,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
