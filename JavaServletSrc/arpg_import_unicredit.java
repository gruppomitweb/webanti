...
public class arpg_import_unicredit extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*NGRNHWUBXW*!";
  ...
  public String getEntityTitle() {
    return "Import Rapporti UNICREDIT";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_import_unicredit";
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("destfin","C",1,0, "hide","Scrivi su Operazioni o Frazionate"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_import_unicreditBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11860612861",p_Context.Translate("Import Rapporti UNICREDIT")},
  ...
  void WriteStyles(arpg_import_unicreditBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_import_unicredit.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_import_unicreditBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_import_unicreditBO BO,ServletStatus status) {
  ...
  // GetPortlet_Object_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_import_unicreditBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_import_unicreditBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_import_unicreditBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(385);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_import_unicreditBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_import_unicreditBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_import_unicreditBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_import_unicreditBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_import_unicreditBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_import_unicredit"),"C",0,0)+ ";");
    status.out.println("var m_bShowSuggest = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    status.out.println("var m_bShowNotifyBtn = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    status.out.println("var m_bShowExtendReduceBtn = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    HttpSession l_session = ((SPContext)status.context).session;
    status.out.print("var m_IDS={");
    status.out.print("'XFVQTXPEXL':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash("MACODICEMACODICE,MADESCRI,MAFLGRAP,MAFLGOPE,MAFLGBON,MADIPSTDC,C,C,C,C,Ctbmacage",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,tbmacage,"+"default",l_session)+"'"+"]");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_import_unicredit","");
  ...
  void WriteMainFormScript(arpg_import_unicreditBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_import_unicreditBO getBO(ServletStatus status) {
    arpg_import_unicreditBO BO = new arpg_import_unicreditBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_import_unicreditBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_import_unicreditBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_import_unicreditBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_import_unicreditBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_import_unicreditBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_import_unicreditBO BO) {
    ...
    item.put("altInterfaceFor","arpg_import_unicredit");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"MacroAgente\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"codmage\",\"page\":\"1\",\"spuid\":\"XFVQTXPEXL\",\"tabindex\":\"8\",\"type\":\"Textbox\",\"w\":\"64\",\"x\":\"88\",\"y\":\"84\",\"zone\":\"YWGFSYCMOZ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"MacroAgente_\",\"page\":\"1\",\"spuid\":\"EFKLJNCNTE_DIV\",\"tabindex\":\"14\",\"type\":\"Label\",\"value\":\"MacroAgente:\",\"w\":\"68\",\"x\":\"13\",\"y\":\"88\",\"zone\":\"YWGFSYCMOZ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"w_xMADESCRI\",\"page\":\"1\",\"spuid\":\"LXTFLROIJK_DIV\",\"tabindex\":\"15\",\"type\":\"Label\",\"value\":\"w_xMADESCRI\",\"w\":\"504\",\"x\":\"176\",\"y\":\"88\",\"zone\":\"YWGFSYCMOZ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"294\",\"zone\":\"YWGFSYCMOZ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"169\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"119\",\"zone\":\"YWGFSYCMOZ_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
  ...
  void Next(ServletStatus status,arpg_import_unicreditBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_import_unicreditBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_import_unicreditBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_import_unicredit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_import_unicredit_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_import_unicreditBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_import_unicreditBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_import_unicreditBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_import_unicreditBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_import_unicreditBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_import_unicreditBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_import_unicredit.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_import_unicreditBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_import_unicreditBO BO) {
    boolean result = true;
    return result;
  }
}
