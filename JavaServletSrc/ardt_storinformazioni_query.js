...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('QPLXZEEVEI').value,'C'),w_RAPPORTO)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('wntestbo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'RAPPORTO') &&  ! (Empty(HtW(Ctrl('QPLXZEEVEI').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.RAPPORTO',HtW(Ctrl('QPLXZEEVEI').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'detail','ardt_storinformazioni');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('wntestbo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['QPLXZEEVEI'],i=0;i>=0;i--){
