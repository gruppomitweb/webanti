...
public class arpg_stpfraz_sto extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*DGXPZZMOHI*!";
  ...
  public String getEntityTitle() {
    return "Stampa Frazionate Storiche (Pagina)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_stpfraz_sto";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_stpfraz_stoBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"10858344272",p_Context.Translate("Stampa Frazionate Storiche (Pagina)")}};
    return l_translations;
  }
  void WriteStyles(arpg_stpfraz_stoBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_stpfraz_sto.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_stpfraz_stoBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_stpfraz_stoBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_stpfraz_stoBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(154);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_stpfraz_stoBO BO,ServletStatus status) {
    status.out.println(
  ...
  static void PrintState(arpg_stpfraz_stoBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_stpfraz_stoBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_stpfraz_stoBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_stpfraz_stoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_stpfraz_sto"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_stpfraz_sto","");
  ...
  void WriteMainFormScript(arpg_stpfraz_stoBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  arpg_stpfraz_stoBO getBO(ServletStatus status) {
    arpg_stpfraz_stoBO BO = new arpg_stpfraz_stoBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_stpfraz_stoBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_stpfraz_stoBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_stpfraz_stoBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_stpfraz_stoBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_stpfraz_stoBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_stpfraz_stoBO BO) {
    ...
    item.put("altInterfaceFor","arpg_stpfraz_sto");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"19\",\"name\":\"tipstp\",\"orientation\":\"horizontal\",\"page\":\"1\",\"spuid\":\"LGLKGKWNEO\",\"tabindex\":\"6\",\"textlist\":\"Stampa Sintetica,Stampa Analitica\",\"type\":\"Radio\",\"valuelist\":\""+"'"+"S"+"'"+","+"'"+"A"+"'"+"\",\"w\":\"304\",\"x\":\"16\",\"y\":\"126\",\"zone\":\"XLVFHSOSYH_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"href\":\"javascript:href_XURLGQZZMU()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"XURLGQZZMU\",\"src\":\"images\\/print.gif\",\"tabindex\":\"7\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"521\",\"y\":\"117\",\"zone\":\"XLVFHSOSYH_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Data_Operazione_\",\"page\":\"1\",\"spuid\":\"HPGJXQIVYG_DIV\",\"tabindex\":\"8\",\"type\":\"Label\",\"value\":\"Da Data Operazione:\",\"w\":\"124\",\"x\":\"16\",\"y\":\"24\",\"zone\":\"XLVFHSOSYH_1\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"left\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Da_Cliente_\",\"page\":\"1\",\"spuid\":\"PCILMATBXL_DIV\",\"tabindex\":\"10\",\"type\":\"Label\",\"value\":\"Da Cliente:\",\"w\":\"61\",\"x\":\"79\",\"y\":\"53\",\"zone\":\"XLVFHSOSYH_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"A_Cliente_\",\"page\":\"1\",\"spuid\":\"JGQCNSZOCQ_DIV\",\"tabindex\":\"13\",\"type\":\"Label\",\"value\":\"A Cliente:\",\"w\":\"61\",\"x\":\"79\",\"y\":\"81\",\"zone\":\"XLVFHSOSYH_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"32\",\"href\":\"javascript:href_PTXYMSWBZO()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"PTXYMSWBZO\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"16\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"569\",\"y\":\"117\",\"zone\":\"XLVFHSOSYH_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
  ...
  void Next(ServletStatus status,arpg_stpfraz_stoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_stpfraz_stoBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_stpfraz_stoBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_stpfraz_sto?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_stpfraz_sto_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_stpfraz_stoBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_stpfraz_stoBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_stpfraz_stoBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_stpfraz_stoBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_stpfraz_stoBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_stpfraz_stoBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_stpfraz_sto.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,arpg_stpfraz_stoBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_stpfraz_stoBO BO) {
    boolean result = true;
    return result;
  }
}
