...
// * --- Fine Area Manuale
public class armt_ae_famiglieBO extends armt_ae_famiglieBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_ae_famiglieBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_ae_famiglie");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        w_IDESITOFAM = l_rsLoad.GetString("IDESITOFAM");
        w_UNIQUECODE = l_rsLoad.GetString("UNIQUECODE");
        w_INVIATOC = l_rsLoad.GetDouble("INVIATOC");
        w_DATAINVIOC = l_rsLoad.GetDate("DATAINVIOC");
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"aetesta")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_aetesta);
      ...
      l_cSql = l_cSql+"IDESITOFAM,";
      l_cSql = l_cSql+"UNIQUECODE,";
      l_cSql = l_cSql+"INVIATOC,";
      l_cSql = l_cSql+"DATAINVIOC,";
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_DESCRI,"C",24,0)+", ";
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_IDESITOFAM,"C",24,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_UNIQUECODE,"C",50,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_INVIATOC,"N",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_DATAINVIOC,"D",8,0)+", ";
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      l_cSql = l_cSql+", DESCRI = "+CPLib.ToSQL(w_DESCRI,"C",24,0);
      ...
      l_cSql = l_cSql+", IDESITOFAM = "+CPLib.ToSQL(w_IDESITOFAM,"C",24,0);
      l_cSql = l_cSql+", UNIQUECODE = "+CPLib.ToSQL(w_UNIQUECODE,"C",50,0);
      l_cSql = l_cSql+", INVIATOC = "+CPLib.ToSQL(w_INVIATOC,"N",1,0);
      l_cSql = l_cSql+", DATAINVIOC = "+CPLib.ToSQL(w_DATAINVIOC,"D",8,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"aetesta")) {
