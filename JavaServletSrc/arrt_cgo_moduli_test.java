...
public class arrt_cgo_moduli_test extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*964e5e51*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (m_bpassedw_CAIDBIGLIETTO) {
        ...
        if (source.IsForwarded()) {
          w_CAIDBIGLIETTO = source.GetCaller().GetString("CAIDBIGLIETTO","C",28,0);
      ...
      if (m_bpassedw_CODTRANSZ) {
        ...
        if (source.IsForwarded()) {
          w_CODTRANSZ = source.GetCaller().GetString("CODTRANSZ","C",50,0);
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_cgo_moduli_test";
  ...
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_CAIDBIGLIETTO)) {
      status.out.println("l_cV=WtA(l_oWnd('w_CAIDBIGLIETTO',"+SPLib.ToJSValue(status.w_CAIDBIGLIETTO,"C",28,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_CAIDBIGLIETTO,"C",28,0)+",'C');");
    }
    ...
    if ( ! (status.m_bpassedw_CODTRANSZ)) {
      status.out.println("l_cV=WtA(l_oWnd('w_CODTRANSZ',"+SPLib.ToJSValue(status.w_CODTRANSZ,"C",50,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_CODTRANSZ,"C",50,0)+",'C');");
    }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_cgo_moduli_testR Rcalled;
    Rcalled = new arrt_cgo_moduli_testR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_cgo_moduli_testR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_cgo_moduli_testR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_cgo_moduli_test"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_cgo_moduli_test");
      if ( ! (CPLib.IsNull(l))) {
