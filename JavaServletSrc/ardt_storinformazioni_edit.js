...
l_bEnabled= ! (Lt(Utilities.Make(window).GetCallerNumber('tipregn'),6)) || IsDisabledByStateDriver('DATAINI');
SetDisabled('HKZAUFLHMG',l_bEnabled);
l_bEnabled= ! (Ge(Utilities.Make(window).GetCallerNumber('tipregn'),6)) || IsDisabledByStateDriver('DATAFINE');
...
w_NUMPROG1=(Lt(Utilities.Make(window).GetCallerNumber('tipregn'),6)?Utilities.Make(window).GetCallerString('NUMPROG'):w_NUMPROG1);
w_NUMPROG2=(Ge(Utilities.Make(window).GetCallerNumber('tipregn'),6)?Utilities.Make(window).GetCallerString('NUMPROG'):w_NUMPROG2);
...
if ( ! (m_oFather.SaveContext_ardt_storinformazioni())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_storinformazioni_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_storinformazioni_dataNotSent',{'action':formaction});
}
}
}
}
...
l_bResult=false;
w_CODINTER=HtW('','C');
} else if (((Lt(Utilities.Make(window).GetCallerNumber('tipregn'),6)) && ( ! (_ChkObl('HKZAUFLHMG',1)))) && FullRow()) {
_SignErr('HKZAUFLHMG');
m_cLastWorkVarErrorMsg='45547614';
l_bResult=false;
w_DATAINI=HtW('','D');
} else if (((Ge(Utilities.Make(window).GetCallerNumber('tipregn'),6)) && ( ! (_ChkObl('NATIUIQLJX',1)))) && FullRow()) {
_SignErr('NATIUIQLJX');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_storinformazioni();
}
}
SaveContext.child=[];
function BlankRow() {
...
l_aRepeatedFields.NUMPROG1={"dec":0,"fixedpos":false,"id":"IOJVSZRBNX","len":11,"name":"NUMPROG1","title":"Numero Progressivo relativo","type":"C"};
l_aRepeatedFields.NUMPROG2={"dec":0,"fixedpos":false,"id":"VNGVCVDHTS","len":11,"name":"NUMPROG2","title":"Numero Progressivo Relativo Chiusura","type":"C"};
