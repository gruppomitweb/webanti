...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('YPYQJILEQY').value,'C'),w_CODINTER)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('intermbo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODINTER') &&  ! (Empty(HtW(Ctrl('YPYQJILEQY').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODINTER',HtW(Ctrl('YPYQJILEQY').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_destwarn(firstAlias) {
return m_cVirtName+".CODINTER = "+firstAlias+".DWCODINT ";
}
function DoQueryFrom_cgo_tipall(firstAlias) {
return m_cVirtName+".CODINTER = "+firstAlias+".TACODINT ";
}
function DoQueryFrom_cgo_subnet(firstAlias) {
return m_cVirtName+".CODINTER = "+firstAlias+".SNCODINT ";
}
function DoQueryFrom_destwarn(firstAlias) {
return m_cVirtName+".CODINTER = "+firstAlias+".DWCODINT ";
}
...
_chNotF(childAlias,'master','armt_intermbo');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('intermbo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['YPYQJILEQY'],i=0;i>=0;i--){
