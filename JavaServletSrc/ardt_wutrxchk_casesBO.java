...
// * --- Fine Area Manuale
public class ardt_wutrxchk_casesBO extends ardt_wutrxchk_casesBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+4,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_2_1.WCCODICE AS WCCODICE201";
        m_cSelect = m_cSelect+" ,link_2_1.WCNOMCAS AS WCNOMCAS201";
        m_cSelect = m_cSelect+" ,link_2_1.WCDESCRI AS WCDESCRI201";
        m_cSelect = m_cSelect+" ,link_2_1.WCBEHAVIOR AS WCBEHAVIOR201";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","TRXCODCASE","WCCODICE");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","TRXCODCASE","WCCODICE",true);
  ...
  // * --- Fine Area Manuale
  public ardt_wutrxchk_casesBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_wutrxchk_cases");
    }
  ...
  public static String[] m_cLoadParameterNames = {"TRXCODICE"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(double p_TRXCODICE) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_wutrxchk_cases,m_cPhName_wutrxchk_cases,m_cVirtName_wutrxchk_cases,m_cVirtName_wutrxchk_cases+".TRXCODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_TRXCODICE,"N",15,0),m_cServer_wutrxchk_cases,p_TRXCODICE),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_1(m_cPhName_wu_cases,m_cServer_wu_cases);
    l_rsLoadBody = l_oSqlDetailLoader.Query();
    try {
      m_bLoaded =  ! (l_rsLoadBody.Eof());
      if (m_bLoaded) {
        m_bCalculating = true;
        w_TRXCODICE = l_rsLoadBody.GetDouble("TRXCODICE");
        // Assegna i valori degli op_
        // Assegna il contesto dei figli di testata/piedi a Empty
        SPExtenderLib.LoadExtendedFields(l_rsLoadBody,extendedFields);
        if (CPLib.HasTableCPCCCHK(m_Ctx,"wutrxchk_cases")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_TRXCODCASE = l_rsLoadBody.GetDouble("TRXCODCASE");
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            w_WCNOMCAS = l_rsLoadBody.GetString("WCNOMCAS201");
            w_WCDESCRI = l_rsLoadBody.GetString("WCDESCRI201");
            w_WCBEHAVIOR = l_rsLoadBody.GetString("WCBEHAVIOR201");
          } else {
            Link_KFGZLKSNHG("Load");
          }
          // Assegna il contesto dei figli del dettaglio a Empty
          work_children = CPLib.BlankChildMNTs(m_MNTs,true);
          opk_TRXCODCASE = w_TRXCODCASE;
          if (CPLib.HasTableCPCCCHK(m_Ctx,"wutrxchk_cases")) {
  ...
  public boolean Delete(double p_TRXCODICE) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_wutrxchk_cases.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"TRXCODICE="+CPLib.ToSQL(w_TRXCODICE,"N",15,0)+" AND TRXCODCASE="+CPLib.ToSQL(opk_TRXCODCASE,"N",3,0);
        l_cSql = l_cSql+m_oWrInfo_wutrxchk_cases.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"wutrxchk_cases")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_wutrxchk_cases,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_TRXCODICE);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"KFGZLKSNHG")) {
      result.m_TableName = "wu_cases";
      result.n_KMode = 2;
      ...
      result.m_PrgZoom = "";
      result.m_rdField = new String[]{"WCCODICE" , "WCNOMCAS" , "WCDESCRI" , "WCBEHAVIOR"};
      result.m_rdTypes = new String[]{"N" , "C" , "M" , "M"};
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("KFGZLKSNHG",CPLib.GetLinkedFieldProps("WCCODICE","TRXCODCASE","N",3,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_wutrxchk_cases);
      l_cSql = "insert into "+m_oWrInfo_wutrxchk_cases.GetTableWriteName()+" (";
      l_cSql = l_cSql+"TRXCODICE,";
      l_cSql = l_cSql+"TRXCODCASE,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_wutrxchk_cases.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wutrxchk_cases")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_TRXCODICE,"N",15,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_TRXCODCASE,"N",3,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_wutrxchk_cases.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wutrxchk_cases")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_wutrxchk_cases,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_wutrxchk_cases);
      l_cSql = "Update "+m_oWrInfo_wutrxchk_cases.GetTableWriteName()+" set";
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wutrxchk_cases")) {
        l_cSql = l_cSql+" CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", TRXCODCASE = "+CPLib.ToSQL(w_TRXCODCASE,"N",3,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"TRXCODICE="+CPLib.ToSQL(w_TRXCODICE,"N",15,0)+" AND TRXCODCASE="+CPLib.ToSQL(opk_TRXCODCASE,"N",3,0);
      l_cSql = l_cSql+m_oWrInfo_wutrxchk_cases.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wutrxchk_cases")) {
