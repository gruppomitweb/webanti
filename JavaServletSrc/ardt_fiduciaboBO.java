...
// * --- Fine Area Manuale
public class ardt_fiduciaboBO extends ardt_fiduciaboBL {
  ...
  // * --- Fine Area Manuale
  public ardt_fiduciaboBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_fiduciabo");
    }
  ...
  public boolean Load(String p_RAPCOL) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_fiduciabo,m_cPhName_fiduciabo,m_cVirtName_fiduciabo,m_cVirtName_fiduciabo+".RAPCOL="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPCOL,"C",25,0),m_cServer_fiduciabo,p_RAPCOL),"");
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            ...
            Link_MPDPVTSMAO("Load");
          }
          w_TIPOFILE = l_rsLoadBody.GetString("TIPOFILE");
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_fiduciabo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPCOL="+CPLib.ToSQL(w_RAPCOL,"C",25,0)+" AND CODSOG="+CPLib.ToSQL(opk_CODSOG,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_fiduciabo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"fiduciabo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_fiduciabo,l_cSql);
  ...
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"MPDPVTSMAO")) {
      ...
      result.AppendFieldsHash("MPDPVTSMAO",CPLib.GetLinkedFieldProps("CONNES","CODSOG","C",16,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_fiduciabo);
      l_cSql = "insert into "+m_oWrInfo_fiduciabo.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDFILEBO,";
      l_cSql = l_cSql+"RAPCOL,";
      l_cSql = l_cSql+"NUMCOL,";
      l_cSql = l_cSql+"CODSOG,";
      l_cSql = l_cSql+"TIPOFILE,";
      l_cSql = l_cSql+"LEGAME,";
      l_cSql = l_cSql+"UCODE,";
      l_cSql = l_cSql+"IDBASE,";
      l_cSql = l_cSql+"NUMIMP,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_fiduciabo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"fiduciabo")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_NUMCOL,"C",11,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_CODSOG,"C",16,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_TIPOFILE,"C",1,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_LEGAME,"C",1,0)+", ";
      ...
      l_cSql = l_cSql+m_oWrInfo_fiduciabo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"fiduciabo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_fiduciabo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_fiduciabo);
      l_cSql = "Update "+m_oWrInfo_fiduciabo.GetTableWriteName()+" set";
      l_cSql = l_cSql+" IDFILEBO = "+CPLib.ToSQL(w_IDFILEBO,"C",10,0);
      l_cSql = l_cSql+", NUMCOL = "+CPLib.ToSQL(w_NUMCOL,"C",11,0);
      l_cSql = l_cSql+", TIPOFILE = "+CPLib.ToSQL(w_TIPOFILE,"C",1,0);
      l_cSql = l_cSql+", LEGAME = "+CPLib.ToSQL(w_LEGAME,"C",1,0);
