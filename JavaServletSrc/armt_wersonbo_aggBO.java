...
// * --- Fine Area Manuale
public class armt_wersonbo_aggBO extends armt_wersonbo_aggBL {
  class SqlLoader {
  ...
  // * --- Fine Area Manuale
  public armt_wersonbo_aggBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_wersonbo_agg");
    }
  ...
  public boolean Load(String p_sanumpro) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_wersonbo_agg,m_cPhName_wersonbo_agg,m_cVirtName_wersonbo_agg,m_cVirtName_wersonbo_agg+".sanumpro="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_sanumpro,"C",15,0),m_cServer_wersonbo_agg,p_sanumpro),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"wersonbo_agg")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_wersonbo_agg);
      l_cSql = "insert into "+m_oWrInfo_wersonbo_agg.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_wersonbo_agg.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wersonbo_agg")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_wersonbo_agg.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wersonbo_agg")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_wersonbo_agg,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_wersonbo_agg);
      l_cSql = "Update "+m_oWrInfo_wersonbo_agg.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"sanumpro="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_sanumpro,"C",15,0),m_cServer_wersonbo_agg,w_sanumpro);
      l_cSql = l_cSql+m_oWrInfo_wersonbo_agg.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"wersonbo_agg")) {
  ...
  public boolean Delete(String p_sanumpro) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_wersonbo_agg.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"sanumpro="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_sanumpro,"C",15,0),m_cServer_wersonbo_agg,w_sanumpro);
          l_cSql = l_cSql+m_oWrInfo_wersonbo_agg.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"wersonbo_agg")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_wersonbo_agg,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
