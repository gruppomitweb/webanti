...
public class arpg_wersonbo extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ZQJBUAHRUB*!";
  ...
  public String getEntityTitle() {
    return "Variazioni Dati Anagrafici";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_wersonbo";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 698;
    ...
    l_eg.m_cQueryName = "qbe_wersonbo";
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10220423475",p_Context.Translate("Data Variazione")},
    {"10662833243",p_Context.Translate("Ragione Sociale")},
    {"10835301768",p_Context.Translate("Variazioni Dati Anagrafici")},
    {"113933141",p_Context.Translate("Update")},
    {"11921223220",p_Context.Translate("Codice Fiscale")}};
    return l_translations;
  }
  void WriteStyles(arpg_wersonboBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_wersonbo.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_wersonboBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_wersonboBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_wersonboBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_wersonboBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_wersonboBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_wersonboBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_wersonboBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_wersonboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(698,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_wersonbo"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_wersonbo","");
  ...
  void WriteMainFormScript(arpg_wersonboBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_wersonboBO getBO(ServletStatus status) {
    arpg_wersonboBO BO = new arpg_wersonboBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_wersonboBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_wersonboBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_wersonboBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_wersonboBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_wersonboBO BO) {
    status.m_bExit = true;
  }
  void WriteBody(ServletStatus status,arpg_wersonboBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_wersonboBO BO) {
    status.out.println("var c_sanumpro;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_wersonboBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_sanumpro,"C",15,0);
  ...
  void Next(ServletStatus status,arpg_wersonboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_wersonboBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_wersonboBO BO,ServletStatus status) {
    ...
    String l_cJsUid;
    status.out.println(
     "<script src='arpg_wersonbo?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_wersonbo_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_wersonboBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_wersonboBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_wersonboBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_wersonboBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_wersonboBO BO,ServletStatus status) {
