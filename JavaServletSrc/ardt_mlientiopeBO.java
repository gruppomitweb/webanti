...
// * --- Fine Area Manuale
public class ardt_mlientiopeBO extends ardt_mlientiopeBL {
  ...
  // * --- Fine Area Manuale
  public ardt_mlientiopeBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_mlientiope");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_mlientiope,m_cPhName_mlientiope,m_cVirtName_mlientiope,m_cVirtName_mlientiope+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_mlientiope,p_IDFILEBO),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"mlientiope")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_mlientiope.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND CODSOG="+CPLib.ToSQL(opk_CODSOG,"C",16,0);
        l_cSql = l_cSql+m_oWrInfo_mlientiope.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"mlientiope")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_mlientiope,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_mlientiope);
      l_cSql = "insert into "+m_oWrInfo_mlientiope.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_mlientiope.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"mlientiope")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_mlientiope.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"mlientiope")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_mlientiope,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_mlientiope);
      l_cSql = "Update "+m_oWrInfo_mlientiope.GetTableWriteName()+" set";
