...
function Set_IBUCODUSR(ctrlValue,kmode,e) {
if (Ne(w_IBUCODUSR,ctrlValue)) {
var ctl = _GetCtl(e,'BRCAXXLEPY');
if (_tracker.goon(ctl,ctrlValue)) {
w_IBUCODUSR=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('BRCAXXLEPY',0);
if (l_bResult) {
l_bResult=Link_BRCAXXLEPY(kmode);
if ( ! (l_bResult)) {
w_IBUCODUSR=HtW('','N');
}
}
DoUpdate(l_bResult);
if ( ! (l_bResult)) {
_RetFoc(e,ctrlValue,WtH(ctrlValue,'N',10,0,'9999999999'));
} else {
_ResetTracker();
}
return l_bResult;
...
function Link_BRCAXXLEPY(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
var l_bEmp = Empty(w_IBUCODUSR);
var l_Appl = LinkApplet();
l_Appl.SetDoubleKey('code',w_IBUCODUSR,10,0);
l_Appl.SetFields('code,name');
l_Appl.SetTypes('N,C');
l_Appl.LinkTable('cpusers');
l_Appl.SetID('BRCAXXLEPY');
l_nRec=l_Appl.DoLink((Eq(kmode,null)?1:kmode));
l_bResult=Ne(l_nRec,0);
if (Eq(l_nRec,1)) {
w_IBUCODUSR=l_Appl.GetDoubleValue('code',10,0);
w_nome=l_Appl.GetStringValue('name',50,0);
l_Appl.GetLinkHash(GetRepeatedCheckVars());
} else {
Link_BRCAXXLEPY_Blank();
}
if (Eq(l_nRec,2)) {
var openername = 'linkview_BRCAXXLEPY';
...
function Link_BRCAXXLEPY_Blank() {
w_IBUCODUSR=0;
w_nome='';
}
function LOpt_BRCAXXLEPY() {
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('cpusers','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
l_oWv.setValue('PKFields',WtA('code','C'));
l_oWv.setValue('LinkedField',WtA('code','C'));
l_oWv.setValue('UID',WtA('BRCAXXLEPY','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['BRCAXXLEPY'][1],'C'));
return l_oWv;
}
function ReportLinkValue(varName,value) {
var bResult = false;
if (Eq(varName,'BRCAXXLEPY')) {
last_focused_comp=GetLastFocusedElementOfLink('BRCAXXLEPY');
function SetMethod_BRCAXXLEPY() {
Set_IBUCODUSR(HtW(value,'N'),0);
}
if (IsDeviceMobile()) {
SetMethod_BRCAXXLEPY();
} else {
setTimeout(SetMethod_BRCAXXLEPY,1);
}
bResult=true;
}
return bResult;
...
if ( ! (m_oFather.SaveContext_ardt_cgo_infbacheca_usr())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_infbacheca_usr_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_cgo_infbacheca_usr_dataNotSent',{'action':formaction});
}
}
}
}
...
l_oWv.setValue('IBUCODICE',WtA(w_IBUCODICE,'N'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
if (( ! (_ChkObl('BRCAXXLEPY',1))) && FullRow()) {
_SignErr('BRCAXXLEPY');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_cgo_infbacheca_usr();
}
}
SaveContext.child=[];
function BlankRow() {
w_IBUCODUSR=0;
w_nome='';
if ( ! (EmptyNumber(w_IBUCODUSR))) {
Link_BRCAXXLEPY(null);
}
BlankRowChild();
if (m_bExtended) {
...
function FullRow() {
return  ! (Empty(w_IBUCODUSR));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.IBUCODUSR={"dec":0,"fixedpos":false,"id":"BRCAXXLEPY","len":10,"name":"IBUCODUSR","title":"Codice Utente","type":"N"};
l_aRepeatedFields.nome={"dec":0,"fixedpos":false,"id":"QIQDQNNLFN","len":50,"name":"nome","title":"","type":"C"};
