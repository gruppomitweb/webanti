...
function RROSTBCQKE_Valid(e) {
SetActiveField(Ctrl('RROSTBCQKE'),false);
e=(Ne(e,null)?e:window.event);
return Set_idesito(HtW(Ctrl('RROSTBCQKE').value,'C'),null,e);
}
function RROSTBCQKE_OnFocus(e) {
if (Ne(m_cFunction,'query') &&  ! (CheckZone('RROSTBCQKE'))) {
DisplayErrorMessage();
}
SetActiveField(Ctrl('RROSTBCQKE'),true);
SetLastFocusedOnParent();
SPSelectCtrl(Ctrl('RROSTBCQKE'),e);
}
...
if(c=Ctrl('RROSTBCQKE')) c.onblur=RROSTBCQKE_Valid;
if(c=Ctrl('RROSTBCQKE')) c.onfocus=RROSTBCQKE_OnFocus;
...
function Help() {
windowOpenForeground('../doc/arpg_modopen1_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_modopen1',m_cSelectedPage);
...
var ctl = _GetCtl(e,'RROSTBCQKE');
if (_tracker.goon(ctl,ctrlValue)) {
w_idesito=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('RROSTBCQKE',0);
DoUpdate(l_bResult);
} else {
ctl.value=WtH('','C',8,0,'');
_ResetTracker();
...
_FillChildren.n=[];
FillWv.n=["ragione","datarett","olddata","oldintest","xcfestero","xcodfisc","xcognome","xnome","xsesso","newintest","xragnew","xtipopers","oldprog","oldidbase","oldrappo","oldnumrig","oldfile","tipochg","xnascomun","xtipinter","xnasstato","xdatnasc","xragold","xdesccit","xprovincia","xcap","xpaese","xdessta","olddatac","idesito","newdatec","newdate","oldappo"];
...
_Obli('idesito',0,'RROSTBCQKE',false,"10615814578")
