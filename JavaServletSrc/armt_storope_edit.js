...
function DisplayErrorMessage_ardt_storsogope() {
if ( ! (ChildToLoad(FrameRef('ardt_storsogope'))) && Ne(typeof(FrameRef('ardt_storsogope').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_storsogope').DisplayErrorMessage();
}
}
function DisplayErrorMessage_ardt_wlientiope() {
if ( ! (ChildToLoad(FrameRef('ardt_wlientiope'))) && Ne(typeof(FrameRef('ardt_wlientiope').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_wlientiope').DisplayErrorMessage();
}
}
function DisplayErrorMessage_ardt_weneficope() {
if ( ! (ChildToLoad(FrameRef('ardt_weneficope'))) && Ne(typeof(FrameRef('ardt_weneficope').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_weneficope').DisplayErrorMessage();
}
}
function DisplayErrorMessage_ardt_werzistiope() {
if ( ! (ChildToLoad(FrameRef('ardt_werzistiope'))) && Ne(typeof(FrameRef('ardt_werzistiope').DisplayErrorMessage),'undefined')) {
FrameRef('ardt_werzistiope').DisplayErrorMessage();
}
}
...
if (IsDisabledByStateDriver('SogTerzi')) {
DisableInputsInContainer(Ctrl('BHLGNNSXQY'),true);
}
if (IsDisabledByStateDriver('box_OGJVIBRWWZ')) {
DisableInputsInContainer(Ctrl('OGJVIBRWWZ'),true);
}
if (IsDisabledByStateDriver('box_KEELAAIUXV')) {
DisableInputsInContainer(Ctrl('KEELAAIUXV'),true);
}
if (IsDisabledByStateDriver('AGE')) {
DisableInputsInContainer(Ctrl('EOKCAXCLZO'),true);
}
if (IsDisabledByStateDriver('box_QVDUCHDNOK')) {
DisableInputsInContainer(Ctrl('QVDUCHDNOK'),true);
}
if (IsDisabledByStateDriver('box_MAHDKXNSXP')) {
DisableInputsInContainer(Ctrl('MAHDKXNSXP'),true);
}
if (IsDisabledByStateDriver('Internet')) {
DisableInputsInContainer(Ctrl('JTWNGHDSFX'),true);
}
if (IsDisabledByStateDriver('box_JELGAQOYWT')) {
DisableInputsInContainer(Ctrl('JELGAQOYWT'),true);
}
if (IsDisabledByStateDriver('box_DGEGXTAAOU')) {
DisableInputsInContainer(Ctrl('DGEGXTAAOU'),true);
}
if (IsDisabledByStateDriver('box_EKUMWOLTPZ')) {
DisableInputsInContainer(Ctrl('EKUMWOLTPZ'),true);
}
...
SetObligatory(Ctrl('ACAPFGYRVI'), ! (Empty(w_CODINT2)) || IsObligatoryByStateDriver('PROV2'));
}
...
if(c=Ctrl('PURPSXEVAI')) c.title=Strtran(AlertErrorMessage('2070114244',w_FLAGCONT),'\\n',"\n");
...
l_bResult=l_bResult && _ChkObl('LKEDJRUQJU',0);
if (l_bResult) {
if ( ! (l_bResult)) {
w_CODVOC=HtW('','C');
}
...
parent.ZtVWeb.raiseEventToEvalParms('armt_storope_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_storope_dataNotSent',{'action':formaction});
}
}
}
}
CheckChild.child=[{'prg':"ardt_storsogope",'BOScId':function(){ return FrameRef('ardt_storsogope')},'repeated':0},{'prg':"ardt_wlientiope",'BOScId':function(){ return FrameRef('ardt_wlientiope')},'repeated':0},{'prg':"ardt_weneficope",'BOScId':function(){ return FrameRef('ardt_weneficope')},'repeated':0},{'prg':"ardt_werzistiope",'BOScId':function(){ return FrameRef('ardt_werzistiope')},'repeated':0},{'prg':"ardt_docopebo",'BOScId':function(){ return FrameRef('ardt_docopebo')},'repeated':0}];
function Check() {
var l_bResult = true;
var l_bChildCheck = true;
ResetErrorVars();
m_nLastError=0;
if (( ! (_ChkObl('YLIKFVPZJA',1))) || ( ! (Ge(w_DATAREG,w_DATAOPE)))) {
...
l_bResult=false;
w_CONNESBEN=HtW('','C');
} else if (( ! (_ChkObl('HWFHDWYMIU',1))) || ( ! (Empty(w_CONNALTRO)) &&  ! (arfn_chkdatdoc(w_CONNALTRO,w_DATAOPE)))) {
_SignErr('HWFHDWYMIU','11525797640');
l_bResult=false;
w_CONNALTRO=HtW('','C');
...
_FillChildren.n=['ardt_storsogope','ardt_wlientiope','ardt_weneficope','ardt_werzistiope','ardt_docopebo'];
FillWv.n=["DATAOPE","CDATOPE","DATAREG","FLAGFRAZ","COLLEG","TIPOOPRAP","CODANA","CODVOC","FLAGLIRE","VALUTA","FLAGCONT","CAMBIO","RAPPORTO","CONNESCLI","CONNESOPER","TIPOLEG","PAGASOGOP","CONNESBEN","CONNALTRO","C_RAG","C_CTA","C_PRV","C_CAP","C_CAB","C_IND","C_STA","TIPOINT2","CODINT2","DESCINTER","PAESE","C_RAPPORTO","CODCAB2","PROV2","DESC2","ZCPARTE","IDBASE","NUMPROG","CODINTER","CODDIPE","DESCCIT","SEGNO","DATARETT","STATOREG","PROVINCIA","CODCAB","OPERAG","NOPROT","RESPINS","INFORM","NATURA","SCOPO","AREAGEO","MODSVOL","RAGIO","TIPO","RAREA","RCOMP","RRAGIO","RTIPO","RFREQ","RIMP","DURAT","AMMONT","CONNESDOC","IDFILE","MTCN","TOTLIRE","TOTCONT","TIPOINF","IDEREG","FLAGRAP","OPRAP","OPERAZMOD","RAPPORTBEN","OPERATORE","AUTOM","FLAGRAP2","PRGIMPOPE","CDATAOPE","CDATREG","IPCOLLEG","ORACOLLEG","DUCOLLEG","ANNOOPE","MITIG","MITGDOC","filtro","appolire","xTOTLIRE","xTOTCONT","xdescmodsvo","xdescareag","xdescplauso","xdesctipor","xdescopeage","xdescfreq","xdescammo","xRAGSOCDOC","tipope","gIntemediario","xdes1","desccauana","xdesccausin","descdiv","xragsocper","xdescrap","xragsocct","xragsocben","xdestipleg","xragint2","xdescpaese2","datpaseuro","xDescDipe","sEdit","gChkDate","errmsg","test","valflg2","gTipInter","gSblocco","xcodfisc","dataoggi","stringaflagrap2","gDataVaria","gStatus","CITINT","CITDIP","PROVDIP","CABDIP","PROVINT","CABINT","gFlgDoc","dessta","xragsocalt","xdciticon","gFlgWU","DATAVALI","DATARILASC","apporeg","gFlgDTP"];
function SetModified() {
m_bUpdated=true;
}
function SetChildVariables_ardt_storsogope() {
}
function CtxLoad_ardt_storsogope() {
if (ChildToLoad(FrameRef('ardt_storsogope'))) {
return [LoadContext(0,'TQDGESLQVD'),'ardt_storsogope'];
}
}
function SetChildVariables_ardt_wlientiope() {
}
function CtxLoad_ardt_wlientiope() {
if (ChildToLoad(FrameRef('ardt_wlientiope'))) {
return [LoadContext(0,'BOAFVDQFRN'),'ardt_wlientiope'];
}
}
function SetChildVariables_ardt_weneficope() {
}
function CtxLoad_ardt_weneficope() {
if (ChildToLoad(FrameRef('ardt_weneficope'))) {
return [LoadContext(0,'OCTDIOXFMI'),'ardt_weneficope'];
}
}
function SetChildVariables_ardt_werzistiope() {
}
function CtxLoad_ardt_werzistiope() {
if (ChildToLoad(FrameRef('ardt_werzistiope'))) {
return [LoadContext(0,'NOKYMTFBFL'),'ardt_werzistiope'];
}
}
