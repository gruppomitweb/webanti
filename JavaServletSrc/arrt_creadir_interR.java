...
// * --- Fine Area Manuale
public class arrt_creadir_interR implements CallerWithObjs {
  ...
  public String _pathorg;
  // * --- Area Manuale = BO - Properties
  // * --- Fine Area Manuale
  public arrt_creadir_interR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_creadir_inter",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    ...
    if (CPLib.eqr("_pathorg",p_cVarName)) {
      return _pathorg;
    }
    return "";
  }
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      while ( ! (Cursor_intermbo.Eof())) {
        ...
        /* If not(Empty(intermbo->PATHCERT)) and not(Empty(intermbo->PATHSID)) */
        if ( ! (CPLib.Empty(Cursor_intermbo.GetString("PATHCERT"))) &&  ! (CPLib.Empty(Cursor_intermbo.GetString("PATHSID")))) {
          /* _pathorg := gPathApp+"/manuale/"+LRTrim(intermbo->PATHSID) */
          _pathorg = gPathApp+"/manuale/"+CPLib.LRTrim(Cursor_intermbo.GetString("PATHSID"));
          /* FileLibMit.FileCopy(_pathorg,LRTrim(intermbo->PATHCERT)+"/manuale/"+LRTrim(intermbo->PATHSID)) */
          FileLibMit.FileCopy(_pathorg,CPLib.LRTrim(Cursor_intermbo.GetString("PATHCERT"))+"/manuale/"+CPLib.LRTrim(Cursor_intermbo.GetString("PATHSID")));
        } // End If
        /* gPathDoc := LRTrim(intermbo->RICEVUTE) // Path Documenti */
        gPathDoc = CPLib.LRTrim(Cursor_intermbo.GetString("RICEVUTE"));
        m_Ctx.SetGlobalString("gPathDoc",gPathDoc);
        /* gVerSim := intermbo->VERSIMPLE // Verifica Semplificata */
        gVerSim = Cursor_intermbo.GetString("VERSIMPLE");
        m_Ctx.SetGlobalString("gVerSim",gVerSim);
  ...
  public static arrt_creadir_interR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_creadir_interR(p_Ctx, p_Caller);
  }
  public void Blank() {
    chkfile = false;
    cAnno = CPLib.Space(4);
