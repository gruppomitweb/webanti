...
public class arrt_stp_stdtec extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*2c476ca1*!";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_flgrae) ||  ! (status.m_bpassedw_aCodDip) ||  ! (status.m_bpassedw_daCodDip) ||  ! (status.m_bpassedw_aCodCli) ||  ! (status.m_bpassedw_daCodCli) ||  ! (status.m_bpassedw_ADatOpe) ||  ! (status.m_bpassedw_DaDatOpe);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_stp_stdtecR Rcalled;
    Rcalled = new arrt_stp_stdtecR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.forwardedto = Rcalled.Run(status.pTipo,status.pAUI);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_stp_stdtecR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_stp_stdtecR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      status.out.println(
       "<!--" +
      "");
      status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(Rcalled.RunAsync(status.pTipo,status.pAUI)))+"");
      status.out.println(
       " -->" +
      "");
    } else {
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_stp_stdtec"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_stp_stdtec");
      if ( ! (CPLib.IsNull(l))) {
