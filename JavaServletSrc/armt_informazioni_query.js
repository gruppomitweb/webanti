...
WindowAlert(AlertErrorMessage('1639410703'));
}
...
m_oFather.DoQueryFromChild('rapopebo',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
function DoQueryFrom_intestbo(firstAlias) {
return m_cVirtName+".RAPPORTO = "+firstAlias+".RAPPORTO ";
}
function DoQueryFrom_delegabo(firstAlias) {
return m_cVirtName+".RAPPORTO = "+firstAlias+".RAPPORTO ";
}
function DoQueryFrom_soginfo(firstAlias) {
return m_cVirtName+".RAPPORTO = "+firstAlias+".RAPCOL ";
}
function DoQueryFrom_fiduciabo(firstAlias) {
return m_cVirtName+".RAPPORTO = "+firstAlias+".RAPCOL ";
}
function DoQueryFrom_terzistirap(firstAlias) {
...
_chNotF(childAlias,'master','armt_informazioni');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('rapopebo',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
