...
var ctl = _GetCtl(e,'UJDYOPDWZF');
if (_tracker.goon(ctl,ctrlValue)) {
w_RAPCOL=ctrlValue;
SetModified(false);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('UJDYOPDWZF',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
if ( ! (m_oFather.SaveContext_ardt_soginfo())) {
...
parent.ZtVWeb.raiseEventToEvalParms('ardt_soginfo_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('ardt_soginfo_dataNotSent',{'action':formaction});
}
}
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_ardt_soginfo();
}
}
SaveContext.child=[];
function BlankRow() {
...
if (enable) {
var c;
if(c=Ctrl('UJDYOPDWZF')) SetDisabled(c,true);
SetQueryRow(Ctrl('GridTable'),false);
} else {
SetEditRow(Ctrl('GridTable'),false);
var c;
if(c=Ctrl('UJDYOPDWZF')) SetDisabled(c,false);
