...
// * --- Fine Area Manuale
public class ardt_sogopeprovBO extends ardt_sogopeprovBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_3(String rightTable,String rightServer) {
      ...
      String m_cJoinCond = "";
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        m_cSelect = m_cSelect+" ,link_2_3.TIPLEG AS TIPLEG203";
        m_cSelect = m_cSelect+" ,link_2_3.DESCRIZ AS DESCRIZ203";
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_3","LEGAME","TIPLEG");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_3","LEGAME","TIPLEG",true);
          if (CPLib.eq(m_cLinkedWhere,"")) {
            m_cFrom = m_cFrom+m_cJoinCond;
          } else {
            m_cWhere = m_cWhere+m_cLinkedWhere+m_cJoinCond;
  ...
  // * --- Fine Area Manuale
  public ardt_sogopeprovBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_sogopeprov");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_xogopebo+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xogopebo,m_cPhName_xogopebo,m_cVirtName_xogopebo,m_cVirtName_xogopebo+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_xogopebo,p_IDFILEBO),l_cOrderBy);
    ...
    l_oSqlDetailLoader.AddJoinedLink_2_3(m_cPhName_tbtipleg,m_cServer_tbtipleg);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xogopebo")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            ...
            Link_VFWKOPPXKC("Load");
          }
          w_LEGAME = l_rsLoadBody.GetString("LEGAME");
          if (l_oSqlDetailLoader.m_bLink_2_3) {
            w_xdescriz = l_rsLoadBody.GetString("DESCRIZ203");
          } else {
            Link_OSQIWZOZQN("Load");
          }
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_xogopebo.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"IDFILEBO="+CPLib.ToSQL(w_IDFILEBO,"C",10,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_xogopebo.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xogopebo")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_xogopebo,l_cSql);
  ...
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    ...
    if (CPLib.eq(p_cID,"VFWKOPPXKC")) {
      ...
      result.AppendFieldsHash("VFWKOPPXKC",CPLib.GetLinkedFieldProps("CONNES","CODSOG","C",16,0));
    } else if (CPLib.eq(p_cID,"OSQIWZOZQN")) {
      ...
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("OSQIWZOZQN",CPLib.GetLinkedFieldProps("TIPLEG","LEGAME","C",1,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xogopebo);
      l_cSql = "insert into "+m_oWrInfo_xogopebo.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDFILEBO,";
      l_cSql = l_cSql+"CODSOG,";
      l_cSql = l_cSql+"LEGAME,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_xogopebo.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xogopebo")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_xogopebo.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xogopebo")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xogopebo,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xogopebo);
      l_cSql = "Update "+m_oWrInfo_xogopebo.GetTableWriteName()+" set";
