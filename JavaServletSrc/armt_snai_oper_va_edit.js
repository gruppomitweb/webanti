...
function Set_CGOFLGVNP(ctrlValue,kmode,e) {
if (Ne(w_CGOFLGVNP,ctrlValue)) {
var ctl = _GetCtl(e,'UQLWTKHQTJ');
if (_tracker.goon(ctl,ctrlValue)) {
w_CGOFLGVNP=ctrlValue;
SetModified();
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('UQLWTKHQTJ',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH('','C',1,0,'');
...
parent.ZtVWeb.raiseEventToEvalParms('armt_snai_oper_va_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('armt_snai_oper_va_dataNotSent',{'action':formaction});
}
}
}
}
...
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('UQLWTKHQTJ',1))) {
_SignErr('UQLWTKHQTJ');
m_cLastWorkVarErrorMsg='448095445';
l_bResult=false;
w_CGOFLGVNP=HtW('','C');
