...
public class arpg_xapotit extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*EYFMHJTYQK*!";
  ...
  public String getEntityTitle() {
    return "Informazioni Titolari (Provvisorio)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_xapotit";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_xapotitBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"2009200748",p_Context.Translate("Informazioni Titolari (Provvisorio)")},
    {"36228043",p_Context.Translate("N° Informazione")},
    {"684190680",p_Context.Translate("Tipo Informazione")}};
    return l_translations;
  }
  void WriteStyles(arpg_xapotitBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_xapotit.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_xapotitBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_xapotitBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_xapotitBO BO,ServletStatus status) {
  ...
  void WriteControls_1_f_0(arpg_xapotitBO BO,ServletStatus status) {
    status.themeWriter.Footer_EndHtml();
  }
  static void PrintState(arpg_xapotitBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_xapotitBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_xapotitBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_xapotitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_xapotit"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_xapotit","");
  ...
  void WriteMainFormScript(arpg_xapotitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_xapotitBO getBO(ServletStatus status) {
    arpg_xapotitBO BO = new arpg_xapotitBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_xapotitBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_xapotitBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_xapotitBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_xapotitBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_xapotitBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_xapotitBO BO) {
    ...
    item.put("altInterfaceFor","arpg_xapotit");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_xapotitBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_xapotitBO BO) {
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_xapotitBO BO) {
    String l_cTrsQ;
  ...
  void Next(ServletStatus status,arpg_xapotitBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_xapotitBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_xapotitBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_xapotit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_xapotit_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_xapotitBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_xapotitBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_xapotitBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_xapotitBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_xapotitBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_xapotitBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_xapotitBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_xapotitBO BO) {
    boolean result = true;
    return result;
  }
}
