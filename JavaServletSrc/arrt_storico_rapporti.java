...
public class arrt_storico_rapporti extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*5bd6bb30*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_storico_rapporti";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_RAPPORTO);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_storico_rapportiR Rcalled;
    Rcalled = new arrt_storico_rapportiR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_storico_rapportiR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_storico_rapportiR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_storico_rapporti"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_storico_rapporti");
      if ( ! (CPLib.IsNull(l))) {
