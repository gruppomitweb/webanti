...
import jxl.*; 
// * --- Fine Area Manuale
public class allinea_wubsR implements CallerWithObjs {
  ...
  public int nfogli;
  // * --- Fine Area Manuale
  public allinea_wubsR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("allinea_wubs",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      double nTry0000005Estatus;
      nTry0000005Estatus = m_Sql.GetTransactionStatus();
      String cTry0000005Emsg;
      cTry0000005Emsg = m_Sql.TransactionErrorMessage();
      try {
        /* nomefileimport := 'Gennaio1.xls' */
        nomefileimport = "Gennaio1.xls";
        ...
        if (CPLib.eqr(m_Sql.GetTransactionStatus(),0) || CPLib.ne(nTry0000005Estatus,0)) {
          m_Sql.SetTransactionStatus(nTry0000005Estatus,cTry0000005Emsg);
        }
      }
    } finally {
      try {
  ...
  void Page_2() throws Exception {
    ...
    try {
      ...
      /* filenome := LRTrim(gPathApp)+'/appo/'+LRTrim(nomefileimport) */
      filenome = CPLib.LRTrim(gPathApp)+"/appo/"+CPLib.LRTrim(nomefileimport);
      // Legge il worksheet
      ...
      m_cSql = "";
      m_cSql = m_cSql+"RIFIMP="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.LRTrim(cell01),"C",15,0,m_cServer),m_cServer,CPLib.LRTrim(cell01));
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on operazbo into routine allinea_wubs returns two or more records. This item should return only a record.");
      ...
      if (CPLib.eqr(_idb,"")) {
        ...
        m_cSql = "";
        m_cSql = m_cSql+"RIFIMP="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.LRTrim(cell01),"C",28,0,m_cServer),m_cServer,CPLib.LRTrim(cell01));
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on fraziobo into routine allinea_wubs returns two or more records. This item should return only a record.");
            CPStdCounter.Error(l_oErrorFault);
          }
          Read_Cursor.Close();
        } else {
      ...
      if ( ! (CPLib.Empty(CPLib.LRTrim(_idb)))) {
        /* If Left(LRTrim(cell02),2)='IT' */
        if (CPLib.eqr(CPLib.Left(CPLib.LRTrim(cell02),2),"IT")) {
          /* _bstato := '086' */
          _bstato = "086";
        } else { // Else
          // * --- Read from tbstati
          ...
          m_cSql = "";
          m_cSql = m_cSql+"ISO2="+CPSql.SQLValueAdapter(CPLib.ToSQL(CPLib.Left(CPLib.LRTrim(cell02),2),"C",2,0,m_cServer),m_cServer,CPLib.Left(CPLib.LRTrim(cell02),2));
          ...
          if ( ! (Read_Cursor.Eof())) {
            _bstato = Read_Cursor.GetString("CODSTA");
            Read_Cursor.Next();
            if ( ! (Read_Cursor.Eof())) {
              Error l_oErrorFault = new Error("Read on tbstati into routine allinea_wubs returns two or more records. This item should return only a record.");
              CPStdCounter.Error(l_oErrorFault);
            }
            Read_Cursor.Close();
          } else {
        ...
        if (CPLib.eqr(_rappd,"O")) {
          ...
          while ( ! (Cursor_operazbo.Eof())) {
            /* If Empty(LRTrim(operazbo->PAESE)) */
            if (CPLib.Empty(CPLib.LRTrim(Cursor_operazbo.GetString("PAESE")))) {
              ...
              m_cSql = CPLib.GetSQLPrefixComment("allinea_wubs",768,"0000009B")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
              m_cSql = m_cSql+"PAESE = "+CPLib.ToSQL(_bstato,"C",3,0,m_cServer)+", ";
          ...
          // * --- End Select
          /* ElseIf _rappd = 'F' */
        } else if (CPLib.eqr(_rappd,"F")) {
          // * --- Select from fraziobo
          m_cServer = m_Ctx.GetServer("fraziobo");
          m_cPhName = m_Ctx.GetPhName("fraziobo");
          if (Cursor_fraziobo!=null)
            Cursor_fraziobo.Close();
          Cursor_fraziobo = m_Sql.Query(m_cServer,QueryLoader.SetSqlFunctions("select *  from "+m_cPhName+((m_cPhName).endsWith(CPSecurity.FILTER_QUERY_POSTFIX)?CPLib.NewCPCCCHK():"")+" where "+"("+"IDBASE="+CPSql.SQLValueAdapter(CPLib.ToSQL(_idb,"?",0,0,m_cServer, m_oParameters),m_cServer,_idb)+" "+")"+(m_Ctx.IsSharedTemp("fraziobo")?" and context_id='"+m_Ctx.GetTempID()+"'":" and 1=1"),m_cServer,m_Ctx,true));
          while ( ! (Cursor_fraziobo.Eof())) {
            /* If Empty(LRTrim(fraziobo->PAESE)) */
            if (CPLib.Empty(CPLib.LRTrim(Cursor_fraziobo.GetString("PAESE")))) {
              ...
              m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
              m_cSql = CPLib.GetSQLPrefixComment("allinea_wubs",768,"0000009E")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
              m_cSql = m_cSql+"PAESE = "+CPLib.ToSQL(_bstato,"C",3,0,m_cServer)+", ";
  ...
  public static allinea_wubsR Make(CPContext p_Ctx, Caller p_Caller) {
    return new allinea_wubsR(p_Ctx, p_Caller);
  }
  public void Blank() {
