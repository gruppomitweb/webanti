...
public class arpg_intestit_sel extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*TOOCFTIFTU*!";
  ...
  public String getEntityTitle() {
    return "Titolari Effettivi (A.U.I.)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_intestit_sel";
    ...
    l_eg.m_cQueryName = "qbe_intestit_sel";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_intestit_selBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10930911516",p_Context.Translate("Titolare")},
    {"113933141",p_Context.Translate("Update")},
    {"123801641",p_Context.Translate("Inizio")},
    {"1280510083",p_Context.Translate("Titolari Effettivi (A.U.I.)")},
    {"1805022818",p_Context.Translate("Rapporto")},
    {"443606",p_Context.Translate("Fine")}};
    return l_translations;
  }
  void WriteStyles(arpg_intestit_selBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_intestit_sel.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_intestit_selBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_intestit_selBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_intestit_selBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_intestit_selBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_intestit_selBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_intestit_selBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_intestit_selBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_intestit_sel"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_intestit_sel","");
  ...
  void WriteMainFormScript(arpg_intestit_selBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_intestit_selBO getBO(ServletStatus status) {
    arpg_intestit_selBO BO = new arpg_intestit_selBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_intestit_selBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_intestit_selBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_intestit_selBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_intestit_selBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_intestit_selBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_intestit_selBO BO) {
    ...
    item.put("altInterfaceFor","arpg_intestit_sel");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_intestit_selBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_intestit_selBO BO) {
    status.out.println("var c_CODINTER;");
    status.out.println("var c_DESCINTER;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_intestit_selBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_CODINTER,"C",16,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_DESCINTER,"C",70,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_DATAINI,"D",8,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_DATAFINE,"D",8,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_RAPPORTO,"C",25,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_intestit_selBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_intestit_selBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_intestit_selBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_intestit_sel?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_intestit_sel_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_intestit_selBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_intestit_selBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_intestit_selBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_intestit_selBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_intestit_selBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_intestit_selBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_intestit_selBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_intestit_selBO BO) {
    boolean result = true;
    return result;
  }
}
