...
public class arrt_famesiti_elab extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*b13bcbb1*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.w_nomsg","w_nomsg");
      }        
      w_nomsg = source.GetParameter("w_nomsg","");
    }
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_famesiti_elabR Rcalled;
    Rcalled = new arrt_famesiti_elabR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aa_retval = Rcalled.Run(status.w_nomefile,status.w_nomsg);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_famesiti_elabR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_famesiti_elabR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_famesiti_elab"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
