...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SUTANCBZLS').value,'C'),w_CODCAU)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('LSVJKMLQUN').value,'C'),w_idbase)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('tbcauana',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODCAU') &&  ! (Empty(HtW(Ctrl('SUTANCBZLS').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODCAU',HtW(Ctrl('SUTANCBZLS').value,'C'),'C');
}
if (Ne(name,'idbase') &&  ! (Empty(HtW(Ctrl('LSVJKMLQUN').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.idbase',HtW(Ctrl('LSVJKMLQUN').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_causalia');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('tbcauana',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['SUTANCBZLS','LSVJKMLQUN'],i=1;i>=0;i--){
