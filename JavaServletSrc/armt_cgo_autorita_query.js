...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('IQAQZDNBKW').value,'C'),w_ARCODICE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_autorita',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'ARCODICE') &&  ! (Empty(HtW(Ctrl('IQAQZDNBKW').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.ARCODICE',HtW(Ctrl('IQAQZDNBKW').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_cgo_docaut(firstAlias) {
return m_cVirtName+".ARCODICE = "+firstAlias+".DACODICE ";
}
...
_chNotF(childAlias,'master','armt_cgo_autorita');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_autorita',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['IQAQZDNBKW'],i=0;i>=0;i--){
