...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('HYBRJINCEX').value,'C'),w_CACODICE)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('VDOLZFWZIW').value,'C'),w_CACODANA)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('RIBSWSIRGO').value,'C'),w_CATIPOPE)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cgo_causali',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CACODICE') &&  ! (Empty(HtW(Ctrl('HYBRJINCEX').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CACODICE',HtW(Ctrl('HYBRJINCEX').value,'C'),'C');
}
if (Ne(name,'CACODANA') &&  ! (Empty(HtW(Ctrl('VDOLZFWZIW').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CACODANA',HtW(Ctrl('VDOLZFWZIW').value,'C'),'C');
}
if (Ne(name,'CATIPOPE') &&  ! (Empty(HtW(Ctrl('RIBSWSIRGO').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CATIPOPE',HtW(Ctrl('RIBSWSIRGO').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','armt_cgo_causali');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_causali',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['HYBRJINCEX','VDOLZFWZIW','RIBSWSIRGO'],i=2;i>=0;i--){
