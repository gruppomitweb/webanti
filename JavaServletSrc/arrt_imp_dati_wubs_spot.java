...
public class arrt_imp_dati_wubs_spot extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*c7c41bf*!";
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_imp_dati_wubs_spot";
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_xMAFLGBON) ||  ! (status.m_bpassedw_xMAFLGRAP) ||  ! (status.m_bpassedw_destfin) ||  ! (status.m_bpassedw_codmage) ||  ! (status.m_bpassedw_opextc) ||  ! (status.m_bpassedw_impzip) ||  ! (status.m_bpassedgMsgProc) ||  ! (status.m_bpassedw_tipodest);
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_imp_dati_wubs_spotR Rcalled;
    Rcalled = new arrt_imp_dati_wubs_spotR(status.context,status);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_imp_dati_wubs_spotR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_imp_dati_wubs_spotR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      ...
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_imp_dati_wubs_spot"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_imp_dati_wubs_spot");
      if ( ! (CPLib.IsNull(l))) {
