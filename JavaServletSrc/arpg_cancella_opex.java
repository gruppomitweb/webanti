...
public class arpg_cancella_opex extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*LCBJCELKXA*!";
  ...
  public String getEntityTitle() {
    return "Crea Cancellazioni Operazioni Extra Conto";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_cancella_opex";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_cancella_opexBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11412856543",p_Context.Translate("Crea Cancellazioni Operazioni Extra Conto")},
    ...
    {"839107543",p_Context.Translate("Confermi la creazione delle cancellazioni ?")}};
    return l_translations;
  }
  void WriteStyles(arpg_cancella_opexBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_cancella_opex.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_cancella_opexBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_cancella_opexBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_cancella_opexBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_cancella_opexBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_cancella_opexBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(350);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_cancella_opexBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.out.println(
     ...
     "<label id='XQSJGZVMDV'>" +
     ""+SPLib.ToHTMLRepresentation(status.context.Translate("Data Cancellazione:"))+"" +
     "</label>" +
     "</span>" +
    "");
    status.m_nTabIndex = status.m_nTabIndex+(1);
  ...
  static void PrintState(arpg_cancella_opexBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_cancella_opexBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_cancella_opexBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_cancella_opexBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_cancella_opex"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_cancella_opex","");
  ...
  void WriteMainFormScript(arpg_cancella_opexBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_cancella_opex");
  ...
  arpg_cancella_opexBO getBO(ServletStatus status) {
    arpg_cancella_opexBO BO = new arpg_cancella_opexBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_cancella_opexBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_cancella_opexBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_cancella_opexBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_cancella_opexBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_cancella_opexBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_cancella_opexBO BO) {
    ...
    item.put("altInterfaceFor","arpg_cancella_opex");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui elaborazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_XWPYHZIJDE()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"XWPYHZIJDE\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"6\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Anno_di_riferimento_\",\"page\":\"1\",\"spuid\":\"QETNOGRJRL_DIV\",\"tabindex\":\"7\",\"type\":\"Label\",\"value\":\"Anno di riferimento:\",\"w\":\"95\",\"x\":\"20\",\"y\":\"74\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"259\",\"zone\":\"YWGFSYCMOZ_6\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"5\",\"name\":\"anno\",\"page\":\"1\",\"spuid\":\"GTMJENMCMN\",\"tabindex\":\"10\",\"type\":\"Textbox\",\"w\":\"36\",\"x\":\"120\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"bg_color\":\" \",\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"16\",\"href\":\" \",\"name\":\"Data_Cancellazione_\",\"page\":\"1\",\"spuid\":\"XQSJGZVMDV_DIV\",\"tabindex\":\"13\",\"type\":\"Label\",\"value\":\"Data Cancellazione:\",\"w\":\"132\",\"x\":\"448\",\"y\":\"74\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"20\",\"maxlength\":\"10\",\"name\":\"datarett\",\"page\":\"1\",\"spuid\":\"JIYGZCDTPQ\",\"tabindex\":\"14\",\"type\":\"Textbox\",\"w\":\"78\",\"x\":\"584\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    SPLib.SetMimeType(status.response,"application/json");
    items.write(status.out);
  }
  void Next(ServletStatus status,arpg_cancella_opexBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_cancella_opexBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_cancella_opexBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_cancella_opex?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_cancella_opex_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_cancella_opexBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_cancella_opexBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_cancella_opexBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_cancella_opexBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_cancella_opexBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_cancella_opexBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_cancella_opex.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_cancella_opexBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_cancella_opexBO BO) {
    boolean result = true;
    return result;
  }
}
