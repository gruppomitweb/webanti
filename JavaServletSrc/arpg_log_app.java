...
public class arpg_log_app extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*PWKBNIZQNP*!";
  ...
  public String getEntityTitle() {
    return "Log Applicativo";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_log_app";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 823;
    ...
    l_eg.m_cQueryName = "qbe_log_app";
    l_eg.m_nRowsPerPage = 25;
    return l_eg;
  }
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_log_appBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"1055289069",p_Context.Translate("OPERAZIONE")},
    {"11374415403",p_Context.Translate("Log Applicativo")},
    {"113933141",p_Context.Translate("Update")},
    {"1175492196",p_Context.Translate("TABELLA")},
    {"376728758",p_Context.Translate("DATA e ORA")},
    {"78195349",p_Context.Translate("UTENTE")},
    {"78273987",p_Context.Translate("CHIAVE")}};
    return l_translations;
  }
  void WriteStyles(arpg_log_appBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_log_app.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_log_appBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_log_appBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_log_appBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_log_appBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_log_appBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_log_appBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_log_appBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      ...
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(823,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_log_app"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_log_app","");
  ...
  void WriteMainFormScript(arpg_log_appBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_log_appBO getBO(ServletStatus status) {
    arpg_log_appBO BO = new arpg_log_appBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_log_appBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_log_appBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_log_appBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,arpg_log_appBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_log_appBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_log_appBO BO) {
    ...
    item.put("altInterfaceFor","arpg_log_app");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_log_appBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_log_appBO BO) {
    status.out.println("var c_LOGDATORA;");
    status.out.println("var c_LOGUTENTE;");
    status.out.println("var c_LOGTIPOPE;");
    status.out.println("var c_LOGTABELLA;");
    status.out.println("var c_LOGCHIAVE;");
    status.out.println("var c_LOGTESTO;");
    status.out.println("var c_code;");
    status.out.println("var c_name;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_log_appBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_LOGDATORA,"T",14,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_LOGUTENTE,"C",10,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_LOGTIPOPE,"C",15,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_LOGTABELLA,"C",20,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_LOGCHIAVE,"C",30,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_LOGTESTO,"M",10,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_code,"N",6,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_name,"C",50,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
  }
  void Next(ServletStatus status,arpg_log_appBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_log_appBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_log_appBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_log_app?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_log_app_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_log_appBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_log_appBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_log_appBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_log_appBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_log_appBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_log_appBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_log_appBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_log_appBO BO) {
    boolean result = true;
    return result;
  }
}
