...
public class arpg_impdel_ip extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*EYOWRWVUPM*!";
  ...
  public String getEntityTitle() {
    return "Import dati da procedure esterne (Cancellazione Operazioni IP)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_impdel_ip";
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("tipodest","C",1,0, "hide",""),
                   ...
                   new SPItem("codmage","C",5,0, "hide","MacroAgente"),
                   new SPItem("xMADESCRI","C",60,0, "hide","Denominazione"),
                   new SPItem("destfin","C",1,0, "hide",""),
                   ...
                   new SPItem("annorif","N",4,0, "hide","Anno di riferimento"),
                   new SPItem("gAzienda","C",10,0, "hide","Azienda"),
                   new SPItem("gPathApp","C",80,0, "hide","Path Applicazione"),
                   new SPItem("fscarti","N",1,0, "hide",""),
                   new SPItem("newrec","C",1,0, "hide",""),
                   new SPItem("opextc","C",1,0, "hide",""),
                   new SPItem("creamo","C",1,0, "hide",""),
                   new SPItem("imperr","C",1,0, "hide",""),
                   new SPItem("chkaui","C",1,0, "hide",""),
                   new SPItem("gMsgImp","C",120,0, "hide","Messaggio Import"),
                   new SPItem("gMsgProc","C",120,0, "hide","Messaggio"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_impdel_ipBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10036132340",p_Context.Translate("Messaggio")},
    ...
    {"11220557556",p_Context.Translate("Messaggio Import")},
    {"11515895386",p_Context.Translate("Chiudi la maschera")},
    {"11572179009",p_Context.Translate("Stampa Elenco Nuove Operazioni ExtraConto da Inviare")},
    ...
    {"1698914546",p_Context.Translate("Crea modifica operazioni in caso di import del file PERSNEW.TXT (IDP)")},
    ...
    {"220123183",p_Context.Translate("Nuovo tracciato")},
    ...
    {"425653493",p_Context.Translate("Import dati da procedure esterne (Cancellazione Operazioni IP)")},
    {"801105630",p_Context.Translate("Associa Operazione a Rapporto")},
    {"863017169",p_Context.Translate("Anno di riferimento")}};
    return l_translations;
  }
  void WriteStyles(arpg_impdel_ipBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_impdel_ip.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_impdel_ipBO BO,ServletStatus status) {
    status.out.println(
  ...
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_DYMVKIQWCA_Vars(arpg_impdel_ipBO BO,ServletStatus status) {
  ...
  // GetPortlet_Log_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_KQMKKBMXYE_Vars(arpg_impdel_ipBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  // GetPortlet_ListaFile_Vars
  com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars GetPortlet_MVXXCZOQGP_Vars(arpg_impdel_ipBO BO,ServletStatus status) {
    com.zucchetti.sitepainter.Library.IncludedPortletUtils.Vars inclusionVars;
    inclusionVars = com.zucchetti.sitepainter.Library.IncludedPortletUtils.buildVars();
    return inclusionVars;
  }
  void WriteControls(arpg_impdel_ipBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_impdel_ipBO BO,ServletStatus status) {
    status.themeWriter.Header_BeginHtml(546);
    WriteControls_1_h_0(BO,status);
  }
  void WriteControls_1_h_0(arpg_impdel_ipBO BO,ServletStatus status) {
    status.out.println(
    ...
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println(
     "<a href='javascript:href_NBDWXOTRLG()' tabindex='-1' id='NBDWXOTRLG_HREF'>" +
     "<img id='NBDWXOTRLG' src="+SPLib.ToHTMLValue(SPLib.ToResource("images/print.gif"),"C",0,0)+" title="+SPLib.ToHTMLValue(status.context.Translate("Stampa Elenco Nuove Operazioni ExtraConto da Inviare"),"C",0,0)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' alt="+SPLib.ToHTMLValue(status.context.Translate("Stampa Elenco Nuove Operazioni ExtraConto da Inviare"),"C",0,0)+">" +
     "</a>" +
    "");
    status.themeWriter.Header_EndHtml();
  }
  static void PrintState(arpg_impdel_ipBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_impdel_ipBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_impdel_ipBL BO,ServletStatus status) {
    ...
    status.out.print(","+"["+SPLib.ToJSValue(BO.w_codmage,"C",5,0)+","+SPLib.ToJSValue(BO.o_codmage,"C",5,0)+"]");
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_newrec,"C",1,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_opextc,"C",1,0));
    status.out.print(","+SPLib.ToJSValue(BO.w_creamo,"C",1,0));
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_gMsgProc,"C",120,0));
  ...
  void WriteMainFormVarScript(arpg_impdel_ipBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_impdel_ip"),"C",0,0)+ ";");
    status.out.println("var m_bShowSuggest = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    status.out.println("var m_bShowNotifyBtn = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    status.out.println("var m_bShowExtendReduceBtn = " + SPLib.ToJSValue(false,"L",0,0)+ ";");
    HttpSession l_session = ((SPContext)status.context).session;
    status.out.print("var m_IDS={");
    status.out.print("'XFVQTXPEXL':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash("MACODICEMACODICE,MADESCRI,MAFLGRAP,MAFLGOPE,MAFLGBON,MAFLGCKR,MAFLGASRC,C,C,C,C,C,Ctbmacage",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,tbmacage,"+"default",l_session)+"'"+"]");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_impdel_ip","");
  ...
  void WriteMainFormScript(arpg_impdel_ipBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("// * --- arpg_impdel_ip");
    ...
    status.out.println("      SetLocationHref(null,'arrt_imp_dati_multipli'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')");
    status.out.println("    }");
    status.out.println("  }");
    status.out.println("  if (w_gTelepass == '2') {  ");
    status.out.println("    if (w_impzip == 0) {");
    status.out.println("      SetLocationHref(null,'arrt_imp_dati_telepass'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')");
    status.out.println("    } else {");
    status.out.println("      SetLocationHref(null,'arrt_imp_dati_multipli_telepass'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')   ");
    status.out.println("    } ");
    status.out.println("  }");
    status.out.println("  if (w_gTelepass == '3') {  ");
    status.out.println("    if (w_impzip == 0) {");
    status.out.println("      SetLocationHref(null,'arrt_imp_dati_storico'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')");
    status.out.println("    } else {");
    status.out.println("      SetLocationHref(null,'arrt_imp_dati_multipli_storico'+'?m_cAction=start&m_cMode=hyperlink','elaborazione')   ");
    status.out.println("    } ");
    status.out.println("  }  ");
  ...
  arpg_impdel_ipBO getBO(ServletStatus status) {
    arpg_impdel_ipBO BO = new arpg_impdel_ipBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_impdel_ipBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_impdel_ipBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_impdel_ipBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_impdel_ipBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_impdel_ipBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_impdel_ipBO BO) {
    ...
    item.put("altInterfaceFor","arpg_impdel_ip");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Esegui elaborazione\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_XWPYHZIJDE()\",\"name\":\"images_elaborazione_gif\",\"page\":\"1\",\"spuid\":\"XWPYHZIJDE\",\"src\":\"images\\/elaborazione.gif\",\"tabindex\":\"11\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"688\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"84\",\"name\":\"Portlet_DYMVKIQWCA\",\"page\":\"1\",\"src\":\"\\/jsp\\/showmsg_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____DYMVKIQWCA\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"455\",\"zone\":\"YWGFSYCMOZ_5\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"147\",\"name\":\"Portlet_KQMKKBMXYE\",\"page\":\"1\",\"src\":\"\\/jsp\\/showlog_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____KQMKKBMXYE\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"112\",\"zone\":\"YWGFSYCMOZ_3\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"\",\"h\":\"198\",\"name\":\"Portlet_MVXXCZOQGP\",\"page\":\"1\",\"src\":\"\\/jsp\\/list_imprich_portlet.jsp?ForcedPortletUID=___com__zucchetti__sitepainter__altinterface__m_cEntityUid____MVXXCZOQGP\",\"type\":\"Portlet\",\"w\":\"760\",\"x\":\"16\",\"y\":\"259\",\"zone\":\"YWGFSYCMOZ_4\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Chiudi la maschera\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_JSKZJOHJOM()\",\"name\":\"images_exit_gif\",\"page\":\"1\",\"spuid\":\"JSKZJOHJOM\",\"src\":\"images\\/exit.gif\",\"tabindex\":\"23\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"736\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"class_Css\":\" \",\"comment\":\"Stampa Elenco Nuove Operazioni ExtraConto da Inviare\",\"font\":\" \",\"font_color\":\" \",\"font_size\":\" \",\"font_weight\":\" \",\"h\":\"31\",\"href\":\"javascript:href_NBDWXOTRLG()\",\"name\":\"images_print_gif\",\"page\":\"1\",\"spuid\":\"NBDWXOTRLG\",\"src\":\"images\\/print.gif\",\"tabindex\":\"34\",\"type\":\"Image\",\"w\":\"40\",\"x\":\"640\",\"y\":\"70\",\"zone\":\"YWGFSYCMOZ_2\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{}";
    ctrl = new org.json.JSONObject(s);
  ...
  void Next(ServletStatus status,arpg_impdel_ipBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_impdel_ipBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_impdel_ipBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_impdel_ip?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_impdel_ip_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_impdel_ipBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_impdel_ipBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_impdel_ipBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_impdel_ipBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_impdel_ipBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_impdel_ipBO BO,ServletStatus status) {
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_impdel_ip.GetCallableNames(callable);
  ...
  public String GetCheckVars(ServletStatus status,arpg_impdel_ipBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_impdel_ipBO BO) {
    boolean result = true;
    return result;
  }
}
