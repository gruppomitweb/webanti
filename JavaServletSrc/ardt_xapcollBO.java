...
// * --- Fine Area Manuale
public class ardt_xapcollBO extends ardt_xapcollBL {
  ...
  // * --- Fine Area Manuale
  public ardt_xapcollBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_xapcoll");
    }
  ...
  public boolean Load(String p_IDFILEBO) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_xapcoll,m_cPhName_xapcoll,m_cVirtName_xapcoll,m_cVirtName_xapcoll+".IDFILEBO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_IDFILEBO,"C",10,0),m_cServer_xapcoll,p_IDFILEBO),"");
    // * --- Area Manuale = BO - Load Record Init
    // * --- Fine Area Manuale
    BlankStatusVariables();
    // * --- Area Manuale = BO - Before Load Detail
    // * --- Fine Area Manuale
    l_oSqlDetailLoader.AddJoinedLink_2_1(m_cPhName_knarapbo,m_cServer_knarapbo);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"xapcoll")) {
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xapcoll);
      l_cSql = "insert into "+m_oWrInfo_xapcoll.GetTableWriteName()+" (";
      l_cSql = l_cSql+"IDFILEBO,";
      l_cSql = l_cSql+"RAPPORTO,";
      l_cSql = l_cSql+"IDBASE,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_xapcoll.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xapcoll")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_xapcoll.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"xapcoll")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_xapcoll,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_xapcoll);
      l_cSql = "Update "+m_oWrInfo_xapcoll.GetTableWriteName()+" set";
