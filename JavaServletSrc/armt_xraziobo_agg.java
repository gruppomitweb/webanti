...
public class armt_xraziobo_agg extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*1cbfde1b*!";
  static public class ServletStatus extends SPMasterChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "xraziobo_agg";
    }
    public String w_IDBASE;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_IDBASE,context,"xraziobo_agg","IDBASE","C",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    armt_xraziobo_aggBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_xraziobo_agg;
  ...
  public String getEntityTitle() {
    return "Dati Aggiuntivi Frazionate Prov.";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_xraziobo_agg";
    ...
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"xraziobo_agg","tbcateg01","tbcateg02","tbcateg03","tbcateg04","tbcateg05"};
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_xraziobo_aggBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"785379127",p_Context.Translate("Dati Aggiuntivi Frazionate Prov.")}};
    return l_translations;
  }
  void WriteStyles(armt_xraziobo_aggBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_xraziobo_agg.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_xraziobo_aggBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_xraziobo_agg',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_xraziobo_agg",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(armt_xraziobo_aggBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(armt_xraziobo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b0(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_xraziobo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b1_CloseBand(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b2(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b3(armt_xraziobo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b3_CloseBand(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b4(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b5(armt_xraziobo_aggBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls_p1_b5_CloseBand(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b6(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_xraziobo_aggBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_xraziobo_aggBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(armt_xraziobo_aggBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(armt_xraziobo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_xraziobo_agg"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_xraziobo_agg","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_xraziobo_agg",BO.GetCurrentState());
  ...
  void WriteMainFormScript(armt_xraziobo_aggBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_armt_xraziobo_agg);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_armt_xraziobo_agg();");
      status.out.println("}");
    }
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_xraziobo_agg',Ne(m_nChildStatus,0));");
    } else {
    ...
    status.out.println("// * --- armt_xraziobo_agg");
    ...
    status.out.println("m_oFather.m_oarmt_xraziobo_agg=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  armt_xraziobo_aggBO getBO(ServletStatus status) {
    armt_xraziobo_aggBO BO = new armt_xraziobo_aggBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(armt_xraziobo_aggBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("xraziobo_agg",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_xraziobo_aggBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,armt_xraziobo_aggBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(armt_xraziobo_aggBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_xraziobo_agg;
    String l_cPhName = BO.m_cPhName_xraziobo_agg;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_xraziobo_aggBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_xraziobo_agg+".IDBASE",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("xraziobo_agg");
    String l_cPhName = CPSql.ManipulateTablePhName("xraziobo_agg",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,armt_xraziobo_aggBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_xraziobo_agg,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_xraziobo_agg,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,armt_xraziobo_aggBO BO) {
  ...
  void Edit(ServletStatus status,armt_xraziobo_aggBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_xraziobo_aggBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_xraziobo_aggBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_xraziobo_aggBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_xraziobo_aggBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_xraziobo_aggBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_xraziobo_aggBO BO,SPParameterSource source) {
    BO.Load(status.w_IDBASE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_xraziobo_aggBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_xraziobo_aggBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_xraziobo_aggBO BO) {
    ...
    item.put("altInterfaceFor","armt_xraziobo_agg");
    items.put(item);
    String s;
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NRWNAJJVRA\",\"page\":1,\"spuid\":\"NRWNAJJVRA\",\"tabindex\":16,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":11,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_WWRYOVTMFY\",\"page\":1,\"spuid\":\"WWRYOVTMFY\",\"tabindex\":17,\"type\":\"Label\",\"value\":\"Categoria 1:\",\"w\":84,\"x\":8,\"y\":11,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_LNOFOPAOYD\",\"page\":1,\"spuid\":\"LNOFOPAOYD\",\"tabindex\":18,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":35,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_HNQZARYXGS\",\"page\":1,\"spuid\":\"HNQZARYXGS\",\"tabindex\":19,\"type\":\"Label\",\"value\":\"Categoria 2:\",\"w\":84,\"x\":8,\"y\":35,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_WBTKNDJDFF\",\"page\":1,\"spuid\":\"WBTKNDJDFF\",\"tabindex\":20,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":59,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BGEVFUGGOS\",\"page\":1,\"spuid\":\"BGEVFUGGOS\",\"tabindex\":21,\"type\":\"Label\",\"value\":\"Categoria 3:\",\"w\":84,\"x\":8,\"y\":59,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_MWCFKQVNHF\",\"page\":1,\"spuid\":\"MWCFKQVNHF\",\"tabindex\":22,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":83,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_RHXCIUHRTU\",\"page\":1,\"spuid\":\"RHXCIUHRTU\",\"tabindex\":23,\"type\":\"Label\",\"value\":\"Categoria 4:\",\"w\":84,\"x\":8,\"y\":83,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HADVHTQAPE\",\"page\":1,\"spuid\":\"HADVHTQAPE\",\"tabindex\":24,\"type\":\"Label\",\"w\":488,\"x\":152,\"y\":107,\"zone\":\"pag1_1_5\",\"zonepath\":\"[1,1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_GRCYFEKJWU\",\"page\":1,\"spuid\":\"GRCYFEKJWU\",\"tabindex\":25,\"type\":\"Label\",\"value\":\"Categoria 5:\",\"w\":84,\"x\":8,\"y\":107,\"zone\":\"pag1_1_5\",\"zonepath\":\"[1,1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_XVUIFQMEIG\",\"page\":1,\"spuid\":\"XVUIFQMEIG\",\"tabindex\":27,\"type\":\"Label\",\"value\":\"SV58724 Modalit\\u00E0 Bonifici:\",\"w\":168,\"x\":336,\"y\":139,\"zone\":\"pag1_2_1\",\"zonepath\":\"[1,2,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_WFICGFEUAH\",\"page\":1,\"spuid\":\"WFICGFEUAH\",\"tabindex\":30,\"type\":\"Label\",\"value\":\"SV58726  Schema di addebito:\",\"w\":168,\"x\":336,\"y\":165,\"zone\":\"pag1_2_2\",\"zonepath\":\"[1,2,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    ...
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_VIBYCNNDWC\",\"page\":1,\"spuid\":\"VIBYCNNDWC\",\"tabindex\":33,\"type\":\"Label\",\"value\":\"SV58726 Modalit\\u00E0 PISP:\",\"w\":168,\"x\":336,\"y\":193,\"zone\":\"pag1_2_3\",\"zonepath\":\"[1,2,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_xraziobo_aggBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_xraziobo_agg?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_xraziobo_agg_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_xraziobo_agg_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_xraziobo_agg_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","xraziobo_agg");
  ...
  void RaiseLoadedEntity(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_xraziobo_aggBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_xraziobo_aggBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_xraziobo_aggBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_xraziobo_aggBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,armt_xraziobo_aggBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_xraziobo_aggBO BO) {
    boolean result = true;
