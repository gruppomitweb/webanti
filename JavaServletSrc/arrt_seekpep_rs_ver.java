...
public class arrt_seekpep_rs_ver extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*3784fc64*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.pDataTic","pDataTic");
      }        
      pDataTic = source.GetParameter("pDataTic","");
    }
  }
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_seekpep_rs_verR Rcalled;
    Rcalled = new arrt_seekpep_rs_verR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aa_retval = Rcalled.Run(status.pTicket,status.pDataTic);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_seekpep_rs_verR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_seekpep_rs_verR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_seekpep_rs_ver"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
