...
public class ardt_jntestit extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*da74e23d*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "jntestit";
    }
    public String w_CODCLI;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_CODCLI,context,"jntestit","CODCLI","C",16,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_jntestit;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "ardt_jntestit";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 740;
    l_eg.m_nEntityHeight = 313;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"jntestit","personbo"};
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","","page"),new ZoneItem("DatiAN","Dati Anagrafici","horizontal_section"),new ZoneItem("Prog","Progressivi","horizontal_section")};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("PRGIMPTITE","N",15,0, "edit",""),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,ardt_jntestitBO BO) {
  ...
  void WriteStyles(ardt_jntestitBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../ardt_jntestit.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(ardt_jntestitBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(".HKZAUFLHMG {top:0px;position:absolute;left:576px;height:16px;width:64px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".NATIUIQLJX {top:0px;position:absolute;left:648px;height:16px;width:64px;text-align:right;background-color:Transparent;z-index:1; }");
      status.out.println(".ZUSOGKEVZC {top:1px;position:absolute;left:136px;height:16px;width:432px;background-color:Transparent;z-index:1; }");
      status.out.println(
       "</style>" +
      "");
    } else {
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_jntestit',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"ardt_jntestit",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(ardt_jntestitBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(ardt_jntestitBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:313px;width:740px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(ardt_jntestitBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_TOP_OF_PAGE_1' style='position:relative;height:128px;display:block' page='1'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      status.out.println(
       "<div name id='GridTable_Header' class='GridTable_Header' style='position:absolute;top:-1px;left:6px;height:18px;width:734px;z-index:0'>" +
       "</div>" +
      "");
      status.out.println(
       "<span class='SPLabel GridTable_HeaderTitle PrimaryKeyHeaderLabel' id='BFMPSJTVZY_DIV'>" +
      ...
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:absolute;top:17px;left:6px;height:106px;width:734px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:106px;width:734px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:734px;height:106px;overflow:auto;'>" +
      ...
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:17px;left:139px;height:106px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:17px;left:580px;height:106px;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:17px;left:652px;height:106px;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='PRGIMPTITE' id='CHJPRCYMKT' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_PRGIMPTITE,"N",15,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='16' size='15' fieldtype='N' style='display:none'>" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='IDBASE' id='QDIDYGGMQV' type='text' class='Field' value="+SPLib.ToHTMLValue(BO.w_IDBASE,"C",10,0,"", status.m_oRegSettings)+" tabindex='"+status.m_nTabIndex+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+" maxlength='10' size='10' fieldtype='C' style='display:none'>" +
      "");
      status.m_nTabIndex = status.m_nTabIndex+1;
      status.out.println(
       "</div>" +
  ...
  void HTMLRowScript(ardt_jntestitBO BO,ServletStatus status) {
    ...
    status.out.println("s='<div style=\"position:relative;width:712px;height:19px;\"';");
    ...
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:712px;height:19px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b1(ardt_jntestitBO BO,ServletStatus status) {
    status.out.println(
     "<div name='DatiAN' id='VRCVINYRJO_DIV' class='BoxContainer SPSection_expanded SectionContainerWithTitle'>" +
    "");
    ...
    status.out.println(
     "<div id='VRCVINYRJO' class='SectionBody' style='position:relative;height:99px;display:block'>" +
    "");
    status.includeZoneAltInterface("DatiAN@BEGIN");
    if (status.includeZoneAltInterface("DatiAN")) {
      WriteControls_p1_b1_CloseBand(BO,status);
    } else {
      if (status.HasZoneAltInterface("DatiAN@BEGIN") || status.HasZoneAltInterface("DatiAN@END")) {
        status.out.println(
         "<div id='VRCVINYRJO_boxContent' style='position:relative;height:99px;display:block'>" +
        "");
      }
      status.out.println(
      ...
      status.out.println(
       ...
       "<label id='RUCUNEMGZA' for='QHHDRAEPOC'>" +
  ...
  void WriteControls_p1_b1_CloseBand(ardt_jntestitBO BO,ServletStatus status) {
    status.includeZoneAltInterface("DatiAN@END");
    status.out.println(
     "</div>" +
    "");
    status.out.println(
    ...
    status.out.println(
     "<script>" +
    "");
    if (status.HasZoneAnyAltInterface("DatiAN")) {
      status.out.println("Ctrl('VRCVINYRJO').style.height='auto';");
    } else {
      status.out.println("Ctrl('VRCVINYRJO').setAttribute('band_height',99);");
    }
  ...
  void WriteControls_p1_b2(ardt_jntestitBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_BEFORE_MINAAOAOZC")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_BEFORE_MINAAOAOZC' style='position:relative;height:1px;display:block' page='1'>" +
    "");
  ...
  void WriteControls_p1_b3(ardt_jntestitBO BO,ServletStatus status) {
    status.out.println(
     "<div name='Prog' id='MINAAOAOZC_DIV' class='BoxContainer SPSection_expanded SectionContainerWithTitle'>" +
    "");
    if (CPLib.eq(status.GetBoxHeaderInclude(),"")) {
      status.out.println(
    ...
    "");
    status.includeZoneAltInterface("Prog@BEGIN");
    if (status.includeZoneAltInterface("Prog")) {
      WriteControls_p1_b3_CloseBand(BO,status);
    } else {
      if (status.HasZoneAltInterface("Prog@BEGIN") || status.HasZoneAltInterface("Prog@END")) {
        status.out.println(
      ...
      status.out.println(
       ...
       "<label id='BHLAQHLOJQ' for='LEZBMECOAR'>" +
      ...
      status.out.println(
       ...
       "<label id='EQUZNNIXBO' for='ORIJTEDVRI'>" +
  ...
  void WriteControls_p1_b3_CloseBand(ardt_jntestitBO BO,ServletStatus status) {
    status.includeZoneAltInterface("Prog@END");
    status.out.println(
     "</div>" +
    "");
    status.out.println(
    ...
    status.out.println(
     "<script>" +
    "");
    if (status.HasZoneAnyAltInterface("Prog")) {
  ...
  void WriteControls_p1_b4(ardt_jntestitBO BO,ServletStatus status) {
  ...
  static void PrintState(ardt_jntestitBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(ardt_jntestitBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(ardt_jntestitBL BO,ServletStatus status) {
    ...
    status.out.print(","+SPLib.ToJSValue(BO.w_PRGIMPTITE,"N",15,0));
  ...
  static void PrintStateDetailWorkVars(ardt_jntestitBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(ardt_jntestitBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      ardt_jntestitBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (ardt_jntestitBO.TrsRow) BO.m_oDeletedRows.get(k);
        ...
        l_cTrs = l_cTrs+SPLib.ToJSValue(l_Row.w_PRGIMPTITE,"N",15,0)+",";
  ...
  void WriteMainFormVarScript(ardt_jntestitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(313,"N",0,0)+ ";");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"ardt_jntestit"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("ardt_jntestit","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"ardt_jntestit",BO.GetCurrentState());
    ...
    if (CPLib.eq(status.GetZoneAltInterface("DatiAN"),"")) {
      status.quoteAndAppend("DatiAN","hide","box");
    }
    if (CPLib.eq(status.GetZoneAltInterface("Prog"),"")) {
      status.quoteAndAppend("Prog","hide","box");
    }
    ...
    status.out.println("this.DESCINTER=arguments[3];");
    status.out.println("this.TIPOPERS=arguments[4];");
    status.out.println("this.xragsoci=arguments[5];");
    status.out.println("this.tiporegr=arguments[6];");
    ...
    status.out.println("this.PRGIMPTITE=arguments[11];");
    status.out.println("this.STATOREG=arguments[12];");
    status.out.println("this.IDEREG=arguments[13];");
    status.out.println("this.TIPORAP=arguments[14];");
    status.out.println("this.NUMPROG1=arguments[15];");
    status.out.println("this.NUMPROG2=arguments[16];");
    status.out.println("this.IDBASE=arguments[17];");
    status.out.println("this.opk_CODINTER=arguments[18];");
  ...
  void WriteMainFormScript(ardt_jntestitBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_ardt_jntestit);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_ardt_jntestit();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('ardt_jntestit',Ne(m_nChildStatus,0));");
    } else {
    ...
    status.out.println("m_oFather.m_oardt_jntestit=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  ardt_jntestitBO getBO(ServletStatus status) {
    ardt_jntestitBO BO = new ardt_jntestitBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(ardt_jntestitBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("jntestit",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,ardt_jntestitBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,ardt_jntestitBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(ardt_jntestitBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_jntestit;
    String l_cPhName = BO.m_cPhName_jntestit;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = ardt_jntestitBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_jntestit+".CODCLI"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_jntestit+".CODCLI"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("jntestit");
    String l_cPhName = CPSql.ManipulateTablePhName("jntestit",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,ardt_jntestitBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_jntestit),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_jntestit,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_jntestit+".CODCLI) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_jntestit,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_jntestit,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_jntestit+".CODCLI",CPLib.GetTablesWithAliases(BO.m_cPhName_jntestit,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,ardt_jntestitBO BO) {
  ...
  void Edit(ServletStatus status,ardt_jntestitBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,ardt_jntestitBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,ardt_jntestitBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,ardt_jntestitBO BO) {
  ...
  void EditLoad(ServletStatus status,ardt_jntestitBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,ardt_jntestitBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,ardt_jntestitBO BO,SPParameterSource source) {
    BO.Load(status.w_CODCLI);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,ardt_jntestitBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,ardt_jntestitBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,ardt_jntestitBO BO) {
    ...
    item.put("altInterfaceFor","ardt_jntestit");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_XCDXQXTRHM\",\"page\":1,\"spuid\":\"XCDXQXTRHM\",\"tabindex\":2,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"Inizio\",\"w\":64,\"x\":584,\"y\":2,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_BFMPSJTVZY\",\"page\":1,\"spuid\":\"BFMPSJTVZY\",\"tabindex\":1,\"type\":\"Label\",\"value\":\"Titolare\",\"w\":61,\"x\":8,\"y\":0,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_QEAAMPSBPR\",\"page\":1,\"spuid\":\"QEAAMPSBPR\",\"tabindex\":3,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"Fine\",\"w\":64,\"x\":656,\"y\":2,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":102,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"RWNUXCRUTV\",\"type\":\"BodyDetail\",\"w\":768,\"x\":8,\"y\":18,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"DatiAN\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Dati Anagrafici\",\"h\":99,\"name\":\"DatiAN\",\"page\":1,\"shrinkable\":true,\"spuid\":\"VRCVINYRJO\",\"stretch\":true,\"tabindex\":5,\"title_caption\":\"Dati Anagrafici\",\"titled\":true,\"type\":\"Box\",\"w\":728,\"x\":8,\"y\":128,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_RUCUNEMGZA\",\"page\":1,\"spuid\":\"RUCUNEMGZA\",\"tabindex\":14,\"type\":\"Label\",\"value\":\"Motivazione Titolarit\\u00E0:\",\"w\":124,\"x\":8,\"y\":141,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"fixed\":true,\"h\":20,\"maxlength\":70,\"name\":\"DESCINTER\",\"page\":1,\"repeated\":true,\"spuid\":\"QHHDRAEPOC\",\"tabindex\":4,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":432,\"x\":136,\"y\":137,\"zone\":\"pag1_3_1\",\"zonepath\":\"[1,3,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"center\",\"h\":16,\"name\":\"label_AVTVZXZUWT\",\"page\":1,\"spuid\":\"AVTVZXZUWT\",\"tabindex\":6,\"type\":\"Label\",\"typevar\":\"character\",\"value\":\"DATI ANAGRAFICI\",\"w\":712,\"x\":16,\"y\":164,\"zone\":\"pag1_3_2\",\"zonepath\":\"[1,3,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_NAKGRMOUXS\",\"page\":1,\"spuid\":\"NAKGRMOUXS\",\"tabindex\":7,\"type\":\"Label\",\"w\":648,\"x\":16,\"y\":185,\"zone\":\"pag1_3_3\",\"zonepath\":\"[1,3,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HMPQLYVATL\",\"page\":1,\"spuid\":\"HMPQLYVATL\",\"tabindex\":8,\"type\":\"Label\",\"w\":648,\"x\":16,\"y\":206,\"zone\":\"pag1_3_4\",\"zonepath\":\"[1,3,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Prog\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Progressivi\",\"h\":45,\"name\":\"Prog\",\"page\":1,\"shrinkable\":true,\"spuid\":\"MINAAOAOZC\",\"stretch\":true,\"tabindex\":11,\"title_caption\":\"Progressivi\",\"titled\":true,\"type\":\"Box\",\"w\":728,\"x\":8,\"y\":228,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_BHLAQHLOJQ\",\"page\":1,\"spuid\":\"BHLAQHLOJQ\",\"tabindex\":12,\"type\":\"Label\",\"value\":\"Prog.Informazione Apertura:\",\"w\":147,\"x\":40,\"y\":245,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"align\":\"right\",\"h\":16,\"name\":\"label_EQUZNNIXBO\",\"page\":1,\"spuid\":\"EQUZNNIXBO\",\"tabindex\":13,\"type\":\"Label\",\"value\":\"Prog.Informazione Chiusura:\",\"w\":163,\"x\":488,\"y\":245,\"zone\":\"pag1_4_2\",\"zonepath\":\"[1,4,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Prog.Informazione Apertura\",\"disabled\":\"true\",\"fixed\":true,\"h\":23,\"maxlength\":11,\"name\":\"NUMPROG1\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"LEZBMECOAR\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":80,\"x\":192,\"y\":238,\"zone\":\"pag1_4_3\",\"zonepath\":\"[1,4,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Prog.Informazione Chiusura\",\"disabled\":\"true\",\"fixed\":true,\"h\":23,\"maxlength\":11,\"name\":\"NUMPROG2\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"ORIJTEDVRI\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":72,\"x\":656,\"y\":238,\"zone\":\"pag1_4_4\",\"zonepath\":\"[1,4,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":39,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"BSOQFQNVFQ\",\"type\":\"Box\",\"w\":712,\"x\":8,\"y\":-1,\"zone\":\"pag1_5\",\"zonepath\":\"[1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"Intestatario (Non Intermediario)\",\"h\":17,\"maxlength\":16,\"name\":\"CODINTER\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"XTDHYWJPZL\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":108,\"x\":8,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Inizio\",\"h\":17,\"maxlength\":10,\"name\":\"DATAINI\",\"page\":1,\"repeated\":true,\"spuid\":\"HKZAUFLHMG\",\"tabindex\":2,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":64,\"x\":584,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Data Fine\",\"h\":17,\"maxlength\":10,\"name\":\"DATAFINE\",\"page\":1,\"repeated\":true,\"spuid\":\"NATIUIQLJX\",\"tabindex\":3,\"type\":\"Textbox\",\"typevar\":\"date\",\"w\":64,\"x\":656,\"y\":20,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Ragione Sociale\",\"disabled\":\"true\",\"h\":17,\"maxlength\":40,\"name\":\"xragsoci\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"ZUSOGKEVZC\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":432,\"x\":144,\"y\":21,\"zone\":\"pag1_5_1\",\"zonepath\":\"[1,5,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,ardt_jntestitBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(ardt_jntestitBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='ardt_jntestit?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_jntestit_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_jntestit_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_DESCINTER=a[3];");
      status.out.println("w_TIPOPERS=a[4];");
      status.out.println("w_CODCLI=a[5];");
      status.out.println("w_PRGIMPTITE=a[6];");
      ...
      status.out.println("w_NUMPROG2=a[11];");
      status.out.println("w_IDBASE=a[12];");
      status.out.println("w_xragsoci=a[13];");
      status.out.println("w_tipreg=a[14];");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/ardt_jntestit_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(313,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void RaiseLoadedEntity(ardt_jntestitBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(ardt_jntestitBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(ardt_jntestitBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(ardt_jntestitBO BO,ServletStatus status) {
  ...
  void WriteExitPage(ardt_jntestitBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(ardt_jntestitBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(ardt_jntestitBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,ardt_jntestitBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,ardt_jntestitBO BO) {
    boolean result = true;
