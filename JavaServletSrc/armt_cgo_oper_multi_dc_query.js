...
bResult=Eq(w_gCodDip,w_CODDIPE);
}
...
WindowAlert(AlertErrorMessage('779449074'));
}
if (Eq(formaction,'delete') &&  ! (bResult)) {
WindowAlert(AlertErrorMessage('10678724575'));
}
}
}
var m_bIsMq = false;
...
m_oFather.DoQueryFromChild('cgo_operazioni_dl_multi_dc',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
function DoQueryFrom_cgo_operazioni_dl_multi_dt_dc(firstAlias) {
...
_chNotF(childAlias,'master','armt_cgo_oper_multi_dc');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cgo_operazioni_dl_multi_dc',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
