...
// * --- Fine Area Manuale
public class armt_tbnaturaageBO extends armt_tbnaturaageBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbnaturaageBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbnaturaage");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbnaturaage,m_cPhName_tbnaturaage,m_cVirtName_tbnaturaage,m_cVirtName_tbnaturaage+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",2,0),m_cServer_tbnaturaage,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnaturaage")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbnaturaage);
      l_cSql = "insert into "+m_oWrInfo_tbnaturaage.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbnaturaage.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnaturaage")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_tbnaturaage.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnaturaage")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbnaturaage,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbnaturaage);
      l_cSql = "Update "+m_oWrInfo_tbnaturaage.GetTableWriteName()+" set";
      ...
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",2,0),m_cServer_tbnaturaage,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbnaturaage.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnaturaage")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbnaturaage.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",2,0),m_cServer_tbnaturaage,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbnaturaage.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbnaturaage")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbnaturaage,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
