...
public class ardt_settoreuicBL extends ardt_settoreuicWV implements CPLib.QueryFilterFromApplication {
  static public class TrsRow {
    public String w_CODATECO;
    public String opk_CODATECO;
  ...
  public static final String i_EntityName = "ardt_settoreuic";
  ...
  public CPPhTableWrInfo m_oWrInfo_tbsetsina;
  // Variabile che contiene il nome virtuale della tabella
  public String m_cVirtName_tbsetsina;
  ...
  public static String[] m_EntityTables = {"tbsetsina"};
  // codice secondo sintassi delle business rules per produrre i dati da usare in un ideale documento che li rappresenti
  public static final String i_StateStructure = "{'SETSINT':SETSINT,'IDSETSIN':IDSETSIN,'m_Detail':{'CODATECO':CODATECO"+FoundlingChildrenList("ardt_settoreuic",true,true)+"}"+FoundlingChildrenList("ardt_settoreuic",true,false)+"}";
  public static final String i_CompleteStateStructure = "{'SETSINT':SETSINT,'IDSETSIN':IDSETSIN,'m_Detail':{'CODATECO':CODATECO"+FoundlingChildrenList("ardt_settoreuic",true,true)+"}"+FoundlingChildrenList("ardt_settoreuic",true,false)+"}";
  public static final String i_ItemSequence = "SETSINT,IDSETSIN,m_Detail[ CODATECO"+FoundlingChildrenList("ardt_settoreuic",false,true)+"]"+FoundlingChildrenList("ardt_settoreuic",false,false)+"";
  public static final com.zucchetti.sitepainter.EntityItem[] i_EditableEntityItems = com.zucchetti.sitepainter.EntityItem.buildEditableItemsForEntity(GetLinkDef(),FoundlingChildrenList("ardt_settoreuic"),new String[][]{new String[]{"SETSINT","Settore Sintetico","C","3","0","campo","key1","false","false"},new String[]{"IDSETSIN","ID Settore Sintetico","N","5","0","campo","key1","false","false"},new String[]{"CODATECO","Codice ATECO (Primi Tre)","C","3","0","campo","key1","true","false"}});
  public static final String i_TrsKeys = ",CODATECO";
  ...
  public ardt_settoreuicBL (CPContext p_ContextObject) {
    ...
    m_cPhName_tbsetsina = p_ContextObject.GetPhName("tbsetsina",p_ContextObject.GetCompany(),false);
    m_cServer_tbsetsina = p_ContextObject.GetServer("tbsetsina");
    m_oWrInfo_tbsetsina = CPPhTableWrInfo.GetCPPhTableWrInfo(p_ContextObject,"tbsetsina",p_ContextObject.GetCompany());
    m_cVirtName_tbsetsina = CPSql.ManipulateTablePhName("tbsetsina",m_cServer_tbsetsina);
    // Assegnazione della variabile di collegamento con il database
    m_Sql = p_ContextObject.GetSql();
    m_Ctx = p_ContextObject;
    // Istanziazione dei business object dei figli
    m_MNTs = CPLib.MNTChilds("ardt_settoreuic",p_ContextObject);
  }
  public void Blank() {
  ...
  public boolean Set_CODATECO(String p_workVariableValue) {
    boolean l_bResult;
    l_bResult = true;
    // Variabile temporanea che conterrà il risultato della funzione di link se l'item corrente è parte di una chiave composta
    boolean l_bTmpRes;
    SetUpdatedRow();
    w_CODATECO = p_workVariableValue;
    // Esecuzione dei calcoli
    Calculate();
    SaveDependsOn();
    return l_bResult;
  ...
  public static String QueryFilter(CPContext m_Ctx) {
    String l_cFilter;
    l_cFilter = CPLib.ApplicationQueryFilter("ardt_settoreuic","tbsetsina",m_KeyColumns,m_Ctx);
    return l_cFilter;
  }
  ...
  public void SetFromContext(String s,java.util.Properties p) {
    ...
    if ( ! (CPLib.Empty(s))) {
      ...
      Properties p_CODATECO;
      p_CODATECO = CPLib.GetProperties(trs_p.getProperty("CODATECO"),new Properties());
      Properties p_opk_CODATECO;
      p_opk_CODATECO = CPLib.GetProperties(trs_p.getProperty("opk_CODATECO"),new Properties());
      ...
      while (CPLib.le(i,rows)) {
        w_CODATECO = "";
        w_CODATECO = CPLib.GetProperty(p_CODATECO,"",w_CODATECO,i);
        opk_CODATECO = CPLib.GetProperty(p_opk_CODATECO,"","",i);
  ...
  void SetUpdated(String p_cName) {
    if (CPLib.eq(p_cName,"SETSINT")) {
      SetUpdated();
      return;
    }
    if (CPLib.eq(p_cName,"IDSETSIN")) {
      SetUpdated();
      return;
    }
    if (CPLib.eq(p_cName,"CODATECO")) {
      SetUpdatedRow();
      return;
    }
  }
  ...
  public static Map<String,List<Map<String,String[]>>> GetLinkDef() {
    ...
    List<Map<String,String[]>> items;
    GetLinkDef0(tables);
    for(String child : CPLib.LoadableMNTs("ardt_settoreuic",false)) {
      ...
      String[][] definitions = CPMNTChild.GetM_Fkd("ardt_settoreuic",child);
    ...
    for(String child : CPLib.LoadableMNTs("ardt_settoreuic",true)) {
      map = new HashMap<String,String[]>();
      items = tables.get(com.zucchetti.sitepainter.Library.getEntityTables(child)[0]);
      if (CPLib.IsNull(items)) {
        items = new ArrayList<Map<String,String[]>>();
  ...
  public void Initialize() {
    ...
    m_MNTs = CPLib.MNTChilds("ardt_settoreuic",m_Ctx);
  ...
  public String[] GetStateSecurityKeyRoles(String p_cState) {
    ...
    if (CPLib.eq(p_cState,CPSecurity.UNKNOWN_STATE)) {
      l_Keys = new String[]{"ardt_settoreuic"};
    }
    if (CPLib.eq(l_Keys.length,0)) {
      l_Keys = new String[]{"ardt_settoreuic\\"+p_cState};
    }
    return l_Keys;
  }
  ...
  public void GetContext(boolean p_bGetChildren,java.util.Properties p) {
    ...
    Properties p_CODATECO;
    p_CODATECO = new Properties();
    Properties p_opk_CODATECO;
    p_opk_CODATECO = new Properties();
    Properties p_m_nRowStatus;
    ...
    while (CPLib.lt(i,Rows())) {
      SetRow(i);
      trsrow = trsrow+1;
      CPLib.PutProperty(p_CODATECO,"",w_CODATECO,trsrow);
      CPLib.PutProperty(p_opk_CODATECO,"",opk_CODATECO,trsrow);
    ...
    trs_p.setProperty("CODATECO",CPLib.ToProperties(p_CODATECO));
    trs_p.setProperty("opk_CODATECO",CPLib.ToProperties(p_opk_CODATECO));
  ...
  public static String PrimaryKey(Caller p_Caller) {
    return "tbsetsina"+"\\"+CPLib.ToCPStr(p_Caller.GetString("SETSINT","C",3,0))+"\\"+CPLib.ToCPStr(p_Caller.GetNumber("IDSETSIN","N",5,0));
  }
  public String RowPrimaryKey() {
    return "tbsetsina"+"\\"+CPLib.ToCPStr(w_CODATECO);
  }
  ...
  protected void TrsToWork() {
    w_CODATECO= m_oTrs.get(m_nCurrentRow).w_CODATECO;
    opk_CODATECO= m_oTrs.get(m_nCurrentRow).opk_CODATECO;
  ...
  void WorkToTrs() {
    m_oTrs.get(m_nCurrentRow).w_CODATECO=w_CODATECO;
    m_oTrs.get(m_nCurrentRow).opk_CODATECO=opk_CODATECO;
  ...
  public void AddTrsDeletedRow() {
    ...
    m_oDeletedRows.get(l_nRows).w_CODATECO=w_CODATECO;
    m_oDeletedRows.get(l_nRows).opk_CODATECO=opk_CODATECO;
  ...
  void SetDeletedRow(double p_nRowNumberToSet) {
    ...
    w_CODATECO= m_oDeletedRows.get(l_RowNumberToSet).w_CODATECO;
    opk_CODATECO= m_oDeletedRows.get(l_RowNumberToSet).opk_CODATECO;
