...
public class arrt_estrai_sv_check_all extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*19cd530a*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.pTri","pTri");
      }        
      pTri = source.GetParameter("pTri",0);
      if (source instanceof SPRequestSource) {
      ...
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.pTipoBase3","pTipoBase3");
      }        
      pTipoBase3 = source.GetParameter("pTipoBase3","");
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.pFraudo","pFraudo");
      }        
      pFraudo = source.GetParameter("pFraudo","");
    }
  }
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "arrt_estrai_sv_check_all";
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_estrai_sv_check_allR Rcalled;
    Rcalled = new arrt_estrai_sv_check_allR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.aaaaaa_retval = Rcalled.Run(status.pAnno,status.pTri,status.pTipo,status.pFile,status.pTipoBase3,status.pFraudo);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_estrai_sv_check_allR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_estrai_sv_check_allR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
      status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_estrai_sv_check_all"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
