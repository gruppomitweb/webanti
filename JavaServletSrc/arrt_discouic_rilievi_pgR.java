...
// * --- Fine Area Manuale
public class arrt_discouic_rilievi_pgR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_discouic_rilievi_pgR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_discouic_rilievi_pg",m_Caller);
  ...
  public String GetString(String p_cVarName,String p_cType,int len,int dec) {
    if (CPLib.eqr("m_cVQRList",p_cVarName)) {
      ...
      return "arrt_discouic_rilievi_pg";
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      /* w_TipoOper Char(2) // Tipo di Operazione */
      ...
      while ( ! (Cursor_intermbo.Eof())) {
        ...
        if ( ! (Read_Cursor.Eof())) {
          ...
          Read_Cursor.Next();
          if ( ! (Read_Cursor.Eof())) {
            Error l_oErrorFault = new Error("Read on anadip into routine arrt_discouic_rilievi_pg returns two or more records. This item should return only a record.");
      ...
      if (CPLib.eqr(_contaop,0)) {
        ...
        m_cSql = CPLib.GetSQLPrefixComment("arrt_discouic_rilievi_pg",136,"00000083")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
  ...
  void Page_2() throws Exception {
    ...
    try {
      ...
      while ( ! (Cursor_qbe_ope_uif_ril.Eof())) {
        ...
        /* _conta := 0 */
        _conta = CPLib.Round(0,0);
        /* System.out.println(qbe_ope_uif_ril->SETTSINT) */
        System.out.println(Cursor_qbe_ope_uif_ril.GetString("SETTSINT"));
        /* System.out.println(qbe_ope_uif_ril->CABCLI) */
        System.out.println(Cursor_qbe_ope_uif_ril.GetString("CABCLI"));
        /* System.out.println(qbe_ope_uif_ril->PAESECLI) */
        System.out.println(Cursor_qbe_ope_uif_ril.GetString("PAESECLI"));
        /* If LRTrim(qbe_ope_uif_ril->SETTSINT)=LRTrim(w_rsetsin) and LRTrim(qbe_ope_uif_ril->CABCLI)=LRTrim(w_rcabcli) and LRTrim(qbe_ope_uif_ril->PAESECLI)=LRTrim(w_rpaecli) */
        if (CPLib.eqr(CPLib.LRTrim(Cursor_qbe_ope_uif_ril.GetString("SETTSINT")),CPLib.LRTrim(w_rsetsin)) && CPLib.eqr(CPLib.LRTrim(Cursor_qbe_ope_uif_ril.GetString("CABCLI")),CPLib.LRTrim(w_rcabcli)) && CPLib.eqr(CPLib.LRTrim(Cursor_qbe_ope_uif_ril.GetString("PAESECLI")),CPLib.LRTrim(w_rpaecli))) {
          /* _conta := _conta + 1 */
        ...
        if (CPLib.eqr(Cursor_qbe_ope_uif_ril.GetString("TIPO_OPE"),"OPE") && _clientiope) {
          ...
          while ( ! (Cursor_clientiope.Eof())) {
            ...
            while ( ! (Cursor_personbo.Eof())) {
              /* If LRTrim(personbo->SETTSINT)=LRTrim(w_rsetsin) and LRTrim(personbo->CODCAB)=LRTrim(w_rcabcli) and LRTrim(personbo->PAESE)=LRTrim(w_rpaecli) */
              if (CPLib.eqr(CPLib.LRTrim(Cursor_personbo.GetString("SETTSINT")),CPLib.LRTrim(w_rsetsin)) && CPLib.eqr(CPLib.LRTrim(Cursor_personbo.GetString("CODCAB")),CPLib.LRTrim(w_rcabcli)) && CPLib.eqr(CPLib.LRTrim(Cursor_personbo.GetString("PAESE")),CPLib.LRTrim(w_rpaecli))) {
                /* _conta := _conta + 1 */
                _conta = CPLib.Round(_conta+1,0);
              } // End If
        ...
        if (CPLib.gt(_conta,0)) {
          ...
          m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_stpoperazioni",m_Ctx.GetCompany());
          m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
          m_cSql = CPLib.GetSQLPrefixComment("arrt_discouic_rilievi_pg",136,"000000A1")+CPLib.InsertIntoStatement(m_Ctx,m_oWrInfo)+" (";
          ...
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_ope_uif_ril.GetDouble("TOTCONT")/100,"?",0,0)+", ";
          m_cSql = m_cSql+""+CPLib.ToSQL(Cursor_qbe_ope_uif_ril.GetDouble("TOTLIRE")/100,"?",0,0)+", ";
  ...
  public String RunAsync(java.sql.Date p_w_DaDatOpe,java.sql.Date p_w_ADatOpe,String p_w_rcoddip,String p_w_rabicab,String p_w_rcabdip,String p_w_rpaecli,String p_w_rcabcli,String p_w_rcodvoc,String p_w_rflglire,String p_w_rsegno,String p_w_rsetsin,String p_w_rpaecon,String p_w_rcabcon,String p_w_rpaeint,String p_w_rcabint,String p_w_TipoOper) {
    w_DaDatOpe = p_w_DaDatOpe;
    w_ADatOpe = p_w_ADatOpe;
    w_rcoddip = p_w_rcoddip;
    w_rabicab = p_w_rabicab;
    w_rcabdip = p_w_rcabdip;
    w_rpaecli = p_w_rpaecli;
    w_rcabcli = p_w_rcabcli;
    w_rcodvoc = p_w_rcodvoc;
    w_rflglire = p_w_rflglire;
    w_rsegno = p_w_rsegno;
    w_rsetsin = p_w_rsetsin;
    w_rpaecon = p_w_rpaecon;
    w_rcabcon = p_w_rcabcon;
    w_rpaeint = p_w_rpaeint;
    w_rcabint = p_w_rcabint;
    w_TipoOper = p_w_TipoOper;
  ...
  public Forward Run(java.sql.Date p_w_DaDatOpe,java.sql.Date p_w_ADatOpe,String p_w_rcoddip,String p_w_rabicab,String p_w_rcabdip,String p_w_rpaecli,String p_w_rcabcli,String p_w_rcodvoc,String p_w_rflglire,String p_w_rsegno,String p_w_rsetsin,String p_w_rpaecon,String p_w_rcabcon,String p_w_rpaeint,String p_w_rcabint,String p_w_TipoOper) {
    w_DaDatOpe = p_w_DaDatOpe;
    w_ADatOpe = p_w_ADatOpe;
    w_rcoddip = p_w_rcoddip;
    w_rabicab = p_w_rabicab;
  ...
  public static arrt_discouic_rilievi_pgR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_discouic_rilievi_pgR(p_Ctx, p_Caller);
  }
  public void Blank() {
    w_DaDatOpe = CPLib.NullDate();
    w_ADatOpe = CPLib.NullDate();
    w_rcoddip = CPLib.Space(6);
    w_rabicab = CPLib.Space(10);
    w_rcabdip = CPLib.Space(6);
    w_rpaecli = CPLib.Space(3);
    w_rcabcli = CPLib.Space(6);
    w_rcodvoc = CPLib.Space(2);
    w_rflglire = CPLib.Space(1);
    w_rsegno = CPLib.Space(1);
    w_rsetsin = CPLib.Space(3);
    w_rpaecon = CPLib.Space(3);
    w_rcabcon = CPLib.Space(6);
    w_rpaeint = CPLib.Space(3);
    w_rcabint = CPLib.Space(6);
    w_TipoOper = CPLib.Space(2);
  ...
  public static String[] m_cRunParameterNames={"w_DaDatOpe","w_ADatOpe","w_rcoddip","w_rabicab","w_rcabdip","w_rpaecli","w_rcabcli","w_rcodvoc","w_rflglire","w_rsegno","w_rsetsin","w_rpaecon","w_rcabcon","w_rpaeint","w_rcabint","w_TipoOper"};
