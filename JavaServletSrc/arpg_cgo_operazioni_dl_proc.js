...
function Help() {
windowOpenForeground('../doc/arpg_cgo_operazioni_dl_usr.xml','','toolbar=0,menubar=0,directories=0,width=500,height=450,resizable=1');
}
...
p_d.setValue('arpg_cgo_operazioni_dl',m_cSelectedPage);
...
l_oWv.setValue('datope',WtA(w_datope,'D'));
l_oWv.setValue('gCodDip',WtA(w_gCodDip,'C'));
l_oWv.setValue('totope',WtA(w_totope,'N'));
l_oWv.setValue('tipope',WtA(w_tipope,'C'));
_FillChildren(l_oWv);
}
_FillChildren.n=[];
FillWv.n=["datope","gCodDip","totope","tipope"];
...
if (start) {
if (Empty(w_datope) && Ne(opener.w_datope,null)) {
TransferBetweenWindows(opener,'w_datope','w_datope');
}
if (Empty(w_totope) && Ne(opener.w_totope,null)) {
TransferBetweenWindows(opener,'w_totope','w_totope');
}
if (Empty(w_tipope) && Ne(opener.w_tipope,null)) {
TransferBetweenWindows(opener,'w_tipope','w_tipope');
}
}
}
m_bCalculating=true;
...
function TrsQueryRow() {
  ...
  this.c_CODFISC = arguments[1]
  this.c_COGNOME = arguments[2]
  this.c_NOME = arguments[3]
  this.c_CAIDBIGLIETTO = arguments[4]
  this.c_DATAOPE = arguments[5]
...
GroupFooter_BeginHtml(document,1,'877204110'+l_nIdx+'','A',true,false)
GroupFooter_EndHtml(document,1,'877204110'+l_nIdx+'','A')
} else {
}
l_nIdx=l_nIdx+1;
m_nCurrentRow=l_nIdx;
...
if (Ge(2,l_nDeep)) {
GroupHeader_BeginHtml(document,1,'1414075022'+l_nIdx+'','A',true,false)
document.writeln(
 "<div id='DIV_0BB397F8"+l_nIdx+"' style='position:relative;height:17px'>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
GroupHeader_EndHtml(document,1,'1414075022'+l_nIdx+'','A')
} else {
}
Row_BeginHtml(document,l_nEvenOdd,'10196538593'+l_nIdx+'','A',true,-1)
document.writeln(
 "<div id='DIV_0B996928"+l_nIdx+"' style='position:relative;height:19px'>" +
"")
document.writeln(
...
document.writeln(
 "</div>" +
"")
Row_EndHtml(document,l_nEvenOdd,'10196538593'+l_nIdx+'','A')
Parms.Idx=l_nIdx;
Parms.Deep=l_nDeep;
Parms.EvenOdd=l_nEvenOdd;
}
