...
// * --- Fine Area Manuale
public class ardt_ae_saldianno_rapBO extends ardt_ae_saldianno_rapBL {
  ...
  // * --- Fine Area Manuale
  public ardt_ae_saldianno_rapBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"ardt_ae_saldianno_rap");
    }
  ...
  public boolean Load(String p_RAPPORTO) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_aeannora+".ANNO";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_aeannora,m_cPhName_aeannora,m_cVirtName_aeannora,m_cVirtName_aeannora+".RAPPORTO="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_RAPPORTO,"C",25,0),m_cServer_aeannora,p_RAPPORTO),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_ANNO = l_rsLoadBody.GetDouble("ANNO");
          w_IDBASE = l_rsLoadBody.GetString("IDBASE");
          w_IMPORTO1 = l_rsLoadBody.GetDouble("IMPORTO1");
          w_IMPORTO2 = l_rsLoadBody.GetDouble("IMPORTO2");
          w_IMPORTO3 = l_rsLoadBody.GetDouble("IMPORTO3");
          w_IMPORTO4 = l_rsLoadBody.GetDouble("IMPORTO4");
          w_IMPORTO5 = l_rsLoadBody.GetDouble("IMPORTO5");
          w_FILEANNO = l_rsLoadBody.GetString("FILEANNO");
          w_PROG1 = l_rsLoadBody.GetString("PROG1");
          w_IMPORTO6 = l_rsLoadBody.GetDouble("IMPORTO6");
          w_NATURA = l_rsLoadBody.GetString("NATURA");
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_aeannora.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND ANNO="+CPLib.ToSQL(opk_ANNO,"N",4,0);
        l_cSql = l_cSql+m_oWrInfo_aeannora.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"aeannora")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_aeannora,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_aeannora);
      l_cSql = "insert into "+m_oWrInfo_aeannora.GetTableWriteName()+" (";
      l_cSql = l_cSql+"RAPPORTO,";
      l_cSql = l_cSql+"TIPRAP,";
      l_cSql = l_cSql+"ANNO,";
      l_cSql = l_cSql+"IDBASE,";
      l_cSql = l_cSql+"IMPORTO1,";
      l_cSql = l_cSql+"IMPORTO2,";
      l_cSql = l_cSql+"IMPORTO3,";
      l_cSql = l_cSql+"IMPORTO4,";
      l_cSql = l_cSql+"IMPORTO5,";
      l_cSql = l_cSql+"FILEANNO,";
      l_cSql = l_cSql+"PROG1,";
      l_cSql = l_cSql+"IMPORTO6,";
      l_cSql = l_cSql+"NATURA,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_aeannora.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"aeannora")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_TIPRAP,"C",2,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_ANNO,"N",4,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IDBASE,"C",10,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IMPORTO1,"N",17,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IMPORTO2,"N",17,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IMPORTO3,"N",17,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IMPORTO4,"N",17,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IMPORTO5,"N",6,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_FILEANNO,"C",50,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_PROG1,"C",10,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_IMPORTO6,"N",17,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_NATURA,"C",2,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_aeannora.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"aeannora")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_aeannora,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_aeannora);
      l_cSql = "Update "+m_oWrInfo_aeannora.GetTableWriteName()+" set";
      l_cSql = l_cSql+" TIPRAP = "+CPLib.ToSQL(w_TIPRAP,"C",2,0);
      l_cSql = l_cSql+", IDBASE = "+CPLib.ToSQL(w_IDBASE,"C",10,0);
      l_cSql = l_cSql+", IMPORTO1 = "+CPLib.ToSQL(w_IMPORTO1,"N",17,0);
      l_cSql = l_cSql+", IMPORTO2 = "+CPLib.ToSQL(w_IMPORTO2,"N",17,0);
      l_cSql = l_cSql+", IMPORTO3 = "+CPLib.ToSQL(w_IMPORTO3,"N",17,0);
      l_cSql = l_cSql+", IMPORTO4 = "+CPLib.ToSQL(w_IMPORTO4,"N",17,0);
      l_cSql = l_cSql+", IMPORTO5 = "+CPLib.ToSQL(w_IMPORTO5,"N",6,0);
      l_cSql = l_cSql+", FILEANNO = "+CPLib.ToSQL(w_FILEANNO,"C",50,0);
      l_cSql = l_cSql+", PROG1 = "+CPLib.ToSQL(w_PROG1,"C",10,0);
      l_cSql = l_cSql+", IMPORTO6 = "+CPLib.ToSQL(w_IMPORTO6,"N",17,0);
      l_cSql = l_cSql+", NATURA = "+CPLib.ToSQL(w_NATURA,"C",2,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"aeannora")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", ANNO = "+CPLib.ToSQL(w_ANNO,"N",4,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"RAPPORTO="+CPLib.ToSQL(w_RAPPORTO,"C",25,0)+" AND ANNO="+CPLib.ToSQL(opk_ANNO,"N",4,0);
      l_cSql = l_cSql+m_oWrInfo_aeannora.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"aeannora")) {
  ...
  // * --- Fine Area Manuale
  // ENTITY_BATCHES: ,arfn_desc_saldi,
  public static final String i_InvokedRoutines = ",arfn_desc_saldi,";
}
