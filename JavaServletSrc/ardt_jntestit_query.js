...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('QPLXZEEVEI').value,'C'),w_CODCLI)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('jntestit',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'CODCLI') &&  ! (Empty(HtW(Ctrl('QPLXZEEVEI').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.CODCLI',HtW(Ctrl('QPLXZEEVEI').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'detail','ardt_jntestit');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('jntestit',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
