...
// * --- Fine Area Manuale
public class arfn_likoper_saveR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arfn_likoper_saveR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arfn_likoper_save",m_Caller);
    ...
    if (CPLib.ne(oValues.RecCount(),0)) {
      ...
      while (CPLib.le(_riga,oValues.RecCount())) {
        ...
        if (CPLib.eqr(oValues.row.ps_rowstatus,"S")) {
          ...
          m_cSql = CPLib.GetSQLPrefixComment("arfn_likoper_save",60,"0000000E")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      ...
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"operazbo",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arfn_likoper_save",60,"00000011")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      m_cSql = m_cSql+"COLLEG = "+CPLib.ToSQL(pColleg,"C",12,0,m_cServer)+", ";
      m_cSql = m_cSql+"FLAGFRAZ = "+CPLib.ToSQL("2","C",1,0,m_cServer)+", ";
      m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"operazbo",false,true);
      m_cSql = m_cSql+" where ";
  ...
  public static arfn_likoper_saveR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arfn_likoper_saveR(p_Ctx, p_Caller);
  }
  public static arfn_likoper_saveR Make(CPContext p_Ctx) {
    return new arfn_likoper_saveR(p_Ctx, CPSecurity.EXECUTE_ALL_ROUTINES);
  }
  public void Blank() {
  ...
  // ENTITY_BATCHES: ,arfn_likoper_save,
  public static final String i_InvokedRoutines = ",arfn_likoper_save,";
