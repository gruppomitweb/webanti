...
public class aosdt_anom_ril extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*975b2e95*!";
  static public class ServletStatus extends SPDetailChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "anom_ril";
    }
    ...
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_IDMOD,context,"anom_ril","IDMOD","C",10,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_ROWRIL,context,"anom_ril","ROWRIL","N",4,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_CONNES,context,"anom_ril","CONNES","C",16,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    status.m_cVirtName = BO.m_cVirtName_anom_ril;
  ...
  public SPEntityGlobals getEntityGlobals() {
    ...
    l_eg.name = "aosdt_anom_ril";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 1733;
    l_eg.m_nEntityHeight = 134;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"anom_ril","ind_def","sub_ind_def"};
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","","page")};
    l_eg.securedBoxes = new String[][] {};
    l_eg.securedTabs = new String[][] {};
    l_eg.keyColumns = new String[]{"IDMOD","ROWRIL","CONNES"};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("CONNES","C",16,0, "hide","NDG Soggetto"),
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,aosdt_anom_rilBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11005667081",p_Context.Translate("Descrizione del indicatore: %1")},
    ...
    {"2113886253",p_Context.Translate("Indicatore di anomalia non corretto")},
    {"389397309",p_Context.Translate("Descrizione del subindicatore: %1")},
  ...
  void WriteStyles(aosdt_anom_rilBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../aosdt_anom_ril.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(aosdt_anom_rilBO BO,ServletStatus status) {
    status.out.println(
    ...
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
      ...
      status.out.println(
       "<style>" +
      "");
      status.out.println(".PWBPXBGJRH {top:0px;position:absolute;left:0px;height:18px;width:83px;background-color:Transparent;z-index:1; }");
      status.out.println(".PWBPXBGJRH_ZOOM { position:absolute;top:0px;left:84px;height:19px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".OSQTGJONIV {top:0px;position:absolute;left:601px;height:18px;width:83px;background-color:Transparent;z-index:1; }");
      status.out.println(".OSQTGJONIV_ZOOM { position:absolute;top:0px;left:685px;height:19px;width:17px;background-color:Transparent;background-repeat:no-repeat;background-position:center;margin:0;border-width:0;padding:0;z-index:1;}");
      status.out.println(".GIWDXVTDUW {top:0px;position:absolute;left:1206px;height:87px;width:406px;overflow:auto;background-color:Transparent;z-index:1; }");
      status.out.println(".GOHQRSLEQN {top:0px;position:absolute;left:106px;height:18px;width:83px;background-color:Transparent;z-index:1; }");
      status.out.println(".AOKURBYNFX {top:0px;position:absolute;left:195px;height:87px;width:400px;overflow:auto;background-color:Transparent;z-index:1; }");
      status.out.println(".NXPQVKMXKS {top:0px;position:absolute;left:704px;height:18px;width:83px;background-color:Transparent;z-index:1; }");
      status.out.println(".PSHCVHVAPS {top:0px;position:absolute;left:791px;height:87px;width:400px;overflow:auto;background-color:Transparent;z-index:1; }");
      status.out.println(
       "</style>" +
      "");
    } else {
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('aosdt_anom_ril',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"aosdt_anom_ril",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(aosdt_anom_rilBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(aosdt_anom_rilBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:134px;width:1733px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(aosdt_anom_rilBO BO,ServletStatus status) {
    if (status.IsHiddenZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      return;
    }
    status.out.println(
     "<div id='shadowBand_TOP_OF_PAGE_1' style='position:relative;height:24px;display:block' page='1'>" +
    "");
    if (status.includeZoneAltInterface("shadowBand_TOP_OF_PAGE_1")) {
      ...
      status.out.println(
       "<div name id='GridTable_Header' class='GridTable_Header' style='position:absolute;top:1px;left:3px;height:22px;width:1634px;z-index:0'>" +
       "</div>" +
      "");
  ...
  void WriteControls_p1_b1(aosdt_anom_rilBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (status.includeZoneAltInterface("GridTable")) {
      ...
      if (status.HasZoneAltInterface("GridTable@BEGIN") || status.HasZoneAltInterface("GridTable@END")) {
        status.out.println(
         "<div id='GridTable_Band_boxContent' style='position:relative;height:457px;display:block'>" +
        "");
      }
      status.m_nTabIndex = status.m_nTabIndex+(1);
      status.out.println(
       "<input name='ROWRIL' id='KHBDKLNADV' type='text' class='Field PrimaryKeyField "+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"")+"' value="+SPLib.ToHTMLValue(BO.w_ROWRIL,"N",4,0,"9999", status.m_oRegSettings)+" title="+SPLib.ToHTMLValue(status.context.Translate("Progressivo rilevazione"),"C",0,0)+" tabindex='"+(CPLib.eq(status.m_cClientMode,"query")?""+status.m_nTabIndex+"":"-1")+"' curTabIndex='"+status.m_nTabIndex+"' "+(CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"view")?"readonly":"")+" maxlength='7' size='4' fieldtype='N'>" +
      "");
      status.out.println(
       "<div id='GridTable_Container' style='position:relative;top:0px;left:3px;min-height:450px;max-height:906px;width:1634px;overflow:hidden' class='GridTable_Container "+SPLib.ToHTML(status.m_cClientMode,"C",0,0)+"'>" +
       "<div id=GridTable_LockedScroll style='position:absolute;top:0px;left:0px;height:456px;width:1634px;overflow:scroll'>" +
       "</div>" +
       "</div>" +
      "");
      status.out.println(
       "<div id='GridTable_DIV' style='position:relative;left:0;top:0;width:1634px;min-height:450px;max-height:906px;overflow:auto;'>" +
       "<table id='GridTable' style='position:relative;top:0px;left:0px;width:1628px' cellspacing='0' cellpadding='0' border='0'>" +
       "<tr>" +
       "<td></td>" +
       "</tr>" +
       "</table>" +
      ...
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:108px;height:100%;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:197px;height:100%;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:603px;height:100%;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:708px;height:100%;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:794px;height:100%;width:1px'>" +
       "</div>" +
      "");
      status.out.println(
       "<div class='GridTable_VerticalLines' style='position:absolute;top:0px;left:1203px;height:100%;width:1px'>" +
       "</div>" +
      "");
      HTMLRowScript(BO,status);
      if (status.HasZoneAltInterface("GridTable@BEGIN") || status.HasZoneAltInterface("GridTable@END")) {
  ...
  void WriteControls_p1_b1_CloseBand(aosdt_anom_rilBO BO,ServletStatus status) {
    status.out.println(
     "</div>" +
    "");
    status.out.println(
  ...
  void HTMLRowScript(aosdt_anom_rilBO BO,ServletStatus status) {
    ...
    status.out.println("s='<div style=\"position:relative;width:1612px;height:89px;\"';");
    ...
    status.out.println("s=s+\""+
     ...
     "<button id='PWBPXBGJRH_ZOOM' type='button' class='GridZoomButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
     "</button>" +
    "\"");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
     ...
     "<button id='OSQTGJONIV_ZOOM' type='button' class='GridZoomButton"+(CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")?"Disabled":"Enabled")+"' tabindex='-1' "+(CPLib.eq(status.m_cClientMode,"query")?"disabled":"")+">" +
     "</button>" +
    "\"");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    status.out.println("s=s+\""+
    ...
    "\"");
    status.out.println("s=s+\""+
     "<textarea name='DESCRI_IND' id='AOKURBYNFX' class='Memo Disabled GridTable_EditRowItem ' title='\"+Strtran(AlertErrorMessage('11005667081',w_DESCRI_IND),'\\\\n','&#10;')+\"' tabindex='-1' readonly cols='20' rows='2'>" +
     "</textarea>" +
    "\"");
    status.out.println("s=s+\""+
    ...
    "\"");
    status.out.println("s=s+\""+
     "<textarea name='DESCRI_SUBIND' id='PSHCVHVAPS' class='Memo Disabled GridTable_EditRowItem ' title='\"+Strtran(AlertErrorMessage('389397309',w_DESCRI_SUBIND),'\\\\n','&#10;')+\"' tabindex='-1' readonly cols='20' rows='2'>" +
     "</textarea>" +
    "\"");
    status.m_nTabIndex = status.m_nTabIndex+(1);
    ...
    status.out.println("s=s+\""+
     ...
     "<button type='button' class='PWBPXBGJRH_ZOOM GridZoomButtonEnabled' value="+SPLib.ToHTMLValue(status.context.Translate("..."),"C",0,0)+" disabled class='PWBPXBGJRH_ZOOM GridZoomButtonEnabled'>" +
     "</button>" +
    "\"");
    status.out.println("}");
    ...
    status.out.println("s=s+\""+
     ...
     "<button type='button' class='OSQTGJONIV_ZOOM GridZoomButtonEnabled' value="+SPLib.ToHTMLValue(status.context.Translate("..."),"C",0,0)+" disabled class='OSQTGJONIV_ZOOM GridZoomButtonEnabled'>" +
     "</button>" +
    "\"");
    status.out.println("}");
    ...
    "\"");
    status.out.println("}");
    status.out.println("s=s+'<div style=\" position:absolute;z-index:2;width:1612px;height:89px;\"></div>';");
    status.out.println("}");
  ...
  void WriteControls_p1_b2(aosdt_anom_rilBO BO,ServletStatus status) {
  ...
  static void PrintState(aosdt_anom_rilBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(aosdt_anom_rilBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(aosdt_anom_rilBL BO,ServletStatus status) {
  ...
  static void PrintStateDetailWorkVars(aosdt_anom_rilBL BO,ServletStatus status) {
    int k;
    int l_nSelectRow = -1;
  ...
  static void PrintStateDetailWorkVars_Deleted(aosdt_anom_rilBL BO,ServletStatus status) {
    String l_cTrs;
    int k;
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      aosdt_anom_rilBO.TrsRow l_Row;
      l_cTrs = "[";
      k = 0;
      while (CPLib.lt(k,BO.m_oDeletedRows.size())) {
        l_Row= (aosdt_anom_rilBO.TrsRow) BO.m_oDeletedRows.get(k);
  ...
  void WriteMainFormVarScript(aosdt_anom_rilBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(134,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(1733,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"aosdt_anom_ril"),"C",0,0)+ ";");
    ...
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
      status.out.print("'PWBPXBGJRH':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash(entityGlobals.name+"PWBPXBGJRH",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,ind_def,"+"default",l_session)+"'"+"]");
      status.out.print(",'OSQTGJONIV':['"+com.zucchetti.PortalStudio.JSPLib.cmdHash(entityGlobals.name+"OSQTGJONIV",l_session)+"','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,sub_ind_def,"+"default",l_session)+"'"+"]");
      AppendCallableIDS(status,",");
    } else {
      status.out.print("'PWBPXBGJRH':['','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,ind_def,"+"default",l_session)+"'"+"]");
      status.out.print(",'OSQTGJONIV':['','"+com.zucchetti.PortalStudio.JSPLib.cmdHash("link,sub_ind_def,"+"default",l_session)+"'"+"]");
      AppendCallableIDS(status,",");
    }
    status.out.println("}");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("aosdt_anom_ril","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"aosdt_anom_ril",BO.GetCurrentState());
    ...
    status.out.println("}");
    status.out.println("_GridRowHeight='89';");
  ...
  void WriteMainFormScript(aosdt_anom_rilBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_aosdt_anom_ril);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_aosdt_anom_ril();");
      status.out.println("}");
    }
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('aosdt_anom_ril',Ne(m_nChildStatus,0));");
    } else {
    ...
    if (CPLib.eq(status.m_cClientMode,"query") && CPLib.gt(status.m_nChildStatus,0)) {
      ...
      status.out.println("SetDisabled('NBMHJRJGQX',m_bRowsChild);");
      status.out.println("SetDisabled('KHBDKLNADV',m_bRowsChild);");
    }
    status.out.println("}");
    if ( ! (CPLib.Empty(status.m_cAltInterface))) {
    ...
    status.out.println("m_oFather.m_oaosdt_anom_ril=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  aosdt_anom_rilBO getBO(ServletStatus status) {
    aosdt_anom_rilBO BO = new aosdt_anom_rilBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(aosdt_anom_rilBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("anom_ril",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,aosdt_anom_rilBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,aosdt_anom_rilBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(aosdt_anom_rilBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_anom_ril;
    String l_cPhName = BO.m_cPhName_anom_ril;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = aosdt_anom_rilBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_anom_ril+".IDMOD, "+BO.m_cVirtName_anom_ril+".ROWRIL, "+BO.m_cVirtName_anom_ril+".CONNES"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," distinct  "+BO.m_cVirtName_anom_ril+".IDMOD, "+BO.m_cVirtName_anom_ril+".ROWRIL, "+BO.m_cVirtName_anom_ril+".CONNES"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("anom_ril");
    String l_cPhName = CPSql.ManipulateTablePhName("anom_ril",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+l_cPhName+".IDMOD, "+l_cPhName+".ROWRIL, "+l_cPhName+".CONNES"+(CPLib.Empty(status.m_cLastOrderBy) || CPLib.gt(CPLib.At(status.m_cLastOrderBy," "+l_cPhName+".IDMOD, "+l_cPhName+".ROWRIL, "+l_cPhName+".CONNES"),0) || CPLib.ne(CPSql.GetDataBaseType(l_cServer),CPSql.m_nPostgres)?"":","+status.m_cLastOrderBy)+"",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    }
    return l_rsKeys;
  }
  void Query(ServletStatus status,aosdt_anom_rilBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (CPLib.eq(CPSql.GetDataBaseType(BO.m_cServer_anom_ril),CPSql.m_nInformix)) {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_anom_ril,CPSql.BuildSQLPhrase(" count( distinct  "+BO.m_cVirtName_anom_ril+".IDMOD, "+BO.m_cVirtName_anom_ril+".ROWRIL, "+BO.m_cVirtName_anom_ril+".CONNES) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_anom_ril,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      } else {
        if (l_rsCount!=null)
          l_rsCount.Close();
        l_rsCount = BO.m_Sql.Query(BO.m_cServer_anom_ril,CPSql.BuildSQLPhrase(" count(*) as reccount","("+CPSql.BuildSQLPhrase(" distinct  "+BO.m_cVirtName_anom_ril+".IDMOD, "+BO.m_cVirtName_anom_ril+".ROWRIL, "+BO.m_cVirtName_anom_ril+".CONNES",CPLib.GetTablesWithAliases(BO.m_cPhName_anom_ril,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,"")+") CountFrom","",""));
      }
      try {
  ...
  void View(ServletStatus status,aosdt_anom_rilBO BO) {
  ...
  void Edit(ServletStatus status,aosdt_anom_rilBO BO,SPParameterSource source) {
    BO.Load(status.w_IDMOD,status.w_ROWRIL,status.w_CONNES);
  ...
  void New(ServletStatus status,aosdt_anom_rilBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,aosdt_anom_rilBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,aosdt_anom_rilBO BO) {
  ...
  void EditLoad(ServletStatus status,aosdt_anom_rilBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,aosdt_anom_rilBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    ...
    if (HasAllKeys(status,source)) {
      ...
      l_bResult = BO.Delete(status.w_IDMOD,status.w_ROWRIL,status.w_CONNES);
      l_cTrsMsgErr = BO.m_Sql.TransactionErrorMessage();
      l_bTrsOk = BO.m_Sql.EndTransaction();
  ...
  void DeleteMessage(ServletStatus status,aosdt_anom_rilBO BO,SPParameterSource source) {
    BO.Load(status.w_IDMOD,status.w_ROWRIL,status.w_CONNES);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,aosdt_anom_rilBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
    ...
    if (l_bTrsOk && l_bResult) {
      status.m_bExit = true;
      status.m_cOldKeysToRaiseParms = SPLib.ToJSValue("({IDMOD:"+SPLib.ToJSValue(BO.w_IDMOD,"C",10,0)+",ROWRIL:"+SPLib.ToJSValue(BO.w_ROWRIL,"N",4,0)+",CONNES:"+SPLib.ToJSValue(BO.w_CONNES,"C",16,0)+",action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0);
  ...
  void Discard(ServletStatus status,aosdt_anom_rilBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,aosdt_anom_rilBO BO) {
    ...
    item.put("altInterfaceFor","aosdt_anom_ril");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"comment\":\"ID del modello\",\"h\":22,\"maxlength\":10,\"name\":\"IDMOD\",\"page\":1,\"spuid\":\"NBMHJRJGQX\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":83,\"x\":1650,\"y\":14,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Progressivo rilevazione\",\"h\":22,\"maxlength\":7,\"name\":\"ROWRIL\",\"page\":1,\"spuid\":\"KHBDKLNADV\",\"tabindex\":2,\"type\":\"Textbox\",\"typevar\":\"numeric\",\"w\":83,\"x\":1650,\"y\":37,\"zone\":\"pag1_2\",\"zonepath\":\"[1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":18,\"name\":\"label_UALKXPJWBM\",\"page\":1,\"spuid\":\"UALKXPJWBM\",\"tabindex\":3,\"type\":\"Label\",\"value\":\"ID del indicatore\",\"w\":598,\"x\":5,\"y\":2,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":18,\"name\":\"label_OKVVWUCLCV\",\"page\":1,\"spuid\":\"OKVVWUCLCV\",\"tabindex\":4,\"type\":\"Label\",\"value\":\"ID del subindicatore \",\"w\":598,\"x\":606,\"y\":2,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":18,\"name\":\"label_RMCEGUPVLI\",\"page\":1,\"spuid\":\"RMCEGUPVLI\",\"tabindex\":5,\"type\":\"Label\",\"value\":\"Nota della rilevazione\",\"w\":406,\"x\":1211,\"y\":2,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"autoexpand\":true,\"h\":452,\"name\":\"BodyDetail0\",\"page\":1,\"spuid\":\"GridTable_Band\",\"type\":\"BodyDetail\",\"w\":1668,\"x\":5,\"y\":24,\"zone\":\"pag1_3\",\"zonepath\":\"[1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"__body__grid__\",\"box_type\":\"layer\",\"h\":113,\"name\":\"Body_Row\",\"page\":1,\"shrinkable\":true,\"spuid\":\"GridTable_Band\",\"type\":\"Box\",\"w\":1612,\"x\":5,\"y\":1,\"zone\":\"pag1_4\",\"zonepath\":\"[1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"ID del subindicatore \",\"h\":19,\"maxlength\":10,\"name\":\"IDSUBIND\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"OSQTGJONIV\",\"tabindex\":2,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":83,\"x\":606,\"y\":26,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Nota della rilevazione\",\"h\":88,\"name\":\"NOTA\",\"page\":1,\"repeated\":true,\"scroll\":true,\"spuid\":\"GIWDXVTDUW\",\"tabindex\":3,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":406,\"x\":1211,\"y\":26,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Progressivo indicatore\",\"disabled\":\"true\",\"h\":19,\"maxlength\":10,\"name\":\"PROGIND\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"GOHQRSLEQN\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":83,\"x\":111,\"y\":26,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Descrizione del indicatore\",\"disabled\":\"true\",\"h\":88,\"name\":\"DESCRI_IND\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"scroll\":true,\"spuid\":\"AOKURBYNFX\",\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":400,\"x\":200,\"y\":26,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Progressivo subindicatore\",\"disabled\":\"true\",\"h\":19,\"maxlength\":10,\"name\":\"PROGSUBIND\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"spuid\":\"NXPQVKMXKS\",\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":83,\"x\":709,\"y\":26,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Descrizione del subindicatore\",\"disabled\":\"true\",\"h\":88,\"name\":\"DESCRI_SUBIND\",\"page\":1,\"readonly\":\"true\",\"repeated\":true,\"scroll\":true,\"spuid\":\"PSHCVHVAPS\",\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":400,\"x\":796,\"y\":26,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"check\":\"file\",\"comment\":\"ID del indicatore\",\"h\":19,\"maxlength\":10,\"name\":\"IDIND\",\"page\":1,\"picker\":true,\"repeated\":true,\"spuid\":\"PWBPXBGJRH\",\"tabindex\":1,\"type\":\"Textbox\",\"typevar\":\"character\",\"w\":83,\"x\":5,\"y\":26,\"zone\":\"pag1_4_1\",\"zonepath\":\"[1,4,1]\",\"zoom\":\"stdz\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void DeleteRow(ServletStatus status,aosdt_anom_rilBO BO,SPParameterSource source) {
    int l_nRow = 0;
  ...
  void WriteFormPage(aosdt_anom_rilBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='aosdt_anom_ril?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/aosdt_anom_ril_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/aosdt_anom_ril_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("w_CONNES=a[5];");
      status.out.println("w_PROGIND=a[6];");
      status.out.println("w_DESCRI_IND=a[7];");
      status.out.println("w_PROGSUBIND=a[8];");
      status.out.println("w_DESCRI_SUBIND=a[9];");
      status.out.println("m_PrimaryKeys=['CONNES','IDSUBIND','IDIND','ROWRIL','IDMOD'];");
      status.out.println("}");
      status.out.println(
       "</script>" +
      "");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/aosdt_anom_ril_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(134,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    if (CPLib.eq(status.m_cClientMode,"edit") || CPLib.eq(status.m_cClientMode,"new")) {
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","anom_ril");
    f.SetParameter("Autozoom",entityGlobals.name);
    f.SetParameter("PKFields","IDMOD,ROWRIL,CONNES");
  ...
  void RaiseLoadedEntity(aosdt_anom_rilBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({IDMOD:"+SPLib.ToJSValue(BO.w_IDMOD,"C",10,0)+",ROWRIL:"+SPLib.ToJSValue(BO.w_ROWRIL,"N",4,0)+",CONNES:"+SPLib.ToJSValue(BO.w_CONNES,"C",16,0)+",action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(aosdt_anom_rilBO BO,ServletStatus status) {
    if (CPLib.eq(status.m_cAction,"save") &&  ! (status.m_bError)) {
      status.out.println(
       "<script>" +
      "");
      RaiseEventSavedEntity(status,(status.m_cOldKeysToRaiseParms!=null?status.m_cOldKeysToRaiseParms:SPLib.ToJSValue("({IDMOD:"+SPLib.ToJSValue(BO.w_IDMOD,"C",10,0)+",ROWRIL:"+SPLib.ToJSValue(BO.w_ROWRIL,"N",4,0)+",CONNES:"+SPLib.ToJSValue(BO.w_CONNES,"C",16,0)+",action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0)));
      status.out.println(
       "</script>" +
      "");
    }
  }
  void RaiseDeletedEntity(aosdt_anom_rilBO BO,ServletStatus status) {
    if (CPLib.eq(status.m_cAction,"delete") &&  ! (status.m_bError)) {
      status.out.println(
       "<script>" +
      "");
      RaiseEventDeletedEntity(status,SPLib.ToJSValue("({IDMOD:"+SPLib.ToJSValue(BO.w_IDMOD,"C",10,0)+",ROWRIL:"+SPLib.ToJSValue(BO.w_ROWRIL,"N",4,0)+",CONNES:"+SPLib.ToJSValue(BO.w_CONNES,"C",16,0)+"})","C",0,0));
      status.out.println(
       "</script>" +
      "");
    }
  }
  void RaiseDiscardedEntity(aosdt_anom_rilBO BO,ServletStatus status) {
    if (CPLib.eq(status.m_cAction,"discard")) {
      status.out.println(
       "<script>" +
      "");
      RaiseEventDiscardedEntity(status,SPLib.ToJSValue("({IDMOD:"+SPLib.ToJSValue(BO.w_IDMOD,"C",10,0)+",ROWRIL:"+SPLib.ToJSValue(BO.w_ROWRIL,"N",4,0)+",CONNES:"+SPLib.ToJSValue(BO.w_CONNES,"C",16,0)+",action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0));
      status.out.println(
       "</script>" +
      "");
    }
  }
  void WriteExitPage(aosdt_anom_rilBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({IDMOD:"+SPLib.ToJSValue(BO.w_IDMOD,"C",10,0)+",ROWRIL:"+SPLib.ToJSValue(BO.w_ROWRIL,"N",4,0)+",CONNES:"+SPLib.ToJSValue(BO.w_CONNES,"C",16,0)+",action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(aosdt_anom_rilBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(aosdt_anom_rilBO BO,ServletStatus status) throws IOException {
  ...
  public String GetCheckVars(ServletStatus status,aosdt_anom_rilBO BO,String p_cUID) {
    ...
    if (CPLib.eq(p_cUID,m_cEntityUid)) {
      ...
      if (BO.IsLoaded()) {
        ...
        hCreator.AddValue(CPLib.ToSQL(BO.w_ROWRIL,"N",4,0));
        hCreator.AddValue(CPLib.ToSQL(CPLib.Trim(BO.w_CONNES),"C",16,0));
      }
    } else if (CPLib.eq(p_cUID,"PWBPXBGJRH")) {
      SPLib.FillLinkHash(hCreator,BO.GetLinkProperties(p_cUID),BO);
    } else if (CPLib.eq(p_cUID,"OSQTGJONIV")) {
      SPLib.FillLinkHash(hCreator,BO.GetLinkProperties(p_cUID),BO);
    }
    return GetEntityHash(p_cUID,status,hCreator);
  }
  public boolean CheckVars(ServletStatus status,aosdt_anom_rilBO BO) {
    boolean result = true;
