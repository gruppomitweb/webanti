...
public class armt_cgo_qadv_operazioni_dl extends SPServlet implements SPInvokable {
  public static String m_cEntityUid = "!*d1dd8b4e*!";
  static public class ServletStatus extends SPMasterChildServletStatus {
    ServletStatus() {
      m_cAllVirtNames = "cgo_qadv_operazioni_dl";
    }
    public String w_SNAINUMOPE;
    public void BuildLastWhereFromPK() {
      if (CPLib.Empty(m_cLastWhere)) {
        m_cLastWhere = m_cLastWhere+CPLib.BuildLastWhereFromPK(w_SNAINUMOPE,context,"cgo_qadv_operazioni_dl","SNAINUMOPE","C",15,0,( ! (CPLib.Empty(m_cLastWhere))?" and ":""));
      }
    }
    void FillParameters() {
  ...
  public void doProcess(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
    ...
    armt_cgo_qadv_operazioni_dlBO BO;
    BO = getBO(status);
    BO.m_bSkipCheckOTPSecurity = false;
    status.m_cVirtName = BO.m_cVirtName_cgo_qadv_operazioni_dl;
  ...
  public String getEntityTitle() {
    return "Questionario ADV DL";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "armt_cgo_qadv_operazioni_dl";
    l_eg.userName = getEntityTitle();
    l_eg.m_nEntityWidth = 976;
    l_eg.m_nEntityHeight = 797;
    l_eg.isPublic = false;
    l_eg.canAccess = true;
    l_eg.canInsert = true;
    l_eg.canChange = true;
    l_eg.canDelete = true;
    l_eg.m_cStates = "[]";
    l_eg.zoomableTables = new String[]{"cgo_qadv_operazioni_dl"};
    l_eg.zoneItems = new ZoneItem[]{new ZoneItem("page_1","","page"),new ZoneItem("Questionario","Questionario","horizontal_section")};
    ...
    l_eg.items = new SPItem[]{
                   ...
                   new SPItem("QADV_PUNTEGGIO01","N",3,0, "hide","Punteggio Domanda 1"),
                   new SPItem("QADV_PUNTEGGIO02","N",3,0, "hide","Punteggio Domanda 2"),
                   new SPItem("QADV_PUNTEGGIO03","N",3,0, "hide","Punteggio Domanda 3"),
                   new SPItem("QADV_PUNTEGGIO04","N",3,0, "hide","Punteggio Domanda 4"),
                   new SPItem("QADV_MOTIVO04","M",10,0, "edit","Motivo Risposta 4"),
                   new SPItem("QADV_PUNTEGGIO05","N",3,0, "hide","Punteggio Domanda 5"),
                   new SPItem("QADV_PUNTEGGIO06","N",3,0, "hide","Punteggio Domanda 6"),
                   new SPItem("QADV_PUNTEGGIO07","N",3,0, "hide","Punteggio Domanda 7"),
                   new SPItem("QADV_PUNTEGGIO08","N",3,0, "hide","Punteggio Domanda 8"),
                   new SPItem("QADV_PUNTEGGIO09","N",3,0, "hide","Punteggio Domanda 9"),
                   new SPItem("QADV_PUNTEGGIO10","N",3,0, "hide","Punteggio Domanda 10"),
                   new SPItem("QADV_PUNTEGGIO11","N",3,0, "hide","Punteggio Domanda 11"),
                   new SPItem("QADV_PUNTEGGIO12","N",3,0, "hide","Punteggio Domanda 12"),
                   new SPItem("QADV_FLGCOMPLETO","C",1,0, "edit","Questionario Completo"),
                   new SPItem("QADV_TOTALE","N",3,0, "hide","Punteggio Questionario")
                 };
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,armt_cgo_qadv_operazioni_dlBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"10029143678",p_Context.Translate("Questionario ADV DL")},
    {"10663169693",p_Context.Translate("Comportamento Sospetto")},
  ...
  void WriteStyles(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../armt_cgo_qadv_operazioni_dl.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    status.out.println(
    ...
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      ...
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_cgo_qadv_operazioni_dl',true);");
    }
    status.out.println(
     "</script>" +
    "");
    PrintPwdOTPDiv(status,"armt_cgo_qadv_operazioni_dl",BO.m_bLoaded,(BO.m_bLoaded?false:false),(BO.m_bLoaded?false:false));
    status.out.println(
     "</div>" +
    "");
  }
  void WriteControls(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
    WriteExtendedTabs(BO.m_MNTs,BO.extendedFields,status,2);
  }
  void WriteControls_p1(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    status.out.println(
     "<div id='page_1' class='PageContainer' style='position:relative;height:797px;width:976px;display:none'>" +
    "");
  ...
  void WriteControls_p1_b0(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
  ...
  void WriteControls_p1_b1(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    status.out.println(
     "<div name='Questionario' id='CJTHKOMYJE_DIV' class='BoxContainer SPSection_expanded SectionContainerWithTitle'>" +
    "");
    if (CPLib.eq(status.GetBoxHeaderInclude(),"")) {
      status.out.println(
       "<div id='CJTHKOMYJE_HEADER' class='BoxHeader'>" +
      "");
      status.out.println(
       "<span id='CJTHKOMYJE_HEADER_CAPTION' class='BoxHeaderCaption'>" +
       ""+status.context.Translate("Questionario")+"" +
       "</span>" +
      "");
      status.out.println(
       "<img id='CJTHKOMYJE_HEADER_ICON' class='BoxHeaderIcon' src='"+com.zucchetti.sitepainter.Library.GetStyleVariable(status.m_cTheme, "boxHeaderCollapseImage", "../"+status.m_cTheme+"/formPage/box_collapse.gif")+"' onclick='ToggleCollapsibleBox(\"CJTHKOMYJE\")' alt='"+status.context.Translate("Expand/Collapse")+"' title='"+status.context.Translate("Expand/Collapse")+"' tabindex='"+status.m_nTabIndex+"'>" +
      "");
      status.out.println(
       "</div>" +
      "");
    } else {
      SPLib.Include(servletContext,status,status.GetBoxHeaderInclude()+"?ForcedPortletUID=CJTHKOMYJE_HEADER&m_cBoxId=CJTHKOMYJE&m_cInitialState=open&m_cTheme="+status.m_cTheme+"&m_nTabIndex="+status.m_nTabIndex+"&m_cCaption="+com.zucchetti.sitepainter.Library.URLencode(status.context.Translate("Questionario"))+"");
    }
    status.out.println(
     "<div id='CJTHKOMYJE' class='SectionBody' style='position:relative;height:770px;display:block'>" +
    "");
    status.includeZoneAltInterface("Questionario@BEGIN");
    if (status.includeZoneAltInterface("Questionario")) {
      WriteControls_p1_b1_CloseBand(BO,status);
    } else {
      if (status.HasZoneAltInterface("Questionario@BEGIN") || status.HasZoneAltInterface("Questionario@END")) {
        status.out.println(
         "<div id='CJTHKOMYJE_boxContent' style='position:relative;height:770px;display:block'>" +
        "");
      }
  ...
  void WriteControls_p1_b1_CloseBand(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    status.includeZoneAltInterface("Questionario@END");
    status.out.println(
     "</div>" +
    "");
    status.out.println(
    ...
    status.out.println(
     "<script>" +
    "");
    if (status.HasZoneAnyAltInterface("Questionario")) {
      status.out.println("Ctrl('CJTHKOMYJE').style.height='auto';");
    } else {
      status.out.println("Ctrl('CJTHKOMYJE').setAttribute('band_height',770);");
    }
    status.out.println("Ctrl('CJTHKOMYJE').setAttribute('status','open');");
    status.out.println(
     "</script>" +
    "");
  }
  void WriteControls_p1_b2(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
  ...
  static void PrintState(armt_cgo_qadv_operazioni_dlBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(armt_cgo_qadv_operazioni_dlBL BO,ServletStatus status) {
    PrintMNTState(status,BO,BO.m_MNTs);
  }
  static void PrintStateWorkVars(armt_cgo_qadv_operazioni_dlBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if ( ! (SPLib.IsMobile(status.request))) {
      status.out.println("var m_nPreferredHeight = " + SPLib.ToJSValue(797,"N",0,0)+ ";");
      status.out.println("var m_nPreferredWidth = " + SPLib.ToJSValue(976,"N",0,0)+ ";");
    }
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"armt_cgo_qadv_operazioni_dl"),"C",0,0)+ ";");
    ...
    status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("armt_cgo_qadv_operazioni_dl","C",0,0)+ ";");
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"armt_cgo_qadv_operazioni_dl",BO.GetCurrentState());
    ...
    if (CPLib.eq(status.GetZoneAltInterface("Questionario"),"")) {
      status.quoteAndAppend("Questionario","hide","box");
    }
  ...
  void WriteMainFormScript(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    if (status.m_bAutoFill) {
      ...
      status.out.println("SetFromContext(m_oFather.m_cWv_armt_cgo_qadv_operazioni_dl);");
      status.out.println("}");
    }
    if (CPLib.eq(status.m_nChildStatus,1)) {
    ...
    if ((CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) && CPLib.gt(status.m_nChildStatus,0)) {
      status.out.println("if (IsWndAccessible(m_oFather)) {");
      status.out.println("m_oFather.SetChildVariables_armt_cgo_qadv_operazioni_dl();");
      status.out.println("}");
    }
    if (CPLib.gt(BO.extendedFields.length,0) || CPLib.HasTabFoundlings(BO.m_MNTs,"")) {
      status.out.println("Z.Tabs.Get('tabs').InitSelect('armt_cgo_qadv_operazioni_dl',Ne(m_nChildStatus,0));");
    } else {
    ...
    status.out.println("// * --- armt_cgo_qadv_operazioni_dl");
    status.out.println("parent.Set_FLGQADVFULL(w_QADV_FLGCOMPLETO)");
    status.out.println("// * --- Fine Area Manuale");
    ...
    status.out.println("m_oFather.m_oarmt_cgo_qadv_operazioni_dl=null;");
    status.out.println("}");
    status.out.println("}");
    status.out.println("}");
  ...
  armt_cgo_qadv_operazioni_dlBO getBO(ServletStatus status) {
    armt_cgo_qadv_operazioni_dlBO BO = new armt_cgo_qadv_operazioni_dlBO(status.context);
    if (CPLib.eq(status.m_nChildStatus,0)) {
      LoadExtensions(BO);
    }
    return BO;
  }
  void LoadExtensions(armt_cgo_qadv_operazioni_dlBO BO) {
    XDCHolder xdc = CPLib.GetXDCHolder(SPLib.GetClassesPath(),new SPXDCReaderFactory(BO.m_Ctx.GetInstance()),BO.m_Ctx.GetInstance());
    XDCTable t = xdc.GetXDCTable("cgo_qadv_operazioni_dl",true);
    BO.extendedFields = CPExtenderLib.GetExtendedItemsFromTable(t);
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  CPResultSet QueryKeyCursor(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    String l_cServer = BO.m_cServer_cgo_qadv_operazioni_dl;
    String l_cPhName = BO.m_cPhName_cgo_qadv_operazioni_dl;
    ...
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      String l_cQueryFilter = armt_cgo_qadv_operazioni_dlBL.QueryFilter(status.context);
      ...
      if (l_rsKeys!=null)
        l_rsKeys.Close();
      l_rsKeys = status.context.GetSql().Query(l_cServer,CPSql.BuildSQLPhrase(" "+BO.m_cVirtName_cgo_qadv_operazioni_dl+".SNAINUMOPE",CPLib.GetTablesWithAliases(l_cPhName,status.m_cAllVirtNames,status.context),l_cLastWhere,status.m_cLastOrderBy));
    } else {
  ...
  CPResultSet QueryKeyCursor(SPMasterServletStatus status) {
    String l_cServer = status.context.GetServer("cgo_qadv_operazioni_dl");
    String l_cPhName = CPSql.ManipulateTablePhName("cgo_qadv_operazioni_dl",l_cServer);
    // Creazione del cursore delle chiavi. Costruisce la frase SQL nella tabella
    // associata alla maschera selezionando solo i campi della chiave di ricerca che
    // soddisfa la clausola di "where" passata come parametro. Riceve altresì come
    // parametro la clausola di "order by".
  ...
  void Query(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO) {
    if ( ! (CPLib.Empty(status.m_cLastWhere))) {
      ...
      if (l_rsCount!=null)
        l_rsCount.Close();
      l_rsCount = BO.m_Sql.Query(BO.m_cServer_cgo_qadv_operazioni_dl,CPSql.BuildSQLPhrase(" count(*) as reccount",CPLib.GetTablesWithAliases(BO.m_cPhName_cgo_qadv_operazioni_dl,status.m_cAllVirtNames,BO.m_Ctx),l_cLastWhere,""));
      try {
  ...
  void View(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO) {
  ...
  void Edit(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO,SPParameterSource source) {
  ...
  void New(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO,SPParameterSource source) {
    BO.Initialize();
    if (BO.CanAdd()) {
  ...
  void Write(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO,SPParameterSource source) {
  ...
  void LoadCurrentRecord(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO) {
  ...
  void EditLoad(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO,SPParameterSource source) {
    Edit(status,BO,source);
    if ( ! (BO.m_bLoaded)) {
      status.m_cErrMsg = "";
      status.m_bError = false;
  ...
  void Delete(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO,SPParameterSource source) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
  ...
  void DeleteMessage(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO,SPParameterSource source) {
    BO.Load(status.w_SNAINUMOPE);
    status.m_cBOWarnMessageDeleted = status.context.Translate("MSG_PROCEED_WITH_DELETE");
    status.m_bExit = true;
    status.m_cClientMode = "bringback";
  }
  void Save(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO) {
    boolean l_bResult = false;
    String l_cTrsMsgErr = "";
    boolean l_bTrsOk = true;
    String l_cTransMsgErr = "";
  ...
  void Discard(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO) {
    BO.SetFromContext(status.m_cWv,status.m_Parameters);
  ...
  void CreatePortlet(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO) {
    ...
    item.put("altInterfaceFor","armt_cgo_qadv_operazioni_dl");
    items.put(item);
    String s;
    org.json.JSONObject ctrl = null;
    s = "{\"bg_color\":\"transparent\",\"box_name\":\"Questionario\",\"box_type\":\"horizontal_section\",\"collapsible\":true,\"comment\":\"Questionario\",\"h\":770,\"name\":\"Questionario\",\"page\":1,\"shrinkable\":true,\"spuid\":\"CJTHKOMYJE\",\"stretch\":true,\"tabindex\":49,\"title_caption\":\"Questionario\",\"titled\":true,\"type\":\"Box\",\"w\":968,\"x\":8,\"y\":7,\"zone\":\"pag1_1\",\"zonepath\":\"[1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 1\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA01\",\"page\":1,\"spuid\":\"QISCOZIXKK\",\"tabindex\":1,\"textlist\":\"Meno di 10,Tra 10 e 20,Tra 20 e 30,Oltre i 30\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2,3,4\",\"w\":432,\"x\":16,\"y\":42,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HVOXPQWYNJ\",\"page\":1,\"spuid\":\"HVOXPQWYNJ\",\"tabindex\":23,\"type\":\"Label\",\"value\":\"1. Presenze clienti in sala al momento della presentazione del ticket:\",\"w\":588,\"x\":16,\"y\":18,\"zone\":\"pag1_1_1\",\"zonepath\":\"[1,1,1]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 2\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA02\",\"page\":1,\"spuid\":\"WBZKOKXYXU\",\"tabindex\":2,\"textlist\":\"Cliente fisso (tutti i giorni),Cliente abituale (almeno 2gg alla settimana),Saltuario (Al max 1 volta alla settimana),Occasionale - Mai visto prima\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2,3,4\",\"w\":432,\"x\":16,\"y\":91,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_YAVCLUSAVL\",\"page\":1,\"spuid\":\"YAVCLUSAVL\",\"tabindex\":25,\"type\":\"Label\",\"value\":\"2. Il presentatore del ticket \\u00E8:\",\"w\":588,\"x\":16,\"y\":70,\"zone\":\"pag1_1_2\",\"zonepath\":\"[1,1,2]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 3\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA03\",\"page\":1,\"spuid\":\"FQNOXMHNDA\",\"tabindex\":3,\"textlist\":\"Si,No\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2\",\"w\":432,\"x\":16,\"y\":140,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_VMTTCEOZYW\",\"page\":1,\"spuid\":\"VMTTCEOZYW\",\"tabindex\":27,\"type\":\"Label\",\"value\":\"3. La realizzazione della vincita \\u00E8 stato oggetto di visione dal personale?\",\"w\":588,\"x\":16,\"y\":119,\"zone\":\"pag1_1_3\",\"zonepath\":\"[1,1,3]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Motivo No 1\",\"h\":21,\"label_text\":\"Affluenza Eccessiva\",\"maxlength\":1,\"name\":\"QADV_DOMANDA03_1\",\"page\":1,\"spuid\":\"ZZAXIZEFKI\",\"tabindex\":4,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":115,\"x\":104,\"y\":168,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Motivo No 2\",\"h\":21,\"label_text\":\"Personale Impegnato in Altro\",\"maxlength\":1,\"name\":\"QADV_DOMANDA03_2\",\"page\":1,\"spuid\":\"YMMKALGQQM\",\"tabindex\":5,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":200,\"x\":232,\"y\":168,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_QFUTYXKNVG\",\"page\":1,\"spuid\":\"QFUTYXKNVG\",\"tabindex\":50,\"type\":\"Label\",\"value\":\"Se No (Motivo): \",\"w\":85,\"x\":16,\"y\":168,\"zone\":\"pag1_1_4\",\"zonepath\":\"[1,1,4]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Motivo No 3\",\"h\":21,\"label_text\":\"Altro\",\"maxlength\":1,\"name\":\"QADV_DOMANDA03_3\",\"page\":1,\"spuid\":\"TIXQDCBYJJ\",\"tabindex\":6,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":115,\"x\":104,\"y\":189,\"zone\":\"pag1_1_5\",\"zonepath\":\"[1,1,5]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Motivo Risposta 3\",\"h\":42,\"name\":\"QADV_MOTIVO03\",\"page\":1,\"scroll\":true,\"spuid\":\"HOWAZKTGQK\",\"tabindex\":7,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":504,\"x\":456,\"y\":140,\"zone\":\"pag1_1_6\",\"zonepath\":\"[1,1,6]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 4\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA04\",\"page\":1,\"spuid\":\"ATNBYAZJII\",\"tabindex\":8,\"textlist\":\"Si,No\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2\",\"w\":432,\"x\":16,\"y\":238,\"zone\":\"pag1_1_7\",\"zonepath\":\"[1,1,7]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_JPWJVELENU\",\"page\":1,\"spuid\":\"JPWJVELENU\",\"tabindex\":29,\"type\":\"Label\",\"value\":\"4. Il ticket presenta anomalie?\",\"w\":588,\"x\":16,\"y\":217,\"zone\":\"pag1_1_7\",\"zonepath\":\"[1,1,7]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Motivo No 1\",\"h\":21,\"label_text\":\"Assenza Vincita\",\"maxlength\":1,\"name\":\"QADV_DOMANDA04_1\",\"page\":1,\"spuid\":\"KBUHMXAJEA\",\"tabindex\":9,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":104,\"x\":104,\"y\":265,\"zone\":\"pag1_1_8\",\"zonepath\":\"[1,1,8]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Motivo No 3\",\"h\":21,\"label_text\":\"Comportamento Sospetto\",\"maxlength\":1,\"name\":\"QADV_DOMANDA04_3\",\"page\":1,\"spuid\":\"SAFBZXXNOG\",\"tabindex\":11,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":152,\"x\":264,\"y\":266,\"zone\":\"pag1_1_8\",\"zonepath\":\"[1,1,8]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_UICEKEORFQ\",\"page\":1,\"spuid\":\"UICEKEORFQ\",\"tabindex\":51,\"type\":\"Label\",\"value\":\"Se Si (Motivo): \",\"w\":85,\"x\":16,\"y\":266,\"zone\":\"pag1_1_8\",\"zonepath\":\"[1,1,8]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Motivo No2\",\"h\":21,\"label_text\":\"Tempo di gioco molto breve (meno di 10 minuti)\",\"maxlength\":1,\"name\":\"QADV_DOMANDA04_2\",\"page\":1,\"spuid\":\"TJQKPGPGYX\",\"tabindex\":10,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":272,\"x\":104,\"y\":287,\"zone\":\"pag1_1_9\",\"zonepath\":\"[1,1,9]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Motivo No 4\",\"h\":21,\"label_text\":\"Altro\",\"maxlength\":1,\"name\":\"QADV_DOMANDA04_4\",\"page\":1,\"spuid\":\"AJOULWEBQJ\",\"tabindex\":12,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":64,\"x\":384,\"y\":287,\"zone\":\"pag1_1_9\",\"zonepath\":\"[1,1,9]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 5\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA05\",\"page\":1,\"spuid\":\"GQFWKUDIZX\",\"tabindex\":13,\"textlist\":\"Si: Richiesta pagamento cash oltre soglia,Si: Richiesta intestazione assegno ad altro soggetto,Si: Richiesta di bonifico a conto non intestato,No: Indicatore ticket nella norma,No: Indicatori di condotta e di gioco regolari,No: Altro\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2,3,4,5,6\",\"w\":432,\"x\":16,\"y\":343,\"zone\":\"pag1_1_10\",\"zonepath\":\"[1,1,10]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_RBOEXYCQKR\",\"page\":1,\"spuid\":\"RBOEXYCQKR\",\"tabindex\":32,\"type\":\"Label\",\"value\":\"5. La vincita presenta anomalie:\",\"w\":588,\"x\":16,\"y\":322,\"zone\":\"pag1_1_10\",\"zonepath\":\"[1,1,10]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Motivo Risposta 4\",\"h\":42,\"name\":\"QADV_MOTIVO05\",\"page\":1,\"scroll\":true,\"spuid\":\"NAOHLRJFKW\",\"tabindex\":14,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":504,\"x\":456,\"y\":343,\"zone\":\"pag1_1_11\",\"zonepath\":\"[1,1,11]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 6\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA06\",\"page\":1,\"spuid\":\"NXAIQESKYE\",\"tabindex\":15,\"textlist\":\"Si,No\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2\",\"w\":432,\"x\":16,\"y\":413,\"zone\":\"pag1_1_12\",\"zonepath\":\"[1,1,12]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_HURAWBJINT\",\"page\":1,\"spuid\":\"HURAWBJINT\",\"tabindex\":34,\"type\":\"Label\",\"value\":\"6. Si \\u00E8 in grado di garantire che il presentatore del ticket \\u00E8 il medesimo soggetto che ha realizzato la vincita?\",\"w\":588,\"x\":16,\"y\":392,\"zone\":\"pag1_1_12\",\"zonepath\":\"[1,1,12]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 7\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA07\",\"page\":1,\"spuid\":\"TANVFQSZEQ\",\"tabindex\":16,\"textlist\":\"Si: Libero Professionista,Si: Commerciante-Artigiano,Si: Imprenditore,Si: Dipendente,Si: Altro,No\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2,3,4,5,6\",\"w\":432,\"x\":16,\"y\":462,\"zone\":\"pag1_1_13\",\"zonepath\":\"[1,1,13]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_UVKJPSBTQL\",\"page\":1,\"spuid\":\"UVKJPSBTQL\",\"tabindex\":36,\"type\":\"Label\",\"value\":\"7. La sala conosce l"+"'"+"attivit\\u00E0 lavorativa del cliente?\",\"w\":588,\"x\":16,\"y\":441,\"zone\":\"pag1_1_13\",\"zonepath\":\"[1,1,13]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 8\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA08\",\"page\":1,\"spuid\":\"OUKEHIZPFY\",\"tabindex\":17,\"textlist\":\"No,Si: Il cliente non gradisce fornire informazioni,Si: Il cliente ha fornito informazioni sul punto\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2,3\",\"w\":432,\"x\":16,\"y\":511,\"zone\":\"pag1_1_14\",\"zonepath\":\"[1,1,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_ETFQYWJUTI\",\"page\":1,\"spuid\":\"ETFQYWJUTI\",\"tabindex\":38,\"type\":\"Label\",\"value\":\"8. In questo caso (no alla 7) si \\u00E8 in grado comunque di ipotizzare la provenienza delle somme impiegate al gioco?\",\"w\":588,\"x\":16,\"y\":490,\"zone\":\"pag1_1_14\",\"zonepath\":\"[1,1,14]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 9\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA09\",\"page\":1,\"spuid\":\"CTLRUBZHIS\",\"tabindex\":18,\"textlist\":\"Si,No\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2\",\"w\":432,\"x\":16,\"y\":560,\"zone\":\"pag1_1_15\",\"zonepath\":\"[1,1,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_AOXLXNKIET\",\"page\":1,\"spuid\":\"AOXLXNKIET\",\"tabindex\":40,\"type\":\"Label\",\"value\":\"9. Il cliente passa tempo in sala anche senza giocare?\",\"w\":588,\"x\":16,\"y\":539,\"zone\":\"pag1_1_15\",\"zonepath\":\"[1,1,15]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 10\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA10\",\"page\":1,\"spuid\":\"BJMTLOURHC\",\"tabindex\":19,\"textlist\":\"Si,No\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2\",\"w\":432,\"x\":16,\"y\":609,\"zone\":\"pag1_1_16\",\"zonepath\":\"[1,1,16]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_TLTEZRKXTA\",\"page\":1,\"spuid\":\"TLTEZRKXTA\",\"tabindex\":42,\"type\":\"Label\",\"value\":\"10. Il cliente appare amico di altri clienti o sembra conoscerne altri?\",\"w\":588,\"x\":16,\"y\":588,\"zone\":\"pag1_1_16\",\"zonepath\":\"[1,1,16]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 11\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA11\",\"page\":1,\"spuid\":\"YOEMCDHBCJ\",\"tabindex\":20,\"textlist\":\"Si,No\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2\",\"w\":432,\"x\":16,\"y\":658,\"zone\":\"pag1_1_17\",\"zonepath\":\"[1,1,17]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_IEKBDQWIOD\",\"page\":1,\"spuid\":\"IEKBDQWIOD\",\"tabindex\":44,\"type\":\"Label\",\"value\":\"11. Il cliente conosce le procedure antiriciclaggio?\",\"w\":588,\"x\":16,\"y\":637,\"zone\":\"pag1_1_17\",\"zonepath\":\"[1,1,17]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Domanda 12\",\"empty_value\":\"false\",\"h\":20,\"maxlength\":5,\"name\":\"QADV_DOMANDA12\",\"page\":1,\"spuid\":\"RGKOIXTSHJ\",\"tabindex\":21,\"textlist\":\"Si,No\",\"type\":\"Combobox\",\"typevar\":\"numeric\",\"valuelist\":\"1,2\",\"w\":432,\"x\":16,\"y\":707,\"zone\":\"pag1_1_18\",\"zonepath\":\"[1,1,18]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"h\":16,\"name\":\"label_LWMEEVTYOG\",\"page\":1,\"spuid\":\"LWMEEVTYOG\",\"tabindex\":46,\"type\":\"Label\",\"value\":\"12. Il cliente \\u00E8 solito chiedere l"+"'"+"emissione di un ticket per giocare?\",\"w\":588,\"x\":16,\"y\":686,\"zone\":\"pag1_1_18\",\"zonepath\":\"[1,1,18]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"comment\":\"Motivo Risposta 4\",\"h\":42,\"name\":\"QADV_MOTIVO04\",\"page\":1,\"scroll\":true,\"spuid\":\"IPNCGLSXHQ\",\"tabindex\":31,\"type\":\"Textbox\",\"typevar\":\"memo\",\"w\":504,\"x\":456,\"y\":238,\"zone\":\"pag1_1_19\",\"zonepath\":\"[1,1,19]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
    s = "{\"checked_value\":\""+"'"+"S"+"'"+"\",\"comment\":\"Questionario Completo\",\"h\":21,\"label_text\":\"Questionario completato\",\"maxlength\":1,\"name\":\"QADV_FLGCOMPLETO\",\"page\":1,\"spuid\":\"RVNBZEITUY\",\"tabindex\":48,\"type\":\"Checkbox\",\"typevar\":\"character\",\"unchecked_value\":\""+"'"+"N"+"'"+"\",\"w\":168,\"x\":792,\"y\":21,\"zone\":\"pag1_1_20\",\"zonepath\":\"[1,1,20]\"}";
    ctrl = new org.json.JSONObject(s);
    items.put(ctrl);
  ...
  void WriteFormPage(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='armt_cgo_qadv_operazioni_dl?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    if (CPLib.eq(status.m_cClientMode,"query") || CPLib.eq(status.m_cClientMode,"view")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_cgo_qadv_operazioni_dl_query.js'>" +
       "</script>" +
      "");
    } else if (CPLib.eq(status.m_cClientMode,"new") || CPLib.eq(status.m_cClientMode,"edit")) {
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_cgo_qadv_operazioni_dl_edit.js'>" +
       "</script>" +
      "");
    }
    if (CPLib.eq(status.m_cClientMode,"painter")) {
      ...
      status.out.println("o_QADV_DOMANDA01=w_QADV_DOMANDA01;");
      status.out.println("o_QADV_DOMANDA02=w_QADV_DOMANDA02;");
      status.out.println("o_QADV_DOMANDA03=w_QADV_DOMANDA03;");
      status.out.println("o_QADV_DOMANDA03_1=w_QADV_DOMANDA03_1;");
      status.out.println("o_QADV_DOMANDA03_2=w_QADV_DOMANDA03_2;");
      status.out.println("o_QADV_DOMANDA03_3=w_QADV_DOMANDA03_3;");
      status.out.println("o_QADV_DOMANDA04=w_QADV_DOMANDA04;");
      status.out.println("o_QADV_DOMANDA04_1=w_QADV_DOMANDA04_1;");
      status.out.println("o_QADV_DOMANDA04_2=w_QADV_DOMANDA04_2;");
      status.out.println("o_QADV_DOMANDA04_3=w_QADV_DOMANDA04_3;");
      status.out.println("o_QADV_DOMANDA04_4=w_QADV_DOMANDA04_4;");
      status.out.println("o_QADV_DOMANDA05=w_QADV_DOMANDA05;");
      status.out.println("o_QADV_DOMANDA06=w_QADV_DOMANDA06;");
      status.out.println("o_QADV_DOMANDA07=w_QADV_DOMANDA07;");
      status.out.println("o_QADV_DOMANDA08=w_QADV_DOMANDA08;");
      status.out.println("o_QADV_DOMANDA09=w_QADV_DOMANDA09;");
      status.out.println("o_QADV_DOMANDA10=w_QADV_DOMANDA10;");
      status.out.println("o_QADV_DOMANDA11=w_QADV_DOMANDA11;");
      status.out.println("o_QADV_DOMANDA12=w_QADV_DOMANDA12;");
      status.out.println("}");
      ...
      status.out.println(
       "<script src='SPPrxy/"+m_cEntityUid+"/armt_cgo_qadv_operazioni_dl_proc.js'>" +
       "</script>" +
      "");
    }
    IncludeJS(status);
    ...
    status.themeWriter.WriteBody_Begin(797,status.m_cBodyAttributes,status.context.Translate("MSG_LOADING")+"...");
    status.out.println(
     "<script>" +
    "");
    status.out.println(
  ...
  void ForwardZoom(String p_cForwardZoomResource,ServletStatus status,SPParameterSource source) {
    ...
    f.SetParameter("Table","cgo_qadv_operazioni_dl");
  ...
  void RaiseLoadedEntity(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
  ...
  void RaiseSavedEntity(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
  ...
  void WriteExitPage(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status,Forward f) {
  ...
  void WriteExitPage(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) {
    WriteExitPage(BO,status,Forward.Unforwarded);
  }
  protected void BringBackPage(armt_cgo_qadv_operazioni_dlBO BO,ServletStatus status) throws IOException {
  ...
  void IncludeJS(ServletStatus status) {
    String l_cJsUid;
    status.out.println(
     "<script src='SPPrxy/"+arrt_cgo_punteggio_adv_dl.m_cEntityUid+"/arrt_cgo_punteggio_adv_dl.js'>" +
     "</script>" +
    "");
  ...
  public static void GetCallableNames(java.util.Set callable) {
    if (CPLib.IsNull(m_CallableNames)) {
      arrt_cgo_punteggio_adv_dl.GetCallableNames(callable);
    } else {
  ...
  public String GetCheckVars(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,armt_cgo_qadv_operazioni_dlBO BO) {
    boolean result = true;
