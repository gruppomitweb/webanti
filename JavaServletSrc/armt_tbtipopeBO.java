...
// * --- Fine Area Manuale
public class armt_tbtipopeBO extends armt_tbtipopeBL {
  class SqlLoader {
    double m_nColumns = 0;
    CPSql m_Sql;
    String m_cServer;
  ...
  // * --- Fine Area Manuale
  public armt_tbtipopeBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"armt_tbtipope");
    }
  ...
  public boolean Load(String p_CODICE) {
    ...
    l_oSqlLoader = new SqlLoader(m_Sql,m_cServer_tbtipope,m_cPhName_tbtipope,m_cVirtName_tbtipope,m_cVirtName_tbtipope+".CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_CODICE,"C",3,0),m_cServer_tbtipope,p_CODICE),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipope")) {
  ...
  void Insert() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbtipope);
      l_cSql = "insert into "+m_oWrInfo_tbtipope.GetTableWriteName()+" (";
      l_cSql = l_cSql+"CODICE,";
      l_cSql = l_cSql+"DESCRI,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbtipope.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipope")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_DESCRI,"C",254,0,m_cServer_tbtipope,m_oParameters)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_tbtipope.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipope")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_tbtipope,l_cSql,m_oParameters);
  ...
  void Update() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_tbtipope);
      l_cSql = "Update "+m_oWrInfo_tbtipope.GetTableWriteName()+" set";
      l_cSql = l_cSql+" DESCRI = "+CPLib.ToSQL(w_DESCRI,"C",254,0,m_cServer_tbtipope,m_oParameters);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipope")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",3,0),m_cServer_tbtipope,w_CODICE);
      l_cSql = l_cSql+m_oWrInfo_tbtipope.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipope")) {
  ...
  public boolean Delete(String p_CODICE) {
    ...
    if (m_bLoaded) {
      if (CanDelete()) {
        ...
        if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
          l_cSql = "Delete from "+m_oWrInfo_tbtipope.GetTableWriteName()+" where ";
          l_cSql = l_cSql+"CODICE="+CPSql.SQLValueAdapter(CPLib.ToSQL(w_CODICE,"C",3,0),m_cServer_tbtipope,w_CODICE);
          l_cSql = l_cSql+m_oWrInfo_tbtipope.WhereFilter();
          if (CPLib.HasTableCPCCCHK(m_Ctx,"tbtipope")) {
            l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldCPCCCHK,"C",10,0);
          }
          l_nUpdatedRows = m_Sql.Update(m_cServer_tbtipope,l_cSql);
          if (CPLib.ne(l_nUpdatedRows,1)) {
            // AbortTransaction
            m_Sql.AbortTransaction();
          }
