...
// * --- Fine Area Manuale
public class arrt_azz_semaforoR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_azz_semaforoR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_azz_semaforo",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    try {
      ...
      if ( ! (Read_Cursor.Eof())) {
        ...
        Read_Cursor.Next();
        if ( ! (Read_Cursor.Eof())) {
          Error l_oErrorFault = new Error("Read on cpwarn into routine arrt_azz_semaforo returns two or more records. This item should return only a record.");
      ...
      /* If not(Empty(_semaforo)) */
      if ( ! (CPLib.Empty(_semaforo))) {
        // * --- Delete from cpwarn
        ...
        m_cSql = CPLib.GetSQLPrefixComment("arrt_azz_semaforo",2,"00000006")+"delete from "+m_oWrInfo.GetTableWriteName();
        m_cSql = m_cSql+" where ";
      ...
      /* Alert Msg 'Azzeramento semaforo completato' */
      // no alert sever-side
    } finally {
      try {
        if (Read_Cursor!=null)
  ...
  public static arrt_azz_semaforoR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_azz_semaforoR(p_Ctx, p_Caller);
  }
  public void Blank() {
    _semaforo = CPLib.Space(10);
  }
