...
public class arrt_aggstatoreg extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*c3f7d441*!";
  ...
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ...
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.pOper,"C",4,0)+",'C');");
      ...
      if (status.m_bApplet) {
        ...
        status.out.println("msgw=windowOpenForeground('arrt_aggstatoreg"+BrowserSink.MessageQueryString(status)+"','','toolbar=0,menubar=0,directories=0,width=400,height=100,resizable=1');");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_aggstatoregR Rcalled;
    Rcalled = new arrt_aggstatoregR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ...
      status.forwardedto = Rcalled.Run(status.pFile,status.pOper);
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_aggstatoregR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_aggstatoregR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      status.out.println(
       "<!--" +
      "");
      status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(Rcalled.RunAsync(status.pFile,status.pOper)))+"");
      status.out.println(
       " -->" +
      "");
    } else {
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_aggstatoreg"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_aggstatoreg");
      if ( ! (CPLib.IsNull(l))) {
