...
public class arpg_filesage_do extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*ISXXPOSGYF*!";
  ...
  public String getEntityTitle() {
    return "Files Generati per Agenzia Entrate (Deleghe/Op. Extra)";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "arpg_filesage_do";
    ...
    l_eg.m_cQueryName = "qbe_filesage_do";
    return l_eg;
  }
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,arpg_filesage_doBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    {"11236572921",p_Context.Translate("Nome File Generato")},
    {"113933141",p_Context.Translate("Update")},
    {"1507039234",p_Context.Translate("Files Generati per Agenzia Entrate (Deleghe/Op. Extra)")},
    {"1739899105",p_Context.Translate("Azienda")},
    {"251106009",p_Context.Translate("Data Generazione")},
    {"285808099",p_Context.Translate("Descrizione")}};
    return l_translations;
  }
  void WriteStyles(arpg_filesage_doBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='SPPrxy/"+m_cEntityUid+"/arpg_filesage_do.css'>" +
      "");
    }
  ...
  void WriteMainForm(arpg_filesage_doBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(arpg_filesage_doBO BO,ServletStatus status) {
    WriteControls_p1(BO,status);
    PrintNotifyContent(status,getNotifyItems());
  }
  void WriteControls_p1(arpg_filesage_doBO BO,ServletStatus status) {
  ...
  static void PrintState(arpg_filesage_doBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(arpg_filesage_doBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(arpg_filesage_doBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(arpg_filesage_doBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"arpg_filesage_do"),"C",0,0)+ ";");
    ...
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"arpg_filesage_do","");
  ...
  void WriteMainFormScript(arpg_filesage_doBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    WriteTrs(status,BO);
  ...
  arpg_filesage_doBO getBO(ServletStatus status) {
    arpg_filesage_doBO BO = new arpg_filesage_doBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(arpg_filesage_doBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,arpg_filesage_doBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void Start(ServletStatus status,arpg_filesage_doBO BO) {
    BO.SetFromContext(status.m_cWv);
    BO.Calculate();
  }
  void Save(ServletStatus status,arpg_filesage_doBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,arpg_filesage_doBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,arpg_filesage_doBO BO) {
    ...
    item.put("altInterfaceFor","arpg_filesage_do");
    items.put(item);
    String s;
  ...
  void WriteBody(ServletStatus status,arpg_filesage_doBO BO) {
  ...
  void WriteTrs(ServletStatus status,arpg_filesage_doBO BO) {
    status.out.println("var c_datafile;");
    status.out.println("var c_nomefile;");
    status.out.println("var c_descfile;");
  ...
  void AddTrsQueryRow(ServletStatus status,int p_nCnt,arpg_filesage_doBO BO) {
    String l_cTrsQ;
    l_cTrsQ = " new TrsQueryRow(";
    l_cTrsQ = l_cTrsQ+SPLib.ToJSValue(BO.c_datafile,"D",8,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_nomefile,"C",20,0);
    l_cTrsQ = l_cTrsQ+","+SPLib.ToJSValue(BO.c_descfile,"C",40,0);
    l_cTrsQ = l_cTrsQ+")";
    status.out.println("m_oTrsQuery["+p_nCnt+"] = "+l_cTrsQ+"");
    String l_cTrsF;
    l_cTrsF = " new TrsFWRow(";
    l_cTrsF = l_cTrsF+SPLib.ToJSValue(BO.w_gUrlApp,"C",80,0);
    l_cTrsF = l_cTrsF+","+SPLib.ToJSValue(BO.w_gAzienda,"C",10,0);
    l_cTrsF = l_cTrsF+",0)";
    status.out.println("m_oTrsFW["+p_nCnt+"] = "+l_cTrsF+"");
  }
  void Next(ServletStatus status,arpg_filesage_doBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,arpg_filesage_doBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(arpg_filesage_doBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='arpg_filesage_do?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/arpg_filesage_do_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(arpg_filesage_doBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(arpg_filesage_doBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(arpg_filesage_doBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(arpg_filesage_doBO BO,ServletStatus status) {
  ...
  void WriteExitPage(arpg_filesage_doBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(arpg_filesage_doBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,arpg_filesage_doBO BO) {
    return "NO_VALUE";
  }
  public boolean CheckVars(ServletStatus status,arpg_filesage_doBO BO) {
    boolean result = true;
    return result;
  }
}
