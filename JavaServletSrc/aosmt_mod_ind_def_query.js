...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('NCOATMFFKT').value,'C'),w_IDMOD)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('mod_ind_def',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'IDMOD') &&  ! (Empty(HtW(Ctrl('NCOATMFFKT').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.IDMOD',HtW(Ctrl('NCOATMFFKT').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_ind_def(firstAlias) {
...
_chNotF(childAlias,'master','aosmt_mod_ind_def');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('mod_ind_def',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['NCOATMFFKT'],i=0;i>=0;i--){
