...
public class arrt_stp_stdtec_rap extends SPRoutineServlet implements SPInvokable {
  public static String m_cEntityUid = "!*6e171daf*!";
  public class ServletStatus extends SPRoutineServletStatus {
    ...
    public void GetNonLocals(SPParameterSource source) {
      if (source instanceof SPRequestSource) {
       ((SPRequestSource)source).request.setAttribute("com.zucchetti.sitepainter.parameters.pAUI","pAUI");
      }        
      pAUI = source.GetParameter("pAUI","");
      ...
      if (m_bpassedw_aRapporto) {
        ...
        if (source.IsForwarded()) {
          w_aRapporto = source.GetCaller().GetString("aRapporto","C",16,0);
  ...
  boolean NeedsFetchPage(ServletStatus status) {
    return  ! (status.m_bpassedw_aCodDip) ||  ! (status.m_bpassedw_daCodDip) ||  ! (status.m_bpassedw_aRapporto) ||  ! (status.m_bpassedw_daRapporto) ||  ! (status.m_bpassedw_ADatOpe) ||  ! (status.m_bpassedw_DaDatOpe);
  }
  private void FetchPage(ServletStatus status) throws IOException {
    ...
    if ( ! (status.m_bpassedw_aRapporto)) {
      status.out.println("l_cV=WtA(l_oWnd('w_aRapporto',"+SPLib.ToJSValue(status.w_aRapporto,"C",16,0)+"),'C');");
    } else {
      status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.w_aRapporto,"C",16,0)+",'C');");
    }
    ...
    status.out.println("l_cV=WtA("+SPLib.ToJSValue(status.pAUI,"C",1,0)+",'C');");
    status.out.println("l_oWv.setValue('pAUI',l_cV);");
  ...
  private void ExecuteRoutine(ServletStatus status) throws IOException {
    arrt_stp_stdtec_rapR Rcalled;
    Rcalled = new arrt_stp_stdtec_rapR(status.context,status);
    if (CPLib.ne(status.m_cAction,"executeAsync")) {
      ExecuteWorkflowStartingScript(status,Rcalled);
      status.forwardedto = Rcalled.Run(status.pAUI);
      if (Rcalled.m_bError) {
  ...
  CallerWithObjs NewRoutine(SPServletStatus status,Caller dataFromClient) {
    return new arrt_stp_stdtec_rapR(status.context,dataFromClient);
  }
  protected void BringBackPage(ServletStatus status,arrt_stp_stdtec_rapR Rcalled) throws IOException {
    String l_cErrMsg;
    String msg;
    ...
    if (CPLib.eqr(status.m_cAction,"executeAsync")) {
      status.out.println(
       "<!--" +
      "");
      status.out.println("Function return value:"+com.zucchetti.sitepainter.Library.ToHTMLEuro(CPLib.ToProperties(Rcalled.RunAsync(status.pAUI)))+"");
      status.out.println(
       " -->" +
      "");
    } else {
      if (status.m_bApplet) {
        ...
        if ( ! (CPLib.Empty(status.m_cFaultTrace))) {
          status.out.println("Fault:["+SPLib.ToJSValue(status.context.Translate("MSG_ROUTINE_FAULTED","arrt_stp_stdtec_rap"),"C",0,0)+","+SPLib.ToJSValue(status.m_cFaultTrace,"C",0,0)+"]");
  ...
  private void Begin(ServletStatus status) throws IOException {
    if ( ! (CPLib.Empty(status.m_cBrowseName))) {
      ...
      Rcalled.m_cLastMsgError = status.context.Translate("MSG_WRONG_ROUTINE_ACTION",status.m_cAction,"arrt_stp_stdtec_rap");
      if ( ! (CPLib.IsNull(l))) {
