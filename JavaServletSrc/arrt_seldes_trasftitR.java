...
// * --- Fine Area Manuale
public class arrt_seldes_trasftitR implements CallerWithObjs {
  ...
  // * --- Fine Area Manuale
  public arrt_seldes_trasftitR (CPContext p_ContextObject, Caller caller) {
    if (caller == null)
    ...
    CPSecurity.RoutineCallableByProgram("arrt_seldes_trasftit",m_Caller);
  ...
  void Page_1() throws Exception {
    ...
    if (CPLib.eqr(pTipo,"S")) {
      // * --- Write into tmp_trasftit from values
      ...
      m_cSql = CPLib.GetSQLPrefixComment("arrt_seldes_trasftit",6,"00000005")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
      m_cSql = m_cSql+"FLGSEL = "+CPLib.ToSQL("S","C",1,0,m_cServer)+", ";
      m_cSql = CPLib.AddCPCCCHKUpdate(m_Ctx,m_cSql,"tmp_trasftit",false,true);
      m_cSql = m_cSql+" where ";
      ...
      m_oWrInfo = CPPhTableWrInfo.GetCPPhTableWrInfo(m_Ctx,"tmp_trasftit",m_Ctx.GetCompany());
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer);
      m_cSql = CPLib.GetSQLPrefixComment("arrt_seldes_trasftit",6,"00000006")+"Update "+m_oWrInfo.GetTableWriteName()+" set ";
  ...
  public static arrt_seldes_trasftitR Make(CPContext p_Ctx, Caller p_Caller) {
    return new arrt_seldes_trasftitR(p_Ctx, p_Caller);
  }
  public void Blank() {
    pTipo = CPLib.Space(1);
  }
