...
<html>
  <head>
    ...
    <script type="text/javascript" src="../<%=SPPrxycizer.proxycizedPath("maskparameters/SPMaskParametersConfigurator_filterTableList.js")%>"></script>
  ...
  <body onload="FillList()" onResize='resize()'>
    ...
    <div>
      ...
      <input id='configName' type='hidden' value=<%=JSPLib.ToHTMLValue(request.getParameter("configName"))%>>
      <input id='offline' type='hidden' value=<%=JSPLib.ToHTMLValue(request.getParameter("offline"))%>>
      <input id='multiple' type='hidden' value=<%=JSPLib.ToHTMLValue(request.getParameter("multiple"))%>>
