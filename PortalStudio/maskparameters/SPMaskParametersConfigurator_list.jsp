...
<html>
  <head>
    ...
    <script type="text/javascript" src="../<%=SPPrxycizer.proxycizedPath("maskparameters/SPMaskParametersConfigurator_list.js")%>"></script>
  ...
  <body onload="FillList()" onResize='resize()'>
    ...
    <div>
      <input id='portletId' type='hidden' value=<%=JSPLib.ToHTMLValue(request.getParameter("portletId"))%>>
      <input id='action' type='hidden' value=<%=JSPLib.ToHTMLValue(request.getParameter("action"))%>>
      <input id='tableName' type='hidden' value=<%=JSPLib.ToHTMLValue(request.getParameter("tableName"))%>>
      <input id='parameter' type='hidden' value=<%=JSPLib.ToHTMLValue(request.getParameter("parameter"))%>>
      <input id='queryName' type='hidden' value=<%=JSPLib.ToHTMLValue(request.getParameter("queryName"))%>>
      ...
      <input id='linkedField' type='hidden' value=<%=JSPLib.ToHTMLValue(request.getParameter("linkedField"))%>>
      <input id='SPModalLayerId' type='hidden' value=<%=JSPLib.ToHTMLValue(sp.getParameter("SPModalLayerId",""))%>>
      <input id='offlinemode' type='hidden' value=<%=JSPLib.ToHTMLValue(sp.getParameter("offlinemode","false"))%>>
    </div>

    <div class='titoli' id='footer'>
      <label id='name'> </label>
      <div style='display:inline-block; '><input type="text" id="result" style='width:292px;' onkeydown='confirmSelection(event)'/> </div>
      <input id='addValue' type="button" value='+' onclick='addRow()' style='display:none' class="bottoni"/>
      <input id='removeValue' type="button" value='-' onclick='deleteRow()' style='display:none' class="bottoni"/>
