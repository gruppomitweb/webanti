  ...
  function computeNodeBreadths(graph) {
    ...
    function initializeNodeBreadth() {
      var ky = d3.min(columns, function(nodes) {
        return (y1 - y0 - (nodes.length - 1) * py) / d3.sum(nodes, value);
      });
...
d3.sankeyLeft = function (node) {
  return node.depth;
}

d3.sankeyRight = function (node, n) {
  return n - 1 - node.height;
}

d3.sankeyJustify = function (node, n) {
  return node.sourceLinks.length ? node.depth : n - 1;
}

d3.sankeyCenter = function (node) {
