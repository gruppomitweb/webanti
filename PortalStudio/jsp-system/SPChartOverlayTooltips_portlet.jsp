...
%>
window.SPChartOverlayTooltips_Static=function(){
...
response.setHeader("Cache-Control","public");
%>
.SPChartOverlayTooltips_container .labelTP{  white-space: nowrap;
  overflow: visible !important;
  float: left;
  font-family: Open Sans, sans-serif;
  color: #0D0F12;
...
.SPChartOverlayTooltips_container .colorrect{
  margin-right: 5px;
  width: 15px;
  height: 15px;
  float: left;
}
.SPChartOverlayTooltips_container .contbox{
  width: 99%;
  height: 20px;
}
.SPChartOverlayTooltips_container {
}
.SPChartOverlayTooltips_portlet{
  position:relative;
  width:250px;
  min-width:250px;
  height:100px;
}
.SPChartOverlayTooltips_portlet[Data-page="1"]{
  height:100px;
  width:250px;
}
.SPChartOverlayTooltips_portlet > .tcont_ctrl {
...
.SPChartOverlayTooltips_portlet > .tcont_ctrl > .box_content {
  height:100%;
}
.SPChartOverlayTooltips_portlet > .tcont_ctrl {
  min-height:90px;
...
<%}else{
if (JSPLib.checkApplication(request, response, out, sp, 37,"SPChartOverlayTooltips","UTF-8")) {return; }
...
if(!included){%>
<head>
<base href=<%=JSPLib.ToHTMLValue( JSPLib.SPWebRootURL(request)+"/jsp-system/SPChartOverlayTooltips_portlet.jsp"+(!Library.Empty(request.getQueryString())?"?"+request.getQueryString():"") )%>/>
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=idPortlet%>','SPChartOverlayTooltips','portletObj','<%=JSPLib.ToJSValue(SPParentObjId,false,true)%>');
/*JS_MARKER_END*/
</script>
...
<div id='<%=idPortlet%>_container' class='SPChartOverlayTooltips_container'>
<div id='<%=idPortlet%>' portlet_id='<%=idPortlet%>' align='left' class='SPChartOverlayTooltips_portlet portlet' style='opacity:0.001'>
<%if(sp.isLogged()){
 if(sp.isInGroup(1) && sp.getGlobal("g_EditPortlet",false)==true){%>
<script>
ZtVWeb.AddPortletToEdit('SPChartOverlayTooltips','<%=idPortlet%>',false,'');
</script><%}}%>
...
<script>
/*JS_MARKER_START*/
ZtVWeb.addPortlet('<%=idPortlet%>','SPChartOverlayTooltips');
<%!
...
ZtVWeb.newForm(this,'<%=idPortlet%>','SPChartOverlayTooltips',["250"],["100"],'portlet',<%=sp.isOfflineInstallation()%>,false,m_cSPPageletId,'', 'fixed','','true',{},false,false,1,m_cSPPortletTitleId, 'NORMAL',[{"layout_steps_values":{},"w":"250","h":"100","title":"","layer":"false","npage":"1"}]);
...
<%if(request.getAttribute("SPChartOverlayTooltips_firstinclusion")==null){%>
<script src='<%=JSPLib.getJSPSelfPath(request)%>?SpStaticCode=<%=m_cJSPUID%>'></script>
<%request.setAttribute("SPChartOverlayTooltips_firstinclusion",idPortlet);
}%>
<script>
/*JS_MARKER_START*/
window.SPChartOverlayTooltips_Static.call(window.<%=idPortlet%>);
ZtVWeb.addPortletObj('<%=idPortlet%>','SPChartOverlayTooltips',<%=idPortlet%>);
ZtVWeb.portletLoaded(<%=idPortlet%>,'SPChartOverlayTooltips');
/*JS_MARKER_END*/
</script>
<%if(!included){%>
</body>
...
<%}
sp.endPage("SPChartOverlayTooltips");
}%>
<%! public String getJSPUID() { return "3861524931"; } %>