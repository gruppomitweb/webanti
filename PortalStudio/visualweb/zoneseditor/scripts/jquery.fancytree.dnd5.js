     ...
     (function (factory) {
      ...
      function handleDragOver(event, data) {
        ...
        var markerOffsetX,
          ...
          markerAt = "center",
          $target = $(targetNode.li),
          $targetTitle =  $(targetNode.li);
        ...
        if (DRAG_ENTER_RESPONSE.after && relPosY > $target.height() - 10) {
          hitMode = "after";
        } else if (
          !DRAG_ENTER_RESPONSE.over &&
          DRAG_ENTER_RESPONSE.after &&
          relPosY > 0.5 * $target.height()
        ) {
          hitMode = "after";
        } else if (DRAG_ENTER_RESPONSE.before && relPosY <= 10) {
          hitMode = "before";
        } else if (
          !DRAG_ENTER_RESPONSE.over &&
        ...
        if (hitMode === "after" || hitMode === "before" || hitMode === "over") {
          ...
          pos = {
            ...
            at: "left " + markerAt,
            of: hitMode == "over" ? $target.find("span") : $targetTitle ,
          };
          if (options.rtl) {
        ...
        console.log( "mode " + hitMode + " " + $target.find("span.fancytree-title").text());
        $(targetNode.li)
          .toggleClass(
            classDropTarget,
            hitMode === "after" ||
      ...
      function onDropEvent(event) {
        ...
        switch (event.type) {
          ...
          case "dragleave":
            ...
            if (!$(node).hasClass(classDropOver)) {
