...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('FETUPRQQAZ').value,'C'),w_codazi)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cpazi',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'codazi') &&  ! (Empty(HtW(Ctrl('FETUPRQQAZ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.codazi',HtW(Ctrl('FETUPRQQAZ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','spadministration_companies');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cpazi',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['FETUPRQQAZ'],i=0;i>=0;i--){
