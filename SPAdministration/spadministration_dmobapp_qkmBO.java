...
// * --- Fine Area Manuale
public class spadministration_dmobapp_qkmBO extends spadministration_dmobapp_qkmBL {
  ...
  // * --- Fine Area Manuale
  public spadministration_dmobapp_qkmBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"spadministration_dmobapp_qkm");
    }
  ...
  public static String[] m_cLoadParameterNames = {"QA_CODE"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(String p_QA_CODE) {
    CPResultSet l_rsLoadBody=null;
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_cpqkmappstore+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cpqkmappstore,m_cPhName_cpqkmappstore,m_cVirtName_cpqkmappstore,m_cVirtName_cpqkmappstore+".QA_CODE="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_QA_CODE,"C",10,0),m_cServer_cpqkmappstore,p_QA_CODE),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          ...
          w_QA_LABEL = l_rsLoadBody.GetString("QA_LABEL");
          w_QA_URL = l_rsLoadBody.GetString("QA_URL");
          w_QA_ICON = l_rsLoadBody.GetString("QA_ICON");
          w_CPROWNUM = l_rsLoadBody.GetDouble("CPROWNUM");
  ...
  public boolean Delete(String p_QA_CODE) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cpqkmappstore.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"CPROWNUM="+CPLib.ToSQL(w_CPROWNUM,"N",6,0)+" AND QA_CODE="+CPLib.ToSQL(w_QA_CODE,"C",10,0);
        l_cSql = l_cSql+m_oWrInfo_cpqkmappstore.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cpqkmappstore")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cpqkmappstore,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cpqkmappstore);
      l_cSql = "insert into "+m_oWrInfo_cpqkmappstore.GetTableWriteName()+" (";
      l_cSql = l_cSql+"QA_CODE,";
      l_cSql = l_cSql+"CPROWNUM,";
      l_cSql = l_cSql+"QA_LABEL,";
      l_cSql = l_cSql+"QA_URL,";
      l_cSql = l_cSql+"QA_ICON,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cpqkmappstore.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cpqkmappstore")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_QA_CODE,"C",10,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_CPROWNUM,"N",6,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_QA_LABEL,"C",30,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_QA_URL,"C",255,0,m_cServer_cpqkmappstore,m_oParameters)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_QA_ICON,"C",255,0,m_cServer_cpqkmappstore,m_oParameters)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cpqkmappstore.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cpqkmappstore")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cpqkmappstore,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cpqkmappstore);
      l_cSql = "Update "+m_oWrInfo_cpqkmappstore.GetTableWriteName()+" set";
      l_cSql = l_cSql+" QA_LABEL = "+CPLib.ToSQL(w_QA_LABEL,"C",30,0);
      l_cSql = l_cSql+", QA_URL = "+CPLib.ToSQL(w_QA_URL,"C",255,0,m_cServer_cpqkmappstore,m_oParameters);
      l_cSql = l_cSql+", QA_ICON = "+CPLib.ToSQL(w_QA_ICON,"C",255,0,m_cServer_cpqkmappstore,m_oParameters);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cpqkmappstore")) {
