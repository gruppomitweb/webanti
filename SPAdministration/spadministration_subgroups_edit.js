...
function Set_groupbase(ctrlValue,kmode,e) {
if (Ne(w_groupbase,ctrlValue)) {
var ctl = _GetCtl(e,'MJILVOVIBU');
if (_tracker.goon(ctl,ctrlValue)) {
w_groupbase=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('MJILVOVIBU',0);
if (l_bResult) {
l_bResult=Link_MJILVOVIBU(kmode);
if ( ! (l_bResult)) {
w_groupbase=HtW('','N');
}
}
...
function Link_MJILVOVIBU(kmode) {
var l_bResult = true;
var l_nRec = 0;
var l_cLinkUrl;
var l_bEmp = Empty(w_groupbase);
var l_Appl = LinkApplet();
l_Appl.SetDoubleKey('code',w_groupbase,6,0);
l_Appl.SetFields('code,name');
l_Appl.SetTypes('N,C');
l_Appl.LinkTable('cpgroups');
l_Appl.SetLinkzoom('spadministration_groups');
l_Appl.SetID('MJILVOVIBU');
l_nRec=l_Appl.DoLink((Eq(kmode,null)?1:kmode));
l_bResult=Ne(l_nRec,0);
if (Eq(l_nRec,1)) {
w_groupbase=l_Appl.GetDoubleValue('code',6,0);
w_group_name=l_Appl.GetStringValue('name',50,0);
l_Appl.GetLinkHash(GetRepeatedCheckVars());
} else {
Link_MJILVOVIBU_Blank();
}
if (Eq(l_nRec,2)) {
var openername = 'linkview_MJILVOVIBU';
...
function Link_MJILVOVIBU_Blank() {
w_groupbase=0;
w_group_name='';
}
function LOpt_MJILVOVIBU() {
var l_oWv = InitWvApplet();
l_oWv.setValue('Table',WtA('cpgroups','C'));
l_oWv.setValue('Linkzoomprg',WtA('spadministration_groups','C'));
l_oWv.setValue('Linkzoom',WtA(true,'L'));
l_oWv.setValue('PKFields',WtA('code','C'));
l_oWv.setValue('LinkedField',WtA('code','C'));
l_oWv.setValue('UID',WtA('MJILVOVIBU','C'));
l_oWv.setValue('SPZTL',WtA(m_IDS['MJILVOVIBU'][1],'C'));
l_oWv.setValue('Caption',WtA(Translate('10266899854'),'C'));
return l_oWv;
}
function ReportLinkValue(varName,value) {
var bResult = false;
if (Eq(varName,'MJILVOVIBU')) {
last_focused_comp=GetLastFocusedElementOfLink('MJILVOVIBU');
function SetMethod_MJILVOVIBU() {
Set_groupbase(HtW(value,'N'),0);
}
if (IsDeviceMobile()) {
SetMethod_MJILVOVIBU();
} else {
setTimeout(SetMethod_MJILVOVIBU,1);
}
bResult=true;
}
return bResult;
...
if ( ! (m_oFather.SaveContext_spadministration_subgroups())) {
...
parent.ZtVWeb.raiseEventToEvalParms('spadministration_subgroups_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('spadministration_subgroups_dataNotSent',{'action':formaction});
}
}
}
}
...
l_oWv.setValue('groupcode',WtA(w_groupcode,'N'));
_FillChildren(l_oWv);
if (m_bExtended) {
ExtendWv(l_oWv);
}
...
if (( ! (_ChkObl('MJILVOVIBU',1))) && FullRow()) {
_SignErr('MJILVOVIBU');
m_cLastWorkVarErrorMsg='1954008362';
l_bResult=false;
w_groupbase=HtW('','N');
}
}
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_spadministration_subgroups();
}
}
SaveContext.child=[];
function BlankRow() {
w_groupbase=0;
w_group_name='';
if ( ! (EmptyNumber(w_groupbase))) {
Link_MJILVOVIBU(null);
}
BlankRowChild();
if (m_bExtended) {
...
function FullRow() {
return  ! (Empty(w_groupbase));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.groupbase={"type":"N","name":"groupbase","id":"MJILVOVIBU","fixedpos":false,"len":6,"dec":0,"title":"Group base"};
l_aRepeatedFields.group_name={"type":"C","name":"group_name","id":"ATYQNYKGKZ","fixedpos":false,"len":50,"dec":0,"title":"ADMIN_NAME"};
