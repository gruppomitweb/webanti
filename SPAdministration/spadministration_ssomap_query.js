...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('KETCZPSXKD').value,'C'),w_ssoid)) {
l_bQuery=true;
}
}
...
if (sendfullkey) {
if (Ne(name,'ssoid') &&  ! (Empty(HtW(Ctrl('KETCZPSXKD').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.ssoid',HtW(Ctrl('KETCZPSXKD').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','spadministration_ssomap');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
...
for(var a=['KETCZPSXKD'],i=0;i>=0;i--){
