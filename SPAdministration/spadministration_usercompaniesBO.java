...
// * --- Fine Area Manuale
public class spadministration_usercompaniesBO extends spadministration_usercompaniesBL {
  ...
  // * --- Fine Area Manuale
  public spadministration_usercompaniesBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"spadministration_usercompanies");
    }
  ...
  public boolean Load(double p_usercode) {
    ...
    String l_cOrderBy;
    l_cOrderBy = m_cVirtName_cpusrazi+".CPRowNum";
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cpusrazi,m_cPhName_cpusrazi,m_cVirtName_cpusrazi,m_cVirtName_cpusrazi+".usercode="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_usercode,"N",9,0),m_cServer_cpusrazi,p_usercode),l_cOrderBy);
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cpusrazi")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_company = l_rsLoadBody.GetString("company");
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cpusrazi.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"usercode="+CPLib.ToSQL(w_usercode,"N",9,0)+" AND cprownum="+CPLib.ToSQL(m_nCPRowNum,"N",10,0);
        l_cSql = l_cSql+m_oWrInfo_cpusrazi.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cpusrazi")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cpusrazi,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cpusrazi);
      l_cSql = "insert into "+m_oWrInfo_cpusrazi.GetTableWriteName()+" (";
      ...
      l_cSql = l_cSql+m_oWrInfo_cpusrazi.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cpusrazi")) {
      ...
      l_cSql = l_cSql+m_oWrInfo_cpusrazi.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cpusrazi")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cpusrazi,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cpusrazi);
      l_cSql = "Update "+m_oWrInfo_cpusrazi.GetTableWriteName()+" set";
