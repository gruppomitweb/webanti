...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('SLTEEWARRT').value,'N'),w_code)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cpgroups',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'code') &&  ! (Empty(HtW(Ctrl('SLTEEWARRT').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.code',HtW(Ctrl('SLTEEWARRT').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_cpgrpgrp(firstAlias) {
return m_cVirtName+".code = "+firstAlias+".groupcode ";
}
...
_chNotF(childAlias,'master','spadministration_groups');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cpgroups',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['SLTEEWARRT'],i=0;i>=0;i--){
