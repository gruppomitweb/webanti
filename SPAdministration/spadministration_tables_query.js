...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('LAGKMWCQMG').value,'C'),w_FileName)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cpttbls',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'FileName') &&  ! (Empty(HtW(Ctrl('LAGKMWCQMG').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.FileName',HtW(Ctrl('LAGKMWCQMG').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','spadministration_tables');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cpttbls',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['LAGKMWCQMG'],i=0;i>=0;i--){
