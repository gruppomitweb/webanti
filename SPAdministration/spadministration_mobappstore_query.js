...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('BXPAETNPNW').value,'C'),w_oa__code)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cpappstore',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'oa__code') &&  ! (Empty(HtW(Ctrl('BXPAETNPNW').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.oa__code',HtW(Ctrl('BXPAETNPNW').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
function DoQueryFrom_cpqkmappstore(firstAlias) {
return m_cVirtName+".oa__code = "+firstAlias+".QA_CODE ";
}
function DoQueryFrom_cppermappstore(firstAlias) {
return m_cVirtName+".oa__code = "+firstAlias+".pecodice ";
}
...
_chNotF(childAlias,'master','spadministration_mobappstore');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cpappstore',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['BXPAETNPNW'],i=0;i>=0;i--){
