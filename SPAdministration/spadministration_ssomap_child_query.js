...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('ZPLLDHVTOQ').value,'N'),w_userid)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cpssomap',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'userid') &&  ! (Empty(HtW(Ctrl('ZPLLDHVTOQ').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.userid',HtW(Ctrl('ZPLLDHVTOQ').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','spadministration_ssomap_child');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cpssomap',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['ZPLLDHVTOQ'],i=0;i>=0;i--){
