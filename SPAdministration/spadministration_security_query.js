...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('ROXNXYYWEU').value,'C'),w_progname)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('SIZORIYAFY').value,'N'),w_grpcode)) {
l_bQuery=true;
}
if (Ne(HtW(Ctrl('TNPTJUMOGJ').value,'N'),w_usrcode)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cpprgsec',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'progname') &&  ! (Empty(HtW(Ctrl('ROXNXYYWEU').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.progname',HtW(Ctrl('ROXNXYYWEU').value,'C'),'C');
}
if (Ne(name,'grpcode') &&  ! (Empty(HtW(Ctrl('SIZORIYAFY').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.grpcode',HtW(Ctrl('SIZORIYAFY').value,'N'),'N');
}
if (Ne(name,'usrcode') &&  ! (Empty(HtW(Ctrl('TNPTJUMOGJ').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.usrcode',HtW(Ctrl('TNPTJUMOGJ').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','spadministration_security');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cpprgsec',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['ROXNXYYWEU','SIZORIYAFY','TNPTJUMOGJ'],i=2;i>=0;i--){
