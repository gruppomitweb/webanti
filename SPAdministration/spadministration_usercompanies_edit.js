...
function Set_company(ctrlValue,kmode,e) {
if (Ne(w_company,ctrlValue)) {
var ctl = _GetCtl(e,'NZYBVMRJHR');
if (_tracker.goon(ctl,ctrlValue)) {
w_company=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('NZYBVMRJHR',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
var ctl = _GetCtl(e,'QPGPPFLICI');
if (_tracker.goon(ctl,ctrlValue)) {
w_datestart=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('QPGPPFLICI',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
...
var ctl = _GetCtl(e,'QSOCWOMGGI');
if (_tracker.goon(ctl,ctrlValue)) {
w_datestop=ctrlValue;
SetModified(true);
var l_bResult = true;
SetErrorField(ctl,false);
l_bResult=_ChkObl('QSOCWOMGGI',0);
DoUpdate(l_bResult);
return l_bResult;
} else {
ctl.value=WtH(NullDate(),'D',8,0,TranslatePicture(GetDatePicture(false,'D'),'D'));
...
if ( ! (m_oFather.SaveContext_spadministration_usercompanies())) {
...
parent.ZtVWeb.raiseEventToEvalParms('spadministration_usercompanies_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('spadministration_usercompanies_dataNotSent',{'action':formaction});
}
}
}
}
...
if (Eq('QPGPPFLICI_ZOOM',lc)) {
return 0;
}
if (Eq('QSOCWOMGGI_ZOOM',lc)) {
return 0;
}
return -1;
}
...
if (( ! (_ChkObl('NZYBVMRJHR',1))) && FullRow()) {
_SignErr('NZYBVMRJHR');
m_cLastWorkVarErrorMsg='966854606';
l_bResult=false;
w_company=HtW('','C');
} else if (( ! (_ChkObl('QPGPPFLICI',1))) && FullRow()) {
_SignErr('QPGPPFLICI');
m_cLastWorkVarErrorMsg='20009153';
l_bResult=false;
w_datestart=HtW('','D');
} else if (( ! (_ChkObl('QSOCWOMGGI',1))) && FullRow()) {
_SignErr('QSOCWOMGGI');
...
if (IsWndAccessible(m_oFather)) {
m_oFather.SetModified_spadministration_usercompanies();
}
}
SaveContext.child=[];
function BlankRow() {
...
function FullRow() {
return  ! (Empty(w_company));
}
function GetRepeatedFields() {
var l_aRepeatedFields;
l_aRepeatedFields={};
l_aRepeatedFields.company={"type":"C","name":"company","id":"NZYBVMRJHR","fixedpos":false,"len":10,"dec":0,"title":"ADMIN_COMPANY"};
l_aRepeatedFields.datestart={"type":"D","name":"datestart","id":"QPGPPFLICI","fixedpos":false,"len":8,"dec":0,"title":"ADMIN_VALIDITY_START"};
l_aRepeatedFields.datestop={"type":"D","name":"datestop","id":"QSOCWOMGGI","fixedpos":false,"len":8,"dec":0,"title":"ADMIN_VALIDITY_END"};
