...
bResult= ! (spadministration_check_server_usage(w_ServerName));
}
if (bResult) {
if (Eq(formaction,'delete')) {
if (m_bLoaded) {
...
WindowAlert(AlertErrorMessage('1105998119'));
}
}
}
var m_bIsMq = false;
...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('JWFUQHQPUG').value,'C'),w_ServerName)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cptsrvr',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'ServerName') &&  ! (Empty(HtW(Ctrl('JWFUQHQPUG').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.ServerName',HtW(Ctrl('JWFUQHQPUG').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','spadministration_servers');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cptsrvr',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['JWFUQHQPUG'],i=0;i>=0;i--){
