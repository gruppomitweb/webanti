...
boolean server_side_included = null!=request.getAttribute("javax.servlet.include.request_uri");%><%sp.setDeviceSelected("DEFAULT");
sp.setContentType(response);%>
<%sp.setSkin(com.zucchetti.sitepainter.DefaultApplicationSkin.m_cDefaultApplicationSkin);%>
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=PageId%>','spma_default_DEVICE_DEFAULT','planObj','<%=SPParentObjId%>');
/*JS_MARKER_END*/
</script>
<%
...
ZtVWeb.PageCtrl(this,'<%=PageId%>', '<%=forcePageletId%>', 'spma_default_DEVICE_DEFAULT');
...
ZtVWeb.addPage('<%=PageId%>', <%=PageId%>, 'spma_default_DEVICE_DEFAULT','<%=includerPageletId%>' );
