...
boolean server_side_included = null!=request.getAttribute("javax.servlet.include.request_uri");%><%sp.setDeviceSelected("TABLET");
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=PageId%>','spma_default_DEVICE_TABLET','planObj','<%=SPParentObjId%>');
/*JS_MARKER_END*/
</script>
<%
...
ZtVWeb.PageCtrl(this,'<%=PageId%>', '<%=forcePageletId%>', 'spma_default_DEVICE_TABLET');
...
ZtVWeb.addPage('<%=PageId%>', <%=PageId%>, 'spma_default_DEVICE_TABLET','<%=includerPageletId%>' );
