...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=PageId%>','spma_index_DEVICE_SMARTPHONE','planObj','<%=SPParentObjId%>');
/*JS_MARKER_END*/
</script>
<%
...
ZtVWeb.PageCtrl(this,'<%=PageId%>', '<%=forcePageletId%>', 'spma_index_DEVICE_SMARTPHONE');
this.Calc=function(pagelet){
 }

}
ZtVWeb.addPage('<%=PageId%>', <%=PageId%>, 'spma_index_DEVICE_SMARTPHONE','<%=includerPageletId%>' );
