...
boolean server_side_included = null!=request.getAttribute("javax.servlet.include.request_uri");%><%sp.setDeviceSelected("SMARTPHONE");
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=PageId%>','spma_default_DEVICE_SMARTPHONE','planObj','<%=SPParentObjId%>');
/*JS_MARKER_END*/
</script>
<%
...
ZtVWeb.PageCtrl(this,'<%=PageId%>', '<%=forcePageletId%>', 'spma_default_DEVICE_SMARTPHONE');
...
ZtVWeb.addPage('<%=PageId%>', <%=PageId%>, 'spma_default_DEVICE_SMARTPHONE','<%=includerPageletId%>' );
