...
// * --- Fine Area Manuale
public class spadministration_subgroupsBO extends spadministration_subgroupsBL {
  class SqlDetailLoader {
    ...
    public void AddJoinedLink_2_1(String rightTable,String rightServer) {
      ...
      if (m_Sql.CanOptimizeLink(m_nColumns+2,m_cServer,rightServer)) {
        ...
        m_cLinkedFrom = m_Sql.GenerateLinkFrom(m_cServer,m_cMainTable,rightTable,"link_2_1","groupbase","code");
        if (CPLib.ne(m_cLinkedFrom,"")) {
          m_cFrom = m_cFrom+m_cLinkedFrom;
          m_cLinkedWhere = m_Sql.GenerateJoinCondition(m_cServer,m_cMainTable,rightTable,"link_2_1","groupbase","code",true);
  ...
  // * --- Fine Area Manuale
  public spadministration_subgroupsBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"spadministration_subgroups");
    }
  ...
  public static String[] m_cLoadParameterNames = {"groupcode"};
  public static String[] m_cDeleteParameterNames = m_cLoadParameterNames;
  public boolean Load(double p_groupcode) {
    CPResultSet l_rsLoadBody=null;
    SqlDetailLoader l_oSqlDetailLoader;
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cpgrpgrp,m_cPhName_cpgrpgrp,m_cVirtName_cpgrpgrp,m_cVirtName_cpgrpgrp+".groupcode="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_groupcode,"N",6,0),m_cServer_cpgrpgrp,p_groupcode),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cpgrpgrp")) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_groupbase = l_rsLoadBody.GetDouble("groupbase");
          if (l_oSqlDetailLoader.m_bLink_2_1) {
            w_group_name = l_rsLoadBody.GetString("name201");
          } else {
            Link_MJILVOVIBU("Load");
          }
          // Assegna il contesto dei figli del dettaglio a Empty
          work_children = CPLib.BlankChildMNTs(m_MNTs,true);
          opk_groupbase = w_groupbase;
          if (CPLib.HasTableCPCCCHK(m_Ctx,"cpgrpgrp")) {
  ...
  public boolean Delete(double p_groupcode) {
    String l_cSql;
    int l_nRow;
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cpgrpgrp.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"groupcode="+CPLib.ToSQL(w_groupcode,"N",6,0)+" AND groupbase="+CPLib.ToSQL(opk_groupbase,"N",6,0);
        l_cSql = l_cSql+m_oWrInfo_cpgrpgrp.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cpgrpgrp")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cpgrpgrp,l_cSql);
  ...
  public boolean Delete() {
    return Delete(w_groupcode);
  }
  public static CPLib.SPLinkProperties GetLinkProperties(String p_cID) {
    CPLib.SPLinkProperties result = CPLib.GetSPLinkerProperties();
    if (CPLib.eq(p_cID,"MJILVOVIBU")) {
      ...
      result.m_PrgZoom = "spadministration_groups";
      result.m_rdField = new String[]{"code" , "name"};
      result.m_rdTypes = new String[]{"N" , "C"};
      result.m_kField = new String[]{};
      result.m_kTypes = new String[]{};
      result.AppendFieldsHash("MJILVOVIBU",CPLib.GetLinkedFieldProps("code","groupbase","N",6,0));
    }
    return result;
  }
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cpgrpgrp);
      l_cSql = "insert into "+m_oWrInfo_cpgrpgrp.GetTableWriteName()+" (";
      l_cSql = l_cSql+"groupcode,";
      l_cSql = l_cSql+"groupbase,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cpgrpgrp.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cpgrpgrp")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_groupcode,"N",6,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQLNull(w_groupbase,"N",6,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cpgrpgrp.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cpgrpgrp")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cpgrpgrp,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cpgrpgrp);
      l_cSql = "Update "+m_oWrInfo_cpgrpgrp.GetTableWriteName()+" set";
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cpgrpgrp")) {
        l_cSql = l_cSql+" CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", groupbase = "+CPLib.ToSQL(w_groupbase,"N",6,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"groupcode="+CPLib.ToSQL(w_groupcode,"N",6,0)+" AND groupbase="+CPLib.ToSQL(opk_groupbase,"N",6,0);
      l_cSql = l_cSql+m_oWrInfo_cpgrpgrp.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cpgrpgrp")) {
