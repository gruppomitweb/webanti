...
parent.ZtVWeb.raiseEventToEvalParms('spadministration_ssomap_beforeSubmit',{'action':formaction});
}
Frm.submit();
...
DisplayErrorMessage();
if (Ne(parent,window) && parent.ZtVWeb) {
parent.ZtVWeb.raiseEventToEvalParms('spadministration_ssomap_dataNotSent',{'action':formaction});
}
}
}
}
...
ResetErrorVars();
m_nLastError=0;
if ( ! (_ChkObl('KETCZPSXKD',1))) {
_SignErr('KETCZPSXKD');
m_cLastWorkVarErrorMsg='7014051';
l_bResult=false;
w_ssoid=HtW('','C');
} else if ( ! (_ChkObl('ZPLLDHVTOQ',1))) {
_SignErr('ZPLLDHVTOQ');
