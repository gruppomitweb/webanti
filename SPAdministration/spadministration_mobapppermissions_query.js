...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('NIIFDUHRBQ').value,'C'),w_pecodice)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cppermappstore',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'pecodice') &&  ! (Empty(HtW(Ctrl('NIIFDUHRBQ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.pecodice',HtW(Ctrl('NIIFDUHRBQ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'detail','spadministration_mobapppermissions');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cppermappstore',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['NIIFDUHRBQ'],i=0;i>=0;i--){
