...
// * --- Fine Area Manuale
public class spadministration_historypwdBO extends spadministration_historypwdBL {
  ...
  // * --- Fine Area Manuale
  public spadministration_historypwdBO (CPContext p_ContextObject) {
  ...
  public boolean CheckOTPSecurity() {
    ...
    if (requiredOTP) {
      ...
      accessType = SPLib.AccessType((m_bLoaded?"edit":"insert"),"spadministration_historypwd");
    }
  ...
  public boolean Load(double p_code) {
    ...
    l_oSqlDetailLoader = new SqlDetailLoader(m_Sql,m_cServer_cphistorypwd,m_cPhName_cphistorypwd,m_cVirtName_cphistorypwd,m_cVirtName_cphistorypwd+".code="+CPSql.SQLValueAdapter(CPLib.ToSQL(p_code,"N",9,0),m_cServer_cphistorypwd,p_code),"");
    ...
    try {
      ...
      if (m_bLoaded) {
        ...
        while ( ! (l_rsLoadBody.Eof())) {
          m_nCurrentRow = Rows();
          w_pwpassword = l_rsLoadBody.GetString("pwpassword");
          w_pwdatcreate = l_rsLoadBody.GetDateTime("pwdatcreate");
          w_pwdtrevoke = l_rsLoadBody.GetDate("pwdtrevoke");
          w_pwactive = l_rsLoadBody.GetString("pwactive");
          // Assegna il contesto dei figli del dettaglio a Empty
          work_children = CPLib.BlankChildMNTs(m_MNTs,true);
          opk_pwpassword = w_pwpassword;
          if (CPLib.HasTableCPCCCHK(m_Ctx,"cphistorypwd")) {
  ...
  void DeleteDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      ...
      if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
        l_cSql = "Delete from "+m_oWrInfo_cphistorypwd.GetTableWriteName()+" where ";
        l_cSql = l_cSql+"code="+CPLib.ToSQL(w_code,"N",9,0)+" AND pwpassword="+CPLib.ToSQL(opk_pwpassword,"C",20,0);
        l_cSql = l_cSql+m_oWrInfo_cphistorypwd.WhereFilter();
        if (CPLib.HasTableCPCCCHK(m_Ctx,"cphistorypwd")) {
          l_cSql = l_cSql+" and CPCCCHK="+CPLib.ToSQL(m_cOldRowCPCCCHK,"C",10,0);
        }
        l_nUpdatedRows = m_Sql.Update(m_cServer_cphistorypwd,l_cSql);
  ...
  void InsertDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cphistorypwd);
      l_cSql = "insert into "+m_oWrInfo_cphistorypwd.GetTableWriteName()+" (";
      l_cSql = l_cSql+"code,";
      l_cSql = l_cSql+"pwpassword,";
      l_cSql = l_cSql+"pwdatcreate,";
      l_cSql = l_cSql+"pwdtrevoke,";
      l_cSql = l_cSql+"pwactive,";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsNameToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cphistorypwd.InsertFields();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cphistorypwd")) {
      ...
      l_cSql = l_cSql+CPLib.ToSQL(w_pwpassword,"C",20,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_pwdatcreate,"T",14,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_pwdtrevoke,"D",8,0)+", ";
      l_cSql = l_cSql+CPLib.ToSQL(w_pwactive,"C",1,0)+", ";
      l_cSql = l_cSql+SPExtenderLib.InsertExtendedFieldsValueToSQL(extendedFields);
      l_cSql = l_cSql+m_oWrInfo_cphistorypwd.InsertValues();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cphistorypwd")) {
      ...
      l_cSql = l_cSql+")";
      l_nUpdatedRows = m_Sql.Update(m_cServer_cphistorypwd,l_cSql,m_oParameters);
      if (CPLib.ne(l_nUpdatedRows,1)) {
        m_Sql.AbortTransaction();
      } else {
  ...
  void UpdateDetailRow() {
    ...
    if (CPLib.eq(m_Sql.GetTransactionStatus(),CPSql.OK)) {
      m_oParameters = CPLib.GetParamsForSqlUpdate(m_cServer_cphistorypwd);
      l_cSql = "Update "+m_oWrInfo_cphistorypwd.GetTableWriteName()+" set";
      l_cSql = l_cSql+" pwdatcreate = "+CPLib.ToSQL(w_pwdatcreate,"T",14,0);
      l_cSql = l_cSql+", pwdtrevoke = "+CPLib.ToSQL(w_pwdtrevoke,"D",8,0);
      l_cSql = l_cSql+", pwactive = "+CPLib.ToSQL(w_pwactive,"C",1,0);
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cphistorypwd")) {
        l_cSql = l_cSql+", CPCCCHK ="+CPLib.ToSQL(l_cCPCCCHK,"C",10,0);
      }
      l_cSql = l_cSql+SPExtenderLib.UpdateExtendedFieldsNameValueToSQL(extendedFields);
      l_cSql = l_cSql+", pwpassword = "+CPLib.ToSQL(w_pwpassword,"C",20,0);
      l_cSql = l_cSql+" where ";
      l_cSql = l_cSql+"code="+CPLib.ToSQL(w_code,"N",9,0)+" AND pwpassword="+CPLib.ToSQL(opk_pwpassword,"C",20,0);
      l_cSql = l_cSql+m_oWrInfo_cphistorypwd.WhereFilter();
      if (CPLib.HasTableCPCCCHK(m_Ctx,"cphistorypwd")) {
