...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('HSYXZWNPAJ').value,'N'),w_code)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cphistorypwd',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'code') &&  ! (Empty(HtW(Ctrl('HSYXZWNPAJ').value,'N')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.code',HtW(Ctrl('HSYXZWNPAJ').value,'N'),'N');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'detail','spadministration_historypwd');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cphistorypwd',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['HSYXZWNPAJ'],i=0;i>=0;i--){
