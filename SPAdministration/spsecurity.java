...
public class spsecurity extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*52a1f06e*!";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,spsecurityBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"2038100684",p_Context.Translate("CP_SECURITY")}};
    return l_translations;
  }
  void WriteStyles(spsecurityBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../spsecurity.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(spsecurityBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(spsecurityBO BO,ServletStatus status) {
    PrintNotifyContent(status,getNotifyItems());
  }
  static void PrintState(spsecurityBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(spsecurityBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(spsecurityBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(spsecurityBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"spsecurity"),"C",0,0)+ ";");
    ...
    if (CPLib.eq(status.context.GetGlobalString("IsPortalStudioInstalled"),"yes") && SPLib.CanAccessEditor(status.context,"PortalStudio")) {
      status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("spsecurity","C",0,0)+ ";");
    }
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"spsecurity","");
  ...
  void WriteMainFormScript(spsecurityBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  spsecurityBO getBO(ServletStatus status) {
    spsecurityBO BO = new spsecurityBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(spsecurityBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,spsecurityBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,spsecurityBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  void Start(ServletStatus status,spsecurityBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,spsecurityBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,spsecurityBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,spsecurityBO BO) {
  }
  void Next(ServletStatus status,spsecurityBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,spsecurityBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(spsecurityBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='spsecurity?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/spsecurity_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(spsecurityBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(spsecurityBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(spsecurityBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(spsecurityBO BO,ServletStatus status) {
  ...
  void WriteExitPage(spsecurityBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(spsecurityBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,spsecurityBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,spsecurityBO BO) {
    boolean result = true;
