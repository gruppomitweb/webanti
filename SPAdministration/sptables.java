...
public class sptables extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*8c094c9d*!";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,sptablesBO BO) {
  ...
  void WriteStyles(sptablesBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../sptables.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(sptablesBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(sptablesBO BO,ServletStatus status) {
    PrintNotifyContent(status,getNotifyItems());
  }
  static void PrintState(sptablesBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(sptablesBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(sptablesBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(sptablesBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"sptables"),"C",0,0)+ ";");
    ...
    if (CPLib.eq(status.context.GetGlobalString("IsPortalStudioInstalled"),"yes") && SPLib.CanAccessEditor(status.context,"PortalStudio")) {
      status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("sptables","C",0,0)+ ";");
    }
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"sptables","");
  ...
  void WriteMainFormScript(sptablesBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  sptablesBO getBO(ServletStatus status) {
    sptablesBO BO = new sptablesBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(sptablesBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,sptablesBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,sptablesBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  void Start(ServletStatus status,sptablesBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,sptablesBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,sptablesBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,sptablesBO BO) {
  }
  void Next(ServletStatus status,sptablesBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,sptablesBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(sptablesBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='sptables?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/sptables_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(sptablesBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(sptablesBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(sptablesBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(sptablesBO BO,ServletStatus status) {
  ...
  void WriteExitPage(sptablesBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(sptablesBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,sptablesBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,sptablesBO BO) {
    boolean result = true;
