...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('QOFJORHBZQ').value,'C'),w_ar__code)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cpappreg',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'ar__code') &&  ! (Empty(HtW(Ctrl('QOFJORHBZQ').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.ar__code',HtW(Ctrl('QOFJORHBZQ').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','spadministration_mobappreg');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cpappreg',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['QOFJORHBZQ'],i=0;i>=0;i--){
