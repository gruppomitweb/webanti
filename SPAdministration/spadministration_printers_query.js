...
var l_bQuery = false;
if (sendfullkey) {
if (Ne(HtW(Ctrl('EASNYTBCXF').value,'C'),w_code)) {
l_bQuery=true;
}
}
...
m_oFather.DoQueryFromChild('cpprinters',_flt_(m_cVirtName+'.'+name,ctrlValue,type),m_cVirtName);
}
} else {
var Frm = document.FSender;
if (m_bIsMq) {
...
if (sendfullkey) {
if (Ne(name,'code') &&  ! (Empty(HtW(Ctrl('EASNYTBCXF').value,'C')))) {
if ( ! (Empty(Frm.m_cLastWhere.value))) {
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+' and ';
}
Frm.m_cLastWhere.value=Frm.m_cLastWhere.value+_flt_(m_cVirtName+'.code',HtW(Ctrl('EASNYTBCXF').value,'C'),'C');
}
}
if (Ne(type,'M')) {
...
_chNotF(childAlias,'master','spadministration_printers');
return;
}
}
if (IsWndAccessible(window['m_oFather'])) {
m_oFather.DoQueryFromChild('cpprinters',keyWhere+' and '+childWhere,m_cVirtName+'|'+allAliases);
} else {
var Frm = document.FSender;
...
for(var a=['EASNYTBCXF'],i=0;i>=0;i--){
