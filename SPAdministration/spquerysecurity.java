...
public class spquerysecurity extends SPDialogServlet implements SPInvokable {
  public static String m_cEntityUid = "!*99b7c7a1*!";
  ...
  public String getEntityTitle() {
    return "ADMIN_QUERY_SECURITY";
  }
  public SPEntityGlobals getEntityGlobals() {
    // Costanti globali dell' entita'
    SPEntityGlobals l_eg = new SPEntityGlobals();
    l_eg.name = "spquerysecurity";
  ...
  public String GetLayerGroupHeader(String p_cGroupName,String p_cLayerName,String p_cLayerId,spquerysecurityBO BO) {
  ...
  public String[][] JSTranslations(CPContext p_Context) {
    ...
    l_translations=new String[][]{{"1056053448",p_Context.Translate("ADMIN_QUERY_SECURITY")}};
    return l_translations;
  }
  void WriteStyles(spquerysecurityBO BO,ServletStatus status) {
    if ( ! ( ! (CPLib.Empty(status.m_cAltInterface)))) {
      status.out.println(
       "<link type='text/css' rel='stylesheet' href='../spquerysecurity.css?"+m_cEntityUid+"'>" +
      "");
    }
  ...
  void WriteMainForm(spquerysecurityBO BO,ServletStatus status) {
    status.out.println(
  ...
  void WriteControls(spquerysecurityBO BO,ServletStatus status) {
    PrintNotifyContent(status,getNotifyItems());
  }
  static void PrintState(spquerysecurityBL BO,ServletStatus status) {
    PrintStateChildren(BO,status);
    PrintStateWorkVars(BO,status);
  }
  static void PrintStateChildren(spquerysecurityBL BO,ServletStatus status) {
  }
  static void PrintStateWorkVars(spquerysecurityBL BO,ServletStatus status) {
  ...
  void WriteMainFormVarScript(spquerysecurityBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
    ...
    status.out.println("var m_cSelectedPage = " + SPLib.ToJSValue(SPLib.GetSelectedPage(status.m_cSelectedPage,"spquerysecurity"),"C",0,0)+ ";");
    ...
    if (CPLib.eq(status.context.GetGlobalString("IsPortalStudioInstalled"),"yes") && SPLib.CanAccessEditor(status.context,"PortalStudio")) {
      status.out.println("var m_cProgNameAlter = " + SPLib.ToJSValue("spquerysecurity","C",0,0)+ ";");
    }
    // hide e disable di pagine ed item
    VRTStateImpl l_vrtState = VRTHolder.GetState(status.context,"spquerysecurity","");
  ...
  void WriteMainFormScript(spquerysecurityBO BO,ServletStatus status) {
    status.out.println(
     "<script>" +
    "");
  ...
  spquerysecurityBO getBO(ServletStatus status) {
    spquerysecurityBO BO = new spquerysecurityBO(status.context);
    LoadExtensions(BO);
    return BO;
  }
  void LoadExtensions(spquerysecurityBO BO) {
  }
  public void NotifyEvent(String p_cEvent,ServletStatus status,spquerysecurityBO BO) {
    super.NotifyEvent(p_cEvent,status,BO);
    CPLib.NotifyEvent(p_cEvent,BO,BO.m_Ctx,BO.specificEventsHandlers,BO.allEventsHandlers);
  }
  void WriteTableCombo(ServletStatus status,spquerysecurityBO BO) {
    String l_cTmp;
    CPResultSet l_rsLoad=null;
  }
  void Start(ServletStatus status,spquerysecurityBO BO) {
    BO.SetFromContext(status.m_cWv);
  }
  void Save(ServletStatus status,spquerysecurityBO BO) {
    boolean l_bResult;
    BO.SetFromContext(status.m_cWv);
    l_bResult = BO.Save();
    if (l_bResult) {
  ...
  void Discard(ServletStatus status,spquerysecurityBO BO) {
    status.m_bExit = true;
  }
  void CreatePortlet(ServletStatus status,spquerysecurityBO BO) {
  }
  void Next(ServletStatus status,spquerysecurityBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow+entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void Previous(ServletStatus status,spquerysecurityBO BO) {
    BO.SetFromContext(status.m_cWv);
    status.m_nStartRow = status.m_nStartRow-entityGlobals.m_nRowsPerPage;
    status.m_cClientMode = "edit";
  }
  void WriteFormPage(spquerysecurityBO BO,ServletStatus status) {
    ...
    status.out.println(
     "<script src='spquerysecurity?"+SPLib.TranslationParameter()+"="+status.context.GetLanguage()+"'>" +
     "</script>" +
    "");
    ...
    "");
    status.out.println(
     "<script src='SPPrxy/"+m_cEntityUid+"/spquerysecurity_proc.js'>" +
     "</script>" +
    "");
    IncludeJS(status);
  ...
  void RaiseLoadedEntity(spquerysecurityBO BO,ServletStatus status) {
    RaiseEventLoadedEntity(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cClientMode,"C",0,0)+"})","C",0,0));
  }
  void RaiseSavedEntity(spquerysecurityBO BO,ServletStatus status) {
  ...
  void RaiseDeletedEntity(spquerysecurityBO BO,ServletStatus status) {
  ...
  void RaiseDiscardedEntity(spquerysecurityBO BO,ServletStatus status) {
  ...
  void WriteExitPage(spquerysecurityBO BO,ServletStatus status,Forward f) {
    WriteExitPage(status,SPLib.ToJSValue("({action:"+SPLib.ToJSValue(status.m_cMode,"C",0,0)+"})","C",0,0), f);
  }
  void WriteExitPage(spquerysecurityBO BO,ServletStatus status) {
  ...
  public String GetCheckVars(ServletStatus status,spquerysecurityBO BO,String p_cUID) {
  ...
  public boolean CheckVars(ServletStatus status,spquerysecurityBO BO) {
    boolean result = true;
