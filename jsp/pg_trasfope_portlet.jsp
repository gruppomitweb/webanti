...
%>
window.pg_trasfope_Static=function(){
...
response.setHeader("Cache-Control","public");
%>
.pg_trasfope_container {
}
.pg_trasfope_title_container {
  margin: auto;
}
.pg_trasfope_portlet{
  position:relative;
...
.pg_trasfope_portlet[Data-page="1"]{
  height:250px;
  width:100%;
}
.pg_trasfope_portlet > .lblDaOpe_ctrl {
...
.pg_trasfope_portlet > .daope_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .daope_ctrl > input{
  vertical-align:top;
  box-sizing:border-box;
  z-index:1;
  width:100%;
...
.pg_trasfope_portlet > .CalDaOpe_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .lblAScad_Copy_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .adope_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .adope_ctrl > input{
  vertical-align:top;
  box-sizing:border-box;
  z-index:1;
  width:100%;
...
.pg_trasfope_portlet > .CalAdOpe_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .flgforce_ctrl {
  box-sizing:border-box;
  z-index:2;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .Label_INFORM_ctrl {
  box-sizing:border-box;
  z-index:6;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .Textbox_INFORM_ctrl {
  box-sizing:border-box;
  z-index:4;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .Textbox_INFORM_ctrl > input{
  vertical-align:top;
  box-sizing:border-box;
  z-index:1;
  width:100%;
...
.pg_trasfope_portlet > .LinkZoom_xperazbo_ctrl {
  box-sizing:border-box;
  z-index:5;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .Elabora_ctrl {
  box-sizing:border-box;
  z-index:0;
  position:absolute;
  display:inline-block;
...
.pg_trasfope_portlet > .Uscita_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
<%}else{
if (JSPLib.checkApplication(request, response, out, sp, 37,"pg_trasfope","UTF-8")) {return; }
...
if(!included){%>
<head>
<base href=<%=JSPLib.ToHTMLValue( JSPLib.SPWebRootURL(request)+"/jsp/pg_trasfope_portlet.jsp"+(!Library.Empty(request.getQueryString())?"?"+request.getQueryString():"") )%>/>
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=idPortlet%>','pg_trasfope','portletObj','<%=JSPLib.ToJSValue(SPParentObjId,false,true)%>');
/*JS_MARKER_END*/
</script>
<%String lblDaOpe= "Da Data:";
java.sql.Date daope= JSPLib.NullDate();
if(request.getAttribute("pg_trasfope_firstinclusion")==null){
...
<div id='<%=idPortlet%>_<%=m_cTITLEUID%>_title_container' class='pg_trasfope_title_container' style='width:100%'>
<%out.flush(); 
 if(!Library.includeResource(request,response,"/"+sp.getSkin()+"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 if(!Library.includeResource(request,response,"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 Library.includeResource(request,response,"/jsp/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",false);%>
</div>
<div id='<%=idPortlet%>_container' class='pg_trasfope_container'>
<div id='<%=idPortlet%>' portlet_id='<%=idPortlet%>' align='left' class='pg_trasfope_portlet portlet' style='opacity:0.001'>
<%if(sp.isLogged()){
 if(sp.isInGroup(1) && sp.getGlobal("g_EditPortlet",false)==true){%>
<script>
ZtVWeb.AddPortletToEdit('pg_trasfope','<%=idPortlet%>',false,'');
</script><%}}%>
...
<script>
/*JS_MARKER_START*/
ZtVWeb.addPortlet('<%=idPortlet%>','pg_trasfope');
<%!
...
ZtVWeb.newForm(this,'<%=idPortlet%>','pg_trasfope',["600"],["250"],'portlet',<%=sp.isOfflineInstallation()%>,false,m_cSPPageletId,'', 'adaptive','<%=m_cTITLEUID%>','',{},false,false,1,m_cSPPortletTitleId, 'NORMAL',[{"layout_steps_values":{},"w":"600","h":"250","title":"TRASFERIMENTO OPERAZIONI DA PROVVISORIO AD AUI","layer":"false","npage":"1"}]);
...
this.LinkZoom_xperazbo=new ZtVWeb.LinkZoomCtrl(this,{"advanced_suggester":"","advanced_suggester_height":"","advanced_suggester_modal":false,"advanced_suggester_width":"","anchor":"top-left(%)-right(%)","class_Css":"","configname":"","ctrlid":"<%=idPortlet%>_LinkZoom_xperazbo","defaultsfunction":"","defaultsfunction_zoom":false,"entityName":"\u002fjsp\u002fpg_trasfope_portlet.jsp","fillemptykeys":false,"fixedvars":"","h":20,"hide":"false","image":"","intovars":"Textbox_INFORM","intovarsType":"C","keyfixedfilters":"","layer":false,"layout_steps_values":{},"link_id_hash":"<%=JSPLib.cmdHash("/jsp/pg_trasfope_portlet.jspLinkZoom_xperazbo",request.getSession())%>","linkedField":"Textbox_INFORM","linkedUsing":"","looselylinked":false,"n_criteria":"1","name":"LinkZoom_xperazbo","offline":false,"openZoomOnMultipleRecord":true,"page":1,"path_type":"","queryparms":"","readfields":"INFORM","row_suggest":"10","searchingfunction":"","sendkeystozoomonzoom":false,"servlet":"","spuid":"","spztl":"<%=JSPLib.cmdHash("link,xperazbo,default",request.getSession())%>","suggest":false,"table":"xperazbo","type":"LinkZoom","w":20,"x":510,"y":17,"zindex":"5","zoomtitle":"<%=JSPLib.ToJSValue(sp.translate("Vista operazioni provvisorie per selezione riferimento 2"),false,true)%>"});
this.Elabora=new ZtVWeb._IC(this,{"alt":"","anchor":"top-right","boundedPosition":"","class_Css":"image Elabora_ctrl","create_undercond":"","ctrlid":"<%=idPortlet%>_Elabora","edit_undercond":"","field":"","font_color":"","font_color_hover":"","font_image":"&#xeb47;","font_image_hover":"&#xeb48;","font_name":"ZucchettiDevelopement","font_path":"","font_size":"","h":30,"help_tips":"<%=JSPLib.ToJSValue("Trasferisce operazioni da provvisorio ad AUI",false,true)%>","hide":"false","hide_undercond":"","href":"","img_type":"font icon","keepProportions":"false","layer":false,"layout_steps_values":{},"name":"Elabora","page":1,"path_type":"","server_side":"","spuid":"","src":"&#xeb47;","srchover":"&#xeb48;","tabindex":"","target":"","type":"Image","type_submit":"false","w":30,"x":533,"y":8,"zindex":"0"});
...
<%request.setAttribute("pg_trasfope_firstinclusion",idPortlet);
}%>
<script>
/*JS_MARKER_START*/
window.pg_trasfope_Static.call(window.<%=idPortlet%>);
window.<%=idPortlet%>.Elabora.dispatchEvent('OnLoad');
window.<%=idPortlet%>.Uscita.dispatchEvent('OnLoad');
ZtVWeb.addPortletObj('<%=idPortlet%>','pg_trasfope',<%=idPortlet%>);
ZtVWeb.portletLoaded(<%=idPortlet%>,'pg_trasfope');
/*JS_MARKER_END*/
</script>
<%if(!included){%>
</body>
...
<%! public String getJSPUID() { return "4170324893"; } %>