...
%>
window.pg_impxls_decodenascita_Static=function(){
...
response.setHeader("Cache-Control","public");
%>
.pg_impxls_decodenascita_container {
}
.pg_impxls_decodenascita_portlet{
  position:relative;
  width:851px;
  min-width:851px;
  height:73px;
...
.pg_impxls_decodenascita_portlet[Data-page="1"]{
  height:73px;
  width:851px;
}
.pg_impxls_decodenascita_portlet > .nomefile_ctrl {
...
.pg_impxls_decodenascita_portlet > .nomefile_ctrl > input{
  vertical-align:top;
  box-sizing:border-box;
  z-index:1;
  font-family:Verdana;
...
.pg_impxls_decodenascita_portlet > .label1_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_impxls_decodenascita_portlet > .btnExit_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_impxls_decodenascita_portlet > .btnExit_ctrl > img{
  width:100%;
  vertical-align:top;
  height:30px;
}
.pg_impxls_decodenascita_portlet > .btnElab_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_impxls_decodenascita_portlet > .btnElab_ctrl > img{
  width:100%;
  vertical-align:top;
  height:30px;
}
.pg_impxls_decodenascita_portlet > .image5_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_impxls_decodenascita_portlet > .image5_ctrl > a, .pg_impxls_decodenascita_portlet > .image5_ctrl > a:hover{
  text-decoration:none;
}
.pg_impxls_decodenascita_portlet > .image5_ctrl > a > img{
  width:100%;
  vertical-align:top;
  height:20px;
}
...
<%}else{
if (JSPLib.checkApplication(request, response, out, sp, 37,"pg_impxls_decodenascita","UTF-8")) {return; }
...
if(!included){%>
<head>
<base href=<%=JSPLib.ToHTMLValue( JSPLib.SPWebRootURL(request)+"/jsp/pg_impxls_decodenascita_portlet.jsp"+(!Library.Empty(request.getQueryString())?"?"+request.getQueryString():"") )%>/>
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=idPortlet%>','pg_impxls_decodenascita','portletObj','<%=JSPLib.ToJSValue(SPParentObjId,false,true)%>');
/*JS_MARKER_END*/
</script>
<%String nomefile= "";
if(request.getAttribute("pg_impxls_decodenascita_firstinclusion")==null){
...
<div id='<%=idPortlet%>_container' class='pg_impxls_decodenascita_container'>
<div id='<%=idPortlet%>' portlet_id='<%=idPortlet%>' align='left' class='pg_impxls_decodenascita_portlet portlet' style='opacity:0.001'>
<%if(sp.isLogged()){
 if(sp.isInGroup(1) && sp.getGlobal("g_EditPortlet",false)==true){%>
<script>
ZtVWeb.AddPortletToEdit('pg_impxls_decodenascita','<%=idPortlet%>',false,' ');
</script><%}}%>
...
<script>
/*JS_MARKER_START*/
ZtVWeb.addPortlet('<%=idPortlet%>','pg_impxls_decodenascita');
<%!
...
ZtVWeb.newForm(this,'<%=idPortlet%>','pg_impxls_decodenascita',["851"],["73"],'portlet',<%=sp.isOfflineInstallation()%>,false,m_cSPPageletId,'', '','','false',{},false,false,1,m_cSPPortletTitleId, 'NORMAL',[{"layout_steps_values":{},"w":"851","h":"73","title":" ","layer":"false","npage":"1"}]);
...
<%request.setAttribute("pg_impxls_decodenascita_firstinclusion",idPortlet);
}%>
<script>
/*JS_MARKER_START*/
window.pg_impxls_decodenascita_Static.call(window.<%=idPortlet%>);
...
ZtVWeb.addPortletObj('<%=idPortlet%>','pg_impxls_decodenascita',<%=idPortlet%>);
ZtVWeb.portletLoaded(<%=idPortlet%>,'pg_impxls_decodenascita');
/*JS_MARKER_END*/
</script>
<%if(!included){%>
</body>
...
<%}
sp.endPage("pg_impxls_decodenascita");
}%>
<%! public String getJSPUID() { return "2485219944"; } %>