...
%>
window.pg_trasftit_Static=function(){
...
response.setHeader("Cache-Control","public");
%>
.pg_trasftit_container {
}
.pg_trasftit_title_container {
  margin: auto;
}
.pg_trasftit_portlet{
  position:relative;
...
.pg_trasftit_portlet[Data-page="1"]{
  height:260px;
  width:100%;
}
.pg_trasftit_portlet > .pMsg_ctrl {
...
.pg_trasftit_portlet > .Elabora_ctrl {
  box-sizing:border-box;
  z-index:0;
  position:absolute;
  display:inline-block;
...
.pg_trasftit_portlet > .Uscita_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
<%}else{
if (JSPLib.checkApplication(request, response, out, sp, 37,"pg_trasftit","UTF-8")) {return; }
...
if(!included){%>
<head>
<base href=<%=JSPLib.ToHTMLValue( JSPLib.SPWebRootURL(request)+"/jsp/pg_trasftit_portlet.jsp"+(!Library.Empty(request.getQueryString())?"?"+request.getQueryString():"") )%>/>
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=idPortlet%>','pg_trasftit','portletObj','<%=JSPLib.ToJSValue(SPParentObjId,false,true)%>');
/*JS_MARKER_END*/
</script>
<%try{
if(!included){%>
...
<div id='<%=idPortlet%>_<%=m_cTITLEUID%>_title_container' class='pg_trasftit_title_container' style='width:100%'>
<%out.flush(); 
 if(!Library.includeResource(request,response,"/"+sp.getSkin()+"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 if(!Library.includeResource(request,response,"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 Library.includeResource(request,response,"/jsp/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",false);%>
</div>
<div id='<%=idPortlet%>_container' class='pg_trasftit_container'>
<div id='<%=idPortlet%>' portlet_id='<%=idPortlet%>' align='left' class='pg_trasftit_portlet portlet' style='opacity:0.001'>
<%if(sp.isLogged()){
 if(sp.isInGroup(1) && sp.getGlobal("g_EditPortlet",false)==true){%>
<script>
ZtVWeb.AddPortletToEdit('pg_trasftit','<%=idPortlet%>',false,'');
</script><%}}%>
...
<script>
/*JS_MARKER_START*/
ZtVWeb.addPortlet('<%=idPortlet%>','pg_trasftit');
<%!
...
ZtVWeb.newForm(this,'<%=idPortlet%>','pg_trasftit',["90"],["260"],'portlet',<%=sp.isOfflineInstallation()%>,false,m_cSPPageletId,'', '','<%=m_cTITLEUID%>','',{},false,false,1,m_cSPPortletTitleId, 'NORMAL',[{"layout_steps_values":{},"w":"90","h":"260","title":"","layer":"false","npage":"1"}]);
...
<%if(request.getAttribute("pg_trasftit_firstinclusion")==null){%>
<script src='<%=JSPLib.getJSPSelfPath(request)%>?SpStaticCode=<%=m_cJSPUID%>'></script>
<%request.setAttribute("pg_trasftit_firstinclusion",idPortlet);
}%>
<script>
/*JS_MARKER_START*/
window.pg_trasftit_Static.call(window.<%=idPortlet%>);
window.<%=idPortlet%>.Elabora.dispatchEvent('OnLoad');
window.<%=idPortlet%>.Uscita.dispatchEvent('OnLoad');
ZtVWeb.addPortletObj('<%=idPortlet%>','pg_trasftit',<%=idPortlet%>);
ZtVWeb.portletLoaded(<%=idPortlet%>,'pg_trasftit');
/*JS_MARKER_END*/
</script>
<%if(!included){%>
</body>
...
<%}
sp.endPage("pg_trasftit");
}%>
<%! public String getJSPUID() { return "1211275210"; } %>