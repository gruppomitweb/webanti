...
%>
window.pg_tbnatura_Static=function(){
...
response.setHeader("Cache-Control","public");
%>
.pg_tbnatura_container {
}
.pg_tbnatura_title_container {
  margin: auto;
}
.pg_tbnatura_portlet{
  position:relative;
...
.pg_tbnatura_portlet[Data-page="1"]{
  height:350px;
  width:100%;
}
.pg_tbnatura_portlet > .Grid0_ctrl {
...
.pg_tbnatura_portlet > .btnExit_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
<%}else{
if (JSPLib.checkApplication(request, response, out, sp, 37,"pg_tbnatura","UTF-8")) {return; }
...
if(!included){%>
<head>
<base href=<%=JSPLib.ToHTMLValue( JSPLib.SPWebRootURL(request)+"/jsp/pg_tbnatura_portlet.jsp"+(!Library.Empty(request.getQueryString())?"?"+request.getQueryString():"") )%>/>
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=idPortlet%>','pg_tbnatura','portletObj','<%=JSPLib.ToJSValue(SPParentObjId,false,true)%>');
/*JS_MARKER_END*/
</script>
<%if(request.getAttribute("pg_tbnatura_firstinclusion")==null){
...
<div id='<%=idPortlet%>_<%=m_cTITLEUID%>_title_container' class='pg_tbnatura_title_container' style='width:100%'>
<%out.flush(); 
 if(!Library.includeResource(request,response,"/"+sp.getSkin()+"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 if(!Library.includeResource(request,response,"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 Library.includeResource(request,response,"/jsp/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",false);%>
</div>
<div id='<%=idPortlet%>_container' class='pg_tbnatura_container'>
<div id='<%=idPortlet%>' portlet_id='<%=idPortlet%>' align='left' class='pg_tbnatura_portlet undefined' style='opacity:0.001'>
<%if(sp.isLogged()){
 if(sp.isInGroup(1) && sp.getGlobal("g_EditPortlet",false)==true){%>
<script>
ZtVWeb.AddPortletToEdit('pg_tbnatura','<%=idPortlet%>',false,'');
</script><%}}%>
...
<script>
/*JS_MARKER_START*/
ZtVWeb.addPortlet('<%=idPortlet%>','pg_tbnatura');
<%!
...
ZtVWeb.newForm(this,'<%=idPortlet%>','pg_tbnatura',["740"],["350"],'undefined',<%=sp.isOfflineInstallation()%>,false,m_cSPPageletId,'', 'adaptive','<%=m_cTITLEUID%>','',{},false,false,1,m_cSPPortletTitleId, 'NORMAL',[{"layout_steps_values":{},"w":"740","h":"350","title":"","layer":"false","npage":"1"}]);
...
this.RTUscita=new ZtVWeb.SPLinkerCtrl(this,{"action":"","allowedEntities":"","anchor":"","async":false,"check_exp":"","ctrlid":"<%=idPortlet%>_RTUscita","decrypted_parms":"","entity_type":"","force_msg_id":false,"h":20,"hide":"false","layer":false,"layout_steps_values":{},"m_cAltInterface":"","name":"RTUscita","offline":false,"page":1,"parms":"","popup":"false","popup_height":"486","popup_maximized":"false","popup_scroll":"false","popup_style":"by skin","popup_width":"300","progressbar":false,"refresh":"","servlet":"start_page_logo_portlet.jsp","target":"","type":"SPLinker","w":120,"x":145,"y":-56});
this.RTUscita.m_cID='<%=JSPLib.cmdHash("entity,start_page_logo_portlet.jsp",request.getSession())%>';
this.Query1=new ZtVWeb._SQP(this,{"allowedcmds":"","anchor":"","appendingData":false,"async":false,"auto_exec":"true","cmd":"qbe_tbnatura","cmdHash":"<%=JSPLib.cmdHash("qbe_tbnatura",request.getSession())%>","count":"true","ctrlid":"<%=idPortlet%>_Query1","fieldstype":"true","h":20,"hide":"false","init_time":0,"layer":false,"layout_steps_values":{},"localDBName":"","long_query":false,"name":"Query1","nrows":"100","page":1,"parms":"","parms_source":"","type":"SQLDataobj","w":120,"x":2,"y":-69});
this.anarapbo=new ZtVWeb.SPLinkerCtrl(this,{"action":"","allowedEntities":"","anchor":"","async":false,"check_exp":"","ctrlid":"<%=idPortlet%>_anarapbo","decrypted_parms":"","entity_type":"master","force_msg_id":false,"h":30,"hide":"false","layer":false,"layout_steps_values":{},"m_cAltInterface":"","name":"anarapbo","offline":false,"page":1,"parms":"RAPPORTO","popup":"true","popup_height":"600","popup_maximized":"false","popup_scroll":"false","popup_style":"","popup_width":"800","progressbar":false,"refresh":"false","servlet":"armt_anarap","target":"_new","type":"SPLinker","w":152,"x":290,"y":-66});
this.anarapbo.m_cID='<%=JSPLib.cmdHash("entity,armt_anarap",request.getSession())%>';
...
<%request.setAttribute("pg_tbnatura_firstinclusion",idPortlet);
}%>
<script>
/*JS_MARKER_START*/
window.pg_tbnatura_Static.call(window.<%=idPortlet%>);
window.<%=idPortlet%>.btnExit.dispatchEvent('OnLoad');
window.<%=idPortlet%>.Query1.firstQuery('true');
ZtVWeb.addPortletObj('<%=idPortlet%>','pg_tbnatura',<%=idPortlet%>);
ZtVWeb.portletLoaded(<%=idPortlet%>,'pg_tbnatura');
/*JS_MARKER_END*/
</script>
<%if(!included){%>
</body>
...
<%}
sp.endPage("pg_tbnatura");
}%>
<%! public String getJSPUID() { return "3085623001"; } %>