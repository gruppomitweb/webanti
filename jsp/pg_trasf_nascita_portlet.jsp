...
%>
window.pg_trasf_nascita_Static=function(){
...
response.setHeader("Cache-Control","public");
%>
.pg_trasf_nascita_container {
}
.pg_trasf_nascita_title_container {
  margin: auto;
}
.pg_trasf_nascita_portlet{
  position:relative;
  margin: auto;
  width:100%;
  min-width:851px;
...
.pg_trasf_nascita_portlet[Data-page="1"]{
  height:300px;
  width:100%;
}
.pg_trasf_nascita_portlet > .nomefile_ctrl {
...
.pg_trasf_nascita_portlet > .nomefile_ctrl > input{
  vertical-align:top;
  box-sizing:border-box;
  z-index:1;
  font-family:Verdana;
...
.pg_trasf_nascita_portlet > .btnElab_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_trasf_nascita_portlet > .label1_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_trasf_nascita_portlet > .image5_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_trasf_nascita_portlet > .pLog_ctrl {
  box-sizing:border-box;
  z-index:3;
  position:absolute;
  display:inline-block;
...
.pg_trasf_nascita_portlet > .pMsg_ctrl {
  box-sizing:border-box;
  z-index:3;
  position:absolute;
  display:inline-block;
...
.pg_trasf_nascita_portlet > .btnExit_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
<%}else{
if (JSPLib.checkApplication(request, response, out, sp, 37,"pg_trasf_nascita","UTF-8")) {return; }
...
if(!included){%>
<head>
<base href=<%=JSPLib.ToHTMLValue( JSPLib.SPWebRootURL(request)+"/jsp/pg_trasf_nascita_portlet.jsp"+(!Library.Empty(request.getQueryString())?"?"+request.getQueryString():"") )%>/>
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=idPortlet%>','pg_trasf_nascita','portletObj','<%=JSPLib.ToJSValue(SPParentObjId,false,true)%>');
/*JS_MARKER_END*/
</script>
<%String nomefile= "";
if(request.getAttribute("pg_trasf_nascita_firstinclusion")==null){
...
<div id='<%=idPortlet%>_<%=m_cTITLEUID%>_title_container' class='pg_trasf_nascita_title_container' style='width:100%'>
<%out.flush(); 
 if(!Library.includeResource(request,response,"/"+sp.getSkin()+"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 if(!Library.includeResource(request,response,"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 Library.includeResource(request,response,"/jsp/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",false);%>
</div>
<div id='<%=idPortlet%>_container' class='pg_trasf_nascita_container'>
<div id='<%=idPortlet%>' portlet_id='<%=idPortlet%>' align='left' class='pg_trasf_nascita_portlet portlet' style='opacity:0.001'>
<%if(sp.isLogged()){
 if(sp.isInGroup(1) && sp.getGlobal("g_EditPortlet",false)==true){%>
<script>
ZtVWeb.AddPortletToEdit('pg_trasf_nascita','<%=idPortlet%>',false,'');
</script><%}}%>
...
<script>
/*JS_MARKER_START*/
ZtVWeb.addPortlet('<%=idPortlet%>','pg_trasf_nascita');
<%!
...
ZtVWeb.newForm(this,'<%=idPortlet%>','pg_trasf_nascita',["851"],["300"],'portlet',<%=sp.isOfflineInstallation()%>,false,m_cSPPageletId,'', 'adaptive','<%=m_cTITLEUID%>','',{},false,false,1,m_cSPPortletTitleId, 'NORMAL',[{"layout_steps_values":{},"w":"851","h":"300","title":"","layer":"false","npage":"1"}]);
...
this.pMsg_ctrl=new ZtVWeb.PortletCtrl(this,{"anchor":"top-left(%)-right(%)","ctrlid":"<%=idPortlet%>_pMsg","h":67,"hide":"false","layer":false,"layout_steps_values":{},"name":"pMsg","page":1,"portlet_id":"<%=idPortlet%>_216gh","type":"Portlet","w":846,"x":2,"y":220,"zindex":"3"});
} else {
...
<%request.setAttribute("pg_trasf_nascita_firstinclusion",idPortlet);
}%>
<script>
/*JS_MARKER_START*/
window.pg_trasf_nascita_Static.call(window.<%=idPortlet%>);
...
ZtVWeb.addPortletObj('<%=idPortlet%>','pg_trasf_nascita',<%=idPortlet%>);
ZtVWeb.portletLoaded(<%=idPortlet%>,'pg_trasf_nascita');
/*JS_MARKER_END*/
</script>
<%if(!included){%>
</body>
...
<%}
sp.endPage("pg_trasf_nascita");
}%>
<%! public String getJSPUID() { return "2378904974"; } %>