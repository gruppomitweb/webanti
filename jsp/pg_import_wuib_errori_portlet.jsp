...
%>
window.pg_import_wuib_errori_Static=function(){
...
response.setHeader("Cache-Control","public");
%>
.pg_import_wuib_errori_container {
  height:100%;
  overflow:auto;
}
.pg_import_wuib_errori_title_container {
  margin: auto;
}
.pg_import_wuib_errori_portlet{
  position:relative;
  margin: auto;
  width:100%;
  min-width:851px;
...
.pg_import_wuib_errori_portlet[Data-page="1"]{
  height:100%;
  width:100%;
}
.pg_import_wuib_errori_portlet > .nomefile_ctrl {
...
.pg_import_wuib_errori_portlet > .nomefile_ctrl > input{
  vertical-align:top;
  box-sizing:border-box;
  z-index:1;
  font-family:Verdana;
...
.pg_import_wuib_errori_portlet > .PortletLogs_ctrl {
  box-sizing:border-box;
  z-index:3;
  position:absolute;
  display:inline-block;
...
.pg_import_wuib_errori_portlet > .label1_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_import_wuib_errori_portlet > .image5_ctrl {
  box-sizing:border-box;
  z-index:1;
  position:absolute;
  display:inline-block;
...
.pg_import_wuib_errori_portlet > .rDest_ctrl {
  box-sizing:border-box;
  z-index:2;
  position:absolute;
  display:inline-block;
...
.pg_import_wuib_errori_portlet > .listaFile_ctrl {
  box-sizing:border-box;
  z-index:6;
  position:absolute;
  display:inline-block;
...
.pg_import_wuib_errori_portlet > .Label18_ctrl {
  box-sizing:border-box;
  z-index:7;
  position:absolute;
  display:inline-block;
...
<%}else{
if (JSPLib.checkApplication(request, response, out, sp, 37,"pg_import_wuib_errori","UTF-8")) {return; }
...
if(!included){%>
<head>
<base href=<%=JSPLib.ToHTMLValue( JSPLib.SPWebRootURL(request)+"/jsp/pg_import_wuib_errori_portlet.jsp"+(!Library.Empty(request.getQueryString())?"?"+request.getQueryString():"") )%>/>
...
<script>
/*JS_MARKER_START*/
  ZtVWeb.POM.addObj('<%=idPortlet%>','pg_import_wuib_errori','portletObj','<%=JSPLib.ToJSValue(SPParentObjId,false,true)%>');
/*JS_MARKER_END*/
</script>
<%String nomefile= "";
if(request.getAttribute("pg_import_wuib_errori_firstinclusion")==null){
...
<div id='<%=idPortlet%>_<%=m_cTITLEUID%>_title_container' class='pg_import_wuib_errori_title_container' style='width:100%'>
<%out.flush(); 
 if(!Library.includeResource(request,response,"/"+sp.getSkin()+"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 if(!Library.includeResource(request,response,"/jsp-decorators/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",true)) 
 Library.includeResource(request,response,"/jsp/custom_toolbar_portlet.jsp?ForcedPortletUID="+idPortlet+"_"+m_cTITLEUID+"",false);%>
</div>
<div id='<%=idPortlet%>_container' class='pg_import_wuib_errori_container'>
<div id='<%=idPortlet%>' portlet_id='<%=idPortlet%>' align='left' class='pg_import_wuib_errori_portlet portlet' style='opacity:0.001'>
<%if(sp.isLogged()){
 if(sp.isInGroup(1) && sp.getGlobal("g_EditPortlet",false)==true){%>
<script>
ZtVWeb.AddPortletToEdit('pg_import_wuib_errori','<%=idPortlet%>',false,'');
</script><%}}%>
...
<script>
/*JS_MARKER_START*/
ZtVWeb.addPortlet('<%=idPortlet%>','pg_import_wuib_errori');
<%!
...
ZtVWeb.newForm(this,'<%=idPortlet%>','pg_import_wuib_errori',["851"],["354"],'portlet',<%=sp.isOfflineInstallation()%>,false,m_cSPPageletId,'', 'adaptive','<%=m_cTITLEUID%>','',{},false,true,1,m_cSPPortletTitleId, 'NORMAL',[{"layout_steps_values":{},"w":"851","h":"354","title":"","layer":""}]);
...
this.PortletLogs_ctrl=new ZtVWeb.PortletCtrl(this,{"anchor":"top-left(%)-right(%)-bottom","ctrlid":"<%=idPortlet%>_PortletLogs","h":80,"hide":"false","layer":false,"layout_steps_values":{},"name":"PortletLogs","page":1,"portlet_id":"<%=idPortlet%>_13wmc2x","spuid":"","type":"Portlet","w":850,"x":0,"y":274,"zindex":"3"});
} else {
...
<%request.setAttribute("pg_import_wuib_errori_firstinclusion",idPortlet);
}%>
<script>
/*JS_MARKER_START*/
window.pg_import_wuib_errori_Static.call(window.<%=idPortlet%>);
...
ZtVWeb.addPortletObj('<%=idPortlet%>','pg_import_wuib_errori',<%=idPortlet%>);
ZtVWeb.portletLoaded(<%=idPortlet%>,'pg_import_wuib_errori');
/*JS_MARKER_END*/
</script>
<%if(!included){%>
</body>
...
<%}
sp.endPage("pg_import_wuib_errori");
}%>
<%! public String getJSPUID() { return "3906232619"; } %>